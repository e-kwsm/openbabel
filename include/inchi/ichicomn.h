...
typedef struct tagEquNeigh {
  int num_to; /* number of neighbors with equal mapping ranks; one of them has
                 min. canon. number */
  AT_RANK to_at[4]; /* to_atom neighbors #s with equal mapping ranks */
  AT_RANK from_at; /* from_at neighbor # which has min. canon. number and can be
                      mapped on any of the above to_at[] */
...
/*******************************************************************/
/* ichiisot.c */
int unpack_iso_sort_key(AT_ISO_SORT_KEY iso_sort_key, S_CHAR *num_1H,
                        S_CHAR *num_2H, S_CHAR *num_3H, S_CHAR *iso_atw_diff);
AT_ISO_SORT_KEY make_iso_sort_key(int iso_atw_diff, int num_1H, int num_2H,
                                  int num_3H);
int set_atom_iso_sort_keys(int num_at, sp_ATOM *at, T_GROUP_INFO *t_group_info,
...
int CompareNeighListLexUpToMaxRank(NEIGH_LIST pp1, NEIGH_LIST pp2,
                                   const AT_RANK *nRank,
...
NEIGH_LIST *CreateNeighListFromLinearCT(AT_NUMB *LinearCT, int nLenCT,
                                        int num_atoms);
