...
int bypass_sdf_data_items(FILE *inp, long *cas_reg_no, char *comment,
                          ...
                          const char *pSdfLabel, char *pSdfValue,
                          char *pStrErr);
MOL_DATA *read_mol_file(FILE *inp, MOL_HEADER_BLOCK *OnlyHeaderBlock,
                        MOL_CTAB *OnlyCtab, int bGetOrigCoord, int *err,
                        char *pStrErr);
...
static int read_properties_block(MOL_CTAB *ctab, MOL_HEADER_BLOCK *pHdr,
                                 FILE *inp, int err, char *pStrErr);

static int identify_sdf_label(char *inp_line, const char *pSdfLabel);
static long extract_cas_rn(char *line);
static int mol_copy_check_empty(char *dest, char *source, int len,
                                char **first_space);
static int mol_read_datum(void *data, int field_len, int data_type,
                          char **line_ptr);
...
/*************** static
 * **********************************************************/
int mol_copy_check_empty(char *dest, char *source, int len,
                         char **first_space) {
  ...
  for (i = (len - 1); i >= 0 && 0 != (c = source[i]) && isspace(UCINT c); i--)
    ;
  *first_space =
      dest + (i + 1); /* first blank or zero terminating byte in dest */
  return len; /* number of actually processed bytes; zero termination not
                 included */
}
/************* static
 * ************************************************************/
int mol_read_datum(void *data, int field_len, int data_type, char **line_ptr) {
   ...
   * 2.  return value: for MOL_STRING_DATA: number of bytes excluding trailing
   * zero for all others:  1=success; 0 = empty; -1= error
  ...
  case MOL_STRING_DATA:
    ...
    *line_ptr += (len + i); /* ptr to the 1st byte of the next input field or to
                               zero termination */
    break;
  ...
  case MOL_LONG_INT_DATA: { /* block start */
    ...
    if (field_len > MOL_MAX_VALUE_LEN) {
      ...
      *q = '\0';
      if (!len || !(q - str)) { /* empty string */
        ret = 0;
      } else if ((ldata = strtol(str, &p_end, 10),
                  p_end != q)) { /* wrong data: incompletely interpreted */
        ret = -1;
      }
    ...
    case MOL_CHAR_INT_DATA:
      if (SCHAR_MIN <= ldata &&
          ldata <= SCHAR_MAX) { /* from || to &&: 11-19-96 */
  ...
  case MOL_FLOAT_DATA: { /* block start */
    ...
    case MOL_DOUBLE_DATA:
      if (ddata != HUGE_VAL &&
          /*ldata*/ ddata !=
              -HUGE_VAL) { /* replaced ldata with ddata 6-30-98 DCh */
        *(double *)data = ddata;
      } else {
        *(double *)data = 0.0;
        ret = -1;
...
int mol_read_hdr(MOL_HEADER_BLOCK *hdr, FILE *inp, char *pStrErr) {
  ...
  */
  len = mol_read_datum(hdr->szMoleculeName, sizeof(hdr->szMoleculeName) - 1,
                       MOL_STRING_DATA, &p);
  /*----------- header line #2 -----------------------*/
  if (NULL == (p = inchi_fgetsLf(line, line_len, inp))) {
  ...
  */
  len = mol_read_datum(hdr->szUserInitials, sizeof(hdr->szUserInitials) - 1,
                       MOL_STRING_DATA, &p);
  len = mol_read_datum(hdr->szProgramName, sizeof(hdr->szProgramName) - 1,
                       MOL_STRING_DATA, &p);
  ...
  len = mol_read_datum(hdr->szDimCode, sizeof(hdr->szDimCode) - 1,
                       MOL_STRING_DATA, &p);
  ...
  p = line;
  len = mol_read_datum(hdr->szMoleculeLine2, sizeof(hdr->szMoleculeLine2) - 1,
                       MOL_STRING_DATA, &p);
  ...
  */
  len = mol_read_datum(hdr->szComment, sizeof(hdr->szComment) - 1,
                       MOL_STRING_DATA, &p);
...
int mol_read_counts_line(MOL_CTAB *ctab, FILE *inp, char *pStrErr) {
  ...
  if (line[MOLFILEMAXLINELEN]) {
    MOLFILE_ERR_SET(err, 0,
                    "Too long counts line"); /* too long input file line */
  }
  if (0 > mol_read_datum(&ctab->nNumberOfAtoms, 3, MOL_SHORT_INT_DATA, &p) ||
      ...
      0 > mol_read_datum(&ctab->nNumberOfStextEntries, 3, MOL_SHORT_INT_DATA,
                         &p)
#if (MOL_CPSS == MOL_PRESENT)
      || 0 > mol_read_datum(&ctab->nNumberOfReactionComponentsPlus1, 3,
                            MOL_SHORT_INT_DATA, &p) ||
      0 > mol_read_datum(&ctab->nNumberOfReactants, 3, MOL_SHORT_INT_DATA,
                         &p) ||
    ...
    MOLFILE_ERR_SET(
        err, 3, "Cannot interpret counts line:"); /* too long input file line */
    RemoveNonPrintable(line);
    AddMOLfileError(pStrErr, line);
    goto err_fin;
  }
  len = mol_read_datum(ctab->csCurrentCtabVersion,
                       sizeof(ctab->csCurrentCtabVersion) - 1, MOL_STRING_DATA,
                       &p);
err_fin:
  return err;
}
...
int read_atom_block(MOL_CTAB *ctab, FILE *inp, int err, char *pStrErr) {
  ...
  /*
    if ( NULL == ctab->MolAtom ){
        err = 1;
        goto err_fin; // internal error: MolAtom structure has not been
    allocated
    }
   */
  ...
  for (i = 0; i < ctab->nNumberOfAtoms; i++) {
    ...
    if (NULL != ctab->MolAtom) {
      if (0 > mol_read_datum(&ctab->MolAtom[i].fX, 10, MOL_DOUBLE_DATA, &p) ||
          ...
          0 > mol_read_datum(NULL, /* undescribed in article*/ 1,
                             MOL_JUMP_TO_RIGHT, &p) ||
          0 == mol_read_datum(
                   &ctab->MolAtom[i].szAtomSymbol, 3, MOL_STRING_DATA,
          ...
          || 0 > mol_read_datum(&ctab->MolAtom[i].cCharge, 3, MOL_CHAR_INT_DATA,
                                &p) ||
          0 > mol_read_datum(&ctab->MolAtom[i].cStereoParity, 3,
                             MOL_CHAR_INT_DATA, &p)
#if (MOL_QUERY == MOL_PRESENT)
          || 0 > mol_read_datum(&ctab->MolAtom[i].cH_countPlus1, 3,
                                MOL_CHAR_INT_DATA, &p) ||
      ...
      if (2 == strlen(ctab->MolAtom[i].szAtomSymbol) &&
          isupper(UCINT ctab->MolAtom[i].szAtomSymbol[1]))
        ctab->MolAtom[i].szAtomSymbol[1] = (char)tolower(
            UCINT ctab->MolAtom[i].szAtomSymbol[1]); /* 5-4-99 DCh*/
      ...
      if ((chg = (S_SHORT)ctab->MolAtom[i].cCharge) < 0 ||
        ...
        ctab->MolAtom[i].cCharge =
            (S_CHAR)(4 - chg); /*  allow greater charges to accommodate NCI
                                  structures. 8-20-2002 */
      ...
      if (
#if (MOL_CPSS == MOL_PRESENT)
          0 > mol_read_datum(&ctab->MolAtom[i].cH0_designator, 3,
                             MOL_CHAR_INT_DATA, &p) ||
          0 > mol_read_datum(&ctab->MolAtom[i].cReactionComponentType, 3,
                             MOL_CHAR_INT_DATA, &p) ||
          0 > mol_read_datum(&ctab->MolAtom[i].cReactionComponentNumber, 3,
                             MOL_CHAR_INT_DATA, &p)
#else
          0 > mol_read_datum(NULL, 3, MOL_JUMP_TO_RIGHT, &p) ||
          0 > mol_read_datum(NULL, 3, MOL_JUMP_TO_RIGHT, &p) ||
          ...
          || 0 > mol_read_datum(&ctab->MolAtom[i].nAtomAtomMappingNumber, 3,
                                MOL_SHORT_INT_DATA, &p) ||
          0 > mol_read_datum(&ctab->MolAtom[i].cReactionComponentType, 3,
                             MOL_CHAR_INT_DATA, &p)
#else
          ...
          || 0 > mol_read_datum(&ctab->MolAtom[i].cExactChargeFlag, 3,
                                MOL_CHAR_INT_DATA, &p)
#else
          || 0 > mol_read_datum(NULL, 3, MOL_JUMP_TO_RIGHT, &p)
#endif
...
int read_bonds_block(MOL_CTAB *ctab, FILE *inp, int err, char *pStrErr) {
  ...
  /*
    if ( NULL == ctab->MolBond ){
        err = 1;
        goto err_fin;    // internal error: memory has not been allocated for
    MolBond structure
    }
   */
  for (i = 0; i < ctab->nNumberOfBonds; i++) {
    ...
    if (ctab->MolBond) {
      if (0 > mol_read_datum(&ctab->MolBond[i].nAtomNo1, 3, MOL_SHORT_INT_DATA,
                             &p) ||
          0 > mol_read_datum(&ctab->MolBond[i].nAtomNo2, 3, MOL_SHORT_INT_DATA,
                             &p) ||
          0 > mol_read_datum(&ctab->MolBond[i].cBondType, 3, MOL_CHAR_INT_DATA,
                             &p) ||
          0 > mol_read_datum(&ctab->MolBond[i].cBondStereo, 3,
                             MOL_CHAR_INT_DATA, &p)
#if (MOL_QUERY == MOL_PRESENT)
...
/************ static
 * *************************************************************/
int read_properties_block(MOL_CTAB *ctab, MOL_HEADER_BLOCK *pHdr, FILE *inp,
   ...
   */
  for (i = 0;
       ctab->csCurrentCtabVersion[0] ? 1 : (i < ctab->nNumberOfPropertyLines);
       i++) { /* the last line should be M END */
    ...
    if (1 != mol_read_datum(charM, sizeof(charM) - 1, MOL_STRING_DATA, &p) ||
        0 != mol_read_datum(szBlank, sizeof(szBlank) - 1, MOL_STRING_DATA,
                            &p) /* must contain 0 bytes */
        || 0 >= mol_read_datum(szType, sizeof(szType) - 1, MOL_STRING_DATA,
                               &p) /* must contain 3 bytes */
    ) {
      if (!strcmp(line, SDF_END_OF_DATA)) {
        err = err ? -abs(err) : -4;
        break;
    ...
    if (!strcmp(szType, "REG")) {
      ...
      mol_read_datum(&pHdr->lInternalRegistryNumber, len, MOL_LONG_INT_DATA,
                     &p);
      continue;
    }
    ...
    if (!strcmp(szType, "CHG") &&
      ...
      for (j = 0; j < num_entries; j++) {
        if (0 > mol_read_datum(&atoms[j], 0, MOL_SHORT_INT_DATA, &p) ||
            ...
            atoms[j] <= 0 || atoms[j] > num_atoms || charges[j] < -15 ||
            charges[j] > 15) {
          goto charge_error;
        }
      }
      if (charge_encountered == -1) {
        for (j = 0; j < num_atoms; j++) {
          if (!ctab->MolAtom[j]
                   .cAtomAliasedFlag) /* do not clear aliased atoms.*/
            ctab->MolAtom[j].cCharge = ctab->MolAtom[j].cRadical = '\0';
        }
        charge_encountered = 1;
      }
      for (j = 0; j < num_entries; j++) {
        if (!ctab->MolAtom[atoms[j] - 1]
                 .cAtomAliasedFlag) /* do not change aliased atoms.*/
    ...
    if (!strcmp(szType, "RAD") &&
      ...
      for (j = 0; j < num_entries; j++) {
        if (0 > mol_read_datum(&atoms[j], 0, MOL_SHORT_INT_DATA, &p) ||
            ...
            atoms[j] <= 0 || atoms[j] > num_atoms || radicals[j] < 0 ||
            radicals[j] > 3) {
          goto radical_error;
        }
      }
      ...
      for (j = 0; j < num_entries; j++) {
        if (!ctab->MolAtom[atoms[j] - 1]
                 .cAtomAliasedFlag) { /* do not change aliased atoms. 5-3-99 DCh
                                       */
    ...
    if (!strcmp(szType, "ISO") &&
        ...
        1 <= num_entries && num_entries <= 8) {
      S_SHORT atoms[8];
      S_SHORT iso_mass[8]; /*  contains istotope mass number, not difference.
                              7-14-00 DCh. */
      ...
      if (isotope_encountered == -1) {
        for (j = 0; j < num_atoms; j++) {
          /*if ( !ctab->MolAtom[j].cAtomAliasedFlag )*/ /* clear even aliased
                                                           atoms */
      ...
      for (j = 0; j < num_entries; j++) {
        ...
        if (1 /* !ctab->MolAtom[atoms[j]-1].cAtomAliasedFlag */) {
          ...
          if (at[1] ||
              at[0] != 'D' && at[0] != 'T') { /*  D & T cannot have ISO */
            ...
            if (iso_mass[j] > 0)
              ...
              */
              ctab->MolAtom[atoms[j] - 1].cMassDifference =
                  iso_mass[j]; /* mass, not difference */
            ...
            if ((atw = get_atw(at)) &&
                abs(atw_diff = (int)iso_mass[j] - atw) < 20) {
              ctab->MolAtom[atoms[j] - 1].cMassDifference =
                  (char)(atw_diff ? atw_diff : ZERO_ATW_DIFF);
            }
#endif
          }
        }
...
MOL_DATA *read_mol_file(FILE *inp, MOL_HEADER_BLOCK *OnlyHeaderBlock,
  ...
  *err = 0;
  ...
  if (bReadAll) {
    if (NULL ==
        (mol_data->ctab.MolAtom = (MOL_ATOM *)inchi_calloc(
             inchi_max(mol_data->ctab.nNumberOfAtoms, 1), sizeof(MOL_ATOM)))) {
      ret = 2; /* can't allocate MolAtom structure */
      MOLFILE_ERR_FIN(ret, 2, err_fin, "Out of RAM");
    }
    if (bGetOrigCoord &&
        NULL == (mol_data->ctab.szCoord = (MOL_COORD *)inchi_calloc(
                     inchi_max(mol_data->ctab.nNumberOfAtoms, 1),
                     sizeof(MOL_COORD)))) {
      ret = 2; /* can't allocate MolAtom structure */
      MOLFILE_ERR_FIN(ret, 2, err_fin, "Out of RAM");
    }
  ...
  if (bReadAll && ret < 30) {
    if (!bEndOfData &&
        NULL == (mol_data->ctab.MolBond = (MOL_BONDS *)inchi_calloc(
                     inchi_max(mol_data->ctab.nNumberOfBonds, 1),
                     sizeof(MOL_BONDS)))) {
  ...
  prev_ret = ret;
  if (!bEndOfData &&
      0 != (ret = read_properties_block(pCtab, pHdr, inp, ret, pStrErr))) {
    if (ret < 0) {
      ret = -ret;
      bEndOfData = 1;
    }
...
int identify_sdf_label(char *inp_line, const char *pSdfLabel) {
  ...
  char *p, *q;
  int i, j, len;
  if ((p = strchr(inp_line, '<')) && (q = strchr(p, '>')) &&
    ...
    len = j - i + 1;
    p = line + i;
    if (pSdfLabel && pSdfLabel[0] && len == (int)strlen(pSdfLabel) &&
        !memicmp(p, pSdfLabel, len))
      return SDF_DATA_HEADER_USER;
    if (len == sizeof(sdf_data_hdr_name) - 1 &&
        !memicmp(p, sdf_data_hdr_name, len))
      return SDF_DATA_HEADER_NAME;
    if (len == sizeof(sdf_data_hdr_comm) - 1 &&
        !memicmp(p, sdf_data_hdr_comm, len))
...
int bypass_sdf_data_items(FILE *inp, long *cas_reg_no, char *comment,
  ...
  while (err == 0 && current_state != SDF_END_OF_DATA_BLOCK &&
    ...
    if (line[MOLFILEMAXLINELEN]) {
      if (current_state != SDF_DATA_HEADER && current_state != SDF_DATA_LINE &&
    ...
    case SDF_EMPTY_LINE: /* Added 9-25-97 DCh */
      ...
      if (0 == strcmp(line, SDF_END_OF_DATA)) {
        ...
        current_state = (bNeedsName || bNeedsComm || bNeedsCASrn || bNeedsUser)
                            ? identify_sdf_label(line, pSdfLabel)
                            : SDF_DATA_HEADER;
  ...
  if (err && err != 5 && current_state != SDF_END_OF_DATA_BLOCK && p) {
    /*  bypass up to $$$$ */
    while ((p = inchi_fgetsLf(line, line_len, inp)) &&
           memcmp(line, SDF_END_OF_DATA, 4))
      ;
    if (p) {
...
/**************** global **************************************************/
MOL_DATA *read_sdfile_segment(FILE *inp, MOL_HEADER_BLOCK *OnlyHeaderBlock,
                              MOL_CTAB *OnlyCtab, int bGetOrigCoord,
                              char *pname, int lname, long *Id,
                              const char *pSdfLabel, char *pSdfValue, int *err,
                              char *pStrErr) {
  MOL_DATA *mol_data = read_mol_file(inp, OnlyHeaderBlock, OnlyCtab,
                                     bGetOrigCoord, err, pStrErr);
  ...
  if (*err < 0) {
    *err = -*err; /* end of data encountered */
    ...
    err_bypass_sdf = bypass_sdf_data_items(inp, Id, NULL, 0, pname, lname, *err,
                                           pSdfLabel, pSdfValue, pStrErr);
    if (err_bypass_sdf) {
      *err =
          err_bypass_sdf; /* important to continue to the next good structure */
    }
  }
  /* } */
  return mol_data;
...
int CopyMOLfile(FILE *inp_file, long fPtrStart, long fPtrEnd, FILE *prb_file,
  ...
  if (inp_file && prb_file && fPtrStart >= 0L && fPtrEnd > fPtrStart &&
