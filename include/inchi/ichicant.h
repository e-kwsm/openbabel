...
typedef struct tagEQUIV_INFO {
  ...
  int *nFirstInSet;    /* first of equivalent atoms in the connected to the
                          cut-vertex atom parts of the structure */
  int *nNumInSet;  /* number of the equivalent atoms connected to the cut-vertex
                      atom */
...
typedef struct tagAtomInvariant {
  ...
  AT_RANK cNum_tautomer; /* 0 or for tautomer endpoint: number of endpoints in
                            the group */
...
typedef struct tagFixHOrTautCanonNumbering {
  ...
  NUM_H *nNumHOrig; /* original  H atoms positions + taut. info, excluding
                       tautomeric H */
  NUM_H *nNumH;     /* canonical H atoms positions + taut. info, excluding
                       tautomeric H */
  int nLenNumH;     /* length = num_atoms + 2*num_taut_groups */

  /* fixed H: original positions of tautomeric H; exists obly for tautomeric
   * structures */
  ...
  /*******************************************************************************/
  Partition PartitionCtIso; /* canonical numbering of isotopic base structure,
                               defined later */
  AT_RANK *nSymmRankCtIso;  /* orbits of isotopic structure */
  AT_ISO_SORT_KEY *
      iso_sort_keys; /* original isotopic sort keys for atoms and taut groups */
  AT_ISO_SORT_KEY *iso_sort_keysOrig; /* canonical isotopic sort keys for atoms
                                         and taut groups */
  int len_iso_sort_keys;
  S_CHAR *iso_exchg_atnos; /* canonical: 0=> tautomeric or may have isotopic H
                              exchanged */
  S_CHAR *iso_exchg_atnosOrig; /* original: 0=> tautomeric or may have isotopic
                                  H exchanged */
...
typedef struct tagCanonStat {
  ...
  int nLenLinearCTAtOnly;  /* connection table only without tautomeric
                              pseudoatoms  */
  int nLenLinearCTAtOnly2; /* connection table only, non-isotopic result without
                              tautomeric pseudoatoms  */
  ...
  S_CHAR *bAtomUsedForStereo; /* 0 if not a stereo atom or during a canon. rank
                                 being mapped on this atom; */
  ...
  AT_RANK *nCanonOrdTaut; /* t-group numbers numbers in order of increasing
                             canon. ranks  */
  ...
  *nCanonOrdIsotopic; /* atom numbers in order of increasing canon. ranks */
  ...
  BCN *pBCN;
  S_CHAR *nNum_H;       /* number of terminal hydrogen atoms on each atom except
                           tautomeric [num_atoms], in order of canonical numbers */
  S_CHAR *nNum_H_fixed; /* number of terminal hydrogen atoms on tautomeric atoms
                           (for non-atautomeric representation) [num_atoms] */
  S_CHAR *nExchgIsoH;
} CANON_STAT;
...
/***********************************************
 tree structure: one segment
   ...
   canon. rank
   at.no          orig. atom numbers on which the canon. rank has been
 successfully mapped
   ...
   at.no          except the last at.no: it is not known if it has been mapped
 until all atoms are mapped num.at+1       number of atoms in this segment
*/
