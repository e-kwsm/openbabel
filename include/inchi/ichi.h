...
#define REQ_MODE_CHIR_FLG_STEREO                                               \
  0x002000 /* SUCF  If Chiral flag then Abs otherwise Rel stereo */
/* end of 10-10-2003 */

/*^^^ 2009-12-05 */
#define REQ_MODE_DIFF_UU_STEREO                                                \
  0x004000 /* SLUUD Make labels for unknown and undefined stereo different */
/*^^^ 2009-12-05 */
#define REQ_MODE_MIN_SB_RING_MASK 0x0F0000 /* RSB  */
#define REQ_MODE_MIN_SB_RING_SHFT 16

#define REQ_MODE_DEFAULT                                                       \
  (REQ_MODE_BASIC | REQ_MODE_TAUT | REQ_MODE_ISO | REQ_MODE_NON_ISO |          \
   REQ_MODE_STEREO)
...
typedef struct tagINChI_Stereo { /* [N] = allocated length */
  ...
  S_CHAR *t_parity; /* tetrahedral (relative, see nCompInv2Abs) atom parities
                       [nNumberOfAtoms] */
  ...
  int nCompInv2Abs; /* 0=>Inv = Abs stereo; -1=> Inv < Abs stereo, +1=> Inv >
                       Abs stereo; +2=> in reading InChI: no /m was found and in
                       /sN N>0 */
  int bTrivialInv;  /* 1=> nCompInv2Abs!= 0 && Inverted = Abs stereo with
                       inverted parities 1<-->2 */
...
#define INCHI_FLAG_ACID_TAUT                                                   \
  0x0001 /* tautomerism of dissociated acid invoked */
...
#define INCHI_FLAG_SC_IGN_ALL_UU                                               \
  0x0008 /* ignored all undefined/unknown stereocenters, non-isotopic */
#define INCHI_FLAG_SB_IGN_ALL_UU                                               \
  0x0010 /* ignored all undefined/unknown stereocenters, non-isotopic */
#define INCHI_FLAG_SC_IGN_ALL_ISO_UU                                           \
  0x0020 /* ignored all undefined/unknown stereocenters, isotopic */
#define INCHI_FLAG_SB_IGN_ALL_ISO_UU                                           \
  0x0040 /* ignored all undefined/unknown stereocenters, isotopic */
#define INCHI_FLAG_HARD_ADD_REM_PROTON                                         \
  0x0080 /* in normalization a proton has been added or removed along alt path \
          */
...
#define INCHI_OUT_PRINT_OPTIONS                                                \
  (INCHI_OUT_EMBED_REC | INCHI_OUT_XML | INCHI_OUT_PLAIN_TEXT |                \
   INCHI_OUT_PLAIN_TEXT_COMMENTS | INCHI_OUT_XML_TEXT_COMMENTS)
...
typedef struct tagINChI { /* [N] = allocated length */
  ...
  AT_NUMB *nTautomer; /* NumGroups; ((NumAt+1, NumH,         At1..AtNumAt),...);
                       * {INCHI_T_NUM_MOVABLE = 1} - old NumGroups; ((NumAt+2,
                       * NumH, Num(-), At1..AtNumAt),...); {INCHI_T_NUM_MOVABLE
  ...
  AT_NUMB *nPossibleLocationsOfIsotopicH; /* [0]=> length including 0th element,
                                             location1,...*/
  int bDeleted;
#if (bREUSE_INCHI == 1)
  int nRefCount;
  ...
  int nLink; /* negative: ignore InChI; positive: index of (Reconnected
                component) + 1 linked to it */
#endif
} INChI;
...
typedef struct tagINChI_Aux { /* [N] = allocated length */
  ...
  int bIsTautomeric; /* filled out even though tautomeric has not been
                        requested; non-zero if taut exists */
  ...
  AT_NUMB *nOrigAtNosInCanonOrdInv;          /* inveterted stereo
                                                [nNumberOfInputAtoms*1.5]; */
