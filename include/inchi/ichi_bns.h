...
typedef S_SHORT Edge[2]; /* Edge[0] = vertex1, Edge[1] = iedge or -(1+vertex1)
                            if vertex2 = s or t */
...
#define BNS_VERT_TYPE__AUX                                                     \
  0x0080 /* vertex added to build charge substructures */
#define BNS_VERT_TYPE_C_NEGATIVE                                               \
  0x0100 /* negative charge group; attribute, should be used with              \
            BNS_VERT_TYPE_C_GROUP */
#define BNS_VERT_TYPE_ACID                                                     \
  0x0200 /* only for this type are allowed paths: t_group-atom-c_group_neg     \
            (path_TACN) */
#define BNS_VERT_TYPE_CARBON_GR                                                \
  0x0400 /* charge of carbon atom; should be used with BNS_VT_C_POS,           \
            BNS_VT_C_NEG */
#define BNS_VERT_TYPE_METAL_GR                                                 \
  0x0800 /* metal atom group; may be used alone or with BNS_VT_M_POS,          \
            BNS_VT_M_NEG */

#define BNS_VERT_TYPE_ANY_GROUP                                                \
  (BNS_VERT_TYPE_TGROUP | BNS_VERT_TYPE_C_GROUP | BNS_VERT_TYPE_SUPER_TGROUP)
...
#define BNS_VT_C_POS_C                                                         \
  (BNS_VT_C_POS |                                                              \
   BNS_VERT_TYPE_CARBON_GR) /* positive charge group, C, Si, Ge, Sn */
#define BNS_VT_C_NEG_C                                                         \
  (BNS_VT_C_NEG |                                                              \
   BNS_VERT_TYPE_CARBON_GR) /* negative charge group, C, Si, Ge, Sn */
#define BNS_VT_C_POS_M                                                         \
  (BNS_VT_C_POS | BNS_VERT_TYPE_METAL_GR) /* positive charge group, metal */
#define BNS_VT_C_NEG_M                                                         \
  (BNS_VT_C_NEG | BNS_VERT_TYPE_METAL_GR)     /* negative charge group, metal */
#define BNS_VT_M_GROUP BNS_VERT_TYPE_METAL_GR /* metal-group, flower vertex */

#define BNS_VT_C_POS_ALL                                                       \
  (BNS_VERT_TYPE_SUPER_TGROUP | BNS_VERT_TYPE_C_GROUP) /* supergroup (+) */
#define BNS_VT_C_NEG_ALL                                                       \
  (BNS_VT_C_POS_ALL | BNS_VERT_TYPE_C_NEGATIVE) /* supergroup (-) */

#define BNS_VT_CHRG_STRUCT                                                     \
  (BNS_VERT_TYPE__AUX | BNS_VERT_TYPE_TEMP)   /* ChargeStruct vertex */
...
#define IS_BNS_VT_CM_GR(X)                                                     \
  (((X) & BNS_VT_C_POS_M) == BNS_VT_C_POS_M) /* metal charge group */
#define IS_BNS_VT_M_GR(X)                                                      \
  ((X) == BNS_VERT_TYPE_METAL_GR) /* metal flower base or vertices */
#define IS_BNS_VT_YVCONNECTOR(X)                                               \
  (((X) & BNS_VERT_TYPE__AUX) && !((X) & BNS_VERT_TYPE_TEMP))
#define IS_BNS_VT_CHRG_STRUCT(X)                                               \
  (((X) & BNS_VERT_TYPE__AUX) && ((X) & BNS_VERT_TYPE_TEMP))
#define IS_BNS_VT_ATOM(X) ((X) & BNS_VERT_TYPE_ATOM)

#define BNS_ADD_SUPER_TGROUP                                                   \
  1 /* reserve one more edge for a t-group to connect to a single              \
       super-t-group */
#define NUM_KINDS_OF_GROUPS                                                    \
  2 /* 1 accounts for t-group kind, one more 1 accounts for c-group kind */

#define BNS_ADD_ATOMS                                                          \
  2 /* max. number of fictitious atoms to add (except t-gtoups) */
#define BNS_ADD_EDGES                                                          \
  1 /* max. number of edges to add to each atom (except edges to a t-group or  \
       c-group) */
...
#define ALTP_MAY_ADD(altp)                                                     \
  (iALTP_NEIGHBOR + (altp)[iALTP_PATH_LEN].number <                            \
...
#define EDGE_FLOW_ST_PATH                                                      \
  0x4000 /* mark: the edge belongs to the augmenting path */
...
#define MAX_AT_FLOW(X)                                                         \
  (((X).chem_bonds_valence - (X).valence) +                                    \
   ((is_centerpoint_elem((X).el_number) ||                                     \
     get_endpoint_valence((X).el_number))                                      \
        ? (((X).radical ==                                                     \
            RADICAL_DOUBLET) /*+2*((X).radical==RADICAL_TRIPLET)*/)            \
        : 0))
...
typedef struct BnsEdge {
  ...
  AT_NUMB neigh_ord[2]; /* ordering number of the neighbor: [0]: at<neighbor,
                           [1]: at>neighbor */
...
typedef struct BnsVertex {
  ...
  /* indexes of Edges */
  BNS_IEDGE *iedge; /* 6 a pointer to the array of edge indexes adjacent to this
                       vertex */
} BNS_VERTEX;
...
typedef struct BalancedNetworkStructure {
  ...
  /*int len_atoms; */  /* size of filled with real atoms portion of the
                          BNS_VERTEX data */
  ...
  int num_vertices; /* total number currently in effect; includes t-groups and
                       added atoms */
  ...
  int num_added_edges; /* number of added edges (not including edges to
                          t-groups) */
  int nMaxAddEdges;    /* max. number edges of add to each atom (not including
                          edges to t-groups) */
  ...
  int bNotASimplePath; /* alternating path traversed same bond 2 times in
                          opposite directions */
  ...
  AT_NUMB type_TACN; /* BNS_VERT_TYPE_ACID: if non-zero than only for it path
                        type_T-type_TACN-type_CN allowed */
...
typedef struct BalancedNetworkData {
  ...
  Edge *SwitchEdge; /*[MAX_VERTEX];  a pair of vertices and an edge, implemented
                       here as [*][2] array */
  S_CHAR *Tree;     /*[MAX_VERTEX];  indicates presence in ScanQ, T, T',
                       s-reachability */
  ...
  int nNumRadEdges;
  int nNumRadicals;
  BRS_MODE bRadSrchMode; /* 1 => connect fict. vertices-radicals to the
                            accessible atoms */
#endif
} BN_DATA;
...
#define ALT_PATH_MODE_4_SALT2                                                  \
  4 /* mark alt bonds along the path, path to taut. group fict. vertex if      \
       exists */
#define ALT_PATH_MODE_REM2H_CHG                                                \
  5 /* remove 2 H along alt. path AH-=-BH => A=-=B and change bonds to         \
       alternating */
#define ALT_PATH_MODE_ADD2H_CHG                                                \
  6 /* add 2 H along alt. path A=-=B => AH-=-BH    and change bonds to         \
       alternating */
#define ALT_PATH_MODE_REM2H_TST                                                \
  7 /* test-remove 2 H along alt. path AH-=-BH => A=-=B; restore changed bonds \
     */
#define ALT_PATH_MODE_ADD2H_TST                                                \
  8 /* test-add 2 H along alt. path A=-=B => AH-=-BH; restore changed bonds */
#define ALT_PATH_MODE_REM_PROTON                                               \
  9 /* remove proton, adjust bonds, charges, H-counts 2004-03-05 */
#if (KETO_ENOL_TAUT == 1)
#define ALT_PATH_MODE_TAUTOM_KET                                               \
  10 /* same as ALT_PATH_MODE_TAUTOM, applies to C=-OH or CH-=O; H may be (-)  \
      */
#endif
...
/*********************************************************************************
  bChangeFlow:
      ...
      3 => change flow inside the BNS search and undo the flow change in the BNS
structure here 4 => change bonds in the structure according to the flow 8 =>
make altern. bonds in the structure

  Note: (bChangeFlow & 1) == 1 is needed for multiple runs
**********************************************************************************/

/* "EF" = "Edge Flow" */
#define BNS_EF_CHNG_FLOW                                                       \
  1 /* change Balanced Network (BN) flow inside the BNS search */
#define BNS_EF_RSTR_FLOW 2 /* undo BN flow changes after BNS */
#define BNS_EF_CHNG_RSTR (BNS_EF_CHNG_FLOW | BNS_EF_RSTR_FLOW)
#define BNS_EF_CHNG_BONDS                                                      \
  4 /* change bonds in the structure according to the BN flow */
#define BNS_EF_ALTR_BONDS                                                      \
  8 /* make altern. bonds in the structure if the flow has changed */
...
#define BNS_EF_UPD_H_CHARGE                                                    \
  64 /* update charges and H-counts according to change flow to c- and t-group \
        vertices */

#define BNS_EF_SAVE_ALL                                                        \
  (BNS_EF_CHNG_FLOW | BNS_EF_CHNG_BONDS | BNS_EF_UPD_RAD_ORI)
...
int nExists2AtMoveAltPath(struct BalancedNetworkStructure *pBNS,
                          ...
                          struct BN_AtomsAtTautGroup *pAATG, inp_ATOM *at,
                          int num_atoms, int jj2, int jj1,
                          struct tagSaltChargeCandidate *s_candidate,
                          int nNumCandidates, AT_NUMB *nForbiddenAtom,
                          int nNumForbiddenAtoms);
int bExistsAltPath(struct BalancedNetworkStructure *pBNS,
                   struct BalancedNetworkData *pBD,
                   struct BN_AtomsAtTautGroup *pAATG, inp_ATOM *at,
                   int num_atoms, int nVertDoubleBond, int nVertSingleBond,
                   int path_type);
int bExistsAnyAltPath(struct BalancedNetworkStructure *pBNS,
                      struct BalancedNetworkData *pBD, inp_ATOM *at,
                      int num_atoms, int nVertDoubleBond, int nVertSingleBond,
                      int path_type);
int AddTGroups2BnStruct(struct BalancedNetworkStructure *pBNS, inp_ATOM *at,
                        int num_atoms, struct tagTautomerGroupsInfo *tgi);
int AddSuperTGroup2BnStruct(struct BalancedNetworkStructure *pBNS, inp_ATOM *at,
                            int num_atoms, struct tagTautomerGroupsInfo *tgi);
int AddCGroups2BnStruct(struct BalancedNetworkStructure *pBNS, inp_ATOM *at,
                        int num_atoms, struct tagChargeGroupsInfo *cgi);

int ReInitBnStruct(struct BalancedNetworkStructure *pBNS, inp_ATOM *at,
                   int num_at, int bRemoveGroupsFromAtoms);
int ReInitBnStructAddGroups(struct BalancedNetworkStructure *pBNS, inp_ATOM *at,
                            int num_atoms, struct tagTautomerGroupsInfo *tgi,
                            struct tagChargeGroupsInfo *cgi);

int DisconnectTestAtomFromTGroup(struct BalancedNetworkStructure *pBNS, int v1,
                                 int *pv2, BNS_FLOW_CHANGES *fcd);
int DisconnectTGroupFromSuperTGroup(struct BalancedNetworkStructure *pBNS,
                                    int v1, int *pv1, int *pv2,
                                    BNS_FLOW_CHANGES *fcd);
int ReconnectTestAtomToTGroup(struct BalancedNetworkStructure *pBNS, int v1,
                              int v2, int ie, BNS_FLOW_CHANGES *fcd);
...
BN_STRUCT *AllocateAndInitBnStruct(inp_ATOM *at, int num_atoms,
                                   ...
                                   int max_altp, int *num_changed_bonds);
BN_STRUCT *DeAllocateBnStruct(BN_STRUCT *pBNS);
int ReInitBnStructAltPaths(BN_STRUCT *pBNS);
int ReInitBnStructForMoveableAltBondTest(BN_STRUCT *pBNS, inp_ATOM *at,
                                         int num_atoms);
