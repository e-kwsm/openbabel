...
/*
   = num_1H + AT_ISO_SORT_KEY_MULT*(num_D +
   AT_ISO_SORT_KEY_MULT*(num_T+AT_ISO_SORT_KEY_MULT*iso_atw_diff))
*/
...
typedef struct tagAtom {
  ...
  AT_NUMB init_rank; /* also used in remove_terminal_HDT() to save orig. at.
                        number */
  ...
  */
  ...
  U_CHAR el_number; /* periodic table number = charge of the nucleus = number of
                       the protons */
  ...
  */
  AT_NUMB stereo_bond_neighbor[MAX_NUM_STEREO_BONDS];  /* Original number of an
                                                          opposite atom */
  AT_NUMB stereo_bond_neighbor2[MAX_NUM_STEREO_BONDS]; /*     (stereo bond
                                                          neighbor) +1; */
  S_CHAR stereo_bond_ord[MAX_NUM_STEREO_BONDS];        /* Ordering number of a
  ...
  S_CHAR stereo_bond_parity
      [MAX_NUM_STEREO_BONDS]; /* parity + MULT_STEREOBOND*chain_length, */
  S_CHAR stereo_bond_parity2[MAX_NUM_STEREO_BONDS]; /* where: */
                                                    /*
                                                       ...
                                                       bit KNOWN_PARITIES_EQL =  0x40: all pairs of const. equ. atoms are
                                                       connected by stereo bonds                                                   and these bonds have identical parities
                                                    */
  ...
  S_CHAR parity;  /* -- Mask 0x07=BITS_PARITY: --
                  ...
                  4 = AB_PARITY_UNDF => parity cannot be defined because of
                  symmetry or not well defined geometry
                 */
  S_CHAR parity2; /* parity including parity due to isotopic terminal H */
  /* bit msks: 0x07 => known parity (1,2,3,4) or AB_PARITY_CALC=6,
   * AB_PARITY_IISO = 6 */
...
#define MASK_CUMULENE_LEN                                                      \
  0x38 /* mask to retrieve (cumulene chain length - 1)*MULT_STEREOBOND */
#define KNOWN_PARITIES_EQL                                                     \
  0x40 /* parity is same for all pairs of constit. equivalent atoms */
#define MAX_CUMULENE_LEN 2 /* max number of bonds in a cumulene chain - 1 */

#define MULT_STEREOBOND                                                        \
  0x08 /* multiplier for cumulene chain length                                 \
...
#define BOND_CHAIN_LEN(X)                                                      \
  (GET_BITS_CUMULENE_LEN(X) /                                                  \
   MULT_STEREOBOND) /* 0 => double bond, 1 => allene, 2 => cumulene,..*/
...
#define AB_PARITY_UNDF                                                         \
  4 /* 4 => parity cannot be defined because of symmetry or not well defined   \
       geometry */
...
#define AB_MAX_KNOWN_PARITY                                                    \
  4 /* precalculated from const. equivalence parities */
#define AB_MIN_KNOWN_PARITY 1

#define AB_MAX_PART_DEFINED_PARITY                                             \
  3 /* 1, 2, 3 => defined parities, uncluding 'unknown' */
#define AB_MIN_PART_DEFINED_PARITY                                             \
  1 /* min(AB_PARITY_ODD, AB_PARITY_EVEN, AB_PARITY_UNKN) */

#define AB_MAX_WELL_DEFINED_PARITY                                             \
  2 /* 1, 2 => well defined parities, uncluding 'unknown' */
...
#define MIN_DOT_PROD                                                           \
  50 /* min value of at->stereo_bond_z_prod[i] to define parity */

#define ATOM_PARITY_VAL(X) (X)
#define ATOM_PARITY_PART_DEF(X)                                                \
  (AB_MIN_PART_DEFINED_PARITY <= (X) && (X) <= AB_MAX_PART_DEFINED_PARITY)
#define ATOM_PARITY_ILL_DEF(X)                                                 \
  (AB_MIN_ILL_DEFINED_PARITY <= (X) && (X) <= AB_MAX_ILL_DEFINED_PARITY)
#define ATOM_PARITY_KNOWN(X)                                                   \
  (AB_MIN_KNOWN_PARITY <= (X) && (X) <= AB_MAX_KNOWN_PARITY)
...
#define PARITY_PART_DEF(X)                                                     \
  (AB_MIN_PART_DEFINED_PARITY <= PARITY_VAL(X) &&                              \
   PARITY_VAL(X) <= AB_MAX_PART_DEFINED_PARITY)
#define PARITY_ILL_DEF(X)                                                      \
  (AB_MIN_ILL_DEFINED_PARITY <= PARITY_VAL(X) &&                               \
   PARITY_VAL(X) <= AB_MAX_ILL_DEFINED_PARITY)
#define PARITY_KNOWN(X)                                                        \
  (AB_MIN_KNOWN_PARITY <= PARITY_VAL(X) && PARITY_VAL(X) <= AB_MAX_KNOWN_PARITY)
#define PARITY_WELL_DEF(X)                                                     \
  (AB_MIN_WELL_DEFINED_PARITY <= PARITY_VAL(X) &&                              \
   PARITY_VAL(X) <= AB_MAX_WELL_DEFINED_PARITY)
