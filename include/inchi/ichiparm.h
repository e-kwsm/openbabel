...
int ReadCommandLineParms(int argc, const char *argv[], INPUT_PARMS *ip,
  ...
  int nMode = 0;
  int nReleaseMode =
      nMode | (REQ_MODE_BASIC | REQ_MODE_TAUT | REQ_MODE_ISO | REQ_MODE_STEREO);
  ...
  int bINChIOutputOptions =
      INCHI_OUT_EMBED_REC; /* embed reconnected & output full aux info */
  ...
  int is_gui = 0;
  int bINChIOutputOptions =
      ((EMBED_REC_METALS_INCHI == 1) ? INCHI_OUT_EMBED_REC : 0);
  ...
  int bAcidTautomerism =
      (DISCONNECT_SALTS == 1) ? (TEST_REMOVE_S_ATOMS == 1 ? 2 : 1) : 0;
...
#define VER103_DEFAULT_MODE                                                    \
  (REQ_MODE_TAUT | REQ_MODE_ISO | REQ_MODE_STEREO | REQ_MODE_SB_IGN_ALL_UU |   \
  ...
  int bPointedEdgeStereo =
      STEREO_WEDGE_ONLY; /*   NEWPS TG_FLAG_POINTED_EDGE_STEREO*/
#endif
#if (FIX_ADJ_RAD == 1)
...
#error Wrong initialization
  ...
  *ulDisplTime = 0;
  ...
  for (i = 1; i < argc; i++) {
    ...
    /* if ( !(bVer1Options & 1) && INCHI_OPTION_PREFX == argv[i][0] &&
     * INCHI_OPTION_PREFX != argv[i][1] ) */
    if (is_gui && INCHI_OPTION_PREFX == argv[i][0] &&
        INCHI_OPTION_PREFX != argv[i][1]) {
      ...
      else if (INPUT_NONE == ip->nInputType && (!memicmp(pArg, "SDF", 3)) &&
               (pArg[3] == ':')) {
        k = 0;
        ...
        if (k == 0) {
          ...
          bINChIOutputOptions &=
              ~(INCHI_OUT_NO_AUX_INFO |
                INCHI_OUT_SHORT_AUX_INFO); /* include full aux info */
        } else if (k == 2) {
          bINChIOutputOptions &=
              ~INCHI_OUT_NO_AUX_INFO; /* include short aux info */
          bINChIOutputOptions |= INCHI_OUT_SHORT_AUX_INFO;
        } else {
      ...
      /*--- All modes (std and non-std InChI) structure perception options ---*/
        ...
        nMode &= ~(REQ_MODE_RACEMIC_STEREO | REQ_MODE_RELATIVE_STEREO |
      ...
      /* Non-std InChI structure perception options */
        ...
        nMode |=
            REQ_MODE_CHIR_FLG_STEREO; /* stereo defined by the Chiral flag */
        nMode |= REQ_MODE_STEREO;
        bStdFormat = 0;
      ...
      /*--- (hidden) Old output and other options ---*/
        ...
        /*for ( k = 0; c=pArg[k]; k ++ )*/
        k = 0;
        c = pArg[k]; /* prohibit multiple option concatenations, strict syntax
                        check 2008-11-05 DT  */
        {
          c = toupper(c);
          switch (c) {
          ...
          case 'W':
            ...
            t = strtod(pArg + k + 1,
                       (char **)&q); /*  cast deliberately discards 'const'
                                        qualifier */
            if ((q > pArg + k + 1 && errno == ERANGE) || t < 0.0 ||
                t * 1000.0 > (double)ULONG_MAX) {
              ul = 0;
            } else {
    ...
    /*=== end of parsing TARGET_LIB_FOR_WINCHI GUI (and v. 0.9xx Beta)options
    ...
    else if ((bVer1Options & 1) && INCHI_OPTION_PREFX == argv[i][0] &&
             argv[i][1]) {
      ...
      /*--- Output options ---*/
        ...
        bDisplay |= 1;
      } else if (!memicmp(pArg, "F", 1) &&
                 (c = (int)strtol(pArg + 1, (char **)&q, 10), q > pArg + 1)) {
      ...
      /* Non-std InChI structure perception options */
        ...
        bVer1DefaultMode |=
            REQ_MODE_STEREO; /* stereo defined by the Chiral flag */
      ...
      /*--- Generation options ---*/
      else if (!memicmp(pArg, "W", 1) &&
               (t = strtod(pArg + 1, (char **)&q), q > pArg + 1)) {
      ...
      /*--- (hidden) Old output and other options ---*/
        ...
        bINChIOutputOptions &=
            ~(INCHI_OUT_NO_AUX_INFO |
              INCHI_OUT_SHORT_AUX_INFO); /* include short aux info */
        ...
        bDisplayIfRestoreWarnings = 1; /* InChI->Structure debugging: Display
                                          Debug Structure Restore Components */
      }
#endif
      else if (!stricmp(pArg, "NoVarH")) {
        bTgFlagVariableProtons = 0;
        ...
        bINChIOutputOptions =
            ((EMBED_REC_METALS_INCHI == 1) ? INCHI_OUT_EMBED_REC : 0) |
            INCHI_OUT_NO_AUX_INFO; /* minimal compressed output */
        ip->bCtPredecessors = 1;
        ip->bAbcNumbers = 1;
        bXml |= INCHI_OUT_PLAIN_TEXT | INCHI_OUT_PLAIN_TEXT_COMMENTS;
        bXml &= ~(INCHI_OUT_XML | INCHI_OUT_XML_TEXT_COMMENTS);
    ...
    /*=== end of parsing stand-alone/library InChI options ===*/
      ...
      if (argv[i] && argv[i][0]) {
        if ((sz =
                 (char *)inchi_malloc((strlen(argv[i]) + 1) * sizeof(sz[0])))) {
          strcpy(sz, argv[i]);
        }
#ifdef CML_DEBUG
...
#ifndef TARGET_LIB_FOR_WINCHI
      inchi_ios_eprint(log_file, "Terminating: generation of InChIKey is not "
                                 "available with 'Compress' option\n");
      return -1;
#endif
      bHashKey = 0;
    }
...
#ifndef TARGET_LIB_FOR_WINCHI
      inchi_ios_eprint(log_file, "Terminating: generation of InChIKey is not "
                                 "available in InChI conversion mode\n");
      return -1;
#endif
      bHashKey = 0;
...
#ifndef TARGET_LIB_FOR_WINCHI
      inchi_ios_eprint(log_file, "Terminating: generation of InChIKey is not "
                                 "available with 'OutputSDF' option\n");
      return -1;
#endif
      bHashKey = 0;
    }
  ...
  if (bNameSuffix || bOutputPath) {
    ...
    int len;
    const char szNUL[] =
        "NUL"; /* fix for AMD processor: use const char[] instead of just "NUL"
                  constant 2008-11-5 DT */
  ...
  if (INPUT_INCHI == ip->nInputType) {
    ...
    if (!(bINChIOutputOptions &
        ...
        && !bOutputMolfileOnly &&
        !(bXml & (INCHI_OUT_XML | INCHI_OUT_XML_TEXT_COMMENTS |
                  INCHI_OUT_PLAIN_TEXT | INCHI_OUT_XML_TEXT_COMMENTS))
#endif
    ) {
  ...
  if (!nMode || nMode == REQ_MODE_STEREO) {
    ...
    nMode |= (REQ_MODE_BASIC | REQ_MODE_TAUT | REQ_MODE_ISO | REQ_MODE_NON_ISO |
              REQ_MODE_STEREO);
  } else {
  ...
  /*  if the user requested isotopic then unconditionally add non-isotopic
   * output. */
  ...
  nMode |=
      (nMinDbRinSize << REQ_MODE_MIN_SB_RING_SHFT) & REQ_MODE_MIN_SB_RING_MASK;
#endif
  /*  input file */
  ...
  ip->bINChIOutputOptions =
      bINChIOutputOptions | (bOutputMolfileOnly ? INCHI_OUT_SDFILE_ONLY : 0);
  if (bOutputMolfileOnly) {
    bXml &=
        ~(INCHI_OUT_XML | INCHI_OUT_PLAIN_TEXT | INCHI_OUT_PLAIN_TEXT_COMMENTS |
          INCHI_OUT_XML_TEXT_COMMENTS | INCHI_OUT_TABBED_OUTPUT);
#if (SDF_OUTPUT_DT == 1)
  ...
  if (bXml & INCHI_OUT_XML) {
    bXml &= ~(INCHI_OUT_PLAIN_TEXT | INCHI_OUT_XML_TEXT_COMMENTS |
              INCHI_OUT_TABBED_OUTPUT);
  }
#ifdef TARGET_LIB_FOR_WINCHI
  if (!(bDisplay & 1)) {
    bXml &= ~(INCHI_OUT_PLAIN_TEXT_COMMENTS |
              INCHI_OUT_XML_TEXT_COMMENTS); /* do not output comments in wINChI
                                               text file results */
  } else {
  ...
  ip->bTautFlags |=
      (bDisconnectCoord && bReconnectCoord) ? TG_FLAG_RECONNECT_COORD : 0;
  ...
  if (bFixFB2) {
    ...
    ip->bTautFlags |=
        TG_FLAG_FIX_ISO_FIXEDH_BUG; /* accomodate FIX_ISO_FIXEDH_BUG */
#endif
#if (FIX_TERM_H_CHRG_BUG == 1)
    ip->bTautFlags |=
        TG_FLAG_FIX_TERM_H_CHRG_BUG; /* accomodate FIX_TERM_H_CHRG_BUG */
#endif
  ...
  if (!bHashKey) {
    if ((bHashXtra1 != 0) || (bHashXtra2 != 0))
      inchi_ios_eprint(
          log_file, "Hash extension(s) not generated: InChIKey not requested");
  } else {
    if (bHashXtra1) {
      if (bHashXtra2)
...
int PrintInputParms(INCHI_IOSTREAM *log_file, INPUT_PARMS *ip) {
...
#ifdef TARGET_LIB_FOR_WINCHI
  int bInChI2Struct =
      0; /* as of December 2008, winchi-1 does not convert InChI to structure */
#else
  int bInChI2Struct = (ip->bReadInChIOptions & READ_INCHI_TO_STRUCTURE) &&
                      ip->nInputType == INPUT_INCHI;
#endif
  ...
  if (ip->bINChIOutputOptions & INCHI_OUT_SDFILE_ONLY) {
    inchi_ios_eprint(log_file,
                     "Output SDfile only without stereochemical information "
  ...
  if (!bStdFormat) {
    if ((nMode & (REQ_MODE_BASIC | REQ_MODE_TAUT)) ==
      ...
      inchi_ios_eprint(log_file,
                       "  Mobile H Perception ON  (omit FixedH layer)\n");
    ...
    if (nMode & REQ_MODE_STEREO) {
      inchi_ios_eprint(log_file, "  %s%s%s%sStereo ON\n",
                       ...
                       : (nMode & REQ_MODE_CHIR_FLG_STEREO) ? "Chiral Flag "
                                                            : "Absolute ");
      if (0 == (nMode & (REQ_MODE_SB_IGN_ALL_UU | REQ_MODE_SC_IGN_ALL_UU)))
        inchi_ios_eprint(
            log_file,
            "  Include undefined/unknown stereogenic centers and bonds\n");
      else if (REQ_MODE_SC_IGN_ALL_UU ==
               (nMode & (REQ_MODE_SB_IGN_ALL_UU | REQ_MODE_SC_IGN_ALL_UU)))
        inchi_ios_eprint(log_file,
                         "  Omit undefined/unknown stereogenic centers\n");
      else if (REQ_MODE_SB_IGN_ALL_UU ==
               (nMode & (REQ_MODE_SB_IGN_ALL_UU | REQ_MODE_SC_IGN_ALL_UU)))
        inchi_ios_eprint(log_file,
  ...
  if (!bStdFormat) {
    ...
    if (TG_FLAG_PHOSPHINE_STEREO & ip->bTautFlags)
      ...
      inchi_ios_eprint(log_file,
                       "  Do not include phosphine stereochemistry\n");
    ...
    if (!(TG_FLAG_FIX_SP3_BUG & ip->bTautFlags))
      inchi_ios_eprint(log_file, "  Turned OFF fix of bug leading to missing "
                                 "or undefined sp3 parity\n");
    if (!(TG_FLAG_FIX_ISO_FIXEDH_BUG & ip->bTautFlags))
      inchi_ios_eprint(log_file,
                       "  Turned OFF bug-fixes found after v.1.02b release\n");
    if (!(ip->bFixNonUniformDraw))
      inchi_ios_eprint(log_file,
                       "  Turned OFF fixes of non-uniform drawing issues\n");
  ...
  if (ip->bSaveWarningStructsAsProblem && ip->path[3] && ip->path[3][0])
    inchi_ios_eprint(log_file,
                     "Saving warning structures into the problem file\n");
  if (ip->bSaveAllGoodStructsAsProblem && ip->path[3] && ip->path[3][0])
    inchi_ios_eprint(log_file,
                     "Saving only all good structures into the problem file\n");
    /*  Report debug modes */
#if (bRELEASE_VERSION != 1)
  ...
  inchi_ios_eprint(log_file,
                   "     REPLACE_ALT_WITH_TAUT=%d; NEUTRALIZE_ENDPOINTS=%d; "
                   "BNS_PROTECT_FROM_TAUT=%d\n",
                   REPLACE_ALT_WITH_TAUT, NEUTRALIZE_ENDPOINTS,
                   BNS_PROTECT_FROM_TAUT);
  inchi_ios_eprint(log_file,
                   "     DISCONNECT_SALTS=%d;   TEST_TAUT_SALTS=%d;    "
                   "TEST_TAUT2_SALTS=%d\n",
  ...
  inchi_ios_eprint(log_file,
                   "     CHARGED_ACID_TAUT_ONLY=%d MERGE_TAUT_SALTS=%d\n",
  ...
  inchi_ios_eprint(log_file,
                   "\nDbg: TEST_RENUMB_ATOMS=%d; TEST_RENUMB_NEIGH=%d; "
                   "TEST_RENUMB_SWITCH=%d\n",
...
void HelpCommandLineParms(INCHI_IOSTREAM *f) {
  ...
  inchi_ios_print_nodisplay(
      ...
      "%s ver %s%s.\n\nUsage:\ninchi_main inputFile [outputFile [logFile "
      "[problemFile]]] [%coption[ %coption...]]\n",
      INCHI_NAME, INCHI_VERSION, TARGET_ID_STRING, INCHI_OPTION_PREFX,
      INCHI_OPTION_PREFX);
#else
      "%s ver %s%s.\n\nUsage:\ninchi-%s inputFile [outputFile [logFile "
      "[problemFile]]] [%coption [%coption...]]\n",
      INCHI_NAME, INCHI_VERSION, TARGET_ID_STRING, INCHI_VERSION,
      INCHI_OPTION_PREFX, INCHI_OPTION_PREFX);
#if (BUILD_WITH_AMI == 1)
  inchi_ios_print_nodisplay(
      f, "inchi-%s inputFiles... %cAMI [%coption[ %coption...]]\n",
  ...
  inchi_ios_print_nodisplay(f, "\nInput\n");
  inchi_ios_print_nodisplay(
      f, "  STDIO       Use standard input/output streams\n");
  inchi_ios_print_nodisplay(f,
                            "  InpAux      Input structures in %s default aux. "
                            "info format\n              (for use with STDIO)\n",
                            INCHI_NAME);
  inchi_ios_print_nodisplay(f, "  SDF:DataHeader Read from the input SDfile "
                               "the ID under this DataHeader\n");
#if (ADD_CMLPP == 1)
  inchi_ios_print_nodisplay(f, "  CML         Input in CML format (default for "
                               "input file .CML extension)\n");
#endif
/*
    inchi_ios_print_nodisplay( f, "  START:n     Skip structures up to n-th
   one\n"); inchi_ios_print_nodisplay( f, "  END:m       Skip structures after
   m-th one\n");
*/
#if (BUILD_WITH_AMI == 1)
  inchi_ios_print_nodisplay(
      f, "  AMI         Allow multiple input files (wildcards supported)\n");
#endif

  inchi_ios_print_nodisplay(f, "Output\n");
  inchi_ios_print_nodisplay(
      f, "  AuxNone     Omit auxiliary information (default: Include)\n");
  inchi_ios_print_nodisplay(f, "  SaveOpt     Save custom InChI creation "
                               "options (non-standard InChI)\n");
  inchi_ios_print_nodisplay(
      f,
      "  NoLabels    Omit structure number, DataHeader and ID from %s output\n",
      INCHI_NAME);
  inchi_ios_print_nodisplay(
      f, "  Tabbed      Separate structure number, %s, and AuxInfo with tabs\n",
      INCHI_NAME);

  /*inchi_ios_print_nodisplay( f, "  Compress    Compressed output\n"); */
  /*inchi_ios_print_nodisplay( f, "    FULL        Standard set of options for
   * Full Verbose Output\n");*/
  /*inchi_ios_print_nodisplay( f, "    MIN         Standard set of options for
   * Minimal Concise Output\n");*/
#if (defined(_WIN32) && defined(_MSC_VER) && !defined(COMPILE_ANSI_ONLY) &&    \
     !defined(TARGET_API_LIB))
  inchi_ios_print_nodisplay(f, "  D           Display the structures\n");
  inchi_ios_print_nodisplay(
      f, "  EQU         Display sets of identical components\n");
  inchi_ios_print_nodisplay(
      f, "  Fnumber     Set display Font size in number of points\n");
#endif
  /*inchi_ios_print_nodisplay( f, "    PLAIN       Plain text output (Default:
   * XML format)\n");*/
  inchi_ios_print_nodisplay(
      f, "  OutputSDF   Convert %s created with default aux. info to SDfile\n",
      INCHI_NAME);
#if (SDF_OUTPUT_DT == 1)
  inchi_ios_print_nodisplay(
      f, "  SdfAtomsDT  Output Hydrogen Isotopes to SDfile as Atoms D and T\n");
#endif
#if (BUILD_WITH_AMI == 1)
  inchi_ios_print_nodisplay(
      f, "  AMIOutStd   Write output to stdout (in AMI mode)\n");
  inchi_ios_print_nodisplay(
      f, "  AMILogStd   Write log to stderr (in AMI mode)\n");
  inchi_ios_print_nodisplay(
      f, "  AMIPrbNone  Suppress creation of problem files (in AMI mode)\n");
#endif
  inchi_ios_print_nodisplay(f, "Structure perception\n");
  inchi_ios_print_nodisplay(
      f, "  SNon        Exclude stereo (default: include absolute stereo)\n");
  inchi_ios_print_nodisplay(f, "  NEWPSOFF    Both ends of wedge point to "
                               "stereocenters (default: a narrow end)\n");
  inchi_ios_print_nodisplay(f, "  DoNotAddH   All H are explicit (default: add "
                               "H according to usual valences)\n");

#ifndef USE_STDINCHI_API
  inchi_ios_print_nodisplay(
      f, "Stereo perception modifiers (non-standard InChI)\n");
  inchi_ios_print_nodisplay(f, "  SRel        Relative stereo\n");
  inchi_ios_print_nodisplay(f, "  SRac        Racemic stereo\n");
  inchi_ios_print_nodisplay(f, "  SUCF        Use Chiral Flag: On means "
                               "Absolute stereo, Off - Relative\n");

  inchi_ios_print_nodisplay(
      f, "Customizing InChI creation (non-standard InChI)\n");
  inchi_ios_print_nodisplay(
      f, "  SUU         Always include omitted unknown/undefined stereo\n");
  inchi_ios_print_nodisplay(
      f,
      "  SLUUD       Make labels for unknown and undefined stereo different\n");
  inchi_ios_print_nodisplay(
      f, "  RecMet      Include reconnected metals results\n");
  inchi_ios_print_nodisplay(f, "  FixedH      Include Fixed H layer\n");
  inchi_ios_print_nodisplay(
      f, "  KET         Account for keto-enol tautomerism (experimental)\n");
  inchi_ios_print_nodisplay(
      f, "  15T         Account for 1,5-tautomerism (experimental)\n");
#endif

  inchi_ios_print_nodisplay(f, "Generation\n");
  inchi_ios_print_nodisplay(f, "  Wnumber     Set time-out per structure in "
                               "seconds; W0 means unlimited\n");
  inchi_ios_print_nodisplay(
      f,
      "  WarnOnEmptyStructure Warn and produce empty %s for empty structure\n",
      INCHI_NAME);
  inchi_ios_print_nodisplay(f, "  Key         Generate InChIKey\n");
  inchi_ios_print_nodisplay(f, "  XHash1      Generate hash extension (to 256 "
                               "bits) for 1st block of InChIKey\n");
  inchi_ios_print_nodisplay(f, "  XHash2      Generate hash extension (to 256 "
                               "bits) for 2nd block of InChIKey\n");
  ...
  inchi_ios_print_nodisplay(
      f, "  InChI2Struct Test mode: Mol/SDfile->%s->Structure->(%s+AuxInfo)\n",
      INCHI_NAME, INCHI_NAME);
#else
  inchi_ios_print_nodisplay(
      f, "  InChI2InChI  Convert %s string(s) into %s string(s)\n", INCHI_NAME,
      INCHI_NAME);
  inchi_ios_print_nodisplay(f, "  InChI2Struct Convert InChI string(s) to "
                               "structure(s) in InChI aux.info format\n");
#endif
...
breleaseversion<1
#else
    inchi_ios_print_nodisplay( f, "%s ver %s. Special testing version
12-12-2002.\n", INCHI_NAME, INCHI_VERSION); inchi_ios_print_nodisplay( f,
"\nUsage:\ncINChI09b inputFile [outputFile [logFile [problemFile]]] [%coption[
%coption...]]\n", INCHI_OPTION_PREFX, INCHI_OPTION_PREFX);
    inchi_ios_print_nodisplay( f, "\nOptions:\n");
    ...
    inchi_ios_print_nodisplay( f, "\tD        Display the structures\n");
    inchi_ios_print_nodisplay( f, "\tALT      produce shorter ALTernative
representation (Abc)\n"); inchi_ios_print_nodisplay( f, "\tSCT      produce
shorter connection table representation\n"); inchi_ios_print_nodisplay( f,
"\tXML      output in xml format\n"); inchi_ios_print_nodisplay( f, "\tPLAIN
output in plain format\n"); inchi_ios_print_nodisplay( f, "\tMERGE    Merge all
MOLfiles from the input file into one compound\n"); inchi_ios_print_nodisplay(
f, "\tWnumber  time-out per structure in seconds, W0 means unlimited\n");
    inchi_ios_print_nodisplay( f, "\tFnumber  set display Font size, points\n");
    inchi_ios_print_nodisplay( f, "\tSREL     Relative Stereo\n");
    inchi_ios_print_nodisplay( f, "\tSRAC     Racemic Stereo\n");
    inchi_ios_print_nodisplay( f, "\tNOUUSB   Omit stereobonds if all are
unknown/undefined\n"); inchi_ios_print_nodisplay( f, "\tNOUUSC   Omit
stereocenters if all are unknown/undefined\n"); inchi_ios_print_nodisplay( f,
"\tSS       Slow Stereo: do not use stereo equivalence\n");
    inchi_ios_print_nodisplay( f, "\tRS       Do not test for Redundant Stereo
elements\n"); inchi_ios_print_nodisplay( f, "\tPW       Save warning structures
in the problems file\n"); inchi_ios_print_nodisplay( f, "\tPGO      Save only
all good structures in the problems file\n"); inchi_ios_print_nodisplay( f,
"\tDSB      Double Stereo Bonds only (ignore alternating bonds stereo)\n");
    inchi_ios_print_nodisplay( f, "\tRSB:n    Min Ring Size for detecting for
Stereo Bonds (n=1 => all)\n"); inchi_ios_print_nodisplay( f, "\tAUXINFO:0 do not
output auxiliary information (default:1)\n"); inchi_ios_print_nodisplay( f,
"\tDISCONSALT:0       do not disconnect salts (default:1)\n");
    inchi_ios_print_nodisplay( f, "\tDISCONMETAL:0      do not disconnect metals
(default:1)\n"); inchi_ios_print_nodisplay( f, "\tDISCONMETALCHKVAL:1 do not
disconnect if typical valence (default:0)\n"); inchi_ios_print_nodisplay( f,
"\tRECONMETAL:0       do not reconnect metals (default:1)\n");
    inchi_ios_print_nodisplay( f, "\tMOVEPOS:0          do not check moveable
positive charges (default:1)\n"); inchi_ios_print_nodisplay( f, "\tACIDTAUT:n
n=1: one H/(-) tautomerism, 2: more (deflt), 0:none\n");
    inchi_ios_print_nodisplay( f, "\tMERGESALTTG:1      merge salt t-groups
(default), 0: do not merge\n"); inchi_ios_print_nodisplay( f,
"\tUNCHARGEDACIDS:1   Apply salt (acid) tautomerism in neutral species\n");
    inchi_ios_print_nodisplay( f, "\tO:[suffix]         Open all 4 files adding
suffix to the inputFile name\n"); inchi_ios_print_nodisplay( f, "\tOP:outputpath
Set output path\n"); inchi_ios_print_nodisplay( f, "\tMOL                input
file is a MOLfile (default)\n"); inchi_ios_print_nodisplay( f,
"\tSDF[:DataHeader]   Include SDfile data for the header into the results\n");
    inchi_ios_print_nodisplay( f, "\tSDFID              extract CAS r.n. in
addition to requested SDfile data\n"); inchi_ios_print_nodisplay( f,
"\tSTART:number       Start at the given structure ordering number\n");
    inchi_ios_print_nodisplay( f, "\tEND:number         Terminate after the
given structure ordering number\n");
*/
#endif
}
...
void HelpCommandLineParmsReduced(INCHI_IOSTREAM *f) {
  ...
  inchi_ios_print_nodisplay(
      ...
      INCHI_OPTION_PREFX);
#else
      "%s ver %s%s.\n\nUsage:\nc%s-%s inputFile [outputFile [logFile "
      "[problemFile]]] [%coption[ %coption...]]\n",
      INCHI_NAME, INCHI_VERSION, TARGET_ID_STRING, INCHI_NAME, INCHI_VERSION,
      INCHI_OPTION_PREFX, INCHI_OPTION_PREFX);
#endif
  /*^^^ */
...
/************************************************************************************/
int OpenFiles(FILE **inp_file, FILE **output_file, FILE **log_file,
              FILE **prb_file, INPUT_PARMS *ip) {
  /*
    -- Files --
    ip->path[0] => Input
  ...
  */
  ...
  if (!ip->path[2] || !ip->path[2][0]) {
    fprintf2(stderr, "%s version %s%s%s\n", INCHI_NAME, INCHI_VERSION,
             ...
             bRELEASE_VERSION
                 ? ""
                 : ""); /* (Pre-release, for evaluation purposes only)" ); */
    ...
    *log_file = stderr;
    ...
    fprintf2(stderr, "%s version %s%s%s\n", INCHI_NAME, INCHI_VERSION,
             ...
             bRELEASE_VERSION
                 ...
                 : ""); /* (Pre-release, for evaluation purposes only)" );*/
    fprintf2(stderr,
             "Cannot open log file '%s'. Using standard error output.\n",
             ip->path[2]);
    *log_file = stderr;
  } else {
  ...
  if ((ip->nInputType == INPUT_MOLFILE || ip->nInputType == INPUT_SDFILE ||
    ...
    fmode = "rb";
    if (!ip->path[0] || !ip->path[0][0] ||
        !(*inp_file = fopen(ip->path[0], "rb"))) {
      ...
      if (ip->nInputType == INPUT_CMLFILE) {
        ...
        if (c == '\r' || c == EOF) {
          ...
          if (!(*inp_file = fopen(ip->path[0], "r"))) {
            fprintf2(
                *log_file,
                "Cannot open input file '%s' (2nd attempt). Terminating.\n",
                ip->path[0]);
            goto exit_function;
          }
...
static int bMatchOnePrefix(int len, char *str, int lenPrefix[],
  ...
  for (i = 0; i < numPrefix; i++) {
    if (len >= lenPrefix[i] && !memcmp(str, strPrefix[i], lenPrefix[i])) {
      return 1;
    }
  }
  return 0;
...
int DetectInputINChIFileType(FILE **inp_file, INPUT_PARMS *ip,
  ...
  static char szPlnAuxVer[NUM_VERSIONS]
                         [LEN_VERSIONS]; /* = "AuxInfo:1.1Beta/";*/
  static int lenPlnAuxVer[NUM_VERSIONS];
  static char szXmlVersion[NUM_VERSIONS]
                          [LEN_VERSIONS]; /* = "<INChI version=\"1.1Beta\">";*/
  static int lenXmlVersion[NUM_VERSIONS];
  static char szXmlStruct[LEN_VERSIONS] = "<structure";
  static int lenXmlStruct;
  static char
      szXmlIdentVer[NUM_VERSIONS]
                   [LEN_VERSIONS]; /*= "<identifier version=\"1.1Beta\"";*/
  ...
  if (ip->nInputType == INPUT_INCHI_XML ||
      ip->nInputType == INPUT_INCHI_PLAIN || ip->nInputType == INPUT_INCHI) {
    return 1;
  }
  if (!bInitilized) {
    lenPlnVersion[0] =
        sprintf(szPlnVersion[0], "%s=%s/", INCHI_NAME, INCHI_VERSION);
    ...
    lenXmlVersion[0] = sprintf(szXmlVersion[0], "<%s version=\"%s\">",
                               INCHI_NAME, INCHI_VERSION);
    ...
    lenXmlIdentVer[0] =
        sprintf(szXmlIdentVer[0], "<identifier version=\"%s\"", INCHI_VERSION);
    lenXmlIdentVer[1] =
        sprintf(szXmlIdentVer[1], "<identifier version=\"1.12Beta\"");
    lenXmlIdentVer[2] =
        sprintf(szXmlIdentVer[2], "<identifier version=\"1.0RC\"");
    lenXmlIdentVer[3] =
        sprintf(szXmlIdentVer[3], "<identifier version=\"1.0RC\"");
    ...
    lenXmlMsgError =
        sprintf(szXmlMsgError, "<message type=\"error (no %s)\"", INCHI_NAME);
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (len < 0)
      break;
    if (bMatchOnePrefix(len, szLine, lenPlnVersion, szPlnVersion,
                        NUM_VERSIONS) ||
        bMatchOnePrefix(len, szLine, lenPlnAuxVer, szPlnAuxVer, NUM_VERSIONS)) {
      bINChI_plain++;
    } else if (bMatchOnePrefix(len, szLine, lenXmlVersion, szXmlVersion,
                               NUM_VERSIONS) ||
               bMatchOnePrefix(len, szLine, &lenXmlStruct, &szXmlStruct, 1) ||
               bMatchOnePrefix(len, szLine, lenXmlIdentVer, szXmlIdentVer,
                               NUM_VERSIONS) ||
               bMatchOnePrefix(len, szLine, &lenXmlMsgError, &szXmlMsgError,
                               1) ||
               bMatchOnePrefix(len, szLine, &lenXmlMsgFatal, &szXmlMsgFatal,
                               1)) {
      bINChI_xml++;
    }
  }
  if (bINChI_plain >= 2 && !bINChI_xml) {
    ip->nInputType = INPUT_INCHI_PLAIN;
    ret = 1;
  } else if (!bINChI_plain && bINChI_xml >= 3) {
