...
typedef struct tagStructRestoreMode {
  int bMetalAddFlower; /* 1 => allow adjustable metal valence and charge; 0=>
                          use std charge/valence */
  ...
  int bStereoRemovesMetalFlag; /* 1=> treat stereogenic atoms and atoms
                                  connected by a stereo bond as non-metals */
...
typedef struct tagInpAtomAddParities {
  ...
  /* cml bond parities */
  S_CHAR sb_ord[MAX_NUM_STEREO_BONDS]; /* stereo bond/neighbor ordering number,
                                          starts from 0 */
  /* neighbors on both sides of stereobond have same sign=> trans/T/E, diff.
   * signs => cis/C/Z */
  S_CHAR sn_ord[MAX_NUM_STEREO_BONDS]; /* ord. num. of the neighbor adjacent to
                                          the SB; starts from 0; -1 means
                                          removed explicit H */
  /* neighbors on both sides of stereobond have same parity => trans/T/E/2,
   * diff. parities => cis/C/Z/1 */
  S_CHAR sb_parity[MAX_NUM_STEREO_BONDS];
  AT_NUMB sn_orig_at_num[MAX_NUM_STEREO_BONDS]; /* orig. at number of sn_ord[]
                                                   neighbors */
} inp_ATOM_STEREO;

#define FIX_STEREO_BOND_ORDER                                                  \
  0 /* 1=> fix stereobonds; treat metal as non-metal if it is stereogenic or   \
       has a stereobond */
...
/* first  bonds valence for charge = c is cValence[0][c+VAL_BASE];
 * VAL_MIN_CHARGE <= c <= VAL_MAX_CHARGE */
/* second bonds valence for charge = c is cValence[1][c+VAL_BASE];
 * VAL_MIN_CHARGE <= c <= VAL_MAX_CHARGE */
/* total number of valences is 2 = VAL_NUMBER */
/* neutral bond valence orders are cValence[0][VAL_NEUTR_ORDER],
 * cValence[1][VAL_NEUTR_ORDER] */
#define VAL_BASE (1)
...
typedef struct tagAtomIonPrperies {
  /* char cValence[VAL_NUMBER][VAL_LENGTH]; */ /* ordering numbers of minimal
                                                  valence, 0-based */
  ...
  char
      cInitValenceToMetal; /* sum of init adjusted bond orders to metal atoms */
  ...
  char cInitFreeValences; /* number of 'dots' to connect; charges are marked
                             separately */
  S_CHAR cInitCharge;     /* initial charge on the atom (not included in
                             ChargeStruct */
...
typedef enum tagTgRestoreFlags { TGRF_MINUS_FIRST = 1 } TGRF;
typedef struct tagTCGroup {
  ...
  short
      tg_RestoreFlags; /* Set (-) to first memberst of a t-group (usually, N) */
} TC_GROUP;
...
typedef struct tagAllTCGroups {
  ...
  int nAddIedges;  /* additional increase of number of iedges for edge switching
                      to another group */
  ...
  /* excess_charge = total_charge - added_charge - tgroup_charge: add to metals
   * etc. */

  int nEdge4charge; /* edge used to add charges; neighbor1=supercharge, another
                       = (+/-) vertex */
...
typedef struct tagOneComponentRemovedAndExchangeableH {
  ...
  NUM_H nNumRemovedIsotopicH
      [NUM_H_ISOTOPES]; /* isotopic H that may be exchanged and considered
...
typedef struct tagRemovedAndExchangeableH {
  ...
  NUM_H nNumRemovedIsotopicH
      [NUM_H_ISOTOPES]; /* isotopic H that may be exchanged and considered
                           randomly distributed, including removed protons */
  /* for individual components from comparing Fixed-H vs Mobile-H formulas; NULL
   * if not available */
...
typedef struct tagInputInChI {
  ...
  int s[INCHI_NUM][TAUT_NUM]
       [2]; /* s[0=non-iso, 1=iso] = 0,1,2,3 <= regular /s; -1=> "/s" (empty) */
  long num_inp;
...
typedef struct tagStructFromInChI {
  ...
  inp_ATOM *at; /* length = num_atoms + num_deleted_H, zero pint struct for
                   BNS->struct conversion */
  inp_ATOM_STEREO
      *st;       /* additional stereo that exists only in Mobile-H layer */
  inp_ATOM *at2; /* length = num_atoms + num_deleted_H, the conversion result */

  /* information from InChI only */
  T_GROUP_INFO ti;   /* from original InChI[0] if Mobile-H from the beginning or
                        later from InChI[1] if Fixed-H  */
  ...
  int nNumRemovedProtonsMobHInChI; /* number of protons removed from Mobile-H
                                      struct in original InChI */
  S_CHAR charge;
  char bIsotopic;
  ...
  /* InChI -> Struct component -> Full InChI result (both disconnected and
   * connected if exist) */
  REV_INCHI RevInChI;
  int nRemovedProtonsByNormFromRevrs; /* number of H(+) removed by normalization
  ...
  AT_NUMB *nAtno2Canon[TAUT_NUM]; /* nAtno2Canon[restored_at_no][*] = (atom
                                     canon number in restored struct)-1*/
  AT_NUMB *nCanon2Atno[TAUT_NUM]; /* nCanon2Atno[(atom canon number in restored
                                     struct)-1][*] = restored_at_no; */

  int nError;
  /* other parms */
  char iInchiRec; /* index in the original InChI array */
  char iMobileH;  /* index in the original InChI array */
  char bDeleted;  /* InChI component marked as Deleted, means a proton in
                     Mobile-H layer */
  ...
  int nChargeRevrs; /* component charge of the reconstructed structure, TAUT_YES
                       layer */
...
#define EL_TYPE_OSt                                                            \
  0x0100 /* terminal -OH, -O(-), -SH, -S(-), ... from fix_special_bonds(...)   \
          */
#define EL_TYPE_PT 0x0200 /* may be a tautomeric endpoint */

/* the atom to which the node is attached has number 1; added atoms have numbers
 * 2,3,... */
#define MAX_CN_VAL 3
...
#define MAKE_CN_BITS(A, B, C, D)                                               \
  ((((((D) << cn_bits_shift | (C)) << cn_bits_shift) | (B))                    \
    << cn_bits_shift) |                                                        \
   (A))
...
typedef struct tagStructCmpTwoFixedH {
  ...
  short nNumEndpInChI;
  short nNumEndpRevrs;
  short nNumTgDiffMinus; /* number of would-be-identical t-groups that have
                            different number of (-) */
  short nNumTgDiffH;     /* number of would-be-identical t-groups that have
                            different number of H */
...
typedef struct tagAtomsCmpTwoMobileH {
  ...
  S_CHAR nMobHRevrs; /* number of H on the atom in InChI from the reconstructed
                        structure */
  S_CHAR nNumHRevrs; /* number of H on the atom in the being reconstructed
                        structure */
  S_CHAR nAtChargeRevrs;
  S_CHAR nValue; /* flag(s) */
} CMP2MHATOMS;
...
typedef struct tagStructCmpTwoMobileH {
  ...
  short nNumTgMRevrs; /* number of (-) in reversed structure t-groups */
  short nNumTgHRevrs; /* number of H in reversed structure t-groups */
  short nNumTgORevrs; /* number of tautomeric O,S,Se in reversed structure
                         t-groups */
  short
      nNumTgNRevrs; /* number of tautomeric N in reversed structure t-groups */

  short nNumTgOMinusRevrs;   /* number of -O(-)  on endpoints found in restored
                                structure */
  short nNumTgOHRevrs;       /* number of -OH    on endpoints found in restored
                                structure */
  short nNumTgDBORevrs;      /* number of =O     on endpoints found in restored
                                structure */
  short nNumTgNMinusRevrs;   /* number of -N(-)- on endpoints found in restored
                                structure */
  short nNumTgNHMinusRevrs;  /* number of -NH(-) on endpoints found in restored
                                structure */
  short nNumTgNHRevrs;       /* number of -NH-   on endpoints found in restored
                                structure */
  short nNumTgNH2Revrs;      /* number of -NH2   on endpoints found in restored
                                structure */
  short nNumTgDBNHRevrs;     /* number of =NH    on endpoints found in restored
                                structure */
  short nNumTgDBNMinusRevrs; /* number of =N(-)  on endpoints found in restored
                                structure */
  short nNumTgDBNRevrs;      /* number of =N-    on endpoints found in restored
                                structure */

  short nNumTgOMinusInChI; /* number of -O(-)  on endpoints according to
                              original InChI */
  short nNumTgOHInChI;  /* number of -OH    on endpoints according to original
                           InChI */
  short nNumTgDBOInChI; /* number of =O     on endpoints according to original
                           InChI */
  short nNumTgNMinusInChI;  /* number of -N(-)- on endpoints according to
                               original InChI */
  short nNumTgNHMinusInChI; /* number of -NH(-) on endpoints according to
                               original InChI */
  short nNumTgNHInChI;   /* number of -NH-   on endpoints according to original
                            InChI */
  short nNumTgNH2InChI;  /* number of -NH2   on endpoints according to original
                            InChI */
  short nNumTgDBNHInChI; /* number of =NH    on endpoints according to original
                            InChI */
  short nNumTgDBNMinusInChI; /* number of =N(-)  on endpoints according to
                                original InChI */
  short nNumTgDBNInChI; /* number of =N-    on endpoints according to original
                           InChI */

  S_CHAR nChargeMobHInChI;
  S_CHAR nChargeMobHRevrs;
  S_CHAR nChargeMobHRevrsNonMetal; /* charge does not include charges on metals;
                                      later add ion pairs rejection */
  char bFixedHLayerExistsRevrs;
  char bHasDifference;
  U_CHAR nNumDiffMobH;
} CMP2MHINCHI;
...
int OneInChI2Atom(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
...
int AddEdgeFlow(int edge_cap, int edge_flow, BNS_EDGE *e01,
                BNS_VERTEX *pv0 /*src*/, BNS_VERTEX *pv1 /*dest*/,
...
/*
int GetAtomBondFlow( inp_ATOM *atom, VAL_AT *pVA, ICHICONST SRM *pSrm, int iat,
int ineigh ); void GetAtomStCapFlow( inp_ATOM *atom, VAL_AT *pVA, ICHICONST SRM
*pSrm, int iat, int *pCap, int *pFlow ); int GetAtomToMCGroupInitEdgeCapFlow(
EdgeFlow *nEdgeCap, EdgeFlow *nEdgeFlow, ICHICONST SRM *pSrm, inp_ATOM *at,
VAL_AT *pVA, int iat ); void GetAtomToMetalInitEdgeCapFlow( EdgeFlow *nEdgeCap,
EdgeFlow *nEdgeFlow );
*/
...
/* create one InChI */
int MakeOneInChIOutOfStrFromINChI2(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
...
int GetPlusMinusVertex(BN_STRUCT *pBNS, ALL_TC_GROUPS *pTCGroups,
                       int bCheckForbiddenPlus, int bCheckForbiddenMinus);
int FixMobileHRestoredStructure(
    ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, BN_STRUCT *pBNS, BN_DATA *pBD,
    StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2, inp_ATOM *at3,
...
int FixRestoredStructureStereo(
    INCHI_MODE cmpInChI, ICR *icr, INCHI_MODE cmpInChI2, ICR *icr2,
    ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, BN_STRUCT *pBNS, BN_DATA *pBD,
    StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2, inp_ATOM *at3,
    VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups, T_GROUP_INFO **ppt_group_info,
    inp_ATOM **ppat_norm, inp_ATOM **ppat_prep, INChI *pInChI[], long num_inp,
...
int AddRemProtonsInRestrStruct(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
...
int AllInchiToStructure(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
...
void FreeStrFromINChI(StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
                      int nNumComponents[INCHI_NUM][TAUT_NUM]);
int OldPrintCompareOneOrigInchiToRevInChI(StrFromINChI *pStruct,
                                          INChI *pInChI[TAUT_NUM], int bMobileH,
                                          int iComponent, long num_inp,
                                          char *szCurHdr);
...
int AddRemIsoProtonsInRestrStruct(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
                                  long num_inp, int bHasSomeFixedH,
                                  StrFromINChI *pStruct, int num_components,
                                  StrFromINChI *pStructR, int num_componentsR,
                                  NUM_H pProtonBalance[],
...
int DetectInpInchiCreationOptions(InpInChI *pOneInput, int *bHasReconnected,
                                  int *bHasMetal, int *bHasFixedH, int *sFlag,
                                  int *bTautFlag);
...
int bIsUnsatCarbonInASmallRing(inp_ATOM *at, VAL_AT *pVA, int iat, BFS_Q *pbfsq,
                               int min_ring_size);
