...
/*******************************************************
  ...
  ---  Each entry has AT_TAUTOMER type members ---
    <fixed portion (6 entries)>
          ...
          number of mobile groups, including number of negative charges
(=num(H)+num(-)) number of negative charges number of 1H atoms number of 2H
(deuterium) atoms number of 3H (tritium) atoms <variable portion, sorted in
ascending order> atom rank #1 (ascending order)
          ...
          atom rank #endpoints
  ...
  --------- Linear CT Tautomer description -----
    <for each tautomeric group>
        ...
        ...
        number of 1H               =
t_group->num[T_NUM_NO_ISOTOPIC+T_NUM_ISOTOPIC-1]
        -- variable length part --
        rank of the first endpoint =
nRank[t_group_info->nEndpointAtomNumber[t_group->nFirstEndpointAtNoPos]];
        ...
        rank of the last endpoint  =
nRank[t_group_info->nEndpointAtomNumber[t_group->nFirstEndpointAtNoPos+t_group->nNumEndpoints-1]];
  ...
  --------- Linear CT Isotopic Tautomer description -----
    <for each isotopic tautomeric group>
        ...
        ...
        number of 1H               =
t_group->num[T_NUM_NO_ISOTOPIC+T_NUM_ISOTOPIC-1] t-group ordering number in the
Linear CT Tautomer, starts from 1
...
#define T_GROUP_HDR_LEN                                                        \
  (1 + T_NUM_NO_ISOTOPIC /*+T_NUM_ISOTOPIC*/) /* LinearCTTautomer */

typedef AT_NUMB AT_TAUTOMER; /* LinearCTTautomer */

typedef AT_ISO_SORT_KEY
    T_GROUP_ISOWT; /* must hold value up to T_GROUP_ISOWT_MULT^3-1 */
                   /* similar to AT_ISO_SORT_KEY */
/* = num_1H + T_GROUP_ISOWT_MULT*(num_D + T_GROUP_ISOWT_MULT*num_T) */

#define T_GROUP_ISOWT_MULT                                                     \
  1024 /* (max. number of identical isotopic hydrogens in a taut. group) + 1   \
        */
...
typedef enum tagTG_NumDA {            /* 2004-02-26 */
                           TG_Num_dH, /* number of H donors that have only H
                                         (all single bonds) */
                           TG_Num_dM, /* number of H donors that have (-)  (all
                                         single bonds) */
                           TG_Num_aH, /* number of H acceptors that have H and
                                         no (-) (+a double bond) */
                           TG_Num_aM, /* number of H acceptors that have (-) and
                                         possibly H (+ one double bond) */
...
typedef struct tagTautomerGroup {
  ...
  union {
      struct {
          ...
          AT_RANK num_T;      // here the isotopic part (num+T_NUM_NO_ISOTOPIC)
  starts AT_RANK num_D; AT_RANK num_1H;
      };
      AT_RANK num[T_NUM_NO_ISOTOPIC+T_NUM_ISOTOPIC];  // same size and meaning
  as num[] in T_ENDPOINT
  };
  */
  AT_RANK num[T_NUM_NO_ISOTOPIC + T_NUM_ISOTOPIC]; /* same size and meaning as
                                                      num[] in T_ENDPOINT */
  /* isotopic inv. order: num_T, num_D, num_1H */
  AT_RANK num_DA[TG_NUM_DA];
  T_GROUP_ISOWT iWeight; /* isotopic "weight" =
                            T_GROUP_ISOWT_MULT*(T_GROUP_ISOWT_MULT*num_T +
                            num_D)+num_1H; */
  AT_NUMB nGroupNumber;  /* positive tautomer group ID = atom->endpoint */
  AT_NUMB nNumEndpoints; /* number of the atom numbers in
                            T_GROUP_INFO::nEndpointAtomNumber[] */
  AT_NUMB nFirstEndpointAtNoPos; /* the first index of the atom number in
                                    T_GROUP_INFO::nEndpointAtomNumber[] */
} T_GROUP;

/* offsets/num_t_groups within T_GROUP_INFO::tGroupNumber  */
#define TGSO_CURR_ORDER 0 /* tGroupNumber:     current sorting order */
#define TGSO_SYMM_RANK                                                         \
  1 /* tSymmRank:     symmetry ranks (no isotopes) = min. ordering number > 0. \
     */
#define TGSO_SYMM_IORDER                                                       \
  2 /* tiGroupNumber: isotopic symmetry rank sorting order */
...
#define FLAG_NORM_CONSIDER_TAUT                                                \
  (FLAG_PROTON_NPO_SIMPLE_REMOVED | FLAG_PROTON_NP_HARD_REMOVED |              \
   FLAG_PROTON_AC_SIMPLE_ADDED | FLAG_PROTON_AC_SIMPLE_REMOVED |               \
   FLAG_PROTON_AC_HARD_REMOVED | FLAG_PROTON_AC_HARD_ADDED |                   \
   FLAG_PROTON_SINGLE_REMOVED | FLAG_PROTON_CHARGE_CANCEL)

#if (FIX_N_MINUS_NORN_BUG == 1)
#define FLAG_FORCE_SALT_TAUT                                                   \
  (FLAG_PROTON_NP_HARD_REMOVED | FLAG_PROTON_AC_HARD_REMOVED |                 \
   FLAG_PROTON_AC_HARD_ADDED | FLAG_PROTON_CHARGE_CANCEL)
#else
...
typedef struct tagTautomerGroupsInfo {
  ...
  AT_NUMB *nEndpointAtomNumber; /* nNumEndpoints elements; also see comments to
                                   T_GROUP */
  ...
  AT_NUMB *nIsotopicEndpointAtomNumber; /* [0]: number of the following atoms;
                                           [1...]: non-tautomeric atoms that may
                                           have isotopic H */
  int nNumIsotopicEndpoints; /* allocated length of nIsotopicEndpointAtomNumber
                              */
  NUM_H num_iso_H[NUM_H_ISOTOPES]; /* isotopic H on tautomeric atoms and those
...
#define CANON_FLAG_NO_H_RECANON                                                \
  0x0001 /* iOther: second canonicalization of the no H structure */
#define CANON_FLAG_NO_TAUT_H_DIFF                                              \
  0x0002 /* iOther: NoTautH eq. partition differs from NoH */
#define CANON_FLAG_ISO_ONLY_NON_TAUT_DIFF                                      \
  0x0004 /* iOther: eq. partition in isotopic only non-taut differs from       \
            non-isotopic */
#define CANON_FLAG_ISO_TAUT_DIFF                                               \
  0x0008 /* iBase:  isotopic eq. partition in isotopic taut differs from       \
            non-isotopic taut */
#define CANON_FLAG_ISO_FIXED_H_DIFF                                            \
  0x0010 /* iOther: isotopic eq. partition in fixed H non-taut differs from    \
...
typedef struct tagTautomerEndpoint {
  ...
  */
  AT_RANK num[T_NUM_NO_ISOTOPIC +
              T_NUM_ISOTOPIC]; /* same size and meaning as num[] in T_GROUP */
  AT_RANK num_DA[TG_NUM_DA];
...
typedef struct tagChargeGroup {
  AT_RANK num[2]; /* [0]: number of (+), [1]: number atoms that have H,
                     including H accessible through tautomerism */
  AT_RANK num_CPoints;
  AT_NUMB nGroupNumber;
  U_CHAR cGroupType;
} C_GROUP;
...
typedef struct tagSaltGroupInfo {
  ...
  int num_p_only_candidates; /* num. non-tautomeric p-donor/acceptor candidates
                                like -CH2-SH */
} S_GROUP_INFO;
...
typedef struct tagAtomSizes {
  ...
  int nLenIsotopic;
  int nLenCTAtOnly;
  int nLenLinearCTStereoDble; /* max. number of stereo bonds in non-isotopic
                                 case */
  int nLenLinearCTStereoCarb; /* max. number of stereo atoms in non-isotopic
                                 case */
...
int nGet15TautIn6MembAltRing(inp_ATOM *atom, int nStartAtom,
                             AT_RANK *nBfsTreePos, DFS_PATH *DfsPath,
                             int nMaxLenBfsTree, T_ENDPOINT *EndPoint,
...
int nGet12TautIn5MembAltRing(inp_ATOM *atom, int nStartAtom,
                             int nStartAtomNeighbor, AT_RANK *nBfsTreePos,
