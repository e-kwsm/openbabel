...
/* Note: (INCHI_LINE_LEN - INCHI_LINE_ADD) > (length of the longest item:
 * szCoord) = 33 */
/*****************************************************************************/

#if (defined(TARGET_API_LIB) || defined(TARGET_EXE_USING_API))

#define AB_MAX_WELL_DEFINED_PARITY                                             \
  inchi_max(INCHI_PARITY_ODD,                                                  \
            INCHI_PARITY_EVEN) /* 1, 2 => well defined parities, uncluding     \
                                  'unknown' */
#define AB_MIN_WELL_DEFINED_PARITY                                             \
  inchi_min(INCHI_PARITY_ODD,                                                  \
            INCHI_PARITY_EVEN) /* min(INCHI_PARITY_ODD, INCHI_PARITY_EVEN) */
#define ATOM_PARITY_WELL_DEF(X)                                                \
  (AB_MIN_WELL_DEFINED_PARITY <= (X) && (X) <= AB_MAX_WELL_DEFINED_PARITY)

#define inchi_NUMH2(AT, CUR_AT)                                                \
  ((AT[CUR_AT].num_iso_H[0] > 0 ? AT[CUR_AT].num_iso_H[0] : 0) +               \
   AT[CUR_AT].num_iso_H[1] + AT[CUR_AT].num_iso_H[2] +                         \
   AT[CUR_AT].num_iso_H[3])
...
int is_element_a_metal(char szEl[]) {
  ...
  if (0 < len && len <= 2 && isalpha(UCINT szEl[0]) && isupper(szEl[0]) &&
...
/*****************************************************************************/
/* local prototypes */
char *FindToken(INCHI_IOSTREAM *inp_molfile, int *bTooLongLine,
                const char *sToken, int lToken, char *szLine, int nLenLine,
                char *p, int *res);
char *LoadLine(INPUT_FILE *inp_molfile, int *bTooLongLine, int *bItemIsOver,
               char **s, char *szLine, int nLenLine, int nMinLen2Load, char *p,
               int *res);

int INChITo_Atom(INPUT_FILE *inp_molfile, MOL_COORD **szCoord,
                 inchi_Stereo0D **stereo0D, int *num_stereo0D, int bDoNotAddH,
                 int vABParityUnknown, INPUT_TYPE nInputType, inchi_Atom **at,
                 int max_num_at, int *num_dimensions, int *num_bonds,
                 char *pSdfLabel, char *pSdfValue, long *Id,
                 INCHI_MODE *pInpAtomFlags, int *err, char *pStrErr);

#if (defined(TARGET_API_LIB) || defined(TARGET_EXE_USING_API))
/*****************************************************************************/
int INChIToInchi_Atom(INCHI_IOSTREAM *inp_molfile, inchi_Stereo0D **stereo0D,
                      int *num_stereo0D, int bDoNotAddH, int vABParityUnknown,
                      INPUT_TYPE nInputType, inchi_Atom **at, int max_num_at,
                      int *num_dimensions, int *num_bonds, char *pSdfLabel,
                      char *pSdfValue, long *Id, INCHI_MODE *pInpAtomFlags,
...
int INChIToInchi_Atom(INCHI_IOSTREAM *inp_molfile, inchi_Stereo0D **stereo0D,
                      ...
                      char *pSdfValue, long *Id, INCHI_MODE *pInpAtomFlags,
                      int *err, char *pStrErr) {
  return INChITo_Atom(inp_molfile, NULL, stereo0D, num_stereo0D, bDoNotAddH,
                      vABParityUnknown, nInputType, at, max_num_at,
                      num_dimensions, num_bonds, pSdfLabel, pSdfValue, Id,
...
int INChIToInpAtom(INCHI_IOSTREAM *inp_molfile, MOL_COORD **szCoord,
                   ...
                   inp_ATOM **at, int max_num_at, int *num_dimensions,
                   int *num_bonds, char *pSdfLabel, char *pSdfValue, long *Id,
                   INCHI_MODE *pInpAtomFlags, int *err, char *pStrErr);
int INChIToInpAtom(INCHI_IOSTREAM *inp_molfile, MOL_COORD **szCoord,
                   int bDoNotAddH, int vABParityUnknown, INPUT_TYPE nInputType,
                   inp_ATOM **at, int max_num_at, int *num_dimensions,
                   int *num_bonds, char *pSdfLabel, char *pSdfValue, long *Id,
                   INCHI_MODE *pInpAtomFlags, int *err, char *pStrErr) {
  return INChITo_Atom(inp_molfile, szCoord, NULL, NULL, bDoNotAddH,
                      vABParityUnknown, nInputType, at, max_num_at,
                      num_dimensions, num_bonds, pSdfLabel, pSdfValue, Id,
                      pInpAtomFlags, err, pStrErr);
}
...
/*****************************************************************************/
char *LoadLine(INCHI_IOSTREAM *inp_molfile, int *bTooLongLine, int *bItemIsOver,
               char **s, char *szLine, int nLenLine, int nMinLen2Load, char *p,
               int *res) {
  ...
  if (!*bItemIsOver && nLenLine - (*res - pos) > nMinLen2Load) {
    ...
    res2 = inchi_ios_getsTab1(szLine + *res, nLenLine - *res - 1, inp_molfile,
                              bTooLongLine);
    if (res2 > 0) {
...
int INChITo_Atom(INCHI_IOSTREAM *inp_molfile, MOL_COORD **szCoord,
                 ...
                 char *pSdfLabel, char *pSdfValue, long *Id,
                 INCHI_MODE *pInpAtomFlags, int *err, char *pStrErr) {
  int num_atoms = 0, bFindNext = 0, len, bHeaderRead, bItemIsOver, bErrorMsg,
      bRestoreInfo;
  int bFatal = 0, num_struct = 0;
  int i, k, k2, res, bond_type, bond_stereo1, bond_stereo2, bond_char, neigh,
      bond_parity, bond_parityNM;
  ...
  INCHI_MODE InpAtomFlags =
      0; /* 0 or FLAG_INP_AT_NONCHIRAL or FLAG_INP_AT_CHIRAL */
  ...
  *num_dimensions = *num_bonds = 0;
  ...
  if (nInputType == INPUT_INCHI_PLAIN) {
    bHeaderRead = hk = 0;
    while (0 < (res = inchi_ios_getsTab(szLine, sizeof(szLine) - 1, inp_molfile,
                                        &bTooLongLine))) {

      /********************* find and interpret structure header ************/
      if (!bTooLongLine && (hlen = sizeof(sStructHdrPln) - 1,
                            !memcmp(szLine, sStructHdrPln, hlen))) {
        p = szLine + hlen;
        longID = 0;
        num_atoms = 0;
        /* structure number */
      ...
      } else if (!memcmp(szLine, sStructHdrPlnAuxStart,
                         lenStructHdrPlnAuxStart)) {
        ...
        /* search for sToken in the line; load next segments of the line if
         * sToken has not found */
        p = FindToken(inp_molfile, &bTooLongLine, sToken, lToken, szLine,
                      sizeof(szLine), p, &res);
        if (!p) {
          *err = INCHI_INP_ERROR_ERR;
          ...
          while (1) {
            p = LoadLine(inp_molfile, &bTooLongLine, &bItemIsOver, &s, szLine,
                         sizeof(szLine), INCHI_LINE_ADD, p, &res);
            if (!i) {
              /* allocate atom */
              ...
              if (!stereo0D || !*stereo0D) {
                ...
                if (!atom_stereo0D) {
                  num_atoms =
                      INCHI_INP_FATAL_RET; /* fatal error: cannot allocate */
                  *err = INCHI_INP_FATAL_ERR;
                  MOLFILE_ERR_SET(*err, 0, "Out of RAM");
                  goto bypass_end_of_INChI_plain;
                }
            ...
            if (isdigit(UCINT * p)) {
              ...
              if (!AT_BONDS_VAL(atom, i))
                AT_BONDS_VAL(atom, i) =
                    ISOLATED_ATOM; /* same convention as in MOLfile, found zero
                                      bonds valence */
              p = q;
            }
            /* charge */
          ...
          *err = INCHI_INP_ERROR_ERR;
          ...
          while (i < num_atoms) {
            ...
            case 'w':
              ...
              bond_stereo1 = bond_stereo2 = INCHI_BOND_STEREO_DOUBLE_EITHER;
              break;
            case 's':
              bond_type = INCHI_BOND_TYPE_SINGLE;
              break;
            ...
            if (bond_parity) {
              if (max_len_stereo0D <= len_stereo0D) {
                ...
                inchi_Stereo0D *new_atom_stereo0D =
                    CreateInchi_Stereo0D(max_len_stereo0D + num_atoms);
                ...
                memcpy(new_atom_stereo0D, atom_stereo0D,
                       len_stereo0D * sizeof(*atom_stereo0D));
                FreeInchi_Stereo0D(&atom_stereo0D);
                atom_stereo0D = new_atom_stereo0D;
                max_len_stereo0D += num_atoms;
              }
              /* (a) i may be allene endpoint and     neigh = allene middle
                 point or (b) i may be allene middle point and neigh = allene
                 endpoint
              ...
              */
              ...
              atom_stereo0D[len_stereo0D].type =
                  INCHI_StereoType_DoubleBond; /* incl allenes & cumulenes */
              len_stereo0D++;
            }
          }
          if (!bItemIsOver || i != num_atoms || (s && p != s)) {
        ...
        if (!p) {
          ...
          *err = INCHI_INP_ERROR_ERR;
          ...
          if ((pszCoord = (MOL_COORD *)inchi_malloc(inchi_max(num_atoms, 1) *
                                                    sizeof(MOL_COORD)))) {
          ...
          while (i < num_atoms) {
            ...
            if (*p == ';') {
              ...
              num_atoms =
                  INCHI_INP_ERROR_RET; /* error in input data: atoms, bonds &
                                          coord must be present together */
              *err = INCHI_INP_ERROR_ERR;
            ...
            *at = atom;
            ...
            for (bNonMetal = 0; bNonMetal < 1; bNonMetal++) {
              for (a1 = 0; a1 < num_atoms; a1++) {
                int num_bond_type[MAX_INPUT_BOND_TYPE - MIN_INPUT_BOND_TYPE +
                                  1];
#if (defined(TARGET_API_LIB) || defined(TARGET_EXE_USING_API))
#else
                ...
                valence = AT_BONDS_VAL(
                    atom, a1); /*  save atom valence if available */
                ...
                for (n1 = 0; n1 < AT_NUM_BONDS(atom[a1]); n1++) {
                  ...
                  if (bond_type < 0 ||
                      bond_type > MAX_INPUT_BOND_TYPE - MIN_INPUT_BOND_TYPE) {
                    bond_type = 0;
                    MOLFILE_ERR_SET(*err, 0,
                                    "Unknown bond type in InChI aux assigned "
                                    "as a single bond");
                  }
                ...
                if (MIN_INPUT_BOND_TYPE <= INCHI_BOND_TYPE_ALTERN &&
                  ...
                  case 3:
                    ...
                    break;
                  default:
                    /*  if 1 or >= 4 aromatic bonds then replace such bonds with
                     * single bonds */
                    for (n1 = 0; n1 < AT_NUM_BONDS(atom[a1]); n1++) {
                      if (atom[a1].bond_type[n1] == INCHI_BOND_TYPE_ALTERN) {
                        ...
                        if (p1) {
                          atom[a1].bond_type[n1] =
                              atom[a2].bond_type[p1 - atom[a2].neighbor] =
                                  INCHI_BOND_TYPE_SINGLE;
                        } else {
                          *err = -2; /*  Program error */
                          MOLFILE_ERR_SET(
                              *err, 0, "Program error interpreting InChI aux");
                          num_atoms = 0;
                    ...
                    *err |= 32; /*  Unrecognized aromatic bond(s) replaced with
                                   single */
                    MOLFILE_ERR_SET(*err, 0,
                ...
                {
                  int num_iso_H;
                  num_iso_H = atom[a1].num_iso_H[1] + atom[a1].num_iso_H[2] +
                              atom[a1].num_iso_H[3];
...
#else
                /* added 2006-07-19 to process aromatic bonds same way as from
                 * molfile */
                if (n2 && !valence) {
                  int num_H = NUMH(atom, a1); /* only isotopic */
                  int chem_valence = chem_bonds_valence;
                  int bUnusualValenceArom = detect_unusual_el_valence(
                      (int)atom[a1].el_number, atom[a1].charge,
                      atom[a1].radical, chem_valence, num_H, atom[a1].valence);
                  int bUnusualValenceNoArom = detect_unusual_el_valence(
                      (int)atom[a1].el_number, atom[a1].charge,
                      atom[a1].radical, chem_valence - 1, num_H,
                      atom[a1].valence);
#if (CHECK_AROMBOND2ALT == 1)
                  if (bUnusualValenceArom && !bUnusualValenceNoArom &&
                      0 == nBondsValToMetal(atom, a1))
#else
                  if (bUnusualValenceArom && !bUnusualValenceNoArom)
#endif
                  {
                ...
                atom[a1].num_H =
                    get_num_H(atom[a1].elname, atom[a1].num_H,
                              atom[a1].num_iso_H, atom[a1].charge,
                              atom[a1].radical, atom[a1].chem_bonds_valence,
                              valence, 0, bDoNotAddH, bHasMetalNeighbor);
#endif
              }
            }
            nNumBonds /= 2;
            ...
            for (i = 0; i < len_stereo0D; i++) {
              ATOM_NUMBER *p1, *p2;
              int sb_ord_from_a1 = -1, sb_ord_from_a2 = -1, bEnd1 = 0,
                  bEnd2 = 0;
              switch (atom_stereo0D[i].type) {

              case INCHI_StereoType_Tetrahedral:
                a1 = atom_stereo0D[i].central_atom;
                if (atom_stereo0D[i].parity && (AT_NUM_BONDS(atom[a1]) == 3 ||
                                                AT_NUM_BONDS(atom[a1]) == 4)) {
                  int ii, kk = 0;
...
#define MAX_CHAIN_LEN 20
                ...
                p1 = IN_NEIGH_LIST(atom[a1].neighbor, (ATOM_NUMBER)a2,
                                   AT_NUM_BONDS(atom[a1]));
                p2 = IN_NEIGH_LIST(atom[a2].neighbor, (ATOM_NUMBER)a1,
                                   AT_NUM_BONDS(atom[a2]));
                if (!p1 || !p2) {
                  atom_stereo0D[i].type = INCHI_StereoType_None;
                  atom_stereo0D[i].central_atom = NO_ATOM;
                  atom_stereo0D[i].neighbor[0] = atom_stereo0D[i].neighbor[3] =
                      -1;
                ...
                if (AT_NUM_BONDS(atom[a1]) == 2 &&
                    atom[a1].bond_type[0] + atom[a1].bond_type[1] ==
                        2 * INCHI_BOND_TYPE_DOUBLE &&
                    0 == inchi_NUMH2(atom, a1) &&
                    (AT_NUM_BONDS(atom[a2]) != 2 ||
                     atom[a2].bond_type[0] + atom[a2].bond_type[1] !=
                         2 * INCHI_BOND_TYPE_DOUBLE)) {
                ...
                if (bEnd2 + bEnd1 == 1) {
                  ...
                  chain[len++] = next;
                  while (len <
                         MAX_CHAIN_LEN) { /* arbitrary very high upper limit to
                  ...
                  if (len > 2 &&
                    ...
                    a2 = cur;
                    /* by design we need to pick up the first
                     * non-stereo-bond-neighbor as "sn"-atom */
                    atom_stereo0D[i].neighbor[0] =
                        atom[a1].neighbor[sb_ord_from_a1 == 0];
                    atom_stereo0D[i].neighbor[1] = a1;
                    atom_stereo0D[i].neighbor[2] = a2;
                    atom_stereo0D[i].neighbor[3] =
                        atom[a2].neighbor[sb_ord_from_a2 == 0];
                    if (len % 2) {
...
#undef MAX_CHAIN_LEN
                  ...
                  /****** a normal possibly stereogenic bond -- not an allene or
                   * cumulene *******/
                  /* by design we need to pick up the first
                   * non-stereo-bond-neighbor as "sn"-atom */
                  sb_ord_from_a1 = p1 - atom[a1].neighbor;
  ...
  bypass_end_of_INChI_plain:
    ...
    while (bTooLongLine && 0 < inchi_ios_getsTab1(szLine, sizeof(szLine) - 1,
                                                  inp_molfile, &bTooLongLine)) {
      ;
    }
#if (defined(TARGET_API_LIB) || defined(TARGET_EXE_USING_API))
  ...
  if (nInputType == INPUT_INCHI_XML) {
    ...
    static const char sStructMsgXmlErr[] =
        "<message type=\"error (no InChI)\" value=\"";
    static const char sStructMsgXmlErrFatal[] =
        "<message type=\"fatal (aborted)\" value=\"";
    ...
    while (0 < (res = inchi_ios_gets(szLine, sizeof(szLine) - 1, inp_molfile,
                                     &bTooLongLine))) {
      ...
      if (!memcmp(szLine, sStructHdrXml, sizeof(sStructHdrXml) - 1)) {
        ...
        bErrorMsg = bRestoreInfo = 0;
                  ...
                  (bFatal = 0, len = sizeof(sStructMsgXmlErr) - 1,
                   !memcmp(szLine, sStructMsgXmlErr, len))) ||
                 (bHeaderRead &&
                  (len = sizeof(sStructMsgXmlErrFatal) - 1,
                   !memcmp(szLine, sStructMsgXmlErrFatal, len)) &&
                  (bFatal = 1))) {
        p = szLine + len;
        q = strchr(p, '\"');
        if (q && !bFindNext) {
        ...
        *err = bFatal ? INCHI_INP_FATAL_ERR : INCHI_INP_ERROR_ERR;
        ...
        goto bypass_end_of_INChI;
                 ...
                 !memcmp(szLine, sStructAuxXml, sizeof(sStructAuxXml) - 1)) {
        bInTheAuxInfo = 1;
      } else if (bHeaderRead && !memcmp(szLine, sStructAuxXmlEnd,
                                        sizeof(sStructAuxXmlEnd) - 1)) {
        *err = INCHI_INP_ERROR_ERR;
        num_atoms = INCHI_INP_ERROR_RET;
        ...
        goto bypass_end_of_INChI; /* reversibility info not found */
                 ...
                 !memcmp(szLine, sStructRevXmlRevHdr,
                         sizeof(sStructRevXmlRevHdr) - 1)) {
        /***********************  atoms xml ***************************/
        num_struct = 1;
        res = inchi_ios_gets(szLine, sizeof(szLine) - 1, inp_molfile,
                             &bTooLongLine);
        if (res <= 0) {
        ...
        if (memcmp(szLine, sStructRevXmlRevAt,
                   sizeof(sStructRevXmlRevAt) - 1)) {
          bHeaderRead =
              0; /* invalid reversibility info; look for another header */
          continue;
        }
        ...
        while (i < num_atoms) {
          ...
          if (!bItemIsOver &&
            ...
            res2 = inchi_ios_gets(szNextLine, sizeof(szNextLine) - 1,
                                  inp_molfile, &bTooLongLine2);
            if (res2 > 0 && memcmp(szNextLine, sStructRevXmlRevAtEnd,
                                   sizeof(sStructRevXmlRevAtEnd) - 1)) {
              if (pos) {
                res -= pos; /* number of chars left to process in szLine */
                memmove(szLine, p,
                        res * sizeof(szLine[0])); /* move them to the start of
                                                     the line */
              }
              memcpy(szLine + res, szNextLine,
                     (res2 + 1) * sizeof(szNextLine[0]));
              res += res2;
              szLine[res] = '\0';
        ...
        if (memcmp(szLine, sStructRevXmlRevBn,
                   sizeof(sStructRevXmlRevBn) - 1)) {
          bHeaderRead =
              0; /* invalid reversibility info; look for another header */
          continue;
        }
        ...
        res2 = bTooLongLine2 = -1;
        p = szLine;
        if (!memcmp(szLine, sStructRevXmlRevBnEnd,
                    sizeof(sStructRevXmlRevBnEnd) - 1)) {
          /* empty bonds section */
          res = 0;
          bItemIsOver = 1;
        }
        ...
        while (i < num_atoms) {
          ...
          if (!bItemIsOver &&
            ...
            res2 = inchi_ios_gets(szNextLine, sizeof(szNextLine) - 1,
                                  inp_molfile, &bTooLongLine2);
            if (res2 > 0 && memcmp(szNextLine, sStructRevXmlRevBnEnd,
                                   sizeof(sStructRevXmlRevBnEnd) - 1)) {
              if (pos) {
                res -= pos; /* number of chars left to process in szLine */
                memmove(szLine, p,
          ...
          if (bond_parity) {
            ...
            /* (a) i may be allene endpoint and     neigh = allene middle point
               or (b) i may be allene middle point and neigh = allene endpoint
               !!!!! CURRENTLY ONLY (b) IS ALLOWED !!!!!
            */
            atom_stereo0D[len_stereo0D].neighbor[1] = neigh; /* neigh < i */
            atom_stereo0D[len_stereo0D].neighbor[2] = i;
        ...
        if ((pszCoord = (MOL_COORD *)inchi_malloc(inchi_max(num_atoms, 1) *
          ...
          if (res <= 0 ||
              /* compare the header */
              memcmp(szLine, sStructRevXmlRevXYZ,
                     sizeof(sStructRevXmlRevXYZ) - 1) ||
              /* read (the head of) the coordinates (xml) line */
              0 >= (res = inchi_ios_gets(szLine, sizeof(szLine) - 1,
                                         inp_molfile, &bTooLongLine))) {
            num_atoms =
                INCHI_INP_ERROR_RET; /* error in input data: atoms, bonds &
                                        coord must be present together */
          ...
          res2 = bTooLongLine2 = -1;
          p = szLine;
          if (!memcmp(szLine, sStructRevXmlRevXYZEnd,
                      sizeof(sStructRevXmlRevXYZEnd) - 1)) {
            /* empty bonds section */
            res = 0;
            bItemIsOver = 1;
          }
          ...
          while (i < num_atoms) {
            ...
            if (!bItemIsOver &&
              ...
              res2 = inchi_ios_gets(szNextLine, sizeof(szNextLine) - 1,
                                    inp_molfile, &bTooLongLine2);
              if (res2 > 0 && memcmp(szNextLine, sStructRevXmlRevXYZEnd,
                                     sizeof(sStructRevXmlRevXYZEnd) - 1)) {
                if (pos) {
                  res -= pos; /* number of chars left to process in szLine */
                  memmove(szLine, p,
...
#undef MAX_CHAIN_LEN
  ...
  bypass_end_of_INChI:
    ...
    while (num_struct > 0 && 0 < inchi_ios_gets(szLine, sizeof(szLine) - 1,
                                                inp_molfile, &bTooLongLine)) {
      if (!memcmp(szLine, sStructHdrXmlEnd, sizeof(sStructHdrXmlEnd) - 1))
        num_struct--;
...
/**********************************************************************************/
int INChIToInchi_Input(INCHI_IOSTREAM *inp_molfile, inchi_Input *orig_at_data,
                       int bMergeAllInputStructures, int bDoNotAddH,
                       int vABParityUnknown, INPUT_TYPE nInputType,
  ...
  do {
    ...
    at_old = orig_at_data ? orig_at_data->atom
                          : NULL; /*  save pointer to the previous allocation */
    stereo0D_old = orig_at_data ? orig_at_data->stereo0D : NULL;
    szCoordOld = NULL;
    num_inp_atoms_new = INChIToInchi_Atom(
        inp_molfile, orig_at_data ? &stereo0D_new : NULL, &num_inp_0D_new,
        bDoNotAddH, vABParityUnknown, nInputType, orig_at_data ? &at_new : NULL,
        MAX_ATOMS, &num_dimensions_new, &num_inp_bonds_new, pSdfLabel,
        pSdfValue, lSdfId, pInpAtomFlags, err, pStrErr);
    if (num_inp_atoms_new <= 0 && !*err) {
      MOLFILE_ERR_SET(*err, 0, "Empty structure");
      *err = 98;
    } else if (orig_at_data && !num_inp_atoms_new && 10 < *err && *err < 20 &&
               orig_at_data->num_atoms > 0 && bMergeAllInputStructures) {
      *err = 0; /* end of file */
      break;
      ...
      if (nNumAtoms >= MAX_ATOMS) {
        ...
        *err = 70;
        ...
        if (orig_at_data->num_atoms) {
          memcpy(orig_at_data->atom, at_old,
                 orig_at_data->num_atoms * sizeof(orig_at_data->atom[0]));
        ...
        memcpy(orig_at_data->atom + orig_at_data->num_atoms, at_new,
        ...
        if (num_inp_0D_new > 0 && stereo0D_new) {
          if (orig_at_data->stereo0D = CreateInchi_Stereo0D(nNumStereo0D)) {
            memcpy(orig_at_data->stereo0D, stereo0D_old,
                   orig_at_data->num_stereo0D *
                       sizeof(orig_at_data->stereo0D[0]));
            /*  adjust numbering in the newly read structure */
  ...
  */
  ...
  /*
  if ( !*err ) {
      if ( ReconcileAllCmlBondParities( orig_at_data->atom,
  orig_at_data->num_atoms ) ) { MOLFILE_ERR_SET (*err, 0, "Cannot reconcile
  stereobond parities"); if (!orig_at_data->num_atoms) { *err = 1;
          }
      }
  }
  */
  ...
  if (stereo0D && num_stereo0D > 0) {
    ...
    for (i0D = 0; i0D < num_stereo0D; i0D++) {
      ...
      if (((type == INCHI_StereoType_Tetrahedral ||
           ...
           0 <= a2 && a2 < nNumAtoms) ||
          (type == INCHI_StereoType_DoubleBond && a2 == NO_ATOM)) {
        /* test the quadruplet */
        for (j = 0, k_prev = -1; j < 4; j++, k_prev = k) {
          ...
          if (type == INCHI_StereoType_Tetrahedral && k != a2 &&
      ...
      /* find in the adjacency lists the double bond neighbor that leads to the
       * opposite atom */
      if (j == 4 && (type == INCHI_StereoType_Allene ||
        ...
        if (!(q1 = is_in_the_list(at[i1].neighbor, i1n, at[i1].valence)) ||
          ...
          if (!(p1 = is_in_the_list(at[i1].neighbor, i2, at[i1].valence))) {
            /* at[i1] and at[i2] are not connected: can be only allene or
             * cumulene */
            ...
            cur = next = i1;
            len = half_len = 0;
            while (len < 20) { /* arbitrary very high upper limit to prevent
                                  infinite loop */
              prev = cur;
              cur = next;
            ...
            if (type == INCHI_StereoType_Allene) {
              ...
              /* double bond only; the bond type is not checked because at the
                 end of the normalization it may happen to be alternating */
      ...
      case INCHI_StereoType_Allene:
        ...
        if (j1 < MAX_NUM_STEREO_BONDS && j2 < MAX_NUM_STEREO_BONDS &&
            sb_ord_from_i1 >= 0 && sb_ord_from_i2 >= 0 && sn_ord_from_i1 >= 0 &&
            sn_ord_from_i2 >= 0) {
