...
template <> struct graph_traits<OpenBabel::OBMol> {
  ...
  public:
    ...
    adjacent_iterator operator++(int) {
      adjacent_iterator tmp(ip_bond, p_atom);
      ++ip_bond;
      return tmp;
    }
  ...
  public:
    ...
    out_edge_iterator operator++(int) {
      out_edge_iterator tmp(ip_bond, p_atom);
      ++ip_bond;
      return tmp;
    }
...
int num_vertices(const OpenBabel::OBMol &m) { return m.NumAtoms(); }

int num_edges(const OpenBabel::OBMol &m) { return m.NumBonds(); }
...
template <class value> class OBAtomMap {
public:
...
template <class value> class OBBondMap {
public:
  typedef value value_type;
  ...
  reference operator[](OpenBabel::OBBond *b) { return (*_values)[b->GetIdx()]; }
...
void put(map_t &map, typename map_t::key_type key,
...
struct functor_caller : public base_visitor<functor_caller<functor, Tag>> {
  typedef Tag event_filter;
  functor_caller(functor f) : m_func(f) {}
  template <class T, class Graph> void operator()(T t, const Graph &g) {
    m_func.operator()(t);
  }
  functor m_func;
};
...
template <class functor, class Tag>
functor_caller<functor, Tag> call_functor(functor func, Tag) {
  return functor_caller<functor, Tag>(func);
}

template <class output_iterator, class Tag>
struct recorder : public base_visitor<recorder<output_iterator, Tag>> {
  typedef Tag event_filter;
