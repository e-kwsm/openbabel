...
int main(int argc, char **argv) {
  ...
  for (c = 0;;) {
    ...
    boost::breadth_first_search(mol, atom, buffer,
                                OpenBabel::OBVerboseBFSVisitor(cout),
    ...
    boost::breadth_first_search(
        ...
        boost::make_bfs_visitor(std::make_pair(
            boost::record_predecessors(predecessor_map_bfs,
                                       boost::on_tree_edge()),
    ...
    for (OpenBabel::OBAtomIterator beg = mol.BeginAtoms();
      ...
      if (parent) {
        std::cout << (*beg)->GetIdx() - 1
                  << " has parent : " << parent->GetIdx() - 1 << "\t";
      } else {
    ...
    std::cout << "\nBACK EDGES\n";
    for (std::vector<
             boost::graph_traits<OpenBabel::OBMol>::edge_descriptor>::iterator
             beg = bv.begin(),
             end = bv.end();
         beg != end; ++beg) {
    ...
    boost::undirected_dfs(mol, OpenBabel::OBVerboseDFSVisitor(cout),
                          OBAtomMap<boost::default_color_type>(mol),
                          OBBondMap<boost::default_color_type>(mol), atom);
    ...
    boost::undirected_dfs(
        ...
        boost::make_dfs_visitor(std::make_pair(
            boost::record_predecessors(predecessor_map_dfs,
                                       boost::on_tree_edge()),
