...
enum LINE_SEARCH_ALGORITHM {
  ...
  /// where \f$x\f$ is the current point, \f$d\f$ is the current search
  /// direction, \f$a\f$ is the step length, and \f$\beta'\f$ is the value
  /// specified by \ref LBFGSParam::ftol. \f$f\f$ and \f$g\f$ are the function
...
///
template <typename Scalar = double> class LBFGSParam {
public:
  ///
  ...
  /// by the LBFGS routine. The default value is
