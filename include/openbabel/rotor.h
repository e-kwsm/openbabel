...
//! dihedral angles in degrees and/or a "delta" (i.e., all multiples of delta
//! will be considered)
class OBAPI OBRotorRule {
  int _ref[4]; //!< Reference atoms specifying the dihedral angle (as integers),
               //!< numbered from 1 inside the SMARTS pattern
  ...
  std::vector<double>
      _vals; //!< At least one torsion angle (in radians) to evaluate
public:
  ...
  //! \return whether this rotor rule is valid (i.e., is the SMARTS pattern
  //! valid)
...
//! \brief Database of default hybridization torsional rules and SMARTS-defined
//! OBRotorRule objects
//!
...
class OBAPI OBRotorRules : public OBGlobalDataBase {
  ...
  std::vector<OBRotorRule *>
      _vr; //!< Database of specific OBRotorRules defined by SMARTS patterns
  std::vector<double>
      _sp3sp3; //!< Default dihedral angles to check for generic sp3 - sp3
               //!< hybridized rotatable bonds (in radians)
  std::vector<double>
      _sp3sp2; //!< Default dihedral angles to check for generic sp3 - sp2
               //!< hybridized rotatable bonds (in radians)
  std::vector<double>
      _sp2sp2; //!< Default dihedral angles to check for generic sp2 - sp2
               //!< hybridized rotatable bonds (in radians)
public:
  OBRotorRules();
  ~OBRotorRules();
...
class OBAPI OBRotor {
  ...
  double _imag,
      _refang;   //!< inverse magnitude and reference angle (see Precompute())
  OBBond *_bond; //!< the bond associated with this rotor
  std::vector<int> _ref,
      _torsion; //!< indexes for atom coordinates (from 0, multiplied by 3)
  ...
  std::vector<std::vector<double>> _sn, _cs,
      _t; //!< the rotation matrix (see Precalc())
  std::vector<OBRing *>
      _rings; //!< the parent ring (if this is a rotor in a ring)
public:
  /**
   * Constructor.
   */
   ...
   */
  void SetFixedBonds(OBBitVec &bv) { _fixedbonds = bv; }
  ///@}
   ...
   *
   */
  void Set(double *coordinates, double sine, double cosine, double translation,
           double invmag);
  /**
   ...
   *
   * // precompute inverse bond length (i.e. the bond length of bond with index
   * 3
   ...
   * // note: on input, the coordinates should be the same as the coordinates
   * used
   * //       to precompute the inverse bond length and reference angle (in
   * other
  ...
  /**
   * Rotate the @p coordinates to set the torsion angle of this rotor to the
   * angle specified by the index @p idx. Make sure to call Precompute before
   ...
   * specified @p index. The coordinates should be the same as the conformer
   * used for calling Precalc (i.e. conformers[conformer] == coordinates). Make
   * sure to call Precalc before calling this method.
   * @param coordinates The conformer coordinates.
   * @param conformer The conformer index in the conformer list given to
   * Precalc().
   * @param idx The torsion value index in the GetTorsionValues() list.
   */
  void Set(double *coordinates, int conformer, int idx) {
    Set(coordinates, _sn[conformer][idx], _cs[conformer][idx],
        _t[conformer][idx], _invmag[conformer]);
  }
  ///@}
   ...
   */
  size_t Size() { return _torsionAngles.size(); }
  /**
...
class OBAPI OBRotorList {
  ...
  OBBitVec _fixedatoms,
      _fixedbonds;  //!< Bit vector of fixed (i.e., invariant) atoms
...
public:
   ...
   */
  size_t Size() { return _rotor.size(); }
  /**
   ...
   */
  bool HasFixedBonds() { return !_fixedbonds.IsEmpty(); }
   ...
   * (i.e. ring bonds are considered rotatable). Fixed bonds, specified using
   * the deprecated fixed atoms or the new fixed bonds methods are not added to
   ...
   * @param mol The molecule.
   * @param sampleRingBonds whether to sample ring bonds from analysis (default
   * = false)
   * @return True.
   */
  ...
  bool HasFixedAtoms() { return (!_fixedatoms.IsEmpty()); }
  //! Has no effect
...
class OBAPI OBRotorKeys {
  ...
  for (int i = 1; i < rl.Size() + 1; ++i, rotor = rl.NextRotor(ri)) { // foreach
  rotor rotorKeys.AddRotor(rotor->GetTorsionValues().size());
  }
...
public:
  //! Constructor
  OBRotorKeys() { _vr.clear(); }

  //! Clear all rotors
  void Clear() { _vr.clear(); }
