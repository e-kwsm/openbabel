 ...
 *
 * The OBQueryAtom class defines an interface for query atoms. The class
 * provides some general methods and properties to access the topology
 * information. The Matches method can be reimplemented in subclasses to get
...
public:
   ...
   * @param isInRing Specify whether the query atom is in a ring. Default is
   * false.
   * @param isAromatic Specify whether the query atom is aromatic. Default is
   * false.
   */
   ...
   * This method is used by OBIsomorphismMapper implementations.
   */
  const std::vector<OBQueryBond *> &GetBonds() const { return m_bonds; }
  /**
   * Get the neighbor query atoms.
   * This method is used by OBIsomorphismMapper implementations.
   */
  const std::vector<OBQueryAtom *> &GetNbrs() const { return m_nbrs; }
  /**
   * This is the match method to verify if an OBQueryAtom and OBAtom class
   * match. The default implementation only checks if the atomic numbers match.
   * Reimplement this method in a subclass for more advances matching. This
   * method is used by OBIsomorphismMapper implementations.
 ...
 *
 * The OBQueryBond class defines an interface for query bonds. The class
 * provides some general methods and properties to access the topology
 * information. The Matches method can be reimplemented in subclasses to get
 * custom matching behavior.
 *
 * The default Matches implementation only checks if the bonds are both
 * aromatic, otherwise the bond orders are compared.
 *
 * See @ref substructure for more information.
 *
...
public:
   ...
   * Constructor.
   */
  OBQueryBond(OBQueryAtom *begin, OBQueryAtom *end, int order = 1,
              bool aromatic = false)
  ...
  /**
   * Get the index for this query bonds. Query bonds are indexed starting from
   * 0.
   */
  unsigned int GetIndex() const { return m_index; }
  /**
  ...
  /**
   * This is the match method to verify if an OBQueryBond and OBBond class
   * match. The default implementation checks if both bonds are aromatic and
   * compares the bond orders otherwise. Reimplement this method in a subclass
   * for more advances matching. This method is used by OBIsomorphismMapper
   * implementations.
...
public:
   ...
   */
  unsigned int NumAtoms() const { return m_atoms.size(); }
  /**
   * @return The number of bonds in the query.
   */
  unsigned int NumBonds() const { return m_bonds.size(); }
  /**
   * @return std::vector with pointers to the query atoms.
   */
  const std::vector<OBQueryAtom *> &GetAtoms() const { return m_atoms; }
  /**
 ...
 * @param mask The mask specifying the atoms to use. Indexed from 1 (i.e.
 * OBAtom::GetIdx()).
 * @return A pointer to an OBQuery object for the smiles string. This pointer
 * should be deleted.
 * @since version 2.3
 */
