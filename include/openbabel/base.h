...
//! \return the version of the Open Babel library for feature-detection (e.g.
//! "2.3.1")
OBAPI std::string OBReleaseVersion();

//! \brief Classification of data stored via OBGenericData class and subclasses.
//!
//! OBGenericDataType can be used as a faster, direct access to a particular
//! category instead of the slower access via GetData(std::string), which must
//! loop through all data to find a match with the supplied key. It is
//! implemented as a set of unsigned integer constants for maximum flexibility
...
//! Macro definitions can be used to define what each data slot is used in your
//! code.
...
enum {
  ...
  //! Storing text comments (one per molecule, atom, bond, etc.) (for other
  //! data, e.g., author, keyword, ... use OBPairData)
  CommentData = 3,
  ...
  //! Information about torsion/dihedral angles, i.e., OBTorsionData and
  //! OBTorsion
  TorsionData = 9,
  ...
  //! Symmetry data -- point and space groups, transforms, etc. i.e.,
  //! OBSymmetryData
  SymmetryData = 15,
...
protected:
  std::string
      _attr; //!< attribute tag (e.g., "UnitCell", "Comment" or "Author")
...
public:
  ...
  //! \return A list of descriptions of command-line options for
  //! DoTransformations()
  static const char *ClassDescription() { return ""; }

  //! \brief By default clears the object. Called from ReadMolecule of most
  //! format classes
  template <class T> T *CastAndClear(bool clear = true) {
  ...
  virtual const char *GetTitle(bool UNUSED(replaceNewlines) = true) const {
    return "";
  }
  ...
  //! Deletes the generic data with the specified attribute, returning false if
  //! not found
  ...
  //! \return any data matching the given attribute name or NULL if nothing
  //! matches
