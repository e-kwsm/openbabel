...
public:
  ...
  Suppose you wanted to insert XML output from OB into into a template XML
  document using the babel interface babel template.text inputfile.xxx
  outputfile.yyy The template file could contain a line:
    <!-- OPENBABEL_INSERT here -->
  and still be well-formed XML.
  The template file would be read by TextFormat and passed to the output as an
  OBText object. This could be processed in the output format's
  WriteChemObject() or WriteMolecule() in the following way (see
  cmlreactformat.cpp) <code> OBText* ptext = dynamic_cast<OBText*>(pOb);
      if(ptext) {
        string::size_type pos = 0;
        *pConv->GetOutStream() << ptext->GetText(pos); //Output text up to
  insertion point _text = ptext->GetText(pos); //Save text after insertion point
  to be output at the end
      }
  </code>
  **/
  std::string GetText(std::string::size_type &pos,
                      bool ToInsertOnly = false) const {
