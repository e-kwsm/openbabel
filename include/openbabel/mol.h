...
//! Total spin on this molecule has been set. See
...
protected:
  ...
  unsigned int _totalSpin; //!< Total spin on the molecule (if not specified,
                           //!< assumes lowest possible spin)
  ...
  std::vector<OBInternalCoord *>
      _internals;          //!< Internal Coordinates (if applicable)
...
public:
  ...
  //! \param forceNewId  whether to make a new atom Id even if the atom already
  //! has one (default is false) \return Whether the method was successful
  ...
  bool AddBond(int beginIdx, int endIdx, int order, int flags = 0,
               int insertpos = -1);
  ...
  //! \param replaceNewlines whether to replace any newline characters with
  //! spaces
  ...
  //! \return the number of rotatable bonds. If sampleRingBonds is true, will
  //! include rotors within rings (see OBBond::IsRotor() for details)
  ...
  //! \return the bond connecting the atom indexed by @p a and @p b or NULL if
  //! none exists. \warning Atom indexing will change. Use atom objects and
  //! iterators instead.
  OBBond *GetBond(int a, int b) const;
  // The safer version of the above method
  //! \return the bond between the atoms @p bgn and @p end or NULL if none
  //! exists
  ...
  /*! \return the dihedral angle (in degrees) between the four atoms supplied
   * a1-a2-a3-a4) WARNING: SetTorsion takes an angle in radians while GetTorsion
   * returns it in degrees
   */
  double GetTorsion(int, int, int, int);
  /*! \return the dihedral angle (in degrees) between the four atoms @p a, @p b,
   * @p c, and @p d) WARNING: SetTorsion takes an angle in radians while
   * GetTorsion returns it in degrees
   */
  ...
  //! \return the size of the smallest ring if a and b are in the same ring, 0
  //! otherwise \since version 2.4
  ...
  //! \return the mass given by isotopes (or most abundant isotope, if not
  //! specified)
  double GetExactMass(bool implicitH = true);
  //! \return the total charge on this molecule (i.e., 0 = neutral, +1, -1...)
  int GetTotalCharge();
  //! \return the total spin on this molecule (i.e., 1 = singlet, 2 =
  //! doublet...)
  unsigned int GetTotalSpinMultiplicity();
  //! \return the dimensionality of coordinates (i.e., 0 = unknown or no coord,
  //! 2=2D, 3=3D)
  unsigned short int GetDimension() const { return _dimension; }
  //! \return the set of all atomic coordinates. See OBAtom::GetCoordPtr for
  //! more
  ...
  //! Mark that aromaticity has been perceived for this molecule (see
  //! OBAromaticTyper)
  void SetAromaticPerceived(bool value = true) {
    SET_OR_UNSET_FLAG(OB_AROMATIC_MOL);
  }
  ...
  //! \name Molecule modification methods
  //@{
  // Description in transform.cpp (command-line transformations to this
  // molecule)
  virtual OBBase *
  DoTransformations(const std::map<std::string, std::string> *pOptions,
                    OBConversion *pConv);
  // Ditto (documentation on transformation options)
  static const char *ClassDescription();
  //! Clear all information from a molecule except OB_PATTERN_STRUCTURE left
  //! unchanged
  bool Clear();
  //! Renumber the atoms of this molecule according to the order in the supplied
  //! vector
  void RenumberAtoms(std::vector<OBAtom *> &);
  //! Renumber the atoms of this molecule using the initial indexes in the
  //! supplied vector
  ...
  //! Translate one conformer and rotate by a rotation matrix (which is
  //! returned) to the inertial frame-of-reference
  ...
  //! Rotate all conformers using the supplied matrix @p u (a 3x3 array of
  //! double)
  void Rotate(const double u[3][3]);
  //! Rotate all conformers using the supplied matrix @p m (a linear 3x3
  //! row-major array of double)
  void Rotate(const double m[9]);
  //! Rotate a specific conformer @p nconf using the supplied rotation matrix @p
  //! m
  ...
  bool AddHydrogens(bool polaronly = false, bool correctForPH = false,
                    double pH = 7.4);
  //! For a particular atom, convert implicit hydrogens to explicit atoms in the
  //! molecular graph
  bool AddHydrogens(OBAtom *);
  //! For polar atoms only, convert implicit hydrogens to explicit atoms in the
  //! molecular graph
  bool AddPolarHydrogens();
  //! For non-polar atoms only, convert implicit hydrogens to explicit atoms in
  //! the molecular graph \since version 2.4
  bool AddNonPolarHydrogens();
  //! For polar and/or non-polar atoms, convert implicit hydrogens to explicit
  //! atoms in the molecular graph \since verison 2.4
  bool AddNewHydrogens(HydrogenType whichHydrogen, bool correctForPH = false,
                       double pH = 7.4);

  //! If @p threshold is not specified or is zero, remove all but the largest
  //! contiguous fragment. If @p threshold is non-zero, remove any fragments
  //! with fewer than @p threshold atoms.
  ...
  // docs in mol.cpp
  bool CopySubstructure(OBMol &newmol, OBBitVec *includeatoms,
                        OBBitVec *excludebonds = (OBBitVec *)nullptr,
  ...
  /** Convert zero-order bonds to single or double bonds and adjust adjacent
   * atom charges in an attempt to achieve the correct valence state.
  ...
  //@{
  //! Are there non-zero coordinates in two dimensions (i.e. X and Y)- and, if
  //! Not3D is true, no Z coordinates?
  ...
  //! \return The array of coordinates for the next conformer, or NULL if none
  //! exist
  double *NextConformer(std::vector<double *>::iterator &i) {
    ++i;
    return ((i == _vconf.end()) ? nullptr : *i);
  }
  //! \return the entire set of conformers for this molecule as a vector of
  ...
  //! \return A constant atom iterator pointing to the beginning of the atom
  //! list
  ...
  //! \return A range over the atoms. This range can be used in a range-based
  //! for loop.
  OBMolAtomRange GetAtoms() const {
    return {_vatom.begin(), _vatom.begin() + NumAtoms()};
  }
  ...
  //! \return A range over the bonds. This range can be used in a range-based
  //! for loop.
  OBMolBondRange GetBonds() const {
    return {_vbond.begin(), _vbond.begin() + NumBonds()};
  }
  ...
  OBInternalCoord *
  BeginInternalCoord(std::vector<OBInternalCoord *>::iterator &i) {
    i = _internals.begin();
  ...
  //! \see SetInternalCoord
  OBInternalCoord *
  NextInternalCoord(std::vector<OBInternalCoord *>::iterator &i) {
    ++i;
    return ((i == _internals.end()) ? nullptr : *i);
  }
  //@}
...
template <typename T, int size = sizeof(T)> struct max_value {
  static const T result =
      (static_cast<T>(0xFF) << (size - 1) * 8) + max_value<T, size - 1>::result;
};

//! \brief a C++ template to return the maximum value of a type (e.g., int)
template <typename T> struct max_value<T, 0> {
...
#ifndef IsUnsatType
#define IsUnsatType(x)                                                         \
  (EQ(x, "Car") || EQ(x, "C2") || EQ(x, "Sox") || EQ(x, "Sac") ||              \
   EQ(x, "Pac") || EQ(x, "So2"))
#endif
