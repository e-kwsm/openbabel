...
*/
  ...
  struct LBFGSState;
  LBFGSState *_lbfgsState =
      nullptr; //!< Used for L-BFGS (Initialize and TakeNSteps); owned,
               //!< destroyed in ~OBForceField()
#ifdef HAVE_EIGEN3
...
public:
   ...
   *  \param mol The OBMol object to copy the coordinates to (from
   * OBForceField::_mol).
   *  \return True if successful.
   */
   ...
   * torsions, oop). This function should be called before Setup().
   ...
   * enabled. This function should be called before Setup().
   ...
   *  OBFF_LOGLVL_LOW:    Number of rotatable bonds, energies for the
   * conformers, which one is the lowest, ... \n OBFF_LOGLVL_MEDIUM: See note
   ...
   * rotating around the rotatable bonds in a molecule (see OBRotamerList
   * class). This rotating generates multiple conformers. The energy for all
   * these conformers is then evaluated and the lowest energy conformer is
   * selected.
   *
   ...
   * rotating around the rotatable bonds in a molecule (see OBRotamerList
   * class). Unlike RandomRotorSearch() the random choice of torsions is
   * reweighted based on the energy of the generated conformer. Over time, the
   * generated conformers for each step should become increasingly better. The
  ...
  /*! Perform L-BFGS optimization for steps steps or until convergence criteria
   * is reached.
   *
   ...
   *  \param method Deprecated. (see HasAnalyticalGradients())
   */
  void LBFGS(int steps, double econv = 1e-6f,
             int method = OBFF_ANALYTICAL_GRADIENT);
  /*! Initialize L-BFGS optimization, to be used in combination with
   * LBFGSTakeNSteps().
   *
   *  example:
   *  \code
   ...
   *  \param method Deprecated. (see HasAnalyticalGradients())
   */
  void LBFGSInitialize(int steps = 1000, double econv = 1e-6f,
                       int method = OBFF_ANALYTICAL_GRADIENT);
  /*! Take n steps in an L-BFGS optimization previously initialized with
   * LBFGSInitialize().
   *
   *  \param n The number of steps to take.
   *  \return False if convergence or the number of steps given by
   * LBFGSInitialize() has been reached.
   */
  ...
  */
   ...
   * dot(ab,cb)/rab*rcb. Used for harmonic (cubic) angle potentials.
   * \param pos_a atom a (coordinates)
   * \param pos_b atom b (coordinates)
   ...
   * arccos(dot(corss(ab,cb),db)/rabbc*rdb).
   * \param pos_a atom a (coordinates)
   * \param pos_b atom b (coordinates)
   * \param pos_c atom c (coordinates)
   ...
   * given by arccos(dot(corss(ab,bc),cross(bc,cd))/rabbc*rbccd).
   * \param pos_a atom a (coordinates)
   * \param pos_b atom b (coordinates)
   * \param pos_c atom c (coordinates)
   * \param pos_d atom d (coordinates)
