...
#define OBFF_LOGLVL_LOW                                                        \
  1 //!< SteepestDescent progress... (no output from Energy())
...
// constraint types
#define OBFF_CONST_IGNORE                                                      \
  (1 << 0) //!< ignore the atom while setting up calculations
#define OBFF_CONST_ATOM (1 << 1) //!< fix the atom position
#define OBFF_CONST_ATOM_X                                                      \
  (1 << 2) //!< fix the x coordinate of the atom position
#define OBFF_CONST_ATOM_Y                                                      \
  (1 << 3) //!< fix the y coordinate of the atom position
#define OBFF_CONST_ATOM_Z                                                      \
  (1 << 4) //!< fix the z coordinate of the atom position
...
const double GAS_CONSTANT =
    8.31446261815324e-3 /
    KCAL_TO_KJ; //!< kcal mol^-1 K^-1 (2018 CODATA recommended value)
...
//! The type of line search to be used for optimization -- simple or Newton
//! numeric
...
public:
  ...
  //! Used to store double type parameters (force constants, bond lengths,
  //! angles, ...)
...
//! \brief Internal class for OBForceField to hold energy and gradient
//! calculations on specific force fields
...
public:
  ...
  /*! Translate indices to OBAtom* objects, this function is called from
   * OBForceField::Setup, this function doesn't have to be called from anywhere
   * else.
   */
...
*/
   ...
   * \param terms OBFF_ENERGY, OBFF_EBOND, OBFF_EANGLE, OBFF_ESTRBND,
   * OBFF_ETORSION, OBFF_EOOP, OBFF_EVDW, OBFF_ELECTROSTATIC \return the
   * negative gradient of atom a
   */
  ...
  bool _init; //!< Used to make sure we only parse the parameter file once, when
              //!< needed
  std::string _parFile; //!< parameter file name
  bool _validSetup;     //!< was the last call to Setup successful
  double *_gradientPtr; //!< pointer to the gradients (used by AddGradient(),
  ...
  int _current_conformer; //!< used to hold i for current conformer (needed by
                          //!< UpdateConformers)
  std::vector<double>
      _energies; //!< used to hold the energies for all conformers
  // minimization variables
  double _econv, _gconv, _e_n1; //!< Used for conjugate gradients and steepest
                                //!< descent(Initialize and TakeNSteps)
  int _cstep, _nsteps;          //!< Used for conjugate gradients and steepest
                                //!< descent(Initialize and TakeNSteps)
  double *_grad1;               //!< Used for conjugate gradients and steepest
                                //!< descent(Initialize and TakeNSteps)
  ...
  // group variables
  std::vector<OBBitVec> _intraGroup; //!< groups for which intra-molecular
                                     //!< interactions should be calculated
  std::vector<OBBitVec> _interGroup; //!< groups for which intra-molecular
                                     //!< interactions should be calculated
  std::vector<std::pair<OBBitVec, OBBitVec>>
      _interGroups; //!< groups for which intra-molecular
                    //!< interactions should be calculated
public:
  /*! Clone the current instance. May be desirable in multithreaded
   * environments, Should be deleted after use
   */
  ...
  const char *TypeID() override { return "forcefields"; }

  /*! \param ID forcefield id (Ghemical, MMFF94, UFF, ...).
   *  \return A pointer to a forcefield (the default if ID is empty), or NULL if
   * not available.
   */
   ...
   * not available.
   */
  static OBForceField *FindForceField(const char *ID) { return FindType(ID); }
  /*
   *
   */
  ...
  /*! \return The unit (kcal/mol, kJ/mol, ...) in which the energy is expressed
   * as std::string.
   */
  ...
  /*! Setup the forcefield for mol (assigns atom types, charges, etc.). Keep
   * current constraints. \param mol The OBMol object that contains the atoms
   * and bonds. \return True if successful.
   */
  bool Setup(OBMol &mol);
  /*! Setup the forcefield for mol (assigns atom types, charges, etc.). Use new
   * constraints. \param mol The OBMol object that contains the atoms and bonds.
   *  \param constraints The OBFFConstraints object that contains the
   * constraints. \return True if successful.
   */
  bool Setup(OBMol &mol, OBFFConstraints &constraints);
  /*! Load the parameters (this function is overloaded by the individual
  ...
  /*! Set the atom types (this function is overloaded by the individual
   * forcefields, and is called autoamically from OBForceField::Setup()).
   */
  // move to protected in future version
  virtual bool SetTypes() { return false; }
  /*! Set the formal charges (this function is overloaded by the individual
   * forcefields, and is called autoamically from OBForceField::Setup()).
   */
  // move to protected in future version
  virtual bool SetFormalCharges() { return false; }
  /*! Set the partial charges (this function is overloaded by the individual
   * forcefields, and is called autoamically from OBForceField::Setup()).
   */
  // move to protected in future version
  virtual bool SetPartialCharges() { return false; }
  /*! Setup the calculations (this function is overloaded by the individual
   * forcefields, and is called autoamically from OBForceField::Setup()).
   */
  // move to protected in future version
  virtual bool SetupCalculations() { return false; }
  /*! Setup the pointers to the atom positions in the OBFFCalculation objects.
   * This method will iterate over all the calculations and call SetupPointers
   * for each one. (This function should be implemented by the individual force
   ...
   *    if (type)
   *      cout << "atom " << atom->GetIdx() << " : " << type->GetValue() <<
   * endl;
   *  }
   *  ...
   *  \endcode
  ...
  /*! Get coordinates for current conformer and attach OBConformerData with
   * energies, forces, ... to mol. \param mol The OBMol object to copy the
   * coordinates to (from OBForceField::_mol). \return True if successful.
   */
  ...
  /*! Get coordinates for all conformers and attach OBConformerData with
   * energies, forces, ... to mol. \param mol The OBMol object to copy the
   * coordinates to (from OBForceField::_mol). \return True if successful.
   */
   ...
   *  \param mol the OBMol object to copy the coordinates from (to
   * OBForceField::_mol). \return true if successful.
   */
  ...
  /*! Create a grid with spacing @p step and @p padding. Place a probe atom of
   * type probe at every grid point, calculate the energy and store it in the
   * grid. These grids can then be used to create isosurfaces to identify
   * locations where the probe atom has favourable interactions with the
   * molecule. \param step The grid step size in A.. \param padding The padding
   * for the grid in A. \param type The force field atom type for the probe.
   *  \param pchg The partial charge for the probe atom.
   *  \return Pointer to the grid constaining the results.
   */
  OBGridData *GetGrid(double step, double padding, const char *type,
                      double pchg);
  ...
  //@{
  /*! Enable intra-molecular interactions for group (bonds, angles, strbnd,
   * torsions, oop). This function should be called before Setup(). \param group
   * OBBitVec with bits set for the indexes of the atoms which make up the
   * group.
   */
  ...
  /*! Enable inter-molecular interactions between group1 and group2 (non-bonded:
   * vdw & ele). Note that this function doesn't enable bonded interactions in
   * either group. Non-bonded interactions in the groups itself are also not
   * enabled. This function should be called before Setup(). \param group1
   * OBBitVec with bits set for the indexes of the atoms which make up the first
   * group. \param group2 OBBitVec with bits set for the indexes of the atoms
   ...
   */
  void EnableCutOff(bool enable) { _cutoff = enable; }
  /*! \return True if Cut-off distances are used.
   */
  bool IsCutOffEnabled() { return _cutoff; }
  /*! Set the VDW cut-off distance to r. Note that this does not enable cut-off
   * distances. \param r The VDW cut-off distance to be used in A.
   */
  void SetVDWCutOff(double r) { _rvdw = r; }
  /*! Get the VDW cut-off distance.
   *  \return The VDW cut-off distance in A.
   */
  double GetVDWCutOff() { return _rvdw; }
   ...
   */
  void SetElectrostaticCutOff(double r) { _rele = r; }
  /*! Get the Electrostatic cut-off distance.
   *  \return The electrostatic cut-off distance in A.
   */
  double GetElectrostaticCutOff() { return _rele; }
   ...
   */
  void SetDielectricConstant(double epsilon) { _epsilon = epsilon; }
  /* Get the dielectric permittivity used for electrostatic calculations
   * \rreturn The current relative permittivity
   */
  double GetDielectricConstant() { return _epsilon; }
  /*! Set the frequency by which non-bonded pairs are updated. Values from 10 to
   * 20 are recommended. Too low will decrease performance, too high will cause
   *  non-bonded interactions within cut-off not to be calculated.
   *  \param f The pair list update frequency.
   */
  void SetUpdateFrequency(int f) { _pairfreq = f; }
  /*! Get the frequency by which non-bonded pairs are updated.
   *  \return The pair list update frequency.
   */
  int GetUpdateFrequency() { return _pairfreq; }
  ...
  /*! Get the pointer to the gradients
   */
  double *GetGradientPtr() { return _gradientPtr; }
   ...
   * OBFF_LOGLVL_HIGH). Inline if statements for logging are available: \code
   *  #define IF_OBFF_LOGLVL_LOW    if(_loglvl >= OBFF_LOGLVL_LOW)
   *  #define IF_OBFF_LOGLVL_MEDIUM if(_loglvl >= OBFF_LOGLVL_MEDIUM)
   *  #define IF_OBFF_LOGLVL_HIGH   if(_loglvl >= OBFF_LOGLVL_HIGH)
   *  \endcode
   ...
   *  minimized for the best results. SystematicRotorSearch works by rotating
   *around the rotatable bond in a molecule (see OBRotamerList class). This
   *rotating generates multiple conformers. The energy for all these conformers
   *is then evaluated and the lowest energy conformer is selected.
   *
   ...
   *
   *	\par Output to log:
   *  This function should only be called with the log level set to
   *OBFF_LOGLVL_NONE or OBFF_LOGLVL_LOW. Otherwise too much information about
   *the energy calculations needed for this function will interfere with the
   *output for this function. \n\n OBFF_LOGLVL_NONE:   None. \n OBFF_LOGLVL_LOW:
   *Number of rotatable bonds, energies for the conformers, which one is the
   *lowest, ... \n OBFF_LOGLVL_MEDIUM: See note above. \n OBFF_LOGLVL_HIGH: See
   *note above. \n
   */
   ...
   *  \endcode
   *
   *  If you don't need any updating in your program, SystematicRotorSearch() is
   * recommended.
   *
   *  \param geomSteps The number of steps to take during geometry optimization.
   *  \param sampleRingBonds Whether to sample ring torsions.
   ...
   *
   *  The initial starting structure here is important, this structure should be
   *  minimized for the best results. RandomRotorSearch works by randomly
   *rotating around the rotatable bonds in a molecule (see OBRotamerList class).
   *This rotating generates multiple conformers. The energy for all these
   *conformers is then evaluated and the lowest energy conformer is selected.
   *
   *  \param conformers The number of random conformers to consider during the
   *search. \param geomSteps The number of steps to take during geometry
   *optimization for each conformer. \param sampleRingBonds Whether to sample
   *ring torsions.
   *
   *	\par Output to log:
   *  This function should only be called with the log level set to
  ...
  void RandomRotorSearch(unsigned int conformers, unsigned int geomSteps = 2500,
                         bool sampleRingBonds = false);
  /*! Generate conformers for the molecule by randomly rotating torsions. To be
   * used in combination with RandomRotorSearchNexConformer().
   *
   *  example:
   *  \code
   *  // pFF is a pointer to a OBForceField class
   ...
   *  \endcode
   *
   *  If you don't need any updating in your program, RandomRotorSearch() is
   * recommended.
   *
   *  \param conformers The number of random conformers to consider during the
   * search \param geomSteps The number of steps to take during geometry
   * optimization \param sampleRingBonds Whether to sample ring torsions.
   */
   ...
   *
   *  The initial starting structure here is important, this structure should be
   *  minimized for the best results. WeightedRotorSearch works by randomly
   *rotating around the rotatable bonds in a molecule (see OBRotamerList class).
   *Unlike RandomRotorSearch() the random choice of torsions is reweighted based
   *on the energy of the generated conformer. Over time, the generated
   *conformers for each step should become increasingly better. The lowest
   *energy conformer is selected.
   *
   * \param conformers The number of random conformers to consider during the
   *search. \param geomSteps The number of steps to take during geometry
   *optimization for each conformer. \param sampleRingBonds Whether to sample
   ...
   * torsion angle to that which minimizes the energy (while keeping the rest of
   * the molecule fixed). In general (for molecules with more than one rotatable
   * bond), this procedure will not find the global minimum, but it will at
   * least get rid of any bad clashes, and it do so quickly.
   *
   ...
   *
   * @param permute Whether or not to permute the order of the 4 most central
   * rotors. Default is true. This does a more thorough search, but takes 4! =
   * 24 times as long.
   * @since version 2.4
   */
...
#ifdef HAVE_EIGEN
  //! \since version 2.4
  int DiverseConfGen(double rmsd, unsigned int nconfs = 0,
                     double energy_gap = 50, bool verbose = false);
#endif
   ...
   *  \param type The LineSearchType to be used in SteepestDescent and
   * ConjugateGradients.
   */
  void SetLineSearchType(int type) { _linesearch = type; }
  /*! Get the LineSearchType.
   *  \return The current LineSearchType.
   */
  int GetLineSearchType() { return _linesearch; }
   ...
   *  This function should only be called with the log level set to
   * OBFF_LOGLVL_NONE or OBFF_LOGLVL_LOW. Otherwise too much information about
   * the energy calculations needed for the minimization will interfere with the
   * list of energies for succesive steps. \n\n OBFF_LOGLVL_NONE:   none \n
  ...
  /*! Initialize steepest descent optimalization, to be used in combination with
   ...
   *  \endcode
   *
   *  If you don't need any updating in your program, SteepestDescent() is
   * recommended.
   *
   *  \param steps The number of steps.
   *  \param econv Energy convergence criteria. (defualt is 1e-6)
  ...
  void SteepestDescentInitialize(int steps = 1000, double econv = 1e-6f,
                                 int method = OBFF_ANALYTICAL_GRADIENT);
  /*! Take n steps in a steepestdescent optimalization that was previously
   * initialized with SteepestDescentInitialize().
   *
   *  \param n The number of steps to take.
   *  \return False if convergence or the number of steps given by
   * SteepestDescentInitialize() has been reached.
   *
   *  \par Output to log:
   *  This function should only be called with the log level set to
   * OBFF_LOGLVL_NONE or OBFF_LOGLVL_LOW. Otherwise too much information about
  ...
  /*! Perform conjugate gradient optimalization for steps steps or until
   * convergence criteria is reached.
   *
   *  \param steps The number of steps.
   *  \param econv Energy convergence criteria. (defualt is 1e-6)
  ...
  void ConjugateGradients(int steps, double econv = 1e-6f,
                          int method = OBFF_ANALYTICAL_GRADIENT);
  /*! Initialize conjugate gradient optimalization and take the first step, to
   * be used in combination with ConjugateGradientsTakeNSteps().
   *
   *  example:
   *  \code
   *  // pFF is a pointer to a OBForceField class
   ...
   *  \endcode
   *
   *  If you don't need any updating in your program, ConjugateGradients() is
   * recommended.
   *
   *  \param steps The number of steps.
   *  \param econv Energy convergence criteria. (defualt is 1e-6)
   ...
   *  \param n The number of steps to take.
   *  \return False if convergence or the number of steps given by
   * ConjugateGradientsInitialize() has been reached.
   *
   *  \par Output to log:
   *  This function should only be called with the log level set to
   * OBFF_LOGLVL_NONE or OBFF_LOGLVL_LOW. Otherwise too much information about
  ...
  /*! Take n steps at temperature T. If no velocities are set, they will be
   * generated.
   *
   *  example:
   *  \code
   ...
   *  \param method OBFF_ANALYTICAL_GRADIENTS (default) or
   * OBFF_NUMERICAL_GRADIENTS
   */
  ...
  /*!
    Validate the analytical gradients by comparing them to numerical ones. This
    function has to be implemented force field specific. (debugging)
  */
  ...
  */
   ...
   * \return The distance between a and b (bondlength for bond stretching,
   * separation for vdw, electrostatic)
   */
   ...
   */
  static double VectorDistanceDerivative(const double *const pos_i,
  ...
  /*! Calculate the derivative of a angle a-b-c. The angle is given by
   * dot(ab,cb)/rab*rcb. Used for harmonic (cubic) angle potentials. \param
   * pos_a atom a (coordinates) \param pos_b atom b (coordinates) \param pos_c
   * atom c (coordinates) \param force_a - return value for the force on atom a
   * \param force_b - return value for the force on atom b
   * \param force_c - return value for the force on atom c
   * \return The angle between a-b-c
   */
  static double VectorAngleDerivative(double *pos_a, double *pos_b,
                                      double *pos_c, double *force_a,
                                      double *force_b, double *force_c);
  //! \deprecated
  OB_DEPRECATED
  static double VectorAngleDerivative(vector3 &a, vector3 &b, vector3 &c);
  /*! Calculate the derivative of a OOP angle a-b-c-d. b is the central atom,
   * and a-b-c is the plane. The OOP angle is given by 90° -
   * arccos(dot(corss(ab,cb),db)/rabbc*rdb). \param pos_a atom a (coordinates)
   * \param pos_b atom b (coordinates)
  ...
  static double VectorOOPDerivative(double *pos_a, double *pos_b, double *pos_c,
                                    double *pos_d, double *force_a,
                                    double *force_b, double *force_c,
                                    double *force_d);
  //! \deprecated
  OB_DEPRECATED
  static double VectorOOPDerivative(vector3 &a, vector3 &b, vector3 &c,
                                    vector3 &d);
  /*! Calculate the derivative of a torsion angle a-b-c-d. The torsion angle is
   * given by arccos(dot(corss(ab,bc),cross(bc,cd))/rabbc*rbccd). \param pos_a
   * atom a (coordinates) \param pos_b atom b (coordinates) \param pos_c atom c
   * (coordinates) \param pos_d atom d (coordinates) \param force_a - return
   * value for the force on atom a \param force_b - return value for the force
   * on atom b \param force_c - return value for the force on atom c \param
   * force_d - return value for the force on atom d \return The tosion angle for
   * a-b-c-d
   */
  static double VectorTorsionDerivative(double *pos_a, double *pos_b,
                                        double *pos_c, double *pos_d,
                                        double *force_a, double *force_b,
                                        double *force_c, double *force_d);
  //! \deprecated
  OB_DEPRECATED
  static double VectorTorsionDerivative(vector3 &a, vector3 &b, vector3 &c,
                                        vector3 &d);
  ...
  static void VectorSubtract(const double *const i, const double *const j,
                             double *result) {
    for (unsigned int c = 0; c < 3; ++c)
      result[c] = i[c] - j[c];
  }
  ...
  static void VectorMultiply(const double *const i, const double n,
                             double *result) {
    for (unsigned int c = 0; c < 3; ++c)
      result[c] = i[c] * n;
  }
  ...
  /*! inline fuction to speed up minimization speed
   * \param i pointer to i[3], multiply this vector by n and set this vector to
   * the result. \param n the scalar value to be multipled
   */
