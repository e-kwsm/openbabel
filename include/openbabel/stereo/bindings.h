...
struct OBTetrahedralConfig {
  ...
  bool operator==(const OBTetrahedralConfig &other) const {
    ...
    OBTetrahedralStereo::Config thisConfig = Convert(*this),
                                otherConfig = Convert(other);

    if (from_or_towards == OBStereo::ImplicitRef) {
      thisConfig =
          OBTetraNonPlanarStereo::ToConfig(thisConfig, refs[0], winding, view);
      otherConfig = OBTetraNonPlanarStereo::ToConfig(
          otherConfig, thisConfig.from, winding, view);
    } else if (other.from_or_towards == OBStereo::ImplicitRef) {
      otherConfig = OBTetraNonPlanarStereo::ToConfig(otherConfig, other.refs[0],
                                                     winding, view);
      thisConfig = OBTetraNonPlanarStereo::ToConfig(
          thisConfig, otherConfig.from, winding, view);
    } else {
      otherConfig = OBTetraNonPlanarStereo::ToConfig(
          otherConfig, thisConfig.from, winding, view);
    }
    ...
    if (!OBStereo::ContainsSameRefs(thisConfig.refs, otherConfig.refs)) {
      if (OBStereo::ContainsRef(thisConfig.refs, OBStereo::ImplicitRef)) {
        ...
        for (unsigned int i = 0; i < otherConfig.refs.size(); ++i) {
          bool found = false;
          for (OBStereo::RefIter j = thisConfig.refs.begin();
               j != thisConfig.refs.end(); ++j)
      ...
      } else if (OBStereo::ContainsRef(otherConfig.refs,
        ...
        for (unsigned int i = 0; i < thisConfig.refs.size(); ++i) {
          ...
          for (OBStereo::RefIter j = otherConfig.refs.begin();
               j != otherConfig.refs.end(); ++j)
...
struct OBCisTransConfig {
  ...
  bool operator==(const OBCisTransConfig &other) const {
    ...
    if (!OBStereo::ContainsSameRefs(refs, other.refs)) {
      ...
      for (OBStereo::ConstRefIter i = refs.begin(); i != refs.end(); ++i)
        if (OBStereo::ContainsRef(other.refs, *i)) {
          u1 = OBTetraPlanarStereo::ToConfig(
              Convert(*this), *i, OBStereo::ShapeU); // refs[0] = u1.refs[0]
          u2 = OBTetraPlanarStereo::ToConfig(
              Convert(other), *i, OBStereo::ShapeU); // refs[0] = u2.refs[0]
        }
      ...
      u1 = OBTetraPlanarStereo::ToConfig(
          Convert(*this), refs.at(0), OBStereo::ShapeU); // refs[0] = u1.refs[0]
      u2 = OBTetraPlanarStereo::ToConfig(
          Convert(other), refs.at(0), OBStereo::ShapeU); // refs[0] = u2.refs[0]
...
#ifndef SWIG
  static OBSquarePlanarStereo::Config
  Convert(const OBSquarePlanarConfig &other) {
