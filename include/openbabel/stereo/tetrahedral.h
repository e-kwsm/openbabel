...
public:
   ...
   * - OBStereo::Refs @p refs: The three remaining atom ids (may also contain
   * one OBStereo::NoRef element if from/towards is set to a real atom id).
   * - OBStereo::View @p view: Specify the viewing from or towards the atom with
   *   @p from/towards id.
   * - OBStereo::Winding @p winding: Clockwise or AntiClockwise (order in the
   * Refs @p refs list)
   *
   * @image html tetrahedral.png
   *
  ...
  struct OBAPI Config {
     ...
     */
    Config()
        : center(OBStereo::NoRef), from(OBStereo::NoRef),
     ...
     * @param from_or_towards The atom id from which to view or view towards
     * (see @p view).
     * @param _refs The 3 reference ids.
     * @param _winding The winding for the 3 ids in @p _refs.
     * @param _view Specify viewing from or towards the atom with @p
     * from_or_towards id.
     */
    Config(unsigned long _center, unsigned long from_or_towards,
        ...
        : center(_center), from(from_or_towards), refs(_refs),
          winding(_winding), view(_view), specified(true) {}
    /**
    ...
    OBStereo::View view; //!< Specify viewing from or towards the atom with @p
                         //!< from/towards id.
  ...
  /**
   * Get the configuration as Config struct viewing from/towards the specified
   * id.
   */
 ...
 *
 * // output
 * OBTetrahedralStereo(center = 0, viewTowards = 4, refs = 1 2 3,
 * anti-clockwise)
 * @endcode
 */
 ...
 *
 * // output
 * OBTetrahedralStereo::Config(center = 0, viewTowards = 4, refs = 1 2 3,
 * anti-clockwise)
 * @endcode
 */
OBAPI ostream &operator<<(ostream &out,
