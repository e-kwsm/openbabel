 ...
 * @brief Base class for handling and storing planar stereochemistry with 4
 * reference atoms.
 *
 ...
 * @endcode
 *
 * Internally the reference ids are stored in a U shape. The
 * OBTetraPlanarStereo::ToInternal() and OBTetraPlanarStereo::ToShape() methods
 * convert between the internally used U shape and the other shapes.
 *
...
public:
  ...
  static ConfigType ToConfig(const ConfigType &cfg, unsigned long start,
    ...
    for (int i = 0; i < 4; ++i) {
      std::rotate(result.refs.begin(), result.refs.begin() + 1,
                  result.refs.end());
      // start at refs[0]?
