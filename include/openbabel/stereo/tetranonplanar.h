...
/**
 * @class OBTetraNonPlanarStereo tetranonplanar.h
 * <openbabel/stereo/tetranonplanar.h>
 * @brief Base class for handling and storing non-planar stereochemistry with 4
 * reference atom ids.
 *
 * @image html tetranonplanar.png
 *
 * @section Combinations
 * The four reference ids can be treated like a sequence of 4 numbers. Each
 * element can only occur once. This means there are 4! = 24 combinations.
 *
 ...
 * of the difference in inversions between the 2 sequences is calculated. If
 * this is even, no extra permutation is needed. If this is odd, an extra
 * permutation is needed.
 *
 * Switching between viewing from and viewing towards reverses the winding.
 *
 * Like all stereo classes, errors, warnings or info is reported using
 * OBMessageHandler.
 *
 * @since version 2.3
 */
...
public:
  ...
  static ConfigType ToConfig(const ConfigType &cfg,
    ...
    if (cfg.from == OBStereo::NoRef) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "OBTetraNonPlanarStereo::ToConfig : Invalid from "
                            "in ConfigType struct.",
                            obError);
      return ConfigType();
    }
    if (cfg.refs.size() != 3) {
      obErrorLog.ThrowError(
          __FUNCTION__, "OBTetraNonPlanarStereo::ToConfig : Invalid refs size.",
          obError);
      return ConfigType();
    }
    ...
    obErrorLog.ThrowError(__FUNCTION__,
                          "OBTetraNonPlanarStereo::ToConfig : Parameter id not "
                          "found in internal refs.",
                          obError);
    return result;
  }
  /**
   * Change the winding of the ConfigType struct while maintaining the
   * stereochemistry.
   */
  template <typename ConfigType> static void ChangeWinding(ConfigType &cfg) {
    cfg.winding = (cfg.winding == OBStereo::Clockwise) ? OBStereo::AntiClockwise
                                                       : OBStereo::Clockwise;
    OBStereo::Permutate(cfg.refs, 1, 2);
  }
  /**
   * Change the view of the ConfigType struct while maintaining the
   * stereochemistry.
   */
  template <typename ConfigType> static void ChangeView(ConfigType &cfg) {
   ...
   */
  template <typename ConfigType> static void Invert(ConfigType &cfg) {
    OBStereo::Permutate(cfg.refs, 1, 2);
  }
};
