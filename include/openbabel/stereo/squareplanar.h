 ...
 *
 * The OBSquarePlanarStereo class is used to represent square planar
 * stereochemistry. Like all OBTetraPlanarStereo subclasses, it uses the
 * OBStereo::Shape parameters to set/get the reference ids.
 *
...
public:
   ...
   * For squareplanar stereocenters, the following data members define the
   * spacial arrengement of the atoms.
   *
  ...
  struct OBAPI Config {
     ...
     */
    Config()
        : center(OBStereo::NoRef), shape(OBStereo::ShapeU), specified(true) {}
    /**
    ...
    Config(unsigned long _center, const OBStereo::Refs &_refs,
        ...
        : center(_center), refs(_refs), shape(_shape), specified(true) {}
    /**
  ...
  /**
   * @return True if this object is valid. This object is valid if all (center
   * and and 4 reference) atom ids are set.
   */
  bool IsValid() const;
