...
struct OBAPI OBStereo {
  ...
  enum Winding {
    ...
    UnknownWinding = 3 //!< The configuration is specified as unknown (squiggly
                       //!< line in depiction)
  };
  ...
  enum {
    ...
    ImplicitRef =
        UINT_MAX - 1 //!< Implicit Ref (i.e. hydrogen, N lone pair, ...).
  };
  /**
   ...
   *
   * @code
   * OBStereo::ContainsSameRefs(OBStereo::MakeRefs(1, 2, 3),
   * OBStereo::MakeRefs(2, 3, 1)) // true
   * OBStereo::ContainsSameRefs(OBStereo::MakeRefs(1, 2, 3),
   * OBStereo::MakeRefs(3, 2, 1)) // true
   * OBStereo::ContainsSameRefs(OBStereo::MakeRefs(1, 2, 3),
   * OBStereo::MakeRefs(3, 4, 1)) // false
   * @endcode
   *
...
struct OBStereoUnit {
   ...
   */
  OBStereoUnit()
      : type(static_cast<OBStereo::Type>(0)), id(OBStereo::NoRef), para(false) {
  }
...
private:
  ...
  bool m_specified; //!< True if the stereochemistry is specified, false if
                    //!< unknown/unspecified.
};
...
public:
  ...
  template <int StereoType> bool HasStereo(unsigned long id);
  template <typename T> T *GetStereo(unsigned long id);
 ...
 */
OBAPI void
StereoFrom2D(OBMol *mol,
             std::map<OBBond *, enum OBStereo::BondDirection> *updown = nullptr,
             bool force = false);
/**
 ...
 * stereochemisrty specified using a '@' character. StereoFrom0D() will detect
 * the second tetrahedral atom and add an OBTetrahedralStereo object to the
 * molecule.
 *
 * @param mol The molecule.
 *
 ...
 * @since version 2.3
 */
OBAPI std::vector<OBTetrahedralStereo *>
 ...
 * (i.e. OBBond::Wedge, OBBond::Hash, OBBond::WedgeOrHash and
 OBBond::CisOrTrans)
 ...
 * The actual meaning of \f$S_t\f$ (i.e. assignment of OBStereo::Winding)
 depends
 * on the 4th atom. When the atom is in front of the plane, the sign should be
 * changed to have the same absolute meaning for an atom behind the plane and
 the
 ...
 * @since version 2.3
 */
OBAPI std::vector<OBCisTransStereo *>
CisTransFrom3D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
               bool addToMol = true);
/**
 * Get a vector with all OBCisTransStereo objects for the molecule. This
 * function is used by StereoFrom2D() with the @p addToMol parameter is set
 ...
 * @since version 2.3
 */
OBAPI std::vector<OBCisTransStereo *> CisTransFrom2D(
 ...
 *
 * If no bond can be found that matches rules 1 and 2 (and in theory this is
 * possible) then an error message is logged and the function returns false. (If
 * you find an example where this occurs, please file a bug.)
 *
 * @param mol The molecule.
 * @param updown A map of OBStereo::BondDirection for each hash/wedge bond
 * @param from A map of OBStereo::Ref indicating the origin of each hash/wedge
 * bond
 * @return True or False depending on whether the conversion was successful
 * @since version 2.3
 */
OBAPI bool
TetStereoToWedgeHash(OBMol &mol,
...
/**
 * Convert any reference to @p atomId in a stereo object to an
 * OBStereo::ImplicitRef. This function is called from OBMol::DeleteHydrogens()
 ...
 * @since version 2.4
 */
OBAPI void ImplicitRefToStereo(OBMol &mol, OBStereo::Ref centerId,
                               OBStereo::Ref newId);
/**
 * Get a vector with all OBCisTransStereo objects for the molecule. This
 * function is used by StereoFrom0D() with the @p addToMol parameter is set
 ...
 */
OBAPI std::vector<OBCisTransStereo *>
CisTransFrom0D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
               bool addToMol = true);
///@}
 ...
 * All rings are merged "mergedRings". A merged ring is simply a fragment
 consisting
 * of all atoms of a ring system (bridged, spiro, adjacent, ...). If two rings
 in the
 * SSSR set share an atom, they are merged.
 *
 * Each merged must at least have two para-stereocenters (or 1 true + 1 para) in
 order
 * for the para-stereocenter to be valid. This is repeated until no new
 stereocenters
 * are identified.
 *
 ...
 * - each ligand contains at least 1 true-stereocenter or 2 para-stereocenters
 (from rule 1)
 *
 ...
 * @verbatim
   ...
   @endverbatim
 */
OBAPI OBStereoUnitSet
FindStereogenicUnits(OBMol *mol, const std::vector<unsigned int> &symClasses);
/**
 * @brief Find the stereogenic units in a molecule making use of the
 automorphisms.
 *
 * The potential stereocenters are identified first. A potential tetrahedral
 * stereogenic atom is any atom meeting the following criteria:
 ...
 * A potential stereocenter really is a stereocenter if there exists no
 automorphic
 ...
 * of only the potential stereogenic unit under consideration. The bond can
 still
 ...
 * units. The details about how the symmetry classes are used depends on the
 type
 ...
 *
 * Both StereoFrom3D() and StereoFrom2D() delete all existing stereochemistry
 objects
 * before adding new ones. For molecules with 3D coordinates, it is evident that
 * all information is specified by the coordinates itself. However, if a file
 format
 * uses stereo parity flags, Config structs must be constructed using lower
 level
 * functions and StereoFrom3D() should not be called. In these cases information
 * could be lost by calling StereoFrom3D() after reading the file (the stereo
 flag might have
 ...
 * stereochemistry would always result in lost information. Instead
 StereoFrom0D()
 * only adds new objects for stereogenic units which were previously not found.
 * For example, a smiles is read which has two tetrahedral centers. Only one has
 * stereochemistry specified using a '@' character. StereoFrom0D() will detect
 the
 * second tetrahedral atom and add an OBTetrahedralStereo object to the
 molecule.
 * The Config::specified flag for the newly added structs is always set to
 false.
 *
 * Assuming the format code has correctly set the molecule dimensions
 (OBMol::GetDimesions),
 * PerceiveStereo() will automatically select the correct function to call.
 * When StereoFrom3D(), StereoFrom2D() or StereoFrom0D() are not used, make sure
 to always
 * set OBMol::HasChiralityPerceived() before returning from the format's
 ReadMolecule().
 *
 *
 ...
 *
 * For many file formats no additional code is needed. For example, if a 3D
 format
 * doesn't require stereo parity flags, writing the coordinates is enough. For
 2D
 * file formats it will often suffice to write the coordinates and bond
 properties.
 * If parity flags are needed, the OBStereoFacade class can be used to retrieve
 the
