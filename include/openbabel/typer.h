...
class OBAPI OBAtomTyper : public OBGlobalDataBase {
  std::vector<std::pair<OBSmartsPattern *, int>>
      _vinthyb; //!< internal hybridization rules
  std::vector<std::pair<OBSmartsPattern *, std::string>>
...
public:
  ...
  // swig is requiring these, but I can't figure out how to make it not, so
  // definte with abort
