...
public:
  ...
  /// The param string can be used in derived types to provide different
  /// outputs.
  virtual bool Display(std::string &txt, const char *param,
                       const char *ID = nullptr);

  /// Make a new instance of the class.
  /// See OpTransform, OBGroupContrib, SmartsDescriptor classes for derived
  /// versions. Usually, the first parameter is the classname, the next three
  /// are parameters(ID, filename, description) for a constructor, and the rest
  /// data.
  ...
  /// If PluginID is not recognized or is NULL, the base types are output and
  /// the return is false.
  static bool ListAsVector(const char *PluginID, const char *param,
                           std::vector<std::string> &vlist);

  /// As ListAsVector but sent to an ostream with a default of cout if not
  /// specified
  ...
  static std::string ListAsString(const char *PluginID,
                                  const char *param = nullptr);

  /// Utility function to return only the first line of a string
  static std::string FirstLine(const char *txt);

  /// Return an iterator at the start of the map of the plugin types PluginID
  /// or,
...
protected:
  ...
  /// Returns the map of a particular plugin type, e.g.
  /// GetMapType("fingerprints")
...
protected:                                                                     \
  ...
  PluginMapType &GetMap() const override { return Map(); }                     \
                                                                               \
public:                                                                        \
  static BaseClass *&Default() {                                               \
    static BaseClass *d;                                                       \
...
and, optionally, whether it is to be regarded as the default type of
YourBaseClass. \code YourSubType1 theType1("whatsit2",true); \endcode
...
\endcode
The ListAsString and ListAsVector functions are alternatives, usable with
scripting.
...
protected:
  ...
  //Making the map accessible to the base class (Cannot be used during
construction) virtual PluginMapType& GetMap()const
  {
   return Map();
  }
...
public:
  ...
  {
    _id = ID;
    if(ID && *ID && *ID!=' ') //do not register if ID is empty or starts with a
space
    {
      if(IsDefault || Map().empty())
        Default() = this;
      Map()[ID]=this;
      //Ensure YourBaseClass is registered in OBPlugin so it can be accessed
from the commandline PluginMap()[TypeID()] =this;
    }
  }

  ///Returns the sub-type associated with the ID, or the default subtype if ID
NULL or empty. static YourBaseClass* FindType(const char* ID)
  {
    if(!ID || *ID==0)
      return Default();
