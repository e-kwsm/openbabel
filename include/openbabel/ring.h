...
public:
  ...
  //! If all atoms in this ring are aromatic, the ring will be considered
  //! aromatic
  //! \todo This method uses implicit bonding -- bond info is not stored in
  //! OBRing
  bool IsAromatic();
  ...
  char *GetType();
  //! \return the index for the root atom. O for furan, S for thiazole, N for
  //! pyrrole. For 6 membered aromatic rings, the first non carbon atom is used
  //! for root. For 5 members rings the O, S or N (BOSum=3, valence=3) will be
  //! used for root
  ...
  bool IsInRing(int i) { return (_pathset.BitIsSet(i)); }
...
class OBAPI OBRingSearch {
  OB_DEPRECATED std::vector<OBBond *>
      _bonds; //!< the internal list of closure bonds (deprecated)
  std::vector<OBRing *> _rlist; //!< the internal list of rings
  std::unique_ptr<OBRingSearchPrivate>
      _d; //!< opaque internal state (dedup index, etc.)
public:
  OBRingSearch();
  ~OBRingSearch();
...
public:
  ...
  int GetAtomIdx();
  //! Recursively find the root of this tree, building up a vector of OBAtom
  //! nodes.
