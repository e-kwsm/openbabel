...
class OBFPRT OBFingerprint : public OBPlugin {
  ...
  const char *TypeID() override { return "fingerprints"; }
...
public:
  ...
  /// \return the Tanimoto coefficient between two vectors (vector<unsigned
  /// int>& SeekPositions)
  ...
  static double Tanimoto(const std::vector<unsigned int> &vec1,
    ...
    /// If used for two vectors, vec1 and vec2, call as Tanimoto(vec1,
    /// &vec2[0]);
...
public:
  /// \return a pointer to a fingerprint (the default if ID is empty), or NULL
  /// if not available
...
struct OBFPRT FptIndexHeader {
  ...
  char seek64; // if true, seek data consists of 64bit long values (only zero in
               // legacy indices)
...
public:
  ...
  /// \brief Does substructure search and returns vector of the file positions
  /// of matches
  bool Find(OBBase *pOb, std::vector<unsigned long> &SeekPositions,
            unsigned int MaxCandidates);

  /// \brief Similar to Find() but all bits of matching fingerprints have to be
  /// the same \since version 2.1
  bool FindMatch(OBBase *pOb, std::vector<unsigned long> &SeekPositions,
                 unsigned int MaxCandidates);

  /// \return A multimap containing objects whose Tanimoto coefficients with the
  /// target is greater than the value specified.
...
/// \brief Class to prepare fingerprint index files See FastSearch class for
/// details
class OBFPRT FastSearchIndexer {
  // see end of cpp file for detailed documentation
public:
  ///\brief Constructor with a new index
  FastSearchIndexer(std::string &datafilename, std::ostream *os,
                    std::string &fpid, int FptBits = 0, int nmols = 0);
