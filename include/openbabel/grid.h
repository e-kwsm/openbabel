...
protected:
  double _xmin, _xmax, _ymin, _ymax, _zmin,
      _zmax; //!< the min/max axes in XYZ axes (i.e., the box)
...
public:
  ...
  bool PointIsInBox(double *c) {
    return (c[0] >= _xmin) && (c[0] <= _xmax) && (c[1] >= _ymin) &&
           (c[1] <= _ymax) && (c[2] >= _zmin) && (c[2] <= _zmax);
  }

  //! \return true if the point falls within the box
  bool PointIsInBox(vector3 v) {
    return (v.x() >= _xmin) && (v.x() <= _xmax) && (v.y() >= _ymin) &&
           (v.y() <= _ymax) && (v.z() >= _zmin) && (v.z() <= _zmax);
  }
};

//! \class OBFloatGrid grid.h <openbabel/grid.h>
//! \brief Handle double precision floating point 3D grids (e.g., charge density
//! around an OBMol)
//!
...
public:
  ...
  vector3 GetXAxis() const { return _xAxis; }

  //! \return Y axis direction.
  vector3 GetYAxis() const { return _yAxis; }

  //! \return Z axis direction.
  vector3 GetZAxis() const { return _zAxis; }
  ...
  void SetLimits(const vector3 &origin, const vector3 &x, const vector3 &y,
                 const vector3 &z);
  //! \deprecated Will be removed.
  //! \sa SetLimits(const vector3& origin, const vector3& x, const vector3& y,
  //! const vector3& z)
  ...
  //! \return Position of the center of the grid.
  vector3 Center() { return vector3(_midx, _midy, _midz); }
  ...
  //! \return the interpolated value for the given point and set the dx, dy, dz
  //! derivatives
...
//! \brief A grid for determining the proximity of a given point to atoms in an
//! OBMol \deprecated May be removed in the future, since docking is not a key
//! feature
...
public:
  ...
  void Setup(OBMol &mol, OBMol &box, double cutoff, std::vector<bool> &use,
...
// scoring function used: PLP = Piecewise Linear Potential or ChemScore
// algorithm
typedef enum { Undefined = -1, PLP, ChemScore } score_t;

//! \class OBScoreGrid grid.h <openbabel/grid.h>
//! \brief A base class for scoring docking interactions between multiple
//! molecules \deprecated Will disappear in future versions. Use your own code.
