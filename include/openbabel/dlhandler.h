...
public:
   ...
   * The appname.exe is removed and ..\\lib\\openbabel\\BABEL_VERSION\\ is
   * appended.
   *
   ...
   * The routine fills a vector of strings with the matching file names
   * (including path). Note that this is not recursive: it only matches files in
   * the specified path. For example, if path = e:\\path\\to\\ and pattern =
   * *.obf it will return vector entries lik e:\\path\\to\\cmlformat.obf \return
   ...
   * contains a wildcard.
   * The routine adds matching file names (including path) to a vector of
   * strings . \return the number of matching files. If no wildcard in name adds
   * name to vector and returns -1.
   */
  static int findFiles(std::vector<std::string> &file_list,
  ...
  //! \return The file extension pattern for Open Babel plugin modules (e.g.
  //! *.obf on Windows)
