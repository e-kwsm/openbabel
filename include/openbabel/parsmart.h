...
public:
  OBSmartsPattern()
      : _pat(nullptr), _buffer(nullptr), LexPtr(nullptr), MainPtr(nullptr) {}
  ...
  //! \param single Whether only a single match is required (faster). Default is
  //! false. \return Whether matches occurred
  ...
  bool Match(OBMol &mol, std::vector<std::vector<int>> &mlist,
  ...
  bool RestrictedMatch(OBMol &mol, std::vector<std::pair<int, int>> &pairs,
                       bool single = false);
  ...
  //! \return An iterator over the (non-unique) match list, starting at the
  //! beginning
...
public:
  ...
  bool match(OBMol &mol, const Pattern *pat,
             std::vector<std::vector<int>> &mlist, bool single = false);
};
