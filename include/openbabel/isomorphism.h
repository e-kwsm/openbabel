 ...
 * @since version 2.3
 * @brief Abstract class defining interface for isomorphism (i.e. substructure)
 * searches.
 *
 ...
 * the desired mapping function can be used to perform the mapping (i.e.
 * MapFirst(), MapUnique() or MapAll()).
 *
 * A typical example:
 * @code
 * OBMol *queried;
...
public:
   ...
   * @return OBIsomorphismMapper instance or 0 if there is no subclass
   * implementing the specified @p algorithm.
   */
  static OBIsomorphismMapper *
  GetInstance(OBQuery *query,
   ...
   * @param mask A mask to restrict the search to a part of the queried
   * molecule. The default empty mask will result in all atoms being considered.
   * The mask indexes start from 1 (i.e. OBAtom::GetIdx()).
   */
  virtual void MapFirst(const OBMol *queried, Mapping &map,
                        const OBBitVec &mask = OBBitVec()) = 0;
  /**
   * Find all unique mappings in @p queried. A mapping is unique when there is
   * no previous mapping covering the same queried atoms. For two mappings, some
   * overlap is allowed but at least one atom should be different.
   * @param queried The molecule to search.
   ...
   * The mask indexes start from 1 (i.e. OBAtom::GetIdx()).
   */
  virtual void MapUnique(const OBMol *queried, Mappings &maps,
                         const OBBitVec &mask = OBBitVec()) = 0;
  /**
   * Find all mappings in @p queried. This function is used by
   * FindAutomorphisms() with a query that is a copy of the queried molecule
   * (taking the mask into account).
   * @param queried The molecule to search.
   * @param maps Reference to the object to store the results in.
   * @param mask A mask to restrict the search to a part of the queried
   * molecule.
   * @param maxMemory Memory limit for the @p maps object in bytes. Default is
   * 300MB. The default empty mask will result in all atoms being considered.
   * The mask indexes start from 1 (i.e. OBAtom::GetIdx()).
   */
  virtual void MapAll(const OBMol *queried, Mappings &maps,
  ...
  /**
   * Find all mappings in @p queried. The functor will be called when a mapping
   * is found.
   ...
   * The mask indexes start from 1 (i.e. OBAtom::GetIdx()).
   */
  virtual void MapGeneric(Functor &functor, const OBMol *queried,
                          const OBBitVec &mask = OBBitVec()) = 0;
...
inline bool MapsTo(const OBIsomorphismMapper::Mapping &map,
                   unsigned int queryIndex, unsigned int &queriedIndex) {
 ...
 * for certain graphs can be large. The default is 300MB, consider using a
 * functor to process automorphisms when they are found.
 *
 * @since version 2.3
 */
OBAPI bool FindAutomorphisms(OBMol *mol,
 ...
 * are provided to find and store all automorphisms but the number of
 * automorphisms can be large for certain graphs making it not feasible to store
 * all automorphisms in memory (RAM).
 *
 * @see  @ref MapGeneric
 * @since version 2.3
 */
 ...
 * the vertexes (i.e. atoms) from the query to vertexes in the molecule such
 * that two vertexes adjacent in the query are also adjacent in the target
 * molecule. In other words, no bonds are broken and no new bonds are formed.
 *
 * @section smarts SMARTS Substructure Search
 * Smarts is an extension of smiles to create powerful queries. Smarts
 * substructure search has been available in OpenBabel for many years. It is
 * also used for many of OpenBabel's algorithms. Although smarts is only a
 * syntax for queries, the implementation has it's own matching algorithm. For
 * many purposes smarts are the easiest way to do substructure searches. See the
 * OBSmartsPattern documentation for details on how to use smarts.
 *
 * @section query Queries
 * Since OpenBabel version 2.3, there are some classes for representing generic
 * queries. The OBQuery, OBQueryAtom and OBQueryBond class define interfaces
 * that can be reimplemented to get custom behavior. The classes also contain
 * some methods to access topological information which are used by the mapping
 ...
 *   public:
 *     MyCustomFunctor(std::vector<OBIsomorphismMapper::Mapping> &data) :
 * m_data(data) {} bool operator()(OBIsomorphismMapper::Mapping &map)
 *     {
 ...
 * automorphisms make it easy to take symmetry into account. See
 * FindAutomorphisms for detials.
 *
 *
 */
