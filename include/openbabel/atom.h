...
protected:
  unsigned char _ele;  //!< atomic number (type unsigned char to minimize space
                       //!< -- allows for 0..255 elements)
  ...
  short _spinmultiplicity;     //!< atomic spin, e.g., 2 for radical  1 or 3 for
                               //!< carbene

  unsigned int _idx; //!< unique node index (GetIdx(), SetIdx())
  OBMol *_parent;    //!< parent molecule (if any)
  std::vector<OBBond *>
      _vbond; //!< bonds to this atom -- assumed to be one of the endpoints
...
public:
  ...
  //! Set isotope number (actual atomic weight is tabulated automatically, 0 =
  //! most abundant)
  ...
  //! Set the position of this atom based on the internal pointer array (i.e.
  //! from SetCoordPtr() )
  void SetVector();
  ...
  //! \return The total number of bonds to this atom including bonds to implicit
  //! hydrogens
  unsigned int GetTotalDegree() const {
    return (unsigned int)(_vbond.size() + _imph);
  }
  //! \return The sum of the bond orders of the explicit bonds to this atom
  unsigned int GetExplicitValence() const;
  //! \return The sum of the bond orders of all bonds to this atom including
  //! bonds to implicit hydrogens
  unsigned int GetTotalValence() const;
  //! \return The hybridization of this atom: 1 for sp, 2 for sp2, 3 for sp3, 4
  //! for sq. planar, 5 for trig. bipy, 6 for octahedral
  ...
  //! \return the partial charge of this atom, calculating a Gasteiger charge if
  //! needed
  ...
  //! Create a vector for a new bond from this atom, with length given by the
  //! supplied parameter \return success or failure
  ...
  //! \return A range over the bonds to this atom. This range can be used in a
  ...
  //! \return A range over the neighbors of this atom. This range can be used in
  //! a range-based for loop.
  OBAtomAtomRange GetNbrs() const {
    return {{_vbond.begin(), this}, {_vbond.end()}};
  }
  //! Set the iterator @p i to the beginning of the bonds
  ...
  void InsertBond(OBBondIterator &i, OBBond *bond) { _vbond.insert(i, bond); }
  //! Find @p bond and remove it from the internal list. Does not update the
  //! bond.
  ...
  //@{
  //! \return The number of oxygen atoms connected that only have one heavy
  //! valence
  unsigned int CountFreeOxygens() const;
  //! \return The number of sulfur atoms connected that only have one heavy
  //! valence \since version 2.4
  ...
  //! \return The size of the smallest ring that contains this atom (0 if not in
  //! a ring)
   ...
   *  \return A pair of integers, where first is acid count and second is base
   * count \since version 2.3
   */
  ...
  //! \return Is the specified element, as specified by atom number (see
  //! OBElement namespace)?
  bool IsElement(const unsigned int e) const { return e == _ele; }
  ...
  bool IsAxial();
  //! \return Is this atom a hydrogen-bond acceptor  (considering also atom
  //! surrounding)
