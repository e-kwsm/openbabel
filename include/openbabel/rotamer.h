...
//! \brief Supports a set of rotamer coordinate sets for some number of
...
public:
  ...
  //! Set up a rotamer list based on the supplied reference atoms and the number
  //! of rotors \param mol The molecule to evaluate \param ref An array of the 4
  //! dihedral atoms for each rotor \param nrotors The number of rotors (i.e.,
  //! the size of ref / 4)
  ...
  //! Add a rotamer to the list based on the supplied coordinate set as a
  //! double*
  void AddRotamer(double *);
  //! Add a rotamer to the list based on @p key as a configuration of the
  //! individual rotor bonds
  ...
  //! Add @p nconf rotamers based on @p as an array of configurations much like
  //! AddRotamer()
  void AddRotamers(unsigned char *arr, int nconf);
  //! \return A reference array (as used by AddRotamer() as a configuration of
  //! the individual rotor bonds
  ...
  //! \return The number of "base" coordinate sets (i.e., the number of
  //! conformers in the base OBMol)
