 ...
 *
 * The OBConformerFilter class defines an interface used by the
 * OBConformerSearch class. A conformer filter is used as a first selection
 * method when generating conformers. A typical example is a steric filter to
 * ensure atoms are never closer than some specified distance.
 *
...
public:
   ...
   */
  virtual bool IsGood(const OBMol &mol, const RotorKey &key,
                      double *coords) = 0;
...
/**
 * @class OBStericConformerFilter conformersearch.h
 * <openbabel/conformersearch.h>
 * @brief A steric conformer filter class.
 *
 * The OBStericConformerFilter class is the default filter used by
 * OBConformerSearch. The filter removes all molecules which have at least 2
 * atoms closer together than the specified distance.
 * @since 2.3
 */
class OBAPI OBStericConformerFilter : public OBConformerFilter {
public:
  OBStericConformerFilter();
  OBStericConformerFilter(double cutoff, double vdw_factor = 0.5,
 ...
 *
 * The OBConformerScore class defines an interface to assign scores to
 * conformers. This class is used by OBConformerSearch to make the class
 * flexible. A higher score means the conformer with index is more favourable. A
 * typical example is the force field energy to find the lowest energy conformer
 * (note: the energy is not directly usable as score since lower values are
 * better). Another example is to use some measure of diversity (e.g. RMSD) to
 * generate a diverse set of conformers.
 * @since 2.3
 */
...
/**
 * @class OBMinimizingEnergyConformerScore conformersearch.h
 * <openbabel/conformersearch.h>
...
/**
 * @class OBMinimizingRMSDConformerScore conformersearch.h
 * <openbabel/conformersearch.h>
 * @brief An RMSD conformer scoring class, after a short minimization
 *
 * This scores conformers by the RMSD between the conformer and the closest, to
 * produce a diverse set of conformers, but after minimization. This ensures
 * each conformer is "reasonable" and avoids steric clashes.
 * @since 2.4
 */
...
public:
   ...
   * @param numConformers The number of conformers that should be generated.
   * This is also the number of conformers selected for each generation.
  ...
  /**
   * Set the convergence (i.e. number of generations that did not change the
   * score before considering the iteration converged).
   */
  ...
  /**
   * Set the filter method used to check if a newly generated is acceptable.
   * Typical examples are a steric filter or electrostatic energy filter. The
   * filters make a binary selection and one group will be scored in the next
   * step. A number of filters are provided by default but implementing a custom
   * filter is easy.
   *
   * There is also a OBConformerFilters class to make it easy to use multiple
   * filters.
   */
  void SetFilter(OBConformerFilter *filter) { m_filter = filter; }
  /**
   ...
   * conformer i and the closest conformer to create a diverse set of
   * conformers. Diversity could also be expressed in other ways and
   * implementing a scoring class is easy.
   */
  void SetScore(OBConformerScore *score) { m_score = score; }

  /**
   * Set whether or not you want rotors to be printed prior to the conformer
   * search.
   */
  ...
  const RotorKeys &GetRotorKeys() const { return m_rotorKeys; }

  void GetConformers(OBMol &mol);

  /* @brief Set an output stream for logging. If NULL pointer is provided,
   * logging is disabled. */
...
private:
  ...
  //! @brief Score, sort the current population, compute shared fitness values
  //! (and niches)
  ...
  unsigned int m_numConformers; //!< The desired number of conformers. This is
                                //!< also the population size.
  ...
  int m_convergence; //!< Number of generations that remain unchanged before
                     //!< quiting

  std::vector<double> vscores; //!< Current population score vector
  std::vector<double>
      vshared_fitnes; //!< Current population shared fitness vector
  std::vector<std::vector<int>>
      dynamic_niches; //!< The dynamically found niches
  std::vector<int>
      niche_map; //!< Procide the sharing niche index, given the key inddex

  void *d; // Opaque pointer - currently for storing OBRandom* which may be
           // removed in future
  ...
  double niche_radius;     //!< A pre-determined niche radius, for dynamic niche
                           //!< sharing.
  double p_crossover;      //!< Crossover probability
  double niche_mating;     //!< Probability of forcing the second parent in the
                           //!< first parent
  int local_opt_rate;      //!< Perform a random local optimization every
                           //!< local_opt_rate generations. Disabled if set to
  ...
  bool m_printrotors;      //!< Wheter or not to print all rotors that are found
                           //!< instead of performing the conformer search

  OBConformerFilter *m_filter;
  OBConformerScore *m_score;

  std::ostream
      *m_logstream; //!< A pointer to a log stream (NULL means no loogging)
};
 ...
 * All conformer searching methods in OpenBabel are based on the concept of
 * rotor keys. A rotor key is simply an array of values specifying the rotations
 * around rotatable bonds. The number of possible values for a rotatable bond
 * depends on the bond type. The classes implementing the rotor key concept are
 * OBRotor, OBRotorList and OBRotamer.
 *
 * Previous OpenBabel releases contained only methods for finding stable (low
 * energy) conformers by using the force fields. The 2.3 release introduces a
 * new flexible class (OBConformerSearch) implementing a genetic algorithm. The
 * scoring or ranking of conformers is done by a separate class derived from the
 * abstract OBConformerScore class. Reimplementing this class allows for all
 * sorts of scoring functions (e.g. RMSD, torson, energy, ... based).
 *
 * @section stable Finding stable conformers
 *
 * @subsection systematic Systematic rotor search
 * This is the simplest of all conformer searching methods and only suitable
 * when there are only a few rotatable bonds. Since the number of conformers
 * grows exponentially with the number of bonds, enumerating all conformers for
 * reasonably sized molecules quickly results in unacceptable performance. A
 * systematic rotor search can be done with the
 * OBForceField::SystematicRotorSearch method.
 *
 ...
 * to optimize good scoring keys. This allows the global minimum energy
 * conformer to be identified much faster compared to the random rotor search.
 * However, finding the global minimum can be time consuming for large molecules
 * and there is no guarantee that the found minimum is the global minimum. A
 * weighted rotor search is performed with the OBForceField::WeightedRotorSearch
 * method.
 *
 ...
 * with a scoring function like OBRMSDConformerScore. A diverse set of
 * conformers is often useful when screening bioactive molecules since the
 * interaction with the target can stabilize a higher conformational energy. See
 * the next section for details.
 *
 * @section OBConformerSearch
 * The genetic algorithm starts by generating the initial population of rotor
 * keys. The initial population contains up to numConformers, duplicate keys are
 * ignored. OBConformerFilter objects are used to filter conformers before
 * including them. A typical filter is a steric filter to ignore all conformers
 * with atoms to close together.
 *
 * For each generation, numChildren children are created by permuting the parent
 * rotor keys. The mutability setting determines how frequent a permutation is
 * made (e.g. 5 means 1/5 bonds are permuted, 10 means 1/10). Again, duplicated
 * and filtered molecules are ignored. The population now contains up to
 * numConformer * (1 + numChildren). All these rotor keys are scored using the
 * specified OBConformerScore class. Next, all keys are ordered by their score
 * and the best numConformers conformers are selected as parents for the next
 * generation.
 *
 ...
 * OBStericConformerFilter filters out all conformers with atoms to close
 * together. Custom filters can be used to consider additional properties (e.g.
 * electrostatic energy, distance between pharmacophore groups, ...). Filters
 * can easily be combined using the OBConformerFilters class.
 *
 * @subsection OBConformerScore
 * OBConformerScore derived classes compute the score for conformers. The
 * scoring function is probably the most important part of the algorithm since
 * it selects which conformers will be used as parents for the next generation.
 * Derived classes also specify if the scores are descending or ascending and
 * what convergence criteria should be used (i.e. lowest, highest, sum or
 * average). There is no easy way to combine scoring functions since scores are
 * not always additive. However, a custom scoring function could be used that
 * combines several other scoring functions with weights for each score.
 *
 * The scoring function determines what kind of conformers are selected. For
 * example, the OBEnergyConformerScore class selects stable, low energy
 * conformers. A simple variation to this would be to only use the non-bonded
 * energy. The result for these two scoring functions should be similar.
 *
 * The OBRMSDConformerScore scoring function can be used to generate a diverse
 * set of conformers. A diverse set is often useful when screening bioactive
 * molecules since the interaction with the target can stabilize a higher
 * conformational energy. To calculate the RMSD between conformers, Kabsch
 * alignment (OBAlign) is used. The alignment takes symmetry into account.
 *
 * @image html energyconformerscore.png
 * @image html rmsdconformerscore.png "30 Conformers for methotrexate. top:
 * OBEnergyConformerScore bottom: OBRMSDConformerScore"
 */
