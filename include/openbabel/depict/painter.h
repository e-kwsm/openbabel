...
public:
  OBColor() { *this = OBColor(0.0, 0.0, 0.0); }
  ...
  OBColor(const std::string &color) {
    if (color[0] == '#') {
      ...
      unsigned c;
      ss >> std::hex >> c;
      *this = OBColor((c / 0x10000) / 256.0, ((c % 0x10000) / 0x100 / 256.0),
                      (c % 0x100) / 256.0);
      return;
    }
    if (color == "black")
  ...
  OBColor(std::vector<double> vec)
      : red(vec[0]), green(vec[1]), blue(vec[2]), alpha(1.0) {}
...
public:
   ...
   */
  virtual void
  DrawPolygon(const std::vector<std::pair<double, double>> &points) = 0;
