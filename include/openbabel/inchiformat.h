...
public:
  ...
  const char *Description() override {
    return "InChI format\n"
           ...
           "    Standard InChI is written unless certain InChI options are "
           "used\n \n"
           ...
           "    **Uniqueness options** (see also ``--unique`` and ``--sort`` "
           ...
           "    This can also be done with :ref:`InChICompare format "
           "<Compare_molecules_using_InChI>`::\n \n"
           ...
           "    These options should be space delimited in a single quoted "
           "string.\n \n"
  ...
  static char CompareInchi(const std::string &Inchi1,
                           const std::string &Inchi2);
  static std::string InChIErrorMessage(const char ch);

  /// Removes layers or truncates InChi, according to \param spec
  /// which can contain any number of:/formula /connect /nostereo /nosp3 /noEZ
  /// /nochg /noiso
  ...
  struct InchiLess {
    bool operator()(const std::string &s1, const std::string &s2) const {
      ...
      std::string::const_iterator p1 = s1.begin(), p2 = s2.begin(),
                                  p1end = find(s1.begin(), s1.end(), ' '),
                                  p2end = find(s2.begin(), s2.end(), ' ');
...
private:
  /// Erases the layer starting with \param str and, if \param all is true, all
  /// the subsequent ones
  static void RemoveLayer(std::string &inchi, const std::string &str,
                          bool all = false);
...
public:
  InChICompareFormat() { OBConversion::RegisterFormat("k", this); }
  virtual const char *Description() // required
  {
    return "Compare molecules using InChI\n"
           "A utility format that allows you to compare molecules using their "
           "InChIs\n"
           "The first molecule is compared with the rest, e.g.::\n\n"

           "  babel first.smi second.mol third.cml -ok\n\n"

           "This is the same as using ``-oinchi -xet`` and can take the same "
           "options as InChI format\n"
...
public:
  InChIKeyFormat() { OBConversion::RegisterFormat("inchikey", this); }
  virtual const char *Description() // required
  {
    return "InChIKey\n"
           ...
           "An InChIKey consists of 14 characters (derived from the "
           "connectivity\n"
           "layer in the InChI), a hyphen, 9 characters (derived from the\n"
           "remaining layers), a character indicating the InChI version, a "
           "hyphen\n"
           "and a final checksum character. Contrast the InChI and InChIKey of "
           "the\n"
           ...
           "This is the same as using ``-oinchi -xK`` and can take the same "
           "options\n"
           ...
           "Note that while a molecule with a particular InChI will always "
           "give the\n"
           "same InChIKey, the reverse is not true; there may exist more than "
           "one\n"
           "molecule which have different InChIs but yield the same "
           "InChIKey.\n";
  }
  virtual bool WriteMolecule(OBBase *pOb, OBConversion *pConv);
  virtual unsigned int Flags() { return NOTREADABLE; };
