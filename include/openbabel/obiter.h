...
public:
  ...
  //! \return Whether the iterator can still advance (i.e., visit more
  //! neighbors)
  operator bool() const { return _ptr != nullptr; }
...
public:
  ...
  //! \return Whether the iterator can still advance (i.e., visit more atoms in
  //! this residue)
  operator bool() const { return _ptr != nullptr; }
  //! Preincrement -- advance to the next atom (if any) and return
  OBResidueAtomIter &operator++();
  //! Postincrement -- return the current state and advance to the next atom (if
  //! any)
...
public:
  ...
  //! \return Whether the iterator can still advance (i.e., visit more 1-4 atom
  //! pairs)
...
inline OBMolAtomRange MolGetAtoms(const OpenBabel::OBMol &mol) {
  return mol.GetAtoms();
}
inline OBMolAtomRange MolGetAtoms(const OpenBabel::OBMol *mol) {
  return mol->GetAtoms();
...
inline OBMolBondRange MolGetBonds(const OpenBabel::OBMol &mol) {
  return mol.GetBonds();
}
inline OBMolBondRange MolGetBonds(const OpenBabel::OBMol *mol) {
  return mol->GetBonds();
...
inline OBAtomBondRange AtomGetBonds(const OpenBabel::OBAtom &atom) {
...
inline OBAtomAtomRange AtomGetNbrs(const OpenBabel::OBAtom &atom) {
  return atom.GetNbrs();
}
inline OBAtomAtomRange AtomGetNbrs(const OpenBabel::OBAtom *atom) {
  return atom->GetNbrs();
...
#define FOR_ATOMS_OF_RESIDUE(a, r)                                             \
