...
public:
  ...
  //@{
  /*! The mol object contains all connectivity information (atomic numbers,
   * bonds, bond orders, ..) but no 3D coordinates. Build generates these
   * coordinates and assigns them. \param mol Molecule with the connectivity
   * (from SMILES for example). The coordinates are also changed in this mol.
   *  \param stereoWarnings Warn if the stereochemistry is incorrect (default is
   * true)
   */
  ...
  //@{
  /*! If the molecule already contains 3D coordinates, if you set KeepRings to
   * true it will use retain the 3D coordinates of the rings. By default
   * KeepRings is false, and ring conformations are obtained by lookup in a
   * library of ring conformers. However, since the ring conformer library is
   * not exhaustive, if the ring system is not found in the library, the
   * resulting 3D structure can be poor, and require geometry optimisation
   * before it is reasonable. If your starting point is a 3D structure, you can
   * set KeepRings to true, and the conformation will be taken from the input.
  ...
  //! Used by LoadFragments to check for invalid (all zero coordinates)
  //! fragments
   ...
   */
  static bool Connect(OBMol &mol, int a, int b, vector3 &newpos,
                      int bondOrder = 1);
  /*! Atoms a and b are part of two fragments that are not connected in mol.
   *  Connect will translate and rotate the fragment that contains b so that
   *  a and b are separated by a bond. This bond is also added.
  ...
  /*! Swap group b, bonded to a with group d, bonded to c. The bonds a-b and b-c
   * cannot be part of a ring. Atoms a and b will not be moved. Atoms b, d and
   * their connected atoms (after deleting bonds ab and cd) will be
   * translated/rotated.
   *
   *  Example:
   *  \code
   ...
   *
   *
   *  This function can also be used to invert chiral centers if a and c are the
   * same atom.
   *
   *  Example
   *  \code
  ...
  /*! Atoms a and b must be bonded and this bond cannot be part of a ring. The
   * bond will be broken and the smiles fragment will be inserted bewteen the
   * two remaining fragments. The fragment that contains a will not be
   * translated or rotated. Parameters c and d are the index in the smiles to
   * which atoms a and b will be connected respectivly.
   *
   */
...
private:
  ...
  static std::vector<std::pair<OBSmartsPattern *, std::vector<vector3>>>
      _ring_fragments;
  ...
  static void ConnectFrags(OBMol &mol, OBMol &workmol, std::vector<int> match,
                           std::vector<vector3> coords, std::vector<int> pivot);
  ...
  static bool SwapWithVector(OBMol &mol, int a, int b, int c,
