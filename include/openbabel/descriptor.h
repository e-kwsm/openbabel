...
public:
  ...
  /// Provides a string value for non-numeric descriptors and returns NaN, or a
  /// string representation and returns a numeric value
  virtual double GetStringValue(OBBase *pOb, std::string &svalue,
                                std::string *param = nullptr);

  /// Parses the filter stream for a relational expression and returns its
  /// result when applied to the chemical object
  virtual bool Compare(OBBase *pOb, std::istream &ss, bool noEval,
                       std::string *param = nullptr);

  /// Write information on a plugin class to the string txt.
  /// If the parameter is a descriptor ID, displays the verbose description for
  /// that descriptor only
  ///  e.g. babel -L descriptors HBA1
  bool Display(std::string &txt, const char *param,
  ...
  /// Interprets the --filter option string and returns the combined result of
  /// all the comparisons it contains
  ...
  /// Deletes all the OBPairDatas whose attribute names are in the list (if they
  /// exist).
  ...
  /// values, each precede by a space or the first character in the list if it
...
protected:
  static double ParsePredicate(std::istream &optionText, char &ch1, char &ch2,
                               std::string &svalue);
  ...
  /// Makes a comparison using the operator and a string read from the filter
  /// stream with a provided string.
  ///  \return the result of the comparison and true if NoCompOK==true and there
  ///  is no comparison operator.
  static bool CompareStringWithFilter(std::istream &optionText, std::string &s,
                                      bool noEval, bool NoCompOK = false);
