...
public:
  ...
  static void RegisterXMLFormat(XMLBaseFormat *pFormat, bool IsDefault = false,
  ...
  /// Because OBConversion::Convert is still using the unextended OBConversion
  /// object we need to obtain the conversion parameters from it when requested
  bool IsLast() { return _pConv->IsLast(); }
  int GetOutputIndex() { return _pConv->GetOutputIndex(); }

  xmlTextReaderPtr GetReader() const { return _reader; };

  xmlTextWriterPtr GetWriter() const { return _writer; };

  void OutputToStream() { xmlOutputBufferFlush(_buf); }

  static XMLBaseFormat *
  GetDefaultXMLClass() // TODO make dependent on object type
  {
    return _pDefault;
  };

  void LookForNamespace() { _LookingForNamespace = true; };
  ...
  /// Sets value to element content as an integer. Returns false if there is no
  /// content.
  bool GetContentInt(int &value);

  /// Sets value to element content as an double. Returns false if there is no
  /// content.
...
protected:
  xmlTextReaderPtr reader() const { return _pxmlConv->GetReader(); }

  xmlTextWriterPtr writer() const { return _pxmlConv->GetWriter(); }

  void OutputToStream() { _pxmlConv->OutputToStream(); }
...
public:
  ...
  const std::type_info &GetType() { return typeid(OBMol *); };
};
