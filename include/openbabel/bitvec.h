...
#define WORDSIZE_OF_BITSIZE(bit_size)                                          \
  ((bit_size >> WORDROLL) + ((bit_size & WORDMASK) ? 1 : 0))
...
public:
  ...
  */
  OBBitVec() : _set(STARTWORDS, 0) { _size = _set.size(); }
  ...
  /** Construct a bit vector with a size in bits
      of \p size_in_bits rounded up to the nearest word
          ...
          \param[in]	size_in_bits The number of bits for which to reserve
     space
  */
  OBBitVec(unsigned size_in_bits) : _set(WORDSIZE_OF_BITSIZE(size_in_bits), 0) {
    _size = _set.size();
  }
  ...
  */
  OBBitVec(const OBBitVec &bv) : _size(0) { (*this) = bv; }
  ...
  /** Searches the vector for the first true value, starting at the \p
     bit_offset 'th bit \param[in] bit_offset the first bit to consider \return
     the bit offset of the first true bit at or after \p bit_offset, or -1 if
     there is none
      */
  ...
  */
  void Negate() {
    for (word_vector::iterator wx = _set.begin(), wy = _set.end(); wx != wy;
         ++wx)
      *wx = ~(*wx);
  }
...
/// The Tanimoto coefficient, which may be regarded as the proportion of the
/// "on-bits" which are shared.
