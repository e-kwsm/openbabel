...
public:
  ...
  const std::string &GetValue() const override { return (_data); }
};
...
//! \brief Used to store information on external bonds (e.g., in SMILES
//! fragments)
...
public:
  ...
  void SetData(std::vector<OBGenericData *> &vdata) { _vdata = vdata; }
  ...
  //! Gets the entire set.
  virtual const std::vector<OBGenericData *> &
  GetData() const // now virtual and const
  {
    return (_vdata);
  }

  //! Get the begin iterator.
  std::vector<OBGenericData *>::iterator GetBegin() { return _vdata.begin(); }

  //! Get the end iterator.
  std::vector<OBGenericData *>::iterator GetEnd() { return _vdata.end(); }
...
public:
  ...
  /*!
  **\brief Constructs the cell matrix in lower triangular form from the values
  *supplied
  ...
  */
  ...
  //! \return The orthogonalization matrix, used for converting from fractional
  //! to Cartesian coords. \see OBUnitCell::GetCellMatrix \see
  //! OBUnitCell::GetFractionalMatrix \see OBUnitCell::GetOrientationMatrix \see
  //! OBUnitCell::FractionalToCartesian \see OBUnitCell::CartesianToFractional
  ...
  //! \return The fractionalization matrix, used for converting from Cartesian
  //! to fractional coords. \see OBUnitCell::GetOrthoMatrix \see
  //! OBUnitCell::GetCellMatrix \see OBUnitCell::GetOrientationMatrix \see
  //! OBUnitCell::FractionalToCartesian \see OBUnitCell::CartesianToFractional
  ...
  //!
  //! GetFractionalMatrix() * GetOrientationMatrix().inverse() * (cart -
...
public:
  ...
  size_t GetSize() const { return _torsions.size(); }
...
public:
  ...
  void SetAngle(double angle) { _radians = angle; }
...
public:
  ...
  size_t GetSize() const { return _angles.size(); }
};

//! \class OBSerialNums generic.h <openbabel/generic.h>
//! \brief Defines a map between serial numbers (e.g., in a PDB file) and OBAtom
//! objects inside a molecule
...
public:
  ...
  void SetData(double, const std::vector<double> &, const std::vector<double> &,
               const std::vector<double> &);
...
//! \brief Used to store energy, occupation, and orbital symmetry of a
//! particular orbital \sa OBOrbitalData
class OBAPI OBOrbital {
  friend class OBOrbitalData;

protected:
  double _energy;     //!< in electron volts
  double _occupation; //!< usually 0, 1, or 2, but can be fractional (e.g.,
                      //!< solid-state calcs)
  std::string _mullikenSymbol; //!< symmetry designation
public:
  void SetData(double energy, double occupation = 2.0,
               std::string symbol = "A") {
    _energy = energy;
...
public:
  ...
  //! \brief Convenience function for common cases of closed-shell calculations
  //! -- pass the energies and symmetries This method will fill the OBOrbital
  //! objects for you
  void LoadClosedShellOrbitals(std::vector<double> energies,
                               std::vector<std::string> symmetries,
                               unsigned int alphaHOMO);
  //! \brief Convenience function to load alpha orbitals in an open-shell
  //! calculation
  void LoadAlphaOrbitals(std::vector<double> energies,
                         std::vector<std::string> symmetries,
                         unsigned int alphaHOMO);
  //! \brief Convenience function to load beta orbitals in an open-shell
  //! calculation
...
protected:
  std::vector<OBOrbital>
      _alphaOrbitals; //!< List of orbitals. In case of unrestricted
                      //!< calculations, this contains the alpha spin-orbitals
  std::vector<OBOrbital>
      _betaOrbitals; //!< Only used if needed (e.g., unrestricted calculations)
  unsigned int
      _alphaHOMO; //!< Highest occupied molecular orbital for _alphaOrbitals
  unsigned int _betaHOMO; //!< Highest occupied for _betaOrbitals (if needed)
  bool _openShell; //!< Whether we store both alpha and beta spin-orbitals
                   //!< (i.e., a restricted open-shell or unrestricted calc.)
};
...
public:
  OBElectronicTransitionData()
      : OBGenericData("ElectronicTransitionData",
...
//! \brief Handle double precision floating point data with coordinates not on a
//! grid Can hold array of random coordinates and associated values.
class OBAPI OBFreeGrid : public OBGenericData {
protected:
  std::vector<OBFreeGridPoint *>
      _points; //!< coordinates with accompanying float values
public:
  OBFreeGrid() {}
  ...
  int NumPoints() { return (int)_points.size(); }

  void AddPoint(double x, double y, double z, double V) {
    _points.push_back(new OpenBabel::OBFreeGridPoint(x, y, z, V));
  }

  OBFreeGridPointIterator BeginPoints() { return _points.begin(); }

  OBFreeGridPointIterator EndPoints() { return _points.begin() + NumPoints(); }

  OBFreeGridPoint *BeginPoint(OBFreeGridPointIterator &i) {
    i = _points.begin();
    return ((i == _points.end()) ? (OBFreeGridPoint *)nullptr
                                 : (OBFreeGridPoint *)*i);
  }
...
public:
  ...
  void AddPartialCharge(std::vector<double> q) { _PartialCharge = q; }

  std::vector<double> GetPartialCharge() { return _PartialCharge; }
};

//! A standard iterator over vectors of OBGenericData (e.g., inherited from
//! OBBase)
