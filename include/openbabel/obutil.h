...
#endif
#endif
// Include C++ version for clock() and CLOCKS_PER_SEC (required for GCC 12+ with
// C++17)
#include <ctime>
#include <math.h>
...
public:
  OBSqrtTbl() : _max(0.0), _incr(0.0), _tbl(nullptr) {}
  //! \brief Create a square root table to handle up to the square root of @p
  //! max (e.g., if you want the square root of 144, supply 12 for max)
  ...
  //! \param max The maximum square root stored in the lookup table (e.g., if
  //! you want the square root of 144, supply 12 for max)
  //! \param incr The floating point resolution of the lookup table
...
/*! Absolute-tolerance comparison for floats/doubles: returns fabs(a - b) <
 * epsilon Provided for backwards compatibility. For relative-tolerance
 * comparisons, see IsApprox(); for absolute checks, prefer an inline
 * std::fabs(a - b) < eps.
 * \deprecated Prefer std::fabs(a - b) < eps (absolute) or IsApprox() (relative)
 */
OB_DEPRECATED_MSG(
    "Prefer std::fabs(a - b) < eps, or IsApprox() for relative comparisons")
...
template <class T1, class T2, class T3, class T4> struct quad {
  // type names for the values
  typedef T1 first_type;
  typedef T2 second_type;
  typedef T3 third_type;
