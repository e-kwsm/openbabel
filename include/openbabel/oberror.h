...
enum obMessageLevel {
  ...
  obAuditMsg, //!< for messages auditing methods which destroy or perceive
              //!< molecular data (e.g., kekulization, atom typing, etc.)
...
public:
  ...
  OBError(const std::string &method = "", const std::string &errorMsg = "",
...
protected:
  //! The method causing the error (typically from the compiler macro
  //! __FUNCTION__)
  std::string _method;
...
protected:
  ...
  //! Filtering level for messages and logging (messages of lower priority will
  //! be ignored
...
//! \brief A minimal streambuf derivative to wrap calls to cerr into calls to
