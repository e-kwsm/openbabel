...
/** \todo
   ...
   - There's a bug in replace(sibling_iterator, ...) when the ranges
     ...
     but has been avoided now.
        - "std::operator<" does not work correctly on our iterators, and for
   some reason a globally defined template operator< did not get picked up.
...
template <class T1> void constructor(T1 *p) { new ((void *)p) T1; }

template <class T1> void destructor(T1 *p) { p->~T1(); }
...
void tree<T, tree_node_allocator>::head_initialise_() {
  head =
      alloc_.allocate(1, nullptr); // MSVC does not have default second argument
...
tree<T, tree_node_allocator>::leaf_iterator::operator++() {
  ...
  if (this->node->first_child !=
      nullptr) { // current node is no longer leaf (children got added)
    while (this->node->first_child)
      this->node = this->node->first_child;
  } else {
    while (this->node->next_sibling == nullptr) {
      if (this->node->parent == nullptr)
        return *this;
      this->node = this->node->parent;
      if (top_node != nullptr && this->node == top_node)
        return *this;
    }
    this->node = this->node->next_sibling;
    while (this->node->first_child)
