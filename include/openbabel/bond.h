...
public:
  enum Flag {
    ...
    Closure = (1 << 10)  //!< A bond which "closes" a ring when walking the
                         //!< molecular graph
  };
  enum StereoFlag {
    Wedge = (1 << 2), //!< A solid black wedge in 2D representations -- i.e.,
                      //!< "up" from the 2D plane
    Hash = (1 << 3),  //!< A dashed "hash" bond in 2D representations -- i.e.,
                      //!< "down" from the 2D plane
    WedgeOrHash =
        (1
         << 11), //!< The bond is either wedge or hash, this is a separate flag!
    CisOrTrans =
        (1
         << 12) //!< Indicates the 2D/3D coordinates are accidentally cis/trans.
  };
  ...
  //! Set the bond order to @p order (i.e., 1 = single, 2 = double, 5 =
  //! aromatic)
  ...
  //! \return The atom index for the end atom in this bond (from
  //! OBAtom::GetIdx()
  ...
  //! Find the smallest ring containing this bond (returns a NULL pointer if
  //! none exists)
  ...
  /**  Currently, this function classifies any bond with at least one heavy
       ...
       rings with more than 3 atoms may be included. No other bond typing is
     attempted. For more detailed rotor detection, check the OBRotorList and
  ...
  /** \return Is the bond a primary amide (i.e., between carbonyl C and a NH2)?
       In versions prior to 2.3, this function incorrectly identified secondary
     amides. **/
  bool IsPrimaryAmide();
  /** \return Is the bond a secondary amide (i.e., between a carbonyl C and a
     NH1)? In versions prior to 2.3, this function incorrectly identified
     tertiary amides. **/
  bool IsSecondaryAmide();
  //! \return Is the bond a teriary amide (i.e., between a carbonyl C and a
  //! NH0)?
  //!  \since version 2.3.
...
inline OBAtom *OBAtomAtomIterAdaptor::operator*() const {
  auto bond = *m_iter;
  return m_atom != bond->GetBeginAtom() ? bond->GetBeginAtom()
                                        : bond->GetEndAtom();
}
