...
private: // internal methods
  ...
  //@{
  /**
   * Determine connected chains (e.g., subunits). Chains will be labeled A, B,
   * C, ... Ligands also get assigned a chain label. The chain for ligands will
   * later be replaced by ' '. The residue numbers will also be updated in this
   * process to make sure all ligands, HOH, ions, etc. have a unique residue
   * number in the ' ' chain.
   *
   ...
   * @param i Index for the current atom. (RecurseChain() will be called for all
   * neighbours)
  ...
  /**
   * First the bitmasks[i] will be OR-ed with Template::flag for all atoms based
   * on on Template::element and Template::count.
   *
   ...
   */
  bool Match4Constraints(Template *templ, OBAtom *na, OBAtom *nb, OBAtom *nc,
                         OBAtom *nd);
  /**
   ...
   * @param mol The molecule.
   * @param i Index for the current atom. (TracePeptideChain() will be called
   * for all neighbours)
   ...
   *
   * Sets resnos[i] and resids[i] for all identified residues (including the
   * N-CA-C-O). (through IdentifyResidue() and AssignResidue())
   */
   ...
   * the seed CA/C1 atom.
   * @param tree Bytecode for the residues. (OBChainsParser::PDecisionTree or
   * OBChainsParser::NDecisionTree)
   * @param mol The molecule.
   ...
   */
  int IdentifyResidue(void *tree, OBMol &mol, unsigned int seed,
                      int resno); // ByteCode *
  /**
  ...
  //@{
  /**
   * Convert the private data vectors to OBResidue objects and add them to @p
   * mol.
   * @param mol The molecule to parse and update
   * @param nukeSingleResidue If only one residue is found, clear information
   * default = false  -- single residue files should still be recognized.
   ...
   * @param mol The molecule.
   * @param i Index for the current atom. (TraceNucleicChain() will be called
   * for all neighbours)
   * @param r The residue number which we are tracing.
   */
   ...
   * @param smiles The pseudo-smiles string (from OpenBabel::AminoAcids or
   * OpenBabel::Nucleotides)
   */
