...
public:
  ...
  /// @brief Copy constructor.  Stream *ownership* is not copied. Source remains
  /// responsible for the memory.
  OBConversion(const OBConversion &o);
  /// @brief Assignment.  Stream *ownership* is not copied.  Source remains
  /// responsible for the memory.
  ...
  /// @brief Searches registered formats for a MIME the same as the chemical
  /// MIME type passed
  static OBFormat *FormatFromMIME(const char *MIME);

  /// Deprecated!.Repeatedly called to recover available Formats
#ifndef SWIG
  static bool GetNextFormat(Formatpos &itr, const char *&str,
                            OBFormat *&pFormat);
#endif
  //@}
  ...
  /// These return a filtered stream for reading/writing (possible filters
  /// include compression, decompression, and newline transformation)
  /// @name Parameter get and set
  //@{
  std::istream *GetInStream() const { return pInput; };
  std::ostream *GetOutStream() const { return pOutput; };

  /// @brief Set input stream.  If takeOwnership is true, will deallocate when
  /// done. If isGzipped is true, will treat as a gzipped stream regardless of
  /// option settings,
  ...
  bool SetInAndOutFormats(const char *inID, const char *outID,
                          bool ingzip = false, bool outgzip = false);
  bool SetInAndOutFormats(OBFormat *pIn, OBFormat *pOut, bool ingzip = false,
                          bool outgzip = false);
  ...
  /** @name Option handling
   ...
   text string. They are set individually by AddOption() or (rarely)
   collectively in SetOptions(). Options cannot be altered but can be replaced
   with AddOption() and deleted with RemoveOption(), which, however, should be
   used in an op derived from OBOp (because of iterator invalidation).
   ...
   All the options are available to input and output formats, etc. via the
   IsOption() function, and the interpretation of any text string needs to be
   done subsequently.

   In the commandline interface, options with single character ids are are
   indicated like -s, and those with multiple character ids like --gen3D. An
   option may have one or more parameters which appear, space separated, in the
   option's text string. With babel, unless the option is at the end of the
   command, it is necessary for the number of its parameters to be exactly that
   specified in RegisterOptionParam(). The default is 0, but if it is more, and
   babel is likely to be used, this function should be called in the constructor
   of a format or op. With obabel (or the GUI), it is not necessary to call
   RegisterOptionParam().
   ...
   use, this is also parsed by the GUI to construct its checkboxes,etc., so it
   is advisable to give new Descriptions the same form as existing ones.
  ...
  ///@brief Determine whether an option is set. \return NULL if option not and a
  ///pointer to the associated text if it is
  const char *IsOption(const char *opt, Option_type opttyp = OUTOPTIONS);

  ///@brief Access the map with option name as key and any associated text as
  ///value
  ...
  void AddOption(const char *opt, Option_type opttyp = OUTOPTIONS,
                 const char *txt = nullptr);

  bool RemoveOption(const char *opt, Option_type optype);

  ///@brief Set several single character options of specified type from string
  ///like ab"btext"c"ctext"
  void SetOptions(const char *options, Option_type opttyp);

  ///@brief For example -h takes 0 parameters; -f takes 1. Call in a format
  ///constructor.
  static void RegisterOptionParam(std::string name, OBFormat *pFormat,
                                  int numberParams = 0,
                                  Option_type typ = OUTOPTIONS);

  /// \return the number of parameters registered for the option, or 0 if not
  /// found
  static int GetOptionParams(std::string name, Option_type typ);
  //@}

  ///@brief Copies the options (by default of all types) from one OBConversion
  ///Object to another.
  ...
  /// makes input and output streams, and carries out normal, batch,
  ...
  //@{
  int AddChemObject(
      OBBase *pOb); ///< @brief Adds to internal array during input
  OBBase *
  GetChemObject(); ///< @brief Retrieve from internal array during output
  bool IsLast();   ///< @brief True if no more objects to be output
  bool
  IsFirstInput(); ///< @brief True if the first input object is being processed
  void
  SetFirstInput(bool b = true); ///< @brief Setwhether or not is the first input
  int GetOutputIndex() const;   ///< @brief Retrieves number of ChemObjects that
                                ///< have been actually output
  void SetOutputIndex(int indx); ///< @brief Sets output index (maybe to control
                                 ///< whether seen as first object)
  void SetMoreFilesToCome(); ///<@brief Used with multiple input files. Off by
                             ///<default.
  void SetOneObjectOnly(bool b = true); ///< @brief Used with multiple input
                                        ///< files. Off by default.
  void SetLast(bool b) {
    SetOneObjectOnly(b);
  } ///< @brief Synonym for SetOneObjectOnly()
  bool IsLastFile() {
    return !MoreFilesToCome;
  } ///< @brief True if no more files to be read
  /// @brief Number of objects read and processed
  /// Incremented after options are processed, so 0 for first object.  Returns
  /// -1 if Convert interface not used.
  int GetCount() const { return Count; }
  //@}
  /// @name Convenience functions
  //@{
  /// The default format is set in a single OBFormat class (generally it is
  /// OBMol)
  ...
  /// The output stream can be specified and the change is retained in the
  ...
  /// This method is primarily intended for scripting languages without "stream"
  /// classes The optional "trimWhitespace" parameter allows trailing whitespace
  /// to be removed (e.g., in a SMILES string or InChI, etc.)
  std::string WriteString(OBBase *pOb, bool trimWhitespace = false);

  /// @brief Outputs an object of a class derived from OBBase as a file (with
  /// the supplied path)

  /// Part of "API" interface.
  /// The output stream is changed to the supplied file and the change is
  /// retained in the OBConversion instance. This method is primarily intended
  /// for scripting languages without "stream" classes
  bool WriteFile(OBBase *pOb, std::string filePath);

  /// @brief Manually closes and deletes the output stream
  /// The file is closed anyway when in the OBConversion destructor or when
  /// WriteFile is called again. \since version 2.1
  void CloseOutFile();

  /// @brief Reads an object of a class derived from OBBase into pOb.

  /// Part of "API" interface.
  /// The input stream can be specified and the change is retained in the
  /// OBConversion instance \return false and pOb=NULL on error
  bool Read(OBBase *pOb, std::istream *pin = nullptr);

  /// Part of "API" interface.
  /// The input stream can be specified and the change is retained in the
  /// OBConversion instance \return NULL on error
  //      OBBase*	ReadObject(std::istream* pin=NULL);

  /// @brief Reads an object of a class derived from OBBase into pOb from the
  /// supplied string

  /// Part of "API" interface.
  /// \return false and pOb=NULL on error
  /// This method is primarily intended for scripting languages without "stream"
  /// classes Any existing input stream will be replaced by stringstream.
  ...
  /// Part of "API" interface.
  /// The output stream is changed to the supplied file and the change is
  /// retained in the OBConversion instance. For multi-molecule files, the
  /// remaining molecules can be read by repeatedly calling the Read() method.
  /// \return false and pOb=NULL on error
  /// This method is primarily intended for scripting languages without "stream"
  /// classes
  ...
  /// This method is primarily intended for scripting languages without "stream"
  /// classes and will usually followed by a call to Convert(). Will set format
  /// from file extension if format has not already been set. Files will be
  /// opened even if format cannot be determined, but not if file path is empty.
...
protected:
  ...
  static std::string IncrementedFileName(std::string &BaseName,
                                         const int Count);
  /// Checks for misunderstandings when using the -m option
  static bool CheckForUnintendedBatch(const std::string &infile,
...
protected:
  ...
  bool SetStartAndEnd();
  //      static FMapType& FormatsMap();///<contains ID and pointer to all
  //      OBFormat classes static FMapType& FormatsMIMEMap();///<contains MIME
  //      and pointer to all OBFormat classes
  ...
  // unlike the z and zin options, these are not sticky - setting formats will
  // reset them
  bool inFormatGzip;
  bool outFormatGzip;
  ...
  OBBase *pOb1;
  std::streampos
      wInpos; ///< position in the input stream of the object being written
  std::streampos
      rInpos;    ///< position in the input stream of the object being read
  ...
  std::vector<std::string>
      SupportedInputFormat; ///< list of supported input format
  std::vector<std::string>
      SupportedOutputFormat; ///< list of supported output format
};
