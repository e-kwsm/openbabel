...
def find_version():
    ...
    try:
        with open(os.path.join(base_dir, 'openbabel', '__init__.py')) as fp:
            ...
            raise Exception(
                'Could not find version string in openbabel/__init__.py.')
    except OSError:
...
def locate_ob():
    ...
    try:
        ...
        if ob_ver.release[:2] != (py_major_ver, py_minor_ver):
            print(
                f'Warning: Open Babel {py_major_ver}.{py_minor_ver}.x is required. '
...
class CustomBuildExt(build_ext):
    ...
    def finalize_options(self):
        ...
        print('- include_dirs: %s\n- library_dirs: %s' %
              (self.include_dirs, self.library_dirs))
...
obextension = Extension('openbabel._openbabel',
                        [os.path.join('openbabel', 'openbabel-python.i')],
                        libraries=['openbabel'])
