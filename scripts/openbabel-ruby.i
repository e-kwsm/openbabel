...
public:
  ...
  RubyTautomerFunctor(VALUE callback)
      : ruby_callback(callback), exception_state(0) {}
  ...
  void operator()(OBMol *mol, const std::string &) override {
    ...
    RubyCallbackArgs args = {ruby_callback,
                             SWIG_NewPointerObj(new OBMol(*mol),
                                                SWIGTYPE_p_OpenBabel__OBMol,
                                                SWIG_POINTER_OWN)};
    rb_protect(call_ruby_callback, reinterpret_cast<VALUE>(&args),
               &exception_state);
  }

  int state() const { return exception_state; }
};
...
%init %{
rb_define_method(SwigClassOBMol.klass, "tautomers",
                 VALUEFUNC(enumerate_tautomers), 0);
rb_define_method(SwigClassOBMol.klass, "canonical_tautomer",
                 VALUEFUNC(canonical_tautomer), 0);
%}
