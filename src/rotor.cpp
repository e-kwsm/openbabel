...
static bool CompareRotor(const pair<OBBond *, int> &,
  ...
  for (rotor = BeginRotor(i); rotor; rotor = NextRotor(i))
    if (!rotor->Size()) {
      ...
      snprintf(buffer, BUFF_SIZE,
               "The rotor has no associated torsion values -> %d %d %d %d",
...
bool OBRotorList::FindRotors(OBMol &mol, bool sampleRingBonds) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::FindRotors", obAuditMsg);
  ...
  for (OBBond *bond = mol.BeginBond(i); bond; bond = mol.NextBond(i)) {
    ...
    if (bond->IsRotor(sampleRingBonds)) {
      // check if the bond is fixed (using deprecated fixed atoms or new fixed
      // bonds)
      ...
      int score =
          gtd[bond->GetBeginAtomIdx() - 1] + gtd[bond->GetEndAtomIdx() - 1];
...
void OBRotorList::RemoveSymVals(OBMol &mol) {
  ...
  for (rotor = BeginRotor(i); rotor; rotor = NextRotor(i)) {
    ...
    for (int here = 0; here <= 1; ++here) { // Try each side of the bond in turn
      ...
      for (unsigned int hyb = 2; hyb <= 3;
           ++hyb) { // sp2 and sp3 carbons, with explicit Hs
        if (this_side->GetAtomicNum() == 6 && this_side->GetHyb() == hyb &&
          ...
          if (syms.size() ==
              1) // All of the rotated atoms have the same symmetry class
            N_fold_symmetry *= hyb;
        }
      }
    }
        ...
        for (a2 = a1->BeginNbrAtom(k); a2; a2 = a1->NextNbrAtom(k))
          if (!eval[a2->GetIdx()])
            if (!((OBBond *)*k)->IsRotor(_ringRotors) ||
                ((!_fixedatoms.IsEmpty() || HasFixedBonds()) &&
                 IsFixedBond((OBBond *)*k))) {
...
bool OBRotorList::AssignTorVals(OBMol &mol) {
  ...
  for (i = _rotor.begin(); i != _rotor.end(); ++i) {
    ...
    // find all atoms for which there is a path to ref[2] without goinf through
    // ref[1]
    ...
    // translate the rotate atom indexes to coordinate indexes (i.e. from 0,
    // multiplied by 3)
...
void OBRotor::Set(double *coordinates, int idx) {
  ...
  for (i = 0; i < _rotatoms.size(); ++i) {
    ...
    coordinates[j + 1] -= ty;
    coordinates[j + 2] -= tz;
    x = coordinates[j] * m[0] + coordinates[j + 1] * m[1] +
        coordinates[j + 2] * m[2];
    y = coordinates[j] * m[3] + coordinates[j + 1] * m[4] +
        coordinates[j + 2] * m[5];
    z = coordinates[j] * m[6] + coordinates[j + 1] * m[7] +
        coordinates[j + 2] * m[8];
    coordinates[j] = x + tx;
...
void OBRotor::SetDihedralAtoms(std::vector<int> &ref) {
  ...
  for (int i = 0; i < 4; ++i)
    _ref[i] = ref[i];
  _torsion.resize(4);
  // convert the indexes (start from 0, multiplied by 3) for easy access to
  // coordinates
...
void OBRotor::SetRotAtoms(vector<int> &vi) { _rotatoms = vi; }
...
void OBRotorRules::GetRotorIncrements(OBMol &mol, OBBond *bond, int ref[4],
                                      vector<double> &vals, double &delta) {
  if (!_init)
    Init();
  ...
  for (i = _vr.begin(); i != _vr.end(); ++i) {
    ...
    if (a1->GetAtomicNum() == OBElements::Hydrogen &&
        a4->GetAtomicNum() == OBElements::Hydrogen)
      continue; // don't allow hydrogens at both ends
    if (a1->GetAtomicNum() == OBElements::Hydrogen ||
        a4->GetAtomicNum() ==
            OBElements::Hydrogen) // need a heavy atom reference - can use
                                  // hydrogen
    {
      bool swapped = false;
    ...
    char buffer[BUFF_SIZE];
    if (!_quiet) {
      snprintf(buffer, BUFF_SIZE, "%3d%3d%3d%3d %s", ref[0], ref[1], ref[2],
               ref[3], ((*i)->GetSmartsString()).c_str());
      obErrorLog.ThrowError(__FUNCTION__, buffer, obDebug);
    }
    return;
  }
