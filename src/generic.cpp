...
#define OBUNITCELL_CALL_CONST_OVERLOAD_ARG2(_type, _name, _argsig, _argsig2) \
  ...
  */
  bool OBAngleData::FillAngleArray(std::vector<std::vector<unsigned int> > &angles)
  {
    ...
    for(vector<OBAngle>::iterator angle=_angles.begin(); angle!=_angles.end(); ++angle,ct++)
      {
        angles[ct].resize(3);
  ...
  */
  bool OBTorsion::IsProtonRotor()
  {
    bool Aprotor = true;
    bool Dprotor = true;
    for(vector<triple<OBAtom*,OBAtom*,double> >::iterator ad = _ads.begin();ad != _ads.end() && (Aprotor || Dprotor);++ad)
      {
  ...
  */
  bool OBTorsionData::FillTorsionArray(std::vector<std::vector<unsigned int> > &torsions)
  {
    ...
    for (vector<quad<OBAtom*,OBAtom*,OBAtom*,OBAtom*> >::iterator thisQuad = quads.begin();thisQuad != quads.end();++thisQuad,++ct)
      {
