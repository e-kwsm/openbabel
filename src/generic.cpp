...
// needed for msvc to have at least one reference to AtomClass, AliasData in
// openbabel library
...
/** \class OBGenericData generic.h <openbabel/generic.h>
    ...
    {
       OBPairData *display = dynamic_cast<OBPairData *>
bond.GetData("DisplayType"); if (display->GetValue() == "wireframe")
       {
...
OBVirtualBond::OBVirtualBond(unsigned int bgn, unsigned int end,
                             unsigned int ord, int stereo)
    : OBGenericData("VirtualBondData", OBGenericDataType::VirtualBondData,
                    perceived),
...
OBUnitCell::OBUnitCell(const OBUnitCell &src)
    : OBGenericData("UnitCell", OBGenericDataType::UnitCell),
      _mOrtho(src._mOrtho), _mOrient(src._mOrient), _offset(src._offset),
...
matrix3x3 OBUnitCell::GetOrthoMatrix() const { return _mOrtho; }

matrix3x3 OBUnitCell::GetOrientationMatrix() const { return _mOrient; }
...
int OBUnitCell::GetSpaceGroupNumber(std::string name) const {
  static const char *const spacegroups[] = {"P1",
                                            "P-1",
                                            "P2",
                                            "P2(1)",
                                            "C2",
...
void OBUnitCell::FillUnitCell(OBMol *mol) {
  ...
  FOR_ATOMS_OF_MOL(atom, *mol) {
    ...
    snprintf(hash, 22, "%03d,%.3f,%.3f,%.3f", atom->GetAtomicNum(), baseV.x(),
             baseV.y(), baseV.z());
  ...
  for (deleteIter = atomsToDelete.begin(); deleteIter != atomsToDelete.end();
       ++deleteIter) {
  ...
  for (atomIter = atoms.begin(); atomIter != atoms.end(); ++atomIter) {
    ...
    for (transformIter = transformedVectors.begin();
      ...
      snprintf(hash, 22, "%03d,%.3f,%.3f,%.3f", (*atomIter)->GetAtomicNum(),
               updatedCoordinate.x(), updatedCoordinate.y(),
               updatedCoordinate.z());
  ...
  SetSpaceGroup(1); // We've now applied the symmetry, so we should act like a
                    // P1 unit cell
}
...
OBUNITCELL_CALL_CONST_OVERLOAD_ARG(OBUnitCell::LatticeType, GetLatticeType,
                                   int);
...
OBUNITCELL_CALL_CONST_OVERLOAD_ARG2(vector3, UnwrapCartesianNear, vector3,
                                    vector3);
OBUNITCELL_CALL_CONST_OVERLOAD_ARG2(vector3, UnwrapFractionalNear, vector3,
                                    vector3);

double OBUnitCell::GetA() const { return _mOrtho.GetColumn(0).length(); }

double OBUnitCell::GetB() const { return _mOrtho.GetColumn(1).length(); }

double OBUnitCell::GetC() const { return _mOrtho.GetColumn(2).length(); }
...
vector3 OBUnitCell::GetOffset() const { return _offset; }
...
OBConformerData::OBConformerData(const OBConformerData &src)
    : OBGenericData("Conformers", OBGenericDataType::ConformerData),
      _vDimension(src._vDimension), _vEnergies(src._vEnergies),
      _vForces(src._vForces), _vVelocity(src._vVelocity),
...
void OBTorsionData::Clear() { _torsions.clear(); }
...
*/
void OBDOSData::SetData(double fermi, const std::vector<double> &vEnergies,
...
*/
void OBElectronicTransitionData::SetData(
    const std::vector<double> &vWavelengths,
...
*/
void OBElectronicTransitionData::SetEDipole(
    const std::vector<double> &vEDipole) {
...
*/
void OBElectronicTransitionData::SetRotatoryStrengthsVelocity(
    const std::vector<double> &vRotatoryStrengthsVelocity) {
...
*/
void OBElectronicTransitionData::SetRotatoryStrengthsLength(
    const std::vector<double> &vRotatoryStrengthsLength) {
...
void OBFreeGrid::Clear() { _points.clear(); }
