...
namespace OpenBabel {
static unsigned int GetMaxAtomIdx(OBMol *mol) { return mol->NumAtoms() + 1; }

static unsigned int GetMaxBondIdx(OBMol *mol) { return mol->NumBonds(); }
    ...
    while (!degreeOneAtoms.empty()) {
      ...
      FOR_BONDS_OF_ATOM(bond, atom) {
        ...
        FOR_BONDS_OF_ATOM(nbrbond, nbr) {
          if (&(*nbrbond) == &(*bond) || !nbrbond->IsAromatic())
            continue;
