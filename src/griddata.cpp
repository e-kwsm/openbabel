...
/** \class OBGridData griddata.h <openbabel/griddata.h>
  \brief Store values for numeric grids such as orbitals or electrostatic
  potential \since version 2.2 \sa OBFloatGrid

  OBGridData facilitates attaching grids and cubes to molecular data. A "grid"
  is data representing some function f(x,y,z), such as a molecule's
  electrostatic potential or molecular orbitals. This need not be a "cube" even
  though this file format from Gaussian is frequently used. Axes need not be
  identical, and indeed do not need to be orthogonal.

  Open Babel supports reading several types of grid file formats, including
  Gaussian cube, and OpenDX. The latter is notably used by the APBS program for
  numeric evaluation of molecular and protein electrostatic potential.

  \code
  OBGridData *gd = new OBGridData;
  gd->SetAttribute("Example Grid"); // the title of the grid -- e.g., for user
  display vector<int> voxels(3); // the number of voxels in each direction
...
OBGridData::OBGridData()
    : OBGenericData("GridData", OBGenericDataType::GridData),
      d(new GridDataPrivate) {}

OBGridData::~OBGridData() { delete d; }
...
vector3 OBGridData::GetXAxis() const { return d->floatGrid.GetXAxis(); }

vector3 OBGridData::GetYAxis() const { return d->floatGrid.GetYAxis(); }

vector3 OBGridData::GetZAxis() const { return d->floatGrid.GetZAxis(); }
...
int OBGridData::GetNumberOfPoints() const {
  return d->floatGrid.GetXdim() * d->floatGrid.GetYdim() *
         d->floatGrid.GetZdim();
}
...
OBGridData::Unit OBGridData::GetUnit() const { return d->_unit; }

double OBGridData::GetMinValue() const { return d->_min; }

double OBGridData::GetMaxValue() const { return d->_max; }

void OBGridData::GetOriginVector(double o[3]) const { d->floatGrid.GetMin(o); }

vector3 OBGridData::GetOriginVector() const { return d->floatGrid.GetMin(); }

vector3 OBGridData::GetMaxVector() const { return d->floatGrid.GetMax(); }

bool OBGridData::GetUnrestricted() const { return d->_unrestricted; }

int OBGridData::GetNumSymmetries() const { return d->_symmetries; }

void OBGridData::SetUnrestricted(bool u) { d->_unrestricted = u; }

void OBGridData::SetNumSymmetries(int s) { d->_symmetries = s; }
...
void OBGridData::SetUnit(OBGridData::Unit u) { d->_unit = u; }

} // namespace OpenBabel

//! \file griddata.cpp
//! \brief OBGenericData class to connect numeric grids (e.g., orbitals,
//! electrostatic potential) to molecules
