  ...
  if (EQn(buffer, "INTHYB", 6)) {
    ...
    if (vs.size() < 3) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          " Could not parse INTHYB line in atom type table from atomtyp.txt",
          obInfo);
      return;
    }
    ...
    if (sp->Init(vs[1]))
      _vinthyb.push_back(
          pair<OBSmartsPattern *, int>(sp, atoi((char *)vs[2].c_str())));
    else {
      delete sp;
      sp = nullptr;
      obErrorLog.ThrowError(
    ...
    if (vs.size() < 3) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          " Could not parse EXTTYP line in atom type table from atomtyp.txt",
          obInfo);
      return;
    }
    sp = new OBSmartsPattern;
...
void OBAtomTyper::AssignTypes(OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::AssignTypes", obAuditMsg);
...
void OBAtomTyper::AssignHyb(OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::AssignHybridization",
                        obAuditMsg);
...
void OBRingTyper::ParseLine(const char *buffer) {
  ...
  if (EQn(buffer, "RINGTYP", 7)) {
    ...
    if (vs.size() < 3) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          " Could not parse RING line in ring type table from ringtyp.txt",
          obInfo);
      return;
    }
    sp = new OBSmartsPattern;
...
void OBRingTyper::AssignTypes(OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OBRing::AssignTypes", obAuditMsg);
  ...
  for (i2 = _ringtyp.begin(); i2 != _ringtyp.end();
       ++i2) { // for each ring type
    std::vector<std::vector<int>> mlist;
    if (i2->first->Match(mol, mlist)) {
      for (j2 = mlist.begin(); j2 != mlist.end();
           ++j2) { // for each found match

        for (i = rlist.begin(); i != rlist.end(); ++i) { // for each ring
          member_count = 0;

          for (j = j2->begin(); j != j2->end();
               ++j) { // for each atom in the match
...
static bool AssignOBAromaticityModel(OBAtom *atm, int &min, int &max) {
  ...
  // The return value indicates a potentially aromatic atom (i.e. any patterns
  // matched)
  //
...
void OBAromaticTyperMolState::AssignAromaticFlags() {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    _vpa[idx] = AssignOBAromaticityModel(&(*atom), _velec[idx].first,
                                         _velec[idx].second);
  }
...
void OBAromaticTyper::AssignAromaticFlags(OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::AssignAromaticFlags",
                        obAuditMsg);
...
**/
bool OBAromaticTyperMolState::TraverseCycle(OBAtom *root, OBAtom *atom,
                                            OBBond *prev,
                                            std::pair<int, int> &er,
                                            int depth) {
  if (atom == root) {
 ...
 * @param avoidInnerRingAtoms inner closure ring atoms with more than 2
 * neighbours will be avoided
 *
 */
void OBAromaticTyperMolState::SelectRootAtoms(bool avoidInnerRingAtoms) {
  ...
  for (OBBondIterator bd(cbonds.begin()), bd_end(cbonds.end()); bd != bd_end;
       ++bd) {
    bond = *bd;
    ...
    if (avoidInnerRingAtoms) {
      ...
      for (nbr = atom->BeginNbrAtom(l); nbr; nbr = atom->NextNbrAtom(l)) {
        ...
        if (ringNbrs > 2) {
          ...
          for (k = ringAtoms[rootAtom].begin(); k != ringAtoms[rootAtom].end();
               ++k) {
            ring = (*k);
            tmp = ring->_path;
            ...
            if (checkThisRing) {
              ...
              for (unsigned int m = 0; m < tmp.size(); ++m) {
                ringNbrs = heavyNbrs = 0;
                for (nbr2 = (mol.GetAtom(tmp[m]))->BeginNbrAtom(nbr2Iter); nbr2;
                     nbr2 = (mol.GetAtom(tmp[m]))->NextNbrAtom(nbr2Iter)) {
                ...
                if (ringNbrs <= 2 &&
                    ring->IsInRing((mol.GetAtom(tmp[m])->GetIdx()))) {
                  newRoot = tmp[m];
                }
              }
            }
