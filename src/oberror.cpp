...
OBError::OBError(const string &method, const string &errorMsg,
                 const string &explanation, const string &possibleCause,
                 const string &suggestedRemedy, const obMessageLevel level)
...
bool OBError::operator==(const OBError &other) const {
...
incorect number of columns"; errorMsg << " found " << vc.size() << " expected "
<< _ncols << "."; obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obInfo);
\endcode
...
void OBMessageHandler::ThrowError(OBError err, errorQualifier qualifier) {
  ...
  // Output error message if level sufficiently high and, if onceOnly set, it
  // has not been logged before
  if (err.GetLevel() <= _outputLevel &&
      (qualifier != onceOnly || find(_messageList.begin(), _messageList.end(),
                                     err) == _messageList.end())) {
    *_outputStream << err;
  }
...
OBMessageHandler::GetMessagesOfLevel(const obMessageLevel level) {
