...
unsigned int OBBondGetSmallestRingSize(OBBond *bond, unsigned int bound) {
  ...
  //   - A potential improvement would be to BFS from both sides at the same
  //   time
  ...
  qatoms.reserve(
      numatoms < 42
          ? numatoms
          : 42); // Value chosen by inspection on ChEMBL when using bound of 8
  ...
  unsigned int depthmarker =
      qatoms.size(); // when qstart reaches this, increment the depth
...
// Return the typical valence for an atom of specified element, bond order sum
// and formal charge
unsigned int GetTypicalValence(unsigned int element, unsigned int bosum,
                               int charge) {
  switch (element) {
  case 1:
    switch (charge) {
...
void OBAtomAssignTypicalImplicitHydrogens(OBAtom *atom) {
  ...
  unsigned int valence =
      GetTypicalValence(atom->GetAtomicNum(), bosum, atom->GetFormalCharge());
