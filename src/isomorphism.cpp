...
public:
  ...
  struct State {
    State(Functor &_functor, const OBQuery *_query, const OBMol *_queried,
          const OBBitVec &mask)
        : functor(_functor), query(_query), queried(_queried),
          queriedMask(mask) {
      abort = false;
   ...
   */
  bool matchCandidate(State &state, OBQueryAtom *queryAtom,
                      OBAtom *queriedAtom) {
    ...
    if (DEBUG) {
      cout << "FOUND:  " << queryAtom->GetIndex() << " -> "
           << queriedAtom->GetIndex() << "       " << state.queryPath.size()
           << endl;
      cout << "queryDepths:   ";
  ...
  Candidate NextCandidate(State &state, const Candidate &lastCandidate) {
    std::size_t lastQueryAtom =
        lastCandidate.queryAtom ? lastCandidate.queryAtom->GetIndex() : 0;
    std::size_t lastQueriedAtom =
        lastCandidate.queriedAtom ? lastCandidate.queriedAtom->GetIndex() + 1
                                  : 0;
    ...
    std::size_t queryTerminalSize =
        state.queryDepths.size() -
        std::count(state.queryDepths.begin(), state.queryDepths.end(), 0);
    std::size_t queriedTerminalSize =
        state.queriedDepths.size() -
        std::count(state.queriedDepths.begin(), state.queriedDepths.end(), 0);
    ...
    if (queryTerminalSize > mappingSize && queriedTerminalSize > mappingSize) {
      while (lastQueryAtom < querySize &&
             (state.queryPathBits.BitIsSet(lastQueryAtom) ||
              !state.queryDepths[lastQueryAtom])) {
        lastQueryAtom++;
        lastQueriedAtom = 0;
      }
    } else {
    ...
    if (queryTerminalSize > mappingSize && queriedTerminalSize > mappingSize) {
      while (lastQueriedAtom < queriedSize &&
             (state.queriedPathBits.BitIsSet(lastQueriedAtom) ||
              !state.queriedDepths[lastQueriedAtom]))
        lastQueriedAtom++;
    } else {
      while (lastQueriedAtom < queriedSize &&
             state.queriedPathBits[lastQueriedAtom])
        lastQueriedAtom++;
    }

    if (lastQueryAtom < querySize && lastQueriedAtom < queriedSize)
      return Candidate(state.query->GetAtoms()[lastQueryAtom],
                       state.queried->GetAtom(lastQueriedAtom + 1));
   ...
   */
  void MapNext(State &state, OBQueryAtom * /*queryAtom*/,
               OBAtom * /*queriedAtom*/) {
  ...
  void Backtrack(State &state) {
    ...
    std::replace(state.queryDepths.begin(), state.queryDepths.end(), depth,
                 static_cast<unsigned int>(0));
    std::replace(
        state.queriedDepths.begin(), state.queriedDepths.end(), depth,
   ...
   * @return The mapping.
   */
  void MapFirst(const OBMol *queried, Mapping &map,
                const OBBitVec &mask) override {
   ...
   */
  void MapUnique(const OBMol *queried, Mappings &maps,
                 const OBBitVec &mask) override {
    ...
    public:
      ...
      bool operator()(OBIsomorphismMapper::Mapping &map) override {
        ...
        for (OBIsomorphismMapper::Mapping::const_iterator it = map.begin();
             it != map.end(); ++it)
        ...
        for (unsigned int k = 0; k < m_maps.size(); ++k) {
          ...
          for (OBIsomorphismMapper::Mapping::iterator it = m_maps[k].begin();
               it != m_maps[k].end(); ++it)
  ...
  void MapGeneric(Functor &functor, const OBMol *queried,
                  const OBBitVec &mask) override {
    ...
    for (unsigned int i = 0; i < queried->NumAtoms(); ++i) {
      ...
      if (m_query->NumAtoms() > 1) {
        ...
        Mapping map;
        map.push_back(
            std::make_pair(queryAtom->GetIndex(), queriedAtom->GetIndex()));
        functor(map);
      }
    }
...
OBIsomorphismMapper::OBIsomorphismMapper(OBQuery *query)
    : m_query(query), m_timeout(60) {}

OBIsomorphismMapper::~OBIsomorphismMapper() {}

OBIsomorphismMapper *
OBIsomorphismMapper::GetInstance(OBQuery *query, const std::string &algorithm) {
  if (algorithm == "VF2")
...
public:
  OBAutomorphismQueryAtom(unsigned int _symClass,
                          const std::vector<unsigned int> &_symClasses)
...
OBQuery *CompileAutomorphismQuery(OBMol *mol, const OBBitVec &mask,
  ...
  FOR_ATOMS_OF_MOL(obatom, mol) {
    ...
    query->AddAtom(new OBAutomorphismQueryAtom(symClasses[obatom->GetIndex()],
                                               symClasses));
  }
  FOR_BONDS_OF_MOL(obbond, mol) {
    ...
    if (!mask.BitIsSet(beginIndex + 1) || !mask.BitIsSet(endIndex + 1))
      continue;
    query->AddBond(new OBQueryBond(query->GetAtoms()[indexes[beginIndex]],
                                   query->GetAtoms()[indexes[endIndex]],
                                   obbond->GetBondOrder(),
                                   obbond->IsAromatic()));
  }
...
bool FindAutomorphisms(OBMol *mol, Automorphisms &maps,
                       const std::vector<unsigned int> &symClasses,
...
void FindAutomorphisms(OBIsomorphismMapper::Functor &functor, OBMol *mol,
  ...
  public:
    AutomorphismFunctor(OBIsomorphismMapper::Functor &functor,
                        const OBBitVec &fragment, unsigned int numAtoms)
    ...
    bool operator()(Automorphism &map) override {
      // convert the continuous mapping map to a mapping with gaps (considering
      // key values)
