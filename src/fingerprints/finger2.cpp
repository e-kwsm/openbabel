...
public:
  ...
  const char *Description() override {
    return "Indexes linear fragments up to 7 atoms."
           ...
           "A molecule structure is analysed to identify linear fragments of "
           "length\n"
           "from one to Max_Fragment_Size = 7 atoms but single atom fragments "
           "of C,N,and O\n"
           "are ignored. A fragment is terminated when the atoms form a ring.\n"
           "For each of these fragments the atoms, bonding and whether they "
           "constitute\n"
           "a complete ring is recorded and saved in a std::set, so that there "
           "is\n"
           "only one of each fragment type. Chemically identical versions, "
           "i.e. ones with\n"
           "the atoms listed in reverse order and rings listed starting at "
           "different\n"
           "atoms, are identified and only a single canonical fragment is "
           "retained\n"
           "Each remaining fragment is assigned a hash number from 0 to 1020 "
           "which is\n"
           "used to set a bit in a 1024 bit vector.\n"
           "For further details see:\n"
           "http://baoilleach.blogspot.co.uk/2012/01/"
           "visualising-fragments-in-path-based.html \n";
  }

  // Calculates the fingerprint
  bool GetFingerprint(OBBase *pOb, vector<unsigned int> &fp,
                      int nbits = 0) override;

  /// \returns fragment info unless SetFlags(OBFingerprint::FPT_NOINFO) has been
  /// called before GetFingerprint() called.
  ...
  **/
  std::string DescribeBits(const std::vector<unsigned int> fp,
...
bool fingerprint2::GetFingerprint(OBBase *pOb, vector<unsigned int> &fp,
                                  int nbits) {
  ...
  for (patom = pmol->BeginAtom(i); patom; patom = pmol->NextAtom(i)) {
    if (patom->GetAtomicNum() == OBElements::Hydrogen)
      continue;
    vector<int> curfrag;
...
void fingerprint2::getFragments(vector<int> levels, vector<int> curfrag,
                                int level, OBAtom *patom, OBBond *pbond) {
  // Recursive routine to analyse schemical structure and populate fragset and
  // ringset Hydrogens,charges(except dative bonds), spinMultiplicity ignored
  ...
  if (pbond) {
    ...
    //		if(patom->GetFormalCharge() && (patom->GetFormalCharge() ==
    //-pprevat->GetFormalCharge()))
    //			++bo; //coordinate (dative) bond eg C[N+]([O-])=O is seen as
    //CN(=O)=O
  }
  ...
  for (pnewbond = patom->BeginBond(itr); pnewbond;
       pnewbond = patom->NextBond(itr)) {
    if (pnewbond == pbond)
      continue; // don't retrace steps
    OBAtom *pnxtat = pnewbond->GetNbrAtom(patom);
    if (pnxtat->GetAtomicNum() == OBElements::Hydrogen)
      continue;
    ...
    {
      if (atlevel == 1) {
        // If complete ring (last bond is back to starting atom) add bond at
        // front and save in ringset
    ...
    } else // no ring
    {
      if (level < Max_Fragment_Size) {
        //				TRACE("level=%d size=%d %p
        //frag[0]=%p\n",level, curfrag.size(),&curfrag, &(curfrag[0])); Do the
        // next atom; levels, curfrag are passed by value and hence copied
  ...
  if (curfrag[0] == 0 &&
    ...
    fragset.insert(
        curfrag); // curfrag ignored if an identical fragment already present
...
void fingerprint2::DoReverses() {
  ...
  for (itr = fragset.begin(); itr != fragset.end();) {
    // Reverse the order of the atoms, add the smallest fragment and remove the
    // larger
    SetItr titr =
        itr++; // Ensure have valid next iterator in case current one is erased
...
void fingerprint2::DoRings() {
  // For each complete ring fragment, find its largest chemically identical
  // representation by rotating and reversing, and insert into the main set of
  // fragments
  SetItr itr;
