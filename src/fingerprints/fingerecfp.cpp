...
public:
  fingerprintECFP(const char *ID, bool IsDefault = false,
      ...
      : OBFingerprint(ID, IsDefault), _radius(radius), _keepdups(keepdups),
        _flags(0){};
...
static unsigned int ECFPHash(unsigned char *ptr, unsigned int length) {
  ...
  while (len >= 12) {
    a += ptr[0] + ((unsigned int)ptr[1] << 8) + ((unsigned int)ptr[2] << 16) +
         ((unsigned int)ptr[3] << 24);
    b += ptr[4] + ((unsigned int)ptr[5] << 8) + ((unsigned int)ptr[6] << 16) +
         ((unsigned int)ptr[7] << 24);
    c += ptr[8] + ((unsigned int)ptr[9] << 8) + ((unsigned int)ptr[10] << 16) +
...
struct AtomInfo {
  unsigned int
      e[5]; // hashcodes for ecfp0, 2, 4... This will segfault at ECFP12
  std::vector<unsigned int>
      b[4]; // for duplicate removal as described in paper?
};
...
static void ECFPPass(OpenBabel::OBMol &mol, AtomInfo *ainfo,
                     unsigned int pass) {
...
static void ECFPFirstPass(OpenBabel::OBMol &mol, AtomInfo *ainfo) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    buffer[1] =
        aptr->GetExplicitValence() -
        aptr->ExplicitHydrogenCount(); // valence of heavy atom connections
    buffer[2] = aptr->GetAtomicNum();
    buffer[3] = (unsigned char)aptr->GetIsotope();
    buffer[4] = (unsigned char)aptr->GetFormalCharge();
    buffer[5] = (unsigned char)(aptr->ExplicitHydrogenCount() +
                                aptr->GetImplicitHCount());
...
bool fingerprintECFP::GetFingerprint(OBBase *pOb, vector<unsigned int> &fp,
                                     int nbits) {
