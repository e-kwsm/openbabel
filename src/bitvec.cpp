...
#define LowBit(set, bit)                                                       \
  {                                                                            \
    ...
    if ((set) != 0) {                                                          \
      (bit) = 31;                                                              \
      if ((set) != 0x80000000) {                                               \
        if ((m = ((set)&0x0000ffff)) != 0) {                                   \
          (set) = m;                                                           \
          (bit) -= 16;                                                         \
        }                                                                      \
        if ((m = ((set)&0x00ff00ff)) != 0) {                                   \
          (set) = m;                                                           \
          (bit) -= 8;                                                          \
        }                                                                      \
        if ((m = ((set)&0x0f0f0f0f)) != 0) {                                   \
          (set) = m;                                                           \
          (bit) -= 4;                                                          \
        }                                                                      \
        if ((m = ((set)&0x33333333)) != 0) {                                   \
          (set) = m;                                                           \
          (bit) -= 2;                                                          \
        }                                                                      \
        if ((m = ((set)&0x55555555)) != 0) {                                   \
          (set) = m;                                                           \
          (bit) -= 1;                                                          \
        }                                                                      \
      }                                                                        \
...
void OBBitVec::SetBitOn(unsigned bit_offset) {
  unsigned const word_offset = bit_offset >> WORDROLL;
  bit_offset &= WORDMASK;
...
void OBBitVec::Fold(unsigned new_bit_size) {
  unsigned const new_word_size = new_bit_size >> WORDROLL;
...
/** Searches the vector for the first true value, starting at the \p
   last_bit_offset 'th bit \param[in] last_bit_offset the bit before the first
   to consider \return the bit offset of the first true bit after \p
   last_bit_offset, or -1 if there is none
*/
...
*/
OBBitVec &OBBitVec::operator&=(const OBBitVec &bv) {
  unsigned const min = (bv.GetSize() < _size) ? bv.GetSize() : _size;
  unsigned i;
...
/** Return a bit vector of the results of Or-ing each bit in \p bv1 with the
   corresponding bit in \p bv2 \param[in] bv1 A bit vector \param[in] bv2
   Another bit vector \return A bit vector
*/
...
/** Return a bit vector of the results of And-ing each bit in \p bv1 with the
   corresponding bit in \p bv2 \param[in] bv1 A bit vector \param[in] bv2
   Another bit vector \return A bit vector
*/
OBBitVec operator&(const OBBitVec &bv1, const OBBitVec &bv2) {
...
/** Return a bit vector of the results of Exclusive-or-ing each bit in \p bv1
   with the corresponding bit in \p bv2 \param[in] bv1 A bit vector \param[in]
   bv2 Another bit vector \return A bit vector
*/
OBBitVec operator^(const OBBitVec &bv1, const OBBitVec &bv2) {
  OBBitVec bv(bv1);
  bv ^= bv2;
...
/** Return a bit vector of the results of clearing each bit in \p bv1 which is
   set in \p bv2 \param[in] bv1 A bit vector \param[in] bv2 Another bit vector
    \return A bit vector
*/
OBBitVec operator-(const OBBitVec &bv1, const OBBitVec &bv2) {
  OBBitVec bv;
...
/** Return true if \p bv1 and \p bv2 are equivalent
    Not that they may be of different size, and still equivalent provided that
   the extra bits are all zero. \param[in] bv1 A bit vector \param[in] bv2
   Another bit vector \return true if equal, false otherwise
*/
...
/** Sets bits on, listed as a string of character-represented integers in a
   stream Only reads one line of input The format is "[ n0 n1 n2 n3 ... ]". The
   square brackets are optional. The whitespace can be SPACE or HTAB For example
   "[ 1 5 6 9 ]" \param[in,out] is The input stream \param[out] bv The bit
   vector to contain the result
*/
...
/** The Tanimoto coefficient may be regarded as the proportion of the "on-bits"
   which are shared. \param[in] bv1 the first bit vector \param[in] bv2 the
   second bit vector \return the ratio of shared bits to bits which either
   vector has set.
*/
