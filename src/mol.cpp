...
namespace OpenBabel
{
  ...
  */
  ...
  void OBMol::SetTorsion(OBAtom *a,OBAtom *b,OBAtom *c, OBAtom *d, double ang)
  {
    ...
    double v2x = NAN,v2y = NAN,v2z = NAN;
    double radang = NAN,m[9];
  ...
  */
  void OBMol::GetGIDVector(vector<unsigned int> &vgid)
  {
    ...
    unsigned int nclass1 = 0,nclass2 = 0; //number of classes
  ...
  */
  ...
  bool OBMol::AddNewHydrogens(HydrogenType whichHydrogen, bool correctForPH, double pH)
  {
    ...
    */
    ...
    OBAtom *atom = nullptr,*h = nullptr;
    int hcount = 0,count=0;
...
#undef  OBBondIncrement
  ...
  */
  void OBMol::ConnectTheDots(void)
  {
    ...
    int idx1 = 0, idx2 = 0;
    double d2 = NAN,cutoff = NAN,zd = NAN;
    ...
    OBBond *maxbond = nullptr, *bond = nullptr;
    double maxlength = NAN;
  ...
  */
  void OBMol::PerceiveBondOrders()
  {
    ...
    OBAtom *atom = nullptr, *b = nullptr, *c = nullptr;
    vector3 v1, v2;
    double angle = NAN;//, dist1, dist2;
    vector<OBAtom*>::iterator i;
    ...
    bool typed = false; // has this ring been typed?
    unsigned int loop = 0, loopSize = 0;
    for (ringit = rlist.begin(); ringit != rlist.end(); ++ringit)
      {
        typed = false;
    ...
    vector<int> sorted;
    int iter = 0, max = 0;
    double maxElNeg = NAN, shortestBond = NAN, currentElNeg = NAN;
    double bondLength = NAN, testLength = NAN;
  ...
  bool OBMol::MakeDativeBonds()
  {
    ...
    for (patom = BeginAtom(ai);patom;patom = NextAtom(ai)) //all atoms
    {
        ...
        && (patom->GetExplicitValence()==5 || (patom->GetExplicitValence()==4 && patom->GetFormalCharge()==0)))
      {
        ...
        OBBond *bond = nullptr, *bestbond = nullptr;
        OBBondIterator bi;
    ...
    FOR_BONDS_OF_MOL(bond, this) {
      ...
      if (posB == AtomMap.end() || posE == AtomMap.end() || skipping_bond) {
        ...
        case 1:
          ...
          break;
        case 2: {
            OBAtom *atomB = nullptr, *atomE = nullptr;
            if (skipping_bond) {
              for(int N=0; N<2; ++N) {
                atomB = nullptr;
                atomE = nullptr;
