...
/** \class OBMol mol.h <openbabel/mol.h>
    ...
    \endcode
    It is important to note that atom arrays currently begin at 1 and bond
   arrays begin at 0. Requesting atom 0 (\code OBAtom *atom = mol.GetAtom(0);
   \endcode will result in an error, but \code OBBond *bond = mol.GetBond(0);
    \endcode
    is perfectly valid.
    ...
    Iterators are then a natural way to loop over the vectors of atoms and
   bonds.
...
double OBMol::GetTorsion(int a, int b, int c, int d) {
  return (GetTorsion((OBAtom *)_vatom[a - 1], (OBAtom *)_vatom[b - 1],
                     (OBAtom *)_vatom[c - 1], (OBAtom *)_vatom[d - 1]));
}
...
double OBMol::GetTorsion(OBAtom *a, OBAtom *b, OBAtom *c, OBAtom *d) {
  if (!IsPeriodic()) {
    return (CalcTorsionAngle(a->GetVector(), b->GetVector(), c->GetVector(),
                             d->GetVector()));
  } else {
    vector3 v1, v2, v3, v4;
    // Wrap the atomic positions in a continuous chain that makes sense based on
    // the unit cell Start by extracting the absolute Cartesian coordinates
    v1 = a->GetVector();
    v2 = b->GetVector();
    v3 = c->GetVector();
    v4 = d->GetVector();
...
void OBMol::FindTorsions() {
  ...
  for (bond = BeginBond(bi1); bond; bond = NextBond(bi1)) {
    ...
    if (b->GetAtomicNum() == OBElements::Hydrogen ||
        c->GetAtomicNum() == OBElements::Hydrogen)
      continue;
...
static void CreateNewClassVector(vector<pair<OBAtom *, unsigned int>> &vp1,
  ...
  for (i = vp1.begin(); i != vp1.end(); ++i) {
    ...
    for (id = i->second, m = 100, k = vtmp.begin(); k != vtmp.end();
         ++k, m *= 100)
      id += *k * m;
...
OBAtom *OBMol::GetAtomById(unsigned long id) const {
  if (id >= _atomIds.size()) {
    obErrorLog.ThrowError(__FUNCTION__, "Requested atom with invalid id.",
                          obDebug);
    return nullptr;
  }
...
OBBond *OBMol::GetBondById(unsigned long id) const {
  if (id >= _bondIds.size()) {
    obErrorLog.ThrowError(__FUNCTION__, "Requested bond with invalid id.",
                          obDebug);
    return nullptr;
  }
...
OBResidue *OBMol::GetResidue(int idx) const {
  if (idx < 0 || (unsigned)idx >= NumResidues()) {
    obErrorLog.ThrowError(__FUNCTION__, "Requested Residue Out of Range",
                          obDebug);
    return nullptr;
  }
...
//! Implements <a
//! href="http://qsar.sourceforge.net/dicts/blue-obelisk/index.xhtml#findSmallestSetOfSmallestRings">blue-obelisk:findSmallestSetOfSmallestRings</a>.
...
string OBMol::GetSpacedFormula(int ones, const char *sp, bool implicitH) {
  ...
  const int alphabetical[NumElements] = {
      89,  47,  13,  95,  18,          33,  85,
      79,  5,   56,  4,   107,         83,  97,
      35,  6,   20,  48,  58,          98,  17,
      96,  112, 27,  24,  55,          29,  NumElements - 1,
      105, 110, 66,  68,  99,          63,  9,
      26,  114, 100, 87,  31,          64,  32,
      1,   2,   72,  80,  67,          108, 53,
      49,  77,  19,  36,  57,          3,   103,
      71,  116, 115, 101, 12,          25,  42,
      109, 7,   11,  41,  60,          10,  113,
      28,  102, 93,  8,   118,         76,  15,
      91,  82,  46,  61,  84,          59,  78,
      94,  88,  37,  75,  104,         111, 45,
      86,  44,  16,  51,  21,          34,  106,
      14,  62,  50,  38,  NumElements, 73,  65,
      43,  52,  90,  22,  81,          69,  117,
      92,  23,  74,  54,  39,          70,  30,
      40};
  ...
  FOR_ATOMS_OF_MOL(a, *this) {
    ...
    if (anum > (NumElements - 2)) {
      ...
      snprintf(buffer, BUFF_SIZE,
               "Skipping unknown element with atomic number %d", anum);
...
unsigned int OBMol::GetTotalSpinMultiplicity() {
  ...
  else // calculate from atomic spin information (assuming high-spin case)
  {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Ran OpenBabel::GetTotalSpinMultiplicity -- calculating from atomic "
        "spins assuming high spin case",
        obAuditMsg);
...
// OB_TSPIN_MOL, OB_AROMATIC_MOL, OB_PERIODIC_MOL, OB_CHAINS_MOL and
// OB_PATTERN_STRUCTURE which are copied
{
  if (this == &source)
    return *this;
  ...
  this->SetTotalSpinMultiplicity(
      src.GetTotalSpinMultiplicity()); // also sets a flag
  ...
  // this->_flags = src.GetFlags(); //Copy all flags. Perhaps too drastic a
  // change
  ...
  if (NumRes) {
    ...
    for (k = 0; k < NumRes; ++k) {
      ...
      *res = *src_res; // does not copy atoms
      for (src_atom = src_res->BeginAtom(ii); src_atom;
           src_atom = src_res->NextAtom(ii)) {
...
OBMol &OBMol::operator+=(const OBMol &source) {
  ...
  for (bond = src.BeginBond(j); bond; bond = src.NextBond(j)) {
    bond->SetId(NoId); // Need to remove ID which relates to source mol rather
                       // than this mol
    AddBond(bond->GetBeginAtomIdx() + prevatms,
            bond->GetEndAtomIdx() + prevatms, bond->GetBondOrder(),
            bond->GetFlags());
  }
  ...
  std::vector<OBGenericData *> vdata =
      src.GetAllData(OBGenericDataType::StereoData);
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (datatype == OBStereo::CisTrans) {
      ...
      for (OBStereo::RefIter ri = ct_cfg.refs.begin(); ri != ct_cfg.refs.end();
           ++ri)
      ...
      for (OBStereo::RefIter ri = ts_cfg.refs.begin(); ri != ts_cfg.refs.end();
           ++ri)
        *ri = correspondingId[*ri];
...
bool OBMol::Clear() {
  if (obErrorLog.GetOutputLevel() >= obAuditMsg)
    obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::Clear Molecule",
                          obAuditMsg);
...
void OBMol::EndModify(bool nukePerceivedData) {
  if (_mod == 0) {
    obErrorLog.ThrowError(
        __FUNCTION__, "_mod is negative - EndModify() called too many times",
        obDebug);
    return;
  }
  ...
  // wipe all but whether it has aromaticity perceived, is a reaction, or has
  // periodic boundaries enabled
  ...
  // Always remove angle and torsion data, since they will interfere with the
  // iterators PR#2812013
...
OBAtom *OBMol::NewAtom() { return NewAtom(_atomIds.size()); }
...
OBBond *OBMol::NewBond() { return NewBond(_bondIds.size()); }
...
bool OBMol::AddAtom(OBAtom &atom, bool forceNewId) {
  ...
  // Use the existing atom Id unless either it's invalid or forceNewId has been
  // specified
  unsigned long id;
  if (forceNewId)
    id = _atomIds.size();
...
static bool IsSuppressibleHydrogen(OBAtom *atom) {
  if (atom->GetIsotope() == 0 && atom->GetHvyDegree() == 1 &&
      atom->GetFormalCharge() == 0 && !atom->GetData("Atom Class"))
    return true;
  else
    return false;
}
...
bool OBMol::DeletePolarHydrogens() {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::DeleteHydrogens -- polar",
                        obAuditMsg);
...
bool OBMol::DeleteNonPolarHydrogens() {
  ...
  obErrorLog.ThrowError(
      __FUNCTION__, "Ran OpenBabel::DeleteHydrogens -- nonpolar", obAuditMsg);
...
bool OBMol::DeleteHydrogens() {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::DeleteHydrogens",
                        obAuditMsg);

  for (atom = BeginAtom(i); atom; atom = NextAtom(i))
    if (atom->GetAtomicNum() == OBElements::Hydrogen &&
        IsSuppressibleHydrogen(atom))
      delatoms.push_back(atom);
...
// deletes the hydrogen atom passed to the function
{
  ...
  if (atomidx != NumAtoms()) {
    ...
    vector<double *>::iterator k;
    for (k = _vconf.begin(); k != _vconf.end(); ++k)
      memmove((char *)&(*k)[idx], (char *)&(*k)[idx + 3],
...
bool OBMol::AddHydrogens(bool polaronly, bool correctForPH, double pH) {
  return (AddNewHydrogens(polaronly ? PolarHydrogen : AllHydrogen, correctForPH,
                          pH));
}
...
bool OBMol::AddNewHydrogens(HydrogenType whichHydrogen, bool correctForPH,
                            double pH) {
  ...
  /*
  //
  // This was causing bug #1892844 in avogadro. We also want to add hydrogens if
  ...
  */
  if (whichHydrogen == AllHydrogen)
    obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::AddHydrogens",
                          obAuditMsg);
  else if (whichHydrogen == PolarHydrogen)
    obErrorLog.ThrowError(
        __FUNCTION__, "Ran OpenBabel::AddHydrogens -- polar only", obAuditMsg);
  else
    obErrorLog.ThrowError(__FUNCTION__,
                          "Ran OpenBabel::AddHydrogens -- nonpolar only",
                          obAuditMsg);
  ...
  SetHydrogensAdded(); // This must come after EndModify() as EndModify() wipes
                       // the flags
  // If chirality was already perceived, remember this (to avoid wiping
  // information
  ...
  for (k = vhadd.begin(); k != vhadd.end(); ++k) {
    atom = k->first;
    double bondlen =
        hbrad + CorrectedBondRad(atom->GetAtomicNum(), atom->GetHyb());
    for (m = 0; m < k->second; ++m) {
      ...
      for (n = 0; n < NumConformers(); ++n) {
        ...
        if (hasCoords) {
          ...
          if (isfinite(v.x()) || isfinite(v.y()) || isfinite(v.z())) {
            ...
            obErrorLog.ThrowError(
                __FUNCTION__,
                "Ran OpenBabel::AddHydrogens -- no reasonable bond geometry "
                "for desired hydrogen.",
                obAuditMsg);
            badh++;
          }
        } else
      ...
      if (badh == 0 || badh < NumConformers()) {
        ...
        if (res) {
          ...
          // hydrogen should inherit hetatm status of heteroatom (default is
          // false)
  ...
  _flags &= (~(OB_PCHARGE_MOL | OB_ATOMTYPES_MOL | OB_SSSR_MOL |
               OB_AROMATIC_MOL | OB_HYBRID_MOL));

  return (true);
}

bool OBMol::AddPolarHydrogens() { return (AddNewHydrogens(PolarHydrogen)); }
...
bool OBMol::CorrectForPH(double pH) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::CorrectForPH",
                        obAuditMsg);
...
/**
   If NoImplicitH is true then the molecule has no implicit hydrogens.
Individual atoms on which ForceNoH() has been called also have no implicit
hydrogens. If NoImplicitH is false (the default), then if there are any explicit
hydrogens on an atom then they constitute all the hydrogen on that atom.
However, a hydrogen atom with its _isotope!=0 is not considered explicit
hydrogen for this purpose. In addition, an atom which has had ForceImplH()called
for it is never considered hydrogen deficient, e.g. unbracketed atoms in SMILES.
   Any discrepancy with the expected atom valency is interpreted as the atom
being a radical of some sort and iits _spinMultiplicity is set to 2 when it is
one hydrogen short and 3 when it is two hydrogens short and similarly for
greater hydrogen deficiency.

   So SMILES C[CH] is interpreted as methyl carbene, CC[H][H] as ethane, and
CC[2H] as CH3CH2D.
**/
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (datatype == OBStereo::CisTrans) {
      ...
      if (ct_cfg.begin == atomId || ct_cfg.end == atomId ||
          std::find(ct_cfg.refs.begin(), ct_cfg.refs.end(), atomId) !=
              ct_cfg.refs.end())
        mol.DeleteData(ct);
    } else if (datatype == OBStereo::Tetrahedral) {
      OBTetrahedralStereo *ts = dynamic_cast<OBTetrahedralStereo *>(*data);
      ...
      if (ts_cfg.from == atomId ||
          std::find(ts_cfg.refs.begin(), ts_cfg.refs.end(), atomId) !=
              ts_cfg.refs.end())
        mol.DeleteData(ts);
    }
  }
...
bool OBMol::AddBond(int first, int second, int order, int flags,
                    int insertpos) {
  ...
  // atoms exist and bond doesn't
  {
    ...
    if (!bgn || !end) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Unable to add bond - invalid atom index", obDebug);
      return (false);
    }
...
bool OBMol::AddBond(OBBond &bond) {
  if (!AddBond(bond.GetBeginAtomIdx(), bond.GetEndAtomIdx(),
...
void OBMol::Align(OBAtom *a1, OBAtom *a2, vector3 &p1, vector3 &p2) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::Align", obAuditMsg);
...
void OBMol::ToInertialFrame(int conf, double *rmat) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::ToInertialFrame",
                        obAuditMsg);
...
bool OBMol::Has2D(bool Not3D) {
  ...
  if (hasX || hasY) // was && but this excluded vertically or horizontally
                    // aligned linear mols
    return (true);
  return (false);
}
...
bool OBMol::Has3D() {
  ...
  //    if (this->_c == NULL) **Test removed** Prevented function use during
...
void OBMol::RenumberAtoms(vector<OBAtom *> &v) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::RenumberAtoms",
                        obAuditMsg);
  ...
  for (j = 0; j < NumConformers(); ++j) {
    ...
    for (k = 0, i = va.begin(); i != va.end(); ++i, ++k)
      memcpy((char *)&ctmp[k * 3],
             (char *)&c[((OBAtom *)*i)->GetCoordinateIdx()],
             sizeof(double) * 3);
...
void OBMol::ConnectTheDots(void) {
  if (Empty())
    return;
  if (_dimension != 3)
    return; // not useful on non-3D structures
  ...
  if (IsPeriodic())
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::ConnectTheDots",
                          obAuditMsg);
  ...
  for (j = 0; j < max; ++j) {
    ...
    for (k = j + 1; k < max; k++) {
      ...
      // Otherwise, use a simpler (faster) distance calculation based on raw
      // coordinates
      if (IsPeriodic()) {
        ...
        // not the radius of k (which might be small, ie H, and cause an early
        // termination) since we sort by z, anything beyond k will also fail
        if (zd > maxcutoff)
          break;
...
void OBMol::PerceiveBondOrders() {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::PerceiveBondOrders",
                        obAuditMsg);
  ...
  for (atom = BeginAtom(i); atom; atom = NextAtom(i)) {
    ...
    if (atom->GetAtomicNum() == OBElements::Nitrogen &&
        atom->ExplicitHydrogenCount() == 1 && atom->GetExplicitDegree() == 2 &&
        angle > 109.5)
      atom->SetHyb(2);
    else if (atom->GetAtomicNum() == OBElements::Nitrogen &&
             atom->GetExplicitDegree() == 2 && atom->IsInRing()) // azete
      atom->SetHyb(2);
  } // pass 1
  ...
  for (ringit = rlist.begin(); ringit != rlist.end(); ++ringit) {
    if ((*ringit)->PathSize() == 5) {
      path = (*ringit)->_path;
      torsions = (fabs(GetTorsion(path[0], path[1], path[2], path[3])) +
                  ...
                  fabs(GetTorsion(path[4], path[0], path[1], path[2]))) /
                 5.0;
      if (torsions <= 7.5) {
      ...
      torsions = (fabs(GetTorsion(path[0], path[1], path[2], path[3])) +
                  ...
                  fabs(GetTorsion(path[5], path[0], path[1], path[2]))) /
                 6.0;
      if (torsions <= 12.0) {
        for (unsigned int ringAtom = 0; ringAtom != path.size(); ++ringAtom) {
          b = GetAtom(path[ringAtom]);
  ...
  for (atom = BeginAtom(i); atom; atom = NextAtom(i)) {
    // if atoms have the same electronegativity, make sure those with shorter
    // bonds are handled first (helps with assignment of conjugated
    // single/double bonds)
    shortestBond = 1.0e5;
    for (b = atom->BeginNbrAtom(j); b; b = atom->NextNbrAtom(j)) {
      if (b->GetAtomicNum() != 1)
        shortestBond = std::min(shortestBond, (atom->GetBond(b))->GetLength());
    }
    pair<OBAtom *, double> entry(
        atom,
  ...
  for (iter = 0; iter < max; iter++) {
    ...
    // Debugging statement
    //        cout << " atom->Hyb " << atom->GetAtomicNum() << " " <<
    //        atom->GetIdx() << " " << atom->GetHyb()
      ...
      for (b = atom->BeginNbrAtom(j); b; b = atom->NextNbrAtom(j)) {
        ...
        if ((b->GetHyb() == 1 || b->GetExplicitDegree() == 1) &&
            b->GetExplicitValence() + 2 <=
                static_cast<unsigned int>(
                    OBElements::GetMaxBonds(b->GetAtomicNum())) &&
          ...
          if (atom->GetExplicitDegree() == 1 || b->GetExplicitDegree() == 1) {
            testLength =
                CorrectedBondRad(atom->GetAtomicNum(), atom->GetHyb()) +
...
vector3 OBMol::Center(int nconf) {
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::Center", obAuditMsg);
...
void OBMol::Translate(const vector3 &v, int nconf) {
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::Translate", obAuditMsg);
...
void OBMol::Rotate(const double m[9], int nconf) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::Rotate", obAuditMsg);
...
void OBMol::SetEnergies(std::vector<double> &energies) {
  ...
  OBConformerData *cd =
      (OBConformerData *)GetData(OBGenericDataType::ConformerData);
...
bool OBMol::ConvertDativeBonds() {
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::ConvertDativeBonds",
                        obAuditMsg);
  ...
  for (patom = BeginAtom(i); patom; patom = NextAtom(i)) {
    ...
    OBBond *pbond;
    for (pbond = patom->BeginBond(itr); patom->GetFormalCharge() && pbond;
         pbond = patom->NextBond(itr)) {
...
bool OBMol::MakeDativeBonds() {
  ...
  //! e.g. -N(=O)=O converted to -[N+]([O-])=O. Returns true if conversion
  //! occurs.
  BeginModify();
  // AddHydrogens();
  bool converted = false;
  ...
  for (patom = BeginAtom(ai); patom; patom = NextAtom(ai)) // all atoms
  {
    if (patom->GetAtomicNum() ==
            OBElements::Nitrogen // || patom->GetAtomicNum() ==
                                 // OBElements::Phosphorus) not phosphorus!
        &&
      ...
      OBBondIterator bi;
      for (bestbond = bond = patom->BeginBond(bi); bond;
           bond = patom->NextBond(bi)) {
        ...
        if (bo >= 2 && bo <= 4) {
          ...
          if ((het && !oldhet) ||
              (((het && oldhet) || (!het && !oldhet)) && higherorder))
            bestbond = bond;
        }
      }
...
/**
 *  This function is useful when writing to legacy formats (such as MDL MOL)
 that do
 *  not support zero-order bonds. It is worth noting that some compounds cannot
 be
 *  well represented using just single, double and triple bonds, even with
 adjustments
 *  to adjacent charges. In these cases, simply converting zero-order bonds to
 single
 ...
 *
 ...
 Zero-Order Bonds and Explicit Hydrogen Counting. Journal of Chemical
 Information and Modeling, 51, 3149-3157 (2011).
...
bool OBMol::ConvertZeroBonds() {
  ...
  // If we must, I think we need to manually remember
  // HasImplicitValencePerceived and re-set it after EndModify()

  // Periodic table block for element (1=s, 2=p, 3=d, 4=f)
  const int BLOCKS[113] = {
      0, 1, 2, 1, 1, 2, 2, 2, 2, 2, 2, 1, 1, 2, 2, 2, 2, 2, 2, 1, 1, 3, 3,
      3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 1, 1, 3, 3, 3, 3, 3, 3, 3,
      3, 3, 3, 2, 2, 2, 2, 2, 2, 1, 1, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4, 4,
      4, 4, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2, 1, 1, 4, 4, 4,
  ...
  for (vector<vector<int>>::iterator i = cfl.begin(); i != cfl.end(); ++i) {
    ...
    for (vector<int>::const_iterator j = i->begin(); j != i->end(); ++j) {
      FOR_BONDS_OF_ATOM(b, GetAtom(*j)) {
        if (b->GetBondOrder() == 0 &&
            !(find(bonds.begin(), bonds.end(), &*b) != bonds.end())) {
          bonds.push_back(&*b);
        }
      }
    }
    ...
    while (bonds.size() > 0) {
      ...
      if (lb.first > 0 && lb.second > 0 && le.first > 0 && le.second > 0) {
        ...
        ord = 2; // p-block lewis acid with d/f-block element: make into double
                 // bond
...
information needed is an OBBitVec indicating which atoms to include and
(optionally) an OBBitVec indicating which bonds to exclude. By default, only
bonds joining included atoms are copied.

When an atom is copied, but not all of its bonds are, by default hydrogen counts
are adjusted to account for the missing bonds. That is, given the SMILES "CF",
if we copy the two atoms but exclude the bond, we will end up with "C.F". This
behavior can be changed by specifiying a value other than 1 for the \p
correctvalence parameter. A value of 0 will yield "[C].[F]" while 2 will yield
"C*.F*" (see \p correctvalence below for more information).

Aromaticity is preserved as present in the original OBMol. If this is not
desired, the user should call OBMol::SetAromaticPerceived(false) on the new
OBMol.

Stereochemistry is only preserved if the corresponding elements are wholly
present in the substructure. For example, all four atoms and bonds of a
tetrahedral stereocenter must be copied.
...
When used from Python, note that "None" may be used to specify an empty value
for the \p excludebonds parameter.
...
\return A boolean indicating success or failure. Currently failure is only
reported if one of the specified atoms is not present, or \p atoms is a NULL
        pointer.

\param newmol   The molecule to which to add the substructure. Note that atoms
are appended to this molecule. \param atoms    An OBBitVec, indexed by atom Idx,
specifying which atoms to copy \param excludebonds  An OBBitVec, indexed by bond
Idx, specifying a list of bonds to exclude. By default, all bonds between the
specified atoms are included - this parameter overrides that. \param
correctvalence  A value of 0, 1 (default) or 2 that indicates how atoms with
missing bonds are handled: 0 - Leave the implicit hydrogen count unchanged; 1 -
Adjust the implicit hydrogen count to correct for the missing bonds; 2 - Replace
the missing bonds with bonds to dummy atoms \param atomorder Record the Idxs of
the original atoms. That is, the first element in this vector will be the Idx of
the atom in the original OBMol that corresponds to the first atom in the new
OBMol. Note that the information is appended to this vector. \param bondorder
Record the Idxs of the original bonds. See \p atomorder above.

**/

bool OBMol::CopySubstructure(OBMol &newmol, OBBitVec *atoms,
                             OBBitVec *excludebonds,
                             unsigned int correctvalence,
  ...
  // If the parent is set to periodic, then also apply boundary conditions to
  // the fragments
  if (IsPeriodic()) {
  ...
  for (int bit = atoms->FirstBit(); bit != atoms->EndBit();
  ...
  std::vector<OBGenericData *> stereoData =
      GetAllData(OBGenericDataType::StereoData);
  for (data = stereoData.begin(); data != stereoData.end(); ++data) {
    if (static_cast<OBStereoBase *>(*data)->GetType() == OBStereo::CisTrans) {
      OBCisTransStereo *ct = dynamic_cast<OBCisTransStereo *>(*data);

      // Check that the entirety of this cistrans cfg occurs in this
      // substructure
      ...
      for (OBStereo::RefIter ri = cfg.refs.begin(); ri != cfg.refs.end();
           ++ri) {
        if (*ri != OBStereo::ImplicitRef &&
            AtomMap.find(GetAtomById(*ri)) == AtomMap.end()) {
          skip_cfg = true;
          break;
        }
      }
      ...
      newcfg.begin = cfg.begin == OBStereo::ImplicitRef
                         ? OBStereo::ImplicitRef
                         : AtomMap[GetAtomById(cfg.begin)]->GetId();
      newcfg.end = cfg.end == OBStereo::ImplicitRef
                       ? OBStereo::ImplicitRef
                       : AtomMap[GetAtomById(cfg.end)]->GetId();
      OBStereo::Refs refs;
      for (OBStereo::RefIter ri = cfg.refs.begin(); ri != cfg.refs.end();
           ++ri) {
        OBStereo::Ref ref = *ri == OBStereo::ImplicitRef
                                ? OBStereo::ImplicitRef
                                : AtomMap[GetAtomById(*ri)]->GetId();
        refs.push_back(ref);
      }
      newcfg.refs = refs;
    ...
    } else if (static_cast<OBStereoBase *>(*data)->GetType() ==
      ...
      if (cfg.from != OBStereo::ImplicitRef &&
          AtomMap.find(GetAtomById(cfg.from)) == AtomMap.end())
        continue;
      bool skip_cfg = false;
      if (bonds_specified) {
      ...
      newcfg.from = cfg.from == OBStereo::ImplicitRef
                        ? OBStereo::ImplicitRef
                        : AtomMap[GetAtomById(cfg.from)]->GetId();
      OBStereo::Refs refs;
      for (OBStereo::RefIter ri = cfg.refs.begin(); ri != cfg.refs.end();
           ++ri) {
        OBStereo::Ref ref = *ri == OBStereo::ImplicitRef
  ...
  FOR_BONDS_OF_MOL(bond, this) {
    bool skipping_bond =
        bonds_specified && excludebonds->BitIsSet(bond->GetIdx());
    ...
    if (posB == AtomMap.end() || posE == AtomMap.end() || skipping_bond) {
      ...
      case 1:
        if (posB == AtomMap.end() || (skipping_bond && posE != AtomMap.end()))
          posE->second->SetImplicitHCount(posE->second->GetImplicitHCount() +
                                          bond->GetBondOrder());
        if (posE == AtomMap.end() || (skipping_bond && posB != AtomMap.end()))
          posB->second->SetImplicitHCount(posB->second->GetImplicitHCount() +
                                          bond->GetBondOrder());
        break;
      case 2: {
        OBAtom *atomB, *atomE;
        if (skipping_bond) {
          ...
          newmol.AddBond(atomB->GetIdx(), atomE->GetIdx(), bond->GetBondOrder(),
                         bond->GetFlags());
          if (record_bondorder)
            bondorder->push_back(bond->GetIdx());
        }
...
// Put the specified molecular charge on a single atom (which is expected for
// InChIFormat). Assumes all the hydrogen is explicitly included in the
// molecule, and that SetTotalCharge() has not been called. (This function is an
// alternative.) Returns false if cannot assign all the charge. Not robust in
// the general case, but see below for the more common simpler cases.
bool OBMol::AssignTotalChargeToAtoms(int charge) {
  int extraCharge =
      charge - GetTotalCharge(); // GetTotalCharge() gets charge on atoms
  ...
  if (extraCharge != 0) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Unable to assign all the charge to atoms", obWarning);
    return false;
  }
  return true;
}
