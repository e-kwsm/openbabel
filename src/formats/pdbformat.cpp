...
bool PDBFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    snprintf(buffer, BUFF_SIZE,
             "%s%5d %-4s %-3s %c%4d%c   %8.3f%8.3f%8.3f%6.2f  0.00          "
             "%2s%2s\n",
             het ? "HETATM" : "ATOM  ", i, type_name, the_res, the_chain,
             res_num, the_insertioncode, atom->GetX(), atom->GetY(),
             atom->GetZ(), occup, element_name, scharge);
    ofs << buffer;
  }
