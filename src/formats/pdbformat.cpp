...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-pdb"; }
...
static bool readIntegerFromRecord(char *buffer,
                                  unsigned int columnAsSpecifiedInPDB,
                                  long int *target);
...
bool PDBFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (EQn(buffer, "CRYST1", 6)) {
      ...
      sscanf(&(buffer[6]), "%9f%9f%9f%7f%7f%7f", &a, &b, &c, &alpha, &beta,
             &gamma);
      buffer[66] = '\0';
      group += &(buffer[55]);
      Trim(group);
...
/*! This function reads a CONECT record, as specified
  ...
  bonded atom 37 - 41         Integer          serial          Serial number of
  hydrogen bonded atom 42 - 46         Integer          serial          Serial
  number of salt bridged atom 47 - 51         Integer          serial Serial
  number of hydrogen bonded atom 52 - 56         Integer          serial Serial
  number of hydrogen bonded atom 57 - 61         Integer          serial Serial
...
bool parseConectRecord(char *buffer, OBMol &mol) {
  ...
  if (mol.NumAtoms() <= 9999) {
    // make sure we don't look at salt bridges or whatever, so cut the buffer
    // short
    buffer[32] = '\0';
    tokenize(vs, buffer);
  ...
  for (OBAtom *a1 = mol.BeginAtom(i); a1; a1 = mol.NextAtom(i)) {
    ...
    if (a1->GetResidue() != nullptr &&
        static_cast<long int>(a1->GetResidue()->GetSerialNum(a1)) ==
            startAtomSerialNumber) {
      firstAtom = a1;
      break;
    }
  }
  ...
  if (mol.NumAtoms() < 9999) {
    if (vs.size() > 1)
      boundedAtomsSerialNumbers[0] = atoi(vs[1].c_str());
    if (vs.size() > 2)
      boundedAtomsSerialNumbers[1] = atoi(vs[2].c_str());
    if (vs.size() > 3)
      boundedAtomsSerialNumbers[2] = atoi(vs[3].c_str());
    if (vs.size() > 4)
      boundedAtomsSerialNumbers[3] = atoi(vs[4].c_str());
    ...
    // exit gracefully.
    boundedAtomsSerialNumbersValid[0] =
        readIntegerFromRecord(buffer, 12, boundedAtomsSerialNumbers + 0);
    if (boundedAtomsSerialNumbersValid[0] == false)
      return (true);
    boundedAtomsSerialNumbersValid[1] =
        readIntegerFromRecord(buffer, 17, boundedAtomsSerialNumbers + 1);
    boundedAtomsSerialNumbersValid[2] =
        readIntegerFromRecord(buffer, 22, boundedAtomsSerialNumbers + 2);
    boundedAtomsSerialNumbersValid[3] =
        readIntegerFromRecord(buffer, 27, boundedAtomsSerialNumbers + 3);
  }
  ...
  for (unsigned int k = 0; boundedAtomsSerialNumbersValid[k]; k++) {
    ...
    if (connectedAtom == nullptr) {
      errorMsg << "WARNING: Problems reading a PDB file:\n"
               << "  Problems reading a CONECT record.\n"
               << "  According to the PDB specification,\n"
               << "  Atoms with serial #" << startAtomSerialNumber << " and #"
               << boundedAtomsSerialNumbers[k] << " should be connected\n"
               << "  However, an atom with serial #"
               << boundedAtomsSerialNumbers[k] << " was not found.\n"
               << "  THIS CONECT RECORD WILL BE IGNORED." << endl;
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
      return (false);
    }
    ...
    while (boundedAtomsSerialNumbersValid[k + order + 1] &&
           (boundedAtomsSerialNumbers[k + order] ==
    ...
    // Generate the bond
    if (firstAtom->GetIdx() <
        connectedAtom->GetIdx()) { // record the bond 'in one direction' only
      OBBond *bond = mol.GetBond(firstAtom, connectedAtom);
      if (!bond)
        mol.AddBond(firstAtom->GetIdx(), connectedAtom->GetIdx(), order + 1);
      else // An additional CONECT record with the same firstAtom that
           // references a bond created in the previous CONECT record. For
           // example, the 1136->1138 double bond in the following:
...
bool PDBFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  std::vector<OBGenericData *> pairData =
      mol.GetAllData(OBGenericDataType::PairData);
  for (std::vector<OBGenericData *>::iterator data = pairData.begin();
       data != pairData.end(); ++data) {
    OBPairData *pd = static_cast<OBPairData *>(*data);
    string attr = pd->GetAttribute();

    // filter to make sure we are writing pdb fields only
    if (attr != "HEADER" && attr != "OBSLTE" && attr != "TITLE" &&
        attr != "SPLIT" && attr != "CAVEAT" && attr != "COMPND" &&
        attr != "SOURCE" && attr != "KEYWDS" && attr != "EXPDTA" &&
        attr != "NUMMDL" && attr != "MDLTYP" && attr != "AUTHOR" &&
        attr != "REVDAT" && attr != "SPRSDE" && attr != "JRNL" &&
        attr != "REMARK" && attr != "DBREF" && attr != "DBREF1" &&
        attr != "DBREF2" && attr != "SEQADV" && attr != "SEQRES" &&
        attr != "MODRES" && attr != "HET" && attr != "HETNAM" &&
        attr != "HETSYN" && attr != "FORMUL" && attr != "HELIX" &&
        attr != "SHEET" && attr != "SSBOND" && attr != "LINK" &&
        attr != "CISPEP" && attr != "SITE" && attr != "ORIGX1" &&
        attr != "ORIGX2" && attr != "ORIGX3" && attr != "SCALE1" &&
        attr != "SCALE2" && attr != "SCALE3" && attr != "MATRIX1" &&
        attr != "MATRIX2" && attr != "MATRIX3" && attr != "MODEL")
      continue;
  ...
  if (!authorWritten) {
    snprintf(buffer, BUFF_SIZE, "AUTHOR    GENERATED BY OPEN BABEL %s",
             BABEL_VERSION);
    ofs << buffer << endl;
  }
  ...
  if (pmol->HasData(OBGenericDataType::UnitCell)) {
    ...
    if (pUC->GetSpaceGroup()) {
      ...
      snprintf(buffer, BUFF_SIZE,
               "CRYST1%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %-11s 1", pUC->GetA(),
               pUC->GetB(), pUC->GetC(), pUC->GetAlpha(), pUC->GetBeta(),
               pUC->GetGamma(), tmpHM.c_str());
    } else
      snprintf(buffer, BUFF_SIZE,
               "CRYST1%9.3f%9.3f%9.3f%7.2f%7.2f%7.2f %-11s 1", pUC->GetA(),
               pUC->GetB(), pUC->GetC(), pUC->GetAlpha(), pUC->GetBeta(),
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    if (atom->HasData("_atom_site_occupancy")) {
      OBPairFloatingPoint *occup_fp = dynamic_cast<OBPairFloatingPoint *>(
    ...
    snprintf(
        buffer, BUFF_SIZE,
        "%s%5d %-4s %-3s %c%4d%c   %8.3f%8.3f%8.3f%6.2f  0.00      %4s%2s%2s\n",
        het ? "HETATM" : "ATOM  ", i, type_name, the_res, the_chain, res_num,
        the_insertioncode, atom->GetX(), atom->GetY(), atom->GetZ(), occup,
        segname, element_name, scharge);
    ofs << buffer;
  }
...
static void fixRhombohedralSpaceGroupWriter(string &strHM) {
  ...
  const int SIZE = 7;
  const char *groups[SIZE] = {"R 3:H",   "R -3:H",   "R 3 2:H", "R 3 m:H",
                              "R 3 c:H", "R -3 m:H", "R -3 c:H"};
...
static void fixRhombohedralSpaceGroupReader(string &strHM) {
  /* This is due to the requirment of PDB to name rhombohedral groups
     with H (http://deposit.rcsb.org/adit/docs/pdb_atom_format.html) */
  const int SIZE = 7;
  const char *groups[SIZE] = {"H 3",   "H -3",   "H 3 2", "H 3 m",
                              "H 3 c", "H -3 m", "H -3 c"};
...
/* ATOMFORMAT "(i5,1x,a4,a1,a3,1x,a1,i4,a1,3x,3f8.3,2f6.2,a2,a2)" */
{
    ...
    if (EQn(buffer, "ATOM", 4)) {
      ...
      if (isdigit(type[0])) {
        ...
        if (!isdigit(type[1]))
          ...
          type = atmid.substr(2, 1);
      } else if ((sbuf[6] == ' ' && strncasecmp(type.c_str(), "Zn", 2) != 0 &&
      ...
      // fix: #2002557
      if (atmid[0] == 'H' &&
          (atmid[1] == 'D' || atmid[1] == 'E' || atmid[1] == 'G' ||
           atmid[1] == 'H' || atmid[1] == 'N')) // HD, HE, HG, HH, HN...
        type = "H";
  ...
  if (elementFound)
    ...
    if (atomic_num ==
        0) { // try one character if two character element not found
      type = type.substr(0, 1);
  ...
  if (res == nullptr || res->GetName() != resname ||
      res->GetNumString() != resnum || res->GetChain() != chain ||
