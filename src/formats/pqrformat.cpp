...
public:
  ...
  PQRFormat() { OBConversion::RegisterFormat("pqr", this, "chemical/x-pqr"); }
  ...
  const char *GetMIMEType() override { return "chemical/x-pqr"; }
...
bool PQRFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    // snprintf(buffer, BUFF_SIZE, "%s%5d %-4s %-3s %c%4d %8.3f%8.3f%8.3f  1.00
    // 0.00          %2s  \n",
    snprintf(buffer, BUFF_SIZE,
             "%s%5d %-4s %-3s %c%4d    %8.3f%8.3f%8.3f %11.8f%8.3f %2s  \n",
             het ? "HETATM" : "ATOM  ", i, type_name, the_res, the_chain,
             res_num, atom->GetX(), atom->GetY(), atom->GetZ(),
