...
#define ANGSTROM_TO_BOHR 1.889725989
  ...
  bool DALTONInputFormat::ReadMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    while(ifs.getline(buffer,BUFF_SIZE))
    {
      ...
      {
        basisformat = INTGRL;
        cout << "Cannot read INTGRL format" << '\n';
        return(false);
      }
      ...
      {
        cout << "AtomTypes not specified in file." << '\n';
        return(false);
      }
      ...
      {
        cout << "Only molecules with NoSymmetry can be read" << '\n';
        return(false);
      }
      ...
      while(atomtypes >= 0 && ifs.getline(buffer, BUFF_SIZE))
      {
        ...
        {
           cout << "ZMAT format not supported" << '\n';
           return(false);
        }
        tokenize(vs,buffer);
        if(vs.size() == 4)
  ...
  bool DALTONInputFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs << mol.GetTitle() << '\n';
    ofs << "Generated by Open Babel. Check overall charge below." << '\n';
