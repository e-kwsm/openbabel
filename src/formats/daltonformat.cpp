...
public:
  ...
  const char *SpecificationURL() override {
    return "http://daltonprogram.org/www/resources/dalton2016manual.pdf";
  } // optional

  const char *GetMIMEType() override { return "chemical/x-dalton-output"; }
...
public:
  ...
  DALTONInputFormat() {
    ...
    OBConversion::RegisterOptionParam(
        "a", nullptr, 0, OBConversion::OUTOPTIONS); // write atomic units
    OBConversion::RegisterOptionParam(
        "b", nullptr, 0, OBConversion::OUTOPTIONS); // write atombasis format
    OBConversion::RegisterOptionParam(
        "k", nullptr, 1,
  ...
  {
    return "DALTON input format\n"
           "Read Options e.g. -as\n"
           "  s  Output single bonds only\n"
           "  b  Disable bonding entirely\n\n"

           "Write Options e.g. -xa\n"
           "  a                write input in atomic units instead of "
           "Angstrom\n"
  ...
  const char *GetMIMEType() override { return "chemical/x-dalton-input"; }
...
bool DALTONInputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  basisformat =
      BASIS; // Always assume BASIS format. We change it below otherwise
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    if (atom->GetAtomicNum() != atomtype) {
      ...
      snprintf(buffer, BUFF_SIZE, "Charge=%d.0 Atoms=%i%s",
               groupcharges[atomtypes - 1], groupcounts[atomtypes - 1],
               atombasis_str.c_str());
      ofs << buffer << endl;
    }
    snprintf(buffer, BUFF_SIZE, "%-3s %22.10f  %14.10f  %14.10f ",
             OBElements::GetSymbol(atom->GetAtomicNum()), atom->GetX() * factor,
             atom->GetY() * factor, atom->GetZ() * factor);
    ofs << buffer << endl;
  }
  return (true);
}
