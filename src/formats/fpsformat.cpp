...
public:
  ...
  {
    return "FPS text fingerprint format (Dalke)\n\n"
           "The FPS file format for fingerprints was developed by Andrew Dalke "
           "to\n"
           ...
           "Any molecule without a title is given its index in the file as "
           "title.\n\n"
...
bool FPSFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pConv->GetOutputIndex() == 1) {
    ...
    if (_nbits ==
        0) // if not folded, use first number on second line of the description
    {
  ...
  // truncate to hex from whole number of bytes (seems to be the way fps does
  // it)
...
string FPSFormat::getTimeStr() {
  ...
  time_res = strftime(timestr, TIME_STR_SIZE, "%Y-%m-%dT%H:%M:%S",
