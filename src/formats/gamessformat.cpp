  ...
  */
  ...
  bool GAMESSInputFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv) {
    ...
    if (keywordsEnable) {
      ...
      if (gmsset) {
        for (std::vector<OBGenericData*>::iterator i=gmsset->GetBegin(); i != gmsset->GetEnd(); ++i) {
          ...
          if (cset) {
            ...
            for (std::vector<OBGenericData*>::iterator j=cset->GetBegin(); j != cset->GetEnd(); ++j) {
                ...
                if (a + 2 + pd->GetAttribute().length() + pd->GetValue().length() > 72) {
                  ...
                  while (s.length() > 0) {
                    if (s.find(' ') != string::npos) {
                      ...
                      // Try wrapping
                      for (std::vector<int>::reverse_iterator rit=spacePositions.rbegin(); rit != spacePositions.rend(); ++rit) {
                        if (a + 1 + (*rit) <= 72) {
                          ofs << " " << s.substr(0, *rit)
                              << endl << "   ";
                          a = 3;
