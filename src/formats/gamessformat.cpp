...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-gamess-output"; }
...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-gamess-input"; }
...
bool GAMESSOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  int numFreq, numIntens,
      numDisp; // GAMESS prints rotations & transl., which we ignore
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "ATOMIC                      COORDINATES (BOHR)") !=
        nullptr) {
    ...
    } else if (strstr(buffer, "COORDINATES OF ALL ATOMS ARE (ANGS)") !=
               nullptr) {
      ifs.getline(buffer, BUFF_SIZE); // column headings
      ifs.getline(buffer, BUFF_SIZE); // ---------------
      ifs.getline(buffer, BUFF_SIZE);
    ...
    } else if (strstr(buffer, "NSERCH=") != nullptr &&
               strstr(buffer, "ENERGY=") != nullptr) {
    ...
    } else if (strstr(buffer, "TAKEN AS ROTATIONS AND TRANSLATIONS") !=
               nullptr) {
      tokenize(vs, buffer);
      if (vs.size() < 4)
        break;
      ...
      for (;;) {
        ...
        if (vs.size() > 2) {
          ...
          for (unsigned int i = 1;
               i < vs.size() && vs[i].substr(0, 4) != "$END"; i++) {
  ...
  if (keywordsEnable) {
    ...
    if (cset) {
      ...
      if (pd) {
        ...
        if (value == "GRADIENT" || value == "HESSIAN" || value == "RAMAN" ||
            value == "OPTIMIZE" || value == "SADPOINT") {
...
bool GAMESSInputFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
              ...
              if (a + 2 + pd->GetAttribute().length() +
                ...
                while (s.length() > 0) {
                  if (s.find(' ') != string::npos) {
                    ...
                    // Try wrapping
                    for (rit = spacePositions.rbegin();
                         rit != spacePositions.rend(); ++rit) {
                      if (a + 1 + (*rit) <= 72) {
                        ofs << " " << s.substr(0, *rit) << endl << "   ";
                        a = 3;
                        s = s.substr(*rit);
                        wrapped = true;
                        break;
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    snprintf(buffer, BUFF_SIZE, "%-3s %4d.0    %14.10f  %14.10f  %14.10f ",
             OBElements::GetSymbol(atom->GetAtomicNum()), atom->GetAtomicNum(),
