...
public:
  ...
  {
    return "CAChe MolStruct format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override { return ""; } // optional

  const char *GetMIMEType() override { return "chemical/x-cache"; }
...
bool CacheFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    snprintf(buffer, BUFF_SIZE, "%3d %10.6f %10.6f %10.6f %2d %2s %2d 0x7052",
             atom->GetIdx(), atom->x(), atom->y(), atom->z(),
             atom->GetAtomicNum(), type_name, atom->GetFormalCharge());
    ofs << buffer << endl;
  }
  ...
  for (bond = mol.BeginBond(j), k = 1; bond; bond = mol.NextBond(j)) {
    snprintf(buffer, BUFF_SIZE, "%3d 0xa1 atom bond %d %d\n", k++,
             bond->GetBeginAtomIdx(), bond->GetIdx() + 1);
    ofs << buffer;
    snprintf(buffer, BUFF_SIZE, "%3d 0xa1 atom bond %d %d\n", k++,
             bond->GetEndAtomIdx(), bond->GetIdx() + 1);
    ofs << buffer;
  }
