...
public:
  ...
  const char *Description() override {
    return "PNG 2D depiction\n"
           ...
           "The parameter of the ``-xO`` option specifies the format "
           "(\"file\"can be added).\n"
           "Note that if you intend to embed a 2D or 3D format, you may have "
           "to call\n"
           "``--gen2d`` or ``--gen3d`` to generate the required coordinates if "
           "they are\n"
           "not present in the input.\n\n"

           "Molecules can also be embedded in an existing PNG file::\n\n"
           "  obabel existing.png mymol1.smi mymol2.mol -O augmented.png -xO "
           "mol\n\n"

           "Reading from a PNG file will extract any embedded chemical "
           "structure data::\n\n"
           ...
           "possible to extract and embed chemical data in :file:`.png` "
           "files.\n"
           "\n"
  ...
  const char *TargetClassDescription() override {
    ...
    txt += OBFormat::TargetClassDescription(); // to display OBMol options in
                                               // GUI
    return txt.c_str();
  }
...
private:
  ...
  unsigned
      bytesToIEND; // number of bytes up to but not including the IEND chunk.
  unsigned origBytesToIEND; // saved between WriteMolecule calls
  bool _hasInputPngFile;

  // Read and write number consisting of 4 bytes with most significant bytes
  // first. Should be independent of compiler and platform.
...
bool PNGFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
    ...
    if (chunkid == "tEXt" || chunkid == "zTXt" || (altid && chunkid == altid)) {
      ...
      if (pFormat) {
        ...
        if (chunkid[0] != 'z') {
          ...
          uLongf uncompLen;
          Bytef *pUncTxt =
              new Bytef[datalength * 6]; // guess uncompressed length. NASTY!
          if (*pCompTxt != 0             /*compression method*/
              || uncompress(pUncTxt, &uncompLen, pCompTxt + 1, datalength) !=
                     Z_OK) {
            obErrorLog.ThrowError("PNG Format", "Errors in decompression",
                                  obError);
            delete[] pUncTxt;
            delete[] pCompTxt;
            return false;
  ...
  if (pConv->GetOutFormat() == this) {
    ...
    copy(istreambuf_iterator<char>(ifs), istreambuf_iterator<char>(),
         back_inserter(CopyOfInput));
  }

  if (pConv->IsLastFile() && _count > 0) {
    pConv->ReportNumberConverted(
        _count); // report the number of chemical objects
    pConv->SetOutFormat(
        this); // so that number of files is reported as "PNG_files"
  }
...
bool PNGFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!CopyOfInput.empty() && bytesToIEND > 0) {
    // copy the generated or saved png file, except the IEND chunk, to the
    // output
    ostreambuf_iterator<char> outiter(pConv->GetOutStream()->rdbuf());
    // In Windows the output stream needs to be in binary mode to avoid extra
    // CRs here
  ...
  OBConversion conv2;
  conv2.CopyOptions(
      pConv); // So that can use commandline options in this conversion
  string formatID;
  if (otxt && *otxt) {
    formatID = otxt;
  ...
  if (ret) {
    ...
    obErrorLog.ThrowError("PNG Format", "Failed when converting the molecule",
                          obError);
...
decompressed.
The keyword in the chunk should be an OpenBabel Format ID, optionally with file
added, e.g. cml, InChI, molfile. There can be multiple molecules in each chunk,
multiple chunks with chemical info and multiple png files can be read together.
...
files in any format. Each can contain multiple molecules. Each molecule is
output in a separate chunk in a format specified by the -xO option. the default
with no option is InChI. The chunk ID is normally tEXt but can be specified in
the -xa option. For example obabel OrigImg.png Firstmol.smi Secondmol.mol2
OutImg.png -xO "cml" -xa "chEm"
