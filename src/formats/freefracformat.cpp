...
public:
  ...
  FreeFormFractionalFormat() { OBConversion::RegisterFormat("fract", this); }

  const char *Description() override // required
  {
    return "Free Form Fractional format\n"
           "General purpose crystallographic format\n"
           "The \"free-form\" fractional format attempts to allow for input "
           "from a\n"
           "range of fractional / crystallography file formats. As such, it "
           "has only\n"
           "a few restrictions on input:\n\n"

           "- Line one of the file contains a title or comment.\n"
           "- Line two of the file contains the unit cell parameters separated "
           "by\n"
           "  whitespace and/or commas (i.e. \"a b c alpha beta gamma\").\n"
           "- Any remaining lines are parsed for atom information. Lines start "
           "with\n"
           "  the element symbol, followed by fractional X, Y, and Z "
           "coordinates\n"
           "  (in angstroms) separated by whitespace.\n\n"

           "Any numeric input (i.e., unit cell parameters, XYZ coordinates) "
           "can include\n"
           "designations of errors, although this is currently ignored. For "
           "example::\n\n"
           ...
           "When used as an **output** format, The first line written is the "
           "title of the\n"
           "molecule or the filename if no title is defined. If a molecule has "
           "a defined\n"
           "unit cell, then the second line will be formatted as::\n\n"

           "  a b c alpha beta gamma\n\n"

           "where a, b, c are the unit cell vector lengths, and alpha, beta, "
           "and gamma are\n"
           "the angles between them. These numbers are formatted as \"10.5\", "
           "which means that\n"
           "5 decimal places will be output for all numbers. In the case where "
           "no unit cell\n"
           "is defined for the molecule, the vector lengths will be defined as "
           "1.0, and the\n"
           "angles to 90.0 degrees.\n\n"

           "Remaining lines define the atoms in the file. The first column is "
           "the atomic\n"
           "symbol, followed by the XYZ coordinates in 10.5 format (in "
           "angstroms).\n\n"
  ...
  const char *SpecificationURL() override {
    return "http://openbabel.org/wiki/Free_Form_Fractional";
  } // optional
...
bool FreeFormFractionalFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading a free form fractional file: Could "
                          "not read the first line (title/comments).",
                          obWarning);
    return (false);
  }
  if (strlen(buffer) != 0)
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Problems reading a free form fractional file: Could not read the "
        "second line (unit cell parameters a b c alpha beta gamma).",
        obWarning);
    return (false);
  }
  vector<string> vs;
...
bool FreeFormFractionalFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!mol.HasData(OBGenericDataType::UnitCell))
    ...
    snprintf(buffer, BUFF_SIZE, "%10.5f%10.5f%10.5f%10.5f%10.5f%10.5f",
             uc->GetA(), uc->GetB(), uc->GetC(), uc->GetAlpha(), uc->GetBeta(),
             uc->GetGamma());
    ofs << buffer << "\n";
  }
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    snprintf(buffer, BUFF_SIZE, "%s %10.5f%10.5f%10.5f",
             OBElements::GetSymbol(atom->GetAtomicNum()), v.x(), v.y(), v.z());
    ofs << buffer << endl;
  }
