...
// since there's no actual molecular data. For now, we'll read in an OBMol with
// no atoms and attach an OBGridData
class OBOpenDXCubeFormat : public OpenBabel::OBMoleculeFormat {
public:
  // Constructor: register "dx" extension code
  OBOpenDXCubeFormat() { OpenBabel::OBConversion::RegisterFormat("dx", this); }

  // Return description.
  const char *Description() override // required
  {
    return "OpenDX cube format for APBS\n"
  ...
  */
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (EQn(buffer, "attribute", 9))
      break; // we're finished with reading data -- although we should probably
             // have a voxel check in here too
    ...
    for (unsigned int l = 0; l < vs.size(); ++l) {
      values.push_back(
          strtod(static_cast<const char *>(vs[l].c_str()), &endptr));
    }
  }
...
bool OBOpenDXCubeFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  snprintf(buffer, BUFF_SIZE, "object 1 class gridpositions counts %5d %5d %5d",
           nx, ny, nz);
  ofs << buffer << "\n";

  // data line 2: origin (x, y, z)
  snprintf(buffer, BUFF_SIZE, "origin %12.6f %12.6f %12.6f", origin[0],
           origin[1], origin[2]);
  ofs << buffer << "\n";

  // data line 3: x-displacement
  snprintf(buffer, BUFF_SIZE, "delta %12.6f %12.6f %12.6f", xAxis[0], xAxis[1],
           xAxis[2]);
  ofs << buffer << "\n";

  // data line 4: y-displacement
  snprintf(buffer, BUFF_SIZE, "delta %12.6f %12.6f %12.6f", yAxis[0], yAxis[1],
           yAxis[2]);
  ofs << buffer << "\n";

  // data line 5: z-displacement
  snprintf(buffer, BUFF_SIZE, "delta %12.6f %12.6f %12.6f", zAxis[0], zAxis[1],
           zAxis[2]);
  ofs << buffer << "\n";

  // data line 6: # of points in x, y, z (nx, ny, nz)
  snprintf(buffer, BUFF_SIZE,
           "object 2 class gridconnections counts %5d %5d %5d", nx, ny, nz);
  ofs << buffer << "\n";

  // data line 7: total # of points
  snprintf(buffer, BUFF_SIZE,
           "object 3 class array type double rank 0 items %5d data follows",
           nx * ny * nz);
  ofs << buffer << "\n";
