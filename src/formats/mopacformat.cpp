...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-mopac-out"; }

  /// The "API" interface functions
  bool ReadMolecule(OBBase *pOb, OBConversion *pConv) override;
  //	virtual bool WriteMolecule(OBBase* pOb, OBConversion* pConv); Is Read
  //Only
};
...
bool MOPACFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "  CARTESIAN COORDINATES") != nullptr) {
      ...
      tokenize(vs, buffer);
      if (vs.size() != 5 ||
          vs[0][0] != '1') { // first character should be atom 1
    ...
    else if (strstr(buffer, "(ANGSTROMS)") != nullptr &&
             strstr(buffer, "(DEGREES)") ==
                 nullptr) { // newer versions don't print CARTESIAN for final
                            // geometry
      mol.Clear();
      ifs.getline(buffer, BUFF_SIZE); // blank
      ifs.getline(buffer, BUFF_SIZE);
    ...
    // Optimized translation vectors:
      ...
      while (vs.size() == 8) {
        ...
        case 'Z':
          ...
          break;
        default:
          cerr << "Reading MOPAC Tv values: unknown coordinate '" << coord
               << "', value: " << val << endl;
          break;
        }
    ...
    } else if (strstr(buffer, "NC:NB:NA:I") != nullptr) // z-matrix
    {
      ...
      if (!ifs.getline(buffer, BUFF_SIZE))
        break;
               ...
               nullptr) { // the correct vibrations -- earlier bits aren't
                          // mass-weighted
      ...
      intensities.push_back(frequencies[currentIntensity] * transDipole *
                            transDipole);
    }
  }
...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-mopac-input"; }
...
// B. Header
//
// MOPAC supports line continuation for keywords using the special keywords &
// and + if & is present, keywords continue on the next line & may be specified
// on lines 1 and 2 only the total length of the header remains fixed at three
// lines; the number of lines available for description is reduced accordingly
...
bool MOPACCARTFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (vs.size() > 3) // Only one label allowed per line
    {
      ...
      cerr << "Invalid format in geometry specification: There appears to be "
              "more than one atom label specified!\n";
      return false;
    ...
    // 1. There are 7 tokens and the first token is a number specifying the
    // isotope mass
    // 2. There are 7 tokens and the first token is a string containing the
    // element symbol
    // 3. There are 6 tokens and the first token is a number specifying the
    ...
    {
      // TODO Replace with correct OBError.ThrowError() call
      cerr << "Mixed Cartesian and internal coordinates are currently not "
              "supported.\n";
      return false;
    } else if (vs.size() == 7) {
      if (elementSymbol == "")
...
bool MOPACCARTFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (uc && writeUnitCell) {
    //      uc->FillUnitCell(&mol); // complete the unit cell with
    //      symmetry-derived atoms

    vector<vector3> cellVectors = uc->GetCellVectors();
    for (vector<vector3>::iterator i = cellVectors.begin();
         i != cellVectors.end(); ++i) {
      snprintf(buffer, BUFF_SIZE, "Tv %8.5f 1 %8.5f 1 %8.5f 1", i->x(), i->y(),
               i->z());
      ofs << buffer << "\n";
    }
  }
...
bool MOPACINTFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    if (t < 0)
      t += 360;
    snprintf(buffer, BUFF_SIZE, "%-2s %10.6f  1  %10.6f  1  %10.6f  1  ", type,
             r, w, t);
    ofs << buffer;
    if (atom->GetIdx() == 1)
