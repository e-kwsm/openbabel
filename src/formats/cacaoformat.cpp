...
public:
  ...
  CacaoFormat() { OBConversion::RegisterFormat("caccrt", this); }

  const char *Description() override // required
  {
    return "Cacao Cartesian format\n"
           "Read Options e.g. -as\n"
  ...
  const char *SpecificationURL() override {
    return "http://www.chembio.uoguelph.ca/oakley/310/cacao/cacao.htm"; // XXX
                                                                        // dead
  }
...
bool CacaoFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!mol.HasData(OBGenericDataType::UnitCell))
    ...
    snprintf(buffer, BUFF_SIZE, "CELL %f,%f,%f,%f,%f,%f\n", uc->GetA(),
             uc->GetB(), uc->GetC(), uc->GetAlpha(), uc->GetBeta(),
             uc->GetGamma());
    ofs << buffer;
  }
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    snprintf(buffer, BUFF_SIZE, "%2s %7.4f, %7.4f, %7.4f\n",
             OBElements::GetSymbol(atom->GetAtomicNum()), atom->x(), atom->y(),
             atom->z());
    ofs << buffer;
  }
...
void CacaoFormat::SetHilderbrandt(OBMol &mol, vector<OBInternalCoord *> &vit) {
  ...
  for (i = 2; i <= mol.NumAtoms(); i++) {
    ...
    for (j = 1; j < i; j++) {
      ...
      if (std::isfinite(r) && (r < sum) && (vit[j]->_a != a2) &&
          (vit[j]->_b != a2)) {
        sum = r;
        ref = a2;
      }
  ...
  for (i = 2; i <= mol.NumAtoms(); i++) {
    ...
    vit[i]->_tor = CalcTorsionAngle(atom->GetVector(), a->GetVector(),
...
public:
  // Register this format type ID
  CacaoInternalFormat() { OBConversion::RegisterFormat("cacint", this); }

  const char *Description() override // required
  {
    return "Cacao Internal format\n"
           "No comments yet\n";
  }
...
bool CacaoInternalFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  strncpy(tmptype, OBElements::GetSymbol(mol.GetAtom(1)->GetAtomicNum()),
          sizeof(tmptype));
  ...
  ofs << buffer;
  for (i = 2; i <= mol.NumAtoms(); i++) {
    strncpy(tmptype, OBElements::GetSymbol(mol.GetAtom(i)->GetAtomicNum()),
            sizeof(tmptype));
    tmptype[sizeof(tmptype) - 1] = '\0';
    ...
    snprintf(buffer, BUFF_SIZE, "%2d,%d,%2s%7.3f,%7.3f,%7.3f",
             vit[i]->_a->GetIdx(), i, tmptype, vit[i]->_dst, vit[i]->_ang,
             vit[i]->_tor);
    ofs << buffer << endl;
  }
