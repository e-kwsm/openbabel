...
public:
  COFFormat() { OBConversion::RegisterFormat("cof", this); }
  ...
  /* Flags() can return be any of the following combined by | or be omitted if
   none apply NOTREADABLE  READONEONLY  NOTWRITABLE  WRITEONEONLY  DEFAULTFORMAT
...
bool COFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  const char *delimstrversion =
      " \t\n\r."; // Except when extracting the version number, as that line has
                  // a special format
  tokenize(vs, buffer, delimstrversion);
  if (vs.size() < 2) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Unable to read Culgi Object File. First line "
                          "incorrectly formatted or file is empty.",
                          obWarning);
    return (false);
  }
  if (vs[0] != "@CulgiVersion:") {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Unable to read Culgi Object File. First line incorrectly formatted.",
        obWarning);
    return (false);
  }
  ...
  if (!bFoundKey) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Unable to read Culgi Object File. File does not contain a molecule",
        obWarning);
    return (false);
  }
  pmol->SetTitle(name);
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (vs[0] == "atom") {
      if (vs.size() < offset + 9) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "Unable to read Culgi Object File. Atom line appears truncated.",
            obWarning);
        return (false);
      }
      ...
      if (vs.size() < 4) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "Unable to read Culgi Object File. Bond line appears truncated.",
            obWarning);
        return (false);
      }
      int iniatom = -1;
      ...
      if (version > 7) {
        ...
        if (iniatom < 0 || finatom < 0) {
          errorMsg << "Unable to read Culgi Object File. Atoms " << vs[0]
                   << " or " << vs[1] << "not found in atom list.\n";
      ...
      if (fabs(order - 1.0) < 1.e-6)
        ...
        errorMsg << "Unable to read Culgi Object File. Bond order " << vs[3]
                 << "not possible.\n";
        obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
        return false;
      }
      ...
      if (vs.size() != 3) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "Unable to read Culgi Object File. Formal charge "
                              "line appears truncated",
                              obWarning);
        return (false);
      }
      ...
      if (chargedAtom == nullptr) {
        errorMsg
            << "Unable to read Culgi Object File. Formal charge atom index "
            << vs[1] << " not found in atom list.\n";
        obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
        return false;
      }
  ...
  // Set atoms on both side of aromatic bond as aromatic, as needed by
  // OBKekulize()
  for (pbond = pmol->BeginBond(bpos); pbond; pbond = pmol->NextBond(bpos)) {
    if (pbond->IsAromatic()) {
  ...
  if (!ok) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Failed to kekulize aromatic bonds in COF file",
                          obWarning);
  }
...
bool COFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    ofs << "atom" << "\t" << i - 1 << "\t" << ename << "\t" << elem << "\t"
        << "X" << "\t" << atom->GetPartialCharge() << "\t"
        << "0" << "\t" << atom->GetX() << "\t" << atom->GetY() << "\t"
        << atom->GetZ() << "\t"
        << "0.00000" << "\t"
        << "0.00000" << "\t"
        << "0.00000" << "\t"
