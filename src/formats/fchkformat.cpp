...
public:
  ...
  FCHKFormat() {
    ...
    OBConversion::RegisterFormat("fch", this, "chemical/x-gaussian-checkpoint");
    OBConversion::RegisterFormat("fck", this, "chemical/x-gaussian-checkpoint");
  }

  const char *Description() override {
    return "Gaussian formatted checkpoint file format\n"
           "A formatted text file containing the results of a Gaussian "
           "calculation\n"
           "Currently supports reading molecular geometries from fchk files. "
           "More to come.\n\n"
...
private:
  ...
  template <class T>
  static bool read_numbers(const char *const, vector<T> &,
                           const unsigned int width = 0);
  template <class T>
  static bool read_section(const char *const, vector<T> &, const unsigned int,
                           bool *const, const char *const, const unsigned int,
                           const unsigned int width = 0);
  bool static validate_section(const char *const, const int, const char *const,
                               const unsigned int);
  bool static validate_number(const int, const char *const, const unsigned int);
  void static construct_mol(OBMol *const, OBConversion *const,
                            const unsigned int, const vector<int> &,
...
bool FCHKFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!pifs->getline(buff, BUFF_SIZE)) {
    obErrorLog.ThrowError("FCHKFormat::ReadMolecule()",
                          "Failed to read the comment.", obError);
    return false;
  }
  ...
  while (pifs->getline(buff, BUFF_SIZE)) {
    ...
    if (buff == strstr(buff, "Number of atoms")) {
      if (!FCHKFormat::read_int(buff, &Natoms)) {
        error_msg << "Could not read the number of atoms from line #" << lineno
                  << ".";

        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
      if (0 >= Natoms) {
    ...
    if (buff == strstr(buff, "Dipole Moment")) {
      if (!pifs->getline(buff, BUFF_SIZE)) {
        error_msg << "Could not read the dipole moment after line #" << lineno
                  << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      ...
      if (!FCHKFormat::read_numbers(buff, moments) || moments.size() != 3) {
        error_msg << "Could not read the dipole moment from line #" << lineno
                  << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
    ...
    if (buff == strstr(buff, "Charge")) {
      if (!FCHKFormat::read_int(buff, &intvar)) {
        error_msg << "Could not read the charge from line #" << lineno << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (buff == strstr(buff, "Multiplicity")) {
      if (!FCHKFormat::read_int(buff, &intvar)) {
        error_msg << "Could not read the multiplicity from line #" << lineno
                  << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
    ...
    if (buff == strstr(buff, "Atomic numbers")) {
      ...
      if (!FCHKFormat::validate_section(buff, Natoms,
                                        "number of atomic numbers", lineno))
        return false;
    ...
    if (buff == strstr(buff, "Current cartesian coordinates")) {
      ...
      if (!FCHKFormat::validate_section(buff, 3 * Natoms,
                                        "number of coordinates", lineno))
        return false;
    ...
    if (buff == strstr(buff, "MxBond")) {
      ...
      if (0 >= MxBond) {
        error_msg << "Invalid number of bonds to each atom : " << MxBond << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (buff == strstr(buff, "NBond")) {
      ...
      if (!FCHKFormat::validate_number(MxBond, "number of bonds to each atom",
                                       lineno))
        return false;
    ...
    if (buff == strstr(buff, "IBond")) {
      ...
      if (!FCHKFormat::validate_section(buff, MxBond * Natoms,
                                        "number of bonds", lineno))
        return false;
    ...
    if (buff == strstr(buff, "Cartesian Force Constants")) {
      ...
      if (!FCHKFormat::validate_section(buff,
                                        ...
                                        "number of force constants", lineno))
        return false;
    ...
    if (buff == strstr(buff, "Dipole Derivatives")) {
      ...
      if (!FCHKFormat::validate_section(buff, 9 * Natoms,
                                        "number of dipole derivatives", lineno))
        return false;
    ...
    if (buff == strstr(buff, "Number of alpha electrons")) {
      ...
      if (0 >= numAElec) {
        error_msg << "Invalid number of alpha electrons: " << MxBond << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (buff == strstr(buff, "Number of beta electrons")) {
      ...
      if (0 >= numBElec) {
        error_msg << "Invalid number of beta electrons: " << MxBond << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (buff == strstr(buff, "Alpha Orbital Energies")) {
      ...
      if (0 >= numAOrb) {
        error_msg << "Invalid number of alpha orbitals: " << MxBond << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (buff == strstr(buff, "Beta Orbital Energies")) {
      ...
      if (0 >= numBOrb) {
        error_msg << "Invalid number of beta orbitals: " << MxBond << ".";
        obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                              obError);
        return false;
      }
    ...
    if (dipder_found && 9 * Natoms > (int)dipder.size()) {
      if (!FCHKFormat::read_section(buff, dipder, 9 * Natoms, &finished,
    ...
    if (alphaorb_found && (numAOrb > alphaorb.size())) {
      if (!FCHKFormat::read_section(buff, alphaorb, numAOrb, &finished,
    ...
    if (betaorb_found && (numBOrb > betaorb.size())) {
      if (!FCHKFormat::read_section(buff, betaorb, numBOrb, &finished,
  ...
  if (-1 != MxBond) {
    ...
    if (NBond.end() !=
            find_if(NBond.begin(), NBond.end(), [](int i) { return i <= 0; }) ||
        NBond.end() != find_if(NBond.begin(), NBond.end(),
                               [=](int i) { return i > MxBond; }) ||
        IBond.end() !=
            find_if(IBond.begin(), IBond.end(), [](int i) { return i < 0; }) ||
        IBond.end() != find_if(IBond.begin(), IBond.end(),
  ...
  /*** finalizing ***/
  FCHKFormat::construct_mol(pmol, pConv, Natoms, atomnos, coords, MxBond, NBond,
                            IBond);
...
bool FCHKFormat::read_numbers(const char *const line, vector<T> &v,
  ...
  if (width == 0) {
    ...
    if (0 < vs.size()) {
      ...
      for (vector<string>::const_iterator it = vs.begin(); vs.end() != it;
           ++it) {
...
*/
bool FCHKFormat::validate_section(const char *const line, const int nreq,
  ...
  if (!FCHKFormat::read_int(line, &intvar)) {
    error_msg << "Could not read the " << desc << " from line #" << lineno
              << ".";
    obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                          obError);
    return false;
  ...
  if (intvar != nreq) {
    error_msg << desc << " must be exactly " << nreq << ", found " << intvar
              << ".";
    obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                          obError);
    return false;
...
*/
bool FCHKFormat::validate_number(const int num, const char *const desc,
                                 const unsigned int lineno) {
  stringstream error_msg;

  if (-1 == num) {
    error_msg << desc << " must be already read before line #" << lineno << ".";
    obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                          obError);
    return false;
  }
...
*/
template <class T>
bool FCHKFormat::read_section(const char *const line, vector<T> &v,
                              const unsigned int nreq, bool *const finished,
                              const char *const desc, const unsigned int lineno,
                              const unsigned int width) {
  stringstream error_msg;
  ...
  *finished = false;
  ...
  if (nreq <= v.size()) {
    *finished = true;
    if (nreq < v.size()) {
      error_msg << "Ignoring the superfluous " << desc << "in line #" << lineno
                << ".";
      obErrorLog.ThrowError("FCHKFormat::ReadMolecule()", error_msg.str(),
                            obWarning);
    }
...
*/
void FCHKFormat::construct_mol(OBMol *const pmol, OBConversion *const pConv,
                               const unsigned int Natoms,
                               const vector<int> &atomnos,
                               const vector<double> &coords, const int MxBond,
