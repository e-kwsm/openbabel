...
public:
  ...
  JaguarOutputFormat() { OBConversion::RegisterFormat("jout", this); }
...
public:
  // Register this format type ID
  JaguarInputFormat() { OBConversion::RegisterFormat("jin", this); }
...
bool JaguarOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "Atomic charges from electrostatic potential") !=
        nullptr) {
      ...
      while (chgcount < mol.NumAtoms()) {
        ...
        tokenize(vs, buffer);
        for (vector<string>::size_type icount = 1; icount < vs.size();
             ++icount) {
          chgcount = chgcount + 1;
          mol.GetAtom(chgcount)->SetPartialCharge(
              atof((char *)vs[icount].c_str()));
        }
      }
...
bool JaguarInputFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    snprintf(buffer, BUFF_SIZE, "  %s%d   %12.7f  %12.7f  %12.7f",
             OBElements::GetSymbol(atom->GetAtomicNum()), i, atom->GetX(),
             atom->GetY(), atom->GetZ());
    ofs << buffer << endl;
  }
