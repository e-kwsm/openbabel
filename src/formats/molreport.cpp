...
public:
  ...
  MolReportFormat() { OBConversion::RegisterFormat("molreport", this); }

  const char *Description() override // required
  {
    return "Open Babel molecule report\n"
  ...
  const char *SpecificationURL() override {
    return "http://openbabel.org/wiki/MolReport";
  } // optional
...
bool MolReportFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    snprintf(buffer, BUFF_SIZE,
             "ATOM: %9d %3s TYPE: %-6s HYB: %2d CHARGE: %8.4f", atom->GetIdx(),
             OBElements::GetSymbol(atom->GetAtomicNum()), atom->GetType(),
