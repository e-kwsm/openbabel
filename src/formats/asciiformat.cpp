...
public:
  ...
  ASCIIFormat() { OBConversion::RegisterFormat("ascii", this); }
  ...
  const char *Description() override // required
  {
    return "ASCII format\n"
           ...
           "This format generates a 2D depiction of a molecule using only "
           "ASCII text\n"
           "suitable for a command-line console, or a text file. For "
           "example::\n\n"
           ...
           "If the image appears elongated or squat, the aspect ratio should "
           "be changed\n"
           "from its default value of 1.5 using the ``-xa <ratio>`` option. To "
           "help\n"
           "determine the correct value, use the ``-xs`` option to display a "
           "square.\n"

           "Write Options e.g. -xw 69\n"

           " w <characters> Image width in characters, default 79\n"
           " h <characters> Image height in characters, default is "
           "width/aspect\n"
           " a <ratio> Aspect ratio of character height:width, default is 1.5\n"
           " s         Display a square - this is useful for correcting the "
           "aspect ratio\n"
...
bool ASCIIFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!workingmol.Has2D(true)) {
    ...
    if (!pOp) {
      obErrorLog.ThrowError("ASCIIFormat", "gen2D not found", obError,
                            onceOnly);
      return false;
    }
  ...
  if (pConv->IsOption("s")) {
    ofs << "The above drawing is supposed to show a square. "
        << "If instead you see a squat rectangle, try again with a smaller "
           "aspect ratio, e.g.\n   -oascii -xs -xa "
        << aspect - 0.1 << "\n"
        << "If you see a tall rectangle, try again with a larger aspect ratio, "
           "e.g.\n   -oascii -xs -xa "
        << aspect + 0.1 << "\n";
  }
