...
public:
  ...
  ZINDOFormat() { OBConversion::RegisterFormat("zin", this); }

  const char *Description() override // required
  {
    return "ZINDO input format\n"
           "The input format for the semiempirical quantum-mechanics program "
           "ZINDO.\n"
...
bool ZINDOFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (cndoStyle) {
    ofs << mol.GetTitle() << '\n';
    ofs << "HAMILT= INDO CHARGE=" << pmol->GetTotalCharge()
        << " RESTART= auto\n";
    ...
    ofs << endl;
  } else { // old-style ZINDO, so we have to calculate the # of valence
           // electrons
    ...
    if (charged) {
      ...
      ofs << " NOP = 1 " << '\n';
      ofs << " NDT = 1 " << '\n';
      snprintf(buffer, BUFF_SIZE, " FOP(1) =% 4d% 10.6f", valenceE - 1, 1.0);
      ofs << buffer << '\n';
    }

    snprintf(buffer, BUFF_SIZE,
             " NAT          %4d   NEL        %4d   MULT           1",
             mol.NumAtoms(), valenceE);
    ofs << buffer << '\n';
    ...
    snprintf(buffer, BUFF_SIZE, " DYNAL(1) =     0%5d%5d    0    0 1200%5d",
             mol.NumAtoms() - mol.NumHvyAtoms(), mol.NumHvyAtoms(),
             orbitals + 25);
    ofs << buffer << '\n';
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    snprintf(buffer, BUFF_SIZE, "% 10.6f% 10.6f% 10.6f%5d", atom->GetX(),
  ...
  if (!cndoStyle) { // old-style ZINDO footer for CI specification
    ...
    if (charged)
      snprintf(buffer, BUFF_SIZE, "%5d%5d%5d%5d", 1, orbitals, orbitals,
               orbitals + 1);
    else
      snprintf(buffer, BUFF_SIZE, "%5d%5d%5d", 1, orbitals, orbitals);
    ofs << buffer << '\n';
    if (charged)
      snprintf(buffer, BUFF_SIZE, "%5d%5d%5d%5d%5d", 21, (orbitals - 8),
               orbitals + 1, orbitals + 1, orbitals + 11);
    else
      snprintf(buffer, BUFF_SIZE, "%5d%5d%5d%5d%5d", 21, (orbitals - 9),
               orbitals, orbitals + 1, orbitals + 10);
    ofs << buffer << "\n\n";
    ofs << " $END \n";
  }
