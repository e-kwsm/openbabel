...
public:
  ...
  ABINITFormat() { OBConversion::RegisterFormat("abinit", this); }

  const char *Description() override // required
  {
    return "ABINIT Output Format\n"
           "Read Options e.g. -as\n"
           "  s  Output single bonds only\n"
           "  b  Disable bonding entirely\n\n";
  }

  const char *SpecificationURL() override { return "https://www.abinit.org/"; }
...
bool ABINITFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (numTranslationVectors > 0) {
    ...
    //      uc->SetData(acell[0] * translationVectors[0], acell[1] *
    //      translationVectors[1], acell[2] * translationVectors[2]);
