...
void insertArrayListElement(ArrayList *array, void *elem, int position) {
  int i;
  while (array->length >= array->allocatedSize ||
         position >= array->allocatedSize)
    extend(array);
  ...
  array->length =
      (array->length <= position) ? position + 1 : array->length + 1;
}
...
void *findArrayListElement(const ArrayList *array, const void *item,
                           int (*comparator)(const void *item1,
                                             const void *item2)) {
  int i;
...
int findIndexOfArrayListElement(const ArrayList *array, const void *item,
                                int (*comparator)(const void *item1,
                                                  const void *item2)) {
  int i;
  for (i = 0; i < array->length; i++) {
...
static void extend(ArrayList *array) {
  ...
  memset(array->data + oldN, 0,
         (size_t)((array->allocatedSize - oldN) * sizeof(void *)));
}
