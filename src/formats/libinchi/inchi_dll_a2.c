...
int CopyLinearCTStereoToINChIStereo(
    INChI_Stereo *Stereo, AT_STEREO_CARB *LinearCTStereoCarb,
    int nLenLinearCTStereoCarb, AT_STEREO_DBLE *LinearCTStereoDble,
    int nLenLinearCTStereoDble, AT_NUMB *pCanonOrd, AT_RANK *pCanonRank,
    sp_ATOM *at, int bIsotopic, AT_STEREO_CARB *LinearCTStereoCarbInv,
    AT_STEREO_DBLE *LinearCTStereoDbleInv, AT_NUMB *pCanonOrdInv,
    AT_RANK *pCanonRankInv);
int MarkAmbiguousStereo(sp_ATOM *at, inp_ATOM *norm_at, int bIsotopic,
                        AT_NUMB *pCanonOrd, AT_STEREO_CARB *LinearCTStereoCarb,
...
int FillOutINChIReducedWarn(INChI *pINChI, INChI_Aux *pINChI_Aux, int num_atoms,
                            int num_at_tg, int num_removed_H, sp_ATOM *at,
                            inp_ATOM *norm_at, CANON_STAT *pCS, int bTautomeric,
...
int CanonOneStructureINChI(INCHIGEN_CONTROL *genctl, int iINChI,
...
int Normalization_step(INChI **ppINChI, INChI_Aux **ppINChI_Aux,
                       ...
                       int num_inp_at, INCHI_MODE *pbTautFlags,
                       INCHI_MODE *pbTautFlagsDone, COMPONENT_TREAT_INFO *cti);
int Canonicalization_step(INChI **ppINChI, INChI_Aux **ppINChI_Aux,
                          INP_ATOM_DATA *out_norm_data[2],
                          struct tagInchiTime *ulMaxTime, T_GROUP_INFO *ti_out,
                          char *pStrErrStruct, COMPONENT_TREAT_INFO *cti);
...
int NormOneStructureINChI(INCHIGEN_DATA *gendata, INCHIGEN_CONTROL *genctl,
  ...
  INCHI_IOSTREAM *output_file = genctl->inchi_file,
                 *log_file = genctl->inchi_file + 1;
  ...
  */
  ...
  /*^^^ visible */
  gendata->NormAtomsNontaut[iINChI] = (NORM_ATOMS *)inchi_calloc(
      sd->num_components[iINChI], sizeof(NORM_ATOMS));
  gendata->NormAtomsTaut[iINChI] = (NORM_ATOMS *)inchi_calloc(
      sd->num_components[iINChI], sizeof(NORM_ATOMS));
  /*^^^ invisible */
  genctl->InpNormAtData[iINChI] = (INP_ATOM_DATA *)inchi_calloc(
      sd->num_components[iINChI], sizeof(INP_ATOM_DATA));
  genctl->InpNormTautData[iINChI] = (INP_ATOM_DATA *)inchi_calloc(
      sd->num_components[iINChI], sizeof(INP_ATOM_DATA));
  genctl->InpCurAtData[iINChI] = (INP_ATOM_DATA *)inchi_calloc(
      sd->num_components[iINChI], sizeof(INP_ATOM_DATA));
  genctl->cti[iINChI] = (COMPONENT_TREAT_INFO *)inchi_calloc(
      sd->num_components[iINChI], sizeof(COMPONENT_TREAT_INFO));
  memset(genctl->cti[iINChI], 0,
         sd->num_components[iINChI] * sizeof(COMPONENT_TREAT_INFO));
...
exit_cycle:
  ...
  default:
    ...
    /*^^^ To save intermediate data... */
    if (all_inp_norm_data) {
      int res = CreateCompositeNormAtom(composite_norm_data, all_inp_norm_data,
...
int CanonOneStructureINChI(INCHIGEN_CONTROL *genctl, int iINChI,
  ...
  for (i = 0; i < TAUT_NUM;
       i++) /* initialize in case no InChI to generate 2008-12-23 DT */
...
int NormOneComponentINChI(INCHIGEN_CONTROL *genctl, int iINChI, int i) {
  ...
  */
  ...
  num_at = Normalization_step(cur_INChI, cur_INChI_Aux, inp_cur_data->at,
                              inp_norm_data, inp_cur_data->num_at, &bTautFlags,
                              &bTautFlagsDone, cti);
...
int Normalization_step(INChI **ppINChI, INChI_Aux **ppINChI_Aux,
                       inp_ATOM *inp_at, INP_ATOM_DATA *out_norm_data[2],
                       int num_inp_at, INCHI_MODE *pbTautFlags,
                       INCHI_MODE *pbTautFlagsDone, COMPONENT_TREAT_INFO *z) {
...
/*^^^ */
  ...
  for (i = 0; i < TAUT_NUM; i++) {
    if (out_norm_data[i]->at) {
      z->at[i] = (sp_ATOM *)inchi_malloc(num_inp_at * sizeof(*(z->at[0])));
  ...
  z->out_at = out_norm_data[TAUT_YES]->at ? out_norm_data[TAUT_YES]->at
  ...
  if (TG_FLAG_H_ALREADY_REMOVED & z->bTautFlags) {
    ...
    z->num_at_tg = z->num_atoms =
        remove_terminal_HDT(num_inp_at, z->out_at, z->fix_termhchrg);
  ...
  if (z->out_at != out_norm_data[TAUT_YES]->at && out_norm_data[TAUT_YES]->at)
    memcpy(out_norm_data[TAUT_YES]->at, z->out_at,
           num_inp_at * sizeof(z->out_at[0]));

  if (out_norm_data[TAUT_YES]->at_fixed_bonds && out_norm_data[TAUT_YES]->at)
    memcpy(out_norm_data[TAUT_YES]->at_fixed_bonds, z->out_at,
           num_inp_at * sizeof(z->out_at[0]));

  if (z->out_at != out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_NON]->at)
    memcpy(out_norm_data[TAUT_NON]->at, z->out_at,
           num_inp_at * sizeof(z->out_at[0]));
  ...
  if (out_norm_data[TAUT_YES]->at && z->at[TAUT_YES]) {
    ...
    ret = mark_alt_bonds_and_taut_groups(
        ...
        z->num_atoms, t_group_info, NULL, NULL);
    ...
    out_norm_data[TAUT_YES]->num_at =
        z->num_atoms +
        z->num_deleted_H_taut; /* protons might have been removed */
    ...
    ret = set_stereo_parity(out_norm_data[TAUT_YES]->at, z->at[TAUT_YES],
    ...
    z->s[TAUT_YES].bMayHaveStereo = (z->s[TAUT_YES].nMaxNumStereoAtoms ||
    ...
    if (RETURNED_ERROR(z->s[TAUT_YES].nLenLinearCTTautomer)) {
      ...
      */
      ...
      z->s[TAUT_YES].nLenLinearCTIsotopicTautomer =
      ...
      out_norm_data[TAUT_YES]->bTautomeric =
          z->s[TAUT_YES].nLenLinearCTTautomer;
    }

    /*  new variable: z->s[TAUT_YES].nLenCT introduced 7-22-2002 */

    GetCanonLengths(z->num_taut_at, z->at[TAUT_YES], &(z->s[TAUT_YES]),
                    t_group_info);

  } /* end of: final normalization of possibly tautomeric structure */

  if (out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_YES]->at &&
      z->at[TAUT_NON] && !z->s[TAUT_YES].nLenLinearCTTautomer) {
    ...
    ret = set_stereo_parity(out_norm_data[TAUT_NON]->at, z->at[TAUT_NON],
                            z->num_atoms, z->num_deleted_H,
                            &(z->s[TAUT_NON].nMaxNumStereoAtoms),
                            &(z->s[TAUT_NON].nMaxNumStereoBonds), z->nUserMode,
                            z->bPointedEdgeStereo, z->vABParityUnknown);

    if (RETURNED_ERROR(ret))
      goto exit_function; /*  stereo bond error */

    z->s[TAUT_NON].bMayHaveStereo = (z->s[TAUT_NON].nMaxNumStereoAtoms ||
  ...
  /*  common */
  z->bMayHaveStereo =
      z->s[TAUT_YES].bMayHaveStereo || z->s[TAUT_NON].bMayHaveStereo;
  z->bHasIsotopicAtoms = z->s[TAUT_NON].num_isotopic_atoms > 0 ||
                         z->s[TAUT_NON].bHasIsotopicTautGroups > 0 ||
                         z->s[TAUT_YES].num_isotopic_atoms > 0 ||
                         z->s[TAUT_YES].bHasIsotopicTautGroups > 0;
  /*^^^ */
  ...
  case (REQ_MODE_BASIC | REQ_MODE_TAUT):
    ...
    break;
  default:
    /*  program error: inconsistent nUserMode or missing taut/non-taut
     * allocation */ /*   <BRKPT> */
    ret = -3;
    goto exit_function;
  }
...
int Canonicalization_step(INChI **ppINChI, INChI_Aux **ppINChI_Aux,
                          INP_ATOM_DATA *out_norm_data[2],
                          struct tagInchiTime *ulMaxTime, T_GROUP_INFO *ti_out,
                          char *pStrErrStruct, COMPONENT_TREAT_INFO *z) {
  int i, ret = 0, ret2 = 0;
  ...
  ret = GetBaseCanonRanking(z->num_atoms, z->num_at_tg, z->at, t_group_info,
                            z->s, pBCN, ulMaxTime, z->fix_isofixedh);
  ...
  for (i = z->n2; i >= z->n1 && !RETURNED_ERROR(ret); i--) {
    ...
    case TAUT_NON: /*  non-tautomeric */
      ...
      z->nMode = 0;
      z->nMode =
          (z->s[i].nLenLinearCTTautomer == 0) ? CANON_MODE_CT : CANON_MODE_TAUT;
      z->nMode |= (z->bHasIsotopicAtoms && (z->nUserMode & REQ_MODE_ISO))
                      ? CANON_MODE_ISO
                      : 0;
      z->nMode |=
          (z->s[TAUT_NON].bMayHaveStereo && (z->nUserMode & REQ_MODE_STEREO))
              ? CANON_MODE_STEREO
              : 0;
      ...
      z->nMode |=
          (z->nUserMode & REQ_MODE_REDNDNT_STEREO) ? CMODE_REDNDNT_STEREO : 0;
      z->nMode |=
          (z->nUserMode & REQ_MODE_NO_ALT_SBONDS) ? CMODE_NO_ALT_SBONDS : 0;

      /* 2010-01-12 */
      z->nMode |=
          (z->vABParityUnknown == AB_PARITY_UNDF) ? 0 : REQ_MODE_DIFF_UU_STEREO;

      if ((z->nMode & CANON_MODE_STEREO) == CANON_MODE_STEREO ||
          (z->nMode & CANON_MODE_ISO_STEREO) == CANON_MODE_ISO_STEREO) {
        z->nMode |= (z->nUserMode & REQ_MODE_RELATIVE_STEREO)
                        ? CMODE_RELATIVE_STEREO
                        : 0;
        z->nMode |=
            (z->nUserMode & REQ_MODE_RACEMIC_STEREO) ? CMODE_RACEMIC_STEREO : 0;
        z->nMode |=
            (z->nUserMode & REQ_MODE_SC_IGN_ALL_UU) ? CMODE_SC_IGN_ALL_UU : 0;
        z->nMode |=
            (z->nUserMode & REQ_MODE_SB_IGN_ALL_UU) ? CMODE_SB_IGN_ALL_UU : 0;
      }

      if ((ret = AllocateCS(pCS, z->num_atoms, z->num_atoms,
                            z->s[TAUT_NON].nLenCT, z->s[TAUT_NON].nLenCTAtOnly,
                            z->s[TAUT_NON].nLenLinearCTStereoDble,
                            z->s[TAUT_NON].nMaxNumStereoBonds,
                            z->s[TAUT_NON].nLenLinearCTStereoCarb,
                            z->s[TAUT_NON].nMaxNumStereoAtoms, 0, 0,
                            z->s[TAUT_NON].nLenIsotopic, z->nMode, pBCN))) {
        goto exit_function;
      }
    ...
    case TAUT_YES: /*  tautomeric */
      ...
      z->nMode |= (z->bHasIsotopicAtoms && (z->nUserMode & REQ_MODE_ISO))
                      ...
                      : 0;
      z->nMode |=
          (z->s[TAUT_YES].bMayHaveStereo && (z->nUserMode & REQ_MODE_STEREO))
              ? CANON_MODE_STEREO
              : 0;
      ...
      if ((ret = AllocateCS(pCS, z->num_atoms, z->num_at_tg,
                            z->s[TAUT_YES].nLenCT, z->s[TAUT_YES].nLenCTAtOnly,
                            z->s[TAUT_YES].nLenLinearCTStereoDble,
                            z->s[TAUT_YES].nMaxNumStereoBonds,
                            z->s[TAUT_YES].nLenLinearCTStereoCarb,
                            z->s[TAUT_YES].nMaxNumStereoAtoms,
                            z->s[TAUT_YES].nLenLinearCTTautomer,
                            z->s[TAUT_YES].nLenLinearCTIsotopicTautomer,
    ...
    pCS->bIgnoreIsotopic =
        !((z->s[TAUT_NON].num_isotopic_atoms ||
           z->s[TAUT_YES].num_isotopic_atoms ||
           z->s[TAUT_YES].bHasIsotopicTautGroups) ||
          (z->nUserMode & REQ_MODE_NON_ISO) || !(z->nUserMode & REQ_MODE_ISO));
    ...
    if (pBCN) {
      ...
      pCS->NeighList = NULL;
      pCS->pBCN = pBCN;
      ret = Canon_INChI(z->num_atoms, i ? z->num_at_tg : z->num_atoms, z->at[i],
                        pCS, z->nMode, i);
    } else {
      /* old way */
      pCS->NeighList =
          CreateNeighList(z->num_atoms, i ? z->num_at_tg : z->num_atoms,
                          z->at[i], pCS->bDoubleBondSquare, pCS->t_group_info);
      pCS->pBCN = NULL;
      ret = Canon_INChI(z->num_atoms, i ? z->num_at_tg : z->num_atoms, z->at[i],
                        pCS, z->nMode, i);
    }
    ...
    if (ret <= 0) {
      ...
      ret = FillOutINChIReducedWarn(
          pINChI, pINChI_Aux, z->num_atoms, i ? z->num_at_tg : z->num_atoms,
...
int CreateCompAtomData(COMP_ATOM_DATA *inp_at_data, int num_atoms,
                       int num_components, int bIntermediateTaut) {
  FreeCompAtomData(inp_at_data);
  if ((inp_at_data->at = CreateInpAtom(num_atoms)) &&
      (num_components <= 1 || bIntermediateTaut ||
       (inp_at_data->nOffsetAtAndH =
            (AT_NUMB *)inchi_calloc(sizeof(inp_at_data->nOffsetAtAndH[0]),
                                    2 * (num_components + 1))))) {
...
int FillOutINChIReducedWarn(INChI *pINChI, INChI_Aux *pINChI_Aux, int num_atoms,
  ...
  AT_NUMB *pSymmRank, *pOrigNosInCanonOrd, *pConstitEquNumb,
      *pCanonOrd = NULL, *pCanonOrdInv = NULL, *pCanonOrdTaut;
  T_GROUP_INFO *t_group_info = pCS->t_group_info;
  T_GROUP *t_group;
  int nErrorCode = 0;
  AT_NUMB *pCanonRank,
      *pCanonRankInv; /* canonical ranks of the atoms or tautomeric groups */
  ...
  /*AT_NUMB  *nOrigAtNosInCanonOrdNonIso = NULL, *nOrigAtNosInCanonOrdIso =
   * NULL;*/
  ...
  pSortOrd = (AT_NUMB *)inchi_calloc(
      num_at_tg + 1, sizeof(pSortOrd[0])); /*  must have more than num_atoms */
  ...
  if (bTautomeric && t_group_info) {
    ...
    /*^^^
            if ( pINChI_Aux->bNormalizationFlags & (FLAG_NORM_CONSIDER_TAUT
    &~FLAG_PROTON_CHARGE_CANCEL) ) { AddMOLfileError(pStrErrStruct, "Proton(s)
    added/removed");
            }
  ...
  if ((pCS->nLenLinearCTStereoCarb > 0 || pCS->nLenLinearCTStereoDble > 0) &&
    ...
    for (i = 0; i < num_at_tg; i++) {
      pCanonRankInv[pCanonOrdInv[i]] = pCanonRank[pCanonOrd[i]] =
          (AT_NUMB)(i + 1);
    }
    /********************************************************************/
    ...
    nErrorCode = CopyLinearCTStereoToINChIStereo(
        Stereo, pCS->LinearCTStereoCarb, pCS->nLenLinearCTStereoCarb,
        pCS->LinearCTStereoDble, pCS->nLenLinearCTStereoDble, pCanonOrd,
        pCanonRank, at, 0 /* non-isotopic */
        ,
        pCS->LinearCTStereoCarbInv, pCS->LinearCTStereoDbleInv, pCanonOrdInv,
        pCanonRankInv);
    ...
    if (Stereo->t_parityInv && Stereo->nNumberInv) {
      ...
      if (Stereo->nCompInv2Abs) {
        if (Stereo->nCompInv2Abs == -1) {
          /* switch pointers in Stereo so that the stereo becomes the smallest
           * (relative)  */
          ...
          /* switch pointers to set rel. stereo to
             pINChI_Aux->nOrigAtNosInCanonOrd and inv. stereo to
             pINChI_Aux->nOrigAtNosInCanonOrdInv */
          switch_ptrs(&pCanonRank, &pCanonRankInv);
          switch_ptrs(&pCanonOrd, &pCanonOrdInv);
          bUseNumberingInv =
              1; /* use inverted stereo numbering instead of normal */
        }
      }
    }

    for (i = 0; i < num_atoms; i++) {
      pINChI_Aux->nOrigAtNosInCanonOrdInv[i] =
          at[pCanonOrdInv[i]].orig_at_number;
    ...
    if (bUseNumberingInv) {
      ...
      switch_ptrs(&pCanonRank, &pCanonRankInv);
      switch_ptrs(&pCanonOrd, &pCanonOrdInv);
      /* save inverted stereo ranks & order because it represents the smallest
       * (relative) */
  ...
  if (pINChI_Aux->OrigInfo) {
    ...
    for (i = 0; i < num_atoms; i++) {
      ...
      if (norm_at[ii].valence || norm_at[ii].num_H) {
        ...
        pINChI_Aux->OrigInfo[i].cRadical =
            (norm_at[ii].radical == RADICAL_SINGLET)   ? 0
            : (norm_at[ii].radical == RADICAL_DOUBLET) ? 1
            : (norm_at[ii].radical == RADICAL_TRIPLET) ? 2
            : norm_at[ii].radical                      ? 3
                                                       : 0;
        pINChI_Aux->OrigInfo[i].cUnusualValence = get_unusual_el_valence(
            norm_at[ii].el_number, norm_at[ii].charge, norm_at[ii].radical,
            norm_at[ii].chem_bonds_valence, norm_at[ii].num_H,
            norm_at[ii].valence);
      } else {
        /* charge of a single atom component is in the INChI; valence = 0 is
         * standard */
        pINChI_Aux->OrigInfo[i].cRadical =
            (norm_at[ii].radical == RADICAL_SINGLET)   ? 0
            : (norm_at[ii].radical == RADICAL_DOUBLET) ? 1
  ...
  pConstitEquNumb =
      pINChI_Aux->nConstitEquNumbers; /*  contitutional equivalence */
  pSymmRank = pCS->nSymmRank;
  if (pCanonOrd && pCanonRank && pSymmRank && pConstitEquNumb) {
    for (i = 0; i < num_atoms; i++) {
      pConstitEquNumb[i] =
          pSymmRank[pCanonOrd[i]]; /*  constit. equ. ranks in order of canonical
                                      numbers */
      pSortOrd[i] = i;
    }
    for (; i < num_at_tg; i++) {
    ...
    for (i = 0, nMinOrd = pSortOrd[0], j = 1; j <= num_atoms; j++) {
      if (j == num_atoms ||
          pConstitEquNumb[pSortOrd[i]] != pConstitEquNumb[pSortOrd[j]]) {
        nMinOrd++;
        if (j - i > 1) {
          /*  found a sequence of equivalent atoms: i..j-1 */
          while (i < j) {
            pConstitEquNumb[pSortOrd[i++]] =
                nMinOrd; /*  = min. canon. rank in the group of equ. atoms */
          }
          /*  at this point j == i */
        } else {
          pConstitEquNumb[pSortOrd[i++]] =
              0; /*  means the atom is not equivalent to any other */
        }
  ...
  /*  connection table: atoms only (before 7-29-2003 pCS->LinearCT2 contained
   * non-isotopic CT) */
  ...
  memcpy(pINChI->nConnTable, pCS->LinearCT,
         sizeof(pINChI->nConnTable[0]) * pCS->nLenLinearCTAtOnly);
  pINChI->lenConnTable = pCS->nLenLinearCTAtOnly;

  /*  tautomeric group(s) canonical representation */
  len = 0;
  if (bTautomeric &&
      0 < (n = SortTautomerGroupsAndEndpoints(t_group_info, num_atoms,
                                              num_at_tg, pCanonRank))) {
    /* SortTautomerGroupsAndEndpoints() produces canonically ordered t-groups */
    pINChI->nFlags |= (t_group_info->bTautFlagsDone & TG_FLAG_ALL_SALT_DONE)
    ...
    for (i = 0; i < n; i++) {
      g = (int)t_group_info
              ->tGroupNumber[i]; /* original group numbers in sorted order */
      ...
      /* tautomeric group endpoint canonical numbers, pre-sorted in ascending
       * order */
      for (j = (int)t_group->nFirstEndpointAtNoPos,
          m = j + (int)t_group->nNumEndpoints;
           j < m; j++) {
        pINChI->nTautomer[len++] =
            pCanonRank[(int)t_group_info->nEndpointAtomNumber[j]]; /*  At[j] */
      }
    }
  ...
  if (pINChI->lenTautomer && (n = pINChI_Aux->nNumberOfTGroups)) {
    pCanonOrdTaut = pCS->nLenCanonOrdStereoTaut > 0 ? pCS->nCanonOrdStereoTaut
                    : pCS->nLenCanonOrdTaut > 0     ? pCS->nCanonOrdTaut
                                                    : NULL;
  ...
  if ((nStereoUnmarkMode =
           UnmarkAllUndefinedUnknownStereo(pINChI->Stereo, nUserMode))) {
    pINChI->nFlags |= (nStereoUnmarkMode & REQ_MODE_SC_IGN_ALL_UU)
                          ? INCHI_FLAG_SC_IGN_ALL_UU
                          : 0;
    pINChI->nFlags |= (nStereoUnmarkMode & REQ_MODE_SB_IGN_ALL_UU)
  ...
  if ((pCS->nLenLinearCTIsotopicStereoCarb > 0 ||
    ...
    /********************************************************************/
    nErrorCode = CopyLinearCTStereoToINChIStereo(
        Stereo, pCS->LinearCTIsotopicStereoCarb,
        pCS->nLenLinearCTIsotopicStereoCarb, pCS->LinearCTIsotopicStereoDble,
        pCS->nLenLinearCTIsotopicStereoDble, pCanonOrd, pCanonRank, at,
        1 /* isotopic */
        ,
        pCS->LinearCTIsotopicStereoCarbInv, pCS->LinearCTIsotopicStereoDbleInv,
    ...
    for (i = 0; i < num_atoms; i++) {
      pINChI_Aux->nIsotopicOrigAtNosInCanonOrdInv[i] =
          at[pCanonOrdInv[i]].orig_at_number;
      pINChI_Aux->nIsotopicOrigAtNosInCanonOrd[i] =
          at[pCanonOrd[i]].orig_at_number;
    }
    ...
    /* no isotopic stereo */
    pCanonOrd = pCS->nLenCanonOrdIsotopicStereo > 0
  ...
  for (i = 0; i < n; i++) {
    ...
    pINChI->IsotopicAtom[i].nIsoDifference =
        pCS->LinearCTIsotopic[i].iso_atw_diff;
  ...
  for (i = 0; i < n; i++) {
    pINChI->IsotopicTGroup[i].nTGroupNumber =
        pCS->LinearCTIsotopicTautomer[i].tgroup_num;
  ...
  if (pCS->nExchgIsoH && pINChI->nPossibleLocationsOfIsotopicH) {
    for (i = 0, j = 1; i < num_atoms; i++) {
      if (pCS->nExchgIsoH[i]) {
        pINChI->nPossibleLocationsOfIsotopicH[j++] =
            (AT_NUMB)(i + 1); /* canonical number */
      }
    }
    pINChI->nPossibleLocationsOfIsotopicH[0] =
        (AT_NUMB)j; /* length including the 0th element */
  }

  if ((nStereoUnmarkMode = UnmarkAllUndefinedUnknownStereo(
           pINChI->StereoIsotopic, nUserMode))) {
    pINChI->nFlags |= (nStereoUnmarkMode & REQ_MODE_SC_IGN_ALL_UU)
  ...
  /* mark ambiguous stereo */
  MarkAmbiguousStereo(
      at, norm_at, 1 /* isotopic */, pCanonOrd, pCS->LinearCTIsotopicStereoCarb,
      pCS->nLenLinearCTIsotopicStereoCarb, pCS->LinearCTIsotopicStereoDble,
   ...
   *  should not depend on switching to rel. stereo numbering
   */
  if (pINChI->lenTautomer && pINChI_Aux->nConstitEquIsotopicTGroupNumbers &&
    ...
    pCanonOrdTaut =
        pCS->nLenCanonOrdIsotopicStereoTaut > 0
            ? (n = pCS->nLenCanonOrdIsotopicStereoTaut,
               pCS->nCanonOrdIsotopicStereoTaut)
        : pCS->nLenCanonOrdIsotopicTaut > 0
            ? (n = pCS->nLenCanonOrdIsotopicTaut, pCS->nCanonOrdIsotopicTaut)
            : (n = 0, (AT_RANK *)NULL);
