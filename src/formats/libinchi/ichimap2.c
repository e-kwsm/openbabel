...
void DeAllocateForNonStereoRemoval(AT_RANK **nAtomNumberCanon1,
                                   AT_RANK **nAtomNumberCanon2, NEIGH_LIST **nl,
...
int CheckNextSymmNeighborsAndBonds(sp_ATOM *at, AT_RANK cur1, AT_RANK cur2,
                                   ...
                                   AT_RANK *nVisited2, AT_RANK *nVisitOrd1,
                                   AT_RANK *nVisitOrd2, const AT_RANK *nRank1,
...
int CalculatedPathsParitiesAreIdentical(
    sp_ATOM *at, int num_atoms, const AT_RANK *nSymmRank, AT_RANK *nCanonRank,
    AT_RANK *nAtomNumberCanon, AT_RANK *nAtomNumberCanon1,
    AT_RANK *nAtomNumberCanon2, AT_RANK *nVisited1, AT_RANK *nVisited2,
    AT_RANK prev_sb_neigh, AT_RANK cur, AT_RANK next1, AT_RANK next2,
    int nNeighMode, int bParitiesInverted, int mode, CANON_STAT *pCS,
    int vABParityUnknown);
int RemoveCalculatedNonStereoBondParities(
    sp_ATOM *at, int num_atoms, int num_at_tg, AT_RANK **pRankStack1,
    AT_RANK **pRankStack2, AT_RANK *nTempRank, NEIGH_LIST *NeighList,
    AT_RANK *nCanonRank, const AT_RANK *nSymmRank, AT_RANK *nAtomNumberCanon,
    AT_RANK *nAtomNumberCanon1, AT_RANK *nAtomNumberCanon2, NEIGH_LIST *nl,
    NEIGH_LIST *nl1, NEIGH_LIST *nl2, AT_RANK *nVisited1, AT_RANK *nVisited2,
    CANON_STAT *pCS, int vABParityUnknown);
...
int SortNeighLists3(int num_atoms, AT_RANK *nRank, NEIGH_LIST *NeighList,
                    AT_RANK *nAtomNumber);
 ...
 *
 */
int SortedEquInfoToRanks(const AT_RANK *nSymmRank, AT_RANK *nRank,
                         const AT_RANK *nAtomNumber, int num_atoms,
                         int *bChanged) {
  AT_RANK rNew, rOld, nNumDiffRanks;
  int i, j, nNumChanges = 0;
  for (i = num_atoms - 1, j = (int)nAtomNumber[i], rOld = nSymmRank[j],
      rNew = nRank[j] = (AT_RANK)num_atoms, nNumDiffRanks = 1;
       i > 0; i--) {
 ...
 *
 */
int SortedRanksToEquInfo(AT_RANK *nSymmRank, const AT_RANK *nRank,
                         const AT_RANK *nAtomNumber, int num_atoms) {
  AT_RANK rNew, rOld, nNumDiffRanks;
  int i, j;
  for (i = 1, j = (int)nAtomNumber[0], rOld = nRank[j], rNew = nSymmRank[j] = 1,
      nNumDiffRanks = 1;
       i < num_atoms; i++) {
    j = (int)nAtomNumber[i];
    if (nRank[j] != rOld) {
...
/**************************************************************************************/
int SetNewRanksFromNeighLists3(int num_atoms, NEIGH_LIST *NeighList,
  ...
  */
  ...
  for (i = 0, r1 = 1; i < num_atoms; r1++) {
    ...
    /*insertions_sort( nAtomNumber+i, r2-i, sizeof( nAtomNumber[0] ),
     * CompNeighLists );*/
    j = r2 - 1;
...
/*  nRank[] and nNewRank[] should refer to different arrays for now */
/**************************************************************************************/
int SetNewRanksFromNeighLists4(int num_atoms, NEIGH_LIST *NeighList,
                               AT_RANK *nRank, AT_RANK *nNewRank,
  ...
  */
  ...
  for (i = 0, r1 = 1; i < num_atoms; r1++) {
    ...
    */
    r1 = r2;
    insertions_sort_AT_NUMBERS(nAtomNumber + i, (int)r2 - i,
...
/*  nRank[] and nNewRank[] should refer to different arrays for now */
/**************************************************************************************/
int SetNewRanksFromNeighLists(int num_atoms, NEIGH_LIST *NeighList,
                              AT_RANK *nRank, AT_RANK *nNewRank,
  ...
  */
  ...
  /*  sorting */
  if (bUseAltSort & 1)
    tsort(nAtomNumber, num_atoms, sizeof(nAtomNumber[0]),
          comp /*CompNeighListRanksOrd*/);
  else
    qsort(nAtomNumber, num_atoms, sizeof(nAtomNumber[0]),
          comp /*CompNeighListRanksOrd*/);

  for (i = num_atoms - 1,
      nCurrentRank = nNewRank[(int)nAtomNumber[i]] = (AT_RANK)num_atoms,
      nNumDiffRanks = 1;
       0 < i; i--) {
    /*  Note: CompNeighListRanks() in following line implicitly reads nRank
     * pointed by pn_RankForSort */
 ...
 */
/**************************************************************************************/
void SortNeighListsBySymmAndCanonRank(int num_atoms, NEIGH_LIST *NeighList,
                                      const AT_RANK *nSymmRank,
                                      const AT_RANK *nCanonRank) {
  int i;
  for (i = 0; i < num_atoms; i++) {
    insertions_sort_NeighListBySymmAndCanonRank(NeighList[i], nSymmRank,
                                                nCanonRank);
  }
}
/**************************************************************************************/
int SortNeighLists2(int num_atoms, AT_RANK *nRank, NEIGH_LIST *NeighList,
                    AT_RANK *nAtomNumber) {
  int k, i;
  AT_RANK nPrevRank = 0;
  /*
   * on entry nRank[nAtomNumber[k]] <= nRank[nAtomNumber[k+1]]  ( k <
   * num_atoms-1 ) nRank[nAtomNumber[k]] >= k+1                      ( k <
   * num_atoms ) nRank[nAtomNumber[k]] == k+1 if this nRank value is not tied OR
   * if nRank[nAtomNumber[k]] < nRank[nAtomNumber[k+1]] OR if k = num_atoms-1.
   *
   */
 ...
 *  Differentiate2
 *
 * Note: on entry nAtomNumber[] must contain a valid transposition of num_atoms
 * length for example, nAtomNumber[i] = i; Note2: this version does not
 * calculate neighbor lists for non-tied ranks
 */
int DifferentiateRanks2(int num_atoms, NEIGH_LIST *NeighList, int nNumCurrRanks,
                        AT_RANK *pnCurrRank, AT_RANK *pnPrevRank,
  ...
  do {
    *lNumIter += 1;
    ...
    nNumCurrRanks =
        SetNewRanksFromNeighLists(num_atoms, NeighList, pnPrevRank, pnCurrRank,
                                  nAtomNumber, 1, CompNeighListRanksOrd);
  } while (/*nNumPrevRanks != nNumCurrRanks ||*/ memcmp(
      pnPrevRank, pnCurrRank, num_atoms * sizeof(pnCurrRank[0])));
 ...
 * calculate neighbor lists for non-tied ranks
 */
int DifferentiateRanks3(int num_atoms, NEIGH_LIST *NeighList, int nNumCurrRanks,
                        AT_RANK *pnCurrRank, AT_RANK *pnPrevRank,
  ...
  */
  ...
  /*  SortNeighLists3 needs sorted ranks: ranks/atnumbers must have been already
   * sorted */
  do {
    *lNumIter += 1;
    switch_ptrs(&pnCurrRank, &pnPrevRank);
    ...
    nNumCurrRanks = SetNewRanksFromNeighLists3(num_atoms, NeighList, pnPrevRank,
      ...
      nNumCurrRanks <
      0 /* memcmp( pnPrevRank, pnCurrRank, num_atoms*sizeof(pnCurrRank[0]) )*/);
 ...
 *
 * Note: on entry nAtomNumber[] must contain a valid transposition of num_atoms
 * length for example, nAtomNumber[i] = i; Note2: this version does not sort
 * neighbor lists for non-tied ranks
 */
int DifferentiateRanks4(int num_atoms, NEIGH_LIST *NeighList, int nNumCurrRanks,
                        AT_RANK *pnCurrRank, AT_RANK *pnPrevRank,
                        AT_RANK *nAtomNumber, AT_RANK nMaxAtRank,
                        long *lNumIter) {
  /*
  ...
  */
  /*  SortNeighLists4 needs sorted ranks: ranks/atnumbers must have been already
   * sorted */
  do {
    *lNumIter += 1;
    switch_ptrs(&pnCurrRank, &pnPrevRank);
    ...
    nNumCurrRanks = SetNewRanksFromNeighLists4(
        num_atoms, NeighList, pnPrevRank, pnCurrRank, nAtomNumber, nMaxAtRank);
  } while (
      nNumCurrRanks <
      0 /* memcmp( pnPrevRank, pnCurrRank, num_atoms*sizeof(pnCurrRank[0]) )*/);
...
int DifferentiateRanksBasic(int num_atoms, NEIGH_LIST *NeighList,
                            int nNumCurrRanks, AT_RANK *pnCurrRank,
                            AT_RANK *pnPrevRank, AT_RANK *nAtomNumber,
                            long *lNumIter, int bUseAltSort) {
  int nNumPrevRanks;
  ...
  do {
    *lNumIter += 1;
    ...
    nNumCurrRanks =
        SetNewRanksFromNeighLists(num_atoms, NeighList, pnPrevRank, pnCurrRank,
                                  nAtomNumber, bUseAltSort, CompNeighListRanks);
  } while (nNumPrevRanks != nNumCurrRanks ||
           memcmp(pnPrevRank, pnCurrRank, num_atoms * sizeof(pnCurrRank[0])));
  return nNumCurrRanks;
}
...
int NumberOfTies(AT_RANK **pRankStack1, AT_RANK **pRankStack2, int length,
                 int at_no1, int at_no2, AT_RANK *nNewRank, int *bAddStack,
                 int *bMapped1) {
  ...
  *nNewRank = 0;
  r = nRank1[at_no1];
  if (r != nRank2[at_no2])
    return CT_MAPCOUNT_ERR;
  /*  atoms cannot be mapped onto each other: they have different ranks */ /*   <BRKPT>
                                                                            */
  iMax = r - 1;
  ...
  if (i2 != i1)
    return CT_MAPCOUNT_ERR;
  /*  program error: must be identical number of equal ranks */ /*   <BRKPT>
                                                                 */
  ...
  if (i1 > 1) {
    ...
    *bAddStack = 2; /*  to break the tie we added 2 more arrays to pRankStack1
                       and pRankStack2 */
  }
  return i1;
}
 ...
 *
 * Input: at_no1     = number of atom for which the half-bond parity is
 * calculated i_sb_neigh = ordering number of the stereo bond in
 * at->stereo_bond_neighbor[]
 *
 * Returns: 0=> no parity can be found; 1=> odd parity; 2=> even parity
 *
 */
int HalfStereoBondParity(sp_ATOM *at, int at_no1, int i_sb_neigh,
                         const AT_RANK *nRank) {
  /*
     Suppose neighbors #0,#1,#2 have ranks a, b, c. Remove rank of the neighbor
     connected by the stereogenic bond (NCSB) from the a, b, c list and denote
     the two left as r[0], r[1], in the same order. Let iNCSB be an ordering
     number (0,1,or 2) of the NCSB. Assume the neighbor connected by the
     stereogenic bond has infinite positive rank. Position the half-bond so that
     the stereogenic bond neighbor is to the right from the atom (see below)

     Definition.
     ===========
                     if rank(X) != rank(Y) then Half-bond parity = (rank(X) >
     rank(Y)), that is,
      Y
       \             if ( rank(X) < rank(Y) ) then Half-bond parity is Even
        C==NCSB      if ( rank(X) > rank(Y) ) then Half-bond parity is Odd
       /             if ( rank(X) = rank(Y) ) then Half-bond parity cannot be
     defined
      X
     ...
     ============================
     If  (1) atom's geometric parity = even (which means neighbors #0, #1, #2
     are located clockwise), and (2) neighbors other than NCSB have different
     ranks, then, assuming that NCSB always has the largest (infinite) rank
     (this is consistent with the assumption that implicit hydrogens have
     smallest ranks), we have 3 possibilities:
         ...
         The following slight modification will work for both odd and even
     geometric parity:

         Half-bond parity     =    (iNCSB + (r[0] > r[1]) + (Atom C geometric
     parity))%2

         even parity (0) => atom above the bond has lower rank than the atom
     below the bond.


     Case 2: 2 explicit neighbors
     ============================
     One implicit hydrogen atom H or hydrogen isotope (implicit rank=0). Assume
     r[1]=0
     ...
     Atom C geometric parity:  Even               Odd          Note. The same
     method works for D                  H \                  \           Even
     and   Odd C==a               C==a /                  /           H N==a H
     D             \             / N==a        H iNCSB =           0 0 Half-bond
     parity =    (0<0)=0         (0<0)+1 = 1 (r[1]=0, r[0]=0)    r[1]<r[0]
     (r[1]<r[0])+atom_parity

         Half-parity
         for this case  =    (iNCSB + (r[0] > r[1]) + (Atom C geometric
     parity))%2
  ...
  */
  ...
  if (at[at_no1].valence > MAX_NUM_STEREO_BOND_NEIGH ||
      (at1_parity = at[at_no1].parity) <= 0) {
    return 0;
  }
  ...
  if ((j == 2 && r[0] == r[1]) || iNeigh < 0) {
    parity = AB_PARITY_CALC; /*  cannot calculate bond parity without additional
                                breaking ties. */
  } else {
...
/**************************************************************************************/
int parity_of_mapped_half_bond(int from_at, int to_at, int from_neigh,
                               int to_neigh, sp_ATOM *at, EQ_NEIGH *pEN,
  ...
  AT_RANK r_canon_from[MAX_NUM_STEREO_BOND_NEIGH],
      at_no_from[MAX_NUM_STEREO_BOND_NEIGH];
  ...
  if (num_neigh > MAX_NUM_STEREO_BOND_NEIGH ||
      num_neigh < MIN_NUM_STEREO_BOND_NEIGH) {
  ...
  if (ATOM_PARITY_KNOWN(parity)) {
    ...
    return 0;
    /* AB_PARITY_UNDF; */ /*  possibly program error: undefined parity */
  }
  /*  locate at[to_at].stereo_bond_neighbor[] ordering numbers */
  for (i = 0, to_sb_neigh_ord = -1;
       i < MAX_NUM_STEREO_BONDS && (k = (int)at[to_at].stereo_bond_neighbor[i]);
       i++) {
    if (k == to_neigh + 1) {
      to_sb_neigh_ord = i;
      break;
  ...
  if (r_to[0] == r_to[1]) {
    ...
    if (from_sb_neigh_ord < 0 || j != 2) {
      return 0;
      /*  program error: rank of a neighbor not found */ /*   <BRKPT> */
    }
    if (pEN) { /*  j == 2 */
      pEN->to_at[0] = at_no_to[0];
      pEN->to_at[1] = at_no_to[1];
      pEN->num_to =
          2; /*  number of stored in pEN->to_at[] central atom neighbors */
  ...
  return 2 -
         (parity + to_sb_neigh_ord + (r_canon_from[1] < r_canon_from[0])) % 2;
}

/**************************************************************************************/
int parity_of_mapped_atom2(int from_at, int to_at, const sp_ATOM *at,
                           EQ_NEIGH *pEN, const AT_RANK *nCanonRankFrom,
                           const AT_RANK *nRankFrom, const AT_RANK *nRankTo) {
  AT_RANK nNeighRankFrom[4], nNeighNumberFrom[4], nNeighRankTo[4],
      nNeighNumberTo[4];
  ...
  for (i = 0; i < num_neigh; i++) { /*  initialization of locals */
    nNeighNumberTo[i] = nNeighNumberFrom[i] = i;
    nNeighRankTo[i] = nRankTo[(int)at[to_at].neighbor[i]]; /* mapping rank */
    nNeighRankFrom[i] =
        nRankFrom[j = (int)at[from_at].neighbor[i]]; /* mapping rank */
  ...
  num_trans_from =
      insertions_sort(nNeighNumberFrom, num_neigh, sizeof(nNeighNumberFrom[0]),
  ...
  if (nNumCompNeighborsRanksCountEql) {
    ...
    if (r) {
      /*  neighbors with tied ranks have been found => parity cannot be
       * determined without additional mapping */
      ...
      if (pEN) {
        ...
        pEN->num_to =
            j; /*  number of stored in pEN->to_at[] central atom neighbors */
        pEN->from_at =
            at[from_at].neighbor[neigh_canon_from_min]; /*  neighbor with min.
                                                           canon number */
        pEN->rank = r1; /*  mapping rank of the tied neighbors */
        pEN->canon_rank =
            r_canon_from_min; /*  canon. rank of the pEN->from_at */
      } else {
        /*  debug only */
        for (i = j = 0; i < num_neigh; i++) {
          if (r1 == nNeighRankTo[i]) {
  ...
  num_trans_to =
      insertions_sort(nNeighNumberTo, num_neigh, sizeof(nNeighNumberTo[0]),
                      CompNeighborsRanksCountEql);

  /*  Map canonical ranks of neighbors. Mapped on each other "to" and "from"
   * atoms have equal mapping ranks */
  for (i = 0; i < num_neigh; i++) {
    if (nNeighRankTo[j = nNeighNumberTo[i]] !=
        nNeighRankFrom[k = nNeighNumberFrom[i]])
      return 0; /*  program error: mapping ranks not equal, from_at neigborhood cannot be mapped on to_at neighbood. */ /*   <BRKPT> */
    nNeighRankToCanon[j] =
        nNeighRankFromCanon[k]; /*  potential problem: other atom(s) may have
                                   same mapping rank and */
  ...
  num_trans_to +=
      ((num_neigh * (num_neigh - 1)) / 2) %
      2; /*  get correct parity for ascending order of canon. numbers */
#endif
...
/**************************************************************************************/
/*  map one atom ("from") onto another ("to"): untie their mapping ranks if they
 * are tied. */
int map_an_atom2(int num_atoms, int num_max, int at_no1 /*from*/,
                 int at_no2 /*to*/, AT_RANK *nTempRank, int nNumMappedRanks,
                 int *pnNewNumMappedRanks, CANON_STAT *pCS,
                 NEIGH_LIST *NeighList, AT_RANK **pRankStack1,
                 AT_RANK **pRankStack2, int *bAddStack) {
  AT_RANK *nRank1, *nAtomNumber1; /*  ranks for mapping "1", "from" */
  AT_RANK *nRank2, *nAtomNumber2; /*  ranks for mapping "2", "to" */
  AT_RANK *nNewRank1 = NULL,
          *nNewAtomNumber1 = NULL; /*  ranks for mapping "1", "from" */
  AT_RANK *nNewRank2 = NULL,
          *nNewAtomNumber2 = NULL; /*  ranks for mapping "2", "to" */
  int length = num_max * sizeof(AT_RANK);
  int nNewNumRanks2, nNewNumRanks1;
  int i, bAtFromHasAlreadyBeenMapped, nNumTies;
  AT_RANK nNewRank;

  nNumTies = NumberOfTies(pRankStack1, pRankStack2, length, at_no1, at_no2,
                          &nNewRank, bAddStack, &bAtFromHasAlreadyBeenMapped);
  ...
  if (nNumTies > 1) {
    ...
    nNewNumRanks2 = DifferentiateRanks2(num_atoms, NeighList, nNumMappedRanks,
                                        nNewRank2, nTempRank, nNewAtomNumber2,
    ...
    if (2 != bAtFromHasAlreadyBeenMapped) {
      ...
      nNewNumRanks1 = DifferentiateRanks2(num_atoms, NeighList, nNumMappedRanks,
                                          nNewRank1, nTempRank, nNewAtomNumber1,
                                          &pCS->lNumNeighListIter, 1);
      pCS->lNumBreakTies++;
    } else {
  ...
  return (nNewRank1) ? nNewRank1[at_no1]
                     : nRank1[at_no1]; /*  mapping rank value */
}

/**************************************************************************************/
int might_change_other_atom_parity(sp_ATOM *at, int num_atoms, int at_no,
                                   AT_RANK *nRank2, AT_RANK *nRank1) {
  int i, j, neighbor_no;
  for (i = 0; i < num_atoms; i++) {
...
int AllocateForNonStereoRemoval(sp_ATOM *at, int num_atoms,
                                ...
                                NEIGH_LIST **nl1, NEIGH_LIST **nl2,
                                AT_RANK **nVisited1, AT_RANK **nVisited2) {
  DeAllocateForNonStereoRemoval(nAtomNumberCanon1, nAtomNumberCanon2, nl, nl1,
                                nl2, nVisited1, nVisited2);
  *nAtomNumberCanon1 =
      (AT_RANK *)inchi_malloc(num_atoms * sizeof(**nAtomNumberCanon1));
  *nAtomNumberCanon2 =
      (AT_RANK *)inchi_malloc(num_atoms * sizeof(**nAtomNumberCanon2));
  ...
  *nVisited2 = (AT_RANK *)inchi_malloc(num_atoms * sizeof(**nVisited2));
  ...
  if (!*nl || !*nl1 || !*nl2 || !*nVisited1 || !*nVisited2 ||
      !*nAtomNumberCanon1 || !*nAtomNumberCanon2) {
    DeAllocateForNonStereoRemoval(nAtomNumberCanon1, nAtomNumberCanon2, nl, nl1,
                                  nl2, nVisited1, nVisited2);
    return 0;
  }
  /*  Sort neighbors according to symm. ranks (primary key) and canon. ranks
   * (secondary key), in descending order */
...
AT_RANK GetMinNewRank(AT_RANK *nAtomRank, AT_RANK *nAtomNumb, AT_RANK nRank1) {
  ...
  AT_RANK nRank2;
  for (i = (int)nRank1 - 1;
       0 <= i && nRank1 == (nRank2 = nAtomRank[(int)nAtomNumb[i]]); i--)
    ;
  if (i >= 0)
    nRank2++;
  else
...
int BreakNeighborsTie(sp_ATOM *at, int num_atoms, int num_at_tg, int ib, int ia,
  ...
  if ((mode == MAP_MODE_S4 && in1) || /* for S4 we need only (in1,in2) = (0,1)
                                         (0,2) (0,3) pairs of neighbors */
  ...
  nNumDiffRanks =
      SortedEquInfoToRanks(pRankStack1[0] /*inp*/, pRankStack1[0] /*out*/,
                           pRankStack1[1], num_at_tg, NULL);
  ...
  if ((mode != MAP_MODE_STD &&
       nSymmRank[other_neigh[0]] != nSymmRank[other_neigh[1]]) ||
  ...
  if (pRankStack1[0][ia] != nSymmRank[ia]) {
    ...
    nNumDiffRanks = DifferentiateRanksBasic(num_at_tg, NeighList, nNumDiffRanks,
                                            pRankStack1[0], nTempRank,
  ...
  if (ib < num_atoms) {
    ...
    if (nRank2 != nRank1) {
      ...
      nNumDiffRanks = DifferentiateRanksBasic(
          num_at_tg, NeighList, nNumDiffRanks, pRankStack1[0], nTempRank,
          pRankStack1[1], lNumIter, 1);
    }
  }
   ...
   *       The ranks assigned to the other neighbors in case of their
   *equivalence should be in the ascending order of their canonical ranks ????
  ...
  nNumDiffRanks1 = DifferentiateRanksBasic(num_at_tg, NeighList, nNumDiffRanks,
                                           pRankStack1[0], nTempRank,
                                           pRankStack1[1], lNumIter, 1);

  pRankStack2[0][n2] = nRank2;
  nNumDiffRanks2 = DifferentiateRanksBasic(num_at_tg, NeighList, nNumDiffRanks,
                                           pRankStack2[0], nTempRank,
  ...
  if (mode == MAP_MODE_C2v || mode == MAP_MODE_C2) {
    /* Check for C2v reflection leading to parity inversion (mode=1) or C2
     * rotation (mode=2) */
      ...
      if (nRank10 == nRank20 && nRank1 == nRank2) {
        if (nRank10 == nRank1) {
          ...
          nNumDiffRanks2 = DifferentiateRanksBasic(
              num_at_tg, NeighList, nNumDiffRanks, pRankStack2[0], nTempRank,
              pRankStack2[1], lNumIter, 1);
          if (nNumDiffRanks1 != nNumDiffRanks2) {
            return -1; /*  <BRKPT> */
          }
  ...
  if (mode == MAP_MODE_S4) {
     ...
     *        [2](1) <--> [3](2)  [y]=[3]
     *        [3](1) <--> [0](2)
     *        this will succeed if C2 axis crosses middle of [0]-[2] and [1]-[3]
     * lines
     *
     ...
     *        [3](1) <--> [1](2)  [y]=[1]
     *        [1](1) <--> [0](2)
     *        this will succeed if C2 axis crosses middle of [0]-[3] and [1]-[2]
     * lines
     *
     ...
     *        [1](1) <--> [2](2)  [y]=[2]
     *        [2](1) <--> [0](2)
     *        this will succeed if C2 axis crosses middle of [0]-[1] and [2]-[3]
     * lines
     *
     * In general:
     *        [in1](1) <--> [in2](2)
  ...
  if (ib >= num_atoms && at[ia].valence == MAX_NUM_STEREO_ATOM_NEIGH) {
    ...
    if (num_eq == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
      for (i = (int)nRank1 - 1;
           0 <= i &&
           nRank1 == (nRank2 = pRankStack1[0][(int)pRankStack1[1][i]]);
           i--)
        ;
      if (i >= 0)
        nRank2++;
  ...
  if (at[n1].nRingSystem == at[n2].nRingSystem && ib >= num_atoms) {
    ...
    NeighNeighList[0] =
        at[n1].valence -
        1; /* for insertions_sort_NeighListBySymmAndCanonRank() */
    ...
    insertions_sort_NeighListBySymmAndCanonRank(NeighNeighList, pRankStack1[0],
                                                nCanonRank);
    for (m = 2; m < at[n1].valence; m++) {
      if (pRankStack1[0][NeighNeighList[m]] ==
          pRankStack1[0][NeighNeighList[m - 1]]) {
    ...
    insertions_sort_NeighListBySymmAndCanonRank(NeighNeighList, pRankStack2[0],
                                                nCanonRank);
    for (m = 2; m < at[n2].valence; m++) {
      if (pRankStack2[0][NeighNeighList[m]] ==
          pRankStack2[0][NeighNeighList[m - 1]]) {
#if (BREAK_ALSO_NEIGH_TIE_ROTATE == 1)
        neigh2 = NeighNeighList[m]; /* [m] to obtain same axis orientation
                                       around ia<neigh */
#else
    ...
    if (neigh1 >= 0 && neigh2 >= 0 &&
        pRankStack1[0][neigh1] == pRankStack2[0][neigh2]) {
  ...
  for (i = 0; i < num_at_tg; i++) {
    if (pRankStack1[0][(int)pRankStack1[1][i]] !=
        pRankStack2[0][(int)pRankStack2[1][i]]) {
      return -1; /*  <BRKPT> */
    }
  }
...
int CheckNextSymmNeighborsAndBonds(sp_ATOM *at, AT_RANK cur1, AT_RANK cur2,
  ...
  if (nRank1[n1] != nRank2[n2]) {
    return -1;
    /*  parallel traversal in stereo removal failed */ /*   <BRKPT> */
  }
  switch (!nVisited1[n1] + !nVisited2[n2]) {
  case 0:
    if (nVisited1[n1] != n2 + 1 || nVisited2[n2] != n1 + 1) {
      return -1; /*  0; */ /*  possibly error???: we have come to an alreardy
                              traversed pair and */
    ...
    break;
  case 1:
    return -1; /*  0; */ /*  possibly error: one is zero, another is not a zero.
                            Happens in C60 */
  ...
  /*  Even though the bond or cumulene might have already been checked, check
   * it: this is */
  /*  the only place we can check stereo bonds and cumulenes that are not edges
   * of the DFS tree */
  /*  The code works both for a stereo bond and a stereogenic cumulene. */

  for (i1 = 0, k1 = 0;
       i1 < MAX_NUM_STEREO_BONDS && (s1 = at[cur1].stereo_bond_neighbor[i1]) &&
       !(k1 = (at[cur1].neighbor[(int)at[cur1].stereo_bond_ord[i1]] == n1));
       i1++)
    ;
  for (i2 = 0, k2 = 0;
       i2 < MAX_NUM_STEREO_BONDS && (s2 = at[cur2].stereo_bond_neighbor[i2]) &&
  ...
  (s1=at[cur1].stereo_bond_neighbor[i1]) && !(k1=(s1-1 == n1)); i1 ++ )
      ;
  for ( i2 = 0, k2 = 0; i2 < MAX_NUM_STEREO_BONDS &&
  (s2=at[cur2].stereo_bond_neighbor[i2]) && !(k2=(s2-1 == n2)); i2 ++ )
      ;
  */
  ...
  if (k1) {
    ...
    if (at[cur1].stereo_bond_parity[i1] != at[cur2].stereo_bond_parity[i2]) {
      /*  different values of  at[].stereo_bond_parity: definitely different
       * bonds */
      /*  known parities */
      ...
      if (!(PARITY_CALCULATE(at[cur1].stereo_bond_parity[i1]) &&
                PARITY_WELL_DEF(at[cur2].stereo_bond_parity[i2]) ||
            PARITY_WELL_DEF(at[cur1].stereo_bond_parity[i1]) &&
                PARITY_CALCULATE(at[cur2].stereo_bond_parity[i2]) ||
            PARITY_CALCULATE(at[cur1].stereo_bond_parity[i1]) &&
                PARITY_CALCULATE(at[cur2].stereo_bond_parity[i2]))) {
        /*  do not reject if: "well defined" and "calculate" or "calculate" and
         * "calculate" */
        return 0;
      }
#endif
  ...
  return 1; /*  stereo bonds to n1 and n2 have same known parities or are not
               stereo bonds */
}
/**************************************************************************************/
int CreateCheckSymmPaths(sp_ATOM *at, AT_RANK prev1, AT_RANK cur1,
  ...
  if (PARITY_WELL_DEF(at[cur1].stereo_atom_parity) &&
    ...
    if (*bParitiesInverted < 0) {
      *bParitiesInverted =
          (at[cur1].stereo_atom_parity + at[cur2].stereo_atom_parity) % 2;
    } else if (*bParitiesInverted !=
               (at[cur1].stereo_atom_parity + at[cur2].stereo_atom_parity) %
                   2) {
      return 0; /*  Different known in advance parities have wrong "inverted"
                   relation */
    }
  ...
  if (cur1 != cur2 && !at[cur1].stereo_bond_neighbor[0] &&
      !at[cur2].stereo_bond_neighbor[0] &&
      PARITY_KNOWN(at[cur1].parity) != PARITY_KNOWN(at[cur2].parity)) {
    return 0; /*  one atom is stereogenic, another (presumably equivalent) is
                 not. 9-11-2002 */
  }
#if (PROPAGATE_ILL_DEF_STEREO != 1)
  if (cur1 != cur2 && (PARITY_ILL_DEF(at[cur1].stereo_atom_parity) ||
  ...
  if (nl1[(int)cur1][0] != nl2[(int)cur2][0] ||
      nl1[(int)cur1][0] != at[cur1].valence) {
    return CT_REMOVE_STEREO_ERR;
    /*  error: different valences */ /*   <BRKPT> */
  }
  ...
  for (k = 1, k1 = 1, k2 = 1; k < at[cur1].valence; k++, k1++, k2++) {
    ...
    if (0 >= (ret = CheckNextSymmNeighborsAndBonds(
                  at, cur1, cur2, n1, n2, nAvoidCheckAtom, nVisited1, nVisited2,
                  nVisitOrd1, nVisitOrd2, nRank1, nRank2))) {
      return ret; /*  different neighbors or bonds                       */
    }

    if (!nVisited1[n1]) { /*  recursion */
      /* allow all inverted parities only inside a single ring system containing
       * the starting point */
      pbParitiesInverted = (at[cur1].nRingSystem == at[n1].nRingSystem)
                               ? bParitiesInverted
                               : &bParitiesInvertedZero;
      if (0 >= (ret = CreateCheckSymmPaths(
                    at, cur1, n1, cur2, n2, nAvoidCheckAtom, nVisited1,
                    nVisited2, nVisitOrd1, nVisitOrd2, nl1, nl2, nRank1, nRank2,
                    nCanonRank, nLength, pbParitiesInverted, mode))) {
        return ret;
      }
    }
  }
 ...
 **************************************************************************************
  ...
  Td(24) > C3v(6) > C3(3) -- does not have 2 pairs of constitutionally identical
neighbors > Cs(2)
  ...
  (b) C2v  #1: discover ACB symmetry plain Cv
      ...
      Traverse DFS from X and Y
      In this case canonical numbers will be reflected in plane ACB if it
exists. o Criterion of the presence of the symmetry plain is:
...
int CalculatedPathsParitiesAreIdentical(
    ...
    int nNeighMode, int bParitiesInverted, int mode, CANON_STAT *pCS,
    int vABParityUnknown) {
  int i, i01, i02, i11, i12, i21, i22, k, parity, parity1, parity2, parity12,
      num_other_neigh;
  int nNumEqStereogenic, nCheckingMode, not_well_def_parities;
  AT_RANK other_neigh[MAX_NUM_STEREO_ATOM_NEIGH], neigh, r1, r2;
  int nNumComparedCenters = 0, nNumComparedBonds = 0,
      bCurParityInv1 = 0 /*, bCurParityInv2=0*/;
  int bCurRotated = 0, nNumDiff = 0, nNumInv = 0;
  int s1, s2;

  nCheckingMode =
      (prev_sb_neigh < num_atoms) ? CHECKING_STEREOBOND : CHECKING_STEREOCENTER;
  not_well_def_parities = 0;
  nNumEqStereogenic = 0;

  if ((nNeighMode != NEIGH_MODE_RING && bParitiesInverted != 0) ||
      abs(bParitiesInverted) != 1) {
    bParitiesInverted = 0;
  }
  ...
  if ((bParitiesInverted > 0 &&
       !(mode == MAP_MODE_C2v || mode == MAP_MODE_S4)) ||
      (bParitiesInverted == 0 &&
       !(mode == MAP_MODE_C2 || mode == MAP_MODE_STD))) {
    return 0;
  }
  /**************************************************************************************
   ...
   *    and P2 respectively of the ordering atom numbers: nVisitedi[k] =
   *Pi(k)+1; In this implementation: P1(k)=k for all k P2(cur)=cur,
   *P2(next1)=next2, P2(next2)=next1
   *
   *    Below we call one of the numberings "old", another "new".
   *
   *    *IF* the old and the new canonical numberings produce same parities for
   *stereogenic elements for the same canonical number(s) (that is,
   *old_parity(canon_number) == new_parity(canon_number) *except* the currently
   *being tested stereocenter at[cur] or stereobond/cumulene
   *    at[cur]=at[prev_sb_neigh], whose parity MUST be inverted
   *
   *    *THEN* the stereocenter or stereobond/cumulene is not stereogenic with
   *one
   *
   *    *EXCEPTION* If the currently tested stereogenic element is
   *constitutionally equivalent to two or more other stereogenic elements that
   *have been permuted then the currently tested one is still stereogenic.
   **************************************************************************************/
   ...
   *
   * If the transposition of next1 and next2 changes only the parity of the
   * starting stereo atom or stereo bond then the stereo bond or stereo atom is
   * not stereogenic
   *
  ...
  if (nCheckingMode == CHECKING_STEREOBOND) {
    ...
    if (nVisited1[prev_sb_neigh] || nVisited2[prev_sb_neigh]) {
      /*  the bond or cumulene is in the ring and the opposite atom has been
       * visited */
      if (nVisited1[prev_sb_neigh] != nVisited2[prev_sb_neigh] ||
          nCanonRank[prev_sb_neigh] != nVisited2[prev_sb_neigh]) {
        return 0;
        /*  error: we came back to the same bond/cumulene and */ /*   <BRKPT>
                                                                  */
        /*  assigned different canon. ranks to the opposite atom. */
      }
      if (at[prev_sb_neigh].valence + at[prev_sb_neigh].num_H > 3)
        return 0; /*  at[prev_sb_neigh] atom can not be adjacent to a stereo
                     bond/cumulene */
      /*  or does not have 3 attachments (hydrogens are not considered here) */
      for (i = 0, k = 0; i < MAX_NUM_STEREO_BONDS &&
                         (neigh = at[prev_sb_neigh].stereo_bond_neighbor[i]) &&
                         !(k = (neigh - 1 == cur));
           i++)
        ;
      if (!k) {
        return -1; /*  program error: could not locate stereogenic bond mark on
                      the opposite atom */
      }
      k = (int)at[prev_sb_neigh]
              .stereo_bond_ord[i]; /*  seq. number of the double or cumulene
                                      bond on at[prev_sb_neigh] */

      for (i = 0, num_other_neigh = 0;
           i < at[prev_sb_neigh].valence && num_other_neigh <= MAX_OTHER_NEIGH;
           i++) {
      ...
      for (i = 0; i < num_other_neigh; i++) {
        ...
        if (nVisited1[k] && nVisited1[k] != nCanonRank[k]) {
          return 0; /*  parity of the statring stereo bond/cumulene has not
                       changed. */
        }
  ...
  if (nCheckingMode == CHECKING_STEREOCENTER) {
    ...
    for (i = 0, num_other_neigh = 0;
         i < at[cur].valence && num_other_neigh <= MAX_OTHER_NEIGH; i++) {
    ...
    */
    ...
    if (bParitiesInverted && at[cur].valence == MAX_NUM_STEREO_ATOM_NEIGH) {
      ...
      if (bParitiesInverted == 1 && parity_vis_1 == canon_parity) {
        return 0; /* not a typical case of inversion during the mapping of D4h
                     stereocenter */
   ...
   ******************************************************/
  for (i = 0; i < pCS->nLenLinearCTStereoCarb;
       i++, nNumComparedCenters += (k > 0)) {
    r1 = pCS->LinearCTStereoCarb[i].at_num;
    i01 = nAtomNumberCanon[r1 - 1]; /*  ord. number of the atom that has canon
                                       rank r1 */

    i11 = nAtomNumberCanon1[r1 - 1]; /*  = (MAX_ATOMS+1) > num_atoms if the atom
    ...
    bCurParityInv1 =
        (bParitiesInverted && at[cur].nRingSystem == at[i11].nRingSystem &&
         at[cur].nRingSystem == at[i12].nRingSystem);

    k = 0;

    /*  check whether the two stereo centers (they can be one and the same atom)
     * have been traversed */
    if (!s1 && !s2) {
      continue; /*  Both stereo centers have not been traversed; check the next
                   pair. */
    }
    ...
    if (nCheckingMode == CHECKING_STEREOCENTER) {
      ...
      case 1:
        ...
        /* case 0: */
        /*     break;  */ /*  the stereo centers are not the sarting stereo
                             center */
      }
      if (cur == i01) {
    ...
    if (nNeighMode == NEIGH_MODE_RING) {
      if (i11 != i12 && !bCurParityInv1) {
        return -1; /*  failed: the two stereo atoms have not been traversed
                      synchronously */
      }
      if (!at[i11].parity || !at[i12].parity) {
        return 0; /*  another atom does not have parity (it might have been
                     removed) 9-11-2002 */
      }
    }
    if (nNeighMode == NEIGH_MODE_CHAIN) {
      if (s1 + s2 != 1) {
        return -1; /*  program error: only one out of s1 and s2 must be 1,
                      another must be 0. */
      }
    ...
    if ((nNeighMode == NEIGH_MODE_RING && (i11 != i01) && (i12 != i01)) ||
        /*  in NEIGH_MODE_RING case we know that i11 == i12 except
           bCurParityInv1 == 1 */
       ...
       */
      parity1 =
          s1 ? GetStereoCenterParity(at, i11, nVisited1) : PARITY_IMPOSSIBLE;
      parity2 =
          s2 ? GetStereoCenterParity(at, i12, nVisited2) : PARITY_IMPOSSIBLE;
      if (!ATOM_PARITY_KNOWN(parity1) && !ATOM_PARITY_KNOWN(parity2)) {
        return -1; /*  should not happen: must have been detected at the time of
                      the traversal */
      }
      if (s1 && s2) {
        if (bCurParityInv1) {
          ...
          if (i11 == i12 ||
              ((parity1 == parity1orig || parity2 == parity2orig ||
                parity1 != parity2) &&
...
#if (PROPAGATE_ILL_DEF_STEREO == 1)
        /*  at least one parity is ill-defined. Use parity1 and parity2 to
         * temporarily save bitmaps */
        parity1 = (parity == vABParityUnknown /*AB_PARITY_UNKN*/)
                      ? NOT_WELL_DEF_UNKN
                  : (parity == AB_PARITY_UNDF) ? NOT_WELL_DEF_UNDF
                                               : 0;
        parity2 = (parity12 == vABParityUnknown /*AB_PARITY_UNKN*/)
                      ? NOT_WELL_DEF_UNKN
                  : (parity12 == AB_PARITY_UNDF) ? NOT_WELL_DEF_UNDF
                                                 : 0;
        if (parity1 | parity2) {
      ...
      /*  the parity of the new neighbors permutation must be same as the old
       * one */
      ...
      /* nNeighMode == NEIGH_MODE_RING and only one out of the two (i11 == i01)
       * (i12 == i01) is true */
      return -1;
    }
  ...
  /******************************************************/
  for (i = 0; i < pCS->nLenLinearCTStereoDble;
       i++, nNumComparedBonds += (k > 0)) {
    r1 = pCS->LinearCTStereoDble[i].at_num1;
    r2 = pCS->LinearCTStereoDble[i].at_num2;
    i01 = nAtomNumberCanon[r1 - 1]; /*  ord. number of the atom that originally
                                       has canon rank r1 */
    i02 = nAtomNumberCanon[r2 - 1]; /*  ord. number of the atom that originally
                                       has canon rank r2 */

    i11 = nAtomNumberCanon1[r1 - 1]; /*  ord. number of the atom that got canon
                                        rank r1 during the parallel traversal */
    i12 = nAtomNumberCanon1[r2 - 1]; /*  ord. number of the atom that got canon
                                        rank r2 during the parallel traversal */
    ...
    /*  check whether the two stereo bonds/allenes (they can be one and the
     * same) have been traversed */
    if (!s1 && !s2) {
      continue; /*  Both stereo bonds/cumulenes have not been traversed; check
                   the next pair. */
    }

    if (nCheckingMode == CHECKING_STEREOBOND) {
      switch (((i11 == cur && i12 == prev_sb_neigh) ||
               (i12 == cur && i11 == prev_sb_neigh)) +
              ((i21 == cur && i22 == prev_sb_neigh) ||
               (i22 == cur && i21 == prev_sb_neigh))) {
      case 2:
      ...
      if ((i01 == cur && i02 == prev_sb_neigh) ||
          (i02 == cur && i01 == prev_sb_neigh)) {
    ...
    if (nNeighMode == NEIGH_MODE_RING) {
      if ((i11 != i21 || i12 != i22) && (i11 != i22 || i12 != i21)) {
        return -1; /*  failed: the two bonds/cumulenes have not been traversed
                      synchronously */
      }
      if (0 > GetStereoNeighborPos(at, i11, i12)) {
        return 0; /*  another bond is not stereo (the stereo might have been
                     removed) 9-11-2002 */
      }
    }
    if (nNeighMode == NEIGH_MODE_CHAIN) {
    ...
    if ((nNeighMode == NEIGH_MODE_RING && (i11 != i01 || i12 != i02) &&
         (i11 != i02 || i12 != i01)) ||
        nNeighMode == NEIGH_MODE_CHAIN /*  in NEIGH_MODE_CHAIN case here we
                                          always have 2 different atoms */
    ) {
      /*******************************************/
      /*  case of two transposed bonds/cumulenes */
      parity1 =
          s1 ? GetStereoBondParity(at, i11, i12, nVisited1) : PARITY_IMPOSSIBLE;
      parity2 =
          s2 ? GetStereoBondParity(at, i21, i22, nVisited2) : PARITY_IMPOSSIBLE;
      if (!ATOM_PARITY_KNOWN(parity1) && !ATOM_PARITY_KNOWN(parity2)) {
        return -1; /*  should not happen: must have been detected at the time of
                      traversal */
      }
      if (s1 && s2 &&
          (((i11 != i21 || i12 != i22) && (i11 != i22 || i12 != i21)) ||
           parity1 != parity2)) {
      ...
      if (ATOM_PARITY_WELL_DEF(parity) && parity == parity12) {
        ...
        if (nCheckingMode == CHECKING_STEREOBOND &&
          ...
          if ((nSymmRank[cur] == nSymmRank[i01] &&
               nSymmRank[prev_sb_neigh] == nSymmRank[i02]) ||
              (nSymmRank[cur] == nSymmRank[i02] &&
               nSymmRank[prev_sb_neigh] == nSymmRank[i01])) {
            nNumEqStereogenic++;
          }
        }
      } else if (ATOM_PARITY_WELL_DEF(parity) &&
      ...
      /*****************************************************************************************/
      /*  i11-i12 and i21-i22 are same as i01-i02 bond/cumulene, nNeighMode ==
       * NEIGH_MODE_RING */
      AT_NUMB n1, n2;
      int j;
      if (!s1 || !s2) {
      ...
      for (j = 0, n1 = MAX_ATOMS + 1;
           j < MAX_NUM_STEREO_BOND_NEIGH && at[i01].stereo_bond_neighbor[j];
           j++) {
      ...
      for (j = 0, n2 = MAX_ATOMS + 1;
           j < MAX_NUM_STEREO_BOND_NEIGH && at[i02].stereo_bond_neighbor[j];
           j++) {
      ...
      /*  actual parity (that includes the geometry) is not important here. */
      /*  old permutation */
      parity = GetPermutationParity(at + i01, n1, nCanonRank) +
               GetPermutationParity(at + i02, n2, nCanonRank);
      /*  new parmutation */
      parity1 = GetPermutationParity(at + i01, n1, nVisited1) +
                GetPermutationParity(at + i02, n2, nVisited1);
      parity2 = GetPermutationParity(at + i01, n1, nVisited2) +
                GetPermutationParity(at + i02, n2, nVisited2);
  ...
  if (nNumEqStereogenic > 0) {
    /*  case similar to trimethylcyclopropane: 3 constitutionally equivalent
  ...
  /* =========================================================================================
      Note
      ====
      At this point the comparison is complete and no difference sufficient to
     establish absence of stereo parity has been found. However, non-zero
     not_well_def_parities means that an ill-defined parity was compared to an
     ill-defined or well-defined parity. This means that the parity of the atom
     or bond being checked cannot be well-defined anymore.
     ========================================================================================*/
...
int RemoveCalculatedNonStereoBondParities(
  ...
  AT_RANK nAtomRank1, nAtomRank2, neigh[3], nAvoidCheckAtom[2], opposite_atom,
      nLength;
...
second_pass:
  ...
  for (i1 = 0; i1 < num_atoms && !RETURNED_ERROR(ret_failed); i1++) {
    ...
    for (n1 = 0; n1 < MAX_NUM_STEREO_BONDS && !RETURNED_ERROR(ret_failed) &&
                 (s2 = at[i1].stereo_bond_neighbor[n1]);
         n1++) {
      if (!PARITY_CALCULATE(at[i1].stereo_bond_parity[n1]) &&
          PARITY_WELL_DEF(at[i1].stereo_bond_parity[n1])) {
        continue;
      }
      opposite_atom = (AT_RANK)(s2 - 1);
      s2 = at[i1].neighbor[(int)at[i1].stereo_bond_ord[n1]]; /*  different from
                                                                opposite_atom in
                                                                case of a
                                                                cumulene */
      for (j = 1, n = 0; j <= (int)at[i1].valence; j++) {
        if (nl[i1][j] != s2) {
          neigh[n++] = nl[i1][j]; /*  sorting guarantees that canon. rank of
                                     neigh[0] is greater or equal */
        }
      }
      if (n != 2) {
      ...
      bRingNeigh =
          (at[(int)neigh[0]].nRingSystem == at[(int)neigh[1]].nRingSystem);
      switch (nNeighMode) {
      ...
      case NEIGH_MODE_RING:
        ...
        bSymmNeigh = BreakNeighborsTie(
            at, num_atoms, num_at_tg, opposite_atom, i1, neigh, 0, 1, 0,
            pRankStack1, pRankStack2, nTempRank, NeighList, nSymmRank,
            nCanonRank, nl1, nl2, &pCS->lNumNeighListIter);
        if (bSymmNeigh <= 0) {
      ...
      bParitiesInverted =
          (nNeighMode == NEIGH_MODE_RING &&
           ...
           at[opposite_atom].valence == MAX_NUM_STEREO_BONDS)
              ? -1
              : 0;
      ret1 = ret2 = 0;
      if (0 < (ret1 = CreateCheckSymmPaths(
                   at, (AT_RANK)i1, neigh[0], (AT_RANK)i1, neigh[1],
                   nAvoidCheckAtom, nVisited1, nVisited2, nAtomNumberCanon1,
                   nAtomNumberCanon2, nl01, nl02, nSymmRank1, nSymmRank2,
                   nCanonRank, &nLength, &bParitiesInverted, 0)) &&
          0 < (ret2 = CalculatedPathsParitiesAreIdentical(
                   at, num_atoms, nSymmRank, nCanonRank, nAtomNumberCanon,
                   nAtomNumberCanon1, nAtomNumberCanon2, nVisited1, nVisited2,
                   opposite_atom, (AT_RANK)i1, neigh[0], neigh[1], nNeighMode,
                   bParitiesInverted, 0, pCS, vABParityUnknown))) {
        if (ret2 & (NOT_WELL_DEF_UNKN | NOT_WELL_DEF_UNDF)) {
          /*  possibly change the parity to unknown or undefined */
          int new_parity = (ret2 & NOT_WELL_DEF_UNKN)
          ...
          if ((PARITY_ILL_DEF(at[i1].stereo_bond_parity[n1]) &&
            ...
            SetOneStereoBondIllDefParity(
                at, i1, /* atom number*/ n1 /* stereo bond ord. number*/,
                new_parity);
            /*  change in pCS */
  ...
  if (nNeighMode == NEIGH_MODE_CHAIN && nNumEqRingNeigh &&
...
exit_function:
  ...
  return RETURNED_ERROR(ret_failed) ? ret_failed
         : ret_failed               ? -(ret_failed + 1)
                                    : ret;
}
/****************************************************************************/
...
int RemoveCalculatedNonStereoCenterParities(
  ...
  AT_RANK nAtomRank1, neigh[MAX_NUM_STEREO_ATOM_NEIGH], nAvoidCheckAtom[2],
      nLength;
  int nNeighMode = NEIGH_MODE_CHAIN;
  int nNumEqRingNeigh = 0, bRingNeigh, bSymmNeigh, bParitiesInverted;
  NEIGH_LIST *nl01, *nl02;
...
second_pass:
  for (i = 0; i < num_atoms && !RETURNED_ERROR(ret_failed); i++) {
    ...
    if (at[i].valence > MAX_NUM_STEREO_ATOM_NEIGH) {
      continue;
      /*  error: stereo center cannot have more than 4 neighbors */ /*   <BRKPT>
                                                                     */
    }
    /*  at[i1] is a stereo center */
    if (!PARITY_CALCULATE(at[i].stereo_atom_parity) &&
        !PARITY_ILL_DEF(at[i].stereo_atom_parity)) {
      continue;
    }
    /* neighbors sorted according to symm. ranks (primary key) and canon. ranks
     * (secondary key), in descending order */
    /* sorting guarantees that for two constit. equ. neighbors canon. ranks of
     * the first is greater */
    /* !!! previously (but not anymore) the canon. rank of neigh[0] was greater
     * than the others !!! */
    for (j = 0; j < at[i].valence; j++) {
      neigh[j] = nl[i][j + 1]; /*  sorting does NOT guarantee that canon. rank
                                  of neigh[0] is greater than others */
    }
    /*
...
#endif
    for (j = 0;
         j < at[i].valence && at[i].parity && !RETURNED_ERROR(ret_failed);
         j++) {
      for (k = j + 1;
           k < at[i].valence && at[i].parity && !RETURNED_ERROR(ret_failed);
           k++) {
        for (mode = 0;
             mode <= max_mode && at[i].parity && !RETURNED_ERROR(ret_failed);
             mode++) {
          ...
          bRingNeigh =
              (at[(int)neigh[j]].nRingSystem == at[(int)neigh[k]].nRingSystem);
          switch (nNeighMode) {
          case NEIGH_MODE_CHAIN:
            if (bRingNeigh) {
              nNumEqRingNeigh++;
          ...
          case NEIGH_MODE_RING:
            if (!bRingNeigh)
              continue;
            /*  break a tie between the two contitutionally equivalent
             * neighbors, */
            /*  refine the two partitions, sort neighbors lists nl1, nl2 */
            bSymmNeigh = BreakNeighborsTie(
                at, num_atoms, num_at_tg, MAX_ATOMS + 1, i, neigh, j, k, mode,
                pRankStack1, pRankStack2, nTempRank, NeighList, nSymmRank,
                nCanonRank, nl1, nl2, &pCS->lNumNeighListIter);
            if (bSymmNeigh <= 0) {
              if (ret_failed > bSymmNeigh)
          ...
          memset(nVisited1, 0, sizeof(nVisited1[0]) * num_atoms);
          memset(nVisited2, 0, sizeof(nVisited2[0]) * num_atoms);
          memset(nAtomNumberCanon1, 0,
                 sizeof(nAtomNumberCanon1[0]) * num_atoms);
          memset(nAtomNumberCanon2, 0,
                 sizeof(nAtomNumberCanon2[0]) * num_atoms);
          nLength = 1;
          ...
          bParitiesInverted =
              (mode == MAP_MODE_C2v || mode == MAP_MODE_S4) ? -1 : 0;
          /*
          if (nNeighMode==NEIGH_MODE_RING &&
          at[i].valence==MAX_NUM_STEREO_ATOM_NEIGH) { AT_RANK other_neigh[2];
              int     n;
              for ( m = n = 0; m < MAX_NUM_STEREO_ATOM_NEIGH; m ++ ) {
                  if ( at[i].neighbor[m] != neigh[j] && at[i].neighbor[m] !=
          neigh[k] ) other_neigh[n++] = at[i].neighbor[m];
              }
              if ( nSymmRank[(int)other_neigh[0]] ==
          nSymmRank[(int)other_neigh[1]] ) bParitiesInverted = -1;
          }
          */
          /* allow matching inverted centers only in case all equivalent
           * neighbors in same ring system */

          ret2 = 0; /* initilize. 1/8/2002 */

          if (0 < (ret1 = CreateCheckSymmPaths(
                       at, (AT_RANK)i, neigh[j], (AT_RANK)i, neigh[k],
                       nAvoidCheckAtom, nVisited1, nVisited2, nAtomNumberCanon1,
                       nAtomNumberCanon2, nl01, nl02, nSymmRank1, nSymmRank2,
                       nCanonRank, &nLength, &bParitiesInverted, mode)) &&
              0 < (ret2 = CalculatedPathsParitiesAreIdentical(
                       at, num_atoms, nSymmRank, nCanonRank, nAtomNumberCanon,
                       nAtomNumberCanon1, nAtomNumberCanon2, nVisited1,
                       nVisited2, (AT_RANK)MAX_ATOMS, (AT_RANK)i, neigh[j],
                       neigh[k], nNeighMode, bParitiesInverted, mode, pCS,
                       vABParityUnknown))) {
            if (ret2 & (NOT_WELL_DEF_UNKN | NOT_WELL_DEF_UNDF)) {
              /*  possibly change the parity to unknown or undefined */
              int new_parity = (ret2 & NOT_WELL_DEF_UNKN)
...
int RemoveCalculatedNonStereo(sp_ATOM *at, int num_atoms, int num_at_tg,
  ...
  AT_RANK *nVisited1 = NULL, *nVisited2 = NULL, *nAtomNumberCanon1 = NULL,
          *nAtomNumberCanon2 = NULL;
  int nNumRemoved = 0, nTotRemoved = 0, ret = 0, ret1 = 0, ret2 = 0;

  if (!AllocateForNonStereoRemoval(at, num_atoms, nSymmRank, nCanonRank,
                                   &nAtomNumberCanon1, &nAtomNumberCanon2, &nl,
                                   &nl1, &nl2, &nVisited1, &nVisited2)) {
  ...
  do {
    nNumRemoved = 0;
    /*  bonds */
    ret = RemoveCalculatedNonStereoBondParities(
        at, num_atoms, num_at_tg, pRankStack1, pRankStack2, nTempRank,
        NeighList, nCanonRank, nSymmRank, nAtomNumberCanon, nAtomNumberCanon1,
        nAtomNumberCanon2, nl, nl1, nl2, nVisited1, nVisited2, pCS,
        vABParityUnknown);
...
exit_function:
  ...
  DeAllocateForNonStereoRemoval(&nAtomNumberCanon1, &nAtomNumberCanon2, &nl,
                                &nl1, &nl2, &nVisited1, &nVisited2);
