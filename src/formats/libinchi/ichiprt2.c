...
int Eql_INChI_Stereo(INChI_Stereo *s1, int eql1, INChI_Stereo *s2, int eql2,
  ...
  if (EQL_SP2 == eql1) {
    if ((len = s1->nNumberOfStereoBonds) > 0 && s1->b_parity &&
        s1->nBondAtom1 && s1->nBondAtom2) {
      if (!s2) {
        if (EQL_EXISTS == eql2) {
      ...
      if (EQL_SP2 == eql2 && len == s2->nNumberOfStereoBonds && s2->b_parity &&
          s2->nBondAtom1 && s2->nBondAtom2 &&
          !memcmp(s1->nBondAtom1, s2->nBondAtom1,
                  len * sizeof(s1->nBondAtom1[0])) &&
          !memcmp(s1->nBondAtom2, s2->nBondAtom2,
                  len * sizeof(s1->nBondAtom2[0])) &&
      ...
      if ((eql2 == EQL_SP3 || (inv2 = (eql2 == EQL_SP3_INV))) &&
        ...
        if (inv2) {
          if (s2->nCompInv2Abs && s1->nCompInv2Abs) {
            ...
            /* if one sp3 is inverted then another should have non-trivial
             * inverted stereo */
            return 0;
          }
        } else {
...
int Eql_INChI_Aux_Equ(INChI_Aux *a1, int eql1, INChI_Aux *a2, int eql2) {
  ...
  t2 = (eql2 & EQL_EQU_TG);
  if (t1 && t2) {
    if ((len = a1->nNumberOfTGroups) > 0 && len == a2->nNumberOfTGroups &&
        !a1->bDeleted && !a2->bDeleted) {
      if (eql1 & EQL_EQU_ISO) {
        if (a1->bIsIsotopic) {
    ...
    if ((len = a1->nNumberOfAtoms) > 0 && len == a2->nNumberOfAtoms &&
        !a1->bDeleted && !a2->bDeleted) {
      if (eql1 & EQL_EQU_ISO) {
        if (a1->bIsIsotopic) {
...
int bHasOrigInfo(ORIG_INFO *OrigInfo, int num_atoms) {
  ...
  if (OrigInfo && num_atoms > 0) {
    for (i = 0; !bFound && i < num_atoms; i++) {
      bFound |= (0 != OrigInfo[i].cCharge) || (0 != OrigInfo[i].cRadical) ||
...
int EqlOrigInfo(INChI_Aux *a1, INChI_Aux *a2) {
  int ret = a1 && a2 && a1->nNumberOfAtoms == a2->nNumberOfAtoms &&
            ...
            !memcmp(a1->OrigInfo, a2->OrigInfo,
                    a1->nNumberOfAtoms * sizeof(a1->OrigInfo[0]));
  return ret;
}
...
/********************************************************************************************/
int MakeMult(int mult, const char *szTailingDelim, char *szLinearCT,
             int nLen_szLinearCT, int nCtMode, int *bOverflow) {
  char szValue[16];
  int len = 0, len_delim;
  if (mult == 1 || *bOverflow)
    return 0;
...
/********************************************************************************************/
int MakeDelim(const char *szTailingDelim, char *szLinearCT, int nLen_szLinearCT,
              int *bOverflow) {
  int len_delim;
...
/********************************************************************************************/
int MakeEqStr(const char *szTailingDelim, int mult, char *szLinearCT,
...
int MakeCtStringNew(AT_NUMB *LinearCT, int nLenCT, int bAddDelim,
                    S_CHAR *nNum_H, int num_atoms, char *szLinearCT,
                    int nLen_szLinearCT, int nCtMode, int *bOverflow) {
  /*  produce output string; */
  ...
  int bCountRingClosures =
      bAbcNumbers && bPredecessors && (nCtMode & CT_MODE_ABC_NUM_CLOSURES);
  if (nLenCT <= 1) {
    return 0; /*  no atoms or a single atom: no connection table */
  }
  /*  make array containing connection string data */
  if (!(nDfsOrderCT =
            GetDfsOrder4CT(LinearCT, nLenCT, nNum_H, num_atoms, nCtMode))) {
    (*bOverflow)++;
    return 0;
  }
  ...
  if (!bOvfl) {
    ...
    for (i = 0; nDfsOrderCT[i] && nLen < nLen_szLinearCT; i += 3) {
        ...
        if (bCountRingClosures) {
          if (nDelim == '-' && i > 3 && bNoNum_H) {
            if (!nNumRingClosures) {
              ...
              if (nNumRingClosures) {
                len += MakeDecNumber(szValue + len, (int)sizeof(szValue) - len,
                                     NULL, nNumRingClosures);
              }
              nNumRingClosures--;
            } else {
              nNumRingClosures--;
      ...
      if (bAbcNumbers) {
        if (nValue ||
            i) { /* the 1st value may be zero in case of presdecessor list */
          len += MakeAbcNumber(szValue + len, (int)sizeof(szValue) - len, NULL,
                               nValue);
        }
        if (num_H) {
...
int MakeCtStringOld(AT_NUMB *LinearCT, int nLenCT, int bAddDelim,
                    char *szLinearCT, int nLen_szLinearCT, int nCtMode,
                    int *bOverflow) {
  /*  produce output string; */
  ...
  if (!bOvfl) {
    for (i = 0; i < nLenCT && nLen < nLen_szLinearCT; i++) {
      ...
      if (!(nCtMode & CT_MODE_NO_ORPHANS) ||
          ((bLessThanPrev = LinearCT[i] < nMax) ||
           (i + 1 < nLenCT && LinearCT[i + 1] < (nMax = LinearCT[i])))) {
        nValue = LinearCT[i];
        if (nCtMode & CT_MODE_ABC_NUMBERS) {
          len = MakeAbcNumber(szValue, (int)sizeof(szValue),
                              (!bNext && bAddDelim) ? ITEM_DELIMETER : NULL,
                              nValue);
        } else if (nCtMode & CT_MODE_NO_ORPHANS) { /*  censored CT */
          /*  output '-' as a delimiter to show a bonding for decimal output of
           * the connection table */
          len = MakeDecNumber(szValue, (int)sizeof(szValue),
                              bLessThanPrev ? "-" : ITEM_DELIMETER, nValue);
        } else {
          len = MakeDecNumber(szValue, (int)sizeof(szValue),
                              i ? ITEM_DELIMETER : NULL, nValue);
        }
        if (0 <= len && nLen + len < nLen_szLinearCT) {
          if (len) {
            strcpy(szLinearCT + nLen, szValue);
 ...
 *            2: compressed CT (Abs numbers)
 **********************************************************************************************/
int MakeHString(int bAddDelim, S_CHAR *LinearCT, int nLenCT, char *szLinearCT,
                int nLen_szLinearCT, int nCtMode, int *bOverflow) {
  ...
  if (!bOvfl && 0 < nLenCT && LinearCT) {
    if (nCtMode & CT_MODE_EQL_H_TOGETHER) {
      ...
      for (curNumH = curMinH; curNumH <= curMaxH; curNumH++) {
        numAt = nNumH[curNumH -
                      curMinH]; /* number of atoms that have curNumH atoms H */
        if (!numAt || !curNumH) {
        ...
        while (j < nLenCT && numAt) {
          if (curNumH == LinearCT[j]) {
            ...
            if (nCtMode & CT_MODE_ABC_NUMBERS) {
              ...
              len = MakeDecNumber(szValue, (int)sizeof(szValue),
                                  bNext ? ITEM_DELIMETER : NULL, iFirst);
            ...
            if (iFirst < j) {
              ...
              if (nCtMode & CT_MODE_ABC_NUMBERS) {
                len +=
                    MakeAbcNumber(szValue + len, (int)sizeof(szValue), NULL, j);
              } else {
                len += MakeDecNumber(szValue + len, (int)sizeof(szValue) - len,
                                     "-", j);
              }
            ...
            if (!numAt || (nCtMode & CT_MODE_ABC_NUMBERS)) {
              ...
              if (nCtMode & CT_MODE_ABC_NUMBERS) {
                ...
                nVal = abs(nVal);
                if (nVal > 1) {
                  len += MakeDecNumber(szValue + len,
                                       (int)sizeof(szValue) - len, pH, nVal);
                } else {
      ...
      for (i = iFirst + 1; i <= nLenCT && nLen < nLen_szLinearCT; i++) {
        ...
        if (LinearCT[iFirst]) { /* output only non-zero values */
          ...
          if (iFirst < i) {
            ...
            if (nCtMode & CT_MODE_ABC_NUMBERS) {
              len +=
                  MakeAbcNumber(szValue + len, (int)sizeof(szValue), NULL, i);
            } else {
              len += MakeDecNumber(szValue + len, (int)sizeof(szValue) - len,
                                   "-", i);
            }
...
/**********************************************************************************************
 *  nCtMode = 0: full
 *            1: censored CT (no orphans, that CT should have only atoms with
 *neighbors) 2: compressed CT (Abc numbers)
 **********************************************************************************************/
int MakeCtString(AT_NUMB *LinearCT, int nLenCT, int bAddDelim, S_CHAR *nNum_H,
                 int num_atoms, /* both parameters are not used here */
                 char *szLinearCT, int nLen_szLinearCT, int nCtMode,
                 int *bOverflow) {

  if (!nNum_H || !(nCtMode & CT_MODE_NO_ORPHANS)) {
    return MakeCtStringOld(LinearCT, nLenCT, bAddDelim, szLinearCT,
                           nLen_szLinearCT, nCtMode, bOverflow);
  } else {
    return MakeCtStringNew(LinearCT, nLenCT, bAddDelim, nNum_H, num_atoms,
 ...
 *            2: compressed CT: do not add comma before the output string if
 *bAddDelim != 0 do not add parentheses around t-groups atom canon numbers an
 *Abc LinearCT format: N      = number of tautomeric groups n      = number of
 *endpoints + 1 in a tautomeric group #1 next INCHI_T_NUM_MOVABLE lines (any
 *after the first non-zero): h      = number of hydrogen atoms in the tautomeric
 *group m      = number of negative charges
 *    ...    (the rest of the INCHI_T_NUM_MOVABLE has not been established,
 *ignore them) c(1)   = canonical number of the first atom in the t-group
 *    ...
...
int MakeTautString(AT_NUMB *LinearCT, int nLenCT, int bAddDelim,
  ...
  if (!bOvfl) {
    for (i = nGroupLen = iGroupOutputCount = 0;
         i < nLenCT && nLen < nLen_szLinearCT; i++) {
      ...
      if (nGroupLen == iGroupOutputCount) {
        ...
        /*  (a) this is the first output value in compressed mode (i==1 &&
         * bCompressed) */
        /*  (b) this is not the first output value in non-compressed mode (
         * iGroupOutputCount && !bCompressed) */
        if (bCompressed) {
          p = NULL;
          len = 0;
          ...
          case 0:
            len = MakeDecNumber(szValue, (int)sizeof(szValue),
                                (i == 1) ? ITEM_DELIMETER : NULL, nValue);
            break;
          case 1:
            p = "-";
            break;
          ...
          if (iGroupOutputCount >= INCHI_T_NUM_MOVABLE) {
            ...
            len = MakeDecNumber(szValue, (int)sizeof(szValue), ITEM_DELIMETER,
                                nValue);
          } else {
            p = NULL;
            len = 0;
 ...
 *  nCtMode = 0: full
 *            2: compressed CT
 *  22+3s3:  22=canon. number; +3=charge; s=singlet (d=doublet, t=triplet, s is
 *omitted if valence=0), 3 = valence 22+3.3, (charge, valence) 22.3 (valence)
 *22t3 (triplet, valence) Ab+3t4:  Ab=canon. number; +3=charge or "." t=triplet
 *(or s, d), 4=valence
 **********************************************************************************************/
    ...
    if (OrigInfo[k].cCharge || OrigInfo[k].cRadical ||
      ...
      if (bAbcNumbers) {
        /*
            ...
            dot before valence:        charge is present, no radical, valence is
           present
         */
...
int MakeEquString(AT_NUMB *LinearCT, int nLenCT, int bAddDelim,
  ...
  for (k = 0; !bOvfl && k < nLenCT && nLen < nLen_szLinearCT; k++) {
    ...
    for (i = k; i < nLenCT && nLen < nLen_szLinearCT; i++) {
      if (k != (int)LinearCT[i] - 1)
        continue;
      /*  equivalence number: a minimal canon_number out of a group of
       * equivalent atoms */
      /*  is at canon_number-1 position of each equivalent atom.  */
      if (bAbcNumbers) {
        len = MakeAbcNumber(szValue, (int)sizeof(szValue),
                            (i == k && bNext) ? ITEM_DELIMETER : NULL, i + 1);
      } else {
        len = MakeDecNumber(szValue, (int)sizeof(szValue),
                            (i == k) ? "(" : ITEM_DELIMETER, i + 1);
      }
      if (0 <= len && nLen + len < nLen_szLinearCT) {
        strcpy(szLinearCT + nLen, szValue);
        nLen += len;
...
int MakeIsoAtomString(INChI_IsotopicAtom *IsotopicAtom,
                      int nNumberOfIsotopicAtoms, char *szLinearCT,
                      int nLen_szLinearCT, int nCtMode, int *bOverflow) {
  /*  produce output string; */
      ...
      for (j = 0; j < 5; j++) {
        ...
        if (!j) {
          ...
          case 1: /* nIsoDifference */
            len =
                MakeDecNumber(p, (int)sizeof(szValue) - tot_len, NULL, nValue);
            break;
          case 2: /* nNum_T */
          case 3: /* nNum_D */
          case 4: /* nNum_H */
            if (nValue) {
              if ((int)sizeof(szValue) - tot_len > 1) {
                ...
                if (1 == nValue) {
                  ...
                  ret = MakeDecNumber(p + len,
                                      (int)sizeof(szValue) - tot_len - len,
                                      NULL, nValue);
          ...
          if (j == 1) { /*  Decimal output */
            ...
            /*  subtract 1 from it so that mass difference for 35Cl or 12C is
             * zero */
            len = MakeDecNumber(p, (int)sizeof(szValue) - tot_len,
                                sign[nValue >= 0], abs(nValue - subtract));
          } else {
            /*  hydrogen isotope */
            if (nValue != 1) {
              len = MakeDecNumber(p, (int)sizeof(szValue) - tot_len, h[j - 2],
                                  nValue);
...
int MakeIsoTautString(INChI_IsotopicTGroup *IsotopicTGroup,
                      int nNumberOfIsotopicTGroups, char *szLinearCT,
                      int nLen_szLinearCT, int nCtMode, int *bOverflow) {
  /*  produce output string; */
      ...
      for (j = 0; j < 4; j++) {
        ...
        if (!j) {
          ...
          if (bAbcNumbers) {
            ...
            /*  hydrogen isotope */
            if (nValue != 1) {
              len = MakeDecNumber(p, (int)sizeof(szValue) - tot_len, h[j - 1],
                                  nValue);
            } else if ((int)sizeof(szValue) - tot_len > 1) {
              strcpy(p, h[j - 1]);
              len = 1;
...
/**********************************************************************************************/
int MakeIsoHString(int num_iso_H[], char *szLinearCT, int nLen_szLinearCT,
...
/**********************************************************************************************/
int MakeStereoString(AT_NUMB *at1, AT_NUMB *at2, S_CHAR *parity, int bAddDelim,
                     int nLenCT, char *szLinearCT, int nLen_szLinearCT,
                     int nCtMode, int *bOverflow) {
  /*  produce output string; */
  int nLen = 0, len, tot_len, i, j, bOvfl = *bOverflow;
  char szValue[32];
    ...
    for (i = 0; i < nLenCT && nLen < nLen_szLinearCT; i++) {
      ...
      for (j = 0; j < 3; j++) {
        if (j == 0 && at1)
          ...
          continue;
        if (nCtMode & CT_MODE_ABC_NUMBERS) {
          len = (j == 2 ? MakeDecNumber : MakeAbcNumber)(
              p, (int)sizeof(szValue) - tot_len, NULL, nValue);
        } else {
          if (j < 2) {
            len = MakeDecNumber(p, (int)sizeof(szValue) - tot_len,
                                tot_len            ? "-"
                                : (i || bAddDelim) ? ITEM_DELIMETER
                                                   : NULL,
                                nValue);
          } else if (tot_len + 1 < (int)sizeof(szValue)) {
            *p++ = (0 <= nValue && nValue <= 4) ? parity_char[nValue]
                                                : parity_char[0];
            *p = '\0';
            len = 1;
          } else {
...
int MakeAbcNumber(char *szString, int nStringLen, const char *szLeadingDelim,
                  int nValue) {
  char *p = szString;
  char *q;
  int nChar;
...
long inchi_strtol(const char *str, const char **p, int base) {
  if (base == ALPHA_BASE) {
    return abctol(str,
                  (char **)p); /* cast deliberately discards const qualifier */
  } else {
    return strtol(str, (char **)p,
                  base); /* cast deliberately discards const qualifier */
  }
}
#endif
#undef ALPHA_BASE
...
/*  szString length nStringLen includes 1 byte for zero termination */
/*  Return Value: length without zero termination; -1 means not enough room */
int MakeDecNumber(char *szString, int nStringLen, const char *szLeadingDelim,
                  int nValue) {
