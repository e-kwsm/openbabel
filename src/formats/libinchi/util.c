...
typedef struct tagElData {
  const char *szElName;
  int nAtMass;     /* Avg. atomic mass from the Periodic Chart of the Elements
  ...
  int bDoNotAddH; /* InChI does not add implicit H to atoms that have bDoNotAddH
                     != 0 */
...
int needed_unusual_el_valence(int nPeriodicNum, int charge, int radical,
  ...
  int i, num_found, num_found_known, chem_valence, rad_adj, known_chem_valence,
      exact_found;
  int num_H_expected;
  char szElement[4];
  /*
  ...
  */
  ...
  chem_valence = bonds_valence + num_H;
  if (charge < MIN_ATOM_CHARGE || charge > MAX_ATOM_CHARGE ||
      !get_el_valence(nPeriodicNum, charge, 0) || do_not_add_H(nPeriodicNum) ||
...
int extract_ChargeRadical(char *elname, int *pnRadical, int *pnCharge) {
  ...
  while ((q = strpbrk(p, "+-^"))) {
    ...
    case '+':
    case '-':
      for (k = 0, nVal = 0; (nSign = ('+' == q[k])) || (nSign = -('-' == q[k]));
           k++) {
      ...
      nCharge += nVal;
      break;
    /* case '.': */ /*  singlet '.' may be confused with '.' in formulas like
                       CaO.H2O */
    case '^':
...
int get_num_H(const char *elname, int inp_num_H, S_CHAR inp_num_iso_H[],
  ...
  if (bAliased) {
    ...
    if (radical && radical != RADICAL_SINGLET) {
      ...
      /* find the smallest valence that is greater than the sum of the chemical
       * bond valences */
      for (i = 0;
           ...
           i++)
        ;
      /* special case: do not add H to N(IV), S(III), S+(II), S-(II) */ /* S
                                                                           ions
                                                                           added
                                                                           2004-05-10
                                                                         */
      if (el_number == el_number_N && !charge && !radical && val == 5)
        ...
        */
        if (el_number == el_number_S && !charge && !radical && val == 4 &&
      ...
      if ( (el_number == el_number_S || el_number == el_number_O) &&
           abs(charge)==1 && !radical && val == 3 && chem_bonds_valence == 2 &&
      bHasMetalNeighbor ) val = 2; else
      */
      num_H = inchi_max(0, val - chem_bonds_valence);
    }
    num_iso_H = 0;
    ...
    if (num_iso_H) {
      if (num_H >= num_iso_H) {
        ...
        /* num_H = (num_iso_H - num_H) % 2; */ /*  keep unchanged parity of the
    ...
    if (inp_num_H > num_H) {
      num_H = inp_num_H; /*  as requested in the alias */
      /* num_H = inp_num_H + (inp_num_H - num_H)%2; */ /*  keep unchanged parity
                                                          of the number of
                                                          non-isotopic H atoms
                                                        */
    }
  } else {
    num_H = inp_num_H;
...
int normalize_name(char *name) {
  /* remove leading & trailing spaces; replace consecutive spaces with a single
   * space */
...
int mystrncpy(char *target, const char *source,
              unsigned maxlen) { /*  protected from non-zero-terminated source
                                    and overlapped target/source. 7-9-99 DCh. */
  const char *p;
  unsigned len;
  ...
  if ((p = (const char *)memchr(source, 0, maxlen))) {
    ...
    len = maxlen - 1; /*  reduced length does not include one more byte for zero
                         termination */
  }
  if (len)
...
/*
    ...
    Here we consider any character not conforming this specification as a
   whitespace which marks the end of the InChI string. For example:
    "InChI=1/Ar%"
    "InChI=1/Ar\n"
    "InChI=1/Ar\r\t"
...
#if (defined(COMPILE_ADD_NON_ANSI_FUNCTIONS) ||                                \
