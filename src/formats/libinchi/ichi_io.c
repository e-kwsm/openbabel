...
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    If INCHI_IOSTREAM type is INCHI_IOSTREAM_STRING,
        flush INCHI_IOSTREAM string buffer to file (if non-NULL); then free
buffer. If INCHI_IOSTREAM type is INCHI_IOSTREAM_FILE, just flush the file.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
...
then free buffer. If INCHI_IOSTREAM type is INCHI_IOSTREAM_FILE, just flush the
both files.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
...
/*^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
    Reset INCHI_IOSTREAM: set string buffer ptr to NULL (but do _not_ free
...
/*******************************************************************/
int inchi_ios_gets(char *szLine, int len, INCHI_IOSTREAM *f,
                   int *bTooLongLine) {
  int length;
  char *p;
  do {
...
/* remove leading and trailing white spaces; keep zero termination */
/*******************************************************************/
int inchi_ios_getsTab(char *szLine, int len, INCHI_IOSTREAM *f,
                      int *bTooLongLine) {
  int length;
  char *p;
  do {
...
/*******************************************************************/
int inchi_ios_getsTab1(char *szLine, int len, INCHI_IOSTREAM *f,
                       int *bTooLongLine) {
  int length;
  char *p;
  /*do {*/
  ...
  if (ios->type == INCHI_IOSTREAM_STRING) {
    ...
    if (max_len >= 0) {
      if (ios->s.nAllocatedLength - ios->s.nUsedLength <= max_len) {
        ...
        char *new_str = (char *)inchi_calloc(
            ios->s.nAllocatedLength + nAddLength, sizeof(new_str[0]));
        if (new_str) {
          if (ios->s.pStr) {
            if (ios->s.nUsedLength > 0)
              memcpy(new_str, ios->s.pStr,
                     sizeof(new_str[0]) * ios->s.nUsedLength);
            inchi_free(ios->s.pStr);
          }
          ios->s.pStr = new_str;
...
int inchi_ios_eprint(INCHI_IOSTREAM *ios, const char *lpszFormat, ...) {
  ...
  if (ios->type ==
      INCHI_IOSTREAM_STRING) /* was #if ( defined(TARGET_API_LIB) ||
                                defined(INCHI_STANDALONE_EXE) ) */
  {
    /* output to string buffer */
    ...
    if (max_len >= 0) {
      if (ios->s.nAllocatedLength - ios->s.nUsedLength <= max_len) {
        ...
        new_str = (char *)inchi_calloc(ios->s.nAllocatedLength + nAddLength,
                                       sizeof(new_str[0]));
        if (new_str) {
          if (ios->s.pStr) {
            if (ios->s.nUsedLength > 0) {
...
int inchi_vfprintf(FILE *f, const char *lpszFormat, va_list argList) {
  int ret = 0;
  if (f == stderr && lpszFormat && lpszFormat[0] &&
      '\r' == lpszFormat[strlen(lpszFormat) - 1]) {
#define CONSOLE_LINE_LEN 80
    ...
    if (ret < 0) {
      ...
      /*^^^ Fixed bug: (CONSOLE_LINE_LEN-4) --> (CONSOLE_LINE_LEN-4-1) 11-22-08
       * IPl */
...
char *inchi_fgetsLf(char *line, int line_len, FILE *inp) {
  ...
  if (NULL != (p = fgets(line, line_len, inp)) && NULL == strchr(p, '\n')) {
    char temp[64]; /* bypass up to '\n' or up to end of file whichever comes
                      first*/
...
int GetMaxPrintfLength(const char *lpszFormat, va_list argList) {
  ...
  for (lpsz = lpszFormat; *lpsz; lpsz++) {
    ...
    case 'h':
      ...
      case 'X':
      case 'u':
        /* short unsigned, short double, etc. -- added to the original MS
         * example */
