...
int CompareInchiStereo(INChI_Stereo *Stereo1, INCHI_MODE nFlags1,
                       INChI_Stereo *Stereo2, INCHI_MODE nFlags2);
int CompareReversedStereoINChI(
    INChI_Stereo *s1 /* InChI from reversed struct */,
...
int FillOutCanonInfAtom(inp_ATOM *norm_at, INF_ATOM_DATA *inf_norm_at_data,
                        int init_num_at, int bIsotopic, INChI *pINChI,
                        INChI_Aux *pINChI_Aux, int bAbcNumbers,
                        INCHI_MODE nMode);
int FillOutOneCanonInfAtom(inp_ATOM *inp_norm_at,
                           INF_ATOM_DATA *inf_norm_at_data,
                           AT_NUMB *pStereoFlags, int init_num_at, int offset,
                           int offset_H, int bIsotopic, INChI *pINChI,
                           INChI_Aux *pINChI_Aux, int bAbcNumbers,
                           INCHI_MODE nMode);
int FillOutInputInfAtom(inp_ATOM *inp_at, INF_ATOM_DATA *inf_at_data,
                        int init_num_at, int num_removed_H,
...
int CheckCanonNumberingCorrectness(int num_atoms, int num_at_tg, sp_ATOM *at,
                                   CANON_STAT *pCS, int bTautomeric,
                                   char *pStrErrStruct);
...
int GetElementAndCount(const char **f, char *szEl, int *count) {
  ...
  strcpy(
      szEl,
      "Zz"); /*  zero termination 'element' is larger than any other element */
  *count = 9999; /*  zero termination 'element count' is larger than any other
                    count */
  return 0;
}
/**********************************************************************************************
...
/**********************************************************************************************/
int CompareHillFormulasNoH(const char *f1, const char *f2, int *num_H1,
                           int *num_H2) {
  char szEl1[4], szEl2[4];
  int count1, count2, ret1, ret2, ret;
 ...
 */
/**********************************************************************************************/
int CompINChITautVsNonTaut(const INCHI_SORT *p1, const INCHI_SORT *p2,
  ...
  n1 = (p1->pINChI[TAUT_YES] && p1->pINChI[TAUT_YES]->nNumberOfAtoms)
           ? TAUT_YES
           : TAUT_NON;

  i1 = p1->pINChI[n1];
  i2 = (n1 == TAUT_YES && p2->pINChI[TAUT_NON] &&
        p2->pINChI[TAUT_NON]->nNumberOfAtoms)
           ? p2->pINChI[TAUT_NON]
           : (const INChI *)NULL;
  ...
  if ((ret = CompareHillFormulasNoH(i1->szHillFormula, i2->szHillFormula,
                                    &num_H1, &num_H2))) {
    return ret; /* lexicographic order except the shorter one is greater (last):
                   CH2O < CH2; C3XX < C2XX */
  }
  ...
  for (i = 0; i < num;
       i++) { /* should always be equal if Hill formulas are same */
  ...
  */
  ...
  if (bCompareIsotopic) {
    ...
    for (i = 0; i < num; i++) {
      if ((ret = (int)i2->IsotopicAtom[i].nAtomNumber -
                 (int)i1->IsotopicAtom[i].nAtomNumber))
        return ret;
      if ((ret = (int)i2->IsotopicAtom[i].nIsoDifference -
                 (int)i1->IsotopicAtom[i].nIsoDifference))
        return ret;
    }
    /* compare isotopic H */
    /* if tautomeric comparison mode then here are compared only non-tautomeric
     * H */
    for (i = 0; i < num; i++) {
      if ((ret = (int)i2->IsotopicAtom[i].nNum_T -
                 (int)i1->IsotopicAtom[i].nNum_T))
        return ret;
      if ((ret = (int)i2->IsotopicAtom[i].nNum_D -
                 (int)i1->IsotopicAtom[i].nNum_D))
        return ret;
      if ((ret = (int)i2->IsotopicAtom[i].nNum_H -
                 (int)i1->IsotopicAtom[i].nNum_H))
        return ret;
    }
    /*****************************************************
    ...
    for ( i = 0; i < num; i ++ ) {
        if ( ret = (int)i2->IsotopicTGroup[i].nTGroupNumber -
    (int)i1->IsotopicTGroup[i].nTGroupNumber ) return ret; if ( ret =
    (int)i2->IsotopicTGroup[i].nNum_T - (int)i1->IsotopicTGroup[i].nNum_T )
            return ret;
        if ( ret = (int)i2->IsotopicTGroup[i].nNum_D -
    (int)i1->IsotopicTGroup[i].nNum_D ) return ret; if ( ret =
    (int)i2->IsotopicTGroup[i].nNum_H - (int)i1->IsotopicTGroup[i].nNum_H )
            return ret;
    }
    */
    /****************************************************
     ...
     ****************************************************/
    ret = CompareInchiStereo(i1->StereoIsotopic, i1->nFlags, i2->StereoIsotopic,
                             i2->nFlags);
    if (ret) {
      return ret;
    }
...
typedef enum tagSp3StereoTypeTmp {
  ...
  /* /t is present: */
  SP3_ONLY =
      1, /* no /s or /m segment: inversion leaves the structure unchanged */
  ...
  SP3_TYPE = (SP3_ABS | SP3_REL |
              SP3_RAC), /* bitmap for checking the presence of /m */
  SP3_ANY = (SP3_ABS | SP3_REL | SP3_RAC |
             SP3_ONLY) /* bitmap for checking the presence of /t */
} SP3_TYPE_TMP;

/**********************************************************************************************/
int GetSp3RelRacAbs(const INChI *pINChI, INChI_Stereo *Stereo) {
  int nRet = SP3_NONE;
  if (pINChI && !pINChI->bDeleted && Stereo &&
      0 < Stereo->nNumberOfStereoCenters) {
    if (0 != Stereo->nCompInv2Abs) {
      if (pINChI->nFlags & INCHI_FLAG_REL_STEREO) {
        ...
        if (!((pINChI->nFlags &
              ...
              1 == Stereo->nNumberOfStereoCenters))
#endif
    {
...
int CompINChILayers(const INCHI_SORT *p1, const INCHI_SORT *p2,
  ...
  if (i1 && !i1->bDeleted && i1->szHillFormula && i1->szHillFormula[0]) {
    ...
    if (i2 && !i2->bDeleted && i2->szHillFormula && i2->szHillFormula[0]) {
      if (!CompareHillFormulasNoH(i1->szHillFormula, i2->szHillFormula, &num_H1,
                                  &num_H2) &&
          num_H1 == num_H2) {
  ...
  /* M: H atoms */
  if (i1 && !i1->bDeleted) {
    num_H1 = (i1->lenTautomer > 0 && i1->nTautomer && i1->nTautomer[0])
                 ? 1
                 : 0; /* number of t-groups */
    if (!num_H1 && i1->nNum_H) {
    ...
    sDifSegs[DIFL_M][DIFS_h_H_ATOMS] |=
        num_H1 ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
  } else {
  ...
  if (i2 && !i2->bDeleted && i2->nNum_H_fixed) {
    ...
    sDifSegs[DIFL_F][DIFS_h_H_ATOMS] |=
        num_H2 ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
  } else {
  ...
  if (i1 && !i1->bDeleted) {
    ...
    if (i2 && !i2->bDeleted) {
      ...
      if (bFixTranspChargeBug == 1) {
        ...
        v1.01 charge compared decided that charge layers are same and omitted
        Fixed-H /q layer

        Solution: when component permutation is detected AND fixed-H component
        does not exist, compare Mobile-H charge [0 (comp#1) in the example] to
        the charge of Mobile-H [-1 (comp#2)] of the component that has none
        Fixed-H charge
        */

        /* Fixed-H i2 is empty because Fixed-H struct is same as Mobile-H */
        if (p1->ord_number != p2->ord_number && /* component order in Fixed-H is
                                                   different from Mobile-H */
            n1 == TAUT_YES && p2->pINChI[TAUT_YES] &&
            !p2->pINChI[TAUT_YES]->bDeleted &&
  ...
  if (IsoStereo1 && IsoStereo1->nNumberOfStereoBonds) {
    ...
    if (Stereo1 && Stereo1->nNumberOfStereoBonds) {
      *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no
                                        isotopes */
    } else {
      *psDifSegs |= DIFV_BOTH_EMPTY;
    }
  ...
  if (SP3_NONE != bRelRac[DIFL_M]) {
    sDifSegs[DIFL_M][DIFS_t_SATOMS] |=
        (bRelRac[DIFL_M] & SP3_ANY) ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
    sDifSegs[DIFL_M][DIFS_m_SP3INV] |=
        (bRelRac[DIFL_M] & SP3_ABS) ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
    sDifSegs[DIFL_M][DIFS_s_STYPE] |=
        (bRelRac[DIFL_M] & SP3_TYPE) ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
  } else {
  ...
  if (SP3_ANY & bRelRac[DIFL_MI]) {
    ...
    *psDifSegs |=
        DIFV_EQL2PRECED; /* isotopic is missing because there is no isotopes */
  } else {
    *psDifSegs |= DIFV_BOTH_EMPTY;
  }
  /* FI sp3 stereo */
  ...
  if (SP3_ANY & bRelRac[DIFL_FI]) {
    if (Eql_INChI_Stereo(IsoStereo2, EQL_SP3, Stereo2, EQL_SP3, 0)) {
      *psDifSegs |= DIFV_EQL2PRECED;
    } else if (!(SP3_ANY & bRelRac[DIFL_M]) && !(SP3_ANY & bRelRac[DIFL_F]) &&
  ...
  if (bRelRac[DIFL_F] & SP3_ABS) {
    /* the order of || operands below is critically important: || is not a
     * commutative operation */
    if (!(bRelRac[DIFL_M] & SP3_ABS) ||
        Stereo2->nCompInv2Abs != Stereo1->nCompInv2Abs) {
      *psDifSegs |= DIFV_NEQ2PRECED;
    } else {
      *psDifSegs |= DIFV_EQL2PRECED;
    }
  ...
  if (SP3_ABS & bRelRac[DIFL_MI]) {
    if ((SP3_ABS & bRelRac[DIFL_M]) &&
        IsoStereo1->nCompInv2Abs == Stereo1->nCompInv2Abs) {
      *psDifSegs |= DIFV_EQL2PRECED;
    } else {
      *psDifSegs |= DIFV_NEQ2PRECED;
    }
  ...
  if (SP3_ABS & bRelRac[DIFL_FI]) {
    if ((SP3_ABS & bRelRac[DIFL_F]) &&
        IsoStereo2->nCompInv2Abs == Stereo2->nCompInv2Abs) {
      *psDifSegs |= DIFV_EQL2PRECED;
    } else if (!(SP3_ABS & bRelRac[DIFL_M]) && !(SP3_ABS & bRelRac[DIFL_F]) &&
               (SP3_ABS &
                bRelRac[DIFL_MI]) && /* make sure IsoStereo1 != NULL */
    ...
    /* the order of || operands below is critically important: || is no a
     * commutative operation */
    if ((SP3_ABS & bRelRac[DIFL_F])) {
      *psDifSegs |= DIFV_EQL2PRECED; /* isotopic is missing because there is no
                                        isotopes */
  ...
  if (SP3_TYPE & bRelRac[DIFL_FI]) {
    if ((SP3_TYPE & bRelRac[DIFL_FI]) == (SP3_TYPE & bRelRac[DIFL_F])) {
      *psDifSegs |= DIFV_EQL2PRECED;
    } else if (!(SP3_TYPE & bRelRac[DIFL_M]) && !(SP3_TYPE & bRelRac[DIFL_F]) &&
  ...
  if (i1 && !i1->bDeleted &&
      (i1->nNumberOfIsotopicAtoms || i1->nNumberOfIsotopicTGroups)) {
    *psDifSegs |= DIFV_NEQ2PRECED;
  } else {
    *psDifSegs |= DIFV_BOTH_EMPTY;
  }
    ...
    if (i2->nNumberOfIsotopicAtoms || i2->nNumberOfIsotopicTGroups) {
      if (!i1 || i1->bDeleted ||
        ...
        *psDifSegs |= DIFV_NEQ2PRECED;
        ...
        for (i = 0; i < num; i++) {
          /* compare isotopic atoms */
          if ((diff = (int)i2->IsotopicAtom[i].nAtomNumber -
                      (int)i1->IsotopicAtom[i].nAtomNumber))
            break;
          if ((diff = (int)i2->IsotopicAtom[i].nIsoDifference -
                      (int)i1->IsotopicAtom[i].nIsoDifference))
            break;
          /* compare isotopic H */
          if ((diff = (int)i2->IsotopicAtom[i].nNum_T -
                      (int)i1->IsotopicAtom[i].nNum_T))
            break;
          if ((diff = (int)i2->IsotopicAtom[i].nNum_D -
                      (int)i1->IsotopicAtom[i].nNum_D))
            break;
          if ((diff = (int)i2->IsotopicAtom[i].nNum_H -
                      (int)i1->IsotopicAtom[i].nNum_H))
            break;
        }
        if (!diff) {
          ...
          for (i = 0; i < num; i++) {
            if ((diff = (int)i2->IsotopicTGroup[i].nTGroupNumber -
              ...
              break;
            if ((diff = (int)i2->IsotopicTGroup[i].nNum_T -
                        (int)i1->IsotopicTGroup[i].nNum_T))
              break;
            if ((diff = (int)i2->IsotopicTGroup[i].nNum_D -
                        (int)i1->IsotopicTGroup[i].nNum_D))
              return diff;
            if ((diff = (int)i2->IsotopicTGroup[i].nNum_H -
                        (int)i1->IsotopicTGroup[i].nNum_H))
              break;
          }
        }
        *psDifSegs |= diff ? DIFV_NEQ2PRECED : DIFV_FI_EQ_MI;
      }
    } else if (i1 && !i1->bDeleted &&
               (i1->nNumberOfIsotopicAtoms || i1->nNumberOfIsotopicTGroups)) {
      *psDifSegs |= DIFV_IS_EMPTY;
    }
  } else if (!i2) {
...
int CompareInchiStereo(INChI_Stereo *Stereo1, INCHI_MODE nFlags1,
  ...
  if (Stereo2 && Stereo1) {
    ...
    num =
        inchi_min(Stereo2->nNumberOfStereoBonds, Stereo1->nNumberOfStereoBonds);
    for (i = 0; i < num; i++) {
    ...
    if ((ret = (int)Stereo2->nNumberOfStereoBonds -
               (int)Stereo1->nNumberOfStereoBonds))
      return ret;
...
#endif
    {
      num = inchi_min(Stereo2->nNumberOfStereoCenters,
                      Stereo1->nNumberOfStereoCenters);
      for (i = 0; i < num; i++) {
      ...
      if ((ret = (int)Stereo2->nNumberOfStereoCenters -
                 (int)Stereo1->nNumberOfStereoCenters))
        return ret;
      /*  compare stereo-abs-is-inverted flags  for non-relative, non-racemic */
      if (!((nFlags1 | nFlags2) &
            (INCHI_FLAG_RAC_STEREO | INCHI_FLAG_REL_STEREO))) {
...
#if (REL_RAC_STEREO_IGN_1_SC == 1)
                             && /*  do not compare single stereocenters in case
                                   of relative stereo */
                             !((nFlags2 & (INCHI_FLAG_REL_STEREO |
 ...
 */
/**********************************************************************************************/
int CompINChI2(const INCHI_SORT *p1, const INCHI_SORT *p2, int bTaut,
               int bCompareIsotopic) {
  int ret, num, i, num_H1, num_H2;
  ...
  const INChI *i1n =
      NULL; /* non-tautomeric if both tautomeric AND non-tautomeric exist */
  const INChI *i2n =
      NULL; /* non-tautomeric if both tautomeric AND non-tautomeric exist */

  /*const INChI *i1t = NULL;*/ /* temp for i1 if both tautomeric AND
                                  non-tautomeric exist */
  /*const INChI *i2t = NULL;*/ /* temp for i2 if both tautomeric AND
                                  non-tautomeric exist */
  ...
  n1 = (p1->pINChI[TAUT_YES] && p1->pINChI[TAUT_YES]->nNumberOfAtoms)
           ? TAUT_YES
           : TAUT_NON;
  n2 = (p2->pINChI[TAUT_YES] && p2->pINChI[TAUT_YES]->nNumberOfAtoms)
           ? TAUT_YES
           : TAUT_NON;

  i1 = p1->pINChI[n1];
  i1n = (n1 == TAUT_YES && p1->pINChI[TAUT_NON] &&
         p1->pINChI[TAUT_NON]->nNumberOfAtoms)
            ? p1->pINChI[TAUT_NON]
            : (const INChI *)NULL;
  ...
  */
   ...
   *********************************************************/
  if (TAUT_NON == bTaut &&
      ((i1n && i1n->nNum_H_fixed) || (i2n && i2n->nNum_H_fixed))) {
    /* first, compare non-tautomeric chem. formulas -- they may be different */
    const char *f1 =
        (i1n /*&& i1n->nNum_H_fixed*/) ? i1n->szHillFormula : i1->szHillFormula;
    const char *f2 =
        (i2n /*&& i2n->nNum_H_fixed*/) ? i2n->szHillFormula : i2->szHillFormula;
    ...
    if (i1n && i1n->nNum_H_fixed && i2n && i2n->nNum_H_fixed) {
      ...
      /* p1 is tautomeric, p2 is not tautomeric; this must have been detected
       * earlier */
      ...
      /* p2 is tautomeric, p1 is not tautomeric; this must have been detected
       * earlier */
      /*return 1; */ /* has fixed H first */ /* <BRKPT> */ /* removed 2004-05-04
                                                            */
    }
  ...
  if (bCompareIsotopic) {
    ...
    for (i = 0; i < num; i++) {
      if ((ret = (int)i2->IsotopicTGroup[i].nTGroupNumber -
                 (int)i1->IsotopicTGroup[i].nTGroupNumber))
        return ret;
      if ((ret = (int)i2->IsotopicTGroup[i].nNum_T -
                 (int)i1->IsotopicTGroup[i].nNum_T))
        return ret;
      if ((ret = (int)i2->IsotopicTGroup[i].nNum_D -
                 (int)i1->IsotopicTGroup[i].nNum_D))
        return ret;
      if ((ret = (int)i2->IsotopicTGroup[i].nNum_H -
                 (int)i1->IsotopicTGroup[i].nNum_H))
        return ret;
    }
...
int CompINChINonTaut2(const void *p1, const void *p2) {
  ...
  if (!ret) {
    ...
    ret =
        CompINChI2((const INCHI_SORT *)p1, (const INCHI_SORT *)p2, TAUT_YES, 1);
  }
#endif
  if (!ret) {
    /* stable sort */
    ret = ((const INCHI_SORT *)p1)->ord_number -
          ((const INCHI_SORT *)p2)->ord_number;
  }
  return ret;
}
/***********************************************************************/
int CompINChITaut2(const void *p1, const void *p2) {
  ...
  if (!ret) {
    /* to obtain canonical transposition 2004-05-10 */
    ret =
        CompINChI2((const INCHI_SORT *)p1, (const INCHI_SORT *)p2, TAUT_NON, 1);
  }
#endif
  if (!ret) {
    /* stable sort */
...
static int CompareDfsDescendants4CT(const void *a1, const void *a2) {
  ...
  if (neigh1 > MAX_ATOMS) {
    ...
    int nDesc1 = nCurDfsNumber > gDfs4CT_nDfsNumber[neigh1]
                     ? 0
                     : (int)gDfs4CT_nNumDescendants[neigh1];
    int nDesc2 = nCurDfsNumber > gDfs4CT_nDfsNumber[neigh2]
                     ? 0
                     : (int)gDfs4CT_nNumDescendants[neigh2];
    int ret;
...
AT_NUMB *GetDfsOrder4CT(AT_NUMB *LinearCT, int nLenCT, S_CHAR *nNum_H,
                        int num_atoms, int nCtMode) {
  AT_NUMB *nStackAtom = NULL;
  int nTopStackAtom = -1;
  AT_NUMB *nNumDescendants =
      NULL; /*  number of descendants incl. closures and the atom itself */
  ...
  nNumDescendants =
      (AT_NUMB *)inchi_malloc(num_atoms * sizeof(nNumDescendants[0]));
  ...
  /*
    vertex information:
      1. Number of (forward edges) + (back edges, first visit -- ring closures):
    nl[i][0]
      2. Number of vertices traversed from this vertex, including the vertex:
    nNumDescendants[i]
      3. Each edge information:
         a. forward edge (0) or back edge (1) indicator: nDfsNumber[i] >
    nDfsNumber[neigh] b. neighbor at another end of the edge neigh = nl[i][k+1],
    k < i
  ...
  */
  ...
  do {
    /* advance */
    while (i = (int)nStackAtom[nTopStackAtom], j = (int)cNeighNumb[i] + 1,
           (int)nl[i][0] >= j)
    /*while ( (int)nl[i=nStackAtom[nTopStackAtom]][0] >= (j =
       (int)cNeighNumb[i]+1) )*/
    /* replaced due to missing sequence point; undefined behavior, pointed by
       Geoffrey Hutchison */
    {
      cNeighNumb[i]++;
  ...
  /* Sort the neighbors in ascending order so that:
     primary key   = number of descendants in the DFS tree; closure neighbor is
     0 secondary key = canonical number (here vertex number = canonical number -
     1)
   */
  ...
  for (i = 0; i < num_atoms; i++) {
    if (nl[i][0] > 1) {
      ...
      insertions_sort(&nl[i][1], nl[i][0], sizeof(nl[i][1]),
  ...
  nTotOutputStringLen = 3 * (num_atoms + num_rings +
                             1); /*  last 3 elements are a 'zero termination' */

  if (bCtPredecessors) {
    if ((nOutputString = (AT_RANK *)inchi_calloc(nTotOutputStringLen,
                                                 sizeof(nOutputString[0])))) {
      cDelim = '-';
      for (u = 0, k = -3; u < num_atoms; u++) {
        ...
        nOutputString[k] =
            nNumDescendants[u] ? nNumDescendants[u] : MAX_ATOMS + 1;
        nOutputString[k + 1] = nNum_H ? 16 + nNum_H[u] : 0;
        nOutputString[k + 2] = k ? ',' : '\0';
        for (j = 1; j <= nl[u][0] && nDfsNumber[u] > nDfsNumber[i = nl[u][j]];
             j++) {
          /* closures */
          k += 3;
          if (k + 6 > nTotOutputStringLen) {
    ...
    /*
        the output string contains:
          ...
          3 characters  for each atom that has 10-99 terminal hydrogen atoms,
       etc. (num_rings) atoms for the ring closures (num_rings) delimiters for
       the ring closures
    */
    ...
    if ((nOutputString = (AT_RANK *)inchi_calloc(nTotOutputStringLen,
      ...
      do {
        ...
        /*while ( (int)nl[i=nStackAtom[nTopStackAtom]][0] >= (j =
           (int)cNeighNumb[i]+1) )*/
        /* replaced due to missing sequence point; undefined behavior, reported
...
/**********************************************************************************************/
int GetInpStructErrorType(INPUT_PARMS *ip, int err, char *pStrErrStruct,
                          int num_inp_atoms) {
  if (err && err == 9)
...
/**********************************************************************************************/
int ProcessStructError(INCHI_IOSTREAM *output_file,
                       INCHI_IOSTREAM *log_file, /*int err,*/
                       char *pStrErrStruct, int nErrorType,
                       int *bXmlStructStarted, long num_inp, INPUT_PARMS *ip,
  ...
  if (nErrorType) {
    if (bPlainText) {
      if (!(b_ok = OutputINChIPlainError(output_file, pStr, nStrLen,
                                         pStrErrStruct, nErrorType))) {
        inchi_ios_eprint(log_file,
                         "Cannot create message for error (structure "
      ...
      if (!(b_ok = OutputINChIXmlError(output_file, pStr, nStrLen, 2,
                                       /*err,*/ pStrErrStruct, nErrorType))) {
        inchi_ios_eprint(log_file,
                         "Cannot create xml tag for error (structure "
                         "#%ld.%s%s%s%s) Terminating.\n",
                         num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
      }
      if (!b_ok || nErrorType == _IS_FATAL || nErrorType == _IS_ERROR) {
        ...
        if (!OutputINChIXmlStructEndTag(output_file, pStr, nStrLen, 1)) {
          inchi_ios_eprint(log_file,
                           "Cannot create end xml tag for structure "
                           "#%ld.%s%s%s%s Terminating.\n",
                           num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
          *bXmlStructStarted = -1;
          b_ok = 0;
...
int CompareStereoINChI(INChI_Stereo *s1, INChI_Stereo *s2) {
  ...
  if (s1->nNumberOfStereoCenters > 0) {
    if (memcmp(s1->nNumber, s2->nNumber,
               s1->nNumberOfStereoCenters * sizeof(s1->nNumber[0])))
      return 22;
    if (memcmp(s1->t_parity, s2->t_parity,
               s1->nNumberOfStereoCenters * sizeof(s1->t_parity[0])))
      return 23;
    if (s1->nNumberInv && s2->nNumberInv) {
      if (memcmp(s1->nNumberInv, s2->nNumberInv,
                 s1->nNumberOfStereoCenters * sizeof(s1->nNumber[0])))
        return 28;
      if (memcmp(s1->t_parityInv, s2->t_parityInv,
                 s1->nNumberOfStereoCenters * sizeof(s1->t_parity[0])))
        return 29;
    ...
    } else if (s1->nNumberInv || s2->nNumberInv) {
      return 31;
    }
  }
  ...
  if (s1->nNumberOfStereoBonds > 0) {
    if (memcmp(s1->nBondAtom1, s2->nBondAtom1,
               s1->nNumberOfStereoBonds * sizeof(s1->nBondAtom1[0])))
      return 25;
    if (memcmp(s1->nBondAtom2, s2->nBondAtom2,
               s1->nNumberOfStereoBonds * sizeof(s1->nBondAtom2[0])))
      return 26;
    if (memcmp(s1->b_parity, s2->b_parity,
               s1->nNumberOfStereoBonds * sizeof(s1->b_parity[0])))
      return 27;
  }
  return 0;
}
...
int CompareINChI(INChI *i1, INChI *i2, INChI_Aux *a1, INChI_Aux *a2) {
  ...
  if (i1->nNumberOfAtoms > 0) {
    if (memcmp(i1->nAtom, i2->nAtom, i1->nNumberOfAtoms * sizeof(i1->nAtom[0])))
      return 4;
    if (memcmp(i1->nNum_H, i2->nNum_H,
               i1->nNumberOfAtoms * sizeof(i1->nNum_H[0])))
      return 5;
    if (i1->nNum_H_fixed && i2->nNum_H_fixed &&
        memcmp(i1->nNum_H_fixed, i2->nNum_H_fixed,
               i1->nNumberOfAtoms * sizeof(i1->nNum_H_fixed[0]))) {
      return 6;
    }
  ...
  if (i1->lenConnTable > 0 &&
      memcmp(i1->nConnTable, i2->nConnTable,
             i1->lenConnTable * sizeof(i1->nConnTable[0])))
    return 9;

  if (i1->lenTautomer != i2->lenTautomer)
    return 10;
  if (i1->lenTautomer > 0 && memcmp(i1->nTautomer, i2->nTautomer,
                                    i1->lenTautomer * sizeof(i1->nTautomer[0])))
    return 11;
  ...
  if (i1->nNumberOfIsotopicTGroups != i2->nNumberOfIsotopicTGroups)
    return 14;
  if (i1->nNumberOfIsotopicTGroups > 0 &&
      memcmp(i1->IsotopicTGroup, i2->IsotopicTGroup,
             i1->nNumberOfIsotopicTGroups * sizeof(i1->IsotopicTGroup[0])))
    return 15;
  if (a1->nNumRemovedProtons != a2->nNumRemovedProtons)
    return 16;
  if (memcmp(a1->nNumRemovedIsotopicH, a2->nNumRemovedIsotopicH,
             sizeof(a1->nNumRemovedIsotopicH)))
    return 17;
  if (i1->nPossibleLocationsOfIsotopicH && i2->nPossibleLocationsOfIsotopicH) {
    if (i1->nPossibleLocationsOfIsotopicH[0] !=
            i2->nPossibleLocationsOfIsotopicH[0] ||
        memcmp(i1->nPossibleLocationsOfIsotopicH,
               i2->nPossibleLocationsOfIsotopicH,
               sizeof(i1->nPossibleLocationsOfIsotopicH[0]) *
                   i1->nPossibleLocationsOfIsotopicH[0]))
      return 18;
  } else if (!i1->nPossibleLocationsOfIsotopicH !=
             !i2->nPossibleLocationsOfIsotopicH) {
    return 19;
  }
  /* ret = 20..31 */
...
int CompareReversedStereoINChI(
  ...
  if (s1->nNumberOfStereoCenters > 0) {
    ...
    if (s1->nCompInv2Abs != s2->nCompInv2Abs && s1->nCompInv2Abs &&
        s2->nCompInv2Abs)
...
int CompareReversedStereoINChI2(
  ...
  int ret = 0;
  int j1, j2, num_eq, num_dif, num_extra_undf, num_miss_undf, num_in1_only,
      num_in2_only;
  int bAddSb = !(picr->num_sb_undef_in1_only + picr->num_sb_in1_only +
                 picr->num_sb_in2_only);
  int bAddSc = !(picr->num_sc_undef_in1_only + picr->num_sc_in1_only +
                 picr->num_sc_in2_only);
  ...
  if ((nNumSc1 || nNumSc2) &&
    ...
    num_eq = num_dif = num_extra_undf = num_miss_undf = num_in1_only =
        num_in2_only = 0;
  ...
  if (s1 && s2 && s1->nCompInv2Abs != s2->nCompInv2Abs && s1->nCompInv2Abs &&
      s2->nCompInv2Abs) {
    ret |= IDIF_SC_INV;
  }
  ...
  if ((nNumSb1 || nNumSb2) &&
      (nNumSb1 != nNumSb2 ||
       memcmp(s1->nBondAtom1, s2->nBondAtom1,
              nNumSb1 * sizeof(s1->nBondAtom1[0])) ||
       memcmp(s1->nBondAtom2, s2->nBondAtom2,
              nNumSb1 * sizeof(s1->nBondAtom2[0])) ||
    ...
    for (j1 = j2 = 0; j1 < nNumSb1 && j2 < nNumSb2;) {
      if (s1->nBondAtom1[j1] == s2->nBondAtom1[j2] &&
        ...
        j2++;
                 ...
                 (s1->nBondAtom1[j1] == s2->nBondAtom1[j2] &&
                  s1->nBondAtom2[j1] < s2->nBondAtom2[j2])) {
        num_in1_only++;
...
int CompareReversedINChI(INChI *i1 /* InChI from reversed struct */,
                         INChI *i2 /* input InChI */, INChI_Aux *a1,
                         INChI_Aux *a2) {
  int ret;
  if (i1 == NULL && i2 == NULL)
    return 0;
  ...
  /* output special cases: different number of t-groups, different sizes of
   * t-groups, different endpoints */
  if (i1->lenTautomer != i2->lenTautomer &&
      (i1->lenTautomer > 1 || i2->lenTautomer > 1))
    return 10; /* Diff: Mobile groups length */ /* in isotopic or deprotonated
                                                   cases i1->lenTautomer == 1 &&
                                                   i1->nTautomer[0] = 0 */
  if ((i1->lenTautomer > 1 && i2->lenTautomer > 1) &&
      memcmp(i1->nTautomer, i2->nTautomer,
             i1->lenTautomer * sizeof(i1->nTautomer[0])))
  ...
  if (i1->nTotalCharge != i2->nTotalCharge)
    return 14; /* Diff: Charge */
               /*
                   if ( i1->nNumberOfIsotopicTGroups != i2->nNumberOfIsotopicTGroups )
                       return 14;
                   if ( i1->nNumberOfIsotopicTGroups > 0 && memcmp( i1->IsotopicTGroup,
                  i2->IsotopicTGroup,
                  i1->nNumberOfIsotopicTGroups*sizeof(i1->IsotopicTGroup[0]) ) )              return 15;
               */
  if (a1 && a2) {
    if (a1->nNumRemovedProtons != a2->nNumRemovedProtons)
  ...
  /*
      if ( i1->nPossibleLocationsOfIsotopicH &&
     i2->nPossibleLocationsOfIsotopicH ) { if (
     i1->nPossibleLocationsOfIsotopicH[0] !=
     i2->nPossibleLocationsOfIsotopicH[0] ||
               memcmp(i1->nPossibleLocationsOfIsotopicH,
     i2->nPossibleLocationsOfIsotopicH,
                      sizeof(i1->nPossibleLocationsOfIsotopicH[0])*i1->nPossibleLocationsOfIsotopicH[0])
     ) return 18; } else if ( !i1->nPossibleLocationsOfIsotopicH !=
     !i2->nPossibleLocationsOfIsotopicH ) { return 19;
      }
  */
  /* ret = 20..31 => 40..51 */
  ...
  if (!i2->StereoIsotopic && i2->Stereo && i1->StereoIsotopic &&
      0 < (i1->StereoIsotopic->nNumberOfStereoBonds +
           i1->StereoIsotopic->nNumberOfStereoCenters) &&
      0 == CompareReversedStereoINChI(i1->StereoIsotopic, i2->Stereo)) {
    /* InChI from reversed structure does not contain fully duplicated isotopic
     * stereo */
    ;
  } else

      if ((ret = CompareReversedStereoINChI(i1->StereoIsotopic,
                                            i2->StereoIsotopic))) {
    return ret + 40;
  }
...
/*******************************************************************************/
int CompareIcr(ICR *picr1, ICR *picr2, INCHI_MODE *pin1, INCHI_MODE *pin2,
               INCHI_MODE mask) {
...
INCHI_MODE CompareReversedINChI2(INChI *i1 /* InChI from reversed struct */,
  ...
  *err = 0;
  ...
  if (i1->nNumberOfAtoms > 0) {
    if (memcmp(i1->nAtom, i2->nAtom,
               i1->nNumberOfAtoms * sizeof(i1->nAtom[0]))) {
      ret |= IDIF_ATOMS;
      goto exit_function;
    }
    ...
    if (memcmp(i1->nNum_H, i2->nNum_H,
      ...
      for (j1 = 0; j1 < i1->nNumberOfAtoms; j1++) {
        if (i1->nNum_H[j1] != i2->nNum_H[j1] &&
            picr->num_diff_pos_H < ICR_MAX_DIFF_FIXED_H) {
          picr->diff_pos_H_at[picr->num_diff_pos_H] = j1;
          picr->diff_pos_H_nH[picr->num_diff_pos_H] =
              i1->nNum_H[j1] - i2->nNum_H[j1];
          picr->num_diff_pos_H++;
        }
      }
    }
        ...
        for (i = j1 = j2 = 0; i < i1->nNumberOfAtoms; i++) {
          if (i1->nNum_H_fixed[i] > i2->nNum_H_fixed[i]) {
            if (j1 < ICR_MAX_DIFF_FIXED_H) {
              ...
              picr->fixed_H_nH1_more[j1] =
                  i1->nNum_H_fixed[i] - i2->nNum_H_fixed[i];
              j1++;
            }
          } else if (i1->nNum_H_fixed[i] < i2->nNum_H_fixed[i]) {
            if (j2 < ICR_MAX_DIFF_FIXED_H) {
              picr->fixed_H_at2_more[j2] = i;
              picr->fixed_H_nH2_more[j2] =
                  i2->nNum_H_fixed[i] - i1->nNum_H_fixed[i];
              j2++;
            }
          }
        }
  ...
  num_H1 = 0;
  num_H2 = 0;
  ret2 = CompareHillFormulasNoH(i1->szHillFormula, i2->szHillFormula, &num_H1,
                                &num_H2);
  ...
  /* output special cases: different number of t-groups, different sizes of
   * t-groups, different endpoints */
  /* in isotopic or deprotonated cases i1->lenTautomer == 1 && i1->nTautomer[0]
   * = 0 */
  /*
      if ( i1->lenTautomer != i2->lenTautomer && (i1->lenTautomer > 1 ||
     i2->lenTautomer > 1) ) { ret |=  IDIF_TAUT_LEN;
      }
  */
  ...
  if (n1 || n2) {
    ...
    AT_NUMB *pe1 =
        (AT_NUMB *)inchi_malloc((i1->lenTautomer + 1) * sizeof(pe1[0]));
    AT_NUMB *pe2 =
        (AT_NUMB *)inchi_malloc((i2->lenTautomer + 1) * sizeof(pe2[0]));
    num_H1 = num_H2 = 0;
    /* collect endpoints, H, (-) */
    if (!pe1 || !pe2) {
      if (pe1)
  ...
  if (i1->nTotalCharge != i2->nTotalCharge)
    ret |= IDIF_CHARGE;
  if (a1 && a1->nNumRemovedProtons &&
      (!a2 || a2->nNumRemovedProtons != a1->nNumRemovedProtons)) {
    ret |= IDIF_REM_PROT;
  }
  if (a1 &&
      (!a2 || a2->nNumRemovedIsotopicH[0] != a1->nNumRemovedIsotopicH[0] ||
  ...
  */
  if (i1->StereoIsotopic && i1->StereoIsotopic->nNumberOfStereoBonds +
                                i1->StereoIsotopic->nNumberOfStereoCenters) {
    Stereo1 = i1->StereoIsotopic;
  } else {
    Stereo1 = i1->Stereo;
  }
  if (i2->StereoIsotopic && i2->StereoIsotopic->nNumberOfStereoBonds +
                                i2->StereoIsotopic->nNumberOfStereoCenters) {
...
/***************************************************************************************/
int Create_INChI(INChI **ppINChI, INChI_Aux **ppINChI_Aux,
                 ORIG_ATOM_DATA *orig_inp_data, /* not used */
                 inp_ATOM *inp_at, INP_ATOM_DATA *out_norm_data[2],
                 int num_inp_at, INCHI_MODE nUserMode, INCHI_MODE *pbTautFlags,
                 INCHI_MODE *pbTautFlagsDone, struct tagInchiTime *ulMaxTime,
                 T_GROUP_INFO *ti_out, char *pStrErrStruct) {
  /*
  #define NON_TAUT 0
  #define TAUT     1
  */
  ...
  int i, n1, n2, num_atoms, num_at_tg, num_removed_H, num_removed_H_taut = 0,
                                                      ret = 0, ret2 = 0;
  INCHI_MODE nMode = 0;
  ...
  CANON_STAT CS, CS2;
  CANON_STAT *pCS = &CS;
  CANON_STAT *pCS2 = &CS2; /*  save all allocations to avoid memory leaks in
                              case Canon_INChI() removes the pointer */
...
/*^^^ */
  ...
  /*norm_at[TAUT_NON] = out_norm_data[TAUT_NON]->at; */ /* output normalized
                                                           non-tautomeric
                                                           component */
  /*norm_at[TAUT_YES] = out_norm_data[TAUT_YES]->at; */ /* output normalized
                                                           tautomeric component
                                                         */
  ...
  out_at = out_norm_data[TAUT_YES]->at ? out_norm_data[TAUT_YES]->at
                                       : out_norm_data[TAUT_NON]->at;
  /* copy the input structure to be normalized to the buffer for the
   * normalization data */
  memcpy(out_at, inp_at, num_inp_at * sizeof(out_at[0]));

  /*  tautomeric groups setting */
  t_group_info->bIgnoreIsotopic =
      0; /*  include tautomeric group isotopic info in MarkTautomerGroups() */
  ...
  /*  ??? Ambiguity: H-D may become HD or DH (that is, H+implicit D or
   * D+implicit H) */
  if (TG_FLAG_H_ALREADY_REMOVED & bTautFlags) {
    INP_ATOM_DATA *out_norm_data1 =
        out_norm_data[TAUT_YES]->at   ? out_norm_data[TAUT_YES]
        : out_norm_data[TAUT_NON]->at ? out_norm_data[TAUT_NON]
                                      : NULL;
    if (out_norm_data1) {
      num_at_tg = num_atoms =
          out_norm_data1->num_at - out_norm_data1->num_removed_H;
    ...
    num_at_tg = num_atoms =
        remove_terminal_HDT(num_inp_at, out_at, bFixTermHChrg);
...
#endif
  /*  duplicate the preprocessed structure so that all supplied
   * out_norm_data[]->at buffers are filled */
  ...
  if (out_norm_data[TAUT_YES]->at_fixed_bonds && out_norm_data[TAUT_YES]->at) {
    memcpy(out_norm_data[TAUT_YES]->at_fixed_bonds, out_at,
           num_inp_at * sizeof(out_at[0]));
  }
  ...
  /*******************************************************************************
   * ??? not true ??? duplicate inp_at and keep inp_at[] unchanged after
   *terminal hydrogens removal set stereo parities in taut_at[], non_taut_at[]
  ...
  if (out_norm_data[TAUT_YES]->at && at[TAUT_YES]) {
    ...
    ret = mark_alt_bonds_and_taut_groups(
        out_norm_data[TAUT_YES]->at, out_norm_data[TAUT_YES]->at_fixed_bonds,
        num_atoms, t_group_info, NULL, NULL);
    if (ret < 0) {
    ...
    ~FLAG_PROTON_SINGLE_REMOVED)) && out_norm_data[TAUT_YES]->at_fixed_bonds) {
         FreeInpAtom( &out_norm_data[TAUT_YES]->at_fixed_bonds );
    }
    */
    /*out_norm_data[TAUT_YES]->num_removed_H = num_removed_H_taut;*/
    out_norm_data[TAUT_YES]->bTautFlags = *pbTautFlags =
        t_group_info->bTautFlags;
    out_norm_data[TAUT_YES]->bTautFlagsDone = *pbTautFlagsDone =
        t_group_info->bTautFlagsDone;
    out_norm_data[TAUT_YES]->bNormalizationFlags =
    ...
    /* set stereo parities to at[]; nUserMode: accept alt. stereo bonds, min
     * ring size */
    ret = set_stereo_parity(out_norm_data[TAUT_YES]->at, at[TAUT_YES],
                            num_taut_at, num_removed_H_taut,
                            &s[TAUT_YES].nMaxNumStereoAtoms,
                            &s[TAUT_YES].nMaxNumStereoBonds, nUserMode,
    ...
    s[TAUT_YES].bMayHaveStereo =
        (s[TAUT_YES].nMaxNumStereoAtoms || s[TAUT_YES].nMaxNumStereoBonds);
    /*
    ...
    s[TAUT_YES].num_isotopic_atoms =
        set_atom_iso_sort_keys(num_taut_at, at[TAUT_YES], t_group_info,
    ...
    s[TAUT_YES].nLenLinearCTTautomer =
        CountTautomerGroups(at[TAUT_YES], num_taut_at, t_group_info);
    if (RETURNED_ERROR(
            s[TAUT_YES]
                .nLenLinearCTTautomer)) { /* added error treatment 9-11-2003 */
      ...
      */
      ...
      /*  ??? -not true- create t_group_info_orig for multiple calls with atom
       * renumbering */
      make_a_copy_of_t_group_info(t_group_info_orig /* dest*/,
                                  t_group_info /* source*/);
      /*  mark isotopic tautomer groups: calculate t_group->iWeight */
      s[TAUT_YES].nLenLinearCTIsotopicTautomer =
          set_tautomer_iso_sort_keys(t_group_info);
  ...
  if (out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_YES]->at &&
      at[TAUT_NON] && !s[TAUT_YES].nLenLinearCTTautomer) {
    /* the structure is non-tautomeric: use tautomeric treatment results only
     * for it */
    inchi_free(at[TAUT_NON]);
    at[TAUT_NON] = NULL;
  } else if (!out_norm_data[TAUT_NON]->at && out_norm_data[TAUT_YES]->at &&
             !at[TAUT_NON] && at[TAUT_YES] &&
             !s[TAUT_YES].nLenLinearCTTautomer) {
    /* requested tautomeric; found non-tautomeric; it is located in
     * out_norm_data[TAUT_YES]->at */
    out_norm_data[TAUT_YES]->bTautomeric = 0;
  } else if (out_norm_data[TAUT_NON]->at && at[TAUT_NON]) {
    /* the structure needs non-tautomeric treatment: final normalization of
     * non-tautomeric structure */
    ret = mark_alt_bonds_and_taut_groups(out_norm_data[TAUT_NON]->at, NULL,
                                         num_atoms, NULL, &bTautFlags,
                                         &bTautFlagsDone);
    if (ret < 0) {
      goto exit_function; /*  out of RAM or other normalization problem */
    }
    ...
    ret = set_stereo_parity(
        ...
        &s[TAUT_NON].nMaxNumStereoAtoms, &s[TAUT_NON].nMaxNumStereoBonds,
    ...
    s[TAUT_NON].bMayHaveStereo =
        (s[TAUT_NON].nMaxNumStereoAtoms || s[TAUT_NON].nMaxNumStereoBonds);
    /*
     * mark isotopic atoms and atoms that have non-tautomeric
     * isotopic terminal hydrogen atoms 1H, 2H(D), 3H(T)
     */
    s[TAUT_NON].num_isotopic_atoms =
        set_atom_iso_sort_keys(num_atoms, at[TAUT_NON], NULL, NULL);
  ...
  bHasIsotopicAtoms = s[TAUT_NON].num_isotopic_atoms > 0 ||
                      s[TAUT_NON].bHasIsotopicTautGroups > 0 ||
                      s[TAUT_YES].num_isotopic_atoms > 0 ||
                      s[TAUT_YES].bHasIsotopicTautGroups > 0;
  /*^^^ */
  if (bFixIsoFixedH) /* 2008-03-21 DT */
    bHasIsotopicAtoms = bHasIsotopicAtoms ||
                        (s[TAUT_YES].nLenLinearCTTautomer > 0 && t_group_info &&
                         ((0 < NUM_H_ISOTOPES &&
                           t_group_info->tni.nNumRemovedProtonsIsotopic[0]) ||
                          (1 < NUM_H_ISOTOPES &&
                           t_group_info->tni.nNumRemovedProtonsIsotopic[1]) ||
                          (2 < NUM_H_ISOTOPES &&
                           t_group_info->tni.nNumRemovedProtonsIsotopic[2])));
  /*^^^ */
  bHasIsotopicAtoms =
      bHasIsotopicAtoms ||
    ...
    if (t_group_info) {
      ...
      if (t_group_info->nIsotopicEndpointAtomNumber) {
        t_group_info->nIsotopicEndpointAtomNumber[0] =
            inchi_min(1, t_group_info->nIsotopicEndpointAtomNumber[0]);
      }
  ...
  ret = GetBaseCanonRanking(num_atoms, num_at_tg, at, t_group_info, s, pBCN,
                            ulMaxTime, bFixIsoFixedH);
#if (TEST_RENUMB_ATOMS == 1)
  ...
  /* Debug only: find whether canonical equivalence is different from equitable
   * partition */
  if (bCanonIsFinerThanEquitablePartition(num_atoms, at[n1],
                                          pBCN->ftcn[TAUT_NON].nSymmRankCt)) {
  ...
  for (i = n2; i >= n1 && !RETURNED_ERROR(ret); i--) {
    ...
    case TAUT_NON: /*  non-tautomeric */
      nMode = 0;
      nMode =
          (s[i].nLenLinearCTTautomer == 0) ? CANON_MODE_CT : CANON_MODE_TAUT;
      nMode |= (bHasIsotopicAtoms && (nUserMode & REQ_MODE_ISO))
                   ? CANON_MODE_ISO
                   : 0;
      nMode |= (s[TAUT_NON].bMayHaveStereo && (nUserMode & REQ_MODE_STEREO))
                   ? CANON_MODE_STEREO
                   : 0;
      nMode |= (bHasIsotopicAtoms && s[TAUT_NON].bMayHaveStereo &&
                (nUserMode & REQ_MODE_ISO_STEREO))
      ...
      if ((nMode & CANON_MODE_STEREO) == CANON_MODE_STEREO ||
        ...
        nMode |=
            (nUserMode & REQ_MODE_RELATIVE_STEREO) ? CMODE_RELATIVE_STEREO : 0;
        nMode |=
            (nUserMode & REQ_MODE_RACEMIC_STEREO) ? CMODE_RACEMIC_STEREO : 0;
        nMode |= (nUserMode & REQ_MODE_SC_IGN_ALL_UU) ? CMODE_SC_IGN_ALL_UU : 0;
        nMode |= (nUserMode & REQ_MODE_SB_IGN_ALL_UU) ? CMODE_SB_IGN_ALL_UU : 0;
      }
      if ((ret = AllocateCS(pCS, num_atoms, num_atoms, s[TAUT_NON].nLenCT,
                            s[TAUT_NON].nLenCTAtOnly,
                            s[TAUT_NON].nLenLinearCTStereoDble,
                            s[TAUT_NON].nMaxNumStereoBonds,
                            s[TAUT_NON].nLenLinearCTStereoCarb,
                            s[TAUT_NON].nMaxNumStereoAtoms, 0, 0,
                            s[TAUT_NON].nLenIsotopic, nMode, pBCN))) {
        goto exit_function;
      }
      *pCS2 = *pCS;
      break;
    case TAUT_YES: /*  tautomeric */
      ...
      nMode |= (bHasIsotopicAtoms && (nUserMode & REQ_MODE_ISO))
                   ? CANON_MODE_ISO
                   : 0;
      nMode |= (s[TAUT_YES].bMayHaveStereo && (nUserMode & REQ_MODE_STEREO))
                   ? CANON_MODE_STEREO
                   : 0;
      nMode |= (bHasIsotopicAtoms && s[TAUT_YES].bMayHaveStereo &&
                (nUserMode & REQ_MODE_ISO_STEREO))
                   ? CANON_MODE_ISO_STEREO
                   : 0;
      nMode |= (nUserMode & REQ_MODE_NOEQ_STEREO) ? CMODE_NOEQ_STEREO : 0;
      ...
      if ((ret = AllocateCS(
               pCS, num_atoms, num_at_tg, s[TAUT_YES].nLenCT,
               s[TAUT_YES].nLenCTAtOnly, s[TAUT_YES].nLenLinearCTStereoDble,
               s[TAUT_YES].nMaxNumStereoBonds,
               s[TAUT_YES].nLenLinearCTStereoCarb,
               s[TAUT_YES].nMaxNumStereoAtoms, s[TAUT_YES].nLenLinearCTTautomer,
               s[TAUT_YES].nLenLinearCTIsotopicTautomer,
    ...
    /*^^^ 2009-12-05 */
    nMode |=
        (nUserMode & REQ_MODE_DIFF_UU_STEREO) ? REQ_MODE_DIFF_UU_STEREO : 0;
    /*^^^ 2009-12-05 */

    /*  settings */
    pCS->lNumDecreasedCT = -1;
    pCS->bDoubleBondSquare =
        DOUBLE_BOND_NEIGH_LIST ? 2 : 0; /*  2 => special mode */
    pCS->bIgnoreIsotopic =
        !((s[TAUT_NON].num_isotopic_atoms || s[TAUT_YES].num_isotopic_atoms ||
           s[TAUT_YES].bHasIsotopicTautGroups) ||
          (nUserMode & REQ_MODE_NON_ISO) || !(nUserMode & REQ_MODE_ISO));
    ...
    if (pBCN) {
      ...
      pCS->NeighList = NULL;
      pCS->pBCN = pBCN;
      ret = Canon_INChI(num_atoms, i ? num_at_tg : num_atoms, at[i], pCS, nMode,
                        i);
    } else {
      /* old way */
      pCS->NeighList =
          CreateNeighList(num_atoms, i ? num_at_tg : num_atoms, at[i],
    ...
    if (ret <= 0) {
      ...
      ret =
          FillOutINChI(pINChI, pINChI_Aux, num_atoms, i ? num_at_tg : num_atoms,
                       i ? num_removed_H_taut : num_removed_H, at[i],
                       out_norm_data[i]->at, pCS, i, nUserMode, pStrErrStruct);
      if (RETURNED_ERROR(ret)) {
        ...
        /****************************/
#if (bRELEASE_VERSION == 0)
        if (pINChI->Stereo && (pINChI->Stereo->nCompInv2Abs &&
                               !pINChI->Stereo->bTrivialInv) ||
            pINChI->StereoIsotopic && (pINChI->StereoIsotopic->nCompInv2Abs &&
                                       !pINChI->StereoIsotopic->bTrivialInv)) {
          bExtract |= EXTR_NON_TRIVIAL_STEREO;
        }
#endif
        /* mark non-tautomeric representation as having another, tautomeric
         * representation */
        ...
        ret2 = CheckCanonNumberingCorrectness(
            num_atoms, i ? num_at_tg : num_atoms, at[i], pCS, i, pStrErrStruct);
        if (ret2) {
...
int GetAtomOrdNbrInCanonOrd(inp_ATOM *norm_at, AT_NUMB *nAtomOrdNbr,
  ...
  /*  get nCanonNbr[]: canon. numbers-1 in order of increasing original atom
   * numbers */
  pn_RankForSort = nOrigAtNosInCanonOrd;
  qsort(nCanonNbr, num_at, sizeof(nCanonNbr[0]), CompRank);
  /*  get nOrigAtNosOrd[]: norm_atom ord. numbers the same order of increasing
   * original atom numbers */
  pn_RankForSort = nOrigAtNos;
  qsort(nOrigAtNosOrd, num_at, sizeof(nOrigAtNosOrd[0]), CompRank);
  /*  check whether the 2 sets of origiginal atom numbers have identical
   * elements */
  for (i = 0; i < num_at; i++) {
...
int FillOutCanonInfAtom(inp_ATOM *norm_at, INF_ATOM_DATA *inf_norm_at_data,
  ...
  int next_atom[MAX_CUMULENE_LEN + 1], best_next_atom[MAX_CUMULENE_LEN + 1],
      cur_atom;
  int next_neigh[MAX_CUMULENE_LEN + 1], best_next_neigh[MAX_CUMULENE_LEN + 1],
      best_len;
  ...
  int nNumberOfTGroups =
      (pINChI->lenTautomer && pINChI->nTautomer && pINChI->nTautomer[0])
          ? (int)pINChI->nTautomer[0]
          : 0;
  ...
  int (*MakeNumber)(char *, int, const char *, int) =
      bAbcNumbers ? MakeAbcNumber : MakeDecNumber;
  ...
  MakeRemovedProtonsString(pINChI_Aux->nNumRemovedProtons,
                           pINChI_Aux->nNumRemovedIsotopicH, NULL, bIsotopic,
                           inf_norm_at_data->szRemovedProtons,
                           &inf_norm_at_data->num_removed_iso_H);
  /* fill out info atom */
  if (bIsotopic &&
      !(pINChI->nNumberOfIsotopicAtoms || pINChI->nNumberOfIsotopicTGroups ||
        pINChI->nPossibleLocationsOfIsotopicH &&
            pINChI->nPossibleLocationsOfIsotopicH[0] > 1))
    bIsotopic = 0;
  ...
  if (bDoDisplaySp3 && bRelRac && Stereo->nNumberOfStereoCenters < 2 &&
    ...
    if (Stereo->nCompInv2Abs) {
      inf_norm_at_data->StereoFlags |= bRel   ? INF_STEREO_REL
                                       : bRac ? INF_STEREO_RAC
                                              : 0;
    }
  }
  ...
  */
  if (bDoDisplaySp3 && Stereo->nCompInv2Abs) {
    ...
    if (bRelRac) {
      inf_norm_at_data->StereoFlags |=
          (Stereo->nCompInv2Abs > 0) ? INF_STEREO_NORM : INF_STEREO_INV;
    }
  }
  ...
  nConstitEquTGroupNumbers = bIsotopic
  ...
  nNormAtNosInCanonOrd =
      (AT_NUMB *)inchi_calloc(num_at, sizeof(nNormAtNosInCanonOrd[0]));
  if (ret = GetAtomOrdNbrInCanonOrd(norm_at, nNormAtNosInCanonOrd,
                                    nOrigAtNosInCanonOrd, num_at)) {
    goto exit_function;
  }
  ...
  if (nNumberOfTGroups) {
    ...
    for (g = 1, i = 1; g <= nNumberOfTGroups; g++) {
      n = (int)pINChI->nTautomer[i] -
          INCHI_T_NUM_MOVABLE; /*  number of atoms in t-group */
      e = nConstitEquTGroupNumbers[(int)g - 1];
      /*  bypass number of hydrogen atoms, negative charges, ... */
      for (i += INCHI_T_NUM_MOVABLE + 1, j = 0;
           j < n && i < pINChI->lenTautomer; j++, i++) {
  ...
  if (bIsotopic && pINChI->nPossibleLocationsOfIsotopicH &&
      (n = (int)pINChI->nPossibleLocationsOfIsotopicH[0])) {
    for (i = 1; i < n; i++) {
  ...
  /*  Write isotopic mass, chemical element symbols and hydrogens, charge,
   * radical, canon. numbers */
  len_str = sizeof(inf_norm_at[0].at_string);
  for (i = 0; i < init_num_at; i++) {
    str = inf_norm_at[i].at_string;
    len = 0;
    bIncludeIsotopicH = bIsotopic && !inf_norm_at[i].nTautGroupCanonNbr &&
                        !(inf_norm_at[i].cFlags & AT_FLAG_ISO_H_POINT);
    /*  isotopic mass */
    atw = 0;
    if (norm_at[i].iso_atw_diff && bIsotopic) {
      if (norm_at[i].at_type == ATT_PROTON) {
        ; /* do not set isotopic mass of a tautomeric proton */
      } else if (norm_at[i].el_number == PERIODIC_NUMBER_H &&
                 norm_at[i].chem_bonds_valence == 1 && !norm_at[i].charge &&
                 !norm_at[i].radical && !norm_at[i].num_H &&
                 (inf_norm_at[j = (int)norm_at[i].neighbor[0]]
                      .nTautGroupCanonNbr ||
                  (inf_norm_at[j].cFlags & AT_FLAG_ISO_H_POINT))) {
        ; /* do not set isotopic mass of an exchangeable proton */
      } else {
        atw = get_atw(norm_at[i].elname);
        atw += (norm_at[i].iso_atw_diff > 0) ? norm_at[i].iso_atw_diff - 1
                                             : norm_at[i].iso_atw_diff;
    ...
    /*  hydrogens */
    /*  find number of previuosly removed terminal hydrogen atoms because these
     * terminal H will be displayed */
    for (j = 0; j < NUM_H_ISOTOPES; j++) {
    ...
    if (norm_at[i].el_number == PERIODIC_NUMBER_H && str[0] == str[1]) {
      ...
      } else if (isdigit(UCINT str[2]) && (n = strtol(str + 2, &q, 10)) &&
                 !q[0]) {
  ...
  /*  stereogenic centers */
  if (bDoDisplaySp3 && Stereo &&
      0 < (num_stereo = Stereo->nNumberOfStereoCenters)) {
    for (i = 0; i < num_stereo; i++) {
      ...
      if (len + 3 < (int)sizeof(inf_norm_at[0].at_string)) {
        ...
        if (norm_at[j].bAmbiguousStereo && (c == '+' || c == '-' || c == '?') &&
            str[0] != '!' && len + 1 < (int)sizeof(inf_norm_at[0].at_string)) {
  ...
  if (Stereo && 0 < (num_stereo = Stereo->nNumberOfStereoBonds)) {
    for (i = 0; i < num_stereo; i++) {
      ...
      if (norm_at[j].bAmbiguousStereo && (c == '+' || c == '-') &&
          (len = strlen(str = inf_norm_at[j].at_string) + 1) <
              (int)sizeof(inf_norm_at[0].at_string) &&
          str[0] != '!') {
      ...
      if (norm_at[k].bAmbiguousStereo && (c == '+' || c == '-') &&
          (len = strlen(str = inf_norm_at[k].at_string) + 1) <
              (int)sizeof(inf_norm_at[0].at_string) &&
          str[0] != '!') {
        memmove(str + 1, str, len);
        str[0] = '!'; /* output the atom in red color */
      ...
      best_len =
          MAX_CUMULENE_LEN + 1; /* moved here from inside the cycle 1-8-2003 */
      for (n = 0; n < norm_at[j].valence; n++) {
        ...
        next_neigh[len] = n;
        cur_atom = j;
        while (len < MAX_CUMULENE_LEN && next_atom[len] != k &&
               2 == norm_at[next_atom[len]].valence) {
          next_neigh[len + 1] =
              ((int)norm_at[next_atom[len]].neighbor[0] == cur_atom);
          next_atom[len + 1] =
              (int)norm_at[next_atom[len]].neighbor[next_neigh[len + 1]];
      ...
      if (best_len <= MAX_CUMULENE_LEN && best_next_atom[best_len] == k) {
        ...
        if (best_len % 2) {
          ...
          if (best_len == 0) {
            ...
            k = best_next_neigh[best_len +
                                1]; /*  added +1 to display cumulene parity on
                                       the middle bond (6-24-2002) */
          }
          /*  mark "forward" bond */
          for (m = 0;
               m < MAX_STEREO_BONDS && inf_norm_at[j].cStereoBondParity[m]; m++)
            ;
          ...
          for (k = 0;
               k < norm_at[n].valence && j != (int)norm_at[n].neighbor[k]; k++)
            ;
          if (k < norm_at[n].valence) {
            j = n;
            for (m = 0;
                 m < MAX_STEREO_BONDS && inf_norm_at[j].cStereoBondParity[m];
                 m++)
              ;
            if (m < MAX_STEREO_BONDS) {
              inf_norm_at[j].cStereoBondParity[m] = c;
  ...
  for (i = 0; i < init_num_at; i++) {
    ...
    if (inf_norm_at[i].nCanonNbr) {
      str = inf_norm_at[i].at_string;
      len = strlen(str);
      len += (*MakeNumber)(str + len, len_str - len, "/",
                           (int)inf_norm_at[i].nCanonNbr);
      if (inf_norm_at[i].nCanonEquNbr || inf_norm_at[i].nTautGroupCanonNbr ||
          (inf_norm_at[i].cFlags & AT_FLAG_ISO_H_POINT)) {
        if (inf_norm_at[i].nCanonEquNbr) {
          len += (*MakeNumber)(str + len, len_str - len, "/",
                               (int)inf_norm_at[i].nCanonEquNbr);
        } else if (len + 1 < len_str) {
          len += 1;
          strcat(str, "/");
        }
      }
      ...
      if (inf_norm_at[i].nTautGroupCanonNbr) {
        len += (*MakeNumber)(str + len, len_str - len, "/",
                             (int)inf_norm_at[i].nTautGroupCanonNbr);
        if (inf_norm_at[i].nTautGroupEquNbr) {
          len += (*MakeNumber)(str + len, len_str - len, "/",
                               (int)inf_norm_at[i].nTautGroupEquNbr);
        }
      }
...
int FillOutOneCanonInfAtom(inp_ATOM *inp_norm_at,
  ...
  inf_ATOM *inf_norm_at = (inf_norm_at_data && inf_norm_at_data->at)
                              ? inf_norm_at_data->at + offset
                              : NULL;
  inp_ATOM *norm_at = inp_norm_at ? inp_norm_at + offset : NULL;
  inf_ATOM *inf_norm_at_H = (inf_norm_at_data && inf_norm_at_data->at)
                                ? inf_norm_at_data->at + offset_H
                                : NULL;
  ...
  if ( bIsotopic ) {
      for ( i = 0, j = 0; i < NUM_H_ISOTOPES; i ++ ) {
          if ( pINChI_Aux->nNumRemovedIsotopicH[i] ) {
              inf_norm_at_data->num_iso_H[i] +=
  pINChI_Aux->nNumRemovedIsotopicH[i]; inf_norm_at_data->num_removed_iso_H ++;
          }
      }
  }
  */
  ...
  /*
  if ( bDoDisplaySp3 && bRelRac && Stereo->nCompInv2Abs &&
  Stereo->nNumberOfStereoCenters < 2 ) { bDoDisplaySp3 = 0;
  }
  */
  if (bDoDisplaySp3 && Stereo->nCompInv2Abs) {
    /* inversion changes stereo */
  ...
  len_str = sizeof(inf_norm_at[0].at_string);
  for (i = 0; i < init_num_at; i++) {
    inf_ATOM *cur_inf_norm_at =
        (i < num_at) ? inf_norm_at + i : inf_norm_at_H + i - num_at;
    inp_ATOM *cur_norm_at = (i < num_at) ? norm_at + i : norm_at_H + i - num_at;
    str = cur_inf_norm_at->at_string;
    len = 0;
    bIncludeIsotopicH =
        bIsotopic &&
        (i >= num_at || !inf_norm_at[i].nTautGroupCanonNbr &&
                            !(inf_norm_at[i].cFlags & AT_FLAG_ISO_H_POINT));
    /*  isotopic mass */
    atw = 0;
    if (cur_norm_at->iso_atw_diff && bIsotopic) {
      if (cur_norm_at->at_type == ATT_PROTON) {
        ; /* do not set isotopic mass of a tautomeric proton */
      } else if (num_at <= i && cur_norm_at->el_number == PERIODIC_NUMBER_H &&
                 cur_norm_at->chem_bonds_valence == 1 && !cur_norm_at->charge &&
                 !cur_norm_at->radical && !cur_norm_at->num_H &&
                 0 <= (j = (int)cur_norm_at->neighbor[0] - offset) &&
                 j < num_at &&
                 (inf_norm_at[j].nTautGroupCanonNbr ||
                  (inf_norm_at[j].cFlags & AT_FLAG_ISO_H_POINT))) {
        ; /* do not set isotopic mass of an exchangeable proton */
      } else {
        atw = get_atw(cur_norm_at->elname);
        atw += (cur_norm_at->iso_atw_diff > 0) ? cur_norm_at->iso_atw_diff - 1
                                               : cur_norm_at->iso_atw_diff;
        /*len += sprintf( str+len, "^%d", atw );*/
      }
    }
    /*  element name */
    ...
    else if (abs(cur_norm_at->charge) == 1)
      len += sprintf(str + len, "%s", cur_norm_at->charge > 0 ? "+" : "-");
    /*  radical */
    if (cur_norm_at->radical)
      len += sprintf(str + len, "%s",
                     cur_norm_at->radical == RADICAL_SINGLET   ? ":"
    ...
    for (i = 0; i < num_stereo; i++) {
      ...
      for (n = 0; n < norm_at[j].valence; n++) {
        ...
        while (len < MAX_CUMULENE_LEN && next_atom[len] != k &&
               2 == norm_at[next_atom[len]].valence) {
          next_neigh[len + 1] =
              ((int)norm_at[next_atom[len]].neighbor[0] - offset == cur_atom);
          next_atom[len + 1] =
              (int)norm_at[next_atom[len]].neighbor[next_neigh[len + 1]] -
              offset;
          cur_atom = next_atom[len];
      ...
      if (best_len <= MAX_CUMULENE_LEN && best_next_atom[best_len] == k) {
        ...
        if (best_len % 2) {
          ...
          for (k = 0; k < norm_at[n].valence &&
                      j != (int)norm_at[n].neighbor[k] - offset;
               k++)
            ;
          if (k < norm_at[n].valence) {
            j = n;
...
int FillOutInputInfAtom(inp_ATOM *inp_at, INF_ATOM_DATA *inf_at_data,
  ...
  MakeRemovedProtonsString(nNumRemovedProtons, nNumRemovedProtonsIsotopic, NULL,
                           bIsotopic, inf_at_data->szRemovedProtons, NULL);
  /*  atom canonical and equivalence numbers > 0 */
  for (i = 0; i < num_at; i++) {
  ...
  for (i = 0; i < init_num_at; i++) {
    ...
    if (inp_at[i].iso_atw_diff && bIsotopic) {
      ...
      atw += (inp_at[i].iso_atw_diff > 0) ? inp_at[i].iso_atw_diff - 1
                                          : inp_at[i].iso_atw_diff;
      /*len += sprintf( str+len, "^%d", atw );*/
    }
    /*  element name */
    ...
    if (n > 1) {
      ...
      len +=
          sprintf(str + len, "H"); /* fixed 12-21-2002: removed 3rd argument */
    }
    if (bIsotopic) {
      /*  isotopic hydrogen atoms */
      for (j = 0; j < NUM_H_ISOTOPES; j++) {
    ...
    */
    ...
    else if (abs(inp_at[i].charge) == 1)
      len += sprintf(str + len, "%s", inp_at[i].charge > 0 ? "+" : "-");
    /*  radical */
    if (inp_at[i].radical)
      len += sprintf(str + len, "%s",
                     inp_at[i].radical == RADICAL_SINGLET   ? ":"
...
/**********************************************************************************************/
int FillOutInfAtom(inp_ATOM *norm_at, INF_ATOM_DATA *inf_norm_at_data,
                   int init_num_at, int num_removed_H, int bAdd_DT_to_num_H,
                   int nNumRemovedProtons, NUM_H *nNumRemovedProtonsIsotopic,
                   int bIsotopic, INChI *pINChI, INChI_Aux *pINChI_Aux,
                   int bAbcNumbers, INCHI_MODE nMode) {
  if (norm_at && inf_norm_at_data && inf_norm_at_data->at) {
    if (pINChI && pINChI_Aux) {
      return FillOutCanonInfAtom(norm_at, inf_norm_at_data, init_num_at,
                                 bIsotopic, pINChI, pINChI_Aux, bAbcNumbers,
                                 nMode);
    } else {
      return FillOutInputInfAtom(norm_at, inf_norm_at_data, init_num_at,
                                 num_removed_H, bAdd_DT_to_num_H,
...
int FillOutCompositeCanonInfAtom(COMP_ATOM_DATA *composite_norm_data,
                                 INF_ATOM_DATA *inf_norm_at_data, int bIsotopic,
                                 int bTautomeric, PINChI2 *pINChI2,
  ...
  if (composite_norm_data && inf_norm_at_data &&
      (bTautomeric == TAUT_INI || pINChI2 && pINChI_Aux2)) {
    ...
    if (bTautomeric == TAUT_INI) {
      ret = FillOutInputInfAtom(
          composite_norm_data->at, inf_norm_at_data,
          composite_norm_data->num_at, composite_norm_data->num_removed_H,
          0 /*bAdd_DT_to_num_H*/, composite_norm_data->nNumRemovedProtons,
          composite_norm_data->nNumRemovedProtonsIsotopic, bIsotopic,
          bAbcNumbers);
      return ret;
    } else {
      for (i = 0; i < num_components; i++) {
        ...
        inf_norm_at_data->nNumRemovedProtons +=
        ...
        if (bIsotopic && bTautomeric == TAUT_YES) {
          for (k = 0; k < NUM_H_ISOTOPES; k++) {
            if (pINChI_Aux2[i][j]->nNumRemovedIsotopicH[k]) {
              inf_norm_at_data->num_iso_H[k] +=
                  pINChI_Aux2[i][j]->nNumRemovedIsotopicH[k];
              inf_norm_at_data->num_removed_iso_H +=
                  pINChI_Aux2[i][j]->nNumRemovedIsotopicH[k];
            }
          }
        ...
        if (num_at != pINChI->nNumberOfAtoms ||
            num_at != pINChI_Aux->nNumberOfAtoms) {
          return 0; /* error */
        }
        ret = FillOutOneCanonInfAtom(inp_norm_at, inf_norm_at_data,
                                     inf_norm_at_data->pStereoFlags + i + 1,
                                     num_inp_at, offset, offset_H, bIsotopic,
    ...
    MakeRemovedProtonsString(inf_norm_at_data->nNumRemovedProtons,
                             inf_norm_at_data->num_iso_H, NULL, bIsotopic,
                             inf_norm_at_data->szRemovedProtons,
                             &inf_norm_at_data->num_removed_iso_H);
  }
  return 1;
...
int CheckCanonNumberingCorrectness(int num_atoms, int num_at_tg, sp_ATOM *at,
  ...
  pCanonRankAtoms =
      (AT_NUMB *)inchi_calloc(num_at_tg + 1, sizeof(pCanonRankAtoms[0]));
  ...
  if (pCanonOrd && pCanonRank) {
    ...
    ret = UpdateFullLinearCT(num_atoms, num_at_tg, at, pCanonRank, pCanonOrd,
                             pCS, 0);
  ...
  if (pCanonOrd && pCanonRank) {
    ...
    if ( ret /*|| memcmp(pCS->LinearCT, pCS->LinearCT2, sizeof(AT_RANK) * pCS->nLenLinearCT )*/ ) {
      nErrorCode |=
          (pCS->nLenCanonOrdIsotopicStereo ? WARN_FAILED_ISOTOPIC_STEREO
