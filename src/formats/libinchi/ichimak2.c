...
int GetHillFormulaCounts(U_CHAR *nAtom, S_CHAR *nNum_H, int num_atoms,
                         AT_NUMB *nTautomer, int lenTautomer, int *pnum_C,
                         int *pnum_H, int *pnLen, int *pnNumNonHAtoms);
int MakeHillFormula(U_CHAR *nAtom, int num_atoms, char *szLinearCT,
                    int nLen_szLinearCT, int num_C, int num_H, int *bOverflow);
...
int AddElementAndCount(const char *szElement, int mult, char *szLinearCT,
                       int nLenLinearCT, int *bOverflow);

int Copy2StereoBondOrAllene(INChI_Stereo *Stereo, int *nNumberOfStereoCenters,
                            int *nNumberOfStereoBonds,
                            AT_STEREO_DBLE *LinearCTStereoDble,
                            AT_NUMB *pCanonOrd, AT_RANK *pCanonRank,
                            sp_ATOM *at, int bIsotopic);
...
/**********************************************************************************************/
int MakeHillFormulaString(char *szHillFormula, char *szLinearCT,
                          int nLen_szLinearCT, int *bOverflow) {
  int nLen;
...
/**********************************************************************************************
 *     MS Windows dependent: sprintf() is supposed to return the length of the
 *output string Carbon atoms are always first Bridging hydrogen atoms are always
 *last
 **********************************************************************************************/
int GetHillFormulaIndexLength(int count) {
  char szCount[16];
...
int GetHillFormulaCounts(U_CHAR *nAtom, S_CHAR *nNum_H, int num_atoms,
  ...
  if (nTautomer && lenTautomer > 0) {
    ...
    for (i = 1; i < lenTautomer && num_groups > 0;
         i += nTautomer[i] + 1, num_groups--) {
      num_H += nTautomer[i + 1];
    }
  }
...
int AddElementAndCount(const char *szElement, int mult, char *szLinearCT,
  ...
  if (mult > 0 && !*bOverflow && 0 < (len1 = strlen(szElement))) {
    ...
    if (len1 + len2 < nLenLinearCT) {
      ...
      memcpy(szLinearCT + len1, szMult,
             len2 + 1); /*  adding zero termination */
      return len1 + len2;
    } else {
      (*bOverflow)++;
    }
...
int MakeHillFormula(U_CHAR *nAtom, int num_atoms, char *szLinearCT,
  ...
  if (num_C) {
    nLen += AddElementAndCount("C", num_C, szLinearCT + nLen,
                               nLen_szLinearCT - nLen, &bOvfl);
    if (num_H) {
      nLen += AddElementAndCount("H", num_H, szLinearCT + nLen,
                                 nLen_szLinearCT - nLen, &bOvfl);
      num_H = 0;
    }
  }
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (nPrevAtom != nAtom[i]) {
      if (mult) {
        nLen += AddElementAndCount(szElement, mult, szLinearCT + nLen,
                                   nLen_szLinearCT - nLen, &bOvfl);
      }
      mult = 1;
      if (GetElementFormulaFromAtNum((int)nAtom[i], szElement)) {
...
char *AllocateAndFillHillFormula(INChI *pINChI) {
  ...
  bOverflow = 0;
  if (!GetHillFormulaCounts(pINChI->nAtom, pINChI->nNum_H,
                            pINChI->nNumberOfAtoms, pINChI->nTautomer,
                            pINChI->lenTautomer, &num_C, &num_H, &nLen,
                            &nNumNonHAtoms)) {
...
int Copy2StereoBondOrAllene(INChI_Stereo *Stereo, int *nNumberOfStereoCenters,
                            ...
                            AT_NUMB *pCanonOrd, AT_RANK *pCanonRank,
                            sp_ATOM *at, int bIsotopic) {
  int cumulene_len, j, next_j /* ordering number of the central allene atom */,
      next_neigh;
  AT_RANK at_num;
  int parity;
  if (pCanonOrd && pCanonRank) {
    ...
    if (bIsotopic) {
      ...
      if (cumulene_len % 2 &&
          (1 >= MAX_NUM_STEREO_BONDS || !at[j].stereo_bond_neighbor2[1])) {
        next_j = at[j].neighbor[(int)at[j].stereo_bond_ord2[0]];
        for (cumulene_len = (cumulene_len - 1) / 2;
             cumulene_len && 2 == at[next_j].valence; cumulene_len--) {
      ...
      if (cumulene_len % 2 &&
          (1 >= MAX_NUM_STEREO_BONDS || !at[j].stereo_bond_neighbor[1])) {
    ...
    if (!cumulene_len) {
      ...
      for (j = 0; j < *nNumberOfStereoCenters && Stereo->nNumber[j] < at_num;
           j++)
        ;
      if (j < *nNumberOfStereoCenters) {
        memmove(nNumber + j + 1, nNumber + j,
                (*nNumberOfStereoCenters - j) * sizeof(nNumber[0]));
        memmove(t_parity + j + 1, t_parity + j,
                (*nNumberOfStereoCenters - j) * sizeof(t_parity[0]));
      }
...
int CopyLinearCTStereoToINChIStereo(
  ...
  for (i = len = 0; i < n; i++) {
    bAllene = Copy2StereoBondOrAllene(Stereo, &Stereo->nNumberOfStereoCenters,
                                      &len, LinearCTStereoDble + i, pCanonOrd,
                                      pCanonRank, at, bIsotopic);
    bAlleneInv = Copy2StereoBondOrAllene(
        Stereo, &lenInv, NULL, LinearCTStereoDbleInv + i, pCanonOrdInv,
        pCanonRankInv, at, bIsotopic);
    /* make sure double bond stereo is identical in original and inverted
     * geometry */
    /* Note: all allenes are AFTER double bonds in LinearCTStereoDble... */
    if (bAllene != bAlleneInv ||
        (!bAllene && CompareLinCtStereoDble(LinearCTStereoDble + i, 1,
  ...
  if (diff == -1 || diff == 1) {
    ...
    for (i = 0, diff = 0; i < n; i++) {
      ...
      /*  parities can be only 1, 2, 3, 4. Therefore only mutually inverted
       * pairs (t_parityInv, t_parity) = (1,2) or (2,1) statisfy conditions
       *  (t_parityInv != t_parity) && (t_parityInv + t_parity == 3)
       */
      if (Stereo->t_parityInv[i] == Stereo->t_parity[i] ||
          Stereo->t_parityInv[i] + Stereo->t_parity[i] != 3) {
        diff = 1; /* parities are same or different and cannot be obtained by
                     simple inversion */
        break;
      }
    }
...
int MarkAmbiguousStereo(sp_ATOM *at, inp_ATOM *norm_at, int bIsotopic,
  ...
  for (i = 0; i < n; i++) {
    /*  mark ambiguous stereo centers (for displaying and "Ambiguous stereo"
     * message) */
    if (ATOM_PARITY_NOT_UNKN(LinearCTStereoCarb[i].parity) &&
        at[j1 = pCanonOrd[(int)LinearCTStereoCarb[i].at_num - 1]]
            .bAmbiguousStereo) {
  ...
  for (i = 0; i < n; i++) {
    /*  mark ambiguous stereo bonds or allenes (for displaying and "Ambiguous
     * stereo" message) */
    if (ATOM_PARITY_WELL_DEF(LinearCTStereoDble[i].parity)) {
      ...
      if (at[j1].bAmbiguousStereo || at[j2].bAmbiguousStereo) {
        ...
        */
        ...
        int cumulene_len = BOND_CHAIN_LEN(
            j1_parity); /* 0 => double bond, 1 => allene, 2 => cumulene,..*/
        if (cumulene_len % 2 &&
            (1 >= MAX_NUM_STEREO_BONDS ||
...
INCHI_MODE UnmarkAllUndefinedUnknownStereo(INChI_Stereo *Stereo,
  ...
  INCHI_MODE nRet = 0;
  int i, n;
  if (!Stereo || (Stereo && !Stereo->nNumberOfStereoCenters &&
                  !Stereo->nNumberOfStereoBonds)) {
    return nRet;
  }

  /* stereocenters */
  if (!Stereo->nCompInv2Abs && (n = Stereo->nNumberOfStereoCenters) > 0 &&
      (nUserMode & REQ_MODE_SC_IGN_ALL_UU)) {
  ...
  /* stereobonds */
  if ((n = Stereo->nNumberOfStereoBonds) > 0 &&
      (nUserMode & REQ_MODE_SB_IGN_ALL_UU)) {
...
*/
/**********************************************************************************************/
int FillOutINChI(INChI *pINChI, INChI_Aux *pINChI_Aux, int num_atoms,
