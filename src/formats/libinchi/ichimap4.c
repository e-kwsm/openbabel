...
/************************************************
   ...
   the following members of CANON_STAT *pCS:
        ...
        pCS->bKeepSymmRank  // ??? almost unused, replaced with nSymmStereo !=
 NULL ??? pCS->bFirstCT pCS->bStereoIsBetter pCS->lNumNeighListIter
        pCS->lNumBreakTies
        pCS->lNumRejectedCT
        pCS->lNumTotCT
        pCS->lNumEqualCT
...
int map_stereo_bonds4(
    ...
    const AT_RANK *nSymmRank, AT_RANK **pRankStack1 /*from*/,
    AT_RANK **pRankStack2 /*to*/, AT_RANK *nTempRank, int nNumMappedRanksInput,
    AT_RANK *nSymmStereo, NEIGH_LIST *NeighList, CANON_STAT *pCS,
    CUR_TREE *cur_tree, int nNumMappedBonds, int vABParityUnknown) {
  int nTotSuccess =
      0; /* 1=>full mapping has been completed;
...
total_restart:
  ...
  if (!nNumMappedBonds) {
    ...
    memset(pCS->bRankUsedForStereo, 0,
           sizeof(pCS->bRankUsedForStereo[0]) * num_atoms);
    ...
    if (pCS->bFirstCT && nSymmStereo && !pCS->bKeepSymmRank) {
      ...
      for (i = 0; i < num_at_tg; i++) {
        ...
        nSymmStereo[i] =
            i; /*  for union-join to keep track of stereo-equivalent atoms */
      }
    }
  }
  ...
  if (nNumMappedBonds < pCS->nLenLinearCTStereoDble) {
    ...
    /* AT_RANK *nRankFrom=*pRankStack1++,  AT_RANK
     * *nAtomNumberFrom=pRankStack1++; */
    /* AT_RANK *nRankTo  =*pRankStack2++,  AT_RANK *nAtomNumberTo
     * =pRankStack2++; */
    ...
    /*  = pCS->LinearCTStereoDble[nNumMappedBonds].at_num1; */ /*  canonical
                                                                  numbers of
                                                                  atoms */
    AT_RANK at_rank_canon2;
    /*  = pCS->LinearCTStereoDble[nNumMappedBonds].at_num2; */ /*  adjacent
                                                                  to the
                                                                  stereogenic
                                                                  bond */
    int nNumChoices, nNumUnkn, nNumUndf, nNumBest, nNumWorse, nNumCalc,
        sb_parity_calc;
    int stereo_bond_parity, prev_stereo_bond_parity, pass,
        bAllParitiesIdentical, bAllParitiesIdentical2;
    ...
    /*
            // find candidates for atom_from1, atom_to1; they must have
       identical mapping ranks
            at_rank1=pRankStack1[0][at_from1=nAtomNumberCanonFrom[(int)at_rank_canon1
       - 1]]; // rank "from" for mapping
  ...
  bypass_next_canon_ranks_check:
     ...
     */
    /*  process allenes AFTER all double bonds and odd-number-of-double-bonds
     * cumulenes */
    if (!(ret1 = Next_SB_At_CanonRanks2(
              &at_rank_canon1, &at_rank_canon2, /*  canonical numbers */
              &canon_min1, &canon_min2, &bFirstCanonRank,
              pCS->bAtomUsedForStereo, pRankStack1, pRankStack2, nCanonRankFrom,
              nAtomNumberCanonFrom, at, num_atoms, bAllene))) {
      ...
      if (!bAllene && bFirstCanonRank) {
        ...
        if ((ret1 = Next_SB_At_CanonRanks2(
                 &at_rank_canon1_Allene, &at_rank_canon2_Allene,
                 &canon_min1_Allene, &canon_min2_Allene, &bFirstCanonRank,
                 pCS->bAtomUsedForStereo, pRankStack1, pRankStack2,
                 nCanonRankFrom, nAtomNumberCanonFrom, at, num_atoms, 1))) {
    ...
    if (!ret1 ||
        (!pCS->bStereoIsBetter &&
         (at_rank_canon1 > pCS->LinearCTStereoDble[nNumMappedBonds].at_num1 ||
          (at_rank_canon1 == pCS->LinearCTStereoDble[nNumMappedBonds].at_num1 &&
           at_rank_canon2 >
               pCS->LinearCTStereoDble[nNumMappedBonds].at_num2)))) {
      /* new ranks provide greater pCS->LinearCTStereoDble[nNumMappedBonds] and
       * therefore rejected */
      if (!nTotSuccess) {
        pCS->LinearCTStereoDble[nNumMappedBonds] =
            prevBond; /* restore stereo bond CT for the current bond */
      }
      return nTotSuccess;
    }
    ...
    at_rank1 =
        pRankStack1[0]
                   [at_from1 = nAtomNumberCanonFrom[(int)at_rank_canon1 -
                                                    1]]; /* atom 1 rank "from"
                                                            for mapping */
    at_rank2 =
        pRankStack1[0]
                   [at_from2 = nAtomNumberCanonFrom[(int)at_rank_canon2 -
                                                    1]]; /* atom 2 rank "from"
                                                            for mapping */
     ...
     */
    iMax = at_rank1 - 1;
    /*  test correctness: sorted pRankStack2[0][] and pRankStack1[0][] should
     * have same ranks for both atoms */
    ...
    */
    ...
    /*  find out if we have a choice in mapping: check all possible pairs
       (at_to1, at_to2) such that at_from1 is possibly constitutionally
       equivalent to at_to1, at_from2 to at_to2 */
    for (j1 = 0; j1 <= iMax &&
                 at_rank1 == pRankStack2[0][at_to1 = pRankStack2[1][iMax - j1]];
         j1++) {
      if (!at[at_to1].stereo_bond_neighbor[0])
        continue; /*  at_to1 does not belong to a stereo bond */
      for (j2 = 0; j2 < MAX_NUM_STEREO_BONDS &&
                   (at_to2 = at[at_to1].stereo_bond_neighbor[j2]);
           j2++) {
        at_to2--;
  ...
  repeat_all:
    ...
    if (!nNumMappedBonds)
      pCS->bStereoIsBetter = 0; /*  the first stereo feature in the canonical
                                   CT; moved here 7-13-2002 */
    ...
    if (!pass++) {
      ...
      if (sb_parity_calc) {
        ...
        stereo_bond_parity = nNumBest    ? BEST_PARITY
                             ...
                             : nNumUndf  ? AB_PARITY_UNDF
                                         : AB_PARITY_NONE;
      }
    } else {
      /* second pass: since the first pass failed, search for a worse result */
      prev_stereo_bond_parity = stereo_bond_parity;
      i = NextStereoParity2Test(&stereo_bond_parity, &sb_parity_calc, nNumBest,
                                nNumWorse, nNumUnkn, nNumUndf, nNumCalc,
                                vABParityUnknown);
      switch (i) {
      case 0:
    ...
    if (!pCS->bStereoIsBetter) {
      c = CompareLinCtStereoDoubleToValues(
          nTotSuccess ? pCS->LinearCTStereoDble + nNumMappedBonds : &prevBond,
    ...
    for (j1 = 0; j1 <= iMax &&
      ...
      if (tpos1 < CurTreeGetPos(cur_tree) &&
        ...
        /* at_to1 is known to be stereogenically equivalent to another atom
         * tried with at_rank_canon1 */
        continue;
      }
      bAllParitiesIdentical2 = 0;
      for (j2 = 0; j2 < MAX_NUM_STEREO_BONDS &&
        ...
        at_to2--;
        if (pRankStack1[0][at_from2] != pRankStack2[0][at_to2])
          continue; /*  at_from2 cannot be mapped on at_to2 even without mapping
                       at_from1 to at_to1 */

        /*  check whether the bond parity corresponds to the requested bond
         * parity */
        if (PARITY_KNOWN(at[at_to1].stereo_bond_parity[j2])) {
          ...
          if (stereo_bond_parity !=
              PARITY_VAL(at[at_to1].stereo_bond_parity[j2])) {
        ...
        bAddStack = 0;
        bAllParitiesIdentical =
            ((at[at_to1].stereo_bond_parity[j2] & KNOWN_PARITIES_EQL)) &&
            PARITY_KNOWN(at[at_to1].stereo_bond_parity[j2]);

        if (!bAllParitiesIdentical && !nNumCalc &&
            (!nNumUndf + !nNumUnkn + !nNumBest + !nNumWorse) == 3) {
          /* only one kind of bond parity is present; check whether all parities
           * are really same */
          bAllParitiesIdentical = All_SB_Same(
              at_rank_canon1, at_rank_canon2, /*  canonical numbers */
              pRankStack1, pRankStack2, nAtomNumberCanonFrom, at);
        ...
        if (!bAllParitiesIdentical) {
          ...
          if (LastMappedTo1 != at_to1) {
            /*  avoid repetitve mapping to the same first at_to1 using
             * LastMappedTo1 variable */
            /*  map atom 1 */
            ret1 =
                map_an_atom2(num_at_tg, num_max, at_from1, at_to1, nTempRank,
                             nNumMappedRanks[istk], &nNumMappedRanks[istk + 1],
                             pCS, NeighList, pRankStack1 + nStackPtr[istk],
                             pRankStack2 + nStackPtr[istk], &bAddStack);
          ...
          if (pRankStack1[nStackPtr[istk]][at_from2] !=
              pRankStack2[nStackPtr[istk]][at_to2]) {
            /*
             ...
             * Such an alternating bond cannot be mapped on possibly stereogenic
             * bond that has neighbors belonging to 1 of the symmetrically
             * attached rings only. For example: A---B---C---D  If all atoms are
             * Carbons then B, C, F, G are constitutionally
             *  ||  ||  ||  ||  equivalent. However, bonds B-C, F-G are not
             * equivalent to
             *  ||  ||  ||  ||  B-F and C-G and cannot be mapped on them.
             *   E---F---G---H  If at_from1=B, at_from2=F, at_to1=B, then
             * at_from2 cannot be mapped on at_to2=C If at_from1=B, at_from2=F,
             * at_to1=C, then at_from2 cannot be mapped on at_to2=B etc.
             */
            if (sb_parity_calc != stereo_bond_parity) {
              ...
              /* can be passed 2 times: for BEST_PARITY and WORSE_PARITY in this
               * order */
          ...
          if (nStackPtr[istk] > nStackPtr[istk - 1]) {
            bAllParitiesIdentical2 = All_SB_Same(
                at_rank_canon1, at_rank_canon2, pRankStack1 + nStackPtr[istk],
                pRankStack2 + nStackPtr[istk], nAtomNumberCanonFrom, at);
          ...
          if (bAllParitiesIdentical2) {
            ...
            /*  map atom 2 */
            ret2 =
                map_an_atom2(num_at_tg, num_max, at_from2, at_to2, nTempRank,
                             nNumMappedRanks[istk], &nNumMappedRanks[istk + 1],
                             pCS, NeighList, pRankStack1 + nStackPtr[istk],
                             pRankStack2 + nStackPtr[istk], &bAddStack);
        ...
        if (stereo_bond_parity != sb_parity_calc) /*  parity is known */
        {
          ...
          /*  same code as under " make a decision to accept current mapping"
           * comment below */
          /*  with one exception: istk instead of istk3 */
          c = CompareLinCtStereoDoubleToValues(
              pCS->LinearCTStereoDble + nNumMappedBonds, at_rank_canon1,
              at_rank_canon2, (U_CHAR)bond_parity);
          if (c < 0 && !pCS->bStereoIsBetter) {
            ...
            ret2 = map_stereo_bonds4(
                at, num_atoms, num_at_tg, num_max, bAllene, nCanonRankFrom,
                nAtomNumberCanonFrom, nCanonRankTo, nSymmRank,
                pRankStack1 + nStackPtr[istk], pRankStack2 + nStackPtr[istk],
                nTempRank, nNumMappedRanks[istk], nSymmStereo, NeighList, pCS,
                cur_tree, nNumMappedBonds + 1, vABParityUnknown);
            if (!bAllParitiesIdentical) {
              pCS->bAtomUsedForStereo[at_to1]++;
              pCS->bAtomUsedForStereo[at_to2]++;
            }
         ...
         * check if at_from1/at_to1 half-bond has neighbors with equal mapping
         *ranks
         */

        parity1 = parity_of_mapped_half_bond(
            at_from1, at_to1, at_from2, at_to2, at, &EN1[0], nCanonRankFrom,
            pRankStack1[nStackPtr[istk]], pRankStack2[nStackPtr[istk]]);
        ...
        num1 = parity1 > 0 ? 1 : 2; /*  parity < 0 means additional mapping is
                                       needed to set parity */
        /*  --- try all possible mappings of the stereo bond ending atoms'
         * neighbors --- */
        at_rank_canon_n1 = 0;
        at_rank_canon_n2 = 0;
        for (i = 0; i < num1; i++) {
          ...
          if (num1 == 2) {
            ...
            /*  we need to map only one at_from1 neighbor to make all neighbors
            ...
            if ( nSymmStereo && !pCS->bFirstCT ) {
                if ( i && nSymmStereo[at_to_n1] ==
            nSymmStereo[(int)EN1[0].to_at[0]] ) { continue; // do not test
            stereo equivalent atoms except the first one
                }
            }
            */
            ...
            ret1 = map_an_atom2(num_at_tg, num_max, at_from_n1, at_to_n1,
                                nTempRank, nNumMappedRanks[istk2],
                                &nNumMappedRanks[istk2 + 1], pCS, NeighList,
                                pRankStack1 + nStackPtr[istk2],
                                pRankStack2 + nStackPtr[istk2], &bAddStack);
            if (RETURNED_ERROR(ret1)) {
            ...
            /*  now that all at_from1 neighbors have been mapped the parity must
             * be defined */
            parity1 = parity_of_mapped_half_bond(
                at_from1, at_to1, at_from2, at_to2, at, &EN1[1], nCanonRankFrom,
                pRankStack1[nStackPtr[istk2]], pRankStack2[nStackPtr[istk2]]);
            if (parity1 <= 0)
              return CT_STEREOCOUNT_ERR; /*  program error */ /*   <BRKPT> */
          } else {
          ...
          /*  check if at_from2/at_to2 half-bond has neighbors with equal
           * mapping ranks */
          parity2 = parity_of_mapped_half_bond(
              at_from2, at_to2, at_from1, at_to1, at, &EN2[0], nCanonRankFrom,
              pRankStack1[nStackPtr[istk2]], pRankStack2[nStackPtr[istk2]]);
          if (!parity2) {
            return CT_STEREOCOUNT_ERR; /*  program error */ /*   <BRKPT> */
          }
          ...
          for (j = 0; j < num2; j++) {
            ...
            if (num2 == 2) {
              ...
              /*  we need to map only one at_from2 neighbor to make its
               * neighbors have different ranks */
              ...
              /*
              if ( nSymmStereo && !pCS->bFirstCT ) {
                  if ( j && nSymmStereo[at_to_n2] ==
              nSymmStereo[(int)EN2[0].to_at[0]] ) { continue; // do not test
              stereo equivalent atoms except the first one
                  }
              }
              */
              /*  neighbors are tied. Untie them by breaking a tie on ONE of
               * them. */
              ret1 = map_an_atom2(num_at_tg, num_max, at_from_n2, at_to_n2,
                                  nTempRank, nNumMappedRanks[istk3],
                                  &nNumMappedRanks[istk3 + 1], pCS, NeighList,
                                  pRankStack1 + nStackPtr[istk3],
                                  pRankStack2 + nStackPtr[istk3], &bAddStack);
              if (RETURNED_ERROR(ret1)) {
              ...
              parity2 = parity_of_mapped_half_bond(
                  at_from2, at_to2, at_from1, at_to1, at, &EN2[1],
            ...
            if (ATOM_PARITY_WELL_DEF(bond_parity) &&
                at[at_to1].stereo_bond_z_prod[j2] < 0)
              bond_parity =
                  2 - (bond_parity + 1) % 2; /*  invert the bond parity */
            ...
            if (sb_parity_calc != bond_parity ||
              ...
              ret2 = map_stereo_bonds4(
                  ...
                  nAtomNumberCanonFrom, nCanonRankTo, nSymmRank,
                  pRankStack1 + nStackPtr[istk3],
                  pRankStack2 + nStackPtr[istk3], nTempRank,
                  nNumMappedRanks[istk3], nSymmStereo, NeighList, pCS, cur_tree,
                  nNumMappedBonds + 1, vABParityUnknown);
              pCS->bRankUsedForStereo[at_from1]--;
              pCS->bRankUsedForStereo[at_from2]--;
              pCS->bAtomUsedForStereo[at_to1]++;
              pCS->bAtomUsedForStereo[at_to2]++;
              ...
              if (ret2 > 0) {
                ...
                nNumAtTo1Success++;
                if (bStereoIsBetterWasSetHere || (ret2 & 2)) {
                  CurTreeKeepLastAtomsOnly(cur_tree, tpos1,
                                           1); /*  start over */
                  nTotSuccess |= 2;            /*  Obtained a smaller CT */
                }
    ...
    if (!nTotSuccess || stereo_bond_parity == sb_parity_calc) {
      goto repeat_all; /*  repeat with next parity if no success or with the
                          same parity, now known */
    }
    ...
    if (!nTotSuccess) {
      pCS->LinearCTStereoDble[nNumMappedBonds] = prevBond;
      CurTreeSetPos(cur_tree, tpos1);
      /*  Occurs when atoms are not really equvalent ( -O= without positive
       * charge in "aromatic" ring) */
      return 0; /* Happens for ID=92439,100318,100319 when
                 * EXCL_ALL_AROM_BOND_PARITY=0 and nNumChoices=0. Results from
                 * impossible previous mapping of symmetric relatively to a
                 * central ring aromatic circles while central ring is not
                 * symmetrical due to alternate bonds (in the central ring
                 * number of pi-electrons, atoms and bonds are symmetrical).
                 * Does not happen when alternate bonds of the central ring
                 * are treated as aromatic by attaching a (+) charge to the
                 * oxygen.
                 */
    }
  } else

  {
    ...
    if (!nNumMappedBonds) {
      pCS->bStereoIsBetter = 0; /*  the first stereo feature in the canonical CT
     ...
     */
    ret = map_stereo_atoms4(at, num_atoms, num_at_tg, num_max, nCanonRankFrom,
                            nAtomNumberCanonFrom, nCanonRankTo, nSymmRank,
                            pRankStack1, pRankStack2, nTempRank,
                            nNumMappedRanksInput, nSymmStereo, NeighList, pCS,
                            cur_tree, 0, vABParityUnknown);
    if (ret == 4) {
      if (nNumMappedBonds) {
        return ret;
      } else {
...
int map_stereo_atoms4(
    ...
    AT_RANK **pRankStack2 /*to*/, AT_RANK *nTempRank, int nNumMappedRanksInput,
    AT_RANK *nSymmStereo, NEIGH_LIST *NeighList, CANON_STAT *pCS,
    CUR_TREE *cur_tree, int nNumMappedAtoms, int vABParityUnknown) {
  /*
  ...
  if (nNumMappedAtoms < pCS->nLenLinearCTStereoCarb) {
    ...
    /* AT_RANK *nRankTo  =*pRankStack2++,  AT_RANK *nAtomNumberTo
     * =pRankStack2++; */
    int j1, at_from1, at_to1, /*at_from2, at_to2,*/ iMax, lvl,
        bStereoIsBetterWasSetHere;
    int istk, istk2, bAddStack, nNumAtTo1Success, c, bFirstTime = 1,
                                                     bAllParitiesIdentical;
    EQ_NEIGH EN[5], *pEN;
    ...
    prevAtom = pCS->LinearCTStereoCarb[nNumMappedAtoms]; /*  save to restore in
                                                            case of failure */
    at_rank_canon1 = nNumMappedAtoms
                         ? pCS->LinearCTStereoCarb[nNumMappedAtoms - 1].at_num
                         : 0;
  ...
  next_canon_rank:
    ...
    if (!pCS->bStereoIsBetter /*??? && !pCS->bFirstCT ???*/ &&
      ...
      if (!nTotSuccess) {
        pCS->LinearCTStereoCarb[nNumMappedAtoms] =
            prevAtom; /*  restore because of failure */
      }
      CurTreeSetPos(cur_tree, tpos1);
      return nTotSuccess;
    }
  ...
  bypass_next_canon_rank_check:
    ...
    if (!Next_SC_At_CanonRank2(&at_rank_canon1, &canon_rank1_min, &bFirstTime,
                               pCS->bAtomUsedForStereo, pRankStack1,
                               pRankStack2, nAtomNumberCanonFrom, num_atoms) ||
        (!pCS->bStereoIsBetter &&
         at_rank_canon1 > pCS->LinearCTStereoCarb[nNumMappedAtoms].at_num)) {
      /*  cannot find next available canonical number */
      if (!nTotSuccess) {
    ...
    at_rank1 =
        pRankStack1[0]
                   [at_from1 = (int)nAtomNumberCanonFrom[at_rank_canon1 - 1]];
    iMax = at_rank1 - 1;
    /*  for debug only */
    if (at_rank1 != pRankStack2[0][pRankStack2[1][at_rank1 - 1]])
      return CT_STEREOCOUNT_ERR; /*  program error */ /*   <BRKPT> */
  ...
  repeat_all:
    ...
    if (!pass++) {
      ...
      j1 = NextStereoParity2Test(&stereo_center_parity, &sb_parity_calc,
                                 nNumBest, nNumWorse, nNumUnkn, nNumUndf,
                                 nNumCalc, vABParityUnknown);
      switch (j1) {
      case 0:
        break; /* obtained next parity to test */
      case 1:
    ...
    if (!pCS->bStereoIsBetter) {
      c = CompareLinCtStereoAtomToValues(
          nTotSuccess ? pCS->LinearCTStereoCarb + nNumMappedAtoms : &prevAtom,
    ...
    */                                      /*  nRankTo                 nAtomNumberTo */
    for (j1 = 0; j1 <= iMax &&
      ...
      nNumAtTo1Success = 0;
      /*
      if ( !(at[at_to1].stereo_atom_parity &&
      !at[at_to1].stereo_bond_neighbor[0] && pCS->bAtomUsedForStereo[at_to1] ==
      STEREO_AT_MARK ) )
      */
      if (!at[at_to1].stereo_atom_parity ||
          at[at_to1].stereo_bond_neighbor[0] ||
          pCS->bAtomUsedForStereo[at_to1] !=
              STEREO_AT_MARK) /* simplify 12-17-2003 */
        continue;
       ...
       */
      /*  check whether the stereocenter parity corresponds to the requested
       * stereocenter parity */
      ...
      bAllParitiesIdentical =
          ((at[at_to1].stereo_atom_parity & KNOWN_PARITIES_EQL) &&
           PARITY_KNOWN(at[at_to1].stereo_atom_parity));

      if (!bAllParitiesIdentical && !nNumCalc &&
          (!nNumUndf + !nNumUnkn + !nNumBest + !nNumWorse) == 3) {
        /* only one kind of stereocenter parity is present; check whether all
         * parities are really same */
        bAllParitiesIdentical =
            All_SC_Same(at_rank_canon1, /*  canonical number */
                        pRankStack1, pRankStack2, nAtomNumberCanonFrom, at);
        if (bAllParitiesIdentical < 0) {
      ...
      bAddStack = 0;
      /*  if all equivalent atoms have same known parity, do not map any of them
       * here */
      if (!bAllParitiesIdentical) {
        /*  map the central atom */
        /*  this mapping is always possible */
        ret1 = map_an_atom2(num_at_tg, num_max, at_from1, at_to1, nTempRank,
                            nMappedRanks[istk], &nMappedRanks[istk + 1], pCS,
                            NeighList, pRankStack1 + nStackPtr[istk],
                            pRankStack2 + nStackPtr[istk], &bAddStack);
        if (RETURNED_ERROR(ret1)) {
          return ret1; /*  error */
        }
      ...
      /*********************************************************************************
       *
       *   Unknown Stereocenter Parity case: possibly need to map stereo center
       * neighbors
       */
      if (stereo_center_parity == sb_parity_calc) {
        /*  find out the parity */
        parity1 = parity_of_mapped_atom2(
            at_from1, at_to1, at, &EN[istk], nCanonRankFrom,
            pRankStack1[nStackPtr[istk]], pRankStack2[nStackPtr[istk]]);
      ...
      /*  known in advance parity = stereo_center_parity */
      {
        ...
        c = CompareLinCtStereoAtomToValues(pCS->LinearCTStereoCarb +
                                               nNumMappedAtoms,
        ...
        if (c < 0 && !pCS->bStereoIsBetter) {
          ...
          ret = map_stereo_atoms4(
              at, num_atoms, num_at_tg, num_max, nCanonRankFrom,
              nAtomNumberCanonFrom, nCanonRankTo, nSymmRank,
              pRankStack1 + nStackPtr[istk], pRankStack2 + nStackPtr[istk],
              nTempRank, nMappedRanks[istk], nSymmStereo, NeighList, pCS,
              cur_tree, nNumMappedAtoms + 1, vABParityUnknown);
        ...
        if ( (at[at_to1].stereo_atom_parity & KNOWN_PARITIES_EQL ) &&
             ATOM_PARITY_KNOWN(stereo_center_parity) && !nSymmStereo ) { // ???
        add && !nSymmStereo ??? break; // do not repeat for the same kind of
        stereo atom with the parity known in advance
        }
        */
        if (bAllParitiesIdentical) {
          break; /*  do not repeat for the same kind of stereo atom with the
                    parity known in advance */
        }
        continue;
      }
    ...
    next_lvl:
      if (pEN[lvl].num_to) {
         ...
         * which the pEN[lvl].from_at "from" neighbor's canonical number is
         * mapped
         */
        j[lvl] = 0;
      next_j:
        ...
        if (j[lvl] && tpos1 < CurTreeGetPos(cur_tree) &&
          ...
          lvl++;
          bLastLvlFailed = 0;
          goto backup; /*  do not test stereo equivalent atoms except the first
                          one */
        }

        ret2 = map_an_atom2(num_at_tg, num_max, pEN[lvl].from_at, /* from */
                            pEN[lvl].to_at[j[lvl]],               /* to */
                            nTempRank, nMR[lvl], &nMR[lvl + 1], pCS, NeighList,
                            pRankStack1 + nSP[lvl], pRankStack2 + nSP[lvl],
                            &bAddStack);
        if (RETURNED_ERROR(ret2)) {
          return ret2; /*  program error */
        }
        ...
        parity1 = parity_of_mapped_atom2(at_from1, at_to1, at, &pEN[lvl],
                                         nCanonRankFrom, pRankStack1[nSP[lvl]],
                                         pRankStack2[nSP[lvl]]);
        if (!parity1) {
          return CT_STEREOCOUNT_ERR; /*  1/25/2002 */ /*   <BRKPT> */
        }
        if (parity1 < 0) {
          goto next_lvl; /*  we need at least one more mapping to define the
                            parity */
        }
        ...
        c = CompareLinCtStereoAtomToValues(pCS->LinearCTStereoCarb +
                                               nNumMappedAtoms,
                                           at_rank_canon1, (U_CHAR)parity1);
        if (sb_parity_calc != parity1 || (c < 0 && !pCS->bStereoIsBetter)) {
          pCS->lNumRejectedCT++;
          bLastLvlFailed = 1;
        } else
        ...
        /*  if ( bAcceptAllParities || parity1 == BEST_PARITY ) */
        {
          ...
          ret = map_stereo_atoms4(
              at, num_atoms, num_at_tg, num_max, nCanonRankFrom,
              nAtomNumberCanonFrom, nCanonRankTo, nSymmRank,
              pRankStack1 + nSP[lvl], pRankStack2 + nSP[lvl], nTempRank,
              nMR[lvl], nSymmStereo, NeighList, pCS, cur_tree,
              nNumMappedAtoms + 1, vABParityUnknown);
          pCS->bRankUsedForStereo[at_from1] = 0;
          pCS->bAtomUsedForStereo[at_to1] += STEREO_AT_MARK;
          if (ret == 4) {
            return ret;
          ...
          /*  check if neighbors mappings have altered another stereo center
           * parity */
          if (!nSymmStereo &&
              !might_change_other_atom_parity(
                  ...
                  pRankStack2
                      [nStackPtr
                           [istk]] /* ranks before the mapping neighbors */)) {
            goto done;
          }
        }
    ...
    if (pCS->bStereoIsBetter || pCS->bFirstCT) {
      /* All stereo atoms have been mapped. Current stereo name is better than
       * all previous. Create new numbering for the new CT break all remaining
       * "from" ties
       */
      int i1, ret;
      AT_RANK rc, n1, n2;
      ret = BreakAllTies(num_at_tg, num_max, pRankStack1, NeighList, nTempRank,
                         pCS);
      if (RETURNED_ERROR(ret)) {
        return ret;
      }
      /*  break all remaining "from" ties */
      ret = BreakAllTies(num_at_tg, num_max, pRankStack2, NeighList, nTempRank,
                         pCS);
      if (RETURNED_ERROR(ret)) {
        return ret;
      }
      ...
      pRankStack1 += 2;
      pRankStack2 += 2;
      /* Now final mapping ranks of "to" atom (*pRankStack2)[i] and "from" atom
       * (*pRankStack1)[i] are equal and all ranks are different, that is, we
       * have a full mapping Copy so far best canonical numbering from "from" to
       * "to".
       */
      memset(pCS->nPrevAtomNumber, 0,
             num_at_tg * sizeof(pCS->nPrevAtomNumber[0]));
      for (i1 = 0; i1 < num_at_tg; i1++) {
        ...
        pCS->nPrevAtomNumber[rc - 1] =
            n2; /*  ord. number of the atom having canon. rank = rc */
      ...
      if (!(pCS->nMode & CMODE_REDNDNT_STEREO)) {
        i1 = RemoveCalculatedNonStereo(
            at, num_atoms, num_at_tg, pRankStack1, pRankStack2, nTempRank,
            NeighList, nSymmRank, nCanonRankTo, pCS->nPrevAtomNumber, pCS,
            vABParityUnknown);
      ...
      /*  current stereo name is same as previous. We do not need a full
       * mapping. */
      if (nSymmStereo) {
        int num_changes = 0;
        ...
        for (r = 1; r <= r_max; r++) {
          if (bUniqueAtNbrFromMappingRank(pRankStack1, r, &n1)) {
            if (bUniqueAtNbrFromMappingRank(pRankStack2, r, &n2)) {
              ...
              cr = nCanonRankFrom[(int)n1] -
                   1; /*  (new at[n2] canonical rank)-1 */
              /*  pCS->nPrevAtomNumber[(int)cr] = */
              /*    previous ordering number of an atom with the canon. rank =
               * cr+1; */
              /*    make this atom equivalent to atom at[n2]: */
              num_changes +=
                  nJoin2Mcrs(nSymmStereo, pCS->nPrevAtomNumber[(int)cr], n2);
            } else {
              return CT_MAPCOUNT_ERR;
              /*  mapping ranks must be either both tied or untied. */ /*   <BRKPT>
                                                                        */
            }
          }
        }
