 ...
 * the terms of the IUPAC/InChI Trust Licence for the International Chemical
 * Identifier (InChI) Software version 1.0; either version 1.0 of the License,
 * or (at your option) any later version.
 *
 * This library is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
 * See the IUPAC/InChI Trust Licence for the International Chemical Identifier
 * (InChI) Software version 1.0 for more details.
 *
...
#define BNS_MARK_ONLY_BLOCKS                                                   \
  1 /* 1 => find only blocks, do not search for ring systems */
#define ALLOW_ONLY_SIMPLE_ALT_PATH                                             \
  0 /* 0 => allow alt. path to contain same bond 2 times (in opposite          \
       directions) */
...
#define FIX_CPOINT_BOND_CAP                                                    \
  1 /* 1=> fix bug in case of double bond from neutral cpoint */
#define RESET_EDGE_FORBIDDEN_MASK                                              \
  1 /* 1: previous; 0: do not apply "edge->forbidden &=                        \
       pBNS->edge_forbidden_mask" */
#if (RESET_EDGE_FORBIDDEN_MASK == 1)
#define IS_FORBIDDEN(EDGE_FORBIDDEN, PBNS) (EDGE_FORBIDDEN)
#else
#define IS_FORBIDDEN(EDGE_FORBIDDEN, PBNS)                                     \
  (EDGE_FORBIDDEN & PBNS->edge_forbidden_mask)
#endif

typedef enum tagAtTypeTotals {
  /* counts do not include:
     charged atom adjacent to another charged atom
     atom in an unusual valence state or adjacent to an atom in an unusual
     valence state radicals different from singlet
  */
  /*ATTOT_NUM_Plus. */ /* number of positive charges, +1, is (ATTOT_NUM_CHARGES
                          + ATTOT_TOT_CHARGE)/2 */
  /*ATTOT_NUM_Minus.*/ /* number of negative charges, -1, is (ATTOT_NUM_CHARGES
                          - ATTOT_TOT_CHARGE)/2 */
  ATTOT_NUM_NP_Plus, /*  0 no H: =N(+)=, #N(+)-, =N(+)<, does not include onium
                        cations >P(+)<, >N(+)< */
  ATTOT_NUM_NP_Proton, /*  1 H(+): -NH3(+), =NH2(+), >NH2(+), =NH(+)-, >NH(+)-,
                          #NH(+), N=N,P */
  ...
  ATTOT_TOT_CHARGE,  /* 31 total of positive and negative single charges, +1 and
                        -1 */
  ATTOT_NUM_CHARGES, /* 32 number of positive and negative single charges, +1
                        and -1 */
...
#define ATBIT_MSK_NP                                                           \
  (ATBIT_NP_Plus | ATBIT_NP_Proton | ATBIT_NP_H | ATBIT_N_Minus | ATBIT_NP)
#define KNOWN_ACIDIC_TYPE                                                      \
  (ATT_ACIDIC_CO | ATT_ACIDIC_S | ATT_OO | ATT_ZOO | ATT_NO)
#define ATBIT_MSK_OS                                                           \
  (ATBIT_COH | ATBIT_CSH | ATBIT_ZOH | ATBIT_OOH | ATBIT_ZOOH | ATBIT_NOH |    \
   ATBIT_N_OH | ATBIT_CO | ATBIT_ZO | ATBIT_NO | ATBIT_N_O | ATBIT_CO_Minus |  \
   ATBIT_CS_Minus | ATBIT_ZO_Minus | ATBIT_OO_Minus | ATBIT_ZOO_Minus |        \
   ATBIT_NO_Minus | ATBIT_N_O_Minus /*| ATBIT_O_Minus*/)
#define ATBIT_MSK_H                                                            \
  (ATBIT_NP_Proton | ATBIT_NP_H | ATBIT_COH | ATBIT_CSH | ATBIT_ZOH |          \
   ATBIT_OOH | ATBIT_ZOOH | ATBIT_NOH | ATBIT_N_OH)

#define ATTYP_OS                                                               \
  (ATT_ACIDIC_CO | ATT_ACIDIC_S | ATT_OO | ATT_ZOO |                           \
   ATT_NO /*| ATT_OTHER_NEG_O*/ | ATT_OTHER_ZO)
...
#define AR_SIMPLE_MSK1                                                         \
  (ATBIT_COH | ATBIT_CSH | ATBIT_OOH | ATBIT_ZOOH | ATBIT_NOH | ATBIT_HalAcid)
#define AR_SIMPLE_TYP1                                                         \
  (ATT_ACIDIC_CO | ATT_ACIDIC_S | ATT_OO | ATT_ZOO | ATT_NO | ATT_HalAcid)
...
#define AA_SIMPLE_MSK1                                                         \
  (ATBIT_CO_Minus | ATBIT_CS_Minus | ATBIT_OO_Minus | ATBIT_ZOO_Minus |        \
   ATBIT_NO_Minus | ATBIT_O_Minus | ATBIT_HalAnion)
#define AA_SIMPLE_TYP1                                                         \
  (ATT_ACIDIC_CO | ATT_ACIDIC_S | ATT_OO | ATT_ZOO | ATT_NO | ATT_OH_MINUS |   \
   ATT_HalAnion)
...
int bRadChangesAtomType(BN_STRUCT *pBNS, BN_DATA *pBD, Vertex v, Vertex v_1,
                        Vertex v_2);
int BnsAdjustFlowBondsRad(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at,
                          int num_atoms);
int SetAtomRadAndChemValFromVertexCapFlow(BN_STRUCT *pBNS, inp_ATOM *atom,
                                          int v1);
...
int SetAtomBondType(BNS_EDGE *edge, U_CHAR *bond_type12, U_CHAR *bond_type21,
...
/* Vertex Get2ndNeighbor1( BN_STRUCT* pBNS, Vertex u, EdgeIndex iedge ); not
 * used */
Vertex Get2ndEdgeVertex(BN_STRUCT *pBNS, Edge uv);
Vertex GetVertexNeighbor(BN_STRUCT *pBNS, Vertex v, int neigh,
                         EdgeIndex *iedge);
int GetEdgePointer(BN_STRUCT *pBNS, Vertex u, Vertex v, EdgeIndex iuv,
                   BNS_EDGE **uv, S_CHAR *s_or_t);
int AugmentEdge(BN_STRUCT *pBNS, Vertex u, Vertex v, EdgeIndex iuv, int delta,
                S_CHAR bReverse, int bChangeFlow);
...
int FindPathToVertex_s(Vertex x, Edge *SwitchEdge, Vertex *BasePtr,
                       Vertex *Path, int MaxPathLen);
Vertex MakeBlossom(BN_STRUCT *pBNS, Vertex *ScanQ, int *pQSize, Vertex *Pu,
                   Vertex *Pv, int max_len_Pu_Pv, Edge *SwitchEdge,
                   Vertex *BasePtr, Vertex u, Vertex v, EdgeIndex iuv,
                   Vertex b_u, Vertex b_v, S_CHAR *Tree);
int PullFlow(BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y, int delta,
             S_CHAR bReverse, int bChangeFlow);
int FindPathCap(BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y,
                int delta);

/*
int SetBondType( BNS_EDGE *edge, U_CHAR *bond_type12, U_CHAR *bond_type21, int
delta, int bChangeFlow ); int SetBondsRestoreBnStructFlow( BN_STRUCT *pBNS,
inp_ATOM *at, int num_atoms, int bChangeFlow );
*/
int SetBondsFromBnStructFlow(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                             int bChangeFlow0);
int MarkAtomsAtTautGroups(BN_STRUCT *pBNS, int num_atoms, BN_AATG *pAATG,
                          int nEnd1, int nEnd2);
...
/* Bonds testing */
/*
int bRestoreFlowToCheckOneBond( BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd, int
nTestFlow, inp_ATOM *at, int num_atoms, int bChangeFlow );
*/
int bSetFlowToCheckOneBond(BN_STRUCT *pBNS, int iedge, int flow,
                           BNS_FLOW_CHANGES *fcd);
int bRestoreFlowAfterCheckOneBond(BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd);
int bSetBondsAfterCheckOneBond(BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd,
                               int nTestFlow, inp_ATOM *at, int num_atoms,
                               int bChangeFlow);
int BnsTestAndMarkAltBonds(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at,
                           int num_atoms, BNS_FLOW_CHANGES *fcd,
...
/* Alt path testing */
int bSetBnsToCheckAltPath(BN_STRUCT *pBNS, int nVertDoubleBond,
                          int nVertSingleBond, AT_NUMB type, int path_type,
                          ALT_PATH_CHANGES *apc, BNS_FLOW_CHANGES *fcd,
                          int *nDots);
int bRestoreBnsAfterCheckAltPath(BN_STRUCT *pBNS, ALT_PATH_CHANGES *apc,
                                 int bChangeFlow);
...
int bAddNewVertex(BN_STRUCT *pBNS, int nVertDoubleBond, int nCap, int nFlow,
                  int nMaxAdjEdges, int *nDots);
int AddNewEdge(BNS_VERTEX *p1, BNS_VERTEX *p2, BN_STRUCT *pBNS, int nEdgeCap,
               int nEdgeFlow);
int bAddStCapToAVertex(BN_STRUCT *pBNS, Vertex v1, Vertex v2,
                       VertexFlow *nOldCapVertSingleBond, int *nDots,
                       int bAdjacentDonors);
...
int GetAtomChargeType(inp_ATOM *atom, int at_no, int nAtTypeTotals[],
                      int *pMask, int bSubtract);
int AddChangedAtHChargeBNS(inp_ATOM *at, int num_atoms, int nAtTypeTotals[],
                           S_CHAR *mark);
int EliminatePlusMinusChargeAmbiguity(BN_STRUCT *pBNS, int num_atoms);
int AddOrRemoveExplOrImplH(int nDelta, inp_ATOM *at, int num_atoms,
                           AT_NUMB at_no, T_GROUP_INFO *t_group_info);
int SubtractOrChangeAtHChargeBNS(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                                 int nAtTypeTotals[], S_CHAR *mark,
                                 T_GROUP_INFO *t_group_info, int bSubtract);
int is_Z_atom(U_CHAR el_number);
int IsZOX(inp_ATOM *atom, int at_x, int ord);
int SimpleRemoveHplusNPO(inp_ATOM *at, int num_atoms, int nAtTypeTotals[],
                         T_GROUP_INFO *t_group_info);
int CreateCGroupInBnStruct(inp_ATOM *at, int num_atoms, BN_STRUCT *pBNS,
                           int nType, int nMask, int nCharge);
int CreateTGroupInBnStruct(inp_ATOM *at, int num_atoms, BN_STRUCT *pBNS,
                           int nType, int nMask);
int RemoveLastGroupFromBnStruct(inp_ATOM *at, int num_atoms, int tg,
                                BN_STRUCT *pBNS);
int SetInitCapFlowToCurrent(BN_STRUCT *pBNS);
int SimpleRemoveAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
                              int num2remove);
int SimpleAddAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
                           int num2add);
int HardRemoveAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
                            int num2remove, int *nNumCanceledCharges,
                            BN_STRUCT *pBNS, BN_DATA *pBD);
int HardAddAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
                         int num2add, int *nNumCanceledCharges, BN_STRUCT *pBNS,
                         BN_DATA *pBD);
int HardRemoveHplusNP(inp_ATOM *at, int num_atoms, int bCancelChargesAlways,
                      int *nNumCanceledCharges, BN_AATG *pAATG, BN_STRUCT *pBNS,
                      BN_DATA *pBD);
int RemoveNPProtonsAndAcidCharges(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
                                  BN_STRUCT *pBNS, BN_DATA *pBD);
Vertex GetPrevVertex(BN_STRUCT *pBNS, Vertex y, Edge *SwitchEdge,
                     EdgeIndex *iuv);
int bIgnoreVertexNonTACN_atom(BN_STRUCT *pBNS, Vertex u, Vertex v);
int bIgnoreVertexNonTACN_group(BN_STRUCT *pBNS, Vertex v, Vertex w,
                               Edge *SwitchEdge);
int bIsRemovedHfromNHaion(BN_STRUCT *pBNS, Vertex u, Vertex v);
int bIsAggressiveDeprotonation(BN_STRUCT *pBNS, Vertex v, Vertex w,
                               Edge *SwitchEdge);

int bIsAtomTypeHard(inp_ATOM *at, int endpoint, int nType, int nMask,
                    int nCharge);
...
int RestoreEdgeFlow(BNS_EDGE *edge, int delta, int bChangeFlow) {
  ...
  case 0: /* the flow has not been permitted to change inside the BNS */
    ...
    /*flow2 = flow1 + delta;*/ /* the flow would be changed to this value by the
                                  BNS if permitted */
    break;
  case BNS_EF_CHNG_FLOW: /* the flow has been changed by the BNS; update flow0
    ...
    edge->flow0 = edge->flow;  /* SAVE NEW EDGE FLOW AS THE INITIAL FLOW FROM
                                  CHEM. BONDS */
    break;
  case BNS_EF_CHNG_RSTR: /* the flow has been changed by the BNS; requested to
                            change it back */
    /*flow2 = edge->flow;*/    /* output from BNS, the changed value */
    /*flow1 = flow2 - delta;*/ /* the original flow value before the BNS */
    edge->flow = edge->flow - delta; /* CHANGE EDGE FLOW BACK (RESTORE) */
    break;
  case BNS_EF_RSTR_FLOW: /* the flow has not been permitted to change inside the
                            BNS */
    /* nothing to do */
    /*flow1 = edge->flow;*/    /* output from BNS, the original flow value */
    /*flow2 = flow1 + delta;*/ /* the flow would be changed to this value by the
...
int SetAtomBondType(BNS_EDGE *edge, U_CHAR *bond_type12, U_CHAR *bond_type21,
  ...
  switch (bChangeFlow & BNS_EF_CHNG_RSTR) {
  case 0: /* the flow has not been permitted to change inside the BNS: simulated
             in case of check one bond */
  case BNS_EF_RSTR_FLOW: /* the flow has not been permitted to change inside the
                            BNS: obsolete mode, unexpected bChangeFlow */
    flow1 = edge->flow0; /* output from BNS, the original (old) flow value */
    flow2 = flow1 + delta; /* the flow would be changed to this value by the BNS
                              if permitted */
    break;
  ...
  if ((bChangeFlow & BNS_EF_CHNG_BONDS) &&
      (bChangeFlow & BNS_EF_ALTR_NS) != BNS_EF_ALTR_NS) {
...
/************************************************************************************/
/* Eliminate neutral representation ambiguity: replace (+)--N==(-) with
 * (+)==N--(-) */
...
/************************************************************************************/
int EliminatePlusMinusChargeAmbiguity(BN_STRUCT *pBNS, int num_atoms) {
  int pass, i, v0, v1, v2, ineigh1, /*ineigh0,*/ /*ineigh2,*/ vLast, n, delta,
      ret, err = 0;
  BNS_EDGE *edge;
  int nFound, k;
  ...
  for (pass = pBNS->num_altp - 1, ret = 0; 0 <= pass; pass--) {
    ...
    for (i = 0; i < n;
         i++, delta = -delta, v0 = v1, v1 = v2 /*, ineigh0 = ineigh1*/) {
      ineigh1 =
          ALTP_THIS_ATOM_NEIGHBOR(pBNS->alt_path, i); /* v1->v2 neighbor */
      /*ineigh2 = ALTP_NEXT_ATOM_NEIGHBOR(pBNS->alt_path, i);*/ /* v2->v1
                                                                   neighbor */
      edge = pBNS->edge + pBNS->vert[v1].iedge[ineigh1];
      /* follow the BN Structure, not the inp_ATOM, to take care of swithching
         to t-groups, c-groups or other fictitious edges/vertices
      */
      v2 = edge->neighbor12 ^ v1;
      if (v1 < num_atoms &&
          ((v0 >= num_atoms && (pBNS->vert[v0].type & BNS_VERT_TYPE_C_GROUP)) ||
           (v2 >= num_atoms &&
            (pBNS->vert[v2].type & BNS_VERT_TYPE_C_GROUP)))) {
        int cgPos, cgNeg;
        int neighPos = -1, neighNeg = -1;
        BNS_EDGE *edgePos, *edgeNeg;
        nFound = 0;
        for (k = pBNS->vert[v1].num_adj_edges - 1;
             k >= 0 && (neighPos < 0 || neighNeg < 0); k--) {
...
int AddOrRemoveExplOrImplH(int nDelta, inp_ATOM *at, int num_atoms,
  ...
  int i, iso, tot_num_iso_H,
      ...
      nNumRemovedExplicitH, nNumExplicit2Implicit;
  ...
  nNumRemovedExplicitH =
      t_group_info->tni.nNumRemovedExplicitH; /* number of explicit H saved
                                                separately in at[num_atoms+i],
                                                i=0..nNumRemovedExplicitH-1 */
  ...
  for (i = 0; i < nNumRemovedExplicitH;) {
    if (at_H[i].neighbor[0] == at_no) {
      ...
      if (nNumRemovedExplicitH > i) {
        ...
        memmove(at_H + i, at_H + i + 1,
                sizeof(at_H[0]) * (nNumRemovedExplicitH - i));
        at_H[nNumRemovedExplicitH] =
            at_i; /* save removed H (for debugging purposes?) */
      }
      /* adjust 0D parities */
        ...
        for (m = 0; m < MAX_NUM_STEREO_BONDS && at[at_no].sb_parity[m]; m++) {
          if (at[at_no].sn_orig_at_num[m] == orig_no) {
            ...
            if (at[at_no].valence >= MIN_NUM_STEREO_BOND_NEIGH) {
              ...
              at[at_no].sn_orig_at_num[m] =
                  at[(int)at[at_no].neighbor[k]].orig_at_number;
              ...
              if (ATOM_PARITY_WELL_DEF(at[at_no].sb_parity[m])) {
                ...
                if (0 < get_opposite_sb_atom(at, at_no, at[at_no].sb_ord[m],
                  ...
                  at[at_no].sb_parity[m] =
                      at[pnxt_atom].sb_parity[pinxt_sb_parity_ord] =
                          AB_PARITY_UNDF;
                }
#ifdef _DEBUG
                else {
      ...
      if (v1 < num_atoms && (v0 >= num_atoms || v2 >= num_atoms)) {
        ...
        if (nDeltaH || nDeltaCharge) {
          if (bSubtract) {
            ...
            if (nDeltaH) {
              AddOrRemoveExplOrImplH(nDeltaH, at, num_atoms, (AT_NUMB)v1,
                                     t_group_info);
            }
            ret++; /* number of changed atoms */
          }
...
int SetBondsFromBnStructFlow(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                             int bChangeFlow0) {
  int pass, i, v0, v1, v2, ineigh1, ineigh2, vLast, n, delta, ret, ret_val,
      err = 0;
  BNS_EDGE *edge;
  ...
  for (pass = pBNS->num_altp - 1, ret = 0; 0 <= pass; pass--) {
    ...
    for (i = 0; i < n; i++, delta = -delta, v0 = v1, v1 = v2) {
      ineigh1 =
          ALTP_THIS_ATOM_NEIGHBOR(pBNS->alt_path, i); /* v1->v2 neighbor */
      ineigh2 =
          ALTP_NEXT_ATOM_NEIGHBOR(pBNS->alt_path, i); /* v2->v1 neighbor */
      edge = pBNS->edge + pBNS->vert[v1].iedge[ineigh1];
      /* follow the BN Structure, not the inp_ATOM, to take care of swithching
         to t-groups, c-groups or other fictitious edges/vertices
      */
      ...
      if (v1 < num_atoms && ineigh1 < at[v1].valence && v2 < num_atoms &&
          ineigh2 < at[v2].valence) {
        if ((bChangeFlow0 & BNS_EF_ALTR_NS) == BNS_EF_ALTR_NS &&
            (bChangeFlow0 & BNS_EF_SAVE_ALL) == BNS_EF_SAVE_ALL) {
          /* 2004-07-02 special mode: save new ring bonds and mark as non-stereo
           * non-ring bonds */
        ...
        /* change bonds on the first pass only: in this case all flow correspond
         * to the BNS output */
        ret_val = SetAtomBondType(edge, &at[v1].bond_type[ineigh1],
                                  &at[v2].bond_type[ineigh2], delta,
    ...
    if (v2 != vLast) {
      ...
      /* "+=" instead of "-=" explanation: delta must have same sign as at the
       * last edge */
...
int MarkAtomsAtTautGroups(BN_STRUCT *pBNS, int num_atoms, BN_AATG *pAATG,
  ...
  for (pass = pBNS->num_altp - 1; 0 <= pass; pass--) {
    pBNS->alt_path = pBNS->altp[pass];
    vFirst = v1 = ALTP_START_ATOM(pBNS->alt_path);
    n = ALTP_PATH_LEN(pBNS->alt_path);
    delta = ALTP_DELTA(pBNS->alt_path);
    vLast = ALTP_END_ATOM(pBNS->alt_path);
    v2 = NO_VERTEX;
    ...
    for (i = 0; i < n; i++, delta = -delta, v1 = v2) {
      ...
      */
      ...
      if (v1 >= num_atoms &&
          ((pBNS->vert[v1].type & BNS_VERT_TYPE_TGROUP) ||
           (pBNS->vert[v1].type & BNS_VERT_TYPE_TEMP)) &&
          0 <= v2 && v2 < num_atoms &&
          (pBNS->vert[v2].type & BNS_VERT_TYPE_ATOM)) {
        /*
        ...
        */
        /* BNS increased bond order in  v1(t-group)-v2(atom) by delta: added
         * delta attachments */
        if (nLenDelta < MAX_ALT_AATG_ARRAY_LEN) {
          ...
          nLenDelta++;
                 ...
                 ((pBNS->vert[v2].type & BNS_VERT_TYPE_TGROUP) ||
                  (pBNS->vert[v2].type & BNS_VERT_TYPE_TEMP)) &&
                 0 <= v1 && v1 < num_atoms &&
                 (pBNS->vert[v1].type & BNS_VERT_TYPE_ATOM)) {
        /*
        ...
        */
        /* BNS increased bond order in  v1(atom)-v2(t-group) by delta: added
         * delta attachments */
        if (nLenDelta < MAX_ALT_AATG_ARRAY_LEN) {
          cDelta[nLenDelta] = delta;
        ...
        /* special case when the testing 'dot' was placed on an atom (should be
         * nEnd1 only) */
        if (((0 <= v1) && (v1 == nEnd1)) ||
            ((v1 == nEnd2) && (0 <= v2) && (v2 < num_atoms))) {
          if (nLenDelta < MAX_ALT_AATG_ARRAY_LEN) {
            cDelta[nLenDelta] = -delta;
            nVertex[nLenDelta] = v1;
            nLenDelta++;
          }
        } else if (((0 <= v2) && (v2 == nEnd1)) ||
                   ((v2 == nEnd2) && (0 <= v1) && (v1 < num_atoms))) {
          if (nLenDelta < MAX_ALT_AATG_ARRAY_LEN) {
            cDelta[nLenDelta] = -delta;
            nVertex[nLenDelta] = v2;
            nLenDelta++;
  ...
  for (pass = pBNS->num_altp - 1, ret = 0; 0 <= pass; pass--) {
    ...
    if ((bChangeFlow & BNS_EF_CHNG_RSTR) == BNS_EF_CHNG_RSTR) {
      pBNS->vert[v1].st_edge.flow -=
          delta; /* restore s-v1 edge flow to the BNS input value */
    } else if ((bChangeFlow & BNS_EF_SAVE_ALL) == BNS_EF_SAVE_ALL) {
      pBNS->vert[v1].st_edge.flow0 = pBNS->vert[v1].st_edge.flow;
    }
...
/***************************************************************************************/
int bNeedToTestTheFlow(int bond_type, int nTestFlow,
    ...
    if (nBondAttrib || nBondType == BOND_ALTERN || nBondType == BOND_ALT12NS) {
      ...
      case 0: /* single: can be 1 (single)?  */
        if (nBondAttrib == BOND_MARK_ALT12NS ||
            nBondAttrib == BOND_MARK_ALT123 || nBondAttrib == BOND_MARK_ALT13) {
      ...
      case 1: /* double: can be 2 (double)? */
        if (nBondAttrib == BOND_MARK_ALT12NS ||
            nBondAttrib == BOND_MARK_ALT123 || nBondAttrib == BOND_MARK_ALT23) {
          return 0; /* yes, already checked */
        }
        break;
      case 2: /* triple: can be 3 (triple)?  */
        if (nBondAttrib == BOND_MARK_ALT13 || nBondAttrib == BOND_MARK_ALT123 ||
...
/***********************************************************************************/
int nBondsValenceInpAt(const inp_ATOM *at, int *nNumAltBonds,
                       int *nNumWrongBonds) {
...
int BnsAdjustFlowBondsRad(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at,
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (!bIgnore && nValMinusBondsVal > 0) {
      if (!pcValMinusBondsVal &&
          !(pcValMinusBondsVal = (int *)inchi_calloc(
                num_atoms, sizeof(pcValMinusBondsVal[0])))) {
    ...
    if (IS_BNS_ERROR(ret)) {
      ...
      if (ret > 0) {
        ...
        ret = SetBondsFromBnStructFlow(
            pBNS, at, num_atoms, BNS_EF_SAVE_ALL); /* must include 1: 5=(4|1) */
        if (IS_BNS_ERROR(ret)) {
          bError = ret;
        }
        ret = RestoreBnStructFlow(
            pBNS, BNS_EF_SAVE_ALL); /* must include 1: 5=(4|1) */
        if (IS_BNS_ERROR(ret)) {
          bError = ret;
        }
      }
  ...
  if (!bError && pcValMinusBondsVal) {
    for (i = 0; i < num_atoms; i++) {
      ...
      is_rad = (at[i].radical == RADICAL_DOUBLET);
      excess = nValMinusBondsVal + is_rad;
      if (bIgnore || (pcValMinusBondsVal[i] - excess) != 1) {
  ...
  for (iat = 0; iat < num_atoms && !bError; iat++) {
    for (ineigh = 0; ineigh < at[iat].valence && !bError; ineigh++) {
      ...
      if (IS_FORBIDDEN(pBNS->edge[iedge].forbidden, pBNS))
        continue;
      if (nBondTypeToTest &&
          (at[iat].bond_type[ineigh] & BOND_TYPE_MASK) != nBondTypeToTest)
        continue;
      ...
      if (nMinFlow == nMaxFlow) {
        if (nMaxFlow && bTestForNonStereoBond) {
          nTestFlow = nMaxFlow -
                      (int)(pBNS->tot_st_cap -
                            pBNS->tot_st_flow); /* temporary use of nTestFlow */
          nMinFlow = inchi_max(0, nTestFlow);
        } else {
          continue;
        }
      }
      for (nTestFlow = nMinFlow; nTestFlow <= nMaxFlow && !bError;
           nTestFlow++) {
        if (nTestFlow == nCurFlow)
          continue;
        if (!bNeedToTestTheFlow(at[iat].bond_type[ineigh], nTestFlow,
                                bTestForNonStereoBond))
          continue;
        bSuccess = 0;
        ...
        if (IS_BNS_ERROR(nDots)) {
          ...
          if (IS_BNS_ERROR(ret)) {
            ...
            if (2 * ret == nDots) {
              ret = bSetBondsAfterCheckOneBond(pBNS, fcd, nTestFlow, at,
                                               num_atoms, bChangeFlow);
              if (IS_BNS_ERROR(ret)) {
                bError = ret;
              } else {
                nChanges += (ret & 1);
                ret =
                    SetBondsFromBnStructFlow(pBNS, at, num_atoms, bChangeFlow);
                if (IS_BNS_ERROR(ret)) {
                  bError = ret;
                } else if (ret >= 0) {
                  nChanges += (ret & 1);
            ...
            /* typically 2*ret < nDots; 2*ret > nDots should not happen. Check
             * later */
          ...
          ret = bSetBondsAfterCheckOneBond(pBNS, fcd, nTestFlow, at, num_atoms,
                                           bChangeFlow);
          if (IS_BNS_ERROR(ret)) {
            bError = ret;
          } else {
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (at[i].el_number == el_number_C && 3 == at[i].valence &&
      ...
      for (j = 0; j < at[i].valence; j++) {
        ...
        if (at[neigh].el_number == el_number_O && at[neigh].valence == 1) {
          num_O++;
      ...
      if (at[i].el_number == el_number_N && 3 == at[i].valence &&
        ...
        if (2 == num_O && (3 == bond_to_O_val || 4 == bond_to_O_val) &&
            1 == forbidden_bond_val) {
          iedge = pBNS->vert[i].iedge[forbidden_bond_pos];
          pBNS->edge[iedge].forbidden |= edge_forbidden_mask;
          num_found++;
        }
...
int TempFix_NH_NH_Bonds(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms) {
  ...
  for (i = 0, num_found = 0; i < num_atoms; i++) {
    ...
    if (at[i].el_number == el_number_N && at[i].valence < 3 && at[i].num_H &&
        ...
        at[i].chem_bonds_valence == at[i].valence && !at[i].charge &&
        !at[i].radical) {
      for (j = 0; j < at[i].valence; j++) {
        neigh = at[i].neighbor[j];
        if (neigh < i && at[neigh].el_number == el_number_N &&
            at[neigh].valence < 3 && at[neigh].num_H &&
...
#define MAX_NEIGH 6
  ...
  for (i = 0, a = at; i < num_atoms; i++, a++) {
    if (!a->charge && !a->radical &&
        2 <= a->chem_bonds_valence + NUMH(a, 0) -
                 get_el_valence(a->el_number, 0, 0) &&
        0 == num_of_H(at, i) &&
        2 == nNoMetalBondsValence(at, i) + NUMH(a, 0) -
                 get_el_valence(a->el_number, 0, 0) &&
        NULL != memchr(en + ELEM_N_FST, at[i].el_number, ELEM_N_LEN)) {
      /* found N(V), no H */
      if (2 == nNoMetalNumBonds(at, i)) {
        /* #N= */
        /* fix bonds: double and triple: =N# so that bonds cannot be changed by
         * the normalization */
#if (FIX_N_V_METAL_BONDS_GPF == 1)
        if (0 > (i1 = nNoMetalNeighIndex(at, i)) ||
            0 > (i2 = nNoMetalOtherNeighIndex(
                     at, i,
                     n1 = a->neighbor[i1] /* non-metal neighbor #1 */))) {
          /*num_err ++; */ /* do not count would-be original InChI v.1 buffer
                              overflow GPF */
          continue; /* v1 bug: 2 bonds to metal yield i1 < 0 and/or i2 < 0 =>
                       bounds violation */
        }
#else
        ...
        pBNS->edge[iedge].forbidden |=
            edge_forbidden_mask; /* fix bond to neighbor #1 */
        ...
        if (0 == NUMH(at, n3) && 2 == nNoMetalNumBonds(at, n3) &&
          ...
          num_changes++;
                 ...
                 0 <= (j[1] = nNoMetalOtherNeighIndex(
                           at, i, m[0] = a->neighbor[j[0]])) &&
                 0 <= (j[2] = nNoMetalOtherNeighIndex2(
                           at, i, m[0], m[1] = a->neighbor[j[1]]))) {
        ...
        for (k = 0; k < 3; k++) {
          ...
          if (NULL != memchr(en + ELEM_N_FST, at[n1].el_number, ELEM_N_LEN)) {
            k_N = k;
            num_N++;
          } else if (NULL != memchr(en + ELEM_O_FST, at[n1].el_number,
                                    ELEM_O_LEN) &&
        ...
        if (1 == num_O && 0 == at[n1 = m[k_O]].charge && 0 == at[n1].radical &&
            BOND_TYPE_DOUBLE ==
                (at[i].bond_type[i1 = j[k_O]] & BOND_TYPE_MASK)) {
        ...
        if (!num_other && num_O <= 1 && 1 == num_N &&
            0 == at[n1 = m[k_N]].charge && 0 == at[n1].radical &&
            BOND_TYPE_DOUBLE ==
                (at[i].bond_type[i1 = j[k_N]] & BOND_TYPE_MASK)) {
         ...
         * if it is =N- then fix the double bond and the single bond beyond the
         * neighbor
         */
        num_N = 0;
        num_other = 0;
        ...
        if (1 == num_N && 0 == NUMH(at, n2) && 2 == nNoMetalNumBonds(at, n2) &&
        ...
        if (BOND_TYPE_DOUBLE == bond_type) {
          ...
          if (NULL != memchr(en + ELEM_O_FST, at[n1].el_number, ELEM_O_LEN)) {
            num_O++;
                   ...
                   NULL !=
                       memchr(en + ELEM_O_FST, at[n1].el_number, ELEM_O_LEN)) {
          if (0 == at[n1].charge) {
            num_OH++;
            n3 = n1;
            i3 = i1;
      ...
      if (1 == num_N && 1 == num_O && 1 == num_OH + num_OM) {
        ...
        if (bDoNotFixAnyBond) {
          ...
          -------------------*/
               ...
               4 <= a->chem_bonds_valence + NUMH(a, 0) -
                        get_el_valence(a->el_number, 0, 0) &&
               0 == num_of_H(at, i) &&
               4 == nNoMetalBondsValence(at, i) + NUMH(a, 0) -
                        get_el_valence(a->el_number, 0, 0) &&
               NULL != memchr(en + ELEM_S_FST, a->el_number, ELEM_S_LEN) &&
      ...
      */
      ...
      num_OH = 0; /* number of neighbors OH connected by a single bond */
      num_OM =
          0; /* number of non-terminal neighbors connected by a single bond */
      num_O = 0; /* number of neighbors =O connected by a double bond */
      ...
      for (i1 = 0; i1 < a->valence; i1++) {
        ...
        if (BOND_TYPE_DOUBLE == bond_type) {
          ...
          if ((0 == at[n1].charge
              ...
              0 == at[n1].radical && 0 == num_of_H(at, n1) &&
          ...
          iN[num_OM++] =
              i1; /* non-terminal neighbor connected by a single bond */
      ...
      if (num_N == num_O && 2 == num_O && 2 == num_OH + num_OM + num_X &&
          0 == num_other) {
        for (i2 = 0; i2 < num_OM; i2++) {
          ...
          num_changes++;
               ...
               6 <= a->chem_bonds_valence + NUMH(a, 0) -
                        get_el_valence(a->el_number, 0, 0) &&
               0 == num_of_H(at, i) &&
               6 == nNoMetalBondsValence(at, i) + NUMH(a, 0) -
                        get_el_valence(a->el_number, 0, 0) &&
               NULL != memchr(en + ELEM_S_FST, a->el_number, ELEM_S_LEN) &&
      ...
      */
      ...
      if (num_N == num_O && 3 == num_O && 2 == num_OH + num_OM + num_X &&
          0 == num_other) {
        for (i2 = 0; i2 < num_OM; i2++) {
          i1 = iN[i2];
          /* fix bond i1 */
...
int IsZOX(inp_ATOM *atom, int at_x, int ord) { /* detect O==Z--X, O=O,S,Se,Te */
  ...
  for (i = 0, num_O = 0; i < at_Z->valence; i++) {
    ...
    if (atom[neigh].valence == 1 && atom[neigh].chem_bonds_valence == 2 &&
        atom[neigh].charge == 0 && atom[neigh].radical == 0 &&
...
int GetAtomChargeType(inp_ATOM *atom, int at_no, int nAtTypeTotals[],
...
#if (FIX_NORM_BUG_ADD_ION_PAIR == 1)
  int i, neigh, mask, bit, type, num_z, num_m, num_o,
      delta = bSubtract > 0 ? -1 : 1; /* 0 or -2 => add, 1 or 2 => subtract */
  ...
  if ((!at->valence && at->charge == 0 && 1 == at->num_H && !at->radical &&
      ...
      (!at->valence && at->charge == 0 && 2 == at->num_H && !at->radical &&
       (at->el_number == el_number_O || at->el_number == el_number_S ||
        at->el_number == el_number_Se || at->el_number == el_number_Te))) {
...
#else
    if (atom[neigh].charge < -1 || atom[neigh].charge > 1 ||
        atom[neigh].charge && at->charge) {
      goto exit_function; /* neighboring charge */
    }
#endif
    if (detect_unusual_el_valence(atom[neigh].el_number, atom[neigh].charge,
                                  atom[neigh].radical,
                                  atom[neigh].chem_bonds_valence,
                                  atom[neigh].num_H, atom[neigh].valence)) {
  ...
  /* O, S, Se, Te */
  if (at->el_number == el_number_O || at->el_number == el_number_S ||
      at->el_number == el_number_Se || at->el_number == el_number_Te) {
    if (at->charge == 1) {
      if (at->num_H) { /* #4 */
        type = ATT_O_PLUS;
      ...
      /* here at->neigh[0] is one of: O, or Z={C, N, P, As, Sb, S, Se, Te, Cl,
       * Br, I} */
      if (at->valence) {
        ...
        if (!atom[neigh].charge && atom[neigh].el_number == el_number_C &&
          ...
          if (at->num_H == 1) {
            mask |=
                (ATBIT_CSH); /* #11: >C-SH, >CH-SH, -CH2-SH; S = S, Se, Te */
                             /*nAtTypeTotals[ATTOT_NUM_CSH] ++;*/
          } else if (at->charge == -1) {
            mask |= (ATBIT_CS_Minus); /* #12: >C-S(-), >CH-S(-), -CH2-S(-); S =
                                         S, Se, Te  */
            ...
            /*nAtTypeTotals[ATTOT_NUM_Errors] ++;*/
                   ...
                   (!atom[neigh].num_H ||
                    ((atom[neigh].num_H == 1) && (atom[neigh].charge == 1)))) {
          /* N or N(-) or NH(+) neighbor */
          type = ATT_NO; /* single bond only */
          if (at->num_H == 1) {
            mask |= (ATBIT_NOH); /* #13: =N-OH, =NH(+)-OH, #N(+)-OH, -N(-)-OH; O
                                    = O, S, Se, Te */
          ...
          if (at->num_H == 1) {
            mask |= (ATBIT_N_OH); /* #16: -NH-OH, >N-OH or >N(+)<OH; O = O, S,
                                     Se, Te */
                                  /*nAtTypeTotals[ATTOT_NUM_NOH] ++;*/
          } else if (at->charge == -1) {
            mask |= (ATBIT_N_O_Minus); /* #17: -NH-O(-), >N-O(-); O = O, S, Se,
                                          Te */
                                       /*nAtTypeTotals[ATTOT_NUM_NO_Minus] ++;*/
        ...
        } else if (atom[neigh].el_number != el_number_C &&
                   atom[neigh].el_number != el_number_O &&
                   !is_el_a_metal(atom[neigh].el_number) &&
                   atom[neigh].chem_bonds_valence > atom[neigh].valence) {
          /* =Z-OH, #Z-OH, =Z-O(-), #Z-O(-), -Z=O, =Z=O;
             =Z(+)-OH, #Z(+)-OH, =Z-O(-), #Z-O(-), -Z(+)=O, =Z(+)=O; O = O, S,
             Se, Te */
          ...
          if (at->chem_bonds_valence == 1 && IsZOX(atom, at_no, 0)) {
            ...
            if (at->num_H == 1) {
              mask |=
                  (ATBIT_ZOOH); /* 18: O=Z-OH; O=O,S,Se,Te; Z may have charge */
            ...
            type = ATT_OTHER_ZO;
            if (at->num_H == 1) {
              mask |= (ATBIT_ZOH); /* 20: =Z-OH, #Z-OH; O=O,S,Se,Te; Z may have
                                      charge */
                                   /*nAtTypeTotals[ATTOT_NUM_ZOH] ++;*/
            } else if (at->charge == -1) {
              mask |=
                  (ATBIT_ZO_Minus); /* 21: =Z-O(-), #Z-O(-); O = O, S, Se, Te */
                                    /*nAtTypeTotals[ATTOT_NUM_ZO_Minus] ++;*/
            } else if (at->num_H == 0) {
              mask |= (ATBIT_ZO); /* 22: -Z=O, =Z=O; O=O,S,Se,Te; Z may have
                                     charge */
    ...
    if ((at->el_number == el_number_N || at->el_number == el_number_P) &&
      ...
      if (at->valence && !(num_z /*|| num_o == at->valence*/)) {
        ...
        goto count_mask_bits; /* N(III), N(-)(II), N(+)(IV) and same P that have
                                 only oxygen neighbors are ignored here */
      }
...
#if (FIX_NP_MINUS_BUG == 1)
          else if (at->valence == 1 && at->chem_bonds_valence >= 2 &&
                   (at->bond_type[0] & BOND_MARK_ALL))
            type |= ATT_NP_MINUS_V23; /* =N(-) created by normalization
                                         2010-03-11 DT */
#endif
        }
      ...
      case 0:
        if (at->num_H) {
          ...
          if (bUnsatNHasTerminalO == 1) {
            ...
            mask |=
                (ATBIT_NP); /* 30: -P=O,-P=OH(+), >N- =N- (incl. =N-OH) , #N */
      ...
      case 1:
        if (at->num_H) {
          mask |= (ATBIT_NP_Proton); /* 31: NH4(+), -NH3(+), =NH2(+), >NH2(+),
                                        =NH(+)-, >NH(+)-, #NH(+) */
...
#endif
      type = GetAtomChargeType(at, i, nAtTypeTotals, &mask,
                               1); /* subtract at[i] */
      at[i].charge = 0;
...
int bIsAtomTypeHard(inp_ATOM *at, int endpoint, int nType, int nMask,
                    int nCharge) {
  int mask;
  if ((nType & GetAtomChargeType(at, endpoint, NULL, &mask, 0)) &&
      (mask & nMask)
...
int bIsHDonorAccAtomType(inp_ATOM *at, int endpoint, int *cSubType) {
  if (bIsAtomTypeHard(at, endpoint, PR_HARD_TYP_H, PR_HARD_MSK_H, 0)) {
    ...
    int neutral_valence = at[endpoint].chem_bonds_valence + at[endpoint].num_H -
                          at[endpoint].charge;
    if (neutral_valence != 2 /* O, S, Se, Te */ &&
      ...
      int edge_cap =
          neutral_valence - num_bonds; /* does not allow to reduce -NH3(+) to #N
                                          or -OH(+)- to -O- */
  ...
  if (bIsAtomTypeHard(at, endpoint, PR_HARD_TYP_NEG, PR_HARD_MSK_NEG, -1)) {
    ...
    if (neutral_valence != 2 /* O, S, Se, Te */ &&
      ...
      int edge_cap = neutral_valence - num_bonds -
                     at[endpoint].num_H; /* does not allow to reduce -NH3(+) to
                                            #N or -OH(+)- to -O- */
      edge_flow = inchi_min(edge_flow, edge_cap);
      /* what this means: */
      if (edge_cap) {
...
int CreateCGroupInBnStruct(inp_ATOM *at, int num_atoms, BN_STRUCT *pBNS,
  ...
  BNS_VERTEX *vert_ficpoint,
      *ver_ficpont_prev; /* fictitious vertex describing charge c-group */
  ...
  for (c_point = 0, num_CPoints = 0; c_point < num_atoms; c_point++) {
    if ((nType & GetAtomChargeType(at, c_point, NULL, &mask, 0)) &&
        (mask & nMask)
#if (OPPOSITE_CHARGE_IN_CGROUP == 0)
  ...
  /* *old* Make sure the last t-group has the largest t-group ID:
     this is necessary to correctly add new edges and vertices for testing
     augmenting paths
  */
  /**************************************/
  ...
  for (cg = 0; cg < num_cg; cg++, ver_ficpont_prev = vert_ficpoint) {
    ...
    */
    ...
    vert_ficpoint->iedge =
        ver_ficpont_prev->iedge + ver_ficpont_prev->max_adj_edges;
    ...
    vert_ficpoint->type =
        BNS_VERT_TYPE_C_GROUP | ((nCharge < 0) ? BNS_VERT_TYPE_C_NEGATIVE : 0);
  }
  ...
  for (c_point = 0; c_point < num_atoms; c_point++) {
    if ((nType & (type = GetAtomChargeType(at, c_point, NULL, &mask, 0))) &&
        (mask & nMask)
#if (OPPOSITE_CHARGE_IN_CGROUP == 0)
        && (at[c_point].charge == nCharge || !at[c_point].charge)
#endif
    ...
    /* Debug: check overflow */
    if (fictpoint >= pBNS->max_vertices || num_edges >= pBNS->max_edges ||
    ...
    for (k = 0; k < vertex_cpoint->num_adj_edges; k++) {
      ...
      if (!pBNS->edge[iedge].cap) {
        ...
        if (centerpoint < pBNS->num_atoms &&
          ...
          if (bond_type == BOND_TAUTOM || bond_type == BOND_ALTERN ||
    ...
    /* create a new edge connecting c_point to the new fictitious c-group vertex
     * vert_ficpoint */
...
#if (RESET_EDGE_FORBIDDEN_MASK == 1)
    edge->forbidden &=
        pBNS->edge_forbidden_mask; /* remove previous temporary ban */
#endif
    /* nCharge = +1: mark edge to c-point having no (+)-moveable charge with
     * flow=1 */
    /* nCharge = -1: mark edge to c-point having -1 moveable charge with flow=1
     */
    if ((nCharge == 1 && (at[c_point].charge != 1)) ||
        (nCharge == -1 && (at[c_point].charge == -1)))
    ...
    for (k = 0; k < vertex_cpoint->num_adj_edges; k++) {
      ...
      else if (centerpoint < pBNS->num_atoms && edge->flow &&
               pBNS->edge[iedge].cap < MAX_BOND_EDGE_CAP) {
        pBNS->edge[iedge].cap++;
      }
#endif
    }
#endif /* } FIX_CPOINT_BOND_CAP */
    /* connect edge to c_point and fictpoint and increment the counters of
     * neighbors and edges */
    edge->neighbor1 =
        c_point; /* the smallest out of v1=endopoint and v2=num_vertices */
...
int CreateTGroupInBnStruct(inp_ATOM *at, int num_atoms, BN_STRUCT *pBNS,
  ...
  BNS_VERTEX *vert_ficpoint,
      *ver_ficpont_prev; /* fictitious vertex describing t-group */
  ...
  /* since t-group IDs may be not contiguous, clear all vertices that will be
     added. all-zeroes-vertex will be ignored by the BNS
  */
  ...
  */
  ...
  for (tg = 0; tg < num_tg; tg++, ver_ficpont_prev = vert_ficpoint) {
    ...
    */
    ...
    vert_ficpoint->max_adj_edges =
        num_endpoints + BNS_ADD_EDGES + BNS_ADD_SUPER_TGROUP;
    vert_ficpoint->num_adj_edges = 0;
    vert_ficpoint->st_edge.flow = vert_ficpoint->st_edge.flow0 = 0;
    vert_ficpoint->st_edge.cap = vert_ficpoint->st_edge.cap0 = 0;
  ...
  for (endpoint = 0; endpoint < num_atoms; endpoint++) {
    ...
    /* obtain donor/acceptor info */
    neutral_valence = at[endpoint].chem_bonds_valence + at[endpoint].num_H -
                      at[endpoint].charge;
    if (neutral_valence != 2 /* O, S, Se, Te */ &&
        neutral_valence != 3 /* N, P */) {
    ...
    edge_cap = neutral_valence - num_bonds; /* does not allow to reduce -NH3(+)
                                               to #N or -OH(+)- to -O- */
    ...
    */
    ...
    /* create a new edge connecting endpoint to the new fictitious t-group
     * vertex vert_ficpoint */
    edge = pBNS->edge + num_edges;
    edge->cap = edge_cap;
    edge->flow = edge_flow;
    edge->pass = 0;
    ...
    /* connect edge to endpoint and fictpoint and increment the counters of
     * neighbors and edges */
    edge->neighbor1 =
        endpoint; /* the smallest out of v1=endopoint and v2=num_vertices */
...
int RemoveLastGroupFromBnStruct(inp_ATOM *at, int num_atoms, int tg,
  ...
  int num_edges = pBNS->num_edges;
  int num_vertices = pBNS->num_vertices;
  BNS_VERTEX *vert_ficpoint /*, *ver_ficpont_prev*/; /* fictitious vertex
                                                        describing t-group */
  BNS_VERTEX *vert_endpoint;
  BNS_EDGE *edge; /* edge between that vertex and the tautomeric endpoint */
  /*int        mask, num_endpoints, neutral_valence, edge_flow, edge_cap,
   * num_bonds;*/
  int is_t_group = 0, is_c_group = 0;

  /* Debug: check overflow */
  if (pBNS->num_added_atoms + pBNS->num_c_groups + pBNS->num_t_groups +
          num_atoms >=
      pBNS->max_vertices) {
    return BNS_VERT_EDGE_OVFL;
  }
  ...
  for (k = vert_ficpoint->num_adj_edges - 1; 0 <= k; k--) {
    ...
    vert_endpoint->st_edge.cap0 = vert_endpoint->st_edge.cap -= edge->flow;
    vert_endpoint->st_edge.flow0 = vert_endpoint->st_edge.flow -= edge->flow;
    if (pBNS->type_TACN &&
        (vert_endpoint->type & pBNS->type_TACN) == pBNS->type_TACN) {
...
int SimpleRemoveAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
  ...
  for (i = 0, num_removed = 0; i < num_atoms && num_removed < num2remove; i++) {
    if (!at[i].charge && at[i].num_H &&
        (type = GetAtomChargeType(at, i, NULL, &mask, 0))) {
      for (j = 0; j <= max_j; j++) {
        if (num[j] && (type & ArTypMask[2 * j]) &&
            (mask && ArTypMask[2 * j + 1])) {
          type = GetAtomChargeType(at, i, pAATG->nAtTypeTotals, &mask,
                                   1); /* subtract at[i] */
          num[j]--;
          at[i].charge--;
          AddOrRemoveExplOrImplH(-1, at, num_atoms, (AT_NUMB)i,
                                 pAATG->t_group_info);
          /*at[i].num_H  --;*/
          num_removed++;
          type = GetAtomChargeType(at, i, pAATG->nAtTypeTotals, &mask,
...
#endif
    AA_SIMPLE_TYP2, AA_SIMPLE_MSK2, AA_SIMPLE_TYP3, AA_SIMPLE_MSK3, 0, 0};
...
int SimpleAddAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
  ...
  for (i = 0, num_added = 0; i < num_atoms && num_added < num2add; i++) {
    if (at[i].charge == -1 &&
        (type = GetAtomChargeType(at, i, NULL, &mask, 0))) {
      for (j = 0; j <= max_j; j++) {
        if (num[j] && (type & AaTypMask[2 * j]) &&
            (mask && AaTypMask[2 * j + 1])) {
          type = GetAtomChargeType(at, i, pAATG->nAtTypeTotals, &mask,
                                   1); /* subtract at[i] */
          num[j]--;
          at[i].charge++;
...
int HardRemoveAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
  ...
  int ret = 0, ret2;
  int nDelta, nNumChanges = 0, nNumMoved2AcidH = 0, nNumNeutralized = 0,
              nPrevNumCharges;
  ...
  */
  ...
  nPosCharges = (pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] +
                 pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]) /
                2;
  nNegCharges = (pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] -
                 pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]) /
                2;
  /*
  ...
  */
  ...
  /* create (+) charge group */
  cg_Plus = CreateCGroupInBnStruct(at, num_atoms, pBNS, AR_HARD_TYP_POS,
                                   AR_HARD_MSK_POS, 1);
  /* create (-) charge group */
  /*
  ...
  */
  cg_Minus = CreateCGroupInBnStruct(at, num_atoms, pBNS, AR_HARD_TYP_NEG,
                                    AR_HARD_MSK_NEG, -1);
  ...
  tg_H_Other = CreateTGroupInBnStruct(at, num_atoms, pBNS, AR_HARD_TYP_HN,
                                      AR_HARD_MSK_HN);

  /* create tautomeric group for possibly acidic O */
  tg_H_Acid = CreateTGroupInBnStruct(at, num_atoms, pBNS, AR_HARD_TYP_HA,
                                     AR_HARD_MSK_HA);
  if (tg_H_Other >= num_atoms && tg_H_Acid >= num_atoms) {
    ...
    do {
      ...
      ret = bExistsAltPath(
          pBNS, pBD, pAATG, at, num_atoms, tg_H_Other /*nVertDoubleBond*/,
          tg_H_Acid /*nVertSingleBond*/, ALT_PATH_MODE_REM_PROTON);
      if (IS_BNS_ERROR(ret)) {
        return ret;
      }
      if (ret & 1) {
        ...
        if (nPrevNumCharges > pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] + 1) {
          nNumNeutralized += (nPrevNumCharges -
                              (pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] - 1)) /
                             2;
        }
      }
    ...
    if ((nNumMoved2AcidH /*|| bCancelChargesAlways*/) &&
        cg_Minus >= num_atoms && cg_Plus >= num_atoms &&
        pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] >
            abs(pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE])) {
      do {
        nPrevNumCharges = pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES];
        ret = bExistsAltPath(
            pBNS, pBD, pAATG, at, num_atoms, cg_Minus /*nVertDoubleBond*/,
            cg_Plus /*nVertSingleBond*/, ALT_PATH_MODE_REM_PROTON);
        if (IS_BNS_ERROR(ret)) {
          return ret;
        }
        if (ret & 1) {
          ...
          if (nPrevNumCharges > pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES]) {
            nNumNeutralized +=
                (nPrevNumCharges - pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES]) / 2;
          }
        }
      } while (ret & 1);
    }
  ...
  if (pAATG->nAtTypeTotals[ATTOT_NUM_CO_Minus] +
          pAATG->nAtTypeTotals[ATTOT_NUM_ZO_Minus] &&
      pAATG->nAtTypeTotals[ATTOT_NUM_N_Minus]) {
  }

  nPosCharges2 = (pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] +
                  pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]) /
                 2;
  nNegCharges2 = (pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] -
                  pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]) /
                 2;
  /*
...
int HardAddAcidicProtons(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
  ...
  int ret = 0, ret2;
  int nDelta, nNumChanges = 0, nNumMoved2AcidMinus = 0, nNumNeutralized = 0,
              nPrevNumCharges;
  ...
  */
  ...
  pBNS->type_TACN = BNS_VERT_TYPE_ACID;
  /* create (+) charge group */
  cg_Plus = CreateCGroupInBnStruct(at, num_atoms, pBNS, AA_HARD_TYP_POS,
                                   AA_HARD_MSK_POS, 1);
  /* create (-) charge group */
  /*
  if ( nAtTypeTotals[ATTOT_NUM_CO_Minus] +
       ...
       nAtTypeTotals[ATTOT_NUM_N_Minus] )
  */
  cg_Minus_CO = CreateCGroupInBnStruct(at, num_atoms, pBNS, AA_HARD_TYP_CO,
                                       AA_HARD_MSK_CO, -1);

  cg_Minus_Other = CreateCGroupInBnStruct(at, num_atoms, pBNS, AA_HARD_TYP_NEG,
                                          AA_HARD_MSK_NEG, -1);
  ...
  tg_H =
      CreateTGroupInBnStruct(at, num_atoms, pBNS, AA_HARD_TYP_H, AA_HARD_MSK_H);
  ...
  if (cg_Minus_Other >= num_atoms && cg_Minus_CO >= num_atoms) {
    ...
    do {
      ...
      nPrevNumCharges = pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES];
      ret = bExistsAltPath(
          pBNS, pBD, pAATG, at, num_atoms, cg_Minus_Other /*nVertDoubleBond*/,
          cg_Minus_CO /*nVertSingleBond*/, ALT_PATH_MODE_REM_PROTON);
      if (IS_BNS_ERROR(ret)) {
        return ret;
      }
      if (ret & 1) {
...
/******************************************************************************************/
/* examples include removal of H from tautomeric O that belongs to the same
 * t-group as N: */
/* >N(+)=-N=-OH =(taut.)=> >N(+)=-NH-=O =(+charge move)=> >N-=NH(+)-=O =>
 * >N-=N-=O + H(+) */
/******************************************************************************************/
int HardRemoveHplusNP(inp_ATOM *at, int num_atoms, int bCancelChargesAlways,
                      int *nNumCanceledCharges, BN_AATG *pAATG, BN_STRUCT *pBNS,
...
#endif
  int ret = 0, ret2;
  int nDelta, nNumChanges = 0, nNumRemovedProtons = 0, nNumNeutralized = 0,
              nPrevNumCharges;
  ...
  */
  ...
  pBNS->type_TACN = BNS_VERT_TYPE_ACID;
  /* create (+) charge group */
  cg_Plus = CreateCGroupInBnStruct(at, num_atoms, pBNS, PR_HARD_TYP_POS,
                                   PR_HARD_MSK_POS, 1);
  /* create (-) charge group */
  /*
  if ( nAtTypeTotals[ATTOT_NUM_CO_Minus] +
  ...
  */
#if (MOVE_PPLUS_TO_REMOVE_PROTONS == 1)
  cg_PlusP = CreateCGroupInBnStruct(at, num_atoms, pBNS, PR_HARD_TYP_POSP,
                                    PR_HARD_MSK_POS, 1);
#endif
  cg_Minus = CreateCGroupInBnStruct(at, num_atoms, pBNS, PR_HARD_TYP_NEG,
                                    PR_HARD_MSK_NEG, -1);

  /* create single tautomeric group */
  tg_H =
      CreateTGroupInBnStruct(at, num_atoms, pBNS, PR_HARD_TYP_H, PR_HARD_MSK_H);

  if (tg_H >= num_atoms && cg_Plus >= num_atoms) {

#if (FIX_N_MINUS_NORN_BUG == 1)
    /* neutralize: remove ion pairs like >N(+)=-O(-) => >N-=O; >N(+)=-NH(-) =>
     * >N-=NH */
    if ((nNumRemovedProtons || bCancelChargesAlways) && cg_Minus >= num_atoms &&
        cg_Plus >= num_atoms &&
        pAATG->nAtTypeTotals[ATTOT_NUM_CHARGES] >
            abs(pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE])) {
      do {
...
#endif
      ret = bExistsAltPath(
          pBNS, pBD, pAATG, at, num_atoms, tg_H /*nVertDoubleBond*/,
          cg_Plus /*nVertSingleBond*/, ALT_PATH_MODE_REM_PROTON);
      if (IS_BNS_ERROR(ret)) {
        return ret;
      }
  ...
  nNumOS_H2    = pAATG->nAtTypeTotals[ATTOT_NUM_COH] +
                 ...
                 pAATG->nAtTypeTotals[ATTOT_NUM_ZOH];
  */
  if ((nPosCharges - nNegCharges) - (nPosCharges2 - nNegCharges2) !=
      nNumRemovedProtons) {
    return BNS_PROGRAM_ERR;
  }
...
int mark_at_type(inp_ATOM *atom, int num_atoms, int nAtTypeTotals[]) {
  ...
  if (nAtTypeTotals) {
    ...
    max_O_Minus = nAtTypeTotals[ATTOT_NUM_CO_Minus] +
    nAtTypeTotals[ATTOT_NUM_CS_Minus] + nAtTypeTotals[ATTOT_NUM_ZO_Minus] +
    nAtTypeTotals[ATTOT_NUM_OO_Minus] + nAtTypeTotals[ATTOT_NUM_ZOO_Minus] +
    nAtTypeTotals[ATTOT_NUM_NO_Minus] + nAtTypeTotals[ATTOT_NUM_O_Minus]
    +nAtTypeTotals[ATTOT_NUM_N_Minus];
                  ;
...
int RemoveNPProtonsAndAcidCharges(inp_ATOM *at, int num_atoms, BN_AATG *pAATG,
  ...
  int ret = 0, bError = 0;
  int bAllowHardRemove =
      (t_group_info->bTautFlags & TG_FLAG_TEST_TAUT__SALTS) &&
  ...
  if (!pAATG->nMarkedAtom && (pAATG->nMarkedAtom = (S_CHAR *)inchi_malloc(
                                  num_atoms * sizeof(pAATG->nMarkedAtom[0])))) {
  ...
  if ((num = pAATG->nAtTypeTotals[ATTOT_NUM_NP_Proton] +
             pAATG->nAtTypeTotals[ATTOT_NUM_OH_Plus])) {
    ret =
        SimpleRemoveHplusNPO(at, num_atoms, pAATG->nAtTypeTotals, t_group_info);
    if (ret != num) {
      bError = BNS_PROGRAM_ERR;
      goto exit_function;
    }
    /*t_group_info->nNumRemovedProtons  += ret;*/
    t_group_info->tni.bNormalizationFlags |=
        (ret > 0) ? FLAG_PROTON_NPO_SIMPLE_REMOVED : 0;
  }

  if ((num = pAATG->nAtTypeTotals[ATTOT_NUM_NP_Plus]) && bAllowHardRemove) {
    /* [TECHMAN-5.1b] Hard removing more protons from cationic N; charges may be
     * canceled */
    ret = HardRemoveHplusNP(at, num_atoms, 1, &nNumCanceledCharges, pAATG, pBNS,
                            pBD);
    if (IS_BNS_ERROR(ret)) {
      bError = ret;
      goto exit_function;
    ...
    /*t_group_info->nNumRemovedProtons  += ret;*/
    t_group_info->tni.bNormalizationFlags |=
        (ret > 0) ? FLAG_PROTON_NP_HARD_REMOVED : 0;
  }
  ...
  if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] > 0) {
    ...
    ret = SimpleRemoveAcidicProtons(at, num_atoms, pAATG,
                                    pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]);
    if (IS_BNS_ERROR(ret)) {
      bError = ret;
      goto exit_function;
    ...
    /*t_group_info->nNumRemovedProtons  += ret;*/
    t_group_info->tni.bNormalizationFlags |=
        (ret > 0) ? FLAG_PROTON_AC_SIMPLE_REMOVED : 0;
    if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] > 0 && bAllowHardRemove) {
      /* (TECHMAN-5.2b) Hard removal */
      ret = HardRemoveAcidicProtons(at, num_atoms, pAATG,
                                    pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE],
      ...
      if (ret > 0) {
        ...
        /*t_group_info->nNumRemovedProtons  += ret;*/
        t_group_info->tni.bNormalizationFlags |=
            (ret > 0) ? FLAG_PROTON_AC_HARD_REMOVED : 0;
        nNumHardRemovedAcidicProtons += ret;
      }
    }
  } else if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] < 0) {
    ret = SimpleAddAcidicProtons(at, num_atoms, pAATG,
                                 -pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE]);
    if (IS_BNS_ERROR(ret)) {
      bError = ret;
      goto exit_function;
    ...
    /*t_group_info->nNumRemovedProtons  -= ret;*/
    /*
       CHECK_TACN == 1 prohibits replacing (-) on N with H unless H can be moved
       to N along an alternating path from another heteroatom (t-group will be
       detected).
    */
    t_group_info->tni.bNormalizationFlags |=
        (ret > 0) ? FLAG_PROTON_AC_SIMPLE_ADDED : 0;
    if (pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE] < 0 && bAllowHardRemove) {
      ret = HardAddAcidicProtons(at, num_atoms, pAATG,
                                 -pAATG->nAtTypeTotals[ATTOT_TOT_CHARGE],
                                 &nNumCanceledCharges, pBNS, pBD);
      if (IS_BNS_ERROR(ret)) {
        bError = ret;
      ...
      if (ret > 0) {
        ...
        /*t_group_info->nNumRemovedProtons  -= ret;*/
        t_group_info->tni.bNormalizationFlags |=
            (ret > 0) ? FLAG_PROTON_AC_HARD_ADDED : 0;
        nNumHardRemovedAcidicProtons -= ret;
      }
    }
  }
  t_group_info->tni.bNormalizationFlags |=
      nNumCanceledCharges ? FLAG_PROTON_CHARGE_CANCEL : 0;
...
/*                              */
/********************************/
int mark_alt_bonds_and_taut_groups(inp_ATOM *at, inp_ATOM *at_fixed_bonds_out,
                                   int num_atoms, T_GROUP_INFO *t_group_info,
...

{
  BN_STRUCT *pBNS = NULL;
  BN_DATA *pBD = NULL;
  int bError, nChanges, nTotChanges, taut_found, salt_found, taut_pass,
      salt_pass, salt_step, ret, ret2, num;
  ...
  INCHI_MODE *pbTautFlags =
      t_group_info ? &t_group_info->bTautFlags : inpbTautFlags;
  INCHI_MODE *pbTautFlagsDone =
      t_group_info ? &t_group_info->bTautFlagsDone : inpbTautFlagsDone;
  ...
  for (i = 0; i < num_atoms; i++) {
    if ((at[i].radical == RADICAL_DOUBLET) && (at[i].valence == 2) &&
        (at[i].bond_type[0] == BOND_ALTERN) &&
        (at[i].bond_type[1] == BOND_ALTERN)) {
      n_arom_radicals++;
      if (!stored_radicals) {
  ...
  if ((*pbTautFlags & TG_FLAG_MOVE_POS_CHARGES) && num_atoms > 1) {
    ...
    c_group_info->c_group = (C_GROUP *)inchi_calloc(
        num_atoms / 2, sizeof(c_group_info->c_group[0]));
    c_group_info->c_candidate = (C_CANDIDATE *)inchi_calloc(
        num_atoms, sizeof(c_group_info->c_candidate[0]));
    if (c_group_info->c_group && c_group_info->c_candidate) {
       ...
       * c_group_info->c_group, c_group_info->c_candidate);*/
      goto exit_function;
    }
  }
  ...
  if (*pbTautFlags & TG_FLAG_TEST_TAUT__SALTS) {
    if (t_group_info) {
      ...
      s_group_info->s_candidate = (S_CANDIDATE *)inchi_calloc(
          num_atoms, sizeof(s_group_info->s_candidate[0]));
  ...
  if (t_group_info) {
    ...
    t_group_info->tGroupNumber = (AT_NUMB *)inchi_calloc(
        2 * num_atoms + 1, sizeof(t_group_info->tGroupNumber[0]));
    if (!t_group_info->tGroupNumber) {
  ...
  again:
  */
  /* allocate Balanced Network Data Strucures; replace Alternating bonds with
   * Single */
  if ((pBNS =
           AllocateAndInitBnStruct(at, num_atoms, BNS_ADD_ATOMS, BNS_ADD_EDGES,
    ...
    if ((*pbTautFlags & TG_FLAG_VARIABLE_PROTONS) && t_group_info &&
      ...
      /*
         the structure is simple to neutralize if it yields exactly
           num[H(+)]     = num[N,P H(+)]
           num[N,S,O(-)] = num[=C-O(-)] + num[C-S(-)] + num[N(-)] + num[other
         O(-), S(-)]

           and n(p) = num[H(+)] - num[N,S,O(-)] (no protons, no negative N,O,S
         condition)
      ...
      if (t_group_info->tni.bNormalizationFlags) {
        ...
        if (at_fixed_bonds_out) {
          /* copy modified initial tautomeric structure for displaying
             Warning: implicit H counts in at_fixed_bonds_out include explicit
             Hs */
          memcpy(at_fixed_bonds_out, at,
                 nNumOrigTotAtoms * sizeof(at_fixed_bonds_out[0]));
          /* -- will be done in FillOutInputInfAtom() --
          RemoveExcessiveImplicitH( num_atoms,
          t_group_info->tni.nNumRemovedExplicitH, at_fixed_bonds_out );
          */
        }
      }
    }
...
#endif
        ret = MarkChargeGroups(at, num_atoms, c_group_info, t_group_info, pBNS,
                               pBD);
#if (RESET_EDGE_FORBIDDEN_MASK == 0)
        pBNS->edge_forbidden_mask &= ~BNS_EDGE_FORBIDDEN_TEMP;
#endif
    ...
    do {
      ...
      /**************** regular bond/H/(-)/positive charges tautomerism cycle
       * begin **************/
      do {
        taut_pass++;
        for (taut_found = 0;
             0 < (ret = MarkTautomerGroups(at, num_atoms, t_group_info,
        ...
        if (taut_found || salt_found) {
          ...
          ret = ReInitBnStructAddGroups(pBNS, at, num_atoms, t_group_info,
                                        c_group_info);
          if (IS_BNS_ERROR(ret)) {
            bError = ret;
            goto exit_function;
...
#if (BNS_RAD_SEARCH == 1)
#else
          /* discovered moveable charges and H-atoms may allow to cancel
           * radicals */
          if (pBNS->tot_st_cap > pBNS->tot_st_flow) {
            ret = BnsAdjustFlowBondsRad(pBNS, pBD, at, num_atoms);
            if (IS_BNS_ERROR(ret)) {
          ...
          if (*pbTautFlags & TG_FLAG_MOVE_POS_CHARGES) {
            ...
            do { /* cycling while ret>0 added 2004-06-04 */
              ...
              CorrectFixing_NH_NH_Bonds(pBNS, at, num_atoms);
#endif
              ret = MarkChargeGroups(at, num_atoms, c_group_info, t_group_info,
                                     pBNS, pBD);
#if (RESET_EDGE_FORBIDDEN_MASK == 0)
              pBNS->edge_forbidden_mask &= ~BNS_EDGE_FORBIDDEN_TEMP;
#endif
              ...
              nChanges += ret;
              if (ret > 0) {
                ret2 = ReInitBnStructAddGroups(pBNS, at, num_atoms,
           ...
           * **************/
          /************************************************************************/
          ret = BnsTestAndMarkAltBonds(pBNS, pBD, at, num_atoms, fcd,
                                       bChangeFlow, 0);
          if (IS_BNS_ERROR(ret)) {
            bError = ret;
            goto exit_function;
      ...
      if (*pbTautFlags & TG_FLAG_TEST_TAUT__SALTS) {
        ...
        if (*pbTautFlags & TG_FLAG_TEST_TAUT2_SALTS) {
          ...
          if (!salt_step) { /* salt step 0: process one attachment migrartion */
...
#endif
            salt_found =
                MarkSaltChargeGroups(at, num_atoms, s_group_info, t_group_info,
                                     c_group_info, pBNS, pBD);
#if (RESET_EDGE_FORBIDDEN_MASK == 0)
            pBNS->edge_forbidden_mask &= ~BNS_EDGE_FORBIDDEN_TEMP;
#endif
            ...
            /* if new 'salt' atoms have been found then repeat regular taut.
             * search MarkTautomerGroups() and do not perform salt step 1 if new
             * 'salt' atoms have NOT been found then switch to salt step 1 and
             * never repeat salt step 0 for the current structure
             */
          }
          if ( salt_step /*||
...
#endif
            salt_found =
                MarkSaltChargeGroups2(at, num_atoms, s_group_info, t_group_info,
                                      c_group_info, pBNS, pBD);
#if (RESET_EDGE_FORBIDDEN_MASK == 0)
            pBNS->edge_forbidden_mask &= ~BNS_EDGE_FORBIDDEN_TEMP;
#endif
    ...
    if (*pbTautFlags & TG_FLAG_MERGE_TAUT_SALTS) {
      if (!bError && s_group_info /*&& s_group_info->num_candidates > 0*/) {
        ret = MergeSaltTautGroups(at, num_atoms, s_group_info, t_group_info,
                                  c_group_info, pBNS);
        if (ret < 0) {
          bError = ret;
        } else if (ret > 0) {
    ...
    /* second time unknown ("Either") alternating bonds are treated as
     * non-stereogenic */
...
exit_function:
  ...
  if (!bError && num_atoms == 1 && at[0].at_type == ATT_PROTON &&
      t_group_info && !t_group_info->tni.nNumRemovedExplicitH) {
  ...
  */
  ...
  return bError ? bError
                : num_atoms; /* ret = 0 => success, any other => error */
}
...
int nMinFlow2Check(BN_STRUCT *pBNS, int iedge) {
  ...
  if (f12 > 0) {
    for (i = 0, rescap1 = 0; i < pBNS->vert[v1].num_adj_edges; i++) {
      ...
      if (iedge_i == iedge)
        continue;
      rescap1 += (pBNS->edge[iedge_i].cap & EDGE_FLOW_MASK) -
                 (pBNS->edge[iedge_i].flow & EDGE_FLOW_MASK);
    }
    for (i = 0, rescap2 = 0; i < pBNS->vert[v2].num_adj_edges; i++) {
      iedge_i = pBNS->vert[v2].iedge[i];
      if (iedge_i == iedge)
        continue;
      rescap2 += (pBNS->edge[iedge_i].cap & EDGE_FLOW_MASK) -
                 (pBNS->edge[iedge_i].flow & EDGE_FLOW_MASK);
    }
...
int bSetBondsAfterCheckOneBond(BN_STRUCT *pBNS, BNS_FLOW_CHANGES *fcd,
  ...
  bChangeFlow0 &=
      ~BNS_EF_CHNG_RSTR; /* do not change pEdge flow in SetBondType */
  if (!bChangeFlow0)
    return 0;
  ...
  for (ifcd -= 1; 0 <= ifcd; ifcd--) {
    ...
    v1 = pEdge->neighbor1;
    v2 = pEdge->neighbor12 ^ v1;
    if (v1 < num_atoms && v2 < num_atoms && bChangeFlow &&
        new_flow != pEdge->flow0) {
      ineigh1 = pEdge->neigh_ord[0];
      ineigh2 = pEdge->neigh_ord[1];
      ret_val = SetAtomBondType(pEdge, &at[v1].bond_type[ineigh1],
                                &at[v2].bond_type[ineigh2],
...
int bSetFlowToCheckOneBond(BN_STRUCT *pBNS, int iedge, int flow,
  ...
  if (f12 < flow) {
    /* Increase edge flow: Grab flow from the neighbors and delete it: set
     * flow12=cap12 = 0 */
    /************************************************************************************/
    /* For example, simulate a new fixed double bond in place of a single bond
     * and      */
    /* creates ONE or NONE (in case of a radical on adjacent atom) augmenting
     * paths and */
    ...
    if (f12 > 0) {
      /* remove old edge flow from the flow and cap of the adjacent vertices'
       * st-edges */
      pBNS->vert[v1].st_edge.cap =
          ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - f12) |
          (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
      pBNS->vert[v2].st_edge.cap =
          ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - f12) |
          (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
      pBNS->vert[v1].st_edge.flow =
          ((pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK) - f12) |
    ...
    st_edge_rescap = (pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) -
                     (pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK);
    while (st_edge_rescap && delta1) {
      st_edge_rescap--; /* grab the radical */
      delta1--;
      pBNS->vert[v1].st_edge.cap =
          ((pBNS->vert[v1].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
          (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
      nDots--;
    }
    /* grab the adjacent vertex2 radical (st_edge_rescap) if it exists */
    st_edge_rescap = (pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) -
                     (pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK);
    while (st_edge_rescap && delta2) {
      st_edge_rescap--; /* grab the radical */
      delta2--;
      pBNS->vert[v2].st_edge.cap =
          ((pBNS->vert[v2].st_edge.cap & EDGE_FLOW_ST_MASK) - 1) |
          (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
      nDots--;
    }
    ...
    for (i = 0; delta1 && i < pBNS->vert[v1].num_adj_edges; i++) {
      ...
      if (f) {
        ...
        while (f && delta1) {
          f--;
          delta1--;
          pEdge_i->flow = ((pEdge_i->flow & EDGE_FLOW_MASK) - 1) |
                          (pEdge_i->flow & ~EDGE_FLOW_MASK);
          pBNS->vert[v_i].st_edge.flow =
              ((pBNS->vert[v_i].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
          ...
          pBNS->vert[v1].st_edge.cap =
              ...
              (pBNS->vert[v1].st_edge.cap & ~EDGE_FLOW_ST_MASK);
          pBNS->vert[v1].st_edge.flow =
              ((pBNS->vert[v1].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
              (pBNS->vert[v1].st_edge.flow & ~EDGE_FLOW_ST_MASK);
      ...
      if (f) {
        ...
        while (f && delta2) {
          ...
          pBNS->vert[v2].st_edge.cap =
              ...
              (pBNS->vert[v2].st_edge.cap & ~EDGE_FLOW_ST_MASK);
          pBNS->vert[v2].st_edge.flow =
              ((pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK) - 1) |
              (pBNS->vert[v2].st_edge.flow & ~EDGE_FLOW_ST_MASK);
  ...
  if (f12 >= flow) {
    /* Decrease edge flow: Redirect flow to the neighbors and delete it on the
     * edge: set flow12=cap12 = 0 */
    /* f12==flow fixes flow through the edge so that BNS cannot change it */
    /**********************************************************************************************/
    /* For example, simulate a removal of a double bond and create ONE or NONE
     * augmenting path    */
    ...
    /* -- seem to be a bug --
    pBNS->vert[v1].st_edge.flow = ((pBNS->vert[v1].st_edge.flow &
    EDGE_FLOW_ST_MASK)-delta) | (pBNS->vert[v1].st_edge.flow &
    ~EDGE_FLOW_ST_MASK); pBNS->vert[v2].st_edge.flow =
    ((pBNS->vert[v2].st_edge.flow & EDGE_FLOW_ST_MASK)-delta) |
    (pBNS->vert[v2].st_edge.flow & ~EDGE_FLOW_ST_MASK);
    */
...
/**********************************************************************************/
/* Connect new (fictitious, temporary) vertex to to nVertDoubleBond by a new
 * edge */
...
int bAddNewVertex(BN_STRUCT *pBNS, int nVertDoubleBond, int nCap, int nFlow,
  ...
  if ((pBNS->vert[vlast].iedge - pBNS->iedge) +
          pBNS->vert[vlast].max_adj_edges + nMaxAdjEdges >=
      pBNS->max_iedges) {
...
int AddNewEdge(BNS_VERTEX *p1, BNS_VERTEX *p2, BN_STRUCT *pBNS, int nEdgeCap,
  ...
  /* debug: check bounds */
  if (ip1 >= pBNS->max_vertices || ip1 < 0 || ip2 >= pBNS->max_vertices ||
      ip2 < 0 || ie >= pBNS->max_edges || ie < 0 ||
  ...
  if (v1 < pBNS->num_atoms) {
    ...
    while (0 <= i) {
      ...
      if (pBNS->vert[v2].type == type) {
        return IS_FORBIDDEN(pEdge1->forbidden, pBNS) ? NO_VERTEX
                                                     : pVert1->iedge[i];
      }
      i--;
    }
...
int bAddStCapToAVertex(BN_STRUCT *pBNS, Vertex v1, Vertex v2,
  ...
  *nDots += 1;
  ...
  /* increment caps of adjacent edges if
     ...
     (2) (edge cap==0) OR (nSumEdgeCap < pVert1->st_edge.cap &&
     pVert->st_edge.flow > pVert1->st_edge.cap)
  */
...
int bSetBnsToCheckAltPath(BN_STRUCT *pBNS, int nVertDoubleBond,
  ...
  if (!pBNS->vert[nVertDoubleBond].st_edge.flow &&
    ...
    if ((*pBNS->pbTautFlags & TG_FLAG_TEST_TAUT2_SALTS) &&
        ...
        ALT_PATH_MODE_4_SALT2 == path_type && (BNS_VERT_TYPE_ENDPOINT & type)) {
      ...
      */
      for (i = 0, n = 0; i < pVert1->num_adj_edges; i++) {
        v = (pBNS->edge + pVert1->iedge[i])->neighbor12 ^
            v1; /* v is adjacent to v1 */
        if (v == v2)
      ...
      if (!bSet_v1) {
        ...
        n = bAddStCapToAVertex(pBNS, v1, v2Act, apc->nOldCapsVert[iapc], nDots,
                               0);
      ...
      if (!bSet_v2) {
        ...
        n = bAddStCapToAVertex(pBNS, v2t, v1Act, apc->nOldCapsVert[iapc], nDots,
                               0);
        apc->bSetOldCapsVert[iapc] = n;
    ...
    /*#endif*/ /*  ( TEST_REMOVE_S_ATOMS == 1 && ALT_PATH_MODE_4_SALT ==
                  path_type ) */
    /*****************************************************************/
    if (path_type == ALT_PATH_MODE_REM2H_CHG ||
      ...
      int bDonors = (path_type == ALT_PATH_MODE_REM2H_CHG) ||
                    (path_type == ALT_PATH_MODE_REM2H_TST);
      ...
      */
      ...
      for (i = 0, n = bAdjacentDonors = 0; i < pVert1->num_adj_edges; i++) {
        ...
        */
        ...
        bAdjacentDonors +=
            bDonors ? (v == v2) && ((pBNS->edge + pVert1->iedge[i])->flow <
                                    MAX_BOND_EDGE_CAP)
                    : 0;
      ...
      for (i = 0, n = bAdjacentDonors = 0; i < pVert2->num_adj_edges; i++) {
        v = (pBNS->edge + pVert2->iedge[i])->neighbor12 ^
            v2; /* v is adjacent to v2 */
        /* do not ignore connection to v1
        if ( v == v1 )
            continue;
         */
        n += bDonors ? (pBNS->vert[v].st_edge.cap > 0)
                     : ((pBNS->edge + pVert2->iedge[i])->flow > 0);
        bAdjacentDonors +=
            bDonors ? (v == v1) && ((pBNS->edge + pVert2->iedge[i])->flow <
                                    MAX_BOND_EDGE_CAP)
                    : 0;
        /* two donors connected by a single or double bond */
      }
      ...
      if (!bSet_v1) {
        ...
        n = bAddStCapToAVertex(pBNS, v1Act, v2Act, apc->nOldCapsVert[iapc],
                               nDots, bAdjacentDonors);
        apc->bSetOldCapsVert[iapc] = n;
      ...
      if (!bSet_v2) {
        ...
        n = bAddStCapToAVertex(pBNS, v2Act, v1Act, apc->nOldCapsVert[iapc],
                               nDots, bAdjacentDonors);
        apc->bSetOldCapsVert[iapc] = n;
    ...
    if (path_type == ALT_PATH_MODE_REM_PROTON) { /* added 2004-03-05 */
      if (v1 >= 0 && v2 >= 0 && (pVert1->type & BNS_VERT_TYPE_ANY_GROUP) &&
    ...
    *nDots = 0;
    ...
    if (type & BNS_VERT_TYPE_ENDPOINT) {
      ...
      if (iedge != NO_VERTEX) {
        /*  set flow=1 on an edge to a c-group vertex to make sure there is no
         * positive charge when moving tautomeric H-atoms
         */
      ...
      if (iedge != NO_VERTEX) {
         ...
         *  no moveable hydrogen atom or (-) on v2 when moving tautomeric
         * H-atoms
         */
#if (FIX_H_CHECKING_TAUT == 1)
      ...
      if (t1 != NO_VERTEX) {
        ...
        vNew = bAddNewVertex(pBNS, t1, 1 /*cap*/, 0 /*flow*/,
                             1 /*max_adj_edges*/, nDots);
        if (IS_BNS_ERROR(vNew)) {
          return vNew;
        }
        apc->vNewVertex[iapc] = vNew;
      ...
      if (t2 == NO_VERTEX) {
        /* create new edge and vertex, connect to v2 */
        vNew = bAddNewVertex(pBNS, v2, 1 /*cap*/, 0 /*flow*/,
                             1 /*max_adj_edges*/, nDots);
        if (IS_BNS_ERROR(vNew)) {
          return vNew;
        }
      ...
      if (t1 == NO_VERTEX) {
        /* add st-cap to v1 */
        n = bAddStCapToAVertex(pBNS, v1, (Vertex)(t2 == NO_VERTEX ? v2 : t2),
                               apc->nOldCapsVert[iapc], nDots, 0);
        apc->bSetOldCapsVert[iapc] = n;
        apc->vOldVert[iapc] = v1;
        iapc++;
      }
      if (t2 != NO_VERTEX) {
        /* add st-cap to t2 */
        n = bAddStCapToAVertex(pBNS, t2, (Vertex)(t1 == NO_VERTEX ? v1 : t1),
                               apc->nOldCapsVert[iapc], nDots, 0);
        apc->bSetOldCapsVert[iapc] = n;
...
int bRestoreBnsAfterCheckAltPath(BN_STRUCT *pBNS, ALT_PATH_CHANGES *apc,
                                 int bChangeFlow)
/* int nVertDoubleBond, int nVertSingleBond, int nNewVertex, AT_NUMB
  ...
  if (bChangeFlow & BNS_EF_UPD_H_CHARGE) {
    ...
    for (i = sizeof(apc->bSetNew) / sizeof(apc->bSetNew[0]) - 1; 0 <= i; i--) {
      if (apc->bSetNew[i]) {
        ...
        /* and decrement the total number of vertices (new vertice ids are
         * contiguous */
        pBNS->num_vertices--;
        ret++;
      }
...
/**********************************************************************************/
int bExistsAnyAltPath(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at,
                      int num_atoms, int nVert2, int nVert1, int path_type) {
  int nRet1, nRet2;
  nRet1 =
      bExistsAltPath(pBNS, pBD, NULL, at, num_atoms, nVert2, nVert1, path_type);
  if (nRet1 > 0)
    return nRet1;
  nRet2 =
      bExistsAltPath(pBNS, pBD, NULL, at, num_atoms, nVert1, nVert2, path_type);
  if (nRet2 > 0)
    return nRet2;
...
int bIsBnsEndpoint(BN_STRUCT *pBNS, int v) {
  ...
  if (0 <= v && v < pBNS->num_atoms && (pVert = pBNS->vert + v) &&
      (pVert->type & BNS_VERT_TYPE_ENDPOINT)) {
...
int bRadChangesAtomType(BN_STRUCT *pBNS, BN_DATA *pBD, Vertex v, Vertex v_1,
  ...
  if (pBNS->vert[pBNS->edge[pBNS->vert[v_O].iedge[1]].neighbor12 ^ v_O].type &
      BNS_VERT_TYPE_ATOM) {
    return 0;
  }
  /* the next to previous vertex vertex along the path: should be a Charge or
   * Taut group vertex */
  if (v_2 == NO_VERTEX) {
  ...
  if (pBNS->vert[v_ChgOrH].type &
      (BNS_VERT_TYPE_TGROUP | BNS_VERT_TYPE_C_GROUP))
    return 1;
  return 0;
}
...
int RegisterRadEndpoint(BN_STRUCT *pBNS, BN_DATA *pBD, Vertex u) {
  ...
  case RAD_SRCH_NORM:
    /* go backwards along alt path and stop at the 1st found atom (not a
     * fictitious vertex) */
    /* we need only vertices where a radical may be moved, therefore exclude
     * u%2=1 (odd) vertices */
    /* atom number = u/2-1; u = 0 or 1 is 's' or 't' vertices, respectively,
     * they are not atoms  */
    num_found = 0;
    ...
    if (Vertex_t < u && w < pBNS->num_atoms &&
        pBNS->vert[w].st_edge.cap ==
            (pBNS->vert[w].st_edge.flow & EDGE_FLOW_ST_MASK)) {
      /* u is an atom; it is not a radical atom */
      /* now search for the starting radical atom by following the path back
       * from u */
      v = u_last = u;
      while (v > Vertex_t) {
        u = v;
        v = GetPrevVertex(pBNS, u, pBD->SwitchEdge,
                          &iuv); /* Radical endpoint */
      }
      /* check whether u is a radical atom */
      if (!(u % 2) && Vertex_t < u && (u = u / 2 - 1) < pBNS->num_atoms &&
          pBNS->vert[u].st_edge.cap >
              (pBNS->vert[u].st_edge.flow & EDGE_FLOW_ST_MASK)) {
        /* at pBNS->vert[u] we have found the radical that originated the path
         */
        /* pBD->RadEndpoints[2k] is the radical, pBD->RadEndpoints[2k+1] is the
         * farthest atom */
        /* to which the radical may be moved (farthest reachable atom) */

        /* add *all* atoms that may receive radical from u_rad */
        /* exception: at2 in: ==(+/-/H)---at1==at2(possible rad endpoint) if
         * pBNS->type_TACN */

        for (v = u_last; v > Vertex_t;
             v = GetPrevVertex(pBNS, v, pBD->SwitchEdge, &iuv)) {
          if (!(v % 2) && (v2 = v / 2 - 1) < pBNS->num_atoms &&
            ...
            for (i = 0; i < pBD->nNumRadEndpoints; i += 2) {
              ...
              if (u == pBD->RadEndpoints[i] && v2 == pBD->RadEndpoints[i + 1]) {
                break;
              }
            }
  ...
  case RAD_SRCH_FROM_FICT:
    ...
    while (v > Vertex_t) {
      ...
      if (!(v % 2) && v / 2 <= pBNS->num_atoms &&
          pBNS->vert[v / 2 - 1].st_edge.cap -
                  pBNS->vert[v / 2 - 1].st_edge.flow <
              2) {
        w = v; /* vertex w is atom that may be singlet or doublet but not
                  triplet */
      }
      v = GetPrevVertex(pBNS, u, pBD->SwitchEdge, &iuv);
    }
    v = u / 2 - 1; /* vertex u may be the radical from which the path
                      originated; w is the nearest atom */
    if (w == NO_VERTEX || u == NO_VERTEX || w % 2 || u == w ||
        v < pBNS->num_atoms ||
    ...
    u = v;
    /* at pBNS->vert[u] we have found the radical that originated the path, w is
     * the nearest atom */
    for (i = 0; i < pBD->nNumRadEndpoints; i += 2) {
      if (u == pBD->RadEndpoints[i] && w == pBD->RadEndpoints[i + 1]) {
...
int RemoveRadEndpoints(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at) {
  ...
  for (i = pBD->nNumRadEdges - 1; 0 <= i; i--) {
    ...
    if (ie + 1 != pBNS->num_edges || v1 < 0 || v1 >= pBNS->num_vertices ||
...
int RestoreRadicalsOnly(BN_STRUCT *pBNS, BN_DATA *pBD, inp_ATOM *at) {
  ...
  for (i = pBD->nNumRadEdges - 1; 0 <= i; i--) {
    ...
    v1 = e->neighbor1;       /* atom */
    v2 = e->neighbor12 ^ v1; /* v2 > v1 <=> v2 was added later */
    if (v1 < 0 || v1 >= pBNS->num_atoms || v2 < pBNS->num_atoms ||
        v2 >= pBNS->num_vertices) {
      goto error_exit;
    }
    p1 = pBNS->vert + v1;
    p2 = pBNS->vert + v2;
...
int SetRadEndpoints(BN_STRUCT *pBNS, BN_DATA *pBD, BRS_MODE bRadSrchMode) {
  ...
  if (!ret && pBD->nNumRadEndpoints >= 2) {
    ...
    qsort(pBD->RadEndpoints, pBD->nNumRadEndpoints / 2,
          2 * sizeof(pBD->RadEndpoints[0]), cmp_rad_endpoints);
    num_new_edges = 0;
    nNumRadicals = 0;
    ...
    for (i = 0; i < pBD->nNumRadEndpoints; i = j) {
      ...
      nNumEdges = 0;
      for (j = i; j < pBD->nNumRadEndpoints && wRad == pBD->RadEndpoints[j];
           j += 2) {
        nNumEdges++;
      }
    ...
    /* all vRad vertex indices should be in the range
     * vFirstNewVertex...vFirstNewVertex+nNumRadicals-1 */
    /* connect new vertices to the radical endpoints thus replacing radicals
     * with even-length alternating cycles */
    for (i = 0; i < pBD->nNumRadEndpoints; i = j) {
      vRad = pBD->RadEndpoints[i];
      pRad = pBNS->vert + vRad;
      for (j = i; j < pBD->nNumRadEndpoints && vRad == pBD->RadEndpoints[j];
           j += 2) {
...
int SetRadEndpoints2(BN_STRUCT *pBNS, BN_DATA *pBD, BRS_MODE bRadSrchMode) {
  ...
  /* save radical atom numbers in vRadList[] and remove radical by making
   * st_cap=st_flow */
  for (i = 0; i < pBNS->num_atoms; i++) {
    if (pBNS->vert[i].st_edge.cap - delta ==
        (pBNS->vert[i].st_edge.flow & EDGE_FLOW_ST_MASK)) {
      if (nNumRad < MAX_NUM_RAD) {
        pBNS->vert[i].st_edge.cap -= delta;
        pBNS->tot_st_cap -= delta;
        vRadList[nNumRad] =
            i; /* radical position; i > j <=> vRadList[i] > vRadList[j]  */
        vRadEqul[nNumRad] =
            nNumRad; /* the smallest radical atom that has reachable
  ...
  for (j = 0; j < nNumRad; j++) {
    ...
    pBNS->tot_st_cap += delta;
    ret = BalancedNetworkSearch(pBNS, pBD,
                                BNS_EF_RAD_SRCH); /* find reachable atoms */
    ReInitBnData(pBD);
    ReInitBnStructAltPaths(pBNS);
    pBD->bRadSrchMode = RAD_SRCH_NORM;
    ...
    if (IS_BNS_ERROR(ret)) {
      ...
      ret = BNS_RADICAL_ERR; /* found augmenting path: should not happen since
                                only one radical was enabled */
      goto error_exit;
    }
    if (!ret && pBD->nNumRadEndpoints >= 2) {
      ...
      qsort(pBD->RadEndpoints, pBD->nNumRadEndpoints / 2,
            2 * sizeof(pBD->RadEndpoints[0]), cmp_rad_endpoints);
      if (pBD->RadEndpoints[0] != i ||
          pBD->RadEndpoints[pBD->nNumRadEndpoints - 2] != i) {
        ret = BNS_RADICAL_ERR; /* more than one radical vertex */
        goto error_exit;
      }
      if (nNumRad > 1) {
        /* if more than one radical then save reachable atoms in bitmaps to
         * allow */
        /* faster finding whether same atoms are reachable by two or more
         * radicals */
        ...
        NodeSetFromRadEndpoints(&VertSet, j, pBD->RadEndpoints,
  ...
  /* defect: if vertex sets i and j do not intersect they will be compared 2
   * times */
        ...
        if (!IsNodeSetEmpty(&VertSet, i)) {
          ...
          for (j = i + 1; j < nNumRad; j++) {
            if (i == vRadEqul[j]) {
              ...
              pBD->RadEndpoints[n++] =
                  -vRadList[j] - 2; /* equivalent radical, alvays not zero */
            }
          }
          /* store endpoints */
          n = AddNodesToRadEndpoints(&VertSet, i, pBD->RadEndpoints,
                                     vRadList[i], n, pBD->max_len_Pu_Pv);
          if (n < 0) {
          ...
          pBD->RadEndpoints[n++] = vRadList[i];
          pBD->RadEndpoints[n++] =
              -1; /* immobile radical, only one edge to add */
        }
      }
    }
  ...
  if (!ret && pBD->nNumRadEndpoints >= 2) {
     ...
     *
     *   The "equivalent radical centers" (which have at least one reachable
     *atom in common) are connected to (*) with "double bonds" (edge cap=1,
     *flow=1). Reachable non-radical atoms are connected by edges with cap=1,
     *flow=0 After running BNS to find alt.path a "double bond" from (*) may
     *move to another atom thus muving the radical.
     *
    ...
    for (i = 0; i < pBD->nNumRadEndpoints; i = j) {
      ...
      for (j = i; j < pBD->nNumRadEndpoints && wRad == pBD->RadEndpoints[j];
           j += 2) {
        nNumEdges += (pBD->RadEndpoints[j + 1] !=
                      -1); /* immobile radicals have one edge only */
      }
      /* add new aux vertex to the radical atom/vertex making st_cap-st_flow=0
       */
      /* in case of immobile radical there will be no additional eddges since
       * nNumEdges=0 */
      vRad = bAddNewVertex(pBNS, wRad, delta, delta, nNumEdges + 1, &nDots);
      if (IS_BNS_ERROR(vRad)) {
        ret = vRad;
...
*/
int bExistsAltPath(BN_STRUCT *pBNS, BN_DATA *pBD, BN_AATG *pAATG, inp_ATOM *at,
                   int num_atoms, int nVertDoubleBond, int nVertSingleBond,
                   int path_type) {
  ALT_PATH_CHANGES apc;
  int ret, ret_val, bError, bSuccess, bChangeFlow = 0, nDots, nDelta,
                                      bDoMarkChangedBonds = 1;
  ...
  case ALT_PATH_MODE_TAUTOM:
    /* Check for alt path allowing to move H and (-). Purpose: confirm possible
     * tautomerism */
  ...
  case ALT_PATH_MODE_4_SALT2:
    ...
    type = BNS_VERT_TYPE_ENDPOINT;
    bChangeFlow = (BNS_EF_CHNG_RSTR | BNS_EF_ALTR_BONDS);
    if (!bIsBnsEndpoint(pBNS,
                        nVertSingleBond) /* !at[nVertSingleBond].endpoint*/
        && (!nGetEndpointInfo(at, nVertSingleBond, &eif) || !eif.cDonor))
      return 0;
    if (!bIsBnsEndpoint(pBNS,
                        nVertDoubleBond) /* !at[nVertDoubleBond].endpoint*/
        && (!nGetEndpointInfo(at, nVertDoubleBond, &eif) || !eif.cAcceptor))
      return 0;
    memset(&apc, 0, sizeof(apc));
    break;
  ...
  case ALT_PATH_MODE_REM2H_TST:
    ...
    if (!bIsBnsEndpoint(
            pBNS,
            nVertSingleBond) /* not linked to a t-group or the edge forbidden */
        && (!nGetEndpointInfo(at, nVertSingleBond, &eif) ||
            !eif.cDonor)) /* not a donor */
      return 0;
  ...
  case ALT_PATH_MODE_ADD2H_TST:
    ...
    if (!bIsBnsEndpoint(pBNS,
      ...
      return 0;
    if (!bIsBnsEndpoint(pBNS,
                        nVertDoubleBond) /* !at[nVertSingleBond].endpoint*/
        && (!nGetEndpointInfo(at, nVertDoubleBond, &eif) || !eif.cAcceptor))
      return 0;
    break;
  ...
  case ALT_PATH_MODE_REM_PROTON:
    ...
    bChangeFlow = (BNS_EF_SAVE_ALL | BNS_EF_UPD_H_CHARGE) |
                  BNS_EF_ALTR_NS; /* added BNS_EF_ALTR_NS: set non-stereo altern
                                     non-ring bonds 2004-07-02*/
    break;
  default:
    type = 0;
    bChangeFlow = BNS_EF_CHNG_RSTR;
  ...
  ret = bSetBnsToCheckAltPath(pBNS, nVertDoubleBond, nVertSingleBond, type,
                              path_type, &apc, fcd, &nDots);
  switch (ret) {
  ...
  bAdjustRadicals =
      ((bChangeFlow & BNS_EF_UPD_RAD_ORI) && !(bChangeFlow & BNS_EF_RSTR_FLOW));
  ...
  if (IS_BNS_ERROR(ret)) {
    ...
    if (2 * ret >= nDots) {
      nDelta = 2 * ret - nDots; /* non-zero means augmentation created another
                                   alt. path -- between radicals */
      if (pAATG && pAATG->nMarkedAtom) {
        if (pAATG->nAtTypeTotals && (bChangeFlow & BNS_EF_UPD_H_CHARGE)) {
          memset(pAATG->nMarkedAtom, 0,
                 num_atoms * sizeof(pAATG->nMarkedAtom[0]));
          /* mark atoms that have charge or H changed, check their input types
             (that is, before changes), and subtract their input charge/H from
             nAtTypeTotals */
          SubtractOrChangeAtHChargeBNS(pBNS, at, num_atoms,
                                       pAATG->nAtTypeTotals, pAATG->nMarkedAtom,
                                       NULL, 1);
          /* ZChange charges and/or H, update t_group_info, do not check types
           * or change nAtTypeTotals */
          /* Atom types will be checked and nAtTypeTotals will be changed in
             AddChangedAtHChargeBNS() later */
          SubtractOrChangeAtHChargeBNS(pBNS, at, num_atoms, NULL, NULL,
                                       pAATG->t_group_info, 0);
        } else if (!pAATG->nAtTypeTotals) {
          bDoMarkChangedBonds = MarkAtomsAtTautGroups(
              pBNS, num_atoms, pAATG, nVertSingleBond, nVertDoubleBond);
      ...
      if (bDoMarkChangedBonds) {
        ...
        ret_val = bSetBondsAfterCheckOneBond(pBNS, fcd, -1, at, num_atoms,
                                             bChangeFlow);
        if (IS_BNS_ERROR(ret_val)) {
          bError = ret_val;
        }
        ...
        if (IS_BNS_ERROR(ret)) {
          ...
          bSuccess = 1;
                   ...
                   (bChangeFlow & BNS_EF_UPD_H_CHARGE)) {
          /* some bonds have been changed to alternating */
          bSuccess = 3;
        } else {
          bError = BNS_BOND_ERR;
        }
        if (!bError && pAATG && pAATG->nMarkedAtom &&
            (bChangeFlow & BNS_EF_UPD_H_CHARGE)) {
          /* Update radicals to avoid errors in atom type check in
          ...
          /* Check atom types of marked atoms and add charge/H changes to
           * nAtTypeTotals */
          /* Changing atoms were marked in the 1st call to
           * SubtractOrChangeAtHChargeBNS(..., 1) above */
          AddChangedAtHChargeBNS(at, num_atoms, pAATG->nAtTypeTotals,
                                 pAATG->nMarkedAtom);
...
/*****************************************************************************/
BN_STRUCT *AllocateAndInitBnStruct(inp_ATOM *at, int num_atoms,
  ...
  int i, j, k, n_edges, num_bonds, num_edges, f1, f2, edge_cap, edge_flow,
      st_cap, st_flow, flag_alt_bond;
   ...
   */
  /* number of atoms is large enough to accommodate max. possible number of
   * t-groups + nMaxAddAtoms */
  ...
  max_edges =
      num_edges + (nMaxAddEdges + NUM_KINDS_OF_GROUPS) * max_vertices + max_tg;
#if (BNS_RAD_SEARCH == 1)
  if (num_rad) {
    max_vertices *= 2;
    max_edges *= 2;
  ...
  len_alt_path = max_vertices + iALTP_HDR_LEN +
                 1; /* may overflow if an edge is traversed in 2 directions */

  if (!(pBNS = (BN_STRUCT *)inchi_calloc(1, sizeof(BN_STRUCT))) ||
      !(pBNS->edge = (BNS_EDGE *)inchi_calloc(max_edges, sizeof(BNS_EDGE))) ||
      !(pBNS->vert =
            (BNS_VERTEX *)inchi_calloc(max_vertices, sizeof(BNS_VERTEX))) ||
      !(pBNS->iedge =
            (BNS_IEDGE *)inchi_calloc(max_iedges, sizeof(BNS_IEDGE)))) {
    return DeAllocateBnStruct(pBNS);
  }
  /* alt path init */
  for (num_altp = 0; num_altp < max_altp && num_altp < BN_MAX_ALTP;
       num_altp++) {
    if (!(pBNS->altp[num_altp] = (BNS_ALT_PATH *)inchi_calloc(
              len_alt_path, sizeof(BNS_ALT_PATH)))) {
      return DeAllocateBnStruct(pBNS);
    }
  ...
  for (i = 0; i < num_atoms; i++) {
    k = pBNS->vert[i].max_adj_edges =
        at[i].valence + (nMaxAddEdges + NUM_KINDS_OF_GROUPS);
  ...
  /*
     ...
     In atoms i = 0..pBNS->num_atoms-1
          pBNS->vert[i].num_adj_edges = pBNS->vert[i].max_adj_edges -
     pBNS->nMaxAddEdges - NUM_KINDS_OF_GROUPS; pBNS->num_vertices    =
     pBNS->num_atoms; pBNS->num_edges       = pBNS->num_bonds;
     pBNS->num_added_atoms = 0;
     pBNS->num_t_groups    = 0;
  ...
  */
  ...
  for (i = 0, n_edges = 0; i < num_atoms; i++) {
    ...
    for (j = 0; j < at[i].valence; j++) {
      ...
      if (neigh > i) {
        ...
        if (edge_flow > MAX_BOND_EDGE_CAP) {
          ...
          edge_cap = inchi_min(
              edge_cap,
              MAX_BOND_EDGE_CAP); /* max capacity = 2 means up to triple bond */
        }
    ...
    vert->st_edge.cap = vert->st_edge.cap0 = MAX_AT_FLOW(at[i]);
    vert->st_edge.flow = vert->st_edge.flow0 = st_flow;
...
/*****************************************************************************/
int ReInitBnStructAddGroups(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                            T_GROUP_INFO *tgi, C_GROUP_INFO *cgi) {
  int ret;
...
/*****************************************************************************/
int ReInitBnStruct(BN_STRUCT *pBNS, inp_ATOM *at, int num_at,
    ...
    if (pBNS->vert && pBNS->edge) {
      ...
      for (vfict = pBNS->num_atoms; vfict < pBNS->num_vertices; vfict++) {
        for (kfict = 0; kfict < pBNS->vert[vfict].num_adj_edges; kfict++) {
          iedgefict = pBNS->vert[vfict]
                          .iedge[kfict]; /* fictitious edge to the endpoint */
          endpoint =
              pBNS->edge[iedgefict].neighbor12 ^ vfict; /* the endpoint */
...
#endif
            pBNS->vert[centerpoint].st_edge.cap =
                pBNS->vert[centerpoint].st_edge.cap0;
            pBNS->vert[centerpoint].st_edge.flow =
                pBNS->vert[centerpoint].st_edge.flow0;
          }
          pBNS->vert[endpoint].st_edge.cap = pBNS->vert[endpoint].st_edge.cap0;
          pBNS->vert[endpoint].st_edge.flow =
              pBNS->vert[endpoint].st_edge.flow0;
...
/*****************************************************************************/
int CompTGroupNumber(const void *tg1, const void *tg2) {
  return (int)((const T_GROUP *)tg1)->nGroupNumber -
         (int)((const T_GROUP *)tg2)->nGroupNumber;
}

/*****************************************************************************/
int CompCGroupNumber(const void *cg1, const void *cg2) {
  return (int)((const C_GROUP *)cg1)->nGroupNumber -
         (int)((const C_GROUP *)cg2)->nGroupNumber;
}

/*****************************************************************************/
int AddTGroups2BnStruct(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                        T_GROUP_INFO *tgi) {
  int ret = 0;
  /* ret = ReInitBnStruct( pBNS ); */
  if (tgi && tgi->num_t_groups && tgi->t_group) {
    ...
    /* since t-group IDs may be not contiguous, clear all vertices that will be
       added. all-zeroes-vertex will be ignored by the BNS
    */
    memset(pBNS->vert + num_vertices, 0,
           nMaxTGroupNumber * sizeof(pBNS->vert[0]));
    /* Make sure the last t-group has the largest t-group ID:
       this is necessary to correctly add new edges and vertices for testing
       augmenting paths
    */
#if (bRELEASE_VERSION != 1)
    insertions_sort(tgi->t_group, num_tg, sizeof(tgi->t_group[0]),
                    CompTGroupNumber);
    for (i = 1; i < num_tg; i++) {
      if (1 !=
          tgi->t_group[i].nGroupNumber - tgi->t_group[i - 1].nGroupNumber) {
        return BNS_BOND_ERR;
      }
    }
#else
    ...
    for (i = 0; i < num_tg; i++, ver_ficpont_prev = vert_ficpoint) {
      ...
      */
      vert_ficpoint =
          pBNS->vert + num_vertices + tgi->t_group[i].nGroupNumber - 1;
      vert_ficpoint->iedge =
          ver_ficpont_prev->iedge + ver_ficpont_prev->max_adj_edges;
      vert_ficpoint->max_adj_edges =
          tgi->t_group[i].nNumEndpoints + BNS_ADD_EDGES + BNS_ADD_SUPER_TGROUP;
      vert_ficpoint->num_adj_edges = 0;
      vert_ficpoint->st_edge.flow = vert_ficpoint->st_edge.flow0 = 0;
      vert_ficpoint->st_edge.cap = vert_ficpoint->st_edge.cap0 = 0;
      vert_ficpoint->type = BNS_VERT_TYPE_TGROUP;
    ...
    for (endpoint = 0; endpoint < num_atoms; endpoint++) {
      ...
      for (k = 0; k < vert_endpoint->num_adj_edges; k++) {
        ...
        if (!pBNS->edge[iedge].cap) {
          ...
          if (centerpoint < pBNS->num_atoms &&
            ...
            if (bond_type == BOND_TAUTOM || bond_type == BOND_ALTERN ||
                bond_type == BOND_ALT12NS || bond_type == BOND_SINGLE) {
              pBNS->edge[iedge].cap = 1;
            }
          }
        }
      ...
      edge->forbidden &= pBNS->edge_forbidden_mask;
#endif
      /* later include case when the charge change allows the endpoint to become
       * tautomeric */
...
/*****************************************************************************/
int AddCGroups2BnStruct(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
                        C_GROUP_INFO *cgi) {
  int ret = 0;
  /* ret = ReInitBnStruct( pBNS ); */
  if (cgi && cgi->num_c_groups && cgi->c_group) {
    ...
    */
    memset(pBNS->vert + num_vertices, 0,
           nMaxCGroupNumber * sizeof(pBNS->vert[0]));
    /* Make sure the last t-group has the largest t-group ID:
       this is necessary to correctly add new edges and vertices for testing
       augmenting paths
    */
#if (bRELEASE_VERSION != 1)
    insertions_sort(cgi->c_group, num_cg, sizeof(cgi->c_group[0]),
                    CompCGroupNumber);
    for (i = 1; i < num_cg; i++) {
      if (1 !=
          cgi->c_group[i].nGroupNumber - cgi->c_group[i - 1].nGroupNumber) {
        return BNS_BOND_ERR;
      }
    }
#else
    ...
    for (i = 0; i < num_cg; i++, ver_ficpont_prev = vert_ficpoint) {
      ...
      */
      vert_ficpoint =
          pBNS->vert + num_vertices + cgi->c_group[i].nGroupNumber - 1;
      vert_ficpoint->iedge =
          ver_ficpont_prev->iedge + ver_ficpont_prev->max_adj_edges;
      vert_ficpoint->max_adj_edges =
          cgi->c_group[i].num_CPoints + BNS_ADD_EDGES;
      vert_ficpoint->num_adj_edges = 0;
      vert_ficpoint->st_edge.flow = vert_ficpoint->st_edge.flow0 = 0;
      vert_ficpoint->st_edge.cap = vert_ficpoint->st_edge.cap0 = 0;
    ...
    for (c_point = 0; c_point < num_atoms; c_point++) {
      if (!at[c_point].c_point)
        continue;
      fictpoint =
          at[c_point].c_point + num_vertices - 1; /* c-group vertex index */
      vert_ficpoint = pBNS->vert + fictpoint;     /* c-group vertex */
      vertex_cpoint = pBNS->vert + c_point;       /* c_point vertex */
      /* Debug: check overflow */
      if (fictpoint >= pBNS->max_vertices || num_edges >= pBNS->max_edges ||
      ...
      /* set capacity = 1 to the edges from the c_point to the centerpoint(s) */
      /* if their current capacity is zero */
      /* the centerpoint is any adjacent atom that is adjacent to a multiple
       * bond */
      for (k = 0; k < vertex_cpoint->num_adj_edges; k++) {
        int iedge = vertex_cpoint->iedge[k];
        if (!pBNS->edge[iedge].cap) {
...
#endif /* } FIX_CPOINT_BOND_CAP */
      /* create a new edge connecting c_point to the new fictitious c-group
       * vertex vert_ficpoint */
      edge = pBNS->edge + num_edges;
      edge->cap = 1;
      edge->flow = 0;
...
BN_DATA *AllocateAndInitBnData(int max_num_vertices) {
  ...
  if (!(pBD = (BN_DATA *)inchi_calloc(1, sizeof(BN_DATA))) ||
      !(pBD->BasePtr =
            (Vertex *)inchi_calloc(max_num_vertices, sizeof(Vertex))) ||
      !(pBD->SwitchEdge =
            (Edge *)inchi_calloc(max_num_vertices, sizeof(Edge))) ||
...
#if (BNS_RAD_SEARCH == 1)
      !(pBD->RadEndpoints =
            (Vertex *)inchi_calloc(max_len_Pu_Pv, sizeof(Vertex))) ||
      !(pBD->RadEdges =
            (EdgeIndex *)inchi_calloc(max_len_Pu_Pv, sizeof(EdgeIndex))) ||
#endif
  ...
  if (pBD) {
    ...
    if (!ret) {
      for (i = 0; i <= pBD->QSize; i++) {
        u = pBD->ScanQ[i];
        v = prim(u);
        pBD->BasePtr[u] = pBD->BasePtr[v] = pBD->SwitchEdge_Vert1(u) =
            pBD->SwitchEdge_Vert1(v) = NO_VERTEX;
        pBD->Tree[u] = pBD->Tree[v] = TREE_NOT_IN_M;
      }
    }
    pBD->QSize = -1;
    if (!pBD->Pu) {
...
Vertex Get2ndEdgeVertex(BN_STRUCT *pBNS, Edge uv) {
  ...
  */
  if (uv[1] >= 0) {
    ...
    /*short t = 2*(((u / 2 - 1) ^ pBNS->edge[uv[1]].neighbor12) + 1) + ((u+1) &
     * 1) + FIRST_INDX; */
    /*return t; */
  }
  if (uv[0] <= 1)
    ...
    return uv[0] % 2; /* vertex1 is x or y, return s or t; never happens? -- NSC
                         3737, 7634,... */
}
  ...
  if ((i = v - 2) >= 0) {
    ...
    if (neigh) {
      ...
      *iedge = pBNS->vert[i / 2].iedge[neigh];
      if (!(pBNS->edge[*iedge].cap & EDGE_FLOW_MASK) ||
          IS_FORBIDDEN(pBNS->edge[*iedge].forbidden, pBNS)) {
        return NO_VERTEX;
      }
      neighbor = (i ^ (2 * pBNS->edge[*iedge].neighbor12 + 1)) +
                 2; /* parity opposite to v parity */
    } else {
    ...
    if (s_or_t) {
      ...
      if (!delta) {
        ...
        if (f < 0 || f > cap) {
          ...
          if (!(bChangeFlow & BNS_EF_CHNG_FLOW)) {
            f -= delta; /* do not actually change the flow, only find the
                           augmenting path */
          } else if (delta) {
          ...
          if (bReverse) {
            /* u <- v; Note: in case of bReverse s_or_t has actually been
               determined for the u' <- v' pair; therefore s and t should be
               switched in order to correctly determine the 1st or the last atom
      ...
      if (!delta) {
        ...
        if (f < 0 || f > pedge->cap) {
          ...
          if (ALTP_MAY_ADD(pBNS->alt_path)) {
            /* bReverse? u <- v : u -> v */
            indx =
                bReverse ? (pedge->neighbor1 == iv) : (pedge->neighbor1 == iu);
            ALTP_CUR_THIS_ATOM_NEIGHBOR(pBNS->alt_path) =
                pedge->neigh_ord[1 - indx];
            ALTP_CUR_NEXT_ATOM_NEIGHBOR(pBNS->alt_path) =
                pedge->neigh_ord[indx];
  ...
  if (!IS_BNS_ERROR(bBackward)) {
    ...
    if (s_or_t) {
      ...
      if (flow & EDGE_FLOW_ST_PATH) {
        ...
        f /= 2; /* this is the second time we pass the same edge: reduce flow by
                   a factor of 2 */
      } else {
...
/*****************************************************************************
 *	Get previous vertex in the searched path * z is SwitchEdge_Vert2(y) !=
 *y. Go backward from z to y                   *
 ******************************************************************************/
Vertex GetPrevVertex(BN_STRUCT *pBNS, Vertex y, Edge *SwitchEdge,
                     EdgeIndex *iuv) {
  Vertex w, z, x2, y2, n;
...
/*****************************************************************************
   ...
   CHECK_TACN == 1 prohibits replacing (-) on N with H unless H can be moved to
N along an alternating path from another heteroatom (t-group will be detected).
*****************************************************************************/
...
#define TYPE_CN 2 /* (-)c-group */
  int i, degree, ret, u_is_taut = 0, w_is_taut, num_allowed = 0,
                      num_found_groups = 0;
  Vertex w;
  EdgeIndex ivw;
  ...
  u_is_taut = ((pBNS->vert[u / 2 - 1].type & pBNS->type_T) == pBNS->type_T)
                  ? TYPE_T
...
int bIgnoreVertexNonTACN_atom(BN_STRUCT *pBNS, Vertex u, Vertex v) {
  ...
  if (!pBNS->type_T || !pBNS->type_CN)
    return 0; /* should not happen */
  if ((u_is_taut =
           (pBNS->vert[u / 2 - 1].type & pBNS->type_T) == pBNS->type_T) ||
      ((pBNS->vert[u / 2 - 1].type & pBNS->type_CN) == pBNS->type_CN)) {
    /* u is either t-group vertex or (-) c-group */
    degree = GetVertexDegree(pBNS, v);
    for (i = 0; i < degree; i++) {
      ...
      if (w != u && (ret = rescap(pBNS, v, w, ivw)) > 0) {
        num_allowed++;
        if ((u_is_taut ? ((pBNS->vert[w / 2 - 1].type & pBNS->type_CN) ==
                          pBNS->type_CN)
                       : ((pBNS->vert[w / 2 - 1].type & pBNS->type_T) ==
                          pBNS->type_T))) {
          num_found_groups++;
        }
      }
...
#define TYPE_CN 2 /* (-)c-group */
  ...
  if (v <= 1 || w <= 1)
    return 0;
#if (CHECK_TACN == 1)
  if (!pBNS->type_TACN || (pBNS->vert[v / 2 - 1].type & pBNS->type_TACN)) {
    return 0;
  }
  if (!pBNS->type_T || !pBNS->type_CN)
    return 0; /* should not happen */
  ...
  */
  ...
  if (pBNS->edge[iuv].neighbor1 != (u / 2 - 1) &&
          pBNS->edge[iuv].neighbor1 != v / 2 - 1 ||
...
int bIsRemovedHfromNHaion(BN_STRUCT *pBNS, Vertex u, Vertex v) {
  ...
  BNS_EDGE *pEdge;
  if (!pBNS->type_TACN || u <= 1 || v <= 1 || u % 2 ||
      !(v % 2) /* the edge flow may only increase */) {
    return 0;
  }
  ...
  if (((pBNS->vert[u2].type & pBNS->type_T) == pBNS->type_T) &&
    ...
    vtg = u;
    vat = v;
  } else if (((pBNS->vert[v2].type & pBNS->type_T) == pBNS->type_T) &&
             u2 < pBNS->num_atoms) {
    vtg = v;
    vat = u;
  } else {
...
#define TYPE_AT 4
  ...
  for (k = 0, u = v;
       1 < (u_next = u, u = GetPrevVertex(pBNS, u, SwitchEdge, &iuv)); k++) {
    u2 = u / 2 - 1;
  ...
  if (v == prim(w)) {
    ...
    if (v2 >= pBNS->num_atoms) {
      type0 = ((pBNS->vert[v2].type & pBNS->type_T) == pBNS->type_T) ? TYPE_T
              : ((pBNS->vert[v2].type & pBNS->type_CN) == pBNS->type_CN)
                  ? TYPE_CN
                  : 0;
    }
  }
...
/*********************************************************************************
    ...
    if t not in S, then let K=[S,S-], the set of all edges directed from S to
S-. K is an edge-cut that has a special structure. Let A  = {x[i], y[i] |x[i]
not in S, y[i]     in S} B  = {x[i], y[i] |x[i]     in S, y[i] not in S} C  =
{x[i], y[i] |x[i]     in S, y[i]     in S} D  = {x[i], y[i] |x[i] not in S, y[i]
not in S} N[C]  = subgraph of N induced by C; it consists of of a number of
connected components C[i] K[i]  = those edges of K with one endpoint in C[i].
    ...
    balcap(K) = cap(K) - odd(K), where odd(K) is the number of connected
components in N[C]. Name "odd(K)" is because each cap(K[i]) is odd.
 ...
 */
/*********************************************************************************/
int BalancedNetworkSearch(
    BN_STRUCT *pBNS, BN_DATA *pBD,
    int bChangeFlow) { /* N has source s, target t, the mirror network M is
                          constructed */
  /* the tree T contains a valid sv-path for each v in T. Simultaneously the
     complementary tree T' is built as indicated in comments. The trees T and T'
     must have no edges or vertices in common. Initially T will be built as in
     breadth-first-search, and T' will be the complementary tree, it will
     contain the complementary valid v't-path.
  */
  ...
  */
  ...
  BasePtr[Vertex_s] =
      BLOSSOM_BASE; /* create initial blossom C(Vertex_s) with base s */
...
#if (BNS_RAD_SEARCH == 1)
      if (!k && bRadSrchMode == RAD_SRCH_FROM_FICT &&
          v / 2 <= pBNS->num_atoms) {
        continue; /* start from fict. vertices only */
      }
      if (bRadSearchPrelim && v / 2 > pBNS->num_atoms) {
        continue; /* during initial add/remove H allow radical movement only
                     through real atoms */
      }
#endif
      if (/* PrevPt[u] != v ** avoid edges of T */
          (SwitchEdge_Vert1(u) != v ||
           SwitchEdge_Vert2(u) != u) /* avoid edges of T */
        ...
        if (b_v == NO_VERTEX) { /* originally 0 instead of NO_VERTEX */
          /* Important note: following "A. Note of Implementing the Algorithm"
             from the article by Kocay and Stone, all references to PrevPt[a]
             have been replaced with SwitchEdge[a][0]=SwitchEdge_Vert1(a); to be
             on a safe side the check whether (SwitchEdge_Vert2(a)==a) has been
             added.
          */
          ...
          /* Switch Edges: If u in T then Pu (a valid su-path) can be
          constructed by successfully executing u=PrevPt[u] until u=s. For
          vertices in T' the situation is different. Suppose uv and v'u' are
          added to a mirror network M creating a blossom:
          ...
          ------------------------ / \      <--- (the edges intersection in the
          picture is shown as X) u'=x2   v=x3      u' it T', v' in T \ / T'
          w'=y1 \   ... \ / t           T'

             Vertices v and u' now become s-reachable;
             The valid paths to v and u' must use the edges uv and v'u'
          respectively.

             For each vertex z in S we define a switch-edge that allows a valid
          sz-path to be constructed, SwitchEdge[v]=uv and SwitchEdge[u']=v'u'.
          (We don't know the direction of the edge uv, it may be (u,v) or (v,u).
          In either case, the complementary edge is indicated by v'u').

             Vertex w' also becomes s-reachable when uv is added to M, and a
          valid sw'-path must use one of uv and v'u'. Therefore we choose one of
          them, say uv (see below the rule of choosing the switch-edge), and
          define SwitchEdge[w'] = uv.

             When the addition of an edge uv to M causes a vertex z to become
          s-reachable (where z was previously non-reachable), z is placed on the
          ScanQ, that is, into S. The edge uv is said to be a switch-edge for z.

             Rule: We choose the the order of the vertices uv to be such that
          the valid sz-path consists of a valid su-path, followed by edge uv,
          followed by a valid vz-path.

             For vertices z in T we can take SwitchEdge[z]=yz where y=PrevPt[z]
          since it is the edge yz that allows z to be s-reachable. For vertices
          z not in S we take SwitchEdge[z]=NONE.
          ...
          */
          ...
          BasePtr[prim(v)] = v;
          BasePtr[v] =
              BLOSSOM_BASE; /* create a trivial blossom C(v) with base v */
#if (BNS_RAD_SEARCH == 1)
          n++;
#endif
                   ...
                   && (SwitchEdge_Vert1(prim(u)) != prim(v) ||
                       SwitchEdge_Vert2(prim(u)) !=
                           prim(u)) /* avoid edges of T' */
                   && b_u != b_v &&
                   !(pBNS->type_TACN &&
                     bIgnoreVertexNonTACN_group(pBNS, prim(v), u, SwitchEdge))
#if (FIX_KEEP_H_ON_NH_ANION == 1)
                   &&
          ...
          /* there is now a valid sv-path via u avoiding b_v (unless v==b_v)
            => u, v, u', and v' now all become part of the same connected
            component of M[C] */
          w = MakeBlossom(pBNS, ScanQ, &QSize, Pu, Pv, max_len_Pu_Pv,
                          SwitchEdge, BasePtr, u, v, iuv, b_u, b_v, Tree);
          ...
          if (prim(w) == Vertex_t) {
            ...
            delta = FindPathCap(
                pBNS, SwitchEdge, Vertex_s, Vertex_t,
            ...
            ret =
                PullFlow(pBNS, SwitchEdge, Vertex_s, Vertex_t, delta, 0,
  ...
  /* if this point is reached, no valid augmenting path exists, ScanQ contains
     the set S of all s-reachable vertices and K=[S,S-] is a minimum balanced
     edge-cut */
...
Blossoms.
  ...
  The vertices of a mirror network M consist T U T'. Intersection T ^ T' is
empty.
  ...
  Let C(i) be a blossom with base b_i. Since C(i)=C(i)', C(i) contains vertices
of T and T'. Since every valid sv-path to v in C(i) contains b_i, b_i is the
first s-reachable vertex of C(i).

  Suppose the mirror network M contains a valid sz-path P(z) to all vertices z
in ScanQ. Every vertex of P(z) is s-reachable therefore its vertices are all in
blossoms or trivial blossoms.

  Let z be an s-reachable vertex and P(z) be a valid path in M.
  Then every valid sz-path in M contains exactly the same sequence of blossom
bases as P(z).
...
Vertex MakeBlossom(BN_STRUCT *pBNS, Vertex *ScanQ, int *pQSize, Vertex *Pu,
  ...
  */
  ...
  i = len_Pu;
  j = len_Pv;
  /* initially Pu[i] and Pv[j] both equal to s, but their first elements are
   * different */
  /* find the last blossom base common to Pu and Pv */
  while (i >= 0 && j >= 0 && Pu[i] == Pv[j]) {
    /* was (Pu[i]==Pv[j] && i>=0 && j>=0) => tested Pu[-1], Pv[-1] <- pointed by
     * W.Ihlenfeldt 08-26-2004*/
    i--;
    j--;
  }
  i++;
  ...
  if (!TREE_IS_ON_SCANQ(
          prim(w)) /* !IsInScanQ(prim(w))*/) { /* add w' to the blossom */
...
/*****************************************************************************
    ...
    a valid vt-path.
    PullFlow is a recursive procedure that constructs the path and its
complement.

    Let wz=SwitchEdge[y]. PullFlow(x, y, delta) uses the xw- and zy-portions of
P (see below). Since it must also augment on P' simultaneously, the zy-portion
...
/*********************************************************************************/
int PullFlow(
    BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y, int delta,
    S_CHAR bReverse,
    int bChangeFlow) { /*
                          Augment the flow by delta on all edges on a path P
                          between x and y in the order of the path;
                          AugmentEdge( pBNS, w, z, iwz, delta, 0 ) means the
                          path is in w->z direction AugmentEdge( pBNS, w, z,
                          iwz, delta, 1 ) means the path is in w<-z direction

                          Unlike PullFlow in the paper by Kocay & Stone, here
                          the augmentation always starts at "s", proceeds
                          sequentially through the path end terminates at "t".
                          Since we do not really need the complement path,
                          PullFlow ignores it.
  ...
  if (bReverse) {
    ...
    if (z != y) {
      ret = PullFlow(pBNS, SwitchEdge, prim(y), prim(z), delta,
                     (S_CHAR)(1 - bReverse),
    ...
    /* Do not augment the complementary path: AugmentEdge( prim(z), prim(w),
     * vert, delta); */
    /* w may equal x, in which case there is no need to call PullFlow(x, w) */
    if (w != x && !IS_BNS_ERROR(ret)) {
      ret = PullFlow(pBNS, SwitchEdge, x, w, delta, bReverse,
                     bChangeFlow); /* augment between x and w */
    }
  } else {
    /* P consists of a path from x to w, then wz, then a path from z to y.  */
    /* w may equal x, in which case there is no need to call PullFlow(x, w) */
...
********************************************************************************/
int FindPathCap(
    BN_STRUCT *pBNS, Edge *SwitchEdge, Vertex x, Vertex y,
    int delta) { /* find the minimum residual capacity of all edges
...
#define BT_OTHER_ALTERN_BOND                                                   \
  2 /* 1-3, 2-3, 1-2-3 alternating non-stereo non-taut bonds */
...
#define BT_NONTAUT_BOND_MASK                                                   \
  (BT_ALTERN_BOND | BT_OTHER_ALTERN_BOND | BT_ALTERN_NS_BOND)

/* BNS members redefinitions for finding non-stereo bonds */
/* BNS_EDGE */
#define nBlockNumberAltBns                                                     \
  flow /* internal variable of the DFS traversal: mark traversed bonds */
#define nNumAtInBlockAltBns cap
#define nBondTypeInpAltBns                                                     \
  pass /* 0=>cannot be stereo at all, 1=>alt or taut non-stereo, 2=>can be     \
          stereo */
#define nBondNonStereoAltBns                                                   \
  cap /* 1=>found to be non-stereogenic although BondTypeInp=2; 0=>as in       \
         BondTypeInp */
...
#define nNumAtInRingSystemAltBns                                               \
  st_edge.flow0 /* number of vertices in a ring system */
#define nBlockSystemAltBns                                                     \
  st_edge.flow /* result of the DFS traversal: even cirquit must be within one \
                  block */
...
int MarkRingSystemsAltBns(BN_STRUCT *pBNS, int bUnknAltAsNoStereo) {
  ...
  nBondStack =
      (AT_NUMB *)(num_edges ? inchi_malloc(num_edges * sizeof(nBondStack[0]))
                            : NULL); /* special case: no bonds 2006-03 */
  cNeighNumb = (S_CHAR *)inchi_malloc(num_atoms * sizeof(cNeighNumb[0]));
  /*  check allocation */
  if (!nStackAtom || !nRingStack || !nDfsNumber || !nLowNumber ||
      (!nBondStack && num_edges) || !cNeighNumb) {
   ...
   *             2/      \6   (1,2,3, has 3 bonds), (3,4, has 1 bond), and
   *(4,5,6, has 3 bonds)
   *
  ...
  *********************************************************/
  ...
  found_alt:
    ...
    do {
      /* advance */
      /*while ( (int)at[i=nStackAtom[nTopStackAtom]].valenceAltBns > (j =
       * (int)cNeighNumb[i]) )*/
      /* replaced due to missing sequence point */
      while (i = (int)nStackAtom[nTopStackAtom], j = (int)cNeighNumb[i],
             (int)at[i].valenceAltBns > j) {
        cNeighNumb[i]++;
        ...
        } else if (!nTopStackAtom ||
                   u != (int)nStackAtom[nTopStackAtom -
                                        1]) { /*  may comment out ? */
      ...
      if (i != start) {
        ...
        if (nLowNumber[i] >= nDfsNumber[u]) {
          ...
          if ( u != start || nNumStartChildren > 1 ) {
              at[u].bCutVertexAltBns += 1;  // mark cut-vertex (articulation
          point)
          }
          */
  ...
  found_alt2:
    ...
    do {
      /* advance */
    advance_ring:
      /*if ( (int)at[i=nStackAtom[nTopStackAtom]].valenceAltBns > (j =
       * (int)cNeighNumb[i]) )*/
      /* replaced due to missing sequence point */
      if (i = (int)nStackAtom[nTopStackAtom], j = (int)cNeighNumb[i],
          (int)at[i].valenceAltBns > j) {
        cNeighNumb[i]++;
      ...
      if (nDfsNumber[i] == nLowNumber[i]) {
        ...
        while (nTopRingStack >= 0) {
          ...
          at[j].nRingSystemAltBns =
              (AT_NUMB)nNumRingSystems; /*  ring system id */
...
int ReInitBnStructForAltBns(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
  ...
  ret = ReInitBnStruct(pBNS, at, num_atoms, 0);
  if (ret || pBNS->num_atoms != num_atoms || pBNS->num_vertices != num_atoms ||
  ...
  for (v = 0; v < num_atoms; v++) {
    ...
    for (j = 0; j < pAtom->valenceAltBns; j++) {
      ...
      if (pBond->neighbor1 == v) {
        ...
        if (at[v].endpoint || at[v2].endpoint) {
          bond_type =
              0; /* any bond to an endpoint considered non-stereogenic */
        }
#if (FIX_EITHER_DB_AS_NONSTEREO == 1)
        if (bUnknAltAsNoStereo) {
          if (bond_type == BOND_ALTERN &&
              at[v].bond_stereo[j] == STEREO_DBLE_EITHER) {
    ...
    pAtom->bCutVertexAltBns = pAtom->nRingSystemAltBns =
        pAtom->nNumAtInRingSystemAltBns = pAtom->nBlockSystemAltBns = 0;
  }
...
int MarkNonStereoAltBns(BN_STRUCT *pBNS, inp_ATOM *at, int num_atoms,
  ...
  if (pBNS->num_atoms != num_atoms || pBNS->num_vertices != num_atoms ||
      pBNS->num_bonds != pBNS->num_edges) {
    ret = BNS_REINIT_ERR;
    goto exit_function;
  }
  if (bUnknAltAsNoStereo) {
    for (ibond = 0; ibond < num_bonds; ibond++) {
      ...
      if (pBond->nBondTypeInpAltBns != BT_ALTERN_BOND &&
          pBond->nBondTypeInpAltBns != BT_IGNORE_BOND) {
        continue;
      }
      ...
      if (/* alt bond non-adjacent to a taut. endpoint: */
        ...
        if ((at[iat1].bond_type[ib1] & BOND_TYPE_MASK) == BOND_ALTERN) {
          /* bond_type = BOND_ALT12NS; */
          at[iat1].bond_stereo[ib1] = at[iat2].bond_stereo[ib2] =
              STEREO_DBLE_EITHER;
          ret++;
        }
      }
    ...
    for (ibond = 0; ibond < num_bonds; ibond++) {
      ...
      if (/* alt bond non-adjacent to a taut. endpoint: */
          ...
          (pBond->nBondTypeInpAltBns == BT_IGNORE_BOND &&
           (at[iat1].bond_type[ib1] & BOND_TYPE_MASK) == BOND_ALTERN)) {
        at[iat1].bond_type[ib1] = at[iat2].bond_type[ib2] = BOND_ALT12NS;
        ret++;
      }
    }
  }
...
/*********************************************************************************
    *num_protons_to_add = nToBeRemovedByNormFromRevrs
    ...
    nToBeRemovedByNormFromRevrs > 0: less protons should be allowed to be
                                     added by the Normalization of the
Reconstructed Structure nToBeRemovedByNormFromRevrs < 0: prepare more H(+) to be
removed by the InChI Normalization of the Reconstructed Structure
    ...
    -----------------------------------------------------------
    - Too many protons were added by the Normalization to the Reconstructed
Structure (a) n(revr) < 0 => protons were added while they should not have been
added; Solution: "neutralize" (-) charged proton acceptors by moving charges to
other atoms on the condition ADP cannot add in another way; (b) n(orig) >
n(revr) => 0  => too few protons were removed Solution: (the easiest) attach
H(+) to =O or -N< or -N= Solution: move (+) from N or OH to an atom adjacent to
(-) charge or to an atom that is not N.

    n(orig) < n(revr) or nToBeRemovedByNormFromRevrs < 0 means:
    -----------------------------------------------------------
    - Too few protons were added by the Normalization to the Reconstructed
Stucture (a) n(orig) < 0 => protons were not added while they should have been
added; Solution: move (-) to O by replacing =O with -O(-) (b) 0 <= n(orig) <
n(revr) => too many protons were removed

   Note: it is critically important to takr into account cumbersome
Normalization Total Charge: if it is >= 0 then no H(+) may be removed from -OH
or by ADP However, if N(+) is present then ADP will always try to remove a
proton
*********************************************************************************/
int AddRemoveProtonsRestr(inp_ATOM *at, int num_atoms, int *num_protons_to_add,
                          int nNumProtAddedByRestr,
                          INCHI_MODE bNormalizationFlags, int num_tg,
                          int nChargeRevrs, int nChargeInChI) {
  int i, j, ret = 0;
  int nAtTypeTotals[ATTOT_ARRAY_LEN];
  ...
  */
  /* if ChargeRevrs > nChargeInChI then we should prevent proton addition or
     facilitate proton removal a typical case is (=) on N or O instead of C(-)

     if ChargeRevrs < nChargeInChI then we should prevent proton removal or
     facilitate proton addition
  */
  ...
  if (num_prot < 0 &&
    ...
    /* remove proton(s) */
    /* use test from SimpleAddAcidicProtons() to test whether removal of H(+)
     * from =C-OH, etc. is correct */
    for (i = 0; i < num_atoms && num_prot; i++) {
      ...
      if (type) {
        for (bSuccess = 0, j = 0; j < max_j_Aa; j++) {
          if ((bSuccess = (type & AaTypMask[2 * j])) &&
              (mask && AaTypMask[2 * j + 1])) {
  ...
  if (num_prot < 0 && num_tg &&
      nAtTypeTotals[ATTOT_TOT_CHARGE] - nNumProtAddedByRestr <= 0) {
    /* alternative proton removal: O=C-NH => (-)O-C=N, O and N are taut.
     * endpoints */
    ...
    for (i = 0; i < num_atoms; i++) {
      ...
      if (1 != at[i].valence || BOND_TYPE_DOUBLE != at[i].bond_type[0] ||
          at[i].num_H || 2 != get_endpoint_valence(at[i].el_number)) {
        continue;
      }
      centp = at[i].neighbor[0];
      if (at[centp].sb_parity[0] || at[centp].p_parity ||
          !is_centerpoint_elem(at[centp].el_number)) {
        continue;
      }
      ...
      for (k = 0; k < at[centp].valence; k++) {
        ...
        if (at[endp2].endpoint != at[i].endpoint || !at[endp2].num_H ||
            at[endp2].charge || at[endp2].sb_parity[0] || at[endp2].p_parity ||
        ...
        for (i0 = 0; i0 < at[centp].valence && i != at[centp].neighbor[i0];
             i0++)
          ;
        for (k0 = 0; k0 < at[endp2].valence && centp != at[endp2].neighbor[k0];
             k0++)
          ;
        ...
        type = GetAtomChargeType(at, i, nAtTypeTotals, &mask,
                                 1); /* subtract at[i] */
        type = GetAtomChargeType(at, endp2, nAtTypeTotals, &mask,
                                 1); /* subtract at[endp2] */
        ...
        type =
            GetAtomChargeType(at, i, nAtTypeTotals, &mask, 0); /* add at[i] */
        type = GetAtomChargeType(at, endp2, nAtTypeTotals, &mask,
                                 0); /* add at[endp2] */
      }
    }
  ...
  if (num_prot > 0) {
    /* add protons */
    /* 1. Use test from SimpleRemoveAcidicProtons() to test whether addition of
     * H(+) to =C-O(-), etc. is correct */
    for (i = 0; i < num_atoms && num_prot &&
                nAtTypeTotals[ATTOT_TOT_CHARGE] - nNumProtAddedByRestr >= 0;
         i++) {
      /* choose an atom */
      ...
      if (type) {
        for (bSuccess = 0, j = 0; j < max_j_Ar; j++) {
          if ((bSuccess = (type & ArTypMask[2 * j])) &&
              (mask && ArTypMask[2 * j + 1])) {
            break;
          }
        }
    ...
    for (i = 0; i < num_atoms && num_prot; i++) {
      /* choose an atom */
      if (at[i].sb_parity[0] || at[i].p_parity || at[i].charge ||
          at[i].radical || bHasChargedNeighbor(at, i)) {
        continue;
      }
      /* try to add a proton and check whether InChI would remove it back */
      at[i].num_H++;
      at[i].charge++;
      bSuccess =
          (PR_SIMPLE_TYP & (type = GetAtomChargeType(at, i, NULL, &mask, 0))) &&
          (PR_SIMPLE_MSK & mask);
      at[i].num_H--; /* failed */
      at[i].charge--;
      if (bSuccess) {
  ...
  if (num_prot < 0 && (bNormalizationFlags & FLAG_PROTON_AC_HARD_ADDED) &&
      1 == num_tg &&
      nAtTypeTotals[ATTOT_TOT_CHARGE] - nNumProtAddedByRestr <= 0) {
    ...
    for (i = 0; i < num_atoms; i++) {
      ...
      if ((type & AA_HARD_TYP_CO) && (mask & AA_HARD_MSK_CO)) {
        nNumAcceptors_DB_O++;
      } else if ((type == ATT_ATOM_N) && (mask == ATBIT_NP_H) &&
                 !at[i].charge && at[i].valence == at[i].chem_bonds_valence) {
        nNumDonors_SB_NH++;
      }
    }
    num_max = inchi_min(nNumAcceptors_DB_O, nNumDonors_SB_NH);
    for (i = 0, num_success = 0;
         i < num_atoms && num_success < num_max && num_prot < 0; i++) {
      /* choose an atom */
      if (!at[i].endpoint || at[i].radical || at[i].sb_parity[0] ||
          at[i].p_parity || bHasChargedNeighbor(at, i)) {
        continue;
...
/*****************************************************************************/
int AddRemoveIsoProtonsRestr(inp_ATOM *at, int num_atoms,
                             NUM_H num_protons_to_add[], int num_tg) {
  int i, j, k, n, ret = 0;
  int nNumSuccess = 0, min_at, max_at, num_H, num_iso_H, num_expl_H,
      num_expl_iso_H;
  int iCurIso;                        /* 0=> 1H, 1=> D, 2=> T */
  int iCurMode, iCurMode1, iCurMode2; /* 0=> Not Endpoints, 1=> Endpoints */
  static U_CHAR el_number_H = 0;

  /* distribute isotopes from  heaviest to lightest; pick up atoms in order 1.
   * Not endpoints; 2. Endpoints */
  iCurMode1 = 0;
  ...
  for (iCurMode = iCurMode1; iCurMode <= iCurMode2; iCurMode++) {
    for (iCurIso = 2; 0 <= iCurIso; iCurIso--) {
      ...
      for (i = min_at; i < max_at && 0 < num_protons_to_add[iCurIso]; i++) {
        ...
        for (k = 0; k < at[j].valence && num_atoms <= (n = at[j].neighbor[k]);
             k++) {
