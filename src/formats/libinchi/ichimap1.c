...
int All_SC_Same(AT_RANK canon_rank1, /*  canonical number */
                ...
                const AT_RANK *nAtomNumberCanonFrom, const sp_ATOM *at) {
  ...
  for (i1 = 1; i1 <= iMax1 &&
               r1 == pRankStack2[0][s1 = (int)pRankStack2[1][iMax1 - i1]];
       i1++) {
    if (at[s1].stereo_bond_neighbor[0]) {
      bFound =
          0; /* at[s1] is not sp3-stereogenic: it belongs to a stereobond */
      break;
    } else if (i1 == 1) {
...
int Next_SC_At_CanonRank2(
    AT_RANK *canon_rank1, /*  1st call input: largest canon number mapped so far
                             or 0 */
    /*  output: suggested canon. rank > than input if success */
    AT_RANK *
        canon_rank1_min, /*  1st call:0 next calls: first tried canon. number */
    int *bFirstTime,     /*  1 at the time of the 1st call  */
    S_CHAR *bAtomUsedForStereo,  /*  STEREO_AT_MARK if the atom has not been
                                    mapped yet */
    const ppAT_RANK pRankStack1, /*  mapping ranks/sort order of atoms with
                                    canon. numbers (from) */
    const ppAT_RANK
        pRankStack2, /*  mapping ranks/sort order of atoms with stereo (to) */
    const AT_RANK
        *nAtomNumberCanonFrom, /*  sorted order of the canon. numbers */
    int num_atoms) {
  ...
  while ((int)cr1 <= num_atoms) {
    n1 = (int)nAtomNumberCanonFrom[(int)cr1 - 1]; /*  atom1 (which has canon.
                                                     rank cr1) ord. number */
    iMax1 = (int)(r1 = pRankStack1[0][n1]);       /*  mapping rank of atom1 */
    /*  find atoms "to" to which the canon. number can be mapped; they have
     * mapping rank r1, number s1 */
    for (i1 = 1; i1 <= iMax1 &&
                 r1 == pRankStack2[0][s1 = (int)pRankStack2[1][iMax1 - i1]];
         i1++) {
    ...
    if (bFound) {
      ...
      /*  for the mapping rank r1 defined by the canonical rank cr1; try next
       * cr1 */
      cr1++;
    }
  }
...
int CompareLinCtStereoDble(AT_STEREO_DBLE *LinearCTStereoDble1,
  ...
  if (LinearCTStereoDble1 && LinearCTStereoDble2) {
    ...
    for (i = 0; i < num; i++) {
      if ((ret = (int)LinearCTStereoDble1[i].at_num1 -
                 (int)LinearCTStereoDble2[i].at_num1))
        break;
      if ((ret = (int)LinearCTStereoDble1[i].at_num2 -
                 (int)LinearCTStereoDble2[i].at_num2))
        break;
      if ((ret = (int)LinearCTStereoDble1[i].parity -
                 (int)LinearCTStereoDble2[i].parity))
        break;
    }
    if (!ret) {
...
int CompareLinCtStereoCarb(AT_STEREO_CARB *LinearCTStereoCarb1,
  ...
  if (LinearCTStereoCarb1 && LinearCTStereoCarb2) {
    ...
    for (i = 0; i < num; i++) {
      if ((ret = (int)LinearCTStereoCarb1[i].at_num -
                 (int)LinearCTStereoCarb2[i].at_num))
        break;
      if ((ret = (int)LinearCTStereoCarb1[i].parity -
                 (int)LinearCTStereoCarb2[i].parity))
        break;
    }
    if (!ret) {
...
/**********************************************************************/
int CompareLinCtStereo(
    AT_STEREO_DBLE *LinearCTStereoDble1, int nLenLinearCTStereoDble1,
 ...
 */
int bUniqueAtNbrFromMappingRank(AT_RANK **pRankStack, AT_RANK nAtRank,
                                AT_NUMB *nAtNumber) {
  int r = (int)nAtRank - 1;
...
/*********************************************************************************
 *  For all pairs of atoms that are: * (a) connected by a possibly stereogenic
 *bond                                 * (b) "equivalent" at this point to
 *canon_rank1-canon_rank2 :                  * Check if they: * 1) are connected
 *by a stereo bond or cumulene bonds of the same length       * 2) have KNOWN
 *parity, and                                                    * 3) their
 *parities are same                                                   *
 *********************************************************************************/
int All_SB_Same(AT_RANK canon_rank1,
  ...
  int n1 =
      (int)nAtomNumberCanonFrom[(int)canon_rank1 - 1]; /* at1 has canon_rank1 */
  int n2 =
      (int)nAtomNumberCanonFrom[(int)canon_rank2 - 1]; /* at2 has canon_rank2 */
  ...
  /*  at the first atom that possibly may have canon_rank1 find one stereo bond
   * such that */
  ...
  for (i1 = 1; i1 <= iMax1 &&
    ...
    for (k1 = 0, s2 = 0, bNotFound = 1;
         k1 < MAX_NUM_STEREO_BONDS &&
         (s2 = (int)at[s1].stereo_bond_neighbor[k1]) &&
         (bNotFound = (r2 != pRankStack2[0][--s2]));
         k1++)
      ; /* continue until the 1st at[s2] (to which at[n2] may be mapped) have
           been found */
    if (!bNotFound) {
  ...
  for (k2 = 0, m = 0; k2 < MAX_NUM_STEREO_BONDS &&
                      (m = (int)at[s2].stereo_bond_neighbor[k2]) && m - 1 != s1;
       k2++)
    ;
  if (m - 1 != s1) {
  ...
  rNeigh1 =
      pRankStack2[0][(int)at[s1].neighbor[(int)at[s1].stereo_bond_ord[k1]]];
  rNeigh2 =
      pRankStack2[0][(int)at[s2].neighbor[(int)at[s2].stereo_bond_ord[k2]]];

  num_equal = 0;
  /*  Search among ALL neighbors because sometimes a stereo bond may be mapped
   * on a non-stereo bond. */
  /*  If is so then return 0: not all mappings are stereo-equivalent */
  for (s1 = 1; s1 <= iMax1 &&
               r1 == pRankStack2[0][i1 = (int)pRankStack2[1][iMax1 - s1]];
       s1++) {
    for (k = 0; k < at[i1].valence; k++) {
      ...
      if (cumulene_len) {
        ...
        if (len != cumulene_len || r2 != pRankStack2[0][next] ||
      ...
      for (k1 = 0; k1 < MAX_NUM_STEREO_BONDS &&
                   (m = (int)at[i1].stereo_bond_neighbor[k1]) && m - 1 != i2;
           k1++)
        ;
      if (m - 1 != i2) {
        return 0;
      ...
      for (k2 = 0; k2 < MAX_NUM_STEREO_BONDS &&
                   (m = (int)at[i2].stereo_bond_neighbor[k2]) && m - 1 != i1;
           k2++)
        ;
      if (m - 1 != i1) {
        return 0;
...
int Next_SB_At_CanonRanks2(AT_RANK *canon_rank1,
  ...
  while ((int)cr1 <= num_atoms) {
    cr2 = cr1;
    n1 = (int)nAtomNumberCanonFrom[(int)cr1 - 1]; /*  atom1=at[n1] (which has
                                                     canon. rank) ord. number */
    iMax1 = (int)(r1 = pRankStack1[0][n1]);       /*  mapping rank of atom1 */
    for (i1 = 1; i1 <= iMax1 &&
                 r1 == pRankStack2[0][s1 = (int)pRankStack2[1][iMax1 - i1]];
         i1++) {
      /*  looking for a stereo bond atom that has mapping rank r1 */
      /*  found at[s1] such that rank cr1 can be mapped on at[s1] because cr1
       * and s1 have equal */
      /*  mapping rank = r1. Check at[s1] stereo bonds */
      if (bAtomUsedForStereo[s1] && bAtomUsedForStereo[s1] < STEREO_AT_MARK) {
        for (k = 0, s2 = 0; k < MAX_NUM_STEREO_BONDS &&
                            (s2 = (int)at[s1].stereo_bond_neighbor[k]);
             k++) {
          /*  stereo bond at[s1]-at[s2] has been found */
          if (bAtomUsedForStereo[--s2]) {
            /*  stereo bonds have not been mapped. however, this check is not
             * needed */
            ...
            /*  Go back to canonical ranks and find an atom that has mapping
             * rank r2 */
            /*  and is connected to the atom with canonical rank cr1 (possibly
             * by cumulene chain) */
            /*  These cr1-cr2 canon. ranks possibly can be mapped on
             * at[s1]-at[s2] stereo bond */
            for (i2 = 1;
                 i2 <= iMax2 &&
                 r2 == pRankStack1[0][n2 = (int)pRankStack1[1][iMax2 - i2]];
                 i2++) {
              if (cumulene_len) {
                int prev, next, len, j;
                ...
                for (m = 0; m < at[n1].valence && n2 != (int)at[n1].neighbor[m];
                     m++)
                  ;
              }
              if (m < at[n1].valence && nCanonRankFrom[n2] < cr2 &&
...
int NextStereoParity2Test(int *stereo_bond_parity, int *sb_parity_calc,
                          int nNumBest, int nNumWorse, int nNumUnkn,
                          int nNumUndf, int nNumCalc, int vABParityUnknown) {
  /* sequence of (stereo_bond_parity, sb_parity_calc) pairs:
        ...
        (BEST_PARITY, WORSE_PARITY) <known>
                    |
        (WORSE_PARITY, WORSE_PARITY) <calc>                (BEST_PARITY, 0)
    <known>
    ...
    stereo_bond_parity==sb_parity_calc  => parity to be calculated from
    canonical numbers stereo_bond_parity!=sb_parity_calc  => parity is already
    known
   */
get_next_parity:
  ...
  case BEST_PARITY:
    ...
    case 0: /*  BEST_PARITY(known) : (BEST_PARITY, 0) -> */
      *stereo_bond_parity =
          WORSE_PARITY; /*  WORSE_PARITY(known): (WORSE_PARITY, 0) */
      if (!nNumWorse) {
    ...
    case BEST_PARITY: /*  BEST_PARITY(calc) : (BEST_PARITY, BEST_PARITY) -> */
      *sb_parity_calc =
          WORSE_PARITY; /*  BEST_PARITY(known): (BEST_PARITY, WORSE_PARITY) */
      if (!nNumBest) {
        goto get_next_parity;
      }
      break;
    case WORSE_PARITY: /*  BEST_PARITY(known): (BEST_PARITY, WORSE_PARITY)-> */
      *stereo_bond_parity =
          WORSE_PARITY; /*  WORSE_PARITY(calc): (WORSE_PARITY,WORSE_PARITY) */
  ...
  case WORSE_PARITY:
    ...
    case 0: /*  WORSE_PARITY(known)  : (WORSE_PARITY, 0) -> */
      *stereo_bond_parity =
          vABParityUnknown /* AB_PARITY_UNKN */; /*  AB_PARITY_UNKN(known):
                                                    (AB_PARITY_UNKN, 0) */
      if (!nNumUnkn) {
        goto get_next_parity;
      }
  ...
  case AB_PARITY_UNKN:   /* AB_PARITY_UNKN(known): (AB_PARITY_UNKN, 0) -> */
    ...
    *stereo_bond_parity =
        AB_PARITY_UNDF; /* AB_PARITY_UNDF(known): (AB_PARITY_UNDF, 0) */
    if (!nNumUndf) {
...
void SetUseAtomForStereo(S_CHAR *bAtomUsedForStereo, sp_ATOM *at,
                         int num_atoms) {
  int i, k;
  memset(bAtomUsedForStereo, 0, sizeof(bAtomUsedForStereo[0]) * num_atoms);
  for (i = 0; i < num_atoms; i++) {
    if (at[i].parity) {
      for (k = 0; k < MAX_NUM_STEREO_BONDS && at[i].stereo_bond_neighbor[k];
           k++)
        ;
...
int CurTreeAlloc(CUR_TREE *cur_tree, int num_atoms) {
  if (cur_tree) {
    if (cur_tree->tree && cur_tree->max_len > 0 &&
    ...
    if ((cur_tree->tree =
             (AT_NUMB *)inchi_calloc(num_atoms, sizeof(cur_tree->tree[0])))) {
      cur_tree->incr_len = cur_tree->max_len = num_atoms;
      return 0; /*  ok */
    }
  }
...
int CurTreeReAlloc(CUR_TREE *cur_tree) {
  if (cur_tree) {
    if (cur_tree->tree && cur_tree->max_len > 0 && cur_tree->incr_len > 0) {
      ...
      if ((cur_tree->tree =
               (AT_NUMB *)inchi_calloc(cur_tree->max_len + cur_tree->incr_len,
                                       sizeof(cur_tree->tree[0])))) {
        memcpy(cur_tree->tree, p,
               cur_tree->cur_len * sizeof(cur_tree->tree[0]));
        inchi_free(p);
...
/**************************************************************************************/
void CurTreeKeepLastAtomsOnly(
    CUR_TREE *cur_tree, int tpos,
    int shift) { /*  on first entry: shift = 1; other values may occur in
  ...
  int cur_length_pos;
  if (cur_tree && cur_tree->tree &&
      (cur_length_pos = cur_tree->cur_len - shift) > tpos) {
    if (cur_tree->tree[cur_length_pos] > 2) {
      /*  current segment contains more than 1 atom. Leave in the segment: rank,
       * the last atom, length value */
      /*  subtract (old segment length)-(new segment length) from the tree
       * length  */
      /*  actual segment length including segment length value =
       * (cur_tree->tree[cur_length_pos]+1) */
      cur_tree->cur_len -= (int)cur_tree->tree[cur_length_pos] - 2;
      memmove(cur_tree->tree + cur_length_pos - cur_tree->tree[cur_length_pos] +
                  1, /*  1st atom pos */
              cur_tree->tree + cur_length_pos -
                  1, /*  last atom in the current segment position */
              (shift + 1) * sizeof(cur_tree->tree[0]));
      /*  (current segment length) distance from the last tree element has not
       * changed */
      cur_tree->tree[cur_tree->cur_len - shift] = 2;
      /*  add 3 to move to the previous segment length position */
      shift += 3; /*  lenghth = 3 accounts for 3 currently present. segment
                     items: (1) the last atom, (2) rank, (3) length value */
    } else {
