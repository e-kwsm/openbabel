...
int str_LineEnd(const char *tag, int tot_len, int nStrLen, int *bOverflow,
                char *pStr, int ind, int bPlainTextTags);
...
int WriteOrigAtoms(int num_inp_atoms, inp_ATOM *at, int *i, char *szBuf,
                   int buf_len, STRUCT_DATA *sd);
int WriteOrigBonds(int num_inp_atoms, inp_ATOM *at, int *i, char *szBuf,
                   int buf_len, STRUCT_DATA *sd);
...
const char x_aux_comm[] =
    "!-- This section is NOT a part of the identifier, it is not unique --";
...
/*
  ...
  1.12Beta/FML       /i      /f[FML]  /i   [/o] /rFML      /i      /f[FML]  /i
[/o] end |          |       |        |         |          |       |        | |
Labels    | chqpbtms | hbtms | hqbtms | btms    | chqpbtms | hbtms | hqbtms |
btms    | inside:   |          |       |        |         |          |       |
|         | | non-iso- | iso-  | fix-   | iso-    | non-iso- | iso-  | fix-   |
iso-    | meaning:  | topic    | topic | ed H   | topic   | topic    | topic |
ed H   | topic   |
          |----------+-------+--------+---------|----------+-------+--------+---------|
          |        mobile-H  |   fixed-H        |        mobile-H  |   fixed-H |
          |----------+-------+--------+---------|----------+-------+--------+---------|
          |                                     | | |     normal  or disconected
metal    |      reconnected bonds to metal     |
...
char *szGetTag(const INCHI_TAG *Tag, int nTag, int bTag, char *szTag,
               int *bAlways);
...
const char szRefChars[sizeof(xmlRef) / sizeof(xmlRef[0])] = {'<', '&',  '>',
                                                             '"', '\'', '\0'};
/**********************************************************************************************/
int PrintXmlStartTag(char *pStr, int indent, int bEnd, const char *tag,
                     const char *l1, int v1, const char *l2, int v2,
  ...
  if ((bEnd & 3)) {
    len += sprintf(pStr + len, "%s%s", (bEnd & 1) ? "/" : "",
                   (bEnd & 2) ? ">" : "");
  }
  return len;
}
...
int AddXmlEntityRefs(const char *p, char *d) {
  ...
  while (*p) {
    ...
    if (*p) {
      ...
      strcpy(d + len_d,
             q->pRef); /*  add entity reference and zero termination */
...
/**********************************************************************************************/
int OutputINChIXmlStructStartTag(INCHI_IOSTREAM *output_file, char *pStr,
                                 int ind /* indent*/, int nStrLen,
                                 int bNoStructLabels, int num_input_struct,
  ...
  /*  substitute special characters (see szRefChars[]) with xml Entity
   * References */
  int len;
  if (bNoStructLabels) {
    /* no labela at all */
    ...
    if (num_input_struct > 0) {
      tot_len +=
          sprintf(pStr + tot_len, " %s=\"%d\"", x_number, num_input_struct);
    }
    tot_len += sprintf(pStr + tot_len, ">");
    inchi_ios_print(output_file, "%s\n", pStr);
    ret = 1; /*  success */
    ...
    nEstLen1 = ind + 1 + sizeof(x_structure) - 1 + 1 + sizeof(x_number) - 1 +
               1 + sprintf(szBuf, "\"%d\"", num_input_struct) + 2;
    nEstLen2 = 1 + sizeof(x_header) - 1 + 1 + 2 +
               (szSdfLabel ? strlen(szSdfLabel) : 0) + 1 + sizeof(x_value) - 1 +
               1 + 2 + (szSdfValue ? strlen(szSdfValue) : 0) + 2;
    if (nEstLen1 <= nStrLen) {
      inchi_ios_print(output_file, "%s\n", ""); /*  empty line */
      tot_len = 0;
      tot_len += sprintf(pStr + tot_len, "%s<%s", SP(ind), x_structure);
      ...
      if (nEstLen1 + nEstLen2 <= nStrLen) {
        tot_len += sprintf(pStr + tot_len, " %s=\"%s\"", x_header,
                           szSdfLabel ? szSdfLabel : x_empty);
        tot_len += sprintf(pStr + tot_len, " %s=\"%s\"", x_value,
                           szSdfValue ? szSdfValue : x_empty);
      }
      tot_len += sprintf(pStr + tot_len, ">");
      inchi_ios_print(output_file, "%s\n", pStr);
      ret = 1; /*  success */
...
/**********************************************************************************************/
int OutputINChIXmlStructEndTag(INCHI_IOSTREAM *output_file, char *pStr,
                               int nStrLen, int ind) {
  if (output_file && pStr) {
...
/**********************************************************************************************/
int OutputINChIXmlError(INCHI_IOSTREAM *output_file, char *pStr, int nStrLen,
                        int ind,
  ...
  nEstLen =
      ind + 1 + sizeof(x_message) - 1 + 1 + sizeof(x_type) - 1 + 1 + 1 +
      strlen(pErr) -
      1
  ...
  if (nEstLen <= nStrLen) {
    ...
    sprintf( pStr, "%s<%s %s=\"%s\" %s=\"%d\" %s=\"%s\"/>",
             SP(ind), x_message, x_type, pErr, x_code, nErrorNumber, x_text,
    szErrorText );
    */
    sprintf(pStr, "%s<%s %s=\"%s\" %s=\"%s\"/>", SP(ind), x_message, x_type,
            pErr, x_text, szErrorText);
    inchi_ios_print(output_file, "%s\n", pStr);
    /*
...
int OutputINChIPlainError(INCHI_IOSTREAM *output_file, char *pStr, int nStrLen,
  ...
  /* <%s: >, x_message */
  nEstLen = sizeof(x_message) - 1 + 1 +
            1
            /* <%s=\"%s\">, x_type, pErr */
            + sizeof(x_type) - 1 + 1 + 1 + strlen(pErr) +
            1
            /* < %s=\"%s\"\n>, x_text, szErrorText */
            + 1 + sizeof(x_text) - 1 + 1 + 1 + strlen(szErrorText) + 1 + 1;
  if (nEstLen < nStrLen) {
    sprintf(pStr, "%s: %s=\"%s\" %s=\"%s\"", x_message, x_type, pErr, x_text,
            szErrorText);
    inchi_ios_print(output_file, "%s\n", pStr);
    ret = 1;
  }
...
const char *EquString(int EquVal) {
  ...
  case (iiNUMB |
        iiSTEREO_INV): /*------------- Inv Stereo Numbering ------------*/
    switch (bType) {
...
int OutputINChI2(char *pStr, int nStrLen,
                 INCHI_SORT *pINChISortTautAndNonTaut2[][TAUT_NUM], int iINChI,
                 ORIG_STRUCT *pOrigStruct, int bDisconnectedCoord,
                 int bOutputType, int bINChIOutputOptions, int bXml,
                 int bAbcNumbers, int bCtPredecessors, int bNoStructLabels,
                 int num_components2[], int num_non_taut2[], int num_taut2[],
                 INCHI_IOSTREAM *output_file, INCHI_IOSTREAM *log_file,
                 int num_input_struct, const char *szSdfLabel,
                 const char *szSdfValue, long lSdfId, int *pSortPrintINChIFlags,
                 unsigned char save_opt_bits) {
  int bINChIOutputOptions0 =
      bINChIOutputOptions &
      ~(INCHI_OUT_XML | INCHI_OUT_PLAIN_TEXT | INCHI_OUT_PLAIN_TEXT_COMMENTS);
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (bINChIOutputOptions & bCurOption) {
      ...
      ret |= OutputINChI1(
          pStr, nStrLen, pINChISortTautAndNonTaut2, iINChI, pOrigStruct,
          bDisconnectedCoord, bOutputType, bINChIOutputOptionsCur, bXml,
          bAbcNumbers, bCtPredecessors, bNoStructLabels, num_components2,
          num_non_taut2, num_taut2, output_file, log_file, num_input_struct,
          szSdfLabel, szSdfValue, lSdfId, pSortPrintINChIFlags, save_opt_bits);
    }
  }
...
char *szGetTag(const INCHI_TAG *Tag, int nTag, int bTag, char *szTag,
  ...
  if (0 < nTag && nTag < 3) {
    ...
    if (j >= 0) {
      strcpy(szTag, nTag == 1   ? Tag[j].szXmlLabel
                    : nTag == 2 ? Tag[j].szPlainLabel
                                : "???");
      if (nTag != 2) {
...
int OutputINChI1(char *pStr, int nStrLen,
  ...
  /*
    bINChIOutputOptions bits:
      ...
      INCHI_OUT_EMBED_REC             0x0008    embed reconnected INChI into
    disconnected INChI
  ...
  */
  ...
  int ATOM_MODE =
      ((bAbcNumbers ? CT_MODE_ABC_NUMBERS : 0) | CT_MODE_ATOM_COUNTS |
       CT_MODE_NO_ORPHANS
  ...
  /* bOutputType =
       ...
       TAUT_NON  => only non-tautomeric output (if no non-taut present then no
     output; TAUT_BOTH => tautomeric and non-tautomeric */

  int i, j, ii, jj, /*ii2, jj2,*/ tot_len, tot_len2, bOverflow,
  ...
  int bAtomEqu[TAUT_NUM], bTautEqu[TAUT_NUM], bIsotopicAtomEqu[TAUT_NUM],
      bIsotopicTautEqu[TAUT_NUM];
  int bInvStereo[TAUT_NUM], bInvIsotopicStereo[TAUT_NUM];
  int bInvStereoOrigNumb[TAUT_NUM], bInvIsotopicStereoOrigNumb[TAUT_NUM],
      bIsotopicOrigNumb[TAUT_NUM];
  int bTautomeric, bNonTautomeric, bTautomericAcid, bHardAddRemProton,
      iCurTautMode;
  ...
  int bPlainTextTags = 2; /* 0 => no plain tags, 1=> plain text tags,
                             2=>plaintext tags without consecutive // */
  int bPlainText = 0 != (bINChIOutputOptions & (INCHI_OUT_PLAIN_TEXT |
                                                INCHI_OUT_PLAIN_TEXT_COMMENTS));
  int bPlainTextCommnts =
      0 != (bINChIOutputOptions & INCHI_OUT_PLAIN_TEXT_COMMENTS);
  ...
  */
  ...
  for (i = 0; i < TAUT_NUM; i++) {
    bHasIsotopicAtoms[i] = num_comp[i] = bStereoSp2[i] = bStereoSp3[i] =
        bIsotopicStereoSp2[i] = bIsotopicStereoSp3[i] = bIsotopicOrigNumb[i] =
            bStereoAbs[i] = bIsotopicStereoAbs[i] = bStereoAbsInverted[i] =
                bIsotopicStereoAbsInverted[i] = bRacemicStereo[i] =
                    bRelativeStereo[i] = bIsotopicRacemicStereo[i] =
                        bIsotopicRelativeStereo[i] = bAtomEqu[i] = bTautEqu[i] =
                            bIsotopicAtomEqu[i] = bIsotopicTautEqu[i] =
                                bInvStereo[i] = bInvIsotopicStereo[i] =
                                    bInvStereoOrigNumb[i] =
                                        bInvIsotopicStereoOrigNumb[i] =
  ...
  /*
       x = bStereo, bStereoSp2, bStereoSp3, bStereoAbsInverted,
           bIsotopicStereo, bIsotopicStereoSp2, bIsotopicStereoSp3,
     bIsotopicStereoAbsInverted

       OUT_N1: x[TAUT_NON] refers to non-tautomeric only
       OUT_T1: x[TAUT_YES] refers to tautomeric if exists otherwise
     non-tautomeric OUT_NT: x[TAUT_NON] refers to non-taut representations of
     tautomeric OUT_TN: x[TAUT_YES] refers to tautomeric if exists otherwise
     non-tautomeric x[TAUT_NON] refers to non-taut representations of tautomeric
   */
  ...
  pINChISort =
      pINChISortTautAndNonTaut[bTautomericOutputAllowed ? TAUT_YES : TAUT_NON];
  is = pINChISort;
  is2 = (bOutType == OUT_TN) ? pINChISortTautAndNonTaut[TAUT_NON] : NULL;

  for (i = 0, is2 = pINChISortTautAndNonTaut[TAUT_NON]; i < num_components;
       i++, is++, is2 ? is2++ : NULL) {
    CompINChILayers(is, is2, sDifSegs, bFixTranspChargeBug);
    bNonTautIsIdenticalToTaut =
        bNonTautIsIdenticalToTaut && !CompINChITautVsNonTaut(is, is2, 1);
    ...
    if (bTautomericOutputAllowed) {
      ...
      for (j = TAUT_YES; j < TAUT_NUM; j++) {
        ...
        case OUT_N1: /* x[TAUT_NON]: non-tautomeric only -- never happens */
          ...
          ii = TAUT_NON;
          break;
        case OUT_T1: /* x[TAUT_YES]: tautomeric if present otherwise
                        non-tautomeric */
          jj = GET_II(bOutType, is);
          if (jj != j)
            continue;
        ...
        case OUT_TN: /* x[TAUT_YES]: tautomeric if present otherwise
                      * non-tautomeric; x[TAUT_NON]: non-taut only if tautomeric
                      * is present */
          jj = (j == TAUT_YES)   ? GET_II(OUT_T1, is)
               : (j == TAUT_NON) ? GET_II(OUT_NT, is)
                                 : -1;
          if (jj == TAUT_YES) {
            /* Fix12 */
        ...
        if ((pINChI = is->pINChI[jj]) && pINChI->nNumberOfAtoms > 0 &&
          ...
          bTautIsoAt += (pINChI->nNumberOfIsotopicAtoms > 0 ||
                         pINChI->nNumberOfIsotopicTGroups > 0);
        }
      }
    }
  }
  sDifSegs[DIFL_M][DIFS_p_PROTONS] =
      nNumRemovedProtons ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
  sDifSegs[DIFL_MI][DIFS_h_H_ATOMS] =
      bHasIsoH ? DIFV_NEQ2PRECED : DIFV_BOTH_EMPTY;
  ...
  for (i = 0, is = pINChISort; i < num_components; i++, is++) {
    int bCurIso, bCurStereo, bCurIsoStereo, bCurHasIsoStereo /* Fix14 */,
        bCurTaut /*, bCurTaut2*/;
    int bCompExists, bCurIsoHPos, bCurIsoHStereo;
    int bCurStereoSp2, bCurIsoStereoSp2, bCurStereoSp3, bCurIsoStereoSp3,
        bCurIsoStereoSp3Inv;
    ...
    for (j = TAUT_NON; j < TAUT_NUM; j++) {
      ...
      case OUT_TN: /* x[TAUT_YES]: tautomeric if present otherwise
        ...
        if (jj < 0) {
          /* Fix12 */
          if (bTautAndNonTaut && bTautIsNonTaut && j == TAUT_NON &&
              0 <= (jj = GET_II(OUT_T1, is)) && !is->pINChI[jj]->bDeleted &&
              !is->pINChI[jj]->lenTautomer) {
            ; /* the requested non-tautomeric component is in tautomeric
                 position (is->pINChI[TAUT_YES]); process it also as
                 non-tautomeric if Fixed-H layer was requested */
          } else {
            continue;
          }
        }
        ii = j; /* ii is what we wanted; jj is what we found (0 = TAUT_NON:
                   fixed_H, 1 = TAUT_YES: mobile_H) */
        /* -- not used 2004-09-16 ---
        if ( is2 ) {
            jj2 = ( j == TAUT_YES )? GET_II(OUT_T1,is2) : ( j == TAUT_NON )?
        GET_II(OUT_NT,is2) : -1; if ( jj2 >= 0 ) { ii2 = j; } else { ii2 = -1;
            }
        } else {
            jj2 = ii2 = -1;
        }
      ...
      if ((pINChI = is->pINChI[jj]) && pINChI->nNumberOfAtoms > 0) {
        ...
        bCurIso = (pINChI->nNumberOfIsotopicAtoms > 0 ||
                   pINChI->nNumberOfIsotopicTGroups > 0);
        bCurIsoHPos = ((pINChI->nPossibleLocationsOfIsotopicH &&
         ...
         */
        bCurIsoHStereo =
            (bCurIsoHPos && (bTautIsoHNum || bTautIsoAt)) || bCurIso;
        ...
        bCurStereoSp2 =
            pINChI->Stereo && (pINChI->Stereo->nNumberOfStereoBonds > 0);
        bCurHasIsoStereo = bCurStereoSp3 =
            pINChI->Stereo && (pINChI->Stereo->nNumberOfStereoCenters > 0);
        bCurIsoStereoSp2 = bCurIsoHStereo && pINChI->StereoIsotopic &&
                           (pINChI->StereoIsotopic->nNumberOfStereoBonds > 0);
        bCurIsoStereoSp3 = bCurIsoHStereo && pINChI->StereoIsotopic &&
                           (pINChI->StereoIsotopic->nNumberOfStereoCenters > 0);
        bCurIsoStereoSp3Inv =
            bCurIsoStereoSp3 &&
            pINChI->StereoIsotopic
                ->nCompInv2Abs; /* inversion changes sp3 stereo */
        bRequestedRacemicStereo |=
            (0 != (pINChI->nFlags & INCHI_FLAG_RAC_STEREO));

        bRequestedRelativeStereo |=
            (0 != (pINChI->nFlags & INCHI_FLAG_REL_STEREO));
        /* check whether isotopic stereo is same as non-isotopic; if same than
         * do not output isotopic stereo */
        if (bCurStereoSp2 && bCurIsoStereoSp2) {
          bCurIsoStereoSp2 = !Eql_INChI_Stereo(
              pINChI->Stereo, EQL_SP2, pINChI->StereoIsotopic, EQL_SP2, 0);
        }
        if (bCurStereoSp3 && bCurIsoStereoSp3) {
          /* bCurIsoStereoSp3=0 means (iso stereo sp3) = (non-iso stereo sp3) or
           * (iso stereo sp3) = Inv(non-iso stereo sp3) */
          bCurIsoStereoSp3 = !Eql_INChI_Stereo(
              pINChI->Stereo, EQL_SP3, pINChI->StereoIsotopic, EQL_SP3,
          ...
          if (!bCurIsoStereoSp3) {
            ...
            bCurIsoStereoSp3Inv &= (pINChI->StereoIsotopic->nCompInv2Abs !=
        ...
        bCurRelative =
            bCurRelative && (pINChI->Stereo->nNumberOfStereoCenters > 1) &&
            (pINChI->Stereo->nCompInv2Abs != 0) &&
#endif

            bCurIsoRelative = bRequestedRelativeStereo &&
                              (bCurIsoStereoSp3 || bCurIsoStereoSp3Inv);
#if (REL_RAC_STEREO_IGN_1_SC == 1)
        bCurIsoRelative =
            bCurIsoRelative &&
...
#if (REL_RAC_STEREO_IGN_1_SC == 1)
        bCurRacemic = bCurRacemic && (pINChI->Stereo->nCompInv2Abs != 0) &&
                              (pINChI->Stereo->nNumberOfStereoCenters > 0)
                          ? pINChI->Stereo->nNumberOfStereoCenters
                          : 0;
#endif

        bCurIsoRacemic = bRequestedRacemicStereo &&
                         (bCurIsoStereoSp3 || bCurIsoStereoSp3Inv);
#if (REL_RAC_STEREO_IGN_1_SC == 1)
        bCurIsoRacemic =
            bCurIsoRacemic & (pINChI->StereoIsotopic->nCompInv2Abs != 0) &&
                    (pINChI->StereoIsotopic->nNumberOfStereoCenters > 0)
        ...
        if (bRequestedRelativeStereo) {
          bCurStereoSp3 =
              bCurRelative ||
              (bCurStereoSp3 &&
               (pINChI->Stereo->nNumberOfStereoCenters > 1)); /* Fix11 */
        ...
        bIgn_UU_Sp3[ii] |=
            !bCurStereoSp3 && (pINChI->nFlags & INCHI_FLAG_SC_IGN_ALL_UU);
        bIgn_UU_Sp2[ii] |=
            !bCurStereoSp2 && (pINChI->nFlags & INCHI_FLAG_SB_IGN_ALL_UU);
        bIsotopicStereoSp2[ii] |= bCurIsoStereoSp2;
        bIsotopicStereoSp3[ii] |= bCurIsoStereoSp3;
        bIgn_UU_Sp3_Iso[ii] |= !bCurIsoStereoSp3 &&
                               (pINChI->nFlags & INCHI_FLAG_SC_IGN_ALL_ISO_UU);
        bIgn_UU_Sp2_Iso[ii] |= !bCurIsoStereoSp2 &&
                               (pINChI->nFlags & INCHI_FLAG_SB_IGN_ALL_ISO_UU);
        bStereoAbs[ii] |= bCurStereoSp3 && (pINChI->Stereo->nCompInv2Abs != 0);
        bStereoAbsInverted[ii] |=
            bCurStereoSp3 && (pINChI->Stereo->nCompInv2Abs < 0);
        /* Fix08: missing isotopic inverted flag if isotopic = inverted
         * non-isotopic */
        bIsotopicStereoAbsInverted[ii] |=
            (bCurIsoStereoSp3 && (pINChI->StereoIsotopic->nCompInv2Abs < 0)) ||
        ...
        bHardAddRemProton |=
            (0 != (pINChI->nFlags & INCHI_FLAG_HARD_ADD_REM_PROTON));
        if (bCurTaut) {
          bTautomeric |= 1; /* tautomeric representation is present */
        ...
        if (!(bINChIOutputOptions & INCHI_OUT_NO_AUX_INFO) &&
          ...
          int bCurEqu, bCurTautEqu = 0, bCurIsoEqu = 0,
                       bCurIsoTautEqu = 0; /* Fix15-disabled */
          bAtomEqu[ii] |=
              (bCurEqu = bHasEquString(pINChI_Aux->nConstitEquNumbers,
                                       pINChI_Aux->nNumberOfAtoms));
          if (bCurTaut) {
            bTautEqu[ii] |= (bCurTautEqu = bHasEquString(
          ...
          if (bCurIso) {
            bIsotopicAtomEqu[ii] |=
                (bCurIsoEqu =
                     bHasEquString(pINChI_Aux->nConstitEquIsotopicNumbers,
                                   pINChI_Aux->nNumberOfAtoms)) /*|| bCurEqu*/;
            if (bCurTaut) {
              bIsotopicTautEqu[ii] |=
                  (bCurIsoTautEqu = bHasEquString(
                       pINChI_Aux->nConstitEquIsotopicTGroupNumbers,
                       pINChI_Aux->nNumberOfTGroups)) /*|| bCurTautEqu*/;
            }
            /* non-zero if isotopic numbering for inverted isotopic stereo is
             * different */
            bIsotopicOrigNumb[ii] |=
                bCurHasIsoStereo && /* Fix14 */
          ...
          if (bCurIsoStereoSp3 && pINChI->StereoIsotopic->nCompInv2Abs) {
            ...
            bInvIsotopicStereoOrigNumb[ii] |=
                pINChI_Aux->nIsotopicOrigAtNosInCanonOrd &&
                pINChI_Aux->nIsotopicOrigAtNosInCanonOrdInv &&
                (0 !=
                 memcmp(pINChI_Aux->nIsotopicOrigAtNosInCanonOrd,
                        pINChI_Aux->nIsotopicOrigAtNosInCanonOrdInv,
                        sizeof(pINChI_Aux->nIsotopicOrigAtNosInCanonOrd[0]) *
                            pINChI_Aux->nNumberOfAtoms));
          }
          if (pINChI_Aux->OrigInfo &&
              bHasOrigInfo(pINChI_Aux->OrigInfo, pINChI_Aux->nNumberOfAtoms)) {
            bChargesRadVal[ii] |= 1;
          }
        }
      }
  ...
  if (bTautomeric /*&& bTautomericAcid*/) /* "&& bTautomericAcid" commented out
                                             2004-06-02 */
  {
  ...
  if (bRequestedRacemicStereo || bRequestedRelativeStereo) {
    ...
    for (i = 0; i < TAUT_NUM; i++) {
      /* Fix11 */
      bStereoAbsInverted[i] = bStereoAbs[i] = bInvStereo[i] =
          bInvStereoOrigNumb[i] = 0;
      /* bIsotopicRelativeStereo[i]=0 may happen because iso stereo is same or
       * inverted non-iso stereo */
      bIsotopicStereoAbsInverted[i] = bIsotopicStereoAbs[i] =
          bInvIsotopicStereo[i] = bInvIsotopicStereoOrigNumb[i] = 0;
      /* -- commented out: Fix11--
  ...
  if (bXml) {
    ...
    */
    ...
    tot_len =
        sprintf(pStr, "%s<%s %s=\"%s\"", SP(ind), x_basic, x_ver, x_curr_ver);
    ...
    if (bAbcNumbers || bCtPredecessors) {
      ...
      /*  type */
      tot_len += sprintf(pStr + tot_len, " %s=\"%s%s%s\"", x_type, pNumber,
                         pDelim, pCtType);
    }
    ...
    if (bNoStructLabels) {
      ...
      */
      ...
      if (!(szSdfLabel && szSdfLabel[0]) && !(szSdfValue && szSdfValue[0])) {
        ...
        tot_len =
            sprintf(pStr, "%sStructure: %d.%s%s%s%s", pLF, num_input_struct,
    ...
    /* inchi_ios_print( output_file, "%s%s=%s", pLF,
     * (FLAG_SORT_PRINT_ReChI_PREFIX & *pSortPrintINChIFlags)? INCHI_REC_NAME :
     * INCHI_NAME, pLF ); */
   ...
   ****************************************************/
  if (INCHI_BAS == iINChI ||
      !(bINChIOutputOptions & INCHI_OUT_EMBED_REC) /* || !bXml */) {
    ...
    if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -1,
                    bPlainTextTags))
      goto exit_function;
    /*}*/
  ...
  if (num_components2[0] || num_components2[1]) {
    szGetTag(IdentLbl, nTag, bTag1 = INCHI_REC == iINChI ? IL_REC_ : IL_FML_,
             szTag1, &bAlways);
    tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
    tot_len = str_HillFormula(pINChISort, pStr, nStrLen, tot_len, &bOverflow,
                              bOutType, num_components, bUseMulipliers);
  ...
  tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
  tot_len2 =
      str_Connections(pINChISort, pStr, nStrLen, tot_len, &bOverflow, bOutType,
                      ATOM_MODE, num_components, bUseMulipliers);
  /* current version does not output empty (";;;;") connectivity */
  if (tot_len != tot_len2 /*|| !bXml*/) { /* 2004-06-30: never output empty
                                             connection table */
    tot_len = tot_len2;
    if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -2,
                    bPlainTextTags))
  ...
  if (INCHI_SEGM_FILL ==
      INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_h_H_ATOMS])) {
    szGetTag(IdentLbl, nTag, bTag1 = IL_ALLH, szTag1, &bAlways);
    tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
    tot_len2 =
        str_H_atoms(pINChISort, pStr, nStrLen, tot_len, &bOverflow, bOutType,
                    ATOM_MODE, TAUT_MODE, num_components, bUseMulipliers);
...
repeat_INChI_output:
  ...
  if (nSegmAction) {
    ...
    if (INCHI_SEGM_FILL == nSegmAction) {
      tot_len = str_Charge2(
          pINChISort, pINChISort2, pStr, nStrLen, tot_len, &bOverflow, bOutType,
          num_components, bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
      bNonTautNonIsoIdentifierNotEmpty += bSecondNonTautPass;
    }
    if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr,
                    bXml ? 0 : -nSegmAction, bPlainTextTags))
      goto exit_function;
    inchi_ios_print(output_file, "%s%s", pStr, pLF);
  } else {
    if (!bXml) {
  ...
  if (iCurTautMode == TAUT_YES && !bSecondNonTautPass) {
    ...
    nSegmAction =
        INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_p_PROTONS]);
    if (nSegmAction) {
  ...
  if (INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_b_SBONDS]) ||
    ...
    if ((nSegmAction =
             INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_b_SBONDS]))) {
      ...
      if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                      bXml ? 0 : -nSegmAction, bPlainTextTags))
        goto exit_function;
      inchi_ios_print(output_file, "%s%s", pStr, pLF);
    } else {
      if (!bXml) {
        if (bPlainTextTags == 1)
          inchi_ios_print(output_file, "/"); /* sp2 */
      }
    }
    ...
    if ((nSegmAction =
      ...
      if (INCHI_SEGM_FILL == nSegmAction) {
        tot_len =
            str_Sp3(pINChISort, pINChISort2, pStr, nStrLen, tot_len, &bOverflow,
                    bOutType, TAUT_MODE, num_components, bRelRac,
                    bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
        bNonTautNonIsoIdentifierNotEmpty += bSecondNonTautPass;
      }
      ...
      if (!bXml) {
        if (bPlainTextTags == 1)
          inchi_ios_print(output_file, "/"); /* sp3 */
      }
    }
    ...
    if ((nSegmAction =
      ...
      if (!bXml) {
        if (bPlainTextTags == 1)
          inchi_ios_print(output_file, "/"); /* stereo-abs-inv */
      }
    }

    /* stereo type */
    /*if ( bRacemicStereo[iCurTautMode] || bRelativeStereo[iCurTautMode] ||
     * bStereoAbs[iCurTautMode] )*/
    if ((nSegmAction =
             INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_s_STYPE]))) {
      const char *p_stereo = bRelativeStereo[iCurTautMode]  ? x_rel
                             : bRacemicStereo[iCurTautMode] ? x_rac
                                                            : x_abs;
      szGetTag(IdentLbl, nTag, bTag2 = bTag1 | IL_TYPS, szTag2, &bAlways);
      tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
      if (INCHI_SEGM_FILL == nSegmAction) {
        tot_len +=
            MakeDelim(p_stereo, pStr + tot_len, nStrLen - tot_len, &bOverflow);
        bNonTautNonIsoIdentifierNotEmpty += bSecondNonTautPass;
      }
      if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                      bXml ? 0 : -nSegmAction, bPlainTextTags))
    ...
    if (bXml) {
      /* close stereo */
      ind -= inc;
      if (str_LineEnd(szTag1, 0, nStrLen, &bOverflow, pStr, ind,
                      bPlainTextTags))
        goto exit_function;
  ...
  if (INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_i_IATOMS])) {
     ...
     * Previous condition if( bHasIsotopicAtoms[iCurTautMode] || bIsotopic &&
     * !bXml) did not optput /i in case of only mobile isotopic H
     */
    if ((nSegmAction =
             INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_i_IATOMS]))) {
      ...
      if (INCHI_SEGM_FILL == nSegmAction) {
        tot_len2 = str_IsoAtoms(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                                &bOverflow, bOutType, TAUT_MODE, num_components,
                                bAbcNumbers, bSecondNonTautPass,
    ...
    if ((nSegmAction =
      ...
      tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
      tot_len += MakeIsoHString(num_iso_H, pStr + tot_len, nStrLen - tot_len,
                                TAUT_MODE, &bOverflow);
      bNonTautIsoIdentifierNotEmpty += bSecondNonTautPass;
      if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                      bXml ? 0 : -nSegmAction, bPlainTextTags))
    ...
    if (INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_b_SBONDS]) ||
      ...
      if ((nSegmAction = INChI_SegmentAction(
               sDifSegs[nCurINChISegment][DIFS_b_SBONDS]))) {
        szGetTag(IdentLbl, nTag, bTag3 = bTag2 | IL_DBND, szTag3, &bAlways);
        tot_len = str_LineStart(szTag3, NULL, 0, pStr, ind);
        if (INCHI_SEGM_FILL == nSegmAction) {
          tot_len =
              str_IsoSp2(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                         &bOverflow, bOutType, TAUT_MODE, num_components,
                         bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
          bNonTautIsoIdentifierNotEmpty += bSecondNonTautPass;
        }
        if (str_LineEnd(szTag3, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -nSegmAction, bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s%s", pStr, pLF);
      } else {
        if (!bXml) {
          if (bPlainTextTags == 1)
            inchi_ios_print(output_file, "/"); /* iso sp2 */
        }
      }
      ...
      if ((nSegmAction = INChI_SegmentAction(
               sDifSegs[nCurINChISegment][DIFS_t_SATOMS]))) {
        bRelRac = bIsotopicRelativeStereo[iCurTautMode] ||
                  bIsotopicRacemicStereo[iCurTautMode];
        szGetTag(IdentLbl, nTag, bTag3 = bTag2 | IL_SP3S, szTag3, &bAlways);
        tot_len = str_LineStart(szTag3, NULL, 0, pStr, ind);
        if (INCHI_SEGM_FILL == nSegmAction) {
          tot_len = str_IsoSp3(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                               &bOverflow, bOutType, TAUT_MODE, num_components,
                               bRelRac, bSecondNonTautPass, bOmitRepetitions,
                               bUseMulipliers);
          bNonTautIsoIdentifierNotEmpty += bSecondNonTautPass;
        }
        if (str_LineEnd(szTag3, tot_len, nStrLen, &bOverflow, pStr,
      ...
      if ((nSegmAction = INChI_SegmentAction(
               sDifSegs[nCurINChISegment][DIFS_m_SP3INV]))) {
      ...
      /* isotopic #5: stereo type. Do not output if it has already been output
       * in non-iso */
      if ((nSegmAction =
               INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_s_STYPE]))) {
        const char *p_stereo = bIsotopicRelativeStereo[iCurTautMode]  ? x_rel
                               : bIsotopicRacemicStereo[iCurTautMode] ? x_rac
                                                                      : x_abs;
        szGetTag(IdentLbl, nTag, bTag3 = bTag2 | IL_TYPS, szTag3, &bAlways);
        tot_len = str_LineStart(szTag3, NULL, 0, pStr, ind);
        if (INCHI_SEGM_FILL == nSegmAction) {
          tot_len += MakeDelim(p_stereo, pStr + tot_len, nStrLen - tot_len,
                               &bOverflow);
          bNonTautIsoIdentifierNotEmpty += bSecondNonTautPass;
        }
        if (str_LineEnd(szTag3, tot_len, nStrLen, &bOverflow, pStr,
      ...
      if (bXml) {
         ...
         ************************/
        ind -= inc;
        if (str_LineEnd(szTag2, 0, nStrLen, &bOverflow, pStr, ind,
                        bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s", pStr);
      }
  ...
  if (bOutType == OUT_NONTAUT && bOutputType == OUT_TN && bSecondNonTautPass &&
    ...
    if (0 < bin_AuxTautTrans(pINChISort, pINChISort2, &nTrans_n, &nTrans_s,
      ...
      *pSortPrintINChIFlags |= (INCHI_BAS == iINChI)
  ...
  /**************************************************************
    ...
    then we need to output non-tautomeric data:
       ...
       isotopic stereo
  ***************************************************************/
  if (bOutType == OUT_TN && !bSecondNonTautPass && bNonTautIsIdenticalToTaut &&
  ...
  if (bOutType == OUT_TN &&
      !bNonTautIsIdenticalToTaut /* added 2004-10-04 Fix16 */
    ...
    bOutType = OUT_NONTAUT; /* pick up only non-tautomeric representation of
                               tautomeric */
    ...
    num_components =
        num_comp[iCurTautMode]; /* number of components could change due to
                                   removal of isolated H(+) from tautomeric */
    bFhTag = IL_FIXH;
    ...
    /***** constitution non-taut: dot-disconnected Hill formulas: <formula> --
     * only if different */
    szGetTag(IdentLbl, nTag, bTag1 = IL_FMLF | bFhTag, szTag1, &bAlways);
    tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
    nSegmAction =
        INChI_SegmentAction(sDifSegs[nCurINChISegment][DIFS_f_FORMULA]);
    if (INCHI_SEGM_FILL == nSegmAction) {
      tot_len2 = str_HillFormula2(pINChISort, pINChISort2, pStr, nStrLen,
                                  tot_len, &bOverflow, bOutType, num_components,
                                  bUseMulipliers);
      bNonTautNonIsoIdentifierNotEmpty += bSecondNonTautPass;
    } else {
      tot_len2 = tot_len;
    ...
    if (INCHI_SEGM_FILL == nSegmAction) {
      ...
      /* output the second non-tautomeric item: fixed H -- do not output in xml
       * if empty */
      tot_len2 =
          str_FixedH_atoms(pINChISort, pStr, nStrLen, tot_len, &bOverflow,
                           bOutType, ATOM_MODE, num_components, bUseMulipliers);
      tot_len = tot_len2;
      if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr,
                      bXml ? 0 : -nSegmAction, bPlainTextTags))
        goto exit_function;
  ...
  if (bDisconnectedCoord && INCHI_BAS == iINChI &&
      (bINChIOutputOptions & INCHI_OUT_EMBED_REC) &&
      num_components2[INCHI_REC]) {
    int nRet;
    ...
    nRet = OutputINChI1(pStr, nStrLen, pINChISortTautAndNonTaut2, INCHI_REC,
                        NULL, 0 /*bDisconnectedCoord*/, bOutputType,
                        bINChIOutputOptions | INCHI_OUT_NO_AUX_INFO, bXml,
                        bAbcNumbers, bCtPredecessors, bNoStructLabels,
                        num_components2, num_non_taut2, num_taut2, output_file,
                        log_file, num_input_struct, szSdfLabel, szSdfValue,
                        lSdfId, pSortPrintINChIFlags, save_opt_bits);
    if (!nRet)
  ...
  if (bXml) {
    ...
    if (!bEmbeddedOutputCalled &&
        !bPlainTextCommnts) { /* plain text comment earlier ended with LF */
...
output_aux_info:
  ...
  if (!(bINChIOutputOptions & INCHI_OUT_NO_AUX_INFO)) {
    ...
    if (bXml) {
      ...
      tot_len = sprintf(pStr, "%s<%s %s=\"%s\"", SP(ind), x_aux_basic, x_ver,
                        x_curr_ver);
      if (INCHI_REC == iINChI || (INCHI_BAS == iINChI && bDisconnectedCoord)) {
        tot_len += sprintf(pStr + tot_len, " %s=\"%d\"", x_reconnected, iINChI);
      }
      ...
      if (INCHI_BAS == iINChI) {
        tot_len =
            sprintf(pStr, "AuxInfo="); /* in wINChI window, separate INChI: from
                                          AuxInfo: with blank line */
        inchi_ios_print(output_file, "%s%s%s",
                        /* blank line before AuxInfo in winchi window unless it
                           is an annotation */
    ...
    if (num_components2[0] || num_components2[1]) {
      ...
      tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
      tot_len +=
          sprintf(pStr + tot_len, "%d",
                  (bTautomeric && bTautomericOutputAllowed) ? bTautomeric : 0);
      if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -1,
                      bPlainTextTags))
        goto exit_function;
      inchi_ios_print(output_file, "%s%s", pStr, pLF);
  ...
  repeat_INChI_Aux_output:
     ...
     **************************************************************/
    if (num_components2[0] || num_components2[1]) {
      szGetTag(AuxLbl, nTag,
               bTag1 = (bSecondNonTautPass ? AL_FIXN : AL_ANBR) | bFhTag,
               szTag1, &bAlways);
      tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
    ...
    if (bAtomEqu[iCurTautMode]) {
      /*  aux equ atoms */
      /* 1. Compare to tautomeric equivalence (in case of second, non-taut, pass
       * only) */
      ...
      tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
      tot_len =
          str_AuxEqu(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                     &bOverflow, bOutType, TAUT_MODE, num_components,
                     bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
     ...
     *****************************************************/
    if (bTautomericOutputAllowed && bTautomeric && bTautEqu[iCurTautMode] &&
        !bSecondNonTautPass) {
      /*****************************************************
      ...
      tot_len = str_LineStart(szTag1, NULL, 0, pStr, ind);
      tot_len =
          str_AuxTgroupEqu(pINChISort, pStr, nStrLen, tot_len, &bOverflow,
                           bOutType, TAUT_MODE, num_components, bUseMulipliers);
      if (str_LineEnd(szTag1, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -1,
                      bPlainTextTags))
        goto exit_function;
      inchi_ios_print(output_file, "%s", pStr);
    ...
    if (bInvStereo[iCurTautMode]) {
      ...
      *****************************/
      ...
      tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
      tot_len =
          str_AuxInvSp3(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                        &bOverflow, bOutType, TAUT_MODE, num_components,
                        bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
      if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr, bXml ? 0 : -1,
                      bPlainTextTags))
        goto exit_function;
      inchi_ios_print(output_file, "%s%s", pStr, pLF);
      ...
      **************************************/
      if (bInvStereoOrigNumb[iCurTautMode]) {
        ...
        tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
        tot_len =
            str_AuxInvSp3Numb(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                              &bOverflow, bOutType, TAUT_MODE, num_components,
                              bSecondNonTautPass, bOmitRepetitions);
        if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -1, bPlainTextTags))
    ...
    if (bXml) {
      if (bIgn_UU_Sp2[iCurTautMode] || bIgn_UU_Sp3[iCurTautMode]) {
        ...
        tot_len = PrintXmlStartTag(
            pStr, ind, 3, szTag1,
            (bIgn_UU_Sp2[iCurTautMode]) ? x_ign_uu_sp2 : NULL, 1,
            (bIgn_UU_Sp3[iCurTautMode]) ? x_ign_uu_sp3 : NULL, 1, NULL, 0, NULL,
            0, NULL, 0, NULL, 0);
        inchi_ios_print(output_file, "%s\n", pStr);
      }
    }
  ...
  repeat_INChI_Aux_Iso_output:
    /* if InChI Fixed-H isotopic is empty then do not output corresponding
     * AuxInfo */
    i = bSecondNonTautPass &&
        (*pSortPrintINChIFlags &
         ((INCHI_BAS == iINChI) ? FLAG_SORT_PRINT_NO_IFIX_H_BAS
                                : FLAG_SORT_PRINT_NO_IFIX_H_REC));

    if (bIsotopic && !i &&
        (bIsotopicOrigNumb[iCurTautMode] || bIsotopicAtomEqu[iCurTautMode] ||
         (bTautomericOutputAllowed && bTautomeric &&
          bIsotopicTautEqu[iCurTautMode]) ||
         bInvIsotopicStereo[iCurTautMode] ||
         (bXml &&
          (bIgn_UU_Sp3_Iso[iCurTautMode] || bIgn_UU_Sp2_Iso[iCurTautMode])))) {
      /*************************************/
      ...
      if (bIsotopicOrigNumb[iCurTautMode]) {
        tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
        tot_len =
            str_AuxIsoNumb(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                           &bOverflow, bOutType, TAUT_MODE, num_components,
                           bSecondNonTautPass, bOmitRepetitions);
        if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -1, bPlainTextTags))
        ...
        if (!bXml) {
          ...
          inchi_ios_print(output_file, "%s%s", szTag2,
                          pLF); /* mark isotopic output */
        }
      }
      ...
      if (bIsotopicAtomEqu[iCurTautMode]) {
        ...
        tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
        tot_len =
            str_AuxIsoEqu(pINChISort, pINChISort2, pStr, nStrLen, tot_len,
                          &bOverflow, bOutType, TAUT_MODE, num_components,
                          bSecondNonTautPass, bOmitRepetitions, bUseMulipliers);
        if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -2 /*was -1: Fix15*/, bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s%s", pStr, pLF);
      } else {
        if (!bXml) {
      ...
      /********************************/
      if (bTautomericOutputAllowed && bTautomeric &&
          bIsotopicTautEqu[iCurTautMode]) {
        /********************************************/
        ...
        tot_len = str_LineStart(szTag2, NULL, 0, pStr, ind);
        tot_len = str_AuxIsoTgroupEqu(
            pINChISort, pStr, nStrLen, tot_len, &bOverflow, bOutType, TAUT_MODE,
            num_components, bOmitRepetitions, bUseMulipliers);
        if (str_LineEnd(szTag2, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -2 /*was -1: Fix15*/, bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s%s", pStr, pLF);
      ...
      if (bInvIsotopicStereo[iCurTautMode]) {
        ...
        **************************************/
        ...
        tot_len = str_LineStart(szTag3, NULL, 0, pStr, ind);
        tot_len = str_AuxInvIsoSp3(pINChISort, pINChISort2, pStr, nStrLen,
                                   tot_len, &bOverflow, bOutType, TAUT_MODE,
                                   num_components, bSecondNonTautPass,
                                   bOmitRepetitions, bUseMulipliers);
        if (str_LineEnd(szTag3, tot_len, nStrLen, &bOverflow, pStr,
                        bXml ? 0 : -1, bPlainTextTags))
          goto exit_function;
        ...
        **********************************************/
        if (bInvIsotopicStereoOrigNumb[iCurTautMode]) {
          ...
          tot_len = str_LineStart(szTag3, NULL, 0, pStr, ind);
          tot_len = str_AuxInvIsoSp3Numb(pINChISort, pINChISort2, pStr, nStrLen,
                                         tot_len, &bOverflow, bOutType,
                                         TAUT_MODE, num_components,
                                         bSecondNonTautPass, bOmitRepetitions);
          if (str_LineEnd(szTag3, tot_len, nStrLen, &bOverflow, pStr,
                          bXml ? 0 : -1, bPlainTextTags))
            goto exit_function;
      ...
      if (bXml) {
        if (bIgn_UU_Sp3_Iso[iCurTautMode] || bIgn_UU_Sp2_Iso[iCurTautMode]) {
          ...
          tot_len = PrintXmlStartTag(
              ...
              (bIgn_UU_Sp3_Iso[iCurTautMode]) ? x_ign_uu_sp3 : NULL, 1, NULL, 0,
    ...
    if (bSecondNonTautPass) {
      ...
      if (0 < bin_AuxTautTrans(pINChISort, pINChISort2, &nTrans_n, &nTrans_s,
        ...
        tot_len = str_AuxTautTrans(nTrans_n, nTrans_s, pStr, nStrLen, tot_len,
                                   &bOverflow, TAUT_MODE, num_components);
        if (str_LineEnd(bXml ? tag : p_aux_at_inv_nbr, tot_len, nStrLen,
                        &bOverflow, pStr, bXml ? 0 : -1, bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s", pStr);
        /* detected transposition */
        *pSortPrintINChIFlags |= (INCHI_BAS == iINChI)
    ...
    ***************************************************************/
    ...
    if (bOutType == OUT_TN && bTautomeric && bNonTautomeric &&
        ...
        (*pSortPrintINChIFlags &
         ((INCHI_BAS == iINChI) ? FLAG_SORT_PRINT_NO_IFIX_H_BAS
                                : FLAG_SORT_PRINT_NO_IFIX_H_REC))) {
      bNonTautomeric =
          0; /* bNonTautIdentifierNotEmpty == 0 => no fixed H info 02-10-2995 */
    }
    ...
    if (bOutType == OUT_TN && bTautomeric && bNonTautomeric) {
      ...
      if (bOutType == OUT_NONTAUT && bOutputType == OUT_TN && bTautomeric &&
          bNonTautomeric) {
        /* the second (non-taut) output has been done; restore variables */
        bOutType = OUT_TN;
        iCurTautMode = TAUT_YES;
    ...
    if (!bSecondNonTautPass && pOrigStruct && pOrigStruct->num_atoms &&
      ...
      char *p;
      nMaxLineLen =
          inchi_min(80, nStrLen); /* restrict line length to 80 characters */
      /**********************
         reversibility info
       **********************/
      ...
      for (cur_pos = 0; cur_pos < length; cur_pos = last_pos) {
        if (length - cur_pos >= line_len) {
          ...
          /* search backward for the nearest first atom letter (always
           * uppercase) */
        ...
        if (last_pos > cur_pos) {
          ...
          inchi_ios_print(output_file, "%s%s", pStr,
                          !bXml && bPlainTextTags ? "" : "\n");
        } else {
          break;
        }
      ...
      if (bXml) {
        ind -= inc;
        pStr[0] = '\0';
        if (str_LineEnd(szTag2, 0, nMaxLineLen, &bOverflow, pStr, ind,
                        bPlainTextTags))
          goto exit_function;
        inchi_ios_print(output_file, "%s", pStr);
      } else {
    ...
    if (bDisconnectedCoord && INCHI_BAS == iINChI &&
      ...
      nRet = OutputINChI1(
          pStr, nStrLen, pINChISortTautAndNonTaut2, INCHI_REC, NULL,
          0 /*bDisconnectedCoord*/, bOutputType,
          INCHI_OUT_ONLY_AUX_INFO | bINChIOutputOptions, bXml, bAbcNumbers,
          bCtPredecessors, bNoStructLabels, num_components2, num_non_taut2,
          num_taut2, output_file, log_file, num_input_struct, szSdfLabel,
          szSdfValue, lSdfId, pSortPrintINChIFlags, save_opt_bits);
      if (!nRet)
        goto exit_function; /* error */
    }
    ...
    if (bXml) {
      ind -= inc;
      if (str_LineEnd(x_aux_basic, 0, nStrLen, &bOverflow, pStr, ind,
                      bPlainTextTags))
        goto exit_function;
      inchi_ios_print(output_file, "%s", pStr);
    } else {
...
exit_function:
  ...
  if (bOverflow) {
    ...
    if (bXml) {
      OutputINChIXmlError(output_file, pStr, nStrLen,
                          ind /*, 0*/ /* err number */, pStr, _IS_FATAL);
    } else {
      inchi_ios_print(output_file, "\nFATAL ERROR: %s\n", pStr);
    }
  }
...
int str_LineStart(const char *tag, char *tag2, int val2, char *pStr, int ind) {
  ...
  if (ind >= 0) {
    ...
    if (tag2) {
      ...
      tot_len +=
          sprintf(pStr + tot_len, " %s=\"%d\"%s", tag2, val2, x_close_line);
...
int str_LineEnd(const char *tag, int tot_len, int nStrLen, int *bOverflow,
                char *pStr, int ind, int bPlainTextTags) {
  static int add_tag_len =
      sizeof(x_line_closing) - 1 + sizeof(x_close_line) - 1;
  int tag_len;
    ...
    if (MIN_BOND_LENGTH > fabs(coord)) {
      ...
      if ((q = strchr(szVal, 'e')) || (q = strchr(szVal, 'E')) ||
        ...
        if (e) {
          ...
          len = last + 1 +
                sprintf(szVal + last + 1, "%d",
                        e); /* print exp without leading zeroes and '+' */
        } else {
          /* exponent is zero */
          len = last;
        }
...
int WriteOrigAtoms(int num_inp_atoms, inp_ATOM *at, int *i, char *szBuf,
                   int buf_len, STRUCT_DATA *sd) {
  int j, k, n, len, len0, cur_len, val, bonds_val, mw, parity, num_trans, is_ok,
      b_self;
  ...
  for (j = *i; j < num_inp_atoms;) {
    ...
    if (at[j].p_parity) {
      ...
      for (n = 0, k = 0; n < MAX_NUM_STEREO_ATOM_NEIGH; n++) {
        ...
        if (is_in_the_list(at[j].neighbor, neigh, at[j].valence) &&
          ...
          num_trans =
              n; /* move this neighbor to 0 position permutation parity */
          b_self++;
        } else {
          is_ok = 0;
          break;
      ...
      if (is_ok && b_self <= 1 && b_self + k == MAX_NUM_STEREO_ATOM_NEIGH) {
        num_trans += insertions_sort(nNeighOrder, k, sizeof(nNeighOrder[0]),
                                     comp_AT_RANK);
    ...
    if ((val = needed_unusual_el_valence(at[j].el_number, at[j].charge,
                                         at[j].radical,
                                         at[j].chem_bonds_valence, bonds_val,
                                         at[j].num_H, at[j].valence)) ||
        at[j].charge || at[j].radical || at[j].iso_atw_diff ||
        NUM_ISO_H(at, j) || parity) {
      /* valence */
      if (val) {
      ...
      if (NUM_ISO_H(at, j)) {
        for (k = 0; k < NUM_H_ISOTOPES; k++) {
          if ((val = at[j].num_iso_H[k])) {
            len += sprintf(szCurAtom + len, "%s%c", len == len0 ? "." : "",
                           szIsoH[k]);
            if (val > 1) {
              len += sprintf(szCurAtom + len, "%d", val);
            }
...
int WriteOrigBonds(int num_inp_atoms, inp_ATOM *at, int *i, char *szBuf,
                   int buf_len, STRUCT_DATA *sd) {
  int j, k, k2, kk, len, cur_len, j2 = 0, bond_stereo, bond_char, bond_parity,
                                  bond_parityNM, num_trans;
  char szCurBonds[7 * MAXVAL + 2]; /* num_neigh*(1 byte bond type + 2 bytes for
                                      bond parity up to 4 digits per neighbor
                                      number) + at the end one ';' */
  ...
  for (j = *i; j < num_inp_atoms;) {
    ...
    if (at[j].valence > 1) {
      ...
      num_trans = insertions_sort(nNeighOrder, at[j].valence,
                                  sizeof(nNeighOrder[0]), CompRank);
    } else {
      num_trans = 0;
      nNeighOrder[0] = 0;
    }
    for (kk = 0; kk < at[j].valence; kk++) {
      ...
      if (j2 < j) {
        ...
        k2 = is_in_the_list(at[j2].neighbor, (AT_NUMB)j, at[j2].valence) -
             at[j2].neighbor;
        chain_len = chain_len2 = 0;
        if (at[j].sb_parity[0]) {
          for (m1 = 0; m1 < MAX_NUM_STEREO_BONDS && at[j].sb_parity[m1]; m1++) {
            if (k == at[j].sb_ord[m1]) {
              chain_len = get_opposite_sb_atom(at, j, k, &pnxt_atom, &pinxt2cur,
                                               &pinxt_sb_parity_ord);
              break;
            }
          }
        ...
        if (at[j2].sb_parity[0]) {
          for (m2 = 0; m2 < MAX_NUM_STEREO_BONDS && at[j2].sb_parity[m2];
               m2++) {
            if (k2 == at[j2].sb_ord[m2]) {
              chain_len2 = get_opposite_sb_atom(
                  at, j2, k2, &pnxt_atom2, &pinxt2cur2, &pinxt_sb_parity_ord2);
              break;
            }
          }
        }
        if ((chain_len == 1 && chain_len2 == 1) || /* regular stereobond */
          ...
          for (m = 0; m < MAX_NUM_STEREO_BONDS && at[pcur_atom].sb_parity[m];
               m++) {
          ...
          chain_len2 = 0;
        } else if (chain_len2 > 1 &&
                   j2 > pnxt_atom2) { /* j2 is a cumulene endpoint */
          int m;
          pcur_atom = j2;
          picur2nxt = k2;
        ...
        if (chain_len) {
          ...
          if (bWellDef || bHasMetal) {
            ...
            if (neigh1 < num_inp_atoms) {
              bNeighSwitched1 =
                  (neigh1 !=
                   at[pcur_atom].neighbor[(int)at[pcur_atom]
                                              .sn_ord[picur_sb_parity_ord]]);
            } else {
              AddMOLfileError(sd->pStrErrStruct,
                              "Cannot find 0D stereobond neighbor");
              /*
            ...
            if (neigh2 < num_inp_atoms) {
              bNeighSwitched2 =
                  (neigh2 !=
                   at[pnxt_atom].neighbor[(int)at[pnxt_atom]
                                              .sn_ord[pinxt_sb_parity_ord]]);
            } else {
              AddMOLfileError(sd->pStrErrStruct,
                              "Cannot find 0D stereobond neighbor");
              /*
            ...
            if (neigh1 < num_inp_atoms && neigh2 < num_inp_atoms) {
              if (ATOM_PARITY_WELL_DEF(p1) && ATOM_PARITY_WELL_DEF(p2)) {
                bond_parity =
                    2 - (p1 + p2 + bNeighSwitched1 + bNeighSwitched2) % 2;
              } else {
              ...
              if (bHasMetal) {
                bond_parityNM =
                    2 - (p1NM + p2NM + bNeighSwitched1 + bNeighSwitched2) % 2;
              } else if (p1NM && p2NM) {
...
/******************************************************************************************/
int FillOutOrigStruct(ORIG_ATOM_DATA *orig_inp_data, ORIG_STRUCT *pOrigStruct,
                      STRUCT_DATA *sd) {
  ...
  if (orig_inp_data->szCoord) {
    ...
    while ((len = WriteOrigCoord(orig_inp_data->num_inp_atoms,
                                 orig_inp_data->szCoord, &i, szBuf,
                                 sizeof(szBuf)))) {
      len_coord += len;
    }
    pOrigStruct->szCoord =
        (char *)inchi_malloc((len_coord + 1) * sizeof(pOrigStruct->szCoord[0]));
    i = 0;
    if (pOrigStruct->szCoord &&
        len_coord == WriteOrigCoord(orig_inp_data->num_inp_atoms,
                                    orig_inp_data->szCoord, &i,
                                    pOrigStruct->szCoord, len_coord + 1) &&
  ...
  /* atoms */
  len_atoms = i = 0;
  while ((len = WriteOrigAtoms(orig_inp_data->num_inp_atoms, orig_inp_data->at,
                               &i, szBuf, sizeof(szBuf), sd))) {
    len_atoms += len;
    if (!orig_inp_data->num_inp_atoms)
      break;
  }
  pOrigStruct->szAtoms =
      (char *)inchi_malloc((len_atoms + 1) * sizeof(pOrigStruct->szAtoms[0]));
  i = 0;
  if (pOrigStruct->szAtoms &&
      len_atoms == WriteOrigAtoms(orig_inp_data->num_inp_atoms,
                                  orig_inp_data->at, &i, pOrigStruct->szAtoms,
                                  len_atoms + 1, sd) &&
      i == orig_inp_data->num_inp_atoms) {
    ; /* success */
  } else {
  ...
  len_bonds = 0;
  i = 1;
  while ((len = WriteOrigBonds(orig_inp_data->num_inp_atoms, orig_inp_data->at,
                               &i, szBuf, sizeof(szBuf), NULL))) {
    len_bonds += len;
    if (!orig_inp_data->num_inp_atoms)
      break;
  }
  pOrigStruct->szBonds =
      (char *)inchi_malloc((len_bonds + 2) * sizeof(pOrigStruct->szBonds[0]));
  i = 1;
  if (pOrigStruct->szBonds &&
      len_bonds == WriteOrigBonds(orig_inp_data->num_inp_atoms,
                                  orig_inp_data->at, &i, pOrigStruct->szBonds,
                                  len_bonds + 2, sd) &&
      i == orig_inp_data->num_inp_atoms) {
    ; /* success */
  } else {
...
The second character encodes experimental (InChI 1 extension) options KET and
15T. Each of these options is a binary switch ON/OFF, so there are 2*2=4
combinations, currently encoded by �A� through �D�. Note that anything but 'A'
here would indicate "extended" InChI 1 Also, there is a reservation for future
needs: the 2nd memo char may accommodate two more ON/OFF binary options (at
26-base encoding).
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
void GetSaveOptLetters(unsigned char save_opt_bits, char *let1, char *let2) {
