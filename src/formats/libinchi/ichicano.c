...
int Canon_INChI1(int num_atoms, int num_at_tg, sp_ATOM *at, CANON_STAT *pCS,
                 INCHI_MODE nMode);
int Canon_INChI2(int num_atoms, int num_at_tg, sp_ATOM *at, CANON_STAT *pCS,
                 INCHI_MODE nMode);
int Canon_INChI3(int num_atoms, int num_at_tg, sp_ATOM *at, CANON_STAT *pCS,
...
#define INCHI_CLOCK_T(X)                                                       \
  (clock_t)((double)(X) / 1000.0 * (double)CLOCKS_PER_SEC)
...
void InchiTimeGet(inchiTime *TickEnd) { TickEnd->clockTime = InchiClock(); }
  ...
  if (FullMaxClock > 0) {
    ...
    if (TickEnd->clockTime > TickStart->clockTime) {
      if (TickEnd->clockTime > HalfMaxClock &&
        ...
        /* overflow in TickStart->clockTime, actually TickStart->clockTime was
         * later */
    ...
    if ((TickEnd->clockTime >= 0 && TickStart->clockTime >= 0) ||
      ...
      delta = (MaxPositiveClock - TickEnd->clockTime) +
              (TickStart->clockTime - MinNegativeClock);
      delta = -delta;
    } else if (TickEnd->clockTime <= HalfMinNegativeClock &&
               TickStart->clockTime >= HalfMaxPositiveClock) {
      /* start was earlier than end */
      delta = (MaxPositiveClock - TickStart->clockTime) +
              (TickEnd->clockTime - MinNegativeClock);
    } else {
...
int GetCanonLengths(
    int num_at, sp_ATOM *at, ATOM_SIZES *s,
    T_GROUP_INFO *t_group_info) { /* include taut. groups as additional "atoms"
                                     to the connection table 07-22-2002 */
  int i, nNumCT, nNumBonds, nNumTBonds = 0, nNumDblBondsStereo = 0,
                            nNumAsymCarbStereo = 0, nNumIsotopic = 0;
  T_GROUP *t_group =
      (s->nLenLinearCTTautomer && t_group_info) ? t_group_info->t_group : NULL;
  for (nNumBonds = 0, i = 0; i < num_at; i++) {
    nNumBonds += at[i].valence;
    if (at[i].iso_sort_key) {
      nNumIsotopic++; /* not including tautomeric endpoints that are isotopic
                         only due to mobile atoms */
    }

    if (at[i].parity >
        0) { /* ignore hydrogen isotope parities in at[i].parity2 */
      int j = 0, nStereoBondsToAtom =
                     0; /* number of stereo double bonds at this atom */
      int k;
      for (; j < MAX_NUM_STEREO_BONDS && (k = at[i].stereo_bond_neighbor[j]);
           j++) {
  ...
  s->nLenLinearCTStereoDble =
      inchi_max(s->nLenLinearCTStereoDble, nNumDblBondsStereo);
  s->nLenLinearCTStereoCarb =
      inchi_max(s->nLenLinearCTStereoCarb, nNumAsymCarbStereo);
  if (t_group_info)
    s->nLenIsotopicEndpoints = inchi_max(s->nLenIsotopicEndpoints,
...
/****************************************************************************/
int AllocateCS(CANON_STAT *pCS, int num_at, int num_at_tg, int nLenCT,
               int nLenCTAtOnly, int nLenLinearCTStereoDble,
               int nLenLinearCTIsotopicStereoDble, int nLenLinearCTStereoCarb,
               int nLenLinearCTIsotopicStereoCarb, int nLenLinearCTTautomer,
               int nLenLinearCTIsotopicTautomer, int nLenIsotopic,
               INCHI_MODE nMode, BCN *pBCN) {
#define pCS_CALLOC(PTR, TYPE, LEN)                                             \
  ...
  if ((nMode & CMODE_CT) && nLenCT > 0) {
    ...
    pCS->nMaxLenLinearCT = pCS->nLenLinearCT = nLenCT;
  ...
  if ((nMode & CMODE_ISO) && nLenIsotopic > 0) {
    ...
    pCS->nMaxLenLinearCTIsotopic = pCS->nLenLinearCTIsotopic = nLenIsotopic;
  }
  /* isotopic tautomeric */
  if ((nMode & CMODE_ISO) && CANON_MODE_TAUT == (nMode & CANON_MODE_TAUT)) {
    if (nLenLinearCTIsotopicTautomer > 0) {
      num_err += !pCS_CALLOC(LinearCTIsotopicTautomer, AT_ISO_TGROUP,
                             nLenLinearCTIsotopicTautomer);
      pCS->nMaxLenLinearCTIsotopicTautomer = pCS->nLenLinearCTIsotopicTautomer =
          nLenLinearCTIsotopicTautomer;
    }
    if (num_t_groups > 0) {
  ...
  if ((nMode & CMODE_STEREO) && nLenLinearCTStereoDble > 0) {
    num_err +=
        !pCS_CALLOC(LinearCTStereoDble, AT_STEREO_DBLE, nLenLinearCTStereoDble);
    num_err += !pCS_CALLOC(LinearCTStereoDbleInv, AT_STEREO_DBLE,
                           nLenLinearCTStereoDble);
    pCS->nLenLinearCTStereoDbleInv = pCS->nMaxLenLinearCTStereoDble =
        pCS->nLenLinearCTStereoDble = nLenLinearCTStereoDble;
  }
  if ((nMode & CMODE_STEREO) && nLenLinearCTStereoCarb > 0) {
    num_err +=
        !pCS_CALLOC(LinearCTStereoCarb, AT_STEREO_CARB, nLenLinearCTStereoCarb);
    num_err += !pCS_CALLOC(LinearCTStereoCarbInv, AT_STEREO_CARB,
                           nLenLinearCTStereoCarb);
    pCS->nLenLinearCTStereoCarbInv = pCS->nMaxLenLinearCTStereoCarb =
  ...
  if ((nMode & CMODE_ISO_STEREO) && nLenLinearCTIsotopicStereoDble > 0) {
    num_err += !pCS_CALLOC(LinearCTIsotopicStereoDble, AT_STEREO_DBLE,
                           nLenLinearCTIsotopicStereoDble);
    num_err += !pCS_CALLOC(LinearCTIsotopicStereoDbleInv, AT_STEREO_DBLE,
                           nLenLinearCTIsotopicStereoDble);
  ...
  if ((nMode & CMODE_ISO_STEREO) && nLenLinearCTIsotopicStereoCarb > 0) {
    num_err += !pCS_CALLOC(LinearCTIsotopicStereoCarb, AT_STEREO_CARB,
                           nLenLinearCTIsotopicStereoCarb);
    num_err += !pCS_CALLOC(LinearCTIsotopicStereoCarbInv, AT_STEREO_CARB,
                           nLenLinearCTIsotopicStereoCarb);
  ...
  if ((nMode & CMODE_ISO_STEREO) && (nLenLinearCTIsotopicStereoDble > 0 ||
                                     nLenLinearCTIsotopicStereoCarb > 0)) {
    ...
    if ((nMode & CMODE_TAUT) && nLenLinearCTTautomer > 0 && num_t_groups > 0) {
      num_err +=
          !pCS_CALLOC(nCanonOrdIsotopicStereoTaut, AT_RANK, num_t_groups);
    }
  }
  if (((nMode & CMODE_STEREO) &&
       (nLenLinearCTStereoDble > 0 || nLenLinearCTStereoCarb > 0)) ||
      ((nMode & CMODE_ISO_STEREO) && (nLenLinearCTIsotopicStereoDble > 0 ||
                                      nLenLinearCTIsotopicStereoCarb > 0))) {
  ...
  if ((nMode & CMODE_CT) && (nMode & CMODE_TAUT) && nLenLinearCTTautomer > 0) {
    ...
    pCS->nMaxLenLinearCTTautomer = pCS->nLenLinearCTTautomer =
        nLenLinearCTTautomer;
    if (num_t_groups > 0) {
...
int FillIsotopicAtLinearCT(int num_atoms, sp_ATOM *at,
  ...
  if (LinearCTIsotopic && nMaxLenLinearCTIsotopic > 0) {
    memset(LinearCTIsotopic, 0,
           nMaxLenLinearCTIsotopic * sizeof(LinearCTIsotopic[0]));
  } else {
    return 0;
  }
  ...
  for (rank = 1; rank <= num_atoms; rank++) {
    ...
    *****************************************************/
    ...
    if (at[i].iso_atw_diff || k) {
      ...
      LinearCTIsotopic[nLinearCTIsotopicLen].num_1H =
          (NUM_H)(k ? at[i].num_iso_H[0] : 0);
      LinearCTIsotopic[nLinearCTIsotopicLen].num_D =
          (NUM_H)(k ? at[i].num_iso_H[1] : 0);
      LinearCTIsotopic[nLinearCTIsotopicLen].num_T =
          (NUM_H)(k ? at[i].num_iso_H[2] : 0);
...
int FillTautLinearCT2(int num_atoms, int num_at_tg, int bIsoTaut,
  ...
  const static int max_num_iso = sizeof(LinearCTIsotopicTautomer->num) /
                                     sizeof(LinearCTIsotopicTautomer->num[0]) +
                                 T_NUM_NO_ISOTOPIC;
  ...
  ****************************************************************************/
  ...
  if (num_at_tg > num_atoms && t_group_info && t_group_info->num_t_groups) {
    ...
    AT_NUMB *tSymmRank =
        tGroupNumber + TGSO_SYMM_RANK * num_t_groups; /*  equivalence */
    ...
    /* memcpy( tPrevGroupNumber, tGroupNumber,
     * num_t_groups*sizeof(tPrevGroupNumber[0])); */
    ...
    /*  Sort enpoints within each tautomeric group according to the canonical
     * ranks */
    pn_RankForSort = nRank;
    for (i = 0; i < num_t_groups; i++) {
      qsort(t_group_info->nEndpointAtomNumber +
                (int)t_group_info->t_group[i].nFirstEndpointAtNoPos,
            t_group_info->t_group[i].nNumEndpoints,
            sizeof(t_group_info->nEndpointAtomNumber[0]), CompRank);
    }
    /* fill out LinearCTTautomer */
    if (nMaxLenLinearCTTautomer) {
      max_len = T_GROUP_HDR_LEN * t_group_info->num_t_groups +
                t_group_info->nNumEndpoints + 1;
     ...
     * 2:       number of neg. charges    ( t_group->num[1] )  {note:
     *T_NUM_NO_ISOTOPIC=2} ENDPOINT RANKS 3..N+2:  sorted tautomer group
     *endpoint ranks; the sorting order is in
     *          t_group_info->nEndpointAtomNumber[t_group->nFirstEndpointAtNoPos+j],
     *j=0..N-1
     *
     * End mark : N==0
     ****************************************************************/
     ...
     */
    num_num = max_num_num; /*  always include isotopic info; ignore it at the CT
                              comparison step. */
    for (i = 0; i < t_group_info->num_t_groups; i++) {
       ...
       *******************************************************/
      /*  check length */
      if (CHECK_OVERFLOW(len + T_GROUP_HDR_LEN + t_group->nNumEndpoints,
                         max_len))
        return CT_OVERFLOW; /*   <BRKPT> */
      ...
      /*  t_group header: */
      /*  (a) number of mobile groups in the t_group (number of H + number of
       * (-) ) and */
      /*  (b) number of mobile negative charges (-) in the t_group */
      for (j = 0; j < T_NUM_NO_ISOTOPIC; j++) {
        LinearCTTautomer[len++] = t_group->num[j];
      }
      /*  t_group endpoint ranks link the group to the tautomeric endpoint atoms
       * in the structure */
      /*  according to their ranks */
      for (j = 0, offset = t_group->nFirstEndpointAtNoPos;
           j < t_group->nNumEndpoints; j++) {
        LinearCTTautomer[len++] =
            nRank[(int)t_group_info->nEndpointAtomNumber[offset + j]];
      }
    }
    if (nMaxLenLinearCTTautomer) {
    ...
    if (nMaxLenLinearCTIsotopicTautomer &&
      ...
      for (i = 0; i < t_group_info->num_t_groups; i++) {
        ...
        for (j = T_NUM_NO_ISOTOPIC; j < max_num_num && j < max_num_iso; j++) {
          /*  num_T, num_D, num_1H */
          LinearCTIsotopicTautomer[len_iso].num[j - T_NUM_NO_ISOTOPIC] =
              t_group->num[j];
        }
        /*  link to tautomer group LinearCTTautomer[i]:  */
        LinearCTIsotopicTautomer[len_iso++].tgroup_num =
            (AT_NUMB)(i + 1); /*  t_group isotopic rank */
      }
    }
    if (nMaxLenLinearCTIsotopicTautomer) {
      if (*pnLenLinearCTIsotopicTautomer &&
...
int UpdateFullLinearCT(int num_atoms, int num_at_tg, sp_ATOM *at,
                       AT_RANK *nRank, AT_RANK *nAtomNumber, CANON_STAT *pCS,
                       int bFirstTime) {
  /* at[i].init_rank = initial ranks before canonizing */
  /* nRank[i]  = new ordering number for atoms: nRank=1,2,.. */
  /* nAtomNumber[r] = orig. atom number= 0,1,...  for r = nRank-1  */
...
#error Undefined or wrong definition of CT_ATOMID
    ...
    ********************************************************/
    ...
    pn_RankForSort = nRank;
    insertions_sort(nNeighborNumber, (size_t)num_neigh,
                    sizeof(nNeighborNumber[0]), CompNeighborsAT_NUMBER);

    for (k = 0; k < num_neigh; k++) {
      /* rank = (new current atom Rank) */
      if ((int)(r_neigh = (AT_NUMB)
                    nRank[(int)at[i].neighbor[(int)nNeighborNumber[k]]])
...
#else
#if (CT_ATOMID == CT_ATOMID_IS_INITRANK)
    r0_at_type = (AT_NUMB)rank; /* current Rank or  (AT_NUMB)at[i].init_rank;
                                   ==> chemical + neighborhood ID */
#else
#if (CT_ATOMID == CT_ATOMID_DONTINCLUDE)
#else
#error Undefined or wrong definition of CT_ATOMID
    ...
    ********************************************************/
    ...
    nEndpointAtomNumber = t_group_info->nEndpointAtomNumber +
                          (int)t_group[i].nFirstEndpointAtNoPos;
    pn_RankForSort = nRank;
    num_neigh = (int)t_group[i].nNumEndpoints;
    insertions_sort(nEndpointAtomNumber, (size_t)num_neigh,
                    sizeof(nEndpointAtomNumber[0]), CompRank);
...
/* if (*bChanged & 1) then nSymmRank has been rearranged because for some r
   ...
   if (*bChanged & 2) then ranks nTempRank[] from nSymmRank[] differ from input
   nCurrRank[]
...
*/
int FixCanonEquivalenceInfo(int num_at_tg, AT_RANK *nSymmRank,
                            AT_RANK *nCurrRank, AT_RANK *nTempRank,
                            AT_NUMB *nAtomNumber, int *bChanged) {
  ...
  */
  pn_RankForSort =
      nSymmRank; /* minimal class representatives: min ranks for equiv. atoms */
  ...
  /* eq. info contains min. possible ranks for eq. atoms; nCurrRank contains
   * max. possible ranks */
  nNumDiffRanks =
      SortedEquInfoToRanks(nSymmRank /*inp*/, nTempRank /*out*/, nAtomNumber,
                           num_at_tg, &bChangeSymmRank);
  /* check whether nCurrRank is same as new initial ranks calculated from
   * nSymmRank[] */
  bChangeCurrRank =
      memcmp(nCurrRank, nTempRank, num_at_tg * sizeof(nTempRank[0]));
  ...
  if ( bChangeSymmRank || bChangeCurrRank ) {
       ...
       For such structures the found canonical numbers of the constitutionally
  equivalent atoms are not contiguous (see nCanonRank and nSymmRank examples
  below). Here arrays nCurrRank, nAtomNumber, and nSymmRank are changed so that
  later the contiguous canonical numbers for equivalent atoms can be obtained
       ...
       the arrays (arranged according to ordering in nAtomNumberTemp) are:
                               before SortedEquInfoToRanks  after
  SortedRanksToEquInfo orig. atom nos.,nAtomNumberTemp:  {4 5 6 7 0 1 2 3}   {4
  5 6 7 0 1 2 3} order numbers for sorted  ranks:  {0 1 2 3 4 5 6 7}   {0 1 2 3
  4 5 6 7} canonical numbering, nCanonRank:  {1 2 5 6 3 4 7 8}   {1 2 5 6 3 4 7
  8} constit. equivalence, nSymmRank:  {1 1 1 1 3 3 7 7}   {1 1 1 1 5 5 7 7}
  used later initial equivalence,  nCurrRank:  {6 6 6 6 6 6 8 8}   {4 4 4 4 6 6
  8 8} used later initial numbering,  nAtomNumber:  {2 3 4 7 0 1 6 7}   {0 1 2 3
  4 5 6 7} used later final, no stereo, no isotopic, after  III.
  GetCanonRanking: final canon. numbers, nCanonRank:                     {1 2 3
  4 5 6 7 8} final
  }
  ----------------------------------------------------------------------------------*/
  if (bChangeCurrRank) {
  ...
  if (bChangeSymmRank) {
    SortedRanksToEquInfo(nSymmRank /*out*/, nTempRank /*inp*/, nAtomNumber,
                         num_at_tg);
  }
  if (bChanged) {
...
int Canon_INChI3(int num_atoms, int num_at_tg, sp_ATOM *at, CANON_STAT *pCS,
  ...
  I.2      GetCanonRanking(): Find constitutionally equivalent atoms and
  possibly canonical numbering 1.3      Fix canonical equivalence info if needed
  (if the fix is needed then the numbering is not canonical)

  II.   Get final non-isotopic canonical numbering. Simultaneously obtain
  non-minimal isotopic and stereo CTs GetCanonRanking() with pCS->bKeepSymmRank
  = 1 FillOutStereoParities() (create initial stereo descriptors) save
  non-isotopic canonicalization final results hide isotopic and tautomeric
  results (for historical reasons only)


  III.  Find constitutionally equivalent isotopic atoms (for isotopic stereo
  canonicalization) III.1    Allocate more memory III.2    fill allocated memory
  with the initial data III.3    duplicate, save old and add isotopic info to
  the new pCS->t_group_info III.4    Prepare initial isotopic ranks for
  GetCanonRanking() III.5    GetCanonRanking() to Find constitutionally
  equivalent ISOTOPIC atoms and tautomer groups III.6    Fix canonical isotopic
  equivalence information and derive ranks out of it
  ...
  *****************************************************************/
  ...
  int bTaut = (num_at_tg > num_atoms) && pCS->t_group_info &&
              pCS->t_group_info->num_t_groups && pCS->t_group_info->t_group;
  /* special case: induced by exchangable isotopic H inequivalence of atoms in
   * formally non-tautomeric structure */
  int bIsoXchgH =
      pCS->t_group_info && pCS->t_group_info->nNumIsotopicEndpoints > 1 &&
  ...
  int bHasIsotopicCanonData =
      (ftcn->PartitionCtIso.AtNumber && ftcn->PartitionCtIso.Rank &&
       ftcn->nSymmRankCtIso);
  /* bHasIsotopicCanonData==0 means
   *       (1) No isotopic atoms in the component OR
   *       (2) the component has only exchangable isotopic H that do not change
   * canonical numbering and equivalence.
   */
  T_GROUP_INFO *t_group_info1 = bTaut ? pCS->t_group_info : NULL;
  /*int bIsoXchgH = t_group_info1 && t_group_info1->nNumIsotopicEndpoints &&
   * t_group_info1->nIsotopicEndpointAtomNumber;*/
  /* isotopic canonicalization */
  int bCanonIsotopic =
      bHasIsotopicCanonData && (nMode & CMODE_ISO) &&
      (pCS->LinearCTIsotopic || pCS->LinearCTIsotopicTautomer || bIsoXchgH);
  /* stereo canonicalization */
  int bCanonStereo = (nMode & CMODE_STEREO) &&
                     (pCS->LinearCTStereoDble || pCS->LinearCTStereoCarb);
  /* stereo isotopic canonicalization */
  int bCanonIsoStereo =
      bHasIsotopicCanonData && (nMode & CMODE_ISO_STEREO) &&
      (pCS->LinearCTIsotopicStereoDble || pCS->LinearCTIsotopicStereoCarb) &&
      bCanonIsotopic;
  ...
  *pCS2 = *pCS; /* save input information and pointers to allocated memory */
  ...
  /* linear CT, H */
  memcpy(pCS->LinearCT, ftcn->LinearCt,
         ftcn->nLenLinearCt * sizeof(pCS->LinearCT[0]));
  ...
  if (pCS->nSymmRank) {
    memcpy(pCS->nSymmRank, ftcn->nSymmRankCt,
           num_at_tg * sizeof(pCS->nSymmRank[0]));
  }
  if (pCS->nCanonOrd) {
    memcpy(pCS->nCanonOrd, ftcn->PartitionCt.AtNumber,
           num_at_tg * sizeof(pCS->nCanonOrd[0]));
    pCS->nLenCanonOrd = num_atoms;
  }
  if (ftcn->iso_exchg_atnos && pCS->nExchgIsoH) {
    for (i = 0; i < num_atoms; i++) {
      pCS->nExchgIsoH[i] =
          !ftcn->iso_exchg_atnos[i]; /* (pCS->nExchgIsoH[i]==1) => tautomeric or
                                        hetero atoms that may exchange isotopic
                                        H */
    }
  }
  /* 2. isotopic */
  ...
  if (bCanonIsotopic) {
    ...
    if (pCS->nSymmRankIsotopic) {
      memcpy(pCS->nSymmRankIsotopic, ftcn->nSymmRankCtIso,
             num_at_tg * sizeof(pCS->nSymmRankIsotopic[0]));
    }
    ...
    nRet = FillIsotopicAtLinearCT(
        num_atoms, at, ftcn->PartitionCtIso.AtNumber, pCS->LinearCTIsotopic,
        pCS->nMaxLenLinearCTIsotopic, &pCS->nLenLinearCTIsotopic);
  ...
  /* fill out tautomeric groups, isotopic and non-isotopic tautomeric CT and
   * t_group_info1->tGroupNumber */
  if (bTaut) {
    bIsoTaut = bIsoTaut && ftcn->PartitionCtIso.Rank &&
               ftcn->PartitionCtIso.AtNumber && ftcn->nSymmRankCtIso;
    nRet = FillTautLinearCT2(
        num_atoms, num_at_tg, bIsoTaut, ftcn->PartitionCt.Rank,
        ftcn->PartitionCt.AtNumber, ftcn->nSymmRankCt,
        ftcn->PartitionCtIso.Rank, ftcn->PartitionCtIso.AtNumber,
        ftcn->nSymmRankCtIso, pCS->LinearCTTautomer,
        pCS->nMaxLenLinearCTTautomer, &pCS->nLenLinearCTTautomer,
        pCS->LinearCTIsotopicTautomer, pCS->nMaxLenLinearCTIsotopicTautomer,
        &pCS->nLenLinearCTIsotopicTautomer, t_group_info1);
    ...
    if (nRet <= 0) {
      ...
      if (pCS->nSymmRankTaut) {
        memcpy(pCS->nSymmRankTaut, tSymmRank,
               num_t_groups * sizeof(pCS->nSymmRank[0])); /* fixed 5-23-02 */
      }
      if (pCS->nCanonOrdTaut) {
        memcpy(pCS->nCanonOrdTaut, tGroupNumber,
               num_t_groups * sizeof(pCS->nCanonOrdTaut[0]));
      ...
      if (bCanonIsotopic /*&& pCS->nLenLinearCTIsotopicTautomer*/) {
        ...
        memcpy(pCS->nCanonOrdIsotopicTaut, tiGroupNumber,
               num_t_groups * sizeof(pCS->nCanonOrdIsotopicTaut[0]));
  ...
  if (pCS->LinearCT2) {
    memcpy(pCS->LinearCT2, pCS->LinearCT,
           sizeof(pCS->LinearCT2[0]) * pCS->nLenLinearCT);
  ...
  ************************************************/
  ...
  if (!NeighList || !nAtomNumber || !nTempRank || !nRank || !pCS->LinearCT) {
  ...
  *pCS2 = *pCS; /* save input information and pointers to allocated memory */
  ...
  if (bCanonStereo || bCanonIsoStereo) {
    ...
    /* stereo or isotopic canonicalization: we need a second set of ranks for
     * mapping */
  ...
  if (bCanonStereo) {
    ...
    /* *pCS2 = *pCS; */ /* save input information and pointers to allocated
                           memory */

    /* initial ranking for non-isotopic mapping */
    memcpy(nAtomNumber, ftcn->PartitionCt.AtNumber,
           num_at_tg * sizeof(nAtomNumber[0]));
    ...
    /* nSymmRank changes if canonical numbers of constitutionally equivalent
     * atoms are not contiguous */
    nNumCurrRanks = FixCanonEquivalenceInfo(num_at_tg, nSymmRank /* in&out*/,
                                            nRank, nTempRank /* out */,
                                            nAtomNumber /* in&out */, NULL);
    /* atom numbers in canonical order */
    memcpy(pCS->nPrevAtomNumber, ftcn->PartitionCt.AtNumber,
           num_at_tg * sizeof(nAtomNumber[0]));

    /* fill stereo part of the connection table with initial (not optimized)
     * parities */
    /* input
    ...
    */
    nRet = FillOutStereoParities(at, num_atoms, ftcn->PartitionCt.Rank,
                                 ftcn->PartitionCt.AtNumber, nRank, nAtomNumber,
                                 pCS, 0 /* bIsotopic */);
    /* output
    pCS->LinearCTStereoDble
    pCS->LinearCTStereoCarb
    ...
    */
    ...
    if (!nCanonRankStereo)
      nCanonRankStereo =
          (AT_RANK *)qmalloc(num_max * sizeof(*nCanonRankStereo));
    ...
    if (pRankStack1 && pRankStack2 && nCanonRankStereo &&
    ...
    /* set the 1st ranks in the rest of the stack to zero: prepare for ranks
     * reuse */
    ...
    pCS->lNumBreakTies = pCS->lNumNeighListIter = pCS->lNumTotCT =
        pCS->lNumDecreasedCT = pCS->lNumRejectedCT = pCS->lNumEqualCT = 0;
    pCS->bKeepSymmRank = 0;
    pCS->bFirstCT =
        1; /* To fill out nCanonRankStereo[] in map_stero_atoms2() */

    /******************************************************************************
         nCanonRank contains input canonical numbering
         nCanonRankStereo will be filled with a transposition of canonical
       numbering which (1) keeps connection table unchanged and (2) provides
       minimal stereo descriptors in pCS->LinearCTStereoDble
       (length=pCS->nLenLinearCTStereoDble) pCS->LinearCTStereoCarb
       (length=pCS->nLenLinearCTStereoCarb)
     */
    nRet = map_stereo_bonds4(
        at, num_atoms, num_at_tg, num_max, 0, ftcn->PartitionCt.Rank,
        ftcn->PartitionCt.AtNumber, nCanonRankStereo, nSymmRank, pRankStack1,
        pRankStack2, nTempRank, nNumCurrRanks, nSymmStereo, NeighList, pCS,
        cur_tree, 0 /* nNumMappedBonds */, vABParityUnknown);
    ...
    if (RETURNED_ERROR(nRet)) {
      ...
      if (!nRet) {
        ...
        pCS2->nLenLinearCTStereoCarb = pCS->nLenLinearCTStereoCarb =
            -abs(pCS->nLenLinearCTStereoCarb);
        pCS2->nLenLinearCTStereoDble = pCS->nLenLinearCTStereoDble =
            -abs(pCS->nLenLinearCTStereoDble);
      ...
      if (bTaut && pCS->nCanonOrdStereoTaut) {
        if (0 < (nRet = SortTautomerGroupsAndEndpoints(
                     t_group_info1, num_atoms, num_at_tg, nCanonRankStereo))) {
          /*non-isotopic contains symmetry ranks */
          int num_t_groups = t_group_info1->num_t_groups;
          AT_NUMB *tGroupNumber = t_group_info1->tGroupNumber;
          /*AT_NUMB *tiSymmRank        = tGroupNumber +
           * TGSO_SYMM_IRANK*num_t_groups; */
          memcpy(pCS->nCanonOrdStereoTaut, tGroupNumber,
                 num_t_groups * sizeof(pCS->nCanonOrdStereoTaut[0]));
          pCS->nLenCanonOrdStereoTaut =
              (bFailed) ? -num_t_groups : num_t_groups;
        } else if (RETURNED_ERROR(nRet)) {
          goto exit_function;
        } else {
          nRet = 0;
        }
        /*SortTautomerGroupsAndEndpoints( t_group_info1, nCanonRank ); */ /* ???
                                                                             return
                                                                             to
                                                                             non-isotopic
                                                                             canonical
    ...
    if (!nCanonRankStereoInv)
      nCanonRankStereoInv =
          (AT_RANK *)qmalloc(num_max * sizeof(*nCanonRankStereoInv));
    if (!nCanonRankStereoInv) {
      nRet = CT_OUT_OF_RAM; /*  <BRKPT> */
      goto exit_function;
    }
    /* copy previous non-isotopic stereo canonicalization results to Inv initial
     * data */
    /* assign pointers */
    ...
    /* copy the lengths */
    pCS2->nLenLinearCTStereoDbleInv = pCS->nLenLinearCTStereoDbleInv =
        pCS->nLenLinearCTStereoDble = pCS2->nLenLinearCTStereoDble;

    pCS2->nLenLinearCTStereoCarbInv = pCS->nLenLinearCTStereoCarbInv =
    ...
    memcpy(nCanonRankStereoInv, nCanonRankStereo,
           num_max * sizeof(nCanonRankStereoInv[0]));
    if (pCS->nCanonOrdStereoInv && pCS->nCanonOrdStereo) {
      /* in case there is nothing to invert */
      memcpy(pCS->nCanonOrdStereoInv, pCS->nCanonOrdStereo,
             num_at_tg * sizeof(pCS->nCanonOrdStereoInv[0]));
    }
    ...
    /*********************************************************************************
     * Create initial approximation for the minimization of the stereo
     * descriptors: invert stereogenic atom parities, one parity in each allene,
     * all parities in pCS->LinearCTStereoCarb and allene parities in
     * pCS->nLenLinearCTStereoDble
     */
    nRet = InvertStereo(at, num_at_tg, nCanonRankStereo, nTempRank, pCS,
                        1 /* bInvertLinearCTStereo */);
    if (RETURNED_ERROR(nRet)) {
      goto exit_function;
    } else if (nRet > 0) {
      /* InvertStereo() has done some changes */
      nRet = 0;
      /* FillOutStereoParities() has already been called to fill out these 2
       * LinearCTs */
      ...
      /******************************************************************************
           ...
           nCanonRankStereoInv will be filled with a transposition of canonical
       numbering which (1) keeps connection table unchanged and (2) provides
       minimal stereo descriptors in pCS->LinearCTStereoDble
       (length=pCS->nLenLinearCTStereoDble) pCS->LinearCTStereoCarb
       (length=pCS->nLenLinearCTStereoCarb)
      ...
      nRet = map_stereo_bonds4(
          at, num_atoms, num_at_tg, num_max, 0, ftcn->PartitionCt.Rank,
          ftcn->PartitionCt.AtNumber, nCanonRankStereoInv, nSymmRank,
          pRankStack1, pRankStack2, nTempRank, nNumCurrRanks, nSymmStereo,
          NeighList, pCS, cur_tree, 0, vABParityUnknown);
      if (RETURNED_ERROR(nRet)) {
        if (nRet == CT_TIMEOUT_ERR)
          goto exit_function;
        else
        ...
        pCS->nLenLinearCTStereoDbleInv = pCS2->nLenLinearCTStereoDbleInv =
            pCS->nLenLinearCTStereoDble;
        pCS->nLenLinearCTStereoCarbInv = pCS2->nLenLinearCTStereoCarbInv =
         ...
         ******************************/
        /*********************************************************************************
         *  invert only stereogenic atom parities and one parity in each allene,
         * DO NOT change parities in pCS->LinearCTStereoCarb and
         * pCS->nLenLinearCTStereoDble
         */
        ...
        if (pCS->nCanonOrdStereoInv) {
          for (i = n = 0; i < num_at_tg; i++) {
            if (nCanonRankStereoInv[i] &&
                (int)nCanonRankStereoInv[i] <= num_at_tg) {
              pCS->nCanonOrdStereoInv[(int)nCanonRankStereoInv[i] - 1] =
                  (AT_NUMB)i;
            } else {
              bFailed++;
            }
  ...
  if (bCanonIsoStereo && bCanonIsotopic) {
    ...
    */
    ...
    memcpy(nAtomNumber, ftcn->PartitionCtIso.AtNumber,
           num_at_tg * sizeof(nAtomNumber[0]));
    ...
    /* nSymmRank will change if canonical numbers of of constitutionally
     * equivalent atoms are not contiguous */
    nNumCurrRanks = FixCanonEquivalenceInfo(num_at_tg, nSymmRank /* in&out*/,
                                            nRank, nTempRank /* out */,
                                            nAtomNumber /* in&out */, NULL);

    memcpy(pCS->nPrevAtomNumber, ftcn->PartitionCtIso.AtNumber,
           num_at_tg * sizeof(nAtomNumber[0]));
    ...
    if (!nCanonRankIsotopicStereo)
      nCanonRankIsotopicStereo =
          (AT_RANK *)qmalloc(num_max * sizeof(*nCanonRankIsotopicStereo));
    if (!nSymmStereo && !(nMode & CMODE_NOEQ_STEREO))
      nSymmStereo = (AT_RANK *)qmalloc((num_max + 1) * sizeof(*nSymmStereo));
    ...
    /* check allocations and assign first 2 elements of pRankStack2 */
    if (pRankStack1 && pRankStack2 && nCanonRankIsotopicStereo &&
        (nSymmStereo || (nMode & CMODE_NOEQ_STEREO))) {

      pRankStack1[0] = pRankStack2[0] =
          nRank; /* pRankStack1[0,1] shall be unchanged */
      pRankStack1[1] = pRankStack2[1] = nAtomNumber;
    } else {
      nRet = CT_OUT_OF_RAM; /*  <BRKPT> */
      goto exit_function;
    ...
    /* at[] has certain members for non-isotopic and isotopic stereo; switch
     * them */
    ...
    nRet = FillOutStereoParities(at, num_atoms, ftcn->PartitionCtIso.Rank,
                                 ftcn->PartitionCtIso.AtNumber, nRank,
                                 nAtomNumber, pCS, 1 /* bIsotopic */);
    if (RETURNED_ERROR(nRet)) {
      goto exit_function; /* program error */
    } else if (!nRet) {
      /* no isotopic stereo */
    ...
    /**************************************************************************************
      ...
      nCanonRankIsotopicStereo will be filled with a transposition of canonical
    numbering that  (1) keeps connection table unchanged and (2) provides
    minimal stereo descriptors in pCS->LinearCTStereoDble
    (length=pCS->nLenLinearCTStereoDble) pCS->LinearCTStereoCarb
    (length=pCS->nLenLinearCTStereoCarb)
    ***************************************************************************************/
    nRet = map_stereo_bonds4(
        ...
        ftcn->PartitionCtIso.AtNumber, nCanonRankIsotopicStereo, nSymmRank,
        pRankStack1, pRankStack2, nTempRank, nNumCurrRanks, nSymmStereo,
        NeighList, pCS, cur_tree, 0, vABParityUnknown);
    if (RETURNED_ERROR(nRet)) {
      goto exit_function;
      ...
      if (!nRet) {
        ...
        if (pCS->nCanonOrdIsotopicStereo) {
          for (i = n = 0; i < num_at_tg; i++) {
            if (nCanonRankIsotopicStereo[i] &&
                (int)nCanonRankIsotopicStereo[i] <= num_at_tg) {
              pCS->nCanonOrdIsotopicStereo[(int)nCanonRankIsotopicStereo[i] -
                                           1] = (AT_NUMB)i;
            } else {
              bFailed++;
            }
        ...
        if (pCS->nCanonOrdIsotopicStereoTaut) {
          if (0 < (nRet = SortTautomerGroupsAndEndpoints(
                       t_group_info1, num_atoms, num_at_tg,
                       nCanonRankIsotopicStereo))) {
            /*non-isotopic contains symmetry ranks */
            int num_t_groups = t_group_info1->num_t_groups;
            AT_NUMB *tGroupNumber = t_group_info1->tGroupNumber;
    ...
    if (!nCanonRankIsotopicStereoInv)
      nCanonRankIsotopicStereoInv =
          (AT_RANK *)qmalloc(num_max * sizeof(*nCanonRankIsotopicStereoInv));
    if (!nCanonRankIsotopicStereoInv) {
      nRet = CT_OUT_OF_RAM; /*  <BRKPT> */
      goto exit_function;
    }
    /* copy previous isotopic stereo canonicalization results to Inv initial
     * data */
    /* assign pointers */
    pCS->LinearCTStereoDble =
        pCS2->LinearCTIsotopicStereoDbleInv; /*  enable stereo */
    pCS->LinearCTStereoCarb = pCS2->LinearCTIsotopicStereoCarbInv;

    /* copy the lengths */
    pCS2->nLenLinearCTIsotopicStereoDbleInv = pCS->nLenLinearCTStereoDbleInv =
        pCS->nLenLinearCTStereoDble = pCS2->nLenLinearCTIsotopicStereoDble;

    pCS2->nLenLinearCTIsotopicStereoCarbInv = pCS->nLenLinearCTStereoCarbInv =
    ...
    memcpy(nCanonRankIsotopicStereoInv, nCanonRankIsotopicStereo,
           num_max * sizeof(nCanonRankIsotopicStereoInv[0]));
     ...
     * pCS->nLenLinearCTStereoDble
     */
    nRet = InvertStereo(at, num_at_tg, nCanonRankIsotopicStereo, nTempRank, pCS,
                        1);
    if (RETURNED_ERROR(nRet)) {
      goto exit_function;
    } else if (nRet > 0) {
      ...
      /**************************************************************************************
        nCanonRankIsotopic contains input canonical numbering
        nCanonRankIsotopicStereo will be filled with a transposition of
        canonical numbering that  (1) keeps connection table unchanged and (2)
        provides minimal stereo descriptors in pCS->LinearCTStereoDble
        (length=pCS->nLenLinearCTStereoDble) pCS->LinearCTStereoCarb
        (length=pCS->nLenLinearCTStereoCarb)
      */
      nRet = map_stereo_bonds4(
          at, num_atoms, num_at_tg, num_max, 0, ftcn->PartitionCtIso.Rank,
          ftcn->PartitionCtIso.AtNumber, nCanonRankIsotopicStereoInv, nSymmRank,
          pRankStack1, pRankStack2, nTempRank, nNumCurrRanks, nSymmStereo,
          NeighList, pCS, cur_tree, 0, vABParityUnknown);
      if (RETURNED_ERROR(nRet)) {
        if (nRet == CT_TIMEOUT_ERR)
         ...
         *  have been restored)
         ******************************/
        nRet = InvertStereo(at, num_at_tg, nCanonRankIsotopicStereo, nTempRank,
                            pCS, 0);
        if (RETURNED_ERROR(nRet)) {
          goto exit_function;
        }
        ...
        if (pCS->nCanonOrdIsotopicStereoInv) {
          for (i = n = 0; i < num_at_tg; i++) {
            if (nCanonRankIsotopicStereoInv[i] &&
                (int)nCanonRankIsotopicStereoInv[i] <= num_at_tg) {
              pCS->nCanonOrdIsotopicStereoInv
                  [(int)nCanonRankIsotopicStereoInv[i] - 1] = (AT_NUMB)i;
            } else {
              bFailed++;
            }
          }
        ...
        pCS->bCmpIsotopicStereo =
            2 + CompareLinCtStereo(pCS->LinearCTIsotopicStereoDbleInv,
                                   pCS->nLenLinearCTIsotopicStereoDbleInv,
...
exit_function:
  ...
  if (pCS->LinearCT2) {
    inchi_swap((char *)&pCS->LinearCT, (char *)&pCS->LinearCT2,
               sizeof(pCS->LinearCT));
    inchi_swap((char *)&pCS->nLenLinearCT, (char *)&pCS->nLenLinearCT2,
               sizeof(pCS->nLenLinearCT));
    inchi_swap((char *)&pCS->nLenLinearCTAtOnly,
               (char *)&pCS->nLenLinearCTAtOnly2,
  ...
  i = 2;
  if (pRankStack1) {
    pRankStack1[0] = pRankStack1[1] = NULL; /* deallocated separately */
  ...
  pCS->NeighList = NULL; /* keep the pointer in pBCN->ftcn[bTaut].NeighList for
  ...
  */
  ...
  return (nRet >= -1) ? num_atoms
                      : nRet; /* cannot easily get number of ranks for now */
}

/**************************************************************************************/
int Canon_INChI(int num_atoms, int num_at_tg, sp_ATOM *at, CANON_STAT *pCS,
                INCHI_MODE nMode, int bTautFtcn) {
