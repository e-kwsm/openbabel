...
/**********************************************************************************/
/*  Sort by insertions */
int insertions_sort(void *base, size_t num, size_t width,
                    int (*compare)(const void *e1, const void *e2)) {
  char *i, *j, *pk = (char *)base;
  int num_trans = 0;
  size_t k;
  for (k = 1; k < num; k++, pk += width) {
    /*for( i = pk, j = pk + width; j > (char*)base && (*compare)(i,j) > 0; j=i,
     * i -= width )*/
    for (i = j = pk + width;
         j > (char *)base && (i -= width, (*compare)(i, j)) > 0;
...
/**********************************************************************************/
/*  Sort by insertions */
int insertions_sort_AT_NUMBERS(AT_NUMB *base, int num,
                               int (*compare)(const void *e1, const void *e2)) {
  AT_NUMB *i, *j, *pk, tmp;
  int k, num_trans = 0;
  for (k = 1, pk = base; k < num; k++, pk++) {
    for (j = (i = pk) + 1, tmp = *j; j > base && (*compare)(i, &tmp) > 0;
         j = i, i--) {
      *j = *i;
      num_trans++;
    }
...
void insertions_sort_NeighList_AT_NUMBERS(NEIGH_LIST base, AT_RANK *nRank) {
  ...
  int k, num = (int)*base++;
  for (k = 1, pk = base; k < num; k++, pk++) {
    for (j = (i = pk) + 1, rj = nRank[(int)*j]; j > base && nRank[(int)*i] > rj;
         j = i, i--) {
      tmp = *i;
      *i = *j;
      *j = tmp;
...
int CompAtomInvariants2Only(const void *a1, const void *a2) {
  const ATOM_INVARIANT2 *pAI1 =
      pAtomInvariant2ForSort + (int)*(const AT_RANK *)a1;
  const ATOM_INVARIANT2 *pAI2 =
      pAtomInvariant2ForSort + (int)*(const AT_RANK *)a2;
  int i;
...
int CompNeighListsUpToMaxRank(const void *a1, const void *a2) {
  ...
  ret = CompareNeighListLexUpToMaxRank(
      pNeighList_RankForSort[*((const AT_RANK *)a1)],
      pNeighList_RankForSort[*((const AT_RANK *)a2)], pn_RankForSort,
...
int CompNeighListRanksOrd(const void *a1, const void *a2) {
  ...
  if (!ret)
    ret = (int)*((const AT_RANK *)a1) -
          (int)*((const AT_RANK *)a2); /*  keep original order if identical */
  return ret;
}
/**********************************************************************************/
...
NEIGH_LIST *CreateNeighListFromLinearCT(AT_NUMB *LinearCT, int nLenCT,
   ...
   * order: parent[i] > neigh[i][0] < neigh[i][1]...<neigh[i][n] < parent[i+1] >
   * neigh[i+1][0] < ... parent[i] < parent[i+1]
   */
  int i, j;
  S_CHAR *valence = NULL;
  NEIGH_LIST *pp = NULL;
...
/***********************************************************************************
 * NEIGH_LIST pp[] is an array of pointers to the lists of neighboring atoms
 * numbers The first number in each list is a number of neighbors. In case of
 * bDoubleBondSquare != 0 neighbors connected by the double bond appear 2 times
...
NEIGH_LIST *CreateNeighList(int num_atoms, int num_at_tg, sp_ATOM *at,
  ...
  NEIGH_LIST *pp =
      (NEIGH_LIST *)inchi_calloc((num_at_tg + 1), sizeof(NEIGH_LIST));
  ...
  if (pp) {
    ...
    if (!bDoubleBondSquare) {
      ...
      for (i = 0, length = 0; i < num_atoms; i++) {
        ...
        for (j = 0; j < val; j++) {
          length +=
              1 + (bDoubleBondSquare && BOND_DOUBLE == at[i].bond_type[j]);
        }
    ...
    if ((pAtList = (AT_NUMB *)inchi_malloc(length * sizeof(*pAtList)))) {
      if (!bDoubleBondSquare) {
        for (i = 0, length = 0; i < num_atoms; i++) {
          ...
          pAtList[start] =
              length - start -
              1; /*  number of neighbors before the list of neighbors */
          pp[i] =
              pAtList + start; /*  pointer to the <num.neigh.><list of neigh> */
        }
        ...
        for (i = 0, length = 0; i < num_atoms; i++) {
          ...
          for (j = 0; j < val; j++) {
            ...
            if (bDoubleBondSquare && BOND_DOUBLE == at[i].bond_type[j]) {
              pAtList[length++] =
                  at[i].neighbor[j]; /*  a list of neighbor orig. numbers */
            }
          }
          /*  add endpoint */
          if (num_t_groups && at[i].endpoint) {
...
int BreakAllTies(int num_atoms, int num_max, AT_RANK **pRankStack,
  ...
  if (nNewRank && nNewAtomNumber) {
    memcpy(nNewAtomNumber, nPrevAtomNumber,
           num_atoms * sizeof(nNewAtomNumber[0]));
    memcpy(nNewRank, nPrevRank, num_atoms * sizeof(nNewRank[0]));

    for (i = 1, nRet = 0; i < num_atoms;
         i++) { /*  12-12-2001: replaced Prev... with New... */
      if (nNewRank[(int)nNewAtomNumber[i - 1]] ==
          nNewRank[(int)nNewAtomNumber[i]]) {
        nNewRank[nNewAtomNumber[i - 1]] = (AT_RANK)i;
        nNumRanks = DifferentiateRanks2(num_atoms, NeighList, nNumRanks,
                                        nNewRank, nTempRank, nNewAtomNumber,
