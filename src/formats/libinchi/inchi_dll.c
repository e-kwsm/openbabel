...
int SetAtomProperties(inp_ATOM *at, MOL_COORD *szCoord, inchi_Atom *ati, int a1,
                      int *nDim, char *pStrErr, int *err);
...
EXPIMP_TEMPLATE INCHI_API void INCHI_DECL
FreeStructFromStdINCHI(inchi_OutputStruct *out) {
...
static int GetINCHI1(inchi_Input *inp, inchi_Output *out, int bStdFormat) {
  ...
  INCHI_IOSTREAM inchi_file[3], *output_file = inchi_file,
                                *log_file = inchi_file + 1;
...
repeat:
  ...
  nRet1 = ReadCommandLineParms(argc, argv, ip, szSdfDataValue, &ulDisplTime,
                               bReleaseVersion, log_file);
  if (szOptions) {
  ...
  if (bStdFormat) {
    ...
    if (0 != (ip->nMode & REQ_MODE_RELATIVE_STEREO)) {
      ip->nMode &= ~(REQ_MODE_RACEMIC_STEREO | REQ_MODE_RELATIVE_STEREO |
  ...
  if (!sd->bUserQuit && !bInterrupted) {
    ...
    nRet =
        ExtractOneStructure(sd, ip, szTitle, inp, log_file, output_file,
                            prb_file, orig_inp_data, &num_inp, pStr, nStrLen);
    ...
    nRet1 = ProcessOneStructure(sd, ip, szTitle, pINChI, pINChI_Aux,
                                NULL, /* inp_file is not necessary as all input
                                         is already saved in 'ip' */
                                log_file, output_file, prb_file, orig_inp_data,
...
exit_function:
  if ((ip->bINChIOutputOptions & INCHI_OUT_XML) && sd->bXmlStructStarted > 0) {
    if (!OutputINChIXmlStructEndTag(output_file, pStr, nStrLen, 1)) {
      inchi_ios_eprint(
          log_file,
          "Cannot create end xml tag for structure #%d.%s%s%s%s Terminating.\n",
  ...
  for (i = 0; i < MAX_NUM_PATHS; i++) {
    if (ip->path[i]) {
      inchi_free(
          (char *)
              ip->path[i]); /*  cast deliberately discards 'const' qualifier */
      ip->path[i] = NULL;
    }
  }
  ...
  if (sd->pStrErrStruct[0]) {
    if (out && (out->szMessage =
                    (char *)inchi_malloc(strlen(sd->pStrErrStruct) + 1))) {
  ...
  if (output_file->s.pStr && output_file->s.nUsedLength > 0 && out) {
    ...
    if (!(INCHI_OUT_SDFILE_ONLY &
          ip->bINChIOutputOptions)) /* do not remove last LF from SDF output -
                                       2008-12-23 DT */
      for (p = strchr(out->szInChI, '\n'); p; p = strchr(p + 1, '\n')) {
        if (!memcmp(p, "\nAuxInfo", 8)) {
          *p = '\0';              /* remove LF after INChI */
          out->szAuxInfo = p + 1; /* save pointer to AuxInfo */
        } else if (out->szAuxInfo ||
                   !p[1]) { /* remove LF after aux info or from the last char */
          *p = '\0';
          break;
        }
      }
...
translate_RetVal:
  ...
  case _IS_FATAL:
    ...
    break; /* Severe error: no INChI has been created (typically; break; memory
              allocation failed) */
  case _IS_UNKNOWN:
  default:
...
Input:
        ...
        strict      if 0, just quickly check for proper layout (prefix, version,
etc.) The result may not be strict. If not 0, try to perform InChI2InChI
conversion and returns success if a resulting InChI string exactly match source.
                    The result may be 'false alarm' due to imperfect algorithm
of conversion. Returns: success/errors codes
...
EXPIMP_TEMPLATE INCHI_API int INCHI_DECL CheckINCHI(const char *szINCHI,
  ...
  slen0 = slen;
  if ((szINCHI[slen - 3] == '\\') && (szINCHI[slen - 2] >= 'A') &&
      (szINCHI[slen - 2] <= 'Z') && (szINCHI[slen - 1] >= 'A') &&
      (szINCHI[slen - 1] <= 'Z'))
    slen0 = slen - 3;
  ...
  if (strict) {
    ...
    if (((ret_i2i != inchi_Ret_OKAY) && (ret_i2i != inchi_Ret_WARNING)) ||
        !inchi_out.szInChI) {
      ret = INCHI_FAIL_I2I;
    } else {
...
/****************************************************************/
int CopyMOLfile(FILE *inp_file, long fPtrStart, long fPtrEnd, FILE *prb_file,
                long lNumb) {
  return 0; /* dummy */
}
/****************************************************************/
...
void SetNumImplicitH(inp_ATOM *at, int num_atoms) {
  ...
  for (bNonMetal = 0; bNonMetal < 2; bNonMetal++) {
    for (a1 = 0; a1 < num_atoms; a1++) {
       ...
       */
      at[a1].num_H = get_num_H(
          at[a1].elname, at[a1].num_H, at[a1].num_iso_H, at[a1].charge,
          at[a1].radical, at[a1].chem_bonds_valence,
          0, /* instead of valence entered by the user: it does not exist here*/
          (at[a1].at_type & 1) /* bAliased */,
          !(at[a1].at_type & 2) /* bDoNotAddH */, bHasMetalNeighbor);
      at[a1].at_type = 0;
    }
  }
}
...
/******************************************************************************************************/
int CreateInpAtomData(INP_ATOM_DATA *inp_at_data, int num_atoms,
                      int create_at_fixed_bonds) {
  FreeInpAtomData(inp_at_data);
  if ((inp_at_data->at = CreateInpAtom(num_atoms)) &&
      (!create_at_fixed_bonds ||
       (inp_at_data->at_fixed_bonds = CreateInpAtom(num_atoms)))) {
...
/******************************************************************************************************/
int CopyInpAtomData(INP_ATOM_DATA *dest_inp_at_data,
                    INP_ATOM_DATA *src_inp_at_data) {
  int ret = 1;
  if (!dest_inp_at_data->at ||
      dest_inp_at_data->num_at != src_inp_at_data->num_at) {
    ret = CreateInpAtomData(dest_inp_at_data, src_inp_at_data->num_at,
                            (NULL != src_inp_at_data->at_fixed_bonds));
  } else {
    inp_ATOM *at =
        dest_inp_at_data->at; /*  save ptr to already allocated memory */
  ...
  if (ret) {
    ...
    if (dest_inp_at_data->at_fixed_bonds && src_inp_at_data->at_fixed_bonds) {
      memcpy(dest_inp_at_data->at_fixed_bonds, src_inp_at_data->at_fixed_bonds,
             src_inp_at_data->num_at *
                 sizeof(dest_inp_at_data->at_fixed_bonds[0]));
    }
  }
  return ret;
}
/******************************************************************************************************/
void RenumbInpAtomData(INP_ATOM_DATA *dest_inp_at_data,
                       INP_ATOM_DATA *src_inp_at_data, AT_RANK *new_ord) {
  int j, n, m, val;
  ...
  for (n = 0; n < num_atoms; n++) {
    ...
    dest_at[m].orig_compt_at_numb =
        (AT_NUMB)(m + 1); /*  new ordering number within the component */
    ...
    for (i = 0; i < 3 * val; i++) {
      ...
      inchi_swap((char *)&dest_at[m].neighbor[j],
                 (char *)&dest_at[m].neighbor[k],
                 sizeof(dest_at[0].neighbor[0]));
      inchi_swap((char *)&dest_at[m].bond_stereo[j],
                 (char *)&dest_at[m].bond_stereo[k],
                 sizeof(dest_at[0].bond_stereo[0]));
      inchi_swap((char *)&dest_at[m].bond_type[j],
                 (char *)&dest_at[m].bond_type[k],
                 sizeof(dest_at[0].bond_type[0]));
...
int parse_options_string(char *cmd, const char *argv[], int maxargs) {
  ...
  while (i < maxargs - 1) {
    ...
    while (1) {
      ...
      /* each pair of backslashes => one backslash; one more backslash =>
       * literal quote */
      if (*p == '\"') {
        /* one " found */
...
int SetAtomProperties(inp_ATOM *at, MOL_COORD *szCoord, inchi_Atom *ati, int a1,
  ...
  if (MIN_BOND_LENGTH < fabs(ati[a1].x) || MIN_BOND_LENGTH < fabs(ati[a1].y) ||
      MIN_BOND_LENGTH < fabs(ati[a1].z)) {
...
int SetBondProperties(inp_ATOM *at, inchi_Atom *ati, int a1, int j,
  ...
  if (p1 && p2) {
    ...
    if ((n1 + 1 < at[a1].valence &&
         is_in_the_list(at[a1].neighbor + n1 + 1, (AT_NUMB)a2,
                        at[a1].valence - n1 - 1)) ||
        (n2 + 1 < at[a2].valence &&
         is_in_the_list(at[a2].neighbor + n2 + 1, (AT_NUMB)a1,
                        at[a2].valence - n2 - 1))) {
  ...
  } else if ((p1 || p2) && (p1 || at[a1].valence < MAXVAL) &&
    ...
    if ((p1 && (cBondType != at[a1].bond_type[n1] ||
                at[a1].bond_stereo[n1] != cStereoType1)) ||
        (p2 && (cBondType != at[a2].bond_type[n2] ||
                at[a2].bond_stereo[n2] != cStereoType2))) {
      MOLFILE_ERR_SET(*err, 0, "Multiple bonds between two atoms");
      *err |= 2; /*  multiple bonds between atoms */
    } else {
      MOLFILE_ERR_SET(*err, 0, "Duplicated bond(s) between two atoms");
  ...
  /* bond type */
  at[a1].bond_type[n1] = at[a2].bond_type[n2] = cBondType;
  /* connection */
  at[a1].neighbor[n1] = (AT_NUMB)a2;
  at[a2].neighbor[n2] = (AT_NUMB)a1;
  /* stereo */
  at[a1].bond_stereo[n1] =
      cStereoType1; /*  >0: the wedge (pointed) end is at this atom */
  at[a2].bond_stereo[n2] =
      cStereoType2; /*  <0: the wedge (pointed) end is at the opposite atom */
  return 0;
err_exit:
  return 1;
}
...
int SetAtomAndBondProperties(inp_ATOM *at, inchi_Atom *ati, int a1,
  ...
  default: {
    ...
    *err |= 8; /*  wrong number of alt. bonds */
    sprintf(szMsg, "Atom '%s' has %d alternating bonds", at[a1].elname,
            num_alt_bonds);
    MOLFILE_ERR_SET(*err, 0, szMsg);
  } break;
  }
  ...
  if (ERR_ELEM == (n1 = get_periodic_table_number(at[a1].elname))) {
    /*  Case when elname contains more than 1 element: extract number of H if
     * possible */
    ...
    if (!at[a1].elname[0] && NUMH(at, a1)) {
       ...
       * Note: this must be consistent with H-H treatment in
       * remove_terminal_HDT()
       */
    ...
    if (el_number_H == n1 && !at[a1].iso_atw_diff) {
      ...
      case 'H':
        if (1 <= ati[a1].isotopic_mass) {
          AT_NUM iso_atw_diff;
          if (ISOTOPIC_SHIFT_FLAG - ISOTOPIC_SHIFT_MAX <=
                  ati[a1].isotopic_mass &&
              ISOTOPIC_SHIFT_FLAG + ISOTOPIC_SHIFT_MAX >=
                  ati[a1].isotopic_mass) {
            /* ati[a1].isotopic_mass is isotopic iso_atw_diff +
             * ISOTOPIC_SHIFT_FLAG */
  ...
  if (num_alt_bonds) {
    ...
    int bUnusualValenceArom = detect_unusual_el_valence(
        (int)at[a1].el_number, at[a1].charge, at[a1].radical, chem_valence_alt,
        num_H, at[a1].valence);
    int bUnusualValenceNoArom = detect_unusual_el_valence(
        (int)at[a1].el_number, at[a1].charge, at[a1].radical,
        chem_valence_alt - 1, num_H, at[a1].valence);
...
int InpAtom0DToInchiAtom(inp_ATOM *at, int num_atoms,
                         inchi_OutputStruct *outStruct) {
  int num_stereo_centers, num_stereo_bonds, num_stereo0D, i, m, m1, m2, n,
      ret = 0;
  ...
  if (num_atoms > 0) {
    outStruct->atom =
        (inchi_Atom *)inchi_calloc(num_atoms, sizeof(outStruct->atom[0]));
  }
  outStruct->num_atoms = num_atoms;
  if (num_stereo0D > 0) {
    outStruct->stereo0D = (inchi_Stereo0D *)inchi_calloc(
        num_stereo0D, sizeof(outStruct->stereo0D[0]));
  }
  if ((num_atoms && !outStruct->atom) ||
      (num_stereo0D > 0 && !outStruct->stereo0D)) {
    /* allocation failed */
    ret = -1;
    goto exit_function;
  }
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (at[i].iso_atw_diff) {
      outStruct->atom[i].isotopic_mass =
          ISOTOPIC_SHIFT_FLAG + (at[i].iso_atw_diff > 0 ? at[i].iso_atw_diff - 1
                                                        : at[i].iso_atw_diff);
    }
  ...
  for (i = n = 0; i < num_atoms; i++) {
    if (at[i].p_parity) {
      ...
      for (m1 = 0; m1 < MAX_NUM_STEREO_BONDS && at[i].sb_parity[m1]; m1++) {
        ...
        /* find the opposite atom at the other end of double bond, allene, or
         * cumulene */
        ...
        if (at[cur].sb_parity[0] && len <= 4 &&
            i < cur /* count bonds only one time */) {
          /* double bond, cumulene, or allene has been found */
          for (m2 = 0; m2 < MAX_NUM_STEREO_BONDS && at[cur].sb_parity[m2];
               m2++) {
            if (chain[len - 2] == at[cur].neighbor[(int)at[cur].sb_ord[m2]]) {
              if (n < num_stereo0D) {
                ...
                int parity;
                if ((INCHI_PARITY_ODD == parity1 ||
                     INCHI_PARITY_EVEN == parity1) &&
                    (INCHI_PARITY_ODD == parity2 ||
                     INCHI_PARITY_EVEN == parity2)) {
                  /* well-defined parity */
                  parity = (parity1 == parity2) ? INCHI_PARITY_EVEN
                                                : INCHI_PARITY_ODD;
                } else {
                  parity = inchi_max(parity1, parity2);
                }
                outStruct->stereo0D[n].central_atom =
                    (len == 3) ? chain[1] : NO_ATOM;
                outStruct->stereo0D[n].parity = parity;
                outStruct->stereo0D[n].type = len == 3
                                                  ? INCHI_StereoType_Allene
                                                  : INCHI_StereoType_DoubleBond;
                outStruct->stereo0D[n].neighbor[0] =
                    at[i].sn_orig_at_num[m1] - 1;
                outStruct->stereo0D[n].neighbor[1] = i;
                outStruct->stereo0D[n].neighbor[2] = cur;
                outStruct->stereo0D[n].neighbor[3] =
                    at[cur].sn_orig_at_num[m2] - 1;
                n++;
              } else {
                ret |= 1;
              }
...
exit_function:
  if (ret < 0) {
    if (outStruct->atom)
      inchi_free(outStruct->atom);
    if (outStruct->stereo0D)
...
int ExtractOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
  ...
  szCoord =
      (MOL_COORD *)inchi_calloc(inchi_max(nNumAtoms, 1), sizeof(MOL_COORD));
  ...
  for (a1 = 0; a1 < nNumAtoms; a1++) {
    /* set temp flags in at[a1].at_type (1: data in atom name; 2: request to add
     * H) */
   ...
   *
   ********************************************************/
  Extract0DParities(at, nNumAtoms, inp->stereo0D, inp->num_stereo0D, pStrErr,
                    err, vABParityUnknown);
   ...
   * - e_IchiMain.c #273 -- main()               -- C example of calling InChI
   *dll
  ...
  if ((ip->nMode & REQ_MODE_CHIR_FLG_STEREO) && (ip->nMode & REQ_MODE_STEREO)) {
    ...
    /*if ( ip->bChiralFlag & FLAG_SET_INP_AT_NONCHIRAL )*/ {
      ...
      sd->bChiralFlag |=
          FLAG_INP_AT_NONCHIRAL; /* write AuxInfo as non-chiral */
    }
...
err_exit:
  ...
  nRet = TreatReadTheStructureErrors(sd, ip, LOG_MASK_NO_WARN, NULL, log_file,
...
/* GetINCHIfromINCHI does same as -InChI2InChI option: converts InChI into InChI
 * for validation purposes */
/* It may also be used to filter out specific layers. For instance, /Snon would
 * remove stereochemical layer */
/* Omitting /FixedH and/or /RecMet would remove Fixed-H or Reconnected layers */
/* To keep all InChI layers use options string "/FixedH /RecMet"; option
 * /InChI2InChI is not needed */
/* inchi_InputINCHI is created by the user; strings in inchi_Output are
 * allocated and deallocated by InChI */
/* inchi_Output does not need to be initilized out to zeroes; see FreeINCHI() on
...
repeat:
  ...
  inchi_ios_close(output_file);
  inchi_ios_close(log_file);
  inchi_ios_reset(input_file); /* do not close input_file - its string buffer
                                  may point to inpInChI->szInChI */
#endif
  ...
  /* options */
  if (inpInChI) {
    int opt_len = (inpInChI->szOptions ? strlen(inpInChI->szOptions) : 0) +
                  sizeof(szMainOption) + 1;
  ...
  if (!out->szMessage) {
    ...
    nRet =
        ReadWriteInChI(input_file, output_file, log_file, ip, sd, NULL, NULL,
                       out->szMessage, MAX_MSG_LEN, NULL /*out->WarningFlags*/);
  }
...
translate_RetVal:
  ...
  case -1:
    nRet = inchi_Ret_FATAL;
    break; /* Severe error: no Structure has been created (typically; break;
...
EXPIMP_TEMPLATE INCHI_API int INCHI_DECL GetStructFromStdINCHI(
    inchi_InputINCHI *inpInChI, inchi_OutputStruct *outStruct) {
  if ((inpInChI) && (inpInChI->szInChI) &&
...
GetStructFromINCHI(inchi_InputINCHI *inpInChI, inchi_OutputStruct *outStruct) {
  STRUCT_DATA struct_data;
  STRUCT_DATA *sd = &struct_data;

  INCHI_IOSTREAM inchi_file[3];
  INCHI_IOSTREAM *output_file = inchi_file, *log_file = inchi_file + 1,
                 *input_file = inchi_file + 2;
...
repeat:
  ...
  /* options */
  if (inpInChI /*&& inpInChI->szOptions*/) { /* fix bug discovered by Burt
                                                Leland 2008-12-23 */
    int opt_len = (inpInChI->szOptions ? strlen(inpInChI->szOptions) : 0) +
                  sizeof(szMainOption) + 1;
    szOptions = (char *)inchi_calloc(opt_len + 1, sizeof(szOptions[0]));
    if (szOptions) {
      if (inpInChI
              ->szOptions) /* fix bug discovered by Burt Leland 2008-12-23 */
        strcpy(szOptions, inpInChI->szOptions);
      strcat(szOptions, szMainOption);
      argc = parse_options_string(szOptions, argv, INCHI_MAX_NUM_ARG);
    } else {
  ...
  /* buffer for the message */
  outStruct->szMessage =
      (char *)inchi_calloc(MAX_MSG_LEN, sizeof(outStruct->szMessage[0]));
  if (!outStruct->szMessage) {
    inchi_ios_eprint(log_file, "Cannot allocate output message buffer.\n");
    nRet = -1;
  } else {
    nRet = ReadWriteInChI(input_file, output_file, log_file, ip, sd, &at,
                          &num_at, outStruct->szMessage, MAX_MSG_LEN,
                          outStruct->WarningFlags);
  }
  if (nRet >= 0 && at && num_at) {
    /* success */
...
/********************************************************************/
     ...
     defined(_USRDLL) && defined(BUILD_LINK_AS_DLL))
...
int cdecl_Get_inchi_Input_FromAuxInfo(char *szInchiAuxInfo, int bDoNotAddH,
                                      int bDiffUnkUndfStereo,
                                      InchiInpData *pInchiInp);
int cdecl_Get_std_inchi_Input_FromAuxInfo(char *szInchiAuxInfo, int bDoNotAddH,
                                          InchiInpData *pInchiInp);
...
/********************************************************/
void cdecl_FreeINCHI(inchi_Output *out) { FreeINCHI(out); }
/********************************************************/
void cdecl_FreeStdINCHI(inchi_Output *out) { FreeStdINCHI(out); }
/********************************************************/
int cdecl_GetStringLength(char *p) { return GetStringLength(p); }
/********************************************************/
int cdecl_Get_inchi_Input_FromAuxInfo(char *szInchiAuxInfo, int bDoNotAddH,
                                      int bDiffUnkUndfStereo,
                                      InchiInpData *pInchiInp) {
...
/********************************************************/
void cdecl_Free_inchi_Input(inchi_Input *pInp) { Free_inchi_Input(pInp); }
/********************************************************/
int cdecl_GetStructFromINCHI(inchi_InputINCHI *inpInChI,
                             inchi_OutputStruct *outStruct) {
  return GetStructFromINCHI(inpInChI, outStruct);
...
int PASCAL pasc_GetStructFromINCHI(inchi_InputINCHI *inp,
                                   inchi_OutputStruct *out);
int PASCAL pasc_GetStructFromStdINCHI(inchi_InputINCHI *inp,
                                      inchi_OutputStruct *out);
...
/********************************************************/
void PASCAL pasc_FreeINCHI(inchi_Output *out) { FreeINCHI(out); }
/********************************************************/
void PASCAL pasc_FreeStdINCHI(inchi_Output *out) { FreeStdINCHI(out); }
/********************************************************/
int PASCAL pasc_GetStringLength(char *p) { return GetStringLength(p); }
/********************************************************/
int PASCAL pasc_Get_inchi_Input_FromAuxInfo(char *szInchiAuxInfo,
                                            int bDoNotAddH,
                                            int bDiffUnkUndfStereo,
...
/********************************************************/
void PASCAL pasc_Free_inchi_Input(inchi_Input *pInp) { Free_inchi_Input(pInp); }
/********************************************************/
void PASCAL pasc_Free_std_inchi_Input(inchi_Input *pInp) {
  Free_std_inchi_Input(pInp);
}
