...
GetStdINCHIKeyFromStdINCHI(const char *szINCHISource, char *szINCHIKey) {
  ...
  if (szINCHISource[LEN_INCHI_STRING_PREFIX + 1] != 'S')
    return INCHIKEY_INVALID_STD_INCHI;
  return GetINCHIKeyFromINCHI(szINCHISource, 0, 0, szINCHIKey, (char *)NULL,
                              (char *)NULL);
}
...
EXPIMP_TEMPLATE INCHI_API int INCHI_DECL GetINCHIKeyFromINCHI(
  ...
  char *str = NULL, *smajor = NULL, *sminor = NULL, *sproto = NULL,
       *stmp = NULL, tmp[MINOUTLENGTH];
  unsigned char digest_major[32], digest_minor[32];
  ...
  */
  ...
  sminor = (char *)inchi_calloc(
      2 * slen + 2, sizeof(char)); /* we may double the length ... */
  if (NULL == sminor) {
    ret = INCHIKEY_NOT_ENOUGH_MEMORY;
    goto fin;
  }
  ...
  /* Treat protonization */
  if (jproto) {
    /* 2009-01-07 fix bug/typo: assigned incorrect length to the protonation
    segment of
  ...
  */
  ...
  sprintf(tmp, "%-.3s%-.3s%-.3s%-.3s%-.2s", base26_triplet_1(digest_major),
          base26_triplet_2(digest_major), base26_triplet_3(digest_major),
          base26_triplet_4(digest_major),
  ...
  strcat(szINCHIKey, "-");
  sprintf(tmp, "%-.3s%-.3s%-.2s", base26_triplet_1(digest_minor),
...
int cdecl_GetINCHIKeyFromINCHI(const char *szINCHISource, const int xtra1,
                               const int xtra2, char *szINCHIKey, char *szXtra1,
                               char *szXtra2);
int cdecl_GetStdINCHIKeyFromStdINCHI(const char *szINCHISource,
                                     char *szINCHIKey);
...
int cdecl_GetINCHIKeyFromINCHI(const char *szINCHISource, const int xtra1,
                               const int xtra2, char *szINCHIKey, char *szXtra1,
                               char *szXtra2) {
  return GetINCHIKeyFromINCHI(szINCHISource, xtra1, xtra2, szINCHIKey, szXtra1,
                              szXtra2);
}
#endif
