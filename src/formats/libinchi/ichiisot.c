...
int unpack_iso_sort_key(AT_ISO_SORT_KEY iso_sort_key, S_CHAR *num_1H,
  ...
  static const AT_ISO_SORT_KEY MultAtwDiff =
      AT_ISO_SORT_KEY_MULT * AT_ISO_SORT_KEY_MULT * AT_ISO_SORT_KEY_MULT;
  if (!iso_sort_key) {
    *num_1H = *num_2H = *num_3H = *iso_atw_diff = 0;
    return 0;
  } else if (iso_sort_key < 0) {
    is_negative = 1;
...
int set_atom_iso_sort_keys(int num_at, sp_ATOM *at, T_GROUP_INFO *t_group_info,
  ...
  T_GROUP *t_group =
      (t_group_info && t_group_info->t_group && t_group_info->num_t_groups > 0)
  ...
  for (i = 0; i < num_at; i++) {
    bMergedTgroup =
        (t_group_info && t_group_info->nIsotopicEndpointAtomNumber &&
         (at[i].cFlags & AT_FLAG_ISO_H_POINT));
    if ((!at[i].endpoint || !t_group) && !bMergedTgroup) {
      iso_sort_key = make_iso_sort_key(at[i].iso_atw_diff, at[i].num_iso_H[0],
