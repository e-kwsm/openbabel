...
/* This contains executable code. Included in lReadAux.c, e_ReadINCH.c,
 * ReadINCH.c,  */
#include "aux2atom.h"
...
Get_inchi_Input_FromAuxInfo(char *szInchiAuxInfo, int bDoNotAddH,
  ...
  inchi_ios_init(&inp, INCHI_IOSTREAM_STRING,
                 NULL); /* fix bug discovered by Burt Leland 2008-12-23 */
  inp.s.pStr = szInchiAuxInfo;
  inp.s.nUsedLength = strlen(szInchiAuxInfo);
  inp.s.nAllocatedLength = inp.s.nUsedLength + 1;
  inp.s.nPtr = 0;

  num_at = INChIToInchi_Input(
      &inp, pInchiInp->pInp, 1, bDoNotAddH, vABParityUnknown, INPUT_INCHI_PLAIN,
      szHeader, szLabel, &lNumber, &bChiral, &err, pInchiInp->szErrMsg);
