...
typedef struct tagStereoBondNeighbor { /*  *n = sort key */
  AT_RANK
  nRank; /*  *1 opposite atom rank; equal ranks mean constit. equivalence */
  AT_RANK nNeighRank1; /*  rank of the neighbor in the direction to the opposite
                          atom */
  AT_RANK nNeighRank2; /*  rank of the opposite atom neighbor in the direction
                          to the current atom */
  AT_RANK num; /*  number of same type bonds to constitutionally equivalent
                  neighbors */
  AT_RANK num_any_parity; /*  at least one atom has parity in 1..4 range */
  AT_RANK
  num_defined_parity; /*  number of neighbors with defined parity <= num */
...
int SetHalfStereoBondIllDefPariy(
    sp_ATOM *at, int jn, /* atom number*/ int k1 /* stereo bond number*/,
    int new_parity);
int RemoveHalfStereoBond(sp_ATOM *at, int jn,
                         /* atom number*/ int k1 /* stereo bond number*/);
int SetKnownStereoBondParities(sp_ATOM *at, int num_atoms,
                               const AT_RANK *nCanonRank, const AT_RANK *nRank,
                               const AT_RANK *nAtomNumber);
int MarkKnownEqualStereoBondParities(sp_ATOM *at, int num_atoms,
                                     const AT_RANK *nRank,
                                     const AT_RANK *nAtomNumber);
int GetNextNeighborAndRank(sp_ATOM *at, AT_RANK cur, AT_RANK prev, AT_RANK *n,
                           AT_RANK *cr, const AT_RANK *nCanonRank);
int GetAndCheckNextNeighbors(sp_ATOM *at, AT_RANK cur1, AT_RANK prev1,
                             AT_RANK cur2, AT_RANK prev2, AT_RANK *n1,
                             AT_RANK *n2, AT_RANK *nVisited1,
                             AT_RANK *nVisited2, const AT_RANK *nRank,
                             const AT_RANK *nCanonRank);
AT_RANK
PathsHaveIdenticalKnownParities(sp_ATOM *at, AT_RANK prev1, AT_RANK cur1,
                                AT_RANK prev2, AT_RANK cur2, AT_RANK *nVisited1,
                                AT_RANK *nVisited2, const AT_RANK *nRank,
                                const AT_RANK *nCanonRank, AT_RANK nLength);
int RemoveKnownNonStereoBondParities(sp_ATOM *at, int num_atoms,
                                     const AT_RANK *nCanonRank,
                                     const AT_RANK *nRank, CANON_STAT *pCS);
int SetKnownStereoCenterParities(sp_ATOM *at, int num_atoms,
                                 const AT_RANK *nCanonRank,
                                 const AT_RANK *nRank,
                                 const AT_RANK *nAtomNumber);
int RemoveKnownNonStereoCenterParities(sp_ATOM *at, int num_atoms,
                                       const AT_RANK *nCanonRank,
                                       const AT_RANK *nRank, CANON_STAT *pCS);
...
int find_atoms_with_parity(sp_ATOM *at, S_CHAR *visited, int from_atom,
                           int cur_atom) {
  int i, next_atom;
  if (visited[cur_atom])
    return 0;
...
int SetHalfStereoBondIllDefPariy(
  ...
  if (k1 < MAX_NUM_STEREO_BOND_NEIGH && at[jn].stereo_bond_neighbor[k1]) {
    parity = at[jn].stereo_bond_parity[k1] ^
             PARITY_VAL(at[jn].stereo_bond_parity[k1]);
...
/**********************************************************************************/
int SetOneStereoBondIllDefParity(
    sp_ATOM *at, int jc, /* atom number*/ int k /* stereo bond ord. number*/,
    int new_parity) {
  ...
  for (k1 = kn = ret = 0;
       ...
       k1++) {
    if (kn - 1 == jc) {
      ret = SetHalfStereoBondIllDefPariy(
          at, jn, /* atom number*/ k1 /* stereo bond number*/, new_parity);
      break;
    }
  }
  if (ret) {
...
/**********************************************************************************/
int RemoveOneStereoBond(sp_ATOM *at, int jc,
                        /* atom number*/ int k /* stereo bond number*/) {
  int k1, ret = 0, kn, jn = (int)at[jc].stereo_bond_neighbor[k] - 1;
...
int UnmarkNonStereo(sp_ATOM *at, int num_atoms, const AT_RANK *nRank,
                    const AT_RANK *nAtomNumber, int bIsotopic) {
  int i, i1, i2, j, k, k1, k2, kn /* neigh*/, val, ic /* center*/, jc,
      num_implicit_H;
  int num_neighbors_with_parity, num_no_parity_atoms, num_removed_parities = -1,
                                                      num_removed_parities0;
  ...
  do {
    ...
    for (i = i1 = 0, nPrevAtomRank = 0; i <= num_atoms; i++) {
      ...
      if ( i == num_atoms || nPrevAtomRank != nRank[j = nAtomNumber[i]]
        ...
        i2 = i;
        if (i2 - i1 >
            num_no_parity_atoms /*&& at[jc = nAtomNumber[i1]].parity*/) {
          /*  at[nAtomNumber[i1]]..at[nAtomNumber[i2-1]] are constitutionally
           * equivalent and some of them have parity */
          jc = nAtomNumber[i1];
          num_no_parity_atoms = 0;
          val = at[jc].valence; /*  all equivalent atoms have equal valences,
                                   etc. (except parities) */
          num_implicit_H = at[jc].endpoint ? 0 : at[jc].num_H;
          /*  Only atoms with valence <= MAX_NUM_STEREO_ATOM_NEIGH may have
           * parity. However, check: */
          if (val + num_implicit_H > MAX_NUM_STEREO_ATOM_NEIGH) {
            continue; /*  program error ??? */ /*   <BRKPT> */
          }
          for (k = 0; k < val; k++) {
            nNeighborNumber[k] =
                k; /*  initialize an array of indexes for sorting */
          }
          /*  check parities */
          for (ic = i1; ic < i2; ic++) {
            jc = nAtomNumber[ic];
            ...
            insertions_sort(nNeighborNumber, val, sizeof(nNeighborNumber[0]),
                            CompNeighborsAT_NUMBER);
            num_neighbors_with_parity = -1; /*  non-zero */
            for (k = k1 = 0, nPrevNeighRank = 0; k <= val; k++) {
              if (k == val || nPrevNeighRank !=
                                  nRank[at[jc].neighbor[nNeighborNumber[k]]]) {
                k2 = k;
                if (k2 - k1 > 1) {
                  ...
                  for (kn = k1, num_neighbors_with_parity = 0; kn < k2; kn++) {
                    ...
                    visited[jc] =
                        1; /*  starting point; the only atom with parity */
                    num_neighbors_with_parity += find_atoms_with_parity(
                        at, visited, jc,
            ...
            if (num_implicit_H > 1) {
              if (((bIsotopic && (at[jc].num_iso_H[0] > 1)) ||
                   at[jc].num_iso_H[1] > 1 || at[jc].num_iso_H[2] > 1) ||
                  num_implicit_H > NUM_H_ISOTOPES || !bIsotopic) {
                num_neighbors_with_parity = 0;
              }
            }
            /*  increment if: */
            /*  (a) constitutionally equivalent neighbors do exist, and */
            /*  (b) all constitutionally equivalent neighbors do not have
             * parity, and */
            /*  (c) all constitutionally equivalent neighbors are not connected
             * to atoms with parity */
          ...
          if (num_no_parity_atoms == i2 - i1) {
            ...
            for (ic = i1; ic < i2; ic++) {
              ...
              at[jc].bHasStereoOrEquToStereo = 0;
              /*  remove stereo bonds */
              for (k = 0; k < MAX_NUM_STEREO_BOND_NEIGH &&
                          (jn = at[jc].stereo_bond_neighbor[k]);
                   k++) {
                jn--; /*  stereo bond neighbor */
                /*  opposite end */
                for (k1 = 0; k1 < MAX_NUM_STEREO_BOND_NEIGH &&
                             (kn = at[jn].stereo_bond_neighbor[k1]);
                     k1++) {
                  if (kn - 1 == jc) {
 ...
 *
 **********************************************************************************/
int FillSingleStereoDescriptors(sp_ATOM *at, int i, int num_trans,
                                const AT_RANK *nRank,
                                AT_STEREO_CARB *LinearCTStereoCarb,
                                int *nStereoCarbLen, int nMaxStereoCarbLen,
  ...
  if (at[i].parity || at[i].stereo_bond_neighbor[0]) {
    ...
    if (ATOM_PARITY_WELL_DEF(at[i].parity) && num_trans < 0) {
      /*  number of neighbors transpositions to the sorted order is unknown.
       * Find it. */
      ...
      pn_RankForSort = nRank;
      num_trans =
          insertions_sort(nNeighborNumber2, num_neigh,
                          sizeof(nNeighborNumber2[0]), CompNeighborsAT_NUMBER);
#ifndef CT_NEIGH_INCREASE
      num_trans += ((num_neigh * (num_neigh - 1)) / 2) %
                   2; /*  get correct parity for ascending order */
#endif
    }
    ...
    if (LinearCTStereoDble && at[i].stereo_bond_neighbor[0]) {
      ...
      /* HalfStereoBondParity( sp_ATOM *at, int at_no1, int i_sb_neigh, AT_RANK
       * *nRank ) */
      AT_NUMB nStereoNeighNumber[MAX_NUM_STEREO_BONDS],
          nStereoNeigh[MAX_NUM_STEREO_BONDS], n;
      int num_stereo, stereo_neigh, stereo_neigh_ord, stereo_bond_parity;
      for (num_stereo = 0; num_stereo < MAX_NUM_STEREO_BONDS &&
                           (n = at[i].stereo_bond_neighbor[num_stereo]);
           num_stereo++) {
      ...
      pn_RankForSort = nRank;
      insertions_sort(nStereoNeighNumber, num_stereo,
                      sizeof(nStereoNeighNumber[0]), CompNeighborsAT_NUMBER);

      /*  process stereo bonds one by one */
      for (k = 0; k < num_stereo; k++) {
        stereo_neigh =
            nStereoNeigh[stereo_neigh_ord = (int)nStereoNeighNumber[k]];
        if ((r_neigh = (AT_NUMB)nRank[stereo_neigh])
                CT_NEIGH_SMALLER_THAN rank) {
          /* accept only neighbors that have smaller ranks */
          stereo_bond_parity =
              PARITY_VAL(at[i].stereo_bond_parity[stereo_neigh_ord]);
          if (stereo_bond_parity == AB_PARITY_NONE)
            continue;

          /* stereo_neigh      =
           * at[i].stereo_bond_neighbor[nStereoNeighNumber[k]]-1; */
          if (ATOM_PARITY_KNOWN(stereo_bond_parity)) {
            parity = stereo_bond_parity;
                     ...
                     MIN_DOT_PROD <=
                         abs(at[i].stereo_bond_z_prod[stereo_neigh_ord])) {
            ...
            if (stereo_neigh_ord2 >= 0) {
              half_parity1 =
                  HalfStereoBondParity(at, i, stereo_neigh_ord, nRank);
              half_parity2 = HalfStereoBondParity(at, stereo_neigh,
                                                  stereo_neigh_ord2, nRank);
              if (ATOM_PARITY_WELL_DEF(half_parity1) &&
                  ATOM_PARITY_WELL_DEF(half_parity2)) {
                parity =
                    2 - (half_parity1 + half_parity2 +
                         (at[i].stereo_bond_z_prod[stereo_neigh_ord] < 0)) %
                            2;
              } else {
            ...
            if (AB_PARITY_NONE ==
                (parity = inchi_max(at[i].parity, at[stereo_neigh].parity)))
              continue;
    ...
    if (LinearCTStereoCarb && !at[i].stereo_bond_neighbor[0]) {
      ...
      /*  stereo atom parity */
      parity = ATOM_PARITY_WELL_DEF(at[i].parity)
                   ? (2 - (at[i].parity + num_trans) % 2)
                   : at[i].parity;
...
/**********************************************************************************/
void SwitchAtomStereoAndIsotopicStereo(sp_ATOM *at, int num_atoms,
                                       int *bSwitched) {
  int i;
  /*  switch atom stereo data */
  for (i = 0; i < num_atoms; i++) {
    inchi_swap((char *)&at[i].parity, (char *)&at[i].parity2,
               sizeof(at[i].parity));
    inchi_swap((char *)&at[i].final_parity, (char *)&at[i].final_parity2,
               sizeof(at[i].final_parity));
    inchi_swap((char *)&at[i].stereo_atom_parity,
               (char *)&at[i].stereo_atom_parity2,
               sizeof(at[i].stereo_atom_parity));
    inchi_swap((char *)&at[i].bHasStereoOrEquToStereo,
               (char *)&at[i].bHasStereoOrEquToStereo2,
               sizeof(at[i].bHasStereoOrEquToStereo));

    inchi_swap((char *)at[i].stereo_bond_neighbor,
               (char *)at[i].stereo_bond_neighbor2,
               sizeof(at[i].stereo_bond_neighbor));
    inchi_swap((char *)at[i].stereo_bond_ord, (char *)at[i].stereo_bond_ord2,
               sizeof(at[i].stereo_bond_ord));
    inchi_swap((char *)at[i].stereo_bond_z_prod,
               (char *)at[i].stereo_bond_z_prod2,
               sizeof(at[i].stereo_bond_z_prod));
    inchi_swap((char *)at[i].stereo_bond_parity,
               (char *)at[i].stereo_bond_parity2,
...
void SetCtToIsotopicStereo(CANON_STAT *pCS, CANON_STAT *pCS2) {
  pCS->LinearCTStereoDble =
      pCS2->LinearCTIsotopicStereoDble; /*  enable stereo */
  pCS->LinearCTStereoCarb = pCS2->LinearCTIsotopicStereoCarb;

  pCS->LinearCTStereoDbleInv =
      pCS2->LinearCTIsotopicStereoDbleInv; /*  enable inv. stereo */
...
void SetCtToNonIsotopicStereo(CANON_STAT *pCS, CANON_STAT *pCS2) {
  ...
  pCS->LinearCTStereoDbleInv =
      pCS2->LinearCTStereoDbleInv; /*  enable inv. stereo */
...
/**********************************************************************************/
int FillAllStereoDescriptors(sp_ATOM *at, int num_atoms,
                             const AT_RANK *nCanonRank,
                             const AT_RANK *nAtomNumberCanon, CANON_STAT *pCS) {
  int ret = 0, i;
  ...
  for (i = 0; !ret && i < num_atoms; i++) {
    ret = FillSingleStereoDescriptors(
        at, (int)nAtomNumberCanon[i], -1, nCanonRank, pCS->LinearCTStereoCarb,
...
int SetKnownStereoBondParities(sp_ATOM *at, int num_atoms,
  ...
  for (i1 = 0, num_set = 0; i1 < num_atoms; i1++) {
    ...
    for (n1 = 0; n1 < MAX_NUM_STEREO_BONDS &&
      ...
      if (m - 1 != i1 ||
          at[i1].stereo_bond_parity[n1] != at[i2].stereo_bond_parity[n2]) {
      ...
      nAtomRank2 = nRank[i2];
      nAtom1NeighRank =
          nRank[(int)at[i1].neighbor[(int)at[i1].stereo_bond_ord[n1]]];
      num_neigh2 = at[i2].valence;
      /*  store ranks of at[i1] stereo bond neighbors except one connected by a
       * stereo bond */
      ...
      if (j == 2) {
        ...
        trans_i1 = insertions_sort(nNeighRank1, j, sizeof(nNeighRank1[0]),
                                   comp_AT_RANK);
      }
      /*  store ranks of at[i2] stereo bond neighbors except one connected by a
       * stereo bond */
      ...
      if (j == 2) {
        ...
        trans_i2 = insertions_sort(nNeighRank2, j, sizeof(nNeighRank2[0]),
                                   comp_AT_RANK);
      }
      prev_trans = -1;
      trans_k1 = -2;
      ...
      for (j1 = 0; j1 <= iMax1 &&
                   nAtomRank1 == nRank[k1 = (int)nAtomNumber[iMax1 - j1]];
           j1++) {
        ...
        for (m1 = 0; m1 < num_neigh1; m1++) {
          ...
          if (j == 2) {
            trans_k1 = insertions_sort(
                nNeighCanonRank1, j, sizeof(nNeighCanonRank1[0]), comp_AT_RANK);
          } else {
            trans_k1 = 0;
          }
          ...
          if (j == 2) {
            trans_k2 = insertions_sort(
                nNeighCanonRank2, j, sizeof(nNeighCanonRank2[0]), comp_AT_RANK);
          } else {
            trans_k2 = 0;
          }
          ...
          if (prev_trans < 0) {
            prev_trans = trans_k;
                     ...
                     trans_k) { /* was != trans_k1, changed 9-23-2003 */
            break;              /*  different number of transpositions */
          }
        } /* end of the second atom mapping cycle */
        if (prev_trans >= 0 &&
            prev_trans != trans_k) { /* was != trans_k1, changed 9-23-2003 */
          break;
        }
      ...
      if (prev_trans == trans_k) { /* was == trans_k1, changed 9-23-2003 */
        ...
        /*  parity_1 = at[i1].parity + (trans_i1 + trans_k1 + num_neigh1 - 1) +
         * (int)at[i1].stereo_bond_ord[n1] */
        /*  expression in parentheses is equivalent to rank[first neigh] >
         * rank[second neigh] */
        /*  same for parity_2. */
        /*  parity_2 = at[i2].parity + (trans_i2 + trans_k2 + num_neigh2 - 1) +
         * (int)at[i2].stereo_bond_ord[n2] */
        /*  Sum of the two parities (without stereo_bond_z_prod) is: */
        parity =
            (at[i1].parity + at[i2].parity + prev_trans + trans_i1 + trans_i2 +
             num_neigh1 + num_neigh2 + (int)at[i1].stereo_bond_ord[n1] +
             (int)at[i2].stereo_bond_ord[n2]) %
            2;
        ...
        at[i1].stereo_bond_parity[n1] =
            ALL_BUT_PARITY(at[i1].stereo_bond_parity[n1]) | parity;
        at[i2].stereo_bond_parity[n2] =
            ALL_BUT_PARITY(at[i2].stereo_bond_parity[n2]) | parity;
        num_set++;
      }
    }
  }
...
int MarkKnownEqualStereoBondParities(sp_ATOM *at, int num_atoms,
  ...
  int i1, i2, k1, k2, n1, n2, m1, m2, s1, s2, stereo_bond_parity,
      stereo_bond_parity2, cumulene_len;
  ...
  for (i1 = 0, num_set = 0; i1 < num_atoms; i1++) {
    ...
    for (n1 = 0; n1 < MAX_NUM_STEREO_BONDS && at[i1].stereo_bond_neighbor[n1];
         n1++)
      ;
    /*num_sb1 = n1;*/
    /*  search for bonds possibly constitutionally equivalent to each of the
     * adjacent bonds */
    /*  and find if all of them have same already known parity */
    for (n1 = 0, i2 = 0; n1 < MAX_NUM_STEREO_BONDS &&
                         (i2 = (int)at[i1].stereo_bond_neighbor[n1]);
         n1++) {
      i2--;
      nAtomRank2 = nRank[i2];
      ...
      /*
      if ( !PARITY_KNOWN(at[i1].stereo_bond_parity[n1]) ||
      (at[i1].stereo_bond_parity[n1] & KNOWN_PARITIES_EQL) ) { continue;
      }
      */
      ...
      for (n2 = 0; n2 < MAX_NUM_STEREO_BONDS && at[i2].stereo_bond_neighbor[n2];
           n2++)
        ;
      /*num_sb2 = n2;*/
      for (n2 = 0, m = 0;
      ...
      nAtom1NeighRank =
          nRank[(int)at[i1].neighbor[(int)at[i1].stereo_bond_ord[n1]]];
      nAtom2NeighRank =
          nRank[(int)at[i2].neighbor[(int)at[i2].stereo_bond_ord[n2]]];
      num_neigh2 = at[i2].valence;
      /*  find all pairs of atoms that possibly can be mapped on at[i1], at[i2]
       * pair */
      /*  (we may also find pairs that cannot be mapped, but we cannot miss any
       * pair */
      /*  that can be mapped) */
      for (j1 = 0; j1 <= iMax1 &&
                   nAtomRank1 == nRank[k1 = (int)nAtomNumber[iMax1 - j1]];
           j1++) {
        /*  at[k1] is constitutionally equivalent to at[i1] */
        /*  find all at[k1] stereo bond neighbors at[k2] that have rank
         * nAtomRank2; */
        /*  then find at[k2] constitutionally equivalent at at[i2] */
        if (at[k1].valence != num_neigh1) {
          return CT_STEREOCOUNT_ERR; /*  program error */ /*   <BRKPT> */
        ...
        for ( s1 = 0; s1 < MAX_NUM_STEREO_BONDS &&
        at[k1].stereo_bond_neighbor[s1]; s1++ )
            ;
        if ( num_sb1 != s1 ) {
        ...
        */
        for (m1 = 0; m1 < num_neigh1; m1++) {
          ...
          /*  This neighbor may be on the bond constit. equivalent to
           * at[i1]-at[i2] stereo bond */
          /*  (or may be constit. equivalent an adjacent to at[i1] atom in a
           * stereogenic cumulene chain) */
          int prev, next, len;
          if (nAtom1NeighRank != nRank[k2 = (int)at[k1].neighbor[m1]]) {
            continue;
          }
          ...
          if (cumulene_len) {
            ...
            if (nAtom2NeighRank != nRank[prev]) {
              /* continue; */ /*  ??? program error ??? If not, must be a very
                                 rare event */
              return CT_STEREOCOUNT_ERR; /*   <BRKPT> */
            }
            k2 = next;
          ...
          for (n = 0; n < num_neigh2; n++) {
            if (prev == (int)at[k2].neighbor[n]) {
              m2 = n; /*  found bond from the opposite end of a possibly
                         stereogenic bond */
              break;
            }
          }
          if (m2 < 0) {
            ...
            /*  program error: opposite direction bond not found */ /*   <BRKPT>
                                                                     */
          }
          ...
          for (s1 = 0, m = 0;
               ...
               (m = (int)at[k1].stereo_bond_neighbor[s1]) && m - 1 != k2;
               s1++)
            ;
          if (m - 1 != k2) {
            bDifferentParities = 1; /*  cannot find the stereo bond */
            at[k1].bHasStereoOrEquToStereo = at[k2].bHasStereoOrEquToStereo = 2;
            continue;
          }
          /*  -- do not check number of stereo bonds, check bonds themselves --
          for ( s2 = 0; s2 < MAX_NUM_STEREO_BONDS &&
          at[k2].stereo_bond_neighbor[s2]; s2++ )
              ;
          if ( num_sb2 != s2 ) {
              bDifferentParities = 1;
              continue;
          ...
          */
          for (s2 = 0, m = 0;
               ...
               (m = (int)at[k2].stereo_bond_neighbor[s2]) && m - 1 != k1;
               s2++)
            ;
          if (m - 1 != k1) {
            /*
      ...
      if (0 == bDifferentParities && PARITY_KNOWN(stereo_bond_parity)) {
        for (j1 = 0; j1 <= iMax1 &&
             ...
             j1++) {
          /*  at[k1] is constitutionally equivalent to at[i1] */
          for (s1 = 0, k2 = 0; s1 < MAX_NUM_STEREO_BONDS &&
                               (k2 = (int)at[k1].stereo_bond_neighbor[s1]);
               s1++) {
            k2--;
 ...
 */
/*   in order of ascending canonical ranks of the next atoms:  *cr(output) >
 * *cr(input) */
/*   To start the sequence let *cr=0 */
/*   If no more neighbors available the return value = 0; if successgul then the
 * return value = 1. */
int GetNextNeighborAndRank(sp_ATOM *at, AT_RANK cur, AT_RANK prev, AT_RANK *n,
                           AT_RANK *cr, const AT_RANK *nCanonRank) {
  int i, val;
  ...
  for (i = 0, val = at[(int)cur].valence; i < val; i++) {
    if ((j = at[cur].neighbor[i]) != prev && cr1 > (crj = nCanonRank[(int)j]) &&
        crj > *cr) {
      cr1 = crj;
      j1 = j;
    }
...
int GetAndCheckNextNeighbors(sp_ATOM *at, AT_RANK cur1, AT_RANK prev1,
  ...
  if (!GetNextNeighborAndRank(at, cur1, prev1, n1, &cr1, nCanonRank) ||
      ...
      nRank[(int)*n1] != nRank[(int)*n2] ||
      nVisited1[(int)*n1] != nVisited2[(int)*n2]) {
  ...
  for (i2 = 0, k2 = 0;
    ...
    ;
  if (k1 != k2) {
    return 0; /*  possibly not an error: constit. equivalent atoms on a stereo
                 bond and not on a stereo bond */
  }
  if (k1 /* yes, it is a stero bond */ &&
      (at[cur1].stereo_bond_parity[i1] != at[cur2].stereo_bond_parity[i2] ||
       /* PARITY_KNOWN (at[cur1].stereo_bond_parity[i1] ) */ /*  replaced
                                                                08-13-2002 with
                                                                the next: */
       !PARITY_WELL_DEF(
           at[cur1].stereo_bond_parity[i1]) /*  it suffices to check only one
                                               parity */
       )) {
...
/**********************************************************************************/
/*  Simultaneously DFS-traverse 2 paths starting at the bonds prev1->cur1 and
 * prev2->cur2 */
...
/*  Only one nVisited[] array is sufficient because the paths from the beginning
 * are in different ring systems. */
AT_RANK
PathsHaveIdenticalKnownParities(sp_ATOM *at, AT_RANK prev1, AT_RANK cur1,
                                AT_RANK prev2, AT_RANK cur2, AT_RANK *nVisited1,
                                AT_RANK *nVisited2, const AT_RANK *nRank,
  ...
  /*  the atoms must be either both stereogenic and have well-defined parities
   * or non-stereogenic at all. */
  if (at[cur1].stereo_atom_parity != at[cur2].stereo_atom_parity ||
      (at[cur1].stereo_atom_parity &&
       !PARITY_WELL_DEF(at[cur1].stereo_atom_parity))) {
  ...
  for (k = 1, n1 = MAX_ATOMS + 1, n2 = MAX_ATOMS + 1; k < at[cur1].valence;
       k++) {
    /*  start from 1: since we do not go back, we have only (at[cur1].valence-1)
     * bonds to try */
    if (!GetAndCheckNextNeighbors(at, cur1, prev1, cur2, prev2, &n1, &n2,
                                  nVisited1, nVisited2, nRank, nCanonRank)) {
      return 0; /*  different neighbors                       */
    }
    /*  In a DFS we do not traverse already visited atoms */
    if (!nVisited1[n1]) { /*  recursion */
      if (!(nLength = PathsHaveIdenticalKnownParities(
                at, cur1, n1, cur2, n2, nVisited1, nVisited2, nRank, nCanonRank,
                nLength))) {
        return 0;
      }
    }
...
int RemoveKnownNonStereoBondParities(sp_ATOM *at, int num_atoms,
  ...
  for (i1 = 0; i1 < num_atoms; i1++) {
    ...
    for (n1 = 0;
         n1 < MAX_NUM_STEREO_BONDS && (s2 = at[i1].stereo_bond_neighbor[n1]);
         n1++) {
      ...
      if (!nVisited && !(nVisited = (AT_RANK *)inchi_malloc(
                             sizeof(nVisited[0]) * num_atoms))) {
        ret = CT_OUT_OF_RAM; /*   <BRKPT> */
        goto exit_function;
      }
      memset(nVisited, 0, sizeof(nVisited[0]) * num_atoms);
      nVisited[i1] = 1;
      if (PathsHaveIdenticalKnownParities(at, (AT_RANK)i1, neigh[0],
                                          (AT_RANK)i1, neigh[1], nVisited,
                                          nVisited, nRank, nCanonRank, 1)) {
        if (!RemoveOneStereoBond(at, i1,
                                 /* atom number*/ n1 /* stereo bond number*/)) {
          ret = CT_STEREOBOND_ERROR; /*   <BRKPT> */
          goto exit_function;
        }
...
int SetKnownStereoCenterParities(sp_ATOM *at, int num_atoms,
  ...
  for (i = 0, num_set = 0; i < num_atoms; i++) {
    ...
    if (num_neigh == 1) { /* other neighbors must be implicit H */
      ...
      trans_i = insertions_sort(nNeighRank, num_neigh, sizeof(nNeighRank[0]),
                                comp_AT_RANK);
      ...
      if (j < num_neigh) {
        /*  at least 2 neighbors are const. identical; parity cannot be
    ...
    for (iMax = (int)nAtomRank - 1, j1 = 0;
      ...
      */
      ...
      trans_k = insertions_sort(nNeighCanonRank, num_neigh,
                                sizeof(nNeighCanonRank[0]), comp_AT_RANK);
      trans_k %= 2;
      if (prev_trans < 0) {
        prev_trans = trans_k;
      } else if (trans_k != prev_trans) {
        /*  different mappings may produce different parities. Cannot find the
         * parity at this time */
        /*  this may happen when a set of constit. equivalent atoms has
         * non-contiguous canonical numbers */
        break;
      }
    }
...
int RemoveKnownNonStereoCenterParities(sp_ATOM *at, int num_atoms,
  ...
  /*AT_RANK nAtomRank;*/
  AT_RANK nNeighRank[MAX_NUM_STEREO_ATOM_NEIGH],
      nNeighOrd[MAX_NUM_STEREO_ATOM_NEIGH];
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    for (j = k = 1; k && j < num_neigh; j++) {
      if (at[i].nRingSystem !=
              at[(int)at[i].neighbor[(int)nNeighOrd[j]]].nRingSystem &&
        ...
        do {
          ...
          if (PathsHaveIdenticalKnownParities(
                  at, (AT_RANK)i, at[i].neighbor[(int)nNeighOrd[j - 1]],
                  (AT_RANK)i, at[i].neighbor[(int)nNeighOrd[k]], nVisited,
                  nVisited, nRank, nCanonRank, 1)) {
            ...
            break;
                 ...
                 nNeighRank[nNeighOrd[j - 1]] == nNeighRank[nNeighOrd[k]]);
      }
    }
  }
...
int MarkKnownEqualStereoCenterParities(sp_ATOM *at, int num_atoms,
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    nAtomRank = nRank[i];
    bDifferentParities = -1;
    /*  find constitutionally equivalent stereo centers and compare their known
     * at this time parities */
    for (iMax = (int)nAtomRank - 1, j1 = 0;
         j1 <= iMax && nAtomRank == nRank[k = (int)nAtomNumber[iMax - j1]];
         j1++) {
      /*  at[k] is constitutionally equivalent to at[i] */
...
/*****************************************************************************/
int InvertStereo(sp_ATOM *at, int num_at_tg, AT_RANK *nCanonRank,
      ...
      if (cumulene_len % 2) {
        ...
        if (1 < MAX_NUM_STEREO_BONDS) {
          if (at[j1].stereo_bond_neighbor[1] ||
            ...
            goto exit_error; /* inconsitency: atom has more than one cumulene
                                bond */
          }
        }
        ...
        if (PARITY_WELL_DEF(at[j1].parity) && PARITY_WELL_DEF(at[j2].parity)) {
          j = inchi_min(j1, j2);
          at[j].parity ^=
              AB_INV_PARITY_BITS; /* for reversability always invert only atom
...
int FillOutStereoParities(sp_ATOM *at, int num_atoms, const AT_RANK *nCanonRank,
                          const AT_RANK *nAtomNumberCanon, const AT_RANK *nRank,
                          const AT_RANK *nAtomNumber, CANON_STAT *pCS,
                          int bIsotopic) {
  int ret;
  ...
  ret = FillAllStereoDescriptors(at, num_atoms, nCanonRank, nAtomNumberCanon,
                                 pCS); /*  ret<0: error */
  if (!ret) {
  ...
  if (ret >= 0) {
    int ret2;
    ret2 = SetKnownStereoCenterParities(at, num_atoms, nCanonRank, nRank,
                                        nAtomNumber);
    if (ret2 >= 0) {
      ret2 =
          MarkKnownEqualStereoCenterParities(at, num_atoms, nRank, nAtomNumber);
    }
    if (ret2 >= 0) {
      ret2 = SetKnownStereoBondParities(at, num_atoms, nCanonRank, nRank,
                                        nAtomNumber);
      if (ret2 >= 0) {
        ret2 =
            MarkKnownEqualStereoBondParities(at, num_atoms, nRank, nAtomNumber);
      }
    }
#if (REMOVE_KNOWN_NONSTEREO == 1) /* { */
    if (ret2 >= 0) {
      int ret3;
      do {
        ret2 = RemoveKnownNonStereoCenterParities(at, num_atoms, nCanonRank,
                                                  nRank, pCS);
        if (ret2 >= 0) {
          ret3 = RemoveKnownNonStereoBondParities(at, num_atoms, nCanonRank,
                                                  nRank, pCS);
...
int GetStereoNeighborPos(sp_ATOM *at, int iAt1, int iAt2) {
  ...
  AT_RANK s;
  for (k1 = 0;
       k1 < MAX_NUM_STEREO_BONDS && (s = at[iAt1].stereo_bond_neighbor[k1]);
       k1++) {
    if (s == sNeigh) {
      return k1;
    }
...
int GetStereoBondParity(sp_ATOM *at, int i, int n, AT_RANK *nRank) {
  ...
  if (at[i].stereo_bond_neighbor[0]) {
    for (k1 = 0;
         k1 < MAX_NUM_STEREO_BONDS && (s = (int)at[i].stereo_bond_neighbor[k1]);
         k1++) {
      if (--s == n) {
        goto neigh1_found;
      }
  ...
  neigh1_found:
    ...
    for (k2 = 0;
         k2 < MAX_NUM_STEREO_BONDS && (s = (int)at[n].stereo_bond_neighbor[k2]);
         k2++) {
      if (--s == i) {
        goto neigh2_found;
      }
...
/*  Extracted from FillSingleStereoDescriptors(...) */
/**********************************************************************************/
int GetPermutationParity(sp_ATOM *at, AT_RANK nAvoidNeighbor,
                         AT_RANK *nCanonRank) {
  AT_RANK nNeighRank[MAX_NUM_STEREO_ATOM_NEIGH];
  int j, k, parity;
  if (at->valence > MAX_NUM_STEREO_ATOM_NEIGH) {
    ...
    if (k) {
      parity =
          insertions_sort(nNeighRank, k, sizeof(nNeighRank[0]), comp_AT_RANK);
      if (nNeighRank[0]) {
        parity = 2 - parity % 2;
      } else {
...
int GetStereoCenterParity(sp_ATOM *at, int i, AT_RANK *nRank) {
  ...
  if (ATOM_PARITY_WELL_DEF(at[i].parity)) {
    /*  number of neighbors transpositions to the sorted order is unknown. Find
     * it. */
    /*  If parity is not well-defined then doing this is a waste of time */
    int num_neigh = at[i].valence;
    for (k = 0; k < num_neigh; k++) {
      if (!nRank[(int)at[i].neighbor[k]])
        return 0; /*  stereo at[i] does not belong to the traversed part of the
                     structure */
      nNeighborNumber2[k] = k;
    }
    pNeighborsForSort = at[i].neighbor;
