...
void CopyAt2St(inp_ATOM *at, inp_ATOM_STEREO *st, int num_atoms) {
  ...
  for (i = 0; i < num_atoms; i++) {
    if (at[i].p_parity) {
      memcpy(st[i].p_orig_at_num, at[i].p_orig_at_num,
             sizeof(st[0].p_orig_at_num));
    ...
    if (at[i].sb_parity[0]) {
      ...
      memcpy(st[i].sn_orig_at_num, at[i].sn_orig_at_num,
             sizeof(st[0].sn_orig_at_num));
    }
  }
}
void CopySt2At(inp_ATOM *at, inp_ATOM_STEREO *st, int num_atoms) {
  ...
  for (i = 0; i < num_atoms; i++) {
    if (st[i].p_parity) {
      memcpy(at[i].p_orig_at_num, st[i].p_orig_at_num,
             sizeof(at[0].p_orig_at_num));
    ...
    if (st[i].sb_parity[0]) {
      ...
      memcpy(at[i].sn_orig_at_num, st[i].sn_orig_at_num,
             sizeof(at[0].sn_orig_at_num));
    }
  }
}

/******************************************************************************************************/
int RestoreAtomConnectionsSetStereo(StrFromINChI *pStruct, int iComponent,
  ...
  int num_stereo_bonds, num_stereo_centers, num_stereo_bonds2,
      num_stereo_centers2;
  ...
  /* atoms */
  pStruct->at = at =
      (inp_ATOM *)inchi_calloc(num_atoms, sizeof(pStruct->at[0]));
  if (!at) {
    ret = RI_ERR_ALLOC;
    goto exit_function;
  }
  ...
  /* coordinates: data from unused members: pInChI->IsotopicTGroup and
  ...
  } else if (pInChI->Stereo && (pInChI->Stereo->nNumberOfStereoBonds +
  ...
  */
  if (pStereo || pStereo2) {
    /* count implicit H needed for parities and reallocate at[]; set
     * at[n_vertex].at_type=1 for these atoms */
    ...
    for (i = i2 = 0; i < len1 || i2 < len2;) {
      ...
      if (at[n_vertex].valence == 2 && at[n_vertex].num_H == 0 &&
    ...
    for (i = i2 = 0; i < len1 || i2 < len2;) {
      ...
      if (!is_in_the_list(at[n_vertex].neighbor, (AT_NUMB)n_neigh,
                          at[n_vertex].valence)) {
        /* must be a cumulene */
    ...
    if (nNumDeletedH) {
      /* add explicit H */
      inp_ATOM *at2 =
          (inp_ATOM *)inchi_calloc(num_atoms + nNumDeletedH, sizeof(at2[0]));
      if (!at2) {
        ret = RI_ERR_ALLOC;
        goto exit_function;
      }
      ...
      for (i = 0; i < num_atoms; i++) {
        if (at[i].at_type == 1) {
          if (0 >
              (ret = AddExplicitDeletedH(at, i, num_atoms, &iDeletedH, &idelH1,
  ...
  if (pStereo) {
    /* mark stereo centers, they have already been connected the added explicit
     * H, if any */
    ...
    for (i = 0; i < pStereo->nNumberOfStereoCenters; i++) {
      ...
      if (bInvertedParity) {
        parity = (parity == AB_PARITY_EVEN)  ? AB_PARITY_ODD
                 : (parity == AB_PARITY_ODD) ? AB_PARITY_EVEN
                                             : parity;
      }
      ...
      if (at[n_vertex].valence == 2 && at[n_vertex].num_H == 0 &&
        ...
        if (at[jv].num_H) {
          if (0 >
              (ret = AddExplicitDeletedH(at, jv, num_atoms, &iDeletedH, &idelH1,
                                         nNumDeletedH, pStereo2 != NULL))) {
            goto exit_function;
          }
        } else {
        ...
        if (at[jn].num_H) {
          ...
          if (0 >
              (ret = AddExplicitDeletedH(at, jn, num_atoms, &iDeletedH, &idelH2,
                                         nNumDeletedH, pStereo2 != NULL))) {
            goto exit_function;
          }
        } else {
        ...
        /*
        if ( 0 > (ret = set_bond_type( at, (AT_NUMB)n_vertex, (AT_NUMB)jv,
        BOND_TYPE_DOUBLE ) ) || 0 > (ret = set_bond_type( at, (AT_NUMB)n_vertex,
        (AT_NUMB)jn, BOND_TYPE_DOUBLE ) ) ) { goto exit_function;
        }
        */
        /* allene: make 0D parity */
        ret = set_cumulene_0D_parity(at, st, num_atoms, idelH1, jv, jn, idelH2,
                                     parity, 2);
        if (ret < 0) {
          goto exit_function;
        }
        ...
        ret = set_atom_0D_parity(at, st, num_atoms, nNumDeletedH, n_vertex,
                                 parity);
        if (ret < 0) {
          goto exit_function;
        }
    ...
    for (i = 0; i < pStereo->nNumberOfStereoBonds; i++) {
      ...
      if (!is_in_the_list(at[jv].neighbor, (AT_NUMB)jn, at[jv].valence)) {
        /* must be a cumulene */
        if (!bFindCumuleneChain(at, (AT_NUMB)jv, (AT_NUMB)jn, nCumulene,
                                MAX_CUMULENE_LEN + 1)) {
      ...
      if (at[jv].num_H) {
        ...
        /* double bond neighbor that has the smallest canonical number; it is
         * either 0th or 1st */
      ...
      if (0 > (ret = set_cumulene_0D_parity(at, st, num_atoms, idelH1, jv, jn,
                                            idelH2, parity, len))) {
        goto exit_function;
      }
    }
  ...
  if (num_stereo_centers2) {
    /* In case of Fixed-H */
    /* mark additional Mobile-H stereo centers, they have already been connected
     * the added explicit H, if any */
  ...
  if (num_stereo_bonds2) {
    /* In case of Fixed-H */
    /* mark additional Mobile-H stereobonds, they have already been connected
     * the added explicit H, if any */
...
int SetStereoBondTypeFor0DParity(inp_ATOM *at, int i1, int m1) {
  ...
  do {
    ...
    n1 = n2;
      ...
      at[n1].valence == 2 && !at[n1].num_H &&
      nLenCumulene < MAX_CUMULENE_LEN + 2 &&
...
found:
  ...
  for (j = 1; j < nLenCumulene; j++) {
    ...
    if (0 > (ret = set_bond_type(at, (AT_NUMB)nCumulene[j - 1],
                                 (AT_NUMB)nCumulene[j], bond_type))) {
  ...
  if (pStereo) {
    ...
    if (num_stereo_bonds) {
      ...
      for (i = 0; i < num_atoms; i++) {
        ...
        if (num_bond_type_stereo + num_bond_type_altern > 1 &&
            num_bond_type_stereo) {
          for (j = 0; j < at[i].valence; j++) {
            if (at[i].bond_type[j] == BOND_TYPE_STEREO) {
              neigh = at[i].neighbor[j];
              /* does not change at[i].chem_bond_valence in case of
               * BOND_TYPE_ALTERN */
              if (0 > (ret = set_bond_type(at, (AT_NUMB)i, neigh,
                                           BOND_TYPE_ALTERN))) {
                goto exit_function;
              }
            }
...
/******************************************************************************************************/
int CopyBnsToAtom(StrFromINChI *pStruct, BN_STRUCT *pBNS, VAL_AT *pVA,
                  ALL_TC_GROUPS *pTCGroups, int bAllowZeroBondOrder) {
...
/******************************************************************************************************/
int CheckBnsConsistency(StrFromINChI *pStruct, BN_STRUCT *pBNS, VAL_AT *pVA,
                        ALL_TC_GROUPS *pTCGroups, int bNoRad) {
  int nOutput = 0;
  ...
  sprintf(s, "%d:%d%sM%02dv%da%de%db%d* ", bNoRad, pTCGroups->iComponent + 1,
          num_electrons % 2 ? "O" : "E", nNumMetalAtoms, pBNS->num_vertices,
          num_at, pBNS->num_edges, pBNS->num_bonds);

  tot_st_flow = tot_st_cap = 0;
  atom_charge = left_charge = 0;
  if (pBNS->num_atoms != num_at) {
    fprintf(stdout, "\n%sNum. atoms discrepancy: %d(BNS) vs. %d(at) ", s,
            pBNS->num_atoms, num_at);
    nOutput++;
  }
  /* check edges */
#ifdef _DEBUG
  ...
  for (i = 0; i < pBNS->num_edges; i++) {
    ...
    if (pe->cap < pe->flow || pe->flow < 0) {
      fprintf(stdout, "\n%sedge %d (%d-%d) has cap=%d flow=%d  ", s, i, v1, v2,
              pe->cap, pe->flow);
      nOutput++;
    }
#ifdef _DEBUG
  ...
  for (i = 0; i < pBNS->num_vertices; i++) {
    ...
    if (bDebugOutput && bNoRad) {
      ...
      fprintf(stderr, "%4d %4d/%-4d %4d/%-4d 0x%03X %5s", i, pv->st_edge.cap,
              pv->st_edge.cap0, pv->st_edge.flow, pv->st_edge.flow0, pv->type,
              s);
      for (j = 0; j < pv->num_adj_edges; j++) {
    ...
    if (pv->num_adj_edges > pv->max_adj_edges) {
      fprintf(stdout, "\n%s%s %d type 0x%X \"%s\" num_edges=%d > max=%d  ", s,
              i < num_at ? "atom" : "vertex", i, pv->type, at[i].elname,
              pv->num_adj_edges, pv->max_adj_edges);
      nOutput++;
    }
    if (i < num_at) {
      /* charge on atoms */
      ...
      if (bNoRad && pv->st_edge.flow != pv->st_edge.cap) {
        fprintf(stdout,
                "\n%s%s %d: type 0x%X \"%s\" unexpected st_cap=%d st_flow=%d ",
                s, i < num_at ? "atom" : "vertex", i, pv->type, at[i].elname,
                pv->st_edge.cap, pv->st_edge.flow);
        nOutput++;
      } else if (bNoRad && charge && !strcmp(at[i].elname, "C")) {
        /* ignore carbonyls */
      ...
      atom_charge += charge;
    } else if ((charge = pv->st_edge.cap - pv->st_edge.flow) > 0) {
      /* excess charge */
      if (!bNoRad && IS_BNS_VT_C_OR_CSUPER_GR(pv->type)) {
        left_charge -= charge;
      } else if (!bNoRad && IS_BNS_VT_YVCONNECTOR(pv->type)) {
        left_charge += charge;
      } else if (!bNoRad && IS_BNS_VT_M_GR(pv->type) &&
                 0 <= (j = pTCGroups->nGroup[TCG_MeFlower3]) &&
                 i == pTCGroups->pTCG[j].nVertexNumber) {
        ; /* additional "radical" on metal flower */
      } else if (!(pv->type & BNS_VERT_TYPE_TGROUP) || bNoRad) {
        /* t-groups before running BFS should have st_cap > st_flow */
        fprintf(stdout, "\n%s%s %d: type 0x%X unexpected st_cap=%d st_flow=%d ",
                s, i < num_at ? "atom" : "vertex", i, pv->type, pv->st_edge.cap,
                pv->st_edge.flow);
        nOutput++;
      }
    }
    if (pv->st_edge.cap < pv->st_edge.flow || pv->st_edge.flow < 0) {
      fprintf(stdout, "\n%s%s %d: type 0x%X \"%s\" st_cap=%d st_flow=%d  ", s,
              i < num_at ? "atom" : "vertex", i, pv->type,
              i < num_at ? at[i].elname : "", pv->st_edge.cap,
              pv->st_edge.flow);
      nOutput++;
    }
    ...
    if (flow != pv->st_edge.flow) {
      fprintf(stdout, "\n%s%s %d: type 0x%X \"%s\" st_flow=%d edge_flow=%d  ",
              s, i < num_at ? "atom" : "vertex", i, pv->type,
              i < num_at ? at[i].elname : "", pv->st_edge.flow, flow);
      nOutput++;
    }
  }
#ifdef _DEBUG
  ...
  /*
  if ( num_electrons %= 2 ) {
      fprintf( stdout, "\n%d*Odd number of electrons (%d atoms) ", bNoRad,
  num_at ); nOutput ++;
  }
  */
  ...
  excess_charge = pTCGroups->total_charge - pTCGroups->added_charge -
                  pTCGroups->tgroup_charge;
  if (charge != pTCGroups->total_charge &&
      excess_charge != pTCGroups->total_charge - charge) {
    fprintf(
        stdout,
...
/******************************************************************************************************/
int AddExplicitDeletedH(inp_ATOM *at, int jv, int num_at, int *iDeletedH,
                        int *iH, int nNumDeletedH, int bTwoStereo) {
  ...
  if (at[jv].at_type > 1) {
    ...
    for (i = 0; i < *iDeletedH; i++) {
      if (at[num_at + i].neighbor[0] == jv) {
        *iH = num_at + i; /* return the first found H, it has the smallest
                             canonical pseudo rank */
        return 0;
      }
    }
    return RI_ERR_PROGR;
  ...
  *iH =
      *iDeletedH + num_at; /* num_H includes all H, both isotopic and normal */
  ...
  for (; num_H && (*iDeletedH) < nNumDeletedH; (*iDeletedH)++) {
    cur_H =
        at + num_at +
        (*iDeletedH); /* first available empty atom will be this explicit H */
    cur_H->neighbor[cur_H->valence] =
        jv; /* connect this new atom H to the real atom */
    ...
    if (num_H > tot_num_iso_H) {
      ...
      if (num_H != tot_num_iso_H) {
        ...
        if (bTwoStereo) {
          ...
          return RI_ERR_SYNTAX; /* two identical H neighbors of a stereo
                                   atom/bond */
        }
      }
    } else {
      ...
      if (iso_H < NUM_H_ISOTOPES) {
        ...
        num_iso_H[iso_H]--;
        if (num_iso_H[iso_H]) {
          return RI_ERR_SYNTAX; /* two identical isotopic H neighbors of a
                                   stereo atom/bond */
        }
      } else {
  ...
  at[jv].at_type++; /* at[jv].at_type==2 => explicit hydrogens have already been
                       added */
  return 0;         /* success */
}
/******************************************************************************************************/
int bFindCumuleneChain(inp_ATOM *at, AT_NUMB i1, AT_NUMB i2,
                       AT_NUMB nCumulene[], int nMaxLen)
...
/* nCumulene[nMaxLen+1] will contain cumulene chain >i1=x=y=i2< in this order */
{
  ...
  for (i = 0; i < at[i1].valence; i++) {
    ...
    while (at[nat].valence == 2 && at[nat].num_H == 0 &&
...
int set_bond_type(inp_ATOM *at, AT_NUMB i1, AT_NUMB i2, int bType) {
  ...
  if (p1 && p2) {
    ...
    if (bTypePrev && bTypePrev <= BOND_TYPE_TRIPLE && bType &&
        bType <= BOND_TYPE_TRIPLE) {
...
/******************************************************************************************************/
int set_cumulene_0D_parity(inp_ATOM *at, inp_ATOM_STEREO *st, int num_at,
                           int idelH1, int i1, int i2, int idelH2, int parity,
                           int len) {
  AT_NUMB nCumulene[MAX_CUMULENE_LEN + 2];
  AT_NUMB *p1, *p2;
  int m1, m2, parity1, parity2, sb_ord_m1, sb_ord_m2, k1, k2, num_neigh1,
      num_neigh2;
  /* the following types must exactly match types in inp_ATOM and
   * inp_ATOM_STEREO */
  ...
  /* stereo bond neighbors: index of a stereo bond in its end-atom adjacency
   * lists */
  if ((p1 = is_in_the_list(at[i1].neighbor, nCumulene[1], at[i1].valence)) &&
      (p2 = is_in_the_list(at[i2].neighbor, nCumulene[len - 1],
                           at[i2].valence))) {
    sb_ord_m1 =
        p1 -
        at[i1].neighbor; /* indes of stereobond in the atom's adjacency list */
  ...
  if (num_neigh1 < MIN_NUM_STEREO_BOND_NEIGH ||
      num_neigh1 > MAX_NUM_STEREO_BOND_NEIGH ||
      num_neigh2 < MIN_NUM_STEREO_BOND_NEIGH ||
      num_neigh2 > MAX_NUM_STEREO_BOND_NEIGH) {
    return RI_ERR_SYNTAX;
  }
  ...
  /* find the first unoccupied locations in the stereobond 0D descriptor lists;
   * check whether the stereo has already been set */
  for (m1 = k1 = 0; m1 < MAX_NUM_STEREO_BONDS && sb_parity1[m1] &&
                    !(k1 = sb_ord1[m1] == sb_ord_m1);
       m1++)
    ;
  for (m2 = k2 = 0; m2 < MAX_NUM_STEREO_BONDS && sb_parity2[m2] &&
                    !(k2 = sb_ord2[m2] == sb_ord_m2);
       m2++)
    ;
  ...
  if (k1 && k2) {
    return 0; /* the stereo descriptor of this bond/allene/cumulene has already
                 been set */
  }
  if (k1 || k2) {
  ...
  if (idelH1 < num_at) {
    if ((p1 = is_in_the_list(at[i1].neighbor, (AT_NUMB)idelH1,
                             at[i1].valence))) {
  ...
  if (idelH2 < num_at) {
    if ((p2 = is_in_the_list(at[i2].neighbor, (AT_NUMB)idelH2,
                             at[i2].valence))) {
  ...
  if (ATOM_PARITY_WELL_DEF(parity)) {
    ...
    int num_inv = (num_neigh1 == MIN_NUM_STEREO_BOND_NEIGH) +
                  (num_neigh2 == MIN_NUM_STEREO_BOND_NEIGH);
    if (num_inv % 2) {
...
/******************************************************************************************************/
int set_atom_0D_parity(inp_ATOM *at, inp_ATOM_STEREO *st, int num_at,
                       int num_deleted_H, int i1, int parity) {
  ...
  /* 3. Normal neighboring atoms, atom numbers (=canonical numbers from InChI -
  ...
  if (!st || !at[i1].p_parity) {
    ...
    /* stereoneighbors: deleted explicit atoms H first, in order of increasing
     * isotopic mass */
    if (at[i1].num_H) {
...
#if (BNS_RAD_SEARCH == 1)
/******************************************************************************************************/
int MoveRadToAtomsAddCharges(BN_STRUCT *pBNS, BN_DATA *pBD,
                             StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2,
                             VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups,
                             int forbidden_mask) {
  int nNumRad, ret = 0, ret2;
  int i, j, k, num_rad_not_atom, num_moved = 0, num_candidates = 0,
                                 extra_charge = 0, added_charge, delta;
  BNS_EDGE *pEdge;
  BNS_VERTEX *pv;
  Vertex v1, v2;
  ...
  if (num_moved) {
    /* find reqired charge */
    extra_charge = 0;
    for (i = pBNS->num_atoms, pv = pBNS->vert + i; i < pBNS->num_vertices;
         i++, pv++) {
    ...
    for (i = 0; i < pBNS->num_vertices; i++) {
      pnRad[i] =
          (pBNS->vert[i].st_edge.cap - pBNS->vert[i].st_edge.flow) - pnRad[i];
  ...
  if (num_candidates > 0) {
    ...
    for (i = 0, j = 0; i < pBNS->num_vertices; i++) {
      if (i < pBNS->num_atoms && pnRad[i] > 0 && !pVA[i].nTautGroupEdge) {
        ...
        cnBits =
            (pVA[i].cnListIndex > 0) ? cnList[pVA[i].cnListIndex - 1].bits : 0;
        while (cnBits > 0) {
    ...
    for (k = 0; k < j; k++) {
      ...
      int this_atom_add_charge =
          rest_of_charge > 0 ? charge_per_left_atom : -charge_per_left_atom;
      ...
      if (this_atom_add_charge) {
        for (i = pBNS->num_vertices - 1, pv = pBNS->vert + i;
          ...
          if ((delta = pv->st_edge.cap - pv->st_edge.flow)) {
            if (this_atom_add_charge < 0 &&
                IS_BNS_VT_C_OR_CSUPER_GR(pv->type)) {
            ...
            } else if (this_atom_add_charge > 0 &&
                       BNS_VERT_TYPE__AUX == pv->type) {
...
int AdjustTgroupsToForbiddenEdges2(BN_STRUCT *pBNS, inp_ATOM *at, VAL_AT *pVA,
  ...
  int neigh, tg_number, num_eql_mobile_gr, num_dif_mobile_gr, bond_type,
      has_mobile_H, has_mobile;
  int num_forbidden, ind_forbidden, forbidden, num_N, num_O, num_P, num_S,
      num_OSt;
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (at[i].chem_bonds_valence == at[i].valence || at[i].num_H ||
        ...
        2 > (delta_val = at[i].chem_bonds_valence -
                         (val = get_el_valence(at[i].el_number, 0, 0))) ||
        2 > (delta_met =
                 (bonds_valence_non_metal = nNoMetalBondsValence(at, i)) -
                 val)) {
      continue;
    }

    num_donors = num_acceptors = num_donor_endpoints = num_acceptor_endpoints =
        0;
    num_mgroups = num_endpoints = num_diff_t_groups = 0;
    has_mobile = has_mobile_H = num_eql_mobile_gr = num_dif_mobile_gr =
        tg_number = 0;
    ind_forbidden = -1;
    num_forbidden = 0;
    ...
    for (j = 0; j < at[i].valence; j++) {
      ...
      MobileGr[num_endpoints].bonds_valence_non_metal =
          nNoMetalBondsValence(at, neigh);

      if (forbidden & forbidden_mask) {
        num_forbidden++;
        ind_forbidden = num_endpoints;
      }
      num_O += 0 != (neigh_type & EL_TYPE_O) &&
               at[neigh].valence == 1; /* ignore -O- */
      num_N += 0 != (neigh_type & EL_TYPE_N) &&
               !(at[neigh].valence == 3 &&
                 at[neigh].chem_bonds_valence == 3); /* ignore -N< */
      num_S += 0 != (neigh_type & EL_TYPE_S) &&
               at[neigh].valence == 1; /* ignore -S- */
      num_P += 0 != (neigh_type & EL_TYPE_P) &&
               !(at[neigh].valence == 3 &&
                 at[neigh].chem_bonds_valence == 3); /* ignore -P< */
      num_OSt += 0 != (neigh_type & EL_TYPE_OSt);
      num_acceptors +=
          (bond_type == BOND_TYPE_DOUBLE) && (neigh_type & EL_TYPE_PT);
      num_donors +=
          (bond_type == BOND_TYPE_SINGLE) && (neigh_type & EL_TYPE_PT) &&
    ...
    /* case_5_1: */
    /***************** determine the case ************************/
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_C) && 2 == num_O && 1 == num_N + num_S &&
        num_OSt && 1 == num_forbidden && 3 == num_eql_mobile_gr) {
      /********************************************************
      ...
      *********************************************************/
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & (EL_TYPE_N | EL_TYPE_S)) &&
            ...
            MobileGr[j].bond_type == BOND_TYPE_SINGLE && jXH < 0) {
          jXH = j;
          n++;
  ...
  case_5_2:
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 5 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_N) && 2 == num_O && 1 == num_N + num_S &&
      ...
      *********************************************************/
      ...
      pBNS->vert[ev2->neighbor12 ^ ev2->neighbor1]
          .st_edge.flow--; /* taut group vertex tg */
      pBNS->tot_st_flow -= 2;
      num_changes++;
      continue;
    }
  case_5_4:
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 5 == bonds_valence_non_metal &&
        (centerpoint_type & (EL_TYPE_N | EL_TYPE_P)) && 1 == num_O + num_S &&
        0 < num_N && 2 == (num_N + num_P) && 1 == num_forbidden &&
      ...
      *********************************************************/
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
            MobileGr[j].forbidden == forbidden_mask &&
            MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
            MobileGr[j].num_bonds_non_metal == 1 && jO < 0) {
          jO = j;
          n++;
  ...
  case_5_6:
    ...
    if (2 == delta_met && 4 == num_bonds_non_metal &&
        5 == bonds_valence_non_metal && 1 == num_forbidden &&
        1 < num_eql_mobile_gr && !num_dif_mobile_gr &&
        (centerpoint_type & (EL_TYPE_N | EL_TYPE_P)) && 1 <= num_N &&
        2 <= num_N + num_O + num_S && 1 == num_acceptor_endpoints &&
        0 < num_donor_endpoints) {
      ...
      /* centerpoint is N, P, As, Sb
       ...
       Y---N===N---       Y---N---NH---
        ...
        3. Fix all single order bonds to not terminal tautomeric N around
       N(centerpoint)
      ...
      */
      ...
      for (j = 0; j < num_endpoints; j++) {
        ...
        if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
            ...
            MobileGr[j].bonds_valence_non_metal == 3 && at[neigh].endpoint &&
      ...
      e = pBNS->edge +
          pBNS->vert[i].iedge[MobileGr[jN].ineigh]; /* fixed edge */
      if (!e->flow) {
        goto case_5_7;
      }
      e->flow--;
  ...
  case_5_7:
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 2 == num_O + num_S && 1 == num_OSt &&
        1 == num_N && 1 == num_forbidden && 3 == num_eql_mobile_gr &&
     ...
     ev2||     \\ev1             \
      ...
      Solution:
        Move H from NH to =O to allow HN=S bond change by making a 2nd terminal
     -OH (this unfixes the bond, see fix_special_bonds(...) )
      *********************************************************/
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N &&
            ...
            MobileGr[j].num_bonds_non_metal <= 2 && jNH < 0) {
          jNH = j;
          n++;
        } else if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
                   !MobileGr[j].forbidden &&
                   MobileGr[j].num_bonds_non_metal == 1) {
          if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE && jO < 0) {
            jO = j;
            n++;
          } else if (jOH < 0) {
            jOH = j;
  ...
  case_5_9a:
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 1 == num_O + num_S && !num_OSt &&
        1 <= num_N && 1 == num_forbidden &&
        num_O + num_S + num_N == num_eql_mobile_gr &&
        MobileGr[ind_forbidden].bond_type == BOND_TYPE_SINGLE) {
      /********************************************************
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & (EL_TYPE_O | EL_TYPE_S)) &&
          ...
          n++;
                   ...
                   !MobileGr[j].forbidden &&
                   MobileGr[j].bond_type == BOND_TYPE_SINGLE && jNH < 0) {
          jNH = j;
          n++;
        } else if (jX < 0) {
          jX = j;
  ...
  case_5_8b_to_9b: /* #1 */
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 0 == num_O + num_S && 2 == num_N &&
        0 == num_P && !num_OSt && 1 == num_forbidden &&
       ...
       ********************************************************
       ...
       not    \  |     |         \ ||     ||  -X is not terminal -O,-S,-Se,-Te
     or an      \ |  e  |          \||  e  ||        any N, P, As endpoint
     NH=====tg          N------tg is an                   f       N, N, X, fixed
     single endpoint                          =====================
      ...
      *********************************************************/
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N &&
            !(MobileGr[j].forbidden & forbidden_mask)) {
          if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
              !at[MobileGr[j].atom_number].endpoint && jN < 0) {
            jN = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds == 2 &&
                     MobileGr[j].bonds_valence == 2 &&
                     at[MobileGr[j].atom_number].endpoint && jNH < 0) {
            jNH = j;
            n++;
          }
        } else if (!((MobileGr[j].atom_type_pVA & (EL_TYPE_N | EL_TYPE_P)) ||
                   ...
                   (MobileGr[j].forbidden & forbidden_mask) &&
                   MobileGr[j].bond_type == BOND_TYPE_SINGLE && jX < 0) {
          jX = j;
          n++;
        }
      }
  ...
  case_5_8c_to_9c: /* #2 */
    /*********************************************************************/
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 0 == num_O + num_S && 3 == num_N &&
        0 == num_P && 1 == num_forbidden && 3 == num_eql_mobile_gr &&
        0 == num_dif_mobile_gr && 2 == num_donor_endpoints &&
        1 == num_acceptor_endpoints && 2 == num_donors && 1 == num_acceptors &&
        MobileGr[ind_forbidden].bond_type == BOND_TYPE_SINGLE) {
      /********************************************************
       ...
       ********************************************************
       ...
       \       / pv1  pv2 \       /            f  =  fixed bond
        \ 1   /  C-----ZH  \     /  C=====Z    tg =  Mobile-H vertex
         N===S  ||     ||   N===S   |     |    X  =  is N not an endpoint;
      is an   \f||ev1  ||ev2     \f |     |   -X is not terminal -O,-S,-Se,-Te
    or endpoint   \||  e  ||         \ |  e  |         any N, P, As 2 N------tg
    NH=====tg   C is a centerpoint of a t-group is an                   f N, N,
    X, fixed single endpoint                          =====================
      ...
      *********************************************************/
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N &&
            !(MobileGr[j].forbidden & forbidden_mask)) {
          if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
              at[MobileGr[j].atom_number].endpoint && jN1 < 0) {
            jN1 = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds == 2 &&
                     ...
                     MobileGr[j].forbidden == forbidden_mask &&
                     at[MobileGr[j].atom_number].endpoint && jN2 < 0) {
            jN2 = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds <= 2 &&
                     MobileGr[j].bonds_valence <= 3 &&
                     at[MobileGr[j].atom_number].endpoint && jX < 0) {
            jX = j;
            n++;
          }
        }
  ...
  case_5_9b_to_8b: /* #3 */
    ...
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 0 == num_O + num_S && 2 == num_N &&
        0 == num_P && 1 == num_forbidden && 2 == num_eql_mobile_gr &&
        0 == num_dif_mobile_gr && 1 == num_donor_endpoints &&
        1 == num_acceptor_endpoints && 1 == num_donors && 1 == num_acceptors &&
   ...
   \       /   endpoint    \       /           f  =  fixed bond
    \ 1ev /  C-----ZH       \     /  C=====Z   tg =  Mobile-H vertex
     N===S  ||     ||        N===S   |     |   X  =  is N not an endpoint;
is an  f  \ ||     ||          f  \  |     |  -X is not terminal -O,-S,-Se,-Te
or endpoint     2\||  e  ||              \ |  e  |        any N, P, As N------tg
NH=====tg is an                          f      N, N, X, fixed double endpoint
=====================
      ...
      Solution:
        Move H from ZH to N2 to make N1=S bond unfixed and fix S-X bond (Table
5, case 8) (see fix_special_bonds(...) for details )
      *********************************************************/
      int jN1 = -1, jN2 = -1, jX = -1, n = 0;
      EdgeIndex ie, ie1, ie2;
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N) {
          if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
              ...
              (MobileGr[j].forbidden == forbidden_mask) && jN1 < 0) {
            jN1 = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds == 2 &&
                     MobileGr[j].bonds_valence == 3 &&
                     at[MobileGr[j].atom_number].endpoint &&
                     !(MobileGr[j].forbidden & forbidden_mask) && jN2 < 0) {
            jN2 = j;
            n++;
          }
        } else if (!((MobileGr[j].atom_type_pVA & (EL_TYPE_N | EL_TYPE_P)) ||
  ...
  case_5_9c_to_8c: /* #4 */
    ...
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 0 == num_O + num_S && 3 == num_N &&
        0 == num_P && 0 == num_forbidden && 2 == num_diff_t_groups &&
        2 == num_mgroups && /* all neighbors belong to 2 t-groups */
        3 == num_eql_mobile_gr +
                 num_dif_mobile_gr && /* all 3 neighbors belong to t-groups */
        2 == num_donor_endpoints &&
        1 == num_acceptor_endpoints && 2 == num_donors && 1 == num_acceptors) {
      /********************************************************
       ***         InChI Tech. Man., Table 5, case 8c->9c   ***
       ********************************************************
       ...
       \       / pv1  pv2 \       /            f  =  fixed bond
        \(1)  /  C=====Z   \     /  C-----ZH   tg =  Mobile-H vertex
         N===S   |     |    N===S  ||     ||   X  =  is N not an endpoint;
      is an   \  |ev1  | ev2     \ ||     ||  -X is not terminal -O,-S,-Se,-Te
    or endpoint   \ |  e  |          \||  e  ||        any N, P, As tg1
    (2)NH=====tg          N------tg   C is a centerpoint of a t-group is an f N,
    N, X, fixed single endpoint                          =====================
             tg2
      Problem:
        N (1) and NH2 are Mobile-H group 1 endpoiints, NH (2) is a Mobile-H
    group 2 endpoint Unfixed bonds N==S--NH(2) allows the two Mobile H groups to
    merge hence prevents the correct structure restoration: H can migrate from
    NH (2) to N (1) because S-NH(1) bond is not fixed Solution: Move H from
    NH(2) to =Z to make S-NH(2) bond fixed (Table 5, case 8c) (this unfixes the
    bond, see fix_special_bonds(...) )
      *********************************************************/
      int jN1 = -1, jN2 = -1, jX = -1, n = 0;
      ...
      for (j = 0; j < num_endpoints; j++) {
        if ((MobileGr[j].atom_type_pVA & EL_TYPE_MASK) == EL_TYPE_N) {
          if (MobileGr[j].bond_type == BOND_TYPE_DOUBLE &&
              at[MobileGr[j].atom_number].endpoint &&
              at[MobileGr[j].atom_number].endpoint != k && jN1 < 0) {
            jN1 = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds == 2 &&
                     MobileGr[j].bonds_valence == 2 &&
                     at[MobileGr[j].atom_number].endpoint == k && jN2 < 0) {
            jN2 = j;
            n++;
          } else if (MobileGr[j].bond_type == BOND_TYPE_SINGLE &&
                     MobileGr[j].num_bonds <= 2 &&
                     MobileGr[j].bonds_valence <= 3 &&
                     at[MobileGr[j].atom_number].endpoint &&
                     at[MobileGr[j].atom_number].endpoint != k && jX < 0) {
            jX = j;
            n++;
          }
        }
  ...
  case_5_9c_to_9d: /* #6 */
    ...
    if (3 == num_bonds_non_metal && 4 == bonds_valence_non_metal &&
        (centerpoint_type == EL_TYPE_S) && 0 == num_O + num_S && 3 == num_N &&
        0 == num_P && 0 == num_forbidden && 3 == num_mgroups &&
        2 == num_diff_t_groups && 2 == num_donor_endpoints &&
        0 == num_acceptor_endpoints && 2 == num_donors && 1 == num_acceptors) {
      /********************************************************
       ***         InChI Tech. Man., Table 5, case 9b->8b   ***
       ********************************************************
           3(X)      --->         e2|          O  =  O, S, Se, Te
   ...
   \       /   endpoint    \       /           f  =  fixed bond
    \ 1   /  C=====Z        \  f  /  C-----Z   tg =  Mobile-H vertex
     N===S   |     |         N===S  ||     ||  X  =  is N not an endpoint;
is an  ev  \  |     |           ev \ ||     || -X is not terminal -O,-S,-Se,-Te
or endpoint     2\ |  e1 |               \||  e1 ||       any N, P, As NH=====tg
N------tg is an                          f      N, N, X, fixed double endpoint
=====================

      Problem:
        N1, N2, and N3 are Mobile-H endpoints and belong to the same Mobile-H
group. Fixed bond N1==S prevents the correct structure restoration: H cannot
migrate N3->N2->N1 because N1=S bond is fixed Solution: Move mobile H to N2 and
N3 to make N1=S bond unfixed (Table 5, case 9c) (see fix_special_bonds(...) for
details )
      *********************************************************/
      int jN1 = -1, jN2 = -1, jX = -1, n = 0;
      for (j = 0; j < num_endpoints; j++) {
...
/******************************************************************************************************/
/* Replace ambiguous neutral (+)edge->flow=0, (-)edge->flow=1 with
 * (+)edge->flow=1, (-)edge->flow=0   */
/******************************************************************************************************/
...
/******************************************************************************************************/
int IncrementZeroOrderBondsToHeteroat(BN_STRUCT *pBNS, BN_DATA *pBD,
                                      StrFromINChI *pStruct, inp_ATOM *at,
                                      inp_ATOM *at2, VAL_AT *pVA,
                                      ALL_TC_GROUPS *pTCGroups,
    ...
    for (j = 0; j < at2[i].valence; j++) {
      ...
      if (!(at2[i].bond_type[j] & BOND_TYPE_MASK)) {
        ...
        if (!bFixedCarbonCharges) {
          ...
          if (0 > (ret = ForbidCarbonChargeEdges(pBNS, pTCGroups,
                                                 &CarbonChargeEdges,
                                                 forbidden_edge_mask))) {
            goto exit_function;
          }
        ...
        for (k = 0; k < at2[i].valence; k++) {
          ...
          if (pe->flow == 1 && !(pe->forbidden & forbidden_edge_mask)) {
            if ((ret = AddToEdgeList(&NewlyFixedEdges, pe - pBNS->edge,
                                     FIX_BOND_ADD_ALLOC))) {
              goto exit_function;
            }
        ...
        for (k = 0; k < num_at; k++) {
          if (!pVA[k].cMetal && pVA[k].cNumValenceElectrons == 5 &&
              at2[k].valence == 2 && !at2[k].num_H &&
              pVA[k].cMinRingSize <= 6 && pVA[k].nCPlusGroupEdge > 0 &&
        ...
        if (ret == 1 &&
            ((vPathEnd == vMeFlower0 && vPathStart == vNeighMeigh) ||
             (vPathEnd == vNeighMeigh && vPathStart == vMeFlower0)) &&
            abs(nDeltaCharge) <= 2) {
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            (*pnNumRunBNS)++;
          ...
          if ((ret = AddToEdgeList(&NewlyFixedEdges, peZero - pBNS->edge,
                                   FIX_BOND_ADD_ALLOC))) {
            goto exit_function;
          }
        } else {
...
/***********************************************************************
        ...
        NH2                NH2
************************************************************************/
int MovePlusFromS2DiaminoCarbon(BN_STRUCT *pBNS, BN_DATA *pBD,
                                StrFromINChI *pStruct, inp_ATOM *at,
                                inp_ATOM *at2, VAL_AT *pVA,
                                ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
  ...
  for (i = 0; i < num_at; i++) {
    if (!pVA[i].cMetal && pVA[i].cNumValenceElectrons == 6 &&
        ...
        0 <= (ePlusS = pVA[i].nCPlusGroupEdge - 1) &&
        !(pePlusS = pBNS->edge + ePlusS)->flow && /* S(+) */
      ...
      for (j = 0; j < k; j++) {
        ...
        if (pVA[vN].cNumValenceElectrons != 5 ||
            ...
            at2[vN].num_H != 2 || at2[vN].endpoint ||
            (pStruct->endpoint && pStruct->endpoint[vN])) {
      ...
      if (ret == 1 &&
          ((vPathEnd == v1 && vPathStart == v2) ||
           (vPathEnd == v2 && vPathStart == v1)) &&
          nDeltaCharge == -1) {
        /* Remover (+)charge from S => nDeltaCharge == -1 */
        /* Flow change on pe (+)charge edge (atom S) is not known to
         * RunBnsTestOnce()) */
        ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
        if (ret > 0) {
          (*pnNumRunBNS)++;
...
/******************************************************************************************************/
int EliminateChargeSeparationOnHeteroatoms(
    BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct, inp_ATOM *at,
    inp_ATOM *at2, VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
    int *pnTotalDelta, int forbidden_edge_mask, int forbidden_stereo_edge_mask)
/********* Avoid charge separation on heteroatoms ******************/
{
  ...
  if (forbidden_stereo_edge_mask) {
    for (i = 0; i < num_at; i++) {
      for (j = 0; j < at2[i].valence; j++) {
        if (pBNS->edge[k = pBNS->vert[i].iedge[j]].forbidden ==
            forbidden_stereo_edge_mask) {
          int nMinRingSize = is_bond_in_Nmax_memb_ring(
              at2, i, j, pStruct->pbfsq->q, pStruct->pbfsq->nAtomLevel,
              pStruct->pbfsq->cSource, 99 /* max ring size */);
          if (0 < nMinRingSize &&
              (ret = AddToEdgeList(&FixedLargeRingStereoEdges, k, 64))) {
            goto exit_function;
          }
        }
      }
    ...
    if (!FixedLargeRingStereoEdges.num_edges) {
      ...
      RemoveForbiddenEdgeMask(pBNS, &FixedLargeRingStereoEdges,
                              forbidden_stereo_edge_mask);
    }
  }
  ...
  if (num_min && (k = pTCGroups->nGroup[TCG_Plus]) >= 0 &&
    ...
    if (num_min && (!nDeltaPlus1 && !nDeltaMinus1)) {
      if (!bFixedCarbonCharges) { /* 02-02-2006 */
        /* do not let carbon atoms get charged */
        if (0 >
            (ret = ForbidCarbonChargeEdges(pBNS, pTCGroups, &CarbonChargeEdges,
                                           forbidden_edge_mask))) {
          goto exit_function;
        }
        bFixedCarbonCharges++;
      ...
      for (i = 0; i < num_min; i += delta) {
        ...
        /* convert a pair of N(IV)(+) into a pair of N(V) neutral with total
         * charge reduced by 2 */
        pEdge->flow -= delta;
        ...
        if (ret == 1 &&
            ((vPathEnd == vPlusSuper && vPathStart == vPlusMinus) ||
             (vPathEnd == vPlusMinus && vPathStart == vPlusSuper)) &&
            nDeltaCharge < 0) {
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          (*pnNumRunBNS)++;
          if (ret < 0) {
    ...
    if (num_min > 1 && (!nDeltaPlus1 && !nDeltaMinus1)) {
      ...
      for (i = 0; i < num_min; i += delta) {
        ...
        pvPlusSuper->st_edge.cap -= delta;
        pBNS->tot_st_cap -= delta;
        if (ret != 1 || (vPathEnd != vPlusSuper || vPathStart != vPlusSuper) ||
            nDeltaCharge >= 0) {
          break;
        }
        ...
        pvPlusMinus->st_edge.cap -= delta;
        pBNS->tot_st_cap -= delta;
        if (ret != 1 || (vPathEnd != vPlusMinus || vPathStart != vPlusMinus) ||
            nDeltaCharge >= 0) {
          break;
        }
...
exit_function:
  ...
  if (forbidden_stereo_edge_mask && FixedLargeRingStereoEdges.num_edges) {
    SetForbiddenEdgeMask(pBNS, &FixedLargeRingStereoEdges,
                         forbidden_stereo_edge_mask);
  }
...
/********************** not used
 * *************************************************************************/
int MoveChargeFromHeteroatomsToMetals(BN_STRUCT *pBNS, BN_DATA *pBD,
                                      StrFromINChI *pStruct, inp_ATOM *at,
                                      inp_ATOM *at2, VAL_AT *pVA,
                                      ALL_TC_GROUPS *pTCGroups,
                                      int *pnNumRunBNS, int *pnTotalDelta,
...
/********* Avoid charge separation on heteroatoms ******************/
{
  ...
  int ePlusSuper,
      eMinusSuper; /* edges from vPlusSuper or vMinusSuper to (+/-) */
  int vPlMn;       /* (+/-) vertex */
  int vPlusHeteroat, vMinusHeteroat; /* (+), (-) heteroatom vertices */
  int ePlusHeteroat, eMinusHeteroat; /* edges from (+) or (-) heteroatom vertex
                                        to super (+) or (-) */
  int vPlusCarbons, vMinusCarbons;   /* (+), (-) carbons vertices */
  int ePlusCarbons, eMinusCarbons;   /* edges from (+), (-) carbons vertices to
                                        super (+) or (-) */
  int vPlusMetals, vMinusMetals;     /* (+), (-) carbons vertices */
  int ePlusMetals, eMinusMetals;     /* edges from (+), (-) carbons vertices to
                                        super (+) or (-) */
  int eMinusHeteroToSuper;           /* edge
                                        (-)vHetero-[eMinusHeteroat]-Y-[eMinusHeteroToSuper]-(-)vPlusSuper
                                      */
  int v1, v2;
  int nDeltaPlus1, nDeltaMinus1, delta;
  ...
  BNS_EDGE *pEdgePlusCarbons, *pEdgeMinusCarbons, *pEdgePlusMetals,
      *pEdgeMinusMetals;
  ...
  if (pEdgePlusHeteroat && pEdgePlusMetals) {
    /* count charges */
    for (i = 0; i < num_at; i++) {
      if (!at2[i].radical && at2[i].charge > 0 &&
...
int RestoreCyanoGroup(BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
                      inp_ATOM *at, inp_ATOM *at2, VAL_AT *pVA,
                      ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                      int *pnTotalDelta, int forbidden_edge_mask) {
  Vertex vPathStart, vPathEnd;
  int nPathLen, nDeltaH, nDeltaCharge, nNumVisitedAtoms;
  ...
  for (i = 0; i < num_at && 0 <= ret; i++) {
    if (at2[i].valence == 1 && at2[i].num_H == 0 &&
        at2[i].chem_bonds_valence == 2 && at2[i].charge == -1 &&
        at2[i].radical == 0 &&
        pVA[i].cNumValenceElectrons == 5 && /* terminal N(-)=, P, As, Sb, Bi */
        pVA[i].nCMinusGroupEdge > 0 && pVA[i].nTautGroupEdge == 0 &&
        at2[j = at2[i].neighbor[0]].valence == 2 && at2[j].num_H == 0 &&
        at2[j].chem_bonds_valence == 4 && at2[j].charge == 0 &&
        at2[j].radical == 0 &&
      ...
      /* found N(-)=C= */
      pe = pBNS->edge +
           (pVA[i].nCMinusGroupEdge - 1); /* N#N(+) triple bond edge */
      ...
      /* do not let carbon atoms get charged */
      if (0 > (ret = ForbidCarbonChargeEdges(
                   pBNS, pTCGroups, &CarbonChargeEdges, forbidden_edge_mask))) {
        goto exit_function;
      }
...
#define INC_EDGE_LIST 16
  Vertex vPathStart, vPathEnd;
  int nPathLen, nDeltaH, nDeltaCharge, nNumVisitedAtoms, num_failed,
      num_success;
  BNS_EDGE *pe;
  Vertex v1, v2;
  ...
  AllocEdgeList(&IsoCyanoCarbonChargeEdges,
                EDGE_LIST_CLEAR); /* C in C(+)#N(+) charge edges */
  ...
  /* 1st attempt: take care of C(+)#N(+)-  => C(-)#N(+)- and remove 2 negative
   * charges */
  ...
  for (i = 0; i < num_at && 0 <= ret; i++) {
    ...
    bIsCarbon =
        (pVA[i].cNumValenceElectrons == 4 && pVA[i].cPeriodicRowNumber == 1);
    eNFlowerEdge1 = NO_VERTEX;
    if ((eNMinusEdge = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
        !pBNS->edge[eNMinusEdge].forbidden) {
      if (bIsCarbon) {
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNMinusEdge,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
      } else if (!pVA[i].cMetal && !at2[i].endpoint && at2[i].charge != -1) {
        if ((ret =
                 AddToEdgeList(&AllChargeEdges, eNMinusEdge, INC_EDGE_LIST))) {
          goto exit_function;
        }
      }
    }
    if ((eNPlusEdge = pVA[i].nCPlusGroupEdge - 1) >= 0 &&
      ...
      if (bIsCarbon) {
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNPlusEdge,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
        ...
        if (pVA[i].cNumValenceElectrons == 5 &&
            NO_VERTEX != (eNFlowerEdge1 = GetChargeFlowerUpperEdge(
                              pBNS, pVA, eNPlusEdge)) &&
            !pBNS->edge[eNFlowerEdge1].flow) {
          if ((ret = AddToEdgeList(&AllChargeEdges, eNFlowerEdge1,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
        }
    ...
    if (bIsCarbon && 0 <= eNMinusEdge && 0 <= eNPlusEdge &&
        at2[i].valence == 1 && at2[i].num_H == 0 && at2[i].radical == 0 &&
        !pBNS->edge[eNMinusEdge].forbidden &&
        ...
        at2[j = at2[i].neighbor[0]].valence == 2 && at2[j].num_H == 0 &&
        at2[j].radical == 0 && pVA[j].cNumValenceElectrons == 5 &&
      ...
      if (NO_VERTEX != (eNFlowerEdge1 =
                            GetChargeFlowerUpperEdge(pBNS, pVA, eNPlusEdge1)) &&
          !pBNS->edge[eNFlowerEdge1].flow) {
        if (ret = AddToEdgeList(&CarbonChargeEdges, eNFlowerEdge1,
                                INC_EDGE_LIST)) {
          goto exit_function;
        }
      }
      ...
      */
      ...
      v1 = pBNS->edge[eNPlusEdge]
               .neighbor1; /* one of two vertices incident with edge 4-5 */
      ...
      if (!IS_BNS_VT_CHRG_STRUCT(pBNS->vert[v1].type) ||
          pBNS->vert[v1].num_adj_edges != 2) {
        continue; /* mismatch */
      }
      /* find edge 3-4 */
      ...
      if ((ret = AddToEdgeList(&IsoCyanoCarbonChargeEdges, eNMinusEdge,
                               INC_EDGE_LIST))) {
        goto exit_function;
      }
      if ((ret = AddToEdgeList(&IsoCyanoCarbonChargeEdges, eNPlusEdge,
                               INC_EDGE_LIST))) {
        goto exit_function;
      }
      if ((ret = AddToEdgeList(&IsoCyanoCarbonChargeEdges, eN34Edge,
                               INC_EDGE_LIST))) {
        goto exit_function;
      }
    }
  ...
  SetForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
  RemoveForbiddenEdgeMask(pBNS, &IsoCyanoCarbonChargeEdges,
                          forbidden_edge_mask);
    ...
    for (i = IsoCyanoCarbonChargeEdges.num_edges - 3; 0 <= i; i -= 3) {
      ...
      if (ret == 1 &&
        ...
        *pnTotalDelta += ret;
        ...
        pBNS->tot_st_flow += 2;
        pe->forbidden &=
            inv_forbidden_edge_mask; /* let it change if it wants */
        num_failed++;
      }
    }
  }
...
#define INC_EDGE_LIST 16
  ...
  for (i = 0; i < num_at && 0 <= ret; i++) {
    if (at2[i].valence == 1 && at2[i].num_H == 0 && at2[i].radical == 0 &&
        pVA[i].cNumValenceElectrons == 6 && /* terminal -O */
        (eNMinusEdge = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
        pBNS->edge[eNMinusEdge].flow == 1 &&
        !pBNS->edge[eNMinusEdge].forbidden && /* terminal O(-) */

        at2[j = at2[i].neighbor[0]].valence == 2 && at2[j].num_H == 0 &&
        at2[j].radical == 0 && pVA[j].cNumValenceElectrons == 5 &&
        (eNMinusEdge1 = pVA[j].nCMinusGroupEdge - 1) >= 0 &&
        pBNS->edge[eNMinusEdge1].flow == 1 &&
      ...
      nMetalCharge =
          (pBNS->edge[eNPlusEdge2].cap - pBNS->edge[eNPlusEdge2].flow) -
          pBNS->edge[eNMinusEdge2].flow;
      if (nMetalCharge == 0) {
        /* change on O is invisible; charge from N(-) goes, charge comes to
         * Metal */
        nDeltaChargeMax = 0;
      ...
      pe =
          pBNS->edge + eNMinusEdge; /* must be already fixed as a charge edge */
      ...
      if (ret == 1 &&
          ((vPathEnd == v1 && vPathStart == v2) ||
           (vPathEnd == v2 &&
            vPathStart == v1)) /*&& nDeltaCharge == nDeltaChargeMax*/) {
        ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
        (*pnNumRunBNS)++;
        *pnTotalDelta += ret;
        num_success++;
...
#define INC_EDGE_LIST 16
  ...
  int len_at = num_at + num_deleted_H;
  int inv_forbidden_edge_mask = ~forbidden_edge_mask;
  EdgeIndex eNMinusEdge, eNPlusEdge, eNMinusEdge1, eNPlusEdge1, eNMinusEdge2,
      eNPlusEdge2;
  EdgeIndex eNFlowerEdge1, eNFlowerEdge2;

  EDGE_LIST CarbonChargeEdges, AllChargeEdges, NNNChargeEdges,
      CurNNNChargeEdges, AllNNNTermAtoms, AllNIIIChargeEdges;
  ...
  /* attepmt #1 N#N(+)-N => N(-)=N(+)=N: naive approach; expecting tp move (-)
   * from some other atom */
  for (i = 0; i < num_at && 0 <= ret; i++) {
    if (at2[i].valence == 1 && at2[i].num_H == 0 &&
        at2[i].chem_bonds_valence == 3 && at2[i].charge == 0 &&
        at2[i].radical == 0 &&
        ...
        !pBNS->edge[eNMinusEdge].forbidden &&
        at2[j = at2[i].neighbor[0]].valence == 2 && at2[j].num_H == 0 &&
        at2[j].chem_bonds_valence == 4 && at2[j].charge == 1 &&
        at2[j].radical == 0 && pVA[j].cNumValenceElectrons == 5 &&
        (eNPlusEdge = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
        pBNS->edge[eNPlusEdge].flow == 0 && !pBNS->edge[eNPlusEdge].forbidden &&
        at2[k = at2[j].neighbor[at2[j].neighbor[0] == i]].valence == 2 &&
        at2[k].num_H == 0 && at2[k].chem_bonds_valence == 3 &&
        pVA[k].cNumValenceElectrons == 5 &&
        (eNPlusEdge2 = pVA[k].nCPlusGroupEdge - 1) >= 0 &&
        pBNS->edge[eNPlusEdge2].flow == 1 &&
        !pBNS->edge[eNPlusEdge2].forbidden && pVA[i].cnListIndex > 0 &&
        cnList[pVA[i].cnListIndex - 1].bits == cn_bits_MN) {
  ...
  for (i = 0; i < num_at && 0 <= ret; i++) {
    ...
    if ((eNPlusEdge = pVA[i].nCPlusGroupEdge - 1) >= 0 &&
      ...
      if (pVA[i].cNumValenceElectrons == 5 && at2[i].valence == 3 &&
        ...
        if ((ret = AddToEdgeList(&AllNIIIChargeEdges, eNPlusEdge,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
      }
      ...
      if (pVA[i].cNumValenceElectrons == 5 && pVA[i].cPeriodicRowNumber == 1 &&
          NO_VERTEX != (eNFlowerEdge1 =
                            GetChargeFlowerUpperEdge(pBNS, pVA, eNPlusEdge)) &&
          pBNS->edge[eNFlowerEdge1].flow == 0 &&
          (ret =
               AddToEdgeList(&AllChargeEdges, eNFlowerEdge1, INC_EDGE_LIST))) {
        goto exit_function;
      }
    } else {
      eNPlusEdge = -1;
    ...
    if (0 <= eNMinusEdge && at2[i].valence == 1 && at2[i].num_H == 0 &&
        at2[i].radical == 0 &&
        pVA[i].cNumValenceElectrons == 5 && /* terminal N# */
      ...
      if (pBNS->edge[eNMinusEdge].flow == 1 && /* N(-) */
          pBNS->edge[eNMinusEdge1].flow == 0 &&
          pBNS->edge[eNPlusEdge1].flow == 0 && /* N(+) */
          pBNS->edge[eNMinusEdge2].flow == 0 &&
          pBNS->edge[eNPlusEdge2].flow == 1 /* N */) {
      ...
      if (pBNS->edge[eNMinusEdge].flow == 0 && /* N */
        ...
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNMinusEdge, INC_EDGE_LIST))) {
          goto exit_function;
        }
        continue;
      }
      ...
      if (pBNS->edge[eNMinusEdge].flow == 1 && /* N(-) */
          pBNS->edge[eNMinusEdge1].flow == 0 &&
          pBNS->edge[eNPlusEdge1].flow == 1 && /* N */
          pBNS->edge[eNMinusEdge2].flow == 0 &&
          pBNS->edge[eNPlusEdge2].flow == 1 /* N */) {
        /* unfix (+) edge on middle N */
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNPlusEdge1, INC_EDGE_LIST))) {
          goto exit_function;
        }
        continue;
      }
      ...
      if (pBNS->edge[eNMinusEdge].flow == 0 && /* N */
        ...
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNMinusEdge2, INC_EDGE_LIST))) {
          goto exit_function;
        }
        continue;
      }
      ...
      if (pBNS->edge[eNMinusEdge].flow == 0 && /* N */
          ...
          pBNS->edge[eNPlusEdge1].flow == 0 && /* N(+) */
          pBNS->edge[eNMinusEdge2].flow == 0 &&
          pBNS->edge[eNPlusEdge2].flow == 0 /* N(+) */) {
        ...
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNPlusEdge2, INC_EDGE_LIST))) {
          goto exit_function;
        }
        continue;
      }
  ...
  for (i = 0; i < num_at && 0 <= ret; i++) {
    ...
    if (0 <= eNMinusEdge && at2[i].valence == 1 && at2[i].num_H == 0 &&
      ...
      if (pBNS->edge[eNMinusEdge].flow == 1 && /* N(-) */
        ...
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNPlusEdge1,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNMinusEdge1,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNPlusEdge2,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
        if ((ret = AddToEdgeList(&CarbonChargeEdges, eNMinusEdge2,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
        continue; /* already good */
      ...
      if (pBNS->edge[eNMinusEdge].flow == 0 && /* N */
        ...
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNMinusEdge1, INC_EDGE_LIST))) {
          goto exit_function;
        }
        if ((ret =
                 AddToEdgeList(&NNNChargeEdges, eNPlusEdge2, INC_EDGE_LIST))) {
        ...
        pe = pBNS->edge + pBNS->vert[i].iedge[0];
        nDeltaChargeMax = 0;
        nDeltaChargeMax = (num_failed && !num_success &&
        ...
        if (pBNS->edge[eNMinusEdge].flow == 1 && /* N(-) */
            ...
            pBNS->edge[eNPlusEdge2].flow == 1 /* N */) {
          /* unfix (+) edge on middle N */
          if ((ret = AddToEdgeList(&NNNChargeEdges, eNMinusEdge,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
          if ((ret = AddToEdgeList(&NNNChargeEdges, eNMinusEdge1,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
          if ((ret = AddToEdgeList(&NNNChargeEdges, eNPlusEdge2,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
          if ((ret = AddToEdgeList(&NNNChargeEdges, eNMinusEdge2,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
          if (NO_VERTEX != eNFlowerEdge1 &&
              (ret = AddToEdgeList(&NNNChargeEdges, eNFlowerEdge1,
                                   INC_EDGE_LIST))) {
            goto exit_function;
          }
          /* decrement flow on (+) charge edge of the middle =N- */
          ...
          if (pBNS->edge[eNMinusEdge].flow == 0 && /* N */
              ...
              pBNS->edge[eNPlusEdge2].flow == 1 /* N(-) */) {
            /* unfix (-) edge on the 1st and 3rd N */
            if ((ret = AddToEdgeList(&NNNChargeEdges, eNPlusEdge1,
                                     INC_EDGE_LIST))) {
              goto exit_function;
            }
            if ((ret = AddToEdgeList(&NNNChargeEdges, eNMinusEdge1,
      ...
      if (NO_VERTEX != eNFlowerEdge1 && !pBNS->edge[eNFlowerEdge1].flow) {
        if ((ret = AddToEdgeList(&NNNChargeEdges, eNFlowerEdge1,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
      }
      if (NO_VERTEX != eNFlowerEdge2 && !pBNS->edge[eNFlowerEdge2].flow) {
        if ((ret = AddToEdgeList(&NNNChargeEdges, eNFlowerEdge2,
                                 INC_EDGE_LIST))) {
          goto exit_function;
        }
      }
      ...
      RemoveForbiddenEdgeMask(pBNS, &NNNChargeEdges, forbidden_edge_mask);
      pe->forbidden &= inv_forbidden_edge_mask;
      /*pBNS->edge[eNPlusEdge].forbidden &= inv_forbidden_edge_mask;*/ /* BC:
                                                                          array
                                                                          index
                                                                          out of
                                                                          range
...
/******************************************************************************************************/
int EliminateNitrogen5Val3Bonds(BN_STRUCT *pBNS, BN_DATA *pBD,
                                StrFromINChI *pStruct, inp_ATOM *at,
                                inp_ATOM *at2, VAL_AT *pVA,
                                ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  for (i = 0; i < num_at; i++) {
    if (pVA[i].cNumValenceElectrons == 5 &&
        0 <= (k = GetChargeFlowerUpperEdge(pBNS, pVA,
                                           pVA[i].nCPlusGroupEdge - 1)) &&
        1 == pBNS->edge[k].flow) {
  ...
  /*------------------------------------------------------------------------------
               (+)  single line => flow = 0 (+)-(Y)=(+)super 01  //    double
  line => flow = 1                fix-> 01   // <-- fix 1 --- 0 1 === 0
        \\ //         edge eij connects vertices i < j:         \   /  02
      12  2   02 <--- edge number: e02 connects vertices v0   12  2(..) <-
  double 'radical' |                        v0 and v2                      | =N=
  vertex N has number i                          =N= | |
  --------------------------------------------------------------------------------*/
  for (i = 0; i < num_at; i++) {
    if (pVA[i].cNumValenceElectrons == 5 && at2[i].valence == 3 &&
        at2[i].chem_bonds_valence == 5 && !at2[i].charge && !at2[i].radical &&
        !(at2[i].endpoint || (pStruct->endpoint && pStruct->endpoint[i])) &&
        pVA[i].cnListIndex > 0 &&
      ...
      v0 = IS_BNS_VT_C_GR(pBNS->vert[pePlus->neighbor1].type)
               ? (pePlus->neighbor1 ^ pePlus->neighbor12)
               : pePlus->neighbor1;
      pv0 = pBNS->vert + v0;
      ...
      ret = RunBnsTestOnce(pBNS, pBD, pVA, &vPathStart, &vPathEnd, &nPathLen,
                           &nDeltaH, &nDeltaCharge, &nNumVisitedAtoms);
      if (ret == 1 && vPathEnd == v2 && vPathStart == v2 &&
...
exit_function:
  ...
  for (i = 0; i < num_at; i++) {
    if (pVA[i].cNumValenceElectrons == 5 &&
      ...
      for (j = 0; j < at2[i].valence; j++) {
        if (BOND_TYPE_SINGLE == (at2[i].bond_type[j] & BOND_TYPE_MASK)) {
          pBNS->edge[pBNS->vert[i].iedge[j]].forbidden &=
...
int Convert_SIV_to_SVI(BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
  ...
  for (i = 0; i < num_at; i++) {
    if ((pVA[i].cNumValenceElectrons == 5 /* N(IV)*/ ||
         pVA[i].cNumValenceElectrons == 6 /* S(VI)*/) &&
        0 <= (k = GetChargeFlowerUpperEdge(pBNS, pVA,
                                           pVA[i].nCPlusGroupEdge - 1)) &&
      ...
      for (j = 0; j < at2[i].valence; j++) {
        if (BOND_TYPE_SINGLE == (at2[i].bond_type[j] & BOND_TYPE_MASK)) {
          ...
          pBNS->edge[k = pBNS->vert[i].iedge[j]].forbidden |=
              forbidden_edge_mask;
          if ((ret = AddToEdgeList(&FlowerEdgesList, k, 64))) {
            goto exit_function;
          }
      ...
      /* fix bonds to neighbors of S(IV) if they are not O,S,Se,Te with 2 or
       * more bonds */
      ...
      if (pVA[i].cNumValenceElectrons == 6 && at2[i].valence == 4 &&
          ...
          cnList[pVA[i].cnListIndex - 1].bits == cn_bits_NPN &&
          0 <= (nFlowerEdge = GetChargeFlowerUpperEdge(
                    pBNS, pVA, pVA[i].nCPlusGroupEdge - 1)) &&
  ...
  /*-------------------------------------------------------------------------------
   found:                                       super(+)=(Y)        super(+)=(Y)
                                                            \                   \
           (+)  single line => flow = 0                     (+) (+) 01  //
  double line => flow = 1         fix-> 01   //             01   // 1 === 0
  triple line => flow = 2          (.)1 --- 0(.)  --->    1 --- 0 \   / edge eij
  connects vertices i<j:      \   /  02  run      \\ //  02 12  2   02 <--- edge
  number: e02 connects        12  2        BFS    12  2
     |||              vertices v0 and v2              |||                  |
...
*******************************************************************************************************/
int PlusFromDB_N_DB_O_to_Metal(BN_STRUCT *pBNS, BN_DATA *pBD,
                               StrFromINChI *pStruct, inp_ATOM *at,
                               inp_ATOM *at2, VAL_AT *pVA,
                               ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                               int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  AllocEdgeList(&NO_ChargeEdgeList,
                EDGE_LIST_CLEAR);               /* charges to be changed */
  ...
  for (i = 0; i < num_at; i++) {
    ...
    if ( pVA[i].cMetal ) {
        ...
        if ( (k = pVA[i].nCMinusGroupEdge-1) >= 0 && !pBNS->edge[k].forbidden )
    { if ( ret = AddToEdgeList( &NO_ChargeEdgeList, k, 64 ) ) { goto
    exit_function;
            }
        }
    } else
    */
    if (!pVA[i].cMetal && pVA[i].cNumValenceElectrons == 6 &&
        at2[i].charge == 0 && !at2[i].num_H && 1 == at2[i].valence &&
        2 == at2[i].chem_bonds_valence &&
        pVA[j = at2[i].neighbor[0]].cNumValenceElectrons == 5 &&
        at2[j].charge == 1 && !at2[j].num_H && 2 == at2[j].valence &&
        4 == at2[j].chem_bonds_valence) {
      /* found =N(+)=O */
      if ((k = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[k].forbidden /* O */ &&
          (n = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
          !pBNS->edge[j].forbidden /* N */) {
        if ((ret = AddToEdgeList(&NO_ChargeEdgeList, k, 64)) ||
...
/******************************************************************************************************/
int MoveMobileHToAvoidFixedBonds(BN_STRUCT *pBNS, BN_DATA *pBD,
                                 StrFromINChI *pStruct, inp_ATOM *at,
                                 inp_ATOM *at2, VAL_AT *pVA,
                                 ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                 int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  if (pTCGroups->num_tgroups) {
    ...
    nNumFixedEdges = ret2;
    ret = AdjustTgroupsToForbiddenEdges2(pBNS, at2, pVA, num_at,
                                         forbidden_edge_mask);
    nNumAdjEdges = ret;
    if (ret) {
      pBNS->edge_forbidden_mask |= forbidden_edge_mask;
...
/******************************************************************************************************/
/* Find and eliminate cases when Mobile H endpoint has radical on it (typical
 * for wrong P(VI)(=O)3OH  */
int RemoveRadFromMobileHEndpoint(BN_STRUCT *pBNS, BN_DATA *pBD,
                                 StrFromINChI *pStruct, inp_ATOM *at,
                                 inp_ATOM *at2, VAL_AT *pVA,
                                 ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                 int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  BNS_VERTEX *ptg1, *pEndp0 = NULL, *pEndp1, *pEndp2, *pCentp, *pCentp_found,
                    *pEndp2_found = NULL;
  BNS_EDGE *etg0 = NULL, *etg1, *etg2, *ecp0, *ecp1, *ecp2;
  BNS_EDGE *etg1_found = NULL, *ecp0_found = NULL, *ecp1_found = NULL,
           *ecp2_found = NULL;
  ...
  while (pBNS->tot_st_cap > pBNS->tot_st_flow && pTCGroups->num_tgroups) {
    ...
    for (itg = 0; itg < pTCGroups->num_tgroups; itg++) {
      ...
      for (i = 0; i < num_endpoints; i++) {
        ...
        pEndp0 =
            pBNS->vert +
            endpoint0; /* taut endpoint vertex (possible location of mobile H */
      ...
      if (i < num_endpoints) {
        ...
        for (j = 0; j < num_endpoints; j++) {
          ...
          etg1 = pBNS->edge + ptg1->iedge[j]; /* another edge from t-group to
                                                 another endpoinr */
          endpoint1 =
              etg1->neighbor12 ^ vtg1;     /* another endpoint vertex index */
          ...
          /* traverse endpoint1 edges to find a single bond connecting it to the
           * centerpoint */
          for (k = 0; k < at2[endpoint1].valence; k++) {
            ...
            /* traverse centerpoint edges to find a single bond to the 2nd
             * endpoint */
            for (n = 0; n < at2[centerpoint].valence; n++) {
              ...
              if (endpoint2 <= endpoint1 || !pVA[endpoint2].nTautGroupEdge) {
                continue; /* don't go back: neighbors are in order of ascending
                             ord. numbers */
              }
              ...
              if (!pCentp_found ||
                  ...
                  ((pVA[centerpoint].cNumValenceElectrons != 4 ||
                   ...
                   (at[centerpoint].valence > at[centerpoint_found].valence ||
                    (at[centerpoint].valence == at[centerpoint_found].valence &&
                     at[centerpoint].el_number >
                         at[centerpoint_found].el_number)))) {
      ...
      if (i < num_endpoints) {
        ...
        for (j = 0; j < num_endpoints; j++) {
          ...
          /* traverse endpoint1 edges to find the edge connecting it to the
           * centerpoint */
          for (k = 0; k < at2[endpoint1].valence; k++) {
            ecp1 = pBNS->edge + pEndp1->iedge[k];
            if (ecp1->flow) {
            ...
            if (pCentp->type & BNS_VERT_TYPE_ENDPOINT) {
              continue; /* do not set another endpoint's valence = an unusual
                           value */
            }

            /* traverse centerpoint edges to find edge connecting it to the
             * endpoint0 */
            ecp2 = NULL;
            pEndp2 = NULL;
            for (n = 0; n < at2[centerpoint].valence; n++) {
              ...
              if (ecp0->flow) {
                ...
                /* check whether ecp0 is stereogenic: if it is then we cannot
                 * decrement its flow */
                for (m = 0;
                     m < MAX_NUM_STEREO_BONDS && at[centerpoint].sb_parity[m];
                     m++) {
      ...
      if (i < num_endpoints) {
        ...
        for (jj = 0; jj < num_at && bNotFixed; jj++) {
          ...
          if (2 == at2[jj].valence &&
              pBNS->vert[jj].st_edge.cap == pBNS->vert[jj].st_edge.flow &&
            ...
            ret2 = RunBnsTestOnce(pBNS, pBD, pVA, &vPathStart, &vPathEnd,
                                  &nPathLen, &nDeltaH, &nDeltaCharge,
                                  &nNumVisitedAtoms);
...
/* Find and eliminate cases when Mobile H endpoint has radical on it (typical
 * for wrong P(VI)(=O)3OH  */
int RemoveRadFromMobileHEndpointFixH(BN_STRUCT *pBNS, BN_DATA *pBD,
                                     StrFromINChI *pStruct, inp_ATOM *at,
                                     inp_ATOM *at2, VAL_AT *pVA,
                                     ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                     int *pnTotalDelta,
                                     int forbidden_edge_mask) {
#define IS_C(x)                                                                \
  (NO_VERTEX != x && pVA[x].cNumValenceElectrons == 4 &&                       \
   pVA[x].cPeriodicRowNumber == 1)
  int i, num_fixes, tot_num_fixes = 0;
  ...
  BNS_VERTEX *pEndp0 = NULL, *pEndp1, *pEndp2, *pCentp, *pCentp_found,
             *pEndp2_found = NULL;
  BNS_EDGE *ecp0, *ecp1, *ecp2, *ecp0_found = NULL, *ecp1_found = NULL,
                                *ecp2_found = NULL;
  int tgroup_number, num_endpoints;
  ...
  while (pBNS->tot_st_cap > pBNS->tot_st_flow && pStruct->ti.num_t_groups) {
    int iEndpoint = 0;
    num_fixes = 0;
    for (itg = 0; itg < pStruct->ti.num_t_groups;
         iEndpoint += num_endpoints, itg++) {
      pCentp_found = NULL;
      ...
      /* decrement st_flow, st_cap on Endp2; decrement flow on ecp2; decrement
       * st_flow on Centp */
      ...
      /*----------------------------------------------------------------------------------------
        ...
        ==============           u,f=>unfixed, fixed edges                    (N
       electrons)%2
                  (-)                       (-)                      (-) | e0/
       \\ e1          =>     u/  \\u            =>     //  \         v
                /    \\ ecp1 ecp2         /    \\   u    f         //    \
            C--X*     Y(-)--C==Z      C--X*     Y(-)--C*--Z      --X(-)
       Y===C---Z* rad.    endp     not      rad.   endp  rad not      rad. endp
       not Endp0   Endp1     endp     endp         to  endp     endp endp Endp2
       cancel
      ...
      if (i < num_endpoints) {
        int e0, e1;
        if ((e0 = pVA[endpoint0].nCMinusGroupEdge - 1) < 0 ||
            pBNS->edge[e0].forbidden) {
        ...
        for (j = 0; j < pStruct->ti.nNumEndpoints; j++) {
          ...
          if (((e1 = pVA[endpoint1].nCMinusGroupEdge - 1) < 0 ||
               !pBNS->edge[e1].flow) ||
    ...
    for (itg = 0; itg < pStruct->ti.num_t_groups;
      ...
      if (i < num_endpoints) {
        ...
        for (j = 0; j < pEndp0->num_adj_edges; j++) {
          ...
          if (centerpoint >= pBNS->num_atoms || ecp0->flow ||
              pStruct->endpoint[centerpoint]) {
            continue; /* ignore non-single bonds, orig. InChI endpoints, and
                         fictitious atoms */
          }
          pCentp = pBNS->vert + centerpoint;
          for (k = 0; k < pCentp->num_adj_edges; k++) {
            ...
            if (endpoint1 >= pBNS->num_atoms || !ecp1->flow ||
                pStruct->endpoint[endpoint1]) {
              continue; /* ignore single bonds, orig. InChI endpoints, and
                           fictitious atoms */
            }
            pEndp1 = pBNS->vert + endpoint1;
            if (endpoint1 == endpoint0 ||
                pEndp1->st_edge.cap != pEndp1->st_edge.flow) {
...
int MoveChargeToMakeCenerpoints(BN_STRUCT *pBNS, BN_DATA *pBD,
                                StrFromINChI *pStruct, inp_ATOM *at,
                                inp_ATOM *at2, VAL_AT *pVA,
                                ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  for (i = 0; i < num_at; i++) {
    if (pVA[i].cNumValenceElectrons != 4 && /* not C, Si, Ge */
        !pVA[i].cMetal && !pVA[i].nTautGroupEdge && !at2[i].num_H &&
        at2[i].valence >= 3 && at2[i].valence == at2[i].chem_bonds_valence &&
      ...
      if (j == at2[i].valence && num_endpoints > 1) {
        ...
        pEdgeMinus = (pVA[i].nCMinusGroupEdge > 0)
                         ? pBNS->edge + (pVA[i].nCMinusGroupEdge - 1)
                         : NULL;
        ...
        if ((delta = pEdgePlus->flow)) {
          ...
          ret =
              RunBnsTestOnce(pBNS, pBD, pVA, &vPathStart, &vPathEnd, &nPathLen,
                             &nDeltaH, &nDeltaCharge, &nNumVisitedAtoms);
          if (ret < 0) {
            goto exit_function;
          }
          if (ret == 1 &&
              ((vPathEnd == v1p && vPathStart == v2p) ||
        ...
        } else if (pEdgeMinus && (delta == pEdgeMinus->flow) &&
                   pEdgePlus->flow == 0) {
