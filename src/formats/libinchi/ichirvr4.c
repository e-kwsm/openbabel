 ...
 * ***********************************/
int ForbidCarbonChargeEdges(BN_STRUCT *pBNS, ALL_TC_GROUPS *pTCGroups,
...
/******************************************************************************************************/
int ForbidNintrogenPlus2BondsInSmallRings(BN_STRUCT *pBNS, inp_ATOM *at,
                                          int num_at, VAL_AT *pVA,
                                          int min_ring_size,
...
/*************************************************************************************************
Problem: Formula in InChI from the reversed structure has less H than in the
input InChI Solutions:

(a)   |                        |
     -B(-)-NH-=..-=N(+)<   => -B(-)-NH(+)=-..=-N<  (H is not removed from the
ion pair) |                        |

                  |                      |
(b)  >N(+)=-=...-=N-NH     =>  >N-=-...=-N(+)-NH  (charge from onium cannot be
moved to remove H+) |                      |
*************************************************************************************************/
int FixLessHydrogenInFormula(BN_STRUCT *pBNS, BN_DATA *pBD,
                             StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2,
                             inp_ATOM *atf, VAL_AT *pVA,
  ...
  if (0 <= iNH && iNH < num_at) {
    ...
    do {
      ...
      for (j = 0; j < at2[i].valence; j++) {
        bond_type_at2 =
            at2[i].bond_type[j] & BOND_TYPE_MASK; /* restored bond */
        bond_type_atf =
            atf[i].bond_type[j] & BOND_TYPE_MASK; /* normalized bond */
        nxt_is_NPlus = 0;
        if ((bond_type_atf - bond_type_at2 == delta ||
             bond_type_atf == BOND_ALT12NS) &&
            BOND_TYPE_SINGLE <= bond_type_at2 + delta &&
            bond_type_at2 + delta <= BOND_TYPE_TRIPLE &&
            !at2[(int)at2[i].neighbor[j]].cFlags) {
          prv = i;
          nxt = at2[i].neighbor[j];
          nxt_is_NPlus = at2[nxt].charge == 1 && atf[nxt].charge == 0 &&
                         pVA[nxt].cNumValenceElectrons == 5 &&
                         pVA[nxt].cPeriodicRowNumber == 1;
          at2[i].cFlags |= 1; /* avoid cycling */
          num_bonds_in_path++;
          if (delta == -1 && at2[prv].valence == 4 &&
              at2[prv].chem_bonds_valence == 5 && !at2[prv].charge &&
              !at2[prv].radical && pVA[prv].cNumValenceElectrons == 5 &&
          ...
          if (at2[nxt].charge != atf[nxt].charge) {
            if ((at2[nxt].charge == 1 || atf[nxt].charge == 1) &&
              ...
              pBNS->edge[pVA[nxt].nCPlusGroupEdge - 1].forbidden &=
                  inv_forbidden_edge_mask;
            }
            if ((at2[nxt].charge == -1 || atf[nxt].charge == -1) &&
                pVA[nxt].nCMinusGroupEdge > 0) {
              pBNS->edge[pVA[nxt].nCMinusGroupEdge - 1].forbidden &=
                  inv_forbidden_edge_mask;
            }
          }
          break; /* found */
    ...
    if (nxt >= 0 && nxt_is_NPlus && delta == -1) {
      ...
      if (iBPlus || iNV) {
        ...
        if ((iNV >= 0 && (j = pVA[iNV].nCPlusGroupEdge - 1) > 0 &&
             pBNS->edge[j].flow > 0) ||
            (iNH >= 0 && (j = pVA[iNH].nCPlusGroupEdge - 1) > 0 &&
             pBNS->edge[j].flow > 0)) {
          int ieFlower;
          ...
          ieFlower =
              GetChargeFlowerUpperEdge(pBNS, pVA, pVA[nxt].nCPlusGroupEdge - 1);
          if (ieFlower >= 0) {
...
************************************************************************************************/
int FixMoreHydrogenInFormula(BN_STRUCT *pBNS, BN_DATA *pBD,
                             StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2,
                             inp_ATOM *atf, VAL_AT *pVA,
                             ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                             int *pnTotalDelta, int forbidden_edge_mask) {
  ...
  for (i = 0; i < num_at; i++) {
    if (!(pStruct->bMobileH ? at2[i].endpoint : pStruct->endpoint[i]) &&
        !pVA[i].cMetal &&
        at2[i].num_H + 1 == atf[i].num_H && /* normalization added H ??? What
                                               would happen in Fixed-H case?*/
        (k = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
        pBNS->edge[k].flow ==
            1 && /* atom had (-) charge before preprocessing */
        at2[i].charge == -1 &&
        atf[i].charge == 0 && /* and has no charge after preprocessing */
        at2[i].valence == 1 &&
        at2[i].chem_bonds_valence == 1 &&   /* connected by a single bond */
      ...
      /* H(+) was added to O in Y=X-O(-), where X is the only neighbor of O,
       * X=neigh, Y=neigh2 */
      iNH = i;
      ...
      if (j < at2[neigh].valence) {
        ...
        pBNS->tot_st_flow -= 2 * delta;
        pe2->forbidden &=
            inv_forbidden_edge_mask; /* allow the charge to move */
...
/******************************************************************************************************/
int FixAddProtonForADP(BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
                       inp_ATOM *at, inp_ATOM *at2, inp_ATOM *atf, VAL_AT *pVA,
...
/******************************************************************************************************
      OH              OH
     /               /
  -NH      =>   -NH(+)        to eliminate false tautomerism. S(IV) or N(V) or
P(V) may be a centerpoint
     \\              \
      O               O(-)
*******************************************************************************************************/
int FixRemoveExtraTautEndpoints(BN_STRUCT *pBNS, BN_DATA *pBD,
                                StrFromINChI *pStruct, inp_ATOM *at,
                                inp_ATOM *at2, inp_ATOM *atf, inp_ATOM *atn,
                                VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups,
                                ICR *picr, int *pnNumRunBNS, int *pnTotalDelta,
                                int forbidden_edge_mask) {
  EDGE_LIST NewlyFixedEdges;
  ...
  for (i = 0; i < picr->num_endp_in1_only; i++) {
    ...
    for (j = 0; j < at2[endpoint1].valence; j++) {
      if (BOND_TYPE_DOUBLE == (BOND_TYPE_MASK & at2[endpoint1].bond_type[j])) {
        ...
        m = GetChargeFlowerUpperEdge( pBNS, pVA,
        pVA[centerpoint].nCPlusGroupEdge-1 ); if ( m < 0 || pBNS->edge[m].flow )
        { continue;
        }
        */
        /* find 2nd endpoint */
        for (k = 0; k < at2[centerpoint].valence; k++) {
          if (BOND_TYPE_SINGLE !=
              (BOND_TYPE_MASK & at2[centerpoint].bond_type[k])) {
            continue;
          }
...
int FillOutExtraFixedHDataRestr(StrFromINChI *pStruct) {
  ...
  for (i = 0; i < TAUT_NUM; i++) {
    ...
    if (pNum) {
      ...
      memcpy(pStruct->nCanon2Atno[i], pNum,
             len); /* ??? the next for(...) fills it out */
...
int FillOutExtraFixedHDataInChI(StrFromINChI *pStruct, INChI *pInChI[]) {
  ...
  if (NULL == pStruct->endpoint) {
    pStruct->endpoint = (AT_NUMB *)inchi_calloc(pStruct->num_atoms,
                                                sizeof(pStruct->endpoint[0]));
  } else {
    memset(pStruct->endpoint, 0,
           pStruct->num_atoms * sizeof(pStruct->endpoint[0]));
  }
  if (NULL == pStruct->fixed_H) {
    pStruct->fixed_H = (S_CHAR *)inchi_malloc(pStruct->num_atoms *
                                              sizeof(pStruct->fixed_H[0]));
  }
  ...
  if (pInChI[0]->nNum_H_fixed) {
    memcpy(pStruct->fixed_H, pInChI[0]->nNum_H_fixed,
           pStruct->num_atoms * sizeof(pStruct->fixed_H[0]));
  } else {
    memset(pStruct->fixed_H, 0,
           pStruct->num_atoms * sizeof(pStruct->fixed_H[0]));
  }
...
/***********************************************************************************************/
int FillOutCMP2FHINCHI(StrFromINChI *pStruct, inp_ATOM *at2, VAL_AT *pVA,
                       INChI *pInChI[], CMP2FHINCHI *pc2i) {
  int ret = 0, i, j;
  int bFixHRevrsExists =
      pInChI[1] && pInChI[1]->nNumberOfAtoms > 0 && !pInChI[1]->bDeleted;
  inp_ATOM *at_Mobile_H_Revrs =
      (pStruct->pOne_norm_data[1] && pStruct->pOne_norm_data[1]->at)
          ? pStruct->pOne_norm_data[1]->at
          : NULL;
  S_CHAR *num_Fixed_H_Revrs = pStruct->pOneINChI[0]->nNum_H_fixed
                                  ? pStruct->pOneINChI[0]->nNum_H_fixed
                                  : NULL;
  /* atom number in structure that produced original InChI is atom number in all
   * inp_ATOM *atoms */
  ...
  S_CHAR *pnMobHInChI = (pInChI[1] && pInChI[1]->nNum_H)   ? pInChI[1]->nNum_H
                        : (pInChI[0] && pInChI[0]->nNum_H) ? pInChI[0]->nNum_H
                                                           : NULL;
  S_CHAR *pnMobHRevrs =
      (pStruct->pOneINChI[1] && pStruct->pOneINChI[1]->nNum_H)
          ? pStruct->pOneINChI[1]->nNum_H
      : (pStruct->pOneINChI[0] && pStruct->pOneINChI[0]->nNum_H)
          ? pStruct->pOneINChI[0]->nNum_H
          : NULL;
  ...
  for (i = 0; i < pStruct->ti.num_t_groups && i < pStruct->One_ti.num_t_groups;
       i++) {
    nNumTgHInChI =
        pStruct->ti.t_group[i].num[0] - pStruct->ti.t_group[i].num[1];
    nNumTgMInChI = pStruct->ti.t_group[i].num[1];
    nNumTgHRevrs =
        pStruct->One_ti.t_group[i].num[0] - pStruct->One_ti.t_group[i].num[1];
  ...
  for (i = j = 0; i < pStruct->num_atoms; i++) {
    ...
    if (/*(!endptInChI || !endptRevrs) &&*/ (nFixHInChI != nFixHRevrs) ||
      ...
      pc2i->c2at[j].nMobHInChI =
          pInChI[1] && pInChI[1]->nNum_H   ? pInChI[1]->nNum_H[i]
          : pInChI[0] && pInChI[0]->nNum_H ? pInChI[0]->nNum_H[i]
                                           : 0;
      pc2i->c2at[j].nMobHRevrs =
          (pStruct->pOneINChI[1] && pStruct->pOneINChI[1]->nNum_H)
              ? pStruct->pOneINChI[1]->nNum_H[iCanonRevrs]
          : (pStruct->pOneINChI[0] && pStruct->pOneINChI[0]->nNum_H)
              ? pStruct->pOneINChI[0]->nNum_H[iCanonRevrs]
              : 0;
      pc2i->nNumDiffMobH +=
          (nMobHInChI != nMobHRevrs && !endptRevrs && !endptInChI);
    ...
    if (!pVA[i].cMetal) {
      ...
      pc2i->nChargeMobHRevrsNonMetal +=
          at_Mobile_H_Revrs ? at_Mobile_H_Revrs[i].charge : 0;
    }
  ...
  pc2i->nChargeMobHRevrs =
      pStruct->pOneINChI[1]   ? pStruct->pOneINChI[1]->nTotalCharge
      : pStruct->pOneINChI[0] ? pStruct->pOneINChI[0]->nTotalCharge
                              : 0;
  pc2i->nChargeFixHRevrs =
...
/***********************************************************************************************/
int FillOutCMP2MHINCHI(StrFromINChI *pStruct, ALL_TC_GROUPS *pTCGroups,
                       inp_ATOM *at2, VAL_AT *pVA, INChI *pInChI[],
                       CMP2MHINCHI *pc2i) {
  int ret = 0, i, j, iat;
  int bFixHRevrsExists =
      pInChI[1] && pInChI[1]->nNumberOfAtoms > 0 && !pInChI[1]->bDeleted;
  inp_ATOM *at_Mobile_H_Revrs =
      (pStruct->pOne_norm_data[0] && pStruct->pOne_norm_data[0]->at)
          ? pStruct->pOne_norm_data[0]->at
          : NULL;
  /* atom number in structure that produced original InChI is atom number in all
   * inp_ATOM *atoms */
  /* atom number in structure that produced restored InChI is in nAtomRevrs[]:
  ...
  for (i = 0; i < pStruct->ti.num_t_groups; i++) {
    ...
    for (j = 0; j < jNum; j++) {
      ...
      is_N = pVA[iat].cNumValenceElectrons == 5 &&
      ...
      if (at2[iat].chem_bonds_valence == at2[iat].valence) {
        ...
        if (is_N) {
          ...
          pc2i->nNumTgNMinusInChI +=
              at2[iat].charge == -1 && at2[iat].num_H == 0;
          pc2i->nNumTgNHMinusInChI +=
              at2[iat].charge == -1 && at2[iat].num_H == 1;
        } else {
          /* O, S, Se, Te */
          pc2i->nNumTgOHInChI += at2[iat].charge == 0 && at2[iat].num_H == 1;
          pc2i->nNumTgOMinusInChI +=
              at2[iat].charge == -1 && at2[iat].num_H == 0;
        }
  ...
  for (i = 0; i < pStruct->One_ti.num_t_groups; i++) {
    ...
    for (j = 0; j < jNum; j++) {
      iat =
          nCanon2AtnoRevrs[(int)pStruct->One_ti.nEndpointAtomNumber[jFst + j]];
      is_N = pVA[iat].cNumValenceElectrons == 5 &&
             pVA[iat].cPeriodicRowNumber == 1;
      is_O = pVA[iat].cNumValenceElectrons == 6;
      if (is_N + is_O != 1) {
      ...
      if (at2[iat].chem_bonds_valence == at2[iat].valence) {
        ...
        if (is_N) {
          ...
          pc2i->nNumTgNMinusRevrs +=
              at2[iat].charge == -1 && at2[iat].num_H == 0;
          pc2i->nNumTgNHMinusRevrs +=
              at2[iat].charge == -1 && at2[iat].num_H == 1;
        } else {
          /* O, S, Se, Te */
          pc2i->nNumTgOHRevrs += at2[iat].charge == 0 && at2[iat].num_H == 1;
          pc2i->nNumTgOMinusRevrs +=
              at2[iat].charge == -1 && at2[iat].num_H == 0;
        }
      } else if (at2[iat].chem_bonds_valence == at2[iat].valence + 1) {
        /* donor */
...
/******************************************************************************************************/
int NormalizeAndCompare(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
                        BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
                        inp_ATOM *at, inp_ATOM *at2, inp_ATOM *at3, VAL_AT *pVA,
                        ALL_TC_GROUPS *pTCGroups, INChI *pInChI[], long num_inp,
  ...
  int err;
  ICR icr, icr2;
  int num_norm_endpoints, num_endpoints, num_norm_t_groups, num_mobile,
      num_norm_mobile, ret = 0;
#if (bRELEASE_VERSION == 0)
#ifndef TARGET_API_LIB
  const char *szCurHdr =
      (ip->pSdfValue && ip->pSdfValue[0]) ? ip->pSdfValue : "???";
  ...
  /***********************************************************/
  ret = MakeOneInChIOutOfStrFromINChI2(ip, sd, pBNS, pStruct, at, at2, at3, pVA,
                                       pTCGroups, &t_group_info, &at_norm,
                                       &at_prep);
  if (ret < 0) {
#if (bRELEASE_VERSION == 0)
#ifndef TARGET_API_LIB
    fprintf(stdout, "\nERROR in MakeOneInchi-1: %ld %s Comp:%d %c%c Err:%d\n",
            num_inp, szCurHdr ? szCurHdr : "???", iComponent,
            pStruct->iInchiRec ? 'R' : 'D', pStruct->iMobileH ? 'M' : 'F', ret);
#endif
#endif
    goto exit_function;
  }
  if (pStruct->bMobileH == TAUT_NON) {
    ...
    iOrigInChI =
        (pInChI[1] && pInChI[1]->nNumberOfAtoms && !pInChI[1]->bDeleted) ? 1
                                                                         : 0;
    iRevrInChI =
        (pStruct->pOneINChI[1] && pStruct->pOneINChI[1]->nNumberOfAtoms &&
         !pStruct->pOneINChI[1]->bDeleted)
            ? 1
            : 0;
  } else {
    iOrigInChI = 0;
  ...
  if (pStruct->iMobileH == TAUT_NON &&
      (ret = FillOutExtraFixedHDataRestr(pStruct))) {
    goto exit_function;
  }
  cmpInChI = CompareReversedINChI2(
      pStruct->pOneINChI[iRevrInChI], pInChI[iOrigInChI],
   ...
   * ******************************/
  if ((cmpInChI & IDIF_LESS_H) && at_prep &&
      0 < (nDeltaCur = icr.tot_num_H2 - icr.tot_num_H1)) {
    do {
      ret = FixLessHydrogenInFormula(pBNS, pBD, pStruct, at, at2, at_prep, pVA,
                                     pTCGroups, pnNumRunBNS, pnTotalDelta,
                                     forbidden_edge_mask);
      if (ret < 0) {
        goto exit_function;
      }
      if (ret) {
        /* Probably success. The changes are in pBNS. Create new InChI out of
         * the new restored structure */
        ret = MakeOneInChIOutOfStrFromINChI2(ip, sd, pBNS, pStruct, at, at2,
                                             at3, pVA, pTCGroups, &t_group_info,
                                             &at_norm, &at_prep);
        if (ret < 0) {
#if (bRELEASE_VERSION == 0)
#ifndef TARGET_API_LIB
    ...
    } while ((cmpInChI & IDIF_LESS_H) && at_prep && nDeltaCur &&
             nDeltaCur < nDeltaPrev);
  }
  /********** InChI from restored structure has MORE hydrogen atoms
   * ******************************/
  if ((cmpInChI & IDIF_MORE_H) && at_prep &&
      0 < (nDeltaCur = icr.tot_num_H1 - icr.tot_num_H2)) {
    do {
      ret = FixMoreHydrogenInFormula(pBNS, pBD, pStruct, at, at2, at_prep, pVA,
                                     pTCGroups, pnNumRunBNS, pnTotalDelta,
                                     forbidden_edge_mask);
      if (ret < 0) {
        goto exit_function;
    ...
    } while ((cmpInChI & IDIF_MORE_H) && at_prep && nDeltaCur &&
             nDeltaCur < nDeltaPrev);
  }
  /***************** Fix non-taut atoms normalized to tautomeric endpoints
   * ***********************/
  if ((cmpInChI & IDIF_EXTRA_TG_ENDP) && at_norm &&
      0 < (nDeltaCur = icr.num_endp_in1_only)) {
    do {
      ret = FixRemoveExtraTautEndpoints(
          pBNS, pBD, pStruct, at, at2, at_prep, at_norm, pVA, pTCGroups, &icr,
    ...
    } while ((cmpInChI & IDIF_EXTRA_TG_ENDP) && at_norm && nDeltaCur &&
             nDeltaCur < nDeltaPrev);
  }
  /************************ case of Fixed-H
   * ******************************************************/
  ...
  if (pStruct->bMobileH == TAUT_NON) {
    int num_tries = 0;
    do {
      if (0 > (ret = FixFixedHRestoredStructure(
                   ip, sd, pBNS, pBD, pStruct, at, at2, at3, pVA, pTCGroups,
                   &t_group_info, &at_norm, &at_prep, pInChI, num_inp,
                   bHasSomeFixedH, pnNumRunBNS, pnTotalDelta,
                   forbidden_edge_mask, forbidden_stereo_edge_mask))) {
        goto exit_function;
      }
  ...
  if (pStruct->bMobileH == TAUT_YES) {
    if (0 > (ret = FixMobileHRestoredStructure(
                 ip, sd, pBNS, pBD, pStruct, at, at2, at3, pVA, pTCGroups,
                 &t_group_info, &at_norm, &at_prep, pInChI, num_inp,
                 bHasSomeFixedH, pnNumRunBNS, pnTotalDelta, forbidden_edge_mask,
  ...
  /**********************************************************************************************/
  /* stereo */
  cmpInChI = CompareReversedINChI2(pStruct->pOneINChI[0], pInChI[0],
                                   pStruct->pOneINChI_Aux[0],
  ...
  if (iRevrInChI || iOrigInChI) {
    ...
    cmpInChI2 = CompareReversedINChI2(pStruct->pOneINChI[iRevrInChI],
                                      pInChI[iOrigInChI],
                                      pStruct->pOneINChI_Aux[iRevrInChI],
                                      NULL /*INChI_Aux *a2*/, &icr2, &err);
    if (cmpInChI & IDIF_PROBLEM) {
      ret = RI_ERR_PROGR; /* severe restore problem */
      goto exit_function;
    }
  ...
  ret = FixRestoredStructureStereo(
      cmpInChI, &icr, cmpInChI2, &icr2, ip, sd, pBNS, pBD, pStruct, at, at2,
      at3, pVA, pTCGroups, &t_group_info, &at_norm, &at_prep, pInChI, num_inp,
      pnNumRunBNS, pnTotalDelta, forbidden_edge_mask,
      forbidden_stereo_edge_mask);
   ...
   * *************************/
  if (cmpInChI && pTCGroups->num_tgroups &&
      pBNS->tot_st_cap > pBNS->tot_st_flow) {
    ret = FixAddProtonForADP(pBNS, pBD, pStruct, at, at2, at_prep, pVA,
                             pTCGroups, &icr, pnNumRunBNS, pnTotalDelta,
                             forbidden_edge_mask);
    if (ret < 0) {
      goto exit_function;
    }
  ...
  /* moved to MakeOneInChIOutOfStrFromINChI():
    ...
    pStruct->One_ti.tni.nNumRemovedProtons : 0;
  */
...
#if (bRELEASE_VERSION == 0)
#ifndef TARGET_API_LIB
  if (num_norm_t_groups != pTCGroups->num_tgroups ||
      num_norm_endpoints != num_endpoints) {
    /* need aggressive (de)protonation */
    /* pStruct->bExtract |= EXTRACT_STRUCT_NUMBER; */
    fprintf(
        stdout,
        "NORMCOMP: %s comp=%d %c%c: InChI/NormRvrs NumTg=%d/%d NumEndp=%d/%d\n",
        (*ip).pSdfValue, (*pTCGroups).iComponent,
        pStruct->iInchiRec ? 'R' : 'D', pStruct->iMobileH ? 'M' : 'F',
        pTCGroups->num_tgroups, num_norm_t_groups, num_endpoints,
        num_norm_endpoints);
  }
#endif
#endif
 ...
 * temporary allow stereobonds   */
/* change and make A=X bonds single */
int CheckAndRefixStereobonds(BN_STRUCT *pBNS, BN_DATA *pBD,
                             StrFromINChI *pStruct, inp_ATOM *at, inp_ATOM *at2,
                             VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups,
  ...
  for (i = 0; i < num_at; i++) {
    if (at2[i].valence == 3 &&
        ...
        (at2[i].bond_type[j1 = (int)at2[i].sb_ord[0]] & BOND_TYPE_MASK) ==
            BOND_TYPE_SINGLE &&
        (at2[i].bond_type[j2 = (int)at2[i].sb_ord[1]] & BOND_TYPE_MASK) ==
            BOND_TYPE_SINGLE &&
        j1 != j2) {
  ...
  /* do not goto exit_function before reaching this point: EdgeLists have not
   * been initiated */

  if (0 > (ret = ForbidCarbonChargeEdges(pBNS, pTCGroups, &CarbonChargeEdges,
                                         forbidden_edge_mask))) {
    goto exit_function;
  }
...
int MoveChargeToRemoveCenerpoints(BN_STRUCT *pBNS, BN_DATA *pBD,
                                  StrFromINChI *pStruct, inp_ATOM *at,
                                  inp_ATOM *at2, VAL_AT *pVA,
                                  ALL_TC_GROUPS *pTCGroups, int *pnNumRunBNS,
                                  int *pnTotalDelta, int forbidden_edge_mask) {
  int i, j, neigh, num_endpoints, num_success;
  int num_donors, num_acceptors, bond_type, num_donors_O, num_acceptors_O,
      is_centerpoint_N, num_known_endpoints, num_wrong_neigh;
  ...
  int bPossiblyIgnore =
      pStruct->charge >= 0 &&
      (!pTCGroups->num_tgroups ||
       (pStruct->iMobileH == TAUT_NON && pStruct->ti.num_t_groups));
  S_CHAR MobileChargeNeigh[MAXVAL], DoubleBondAcceptors[MAXVAL],
      DoubleBondNotONeigh[MAXVAL];
  int numMobileChargeNeigh, numDoubleBondAcceptors, numDoubleBondNotONeigh,
  ...
  /*
  if ( pStruct->charge >= 0 && (!pTCGroups->num_tgroups || pStruct->iMobileH ==
  TAUT_NON && pStruct->ti.num_t_groups) ) { goto exit_function;
  }
  */
...
#endif
  /* mark bonds that cannot be tautomeric; do not forget to remove the marks
   * later */
  ...
  for (i = 0; i < num_at; i++) {
    if (pVA[i].cNumValenceElectrons != 4 && /* not C, Si, Ge */
        !(pVA[i].nTautGroupEdge ||
          (pStruct->iMobileH == TAUT_NON && pStruct->endpoint &&
           pStruct->endpoint[i])) &&
        !at2[i].num_H && !at2[i].charge && at2[i].valence >= 2 &&
        at2[i].valence < at2[i].chem_bonds_valence &&
        is_centerpoint_elem(at2[i].el_number)) {

      is_centerpoint_N = (pVA[i].cNumValenceElectrons == 5 &&
                          (pVA[i].cPeriodicRowNumber == 1 || pVA[i].cMetal));
      /* look at the neighbors */
      numMobileChargeNeigh = numDoubleBondAcceptors = numDoubleBondNotONeigh =
          num_donors = num_acceptors = 0;
      ...
      for (j = 0, num_endpoints = 0; j < at2[i].valence; j++) {
        ...
        if (pBNS->edge[pBNS->vert[i].iedge[j]].forbidden &
            forbidden_edge_test) {
          continue;
        }
        ...
        if (at2[neigh].chem_bonds_valence - at2[neigh].charge !=
          ...
          if (bond_type == BOND_TYPE_DOUBLE &&
              pVA[neigh].cNumValenceElectrons != 6) {
            DoubleBondNotONeigh[numDoubleBondNotONeigh++] = j;
          }
          continue;
        }
        if (at2[neigh].charge == -1 && bond_type == BOND_TYPE_SINGLE &&
            (pVA[neigh].nCMinusGroupEdge < 1 ||
             pBNS->edge[pVA[neigh].nCMinusGroupEdge - 1].flow != 1)) {
          break;
        }
        switch (bond_type) {
        case BOND_TYPE_SINGLE:
          ...
          num_donors++;
          num_donors_O += (pVA[neigh].cNumValenceElectrons == 6 &&
                           pVA[neigh].cPeriodicRowNumber <= 4);
        ...
        case BOND_TYPE_DOUBLE:
          ...
          num_acceptors++;
          num_acceptors_O += (pVA[neigh].cNumValenceElectrons == 6 &&
                              pVA[neigh].cPeriodicRowNumber <= 4);
        }
      }
      ...
      /* special case NOn(-) */
      if (is_centerpoint_N && (num_donors == num_donors_O) &&
          (num_acceptors == num_acceptors_O)) {
        continue;
      }
      if (pStruct->iMobileH == TAUT_NON &&
        ...
        if (!ChargeListAllExcept_DB_O.num_edges) {
          ...
          for (k = 0; k < num_at; k++) {
            if ((1 == at2[k].valence &&
                 pBNS->edge[pBNS->vert[k].iedge[0]].flow &&
                 !pBNS->edge[pBNS->vert[k].iedge[0]].forbidden &&
                 !((e = pVA[k].nCMinusGroupEdge - 1) >= 0 &&
                   pBNS->edge[e].flow) &&
                 !((e = pVA[k].nCPlusGroupEdge - 1) >= 0 &&
                   !pBNS->edge[e].flow) &&
                 /* 0 == at2[k].charge && */
            ...
            } else if ((e = pVA[k].nCMinusGroupEdge - 1) >= 0 &&
                       !pBNS->edge[e].flow && !pBNS->edge[e].forbidden &&
                       (ret =
                            AddToEdgeList(&ChargeListAllExcept_DB_O, e, 64))) {
              goto exit_function;
            }
        ...
        SetForbiddenEdgeMask(pBNS, &ChargeListAllExcept_DB_O,
                             forbidden_edge_mask);
        for (k = 0; k < numDoubleBondNotONeigh &&
                    num_MovedCharges < numMobileChargeNeigh;
             k++) {
        ...
        RemoveForbiddenEdgeMask(pBNS, &ChargeListAllExcept_DB_O,
                                forbidden_edge_mask);
      } else if (!bPossiblyIgnore ||
                 (!num_known_endpoints && !num_wrong_neigh &&
                  (num_acceptors_O + num_donors_O) >= 3)) {
        /* remove negative charges from the neighbors */
        pBNS->vert[i].st_edge.cap +=
            num_donors; /* enough to make all bonds to donors double */
        pBNS->tot_st_cap += num_donors;
        pVA[i].cInitCharge -=
            num_donors; /* work no matter what are known charge/valence */
...
int MakeSingleBondsMetal2ChargedHeteroat(BN_STRUCT *pBNS, BN_DATA *pBD,
                                         StrFromINChI *pStruct, inp_ATOM *at,
                                         inp_ATOM *at2, VAL_AT *pVA,
                                         ALL_TC_GROUPS *pTCGroups,
                                         int *pnNumRunBNS, int *pnTotalDelta,
  ...
  cur_num_edges = 0;  /* count cur_num_edges  only when pass==1; at the end they
                         must be equal */
  for (pass = 0; pass < 2; pass++) {
    if (pass) {
      ...
      pFixedEdges =
          (EdgeIndex *)inchi_malloc(nNumEdgesToFix * sizeof(pFixedEdges[0]));
      if (!pFixedEdges) {
        ret = RI_ERR_ALLOC;
        goto exit_function;
      }
      ...
      if (pVA[i].cMetal) {
        for (j = 0; j < at2[i].valence; j++) {
          ...
          if (at2[i].bond_type[j] > BOND_TYPE_SINGLE && at2[neigh].charge &&
            ...
            int cnBits = at2[neigh].charge > 0
                             ? MAKE_CN_BITS(cn_bits_N, cn_bits_P, 0, 0)
                             : MAKE_CN_BITS(cn_bits_N, cn_bits_M, 0, 0);
            int atBits = cnList[pVA[neigh].cnListIndex - 1].bits;
            for (k = 0; k < MAX_NUM_CN_BITS - 1;
                 k++, atBits >>= cn_bits_shift) { /* ??? */
...
int SaltBondsToCoordBonds(BN_STRUCT *pBNS, BN_DATA *pBD, StrFromINChI *pStruct,
  ...
  if (pStruct->iInchiRec == INCHI_BAS || !pStruct->pSrm->bMetalAddFlower ||
      pStruct->pSrm->nMetalMinBondOrder) {
    goto exit_function;
  }
    ...
    if (bIsMetalSalt(at2, i)) {
      if (!AllChargeEdges.num_edges) {
        ...
        for (j = 0; j < num_at; j++) {
          ...
          /* charge edges */
          if ((ie = pVA[j].nCMinusGroupEdge - 1) >= 0 &&
              !pBNS->edge[ie].forbidden &&
              (ret = AddToEdgeList(&AllChargeEdges, ie, num_edges))) {
            goto exit_function;
          }
          if ((ie = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
              !pBNS->edge[ie].forbidden &&
              (ret = AddToEdgeList(&AllChargeEdges, ie, num_edges))) {
            goto exit_function;
          }
        ...
        for (j = 0; j < pTCGroups->num_tgroups; j++) {
          pv1 = pBNS->vert +
                (v1 = pTCGroups->pTCG[j].nVertexNumber); /* t-group vertex */
          for (k = 0; k < pv1->num_adj_edges; k++) {
            /* ie, pe - tautomeric atom edge; pv2 - endpoint vertex */
            /* Note: pe, pv2, v1 are not used here; they are to show how to
             * traverse t-group */
            pv2 = pBNS->vert +
                  (pe = pBNS->edge + (ie = pv1->iedge[k]))->neighbor1;
      ...
      for (k = 0; k < at2[i].valence; k++) {
        ...
        if (ieMinusO >= 0) {
          ...
          RemoveForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
                   ...
                   (vPlusMinus = GetPlusMinusVertex(pBNS, pTCGroups, 1, 1))) {
          ...
          for (i1 = 0; i1 < pBNS->vert[vPlusMinus].num_adj_edges; i1++) {
            j1 = pBNS->edge[pBNS->vert[vPlusMinus].iedge[i1]].neighbor12 ^
                 vPlusMinus;
          ...
          if (ret == 1 &&
            ...
            /* Flow change on pe (-)charge edge (atom B-O(-)) is not known to
             * RunBnsTestOnce()) */
            ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
            if (ret > 0) {
              (*pnNumRunBNS)++;
          ...
          if (cur_success) {
            ...
            pBNS->vert[vPlusMinus].st_edge.cap -= delta;
            pvO->st_edge.cap -= delta;
            /*pTCGroups->pTCG[itg].edges_cap     -= delta;*/ /* ???bug??? -
                                                                commented out
                                                                2006-03-22 */
            pBNS->tot_st_cap -= 2 * delta;
...
/******************************************************************************************************/
int ForbidMetalCarbonEdges(BN_STRUCT *pBNS, inp_ATOM *at, int num_at,
                           VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups,
...
#endif
/******************************************************************************************************/
int RunBnsRestore1(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, BN_STRUCT *pBNS,
                   BN_DATA *pBD, StrFromINChI *pStruct, VAL_AT *pVA,
                   ALL_TC_GROUPS *pTCGroups, INChI *pInChI[], long num_inp,
                   int bHasSomeFixedH) {
  int nNumRunBNS = 0;
  ...
  if ((!at2 && !(at2 = (inp_ATOM *)inchi_malloc((num_at + num_deleted_H) *
                                                sizeof(at2[0])))) ||
      (!at3 && !(at3 = (inp_ATOM *)inchi_malloc((num_at + num_deleted_H) *
                                                sizeof(at3[0]))))) {
    return RI_ERR_ALLOC;
  }
  ...
  if (0 > (ret = ForbidMetalCarbonEdges(pBNS, at, num_at, pVA, pTCGroups,
                                        &MetalCarbonEdges,
                                        BNS_EDGE_FORBIDDEN_TEMP))) {
    goto exit_function;
  }
#endif
  if (0 > (ret = ForbidNintrogenPlus2BondsInSmallRings(
               pBNS, at, num_at, pVA, 6, pTCGroups, &Nplus2BondsEdges,
               BNS_EDGE_FORBIDDEN_TEMP))) {
    goto exit_function;
  }
   ...
   * ****************************/
  if (0 > (ret = ForbidNintrogenPlus2BondsInSmallRings(
               pBNS, at, num_at, pVA, 8, pTCGroups, &Nplus2BondsEdges,
               BNS_EDGE_FORBIDDEN_TEMP))) {
    goto exit_function;
  }
  if (Nplus2BondsEdges.num_edges) {
  ...
  ret = MoveRadToAtomsAddCharges(pBNS, pBD, pStruct, at, at2, pVA, pTCGroups,
                                 BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  /* rearrange (+) and (-) edges flow so that there is no (+)flow=0 and
   * (-)flow=1 */
  ret = RearrangePlusMinusEdgesFlow(pBNS, pBD, pVA, pTCGroups,
                                    BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  /*****************************************************************/
  ret = IncrementZeroOrderBondsToHeteroat(pBNS, pBD, pStruct, at, at2, pVA,
                                          pTCGroups, &nNumRunBNS, &nTotalDelta,
                                          BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  /*****************************************************************/
  ret = MoveChargeFromHeteroatomsToMetals(pBNS, pBD, pStruct, at, at2, pVA,
                                          pTCGroups, &nNumRunBNS, &nTotalDelta,
                                          BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  ...
  ************************************************************************/
  ...
  ret = EliminateChargeSeparationOnHeteroatoms(
      pBNS, pBD, pStruct, at, at2, pVA, pTCGroups, &nNumRunBNS, &nTotalDelta,
      BNS_EDGE_FORBIDDEN_TEMP, 0);
  if (ret < 0) {
    goto exit_function;
  }
  if (ret) {
    /*charge separation remains; allow changes of stereobonds in a ring and try
     * again */
    ret = EliminateChargeSeparationOnHeteroatoms(
        pBNS, pBD, pStruct, at, at2, pVA, pTCGroups, &nNumRunBNS, &nTotalDelta,
        BNS_EDGE_FORBIDDEN_TEMP, BNS_EDGE_FORBIDDEN_MASK);
    if (ret < 0) {
      goto exit_function;
    }
  }
  ...
  /*****************************************************************/
  ret =
      RestoreIsoCyanoGroup(pBNS, pBD, pStruct, at, at2, pVA, pTCGroups,
                           &nNumRunBNS, &nTotalDelta, BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  /*****************************************************************/
  ret = MoveMobileHToAvoidFixedBonds(pBNS, pBD, pStruct, at, at2, pVA,
                                     pTCGroups, &nNumRunBNS, &nTotalDelta,
                                     BNS_EDGE_FORBIDDEN_TEMP);
  ...
  tot_num_fixes = 0;
  if (pStruct->iMobileH == TAUT_NON) {
    ret = RemoveRadFromMobileHEndpointFixH(pBNS, pBD, pStruct, at, at2, pVA,
                                           pTCGroups, &nNumRunBNS, &nTotalDelta,
                                           BNS_EDGE_FORBIDDEN_TEMP);
  } else {
    ret = RemoveRadFromMobileHEndpoint(pBNS, pBD, pStruct, at, at2, pVA,
                                       pTCGroups, &nNumRunBNS, &nTotalDelta,
                                       BNS_EDGE_FORBIDDEN_TEMP);
  }
  if (ret < 0) {
  ...
  ret = MakeSingleBondsMetal2ChargedHeteroat(
      pBNS, pBD, pStruct, at, at2, pVA, pTCGroups, &nNumRunBNS, &nTotalDelta,
      BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  ...
  /**************************************************************************/
  /* Find and eliminate false Mobile-H groups: Cl(=O)3(-O(-)) => Cl(-)(=O)4 */
  ret = MoveChargeToRemoveCenerpoints(pBNS, pBD, pStruct, at, at2, pVA,
                                      pTCGroups, &nNumRunBNS, &nTotalDelta,
                                      BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  ...
  /* However, in this release metal-organic compounds do not get much care  */
  ret =
      SaltBondsToCoordBonds(pBNS, pBD, pStruct, at, at2, pVA, pTCGroups,
                            &nNumRunBNS, &nTotalDelta, BNS_EDGE_FORBIDDEN_TEMP);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  ret = NormalizeAndCompare(ip, sd, pBNS, pBD, pStruct, at, at2, at3, pVA,
                            pTCGroups, pInChI, num_inp, bHasSomeFixedH,
                            &nNumRunBNS, &nTotalDelta, BNS_EDGE_FORBIDDEN_TEMP,
...
/******************************************************************************************************/
int RestoreAtomMakeBNS(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
                       StrFromINChI *pStruct, int iComponent, int iAtNoOffset,
                       INChI *pInChI[], const char *szCurHdr, long num_inp,
                       int bHasSomeFixedH) {
  int i, j, ret = 0, ret2;
  ...
  if (num_at == 1) {
    ...
    inp_ATOM *at2 = (inp_ATOM *)inchi_malloc(
        sizeof(at2[0]) * (pStruct->num_atoms + pStruct->num_deleted_H));
    inp_ATOM *at3 = (inp_ATOM *)inchi_malloc(
        sizeof(at3[0]) * (pStruct->num_atoms + pStruct->num_deleted_H));
    pStruct->at2 = at2;
    ...
    ret = MakeOneInChIOutOfStrFromINChI(ip, sd, pStruct, pStruct->at2, at3,
                                        pTCGroups);
    /* clean up */
    for (i = 0; i < TAUT_NUM; i++) {
      Free_INChI(&pStruct->pOneINChI[i]);
  ...
  if (0 >
      (ret = AllocBfsQueue(&bfsq, num_at, 0 /* min ring size undefined */))) {
    goto exit_function;
  }
  pStruct->pbfsq = &bfsq;

  if (pStruct->iMobileH == TAUT_NON && pInChI[1] &&
      pInChI[1]->nNumberOfAtoms > 1 &&
  ...
  for (i = 0; i < num_at; i++) {
    ...
    pVA[i].cNumValenceElectrons--; /* = -1 d- and f- metals, 0 for H, 1 for Na,
                                      2 for Mg,.. = (ATYPE_Xx-1)  */
    ...
    if (at[i].valence == 2 && !at[i].num_H) {
      pVA[i].cMinRingSize =
          is_bond_in_Nmax_memb_ring(at, i, 0, bfsq.q, bfsq.nAtomLevel,
...
repeat_for_new_metals:
  /* set valences for the first time; find ChargeValence structures for each
   * atom */
  for (i = 0; i < num_at; i++) {
    /* get additional fictitious atoms information */
    pVA[i].cInitFreeValences = 0;
    ret = GetAtomRestoreInfo(at, i, pVA, pStruct->pSrm, pStruct->bMobileH,
                             pStruct->endpoint);
    if (ret < 0) {
      goto exit_function;
    }
  ...
  /* count atoms, bonds, additional edges and vertices in ChargeValence
   * structures and t-groups */
  ret = nCountBnsSizes(at, num_at, nAddEdges2eachAtom, nAddVertices,
                       &pStruct->ti, pVA, pStruct->pSrm, pTCGroups);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  pBNS =
      AllocateAndInitTCGBnStruct(pStruct, pVA, pTCGroups, nMaxAddAtoms,
                                 nMaxAddEdges, max_altp, &nNum_changed_bonds);
  if (!pBNS) {
    ret = BNS_OUT_OF_RAM;
    goto exit_function;
  }
  ...
  ret = RunBnsRestore1(ip, sd, pBNS, pBD, pStruct, pVA, pTCGroups, pInChI,
                       num_inp, bHasSomeFixedH);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  if (ret) {
    fprintf(stdout, "Msg for: %ld %s comp=%d %c%c\n", num_inp,
            (szCurHdr && szCurHdr[0]) ? szCurHdr : "", iComponent,
            pStruct->iInchiRec ? 'R' : 'D', pStruct->iMobileH ? 'M' : 'F');
  }
...
/******************************************************************************************************/
int OneInChI2Atom(ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd,
                  const char *szCurHdr, long num_inp, StrFromINChI *pStruct,
                  int iComponent, int iAtNoOffset, int bHasSomeFixedH,
                  INChI *pInChI[]) {
  int ret;
  ...
  ret = RestoreAtomConnectionsSetStereo(pStruct, iComponent, iAtNoOffset,
                                        pInChI[0], pInChI[1]);
  if (ret < 0) {
    goto exit_function;
  }
  ...
  ret = RestoreAtomMakeBNS(ip, sd, pStruct, iComponent, iAtNoOffset, pInChI,
                           szCurHdr, num_inp, bHasSomeFixedH);

#ifndef COMPILE_ANSI_ONLY
  if ((pStruct->num_inp_actual > 0 ? pStruct->num_inp_actual : num_inp) >=
          ip->first_struct_number &&
      ((/*ret > 0 &&*/ ip->bDisplayIfRestoreWarnings) && pStruct->pXYZ)) {
    inchiTime ulTStart;
    InchiTimeGet(&ulTStart);
    DisplayRestoredComponent(pStruct, iComponent, iAtNoOffset, pInChI[0],
                             szCurHdr);
  ...
  if ((pStruct->num_inp_actual ? pStruct->num_inp_actual : num_inp) >=
          ip->first_struct_number &&
      ret >= 0) {
    ...
    */
    ...
    /***************************************************************************************/
    /* !!! pStruct->One_InChI etc. were removed at the exit from
     * NormalizeAndCompare() !!! */
    /***************************************************************************************/
    if (bHasSomeFixedH && pStruct->iInchiRec == INCHI_REC &&
        pStruct->iMobileH == TAUT_YES && !pStruct->bFixedHExists &&
        !(ip->nMode & REQ_MODE_BASIC)) {
      /* reconnected components without Fixed-H layer may produce 'tautomeric'
       * fragments like Cl(-) */
      ip->nMode |= REQ_MODE_BASIC;
    }
    ret = MakeInChIOutOfStrFromINChI2(ip, sd, pStruct, iComponent, iAtNoOffset,
                                      num_inp);
    if (ret >= 0) {
      ;
    }
...
/********************************************************************************************/
int AddRemProtonsInRestrStruct(
    ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd, long num_inp,
    int bHasSomeFixedH, StrFromINChI *pStruct, int num_components,
    StrFromINChI *pStructR, int num_componentsR,
    NUM_H *nProtonsToBeRemovedByNormFromRevrs,
    int *recmet_change_balance) { /* on entry and exit, all at[i].num_H do not
                                     include isotopic H  and explicit terminal H
                                     are connected */
  int iComp, q, ret = 0;
  ...
  int delta_recmet_prot, num_prot_prev, bAccumulateChanges = 0,
  ...
  ip = &ip_loc;
  /*----------------------------------------------------------------------------------
  nLink < 0 && num_componentsR > 0 => This is a Disconnected structure
  component; it is same as already processed reconnected one Do no preicess it

  nLink > 0 && num_componentsR > 0 => This is a Disconnected structure
  component; (should not happen)                 It it is a result of
  (nLink-1)th Reconeected component disconnection (NOT IMPLEMENTED YET)

  nLink = 0                        => Process this component. It is either a
  reconnected component, or a result of a disconnection (for now)

  nLink > 0 && num_componentsR = 0 => This is a Reconnected component that is
  same as a disconnected one that will not be processed. Process and save charge
  delta.
  -----------------------------------------------------------------------------------*/
  ...
  for (iComp = 0; iComp < num_components && num_prot; iComp++) {
    ...
    if (pStruct[iComp].nLink < 0 && num_componentsR > 0) {
      ...
      if (!pStructR || !num_componentsR || q >= num_componentsR ||
          pStructR[q].nLink != (iComp + 1)) {
        ret = RI_ERR_PROGR;
        goto exit_function;
      }
      continue; /* Disconnected structure component has already been processed
                   as a Reconnected one */
    }

    at = pStruct[iComp].at2;
    num_atoms = pStruct[iComp].num_atoms;
    tot_num_at = pStruct[iComp].num_atoms +
                 (num_deleted_H = pStruct[iComp].num_deleted_H);
    ...
    q = bRevInchiComponentExists(pStruct + iComp, INCHI_REC, TAUT_YES, 0)
            ? INCHI_REC
            : INCHI_BAS;
    /*
    q = pStruct[iComp].RevInChI.pINChI_Aux[INCHI_REC] &&
        pStruct[iComp].RevInChI.pINChI_Aux[INCHI_REC][0][TAUT_YES] &&
        pStruct[iComp].RevInChI.pINChI_Aux[INCHI_REC][0][TAUT_YES]->nNumberOfAtoms?
    INCHI_REC : INCHI_BAS;
    */
    pINChI_Aux =
        pStruct[iComp].RevInChI.pINChI_Aux[q][0][TAUT_YES]; /* 0 = 1st component
                                                               in RevInChI */
    ...
    /* disconnect all explicit H and add the number of implicit iso H and all
     * explicit terminal H to the number of implicit H */
    ...
    ret = AddRemoveProtonsRestr(at, num_atoms, &num_prot, nNumProtAddedByRevrs,
                                bNormalizationFlags, num_tg, nChargeRevrs,
                                nChargeInChI);
    ...
    if (ret > 0) {
      ...
      if (bHasSomeFixedH && pStruct1->iInchiRec == INCHI_REC &&
          pStruct1->iMobileH == TAUT_YES && !pStruct1->bFixedHExists &&
          !(ip->nMode & REQ_MODE_BASIC)) {
        /* reconnected components without Fixed-H layer may produce 'tautomeric'
         * fragments like Cl(-) */
        ip->nMode |= REQ_MODE_BASIC;
      }
      /* calls ConnectDisconnectedH(...): subtracts number of implicit iso H
       * from implicit H */
      ...
      /* reconnect disconnected terminal H and subtracts number of implicit iso
       * H from implicit H */
    ...
    if (bAccumulateChanges && recmet_change_balance) {
      /* processed Reconnected layer component that is also present in
       * Disconnected layer */
  ...
  if (!bHasSomeFixedH && num_prot > 0 && 1 == num_deleted_components &&
      iComp >= 0 && pStruct[iComp].bDeleted) {
    /* add bare protons to the deleted Mobile-H component; undelete the
     * component */
    num_prot_prev = num_prot;
...
/**********************************************************************************/
int AddRemIsoProtonsInRestrStruct(
    ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd, long num_inp,
    int bHasSomeFixedH, StrFromINChI *pStruct, int num_components,
    StrFromINChI *pStructR, int num_componentsR, NUM_H pProtonBalance[],
    NUM_H recmet_change_balance[]) { /* on entry and exit, all at[i].num_H do
                                        not include isotopic H and explicit
                                        terminal H are connected */
  int iComp, q, k, ret = 0, bNotEmpty;
  int num_atoms, tot_num_at, num_deleted_H, num_tg, num_changed;
  inp_ATOM *at;
  NUM_H num_prot[NUM_H_ISOTOPES], delta_recmet_prot[NUM_H_ISOTOPES],
      num_prot_prev[NUM_H_ISOTOPES];
  ...
  for (iComp = 0; iComp < num_components && num_prot; iComp++) {
    ...
    q = pStruct[iComp].RevInChI.pINChI_Aux[INCHI_REC] &&
                pStruct[iComp].RevInChI.pINChI_Aux[INCHI_REC][0][TAUT_YES] &&
                pStruct[iComp]
                    .RevInChI.pINChI_Aux[INCHI_REC][0][TAUT_YES]
                    ->nNumberOfAtoms
            ? INCHI_REC
            : INCHI_BAS;
    ...
    pINChI_Aux =
        pStruct[iComp].RevInChI.pINChI_Aux[q][0][TAUT_YES]; /* 0 = 1st component
                                                               in RevInChI */
    pINChI =
        pStruct[iComp].RevInChI.pINChI[q][0][TAUT_YES]; /* 0 = 1st component in
                                                           RevInChI */
    bNormalizationFlags = pINChI_Aux->bNormalizationFlags;
    num_tg = pINChI_Aux->nNumberOfTGroups;
    memcpy(num_prot_prev, num_prot, sizeof(num_prot_prev));

    /* pass CONNECTED explicit H to AddRemoveIsoProtonsRestr() for isotopic H
     * addition */
    ...
    if (ret > 0) {
      StrFromINChI *pStruct1 = pStruct + iComp;
      INCHI_MODE nMode = ip->nMode;
      /* recalculate InChI; MakeInChIOutOfStrFromINChI2() will reconnect
       * explicit H */
      /* disconnect all explicit H and add the number of implicit iso H and all
       * explicit terminal H to the number of implicit H */
      if (0 > (ret = DisconnectedConnectedH(at, num_atoms, num_deleted_H))) {
    ...
    /* the following was commented out 2007-08-28 by DT. Reason: it's a bug
     * since H must be already connected */
    /* else {
        if ( 0 > ( ret = ConnectDisconnectedH( at, num_atoms, num_deleted_H ) )
    ) { goto exit_function;
        }
