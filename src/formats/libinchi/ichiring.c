...
/* local prototypes */
int GetMinRingSize(inp_ATOM *atom, QUEUE *q, AT_RANK *nAtomLevel,
                   S_CHAR *cSource, AT_RANK nMaxRingSize);
...
int QueueAdd(QUEUE *q, QINT_TYPE *Val) {
  if (q && Val && q->nLength < q->nTotLength) {
    memcpy((char *)q->Val +
               ((q->nFirst + q->nLength) % q->nTotLength) * q->nSize,
           Val, q->nSize);
    q->nLength++;
    return q->nLength;
  }
...
int QueueReinit(QUEUE *q) {
  if (q) {
    q->nFirst = 0;
    q->nLength = 0;
    /* memset( q->Val, 0, q->nTotLength*sizeof(q->Val[0])); */ /*  for debug
                                                                  only */
    return q->nTotLength;
  }
  return -1;
    ...
    for (i = 0; i < qLen; i++) {
      if (0 <= QueueGet(q, &at_no)) {
        ...
        for (j = 0; j < atom[iat_no].valence; j++) {
          ...
          inext = (int)next;
          if (!nAtomLevel[inext]) {
            /*  the at_no neighbor has not been traversed yet. Add it to the
             * queue */
            ...
            if ((nRingSize = nAtomLevel[inext] + nCurLevel - 2) <
                nMinRingSize) {
...
/* Return value:
     0:    nMaxRingSize < 3 or
           ...
           not a ring bond (the last is currently impossible: bond is known to
  belong to a ring system. n>0:  min. ring size < nMaxRingSize n<0:  error

  Input:
     atom[]
     at_no      number of the 1st atom adjacent to the bond
     neigh_ord  ordering number of the bond in question:
  at[at_no].bond_type[neigh_ord] q          queue structure nAtomLevel work
  array, DFS distance cSource    work array, origin mark
*/

int is_bond_in_Nmax_memb_ring(inp_ATOM *atom, int at_no, int neigh_ord,
