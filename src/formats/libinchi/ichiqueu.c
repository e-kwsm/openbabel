...
int AddBondsPos(inp_ATOM *atom, T_BONDPOS *BondPosTmp, int nNumBondPosTmp,
                T_BONDPOS *BondPos, int nMaxNumBondPos, int nNumBondPos);
int AddEndPoints(T_ENDPOINT *EndPointTmp, int nNumNewEndPoint,
                 T_ENDPOINT *EndPoint, int nMaxNumEndPoint, int nNumEndPoint);
...
*/
typedef int CHECK_DFS_RING(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                           int nStartAtomNeighbor, int nStartAtomNeighbor2,
                           int nStartAtomNeighborNeighbor, T_ENDPOINT *EndPoint,
                           int nMaxNumEndPoint, T_BONDPOS *BondPos,
                           int nMaxNumBondPos, int *pnNumEndPoint,
                           int *pnNumBondPos,
...
typedef int CHECK_DFS_PATH(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                           int jNxtNeigh, int nStartAtomNeighbor,
...
typedef int CHECK_DFS_CENTERPOINT(inp_ATOM *atom, DFS_PATH *DfsPath,
                                  int nLenDfsPath, int jNxtNeigh,
                                  struct BalancedNetworkStructure *pBNS,
...
int DFS_FindTautAltPath(inp_ATOM *atom, int nStartAtom, int nStartAtomNeighbor,
                        ...
                        int nCycleLen, AT_RANK *nDfsPathPos, DFS_PATH *DfsPath,
...
#define IS_ALT_OR_DBLBOND(X)                                                   \
  (((X) == BOND_SINGLE || (X) == BOND_DOUBLE) ? (X)                            \
   : ((X) == BOND_ALTERN || (X) == BOND_TAUTOM || (X) == BOND_ALT12NS)         \
       ? BOND_ALTERN                                                           \
       : BOND_WRONG);
...
#define REPLACE_THE_BOND(X)                                                    \
  ((X) == BOND_SINGLE || (X) == BOND_DOUBLE || (X) == BOND_ALTERN ||           \
   (X) == BOND_ALT12NS)
#else
...
int bIsCenterPointStrict(inp_ATOM *atom, int iat) {
  if (atom[iat].valence == atom[iat].chem_bonds_valence) {
    ...
    if (endpoint_valence &&
        ((endpoint_valence > atom[iat].valence && /* added a check for negative
                                                     charge or H 3-31-03 */
...
int nGet14TautIn7MembAltRing(
    ...
    T_ENDPOINT *EndPoint, int nMaxNumEndPoint, T_BONDPOS *BondPos,
    int nMaxNumBondPos, int *pnNumEndPoint, int *pnNumBondPos,
  ...
  *pnNumBondPos = 0;
  ...
  nRet = DFS_FindTautInARing(atom, nStartAtom, nStartAtomNeighbor,
                             ...
                             nStartAtomNeighborNeighborEndpoint, 7, nDfsPathPos,
                             DfsPath, Check7MembTautRing, bIsCenterPointStrict,
...
int nGet14TautIn5MembAltRing(
  ...
  *pnNumBondPos = 0;
  ...
  nRet = DFS_FindTautInARing(atom, nStartAtom, nStartAtomNeighbor,
                             nStartAtomNeighborEndpoint,
                             nStartAtomNeighborNeighborEndpoint, 5, nDfsPathPos,
                             DfsPath, Check7MembTautRing, bIsCenterPointStrict,
                             EndPoint, nMaxNumEndPoint, BondPos, nMaxNumBondPos,
                             pnNumEndPoint, pnNumBondPos, pBNS, pBD, num_atoms);
...
/********************************************************************************/
int nGet12TautIn5MembAltRing(inp_ATOM *atom, int nStartAtom,
                             int nStartAtomNeighbor, AT_RANK *nDfsPathPos,
...
/********************************************************************************/
int nGet15TautIn6MembAltRing(inp_ATOM *atom, int nStartAtom,
                             AT_RANK *nDfsPathPos, DFS_PATH *DfsPath,
                             int nMaxLenDfsPath, T_ENDPOINT *EndPoint,
                             int nMaxNumEndPoint, T_BONDPOS *BondPos,
                             int nMaxNumBondPos, int *pnNumEndPoint,
                             int *pnNumBondPos,
                             struct BalancedNetworkStructure *pBNS,
  ...
  *pnNumBondPos = 0;
  ...
  nRet = DFS_FindTautInARing(
      atom, nStartAtom, -1 /*nStartAtomNeighbor*/, -1 /*nStartAtomNeighbor2*/,
      -1 /*nStartAtomNeighborNeighbor*/, 6 /* nCycleLen*/, nDfsPathPos, DfsPath,
      Check6MembTautRing, bIsCenterPointStrict, EndPoint, nMaxNumEndPoint,
      BondPos, nMaxNumBondPos, pnNumEndPoint, pnNumBondPos, pBNS, pBD,
      num_atoms);
  return nRet;
}
#if (TAUT_15_NON_RING == 1) /***** post v.1 feature *****/
...
int nGet15TautInAltPath(inp_ATOM *atom, int nStartAtom, AT_RANK *nDfsPathPos,
  ...
  *pnNumBondPos = 0;
  ...
  nRet = DFS_FindTautAltPath(
      atom, nStartAtom, -1 /*nStartAtomNeighbor*/, -1 /*nStartAtomNeighbor2*/,
      -1 /*nStartAtomNeighborNeighbor*/, 4 /* nCycleLen*/, nDfsPathPos, DfsPath,
      Check15TautPath, Check15TautPathCenterpoint, EndPoint, nMaxNumEndPoint,
      BondPos, nMaxNumBondPos, pnNumEndPoint, pnNumBondPos, pBNS, pBD,
      num_atoms);
  return nRet;
}
...
int DFS_FindTautInARing(inp_ATOM *atom, int nStartAtom, int nStartAtomNeighbor,
                        ...
                        struct BalancedNetworkData *pBD, int num_atoms) {
  /*  Depth First Search */
  /*  Ignore all atoms not belonging to the current ring system (=biconnected
   * component) */
  ...
  if (nStartAtomNeighbor >= 0) {
    ...
    DfsPath[nLenDfsPath].bond_type =
        (atom[prv_at].bond_type[j] & ~BOND_MARK_ALL);
#if (FIX_BOND23_IN_TAUT == 1)
    DfsPath[nLenDfsPath].bond_type =
        ACTUAL_ORDER(pBNS, prv_at, j, DfsPath[nLenDfsPath].bond_type);
#endif
  ...
  /*  MAIN DFS CYCLE: may find one and the same t-group 2 times; saves only one
   * instance */
  /*  traverse *all* paths starting at atom[nStartAtom]; max. path length =
   * (nCycleLen+1)  */
  while (nLenDfsPath >= nMinLenDfsPath) {
    j = ++DfsPath[nLenDfsPath].bond_pos;
    if (j < atom[cur_at = (int)DfsPath[nLenDfsPath].at_no].valence) {
      DfsPath[nLenDfsPath].bond_type =
          (atom[cur_at].bond_type[j] & ~BOND_MARK_ALL);
#if (FIX_BOND23_IN_TAUT == 1)
      DfsPath[nLenDfsPath].bond_type =
          ACTUAL_ORDER(pBNS, cur_at, j, DfsPath[nLenDfsPath].bond_type);
#endif
      ...
      if (nxt_at == nDoNotTouchAtom1 || nxt_at == nDoNotTouchAtom2) {
        ...
        if (1 == nDfsPathPos[nxt_at] && nLenDfsPath == nCycleLen) {
          ...
          ret = (*CheckDfsRing)(atom, DfsPath, nLenDfsPath, nStartAtomNeighbor,
                                ...
                                EndPoint, nMaxNumEndPoint, BondPos,
                                nMaxNumBondPos, pnNumEndPoint, pnNumBondPos,
                                pBNS, pBD, num_atoms);
          if (ret < 0) {
            nNumFound = ret;
            goto clear_path;
      ...
      } else if (!(*CheckCenterPoint)(atom, nxt_at)) {
...
int DFS_FindTautAltPath(inp_ATOM *atom, int nStartAtom, int nStartAtomNeighbor,
                        ...
                        struct BalancedNetworkStructure *pBNS,
                        struct BalancedNetworkData *pBD, int num_atoms) {
  /*  Naive Depth First Search: same atom may be approached along different alt
   * paths */
  /*  Ignore all atoms not belonging to the current ring system (=biconnected
   * component) */
  AT_RANK nMinLenDfsPath;
  ...
  DfsPath[nLenDfsPath].at_no = cur_at = nStartAtom;
  DfsPath[nLenDfsPath].bond_type = 0;
  DfsPath[nLenDfsPath].bond_pos =
      -1; /* initialize index of the bond to the next atom */
...
#endif
    DfsPath[nLenDfsPath].bond_pos =
        j; /* fix index of the bond to the next atom */
  ...
  while (nLenDfsPath >= nMinLenDfsPath) {
    ...
    if (j < atom[cur_at = (int)DfsPath[nLenDfsPath].at_no].valence) {
      ...
      if (nxt_at == nDoNotTouchAtom1 || /* forbidden */
          ...
          (nLenDfsPath &&
           nxt_at == (int)DfsPath[nLenDfsPath - 1].at_no) /* step backwards */
      ) {
        ; /* ignore nxt_at */
      } else if (nLenDfsPath == nCycleLen &&
                 /* 1,5 and at least one of the endpoints is not in a ring */
                 (atom[nxt_at].nNumAtInRingSystem == 1 ||
                  atom[nStartAtom].nNumAtInRingSystem == 1) &&
                 /*  we have found the alt path of the requested length; check
                    it */
                 /* calling Check15TautPath() */
                 (ret = (*CheckDfsPath)(
                      atom, DfsPath, nLenDfsPath, j, nStartAtomNeighbor,
                      nStartAtomNeighbor2, nStartAtomNeighborNeighbor, EndPoint,
                      nMaxNumEndPoint, BondPos, nMaxNumBondPos, pnNumEndPoint,
                      pnNumBondPos, pBNS, pBD, num_atoms))) {
        if (ret < 0) {
          nNumFound = ret;
        ...
        if (!(*CheckCenterPointPath)(atom, DfsPath, nLenDfsPath, j, pBNS, pBD,
                                     num_atoms)) {
          ; /*  cannot advance to a non-centerpoint; ignore */
        } else if (nLenDfsPath < nCycleLen) {
          /*  advance */
...
int are_alt_bonds(U_CHAR *bonds, int len) {
  ...
  next_bond = bonds[0] == BOND_SINGLE   ? BOND_DOUBLE
              : bonds[0] == BOND_DOUBLE ? BOND_SINGLE
                                        : 0;
  ...
  return !next_bond                   ? bTautBondPresent
         : (next_bond == BOND_SINGLE) ? BOND_DOUBLE
                                      : BOND_SINGLE; /* bond to the end atom */
}
...
*/
/********************************************************************************/
int Check7MembTautRing(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                       int nStartAtomNeighbor, int nStartAtomNeighbor2,
                       int nStartAtomNeighborNeighbor, T_ENDPOINT *EndPoint,
                       int nMaxNumEndPoint, T_BONDPOS *BondPos,
                       int nMaxNumBondPos, int *pnNumEndPoint,
                       int *pnNumBondPos, struct BalancedNetworkStructure *pBNS,
                       struct BalancedNetworkData *pBD, int num_atoms) {
#define PATH_LEN 8

  int i, j, k, /*m,*/ nNumEndPoint, nNumEndPointTmp, nNumBondPos,
      nNumBondPosTmp;
  ...
  */
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (!atom[endpoint].endpoint) {
      AddAtom2num(EndPointTmp[nNumEndPointTmp].num, atom, endpoint,
                  2); /* fill out */
      ...
      /*
         ...
         if ( nMobile ) {
             EndPointTmp[nNumEndPointTmp].num[1] = (atom[endpoint].charge ==
         -1); EndPointTmp[nNumEndPointTmp].num[0] = nMobile; for ( m = 0; m <
         T_NUM_ISOTOPIC; m ++ ) {
                 EndPointTmp[nNumEndPointTmp].num[T_NUM_NO_ISOTOPIC+m] =
         atom[endpoint].num_iso_H[NUM_H_ISOTOPES-m-1];
             }
      */
    } else {
  ...
  bond_type = (atom[(int)DfsPath[0].at_no].bond_type[nStartAtomNeighbor2] &
               ~BOND_MARK_ALL);
#if (FIX_BOND23_IN_TAUT == 1)
  bond_type =
      ACTUAL_ORDER(pBNS, (int)DfsPath[0].at_no, nStartAtomNeighbor2, bond_type);
#endif
  path_bonds[i++] = bond_type;
  if (REPLACE_THE_BOND(bond_type)) {
  ...
  /* j is a bond type of the last bond to o2_at, the first bond from o1_at is
   * 2-j if j=1 or 2 */

  /* single bond at o2_at: it should have a mobile atom, o1_at should not */
  if ((j == BOND_SINGLE && ((!atom[o2_at].endpoint && !eif2.cDonor) ||
                            (!atom[o1_at].endpoint && !eif1.cAcceptor))) ||
      /* double bond at o2_at: it should not have a mobile atom, o1_at should */
      (j == BOND_DOUBLE && ((!atom[o2_at].endpoint && !eif2.cAcceptor) ||
                            (!atom[o1_at].endpoint && !eif1.cDonor)))) {
    return 0; /* bond pattern does not fit */
  }

  nNumBondPos = AddBondsPos(atom, BondPosTmp, nNumBondPosTmp, BondPos,
                            nMaxNumBondPos, nNumBondPos);
  nNumEndPoint = AddEndPoints(EndPointTmp, nNumEndPointTmp, EndPoint,
                              nMaxNumEndPoint, nNumEndPoint);

  if (nNumBondPos >= 0 && nNumEndPoint >= 0) {
    if ((ret = (nNumBondPos > *pnNumBondPos) ||
               (nNumEndPoint > *pnNumEndPoint))) {
  ...
  if (ret) {
    ...
    if ((atom[o1_at].endpoint != atom[o2_at].endpoint ||
         !atom[o1_at].endpoint)) {
      nErr = bExistsAnyAltPath(pBNS, pBD, atom, num_atoms, o1_at, o2_at,
...
int Check6MembTautRing(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                       int nStartAtomNeighbor, int nStartAtomNeighbor2,
                       int nStartAtomNeighborNeighbor, T_ENDPOINT *EndPoint,
                       int nMaxNumEndPoint, T_BONDPOS *BondPos,
                       int nMaxNumBondPos, int *pnNumEndPoint,
...
#define PATH_LEN 4
  ...
  if (nStartAtomNeighbor >= 0 || nStartAtomNeighbor2 >= 0 ||
      nStartAtomNeighborNeighbor >= 0)
    return -1; /*  wrong call */
  ...
  nxt_at =
      DfsPath[middle_pos = (nLenDfsPath + 1) / 2]
          .at_no; /*  must have tautomeric neighbor -OH or =O or -NH2 or =NH */
  ...
  for (i = 0; i < atom[nxt_at].valence; i++) {
    ...
    if (o_at != DfsPath[middle_pos - 1].at_no &&
        o_at != DfsPath[middle_pos + 1].at_no) {
  ...
  if (bond_type != BOND_SINGLE && bond_type != BOND_DOUBLE &&
      bond_type != BOND_TAUTOM && bond_type != BOND_ALT12NS &&
  ...
  */
  ...
  num_taut_endpoints = (0!=atom[n_at].endpoint) + (0!=atom[o_at].endpoint); //
  if O, N already are endpoints if ( num_H != 1 && num_taut_endpoints != 2 &&
  !(num_H==2 && num_taut_endpoints >= 1) ) { return 0;
  }
  */
  /*  extract -OH bond */
  nNumBondPosTmp = 0;
  ...
  if (REPLACE_THE_BOND(bond_type)) {
    BondPosTmp[nNumBondPosTmp].nAtomNumber =
        nxt_at;                                    /*  accumulate bonds to be */
    BondPosTmp[nNumBondPosTmp].neighbor_index = i; /*  marked as tautomeric */
    nNumBondPosTmp +=
        2; /*  leave room for the same bond in the opposite direction */
  }
  ...
  */
  for (j = 0; j < middle_pos; j++) {
    for (i = 0; i < 2; i++) {
      ...
      if (REPLACE_THE_BOND(bond_type)) {
        BondPosTmp[nNumBondPosTmp].nAtomNumber =
            DfsPath[k].at_no; /*  accumulate bonds to be */
        BondPosTmp[nNumBondPosTmp].neighbor_index =
            DfsPath[k].bond_pos; /*  marked as tautomeric */
        nNumBondPosTmp +=
            2; /*  leave room for the same bond in the opposite direction */
      }
    }
  }
  if (!are_alt_bonds(path_bonds[0], middle_pos + 1) ||
      !are_alt_bonds(path_bonds[1], middle_pos + 1)) {
    return 0;
  }
  ...
  if ((atom[o_at].endpoint != atom[n_at].endpoint || !atom[o_at].endpoint)) {
    int nErr;
    nErr = bExistsAnyAltPath(pBNS, pBD, atom, num_atoms, n_at, o_at,
                             ALT_PATH_MODE_TAUTOM);
    if (nErr <= 0)
      return nErr;
  }
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (!atom[endpoint].endpoint) { /* not a known endpoint */
      ...
      chem_bonds_valence =
          j ? eif2.cNeutralBondsValence : eif1.cNeutralBondsValence;
      /* endpoint_valence = get_endpoint_valence( atom[endpoint].el_number ); */
      nMobile = j ? nMobile2 : nMobile1;
      /* nMobile  = (atom[endpoint].charge == -1) + atom[endpoint].num_H; */
      /* if ( nMobile + atom[endpoint].chem_bonds_valence != endpoint_valence )
       * -- fixed 02-06-2003*/
      ...
      */
    } else { /* already an endpoint */ /* **now it is wrong:** no mobile
                                          atom/charge at this endpoint */
      memset(EndPointTmp + nNumEndPointTmp, 0, sizeof(EndPointTmp[0]));
    }
    EndPointTmp[nNumEndPointTmp].nAtomNumber = endpoint;
    EndPointTmp[nNumEndPointTmp].nGroupNumber = atom[endpoint].endpoint;
...
*********************************************************************************/
int Check15TautPathCenterpoint(inp_ATOM *atom, DFS_PATH *DfsPath,
                               int nLenDfsPath, int jNxtNeigh,
                               struct BalancedNetworkStructure *pBNS,
                               struct BalancedNetworkData *pBD, int num_atoms) {
...
/*
  ...
  0   1  2  3  4             as well as A or E may be previously detected such a
  carbon ^   nxt_at
            |
            +-- = nLenDfsPath
...
int Check15TautPath(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                    int jNxtNeigh, int nStartAtomNeighbor,
                    int nStartAtomNeighbor2, int nStartAtomNeighborNeighbor,
                    T_ENDPOINT *EndPoint, int nMaxNumEndPoint,
                    T_BONDPOS *BondPos, int nMaxNumBondPos, int *pnNumEndPoint,
                    int *pnNumBondPos, struct BalancedNetworkStructure *pBNS,
                    struct BalancedNetworkData *pBD, int num_atoms) {
#define PATH_LEN 4
  int i, j, k, /*m,*/ nNumBondPos, nNumEndPoint, cur_at, prv_at, at1,
      at2 /*, at3, step_at*/;
  ...
  DfsPath[nLenDfsPath].bond_type = 0;
  DfsPath[nLenDfsPath].bond_pos = -1;
  /*nDfsPathPos[cur_at]                = nLenDfsPath+1;*/ /* mark with distance
                                                             + 1 */
  /*------------------------------------------*/
...
#ifdef NEVER /* do not use C-endpoint of keto-enol tautomer to find 1,5 the    \
                taut path */
  ...
  if ((atom[at1].endpoint != atom[at2].endpoint || !atom[at1].endpoint)) {
    int nErr;
    nErr = bExistsAnyAltPath(pBNS, pBD, atom, num_atoms, at1, at2,
                             ALT_PATH_MODE_TAUTOM);
    if (nErr <= 0) {
      ret = nErr;
      goto exit_function;
  ...
  for (k = 0; k < nLenDfsPath; k++) {
    ...
    if (REPLACE_THE_BOND(bond_type)) {
      ...
      BondPosTmp[nNumBondPosTmp].neighbor_index =
          DfsPath[k].bond_pos; /*  marked as tautomeric */
      nNumBondPosTmp +=
          2; /*  leave room for the same bond in opposite direction */
    }
  }
  /*  save endpoints */
  for (j = 0; j < 2; j++) {
...
int Check5MembTautRing(inp_ATOM *atom, DFS_PATH *DfsPath, int nLenDfsPath,
                       int nStartAtomNeighbor, int nStartAtomNeighbor2,
                       int nStartAtomNeighborNeighbor, T_ENDPOINT *EndPoint,
                       int nMaxNumEndPoint, T_BONDPOS *BondPos,
                       int nMaxNumBondPos, int *pnNumEndPoint,
...
#define PATH_LEN 4
  ...
  int num_taut_endpoints, nNumBondPos, nNumBondPosTmp, nNumEndPoint,
      nNumEndPointTmp, ret;
  int endpoint;
  ...
  /*  to one and the same tautomeric group: it has been verified in
   * MarkTautomerGroups() */
  ...
  num_taut_endpoints =
      (0 != atom[n1_at].endpoint) +
      (0 != atom[n2_at].endpoint); /*  if both N atoms already are endpoints */
  /*
  if ( !(nMobile == 1 || num_taut_endpoints == 2) && !(nMobile>1 &&
  num_taut_endpoints >= 1) ) { return 0;
  }
  */
  ...
  if ((atom[n1_at].endpoint != atom[n2_at].endpoint || !atom[n1_at].endpoint)) {
    int nErr;
    nErr = bExistsAnyAltPath(pBNS, pBD, atom, num_atoms, n1_at, n2_at,
                             ALT_PATH_MODE_TAUTOM);
    if (nErr <= 0)
      return nErr;
  }
  ...
  /* i is a bond type of the last bond to at_n1, the first bond from at_n2 is
   * 2-i if i=1 or 2 */

  /* single bond at n1_at: it should have a mobile atom, n2_at should not */
  if ((i == BOND_SINGLE && ((!atom[n1_at].endpoint && !eif1.cDonor) ||
                            (!atom[n2_at].endpoint && !eif2.cAcceptor))) ||
      /* double bond at n1_at: it should not have a mobile atom, n2_at should */
      (i == BOND_DOUBLE && ((!atom[n1_at].endpoint && !eif1.cAcceptor) ||
                            (!atom[n2_at].endpoint && !eif2.cDonor)))) {
    return 0; /* bond pattern does not fit */
  }
