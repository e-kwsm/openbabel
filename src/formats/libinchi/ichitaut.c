...
int MoveEndpoint(inp_ATOM *at, S_CANDIDATE *s_candidate, AT_NUMB endpoint,
                 AT_NUMB *nTGroupNewNumbers, AT_NUMB *nTGroupPosition,
                 int nNewTGroupOrd, T_GROUP_INFO *t_group_info);

int FindAccessibleEndPoints(T_ENDPOINT *EndPoint, int *nNumEndPoints,
                            T_BONDPOS *BondPos, int *nNumBondPos,
                            struct BalancedNetworkStructure *pBNS,
                            struct BalancedNetworkData *pBD, inp_ATOM *at,
                            int num_atoms, C_GROUP_INFO *cgi, int taut_mode);
...
int GetNeutralRepsIfNeeded(AT_NUMB *pri, AT_NUMB *prj, inp_ATOM *at,
                           int num_atoms, T_ENDPOINT *EndPoint,
                           int nNumEndPoints, C_GROUP_INFO *cgi);
int bCanBeACPoint(inp_ATOM *at, S_CHAR cCharge, S_CHAR cChangeValence,
                  S_CHAR neutral_bonds_valence, S_CHAR neutral_valence,
                  S_CHAR nEndpointValence, S_CHAR *cChargeSubtype);
int CmpCCandidates(const void *a1, const void *a2);
int RegisterCPoints(C_GROUP *c_group, int *pnum_c, int max_num_c,
                    T_GROUP_INFO *t_group_info, int point1, int point2,
                    int ctype, inp_ATOM *at, int num_atoms);
int GetSaltChargeType(inp_ATOM *at, int at_no, T_GROUP_INFO *t_group_info,
                      int *s_subtype);
int GetOtherSaltChargeType(inp_ATOM *at, int at_no, T_GROUP_INFO *t_group_info,
                           int *s_subtype, int bAccept_O);
int MergeSaltTautGroupsBlind(inp_ATOM *at, int s_type, int num_atoms,
                             S_GROUP_INFO *s_group_info, int nNumCandidates,
                             T_GROUP_INFO *t_group_info,
                             C_GROUP_INFO *c_group_info,
                             struct BalancedNetworkStructure *pBNS);
int ConnectSaltTGroups2SuperTGroup(inp_ATOM *at, int num_atoms,
...
/*****************************************************************************/
int AddAtom2num(
    AT_RANK num[], inp_ATOM *atom, int at_no,
...
/*****************************************************************************/
void AddAtom2DA(
    AT_RANK num_DA[], inp_ATOM *atom, int at_no,
    int bSubtract) { /*  bSubtract: 0=> add, 1=>subtract, 2=> fill */
  inp_ATOM *at = atom + at_no;
...
int AddEndPoint(T_ENDPOINT *pEndPoint, inp_ATOM *at, int iat) {
  ...
  if (at[iat].endpoint) {
    ...
    for ( k = 0; k < T_NUM_ISOTOPIC; k ++ ) {
        pEndPoint->num[T_NUM_NO_ISOTOPIC+k] =
    at[iat].num_iso_H[NUM_H_ISOTOPES-k-1];
    }
    */
  }
  return 0;
...
int nGetEndpointInfo(inp_ATOM *atom, int iat, ENDPOINT_INFO *eif) {
  ...
  if (nEndpointValence <= atom[iat].valence)
    return 0; /* not an endpoint, for example >N(+)< or >N<  or >O(+)- or >O- or
                 >N- or -O- */
...
int nGetEndpointInfo_KET(inp_ATOM *atom, int iat, ENDPOINT_INFO *eif) {
  ...
  */
  ...
  if (!(nEndpointValence = get_endpoint_valence_KET(atom[iat].el_number)))
    return 0; /* not an endpoint; only O and C can be an endpoint for keto-enol
                 tautomerism */
  if (nEndpointValence <= atom[iat].valence)
    return 0; /* not an endpoint, for example >N(+)< or >N<  or >O(+)- or >O- or
                 >N- or -O- */
  ...
  if (atom[iat].charge == -1 || atom[iat].charge == 0) {
    ...
    eif->cNeutralBondsValence = nEndpointValence - nMobile;
    eif->cMoveableCharge = 0;
    eif->cKetoEnolCode = (nEndpointValence == 2)   ? 1
                         : (nEndpointValence == 4) ? 2
                                                   : 0;
    return nEndpointValence;
  } else if (atom[iat].c_point &&
             0 <= GetChargeType(atom, iat, &cChargeSubtype) &&
...
int RegisterEndPoints(T_GROUP_INFO *t_group_info,
  ...
  /* find min non-zero group number nLeastGroupNumber;
     ...
     if all EndPoint[i].nGroupNumber are equal and non-zero then exit: nothing
     to do.
  */
  ...
  for (i = j = k = 0; i < nNumEndPoints; i++) {
    ...
    j += (prev_group ==
          EndPoint[i]
              .nGroupNumber); /* count endpoints that belong to the 1st group */
    k += (prev_eqnum ==
          EndPoint[i].nEquNumber); /* count endpoints that belongo to a group
                                      equivalent to the 1st group */
    nNumZeroEqu +=
        !EndPoint[i].nEquNumber; /* count endpoints that have been processed by
  ...
  if (!nNumZeroEqu) {
    ...
    for (i = 0; i < nNumEndPoints; i++) {
      if ((group = EndPoint[i].nEquNumber) >= nNextGroupNumber) {
        /* replace fict. IDs assigned by FindAccessibleEndPoints() with new
         * t-group numbers */
        /* these fict. IDs have values = (num_atoms+1), (num_atoms+2),...; they
        ...
        if (j == nNumNewTGroups) {
          ...
          if (j == MAX_STACK_ARRAY_LEN &&
            ...
            nGroupNewNumber = (AT_NUMB *)inchi_malloc(
                nNumEndPoints * sizeof(nGroupNewNumber[0]));
            if (!nGroupNewNumber) {
              ret = -1;
              goto exit_function;
            }
            memcpy(nGroupNewNumber, nGroupNewNumberStackArray,
                   nNumNewTGroups * sizeof(nGroupNewNumber[0]));
          }
          /* save newly found fict. t-group ID to compare to the next values of
           * EndPoint[].nEquNumber  */
    ...
    /* EndPoint[] has NOT been processed by FindAccessibleEndPoints;
       all atoms and t-groups to which endpoints belong should be merged into a
       single t-group
     */
  ...
  /* At this point:
   *   EndPoint[i].nGroupNumber == 0   => the endpoint atom does not belong to a
   * t-group yet EndPoint[i].nGroupNumber  > 0   => current t-group ID of the
   * endpoint atom EndPoint[i].nEquNumber      -->    new ID of a tautomeric
   * group of this endpoint atom EndPoint[i].nAtomNumber     -->    number of
   * the endpoint atom
   */
  ...
  for (i = j = 0; i < nNumEndPoints; i++) {
    if ((group = EndPoint[i].nGroupNumber)) {
      ...
      if (j == nNumGroups) {
        /* discovered a new t-group number; store it together with its
         * nEquNumber */
        if (j == MAX_STACK_ARRAY_LEN) {
          if (nGroupNewNumber == nGroupNewNumberStackArray) {
            ...
            memcpy(nGroupNewNumber, nGroupNewNumberStackArray,
                   nNumGroups * sizeof(nGroupNewNumber[0]));
          }
          if (nGroupNumber == nGroupNumberStackArray) {
            nGroupNumber = (AT_NUMB *)inchi_malloc(nNumEndPoints *
                                                   sizeof(nGroupNumber[0]));
            if (!nGroupNumber) {
              ret = -1;
              goto exit_function;
            }
            memcpy(nGroupNumber, nGroupNumberStackArray,
                   nNumGroups * sizeof(nGroupNumber[0]));
          }
        }
      ...
      if (group >= nNextGroupNumber) {
        ...
        j = num_t + group -
            nNextGroupNumber; /* newly assigned IDs are contiguous */
      } else {
        /* old t-group */
      ...
      for (k = 0; k < (int)(sizeof(t_group->num) / sizeof(t_group->num[0]));
           k++)
        t_group[j].num[k] += EndPoint[i].num[k];
      for (k = 0;
           k < (int)(sizeof(t_group->num_DA) / sizeof(t_group->num_DA[0])); k++)
  ...
  if (!difference) {
    ret = 0; /* nothing to do. Not necessarily a program error: happens if all
                EndPoint[i].nGroupNumber==EndPoint[i].nEquNumber  */
    goto exit_function;
  }
  ...
  for (i = 0; i < nNumGroups; i++) {
    ...
    for (k = 0; k < (int)(sizeof(t_group->num_DA) / sizeof(t_group->num_DA[0]));
         k++)
      t_group[i2].num_DA[k] += t_group[i1].num_DA[k];
    t_group[i2].nNumEndpoints += t_group[i1].nNumEndpoints;
    num_t--;
    if (num_t > i1) {
      memmove(t_group + i1, t_group + i1 + 1,
              (num_t - i1) * sizeof(t_group[0]));
    }
  }
  ...
  if (nNumGroups) {
    ...
    if (nNextGroupNumber >= MAX_STACK_ARRAY_LEN) {
      nNewTgNumber = (AT_NUMB *)inchi_malloc((nNextGroupNumber + 1) *
                                             sizeof(*nNewTgNumber));
      if (!nNewTgNumber) {
        ret = -1;
    ...
    for (i = 0; i < num_atoms; i++) {
      if ((group = at[i].endpoint)) {
        if (!(at[i].endpoint = nNewTgNumber[group]) ||
            nNextGroupNumber <= nNewTgNumber[group]) {
          ret = -1; /* program error */
          goto exit_function;
        }
      }
  ...
  if (!t_group_info->tGroupNumber) {
    t_group_info->tGroupNumber = (AT_NUMB *)inchi_malloc(
        2 * max_num_t * sizeof(t_group_info->tGroupNumber[0]));
    if (!t_group_info->tGroupNumber) {
      ret = -1;
      goto exit_function;
    }
  ...
  memset(t_group_info->tGroupNumber, 0,
         2 * max_num_t * sizeof(t_group_info->tGroupNumber[0]));
  for (i = 0; i < num_t; i++) {
  ...
  if (pBNS &&
      (pBNS->tot_st_cap == pBNS->tot_st_flow || ALWAYS_ADD_TG_ON_THE_FLY)) {
    T_GROUP_INFO tgi;
    int ret_bns;
    ...
    tgi.bTautFlags |=
        (t_group_info->bTautFlags &
         TG_FLAG_KETO_ENOL_TAUT); /* needed in AddTGroups2BnStruct() */
#endif
...
int GetNeutralRepsIfNeeded(AT_NUMB *pri, AT_NUMB *prj, inp_ATOM *at,
  ...
  if ((c_point = at[ri].c_point) && (c_point == at[rj].c_point) &&
      (at[ri].charge == 1 || at[rj].charge == 1) && cgi &&
      cgi->num_c_groups > 0) {
    /* at[ri] and at[rj] belong to the same charge group, at least one is
     * charged */
    ...
    for (k = 0; k < cgi->num_c_groups; k++) {
      if (cgi->c_group[k].nGroupNumber == c_point) {
        ...
        if (cgi->c_group[k].num_CPoints - cgi->c_group[k].num[0] < 2) {
          /* Only one neutral in the c-group: we will not be able to neutralize
             both when looking for the alt path to discover the tautomerism.
          ...
          if ((endpoint = at[rj].endpoint)) {
            for (i = 0; i < nNumEndPoints; i++) {
              ...
              if (at[r].endpoint != endpoint)
                continue; /* at[r] does not belong to the same t-group as
                             at[*prj]; ignore the atom */
              if (!at[r].c_point) {
                rj = r; /* found a neutral t-group representative */
                break;
              }
              if (at[r].c_point != c_point && c_point == at[rj].c_point) {
                /* replace only once because of (c_point == at[rj].c_point)
                 * condition  */
                rj = r;
              }
            }
            if (rj == *prj /*&& at[ri].endpoint*/) {
              /* !!! "&& at[ri].endpoint": only between 2 t-groups 2004-02-27;
                  the change disabled due to undiscovered yet possibility of
                 ambiguity*/
              /* no replacement has been found in EndPoint[]; try all atoms in
               * the t-group */
              for (i = 0; i < num_atoms; i++) {
...
int FindAccessibleEndPoints(T_ENDPOINT *EndPoint, int *nNumEndPoints,
                            ...
                            struct BalancedNetworkData *pBD, inp_ATOM *at,
                            int num_atoms, C_GROUP_INFO *cgi, int taut_mode) {
  AT_NUMB nTGroupRepresenative[MAXVAL], nTGroupEqu[MAXVAL],
      nTGEndPointNo[MAXVAL], ri, rj;
  ...
  for (i = 0; i < *nNumEndPoints; i++) {
    nCurTGroupNumber = EndPoint[i].nEquNumber =
        EndPoint[i].nGroupNumber; /* initial equivalence */
    if (nCurTGroupNumber) {
  ...
  /* check whether each pair belongs to the same t-group and establish the
   * equivalence(s) */
  for (i = 0, nNumFoundEqu = 0; i < nNumDiffTGroupNumbers; i++) {
    for (j = i + 1; j < nNumDiffTGroupNumbers; j++) {
      ...
      GetNeutralRepsIfNeeded(&ri, &rj, at, num_atoms, EndPoint, *nNumEndPoints,
                             cgi);
  ...
  /* check whether each pair belongs to the same group and establish the
   * equivalence(s) */
  for (i = 0, nNumFoundEqu = 0; i < nNumTgroupNumbers; i++) {
    for (j = i + 1; j < nNumTgroupNumbers; j++) {
      if ((nTGroupEqu[i] != nTGroupEqu[j] &&
           (i >= nNumDiffTGroupNumbers || j >= nNumDiffTGroupNumbers)) ||
        ...
        rj = nTGroupRepresenative[j];
        /*------------------------------!!!---------------------------------------------
            Explanation why GetNeutralRepsIfNeeded() may need to be changed
         2004-02-27 The change has been disabled due to undiscovered yet
         possibility of ambiguity to search for neutral only among EndPoint[] in
         case taut-not_taut pairs
            ...
            Now testing: 2-3 tautomerism. If not commented out,
               GetNeutralRepsIfNeeded() would replace 2-3 test with 1-3 test
         because: o Charge group has only one neutral and both 2 and 3 belong to
         it, therefore we cannot neutralize both; search for neutral
         representative; o Since 1 and 2 belong to the same t-group and 1 is
         neutral, test 1-3 instead of 2-3. This breaks our condition: Test
         tautomeric H movement only between neutral atoms.
         -----------------------------------------------------------------------------*/
        GetNeutralRepsIfNeeded(&ri, &rj, at, num_atoms, EndPoint,
                               *nNumEndPoints, cgi);
        ...
        if (nErr <= 0)
          continue;
        if (nTGroupEqu[i] && nTGroupEqu[j]) {
          /* found equivalence of two t-groups; at least one of them must be a
           * new one */
          nCurTGroupNumber = inchi_min(nTGroupEqu[i], nTGroupEqu[j]);
          nMaxTGroupNumber = inchi_max(nTGroupEqu[i], nTGroupEqu[j]);
          ...
          nTGroupEqu[i] = nTGroupEqu[j] =
              nMaxEquNumber; /* assign a fict. ID to establish equivalence */
  ...
  */
  for (i = 0, j = 0; i < *nNumEndPoints; i++) {
    if (EndPoint[i].nEquNumber) {
      ...
      if (nNumFoundEqu <=
          1) { /* one time it is equal to itself when i == k above */
        /* if EndPoint[i] is not "equivalent" to any other EndPoint then ignore
         * it */
        continue;
      }
#endif
      if (i != j) { /* save endpoints that are found to be connected to other
                       endpoints by alt paths */
  ...
  if (j == 1) {
    ...
    for (i = 0, k = 0; i < at[BondPos[0].nAtomNumber].valence; i++) {
      k += (i != BondPos[0].neighbor_index &&
            BOND_TAUTOM ==
                (at[BondPos[0].nAtomNumber].bond_type[i] & ~BOND_MARK_ALL));
    }
    if (!k) {
      j = 0;
    }
...
CHARGE_TYPE CType[] = {
    {"N\0", 1, 3, 3, 1, 0, 0}, {"P\0", 1, 3, 3, 1, 1, 0},
...
#define C_SUBTYPE_CHARGED_H_ACCEPT_p_DONOR                                     \
  (C_SUBTYPE_CHARGED | C_SUBTYPE_H_ACCEPT | C_SUBTYPE_p_DONOR)
#define C_SUBTYPE_CHARGED_H_DONOR                                              \
  (C_SUBTYPE_CHARGED | C_SUBTYPE_H_DONOR | C_SUBTYPE_p_DONOR)
/* neutral */
...
#define C_SUBTYPE_NEUTRAL_H_ACCEPT_p_ACCEPT                                    \
  (C_SUBTYPE_NEUTRAL | C_SUBTYPE_H_ACCEPT | C_SUBTYPE_p_ACCEPT)
...
int bCanBeACPoint(inp_ATOM *at, S_CHAR cCharge, S_CHAR cChangeValence,
  ...
  if (at->charge == cCharge && at->valence == at->chem_bonds_valence &&
    ...
    nChangeValence =
        at->charge * cChangeValence; /* +1 or -1; currently only +1 */
    nBondsValence = at->chem_bonds_valence + at->num_H;
    if (nBondsValence == neutral_bonds_valence + nChangeValence &&
        nEndpointValence) {
      *cChargeSubtype =
          C_SUBTYPE_CHARGED_p_DONOR; /* ignore Phosphorus p-donors for now */
    }
    return 0;
    ...
    if (nBondsValence ==
      ...
      if (nNumBonds == neutral_valence - 1) {
        ...
        if (nEndpointValence) {
          *cChargeSubtype = at->num_H ? C_SUBTYPE_CHARGED_H_ACCEPT_p_DONOR
    ...
    nBondsValence = at->chem_bonds_valence + at->num_H +
                    bNegCharge; /* add fict. bonds to (-) 2004-02-24*/
    nNumBonds = at->valence + at->num_H +
                bNegCharge; /* add fict. bonds to (-) 2004-02-24*/
    if (nBondsValence == neutral_bonds_valence) {
      if (nNumBonds == neutral_valence) {
        ...
        if (at->valence == neutral_valence || !nEndpointValence) {
          ...
          *cChargeSubtype = at->num_H ? C_SUBTYPE_NEUTRAL_H_DONOR
  ...
  *cChargeSubtype = 0;
  ...
  if (abs(at->charge) == 1) {
    for (i = 0; i < at->valence; i++) {
      ...
      if (abs(atom[n].charge + at->charge) < abs(atom[n].charge - at->charge) &&
          !atom[n].endpoint) {
  ...
  for (i = 0; i < NUM_C_TYPES; i++) {
    if (!strcmp(at->elname, CType[i].elname) &&
        (!CType[i].num_bonds ||
         (CType[i].num_bonds == at->valence && at->nNumAtInRingSystem >= 5))) {
      nEndpointValence = (S_CHAR)get_endpoint_valence(at->el_number);
      if (bCanBeACPoint(at, CType[i].charge, CType[i].cChangeValence,
                        CType[i].neutral_bonds_valence,
                        CType[i].neutral_valence, nEndpointValence,
                        cChargeSubtype)) {
...
int RegisterCPoints(C_GROUP *c_group, int *pnum_c, int max_num_c,
  ...
  if (at[point1].c_point == at[point2].c_point) {
    ...
    if (num_c < max_num_c) {
      c_group[num_c].num[0] =
          CHARGED_CPOINT(at, point1) + CHARGED_CPOINT(at, point2);
      ...
      nGroupNumber++;
      c_group[num_c].nGroupNumber = at[point1].c_point = at[point2].c_point =
          nGroupNumber;
      *pnum_c = num_c + 1;
      /* count protons */
      if (at[point1].num_H) {
        ...
        c_group[num_c].num[1]++;
      } else if ((at[point1].endpoint || at[point2].endpoint) && t_group_info &&
                 t_group_info->t_group && t_group_info->num_t_groups) {
        /* !!! add later !!! */
      }
  ...
  if (!at[point1].c_point) {
    ...
    num_c--;
    if (num_c > i2) {
      memmove(c_group + i2, c_group + i2 + 1,
              (num_c - i2) * sizeof(c_group[0]));
    }
    *pnum_c = num_c;
    /* renumber c-groups */
    for (i = 0; i < num_c; i++) {
...
/*****************************************************************************/
int MarkChargeGroups(inp_ATOM *at, int num_atoms, C_GROUP_INFO *c_group_info,
                     T_GROUP_INFO *t_group_info,
                     struct BalancedNetworkStructure *pBNS,
                     struct BalancedNetworkData *pBD) {
  int nNumChanges = 0;

  if (c_group_info && c_group_info->c_candidate &&
      c_group_info->max_num_candidates > 0) {
    ...
    if (nNumCandidates == -1) {
      nNumCandidates =
          0; /* 2004-02-26 they could appear after t-group discovery */
             /*return 0;*/
    }
    if (nNumCandidates == 0) {
    ...
    */
    ...
    while (i1 < nNumCandidates) {
      ...
      for (;
           i1 < nNumCandidates && (c_candidate[i1].subtype & C_SUBTYPE_NEUTRAL);
           i1++)
        ;
      ...
      for (i3 = i2;
           i3 < nNumCandidates && c_candidate[i3].type == c_candidate[i1].type;
           i3++)
        ;
      ...
      for (i = i1; i < i2; i++) {
        ...
        for (j = i2; j < i3; j++) {
          /* check alt path at[iat1]=-=-...-at[iat2]; at[iat1] is charged,
           * at[iat2] is neutral */
          num_tested++;
          iat2 = c_candidate[j].atnumber;
          if (at[iat1].c_point && at[iat1].c_point == at[iat2].c_point)
            continue;
          ret = bExistsAltPath(pBNS, pBD, NULL, at, num_atoms, iat1, iat2,
          ...
          if (ret & 1) {
            ...
            nNumChanges += (ret & 2);
            ret = RegisterCPoints(
                c_group_info->c_group, &c_group_info->num_c_groups,
                c_group_info->max_num_c_groups, t_group_info, iat1, iat2,
                c_candidate[i1].type, at, num_atoms);
            if (IS_BNS_ERROR(ret)) {
              return ret;
            }
            if (nDelta) {
  ...
  quick_exit:
    if (c_group_info->num_candidates == 0) {
      ...
      c_group_info->num_candidates =
          num_tested ? nNumCandidates : -1; /* no candidate exists */
    }
  }
  return nNumChanges;
}
...
int GetSaltChargeType(inp_ATOM *at, int at_no, T_GROUP_INFO *t_group_info,
  ...
  /*
     ...
     O-atom should be:
       - a terminal atom
       - connected to unsaturated, uncharged, non-radical atom C that has
     chemical valence 4: H-donors:             =CH-OH, =C(-X)-OH possible
     H-acceptors: -CH=O, >C=O H-acceptors are true if O is tautomeric
  */
  int iC, tg, i, type;
  ...
  *s_subtype = 0; /* initialize the output */
  ...
  if (at[at_no].valence != 1 ||
      ...
      at[at_no].charge < -1 || (at[at_no].charge > 0 && !at[at_no].c_point)) {
    return -1;
  }
  ...
  if (at[iC].el_number != el_number_C ||
      at[iC].chem_bonds_valence + at[iC].num_H !=
          4 || /* allow =C(H)-OH or -C(H)=O */
      at[iC].charge ||
...
#else
  if (at[iC].el_number != el_number_C || at[iC].num_H ||
      at[iC].chem_bonds_valence != 4 || /* allow only no H on C */
      at[iC].charge || at[iC].radical && at[iC].radical != RADICAL_SINGLET ||
      at[iC].valence == at[iC].chem_bonds_valence) {
    return -1; /* oxigen is connected to a wrong atom */
  }
#endif
  if ((tg = at[at_no].endpoint) && t_group_info && t_group_info->t_group) {
    ...
    for (i = 0; i < t_group_info->num_t_groups; i++) {
      if (tg == t_group_info->t_group[i].nGroupNumber) {
        /*
        t_group_info->t_group[i].num[0] = number of attached H-atoms and
        negative charges t_group_info->t_group[i].num[1] = number of attached
        negative charges
        */
        ...
        *s_subtype |=
            SALT_ACCEPTOR; /* there is always an acceptor in a t-group */
        return type;
      }
    }
  ...
  if (at[at_no].charge == 1 && at[at_no].c_point &&
      at[at_no].chem_bonds_valence == 2 && at[at_no].num_H) {
    *s_subtype |= SALT_DONOR_H; /* has H */
  }
  return type;
}
...
int GetOtherSaltChargeType(inp_ATOM *at, int at_no, T_GROUP_INFO *t_group_info,
  ...
  /*
     ...
     if eif.cAcceptor then the bond between the atom and the centerpoint must be
     possibly double if eif.cAcceptor then the bond must be possibly single
     Donors that belong to a t-group are also acceptors


  */
  int tg, i, j, type, endpoint_valence, num_centerpoints, bond_type,
      centerpoint;
  ENDPOINT_INFO eif;
  /* one-time initialization */
  ...
  *s_subtype = 0; /* initialize the output */
  if (!bAccept_O /* only N */ && (at[at_no].el_number == el_number_O ||
                                  at[at_no].el_number == el_number_S ||
                                  at[at_no].el_number == el_number_Se ||
                                  at[at_no].el_number == el_number_Te)) {
  ...
  if (!(endpoint_valence = nGetEndpointInfo(at, at_no, &eif))) {
    ...
    /* at[at_no] is not not in a tautomeric group; use eif previously filled out
     * by nGetEndpointInfo */
    ...
    for (j = 0; j < at[at_no].valence; j++) {
      ...
      if (((eif.cAcceptor &&
            (bond_type == BOND_DOUBLE ||
             ...
             bond_type == BOND_ALT12NS || bond_type == BOND_TAUTOM)) ||
           (eif.cDonor &&
          ...
          (at[centerpoint].chem_bonds_valence > at[centerpoint].valence ||
           ...
           (at[centerpoint].chem_bonds_valence == at[centerpoint].valence && (at[centerpoint]
                                                                                  .endpoint ||
                                                                              at[centerpoint]
                                                                                  .c_point)) /* tautomerism or charge may increment at[centerpoint].chem_bonds_valence*/) &&
...
int GetOtherSaltType(inp_ATOM *at, int at_no, int *s_subtype) {
  ...
  *s_subtype = 0; /* initialize the output */
  ...
  if (!(endpoint_valence = nGetEndpointInfo(at, at_no, &eif)) ||
      (eif.cMoveableCharge && !at[at_no].c_point) || !eif.cDonor ||
      eif.cAcceptor) {
    ...
    if (at[centerpoint].el_number != el_number_C || at[centerpoint].charge ||
        (at[centerpoint].radical &&
         at[centerpoint].radical != RADICAL_SINGLET) ||
...
/********************************************************************************************************/
/* new version: merge all, check alt paths, then unmerge unreachable O-atoms if
 * any */
/* Check for oxygen negative charge-H tautomerism (Salts)
   allowed long-range tautomerism; more than one H or (-) can be moved, for
   example: HO-C=C-O(-)         O=C-C=O
     /   \              /   \
    R     R            R     R
    |     |       =>   |     |
    R'    R'           R'    R'
    ...
    To check:
                          ...
                          |             |
     -add all possible HO-C=, O=C, (-)O-C= (including all containing O t-groups)
   into one t-group; -temporarily disconnect one of previously not belonging to
   any t-group O-atoms from the one t-group; -find whether there is an alt path
   allowing H or (-) to migrate from the temp. disconnected O to any one left in
   the group. If the alt path does not exist then the temp. disconnected atom
   does not participate in the H/(-) migrartion and it will be
   unmarked/unmerged.
...
int comp_candidates(const void *a1, const void *a2) {
  ...
  if (!s1->endpoint && s2->endpoint)
    return 1; /* tautomeric goes first; only tautomeric may be disabled */
  if (s1->endpoint && s2->endpoint &&
      (ret = (int)s1->endpoint - (int)s2->endpoint)) {
    return ret;
  }
...
/*****************************************************************************/
int MarkSaltChargeGroups2(inp_ATOM *at, int num_atoms,
                          S_GROUP_INFO *s_group_info,
                          T_GROUP_INFO *t_group_info,
                          C_GROUP_INFO *c_group_info,
                          struct BalancedNetworkStructure *pBNS,
...
#define DONOR_PAIR 2
  ...
  int nNumChanges = 0, nNumOtherChanges = 0, nNumAcidicChanges = 0,
      nTotNumChanges = 0;
  S_CHAR *cPair = NULL;
  T_ENDPOINT *EndPoint = NULL;
  if (s_group_info && s_group_info->s_candidate &&
      s_group_info->max_num_candidates > 0) {
    int i, j, i1, j1;
    ...
    int ret, nDelta;
    int bHardAddedRemovedProtons =
        t_group_info &&
        (t_group_info->tni.bNormalizationFlags & FLAG_FORCE_SALT_TAUT);

    int s_subtype_all = 0;
    int nDonorPairs, nAcceptorPairs, nCurDonorPairs, nCurAcceptorPairs,
        bAlreadyTested;
    /*
            ENDPOINT_INFO    eif;
    */
    ...
    /*************************************************************************/
    for (i = 0, nNumCandidates = nNumOtherCandidates = nNumPOnlyCandidates = 0;
         i < num_atoms; i++) {
      if (0 == (s_type = GetSaltChargeType(at, i, t_group_info, &s_subtype)) ||
          /* -C=O or =C-OH, O = S, Se, Te */
          1 == (s_type = GetOtherSaltChargeType(at, i, t_group_info, &s_subtype,
                                                1 /* bAccept_O*/)) ||
          /* =Z-MH or -Z=M, Z = centerpoint, M = endpoint, other than above */
          2 == (s_type = GetOtherSaltType(at, i, &s_subtype)) ||
          (bHardAddedRemovedProtons &&
           4 == (s_type = bIsHardRemHCandidate(at, i, &s_subtype)))
          /* >C-SH, >C-S(-); S=S,Se,Te */
      ) {
    ...
    if (nNumCandidates <=
            1 || /* TG_FLAG_ALLOW_NO_NEGTV_O <=> CHARGED_SALTS_ONLY=0 */
        ...
        (((t_group_info->bTautFlags & TG_FLAG_ALLOW_NO_NEGTV_O) ||
          ...
          (t_group_info->tni.bNormalizationFlags & FLAG_FORCE_SALT_TAUT))
             ...
             : (!(s_subtype_all & SALT_DONOR_Neg) ||
                nNumOtherCandidates == nNumCandidates))) {
    ...
    /************************************************************************************/
    /* Mark redundant candidates so that only one candidate from one t-group is
     * left in */
    /************************************************************************************/
      ...
      if (s_candidate[i].endpoint) {
        for (j = i - 1; 0 <= j; j--) {
          if (s_candidate[i].endpoint == s_candidate[j].endpoint) {
            s_candidate[i].type -=
                DISABLE_CANDIDATE; /* disable subsequent redundant */
            nNumLeftCandidates++;  /* count rejected */
            break;
          }
        }
    ...
    nNumLeftCandidates =
        nNumCandidates -
        nNumLeftCandidates; /* subtract num. rejected from the total */
    ...
    cPair = (S_CHAR *)inchi_calloc(nNumLeftCandidates * nNumLeftCandidates,
                                   sizeof(cPair[0]));
    if (!cPair) {
    ...
    for (i = 0; i < nNumLeftCandidates; i++) {
      ...
      for (j = 0; j <= i; j++) {
        ...
        if ((s_candidate[i].subtype & SALT_ACCEPTOR) &&
            (s_candidate[j].subtype & SALT_ACCEPTOR) &&
            (ret = bExistsAltPath(
                 pBNS, pBD, NULL, at, num_atoms, s_candidate[i].atnumber,
          ...
          if (ret & 1) {
            ...
            /*nNumChanges += (ret & 2);*/
            if (nDelta) {
              /* alt path unleashed previously localized radicals and they
               * annihilated */
              nNumChanges = 0;
        ...
        if ((s_candidate[i].subtype & SALT_DONOR) &&
            (s_candidate[j].subtype & SALT_DONOR) &&
            (ret = bExistsAltPath(
                 pBNS, pBD, NULL, at, num_atoms, s_candidate[i].atnumber,
        ...
        if (cPAIR(i, j) & ACCEPTOR_PAIR) {
          ...
          if (nDonorPairs) {
            ...
            for (i1 = 0; i1 < i; i1++) {
              for (j1 = 0; j1 <= i1; j1++) {
                /* here always j1 < i && i1 < i therefore we do not compare i to
                 * i1 or j1 */
  ...
  bFound2Pairs:
    ...
    for (i = 0; i < nNumLeftCandidates; i++) {
      ...
      for (j = 0; j <= i; j++) {
        ...
        if ((bAlreadyTested && (cPAIR(i, j) & ACCEPTOR_PAIR)) ||
            !bAlreadyTested) {
    ...
    if (nNumMarkedCandidates) {
      EndPoint =
          (T_ENDPOINT *)inchi_calloc(nNumMarkedCandidates, sizeof(EndPoint[0]));
      if (!EndPoint) {
      ...
      for (i = 0, j = 0; i < nNumLeftCandidates; i++) {
        if (s_candidate[i].subtype & SALT_SELECTED) {
          ...
          if (j < nNumMarkedCandidates) {
            i1 = s_candidate[i].atnumber; /* save a representative of the
                                             t-group to be created */
      ...
      ret = RegisterEndPoints(t_group_info, EndPoint, nNumMarkedCandidates, at,
      ...
      for (i = 0, bTGroupHasNegativeChargesOnly = 0;
           i < t_group_info->num_t_groups; i++) {
        if (t_group_info->t_group[i].nGroupNumber == at[i1].endpoint &&
            t_group_info->t_group[i].num[0] ==
                t_group_info->t_group[i].num[1]) {
...
quick_exit:
  ...
  return nTotNumChanges; /* 0=>no changes, 1=>new salt tautomerism found,
                            2=>only new charge tautomerism found */
...
typedef struct tagTGroupData {
  S_SHORT nGroupNumber; /* t-group number from
                           t_group_info->t_group->nGroupNumber */
  ...
  S_SHORT nAccepM; /* number of endpoint-acceptors that have negative charge
                      (Minus) */
  S_SHORT nAccepH; /* number of endpoint-acceptors that have H and no negative
                      charge */
  S_SHORT nAccep0; /* number of endpoint-acceptors that have no H and no
                      negative charge */
...
/*****************************************************************************/
int MarkSaltChargeGroups(inp_ATOM *at, int num_atoms,
                         S_GROUP_INFO *s_group_info, T_GROUP_INFO *t_group_info,
                         C_GROUP_INFO *c_group_info,
                         struct BalancedNetworkStructure *pBNS,
                         struct BalancedNetworkData *pBD) {
  ...
  if (s_group_info && s_group_info->s_candidate &&
    ...
    for (i = 0, nGroupNumber = 0; i < t_group_info->num_t_groups; i++) {
      if (nGroupNumber < t_group_info->t_group[i].nGroupNumber) {
        nGroupNumber =
            t_group_info->t_group[i].nGroupNumber; /* max. t-group number */
      }
    }
#if (MAX_LOCAL_TGNUM > 0)
    /* prepare memory */
    if (nGroupNumber >= MAX_LOCAL_TGNUM) {
      if (!(ptgData = (TGroupData *)inchi_calloc(nGroupNumber + 1,
                                                 sizeof(TGroupData)))) {
        err = BNS_OUT_OF_RAM;
        goto quick_exit;
      }
    ...
    if (nNumCandidates == 0) {
      for (i = 0,
          nNumCandidates = nNumOtherCandidates = nNumPOnlyCandidates = 0;
           i < num_atoms; i++) {
        if (0 ==
                (s_type = GetSaltChargeType(at, i, t_group_info, &s_subtype)) ||
        /* -C=O or =C-OH, O = S, Se, Te */
#if (INCL_NON_SALT_CANDIDATATES == 1)
            1 == (s_type = GetOtherSaltChargeType(at, i, t_group_info,
                                                  &s_subtype, 1)) ||
        /* =Z-MH or -Z=M, Z = centerpoint, M = endpoint, other than above */
#endif
            2 == (s_type = GetOtherSaltType(at, i, &s_subtype))
      ...
      if (cond2a || cond2b || cond2c)
        ...
        cond2 = !(s_subtype_all & SALT_DONOR_Neg) ||
                nNumOtherCandidates == nNumCandidates;
...
#if (INCL_NON_SALT_CANDIDATATES == 1)
            || 0 < (s_type = GetOtherSaltChargeType(
                        at, i1, t_group_info, &s_subtype, 1 /* bAccept_O*/))
#endif
        ) {
          s_candidate[nNumCandidates].type = s_type;
          s_candidate[nNumCandidates].subtype = s_subtype;
    ...
    /* Look for alt paths connecting:
       SALT_DONOR_Neg to SALT_ACCEPTOR  : long distance migration of negative
       charges SALT_DONOR_H   to SALT_ACCEPTOR  : long distance migration of
       H-atoms
    */
    do {
      nNumChanges = 0;
      for (i1 = 0; i1 < nNumCandidates; i1++) {
        ...
        for (i2 = i1 + 1; i2 < nNumCandidates; i2++) {
          /* prev. approach: do not test if both candidates are not "salt-type".
          Disabled 2004-03-18 if ( s_candidate[i1].type && s_candidate[i2].type
          ) continue;
          */
          ...
          for (j = 0; j < 2; j++) {
            ...
            if ((s_candidate[ii1].subtype & (SALT_DONOR_Neg | SALT_DONOR_H)) &&
              ...
              ret = bExistsAltPath(pBNS, pBD, NULL, at, num_atoms, jj2, jj1,
              ...
              if (ret & 1) {
                ...
                ret = RegisterEndPoints(t_group_info, EndPoint, 2, at,
                                        num_atoms, c_group_info, pBNS);
                if (ret == -1) {
                  ret = BNS_PROGRAM_ERR;
                }
  ...
  quick_exit:
    if (!err) {
      nTotNumChanges +=
          nNumChanges; /* nNumChanges != 0 only in case of 'goto quick_exit' */
      if (s_group_info->num_candidates == 0) {
        /* first time: initialize */
        s_group_info->num_candidates =
            num_tested ? nNumCandidates : -1; /* no candidate exists */
      }
    } else {
      nTotNumChanges = err;
...
int MarkSaltChargeGroups(inp_ATOM *at, int num_atoms,
  ...
  if (s_group_info && s_group_info->s_candidate &&
    ...
    if (nNumCandidates <= -1 || !t_group_info || !t_group_info->t_group) {
      return 0;
    } else if (nNumCandidates == 0) {
      for (i = 0, nNumCandidates = nNumOtherCandidates = 0; i < num_atoms;
           i++) {
        if (0 <=
            (s_type = GetSaltChargeType(at, i, t_group_info, &s_subtype))) {
          if (nNumCandidates >= nMaxNumCandidates) {
          ...
          if (0 < (s_type = GetOtherSaltChargeType(
                       at, i, t_group_info, &s_subtype, 1 /* bAccept_O*/))) {
            if (nNumCandidates >= nMaxNumCandidates) {
              return BNS_VERT_EDGE_OVFL;
            }
            s_candidate[nNumCandidates].atnumber = i;
              ...
              if (ret & 1) {
                ...
                for (i = 0; i < 2; i++) {
                  ...
                  if (at[jj].endpoint) {
                    ...
                    for ( k = 0; k < T_NUM_ISOTOPIC; k ++ ) {
                        EndPoint[i].num[T_NUM_NO_ISOTOPIC+k] =
                    at[jj].num_iso_H[NUM_H_ISOTOPES-k-1];
                    }
                    */
                  }
                }
...
int MergeSaltTautGroups(inp_ATOM *at, int num_atoms, S_GROUP_INFO *s_group_info,
                        ...
                        struct BalancedNetworkStructure *pBNS) {
  /* count candidates to be connected: exclude pure donors that do not belong to
   * any t-group */
  ...
  T_ENDPOINT EndPointStackArray[MAX_STACK_ARRAY_LEN]; /* will be reallocated if
                                                         too short */
  T_ENDPOINT *EndPoint = EndPointStackArray;

  if (!s_group_info ||
      !s_group_info->s_candidate || /*s_group_info->num_candidates <= 0 ||*/
  ...
  /* collect tautomeric acidic O and previously non-tautomeric C-OH, C-SH,
   * C-O(-), C-S(-)  */
  ...
  if (1 || (s_group_info->num_candidates < 0)) {
    ...
    for (i = 0, nNumCandidates = nNumCandidates2 = 0; i < num_atoms; i++) {
      ...
      if (0 == (s_type = GetSaltChargeType(at, i, t_group_info, &s_subtype)) ||
          ...
          1 == (s_type = GetOtherSaltChargeType(at, i, t_group_info, &s_subtype,
                                                1 /* bAccept_O*/)) ||
          /* =Z-MH or -Z=M, Z = centerpoint, M = endpoint, other than above. M
             may be N */
          ...
          (bHasAcidicHydrogen(at, i) &&
           ((s_type = 3), (s_subtype = SALT_p_DONOR))) ||
          (bHasAcidicMinus(at, i) &&
           ((s_type = 3), (s_subtype = SALT_p_ACCEPTOR)))) {
    ...
    if ((t_group_info->bTautFlags & TG_FLAG_ALLOW_NO_NEGTV_O) ||
      ...
      if (nNumCandidates <= 1 || ((!(s_subtype_all & SALT_DONOR_Neg2) ||
                                   !(s_subtype_all & SALT_DONOR_H2)) &&
  ...
  for (i = 0; i < s_group_info->num_candidates; i++) {
    ...
    if ((s_group_info->s_candidate[i].subtype & SALT_ACCEPTOR) &&
        !at[iat].endpoint) {
  ...
  if (nMinNumEndpoints > MAX_STACK_ARRAY_LEN) {
    if (!(EndPoint = (T_ENDPOINT *)inchi_calloc(nMinNumEndpoints,
                                                sizeof(EndPoint[0])))) {
  ...
  ret = RegisterEndPoints(t_group_info, EndPoint, j, at, num_atoms,
                          c_group_info, pBNS);
  if (ret == -1) {
    ret = BNS_PROGRAM_ERR;
  }
...
int MakeIsotopicHGroup(inp_ATOM *at, int num_atoms, S_GROUP_INFO *s_group_info,
  ...
  if (1 || (s_group_info->num_candidates < 0)) {
    int s_type, s_subtype;
    S_CANDIDATE *s_candidate = s_group_info->s_candidate;
    for (i = 0, nNumCandidates = nNumNonTautCandidates = 0; i < num_atoms;
         i++) {
      s_subtype = 0;
      s_type = 0;
      if (at[i].endpoint) {
        if ((tg = t_group_info->tGroupNumber[at[i].endpoint]) &&
          ...
          bHasH = (int)t_group_info->t_group[tg].num[0] -
                  (int)t_group_info->t_group[tg].num[1];
        } else {
      ...
      if ((bHasH && at[i].endpoint) || /* tautomeric atoms */
          ...
          (bHasH &&
           (0 ==
            ...
            /*(t_group_info->tni.bNormalizationFlags & FLAG_FORCE_SALT_TAUT)
               &&*/
            1 == (s_type = GetOtherSaltChargeType(
                      at, i, t_group_info, &s_subtype, 1 /* bAccept_O*/)) ||
            /* =Z-MH or -Z=M, Z = centerpoint, M = endpoint, other than above. M
               may be N */
            ...
            (bHasAcidicMinus(at, i) &&
             ((s_type = 3), (s_subtype = SALT_p_ACCEPTOR))) ||
            (bHasOtherExchangableH(at, i) &&
             ((s_type = 3), (s_subtype = SALT_DONOR_H)))))
    ...
    if (nNumCandidates > 0) {
      t_group_info->nIsotopicEndpointAtomNumber = (AT_NUMB *)inchi_calloc(
          nNumNonTautCandidates + 1,
...
+---+-------------------------------------------------------------------------+
  ...
  *) Cannot be a tautomeric endpoint
  ...
  Other possibilities:  -NH3(+)  >NH2(+)  >N(+)<  cannot be H-tautomeric
endpoints.
...
/***********************************************************************************/
/*   MarkTautomerGroups: do not identify positively charged N as endpoints for
 * now */
/***********************************************************************************/
int MarkTautomerGroups(inp_ATOM *at, int num_atoms, T_GROUP_INFO *t_group_info,
                       C_GROUP_INFO *c_group_info,
                       struct BalancedNetworkStructure *pBNS,
                       struct BalancedNetworkData *pBD) {
  int i, j, k, m, endpoint_valence, centerpoint, endpoint, bond_type, nMobile,
      num_changes = 0, tot_changes = 0;
  ...
  int nErr = 0;
#define ALLOWED_EDGE(PBNS, IAT, IBOND)                                         \
  (!(PBNS) || !(PBNS)->edge || !(PBNS)->vert ||                                \
   !(PBNS)->edge[(PBNS)->vert[IAT].iedge[IBOND]].forbidden)
#define ACTUAL_ORDER(PBNS, IAT, IBOND, BTYPE)                                  \
  (((PBNS) && (PBNS)->edge && (PBNS)->vert &&                                  \
    ((BTYPE) == BOND_ALT_123 || (BTYPE) == BOND_ALT_13 ||                      \
     (BTYPE) == BOND_ALT_23))                                                  \
       ? (PBNS)->edge[(PBNS)->vert[IAT].iedge[IBOND]].flow + BOND_TYPE_SINGLE  \
       : (BTYPE))
  ...
  if (!t_group_info->t_group && !t_group_info->max_num_t_groups) {
    INCHI_MODE bTautFlags =
        t_group_info->bTautFlags; /*  save initial setting */
    INCHI_MODE bTautFlagsDone =
        t_group_info->bTautFlagsDone; /*  save previous findings, if any */
    ...
    t_group_info->bIgnoreIsotopic =
        bIgnoreIsotopic; /*  restore initial setting */
    ...
    if (!(t_group_info->t_group = (T_GROUP *)inchi_calloc(
              t_group_info->max_num_t_groups, sizeof(t_group[0])))) {
  ...
  pnum_t = &t_group_info
                ->num_t_groups; /*  number of found tautomer endpoint groups */
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if ((endpoint_valence = nGetEndpointInfo(at, i, &eif1))) {
      ...
      for (j = 0; j < at[i].valence; j++) {
        ...
        if ((bond_type == BOND_DOUBLE || bond_type == BOND_ALTERN ||
             bond_type == BOND_ALT12NS || bond_type == BOND_TAUTOM) &&
            is_centerpoint_elem(at[centerpoint].el_number) &&
          ...
          nGroupNumber = (AT_NUMB)
              num_atoms;    /*  greater than any tautomeric group number */
          bDiffGroups = -1; /*  ignore the first difference */
          nNumDonor = nNumAcceptor = 0;
          for (k = 0, nNumEndPoints = 0, nNumBondPos = 0;
               k < at[centerpoint].valence; k++) {
...
#endif
            bTautBond = bNonTautBond = bAltBond = bPossiblyEndpoint = 0;
...
#if (REPLACE_ALT_WITH_TAUT == 1)
              bAltBond =
                  (bond_type == BOND_ALTERN || bond_type == BOND_ALT12NS);
#endif
            ...
            nMobile = AddAtom2num(EndPoint[nNumEndPoints].num, at, endpoint,
                                  2); /* fill out */
            AddAtom2DA(EndPoint[nNumEndPoints].num_DA, at, endpoint, 2);
            /* --- why is isitopic info missing ? -- see below
            nMobile  = EndPoint[nNumEndPoints].num[1] = (at[endpoint].charge ==
            -1); nMobile  = EndPoint[nNumEndPoints].num[0] = at[endpoint].num_H
            + nMobile;
            */
            if (bNonTautBond) {
              m = (bond_type == BOND_SINGLE &&
                   (nMobile || at[endpoint].endpoint));
              nNumDonor += m;
              bPossiblyEndpoint += m;
            ...
            if (!bPossiblyEndpoint)
              continue;
            EndPoint[nNumEndPoints].nGroupNumber =
                at[endpoint]
                    .endpoint; /* =0 if it is an endpoint for the 1st time */
            ...
            if (bNonTautBond || bAltBond) {
              ...
              BondPos[nNumBondPos].neighbor_index =
                  (AT_NUMB)k; /* bond ordering number; used to change bonds to
                                 tautomeric only  */
              nNumBondPos++;
#if (REPLACE_ALT_WITH_TAUT != 1)
            }
#endif
            /*  mobile group is possible if (a) the endpoint has a mobile group
             * or */
            /*                              (b) the centerpoint is adjacent to
             * another endpoint */
            nNumPossibleMobile += (nMobile > 0 || at[endpoint].endpoint);
            nNumEndPoints++;
          }
          if (nNumEndPoints > 1 && nNumPossibleMobile && nNumDonor &&
              nNumAcceptor) {
            /*
             * a tautomeric group has been found
             *
             * at this point:
             * nGroupNumber = 0 if all endpoints belong to a newly discovered
             * tautomeric group bDiffGroups  > 0 if at least 2 tautomeric groups
             * are to be merged (one of them can be new) case (nGroupNumber != 0
             * && bDiffGroups = 0 ) ignored because all endpoints belong to the
             * same known t-group case (nGroupNumber != 0 && bDiffGroups < 0 )
             * cannot happen
             */

            nErr = FindAccessibleEndPoints(
                EndPoint, &nNumEndPoints, BondPos, &nNumBondPos, pBNS, pBD, at,
                num_atoms, c_group_info, ALT_PATH_MODE_TAUTOM);
            if (IS_BNS_ERROR(nErr)) {
              return nErr;
            }
            nErr = 0;
            ...
            if (nNumEndPoints > 0) {
              if (!nGroupNumber || bDiffGroups > 0) {
                num_changes =
                    RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints, at,
                                      num_atoms, c_group_info, pBNS);
                if (num_changes == -1) {
      ...
      if ((endpoint_valence = nGetEndpointInfo_KET(at, i, &eif1))) {
        ...
        for (j = 0; j < at[i].valence; j++) {
          ...
          if ((bond_type == BOND_DOUBLE || bond_type == BOND_ALTERN ||
            ...
            int num_C = 0;
            /*  test a centerpoint candidate. */
            /*  find all endpoints including at[i] and store them into
             * EndPoint[] */
            nNumPossibleMobile = 0;
            nGroupNumber = (AT_NUMB)
                num_atoms;    /*  greater than any tautomeric group number */
            ...
            for (k = 0, nNumEndPoints = 0, nNumBondPos = 0;
              ...
              if (!(endpoint_valence =
                        nGetEndpointInfo_KET(at, endpoint, &eif2))) {
                continue;
              }
              /*
              ...
              */
              ...
              /* --- why is isitopic info missing ? -- see below
              nMobile  = EndPoint[nNumEndPoints].num[1] = (at[endpoint].charge
              == -1); nMobile  = EndPoint[nNumEndPoints].num[0] =
              at[endpoint].num_H + nMobile;
              */
              if (bNonTautBond) {
                m = (bond_type == BOND_SINGLE &&
                     (nMobile || at[endpoint].endpoint));
...
#endif
              /*  mobile group is possible if (a) the endpoint has a mobile
               * group or */
              /*                              (b) the centerpoint is adjacent to
               * another endpoint */
              nNumPossibleMobile += (nMobile > 0 || at[endpoint].endpoint);
            ...
            if (nNumEndPoints > 1 && nNumPossibleMobile && nNumDonor &&
                nNumAcceptor && num_O == 1 && num_C) {
              /*
               * a tautomeric group has been found
               *
               * at this point:
               * nGroupNumber = 0 if all endpoints belong to a newly discovered
               * tautomeric group bDiffGroups  > 0 if at least 2 tautomeric
               * groups are to be merged (one of them can be new) case
               * (nGroupNumber != 0 && bDiffGroups = 0 ) ignored because all
               * endpoints belong to the same known t-group case (nGroupNumber
               * != 0 && bDiffGroups < 0 ) cannot happen
               */

              nErr = FindAccessibleEndPoints(
                  EndPoint, &nNumEndPoints, BondPos, &nNumBondPos, pBNS, pBD,
                  at, num_atoms, c_group_info, ALT_PATH_MODE_TAUTOM_KET);
              if (IS_BNS_ERROR(nErr)) {
                return nErr;
              }
              nErr = 0;
              ...
              if (nNumEndPoints > 0) {
                if (!nGroupNumber || bDiffGroups > 0) {
                  num_changes =
                      RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints,
                                        at, num_atoms, c_group_info, pBNS);
                  if (num_changes == -1) {
                    nErr = CT_TAUCOUNT_ERR;
                  }
                  if (num_changes < 0) {
...
#define MAX_ALT_PATH_LEN 8
    ...
    int i1, i2;
    AT_RANK *nDfsPathPos =
        (AT_RANK *)inchi_calloc(num_atoms, sizeof(nDfsPathPos[0]));
    ...
    if (t_group_info->bTautFlags & TG_FLAG_1_5_TAUT) {
      ...
      /*
         ...
         Note: few recent modifications now allow the terminal N be in a ring,
         too
       */
      for (i1 = 0; i1 < num_atoms; i1++) {
        ...
        if (1) {
          ...
          ret = nGet15TautInAltPath(
              at, i1, nDfsPathPos, DfsPath, nMaxLenDfsPath, EndPoint,
              sizeof(EndPoint) / sizeof(EndPoint[0]), BondPos,
              sizeof(BondPos) / sizeof(BondPos[0]), &nNumEndPoints,
              &nNumBondPos, pBNS, pBD, num_atoms);
          if (ret > 0) {
            if (nNumEndPoints) {
              num_changes =
                  RegisterEndPoints(t_group_info, EndPoint, nNumEndPoints, at,
    ...
    for (i1 = 0; i1 < num_atoms; i1++) {
      ...
      if (at[i1].nNumAtInRingSystem >= 6) {
        ...
        ret = nGet15TautIn6MembAltRing(
            at, i1, nDfsPathPos, DfsPath, nMaxLenDfsPath, EndPoint,
            sizeof(EndPoint) / sizeof(EndPoint[0]), BondPos,
            sizeof(BondPos) / sizeof(BondPos[0]), &nNumEndPoints, &nNumBondPos,
            pBNS, pBD, num_atoms);
        if (ret > 0) {
          if (nNumEndPoints) {
            num_changes =
    ...
    */
    ...
    for (i1 = 0; i1 < num_atoms; i1++) {
      if (2 == at[i1].valence && at[i1].nNumAtInRingSystem >= 5 &&
        ...
        for (j = 0; j < at[i1].valence; j++) {
          ...
          if ((bond_type != BOND_SINGLE && bond_type != BOND_TAUTOM &&
            ...
            continue; /*  not a nitrogen atom or a wrong valence or not a single
                         bond */
          }
          nMobile2 = at[i2].num_H +
                     (at[i2].charge == -1); /*  number of mobile groups */
          ...
          ret = nGet12TautIn5MembAltRing(
              at, i1, j, nDfsPathPos, DfsPath, nMaxLenDfsPath, EndPoint,
              sizeof(EndPoint) / sizeof(EndPoint[0]), BondPos,
              sizeof(BondPos) / sizeof(BondPos[0]), &nNumEndPoints,
              &nNumBondPos, pBNS, pBD, num_atoms);
          if (ret > 0) {
     ...
     * where A-Q-R=B belong to a 7-member alt. (except Q-R bond) ring:
     * ..=A-(Q-R)=B-.. Bond Q-R should be single or tautomeric or alternating
      ...
      if (at[i1].nNumAtInRingSystem >=
        ...
        for (j = 0; j < at[i1].valence; j++) {
          ...
          */
          ...
          bond_type = (at[i1].bond_type[j] & ~BOND_MARK_ALL);
          if (bond_type != BOND_SINGLE && bond_type != BOND_TAUTOM &&
              bond_type != BOND_ALT12NS && bond_type != BOND_ALTERN) {
          ...
          for (k = 0; k < at[i1].valence; k++) {
            ...
            nMobile1 =
                at[endpoint1].num_H +
                (at[endpoint1].charge == -1); /*  number of mobile groups */
            if (nMobile1 + at[endpoint1].chem_bonds_valence !=
                endpoint1_valence)
            ...
            if (bond_type1 != BOND_SINGLE && bond_type1 != BOND_DOUBLE &&
                bond_type1 != BOND_TAUTOM && bond_type1 != BOND_ALT12NS &&
            ...
            for (m = 0; m < at[i2].valence; m++) {
              ...
              if (!(endpoint2_valence = nGetEndpointInfo(at, endpoint2, &eif2)))
                continue; /*  not an endpoint2 element or can't have mobile
                             groups */
#if (TAUT_RINGS_ATTACH_CHAIN == 1)
              ...
              nMobile2 =
                  at[endpoint2].num_H +
                  (at[endpoint2].charge == -1); /*  number of mobile groups */
              bond_type2 = (at[i2].bond_type[m] & ~BOND_MARK_ALL);

              if (bond_type2 != BOND_SINGLE && bond_type2 != BOND_DOUBLE &&
                  bond_type2 != BOND_TAUTOM && bond_type2 != BOND_ALT12NS &&
              ...
              if (ALLOWED_EDGE(pBNS, i1, k) && ALLOWED_EDGE(pBNS, i2, m)) {
                ...
                nMobile +=
                    (at[endpoint1].endpoint ||
                     nMobile1) && /*  from endpoint1 */
                ...
                nMobile +=
                    (at[endpoint2].endpoint ||
                     nMobile2) && /*  from endpoint2 */
                    (bond_type2 !=
                     BOND_DOUBLE) && /*changed from BOND_SINGLE 2004-02-26 */
              ...
              if (bond_type1 == bond_type2 &&
                ...
                continue;
                /* -- old --
                if ( !at[endpoint1].endpoint && !at[endpoint2].endpoint && 1 !=
                nMobile1 + nMobile2 ) continue;
                */
                /* -- new --

                if ( !at[endpoint1].endpoint && !at[endpoint2].endpoint ) {
                    if ( !(bond_type1 == BOND_SINGLE || bond_type1 ==
                BOND_DOUBLE) ||
                         !(bond_type2 == BOND_SINGLE || bond_type2 ==
                BOND_DOUBLE) ) {
              ...
              if (at[i1].nNumAtInRingSystem >= 7) {
                ret = nGet14TautIn7MembAltRing(
                    at, i1, j, k, m, nDfsPathPos, DfsPath, nMaxLenDfsPath,
                    EndPoint, sizeof(EndPoint) / sizeof(EndPoint[0]), BondPos,
                    sizeof(BondPos) / sizeof(BondPos[0]), &nNumEndPoints,
                    &nNumBondPos, pBNS, pBD, num_atoms);
                if (ret > 0) {
                  if (nNumEndPoints) {
                  ...
                  if (nNumBondPos) {
                    tot_changes +=
                        (0 < SetTautomericBonds(at, nNumBondPos, BondPos));
                  }
                } else if (IS_BNS_ERROR(ret)) {
                  nErr = ret;
                  goto free_memory;
...
int make_a_copy_of_t_group_info(T_GROUP_INFO *t_group_info,
  ...
  if (t_group_info_orig && t_group_info) {
    if ((len = t_group_info_orig->max_num_t_groups) > 0) {
      if ((t_group_info->t_group = (T_GROUP *)inchi_malloc(
               len * sizeof(t_group_info->t_group[0])))) {
        memcpy(t_group_info->t_group, t_group_info_orig->t_group,
               len * sizeof(t_group_info->t_group[0]));
      } else {
        err++;
      }
    ...
    if ((len = t_group_info_orig->nNumEndpoints) > 0) {
      if ((t_group_info->nEndpointAtomNumber = (AT_NUMB *)inchi_malloc(
    ...
    if ((len = t_group_info_orig->num_t_groups) > 0) {
      if ((t_group_info->tGroupNumber = (AT_NUMB *)inchi_malloc(
               len * TGSO_TOTAL_LEN * sizeof(t_group_info->tGroupNumber[0])))) {
        memcpy(t_group_info->tGroupNumber, t_group_info_orig->tGroupNumber,
               len * TGSO_TOTAL_LEN * sizeof(t_group_info->tGroupNumber[0]));
      } else {
        err++;
      }
    ...
    if ((len = t_group_info_orig->nNumIsotopicEndpoints) > 0) {
      if ((t_group_info->nIsotopicEndpointAtomNumber = (AT_NUMB *)inchi_malloc(
    ...
    if (!err) {
      ...
      t_group_info->nNumIsotopicEndpoints =
          t_group_info_orig->nNumIsotopicEndpoints;
      t_group_info->tni = t_group_info_orig->tni;
      /*
      t_group_info->nNumRemovedExplicitH       =
      t_group_info_orig->nNumRemovedExplicitH; t_group_info->nNumRemovedProtons
      = t_group_info_orig->nNumRemovedProtons; t_group_info->bNormalizationFlags
      = t_group_info_orig->bNormalizationFlags;
      */
      /*
      t_group_info->bHardAddedRemovedProtons   =
      t_group_info_orig->bHardAddedRemovedProtons;
      t_group_info->bSimpleAddedRemovedProtons =
      t_group_info_orig->bSimpleAddedRemovedProtons;
      t_group_info->nNumCanceledCharges        =
      t_group_info_orig->nNumCanceledCharges;
      */
    }
...
int set_tautomer_iso_sort_keys(T_GROUP_INFO *t_group_info) {
  ...
  if (!t_group_info || !(t_group = t_group_info->t_group) ||
      0 >= (num_t_groups = t_group_info->num_t_groups) ||
      t_group_info->nNumIsotopicEndpoints)
    return 0;
  for (i = 0; i < num_t_groups; i++) {
    ...
    do {
      t_group[i].iWeight +=
          Mult * (T_GROUP_ISOWT)t_group[i].num[T_NUM_NO_ISOTOPIC + j];
 ...
 *  Note: on input, t_group_info should contain information created by
 * MarkTautomerGroups() No previous t_group_info adjustment due to throwing out
 * disconnected parts of the chemical structure is needed.
 *
 *  Note2: throws out t_groups containing negative charges only
 * (IGNORE_TGROUP_WITHOUT_H==1) (leave their tautomeric bonds unchanged) Note3:
 * removes negative charges from other tautomeric groups and adjust counts of
 * mobile atoms if permitted         (REMOVE_TGROUP_CHARGE==1)
 */
int CountTautomerGroups(sp_ATOM *at, int num_atoms,
                        T_GROUP_INFO *t_group_info) {
  ...
  if (!t_group_info || !t_group_info->t_group ||
      0 >= t_group_info->max_num_t_groups) {
  ...
  if (max_t_group &&
      !(nTautomerGroupNumber = (AT_NUMB *)inchi_calloc(
            max_t_group + 1, sizeof(nTautomerGroupNumber[0])) /*temp*/)) {
  ...
  if (!(nEndpointAtomNumber = (AT_NUMB *)inchi_calloc(
            nNumEndpoints, sizeof(nEndpointAtomNumber[0]))) ||
      !(nCurrEndpointAtNoPos = (AT_NUMB *)inchi_calloc(
            num_t, sizeof(nCurrEndpointAtNoPos[0])) /*temp*/)) {
    goto err_exit_function; /*   program error: out of RAM */ /*   <BRKPT> */
  }
  /*
  ...
  for (i = 0, nNewGroupNumber = 0; i < num_t; /*i ++*/) {
    ...
    if (t_group[i].nNumEndpoints != nTautomerGroupNumber[(int)nGroupNumber]
        ...
        || (bNoH = (t_group[i].num[0] ==
                    t_group[i].num[1])) /* only for (H,-) t-groups; (+) t-groups
                                           are not removed */
#endif
    ) {
      if (!nTautomerGroupNumber[(int)nGroupNumber] || bNoH) {
        /*  the group belongs to another disconnected part of the structure or
         * has only charges */
        /*  Remove the group */
        num_t--;
        if (i < num_t)
          memmove(t_group + i, t_group + i + 1,
                  (num_t - i) * sizeof(t_group[0]));
        if (bNoH) {
          /*  group contains no mobile hydrogen atoms, only charges. Prepare to
           * remove it. */
      ...
      nTautomerGroupNumber[(int)nGroupNumber] = t_group[i].nGroupNumber =
          ++nNewGroupNumber; /*  = i+1 */
      /*  get first group atom orig. number position in the
       * nEndpointAtomNumber[] */
      /*  and in the tautomer endpoint canon numbers part of the connection
       * table */
      t_group[i].nFirstEndpointAtNoPos = nCurrEndpointAtNoPos[i] =
          i ? (t_group[i - 1].nFirstEndpointAtNoPos +
               t_group[i - 1].nNumEndpoints)
            : 0;
  ...
  /*
   * an array for tautomer group sorting later, at the time of storing
   * Connection Table Later the sorting consists out of 2 steps: 1) Sort
   * t_group[i].nNumEndpoints endpoint atom ranks within each endpoint group
   *    starting from t_group[i].nFirstEndpointAtNoPos; i =
   * 0..t_group_info->num_t_groups-1 2) Sort the groups indexes
   * t_group_info->tGroupNumber[]
   */
  if (!(tGroupNumber = (AT_NUMB *)inchi_calloc(nNewGroupNumber * TGSO_TOTAL_LEN,
                                               sizeof(tGroupNumber[0])))) {
    goto err_exit_function; /*  out of RAM */
  }
  for (i = 0; i < nNewGroupNumber; i++) {
    tGroupNumber[i] = (AT_NUMB)
        i; /*  initialization: original t_group number = (at[i]->endpoint-1) */
  }
  /*
   * renumber endpoint atoms and save their orig. atom
   * numbers for filling out the tautomer part of the LinearCT.
   * nCurrEndpointAtNoPos[j] is an index of the atom number in the
   * nEndpointAtomNumber[]
   */
  for (i = 0; i < num_atoms; i++) {
    if ((j = (int)at[i].endpoint)) {
      j = (int)(at[i].endpoint = nTautomerGroupNumber[j]) -
          1;        /*  new t_group number */
      if (j >= 0) { /*  j=-1 in case of no mobile hydrogen atoms (charges only),
                       group being removed */
  ...
  return nNumEndpoints + T_GROUP_HDR_LEN * nNewGroupNumber +
         1; /*  nLenLinearCTTautomer */
...
/*****************************************************************************/
int CountTautomerGroupsInpAt(inp_ATOM *at, int num_atoms,
                             T_GROUP_INFO *t_group_info) {
  int i, j, ret = 0, nNumEndpoints, max_t_group, num_groups_noH;
...
/*****************************************************************************/
int SortTautomerGroupsAndEndpoints(T_GROUP_INFO *t_group_info, int num_atoms,
                                   int num_at_tg, AT_RANK *nRank) {
  ...
  for (i = 0; i < num_t_groups; i++) {
    ...
    if (nNumEndpoints + nFirstEndpointAtNoPos >
        t_group_info->nNumEndpoints) {             /*  for debug only */
      return CT_TAUCOUNT_ERR; /*  program error */ /*   <BRKPT> */
    }
    nEndpointAtomNumber =
        t_group_info->nEndpointAtomNumber + (int)nFirstEndpointAtNoPos;
    pn_tRankForSort = nRank;
    insertions_sort(nEndpointAtomNumber, nNumEndpoints,
                    sizeof(nEndpointAtomNumber[0]), CompRankTautomer);
  }
  /*  sort the tautomeric groups according to their ranks only
      (that is, ignoring the isotopic composition of the mobile groups and ranks
     of the endpoints) */
