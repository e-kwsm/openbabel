...
/********************************************************************************/
int cmp_iso_atw_diff_component_no(const void *a1, const void *a2) {
  int ret = (int)((const inp_ATOM *)a1)->iso_atw_diff -
            (int)((const inp_ATOM *)a2)->iso_atw_diff;
  if (!ret) /*  make the sort stable */
    ret = (int)((const inp_ATOM *)a1)->component -
          (int)((const inp_ATOM *)a2)->component;
  return ret;
}
typedef struct tagTreeAtom {
  AT_NUMB neighbor[MAXVAL]; /* positions (from 0) of the neighbors in the
                               inp_ATOM array */
...
int MarkRingSystemsInp(inp_ATOM *at, int num_atoms, int start) {
...
#if (FIND_RINS_SYSTEMS_DISTANCES == 1)
  nRsConnect =
      (AT_NUMB *)inchi_calloc(3 * num_atoms + 3, sizeof(nRsConnect[0]));
#endif
  /*  check allocation */
  ...
  do {
    /* advance */
  advance_block:
    /*if ( (int)at[i=nStackAtom[nTopStackAtom]].valence > (j =
     * (int)cNeighNumb[i]) )*/
  ...
  advance_ring:
    ...
    if (nDfsNumber[i] == nLowNumber[i]) {
       ...
       * 1) nNumConnect+1 = (number of already discovered neighboring "ring
       * systems" + 1)+1 2) nNumAtInRingSystem 3) (nNumConnect-1) numbers (IDs)
       * of neighboring ring systems. BFS guarantees that each neighboring ring
       * system is encountered only one time Number of all neighboring ring
       * systems = (nNumConnect-1)+1 = nNumConnect (One additional ring system
       * is where the BFS retracts from the vertex #i, except when i=DFS root
       * node. In the latter case there is/are only (nNumConnect-1) neighboring
       * ring system(s).
       */
#endif
      ...
      while (nTopRingStack >= 0) {
        ...
        for (k = 0; k < at[j].valence; k++) {
          if ((nRs = at[at[j].neighbor[k]].nRingSystem) &&
              (int)nRs != nNumRingSystems) {
            nRsConnect[nLenConnect + (nNumConnect++)] =
                nRs; /*  adjacent ring system id */
          }
        }
#endif
        if (i == j) {
...
#define T(a, b) tree[(a)*nCol + b]
    if (tree = (AT_NUMB *)inchi_calloc(nCol * (nNumRingSystems + 1),
                                       sizeof(tree[0]))) {
      int len, neigh;
       ...
       * each vertex corresponds to a ring system T(row, col) = T(ring syst,
       * neighbors) Number of rows = column length = max. number of ring system
       * neighbors + 2 Number of cols = row length    = number of ring systems +
       * 1 Neighboring ring systems are contiguously stored in a row T(i,0) =
       * number of neighbors,  1 <= i <= nNumRingSystems; T(i,k) = number of a
       * neighboring ring system, 1 <= k <= T(i,0) T(i,nCol-1) = number of atoms
       * in the system #i
       */
      ...
      /*  clear memory */
      memset(nNumVisitedNeighbors, 0,
             nNumRingSystems * sizeof(nNumVisitedNeighbors[0]));
      memset(nDistanceFromTerminal, 0,
             nNumRingSystems * sizeof(nDistanceFromTerminal[0]));
      memset(nCurrActiveRingSystem, 0,
             nNumRingSystems * sizeof(nCurrActiveRingSystem[0]));
      memset(nNextActiveRingSystem, 0,
      ...
      do {
        ...
        for (i = 0; i < nNumRingSystems; i++) {
          if (T(i + 1, 0) == nCurrActiveRingSystem[i]) {
            /* on the previous pass currently active ring system i+1 bas been
             * reached from all neighbors except one; the neighbors from which
             * it was reached have T(neigh,0)+1 == nCurrActiveRingSystem[i] this
             * ring system has not been left yet
             */
            ...
            if (k <= len) {
               ...
               * If chain system next to a terminal system has already been
               * reached then walk along it according to Note below
               *
               * pass > 0: started from branching points
               * 2a) If the branching point has not been reached from 2 or more
               * neighbors, then include it 2b) If the branching point has not
               * been reached from 1 neighbor only, then do not include it: it
               * will be a starting point later Note: if a chain atom already
               * has nDistanceFromTerminal[i] > 0, then the last atom should be
               * the one such that its nDistanceFromTerminal[]+1>=
               * nDistanceFromTerminal[] of the next in the chain
               */
              int bOk = 0;
              ...
              while (2 == T(neigh, 0)) {
                ...
                if (!nNextActiveRingSystem[neigh - 1]) {
                  nNextActiveRingSystem[neigh - 1] =
                      1; /*  make neighbor active */
                } else if (nDistanceFromTerminal[k - 1] + 1 <=
                           nDistanceFromTerminal[neigh - 1]) {
                  /*  walking along the chain; already have had a walk */
                  /*  in the opposite direction at this pass */
                } else {
                  /*  k is the last; neigh (it is a bridge -X-) has not been
                   * reached */
                  bOk = 1;
                  break;
                }
                ...
                if (nNextActiveRingSystem[neigh - 1] < T(neigh, 0)) {
                  nNextActiveRingSystem[neigh -
                                        1]++; /*  add one connection to neigh */
                }
                nDistanceFromTerminal[neigh - 1] =
                    nDistanceFromTerminal[k - 1] + 1;
                j = (T(neigh, 1) == k) ? 2 : 1;
                k = neigh;
              ...
              if (2 > T(neigh, 0)) {
                ...
                if (1 < pass) {
                  nNumRingSystems = CT_UNKNOWN_ERR;
                      /*  error (debug only) */ /*   <BRKPT> */
                  goto exit_function;
                }
                continue;
              }
              ...
              if (T(neigh, 0) > nCurrActiveRingSystem[neigh - 1]) {
                ...
                if (nDistanceFromTerminal[neigh - 1] <
                    nDistanceFromTerminal[k - 1] + 1) {
                  nDistanceFromTerminal[neigh - 1] =
                      nDistanceFromTerminal[k - 1] + 1;
                }
      ...
      for (i = 0; i < num_atoms; i++) {
        at[i].nDistanceFromTerminal =
            nDistanceFromTerminal[(int)at[i].nRingSystem - 1];
      }
...
int remove_terminal_HDT(int num_atoms, inp_ATOM *at, int bFixTermHChrg) {
  ...
  new_ord = (AT_NUMB *)inchi_calloc(
      num_atoms, sizeof(new_ord[0])); /* changed malloc to calloc 9-11-2003 */
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    k = at[i].elname[1]                                ? kMax
        : (p = (char *)strchr(szHDT, at[i].elname[0])) ? p - szHDT
                                                       : kMax;
    /*  set hydrogen isotope atw differences */
    /*  Notes: k-value of isotopic H is incremented to correct iso_atw_diff
     * value later. */
    ...
    if (k == ATW_H || k == ATW_H + 1) {
      ...
      at[i].iso_atw_diff =
          ++k; /*  increment k to make k = iso_atw_diff ( 2 for D, 3 for T ) */
    }
    num_H += (k != kMax && at[i].valence == 1 &&
              at[i].chem_bonds_valence == 1 && !NUMH(at, i));
  }
  ...
  if (2 == num_H && 2 == num_atoms && !NUMH(at, 0) && !NUMH(at, 1)) {
    ...
    for (i = 0; i < num_atoms; i++) {
      k = (at[i].elname[1] || NUMH(at, i)) ? kMax
          : (at[i].elname[0] == 'H')       ? at[i].iso_atw_diff
                                           : kMax;
      if ( k < kMax && at[i].valence == 1 && at[i].chem_bonds_valence == 1 &&
        ...
        num_hydrogens++;
        if (k == 0 && ATW_H <= at[i].iso_atw_diff &&
            at[i].iso_atw_diff < ATW_H + NUM_H_ISOTOPES) {
          k = at[i].iso_atw_diff; /*  H isotope has already been marked above or
                                     elsewhere */
        }
        if (at[i].charge) {
          ...
          if (bFixTermHChrg) {
            /*^^^^^ Fixed bug (July 6, 2008 IPl) :
                    if terminal H was charged (not neutralized before call of
            remove_terminal_HDT) and had an ordering number > than that of
            heavy-atom neighbour, then charge on neighbour atom was not adjusted
            (though charge on H was removed).
            ^^^^^ */
            if (i > n)
        ...
        new_ord[i] = num_atoms -
                     num_hydrogens; /*  move hydrogens to the end of the list */
      } else {
  ...
  if (num_hydrogens) {
    int num_others =
        num_atoms - num_hydrogens; /*  atoms which are not terminal H, D, T */
    if (num_hydrogens > 1) {
      /*  sort hydrogen isotopes in ascending order, */
      /*  orig, numbers being the secondary sorting key */
      qsort(new_at + num_others, num_hydrogens, sizeof(new_at[0]),
            cmp_iso_atw_diff_component_no);
    }
    /*  save new numbering of hydrogen atoms using temporarily saved orig
     * numbering */
    ...
    for (i = 0; i < num_others; i++) {
      ...
      for (m = 0; m < MAX_NUM_STEREO_BONDS && new_at[i].sb_parity[m]; m++) {
        old_trans[m] = 2 - (new_at[i].sn_ord[m] + new_at[i].sb_ord[m] +
                            (new_at[i].sn_ord[m] > new_at[i].sb_ord[m])) %
                               2;
      }
      ...
      for (k = j = val = 0; k < new_at[i].valence; k++) {
        if (num_others <= (n = new_ord[new_at[i].neighbor[k]])) {
           ...
           */
          if (0 < new_at[n].iso_atw_diff &&
              new_at[n].iso_atw_diff < ATW_H + NUM_H_ISOTOPES) {
          ...
          new_at[i].chem_bonds_valence =
              inchi_max(0, new_at[i].chem_bonds_valence - 1);
          new_at[n].neighbor[0] =
              i; /*  update removed hydrogen neighbor number */
          if (new_at[i].sb_parity[0]) {
            /* if the removed H is an SB neighbor then mark it as removed */
            for (m = 0; m < MAX_NUM_STEREO_BONDS && new_at[i].sb_parity[m];
                 m++) {
              if (k == (int)new_at[i].sn_ord[m]) {
                new_at[i].sn_ord[m] = -(new_at[n].iso_atw_diff + 1);
                /* means the SB neighbor has been removed; (-4)=H, (-3)=1H,
                 * (-2)=D, (-1)=T */
              }
            }
          }
      ...
      if (new_at[i].valence > val && new_at[i].sb_parity[0]) {
        if (num_HydrogenAt == new_at[i].valence - val &&
            num_HydrogenAt + num_OtherNeigh <= MAXVAL) {
          /* recalculate parity so that it would describe neighbor sequence
           * H,1H,D,T,neigh[0],neigh[1]... */
          ...
          for (k = 0, j = 1; k <= NUM_H_ISOTOPES; k++) {
            if (new_HydrogenAt_order[k]) {
              new_OtherNeigh_order[num_HydrogenAt - j] =
                  new_HydrogenAt_order[k];
              for (m = 0; m < MAX_NUM_STEREO_BONDS && new_at[i].sb_parity[m];
                   m++) {
          ...
          k = insertions_sort_AT_RANK(new_OtherNeigh_order,
                                      num_HydrogenAt + num_OtherNeigh);
...
/********************************************************************
             ...
             (v. 1.04 : underivatize is still experimental and for engineering
 mode)
...
int add_inp_ATOM(inp_ATOM *at, int len_at, int len_cur, inp_ATOM *add,
                 int len_add);
int cmp_r2c_atpair(const void *p1, const void *p2);
int has_atom_pair(R2C_ATPAIR *ap, int num_ap, AT_NUMB at1, AT_NUMB at2);
int mark_atoms_ap(inp_ATOM *at, AT_NUMB start, R2C_ATPAIR *ap, int num_ap,
                  int num, AT_NUMB cFlags);
...
#define CFLAG_MARK_BLOCK_INV                                                   \
  ((char)~(CFLAG_MARK_BLOCK)) /* for block detection */
#define COUNT_ALL_NOT_DERIV                                                    \
  1 /* 1=> count ALL atoms that are not in deriv. agents */
    /* 0=> only atoms that are not in DERIV_RING */

int mark_atoms_cFlags(inp_ATOM *at, int start, int num, char cFlags);
int un_mark_atoms_cFlags(inp_ATOM *at, int start, int num, char cFlags,
                         char cInvFlags);
...
int is_possibly_deriv_neigh(inp_ATOM *at, int iat, int iord, int type,
                            char cFlags);
int get_traversed_deriv_type(inp_ATOM *at, DERIV_AT *da, int k, DERIV_AT *da1,
                             char cFlags);
int add_to_da(DERIV_AT *da, DERIV_AT *add);
int mark_atoms_deriv(inp_ATOM *at, DERIV_AT *da, int start, int num,
                     char cFlags, int *pbFound);
int count_one_bond_atoms(inp_ATOM *at, DERIV_AT *da, int start, int ord,
                         char cFlags, int *bFound);
...
int is_deriv_ring(inp_ATOM *at, int start, int num_atoms, DERIV_AT *da1,
                  int idrv);
int is_deriv_chain(inp_ATOM *at, int start, int num_atoms, DERIV_AT *da1,
                   int idrv);
int is_deriv_chain_or_ring(inp_ATOM *at, int start, int num_atoms,
                           DERIV_AT *da1, int *idrv);
...
int un_mark_atoms_cFlags(inp_ATOM *at, int start, int num, char cFlags,
  ...
  if (at[start].cFlags & cFlags) {
    ...
    num++;
    for (i = 0; i < at[start].valence; i++) {
      num = un_mark_atoms_cFlags(at, at[start].neighbor[i], num, cFlags,
                                 cInvFlags);
    }
  }
  return num; /* number of atoms traversed forward from at[start] */
...
int is_C_or_S_DB_O(inp_ATOM *at, int i) {
  int j, neigh;
  if (at[i].el_number != el_number_C && at[i].el_number != el_number_S ||
...
int is_C_unsat_not_arom(inp_ATOM *at, int i) {
  ...
  if (at[i].el_number != el_number_C ||
      ...
      at[i].chem_bonds_valence + at[i].num_H != 4 || at[i].charge ||
      at[i].radical)
    return 0;
  num_arom = num_DB = 0;
  for (j = 0; j < at[i].valence; j++) {
  ...
  if (at[i].el_number == el_number_C &&
    ...
    for (j = 0; j < at[i].valence; j++) {
      ...
      if (at[k].cFlags & cFlags)
        continue;
      el = at[k].el_number;
      if (el != el_number_C && el != el_number_F && el != el_number_Cl &&
          el != el_number_Br && el != el_number_I) {
        return 0;
      }
    }
    return 1;
...
/***************************************************************/
int is_Si_IV(inp_ATOM *at, int i) {
  if (at[i].el_number != el_number_Si || at[i].charge || at[i].radical ||
      at[i].valence != 4 || at[i].chem_bonds_valence != 4)
    return 0;
  return 1;
}
/***************************************************************/
int is_P_TB_N(inp_ATOM *at, int i) {
  int j, k;
  if (at[i].el_number != el_number_P ||
      at[i].chem_bonds_valence - at[i].valence != 2)
    return 0;
  for (j = 0; j < at[i].valence; j++) {
    k = at[i].neighbor[j];
    if (at[k].el_number == el_number_N && at[k].valence == 1 &&
...
int is_possibly_deriv_neigh(inp_ATOM *at, int iat, int iord, int type,
  ...
  case DERIV_BRIDGE_O:
    ...
    /* -> A--O--B -> traversing from A(neigh_from) to B(neigh); may we cut O--B
     * bond? */
    ...
    if (!(is_C_or_S_DB_O(at, neigh) && is_Si_IV(at, neigh_from)) &&
      ...
      bOk = (el == el_number_C || el == el_number_Si || el == el_number_S ||
             el == el_number_P);
    }
    break;
  case DERIV_BRIDGE_NH:
    /* -> A--NH--B -> traversing from A(neigh_from) to B(neigh); may we cut
     * NH--B bond? */
    bOk = (is_C_or_S_DB_O(at, neigh) || is_C_Alk(at, neigh, cFlags) ||
           is_Si_IV(at, neigh) || is_P_TB_N(at, neigh)) &&
          !(is_C_unsat_not_arom(at, neigh));
    break;
  case DERIV_AMINE_tN:
    bOk = (is_C_or_S_DB_O(at, neigh) || is_C_Alk(at, neigh, cFlags) ||
...
int get_traversed_deriv_type(inp_ATOM *at, DERIV_AT *da, int k, DERIV_AT *da1,
                             char cFlags) {
  int i, j, m, nBlockSystemFrom, nOrdBack1, nOrdBack2, nOrdBack3, nBackType1,
      nBackType2;
  ...
  for (m = 0;
       m < at[k].valence && !(at[(int)at[k].neighbor[m]].cFlags & cFlags); m++)
    ;
  if (m == at[k].valence)
    return -1; /* error: at least one neighbor must have cFlags */
  if (at[k].valence == 1 && at[k].num_H &&
      (at[k].el_number == el_number_O || at[k].el_number == el_number_N ||
       at[k].el_number == el_number_S || at[k].el_number == el_number_P)) {
    return DERIV_NOT;
  }
...
#ifdef NEVER
  if (at[k].el_number == el_number_N && at[k].valence >= 2 &&
      at[k].chem_bonds_valence <= 3) {
  ...
  if (at[k].nNumAtInRingSystem == 1 &&
      (at[k].el_number == el_number_O || at[k].el_number == el_number_S) &&
      at[k].valence == 2 && at[k].chem_bonds_valence == 2 && !at[k].num_H &&
    ...
    if (is_A_Si_IV && has_B_CO) {
      ; /* do not cut bond --- in A=Si(IV), B(=O), B=C: Si(IV)-O---B(=O) */
    } else if (is_possibly_deriv_neigh(at, k, !m, DERIV_BRIDGE_O, cFlags)) {
  ...
  if (at[k].bCutVertex && at[k].el_number == el_number_N &&
      ...
      at[k].valence + at[k].num_H == 3 && !at[k].charge && !at[k].radical) {
    int rm1 = (at[at[k].neighbor[m]].nRingSystem ==
               at[at[k].neighbor[(m + 1) % 3]].nRingSystem);
    int rm2 = (at[at[k].neighbor[m]].nRingSystem ==
    ...
    case 0:
      ...
      if (is_possibly_deriv_neigh(at, k, (m + 1) % 3, DERIV_AMINE_tN, cFlags)) {
        ord[i++] =
            (m + 1) % 3; /* ord of a non-ring neighbor, not traversed yet */
      }
      if (is_possibly_deriv_neigh(at, k, (m + 2) % 3, DERIV_AMINE_tN, cFlags)) {
        ord[i++] = (m + 2) %
                   3; /* ord of another non-ring neighbor, not traversed yet */
      }
      if (i == 2 && ord[0] > ord[1]) {
        int tmp = ord[0];
        ord[0] = ord[1];
    ...
    case 1:
      if (rm1 &&
          is_possibly_deriv_neigh(at, k, (m + 2) % 3, DERIV_AMINE_tN, cFlags)) {
        ord[i++] = (m + 2) %
                   3; /* ord of a single non-ring neighbor, not traversed yet */
      } else if (rm2 && is_possibly_deriv_neigh(at, k, (m + 1) % 3,
                                                DERIV_AMINE_tN, cFlags)) {
        ord[i++] = (m + 1) %
                   3; /* ord of a single non-ring neighbor, not traversed yet */
      }
    }
  ...
  if (at[k].bCutVertex && /* DD */
    ...
    if ((at[j].el_number == el_number_O || at[j].el_number == el_number_S) &&
        at[j].valence == 2 && at[j].chem_bonds_valence == at[j].valence &&
        at[j].nNumAtInRingSystem >= 5 && !at[j].num_H && !at[j].charge &&
        !at[j].radical) {
      nBackType1 = DERIV_RING_O;
      nBlockSystemFrom = at[j].nBlockSystem;
      nOrdBack1 = m; /* predecessor */
    } else if (at[j].el_number == el_number_N && at[j].valence == 2 &&
               at[j].chem_bonds_valence == at[j].valence &&
               at[j].nNumAtInRingSystem >= 5 && 1 == at[j].num_H &&
               !at[j].charge && !at[j].radical) {
    ...
    if (nBlockSystemFrom) {
      ...
      num1 =
          mark_atoms_cFlags(at, at[k].neighbor[nOrdBack1], 1, CFLAG_MARK_BLOCK);
      for (i = 0; i < at[k].valence; i++) {
        ...
        if (at[j].cFlags & CFLAG_MARK_BLOCK) {
          if ((at[j].el_number == el_number_O ||
               at[j].el_number == el_number_S) &&
              at[j].valence == 2 && at[j].chem_bonds_valence == at[j].valence &&
              at[j].nNumAtInRingSystem >= 5 && !at[j].num_H && !at[j].charge &&
              !at[j].radical) {
            bFound = 1;
          ...
          if (!bFound) {
            nOrdBack3 =
                99; /* reject: wrong neighboring atom in the same block */
            break;
          }
        }
      }
      num2 = un_mark_atoms_cFlags(at, k, 0, CFLAG_MARK_BLOCK,
      ...
      if (nOrdBack2 >= 0 && nOrdBack3 < 0) {
        if (nOrdBack1 < nOrdBack2) {
          ...
          da1->ord[1] =
              nOrdBack2; /* ord of another ring neighbor, not traversed yet */
          da1->typ[1] = nBackType2;
        } else {
          da1->ord[0] = nOrdBack2; /* ord of a ring neighbor, traversed */
          da1->typ[0] = nBackType2;
          da1->ord[1] =
              nOrdBack1; /* ord of another ring neighbor, not traversed yet */
...
/***************************************************************/
int add_to_da(DERIV_AT *da, DERIV_AT *add) {
  /* if add has more than 1 element the elements are in ascending add.ord[]
   * order */
  int i, j, len;
...
int mark_atoms_deriv(inp_ATOM *at, DERIV_AT *da, int start, int num,
  ...
  if (!(at[start].cFlags & cFlags)) {
    ...
    if (da1.typ[0]) {
      ...
      case DERIV_BRIDGE_O:
      case DERIV_BRIDGE_NH:
        n1 = mark_atoms_deriv(at, da, at[start].neighbor[(int)da1.ord[0]], 0,
                              cFlags, &nFound1);
        ...
        break;
      case DERIV_AMINE_tN:
        n1 = mark_atoms_deriv(at, da, at[start].neighbor[i1 = da1.ord[0]], 0,
                              cFlags, &nFound1);
        if (da1.typ[1]) {
          n2 = mark_atoms_deriv(at, da, at[start].neighbor[i2 = da1.ord[1]], 0,
                                cFlags, &nFound2);
        }
      ...
      case DERIV_RING_NH:
        for (i = 0; i < at[start].valence; i++) {
          if (i != da1.ord[0] && i != da1.ord[1] &&
              !(at[at[start].neighbor[i]].cFlags & cFlags)) {
            if (!n1)
              n1 = mark_atoms_deriv(at, da, at[start].neighbor[i1 = i], 0,
                                    cFlags, &nFound1);
            else
              n2 = mark_atoms_deriv(at, da, at[start].neighbor[i2 = i], 0,
                                    cFlags, &nFound2);
          }
        }
    ...
    for (i = 0; i < at[start].valence; i++) {
      num =
          mark_atoms_deriv(at, da, at[start].neighbor[i], num, cFlags, pbFound);
      if (num < 0)
        return num;
    }
  }
  /* *pbFound =  number of derivatizer attachment points traversed forward from
   * at[start] */
  return num; /* number of atoms traversed forward from at[start] */
}
/***************************************************************/
...
int is_silyl(inp_ATOM *at, int start, int ord_prev) {
  ...
  if (at[start].el_number != el_number_Si || at[start].valence != 4 ||
      at[start].valence != at[start].chem_bonds_valence || at[start].charge ||
      at[start].radical)
    return 0;
  for (i = 0; i < at[start].valence; i++) {
    if (i == ord_prev)
    ...
    if (at[neigh].valence == 4) {
      if (at[neigh].el_number == el_number_C && iC_IV < 0 && iSi_IV < 0)
        iC_IV = neigh;
      else if (at[neigh].el_number == el_number_Si && iC_IV < 0 && iSi_IV < 0)
        iSi_IV = neigh;
      else
        return 0;
    } else if (at[neigh].valence == 1 &&
               at[neigh].valence == at[neigh].chem_bonds_valence &&
...
int is_Me_or_Et(inp_ATOM *at, int start, int ord_prev) {
  ...
  if (at[start].el_number != el_number_C || at[start].valence > 2 ||
      ...
      at[start].chem_bonds_valence + at[start].num_H != 4 || at[start].charge ||
      at[start].radical)
    return 0;
  for (i = 0; i < at[start].valence; i++) {
    if (i == ord_prev)
...
int is_CF3_or_isoC3F7(inp_ATOM *at, int start, int ord_prev) {
  ...
  for (i = num_C_IV = 0; i < at[start].valence; i++) {
    ...
    neigh = at[start].neighbor[i];
    if (at[neigh].valence != at[neigh].chem_bonds_valence || at[neigh].charge ||
        at[neigh].radical)
      return 0;
    if (at[neigh].el_number == el_number_F) {
      if (at[neigh].chem_bonds_valence + at[neigh].num_H != 1)
        return 0;
      num_F++;
    } else if (at[neigh].el_number == el_number_C && at[neigh].valence == 4 &&
...
int is_CF3_or_linC3F7(inp_ATOM *at, int start, int ord_prev) {
  ...
  while (num_C < 4) {
    ...
    if (p = is_in_the_list(at[iC_IV].neighbor, (AT_NUMB)start,
                           at[iC_IV].valence)) {
      start = iC_IV;
...
int is_deriv_ring(inp_ATOM *at, int start, int num_atoms, DERIV_AT *da1,
  ...
  if (da1->typ[idrv] != DERIV_RING_O || da1->typ[idrv + 1] != DERIV_RING_O)
    return 0;
  if (at[start].charge || at[start].radical ||
      at[start].valence != at[start].chem_bonds_valence)
    return 0;
  if (at[start].el_number == el_number_B && at[start].valence == 3)
    is_B = 1;
  else if (at[start].el_number == el_number_C &&
           (at[start].valence == 3 || at[start].valence == 4) &&
  ...
  for (i = k = 0; i < at[start].valence; i++) {
    if (i != da1->ord[idrv] && i != da1->ord[idrv + 1]) {
      neigh = at[start].neighbor[i];
      if (k < 2 && (p = is_in_the_list(at[neigh].neighbor, (AT_NUMB)start,
                                       at[neigh].valence))) {
        neigh_at[k] = neigh;
  ...
  if (is_B && k == 1 && is_Me_or_Et(at, neigh_at[0], prev_ord[0]))
    return 1;
  if (is_C && k == 1 && at[start].num_H == 1 &&
      is_phenyl(at, neigh_at[0], prev_ord[0]))
    return 1;
...
int is_deriv_chain_or_ring(inp_ATOM *at, int start, int num_atoms,
  ...
  if (da1->typ[*idrv] & DERIV_RING) {
    /* find the first ord of this derivative; ord of ring derivatives are in
     * pairs */
    int j = -1;
    ...
    /* check consistency */
    if (j == -1 || j + 1 >= DERIV_AT_LEN || !(da1->typ[j] & DERIV_RING) ||
        !(da1->typ[j + 1] & DERIV_RING)) {
      ret = -1; /* program error */
    } else {
      ret = is_deriv_ring(at, start, num_atoms, da1, j);
    }
  } else if (da1->typ[*idrv]) {
...
int remove_deriv(DERIV_AT *da1, int idrv) {
  ...
  if (da1->typ[idrv] & DERIV_RING) {
    ...
    if (j >= 0 && j + 1 < DERIV_AT_LEN && (da1->typ[j] & DERIV_RING) &&
        (da1->typ[j + 1] & DERIV_RING)) {
...
int EliminateDerivNotInList(inp_ATOM *at, DERIV_AT *da, int num_atoms) {
  ...
  for (i = 0; i < num_atoms; i++) {
    ...
    if (da[i].typ[0] & DERIV_RING) {
      ...
      ret = 0;
      if (da[i].num[0] <= MAX_AT_DERIV &&
          0 < (ret = is_deriv_chain(at, i, num_atoms, da + i, 0))) {
        num_cuts++;
        j = 1;
      } else if (ret < 0) {
        return ret;
...
int make_single_cut(inp_ATOM *at, DERIV_AT *da, int iat, int icut) {
  ...
  if (da[iat].typ[icut] & DERIV_DUPLIC) {
    ...
    AT_NUMB *p =
        is_in_the_list(at[jat].neighbor, (AT_NUMB)iat, at[jat].valence);
    ...
    if (ret == 1) {
      ...
      for (i = 0; i < DERIV_AT_LEN && da[iat].typ[i]; i++) {
        if (da[iat].ord[i] == iord) {
          da[iat].ord[i] =
              -(1 + da[iat].ord[i]); /* mark the bond being disconnected */
      ...
      for (i = 0; i < DERIV_AT_LEN && da[jat].typ[i]; i++) {
        if (da[jat].ord[i] == jord) {
          ...
          } else if (da[iat].num[icut] > da[jat].num[i]) {
...
#define REMOVE_CUT_DERIV 1 /* remove disconnected derivatizing agents */
  int ret = 0, i, j, k, m, n, num_atoms, num_components, i_component, nFound,
      num, cur_num_at, len;
  ...
  inp_cur_data =
      (INP_ATOM_DATA *)inchi_calloc(num_components, sizeof(inp_cur_data[0]));
  for (i_component = 0; i_component < num_components; i_component++) {
    CreateInpAtomData(inp_cur_data + i_component,
                      orig_inp_data->nCurAtLen[i_component], 0);
    inp_cur_data[i_component].num_at = ExtractConnectedComponent(
        orig_inp_data->at, orig_inp_data->num_inp_atoms, i_component + 1,
        inp_cur_data[i_component].at);
    /*  error processing */
    if (inp_cur_data[i_component].num_at <= 0 ||
        orig_inp_data->nCurAtLen[i_component] !=
            inp_cur_data[i_component].num_at) {
    ...
    for (i = num = 0; i < num_atoms; i++) {
      ...
      case 2:
        if ((da[i].typ[0] & DERIV_RING) && (da[i].typ[1] & DERIV_RING)) {
          ...
          num_cut_pieces += 1;
          continue;
        } else if (da[i].typ[0] == DERIV_AMINE_tN &&
                   da[i].typ[1] == DERIV_AMINE_tN) {
          /* double cut, unconditional */
          num_cuts += 2;
          num_cut_pieces += 2;
          continue;
      ...
      case 3:
        if (da[i].typ[0] == da[i].typ[1] && da[i].typ[0] == da[i].typ[2] &&
        ...
        if ((da[i].typ[0] & DERIV_RING) && (da[i].typ[1] & DERIV_RING) &&
          ...
          if (n01 < n23 && 0 < is_deriv_ring(at, i, num_atoms, da + i, 0)) {
            ...
            num_cut_pieces += 1;
                     ...
                     0 < is_deriv_ring(at, i, num_atoms, da + i, 2)) {
    ...
    /* eliminate cases when
         ...
         at[i1].neighbor[da[i1].ord[j1]] == i2 &&
       at[i2].neighbor[da[i2].ord[j2]] == i1 that is, same bond is in the da[]
       elements of the adjacent atoms */
    ...
    */
    ...
    if (num_cuts_to_check >= 2) {
      /* check */
      R2C_ATPAIR *ap =
          (R2C_ATPAIR *)inchi_malloc(num_cuts_to_check * sizeof(ap[0]));
      AT_NUMB comp_num;
      int /*n,*/ m_at, m_ord;
      AT_NUMB at1, at2;
      if (!ap) {
    ...
    repeat_without_deriv_ring:
      ...
      for (i = 0; i < num_cuts_to_check; i += j) {
        for (j = n = 0; j < 2; j++) {
          ...
          if (da[atj].typ[0] &&
              at[atj].neighbor[(int)da[atj].ord[0]] == ap[i].at[1 - j]) {
            k = j;
            n++;
            m_at = atj;
            m_ord = 0;
          } else if (da[atj].typ[1] &&
                     at[atj].neighbor[(int)da[atj].ord[1]] == ap[i].at[1 - j]) {
            k = j;
            n++;
            m_at = atj;
            m_ord = 1;
        ...
        if ((da[m_at].typ[m_ord] & DERIV_RING)) {
          ...
          if (i + j > num_cuts_to_check ||
              (int)ap[i + 1].at[0] != n && (int)ap[i + 1].at[1] != n) {
            ret = -3;
            goto exit_r2c_num; /* wrong atom pair */
          }
        } else {
      ...
      cur_num_at = 0; /* number of atoms left after disconnecting the
                         derivatizing agent */
      UnMarkOtherIndicators(at, num_atoms);
      for (i = 0; i < num_cuts_to_check; i++) {
        ...
        if (!at[n].at_type) {
          comp_num++;
          cur_num_at =
              mark_atoms_ap(at, n, ap, num_cuts_to_check, cur_num_at, comp_num);
        }
      }
      if (comp_num > 1) {
    ...
    if (num_cuts) {
      ...
      inp_cur_data1 = (INP_ATOM_DATA *)inchi_calloc(num_components1,
                                                    sizeof(inp_cur_data1[0]));
      ...
      for (i_component1 = 0; i_component1 < num_components1; i_component1++) {
        CreateInpAtomData(inp_cur_data1 + i_component1,
                          orig_inp_data1->nCurAtLen[i_component1], 0);
        inp_cur_data1[i_component1].num_at = ExtractConnectedComponent(
            orig_inp_data1->at, orig_inp_data1->num_inp_atoms, i_component1 + 1,
            inp_cur_data1[i_component1].at);
        /*  error processing */
        if (inp_cur_data1[i_component1].num_at <= 0 ||
            orig_inp_data1->nCurAtLen[i_component1] !=
                inp_cur_data1[i_component1].num_at) {
        ...
        for (i = 0; i < inp_cur_data1[i_component1].num_at; i++) {
          if (inp_cur_data1[i_component1].at[i].num_iso_H[1]) {
            inp_cur_data1[i_component1].at[i].num_iso_H[1] =
                0; /* remove deuterium */
          }
      ...
      for (i_component1 = 0, num_atoms = 0; i_component1 < num_components1;
           i_component1++) {
      ...
      for (i_component1 = 0; i_component1 < num_components1; i_component1++) {
        ...
        /*UnMarkOneComponent( inp_cur_data1[i_component1].at,
         * inp_cur_data1[i_component1].num_at );*/
        /* merge one by one */
        cur_num_at = add_inp_ATOM(at, num_atoms, cur_num_at,
                                  inp_cur_data1[i_component1].at,
                                  inp_cur_data1[i_component1].num_at);
      ...
      UnMarkDisconnectedComponents(
          orig_inp_data1); /* orig_inp_data1->at is same as
                              inp_cur_data[i_component].at */
      FreeInpAtomData(inp_cur_data +
                      i_component); /* cleanup the original component */
  ...
  if (nTotNumCuts) {
    ...
    for (i = 0; i < num_components; i++) {
      ...
      /* merge one by one */
      cur_num_at = add_inp_ATOM(at, num_atoms, cur_num_at, inp_cur_data[i].at,
                                inp_cur_data[i].num_at);
    }
...
typedef struct tagRing2Chain { /* atom Z */
  ...
  char
      ordY; /* ordering number of YH-neighbor; bond to increment; H to remove */
...
int detect_r2c_Zatom(inp_ATOM *at, R2C_AT *da, int iZ) {
  ...
  for (i = 0, num_found = 0; i < at[iZ].valence; i++) {
    ...
    if (at[neigh].el_number == el_number_O && at[neigh].valence == 1 &&
        at[neigh].chem_bonds_valence == 1 && at[neigh].num_H == 1) {
      /* found Z-OH, i.e. Z-YH */
    ...
    if (at[neigh].el_number == el_number_O && at[neigh].valence == 2 &&
        at[neigh].chem_bonds_valence == 2 && at[neigh].num_H == 0 &&
    ...
    if (at[neigh].el_number == el_number_C && at[neigh].valence > 2 &&
        ...
        at[neigh].num_H <= 1 && at[neigh].nRingSystem == nRingSystem) {
      /* sugar-specfic: carbon in the ring should have -OH neighbor */
      int iOH;
      for (j = 0; j < at[neigh].valence; j++) {
        iOH = at[neigh].neighbor[j];
        if (at[iOH].el_number == el_number_O && at[iOH].valence == 1 &&
            at[iOH].chem_bonds_valence == 1 && at[iOH].num_H == 1 &&
...
int cut_ring_to_chain(inp_ATOM *at, R2C_AT *da, int iZ) {
  ...
  if (0 > iordW || iordW >= at[iZ].valence || 0 > iordY ||
      iordY >= at[iZ].valence || 0 > iordC ||
      iordC >= at[iZ].valence /* suger-specific*/) {
    return -1; /* program error */
  }
  /* find other da[] that affect at[iZ] */
  iW = at[iZ].neighbor[iordW]; /* opposite atom to disconnect and add H */
  iY = at[iZ].neighbor[iordY]; /* opposite atom to increment the bond and remove
                                  H*/
...
int Ring2Chain(ORIG_ATOM_DATA *orig_inp_data) {
  ...
  for (j = 0; j < num_components; j++) {
    ...
    inp_cur_data[j].num_at = ExtractConnectedComponent(
        orig_inp_data->at, orig_inp_data->num_inp_atoms, j + 1,
        inp_cur_data[j].at);
    /*  error processing */
    if (inp_cur_data[j].num_at <= 0 ||
        orig_inp_data->nCurAtLen[j] != inp_cur_data[j].num_at) {
      ...
      for (i = 0; i < num; i++) {
        ...
        if (at[i1].at_type == at[i2].at_type) {
          ...
          if (da[i1].type == 1 && at[i1].neighbor[(int)da[i1].ordW] == i2) {
            iZ = i1;
          } else if (da[i2].type == 1 &&
                     at[i2].neighbor[(int)da[i2].ordW] == i1) {
            iZ = i2;
          } else {
            ret = -3;
            goto exit_r2c_num;
