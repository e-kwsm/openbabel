...
/* local prototypes */
int GetProcessingWarningsOneINChI(INChI *pINChI, INP_ATOM_DATA *inp_norm_data,
                                  char *pStrErrStruct);
int GetProcessingWarnings(INChI *cur_INChI[], INP_ATOM_DATA **inp_norm_data,
                          STRUCT_DATA *sd);
int DisplayTheWholeStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                             ...
                             int iINChI, int bShowStruct, int bINCHI_LIB_Flag);
int DuplicateOrigAtom(ORIG_ATOM_DATA *new_orig_atom, ORIG_ATOM_DATA *orig_atom);
int bCheckUnusualValences(ORIG_ATOM_DATA *orig_at_data, int bAddIsoH,
                          char *pStrErrStruct);
int CreateCompositeNormAtom(COMP_ATOM_DATA *composite_norm_data,
                            INP_ATOM_DATA2 *all_inp_norm_data, PINChI2 *pINChI,
                            PINChI_Aux2 *pINChI_Aux, int num_components,
                            INCHI_MODE nMode);
...
struct DrawData *(*GET_DRAWDATA)(int nComponent, int nType,
                                 int bReconnected) = NULL;
#endif
...
typedef struct tagRenumbData {
  ...
  unsigned long ulCurTimeNorm0, ulCurTimeCanon0, ulCurTimeNorm1,
      ulCurTimeCanon1;
...
int RenumberingTest(PINChI2 *pICh, PINChI_Aux2 *pINChI_Aux,
                    ORIG_ATOM_DATA *orig_inp_data, int iINChI,
                    RENUMB_DATA *pRenumbData, INP_ATOM_DATA *inp_cur_data,
                    INP_ATOM_DATA **inp_norm_data, STRUCT_DATA *sd,
                    INPUT_PARMS *ip, char *szTitle, INCHI_IOSTREAM *log_file,
                    INCHI_IOSTREAM *prb_file, int i, long num_inp,
                    NORM_CANON_FLAGS *pncFlags);
/*
int RenumberingTest( INChI *pINChI[][TAUT_NUM], INChI_Aux
*pINChI_Aux[][TAUT_NUM], int iINChI, RENUMB_DATA *pRenumbData, INP_ATOM_DATA
*inp_cur_data, INP_ATOM_DATA **inp_norm_data, STRUCT_DATA *sd, INPUT_PARMS *ip,
char *szTitle, INCHI_IOSTREAM *log_file, int i, long num_inp);
*/
#endif /* } TEST_RENUMB_ATOMS */

#ifndef COMPILE_ANSI_ONLY
/********************************************************************/
void FillTableParms(SET_DRAW_PARMS *sdp, INChI **cur_INChI,
                    INChI_Aux **cur_INChI_Aux, INCHI_MODE nMode,
  ...
  int bRelRac =
      0 != (nMode & (REQ_MODE_RELATIVE_STEREO | REQ_MODE_RACEMIC_STEREO));
#endif
  ...
  if (cur_INChI[i]) {
    Stereo =
        bShowIsotopic ? cur_INChI[i]->StereoIsotopic : cur_INChI[i]->Stereo;
  } else {
    Stereo = NULL;
  }
#if (REL_RAC_STEREO_IGN_1_SC == 1)
  ...
  if (Stereo &&
    ...
    if (Stereo->nNumberOfStereoCenters && Stereo->nCompInv2Abs == -1 &&
      ...
      /*
      if ( Stereo->nNumberOfStereoCenters < 2 && !Stereo->nNumberOfStereoBonds )
      { ReqShownFound[ilSHOWN][itSTEREO] = '\0'; } else if (
...
void FillCompositeTableParms(SET_DRAW_PARMS *sdp, AT_NUMB StereoFlags,
                             INCHI_MODE nMode, int bShowIsotopic,
                             int bShowTaut) {
  TBL_DRAW_PARMS *tdp = sdp->tdp;
  char(*ReqShownFound)[TDP_NUM_PAR] = tdp->ReqShownFound;
  int i, j;
...
int DisplayStructure(inp_ATOM *at, int num_at, int num_removed_H,
                     int bAdd_DT_to_num_H, int nNumRemovedProtons,
                     NUM_H *nNumRemovedProtonsIsotopic, int bIsotopic,
                     int j /*bTautomeric*/, INChI **cur_INChI,
                     INChI_Aux **cur_INChI_Aux, int bAbcNumbers, DRAW_PARMS *dp,
                     INCHI_MODE nMode, char *szTitle) {
...
/*******************************************************************/
int DisplayCompositeStructure(COMP_ATOM_DATA *composite_norm_data,
                              int bIsotopic, int bTautomeric, PINChI2 *pINChI2,
                              PINChI_Aux2 *pINChI_Aux2, int bAbcNumbers,
                              DRAW_PARMS *dp, INCHI_MODE nMode, char *szTitle) {
  INF_ATOM_DATA inf_data;
  int err = -1, ret;
  ...
  if (CreateInfoAtomData(&inf_data, (composite_norm_data + bTautomeric)->num_at,
    ...
    ret = FillOutCompositeCanonInfAtom(composite_norm_data, &inf_data,
                                       bIsotopic, bTautomeric, pINChI2,
                                       pINChI_Aux2, bAbcNumbers, nMode);
    if (!ret) {
      goto exit_function; /* error */
    }
    if (bTautomeric == TAUT_INI) {
      /*
      FillOutInfAtom( (composite_norm_data+bTautomeric)->at, &inf_data,
      (composite_norm_data+bTautomeric)->num_at,
                      (composite_norm_data+bTautomeric)->num_removed_H,
      bAdd_DT_to_num_H, (composite_norm_data+bTautomeric)->nNumRemovedProtons,
                      (composite_norm_data+bTautomeric)->nNumRemovedProtonsIsotopic,
      bIsotopic, NULL, NULL, bAbcNumbers, nMode);
      */
      ;
    } else {
      ...
      FillCompositeTableParms(&dp->sdp, inf_data.StereoFlags, nMode, bIsotopic,
                              nNumTautComponents);
    }
    err = DisplayInputStructure(
        szTitle, (composite_norm_data + bTautomeric)->at, &inf_data,
        (composite_norm_data + bTautomeric)->num_at, dp);
    FreeInfoAtomData(&inf_data);
  }
exit_function:
  return err;
...
#ifndef COMPILE_ANSI_ONLY /* { */
/***********************************************************************************/
int SaveEquComponentsInfoAndSortOrder(
    int iINChI, INCHI_SORT *pINChISort[TAUT_NUM], int *num_components,
    ORIG_ATOM_DATA *orig_inp_data, ORIG_ATOM_DATA *prep_inp_data,
    COMP_ATOM_DATA composite_norm_data[TAUT_NUM + 1], int bCompareComponents) {
  ...
  bCompareTaut =
      (bCompareComponents & CMP_COMPONENTS_NONTAUT) ? TAUT_NON : TAUT_YES;
  ...
  if (num_components[iINChI] > 1) {
    ...
    if (inp_data && !inp_data->nEquLabels &&
      ...
      if ((prep_inp_data[iINChI].nSortedOrder = (AT_NUMB *)inchi_calloc(
               num_components[iINChI] + 1,
        ...
        for (i1 = 0, nSet = 0; i1 < num_components[iINChI]; i1 = i2) {
          ...
          for (i2 = i1 + 1; i2 < num_components[iINChI]; i2++) {
            /* isotopic/non-isotopic comparison does not separate equivalent
             * components */
            if (CompINChI2(pINChISort[bCompareTaut] + i1,
                           pINChISort[bCompareTaut] + i2, bCompareTaut,
                           bCompareIsotopic)) {
              break;
            } else {
              nNumDeleted +=
          ...
          if (i2 - i1 - nNumDeleted > 1) {
            if (inp_data->nEquLabels ||
                (inp_data->nEquLabels = (AT_NUMB *)inchi_calloc(
                     inp_data->num_inp_atoms + 1,
                     sizeof(inp_data->nEquLabels[0])))) {
              nSet++; /* found i2-i1 equivalent components && memory has been
                         allocated */
              for (i = i1; i < i2; i++) {
                ...
                pINChI_Aux =
                    (pINChISort[bCompareTaut][i].pINChI_Aux[bCompareTaut] &&
                     pINChISort[bCompareTaut][i]
                         .pINChI_Aux[bCompareTaut]
                         ->nNumberOfAtoms)
                        ? pINChISort[bCompareTaut][i].pINChI_Aux[bCompareTaut]
                    : (pINChISort[bCompareTaut][i].pINChI_Aux[bCompareAlt] &&
                       pINChISort[bCompareTaut][i]
                           .pINChI_Aux[bCompareAlt]
                           ->nNumberOfAtoms)
                        ? pINChISort[bCompareTaut][i].pINChI_Aux[bCompareAlt]
                        : (INChI_Aux *)NULL;
...
#ifdef TARGET_LIB_FOR_WINCHI /* { */
      if (DRAWDATA && GET_DRAWDATA && inp_data->nNumEquSets > 0 &&
          inp_data->nEquLabels) {
...
/************************************************************************************************/
int DisplayTheWholeCompositeStructure(
    INPUT_PARMS *ip, STRUCT_DATA *sd, long num_inp, int iINChI,
    PINChI2 *pINChI2, PINChI_Aux2 *pINChI_Aux2, ORIG_ATOM_DATA *orig_inp_data,
  ...
  int bCompareTaut =
      (ip->bCompareComponents & CMP_COMPONENTS_NONTAUT) ? TAUT_NON : TAUT_YES;
  ...
  for (jj = 0; ip->bDisplayCompositeResults && !sd->bUserQuitComponentDisplay &&
               jj <= TAUT_INI;
       jj++) {
    /*for ( j = 0; ip->bDisplayCompositeResults &&
     * !sd->bUserQuitComponentDisplay && j <= TAUT_INI; j ++ )*/
    ...
    if (composite_norm_data[j].bExists &&
      ...
      for (k = 0; k <= composite_norm_data[j].bHasIsotopicLayer &&
                  !sd->bUserQuitComponentDisplay;
           k++) {
        /*  added number of components, added another format for a single
         * component case - DCh */
        ...
        nNumIntermediateTaut +=
            (j == TAUT_INI); /* display TAUT_INI (preprocessed) only once */
        if (j != TAUT_INI || nNumIntermediateTaut == 1) {
          err = DisplayCompositeStructure(
              composite_norm_data, j == TAUT_INI ? 1 : k /* bIsotopic*/,
              j /*tautomeric*/, j == TAUT_INI ? NULL : pINChI2,
              j == TAUT_INI ? NULL : pINChI_Aux2, ip->bAbcNumbers, &ip->dp,
              ip->nMode, szTitle);
        }
        ...
        if (inp_data && inp_data->nEquLabels && inp_data->nNumEquSets &&
          ...
          for (nEquSet = 1; nEquSet <= inp_data->nNumEquSets; nEquSet++) {
            sprintf(szTitle,
                    "Equ set %d of %d, %s Structure #%ld%s%s.%s%s%s%s%s",
                    nEquSet, inp_data->nNumEquSets,
                    j == TAUT_INI ? "Preprocessed" : "Result for", num_inp,
                    (bDisplayTaut > 0 && nNumTautComponents) ? ", mobile H"
                    : bDisplayTaut == 0                      ? ", fixed H"
                                                             : "",
                    /*j? ", mobile H":", fixed H",*/
                    k ? ", isotopic" : "",
            ...
            err = DisplayCompositeStructure(composite_norm_data, k, j, pINChI2,
                                            pINChI_Aux2, ip->bAbcNumbers,
                                            &ip->dp, ip->nMode, szTitle);
        ...
        if (DRAWDATA && j <= TAUT_YES) {
          ...
          vDrawData.pWindowData = CreateWinDataComposite_(
              composite_norm_data, k, j, pINChI2, pINChI_Aux2, ip->bAbcNumbers,
              &ip->dp, ip->nMode);
          /* vDrawData.pWindowData = CreateWinData_( composite_norm_data[j].at,
             composite_norm_data[j].num_at, k, j, pINChI[i],
             pINChI_Aux[i],ip->bAbcNumbers, &ip->dp, ip->nMode ); */
          if (vDrawData.pWindowData != NULL) {
            ...
            if (inp_data && inp_data->nEquLabels && inp_data->nNumEquSets &&
                ...
                (k == bCompareIsotopic ||
                 bCompareIsotopic &&
                     !composite_norm_data[j].bHasIsotopicLayer) &&
                (vDrawData.pWindowData->nEquLabels = (AT_NUMB *)inchi_calloc(
          ...
          /*
          vDrawData.pWindowData = CreateWinDataComposite_( composite_norm_data,
          k, j, pINChI2, pINChI_Aux2, ip->bAbcNumbers, &ip->dp, ip->nMode);
          */
          vDrawData.pWindowData = CreateWinDataComposite_(
              composite_norm_data, 1 /*k*/, j, NULL, NULL, ip->bAbcNumbers,
              &ip->dp, ip->nMode);
          if (vDrawData.pWindowData != NULL) {
            ...
            /* vDrawData.pWindowData = CreateWinData_(
               composite_norm_data[j].at, composite_norm_data[j].num_at, k, j,
               pINChI[i], pINChI_Aux[i],ip->bAbcNumbers, &ip->dp, ip->nMode );
             */
            ...
            if (inp_data && inp_data->nEquLabels && inp_data->nNumEquSets &&
                /*(j == bCompareTaut     || bCompareTaut     &&
                   !composite_norm_data[bCompareTaut].bExists) &&*/
                /*(k == bCompareIsotopic || bCompareIsotopic &&
                   !composite_norm_data[j].bHasIsotopicLayer)  &&*/
                (pDrawData->pWindowData->nEquLabels = (AT_NUMB *)inchi_calloc(
                     inp_data->num_inp_atoms,
                     sizeof(inp_data->nEquLabels[0])))) {
...
/*                  and only if the input structure has been disconnected, that
 * is,*/
/*                  num_components[INCHI_REC] > 0 */
/***********************************************************************************/
int SortAndPrintINChI(
    INCHI_IOSTREAM *output_file, char *pStr, int nStrLen,
    INCHI_IOSTREAM *log_file, INPUT_PARMS *ip, ORIG_ATOM_DATA *orig_inp_data,
    ORIG_ATOM_DATA *prep_inp_data,
    ...
    NORM_CANON_FLAGS *pncFlags, long num_inp, PINChI2 *pINChI[INCHI_NUM],
    PINChI_Aux2 *pINChI_Aux[INCHI_NUM], int *pSortPrintINChIFlags,
  ...
  int bDisconnectedCoord =
      (0 != (bTautFlagsDone[0] & TG_FLAG_DISCONNECT_COORD_DONE));
  int bINChIOutputOptions0, bCurOption, bINChIOutputOptionsCur,
      bEmbedReconnected, bAnnInXmlBrackets;
  ...
  for (j = 0, i = 0; j < INCHI_NUM; j++) {
    if (num_components[j]) {
      for (k1 = 0; k1 < TAUT_NUM; k1++) {
        pINChISort[j][k1] = (INCHI_SORT *)inchi_calloc(
            max_num_components, sizeof(pINChISort[0][0][0]));
  ...
  for (j = 0; j < INCHI_NUM; j++) {
...
#if (OUTPUT_CONNECTED_METAL_ONLY ==                                            \
     1) /* test: output connected as the only one INChI */
    ...
    for (k1 = 0; k1 < TAUT_NUM; k1++) {
      switch (k1) {
      case TAUT_NON:
        qsort(pINChISort[j][k1], num_components[j], sizeof(pINChISort[0][0][0]),
              CompINChINonTaut2);
        break;
      case TAUT_YES:
        qsort(pINChISort[j][k1], num_components[j], sizeof(pINChISort[0][0][0]),
...
#ifndef COMPILE_ANSI_ONLY
    /* find equivalent and wINChI display order; use requested in
     * ip->bCompareComponents comparison */
    ret = SaveEquComponentsInfoAndSortOrder(
        iINChI, pINChISort[j], num_components, orig_inp_data, prep_inp_data,
#if (FIX_DALKE_BUGS == 1)
  ...
  if (!(ip->bINChIOutputOptions & INCHI_OUT_PRINT_OPTIONS)) {
    /* prepare InChI from the structures obtained by reversing InChI for
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (ip->bINChIOutputOptions & bCurOption) {
        ...
        case 3:
          /* output annotation */
          bAnnInXmlBrackets =
              (i == 2 && (ip->bINChIOutputOptions & INCHI_OUT_XML));
        ...
        ret &= OutputINChI2(
            pStr, nStrLen, pINChISort, INCHI_BAS /*iINChI*/, pOrigStruct,
            bDisconnectedCoord, OUT_TN, bINChIOutputOptionsCur,
            0 != (bINChIOutputOptionsCur & INCHI_OUT_XML), ip->bAbcNumbers,
            ip->bCtPredecessors, ip->bNoStructLabels, num_components,
            num_non_taut, num_taut, output_file, log_file, num_inp,
            ip->pSdfLabel, ip->pSdfValue, ip->lSdfId, pSortPrintINChIFlags,
            save_opt_bits);

        if (ret && !(bINChIOutputOptionsCur & INCHI_OUT_EMBED_REC)) {
          ret &= OutputINChI2(
              pStr, nStrLen, pINChISort, INCHI_REC /*iINChI*/, pOrigStruct,
              bDisconnectedCoord, OUT_TN, bINChIOutputOptionsCur,
              0 != (bINChIOutputOptionsCur & INCHI_OUT_XML), ip->bAbcNumbers,
              ip->bCtPredecessors, ip->bNoStructLabels, num_components,
              num_non_taut, num_taut, output_file, log_file, num_inp,
        ...
        if (ikflag == 1) {
          if (ret) {
            ...
            if (ik_ret == INCHIKEY_OK) {
              /* NB: correctly treat tabbed output with InChIKey & hash
               * extensions */
              char csep = '\n';
...
/**********************************************************************************/
void FreeINChIArrays(PINChI2 *pINChI, PINChI_Aux2 *pINChI_Aux,
                     int num_components) {
  int i, k;
...
/*********************************************************************************************************/
void SplitTime(unsigned long ulTotalTime, int *hours, int *minutes,
...
/*********************************************************************************************************/
int ReadTheStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, INCHI_IOSTREAM *inp_file,
  ...
  INCHI_MODE InpAtomFlags =
      0; /* reading Molfile may set FLAG_INP_AT_CHIRAL bit */
  ...
  case INPUT_SDFILE:
    if (orig_inp_data) {
      if (ip->pSdfValue && ip->pSdfValue[0]) {
        ...
        int n;
        if ((p = strrchr(ip->pSdfValue, '+')) && '[' == *(p - 1) &&
            0 < (n = strtol(p + 1, &q, 10)) && q[0] && ']' == q[0] && !q[1]) {
          sprintf(p + 1, "%d]", n + 1);
        } else {
      ...
      nRet2 = MolfileToOrigAtom(inp_file->f, orig_inp_data,
                                ip->bMergeAllInputStructures, bGetOrigCoord,
                                ip->bDoNotAddH, ip->pSdfLabel, ip->pSdfValue,
                                &ip->lSdfId, &ip->lMolfileNumber, &InpAtomFlags,
       ...
       * - e_IchiMain.c #273 -- main()               -- C example of calling
       *InChI dll
      ...
      } else if ((InpAtomFlags & FLAG_INP_AT_CHIRAL) &&
      ...
      sd->bChiralFlag |=
          InpAtomFlags & (FLAG_INP_AT_CHIRAL | FLAG_INP_AT_NONCHIRAL);
      ...
      if ((ip->nMode & REQ_MODE_CHIR_FLG_STEREO) &&
        ...
        if (InpAtomFlags & FLAG_INP_AT_CHIRAL) {
          ...
          sd->bChiralFlag |= FLAG_INP_AT_NONCHIRAL; /* write AuxInfo as
                                                       explicitly not chiral */
        }
      }
    } else {
      ...
      if (!ip->bMergeAllInputStructures) {
        nRet2 = MolfileToOrigAtom(inp_file->f, NULL, 0, 0, 0, NULL, NULL, NULL,
                                  NULL, NULL, &nStructReadError, NULL);
  ...
  case INPUT_INCHI_PLAIN:
    if (orig_inp_data) {
      ...
      sd->fPtrStart = (inp_file->f == stdin) ? -1 : ftell(inp_file->f);
      /*  read the original structure */
      nRet2 = INChIToOrigAtom(
          inp_file, orig_inp_data, ip->bMergeAllInputStructures, bGetOrigCoord,
          ip->bDoNotAddH, vABParityUnknown, ip->nInputType, ip->pSdfLabel,
          ip->pSdfValue, &ip->lMolfileNumber, &InpAtomFlags,
          &sd->nStructReadError, sd->pStrErrStruct);
      ...
      if (!ip->bMergeAllInputStructures) {
        nRet2 =
            INChIToOrigAtom(inp_file, NULL, 0, 0, 0, 0, ip->nInputType, NULL,
  ...
  case INPUT_CMLFILE:
    if (orig_inp_data) {
      ...
      */
      ...
      sd->fPtrEnd = -1;
      /*  read the original structure */
      nRet = CmlfileToOrigAtom(
          inp_file->f, orig_inp_data, ip->bMergeAllInputStructures,
          bGetOrigCoord, ip->bDoNotAddH, inp_index, out_index, ip->pSdfLabel,
          ip->pSdfValue, &ip->lSdfId, &sd->nStructReadError, sd->pStrErrStruct);
      ...
      if (!ip->bMergeAllInputStructures) {
        nRet2 =
            CmlfileToOrigAtom(inp_file->f, NULL, 0, 0, 0, inp_index, out_index,
                              NULL, NULL, NULL, &nStructReadError, NULL);
...
#endif
/*****************************************************************************************************/
int TreatReadTheStructureErrors(
    STRUCT_DATA *sd, INPUT_PARMS *ip, int nLogMask, INCHI_IOSTREAM *inp_file,
  ...
  if (10 < sd->nStructReadError && sd->nStructReadError < 20) {
    if (sd->pStrErrStruct[0]) {
      inchi_ios_eprint(log_file,
                       "%s inp structure #%ld: End of file.%s%s%s%s    \n",
  ...
  if (*num_inp < ip->first_struct_number) {
    ...
    if (log_file->f != stderr) {
      inchi_fprintf(stderr, "\rSkipping structure #%ld.%s%s%s%s...", *num_inp,
                    SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    }
#endif
  ...
  sd->nErrorType =
      GetInpStructErrorType(ip, sd->nStructReadError, sd->pStrErrStruct,
  ...
  if ((ip->bINChIOutputOptions & INCHI_OUT_XML) && !sd->bXmlStructStarted) {
    if (!OutputINChIXmlStructStartTag(output_file, pStr, 1, nStrLen,
                                      ip->bNoStructLabels, *num_inp,
                                      ip->pSdfLabel, ip->pSdfValue)) {
      inchi_ios_eprint(log_file,
                       "Cannot create start xml tag for structure "
                       "#%ld.%s%s%s%s Terminating.\n",
  ...
  if (sd->nErrorType == _IS_FATAL) {
    if (nLogMask & LOG_MASK_FATAL)
      inchi_ios_eprint(
          log_file,
          "Fatal Error %d (aborted; %s) inp structure #%ld.%s%s%s%s\n",
    ...
    if (prb_file->f && 0L <= sd->fPtrStart && sd->fPtrStart < sd->fPtrEnd &&
      ...
      CopyMOLfile(inp_file->f, sd->fPtrStart, sd->fPtrEnd, prb_file->f,
                  *num_inp);
    }
#endif
    /* goto exit_function; */
  ...
  if (sd->nErrorType == _IS_ERROR) { /*  70 => too many atoms */
    if (nLogMask & LOG_MASK_ERR)
      inchi_ios_eprint(
          log_file, "Error %d (no %s; %s) inp structure #%ld.%s%s%s%s\n",
          sd->nStructReadError,
          (ip->bINChIOutputOptions & INCHI_OUT_SDFILE_ONLY) ? "Molfile"
                                                            : INCHI_NAME,
          sd->pStrErrStruct, *num_inp,
          SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
#if (bRELEASE_VERSION == 1 || EXTR_FLAGS == 0)
  ...
  if ((ip->bINChIOutputOptions & INCHI_OUT_XML)
    ...
    if (sd->nErrorType != _IS_OKAY && sd->nErrorType != _IS_WARNING) {
      sd->nErrorType = ProcessStructError(
          output_file, log_file, /*sd->nStructReadError,*/
          sd->pStrErrStruct, sd->nErrorType, &sd->bXmlStructStarted, *num_inp,
          ip, pStr, nStrLen);
    }
  }
exit_function:
...
/******************************************************************************************************/
int GetOneComponent(STRUCT_DATA *sd, INPUT_PARMS *ip, INCHI_IOSTREAM *log_file,
                    INCHI_IOSTREAM *output_file, INP_ATOM_DATA *inp_cur_data,
                    ORIG_ATOM_DATA *orig_inp_data, int i, long num_inp,
                    char *pStr, int nStrLen) {
  inchiTime ulTStart;
  InchiTimeGet(&ulTStart);
  CreateInpAtomData(inp_cur_data, orig_inp_data->nCurAtLen[i], 0);
  inp_cur_data->num_at = ExtractConnectedComponent(
      orig_inp_data->at, orig_inp_data->num_inp_atoms, i + 1, inp_cur_data->at);
  sd->ulStructTime += InchiTimeElapsed(&ulTStart);

  /*  error processing */
  if (inp_cur_data->num_at <= 0 ||
      orig_inp_data->nCurAtLen[i] != inp_cur_data->num_at) {
    /*  log error message */
    ...
    inchi_ios_eprint(log_file, "%s #%d structure #%ld.%s%s%s%s\n",
                     ...
                     SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    sd->nErrorCode = inp_cur_data->num_at < 0 ? inp_cur_data->num_at
                     : (orig_inp_data->nCurAtLen[i] != inp_cur_data->num_at)
                         ? CT_ATOMCOUNT_ERR
                         : CT_UNKNOWN_ERR;
    /* num_err ++; */
    ...
    if ((ip->bINChIOutputOptions & INCHI_OUT_XML)
      ...
      /*  xml error message */
      sd->nErrorType = ProcessStructError(
          output_file, log_file, /*sd->nErrorCode,*/ sd->pStrErrStruct,
...
int GetProcessingWarningsOneINChI(INChI *pINChI, INP_ATOM_DATA *inp_norm_data,
  ...
  if (inp_norm_data->at) {
    for (j = 0; j < pINChI->nNumberOfAtoms; j++) {
      if (inp_norm_data->at[j].bAmbiguousStereo &
          (AMBIGUOUS_STEREO_ATOM | AMBIGUOUS_STEREO_ATOM_ISO)) {
        nAmbiguousStereoAtoms++;
      }
      if (inp_norm_data->at[j].bAmbiguousStereo &
          (AMBIGUOUS_STEREO_BOND | AMBIGUOUS_STEREO_BOND_ISO)) {
...
int GetProcessingWarnings(INChI *cur_INChI[], INP_ATOM_DATA **inp_norm_data,
  ...
  for (i = 0; i < TAUT_NUM; i++) {
    if (cur_INChI[i] && cur_INChI[i]->nNumberOfAtoms > 0) {
      ret |= GetProcessingWarningsOneINChI(cur_INChI[i], inp_norm_data[i],
                                           sd->pStrErrStruct);
    }
  }
  return ret;
...
int CreateOneComponentINChI(STRUCT_DATA *sd, INPUT_PARMS *ip,
                            ...
                            ORIG_ATOM_DATA *orig_inp_data, PINChI2 *pINChI,
                            PINChI_Aux2 *pINChI_Aux, int iINChI, int i,
                            long num_inp, INP_ATOM_DATA **inp_norm_data,
                            NORM_CANON_FLAGS *pncFlags,
  ...
  INCHI_MODE bTautFlagsDone =
      (ip->bTautFlagsDone | sd->bTautFlagsDone[INCHI_BAS]);
  ...
  */
  InchiTimeGet(&ulTStart);
  bOrigCoord = !(ip->bINChIOutputOptions &
                 (INCHI_OUT_NO_AUX_INFO | INCHI_OUT_SHORT_AUX_INFO));
...
/*^^^#if ( !defined( TARGET_LIB_FOR_WINCHI ) && !defined( TARGET_API_LIB ) ) */
#if (!defined(TARGET_LIB_FOR_WINCHI) && !defined(TARGET_API_LIB) &&            \
     !defined(TARGET_EXE_STANDALONE))
#if (TEST_RENUMB_ATOMS != 1)
  /*  log file / console output */
  if (log_file->f &&
      log_file->f != stderr) { /* NULL log_file now ignored. 11-23-2005 */
    if (ip->bDisplay)
      inchi_ios_eprint(log_file, "Component #%d structure #%ld.%s%s%s%s...\n",
                       i + 1, num_inp,
                       SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    else
      inchi_fprintf(stderr, "Component #%d structure #%ld.%s%s%s%s...\r", i + 1,
                    num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
  }
#endif
#endif
  ...
  num_at = Create_INChI(cur_INChI, cur_INChI_Aux, orig_inp_data /* not used */,
                        inp_cur_data->at, inp_norm_data, inp_cur_data->num_at,
                        ip->nMode, &bTautFlags, &bTautFlagsDone, pulTEnd, NULL,
                        sd->pStrErrStruct);
...
#if (bRELEASE_VERSION == 0)
  if (cur_INChI[TAUT_NON])
    sd->bExtract |= cur_INChI[TAUT_NON]->bExtract;
  if (cur_INChI[TAUT_YES])
    sd->bExtract |= cur_INChI[TAUT_YES]->bExtract;
  ...
  /*  b) Count one component structure and/or INChI results only if there was no
   * error */
  ...
  if (!sd->nErrorCode) {
    ...
    int cur_is_in_non_taut =
        (pINChI[i][TAUT_NON] && pINChI[i][TAUT_NON]->nNumberOfAtoms > 0);
    int cur_is_in_taut =
        (pINChI[i][TAUT_YES] && pINChI[i][TAUT_YES]->nNumberOfAtoms > 0);
    int cur_is_non_taut =
        (cur_is_in_non_taut && 0 == pINChI[i][TAUT_NON]->lenTautomer) ||
    ...
    if (cur_is_non_taut + cur_is_taut) {
      ...
      for (j = j1; j <= j2; j++) {
        ...
        inp_norm_data[j]->bExists =
            1; /*  j=0: non-taut exists, j=1: taut exists */
  ...
  /*
      return (sd->nErrorCode==CT_OUT_OF_RAM ||
     sd->nErrorCode==CT_USER_QUIT_ERR)? _IS_FATAL : sd->nErrorCode? _IS_ERROR :
     0;
  */
...
/****************************************************************************************************/
int TreatCreateOneComponentINChIError(
    STRUCT_DATA *sd, INPUT_PARMS *ip, ORIG_ATOM_DATA *orig_inp_data, int i,
    long num_inp, INCHI_IOSTREAM *inp_file, INCHI_IOSTREAM *log_file,
  ...
  if (sd->nErrorCode) {
    ...
    inchi_ios_eprint(log_file,
                     ...
                     sd->nErrorCode, sd->pStrErrStruct, num_inp, i + 1,
                     SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    sd->nErrorType =
        (sd->nErrorCode == CT_OUT_OF_RAM || sd->nErrorCode == CT_USER_QUIT_ERR)
            ? _IS_FATAL
            : _IS_ERROR;
    if ((ip->bINChIOutputOptions & INCHI_OUT_XML)
#ifdef TARGET_LIB_FOR_WINCHI
      ...
      if (sd->nErrorCode && prb_file->f && 0L <= sd->fPtrStart &&
          sd->fPtrStart < sd->fPtrEnd && !ip->bSaveAllGoodStructsAsProblem) {
        CopyMOLfile(inp_file->f, sd->fPtrStart, sd->fPtrEnd, prb_file->f,
                    num_inp);
      }
    }
...
/*^^^ #ifndef TARGET_API_LIB */
  ...
  if (log_file->f && log_file->f != stderr &&
      (sd->ulStructTime >= 1000 || sd->nErrorCode)) {
    fprintf(log_file->f,
            "%10lu msec structure #%ld.%s%s%s%s (%d component%s, %d atom%s, "
            "error=%d).\n",
            sd->ulStructTime, num_inp,
            SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
            orig_inp_data->num_components,
            orig_inp_data->num_components == 1 ? "" : "s",
            orig_inp_data->num_inp_atoms,
            orig_inp_data->num_inp_atoms == 1 ? "" : "s", sd->nErrorCode);
  }
#endif
  return sd->nErrorType;
}
/****************************************************************************************************/
int TreatCreateINChIWarning(
    STRUCT_DATA *sd, INPUT_PARMS *ip, ORIG_ATOM_DATA *orig_inp_data,
    long num_inp, INCHI_IOSTREAM *inp_file, INCHI_IOSTREAM *log_file,
    INCHI_IOSTREAM *output_file,
    INCHI_IOSTREAM *prb_file, /*^^^ was: INCHI_IOSTREAM */
    char *pStr, int nStrLen) {
  ...
  if (!sd->nErrorCode && sd->pStrErrStruct[0]) {
    ...
    else if ((EXTR_MASK ? ((sd->bExtract & EXTR_MASK) == EXTR_FLAGS)
                        : (sd->bExtract & EXTR_FLAGS)) &&
             !ip->bSaveAllGoodStructsAsProblem && prb_file->f &&
  ...
  if (log_file && log_file != stderr) {
    fprintf(log_file,
            "%10lu msec structure %1dD #%ld.%s%s%s%s (%d component%s, %d "
            "atom%s, error=%d).\n",
            sd->ulStructTime, orig_inp_data->num_dimensions, num_inp,
            SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
            orig_inp_data->num_components,
            orig_inp_data->num_components == 1 ? "" : "s",
            orig_inp_data->num_inp_atoms,
  ...
  if (log_file) {
    inchi_ios_eprint(log_file,
                     "%10lu msec structure %1dD #%ld.%s%s%s%s (%d component%s, "
                     "%d atom%s, error=%d).\n",
                     sd->ulStructTime, orig_inp_data->num_dimensions, num_inp,
                     SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
                     orig_inp_data->num_components,
                     orig_inp_data->num_components == 1 ? "" : "s",
                     orig_inp_data->num_inp_atoms,
                     orig_inp_data->num_inp_atoms == 1 ? "" : "s",
                     sd->nErrorCode);
  }
#endif
#endif
...
int DuplicateOrigAtom(ORIG_ATOM_DATA *new_orig_atom,
  ...
  if (at && nCurAtLen && nOldCompNumber) {
    ...
    *new_orig_atom = *orig_atom;
    ...
    memset(new_orig_atom->bSavedInINCHI_LIB, 0,
           sizeof(new_orig_atom->bSavedInINCHI_LIB));
    memset(new_orig_atom->bPreprocessed, 0,
           sizeof(new_orig_atom->bPreprocessed));
...
int GetOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                    ...
                    INCHI_IOSTREAM *output_file,
                    INCHI_IOSTREAM *prb_file, /*^^^ was: INCHI_IOSTREAM */
                    ORIG_ATOM_DATA *orig_inp_data, long *num_inp, char *pStr,
                    int nStrLen, STRUCT_FPTRS *struct_fptrs) {
  ...
  */
  ...
  if (struct_fptrs) {
    ...
    if (!struct_fptrs->fptr ||
      ...
      INCHI_FPTR *new_fptr = (INCHI_FPTR *)inchi_calloc(
          struct_fptrs->len_fptr + ADD_LEN_STRUCT_FPTRS, sizeof(new_fptr[0]));
      if (new_fptr) {
        if (struct_fptrs->fptr) {
          if (struct_fptrs->len_fptr) {
            memcpy(new_fptr, struct_fptrs->fptr,
    ...
    if (struct_fptrs->fptr[struct_fptrs->cur_fptr] == EOF) {
      ...
      if (bUseFptr) {
        if (fseek(inp_file->f, struct_fptrs->fptr[struct_fptrs->cur_fptr],
                  SEEK_SET)) {
          return _IS_FATAL;
        }
  ...
  nRet =
      ReadTheStructure(sd, ip, inp_file, orig_inp_data, inp_index, &out_index);
  ...
  if (!nRet) {
    ...
    if (ip->nInputType == INPUT_INCHI_PLAIN ||
        ip->nInputType == INPUT_INCHI_XML || ip->nInputType == INPUT_MOLFILE ||
    ...
    nRet = TreatReadTheStructureErrors(sd, ip, LOG_MASK_ALL, inp_file, log_file,
                                       output_file, prb_file, orig_inp_data,
  ...
  /************************************************************/
  if (struct_fptrs && struct_fptrs->fptr &&
    ...
    if (nRet == _IS_EOF || nRet == _IS_FATAL || nRet2 == _IS_EOF ||
        nRet2 == _IS_FATAL) {
      struct_fptrs->fptr[struct_fptrs->cur_fptr + 1] = EOF;
    } else {
      struct_fptrs->fptr[struct_fptrs->cur_fptr + 1] =
          bUseFptr ? sd->fPtrEnd : inp_index;
    }
...
int RenumberingTestInit(RENUMB_DATA *pRenumbData, INP_ATOM_DATA *inp_cur_data) {
  ...
  memset(&pRenumbData->orig_inp_cur_data, 0,
         sizeof(pRenumbData->orig_inp_cur_data));
  memset(&pRenumbData->saved_inp_cur_data, 0,
         sizeof(pRenumbData->saved_inp_cur_data));
  memset(pRenumbData->ren_inp_norm_data[0], 0,
         sizeof(*pRenumbData->ren_inp_norm_data[0]));
  memset(pRenumbData->ren_inp_norm_data[1], 0,
         sizeof(*pRenumbData->ren_inp_norm_data[1]));
#if (TEST_RENUMB_ATOMS_SAVE_LONGEST == 1)
  memset(&pRenumbData->longest_inp_cur_data, 0,
         sizeof(pRenumbData->longest_inp_cur_data));
#endif
  CopyInpAtomData(&pRenumbData->orig_inp_cur_data, inp_cur_data);
  pRenumbData->ren_counter = pRenumbData->orig_inp_cur_data.num_at *
                             pRenumbData->orig_inp_cur_data.num_at;
  srand(1); /*  for reproducibility */
  rand();   /*  shift to avoid prev. sequences */
  pRenumbData->nComp = 0;
  /*ren_counter = 29;*/
  pRenumbData->new_ord = (AT_RANK *)inchi_calloc(
      pRenumbData->orig_inp_cur_data.num_at, sizeof(pRenumbData->new_ord[0]));
...
/************************************************************************************************/
int RenumberingTest(PINChI2 *pINChI, PINChI_Aux2 *pINChI_Aux,
                    ORIG_ATOM_DATA *orig_inp_data, int iINChI,
                    RENUMB_DATA *pRenumbData, INP_ATOM_DATA *inp_cur_data,
                    INP_ATOM_DATA **inp_norm_data, STRUCT_DATA *sd,
                    INPUT_PARMS *ip, char *szTitle, INCHI_IOSTREAM *log_file,
  ...
  while (--pRenumbData->ren_counter >= 0 && !pRenumbData->nRet2) {
    ...
    RenumbInpAtomData(inp_cur_data /* output*/,
                      &pRenumbData->orig_inp_cur_data /* input*/,
                      pRenumbData->new_ord /* input*/);
#if (TEST_RENUMB_ATOMS_SAVE_LONGEST == 1)
    ...
    pRenumbData->nRet2 = CreateOneComponentINChI(
        sd, ip, inp_cur_data, NULL /*orig_inp_data*/, pRenumbData->ren_INChI2,
        pRenumbData->ren_INChI_Aux, iINChI, 0, num_inp,
        pRenumbData->ren_inp_norm_data, pncFlags, log_file);
    /*
    CreateOneComponentINChI( sd, ip, inp_cur_data, orig_inp_data,
    pINChI2[iINChI], pINChI_Aux2[iINChI], iINChI, i, num_inp, inp_norm_data,
    log_file );
    */
    if (!pRenumbData->nRet2) {
      pRenumbData->c1 = CompareINChI(
          pINChI[i][TAUT_NON], pRenumbData->ren_INChI2[0][TAUT_NON],
          pINChI_Aux[i][TAUT_NON], pRenumbData->ren_INChI_Aux[0][TAUT_NON]);
      pRenumbData->c2 = CompareINChI(
          pINChI[i][TAUT_YES], pRenumbData->ren_INChI2[0][TAUT_YES],
      ...
      if (pRenumbData->c1 || pRenumbData->c2 || pRenumbData->nRet2) {
        ...
        inchi_ios_eprint(log_file,
                         "Compare (%d,%d) %d (err=%d) %s structure #%d "
                         "component %d.%s%s%s%s\n",
                         pRenumbData->c1, pRenumbData->c2, pRenumbData->nComp,
                         pRenumbData->nRet2, INCHI_NAME, num_inp, i + 1,
                         SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
        ...
        if (pRenumbData->longest_inp_cur_data.at) {
          for (k = 0; k < pRenumbData->longest_inp_cur_data.num_at; k++) {
            pRenumbData->longest_inp_cur_data.at[k].orig_at_number =
                k + 1; /* display new atom numbers */
          }
        }
#endif
      }
      ...
      inchi_ios_eprint(
          stderr,
          "\rComp#%d str#%ld/%d%s%s%s%s Ren %d/%d n(%lu:%lu)c(%lu:%lu)...\r",
          i + 1, num_inp, iINChI, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
          pRenumbData->nComp, pRenumbData->ren_counter + pRenumbData->nComp,
          pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulNormTime,
          pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulCanonTime,
          pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulNormTime,
          pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulCanonTime);
#endif
      /*  make sure the max. time is not overwritten */
      pRenumbData->ulCurTime0 =
          pRenumbData->ren_INChI_Aux[0][TAUT_NON]
              ? (pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulNormTime +
                 pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulCanonTime)
              : 0;
      pRenumbData->ulCurTime1 =
          pRenumbData->ren_INChI_Aux[0][TAUT_YES]
              ? (pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulNormTime +
                 pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulCanonTime)
              : 0;
      pRenumbData->ulCurTime =
          inchi_max(pRenumbData->ulCurTime0, pRenumbData->ulCurTime1);

      pRenumbData->ulCurTimeCanon0 =
          pRenumbData->ren_INChI_Aux[0][TAUT_NON]
              ? pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulCanonTime
              : 0;
      pRenumbData->ulCurTimeCanon1 =
          pRenumbData->ren_INChI_Aux[0][TAUT_YES]
              ? pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulCanonTime
              : 0;
      pRenumbData->ulCurTimeCanon =
          inchi_max(pRenumbData->ulCurTimeCanon0, pRenumbData->ulCurTimeCanon1);

      pRenumbData->ulCurTimeNorm0 =
          pRenumbData->ren_INChI_Aux[0][TAUT_NON]
              ? pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulNormTime
              : 0;
      pRenumbData->ulCurTimeNorm1 =
          pRenumbData->ren_INChI_Aux[0][TAUT_YES]
              ? pRenumbData->ren_INChI_Aux[0][TAUT_YES]->ulNormTime
              : 0;
      pRenumbData->ulCurTimeNorm =
          inchi_max(pRenumbData->ulCurTimeNorm0, pRenumbData->ulCurTimeNorm1);
...
#if (TEST_RENUMB_ATOMS_SAVE_LONGEST == 1 || TEST_RENUMB_SWITCH == 1)
      if (bLongerTime ||
          TEST_RENUMB_SWITCH == 1 &&
              (pRenumbData->c1 || pRenumbData->c2 || pRenumbData->nRet2)) {
        char szLine[512];
        char szValue[512];
        inchi_ios_eprint(stderr, "\n");
        sprintf(
            szLine, "Comp#%d str#%ld/%d%s%s%s%s Ren %d/%d n=%lu:%lu c=%lu:%lu",
            i + 1, num_inp, iINChI, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
            pRenumbData->nComp, pRenumbData->ren_counter + pRenumbData->nComp,
            pRenumbData->ren_INChI_Aux[0][TAUT_NON]
                ? pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulNormTime
        ...
        sprintf(szValue, "%s (c%d/s%ld/i%d, r%d/%d n=%lu:%lu c=%lu:%lu)",
                ...
                i + 1, num_inp, iINChI, pRenumbData->nComp,
                pRenumbData->ren_counter + pRenumbData->nComp,
                pRenumbData->ren_INChI_Aux[0][TAUT_NON]
                    ? pRenumbData->ren_INChI_Aux[0][TAUT_NON]->ulNormTime
                    : 0,
                pRenumbData->ren_INChI_Aux[0][TAUT_NON]
        ...
        WriteToSDfile(&pRenumbData->longest_inp_cur_data, prb_file, szLine,
                      NULL, ip->pSdfLabel, szValue);
      }
#endif

#if (TEST_RENUMB_SWITCH == 1)
      if (pRenumbData->c1 || pRenumbData->c2 || !pRenumbData->ren_counter) {
        inchi_swap((char *)&pINChI[i][TAUT_NON],
                   (char *)&pRenumbData->ren_INChI2[0][TAUT_NON],
                   sizeof(&pRenumbData->ren_INChI2[0][0]));
        inchi_swap((char *)&pINChI[i][TAUT_YES],
                   (char *)&pRenumbData->ren_INChI2[0][TAUT_YES],
                   sizeof(&pRenumbData->ren_INChI2[0][0]));
        inchi_swap((char *)&pINChI_Aux[i][TAUT_NON],
                   (char *)&pRenumbData->ren_INChI_Aux[0][TAUT_NON],
                   sizeof(&pRenumbData->ren_INChI_Aux[0][0]));
        inchi_swap((char *)&pINChI_Aux[i][TAUT_YES],
                   (char *)&pRenumbData->ren_INChI_Aux[0][TAUT_YES],
                   sizeof(&pRenumbData->ren_INChI_Aux[0][0]));
      }
#endif
    }
  ...
  pRenumbData->num_non_taut =
      sd->num_non_taut[iINChI] - pRenumbData->num_non_taut0;
  sd->num_taut[iINChI] = pRenumbData->num_taut0;
  sd->num_non_taut[iINChI] = pRenumbData->num_non_taut0;
  if (pRenumbData->num_taut % pRenumbData->nComp ||
      pRenumbData->num_non_taut % pRenumbData->nComp) {
    inchi_ios_eprint(
        log_file,
        "Compare (%d,%d) %d (err=%d) %s structure #%ld component %d.%s%s%s%s\n",
        pRenumbData->num_non_taut % pRenumbData->nComp,
        pRenumbData->num_taut % pRenumbData->nComp, pRenumbData->nComp, 333,
        INCHI_NAME, num_inp, i + 1, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
  }
#if (TEST_RENUMB_SWITCH == 1) /* { */
  CopyInpAtomData(inp_norm_data[TAUT_NON],
                  pRenumbData->ren_inp_norm_data[TAUT_NON]);
  CopyInpAtomData(inp_norm_data[TAUT_YES],
                  pRenumbData->ren_inp_norm_data[TAUT_YES]);
  /*  renumbered input structure */
#ifndef COMPILE_ANSI_ONLY /* { */
  if (/*ip->bDisplayEachComponentINChI &&*/ !pRenumbData->nRet2) {
    int err, len;
    /*
    err = DisplayStructure( inp_cur_data->at, inp_cur_data->num_at, 0, 1, 0,
    NULL. 1, 0, NULL, NULL, ip->bAbcNumbers, &ip->dp, ip->nMode, szTitle );
    */
    err = DisplayStructure(inp_cur_data->at, inp_cur_data->num_at, 0, 1, 0,
                           NULL, 1 /*isotopic*/, 0 /*taut*/, NULL, NULL,
                           ip->bAbcNumbers, &ip->dp, ip->nMode, szTitle);
    if (pRenumbData->c1 || pRenumbData->c2) {
      len = strlen(szTitle);
      strcat(szTitle, " (Renumbered)");
      err = DisplayStructure(pRenumbData->longest_inp_cur_data.at,
                             pRenumbData->longest_inp_cur_data.num_at, 0, 1, 0,
                             NULL, 1, 0, NULL, NULL, ip->bAbcNumbers, &ip->dp,
                             ip->nMode, szTitle);
      szTitle[len] = '\0';
    }
...
int bCheckUnusualValences(ORIG_ATOM_DATA *orig_at_data, int bAddIsoH,
  ...
  char msg[32];
  int len, num_H;
  inp_ATOM *at = (orig_at_data && orig_at_data->num_inp_atoms > 0)
                     ? orig_at_data->at
                     : NULL;
  ...
  if (at) {
    for (i = 0, num_found = 0; i < orig_at_data->num_inp_atoms; i++) {
      ...
      val = detect_unusual_el_valence(at[i].el_number, at[i].charge,
                                      at[i].radical, at[i].chem_bonds_valence,
                                      num_H, at[i].valence);
      if (val) {
        num_found++;
        /* produce message */
        ...
        if (at[i].radical) {
          len += sprintf(msg + len, ",%s",
                         at[i].radical == RADICAL_SINGLET   ? "s"
...
int PreprocessOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip,
  ...
  if (0 < fix_odd_things(prep_inp_data->num_inp_atoms, prep_inp_data->at,
                         /*0*/ ip->bTautFlags & TG_FLAG_FIX_SP3_BUG,
  ...
  if (ip->bTautFlags & TG_FLAG_FIX_ADJ_RADICALS) {
    if (0 <
        FixAdjacentRadicals(prep_inp_data->num_inp_atoms, prep_inp_data->at)) {
  ...
  if (ip->bTautFlags & TG_FLAG_DISCONNECT_COORD) {
    ...
    bMayDisconnectMetals(
        prep_inp_data, i,
        &bTautFlagsDone); /* changes prep_inp_data->bDisconnectCoord */
    sd->bTautFlagsDone[INCHI_BAS] |=
        bTautFlagsDone; /* whether any disconnection has been rejected because
                           of the metal proper valence */
#if (bRELEASE_VERSION == 0)
  ...
  if ((ip->bTautFlags & TG_FLAG_DISCONNECT_SALTS) &&
      prep_inp_data->bDisconnectSalts &&
    ...
    if ((i = ReconcileAllCmlBondParities(prep_inp_data->at,
                                         prep_inp_data->num_inp_atoms, 0))) {
      char szErrCode[16];
  ...
  if ((i = bNumHeterAtomHasIsotopicH(prep_inp_data->at,
                                     prep_inp_data->num_inp_atoms))) {
    if (i & 1) {
  ...
  if (prep_inp_data->bDisconnectCoord) {
    ...
    prep_inp_data->num_components =
        MarkDisconnectedComponents(prep_inp_data, 0);
    if (prep_inp_data->num_components < 0) {
      AddMOLfileError(sd->pStrErrStruct, "Out of RAM");
      sd->nStructReadError = 99;
      sd->nErrorType = _IS_FATAL;
    ...
    if (0 != (ip->bTautFlags & TG_FLAG_RECONNECT_COORD)) {
      ...
      { /* remove "parity undefined in disconnected structure" flag from
           reconnected structure */
        int k, m, p;
        inp_ATOM *at = (prep_inp_data + 1)->at;
        int num_at = (prep_inp_data + 1)->num_inp_atoms;
        for (k = 0; k < num_at; k++) {
          for (m = 0; m < MAX_NUM_STEREO_BONDS && (p = at[k].sb_parity[m]);
               m++) {
    ...
    if (0 < (i = DisconnectMetals(prep_inp_data, i, &bTautFlagsDone))) {
...
#endif
      /* last parm=1 means find link between unchanged by Metal Disconnection
       * components */
      prep_inp_data->num_components =
          MarkDisconnectedComponents(prep_inp_data, 1);
      if (prep_inp_data->num_components < 0) {
        AddMOLfileError(sd->pStrErrStruct, "Out of RAM");
        sd->nStructReadError = 99;
        sd->nErrorType = _IS_FATAL;
      ...
      if ((i = ReconcileAllCmlBondParities(prep_inp_data->at,
                                           prep_inp_data->num_inp_atoms, 1))) {
        char szErrCode[16];
        sprintf(szErrCode, "%d", i);
        AddMOLfileError(sd->pStrErrStruct,
                        "0D Parities Reconciliation failed:");
        AddMOLfileError(sd->pStrErrStruct, szErrCode);
      }

#if (REMOVE_ION_PAIRS_DISC_STRU == 1)
      if (0 <
          remove_ion_pairs(prep_inp_data->num_inp_atoms, prep_inp_data->at)) {
        AddMOLfileError(sd->pStrErrStruct, "Charges were rearranged");
        if (sd->nErrorType < _IS_WARNING) {
          sd->nErrorType = _IS_WARNING;
        }
      ...
      /*
        ...
        component #(i+1) in prep_inp_data is identical to component #(iINChI+1)
        in prep_inp_data+1
      */
    } else if (i < 0) {
...
exit_function:
  ...
  if (sd->nErrorType < _IS_ERROR && prep_inp_data) {
    ...
    if (0 <
        post_fix_odd_things(prep_inp_data->num_inp_atoms, prep_inp_data->at)) {
      AddMOLfileError(sd->pStrErrStruct, "Charges were rearranged");
      if (sd->nErrorType < _IS_WARNING) {
        sd->nErrorType = _IS_WARNING;
      }
    ...
    if ((sd->bTautFlagsDone[INCHI_BAS] & TG_FLAG_DISCONNECT_COORD_DONE) &&
      ...
      if (0 < post_fix_odd_things((prep_inp_data + 1)->num_inp_atoms,
                                  (prep_inp_data + 1)->at)) {
        AddMOLfileError(sd->pStrErrStruct, "Charges were rearranged");
        if (sd->nErrorType < _IS_WARNING) {
          sd->nErrorType = _IS_WARNING;
  ...
  sd->bTautFlags[INCHI_BAS] |= bTautFlags; /* TG_FLAG_CHECK_VALENCE_COORD_DONE,
...
int DisplayTheWholeStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
  ...
  /* in case of TARGET_LIB_FOR_WINCHI equivalence info is always unknown here
   * and bOriginalReconnected=0 */
  int bOriginalReconnected =
      iINChI < 0 && orig_inp_data && orig_inp_data->nEquLabels &&
      (sd->bTautFlagsDone[INCHI_BAS] & TG_FLAG_DISCONNECT_COORD_DONE) &&
  ...
  bDisplayEqu = bShowStruct && ip->bDisplay && ip->dp.nEquLabels &&
                0 < ip->dp.nCurEquLabel &&
  ...
  if (ip->bDisplay && inp_file->f != stdin) {
    if (user_quit(bDisplayEqu ? "Enter=Display identical components, Esc=Stop ?"
  ...
  if (bShowStruct && ip->bDisplay) {
    if (bDisplayEqu) {
      sprintf(szTitle, " Equ Set %d of %d, Input Structure #%ld.%s%s%s%s%s",
              ip->dp.nCurEquLabel, ip->dp.nNumEquSets, num_inp,
              SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue), lpszType);
    } else {
      sprintf(szTitle, "Input Structure #%ld.%s%s%s%s%s", num_inp,
              SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue), lpszType);
    }
    err = DisplayStructure(orig_inp_data->at, orig_inp_data->num_inp_atoms, 0,
                           1, 0, NULL, 1 /*isotopic*/, 0 /*taut*/, NULL, NULL,
                           ip->bAbcNumbers, &ip->dp, ip->nMode, szTitle);
  ...
  if (!bDisplayEqu) {
    ...
    if (ip->bDisplay && !sd->bUserQuitComponent) {
      if (iINChI == 1) {
        if (ip->bDisplay)
          inchi_ios_eprint(log_file,
                           "Processing (rec) structure #%ld.%s%s%s%s...\n",
                           num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
        else
          inchi_fprintf(stderr, "Processing (rec) structure #%ld.%s%s%s%s...\r",
                        num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
      } else {
        if (ip->bDisplay)
          ...
          inchi_fprintf(stderr, "Processing structure #%ld.%s%s%s%s...\r",
                        num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
      }
    }
  }
...
#else
  if ((ip->bDisplay || (ip->bCompareComponents & CMP_COMPONENTS)) &&
      bINCHI_LIB_Flag)
#endif
  {
    ...
    for (bBit = 1, k = 0; k < 8; k++, bBit <<= 1) {
      ...
      sprintf(szTitle, "%s Structure #%ld.%s%s%s%s",
              bPreprocessed  ? "Preprocessed"
              : bReconnected ? "Reconnected"
                             : "Input",
              num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
      ...
      if (DRAWDATA && DRAWDATA_EXISTS) {
        struct DrawData vDrawData;
        int nType = bPreprocessed ? COMPONENT_ORIGINAL_PREPROCESSED
                                  : COMPONENT_ORIGINAL;
        ...
        vDrawData.pWindowData =
            CreateWinData_(orig_inp_data->at, orig_inp_data->num_inp_atoms, 0,
                           1 /* bAdd_DT_to_num_H */, 0, NULL, 1, 0, NULL, NULL,
                           ip->bAbcNumbers, &ip->dp, ip->nMode);
        if (vDrawData.pWindowData != NULL) {
          vDrawData.nComponent = nComponent;
          vDrawData.nType =
              nType; /* COMPONENT_ORIGINAL or COMPONENT_ORIGINAL_PREPROCESSED */
...
int ProcessOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                        ...
                        INCHI_IOSTREAM *prb_file, /*^^^ was: INCHI_IOSTREAM */
                        ORIG_ATOM_DATA *orig_inp_data,
                        ORIG_ATOM_DATA *prep_inp_data, long num_inp, char *pStr,
  ...
  if (ip->bUnderivatize && 0 > (ret2 = underivatize(orig_inp_data))) {
    ...
    sd->nErrorType = _IS_ERROR;
    nRet = _IS_ERROR;
    TreatReadTheStructureErrors(sd, ip, LOG_MASK_ALL, inp_file, log_file,
                                output_file, prb_file, prep_inp_data, &num_inp2,
                                pStr, nStrLen);
...
/*^^^ #if ( !defined( TARGET_LIB_FOR_WINCHI ) && !defined( TARGET_API_LIB ) ) */
    ...
    if (log_file->f != stderr) {
      if (ip->bDisplay)
        ...
        inchi_fprintf(stderr, "Writing structure #%ld.%s%s%s%s...\r", num_inp,
                      SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    }
#endif
#endif
    ret1a = sprintf(szNumber, "Structure #%ld", num_inp);
    ret2a = WriteOrigAtomDataToSDfile(
        orig_inp_data, output_file, szNumber, NULL,
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
    nRet1 = CreateOneStructureINChI(
        sd, ip, szTitle, pINChI, pINChI_Aux, INCHI_BAS, inp_file, log_file,
        output_file, prb_file, orig_inp_data, prep_inp_data,
        composite_norm_data, num_inp, pStr, nStrLen, pncFlags);
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR &&
    ...
    nRet1 = CreateOneStructureINChI(
        sd, ip, szTitle, pINChI, pINChI_Aux, INCHI_REC, inp_file, log_file,
        output_file, prb_file, orig_inp_data, prep_inp_data,
        composite_norm_data, num_inp, pStr, nStrLen, pncFlags);
    nRet = inchi_max(nRet, nRet1);
    if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
    ...
    if (/*!sd->nErrorCode &&*/ !sd->bUserQuitComponent && !sd->bUserQuit) {
      ...
      nRet1 = TreatCreateINChIWarning(sd, ip, prep_inp_data, num_inp, inp_file,
                                      log_file, output_file, prb_file, pStr,
                                      nStrLen);
      nRet = inchi_max(nRet, nRet1);
    }
  }
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
    ...
    nRet1 = SortAndPrintINChI(
        output_file, pStr, nStrLen, log_file, ip, orig_inp_data, prep_inp_data,
        composite_norm_data, pOrigStruct, sd->num_components, sd->num_non_taut,
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR && /*ip->bDisplay &&*/
      (ip->bCompareComponents & CMP_COMPONENTS) && !sd->bUserQuit &&
    ...
    for (ord = -1; ord < INCHI_NUM; ord++) {
      ...
      if (inp_data && inp_data->num_inp_atoms && inp_data->at &&
          inp_data->nEquLabels && inp_data->nNumEquSets) {
        for (nEquSet = 1; nEquSet <= inp_data->nNumEquSets; nEquSet++) {
          ip->dp.nEquLabels = inp_data->nEquLabels;
          ip->dp.nCurEquLabel = nEquSet;
          ip->dp.nNumEquSets = inp_data->nNumEquSets;
          ip->bDisplay = 1; /* force display if it was not requested */
          ret = DisplayTheWholeStructure(sd, ip, szTitle, inp_file, log_file,
                                         inp_data, num_inp, j,
                                         1 /*bShowStructure*/, 0);
          ip->dp.nEquLabels = NULL;
          ip->dp.nCurEquLabel = 0;
  ...
  if (nRet != _IS_FATAL && nRet != _IS_ERROR && /*ip->bDisplay &&*/
    ...
    int iINChI;
    for (iINChI = 0; iINChI < maxINChI && !sd->bUserQuitComponentDisplay;
         iINChI++) {
      DisplayTheWholeCompositeStructure(
          ip, sd, num_inp, iINChI, pINChI[iINChI], pINChI_Aux[iINChI],
  ...
  if ((ip->bINChIOutputOptions & INCHI_OUT_XML) && sd->bXmlStructStarted > 0) {
    if (!OutputINChIXmlStructEndTag(output_file, pStr, nStrLen, 1)) {
      inchi_ios_eprint(log_file,
                       "Cannot create end xml tag for structure #%ld.%s%s%s%s "
                       "Terminating.\n",
                       num_inp, SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
      ...
      sd->bXmlStructStarted =
          0; /*  do not continue xml output for this structure */
    }
  }
  if (nRet != _IS_FATAL && nRet != _IS_ERROR) {
     ...
     */
    if (prb_file && prb_file->f && 0L <= sd->fPtrStart &&
        sd->fPtrStart < sd->fPtrEnd && ip->bSaveAllGoodStructsAsProblem) {
      CopyMOLfile(inp_file->f, sd->fPtrStart, sd->fPtrEnd, prb_file->f, 0);
    }
#if (/*bRELEASE_VERSION != 1 &&*/ EXTR_FLAGS == EXTR_TRANSPOSITION_EXAMPLES && \
     EXTR_MASK == EXTR_FLAGS)
    else if (prb_file->f &&
             (bSortPrintINChIFlags &
  ...
  for (j = 0; j < INCHI_NUM; j++) { /* disconnected / reconnected */
    ...
    for (i = 0; i < num_components[j]; i++) { /* component */
      for (k = 0; k < TAUT_NUM; k++) {        /* mobile/immobile H */
        if ((pINChI = pINChI2[j][i][k]) && !pINChI->bDeleted &&
            pINChI->nNumberOfAtoms > 0) {

          if ((Stereo = pINChI->Stereo) && Stereo->t_parity &&
              Stereo->nNumberOfStereoCenters > 0 && Stereo->nCompInv2Abs) {
            return 1; /* inversion changed stereo */
          }
          if ((Stereo = pINChI->StereoIsotopic) && Stereo->t_parity &&
              Stereo->nNumberOfStereoCenters > 0 && Stereo->nCompInv2Abs) {
            return 1; /* inversion changed stereo */
          }
        }
...
/************************************************************************************************/
int CreateOneStructureINChI(
    STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
    PINChI2 *pINChI2[INCHI_NUM], PINChI_Aux2 *pINChI_Aux2[INCHI_NUM],
    int iINChI, INCHI_IOSTREAM *inp_file, INCHI_IOSTREAM *log_file,
    INCHI_IOSTREAM *output_file,
    INCHI_IOSTREAM *prb_file, /*^^^ was: INCHI_IOSTREAM */
    ORIG_ATOM_DATA *orig_inp_data, ORIG_ATOM_DATA *prep_inp_data,
    COMP_ATOM_DATA composite_norm_data2[][TAUT_NUM + 1], long num_inp,
    char *pStr, int nStrLen, NORM_CANON_FLAGS *pncFlags) {
  int i, j, k, /*m,*/ nRet = 0;
#ifndef TARGET_LIB_FOR_WINCHI
  int n;
#ifndef COMPILE_ANSI_ONLY
  ...
  */
  ...
  ip->msec_LeftTime =
      ip->msec_MaxTime; /* start timeout countdown for each component */
...
#endif
  if (ip->bAllowEmptyStructure && !orig_inp_data->at &&
      !orig_inp_data->num_inp_atoms) {
    ;
  ...
  if ((!prep_inp_data->at || !prep_inp_data->num_inp_atoms) &&
...
#ifndef COMPILE_ANSI_ONLY
    /* in this location the call happens once for each input structure, before
     * preprocessing */
    bStructurePreprocessed =
        (0 != (sd->bTautFlagsDone[INCHI_BAS] &
               (TG_FLAG_MOVE_HPLUS2NEUTR_DONE | TG_FLAG_DISCONNECT_SALTS_DONE |
                TG_FLAG_MOVE_POS_CHARGES_DONE | TG_FLAG_FIX_ODD_THINGS_DONE)));
    bStructureDisconnected =
        (0 != (sd->bTautFlagsDone[INCHI_BAS] & TG_FLAG_DISCONNECT_COORD_DONE));

    bShowStructure = (bStructurePreprocessed || bStructureDisconnected ||
    ...
    */
     ...
     *  COMPONENT_ORIGINAL_PREPROCESSED #0:  (num_components > 1) &&
     *(preprocessed) COMPONENT_ORIGINAL              #1:  (num_components = 1)
     *&& (preprocessed)
     *
     *  Flags explanation:
     ...
     *
     *  COMP_PREP_0_MAIN  0x0004  2  COMPONENT_ORIGINAL_PREPROCESSED, bMain,
     *component #0 COMP_PREP_0_RECN  0x0008  3  COMPONENT_ORIGINAL_PREPROCESSED,
     *bRecn, component #0
     *
    ...
    if (DisplayTheWholeStructure(sd, ip, szTitle, inp_file, log_file,
                                 orig_inp_data, num_inp, -1, bShowStructure,
                                 bINCHI_LIB_Flag)) {
      goto exit_function;
    }
#endif
    ...
    case _IS_FATAL:
      /* error message */
      nRet = TreatReadTheStructureErrors(
          sd, ip, LOG_MASK_ALL, inp_file, log_file, output_file, prb_file,
  ...
  if (ip->bDisplayCompositeResults && !sd->bUserQuitComponentDisplay &&
    ...
    all_inp_norm_data = (INP_ATOM_DATA2 *)inchi_calloc(
        prep_inp_data[iINChI].num_components, sizeof(all_inp_norm_data[0]));
  }
  ...
  case INCHI_BAS:
    ...
    bStructurePreprocessed =
        0 != (sd->bTautFlagsDone[iINChI] &
              (TG_FLAG_MOVE_HPLUS2NEUTR_DONE | TG_FLAG_DISCONNECT_SALTS_DONE |
               TG_FLAG_MOVE_POS_CHARGES_DONE | TG_FLAG_MOVE_CHARGE_COORD_DONE |
               TG_FLAG_DISCONNECT_COORD_DONE | TG_FLAG_FIX_ODD_THINGS_DONE));
    bINCHI_LIB_Flag = 0;
    ...
    bShowStructure =
        (bStructurePreprocessed && prep_inp_data[iINChI].num_components > 1);
    break;
  ...
  if (prep_inp_data[iINChI].num_inp_atoms > 0) {
    if (DisplayTheWholeStructure(sd, ip, szTitle, inp_file, log_file,
                                 prep_inp_data + iINChI, num_inp, iINChI,
                                 bShowStructure, bINCHI_LIB_Flag)) {
      goto exit_function;
    }
  }
#endif /* } ifndef COMPILE_ANSI_ONLY */

  /* allocate pINChI[iINChI] and pINChI_Aux2[iINChI] -- arrays of pointers to
   * INChI and INChI_Aux */
  /* assign values to sd->num_components[] */
  MYREALLOC2(PINChI2, PINChI_Aux2, pINChI2[iINChI], pINChI_Aux2[iINChI],
             sd->num_components[iINChI], cur_prep_inp_data->num_components, k);
  if (k) {
    AddMOLfileError(sd->pStrErrStruct,
  ...
  for (i = 0, nRet = 0;
       !sd->bUserQuitComponent && i < cur_prep_inp_data->num_components; i++) {
    if (ip->msec_MaxTime) {
      InchiTimeGet(&ulTStart);
    }
#ifndef TARGET_LIB_FOR_WINCHI /* { */
#if (bREUSE_INCHI == 1)
    if (iINChI == INCHI_REC &&
        ((!ip->bDisplay && !ip->bDisplayCompositeResults &&
          !(ip->bCompareComponents & CMP_COMPONENTS)) ||
         sd->bUserQuitComponentDisplay)) {
      /* reconnected structure (06-20-2005: added "&&
       * !ip->bDisplayCompositeResults" to display composite structure) */
      int m = iINChI - 1;
      /* find whether we have already calculated this INChI in basic
       * (disconnected) layer */
      ...
      if (n > 1) { /* ith component is equivalent to more than one another
                      component */
    ...
    if (inp_cur_data->at && ip->bDisplay && !sd->bUserQuitComponentDisplay) {
      if (cur_prep_inp_data->num_components == 1) {
        sprintf(szTitle, "%sInput Structure #%ld.%s%s%s%s%s",
                bStructurePreprocessed ? "Preprocessed " : "", num_inp,
                SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
                iINChI ? " (Reconnected)" : "");
      } else {
      ...
      if (DRAWDATA && DRAWDATA_EXISTS) {
        ...
        vDrawData.pWindowData = CreateWinData_(
            inp_cur_data->at, inp_cur_data->num_at, 0, 1 /* bAdd_DT_to_num_H */,
            0, NULL, 1 /* display isotopic if present */, 0, NULL, NULL,
            ip->bAbcNumbers, &ip->dp, ip->nMode);
        if (vDrawData.pWindowData != NULL) {
          if (DRAWDATA_EXISTS(i + 1, nType,
                              iINChI)) { /* i = component number */
    ...
    if (log_file != stderr) {
      if (ip->bDisplay)
        ...
        inchi_ios_eprint(stderr, "Component #%d structure #%ld.%s%s%s%s...\r",
                         i + 1, num_inp,
                         SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue));
    }
    /****************************************************************************/
    ...
    /*                                                                             */
    /*******************************************************************************/
    /*  c) Create the component's INChI ( copies ip->bTautFlags into
     * sd->bTautFlags)*/
    /*******************************************************************************/
    nRet = CreateOneComponentINChI(sd, ip, inp_cur_data, orig_inp_data,
                                   pINChI /*2[iINChI]*/,
                                   pINChI_Aux /*2[iINChI]*/, iINChI, i, num_inp,
    ...
    /****************************************************************************/
    if (!nRet) {
      nRet = RenumberingTest(pINChI /*2[iINChI]*/, pINChI_Aux /*2[iINChI]*/,
                             orig_inp_data, iINChI, pRenumbData, inp_cur_data,
                             inp_norm_data, sd, ip, szTitle, log_file, prb_file,
                             i, num_inp, pncFlags);
    }
    ...
    /*  d) Display one component structure and/or INChI results only if there
     * was no error */
#ifndef COMPILE_ANSI_ONLY /* { */
    if (!nRet) {
      ...
      if ( ip->bDisplayEachComponentINChI ) {
          int cur_num_non_taut = (pINChI[i][TAUT_NON] &&
      pINChI[i][TAUT_NON]->nNumberOfAtoms>0); int cur_num_taut     =
      (pINChI[i][TAUT_YES] && pINChI[i][TAUT_YES]->nNumberOfAtoms>0); if (
      ip->bDisplayEachComponentINChI && cur_num_non_taut + cur_num_taut ) {
      ...
      */
       ...
       ** Enable buttons: * BN (non-tautomeric non-isotopic):
       *inp_norm_data[0]->bExists            * TN (tautomeric non-isotopic):
       *inp_norm_data[1]->bExists            * BI (non-tautomeric isotopic):
       *inp_norm_data[0]->bExists &&         *
       *                                   inp_norm_data[0]->bHasIsotopicLayer *
       * TI (tautomeric isotopic):         inp_norm_data[1]->bExists && *
       *                                   inp_norm_data[1]->bHasIsotopicLayer *
       **************************************************************************/
      int bIsotopic, bTautomeric, bDisplayTaut, bHasIsotopicLayer,
          bFixedBondsTaut, m_max, m, nNumDisplayedFixedBondTaut = 0;
      for (j = 0;
           ip->bDisplay && !sd->bUserQuitComponentDisplay && j < TAUT_NUM;
           j++) {
        if (inp_norm_data[j]->bExists && !inp_norm_data[j]->bDeleted) {
          bTautomeric = (pINChI[i][j]->lenTautomer >
                         0); /* same as (inp_norm_data[j]->bTautomeric > 0) */
          /* if requested tautomeric and no tautmerism found then do not say
           * mobile or fixed H. 2004-10-27 */
          bDisplayTaut = (!(ip->nMode & REQ_MODE_BASIC) && !bTautomeric)
                             ? -1
                             : bTautomeric;
          bHasIsotopicLayer = (inp_norm_data[j]->bHasIsotopicLayer > 0);
          for (k = 0; k <= bHasIsotopicLayer; k++) {
            bIsotopic = (k > 0);
            m_max = inp_norm_data[j]->at_fixed_bonds &&
                            inp_norm_data[j]->bTautPreprocessed
                        ? 1
                        : 0;
            for (m = m_max; 0 <= m; m--) {
              bFixedBondsTaut = (m > 0);
              nNumDisplayedFixedBondTaut +=
                  bFixedBondsTaut; /* display only one time */
              /*  added number of components, added another format for a single
               * component case - DCh */
              if (cur_prep_inp_data->num_components > 1) {
                sprintf(szTitle,
                        "%s Component #%d of %d, Structure #%ld%s%s.%s%s%s%s%s",
                        bFixedBondsTaut ? "Preprocessed" : "Result for", i + 1,
                        cur_prep_inp_data->num_components, num_inp,
                        bDisplayTaut == 1   ? ", mobile H"
                        : bDisplayTaut == 0 ? ", fixed H"
                                            : "",
                        bIsotopic ? ", isotopic" : "",
                        SDF_LBL_VAL(ip->pSdfLabel, ip->pSdfValue),
              ...
              if (bFixedBondsTaut && nNumDisplayedFixedBondTaut != 1)
                continue;
              if (bFixedBondsTaut) {
                err = DisplayStructure(
                    inp_norm_data[j]->at_fixed_bonds, inp_norm_data[j]->num_at,
                    ...
                    bHasIsotopicLayer, j, NULL, NULL, ip->bAbcNumbers, &ip->dp,
                    ip->nMode, szTitle);
              } else {
                err = DisplayStructure(
                    inp_norm_data[j]->at, inp_norm_data[j]->num_at, 0,
                    0 /*bAdd_DT_to_num_H*/, 0, NULL, k, j, pINChI[i],
                    pINChI_Aux[i], ip->bAbcNumbers, &ip->dp, ip->nMode,
                    szTitle);
              }
              if (sd->bUserQuitComponentDisplay = (err == ESC_KEY)) {
              ...
              if (DRAWDATA && !bFixedBondsTaut) {
                ...
                if ((ip->bCompareComponents & CMP_COMPONENTS) &&
                  ...
                  vDrawData.pWindowData = CreateWinData_(
                      ...
                      inp_norm_data[j]->num_at, inp_norm_data[j]->num_removed_H,
                      0 /* bAdd_DT_to_num_H */,
                      inp_norm_data[j]->nNumRemovedProtons,
                      inp_norm_data[j]->nNumRemovedProtonsIsotopic, k, j, NULL,
                      NULL, ip->bAbcNumbers, &ip->dp, ip->nMode);
                } else {
                  continue;
                }
                if (vDrawData.pWindowData != NULL) {
...
#endif /* } COMPILE_ANSI_ONLY */
    if (nRet) {
      nRet = TreatCreateOneComponentINChIError(
          sd, ip, cur_prep_inp_data, i, num_inp, inp_file, log_file,
          output_file, prb_file, pStr, nStrLen);
...
exit_cycle:
  ...
  if (pRenumbData->bRenumbErr && (!nRet || nRet == _IS_WARNING)) {
    ...
    nRet = TreatCreateOneComponentINChIError(
        sd, ip, cur_prep_inp_data, -1, num_inp, inp_file, log_file, output_file,
        prb_file, pStr, nStrLen);
    /* nRet = _IS_ERROR; */
    sd->nErrorCode = 0;
    nRet = 0;
  }
  ...
  default:
    ...
    if (all_inp_norm_data) {
      int res = CreateCompositeNormAtom(
          composite_norm_data, all_inp_norm_data, pINChI, pINChI_Aux,
...
int CreateCompositeNormAtom(COMP_ATOM_DATA *composite_norm_data,
                            ...
                            PINChI_Aux2 *pINChI_Aux, int num_components,
                            INCHI_MODE nMode) {
  int i, j, jj, k, n, m, tot_num_at, tot_num_H, cur_num_at, cur_num_H,
      nNumRemovedProtons;
  int num_comp[TAUT_NUM + 1], num_taut[TAUT_NUM + 1], num_del[TAUT_NUM + 1],
      num_at[TAUT_NUM + 1], num_inp_at[TAUT_NUM + 1];
  ...
  for (jj = 0; jj <= TAUT_INI; jj++) {
    ...
    if (num_comp[jj]) {
      for (i = 0; i < num_components; i++) {
        ...
        num_inp_at[jj] +=
            all_inp_norm_data[i][k].num_at; /* all atoms including terminal H */
        num_at[jj] += all_inp_norm_data[i][k].num_at -
                      all_inp_norm_data[i][k].num_removed_H;
      }
      if (num_inp_at[jj]) {
        if (!CreateCompAtomData(composite_norm_data + jj, num_inp_at[jj],
                                num_components, jj == TAUT_INI))
          goto exit_error;
  ...
  for (jj = 0; jj <= TAUT_INI; jj++, indicator <<= 1) {
    ...
    if (num_comp[jj]) {
      ...
      for (i = 0; i < num_components; i++) {
        if (all_inp_norm_data[i][j].bDeleted) {
          composite_norm_data[jj].nNumRemovedProtons +=
              all_inp_norm_data[i][j].nNumRemovedProtons;
          for (n = 0; n < NUM_H_ISOTOPES; n++) {
            composite_norm_data[jj].nNumRemovedProtonsIsotopic[n] +=
                all_inp_norm_data[i][j].nNumRemovedProtonsIsotopic[n];
          }
          continue;
        }
        ...
        /* copy main atoms */
        cur_num_H = all_inp_norm_data[i][k]
                        .num_removed_H; /* number of terminal H atoms */
        cur_num_at =
            all_inp_norm_data[i][k].num_at -
            cur_num_H; /* number of all but explicit terminal H atoms */

        if ((tot_num_at + cur_num_at) > num_at[jj] ||
            (num_at[jj] + tot_num_H + cur_num_H) > num_inp_at[jj]) {
          goto exit_error; /* miscount */
        }
        at = composite_norm_data[jj].at +
             tot_num_at; /* points to the 1st destination atom */
        at_from = (jj == TAUT_INI && k == TAUT_YES &&
                   all_inp_norm_data[i][k].at_fixed_bonds)
                      ? all_inp_norm_data[i][k].at_fixed_bonds
                      : all_inp_norm_data[i][k].at;
        memcpy(at, at_from,
               sizeof(composite_norm_data[0].at[0]) *
                   cur_num_at); /* copy atoms except terminal H */
        ...
        /* copy explicit H */
        if (cur_num_H) {
          at = composite_norm_data[jj].at + num_at[jj] +
               tot_num_H; /* points to the 1st destination atom */
        ...
        /* composite counts */
        composite_norm_data[jj].bHasIsotopicLayer |=
            all_inp_norm_data[i][k].bHasIsotopicLayer;
        composite_norm_data[jj].num_isotopic +=
            all_inp_norm_data[i][k].num_isotopic;
        composite_norm_data[jj].num_bonds += all_inp_norm_data[i][k].num_bonds;
        composite_norm_data[jj].bTautomeric +=
            (j == jj) && all_inp_norm_data[i][k].bTautomeric;
        composite_norm_data[jj].nNumRemovedProtons +=
            all_inp_norm_data[i][k].nNumRemovedProtons;
        for (n = 0; n < NUM_H_ISOTOPES; n++) {
          composite_norm_data[jj].nNumRemovedProtonsIsotopic[n] +=
              all_inp_norm_data[i][k].nNumRemovedProtonsIsotopic[n];
          composite_norm_data[jj].num_iso_H[n] +=
              all_inp_norm_data[i][k].num_iso_H[n];
        }
        /*
        ...
        */
        ...
        if (composite_norm_data[jj].nOffsetAtAndH) {
          ...
          composite_norm_data[jj].nOffsetAtAndH[2 * i + 1] =
              num_at[jj] + tot_num_H;
        }
      }
