...
int GetPlusMinusVertex(BN_STRUCT *pBNS, ALL_TC_GROUPS *pTCGroups,
  ...
  int k, ePlusSuper, eMinusSuper, vPlusSuper, vMinusSuper,
      vPlusMinus1 = NO_VERTEX, vPlusMinus2 = NO_VERTEX;
  BNS_EDGE *pEdge;
  ...
  if ((k = pTCGroups->nGroup[TCG_Minus]) >= 0 &&
      ...
      !((pEdge = pBNS->edge + eMinusSuper)->forbidden &&
        bCheckForbiddenMinus)) {
...
/***********************************************************************************************/
int bIsUnsatCarbonInASmallRing(inp_ATOM *at2, VAL_AT *pVA, int iat,
                               BFS_Q *pbfsq, int min_ring_size) {
  int j, nCurRingSize, nMinRingSize;
  if (min_ring_size < 5) {
    /* =C= in a small ring  */
    if (at2[iat].valence == 2 && pVA[iat].cMinRingSize <= 5 &&
        at2[iat].chem_bonds_valence == 4) {
      return 1;
    }
  } else {
    if (at2[iat].valence == 2 && pVA[iat].cMinRingSize &&
    ...
    if ((at2[iat].valence == 2 || at2[iat].valence == 3) &&
      ...
      for (j = 0; j < at2[iat].valence; j++) {
        nCurRingSize = is_bond_in_Nmax_memb_ring(
            at2, iat, j, pbfsq->q, pbfsq->nAtomLevel, pbfsq->cSource,
            (AT_RANK)nMinRingSize /* max ring size */);
        if (0 < nCurRingSize && nCurRingSize < nMinRingSize) {
          nMinRingSize = nCurRingSize;
        }
      }
      return (0 <= nCurRingSize) ? (nMinRingSize <= min_ring_size)
                                 : nCurRingSize;
    }
  }
  return 0;
...
int FixMobileHRestoredStructure(
  ...
  EDGE_LIST AllChargeEdges, CurrEdges, CurrEdges2, CurrEdges3, TautEdges,
      NFlowerEdges, OtherNFlowerEdges, FixedLargeRingStereoEdges;
  ...
  for (i = 0; i < pTCGroups->num_tgroups; i++) {
    pv1 = pBNS->vert +
          (v1 = pTCGroups->pTCG[i].nVertexNumber); /* t-group vertex */
    for (j = 0; j < pv1->num_adj_edges; j++) {
      /* e, pe - tautomeric atom edge; pv2 - endpoint vertex */
      /* Note: pe, pv2, v1 are not used here; they are to show how to traverse
       * t-group */
  ...
  if (pc2i->nNumTgInChI == 1 &&
      ...
      pc2i->nNumTgDBNMinusRevrs + pc2i->nNumTgNHMinusRevrs == 0 &&
    ...
    if ((ret =
             FillOutCMP2MHINCHI(pStruct, pTCGroups, at2, pVA, pInChI, pc2i))) {
      goto exit_function;
    }
  ...
  if (pc2i->nNumTgInChI == 1 &&
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      if (pVA[iat].cNumValenceElectrons == 5 &&
          pVA[i].cPeriodicRowNumber == 1 && !at2[iat].endpoint &&
        ...
        if (num_DB_N_III < MAX_DIFF_MOBH && !at2[iat].num_H &&
            ...
            !pBNS->edge[e].forbidden && pBNS->edge[e].cap &&
            !pBNS->edge[e].flow) {
          /* -N= */
          ...
          iat_DB_N_III[num_DB_N_III++] = iat;
                   ...
                   at2[iat].valence == 3 && at2[iat].chem_bonds_valence == 3 &&
    ...
    if (num_DB_N_III && num_SB_N_III) {
      ...
      for (i = 0; i < num_DB_N_III && !cur_success; i++) {
        ...
        e = pBNS->edge[k1 = pBNS->vert[iat].iedge[0]].flow   ? k1
            : pBNS->edge[k2 = pBNS->vert[iat].iedge[1]].flow ? k2
                                                             : NO_VERTEX;
        if (e == NO_VERTEX) {
        ...
        peMinus->forbidden &=
            forbidden_edge_mask_inv; /* allow negative charge */
        delta = 1;
      ...
      if (cur_success) {
        ...
        if ((ret = FillOutCMP2MHINCHI(pStruct, pTCGroups, at2, pVA, pInChI,
                                      pc2i))) {
          goto exit_function;
        }
        if (!pc2i->bHasDifference) {
  ...
  if (pc2i->nNumTgInChI == 1 &&
      (pc2i->nNumEndpRevrs < pc2i->nNumEndpInChI ||
       pc2i->nNumTgRevrs > 1) && /* ADP */
      pc2i->nNumTgMInChI == 0 &&
      pc2i->nNumTgNInChI && pc2i->nNumTgOInChI) {
    /*-------------------------------------------------------*/
    ...
    */
    ...
    for (itg = 0; itg < pTCGroups->num_tgroups && !cur_success; itg++) {
      ...
      for (i = 0; i < pTg->num_adj_edges && !cur_success; i++) {
        pvEndp =
            pBNS->vert +
            (vEndp = (peTg = pBNS->edge + (eTg = pTg->iedge[i]))->neighbor1);
        eTg2 = -1;
        if (pVA[vEndp].cNumValenceElectrons == 6 && peTg->cap) {
          ...
          for (j1 = 0; j1 < at2[vEndp].valence && eTg2 < 0; j1++) {
            peCent1 = pBNS->edge +
                      pvEndp->iedge[j1]; /* edge from O to a centerpoint */
            pvCent = pBNS->vert +
                     (vCent = peCent1->neighbor12 ^ vEndp); /* centerpoint */
        ...
        if (eTg2 >= 0) {
          ...
          if (1 == peTg->cap - peTg->flow && 0 == peTg2->cap - peTg2->flow &&
            ...
            pStruct->ti.t_group[itg].num[1]++; /* increment number of (-), keep
                                                  number of taut attachments */
            pTCGroups->total_charge--;
    ...
    if (0 == pc2i->nNumTgNHInChI + pc2i->nNumTgNH2InChI &&
        pc2i->nNumTgOHInChI && !cur_success) {
      ...
      pEdgeList = CurrEdges2.num_edges  ? &CurrEdges2
                  : CurrEdges.num_edges ? &CurrEdges
                                        : NULL;
      ...
      RemoveForbiddenEdgeMask(pBNS, &CurrEdges, forbidden_edge_mask);
      RemoveForbiddenEdgeMask(pBNS, &CurrEdges, forbidden_edge_mask);
    } else if (pc2i->nNumTgNHInChI + pc2i->nNumTgNH2InChI &&
               pc2i->nNumTgOInChI && !cur_success) {
      /* change an attachement to N from H to (-) */
      for (itg = 0; itg < pTCGroups->num_tgroups && !cur_success; itg++) {
        pTg = pBNS->vert + pTCGroups->pTCG[itg].nVertexNumber;
        for (i = 0; i < pTg->num_adj_edges && !cur_success; i++) {
          pvEndp2 = pBNS->vert +
                    (vEndp2 = (peTg = pBNS->edge + pTg->iedge[i])->neighbor1);
          if (pVA[vEndp2].cNumValenceElectrons == 5 &&
              pVA[vEndp2].cPeriodicRowNumber == 1 &&
    ...
    if (cur_success) {
      ...
      if (pStruct->One_ti.num_t_groups == 1 &&
          pStruct->One_ti.t_group[0].num[1]) {
  ...
  if (pc2i->nNumTgInChI == 1 &&
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      /* -N< */
      if (!at2[iat].endpoint && !at2[iat].charge && !at2[iat].radical &&
          !at2[iat].num_H && pVA[i].cNumValenceElectrons == 5 &&
          pVA[i].cPeriodicRowNumber == 1 &&
          0 <= (e = pVA[iat].nCPlusGroupEdge - 1) && pBNS->edge[e].flow &&
    ...
    for (itg = 0; itg < pTCGroups->num_tgroups && !cur_success; itg++) {
      ...
      for (i = 0; i < pTg->num_adj_edges; i++) {
        pvEndp =
            pBNS->vert +
            (vEndp = (peTg = pBNS->edge + (eTg = pTg->iedge[i]))->neighbor1);
        if (at2[vEndp].charge || at2[vEndp].radical ||
            peTg->cap - peTg->flow != 1) {
          continue;
        }
      ...
      if (ret == 1 &&
        ...
        for (i = 0; i < pBNS->vert[vPlusMinus].num_adj_edges; i++) {
          j = pBNS->edge[pBNS->vert[vPlusMinus].iedge[i]].neighbor12 ^
              vPlusMinus;
          for (k = 0; k < pBNS->vert[j].num_adj_edges; k++) {
            pBNS->edge[pBNS->vert[j].iedge[k]].cap -= delta;
          }
  ...
  while (pc2i->nNumDiffMobH &&
         pc2i->nChargeMobHRevrs > pc2i->nChargeMobHInChI) {
    /*----------------------------------------------------*/
        ...
        for (mode = 4; !cur_success && mode <= 8; mode++) {
          ...
          for (iat = 0; !cur_success && iat < pStruct->num_atoms; iat++) {
            ...
            if (!at2[iat].charge && !at2[iat].radical &&
                ...
                0 <= (e2 = pVA[iat].nCMinusGroupEdge - 1) &&
                !(pe2 = pBNS->edge + e2)->flow &&
                0 < bIsUnsatCarbonInASmallRing(at2, pVA, iat, pStruct->pbfsq,
                                               mode)) {
    ...
    */
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      if (/* orig. InChI info: taut in orig. InChI =O located in -NO2 that is
          ...
          !(at_Mobile_H_Revrs &&
            at_Mobile_H_Revrs[iat].endpoint) /*|| pc2i->c2at[i].nMobHRevrs*/
          && !at2[iat].charge && at2[iat].valence == 1 &&
        ...
        for (k = 0; k < at2[iN].valence; k++) {
          ...
          if (pVA[neigh].cNumValenceElectrons == 6 && !at2[neigh].endpoint &&
      ...
      for (i = 0; i < pc2i->len_c2at; i++) {
        ...
        if (pc2i->c2at[i].endptRevrs && !pc2i->c2at[i].endptInChI &&
          ...
          if (pc2i->c2at[i].nValElectr == 6) {
            if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_WRONG_TAUT_O], e,
                                     INC_ADD_EDGE))) {
              goto exit_case_07;
            }
          } else if (pc2i->c2at[i].nValElectr == 5) {
            if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_WRONG_TAUT_N], e,
                                     INC_ADD_EDGE))) {
              goto exit_case_07;
            }
          }
          if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_WRONG_TAUT_ALL], e,
                                   INC_ADD_EDGE))) {
            goto exit_case_07;
          }
        }
