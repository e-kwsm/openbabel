...
/******************************************************************************************************/
int InChI2Atom(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, const char *szCurHdr,
               long num_inp, StrFromINChI *pStruct, int iComponent,
  ...
  if (bMobileH == TAUT_NON &&
      OneInput->nNumProtons[iINChI][TAUT_YES].pNumProtons) {
    pStruct->nNumRemovedProtonsMobHInChI =
        OneInput->nNumProtons[iINChI][TAUT_YES]
            .pNumProtons[iComponent]
            .nNumRemovedProtons;
  }

  if (bMobileH == TAUT_NON ||
      (bMobileH == TAUT_YES && OneInput->pInpInChI[iINChI][TAUT_NON] &&
  ...
  ret = OneInChI2Atom(ip, sd, szCurHdr, num_inp, pStruct, iComponent,
...
void RemoveFixHInChIIdentical2MobH(InpInChI *pOneInput) {
  ...
  /* eliminate Fixed-H InChI that are exactly came as the corresponding Mobile-H
   * structures */
  for (iInchiRec = 0; iInchiRec < INCHI_NUM; iInchiRec++) {
    ...
    for (k = 0; k < cur_num_comp; k++) {
      if (!CompareReversedINChI(pOneInput->pInpInChI[iInchiRec][TAUT_YES] + k,
                                pOneInput->pInpInChI[iInchiRec][TAUT_NON] + k,
                                NULL, NULL)) {
        Free_INChI_Members(pOneInput->pInpInChI[iInchiRec][TAUT_NON] + k);
        memset(pOneInput->pInpInChI[iInchiRec][TAUT_NON] + k, 0,
               sizeof(pOneInput->pInpInChI[0][0][0]));
      }
    }
  }
}
...
int MarkDisconectedIdenticalToReconnected(InpInChI *pOneInput) {
  /* mark Disconnected InChI components that are exactly came as Reconnected
   * ones */
  ...
  for (k1 = 0; k1 < inchi_max(pOneInput->nNumComponents[INCHI_BAS][TAUT_YES],
    ...
    for (k2 = 0; k2 < inchi_max(pOneInput->nNumComponents[INCHI_REC][TAUT_YES],
         ...
         k2++) {
      int eqM =
          (k1 < pOneInput->nNumComponents[INCHI_BAS][TAUT_YES] &&
           k2 < pOneInput->nNumComponents[INCHI_REC][TAUT_YES] &&
           !pOneInput->pInpInChI[INCHI_REC][TAUT_YES][k2]
                .nLink && /* already linked */
      ...
      if (eqM && ((!isF1 && !isF2) || eqF)) {
        ...
        num_marked++;
        break; /* equal InChI has been deleted from the disconnected layer, get
                  next k1 */
      }
    }
  }
...
void SetUpSrm(SRM *pSrm) {
  ...
  */
  pSrm->nMetal2EndpointInitBondOrder =
      pSrm->nMetal2EndpointMinBondOrder + pSrm->nMetal2EndpointInitEdgeFlow;
}
/**************************************************************************************/
int MergeStructureComponents(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
                             long num_inp, char *szCurHdr, ICHICONST SRM *pSrm,
                             int bReqNonTaut,
                             StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
                             InpInChI *pOneInput) {
  int iInchiRec, iMobileH, iAlternH, num_components, tot_just_atoms,
      tot_removed_H, tot_atoms, cur_nA, cur_nH;
  int k, i, j, ret, iCurAtomOffs, iNxtAtomOffs, iCurDelHOffs, iNxtDelHOffs, len,
      len2, iShiftH, icomp;
  ...
  if ((num_components = pOneInput->nNumComponents[INCHI_REC][TAUT_NON])) {
    ...
    iMobileH = TAUT_NON;
                  ...
                  pOneInput->nNumComponents[INCHI_REC][TAUT_YES])) {
    iInchiRec = INCHI_REC;
    iMobileH = TAUT_YES;
  } else if ((num_components =
                  pOneInput->nNumComponents[INCHI_BAS][TAUT_NON])) {
    ...
    iMobileH = TAUT_NON;
                  ...
                  pOneInput->nNumComponents[INCHI_BAS][TAUT_YES])) {
    iInchiRec = INCHI_BAS;
    iMobileH = TAUT_YES;
  } else {
  ...
  iAlternH =
      (iMobileH == TAUT_NON && pOneInput->nNumComponents[iInchiRec][TAUT_YES])
          ? TAUT_YES
          : -1;
  nAtomOffs[0] = nDelHOffs[0] = 0;
  for (k = 0; k < num_components; k++) {
    pStruct1 = pStruct[iInchiRec][iMobileH][k].num_atoms
                   ? pStruct[iInchiRec][iMobileH] + k
               : iAlternH >= 0 && pStruct[iInchiRec][iAlternH][k].num_atoms
                   ? pStruct[iInchiRec][iAlternH] + k
                   : NULL;
  ...
  if (NULL ==
      (at = (inp_ATOM *)inchi_malloc((tot_atoms + 1) * sizeof(at[0])))) {
    ret = RI_ERR_ALLOC;
    goto exit_function;
  }
  if (!tot_atoms) {
  ...
  for (k = 0; k < num_components; k++) {
    ...
    len =
        nAtomOffs[k + 1] -
        nAtomOffs[k]; /* number of atoms in a component excluding explicit H */
...
/******************************************************************************************************/
int DisplayAllRestoredComponents(inp_ATOM *at, int num_at,
                                 const char *szCurHdr) {
  int ret;
  char szTitle[512];
  DRAW_PARMS dp;
  ...
  dp.pdp = &pdp;
  dp.sdp.nFontSize = -9;
  sprintf(szTitle, "All Components of Restored %s Structure",
          szCurHdr ? szCurHdr : "(No structure name)");
  ret = DisplayStructure(
      at, num_at, 0 /* nNumDeletedH*/, 0 /*bAdd_DT_to_num_H*/,
      0 /*nNumRemovedProtons*/, NULL /*NUM_H *nNumRemovedProtonsIsotopic*/,
      1 /*int bIsotopic*/, 0 /*bTautomeric*/, NULL /* pINChI */,
      NULL /* INChI_Aux **cur_INChI_Aux*/, 0 /*bAbcNumbers*/, &dp,
      0 /*INCHI_MODE nMode*/, szTitle);
  return 0;
}
/******************************************************************************************************/
int DisplayOneRestoredComponent(StrFromINChI *pStruct, inp_ATOM *at,
                                int iComponent, int nNumComponents,
                                int bMobileH, const char *szCurHdr) {
  int ret, k;
  ...
  NUM_H *nNumRemovedProtonsIsotopic =
      pAux ? pAux[iMobileH]->nNumRemovedIsotopicH : NULL;
  ...
  sprintf(szTitle, "Restored %s Component %d of %d %c%c",
          szCurHdr ? szCurHdr : "(No structure name)", iComponent + 1,
          nNumComponents, pStruct->iInchiRec ? 'R' : 'D',
          pStruct->iMobileH ? 'M' : 'F');
  ret =
      DisplayStructure(at, num_at, 0 /* nNumDeletedH*/, 0 /*bAdd_DT_to_num_H*/,
                       nNumRemovedProtons, /*NULL*/ nNumRemovedProtonsIsotopic,
                       1 /*int bIsotopic*/, k, pInChI, pAux, 0 /*bAbcNumbers*/,
                       &dp, 0 /*INCHI_MODE nMode*/, szTitle);
  return 0;
}
/******************************************************************************************************/
int DisplayRestoredComponent(StrFromINChI *pStruct, int iComponent,
  ...
  dp.pdp = &pdp;
  dp.sdp.nFontSize = -9;
  sprintf(szTitle, "DBG Restored %s Component %d %c%c",
          szCurHdr ? szCurHdr : "(No structure name)", iComponent + 1,
          pStruct->iInchiRec ? 'R' : 'D', pStruct->iMobileH ? 'M' : 'F');
  ret = DisplayStructure(at, num_at, 0 /* nNumDeletedH*/,
                         0 /*bAdd_DT_to_num_H*/, 0 /*nNumRemovedProtons*/,
                         NULL /*NUM_H *nNumRemovedProtonsIsotopic*/,
                         1 /*int bIsotopic*/, 0 /*bTautomeric*/, &pInChI,
                         NULL /* INChI_Aux **cur_INChI_Aux*/, 0 /*bAbcNumbers*/,
                         &dp, 0 /*INCHI_MODE nMode*/, szTitle);
  inchi_free(at);
  return 0;
}
/**************************************************************************************/
int DisplayStructureComponents(ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd,
                               long num_inp, char *szCurHdr,
                               ICHICONST SRM *pSrm, int bReqNonTaut,
                               StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
                               InpInChI *pOneInput) {
  int iInchiRec, iMobileH, iCurMobH, iAlternH, num_components, tot_just_atoms,
      tot_removed_H, tot_atoms, cur_nA, cur_nH;
  int k, i, j, ret, iCurAtomOffs, iNxtAtomOffs, iCurDelHOffs, iNxtDelHOffs, len,
      len2, iShiftH, icomp;
  int *nAtomOffs = NULL, *nDelHOffs = NULL, bNoCoord = 0, iNewCoord = 0,
      nNewCoord = 0;
  double x_max = -1.0e16, x_min = 1.0e16, y_max = -1.0e16, y_min = 1.0e16,
         delta = 0.0;
  StrFromINChI *pStruct1;
  inp_ATOM *at = NULL, *a;
  ...
  for (k = 0; k < num_components; k++) {
    pStruct1 = pStruct[iInchiRec][iMobileH][k].num_atoms
                   ? pStruct[iInchiRec][iCurMobH = iMobileH] + k
               : iAlternH >= 0 && pStruct[iInchiRec][iAlternH][k].num_atoms
                   ? pStruct[iInchiRec][iCurMobH = iAlternH] + k
                   : NULL;
    if (len = nAtomOffs[k + 1] - nAtomOffs[k]) {
      XYZ_COORD *pxyz = pStruct1->pXYZ;
      len2 = nDelHOffs[k + 1] - nDelHOffs[k]; /* do not separate H from the
                                                 atom: we will not need them */
      iCurAtomOffs = nAtomOffs[k];
      a = at + iCurAtomOffs;
      ...
      if (pxyz) {
        ...
        if (pStruct1->pXYZ =
                (XYZ_COORD *)inchi_calloc(len, sizeof(pStruct1->pXYZ[0]))) {
      ...
      if (ip->bDisplay || ip->bDisplayCompositeResults && 1 == num_components) {
        DisplayOneRestoredComponent(pStruct1, a, k, num_components, iCurMobH,
                                    szCurHdr);
      }
  ...
  if (ip->bDisplayCompositeResults && num_components > 1) {
    for (k = 0; k < num_components; k++) {
      ...
      iCurDelHOffs = nDelHOffs[k];
      iNxtDelHOffs = nDelHOffs[k + 1];
      len = nAtomOffs[k + 1] - nAtomOffs[k]; /* number of atoms in a component
                                                excluding explicit H */
      iShiftH = iCurDelHOffs - len;
      if (!len) {
        continue;
      }
...
#endif
/**************************************************************************************/
int AllInchiToStructure(ICHICONST INPUT_PARMS *ip_inp, STRUCT_DATA *sd_inp,
                        long num_inp, char *szCurHdr, ICHICONST SRM *pSrm,
                        int bHasSomeFixedH,
                        StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
                        InpInChI *pOneInput) {
  ...
  *ip = *ip_inp;
  ...
  ret = 0;
  num_err = 0;
  for (iInchiRec = 0; iInchiRec < INCHI_NUM;
       iInchiRec++) { /* Disconnected/Connected */
    for (iMobileH = 0; iMobileH < TAUT_NUM; iMobileH++) { /* Mobile/Fixed H */
      ...
      pStruct[iInchiRec][iMobileH] =
          (StrFromINChI *)inchi_calloc(cur_num_comp, sizeof(pStruct[0][0][0]));
      ...
      /* set conversion mode */
      bCurI2A_Flag =
          (iMobileH ? 0 : I2A_FLAG_FIXEDH) | (iInchiRec ? I2A_FLAG_RECMET : 0);
      if (iMobileH) {
      ...
      for (k = 0; k < cur_num_comp; k++) { /* components */
        if ((!iMobileH &&
             !pOneInput->pInpInChI[iInchiRec][iMobileH][k].nNumberOfAtoms) ||
            pOneInput->pInpInChI[iInchiRec][iMobileH][k].bDeleted ||
            pOneInput->pInpInChI[iInchiRec][iMobileH][k].nLink < 0) {

          pStruct[iInchiRec][iMobileH][k].nLink =
              pOneInput->pInpInChI[iInchiRec][iMobileH][k].nLink;
          pStruct[iInchiRec][iMobileH][k].bDeleted =
              pOneInput->pInpInChI[iInchiRec][iMobileH][k].bDeleted;
          continue; /* do not create a structure out of an unavailable
                       Fixed-H InChI or out of the one present in Reconnected
                       layer */
#ifdef NEVER        /* a wrong attempt to process deleted components here */
          if (pStruct[iInchiRec][iMobileH][k].nLink =
                  pOneInput->pInpInChI[iInchiRec][iMobileH][k].nLink) {
            continue; /* do not create a structure out of an unavailable
                         Fixed-H InChI or out of the one present in Reconnected
                         layer */
          } else if (iMobileH &&
                     pOneInput->pInpInChI[iInchiRec][iMobileH][k]
                         .nNumberOfAtoms &&
                     pOneInput->pInpInChI[iInchiRec][iMobileH][k].bDeleted &&
        ...
        if (bHasSomeFixedH && iMobileH &&
            k < pOneInput->nNumComponents[iInchiRec][TAUT_NON] &&
            pOneInput->pInpInChI[iInchiRec][TAUT_NON][k].nNumberOfAtoms) {
          continue; /* do not process Mobile-H if Fixed-H is requested and
                       exists */
        }
        ...
        ret = InChI2Atom(ip, sd, szCurHdr, num_inp,
                         pStruct[iInchiRec][iMobileH] + k, k, 0 /* AtNoOffset*/,
                         bCurI2A_Flag, bHasSomeFixedH, pOneInput);
        pStruct[iInchiRec][iMobileH][k].nLink =
            pOneInput->pInpInChI[iInchiRec][iMobileH][k].nLink;
        if (ret < 0) {
          ...
          fprintf(stdout, "%ld %s Conversion failed: %d, %c%c comp %d\n",
                  num_inp, szCurHdr ? szCurHdr : "Struct", ret,
                  iInchiRec ? 'R' : 'D', iMobileH ? 'M' : 'F', k + 1);
#endif
#endif
...
/**************************************************************************************/
int AddProtonAndIsoHBalanceToMobHStruct(
    ICHICONST INPUT_PARMS *ip, STRUCT_DATA *sd, long num_inp,
    int bHasSomeFixedH, char *szCurHdr,
  ...
  int nDeltaFromDisconnected = 0, nRemovedProtonsByNormFromRevrs,
      nRemovedProtonsByRevrs, num_changes = 0;
  ...
  for (i = 0; i < INCHI_NUM; i++) {
    nToBeRemovedByNormFromRevrs[i].nNumRemovedProtons =
        pOneInput->nNumProtons[i][TAUT_YES].nNumRemovedProtons;
    for (k = 0; k < NUM_H_ISOTOPES; k++) {
      nToBeRemovedByNormFromRevrs[i].nNumRemovedIsotopicH[k] =
          pOneInput->nNumProtons[i][TAUT_YES].nNumRemovedIsotopicH[k];
    }
  }
  /* accumulate here num. protons removed by the normalization from the reversed
   * structure */
  nRemovedByNormFromRevrs[INCHI_BAS] = nRemovedByNormFromRevrs[INCHI_REC] = 0;
  nRemovedByRevrs[INCHI_REC] = nRemovedByRevrs[INCHI_BAS] = 0;
  /* protons added/removed by InChI Normalization to/from Restored Structure
   * might have been added by StructureRestore */
  for (iInchiRec = 0; iInchiRec < INCHI_NUM; iInchiRec++) {
    for (k = 0; k < pOneInput->nNumComponents[iInchiRec][TAUT_YES]; k++) {
      if (!bInpInchiComponentExists(pOneInput, iInchiRec, TAUT_YES, k)) {
        continue;
      }
      nRemovedProtonsByNormFromRevrs =
          0; /* Num protons removed from the Restored Structure by InChI
                Normalization */
      nRemovedProtonsByRevrs = 0; /* Num protons removed by the Reconstruction
                                     from the Restored Structure */
      if (iInchiRec == INCHI_REC ||
          (iInchiRec == INCHI_BAS &&
           (k1 = pStruct[iInchiRec][TAUT_YES][k].nLink) >= 0)) {

        REV_INCHI *pRevInChI = &pStruct[iInchiRec][TAUT_YES][k].RevInChI;
        INChI_Aux **pINChI_Aux2 =
            pRevInChI->pINChI_Aux[iInchiRec][0]; /* component 0*/
        INChI **pINChI_Revr = pRevInChI->pINChI[iInchiRec][0];
        INChI *pINChI_Orig = pOneInput->pInpInChI[iInchiRec][TAUT_YES] + k;
        nChargeRevrs =
            pINChI_Revr ? pINChI_Revr[TAUT_YES]->nTotalCharge : NO_VALUE_INT;
        nChargeInChI = pINChI_Orig->nTotalCharge;
        if (pINChI_Aux2) {
          nRemovedProtonsByNormFromRevrs =
              pINChI_Aux2[TAUT_YES]->nNumRemovedProtons;
        }
        nRemovedProtonsByRevrs =
            pStruct[iInchiRec][TAUT_YES][k].nNumRemovedProtonsByRevrs;
        ...
        INChI_Aux **pINChI_Aux2 =
            pRevInChI->pINChI_Aux[INCHI_BAS][0]; /* component 0 */
        ...
        /* this component cannot be disconnected because it is same as in
         * reconnected layer */
        nRemovedProtonsByRevrs =
            pStruct[INCHI_REC][TAUT_YES][k1].nNumRemovedProtonsByRevrs;
      ...
      /* how many protons (to be removed by InChI Normalization) to add =
         ...
         {number of protons known to be removed by InChI Normalization from
      ...
      pStruct[iInchiRec][TAUT_YES][k].nRemovedProtonsByNormFromRevrs =
          nRemovedProtonsByNormFromRevrs;
    }
  }

  /* Since fixed-H layer is missing we need to add proton balance to the
   * components */
  ...
  for (iInchiRec = INCHI_REC; INCHI_BAS <= iInchiRec; iInchiRec--) {
    ...
    */
    int bHasRecMobH = (iInchiRec == INCHI_BAS &&
                       pOneInput->nNumComponents[INCHI_REC][TAUT_YES]);
    /* bHasRecMobH means all components that could not be disconnected are in
     * reconnected part */
    if (iInchiRec == INCHI_BAS) {
      /* second pass: common structures have been changed */
      nToBeRemovedByNormFromRevrs[INCHI_BAS].nNumRemovedProtons +=
          nDeltaFromDisconnected;
    }
    /* after proton removal InChI is recalculated */

    ret = AddRemProtonsInRestrStruct(
        ip, sd, num_inp, bHasSomeFixedH, pStruct[iInchiRec][TAUT_YES],
        pOneInput->nNumComponents[iInchiRec][TAUT_YES],
  ...
  /* if fixed-H layer is missing then we need to add isotopic exchangeable
   * proton balance to the components */
  for (iInchiRec = INCHI_REC; INCHI_BAS <= iInchiRec; iInchiRec--) {
    /*
    if ( !pOneInput->nNumComponents[iInchiRec][TAUT_NON] &&
          pOneInput->nNumComponents[iInchiRec][TAUT_YES] ) {
    */
    ...
    if (iInchiRec == INCHI_BAS) {
      /* second pass: common structures have been changed */
      for (k = 0; k < NUM_H_ISOTOPES; k++) {
        nToBeRemovedByNormFromRevrs[INCHI_BAS].nNumRemovedIsotopicH[k] +=
            nIsoDeltaFromDisconnected[k];
      }
    }
    /* after proton removal InChI is recalculated */
    ret = AddRemIsoProtonsInRestrStruct(
        ip, sd, num_inp, bHasSomeFixedH, pStruct[iInchiRec][TAUT_YES],
        pOneInput->nNumComponents[iInchiRec][TAUT_YES],
        bHasRecMobH ? pStruct[INCHI_REC][TAUT_YES] : NULL,
        bHasRecMobH ? pOneInput->nNumComponents[INCHI_REC][TAUT_YES] : 0,
...
int CompareAllOrigInchiToRevInChI(StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM],
                                  InpInChI *pOneInput, int bReqNonTaut,
                                  long num_inp, char *szCurHdr) {
  int i, iInchiRec, iMobileH, iMobileHpStruct, num_components, iComponent,
      ret = 0;
  COMPONENT_REM_PROTONS nCurRemovedProtons, nNumRemovedProtons;
  INChI *pInChI[TAUT_NUM];
  INCHI_MODE CompareInchiFlags[TAUT_NUM];
  memset(pOneInput->CompareInchiFlags[0], 0,
         sizeof(pOneInput->CompareInchiFlags[0]));
  memset(&nNumRemovedProtons, 0, sizeof(nNumRemovedProtons));

  /* do we have reconnected InChI ?*/
  iInchiRec = INCHI_REC;
  iMobileH = TAUT_NON;
  if (!pOneInput->nNumComponents[iInchiRec][TAUT_YES] &&
      !pOneInput->nNumComponents[iInchiRec][TAUT_NON]) {
    iInchiRec = INCHI_BAS;
  }
  ...
  /* if a restored structure has Fixed-H InChI then its mobile-H restored InChI
   * is in Fixed-H pStruct */
  ...
  for (iComponent = 0; iComponent < num_components; iComponent++) {
    ...
    if (pOneInput->pInpInChI[iInchiRec][bMobileH][iComponent].nNumberOfAtoms &&
      ...
      if (bMobileH == TAUT_NON) {
        pInChI[1] = &pOneInput->pInpInChI[iInchiRec][TAUT_YES][iComponent];
               ...
               pOneInput->pInpInChI[iInchiRec][TAUT_YES][iComponent]
                   .nNumberOfAtoms &&
               !pOneInput->pInpInChI[iInchiRec][TAUT_YES][iComponent]
                    .bDeleted) {
      /* the requested Fixed-H InChI layer does not exist; however, the Mobile-H
       * does exist */
    ...
    iMobileHpStruct =
        ...
        OldPrintCompareOneOrigInchiToRevInChI(
            pStruct[iInchiRec][bMobileH] + iComponent, pInChI, bMobileH,
            iComponent, num_inp, szCurHdr);
#endif
#endif
    /* one component comparison result bits */
    ret = CompareOneOrigInchiToRevInChI(
        pStruct[iInchiRec][bMobileH] + iComponent, pInChI, bMobileH, iComponent,
        num_inp, szCurHdr, &nCurRemovedProtons, CompareInchiFlags);
    if (ret >= 0) {
      /* no errors encountered -> accumulate removed protons from individual
       * Mobile-H layers of components */
      ...
      for (i = 0; i < NUM_H_ISOTOPES; i++) {
        nNumRemovedProtons.nNumRemovedIsotopicH[i] +=
            nCurRemovedProtons.nNumRemovedIsotopicH[i];
      }
  ...
  if (iMobileH == TAUT_YES) {
    if (pOneInput->nNumProtons[iInchiRec][iMobileH].pNumProtons) {
      ret = RI_ERR_PROGR; /* in Mobile-H case proton balances are split between
                             compoments */
    } else {
      /*   num removed protons in orig. InChI      num removed protons in
       * restored InChi */
      if (nNumRemovedProtons.nNumRemovedProtons !=
        ...
        /* debug output only */
        {
          int num_H_AddedByRevrs =
              pOneInput->nNumProtons[iInchiRec][iMobileH].nNumRemovedProtons -
              nNumRemovedProtons.nNumRemovedProtons;
          fprintf(
              stdout,
              "COMPARE_INCHI: %ld: %s %cM: Proton balance (Diff: %d, "
              "RevrsRem=%d)\n",
      ...
      for (i = 0; i < NUM_H_ISOTOPES; i++) {
        if (nNumRemovedProtons.nNumRemovedIsotopicH[i] !=
            pOneInput->nNumProtons[iInchiRec][TAUT_YES]
                .nNumRemovedIsotopicH[i]) {
          pOneInput->CompareInchiFlags[0][TAUT_YES] |= INCHIDIFF_MOB_ISO_H;
#if (bRELEASE_VERSION == 0)
          /* debug output only */
          {
            int num_H_AddedByRevrs = pOneInput->nNumProtons[iInchiRec][TAUT_YES]
                                         .nNumRemovedIsotopicH[i] -
                                     nNumRemovedProtons.nNumRemovedIsotopicH[i];
            fprintf(stdout,
                    "COMPARE_INCHI: %ld: %s %cM: Iso Xchg %dH balance (Diff: "
                    "%d, RevrsRem=%d)\n",
                    num_inp, szCurHdr ? szCurHdr : "Struct",
                    iInchiRec ? 'R' : 'D', i + 1,
                    pOneInput->nNumProtons[iInchiRec][TAUT_YES]
                        .nNumRemovedIsotopicH[i],
                    num_H_AddedByRevrs);
...
/***********************************************************************************************/
int CompareAllDisconnectedOrigInchiToRevInChI(
    StrFromINChI *pStruct[INCHI_NUM][TAUT_NUM], InpInChI *pOneInput,
    int bHasSomeFixedH, long num_inp, char *szCurHdr) {
  ...
  int num_fragments_D, num_fragments_R, num_fragments_DR, num_fragments,
      iComponent, ret;
  ...
  ret = 0;
  memset(pOneInput->CompareInchiFlags[1], 0,
         sizeof(pOneInput->CompareInchiFlags[1]));
  ...
  num_components_R = inchi_max(pOneInput->nNumComponents[INCHI_REC][TAUT_YES],
                               pOneInput->nNumComponents[INCHI_REC][TAUT_NON]);
  /***********************************************************************************************/
  /* InpInChI: count fragments -- disconnected components that do not match
   * reconnected          */
  /* Accumulate removed H and isotopic H from ALL Fixed-H disconnected
   * components except deleted */
  ...
  iInChI = INCHI_BAS;
  iMobileH =
      bHasSomeFixedH ? !pOneInput->nNumComponents[iInChI][TAUT_NON] : TAUT_YES;
  nNumCompHaveSeparateProtons_D = 0;

  /* in case of Mobile-H components here are the proton totals from the original
   * InChI disconn. layer */
  nNumRemovedProtons_D.nNumRemovedProtons =
      pOneInput->nNumProtons[iInChI][TAUT_YES].nNumRemovedProtons;
  memcpy(
      nNumRemovedProtons_D.nNumRemovedIsotopicH,
      pOneInput->nNumProtons[iInChI][TAUT_YES].nNumRemovedIsotopicH,
      sizeof(nNumRemovedProtons_D
                 .nNumRemovedIsotopicH)); /* total for the disconnected layer */
  ...
  for (k = 0; k < num_components_D; k++) {
    ...
    if (0 > (nLink = pOneInput->pInpInChI[iInChI][bMobileH][k].nLink)) {
      /* component in Disconnected layer is linked to the identical one in the
       * Reconnected layer */
      if (pOneInput->nNumProtons[INCHI_REC][TAUT_YES].pNumProtons) {
        nNumCompHaveSeparateProtons_D++;
        nLink = -(1 + nLink);
        nNumRemovedProtons_D.nNumRemovedProtons +=
            pOneInput->nNumProtons[INCHI_REC][TAUT_YES]
                .pNumProtons[nLink]
                .nNumRemovedProtons;
        for (m = 0; m < NUM_H_ISOTOPES; m++) {
          nNumRemovedProtons_D.nNumRemovedIsotopicH[m] +=
              pOneInput->nNumProtons[INCHI_REC][TAUT_YES]
                  .pNumProtons[nLink]
                  .nNumRemovedIsotopicH[m];
        }
      }
    ...
    if (pOneInput->nNumProtons[iInChI][TAUT_YES].pNumProtons) {
      nNumCompHaveSeparateProtons_D++;
      nNumRemovedProtons_D.nNumRemovedProtons +=
          pOneInput->nNumProtons[iInChI][TAUT_YES]
              .pNumProtons[k]
              .nNumRemovedProtons;
      for (m = 0; m < NUM_H_ISOTOPES; m++) {
        nNumRemovedProtons_D.nNumRemovedIsotopicH[m] +=
    ...
    num_fragments_D++; /* number of disconnected fragments from original
                          reconnected structure */
  }
  /* in case of Mobile-H components here are the proton totals from the original
   * InChI */
  /*
  nNumRemovedProtons_D_all.nNumRemovedProtons =
  pOneInput->nNumProtons[iInChI][TAUT_YES].nNumRemovedProtons; memcpy(
  nNumRemovedProtons_D_all.nNumRemovedIsotopicH,
          pOneInput->nNumProtons[iInChI][TAUT_YES].nNumRemovedIsotopicH,
  ...
  */
  ...
  /* accumulate removed H and isotopic H from ALL reconstructed reconnected
   * components except deleted */
  ...
  for (k = 0; k < num_components_R; k++) {
    ...
    if (0 < pOneInput->pInpInChI[iInChI][bMobileH][k].nLink) {
      ...
      /* same component is in the disconnected layer, it has no metal atoms or
       * is an isolated metal atom */
      pStruct1 = pStruct[iInChI][bMobileH] + k;
      ifMobileH =
          TAUT_YES; /* Mobile-H Aux_Info contains number removed protons */
      ifInChI = INCHI_BAS; /* this component cannot be reconnected */
      ifk = 0; /* 0th component since it is InChI of a single component */
      /* The statement in the following line is *WRONG*, component number mixed
       * with bMobileH:  */
      /* in RevInchi, when only Mobile-H is present then its only non-NULL InChI
       * has index 0==TAUT_NON */
      if (bRevInchiComponentExists(pStruct1, ifInChI, ifMobileH, ifk)) {
        ...
        if (pINChI_Aux) {
          nNumRemovedProtons_R.nNumRemovedProtons +=
              pINChI_Aux->nNumRemovedProtons;
          for (m = 0; m < NUM_H_ISOTOPES; m++) {
            nNumRemovedProtons_R.nNumRemovedIsotopicH[m] +=
                pINChI_Aux->nNumRemovedIsotopicH[m];
          }
        }
      }
      nNumCompHaveSeparateProtons_R +=
          bRevInchiComponentExists(pStruct1, ifInChI, ALT_TAUT(ifMobileH), ifk);
      continue; /* same as disconnected, has no metal atoms */
    }
    /* this reconstructed reconnected component WAS DISCONNECTED; check its
     * fragments */
    ...
    for (ifk = 0; ifk < num_fragments; ifk++) {
      ...
      num_fragments_R++; /* number of disconnected fragments from reconstructed
                            reconnected structure */
    }
  }
  /*---------------- special treatment of the last reconstructed component
   * -----------------*/
  /*---------------- this may contain separate protons added by the
   * reconstruction ---------*/
  ...
  if (iMobileH == TAUT_YES && !bHasSomeFixedH &&
    ...
    for (ifk = 0; ifk < num_fragments; ifk++) {
      ...
      */
      ...
      /*num_fragments_R ++;*/ /* number of disconnected fragments from
                                 reconstructed reconnected structure */
    }
  }
  ...
  pINChISort1 =
      (INCHI_SORT *)inchi_calloc(num_fragments_DR, sizeof(pINChISort1[0]));
  pINChISort2 =
      (INCHI_SORT *)inchi_calloc(num_fragments_DR, sizeof(pINChISort2[0]));
  if (!pINChISort1 || !pINChISort2) {
    ret = RI_ERR_ALLOC;
    goto exit_function;
  }

  /* accumulate original InChI of fragments -- disconnected components that do
   * not match reconnected */
  iInChI = INCHI_BAS;
  iMobileH =
      bHasSomeFixedH ? !pOneInput->nNumComponents[iInChI][TAUT_NON] : TAUT_YES;
  for (k = n = 0; k < num_components_D; k++) {
    ...
    /* the component exists in disconnected layer of the orig. InChI only: it is
     * a fragment */
    pINChISort2[n].pINChI[bMobileH] =
        pOneInput->pInpInChI[iInChI][bMobileH] + k;
    if (bMobileH == TAUT_NON &&
        (bInpInchiComponentExists(pOneInput, iInChI, TAUT_YES, k) ||
         bInpInchiComponentDeleted(pOneInput, iInChI, TAUT_YES, k))) {
      pINChISort2[n].pINChI[TAUT_YES] =
          pOneInput->pInpInChI[iInChI][TAUT_YES] + k;
    }
    /* the last sort key is a number of removed protons */
    pINChISort2[n].ord_number =
        pOneInput->nNumProtons[iInChI][TAUT_YES].pNumProtons
            ? pOneInput->nNumProtons[iInChI][TAUT_YES]
                  .pNumProtons[k]
                  .nNumRemovedProtons
            : 0;
  ...
  for (k = n = 0; k < num_components_R; k++) {
    ...
    for (i = 0; i < num_fragments; i++) {
      ...
      pINChISort1[n].pINChI[bfMobileH] =
          pStruct1->RevInChI.pINChI[ifInChI][i][bfMobileH];
      if ( bfMobileH == TAUT_NON /*&& bRevInchiComponentExists( pStruct1, ifInChI, TAUT_YES, i )*/ ) {
        pINChISort1[n].pINChI[TAUT_YES] =
            pStruct1->RevInChI.pINChI[ifInChI][i][TAUT_YES];
        /* remove Fixed-H InChI if is is identical to Mobile-H */
        /* do it exactly same way the identical components were removed from
         * InpInChI */
        if (!CompareReversedINChI(pINChISort1[n].pINChI[bfMobileH],
                                  pINChISort1[n].pINChI[TAUT_YES], NULL,
                                  NULL)) {
          pINChISort1[n].pINChI[bfMobileH] = NULL; /* remove Fixed-H layer */
        } else {
          pINChISort1[n].ord_number =
              pStruct1->RevInChI.pINChI_Aux[ifInChI][i][TAUT_YES]
                  ->nNumRemovedProtons;
        }
      }

      pINChISort1[n].n1 =
          k; /* reconstructed reconnected structure component index */
      pINChISort1[n].n2 =
          i; /* index of a fragment made out of this component */
      n++;
    }
  }
  ...
  for (iComponent = 0; iComponent < num_fragments_DR; iComponent++) {
    ...
    CompareTwoPairsOfInChI(pInChI1, pInChI2, !bHasSomeFixedH,
                           CompareInchiFlags);
  }
  ...
  if (/*nNumNonTaut1 && nNumNonTaut2 &&*/ bHasSomeFixedH) {
    if (nNumCompHaveSeparateProtons_D || nNumCompHaveSeparateProtons_R) {
      ...
      /* comparison does not make sense if Disconnected Fixed-H layer is not
       * present */
      for (iComponent = 0; iComponent < num_fragments_DR; iComponent++) {
        ...
        if (pINChISort1[iComponent].ord_number !=
            pINChISort2[iComponent].ord_number) {
          CompareInchiFlags[TAUT_YES] |=
              INCHIDIFF_MOBH_PROTONS; /* diff number of removed protons */
        }
        ...
        if (pOneInput->nNumProtons[INCHI_BAS][TAUT_YES].pNumProtons) {
          memcpy(nNumRemovedIsotopicH2,
                 pOneInput->nNumProtons[INCHI_BAS][TAUT_YES]
                     .pNumProtons[k]
        ...
        iInChI = INCHI_REC;
        iMobileH = bHasSomeFixedH ? !pOneInput->nNumComponents[iInChI][TAUT_NON]
                                  : TAUT_YES;
        bMobileH = iMobileH;
        if (!bInpInchiComponentExists(pOneInput, iInChI, bMobileH, k)) {
          if (bInpInchiComponentExists(pOneInput, iInChI, TAUT_YES, k)) {
      ...
      compare_iso_H:
        if (memcmp(nNumRemovedIsotopicH1, nNumRemovedIsotopicH2,
                   sizeof(nNumRemovedIsotopicH1))) {
  ...
  if (!nNumNonTaut1 == !nNumNonTaut2) {
    ; /* difference if(nNumNonTaut1 != nNumNonTaut2) will be caught in InChI
         comparison */
    ...
    /* the original InChI has Fixed-H while the reconstructed one has not:
     * missing Fixed-H layer */
  ...
  /* compare totals */
  if (nNumRemovedProtons_R.nNumRemovedProtons !=
      nNumRemovedProtons_D.nNumRemovedProtons) {
    CompareInchiFlags[TAUT_YES] |=
        INCHIDIFF_MOBH_PROTONS; /* diff number of removed protons */
  }
...
int CompareTwoPairsOfInChI(INChI *pInChI1[TAUT_NUM], INChI *pInChI2[TAUT_NUM],
  ...
  for (iMobileH = 0; iMobileH < TAUT_NUM; iMobileH++) {
    if (!pInChI1[iMobileH] != !pInChI2[iMobileH]) {
      if (iMobileH == TAUT_NON && pInChI1[TAUT_YES] && pInChI2[TAUT_YES]) {
    ...
    if (pInChI1[iMobileH] && pInChI2[iMobileH]) {
      cmp = CompareReversedINChI3(pInChI1[iMobileH], pInChI2[iMobileH], NULL,
                                  NULL, &err);
      if (cmp) {
...
int CompareOneOrigInchiToRevInChI(StrFromINChI *pStruct,
                                  INChI *pInChI[TAUT_NUM], int bMobileH,
  ...
  if (ret == _IS_OKAY || ret == _IS_WARNING) {
    ...
    if (b == TAUT_NON &&
        (!pStruct->RevInChI.pINChI[i] || !pStruct->RevInChI.pINChI[i][0][b] ||
    ...
    if (pStruct->RevInChI.num_components[i]) {
      /* compare InChI from restored structure; '0' in [i][0][b] is the first
       * component */
      if (b == TAUT_YES && pStruct->RevInChI.pINChI[i][0][b]->bDeleted &&
          (!pInChI[0] || pInChI[0]->bDeleted)) {
        /* the 1st component is made out of proton(s) and the input component is
         * missing or also a proton */
        cmp = 0;
      } else {
        cmp = CompareReversedINChI3(pStruct->RevInChI.pINChI[i][0][b],
                                    pInChI[0], NULL, NULL, &err);
        if (cmp) {
      ...
      if (b == b0 && b == TAUT_NON) {
        if ((pStruct->RevInChI.pINChI[i][0][TAUT_YES] &&
          ...
          cmp = CompareReversedINChI3(pStruct->RevInChI.pINChI[i][0][TAUT_YES],
                                      pInChI[1], NULL, NULL, &err);
          if (cmp) {
      ...
      for (k = 0; k < pStruct->RevInChI.num_components[i]; k++) {
        if (!k || pStruct->RevInChI.pINChI[i][k][TAUT_YES]->bDeleted) {
          /* get removed protons from the 1st component; add othere only if they
           * are deleted protons */
          nCurRemovedProtons->nNumRemovedProtons +=
              pStruct->RevInChI.pINChI_Aux[i][k][TAUT_YES]->nNumRemovedProtons;
          for (j = 0; j < NUM_H_ISOTOPES; j++) {
            nCurRemovedProtons->nNumRemovedIsotopicH[j] +=
                pStruct->RevInChI.pINChI_Aux[i][k][TAUT_YES]
                    ->nNumRemovedIsotopicH[j];
          }
        }
      }
    }
...
CompareReversedStereoINChI3(INChI_Stereo *s1 /* InChI from reversed struct */,
  ...
  if (s1 && s2 && (s2->nCompInv2Abs != 2) &&
      s1->nCompInv2Abs != s2->nCompInv2Abs && s1->nCompInv2Abs &&
      s2->nCompInv2Abs) {
    ret |= INCHIDIFF_SC_INV; /* 2007-07-13 DT: added (s2->nCompInv2Abs != 2) to
                                fix bug reoprted by Yerin on 2007/02/28 */
    /* Bug description: falsely reported "Stereo centers/allenes: Falsely
     * inverted" for /S2 or /S3 */
  }
...
INCHI_MODE CompareReversedINChI3(INChI *i1 /* InChI from reversed struct */,
  ...
  *err = 0;
  ...
  /*
     ...
     i2->lenTautomer > 1) ) { ret |=  INCHIDIFF_TAUT_LEN;
      }
  */
...
CMP_INCHI_MSG CompareInchiMsgs[] = {
    {INCHIDIFF_PROBLEM, IDGRP_ERR,
     " Wrong result"}, /*0x00000001,  severe: at least one InChI does not exist
                        */
    {INCHIDIFF_POSITION_H, IDGRP_H,
     " Locations or number"}, /*0x00000002,  difference in non-taut {Mobile-H}
                                 or all H {Fixed-H} location/number */
    {INCHIDIFF_MORE_FH, IDGRP_H, " Fixed-H"}, /*0x00000004,  extra fixed H */
    {INCHIDIFF_LESS_FH, IDGRP_H, " Fixed-H"}, /*0x00000004,  missing fixed H */
    {INCHIDIFF_MORE_H, IDGRP_H,
     " Number"}, /*0x00000008,  formulas differ in number of H */
    {INCHIDIFF_LESS_H, IDGRP_H,
     " Number"}, /*0x00000008,  formulas differ in number of H */
    {INCHIDIFF_NO_TAUT, IDGRP_MOB_GRP,
     " Missing"}, /*0x00000010,  restored structure has no taut groups while the
                     original InChI has some */
    {INCHIDIFF_WRONG_TAUT, IDGRP_MOB_GRP,
     " Falsely present"}, /*0x00000020,  restored has tautomerism while the
                             original does not have it */
    {INCHIDIFF_SINGLE_TG, IDGRP_MOB_GRP,
     " One instead of multiple"}, /*0x00000040,  restored has 1 taut. group
                                     while the original InChI has multiple tg */
    {INCHIDIFF_MULTIPLE_TG, IDGRP_MOB_GRP,
     " Multiple instead of one"}, /*0x00000080,  restored has multiple tg while
                                     the original InChI has only one tg */
    {INCHIDIFF_EXTRA_TG_ENDP, IDGRP_MOB_GRP,
     " Attachment points"}, /*0x00000100,  extra tautomeric endpoint{s} in
                               restored structure */
    {INCHIDIFF_MISS_TG_ENDP, IDGRP_MOB_GRP,
     " Attachment points"}, /*0x00000100,  one or more tg endpoint is not in the
                               restored structure */
    {INCHIDIFF_DIFF_TG_ENDP, IDGRP_MOB_GRP,
     " Attachment points"}, /*0x00000100,  lists of tg endpoints are different
                             */
    {INCHIDIFF_NUM_TG, IDGRP_MOB_GRP,
     " Number"}, /*0x00000200,  different number of tautomeric groups */
    {INCHIDIFF_TG, IDGRP_MOB_GRP,
     " Do not match"}, /*0x00000200,  different tautomeric groups */
    {INCHIDIFF_NUM_ISO_AT, IDGRP_ISO_AT,
     " Atoms do not match"}, /*0x00000400,  ?severe: restored struct. has
                                different number of isotopic atoms */
    {INCHIDIFF_ISO_AT, IDGRP_ISO_AT,
     " Atoms do not match"}, /*0x00000400,  ?severe: restored struct. has
                                different locations/isotopes of isotopic atoms
                              */
    {INCHIDIFF_REM_ISO_H, IDGRP_ISO_H,
     " Does not match for a component"}, /*0x00000800,  isotopic H removed */
    {INCHIDIFF_MOB_ISO_H, IDGRP_ISO_H,
     " Do not match"}, /*0x00001000,  different number of mobile exchangeable
                          isotopic H */
    {INCHIDIFF_CHARGE, IDGRP_CHARGE,
     " Do not match"}, /*0x00002000,  restored structure has different charge */
    {INCHIDIFF_REM_PROT, IDGRP_PROTONS,
     " Does not match for a component"}, /*0x00004000,  proton{s} removed/added
                                            from the restored structure */
    {INCHIDIFF_MOBH_PROTONS, IDGRP_PROTONS,
     " Does not match"}, /*0x00008000,  different proton balance */
    {INCHIDIFF_SC_INV, IDGRP_SC,
     " Falsely inverted"}, /*0x00010000,  restores structure has different
                              inversion stereocenter mark */
    {INCHIDIFF_SC_PARITY, IDGRP_SC,
     " Wrong parity"}, /*0x00020000,  restored structure has stereoatoms or
                          allenes with different parity */
    {INCHIDIFF_SC_EXTRA_UNDF, IDGRP_SC,
     " Extra undefined"}, /*0x00040000,  restored structure has extra undefined
                             stereocenter{s} */
    {INCHIDIFF_SC_EXTRA, IDGRP_SC,
     " Extra known"}, /*0x00080000,  restored structure has extra
                         stereocenter{s} */
    {INCHIDIFF_SC_MISS_UNDF, IDGRP_SC,
     " Missing undefined"}, /*0x00100000,  restored structure has not some
                               undefined stereocenter{s} */
    {INCHIDIFF_SC_MISS, IDGRP_SC,
     " Missing known"}, /*0x00200000,  restored structure has not some
                           stereocenters that are not undefined */
    {INCHIDIFF_SB_PARITY, IDGRP_SB,
     " Wrong parity"}, /*0x00400000,  restored structure has stereobonds or
                          cumulenes with different parity */
    {INCHIDIFF_SB_EXTRA_UNDF, IDGRP_SB,
     " Extra undefined"}, /*0x00800000,  restored structure has extra undefined
                             stereobond{s} */
    {INCHIDIFF_SB_EXTRA, IDGRP_SB,
     " Missing known"}, /*0x01000000,  restored structure has extra
                           stereobond{s} */
    {INCHIDIFF_SB_MISS_UNDF, IDGRP_SB,
     " Missing undefined"}, /*0x02000000,  restored structure has not some
                               undefined stereocenters */
    {INCHIDIFF_SB_MISS, IDGRP_SB,
     " Missing known"}, /*0x04000000,  restored structure has not some
                           stereobonds that are not undefined */
    {INCHIDIFF_COMP_HLAYER, IDGRP_HLAYER,
     " Missing or extra"}, /*0x08000000,  Restored component has Mobile-H layer
                              instead of both Mobile-H & Fixed-H or both instead
                              of one */
    {INCHIDIFF_COMP_NUMBER, IDGRP_COMP,
     " Does not match"}, /*0x10000000,  wrong number of components */
    {INCHIDIFF_STR2INCHI_ERR, IDGRP_CONV_ERR,
     " Error"}, /*0x20000000   Restored structure to InChI conversion error */
    {INCHIDIFF_ZERO, IDGRP_ZERO, ""}};

/*************************************************************************/
int AddOneMsg(char *szMsg, int used_len, int tot_len, const char *szAddMsg,
              const char *szDelim) {
  const char ellip[] = "...";
  ...
  if (len + len_delim + used_len < tot_len) {
    ...
    used_len += len;
                  ...
                  (tot_len - used_len - len_delim - (int)sizeof(ellip))) > 10) {
    if (len_delim) {
      strcpy(szMsg + used_len, szDelim);
      used_len += len_delim;
    }
    ...
    for (bMobileH = TAUT_YES; 0 <= bMobileH; bMobileH--) {
      ...
      do {
        if (bit & bits[bMobileH]) {
          ...
          bFound = 0;
          for (k = 0; CompareInchiMsgs[k].nBit != INCHIDIFF_ZERO && !bFound;
               k++) {
            if (bit & (INCHI_MODE)CompareInchiMsgs[k].nBit) {
              /* message found */
              for (n = 0; CompareInchiMsgsGroup[n].nGroupID != IDGRP_ZERO;
                   n++) {
                if (CompareInchiMsgsGroup[n].nGroupID ==
                    CompareInchiMsgs[k].nGroupID) {
                  iCurGrpIdx = n;
                  ...
                  len =
                      AddOneMsg(szMsg, len, nLenMsg, CompareInchiMsgs[k].szMsg,
