...
#define fNumRPosChgH                                                           \
  0 /* number of positive charges on endpoints that have H in at2[] */
#define fNumRPosChgU                                                           \
  1 /* number of positive charges on endpoints that have no H in at2[] */
...
#define fNumNPosChgH                                                           \
  5 /* number of positive charges on endpoints that have H in atf[] */
#define fNumNPosChgU                                                           \
  6 /* number of positive charges on endpoints that have no H in atf[] */
...
/* local prototypes */
static int FillTgDiffHChgFH(TgDiffHChgFH tdhc[], int max_tdhc, inp_ATOM at2[],
                            inp_ATOM atf[], AT_NUMB *nCanon2AtnoRevrs,
                            VAL_AT *pVA, T_GROUP_INFO *ti,
...
int bHas_N_V(inp_ATOM *at2, int num_atoms) {
  ...
  for (i = 0; i < num_atoms; i++) {
    if (at2[i].el_number == el_number_N && !at2[i].charge && !at2[i].num_H &&
        !at2[i].radical && at2[i].chem_bonds_valence == 5 &&
        (at2[i].valence == 3)) {
      num_found++;
    }
  }
      ...
      for (i = 0; i < itg_out; i++) {
        ...
        while (IndList.pnEdges[j + 1] == itg) {
          if (IndList.pnEdges[j] == type) {
            ...
            if ((ret = AddToEdgeList(pAtomIndList, IndList.pnEdges[j + 2],
                                     INC_ADD_EDGE))) {
              goto exit_function;
            }
          }
...
/***********************************************************************************************/
int FixFixedHRestoredStructure(
    ...
    VAL_AT *pVA, ALL_TC_GROUPS *pTCGroups, T_GROUP_INFO **ppt_group_info,
    inp_ATOM **ppat_norm, inp_ATOM **ppat_prep, INChI *pInChI[], long num_inp,
  ...
  int i, j, k, delta, num_try, tot_succes, cur_success,
      ret = 0, bAllowedNFlowerEdges = 0, num_zero_ret;
  CMP2FHINCHI c2i;
  CMP2FHINCHI *pc2i = &c2i;

  EDGE_LIST AllChargeEdges, CurrEdges, SFlowerEdges, NFlowerEdges,
      OtherNFlowerEdges, FixedLargeRingStereoEdges;
  ...
  for (i = 0; i < pStruct->num_atoms; i++) {
    ...
    if ((e = pVA[i].nCPlusGroupEdge - 1) >= 0 && !pBNS->edge[e].forbidden) {
      ...
      /* in addition, disallow N(V) creation by forbidding charge flower edge
       * that has flow=1 */
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      for (j = 0; j < at2[i].valence; j++) {
        if (pBNS->edge[k = pBNS->vert[i].iedge[j]].forbidden ==
          ...
          if (0 < nMinRingSize &&
              (ret = AddToEdgeList(&FixedLargeRingStereoEdges, k,
                                   INC_ADD_EDGE))) {
            goto exit_function;
          }
        }
  ...
  if (!pc2i->bHasDifference ||
      (!pc2i->len_c2at && pc2i->nNumTgRevrs == pc2i->nNumTgInChI &&
       ...
       pc2i->nNumEndpRevrs == pc2i->nNumEndpInChI && !pc2i->nNumTgDiffMinus &&
       !pc2i->nNumTgDiffH)) {
  ...
  if (pc2i->len_c2at >= 2) {
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (pc2i->c2at[i].nValElectr ==
              6 /* && !pc2i->c2at[i].endptInChI -- mod#1*/
          && (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e].forbidden) {
        if (/* orig. InChI info: */
            num_SB_O_Minus < MAX_DIFF_FIXH && pc2i->c2at[i].nFixHInChI == 0 &&
            pc2i->c2at[i].nMobHInChI == 0 &&
            /* reversed structure info: */
            pc2i->c2at[i].nFixHRevrs == -1 && pc2i->c2at[i].nMobHRevrs == 1 &&
            pc2i->c2at[i].nAtChargeRevrs == -1 &&
            !at2[iat].num_H && /* at2 is Fixed-H */
          ...
          if ((ret = AddToEdgeList(&CurrEdges, e, INC_ADD_EDGE))) {
            goto exit_function;
                   ...
                   num_DB_O < MAX_DIFF_FIXH && pc2i->c2at[i].nFixHInChI == -1 &&
                   pc2i->c2at[i].nMobHInChI == 1 &&
                   /* reversed structure info: */
                   pc2i->c2at[i].nFixHRevrs == 0 &&
                   pc2i->c2at[i].nMobHRevrs == 0 &&
    ...
    if (cur_success) {
      ...
      if (0 > (ret = MakeOneInChIOutOfStrFromINChI2(
                   ip, sd, pBNS, pStruct, at, at2, at3, pVA, pTCGroups,
  ...
  if (pc2i->len_c2at >= 1) {
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      iat = nCanon2AtnoRevrs[i];
      if (/* in restored atom: charge=+1, no H, has double bond, P, As, O, S,
             Se, Te, F, Cl, Br, I */
          num_DB_O_Plus < MAX_DIFF_FIXH && at2[iat].charge == 1 &&
          !at2[iat].num_H && at2[iat].valence < at2[iat].chem_bonds_valence &&
          !pVA[iat].cMetal &&
          (pVA[iat].cNumValenceElectrons == 6 ||
           pVA[iat].cNumValenceElectrons == 7 ||
           (pVA[iat].cNumValenceElectrons == 5 &&
            pVA[iat].cPeriodicRowNumber > 1)) &&
          /* in orig.InChI: not an endpoint, has no H */
          !pStruct->endpoint[i] && !(pStruct->fixed_H && pStruct->fixed_H[i]) &&
    ...
    repeat_02_allow_NV:
      for (i = 0; i < num_SB_NH && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
          ...
          /* Flow change on pe (+)charge edge (atom NH2) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
      ...
      if (num_zero_ret == num_try && !bAllowedNFlowerEdges &&
          NFlowerEdges.num_edges) {
  ...
  if (pc2i->len_c2at >= 1 && pc2i->nNumTgRevrs == 1 &&
      (pc2i->nNumEndpRevrs > pc2i->nNumEndpInChI ||
       pc2i->nNumTgInChI > 1) /* ADP in Revrs */) {
    /*--------------------------------------------------------------*/
    ...
    */
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: -O(-) */
          ...
          (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e].forbidden && pc2i->c2at[i].nFixHInChI == -1 &&
          pc2i->c2at[i].nMobHInChI == 1 &&
          /* reversed structure info: */
          pc2i->c2at[i].endptRevrs && pc2i->c2at[i].nFixHRevrs == 0 &&
          pc2i->c2at[i].nMobHRevrs == 0 && pc2i->c2at[i].nAtChargeRevrs == 0 &&
          !at2[iat].num_H && at2[iat].valence == 1 &&
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (/* in restored atom N: charge=-1, no H, has no double bond, endpoint
          ...
          pVA[iat].cNumValenceElectrons == 5 &&
          pVA[iat].cPeriodicRowNumber == 1 && at_Mobile_H_Revrs &&
          at_Mobile_H_Revrs[iat].endpoint &&
          /* in orig.InChI: not an endpoint, has no H */
          /* !pStruct->endpoint[i] && */
          /*
    ...
    if ((num_try = inchi_min(num_SB_N_Minus, num_DB_O))) {
      ...
      for (i = 0; i < num_SB_N_Minus && cur_success < num_try; i++) {
        ...
        pe = pBNS->edge + pVA[iat].nCMinusGroupEdge -
             1; /* 2006-03-03: changed from CPlusGroupEdge */
        if (!pe->flow)
          continue;
        pv1 = pBNS->vert + (v1 = pe->neighbor1);
        pv2 = pBNS->vert + (v2 = pe->neighbor12 ^ v1);
        ...
        if (ret == 1 &&
          ...
          /* Flow change on pe (-)charge edge (atom -N(-)-) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (/* in restored atom N: charge=-1, no H, has no double bond, endpoint
        ...
        iat_SB_N_Minus[num_SB_N_Minus++] = iat;
                 ...
                 !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 6 &&
                 at_Mobile_H_Revrs &&
                 at_Mobile_H_Revrs[iat]
                     .endpoint &&         /* endpoint in Reconstructed */
                 (pStruct->endpoint[i] || /* endpoint or H(+) acceptor in
                                             original */
                  (pnMobHInChI && pnMobHInChI[i] == 1 && pnFixHInChI &&
                   pnFixHInChI[i] == -1)) &&
                 /* has (-) edge */
                 (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
                 !pBNS->edge[e].forbidden) {
  ...
  if (pc2i->len_c2at >= 1 && pc2i->nNumTgInChI == 1 && /* ADP in InChI */
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      iat = nCanon2AtnoRevrs[i];
      if (/* in restored atom: charge=+1, has H, has double bond, N, O, S, Se,
             Te */
          num_DB_Charged < MAX_DIFF_FIXH && at2[iat].charge == 1 &&
          at2[iat].num_H && at2[iat].valence < at2[iat].chem_bonds_valence &&
          !pVA[iat].cMetal &&
          (pVA[iat].cNumValenceElectrons == 6 ||
           (pVA[iat].cNumValenceElectrons == 5 &&
      ...
      } else if (/* in restored atom: charge=0, has no H, has no double bond, N,
                    P, O, S, Se, Te */
                 num_SB_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
                 !at2[iat].num_H &&
  ...
  if (pStruct->fixed_H && pStruct->endpoint && pc2i->nChargeFixHInChI > 0 &&
    ...
    inp_ATOM *at_Mobile_H_Revrs = (pStruct->pOne_norm_data[1] &&
                         pStruct->pOne_norm_data[1]->at)?
    pStruct->pOne_norm_data[1]->at : NULL; inp_ATOM *atfMobile_H_Revrs =
    pStruct->pOne_norm_data[TAUT_YES] &&
                                  pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds?
                                  pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds
    : NULL; S_CHAR   *nMobHInChI = pInChI[1] && pInChI[1]->nNum_H?
    pInChI[1]->nNum_H : pInChI[0] && pInChI[0]->nNum_H? pInChI[0]->nNum_H :
    NULL;
    */
    ...
    CurrEdges.num_edges = 0;
    for (i = 0; i < pc2i->len_c2at; i++) {
      /* atoms -NH- from which H(+) were removed by the Normalization in orig.
       * InChI */
      iat = pc2i->c2at[i].atomNumber;
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      iat = nCanon2AtnoRevrs[i];
      if (pStruct->endpoint[i] || at2[iat].charge != 1 || at2[iat].radical ||
          pVA[iat].cMetal) {
        continue;
      }
      if (0 <= (e = pVA[iat].nCPlusGroupEdge - 1) && !pBNS->edge[e].forbidden &&
          !pBNS->edge[e].flow && pVA[iat].cNumValenceElectrons >= 5) {
        /* positively charged atom */
        for (j = 0; j < at2[iat].valence; j++) {
          if (at2[k = (int)at2[iat].neighbor[j]].charge == 1 &&
              !pVA[k].cMetal && 0 <= (e2 = pVA[k].nCPlusGroupEdge - 1) &&
              !pBNS->edge[e2].forbidden && !pBNS->edge[e2].flow) {
    ...
    if ((num_try = inchi_min(CurrEdges.num_edges, CurChargeEdges.num_edges))) {
      ...
      for (i = 0; i < CurrEdges.num_edges && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
          ...
          /* Flow change on pe (+)charge edge (atom NHm(+)) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
  ...
  if (pc2i->len_c2at >= 2) {
    ...
    pStruct->pOne_norm_data[TAUT_YES] &&
                                  pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds?
                                  pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds
    : pStruct->pOne_norm_data[TAUT_NON]->at; S_CHAR   *nMobHInChI = pInChI[1] &&
    pInChI[1]->nNum_H? pInChI[1]->nNum_H : pInChI[0] && pInChI[0]->nNum_H?
    pInChI[0]->nNum_H : 0;
    */
    EDGE_LIST CurChargeEdges;
    cur_success = 0;
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* reconstructed: non-taut and (+) */
          (pc2i->c2at[i].nMobHRevrs + 1 == pc2i->c2at[i].nFixHRevrs &&
           ...
           ((!pc2i->c2at[i].nFixHInChI &&
             pc2i->c2at[i].nMobHInChI == pc2i->c2at[i].nFixHRevrs) ||
            (pc2i->c2at[i].nFixHInChI == pc2i->c2at[i].nFixHRevrs &&
             pc2i->c2at[i].endptInChI))) &&
        ...
        if (/* original InChI: has H and is not an endpoint */
            (pc2i->c2at[i].nMobHInChI + 1 == pc2i->c2at[i].nFixHInChI &&
             ...
             pc2i->c2at[i].nAtChargeRevrs == 0 &&
             /* reconstructed InChI: non-taut & has H or an endpoint, has Fixed
                H */
             ((!pc2i->c2at[i].nFixHRevrs &&
               pc2i->c2at[i].nMobHRevrs == pc2i->c2at[i].nFixHInChI) ||
              (pc2i->c2at[i].nFixHRevrs == pc2i->c2at[i].nFixHInChI &&
               pc2i->c2at[i].endptRevrs))) &&
            0 <= (e = pVA[iat].nCPlusGroupEdge - 1) &&
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if ((pc2i->c2at[i].nValElectr == 6 ||
        ...
        if (/* orig. InChI info: NHm */
            ...
            pc2i->c2at[i].nFixHInChI ==
                1 && /*pc2i->c2at[i].nMobHInChI == 0 &&*/
            /* reversed structure info: */
            pc2i->c2at[i].nFixHRevrs ==
                0 && /*pc2i->c2at[i].nMobHRevrs == 1 &&*/
            pc2i->c2at[i].nAtChargeRevrs == 0 &&
            at2[iat].num_H && /* at2 is Fixed-H */
            at2[iat].valence == at2[iat].chem_bonds_valence) {
          ...
          if ((ret = AddToEdgeList(&CurrEdges, e, INC_ADD_EDGE))) {
            goto exit_function;
                   ...
                   pc2i->c2at[i].nFixHInChI ==
                       0 && /*pc2i->c2at[i].nMobHInChI &&*/
                   /* reversed structure info: */
                   pc2i->c2at[i].nFixHRevrs ==
                       1 && /*pc2i->c2at[i].nMobHRevrs ==  0 &&*/
                   pc2i->c2at[i].nAtChargeRevrs == 1 &&
                   at2[iat].num_H &&
                   at2[iat].valence < at2[iat].chem_bonds_valence) {
  ...
  if (((pc2i->nNumTgInChI > pc2i->nNumTgRevrs && pc2i->nNumTgRevrs == 1) ||
    ...
    inp_ATOM *at_Mobile_H_Revrs = (pStruct->pOne_norm_data[1] &&
                         pStruct->pOne_norm_data[1]->at)?
    pStruct->pOne_norm_data[1]->at : NULL;
    */
    inp_ATOM *atfMobile_H_Revrs =
        pStruct->pOne_norm_data[TAUT_YES] &&
                pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds
            ? pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds
            : NULL;
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      /* -NH-, -OH */
      if (pStruct->fixed_H[i] && !nMobHInChI[i] && at2[iat].charge == 0 &&
          at2[iat].radical == 0 && 0 <= (e = pVA[iat].nCPlusGroupEdge - 1) &&
          !pBNS->edge[e].forbidden && pBNS->edge[e].flow &&
        ...
        if (at2[iat].charge == 1 && !at2[iat].num_H &&
            pVA[iat].cNumValenceElectrons == 5 &&
            pVA[iat].cPeriodicRowNumber == 1 && atfMobile_H_Revrs &&
            atfMobile_H_Revrs[iat].charge == 0 &&
            0 <= (e = pVA[iat].nCPlusGroupEdge - 1) &&
            !pBNS->edge[e].forbidden && !pBNS->edge[e].flow &&
            (ret = AddToEdgeList(&CurChargeEdges, e, INC_ADD_EDGE))) {
          goto exit_case_06a;
  ...
  if (pc2i->nNumTgInChI > 1 &&
      (pStruct->nNumRemovedProtonsMobHInChI > 0 ||
       pStruct->ti.tni.nNumRemovedProtons > 0) &&
      pStruct->fixed_H && pStruct->endpoint &&
      pStruct->pOne_norm_data[TAUT_YES] &&
      pStruct->pOne_norm_data[TAUT_YES]->at_fixed_bonds) {
    /*----------------------------------------------------------*/
    ...
    int iat, nNumWrongTg, jjoffs, jj, nNum2RemovePlus, nNum2AddPlus,
        nNum2MovePlus;
    AT_NUMB *nCanon2AtnoRevrs = pStruct->nCanon2Atno[0];
    /*
    inp_ATOM *at_Mobile_H_Revrs = (pStruct->pOne_norm_data[1] &&
    ...
    */
    ...
    nNumWrongTg =
        FillTgDiffHChgFH(tdhc, MAX_DIFF_FIXH, at2, atfMobile_H_Revrs,
                         nCanon2AtnoRevrs, pVA, &pStruct->ti, &EndpList);
    if (nNumWrongTg < 1) {
      goto exit_case_06e; /* for now only transfer (+) from one Mobile-H group
                             to another */
    }
    ...
    for (i = 0; i < nNumWrongTg; i++) {
      ...
      if (tdhc[i].nNumHInchi > tdhc[i].nNumHNorml &&
        ...
        int nNumNeeded = inchi_min(tdhc[i].nNumHInchi - tdhc[i].nNumHNorml,
                                   tdhc[i].n[fNumRPosChgH]);
        ...
        if (tdhc[i].nNumHInchi < tdhc[i].nNumHNorml &&
            tdhc[i].n[fNumRNeutrlH]) {
          /* store XH atom numbers */
          int nNumNeeded = inchi_min(tdhc[i].nNumHNorml - tdhc[i].nNumHInchi,
                                     tdhc[i].n[fNumRNeutrlH]);
    ...
    if (CurrEdges.num_edges > 0 && CurChargeEdges.num_edges > 0) {
      for (i = 0; 0 < nNum2MovePlus && i < nNumWrongTg; i++) {
        /* detect t-group that has extra (+) on H */
        if (tdhc[i].nNumHInchi > tdhc[i].nNumHNorml &&
            tdhc[i].nNumPRevrs > tdhc[i].nNumPNorml &&
            tdhc[i].n[fNumRPosChgH]) {
          ...
          SetForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
          RemoveForbiddenEdgeMask(pBNS, &CurrEdges, forbidden_edge_mask);
          for (jj = 0; 0 < nNum2MovePlus && 0 < nNum2Remove &&
                       jj < tdhc[i].n[fNumRPosChgH];
               jj++) {
            iat = EndpList.pnEdges[jjoffs + jj];
            e = pVA[iat].nCPlusGroupEdge - 1;
            pe = pBNS->edge + pVA[iat].nCPlusGroupEdge - 1;
            ...
            ret = RunBnsTestOnce(pBNS, pBD, pVA, &vPathStart, &vPathEnd,
                                 &nPathLen, &nDeltaH, &nDeltaCharge,
                                 &nNumVisitedAtoms);

            if (ret == 1 &&
                ((vPathEnd == v1n && vPathStart == v2n) ||
  ...
  if (pc2i->len_c2at >= 1) {
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (/* in restored atom: charge=-1, no H, has single bond, O, S, Se, Te */
          num_SB_O_Minus < MAX_DIFF_FIXH && at2[iat].charge == -1 &&
          !at2[iat].num_H && at2[iat].valence == at2[iat].chem_bonds_valence &&
          !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 6 &&
    ...
    if ((num_try = inchi_min(num_SB_O_Minus, num_DB_O_Neutr))) {
      ...
      for (i = 0; i < num_SB_O_Minus && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
          ...
          /* Flow change on pe (-)charge edge (O(-)-AB) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
  ...
  if (pc2i->len_c2at >= 1) {
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (/* in restored atom: charge=-1, no H, has single bond, O, S, Se, Te */
          ...
          /* has N(V) neighbor */
          1 == at2[iat].valence &&
          at2[iN = at2[iat].neighbor[0]].chem_bonds_valence == 5 &&
  ...
  if (/*(pc2i->len_c2at >= 1 || pc2i->nNumRemHRevrs) &&*/ pc2i->nNumTgInChI ==
          1 && /* ADP in InChI */
      (pc2i->nNumEndpRevrs < pc2i->nNumEndpInChI || pc2i->nNumTgRevrs > 1)) {
    /*----------------------------------------------------------------*/
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      iat = nCanon2AtnoRevrs[i];
      if (/* in restored atom: charge=-1, has no H, has single bond, O, S, Se,
             Te */
          num_SB_O_Minus < MAX_DIFF_FIXH && at2[iat].charge == -1 &&
          !at2[iat].num_H && at2[iat].valence == at2[iat].chem_bonds_valence &&
          !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 6 &&
      ...
      } else if (/* in restored atom: charge=0, has no H, has double
                    non-stereogenic bond, N */
                 num_DB_N_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
                 !at2[iat].num_H && !at2[iat].sb_parity[0] &&
                 at2[iat].valence < at2[iat].chem_bonds_valence &&
                 !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 5 &&
                 pVA[iat].cPeriodicRowNumber == 1 &&
    ...
    if ((num_try = inchi_min(num_DB_N_Neutr, num_SB_O_Minus))) {
      ...
      for (i = 0; i < num_SB_O_Minus && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
          ...
          /* Flow change on pe (-)charge edge (atom (-)O-) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
  ...
  if (pc2i->len_c2at >= 2) {
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if ((pc2i->c2at[i].nValElectr == 5 && pc2i->c2at[i].nPeriodNum == 1) &&
          /* orig. InChI info: */
          !pc2i->c2at[i].endptInChI && pc2i->c2at[i].nFixHInChI == 0 &&
          pc2i->c2at[i].nMobHInChI &&
          /* reversed structure info: */
          pc2i->c2at[i].endptRevrs && pc2i->c2at[i].nFixHRevrs &&
          !pc2i->c2at[i].nMobHRevrs && pc2i->c2at[i].nAtChargeRevrs == 1 &&
          at2[iat].valence + 1 == at2[iat].chem_bonds_valence &&
          (e = pVA[iat].nCPlusGroupEdge - 1) >= 0 && !pBNS->edge[e].forbidden) {
        ...
        if (pVA[iatC].cNumValenceElectrons != 4 || pVA[iatC].cMetal ||
            at2[iatC].charge || at2[iatC].valence != 3 ||
            at2[iatC].valence + 1 != at2[iatC].chem_bonds_valence ||
            (eCPlusC = pVA[iatC].nCPlusGroupEdge - 1) < 0 ||
            pBNS->edge[eCPlusC].forbidden)
          continue;
        for (j = 0; j < at2[iatC].valence; j++) {
          iatNH2 = at2[iatC].neighbor[j];
          if (iatNH2 == iat || pVA[iatNH2].cNumValenceElectrons != 5 ||
              pVA[iatNH2].cPeriodicRowNumber != 1 || !at2[iatNH2].num_H ||
              at2[iatNH2].charge)
            continue;
          ...
          if ((pc2i->c2at[iNH2].nValElectr == 5 &&
               pc2i->c2at[iNH2].nPeriodNum == 1) &&
              /* orig. InChI info: */
              !pc2i->c2at[iNH2].endptInChI &&
              pc2i->c2at[iNH2].nFixHInChI == 0 && pc2i->c2at[iNH2].nMobHInChI &&
              /* reversed structure info: */
              pc2i->c2at[iNH2].endptRevrs && pc2i->c2at[iNH2].nFixHRevrs &&
              !pc2i->c2at[iNH2].nMobHRevrs &&
  ...
  if (pc2i->len_c2at >= 2) {
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if ((pc2i->c2at[i].nValElectr == 6 ||
           (pc2i->c2at[i].nValElectr == 5 && pc2i->c2at[i].nPeriodNum == 1)) &&
          /* orig. InChI info: */
          pc2i->c2at[i].endptInChI && pc2i->c2at[i].nFixHInChI &&
          !pc2i->c2at[i].nMobHInChI &&
          /* reversed structure info: */
          !pc2i->c2at[i].endptRevrs && !pc2i->c2at[i].nFixHRevrs &&
          pc2i->c2at[i].nMobHRevrs && pc2i->c2at[i].nAtChargeRevrs == 0 &&
          at2[iat].valence == at2[iat].chem_bonds_valence &&
          (e = pVA[iat].nCPlusGroupEdge - 1) >= 0 && !pBNS->edge[e].forbidden) {
        ...
        for (j1 = 0; j1 < at2[iat].valence && bContinue; j1++) {
          if (at2[iat].bond_type[j1] == BOND_TYPE_SINGLE &&
              ...
              (4 <= pVA[iatC].cNumValenceElectrons &&
               pVA[iatC].cNumValenceElectrons <= 6) &&
            ...
            for (j2 = 0; j2 < at2[iatC].valence && bContinue; j2++) {
              if (at2[iatC].bond_type[j2] == BOND_TYPE_SINGLE &&
                  ...
                  (pVA[iatNH2].cNumValenceElectrons == 5 ||
                   pVA[iatNH2].cNumValenceElectrons == 6) &&
                  at2[iatNH2].valence == at2[iatNH2].chem_bonds_valence &&
                  (eCPlusNH2 = pVA[iatNH2].nCPlusGroupEdge - 1) >= 0 &&
                  !pBNS->edge[eCPlusNH2].forbidden) {
                for (iNH2 = 0; iNH2 < pc2i->len_c2at; iNH2++) {
                  if (iatNH2 != pc2i->c2at[iNH2].atomNumber ||
                      pc2i->c2at[iNH2].nValue)
                    continue;
                  /* check the second -NH */
                  icNH2 = pStruct->nAtno2Canon[0][iatNH2]; /* canon number -1 */
                  if (/* orig. InChI info: */
                      pc2i->c2at[iNH2].endptInChI &&
                      pc2i->c2at[iNH2].nFixHInChI &&
                      !pc2i->c2at[iNH2].nMobHInChI &&
                      /* reversed structure info: */
                      !pc2i->c2at[iNH2].endptRevrs &&
                      !pc2i->c2at[iNH2].nFixHRevrs &&
                      pc2i->c2at[iNH2].nMobHRevrs &&
                      pc2i->c2at[iNH2].nAtChargeRevrs == 0) {
                    ...
                    if (!pe_iat->forbidden && pBNS->edge[e].flow) {
                      pe = pBNS->edge + e;
                    } else if (!pe_iNH2->forbidden &&
                               pBNS->edge[eCPlusNH2].flow) {
                      pe = pBNS->edge + eCPlusNH2;
                    } else {
                      continue; /* none of the two -X(+)- bonds may be changed
                                 */
                    }
                    if ((ret = AddToEdgeList(&CurrEdges, eCPlusC,
                                             INC_ADD_EDGE))) {
                      goto exit_function;
                    }
                    SetForbiddenEdgeMask(pBNS, &AllChargeEdges,
                                         forbidden_edge_mask);
                    RemoveForbiddenEdgeMask(pBNS, &CurrEdges,
                                            forbidden_edge_mask);
                    delta = 1;
                    ...
                    if (ret == 1 &&
                        ...
                        nDeltaCharge == -1) {
                      /* Removed (+)charge from -NHn => nDeltaCharge == -1 */
                      /* Flow change on pe (+)charge edge (atom NHm(+)) is not
                       * known to RunBnsTestOnce()) */
                      ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
                      if (ret > 0) {
                        nNumRunBNS++;
                        cur_success++; /* 10 */
                    ...
                    /*pe->forbidden &= forbidden_edge_mask_inv;*/
                    RemoveForbiddenEdgeMask(pBNS, &AllChargeEdges,
                                            forbidden_edge_mask);
                    CurrEdges.num_edges = 0; /* clear current edge list */
                    break;
                  }
      ...
      for (i = 0; i < num_DB_Charged && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
          ...
          /* before setting flows the structure could be:
             [NH+ neigh, v1n]=e1n=[NH+,v1]-pe-[+,v2]=e2n=[another at or its
             chargeStruct] or

               [NH+ or ChStr, v1n]=pe1n=[NH+ or ChStr, v1]-pe-[+,v2]=pe2n=[at2
             or ChStr, v2n] ^    ^    ^ NH+(+)edge |  N (+) edge: only |  these
             are not forbidden
                                                                |
                                                         hetero (+) vertex
             ...
             *[NH+ or ChStr, v1n]-pe1n-[NH+ or ChStr, v1]=pe=[+,v2]-pe2n-[at2 or
             ChStr, v2n]* ^    ^    ^ NH+(+)edge |  N (+) edge: only |  these
             are not forbidden
                                                                |
                                                         hetero (+) vertex
              ...
              [at2 or ChStr, v2n] is at2 then at2 charge would change from (+)
             to 0, and another N charge would change from 0 to (+), giving tot.
             change of number of charges  (-1)+(+1)=0. However, if [at2 or
             ChStr, v2n] is ChargeStruct then at2 will not be on the alt path
             and only the creation of another (+) will be detected.
          */
          /* Removed charge from O(+) => nDeltaCharge == -1 */
          /* Flow change on pe (+)charge edge (atom NH2) is not known to
           * RunBnsTestOnce()) */
  ...
  if (pc2i->len_c2at >= 1 && pc2i->nNumTgInChI == 1 &&
      ...
      (pc2i->nNumEndpInChI > pc2i->nNumEndpRevrs ||
       pc2i->nNumTgRevrs > 1) /* ADP in InChI */) {
    /*--------------------------------------------------------------*/
    ...
    while (1) {
      ...
      for (i = 0; i < pc2i->len_c2at; i++) {
        ...
        if (/* orig. InChI info: -O(-) */
            ...
            pc2i->c2at[i].endptInChI &&
            (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
            !pBNS->edge[e].forbidden && pc2i->c2at[i].nFixHInChI == 0 &&
            pc2i->c2at[i].nMobHInChI == 0 &&
            /* reversed structure info: */
            !pc2i->c2at[i].endptRevrs && pc2i->c2at[i].nFixHRevrs == 0 &&
            pc2i->c2at[i].nMobHRevrs == 0 &&
      ...
      for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
        iat = nCanon2AtnoRevrs[i];
        bN_V = 0;
        if (/* in restored atom N: charge=0, no H, has no double bond, not an
               endpoint */
            num_SB_N_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
            !at2[iat].num_H &&
            (at2[iat].valence == at2[iat].chem_bonds_valence ||
          ...
          if (bOnly_N_V && bN_V &&
            ...
            /* in addition, permit N(V)=>N(IV)(+) change by allowing charge
             * flower edge change flow */
            if (bN_V &&
                NO_VERTEX != (j = GetChargeFlowerUpperEdge(pBNS, pVA, e)) &&
                !pBNS->edge[j].forbidden && !pBNS->edge[j].flow) {
              if ((ret = AddToEdgeList(&CurrEdges, j, INC_ADD_EDGE))) {
      ...
      if ((num_try = inchi_min(num_SB_N_Neutr, num_DB_O))) {
        ...
        for (i = 0; i < num_DB_O && cur_success < num_try; i++) {
          ...
          pe->forbidden |=
              forbidden_edge_mask; /* change bond O=X to O(rad)-X(rad) */
          pe->flow -= delta;
          pv1->st_edge.flow -= delta;
          pv2->st_edge.flow -= delta;
          pBNS->tot_st_flow -= 2 * delta;
          ...
          pe->forbidden &=
              forbidden_edge_mask_inv; /* allow changes to O=X bond */
          INCHI_HEAPCHK
        }
        RemoveForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
      }
  ...
  if (pc2i->nNumTgDiffMinus /*|| pc2i->nNumTgDiffH */
      /* no ADP in InChI needed */) {
    /*--------------------------------------------------------------*/
    /*                         |                            |       */
    ...
    for (itg = 0;
         itg < pStruct->ti.num_t_groups && itg < pStruct->One_ti.num_t_groups;
         itg++) {
      if (pStruct->ti.t_group[itg].nNumEndpoints ==
              pStruct->One_ti.t_group[itg].nNumEndpoints &&
          pStruct->ti.t_group[itg].num[0] - pStruct->ti.t_group[itg].num[1] ==
              pStruct->One_ti.t_group[itg].num[0] -
                  pStruct->One_ti.t_group[itg].num[1] &&
        ...
        short iat_SB_N_Neutr[MAX_DIFF_FIXH], iat_DB_O[MAX_DIFF_FIXH];
        cur_success = 0;
        for (i = 0; i < pStruct->num_atoms;
             i++) { /* i = canonical number - 1 */
          iat = nCanon2AtnoRevrs[i];
          if (/* orig. InChI info: -O(-) */
              num_DB_O < MAX_DIFF_FIXH &&
              (pVA[i].cNumValenceElectrons == 6 /* O, S, Se, Te */) &&
              pStruct->endpoint[i] == itg + 1 &&
              (e = pVA[i].nCMinusGroupEdge - 1) >= 0 &&
              !pBNS->edge[e].forbidden &&
              !(pStruct->fixed_H && pStruct->fixed_H[i]) &&
              !(nMobHInChI && nMobHInChI[i]) &&
              /* reversed structure info: */
              ...
              !(pnMobHRevrs && pnMobHRevrs[iat]) && at2[iat].charge == 0 &&
              at2[iat].num_H == 0 && at2[iat].valence == 1 &&
              at2[iat].chem_bonds_valence == 2) {
            ...
            */
          } else if (/* in restored atom N: charge=0, no H, has no double bond,
                        not an endpoint */
                     num_SB_N_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
                     !at2[iat].num_H &&
                     /*at2[iat].valence == at2[iat].chem_bonds_valence ||*/
                     (at2[iat].valence == 4 &&
                      at2[iat].chem_bonds_valence == 5) &&
                     !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 5 &&
                     pVA[iat].cPeriodicRowNumber >= 1 &&
                     !(at_Mobile_H_Revrs && at_Mobile_H_Revrs[iat].endpoint) &&
                     /* in orig.InChI: not an endpoint, has no H */
  ...
  if (((pc2i->nNumTgInChI <= 1 && pc2i->nNumRemHInChI > pc2i->nNumRemHRevrs) ||
       pc2i->len_c2at) &&
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      if (/* restored structure */
          num_N_V < MAX_DIFF_FIXH && at2[iat].chem_bonds_valence == 5 &&
          at2[iat].valence == 3 && !at2[iat].charge && !at2[iat].radical &&
          pVA[iat].cNumValenceElectrons == 5 &&
          pVA[iat].cPeriodicRowNumber == 1 &&
          !(at_Mobile_H_Revrs && at_Mobile_H_Revrs[i].endpoint) &&
          !at2[iat].num_H && (e = pVA[iat].nCPlusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e].forbidden && pBNS->edge[e].flow /* no charge */ &&
          NO_VERTEX != (j = GetChargeFlowerUpperEdge(pBNS, pVA, e)) &&
          !pBNS->edge[j].forbidden &&
          !pBNS->edge[j].flow /* neutral, valence=5 */ &&
          /* orig. InChI */
          !pStruct->endpoint[i] && !(nMobHInChI && nMobHInChI[i]) &&
          !pStruct->fixed_H[i]) {
        ...
        if (/* restored structure */
            ...
            pVA[iat].cPeriodicRowNumber == 1 &&
            !(at_Mobile_H_Revrs && at_Mobile_H_Revrs[i].endpoint) &&
            !at2[iat].num_H && (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
            !pBNS->edge[e].forbidden && !pBNS->edge[e].flow /* no charge */ &&
            /* orig. InChI */
            /*!pStruct->endpoint[i] &&*/
    ...
    if (num_N_V && iat_N_III_List.num_edges && iat_X_List.num_edges) {
      for (i1 = 0; i1 < num_N_V && cur_success < max_success; i1++) {
        ...
        if (NO_VERTEX == iat_N_V ||
            ...
            NO_VERTEX ==
                (e1Flower = GetChargeFlowerUpperEdge(pBNS, pVA, e1Plus)) ||
            1 != pBNS->edge[e1Plus].flow || 0 != pBNS->edge[e1Flower].flow) {
          continue;
        }
        for (i2 = iat_N_III_List.num_edges - 1;
             0 <= i2 && cur_success < max_success; i2--) {
          ...
          if (NO_VERTEX == iat_N_III ||
              ...
              0 != pBNS->edge[e2Minus].flow || 1 != pBNS->edge[e2Plus].flow) {
            /* do not consider this atom anymore */
            iat_N_III_List.pnEdges[i2] = NO_VERTEX;
            continue;
          }
          for (i3 = iat_X_List.num_edges - 1;
               0 <= i3 && cur_success < max_success; i3--) {
            ...
            */
            ...
            for (j = pv2->num_adj_edges - 1; 0 <= j;
                 j--) { /* was -2; changed 2006-2-28 12:35pm*/
              pe2n = pBNS->edge + pv2->iedge[j];
            ...
            RemoveForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
            RemoveForbiddenEdgeMask(pBNS, &OtherNFlowerEdges,
                                    forbidden_edge_mask);
            if (ret > 0) {
  ...
  if (pc2i->nNumTgMRevrs > pc2i->nNumTgMInChI ||
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (/* -O(+)= in restored atom: charge=1, has no H, a double bond */
        ...
        */
      } else if (/* -N< in restored atom: charge=0, has no H, has no double
                    bond, N only */
                 num_SB_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
                 !at2[iat].num_H &&
                 at2[iat].valence == at2[iat].chem_bonds_valence &&
      ...
      for (i = 0; i < num_DB_Charged && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
            ((vPathEnd == v1n && vPathStart == v2n) ||
             (vPathEnd == v2n && vPathStart == v1n)) &&
            (nDeltaCharge == 0 || nDeltaCharge == 1)) {
          /* Moved charge from O(+) to -N< => nDeltaCharge == 1 or 0 if pe2n =
           * -N< charge edge */
          /* Flow change on pe (+)charge edge (atom NH2) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
  ...
  if (pc2i->nNumTgDiffMinus) {
    ...
    for (itg = 0;
      ...
      for (j = 0, k = pStruct->One_ti.t_group[itg].nFirstEndpointAtNoPos;
        ...
        i = pStruct->One_ti.nEndpointAtomNumber[k + j]; /* canonical number in
                                                           restored struct. */
        iat = nCanon2AtnoRevrs[i];
        if (/* in restored atom: charge=0, has no H, has double bond, O, S, Se,
               Te */
            num_DB_O_Neutr < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
            !at2[iat].num_H && at2[iat].valence < at2[iat].chem_bonds_valence &&
            !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 6 &&
            /* in orig.InChI: an endpoint, may have fixed-H */
            pStruct->endpoint[i] &&
          ...
          if ((ret = AddToEdgeList(&CurrEdges, e, INC_ADD_EDGE))) {
            goto exit_function;
                   ...
                   num_SB_N_Minus < MAX_DIFF_FIXH && at2[iat].charge == -1 &&
                   at2[iat].num_H &&
                   at2[iat].valence == at2[iat].chem_bonds_valence &&
                   !pVA[iat].cMetal && pVA[iat].cNumValenceElectrons == 5 &&
                   pVA[iat].cPeriodicRowNumber == 1 &&
        ...
        */
        ...
        for (i = 0; i < num_SB_N_Minus && cur_success < num_try; i++) {
          ...
          if (ret == 1 &&
            ...
            /* Flow change on pe (-)charge edge (atom -NH(-)) is not known to
             * RunBnsTestOnce()) */
            ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
            if (ret > 0) {
              nNumRunBNS++;
      ...
      for (i = 0; i < num_DB_Charged && cur_success < num_try; i++) {
        ...
        if (ret == 1 &&
            ((vPathEnd == v1n && vPathStart == v2n) ||
             (vPathEnd == v2n && vPathStart == v1n)) &&
            (nDeltaCharge == 0 || nDeltaCharge == 1)) {
          /* Moved charge from OH(+) to -O- => nDeltaCharge == 1 or 0 if pe2n =
           * -O- charge edge */
          /* Flow change on pe (+)charge edge (atom OH(+)) is not known to
           * RunBnsTestOnce()) */
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
  ...
  if ((pc2i->nNumTgInChI && pStruct->endpoint &&
       pc2i->nNumTgMInChI > pc2i->nNumTgMRevrs &&
       pc2i->nNumEndpInChI > pc2i->nNumEndpRevrs)) {
    /*-----------------------------------------------------------------*/
    /*                                                                 */
    ...
    for (itg = 0;
      ...
      for (j = 0, k = pStruct->ti.t_group[itg].nFirstEndpointAtNoPos;
        ...
        i = pStruct->ti.nEndpointAtomNumber[k + j]; /* canonical number in
                                                       restored struct. */
        iat = nCanon2AtnoRevrs[i];
        if (!pStruct->endpoint[i] || !at_Mobile_H_Revrs ||
            at_Mobile_H_Revrs[iat].endpoint ||
            pVA[i].cNumValenceElectrons != 5 ||
            pVA[i].cPeriodicRowNumber != 1 || 2 != at2[iat].valence ||
            at2[iat].num_H || at2[iat].radical ||
            (0 <= (e1 = pVA[iat].nCPlusGroupEdge - 1) &&
             !pBNS->edge[e1].flow) ||
    ...
    for (i = 0; i < pStruct->num_atoms && cur_success < max_success; i++) {
      iat = nCanon2AtnoRevrs[i];
      if (pStruct->endpoint[i] || !pVA[i].cNumValenceElectrons ||
          pVA[i].cNumValenceElectrons == 4 || at2[iat].num_H ||
          at2[iat].radical ||
      ...
      if (ret == 1 &&
        ...
        /* Flow change on pe (+)charge edge (atom X) is not known to
         * RunBnsTestOnce()) */
        ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
        if (ret > 0) {
          nNumRunBNS++;
  ...
  if (pc2i->len_c2at >= 1) {
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: =NH2(+), =OH(+) */
          ...
          (eMPlus = pVA[iatMetal].nCPlusGroupEdge - 1) >= 0 &&
          !pBNS->edge[eMPlus].forbidden &&
          (eMMinus = pVA[iatMetal].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[eMMinus].forbidden &&
    ...
    if (CurrEdges.num_edges) {
      ...
      for (i = 0; i < CurrEdges.num_edges; i++) {
        ...
        if (ret == 1 &&
            ((vPathEnd == v1 && vPathStart == v2) ||
             (vPathEnd == v2 && vPathStart == v1)) &&
            nDeltaCharge == DeltaChargeExpected) {
          ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
          if (ret > 0) {
            nNumRunBNS++;
            cur_success++; /* 19 */
  ...
  if (pc2i->len_c2at > 1 && pc2i->nNumTgRevrs && pc2i->nNumTgInChI) {
    ...
    inp_ATOM *at_Mobile_H_Revrs = (pStruct->pOne_norm_data[1] &&
                         pStruct->pOne_norm_data[1]->at)?
    pStruct->pOne_norm_data[1]->at : NULL; S_CHAR   *nMobHInChI = pInChI[1] &&
    pInChI[1]->nNum_H? pInChI[1]->nNum_H : pInChI[0] && pInChI[0]->nNum_H?
    pInChI[0]->nNum_H : 0;
    */
    cur_success = 0;
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: =O or -N= */
          num_DB_N < MAX_DIFF_FIXH && pc2i->c2at[i].endptInChI &&
          (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e].forbidden && pBNS->edge[e].flow == 0 &&
          pc2i->c2at[i].nFixHInChI == 0 && pc2i->c2at[i].nMobHInChI == 0 &&
          /* if  more than 1 t-group are in orig. InChI then do not move (-) to
             N */
        ...
        if ((ret = AddToEdgeList(&CurrEdges, e, INC_ADD_EDGE))) {
          goto exit_function;
                 ...
                 num_SB_O_Minus < MAX_DIFF_FIXH && !pc2i->c2at[i].endptInChI &&
                 (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
                 !pBNS->edge[e].forbidden && pBNS->edge[e].flow == 1 &&
                 pc2i->c2at[i].nFixHInChI == 0 &&
                 pc2i->c2at[i].nMobHInChI == 0 &&
                 pc2i->c2at[i].nValElectr == 6 &&
                 /* reversed structure info: */
                 pc2i->c2at[i].endptRevrs && pc2i->c2at[i].nFixHRevrs == 0 &&
    ...
    if (!num_DB_N) {
      ...
      for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
        ...
        if (/* in restored atom O: charge=-1, no H, has no double bond, endpoint
             */
            num_DB_N < MAX_DIFF_FIXH && at2[iat].charge == 0 &&
            !at2[iat].num_H &&
            at2[iat].valence + 1 == at2[iat].chem_bonds_valence &&
            !pVA[iat].cMetal &&
  ...
  if (pc2i->len_c2at && pc2i->nNumTgRevrs && pc2i->nNumTgHInChI &&
      pStruct->endpoint) {
    /*--------------------------------------------------------------*/
    /*                      O(-)                      O             */
    /*                      |                         ||            */
    ...
    short iat_SB_O_Minus[MAX_DIFF_FIXH], iat_Central[MAX_DIFF_FIXH],
        iat_DB_O[MAX_DIFF_FIXH];
    AT_NUMB *nCanon2AtnoRevrs = pStruct->nCanon2Atno[0];
    inp_ATOM *at_Mobile_H_Revrs =
        (pStruct->pOne_norm_data[1] && pStruct->pOne_norm_data[1]->at)
            ? pStruct->pOne_norm_data[1]->at
    ...
    */
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: =O    */
          ...
          pc2i->c2at[i].nFixHRevrs == 0 && pc2i->c2at[i].nAtChargeRevrs == 0 &&
          !at2[iat].num_H && at2[iat].valence == 1 &&
          at2[iat].chem_bonds_valence == 2) {
        iat_DB_O[num_DB_O++] = iat;
        if ((ret = AddToEdgeList(&CurrEdges, e, INC_ADD_EDGE))) {
    ...
    for (i = 0; num_DB_O && i < pStruct->num_atoms;
         i++) { /* i = canonical number - 1 */
      if (!pStruct->endpoint[i]) {
        continue;
      }
      ...
      if (/* in restored atom O: charge=-1, no H, has no double bond, endpoint
          ...
          num_SB_O_Minus < MAX_DIFF_FIXH && at2[iat].charge == -1 &&
          !at2[iat].num_H && at2[iat].valence == 1 &&
          at2[iat].chem_bonds_valence && !pVA[iat].cMetal &&
          pVA[iat].cNumValenceElectrons == 6 &&
          (at_Mobile_H_Revrs && at_Mobile_H_Revrs[iat].endpoint) &&
          /* in orig.InChI: an endpoint, has no H */
          !(pStruct->fixed_H && pStruct->fixed_H[i]) &&
  ...
  if (pc2i->len_c2at && pc2i->nNumTgRevrs &&
      pc2i->nNumEndpInChI < pc2i->nNumEndpRevrs) {
    /*--------------------------------------------------------------*/
    /*                      O                         O             */
    /*                      ||                        ||            */
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: -X(-)    */
        ...
        if (pVA[iat].cNumValenceElectrons == 6 && at2[iat].charge == -1 &&
            pBNS->edge[e].flow && at2[iat].valence == 1 &&
            at2[iat].chem_bonds_valence == 1 &&
            pVA[iS = (int)at2[iat].neighbor[0]].cNumValenceElectrons == 6 &&
            pVA[iS].cPeriodicRowNumber > 1 && at2[iS].valence >= 4) {
          ...
          for (j = 0; j < at[iS].valence; j++) {
            ...
            if (pc2i->c2at[i].endptRevrs == at_Mobile_H_Revrs[k].endpoint &&
              ...
              nNumTautDB += (bond_type == BOND_TYPE_DOUBLE);
              nNumEndpO +=
                  (pVA[k].cNumValenceElectrons == 6 && at2[k].valence == 1);
            } else if (bond_type == BOND_TYPE_DOUBLE) {
              nNumOtherDB++;
            } else if (bond_type == BOND_TYPE_SINGLE) {
              nNumOtherSB++;
          ...
          for (j = 0; j < at[iS].valence; j++) {
            ...
            k = at2[iS].neighbor[j];
            if (pc2i->c2at[i].endptRevrs == at_Mobile_H_Revrs[k].endpoint &&
                !at2[k].endpoint && pVA[k].cNumValenceElectrons == 6 &&
                at2[k].valence == 1 && 0 <= (e = pVA[k].nCMinusGroupEdge - 1) &&
                !pBNS->edge[e].forbidden) {
              if (bond_type == BOND_TYPE_DOUBLE && !at2[k].charge &&
                  !pBNS->edge[e].flow) {
                /* charges to be unchanged */
                if ((ret = AddToEdgeList(&OtherSO, e, INC_ADD_EDGE))) {
                  goto exit_case_21a;
                }
              } else if (bond_type == BOND_TYPE_SINGLE && at2[k].charge == -1 &&
                         pBNS->edge[e].flow) {
  ...
  if (pc2i->len_c2at) {
    ...
    */
    ...
    for (i = 0; num_DB_O && i < pStruct->num_atoms;
      ...
      if (/* in restored atom O: charge=-1, no H, has no double bond, endpoint
           */
          at2[iat].charge == -1 && !at2[iat].num_H && at2[iat].valence == 1 &&
          at2[iat].chem_bonds_valence == 2 && !pVA[iat].cMetal &&
          pVA[iat].cNumValenceElectrons == 5 &&
          (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e].forbidden && pBNS->edge[e].flow &&
          !(at_Mobile_H_Revrs && at_Mobile_H_Revrs[iat].endpoint) &&
          pVA[iN2 = at2[iat].neighbor[0]].cNumValenceElectrons == 5 &&
          at2[iat].bond_type[0] == BOND_TYPE_DOUBLE && at2[iN2].charge == 1 &&
          at2[iN2].valence == 2 && at2[iN2].chem_bonds_valence == 4 &&
          pVA[iC = at2[iN2].neighbor[at2[iN2].neighbor[0] == iN2]]
...
#define CHG_SET_NUM 4
    ...
    */
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      iat = pc2i->c2at[i].atomNumber;
      if (/* orig. InChI info: taut in orig. InChI =O located in -NO2 that is
             not taut in Reconstructed InChI */
          num_DB_O < MAX_DIFF_FIXH &&
          pc2i->c2at[i].nValElectr == 6 /* O, S, Se, Te */ &&
          (pc2i->c2at[i].endptInChI /*|| pc2i->c2at[i].nMobHInChI*/) &&
          (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          ...
          at2[iN].valence == 3 &&
          (at2[iN].charge == 0 || at2[iN].charge == 1) &&
        ...
        for (k = 0; k < at2[iN].valence; k++) {
          ...
          if (pVA[neigh].cNumValenceElectrons == 6 &&
              ...
              at2[neigh].radical == 0 &&
              (at2[neigh].charge == 0 || at2[neigh].charge == -1) &&
        ...
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e,
                                 INC_ADD_EDGE))) {
          goto exit_case_23;
        }
      }
    ...
    if (num_DB_O) {
      ...
      for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
        ...
        if (!pStruct->endpoint[i] || pVA[i].cNumValenceElectrons != 6 ||
            ...
            !pBNS->edge[e].flow || at2[iN].charge || at2[iN].valence != 3 ||
        ...
        for (k = 0; k < at2[iN].valence; k++) {
          ...
          if (pVA[neigh].cNumValenceElectrons == 6 &&
              pStruct->endpoint[neigh] && at2[neigh].valence == 1 &&
              at2[neigh].num_H == 1 && at2[neigh].radical == 0 &&
              (at2[neigh].charge == 0)) {
            nNumO++;
        ...
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NOOH], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e,
                                 INC_ADD_EDGE))) {
          goto exit_case_23;
        }
        if (NO_VERTEX != (j = GetChargeFlowerUpperEdge(pBNS, pVA, e)) &&
            ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NOOH], j,
                                  INC_ADD_EDGE)) ||
             (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e,
                                  INC_ADD_EDGE)))) {
          goto exit_case_23;
      ...
      for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
        iat = nCanon2AtnoRevrs[i];
        if (at2[iat].charge == -1 && !pStruct->endpoint[i] &&
            (at_Mobile_H_Revrs &&
             (at_Mobile_H_Revrs[i].endpoint ||
              at2[iat].num_H < at_Mobile_H_Revrs[i].num_H))) {

          if (0 <= (e = pVA[iat].nCMinusGroupEdge - 1) &&
              0 > FindInEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e) &&
              !pBNS->edge[e].forbidden && pBNS->edge[e].flow &&
              ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_WRONG_TAUT], e,
                                    INC_ADD_EDGE)) ||
               (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e,
                                    INC_ADD_EDGE)))) {
            goto exit_case_23;
          ...
          /* negatively charged atom in Reconstructed structure got H(+) from
           * Normalization */
      ...
      for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
        ...
        if (pStruct->endpoint[i] &&
          ...
          if (0 <= (e = pVA[iat].nCMinusGroupEdge - 1) &&
              !pBNS->edge[e].forbidden && pBNS->edge[e].flow &&
              0 > FindInEdgeList(&ChangeableEdges[CHG_SET_O_FIXED], e) &&
              (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_TAUT], e,
                                   INC_ADD_EDGE))) {
            goto exit_case_23;
          }
        }
      ...
      for (i = 0; i < num_DB_O; i++) {
        ...
        for (k = 0; !one_success && k <= CHG_LAST_SET; k++) {
          ...
          SetForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
          RemoveForbiddenEdgeMask(pBNS, &ChangeableEdges[k],
                                  forbidden_edge_mask);
          ...
          if (ret == 1 &&
            ...
            /* Move (-) charge to =O and remove it an endpoint => nDeltaCharge
             * == 0 */
            ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
            if (ret > 0) {
              nNumRunBNS++;
  ...
  if (pc2i->len_c2at && pc2i->nNumTgInChI == 1) {
    ...
    inp_ATOM *at_Mobile_H_Revrs =
        (pStruct->pOne_norm_data[1] && pStruct->pOne_norm_data[1]->at)
            ? pStruct->pOne_norm_data[1]->at
            : NULL;
    inp_ATOM *atf = (pStruct->pOne_norm_data[1] &&
                     pStruct->pOne_norm_data[1]->at_fixed_bonds)
                        ? pStruct->pOne_norm_data[1]->at_fixed_bonds
                        : NULL;
    int iN, one_success;
    ...
    */
    ...
    for (i = 0; i < pc2i->len_c2at; i++) {
      ...
      if (/* orig. InChI info: -N=N(V)    */
          ...
          (e1 = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
          !pBNS->edge[e1].forbidden && pc2i->c2at[i].nFixHInChI == 0 &&
          pc2i->c2at[i].nMobHInChI == 0 &&
          /* reversed structure info: */
          !pc2i->c2at[i].endptRevrs && pc2i->c2at[i].nFixHRevrs == 0 &&
          pc2i->c2at[i].nAtChargeRevrs == 0 && !at2[iat].num_H &&
          at2[iat].valence == 2 && at2[iat].chem_bonds_valence == 3 &&
          /* find whether -N= has =N(V) neighbor; Note: operator comma: (A,B)
             returns B */
          (iN = at2[iat].neighbor[at2[iat].bond_type[0] != BOND_TYPE_DOUBLE],
           pVA[iN].cNumValenceElectrons == 5) &&
          at2[iN].chem_bonds_valence == 5 && at2[iN].charge == 0 &&
          !at2[iN].num_H && !at2[iN].radical &&
          0 <= (e = pVA[iN].nCPlusGroupEdge - 1) && !pBNS->edge[e].forbidden &&
          pBNS->edge[e].flow &&
          0 > FindInEdgeList(&ChangeableEdges[CHG_SET_AVOID], e)) {

        ef = GetChargeFlowerUpperEdge(pBNS, pVA,
                                      e); /* == NO_VERTEX if N(V) has 4 bonds */
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], ef,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], 1,
                                 INC_ADD_EDGE)) || /* expected nDeltaCharge */
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e1,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], ef,
                                 INC_ADD_EDGE))) {
          goto exit_case_24;
        }
        ...
        if (0 <= (e = pVA[iat].nCMinusGroupEdge) && !pBNS->edge[e].forbidden &&
          ...
          goto exit_case_24;
                 ...
                 atf && pc2i->c2at[i].nValElectr == 5 /* N or P */ &&
                 pc2i->c2at[i].endptInChI /* only N */ &&
                 (e = pVA[iat].nCMinusGroupEdge - 1) >= 0 &&
                 !pBNS->edge[e].forbidden && pc2i->c2at[i].nFixHInChI == 0 &&
                 pc2i->c2at[i].nMobHInChI == 0 &&
                 ...
                 /* find whether -N= has =N(V) neighbor; Note: operator comma:
                    (A,B) returns B */
                 (iN = atf[iat]
                           .neighbor[atf[iat].bond_type[0] != BOND_TYPE_DOUBLE],
                  pVA[iN].cNumValenceElectrons == 5) &&
                 at2[iN].charge == 1 && /* double bond neighbor */
                 at2[iN].chem_bonds_valence == 4 && atf[iN].charge == 0 &&
                 atf[iN].chem_bonds_valence ==
                     5 && /* InChI normalization created N(V)=N- out of
                             N(IV)(+)-N(-)- */
                 !at2[iN].num_H &&
                 !at2[iN].radical && 0 <= (e = pVA[iat].nCMinusGroupEdge - 1) &&
                 !pBNS->edge[e].forbidden && pBNS->edge[e].flow &&
                 0 > FindInEdgeList(&ChangeableEdges[CHG_SET_AVOID], e)) {
        /* save (-) edge */
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], NO_VERTEX,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NN], 1,
                                 INC_ADD_EDGE)) || /* expected nDeltaCharge */
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
    ...
    for (i = 0; i < pStruct->num_atoms; i++) { /* i = canonical number - 1 */
      ...
      if (!(at_Mobile_H_Revrs && at_Mobile_H_Revrs[iat].endpoint)) {
        if (0 > FindInEdgeList(&ChangeableEdges[CHG_SET_AVOID], e) &&
            ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_MISSED_TAUT], e,
                                  INC_ADD_EDGE)) ||
             (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                  INC_ADD_EDGE)))) {
          goto exit_case_24;
        ...
        if (pVA[iat].cNumValenceElectrons == 6) {
          if (0 > FindInEdgeList(&ChangeableEdges[CHG_SET_AVOID], e) &&
              ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_OTHER_TAUT_O], e,
                                    INC_ADD_EDGE)) ||
               (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                    INC_ADD_EDGE)))) {
            goto exit_case_24;
          ...
          if (pVA[iat].cNumValenceElectrons == 5) {
            if (0 > FindInEdgeList(&ChangeableEdges[CHG_SET_AVOID], e) &&
                ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_OTHER_TAUT_N], e,
                                      INC_ADD_EDGE)) ||
                 (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                      INC_ADD_EDGE)))) {
              goto exit_case_24;
    ...
    for (i = 0; i < ChangeableEdges[CHG_SET_NN].num_edges; i += 3) {
      ...
      pef = (NO_VERTEX != ChangeableEdges[CHG_SET_NN].pnEdges[i + 1])
                ? pBNS->edge + ChangeableEdges[CHG_SET_NN].pnEdges[i + 1]
                : NULL;
    ...
    */
    ...
    for (i = 0; i < pStruct->num_atoms; i++) {
      ...
      if (pStruct->endpoint[i]) {
        ...
        bFirst =
            ((pVA[iat].cNumValenceElectrons == 5 && pc2i->nNumTgInChI == 1) ||
        ...
        if (!(at_Mobile_H_Revrs && at_Mobile_H_Revrs[i].endpoint)) {
          ...
          if (bFirst &&
              (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_MISSED_TAUT_1], e,
                                   INC_ADD_EDGE))) {
            goto exit_case_25;
          }
          if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_MISSED_TAUT_ALL], e,
                                   INC_ADD_EDGE))) {
            goto exit_case_25;
          }
        }
        if (bFirst &&
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_OTHER_TAUT_1], e,
                                 INC_ADD_EDGE))) {
          goto exit_case_25;
        }
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_OTHER_TAUT_ALL], e,
                                 INC_ADD_EDGE))) {
          goto exit_case_25;
        }
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                 INC_ADD_EDGE))) {
          goto exit_case_25;
        }
      } else if (at2[iat].valence == 1 && at2[iat].charge == -1 &&
                 pVA[iat].cNumValenceElectrons == 6 &&
                 pVA[iN = at2[iat].neighbor[0]].cNumValenceElectrons ==
                     5 && /* -O(-) */
                 ...
                 0 <= (e1 = pVA[iat].nCMinusGroupEdge - 1) &&
        ...
        for (k = 0; k < at2[iN].valence; k++) {
          ...
          if (pVA[neigh].cNumValenceElectrons == 6 &&
              !pStruct->endpoint[neigh] && at2[neigh].valence == 1 &&
              at2[neigh].num_H == 0 && at2[neigh].radical == 0 &&
              at2[neigh].charge == -1 && at2[neigh].chem_bonds_valence == 1) {
            nNumO++;
          } else if (at2[iN].bond_type[k] == BOND_TYPE_SINGLE &&
                     at2[neigh].valence > 1 &&
                     at2[neigh].valence < at2[neigh].chem_bonds_valence) {
        ...
        if ((ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NO_IN_NO2M2], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_NO_IN_NO2M2], e1,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e,
                                 INC_ADD_EDGE)) ||
            (ret = AddToEdgeList(&ChangeableEdges[CHG_SET_AVOID], e1,
    ...
    /* ------- finally, try to move charges from -NO2(2-) or to tautomeric
     * endpoints ----*/
    for (i = 0; i < ChangeableEdges[CHG_SET_NO_IN_NO2M2].num_edges; i += 2) {
      ...
      /* change flow on O(-) to make it neutral; 3 new charges will be created:
         ...
         be nutralized (-1 charge) and two (-) charges on taut. endpoints will
         be created (+2); the total change in this case would be (-1)+(+2) = +1
      */
      one_success = 0;
      delta = 1;
      pe = pBNS->edge +
           ChangeableEdges[CHG_SET_NO_IN_NO2M2].pnEdges[i + 1]; /* O(-) edge */
      pef = pBNS->edge +
            ChangeableEdges[CHG_SET_NO_IN_NO2M2].pnEdges[i]; /* >N- (+) edge */
