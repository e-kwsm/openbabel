...
int FixRestoredStructureStereo(
  ...
  BNS_EDGE *pe, *peRad;
  EDGE_LIST AllChargeEdges, CurrEdges, NFlowerEdges, OtherNFlowerEdges,
      FixedStereoEdges, AllRadList;
  ...
  /* mobile-H layer in case of Fixed-H */
  pStereo2Revrs =
      (pStruct->bMobileH == TAUT_YES || !pStruct->pOneINChI[1] ||
       !pStruct->pOneINChI[1]->nNumberOfAtoms ||
  ...
  if (iRevrInChI || iOrigInChI) {
    ...
    cmpInChI2 = CompareReversedINChI2(
  ...
  if ((cmpInChI & IDIF_SB_MISS) && (!cmpInChI2 || (cmpInChI2 & IDIF_SB_MISS)) &&
    ...
    RemoveForbiddenEdgeMask(pBNS, &FixedStereoEdges,
    ...
    for (i = 0; i < icr->num_sb_in2_only && cur_success < max_success; i++) {
      ...
      if (ret == 1 /*&& !nDeltaH*/ && !nDeltaCharge &&
          (v2 == vPathStart || v2 == vPathEnd)) {
  ...
  if (!(cmpInChI & IDIF_SB_MISS) && (cmpInChI2 & IDIF_SB_MISS) &&
    ...
    */
    ...
    if (0 >= retC && 0 >= retS && 0 >= ret2C && 0 > ret2S) {
      ; /* accept */
    } else {
      /* reject */
      for (i = 0; i < AllRadList.num_edges; i++) {
  ...
  if (pStruct->bMobileH == TAUT_NON && (cmpInChI & IDIF_SB_EXTRA_UNDF) &&
    ...
    for (i = 0; i < icr->num_sb_undef_in1_only; i++) {
      ...
      if (!AllChargeEdges.num_edges && !TautMinusEdges[0].num_edges &&
          !TautMinusEdges[1].num_edges) {
        for (j = 0; j < pStruct->num_atoms; j++) {
          if ((k = pVA[j].nCMinusGroupEdge - 1) >= 0 &&
          ...
          if ((k = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
              !pBNS->edge[k].forbidden) {
            if ((ret = AddToEdgeList(&AllChargeEdges, k, INC_ADD_EDGE))) {
              goto exit_function;
            }
            /* in addition, disallow N(V) creation by forbidding charge flower
             * edge that has flow=1 */
            if (pVA[j].cNumValenceElectrons == 5 &&
                !pVA[j].cMetal && /* N, P, As */
      ...
      if (!pe->flow)
        continue;
      /* fix all charges except tautomeric; first allow only O, then only N,
       * finally both N and O */
      SetForbiddenEdgeMask(pBNS, &AllChargeEdges, forbidden_edge_mask);
      for (k = 1, bDone = 0; k < 4 && !bDone; k++) {
        /* fix tautomeric charges */
        num_endpoints =
            (TautMinusEdges + 0)->num_edges + (TautMinusEdges + 1)->num_edges;
        if (k == 2) {
          /* fix charges on O */
        ...
        if (k == 2)
          RemoveForbiddenEdgeMask(pBNS, TautMinusEdges + 0,
                                  forbidden_edge_mask);
        if (k == 1)
          RemoveForbiddenEdgeMask(pBNS, TautMinusEdges + 1,
                                  forbidden_edge_mask);
      }
      ...
      if (!AllChargeEdges.num_edges && !TautMinusEdges[0].num_edges &&
        ...
        for (j = 0; j < pStruct->num_atoms; j++) {
          ...
          if ((k = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
              !pBNS->edge[k].forbidden) {
            int bMayBeUnfixed =
                !at2[j].num_H && !(pStruct->endpoint && pStruct->endpoint[j]);
            if ((bMayBeUnfixed && pVA[j].cNumValenceElectrons == 6) ||
                (pVA[j].cNumValenceElectrons == 5 &&
                 pVA[j].cPeriodicRowNumber > 1)) {
              /* O & P */
              if ((ret = AddToEdgeList(TautMinusEdges + 0, k, INC_ADD_EDGE))) {
                goto exit_function;
              }
            } else if (bMayBeUnfixed && pVA[j].cNumValenceElectrons == 5 &&
                       pVA[j].cPeriodicRowNumber == 1) {
              /* N */
              if ((ret = AddToEdgeList(TautMinusEdges + 1, k, INC_ADD_EDGE))) {
                goto exit_function;
  ...
  if (pStruct->bMobileH == TAUT_YES &&
      (cmpInChI & IDIF_SB_EXTRA_UNDF && !pStruct->ti.num_t_groups)
    ...
    for (i = 0; i < icr->num_sb_undef_in1_only; i++) {
      ...
      /* Replace =NH with -NH2
         Create such a charge on some -N< that may be moved to NH2 to remove
         H(+): transformation: from:  HN=C-=-N=(+vert)-Y=(+super)-(+/-) to:
         2HN-C*-=-N=(+vert)-Y=(+super)-(+/-)* Run BNS to obtain:
      ...
      */
      ...
      for (i1 = 0; i1 < pBNS->vert[vPlusMinus].num_adj_edges; i1++) {
        i2 = pBNS->edge[pBNS->vert[vPlusMinus].iedge[i1]].neighbor12 ^
             vPlusMinus;
      ...
      if (!AllChargeEdges.num_edges) {
        for (j = 0; j < pStruct->num_atoms; j++) {
          ...
          if ((k = pVA[j].nCPlusGroupEdge - 1) >= 0 &&
              !pBNS->edge[k].forbidden) {
            if (pVA[j].cNumValenceElectrons == 5 &&
                pVA[j].cPeriodicRowNumber == 1 && !at2[j].num_H &&
                at2[j].valence == 3 &&
                !(at2[j].endpoint ||
                  (pStruct->endpoint && pStruct->endpoint[j]))) {
              ; /* do not fix -N< or =N(+)< */
            } else {
              /* all others */
              if ((ret = AddToEdgeList(TautMinusEdges + 0, k, INC_ADD_EDGE))) {
      ...
      SetForbiddenEdgeMask(pBNS, &AllChargeEdges,
                           forbidden_edge_mask); /* fix aC edges */
      pe->cap++;
      ret = RunBnsTestOnce(pBNS, pBD, pVA, &vPathStart, &vPathEnd, &nPathLen,
                           &nDeltaH, &nDeltaCharge, &nNumVisitedAtoms);

      if (ret == 1 &&
          ((vPathEnd == vPlusMinus && vPathStart == aC) ||
           (vPathEnd == aC && vPathStart == vPlusMinus)) &&
          nDeltaCharge == 1) {
        /* Negative charge has been moved, no change in number of charges */
        ret = RunBnsRestoreOnce(pBNS, pBD, pVA, pTCGroups);
        if (ret > 0) {
        ...
        pBNS->vert[aN].st_edge.cap += delta;  /* avoid radical on N */
        pBNS->vert[aC].st_edge.flow += delta; /* create radical on C */
        pBNS->vert[vPlusMinus].st_edge.cap -=
            delta; /* create radical on (+/-) */
        pBNS->tot_st_flow += 2 * delta;
  ...
  cur_success = 0;
  if (pStruct->bMobileH == TAUT_NON &&
      pStereo2Revrs /* added check 2006-04-05 */ &&
      (cmpInChI2 & IDIF_SB_EXTRA_UNDF && !pStruct->ti.num_t_groups)
      /*pStruct->bMobileH == TAUT_NON && (cmpInChI2 & IDIF_SB_EXTRA_UNDF)*/) {
    /*----------------------------------------------------------*/
