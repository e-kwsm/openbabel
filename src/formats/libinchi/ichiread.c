...
static int AddInChIChar(INCHI_IOSTREAM *pInp, SEGM_LINE *Line,
                        const char *pszToken);
static int AddLinkedBond(AT_NUMB at1, AT_NUMB at2, AT_NUMB num_at,
                         LINKED_BONDS *pLB);
static int bInChIHasReconnectedMetal(INChI *pInChI);
static int SetProtonsAndXchgIsoH(int bInChI2Structure, int bReqSplitOutputInChI,
...
static int nGetInChISegment(INCHI_IOSTREAM *pInp, SEGM_LINE *Line,
                            const char *pszToken);
...
static int CopyAtomNumbers(INChI *pInChI_To, int bIsoTo, INChI *pInChI_From,
                           int bIsoFrom);

static int ParseSegmentFormula(const char *str, int bMobileH,
                               INChI *pInpInChI[], int nNumComponents[]);
static int ParseSegmentConnections(const char *str, int bMobileH,
                                   INChI **pInpInChI, int *pnNumComponents,
                                   int *pbAbc);
...
static int bIsSp3LayerNotEmpty(INChI *pInpInChI[], int bMobileH, int bIso,
                               int nNumComponents);
static int ParseSegmentSp3s(const char *str, int bMobileH, INChI *pInpInChI[],
                            int s[TAUT_NUM][2], int ppnNumComponents[],
                            int state);
static int ParseSegmentIsoAtoms(const char *str, int bMobileH,
                                INChI *pInpInChI[], int nNumComponents[],
                                int state, int *pbAbc);
...
static int ReadInChILine(INCHI_IOSTREAM *pInp, SEGM_LINE *pLine, char **pStr,
                         int *pState, INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                         int nNumComponents[INCHI_NUM][TAUT_NUM],
                         REM_PROTONS nNumProtons[INCHI_NUM][TAUT_NUM],
                         int s[INCHI_NUM][TAUT_NUM][2], int *bStdFormat,
                         int *bInputHasSaveOpt,
                         unsigned char *inp_save_opt_bits);

int InChILine2Data(INCHI_IOSTREAM *pInp, SEGM_LINE *pLine, char **pStr,
                   int *pState, int *nErr,
                   INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                   int nNumComponents[INCHI_NUM][TAUT_NUM],
                   REM_PROTONS nNumProtons[INCHI_NUM][TAUT_NUM],
                   int s[INCHI_NUM][TAUT_NUM][2], int bReadCoord,
                   int bInchi2Struct, INCHI_MODE nMode, int *bStdFormat,
...
static int OutputInChIAsRequested(INCHI_IOSTREAM *pOut, INCHI_IOSTREAM *pLog,
                                  ...
                                  STRUCT_DATA *sd_inp, InpInChI *OneInput,
                                  int num_components[INCHI_NUM],
                                  MODE_PIXH nModeProtonIsoExchgH[INCHI_NUM],
                                  long num_inp, unsigned char save_opt_bits);

static int ParseAuxSegmentVersion(const char *str, int bMobileH,
                                  INChI *pInpInChI[], int ppnNumComponents[],
                                  int state);
static int ParseAuxSegmentNumbers(const char *str, int bMobileH,
                                  INChI *pInpInChI[], int ppnNumComponents[],
                                  int state, int *pbAbc);
static int ParseAuxSegmentAtomEqu(const char *str, int bMobileH,
                                  INChI *pInpInChI[], int ppnNumComponents[],
                                  int state);
static int ParseAuxSegmentGroupEqu(const char *str, int bMobileH,
                                   INChI *pInpInChI[], int ppnNumComponents[],
                                   int state);
static int ParseAuxSegmentSp3Inv(const char *str, int bMobileH,
                                 INChI *pInpInChI[], int ppnNumComponents[],
                                 int state);
static int ParseAuxSegmentSp3InvNumbers(const char *str, int bMobileH,
                                        INChI *pInpInChI[],
                                        int ppnNumComponents[], int state);
static int ParseAuxSegmentReverseCRV(const char *str, int bMobileH,
                                     INChI *pInpInChI[], int ppnNumComponents[],
                                     int state);
static int ParseAuxSegmentReverseAtoms(const char *str, int bMobileH,
                                       INChI *pInpInChI[],
                                       int ppnNumComponents[], int state);
static int ParseAuxSegmentReverseBonds(const char *str, int bMobileH,
                                       INChI *pInpInChI[],
                                       int ppnNumComponents[], int state);
static int ParseAuxSegmentReverseXYZ(const char *str, int bMobileH,
                                     XYZ_COORD **ppXYZ, INChI *pInpInChI[],
                                     int ppnNumComponents[], int state);
...
#define SEG_END '/'
/* the following 2 definitions are used to allow tab-delimited InChI input -
 * 2008-11-17 DT */
...
int SetHillFormFromInChI(InpInChI *OneInput) {
  ...
  for (iINChI = 0, num_diff = 0; iINChI < INCHI_NUM; iINChI++) {
    for (iTaut = TAUT_NON; iTaut < TAUT_NUM; iTaut++) {
      for (iComp = 0; iComp < OneInput->nNumComponents[iINChI][iTaut];
           iComp++) {
        pINChI = &OneInput->pInpInChI[iINChI][iTaut][iComp];
        if (!pINChI->nNumberOfAtoms || pINChI->bDeleted ||
            !pINChI->szHillFormula || !pINChI->szHillFormula[0]) {
          continue;
        }
...
int ReadWriteInChI(INCHI_IOSTREAM *pInp, INCHI_IOSTREAM *pOut,
                   INCHI_IOSTREAM *pLog, INPUT_PARMS *ip_inp,
                   STRUCT_DATA *sd_inp,
                   /* the following are InChI library-specific parameters */
                   inp_ATOM **at, int *num_at, char *szMsg, int nMsgLen,
  ...
  */
  ...
  int nModeFlagsStereo = 0,
      bTautFlags =
          0; /* InChI creation flags modifications derived from current InChI */
  MODE_PIXH nModeProtonIsoExchgH[INCHI_NUM];
  ...
  const int bInChI2Structure =
      0 != (ip_inp->bReadInChIOptions & READ_INCHI_TO_STRUCTURE);
  const int bInChI2InChI =
      0 != (ip_inp->bReadInChIOptions & READ_INCHI_OUTPUT_INCHI);
#else
  const int bInChI2Structure =
      0 != (ip_inp->bReadInChIOptions & READ_INCHI_TO_STRUCTURE);
  const int bInChI2InChI =
  ...
  while (nReadStatus != RI_ERR_EOF) {
    ...
    if (bInChI2Structure) {
      ...
      bReqNonTaut = 1; /* bReqNonTaut=0 ignores Fixed-H layer in input InChI,
                          for testing only */
#endif
      /* bReqRecmet  = 1; */
      bReqStereo = 1;
      ...
      ip->nMode |= (REQ_MODE_BASIC | REQ_MODE_TAUT | REQ_MODE_STEREO |
                    REQ_MODE_ISO_STEREO | REQ_MODE_ISO);
    }
    ...
    nReadStatus = InChILine2Data(
        pInp, pLine, &strHdr, &pState, &nErr, OneInput.pInpInChI,
        OneInput.nNumComponents, OneInput.nNumProtons, OneInput.s, bReadCoord,
        bInChI2Structure, ip_inp->nMode, &bInputInStdFormat, &bInputHasSaveOpt,
        &inp_save_opt_bits);
    ...
    if ((nReadStatus == RI_ERR_EOL || nReadStatus == RI_ERR_EOF) && !nErr &&
        ...
        if (!bInputInStdFormat) /* Input InChI is a non-standard one  */
        {
          ...
          if (szCurHdr && szCurHdr[0])
            inchi_ios_eprint(pLog,
                             "Warning: forced conversion to non-standard InChI "
                             "for non-std input, %s\n",
                             szCurHdr);
          else
            inchi_ios_eprint(pLog,
                             "Warning: forced conversion to non-standard InChI "
                             "for non-std input, Structure %ld\n",
                             num_inp);
        }
      }
      ...
      if (ip->bINChIOutputOptions & INCHI_OUT_SAVEOPT) {
        if (!bInputHasSaveOpt) {
          ...
          if (szCurHdr && szCurHdr[0])
            ...
            inchi_ios_eprint(pLog,
                             "Warning: ignore SaveOpt request for SaveOpt-less "
                             "input, Structure %ld\n",
                             num_inp);
        } else {
          ...
          if (0 != (ip->bTautFlags & TG_FLAG_RECONNECT_COORD)) {
            ...
            if (0 != (inp_save_opt_bits & SAVE_OPT_RECMET)) {
              ...
              if (szCurHdr && szCurHdr[0])
                inchi_ios_eprint(pLog,
                                 "Warning: input created w/o RecMet - ignoring "
                                 "RecMet request, %s\n",
                                 szCurHdr);
              else
                inchi_ios_eprint(pLog,
                                 "Warning: input created w/o RecMet - ignoring "
                                 "RecMet request, Structure %ld\n",
                                 num_inp);
            }
          }
          if (0 != (ip->nMode & REQ_MODE_BASIC)) {
            ...
            if (0 != (inp_save_opt_bits & SAVE_OPT_FIXEDH))
              ...
              if (szCurHdr && szCurHdr[0])
                inchi_ios_eprint(pLog,
                                 "Warning: input created w/o FixedH - ignoring "
                                 "FixedH request, %s\n",
                                 szCurHdr);
              else
                inchi_ios_eprint(pLog,
                                 "Warning: input created w/o FixedH - ignoring "
                                 "FixedH request, Structure %ld\n",
                                 num_inp);
            }
          }
      ...
      */
      ...
      if (!ip->bNoStructLabels &&
        ...
        /* Added 2nd item: Do not output this extra line into the output SDfile.
         * 2008-11-17 DCh */
        if (strHdr && strstr(strHdr, "Structure:")) {
          ...
          /*
          inchi_ios_print(pOut,  "Structure %ld (%s)%s", num_inp, strHdr? strHdr
          : "", pTAB); sprintf(szMessage, "Structure %ld (%s)%s", num_inp,
          strHdr? strHdr : "" , pTAB);
          */
          inchi_ios_print(pOut, "Structure: %ld. (%s)%s", num_inp,
                          strHdr ? strHdr : "No struct name",
                          pTAB); /* output header */
#if (FIX_DALKE_BUGS == 1)
#else
          sprintf(szMessage, "Structure: %ld. (%s)%s", num_inp,
                  strHdr ? strHdr : "No struct name", pTAB);
#endif
        }
        if (strHdr && strHdr[0]) {
      ...
      if (szCurHdr && ip && ip->first_struct_number > 0) {
        ...
        if (pStrNum) {
          pStrNum += sizeof(szStruct) -
                     1; /* -1 takes care of the string terminal zero */
          ...
          if (cur_struct_number > 0 &&
              cur_struct_number < ip->first_struct_number) {
      ...
      ret = SetProtonsAndXchgIsoH(bInChI2Structure, bReqSplitOutputInChI,
                                  bReqProtonsForEachComponent, bReqNonTaut,
                                  bReqStereo, num_components,
                                  nModeProtonIsoExchgH, &OneInput);
      INCHI_HEAPCHK
      if (ret < 0) {
        num_errors++;
        goto exit_error;
      ...
      if (bInChI2Structure && !bInChI2InChI) {
        ...
        int bINChIOutputOptions =
            ...
            (INCHI_OUT_NO_AUX_INFO |    /* do not output Aux Info */
             ...
             /* INCHI_OUT_EMBED_REC             |*/ /* embed reconnected INChI
                                                       into disconnected INChI
                                                     */
             INCHI_OUT_SDFILE_ONLY | /* save input data in a Molfile instead of
                                        creating INChI */
             ...
             /* INCHI_OUT_WINCHI_WINDOW         |*/ /* output into wINChI text
                                                       window */
        ...
        /* eliminate Fixed-H InChI that are exactly same as the corresponding
         * Mobile-H structures */
        RemoveFixHInChIIdentical2MobH(&OneInput);

        /*-- recheck layers after the elimination; get optional stereo flags
         * --*/
        ret = DetectInpInchiCreationOptions(&OneInput, &bHasSomeReconnected,
        ...
        ip->nMode &=
            ~(REQ_MODE_STEREO | REQ_MODE_ISO_STEREO | REQ_MODE_RELATIVE_STEREO |
              REQ_MODE_RACEMIC_STEREO | REQ_MODE_CHIR_FLG_STEREO |
        ...
        /* mark Disconnected InChI components that are exactly came as
         * Reconnected ones */
        ...
        /*****************************************************************************/
        /* pay attention to: */
        /* 1) .nLink < 0 in Disonnected which means InChI is same as in
         * Reconnected  */
        /*    The component in Reconnected has .nLink pointing to the
         * Disconnected;  */
        /*    each .nLink = (1+component index) or -(1+component index) */
        /*    In the future .nLink>0 in Disconnected shall point to the
         * Reconnectrd  */
        /*    component from which it was created */
        /* 2) Currently reversed structures from Disconnected components are
         * created */
        /*    and abandoned if Reconnected layer exists */
        /* 3) Connect/disconnect H depends on the presence of atom/bond parity
         */
        /*    The combined Mobile/Fixed-H parity should be set for Fixed-H
         * components*/
        /* 4) No comparison of the Disconnected layer is done if Reconnected
         * exists  */
        /* 5) Reading InChI was not fully tested in case one component has
         * stereo in */
        /*    both Mobile-H and Fixed-H layers while another component has
         * stereo    */
        ...
        /* pStruct[iRec][iMobH][iComponent].RevInChI full InChI for the
         * structure */
        /* In case of both Fixed-H and Mobile-H layers the results are in
         * iMobH=0 */
        /* In case of only Mobile-H/Main layer the results are in iMobH=1 */
        ulProcessingTime += InchiTimeElapsed(&ulTStart);

        sd->ulStructTime = 0;
        ret = AllInchiToStructure(ip, sd, num_inp, szCurHdr, &srm,
                                  bHasSomeFixedH, pStruct, &OneInput);
        ...
        if (!OneInput.nNumProtons[INCHI_BAS][TAUT_YES].pNumProtons &&
          ...
          ret = AddProtonAndIsoHBalanceToMobHStruct(
              ip, sd, num_inp, bHasSomeFixedH, szCurHdr, pStruct, &OneInput);
          if (ret < 0) {
            AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
        ...
        ret = CompareAllOrigInchiToRevInChI(pStruct, &OneInput, bHasSomeFixedH,
                                            num_inp, szCurHdr);
        if (ret < 0) {
          AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
                    "InChI compare error", "; ");
        ...
        ret = CompareAllDisconnectedOrigInchiToRevInChI(
            pStruct, &OneInput, bHasSomeFixedH, num_inp, szCurHdr);
        if (ret < 0) {
          AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
        ...
        if (WarningFlags) {
          for (i = 0; i < 2; i++) {
            for (j = 0; j < TAUT_NUM; j++) {
              WarningFlags[i][j] =
                  (unsigned long)OneInput.CompareInchiFlags[i][j];
            }
          }
        }
        ...
        ret = DisplayStructureComponents(ip, sd, num_inp, szCurHdr, &srm,
                                         bReqNonTaut, pStruct, &OneInput);
        if (ret < 0) {
          AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
...
#endif
        InchiTimeGet(&ulTStart);
        ret = MergeStructureComponents(ip, sd, num_inp, szCurHdr, &srm,
                                       bReqNonTaut, pStruct, &OneInput);
        ulProcessingTime += InchiTimeElapsed(&ulTStart);
        if (ret < 0) {
          AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
        ...
        /*------------- for debug only -------------------
                        ...
                        ret = OutputInChIOutOfStrFromINChI(ip, sd, num_inp, 0,
                                                           pOut, pLog,
        &OneInput, save_opt_bits); ulProcessingTime +=
        InchiTimeElapsed(&ulTStart); if ( ret < 0 ) { AddOneMsg(szMessage,
        (int)strlen(szMessage), nMessageLen, "Restored structure to InChI
        conversion failed", "; "); goto dealloc;
                        }
        -------------------------------------------------*/
        if (at && num_at) {
          *at = OneInput.atom;
        ...
        if (ret < 0) {
          AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
                    "Restored structure to InChI conversion error", "; ");
          num_errors++;
          goto dealloc;
        }
#endif
        ...
        if (szMessage) {
          int len;
          InchiTimeGet(&ulTStart);
          FillOutCompareMessage(szMessage, nMessageLen,
                                OneInput.CompareInchiFlags[0]);
          if (OneInput.CompareInchiFlags[1][0] ||
              OneInput.CompareInchiFlags[1][1]) {
            AddOneMsg(szMessage, (int)strlen(szMessage), nMessageLen,
                      "Disconnected: ", "; ");
            FillOutCompareMessage(szMessage, nMessageLen,
                                  OneInput.CompareInchiFlags[1]);
          }
          /* add a metal warning */
          if (bHasMetal && nInitLenMessage < (len = (int)strlen(szMessage))) {
            ...
            memcpy(szMessage + nInitLenMessage, szMetal,
                   sizeof(szMetal) - sizeof(szMessage[0]));
      ...
      dealloc:
        if (ret) {
          if (ret > 0) {
            ...
            for (iRec = 0; iRec < INCHI_NUM; iRec++) {
              for (iMob = bHasSomeFixedH ? TAUT_NON : TAUT_YES; iMob < TAUT_NUM;
                   iMob++) {
                ...
                for (iComp = 0; iComp < nComp; iComp++) {
                  if (pStruct[iRec][iMob][iComp].nError) {
                    char *szFormula =
                        OneInput.pInpInChI[iRec][iMob][iComp].szHillFormula;
                    sprintf(szTemp,
#if (FIX_DALKE_BUGS == 1)
                            " %s%s%d(%.96s)",
#else
        ...
        /* However, do not calculat/write it if this function is called from
         * within dll */
        {
          ...
          /*^^^ post-1.02b addition - correctly treat tabbed output with
           * InChIKey */
          ...
          if (ip->bCalcInChIHash != INCHIHASH_NONE) {
            ...
            if (NULL != buf) {
              ...
              if ((ip->bCalcInChIHash == INCHIHASH_KEY_XTRA2) ||
                  (ip->bCalcInChIHash == INCHIHASH_KEY_XTRA1_XTRA2))
                xhash2 = 1;
              ik_ret = GetINCHIKeyFromINCHI(buf, xhash1, xhash2, ik_string,
                                            szXtra1, szXtra2);
              inchi_free(buf);
            } else
            ...
            if (ik_ret == INCHIKEY_OK) {
              ...
              inchi_ios_print(pLog,
                              "Warning (Could not compute InChIKey: ", num_inp);
              switch (ik_ret) {
              ...
              case INCHIKEY_NOT_ENOUGH_MEMORY:
                ...
                break;
              default:
                inchi_ios_print(pLog, "internal program error)");
                break;
              }
        ...
        if (ret < 0) {
          if (szCurHdr && szCurHdr[0]) {
            ...
            inchi_ios_eprint(pLog,
                             "Error %d creating InChI string, Structure %ld\n",
                             ret, num_inp);
          }
          num_errors++;
        }
#if (!defined(TARGET_API_LIB) && !defined(TARGET_EXE_STANDALONE))
        else if (szCurHdr && szCurHdr[0]) {
        ...
        inchi_ios_eprint(pLog,
                         "\nWrong command line options: expected Inch2Struct "
                         "or Inchi2Inchi\n",
                         num_inp);
        break;
      }
...

    {
      ...
      if (nReadStatus == RI_ERR_EOF && nErr == 0 && pState == 0 && !strHdr) {
        ...
        inchi_ios_eprint(pLog, "\n%s %s (%d) in %s (%d)\n",
                         ...
                         getInchiStateReadErr(pState), pState);
        num_errors++;
        num_processed++;
      }
      if (strHdr) {
...
int OutputInChIAsRequested(INCHI_IOSTREAM *pOut, INCHI_IOSTREAM *pLog,
  ...
  *sd = *sd_inp;
  ...
  if (num_components[INCHI_BAS]) {
    MYREALLOC2(PINChI2, PINChI_Aux2, pINChI[INCHI_BAS], pINChI_Aux[INCHI_BAS],
               num_components[INCHI_BAS], num_components[INCHI_BAS], k1);
  }
  if (num_components[INCHI_REC]) {
    MYREALLOC2(PINChI2, PINChI_Aux2, pINChI[INCHI_REC], pINChI_Aux[INCHI_REC],
               num_components[INCHI_REC], num_components[INCHI_REC], k2);
  }
  ...
  if (num_components[INCHI_REC] && (ip->bTautFlags & TG_FLAG_RECONNECT_COORD) &&
      (ip->bTautFlags & TG_FLAG_DISCONNECT_COORD)) {
    sd->bTautFlagsDone[0] |= TG_FLAG_DISCONNECT_COORD_DONE;
    bHasSomeReconnected = 1;
  }
    ...
    for (j = 0; j < TAUT_NUM; j++) {
      if (bReqNonTaut || (j != TAUT_NON && OneInput->pInpInChI[iINChI][j])) {
        for (k = 0; k < num_components[iINChI]; k++) {
          ...
          if (!(pINChI[iINChI][k][j] =
                    (INChI *)inchi_calloc(1, sizeof(INChI)))) {
            ret2 = RI_ERR_ALLOC;
            goto exit_error;
          }
          if (!(pINChI_Aux[iINChI][k][j] =
                    (INChI_Aux *)inchi_calloc(1, sizeof(INChI_Aux)))) {
            ret2 = RI_ERR_ALLOC;
            goto exit_error;
          }
          /* copy InChI & AuxInfo */
          if (k < OneInput->nNumComponents[iINChI][j]) {
            ...
            *pINChI[iINChI][k][j] = OneInput->pInpInChI[iINChI][j][k];
            memset(&OneInput->pInpInChI[iINChI][j][k], 0,
                   sizeof(OneInput->pInpInChI[iINChI][j][k]));
            INCHI_HEAPCHK
            /* take care of protons in AuxInfo */

            if (nModeProtonIsoExchgH[iINChI] == MODE_PIXH_ADD_TO_EACH &&
                j == TAUT_YES) {
              pINChI_Aux[iINChI][k][j]->nNumRemovedProtons =
                  OneInput->nNumProtons[iINChI][j]
                      .pNumProtons[k]
                      .nNumRemovedProtons;
              for (k1 = 0; k1 < NUM_H_ISOTOPES; k1++) {
                pINChI_Aux[iINChI][k][j]->nNumRemovedIsotopicH[k1] =
                    OneInput->nNumProtons[iINChI][j]
                        .pNumProtons[k]
                        .nNumRemovedIsotopicH[k1];
              }
              INCHI_HEAPCHK
            } else if ((!k && nModeProtonIsoExchgH[iINChI] ==
                                  MODE_PIXH_ADD_TO_FIRST) ||
                       (k + 1 == OneInput->nNumComponents[iINChI][j] &&
                        nModeProtonIsoExchgH[iINChI] ==
                            MODE_PIXH_ADD_A_PIXH_COMPONENT)) {
              /* add protons and exchangeable isotopic H to the first
               * component's AuxInfo */
              pINChI_Aux[iINChI][k][j]->nNumRemovedProtons =
              ...
              pINChI_Aux[iINChI][k][j]->bDeleted =
                  pINChI[iINChI][k][j]->bDeleted;
            }

            if (j == TAUT_YES && pINChI[iINChI][k][j] &&
                pINChI[iINChI][k][j]->nNumberOfAtoms &&
              ...
              pINChI[iINChI][k][j]->nNum_H_fixed = (S_CHAR *)inchi_calloc(
                  pINChI[iINChI][k][j]->nNumberOfAtoms + 1,
                  sizeof(pINChI[0][0][0]->nNum_H_fixed[0]));
            }

            if (j == TAUT_YES &&
                k < OneInput->nNumComponents[iINChI][TAUT_NON] &&
                pINChI[iINChI][k][j] && pINChI[iINChI][k][j]->nNumberOfAtoms &&
                pINChI[iINChI][k][TAUT_NON] &&
                pINChI[iINChI][k][TAUT_NON]->nNumberOfAtoms &&
                !CompareReversedINChI(pINChI[iINChI][k][j],
                                      pINChI[iINChI][k][TAUT_NON], NULL,
                                      NULL)) {
              pINChI[iINChI][k][TAUT_NON]->nNumberOfAtoms =
                  0; /* eliminate non-taut equal to taut */
            }
  ...
  for (k1 = 0, k2 = (bReqSplitOutputInChI ? k1 + 1 : nNumOutputComponents);
    ...
    if (bReqSplitOutputInChI) {
      ...
      /* additional data */
      sd->num_non_taut[INCHI_BAS] = sd->num_taut[INCHI_BAS] =
          sd->num_non_taut[INCHI_REC] = sd->num_taut[INCHI_REC] = 0;
      iINChI = iINChI1;
      for (j = 0; j < TAUT_NUM && sd->num_components[iINChI]; j++) {
        for (k = k1; k < k2; k++) {
          /*  find where the current processed structure is located */
          int cur_is_in_non_taut =
              (pINChI[iINChI][k][TAUT_NON] &&
               pINChI[iINChI][k][TAUT_NON]->nNumberOfAtoms > 0);
          int cur_is_in_taut =
              (pINChI[iINChI][k][TAUT_YES] &&
               pINChI[iINChI][k][TAUT_YES]->nNumberOfAtoms > 0);
          int cur_is_non_taut =
              (cur_is_in_non_taut &&
               0 == pINChI[iINChI][k][TAUT_NON]->lenTautomer) ||
      ...
      sd->num_components[INCHI_BAS] =
          inchi_max(OneInput->nNumComponents[INCHI_BAS][TAUT_YES],
                    OneInput->nNumComponents[INCHI_BAS][TAUT_NON]);
      sd->num_components[INCHI_REC] =
          inchi_max(OneInput->nNumComponents[INCHI_REC][TAUT_YES],
                    OneInput->nNumComponents[INCHI_REC][TAUT_NON]);

      /* additional data needed for SortAndPrintINChI() */
      for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
        sd->num_non_taut[iINChI] = sd->num_taut[iINChI] = 0;
        for (j = 0; j < TAUT_NUM && sd->num_components[iINChI]; j++) {
          for (k = k1; k < k2; k++) {
            /*  find where the current processed structure is located */
            int cur_is_in_non_taut =
            ...
            int cur_is_non_taut =
                (cur_is_in_non_taut &&
                 0 == pINChI[iINChI][k][TAUT_NON]->lenTautomer) ||
                (cur_is_in_taut &&
                 0 == pINChI[iINChI][k][TAUT_YES]->lenTautomer);
            int cur_is_taut =
                cur_is_in_taut && 0 < pINChI[iINChI][k][TAUT_YES]->lenTautomer;
            if (cur_is_non_taut + cur_is_taut) {
              /*  count tautomeric and non-tautomeric components of the
               * structures */
              /*
              int j1 = cur_is_in_non_taut? TAUT_NON:TAUT_YES;
              int j2 = cur_is_in_taut?     TAUT_YES:TAUT_NON;
    ...
    if (bReqSplitOutputInChI) {
      /* output components one by one (for splitting input InChI into
       * components) */
      ...
      INCHI_HEAPCHK
      nRet1 = SortAndPrintINChI(
          pOut, pStr, nStrLen, pLog, ip, NULL /*orig_inp_data*/,
          NULL /*prep_inp_data*/, NULL /*composite_norm_data*/,
          NULL /*pOrigStruct*/, sd->num_components, sd->num_non_taut,
          sd->num_taut, sd->bTautFlags, sd->bTautFlagsDone, pncFlags, num_inp,
          pInChI_2, pInChI_Aux_2, &bSortPrintINChIFlags, save_opt_bits);
      INCHI_HEAPCHK
    } else {
      INCHI_HEAPCHK
      bSortPrintINChIFlags = 0;
      nRet1 = SortAndPrintINChI(
          ...
          NULL /*pOrigStruct*/, sd->num_components, sd->num_non_taut,
          sd->num_taut, sd->bTautFlags, sd->bTautFlagsDone, pncFlags, num_inp,
          pINChI, pINChI_Aux, &bSortPrintINChIFlags, save_opt_bits);
      INCHI_HEAPCHK
    }
...
int GetNumNeighborsFromInchi(INChI *pInChI, AT_NUMB nAtNumber) {
  int i, j, n_vertex, n_neigh, nNumNeigh, bTautAtom, nNumH, nTotNumNeigh,
      num_atoms;
  ...
  /* Connection table structure:
     Vert(1) [, Neigh(11), Neigh(12),...], Vert(2) [, Neigh(2,1),
     Neigh(2,2),...] ... where Neigh(i,1) < Neigh(i,2) <... < Vert(i); Vert(i) <
     Vert(i+1)
   */
  for (i = 1, n_vertex = pInChI->nConnTable[0] - 1; i < pInChI->lenConnTable;
       i++) {
  ...
  if (pInChI && pInChI->lenTautomer > 1 && pInChI->nTautomer &&
      pInChI->nTautomer[0] > 0) {
    int itg, len_tg;
    int tot_len_tg = pInChI->lenTautomer -
                     T_GROUP_HDR_LEN * pInChI->nTautomer[0] -
                     1; /* number of endpoints */
    j = 1;              /* index in pInChI->nTautomer[] */
    i = 0;              /* index in ti->nEndpointAtomNumber[] */
    for (itg = 0; itg < pInChI->nTautomer[0]; itg++) {
      len_tg = pInChI->nTautomer[j]; /* t-group length not including
                                        pInChI->nTautomer[j] */
      j += T_GROUP_HDR_LEN;          /* skip t-group header */
      len_tg -= T_GROUP_HDR_LEN - 1;
      for (; 0 < len_tg--; j++, i++) {
        taut_at_number =
            pInChI->nTautomer[j] - 1; /* Mobile-H group atom number */
  ...
  /* conclusion: if not tautomeric then return positive number, otherwise add
   * 1000 */
...
int CountStereoTypes(INChI *pInChI, int *num_known_SB, int *num_known_SC,
                     int *num_unk_und_SB, int *num_unk_und_SC, int *num_SC_PIII,
                     int *num_SC_AsIII) {
  ...
  for (i = 0; i < Stereo->nNumberOfStereoCenters; i++) {
    if (!(nAtNumber = Stereo->nNumber[i]) ||
        nAtNumber > pInChI->nNumberOfAtoms) {
...
/**************************************************************************************/
int bInpInchiComponentExists(InpInChI *pOneInput, int iInChI, int bMobileH,
                             int k) {
...
/**************************************************************************************/
int bInpInchiComponentDeleted(InpInChI *pOneInput, int iInChI, int bMobileH,
                              int k) {
  if ((INCHI_BAS != iInChI && iInChI != INCHI_REC) ||
      (TAUT_NON != bMobileH && TAUT_YES != bMobileH) || k < 0) {
    return 0;
...
/**************************************************************************************/
int bRevInchiComponentExists(StrFromINChI *pStruct, int iInChI, int bMobileH,
                             int k) {
...
/**************************************************************************************/
int bRevInchiComponentDeleted(StrFromINChI *pStruct, int iInChI, int bMobileH,
                              int k) {
  if (!pStruct || /*!pStruct->at2 ||*/ !pStruct->num_atoms ||
      (INCHI_BAS != iInChI && iInChI != INCHI_REC) ||
      (TAUT_NON != bMobileH && TAUT_YES != bMobileH) || k < 0) {
...
/**************************************************************************************/
int DetectInpInchiCreationOptions(InpInChI *pOneInput, int *bHasReconnected,
                                  int *bHasMetal, int *bHasFixedH,
  ...
  *bHasReconnected = *bHasFixedH = *nModeFlagsStereo = *bTautFlagsStereo = 0;
  ...
  num_known_SB = num_known_SC = num_unk_und_SB = num_unk_und_SC = num_SC_PIII =
      num_SC_AsIII = 0;
  *bHasMetal = 0;
  ...
  for (iInChI = 0; iInChI < INCHI_NUM; iInChI++) {
    for (iMobileH = 0; iMobileH < TAUT_NUM; iMobileH++) {
      ...
      max_components = pOneInput->pInpInChI[iInChI][iMobileH]
                           ? pOneInput->nNumComponents[iInChI][iMobileH]
                           : 0;

      for (k = num_components = 0; k < max_components; k++) {
        pInChI = pOneInput->pInpInChI[iInChI][iMobileH] + k;
        ret = CountStereoTypes(pInChI, &num_known_SB, &num_known_SC,
                               &num_unk_und_SB, &num_unk_und_SC, &num_SC_PIII,
                               &num_SC_AsIII);
        if (ret < 0) {
          return ret; /* error */
        }
  ...
  if ((nModeFlagsValue & REQ_MODE_RELATIVE_STEREO) &&
      (nModeFlagsValue & REQ_MODE_RACEMIC_STEREO)) {
    return RI_ERR_SYNTAX;
  }
  if (bHasStereo &&
      !nModeFlagsValue) /* REQ_MODE_SB_IGN_ALL_UU | REQ_MODE_SC_IGN_ALL_UU*/
  {
  ...
  if (!num_known_SB && num_unk_und_SB) {
    ...
    nModeFlagsValue |= REQ_MODE_SB_IGN_ALL_UU; /* ignore Unknown/Undefind SB if
                                                  no well-defined SB exist */
  }

  if (!num_known_SC && num_unk_und_SC) {
    ; /* full SUU option or SB part of it */
  } else {
    nModeFlagsValue |= REQ_MODE_SC_IGN_ALL_UU; /* ignore Unknown/Undefind SC if
                                                  no well-defined SB exist */
  }
  ...
  if (pInChI && !pInChI->bDeleted && pInChI->nNumberOfAtoms && pInChI->nAtom) {
    for (i = 0; i < pInChI->nNumberOfAtoms; i++) {
      if (is_el_a_metal((int)pInChI->nAtom[i])) {
        if (pInChI->nNumberOfAtoms > 1 ||
            (pInChI->nNum_H && pInChI->nNum_H[0])) {
          return 1;
        }
      }
    }
...
/*****************************************************************************************/
int SetProtonsAndXchgIsoH(int bInChI2Structure, int bReqSplitOutputInChI,
                          int bReqProtonsForEachComponent, int bReqNonTaut,
                          int bReqStereo, int num_components[INCHI_NUM],
                          MODE_PIXH nModeProtonIsoExchgH[INCHI_NUM],
                          InpInChI *OneInput) {
  ...
  for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
    ...
    bAvailableProtonsTotal =
        0 != OneInput->nNumProtons[iINChI][TAUT_YES].nNumRemovedProtons;
    for (k1 = 0; k1 < NUM_H_ISOTOPES; k1++) {
      bAvailableProtonsTotal |=
          0 != OneInput->nNumProtons[iINChI][TAUT_YES].nNumRemovedIsotopicH[k1];
    }
    /* are /p and/or /i/h available for each component ? */
    bAvailableProtonsForEachComponent =
        (NULL != OneInput->nNumProtons[iINChI][TAUT_YES].pNumProtons);
    /* decision: add /p to each component, add total to the 1st, add total as
     * one more component */
    ...
    if ((nModeProtonIsoExchgH[iINChI] == MODE_PIXH_ADD_A_PIXH_COMPONENT &&
      ...
      if (0 > (ret2 = nFillOutProtonMobileH(OneInput->pInpInChI[iINChI][j] +
                                            nLen))) {
        goto exit_error;
      }
    }
  ...
  num_components[INCHI_BAS] =
      inchi_max(OneInput->nNumComponents[INCHI_BAS][TAUT_YES],
                OneInput->nNumComponents[INCHI_BAS][TAUT_NON]);
  num_components[INCHI_REC] =
      inchi_max(OneInput->nNumComponents[INCHI_REC][TAUT_YES],
                OneInput->nNumComponents[INCHI_REC][TAUT_NON]);
...
/******************************************************************************************/
int GetInChIFormulaNumH(
    INChI *pInChI,
    int *nNumH) { /* get number of H including bridging hydrogen atoms */
  const char *p, *q;
  *nNumH = 0;
...
int GetInChINumH(INChI *pInChI, int *nNumH) {
  ...
  */
  if (pInChI->lenTautomer > 3 && pInChI->nTautomer) {
    ...
    for (iTautGroup = 0; j < lenTautomer && iTautGroup < nNumTautGroups;
         iTautGroup++, j += nTautGroupLen) {
...
int GetInChIIsoH(INChI *pInChI, int nNumIsotopicH[NUM_H_ISOTOPES]) {
  ...
  for (i = 0; i < pInChI->nNumberOfIsotopicAtoms; i++) {
    if (pInChI->IsotopicAtom[i].nIsoDifference > 0 &&
      ...
      if (!pInChI->nAtom || !pInChI->IsotopicAtom[i].nAtomNumber ||
          pInChI->IsotopicAtom[i].nAtomNumber > pInChI->nNumberOfAtoms) {
        return RI_ERR_PROGR;
      }
      if (pInChI->nAtom[pInChI->IsotopicAtom[i].nAtomNumber - 1] ==
          EL_NUMBER_H) {
...
int InChILine2Data(INCHI_IOSTREAM *pInp, SEGM_LINE *pLine, char **pStr,
  ...
  ret = ReadInChILine(pInp, pLine, pStr, pState, pInpInChI, nNumComponents,
                      nNumProtons, s, bStdFormat, bInputHasSaveOpt,
                      inp_save_opt_bits);
    ...
    *pState = IST_MATERIAL_BALANCE_ERROR;
    for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
      ...
      for (j = 0; j < TAUT_NUM; j++) {
        ...
        int nMH2FH_AltInv = 0,
            nFH2iFH_AltInv = 0 /*, niMH2iFH_AltInv=0, nMH2iMH_AltInv=0*/;
        int jAlt = ALT_TAUT(j);
        INCHI_MODE nFlags = 0, nFlagsAlt = 0;
        /* get stereo type: ABS, REL, RAC, or nothing */
        tot_charge[iINChI][j] = 0;
        for (bIsotopic = bStereoType = bStereoTypeAlt = 0; bIsotopic < 2;
             bIsotopic++) {
          ...
          nFlags = bStereoType == 2   ? INCHI_FLAG_REL_STEREO
                   : bStereoType == 3 ? INCHI_FLAG_RAC_STEREO
                                      : 0;
          nFlagsAlt = bStereoTypeAlt == 2   ? INCHI_FLAG_REL_STEREO
                      : bStereoTypeAlt == 3 ? INCHI_FLAG_RAC_STEREO
                                            : 0;
        }
        /* set stereo type to each component */
        /* add missing nNum_H and nConnTable */
        if (nNumComponents[iINChI][j]) {
          num_elem[iINChI][j] = (NUM_ELEM *)inchi_calloc(
              nElDataLen + 1, sizeof(num_elem[0][0][0]));
        ...
        for (k = 0; k < nNumComponents[iINChI][j]; k++) {
          ...
          if (pInpInChI[iINChI][j]) {
            ...
            if (!pInChI->nNum_H &&
                !(pInChI->nNum_H = (S_CHAR *)inchi_calloc(
                      pInChI->nNumberOfAtoms + 1, sizeof(pInChI->nNum_H[0])))) {
              ret2 = RI_ERR_ALLOC;
              goto exit_function;
            }
            ...
            if (!pInChI->nConnTable) {
              ...
              if (j == TAUT_NON && k < nNumComponents[iINChI][TAUT_YES] &&
                ...
                if (!(pInChI->nConnTable = (AT_NUMB *)inchi_calloc(
                          lenCT + 1, sizeof(pInChI->nConnTable[0])))) {
                  ret2 = RI_ERR_ALLOC;
                  goto exit_function;
                }
                memcpy(pInChI->nConnTable, pCT,
                       lenCT * sizeof(pInChI->nConnTable[0]));
            ...
            } else if (pInChI->nConnTable && !pInChI->lenConnTable &&
                       pInChI->nNumberOfAtoms == 1) {
              pInChI->nConnTable[0] = 1;
              pInChI->lenConnTable = 1;
            }
            ...
            if (j == TAUT_NON) {
              ...
              */
              if (!pInChI->nTotalCharge &&
                  k < nNumComponents[iINChI][TAUT_YES]) {
                INChI *pAltInChI =
                    &pInpInChI[iINChI][TAUT_YES][k]; /* Mobile H InChI */
                if (pAltInChI->nTotalCharge &&
                    pAltInChI->nTotalCharge != NO_VALUE_INT) {
            ...
            if (j == TAUT_YES && pInChI->nNumberOfIsotopicAtoms &&
              ...
              INChI *pAltInChI =
                  &pInpInChI[iINChI][TAUT_NON][k]; /* Fixed H InChI */
            ...
            if (j == TAUT_YES && pInChI->IsotopicTGroup &&
              ...
              if (!pAltInChI->IsotopicTGroup) {
                XYZ_COORD *pxyz = (XYZ_COORD *)inchi_calloc(
                    pInChI->nNumberOfAtoms, sizeof(pxyz[0]));
                if (pxyz) {
                  memcpy(pxyz, pInChI->IsotopicTGroup,
                         pInChI->nNumberOfAtoms * sizeof(pxyz[0]));
            ...
            if (j == TAUT_YES) {
              ...
              if (bIso) {
                if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                    (!pInChI->StereoIsotopic ||
                     !pInChI->StereoIsotopic->t_parity)) {
                  if (0 > (ret2 = CopySegment(pInChI, pInChI, CPY_SP3, 1, 0)) ||
                      ((!pInChI->StereoIsotopic->nCompInv2Abs ||
                       ...
                       0 > (ret2 = CopySegment(pInChI, pInChI, CPY_SP3_M, 1,
                                               0)))) {
                    goto exit_function;
                  }
                  if ((nFlags &
                  ...
                  /* copy sp3 inversion info: non-isotopic Mobile H => isotopic
                   * Mobile H  */
                  if (pInChI->Stereo &&
                      pInChI->Stereo->nNumberOfStereoCenters &&
                      pInChI->StereoIsotopic &&
              ...
              if (bIso && pInChI->Stereo &&
                  pInChI->Stereo->nNumberOfStereoBonds &&
                  (!pInChI->StereoIsotopic ||
                   !pInChI->StereoIsotopic->b_parity)) {
            ...
            if (j == TAUT_NON) {
              if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                  pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT) {
                /* case of /sN and /t... in non-isotopic Mobile-H, no /s in
                 * non-isotopic Fixed-H */
                if (!s[iINChI][j][0] &&
                    s[iINChI][jAlt][0] >
                        0 && /* /sN is not present in F and is present in M */
                    pInChI_Alt &&
                    pInChI_Alt->Stereo &&
                    pInChI_Alt->Stereo->nNumberOfStereoCenters) {
                  /* inherit from Mobile-H */
                  /* /s1 in M and MI; /m1 or /m0 in MI; /m. in M; no /m in F.
                   * Inherit MI->FI. Added 10-15-2007 */
                  if (pInChI_Alt->Stereo->nCompInv2Abs ==
                          0 && /*  M: /m. ; means no /m for this component */
                      pInChI->Stereo->nCompInv2Abs ==
                          NO_VALUE_INT &&           /*  F: no /m segment for all
                                                       components */
                      pInChI_Alt->StereoIsotopic && /*  MI: present */
                      pInChI_Alt->StereoIsotopic->nCompInv2Abs != 0 &&
                      pInChI_Alt->StereoIsotopic->nCompInv2Abs !=
                          NO_VALUE_INT && /* MI:    /m0 or /m1  */
                      !s[iINChI][j][0] &&
                      !s[iINChI][j][1] && /* F, FI: no /s       */
                      s[iINChI][jAlt][0] == 1 &&
                      s[iINChI][jAlt][1] == 1 /* M, MI: /s1 and /s1 */
                  ) {
                    /* copy /m from MI to FI */
                    if (0 > (ret2 = CopySegment(pInChI, pInChI_Alt, CPY_SP3_M,
                                                1, 1))) {
                      goto exit_function;
                    }
                  } else
                    /* the following if(){...} was added to fix m1 bug
                     * 2007-09-25 DT */
                    if (pInChI_Alt->Stereo->nCompInv2Abs != NO_VALUE_INT &&
                        s[iINChI][jAlt][0] == 1) {
                      pInChI->Stereo->nCompInv2Abs =
                          pInChI_Alt->Stereo->nCompInv2Abs;
                    } else
                      ...
                      if (pInChI_Alt->Stereo->nCompInv2Abs == NO_VALUE_INT &&
                        ...
                        if (bIso) {
                          ...
                          pInChI_Alt->StereoIsotopic->nCompInv2Abs =
                              2; /* MI: /m1 or /m0 */
                          pInChI_Alt->Stereo->nCompInv2Abs =
                              0; /* M:  /m. ; no /m for this component */
                          pInChI->Stereo->nCompInv2Abs =
                              NO_VALUE_INT +
                              1; /* FI: Stereo->CompInv2Abs=0,
                                    StereoIsotopic->CompInv2Abs=1 or -1 */
                        } else {
                          pInChI->Stereo->nCompInv2Abs =
                              2; /* F:  /m1 or /m0, omitted from InChI as a
                                    repetition */
                          pInChI_Alt->Stereo->nCompInv2Abs =
                              2; /* M:  /m1 or /m0; in Srel/SRac case the value
                                    = 2 */
                        }
                      } else {
                        pInChI->Stereo->nCompInv2Abs =
                  ...
                  /* case of /sN in Isotopic Fixed-H only, /t... in Fixed-H, no
                   * /m (2007-08-27 DT) */
                  if (!s[iINChI][j][0] &&
                      !s[iINChI][jAlt][0] && /* /sN in Fixed-H isotopic only */
                      (nFlags &
                       (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO)) &&
                      !(pInChI->StereoIsotopic &&
                        pInChI->StereoIsotopic->nNumberOfStereoCenters) &&
                      /*!(pInChI_Alt && pInChI_Alt->Stereo &&
                         pInChI_Alt->Stereo->nNumberOfStereoCenters) &&*/
                      !(pInChI_Alt && pInChI_Alt->StereoIsotopic &&
                        pInChI_Alt->StereoIsotopic->nNumberOfStereoCenters)) {
                    pInChI->Stereo->nCompInv2Abs =
                        NO_VALUE_INT +
                        1; /* Stereo->CompInv2Abs=0,
                              StereoIsotopic->CompInv2Abs=1 or -1 */
                  } else {
            ...
            if (j == TAUT_NON) {
              ...
              if (bIso) {
                if (pInChI->Stereo && pInChI->Stereo->nNumberOfStereoCenters &&
                  ...
                  ||
                       !(pInChI->StereoIsotopic->nCompInv2Abs || NO_VALUE_INT ==
                  pInChI->StereoIsotopic->nCompInv2Abs) && 0 > (ret2 =
                  CopySegment( pInChI, pInChI, CPY_SP3_M, 1, 0))) { goto
                  exit_function;
                  }
                  */
                  ...
                  if (pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT + 1) {
                    ...
                    pInChI->StereoIsotopic->nCompInv2Abs = 2;
                               ...
                               NO_VALUE_INT ==
                                   pInChI->StereoIsotopic->nCompInv2Abs) &&
                             0 > (ret2 = CopySegment(pInChI, pInChI, CPY_SP3_M,
                                                     1, 0))) {
                    goto exit_function;
                  }
                  ...
                  if ((nFlags &
                    ...
                    /* copied Rel/Rac stereo to Iso; /s is in Iso /s is not in
                     * non-Iso */
                  ...
                  /* copy sp3 inversion info: non-isotopic Fixed H --> isotopic
                   * Fixed H  */
                  if (pInChI->Stereo &&
                      pInChI->Stereo->nNumberOfStereoCenters &&
                      pInChI->StereoIsotopic &&
            ...
            if (j == TAUT_NON && k < nNumComponents[iINChI][TAUT_YES]) {
              ...
              int bNo_InChI_m = (!pInChI->Stereo ||
                                 NO_VALUE_INT == pInChI->Stereo->nCompInv2Abs);

              /* (2) non-isotopic Mobile H => non-isotopic Fixed H */
              if (pAltInChI->Stereo &&
                  pAltInChI->Stereo->nNumberOfStereoCenters &&
                  (!pInChI->Stereo || !pInChI->Stereo->t_parity)) {
#if (FIX_I2I_STEREOCONVERSION_BUG2 == 1)
                /* (2008-04-02)   1=> Fix bug of i2i conversion
                 * SAbs-->(SRel||Srac) */
                /*                    (converter skipped empty '/t' or sometimes
                 * produced an excess one */
...
#endif
                if (0 > (ret2 =
                             CopySegment(pInChI, pAltInChI, CPY_SP3, 0, 0)) ||
                    ((!pInChI->Stereo->nCompInv2Abs ||
                      NO_VALUE_INT == pInChI->Stereo->nCompInv2Abs) &&
                     0 > (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP3_M, 0,
                                             0)))) {
                  goto exit_function;
                }
                ...
                if ((s[iINChI][j][0] < 1) && (bHave_t_stereo < 1) &&
                    (pAltInChI->Stereo->nNumberOfStereoCenters > 0) &&
                    (s[iINChI][jAlt][0] < 1)) {
                  /* (2010-02-28) if not all stereo centers are
                   * unknown/undefined */
                  /*  at which condition stereo still should present .. */
                  int all_UU = 1;
                  for (kc = 0; kc < pAltInChI->Stereo->nNumberOfStereoCenters;
                       kc++) {
                ...
                if ((pInChI->Stereo->nCompInv2Abs == NO_VALUE_INT) &&
                  ...
                  if (s[iINChI][jAlt][0] > 0 && s[iINChI][j][0] > 0) {
                    /* suppose once in a while only non-taut stereo changes if
                     * inverted */
                    pAltInChI->Stereo->nCompInv2Abs =
                        (++nMH2FH_AltInv) % 2 ? 2 : 0;
                    pInChI->Stereo->nCompInv2Abs = 2;
                  } else
                    /* Mobile-H: /t.. /sN; Mobile-H isotopic: /sN (n=2 or 3),
                     * not /t...; Fixed-H layer is present, has no /t, no /i/t
                     */
                    /* Mobile-H /sN was caused by another component that would
                     * have same /mN in all layers */
                    /* therefore, in case of Abs. Stereo, Mobile-H stereo
                     * isotopic stereo would have /m1 */
                    /* In case of Rel/Rac stereo, since no /m1 could occur in
                     * Mobile-H isotopic, */
                    /* no pAltInChI->StereoIsotopic or pInChI->StereoIsotopic
                     * have been created yet. */
                    /* added 10-11-2007 to fix i2i bug for Rel/Rac stereo */
                    if (nNumComponents[iINChI][j] > 1 && bNo_InChI_t &&
                        bNo_InChI_m /* no /t... or /mN in Fixed-H  */ &&
                        !nFlags &&
                        !(pAltInChI->StereoIsotopic &&
                          pAltInChI->StereoIsotopic->t_parity) &&
                        !(pInChI->StereoIsotopic &&
                          pInChI->StereoIsotopic->t_parity) &&
                        ...
                        (s[iINChI][jAlt][1] &
                         ((INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO)))) {
                      if (bIso) {
                        /* create two zero/NULL-initialized isotopic stereo if
                         * they do not exist */
                        if ((!pInChI->StereoIsotopic &&
                             0 > (ret2 = CopySegment(pInChI, pAltInChI,
                                                     CPY_SP3_M, 1, -1)))
                            /* -- the following will be created later, in
                               TAUT_YES part of the code -- */
                            || (!pAltInChI->StereoIsotopic &&
                                0 > (ret2 = CopySegment(pAltInChI, pAltInChI,
                                                        CPY_SP3_M, 1, -1)))) {
                          goto exit_function;
                        }
                        ...
                        pInChI->StereoIsotopic->nCompInv2Abs = 2;
                        pInChI->Stereo->nCompInv2Abs = 0;
                        /* -- the following will NOT be assigned later, in
                         * TAUT_YES part of the code -- */
                    ...
                    } else if ((s[iINChI][jAlt][0] > 0 ||
                                s[iINChI][j][0] > 0) &&
                               s[iINChI][j][0] >= 0)
                      pInChI->Stereo->nCompInv2Abs = 2;
                    else
                      /* Mobile-H: /t..., no /sN; Mobile-H isotopic: /s2 or /s3,
                       * not /t; Fixed-H layer is present, has no /t, no /i/t */
                      /* therefore, in case of Abs. Stereo, Mobile-H stereo
                       * isotopic stereo would have /m1 */
                      /* In case of Rel/Rac stereo, since no /m1 could occur in
                      ...
                      if (bIso && bNo_InChI_t &&
                          ...
                          !(pInChI->StereoIsotopic &&
                            pInChI->StereoIsotopic->t_parity) &&
                          s[iINChI][jAlt][0] == 0 && s[iINChI][j][0] == 0 &&
                          s[iINChI][j][1] == 0 &&
                        ...
                        pInChI->StereoIsotopic->nCompInv2Abs = 2;
                        pInChI->Stereo->nCompInv2Abs = 0;
                        /* -- the following will be assigned later, in TAUT_YES
                         * part of the code -- */
                        /*
                        pAltInChI->StereoIsotopic->nCompInv2Abs = 2;
                        pAltInChI->Stereo->nCompInv2Abs = 0;
                ...
                /* copy sp3 inversion info: non-isotopic Mobile H =>
                 * non-isotopic Fixed H  */
                if (pAltInChI->Stereo &&
                    pAltInChI->Stereo->nNumberOfStereoCenters &&
              ...
              if (pAltInChI->Stereo &&
                  pAltInChI->Stereo->nNumberOfStereoBonds &&
                  (!pInChI->Stereo || !pInChI->Stereo->b_parity)) {
                if (0 >
                    (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP2, 0, 0))) {
                  goto exit_function;
                }
              }
              /* (3) isotopic Mobile H -> isotopic Fixed H */
              /* if !FH_Stereo && !MH_Stereo && MH_IsoStereo!=NULL &&
               * FH_IsoStereo==NULL */
              if (bIso) {
                if (!(pInChI->Stereo &&
                      pInChI->Stereo->t_parity) && /* !FH_Stereo */
                    !(pAltInChI->Stereo &&
                      pAltInChI->Stereo->t_parity) && /* !MH_Stereo */
                    (pAltInChI->StereoIsotopic &&
                     pAltInChI->StereoIsotopic
                         ->nNumberOfStereoCenters) && /*  MH_IsoStereo */
                    (!pInChI->StereoIsotopic ||
                     !pInChI->StereoIsotopic->t_parity)) { /* !FH_IsoStereo */
                  /* copy sp3 iso stereo MI->FI (/t) and, if FH nCompInv2Abs
                   * (/m) is missing, copy it, too, MI->FI */
                  if (0 > (ret2 =
                               CopySegment(pInChI, pAltInChI, CPY_SP3, 1, 1)) ||
                      ((!pInChI->StereoIsotopic->nCompInv2Abs ||
                        NO_VALUE_INT == pInChI->StereoIsotopic->nCompInv2Abs) &&
                       0 > (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP3_M, 1,
                                               1)))) {
                    goto exit_function;
                  }
                  /* in case of missing nCompInv2Abs, Relative or Racemic stereo
                   * 2005-05-10 */
                  if (pInChI->StereoIsotopic->nCompInv2Abs == NO_VALUE_INT &&
                      (nFlagsAlt &
                       (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO))) {
                    pInChI->StereoIsotopic->nCompInv2Abs =
                        s[iINChI][jAlt][1] > 0 ? 2 : 0;
                    if (!(pInChI->nFlags &
                          (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO))) {
                      pInChI->nFlags |= (nFlagsAlt & (INCHI_FLAG_REL_STEREO |
                                                      INCHI_FLAG_RAC_STEREO));
                    }
                  }
                } else
                  /* copy sp3 inversion info only: isotopic Mobile H -> isotopic
                   * Fixed H  */
                  if (!(pInChI->Stereo &&
                        pInChI->Stereo->t_parity) && /* !FH_Stereo    /t */
                      !(pAltInChI->Stereo &&
                        pAltInChI->Stereo->t_parity) && /* !MH_Stereo    /t */
                      (pAltInChI->StereoIsotopic &&
                       pAltInChI->StereoIsotopic
                           ->nNumberOfStereoCenters) && /*  MH_IsoStereo /t */
                      (pInChI->StereoIsotopic &&
                       pInChI->StereoIsotopic
                           ->nNumberOfStereoCenters) && /*  FH_IsoStereo /t */
                      pAltInChI->StereoIsotopic
                          ->nCompInv2Abs && /*  MH_IsoStereo /m */
                      (!pInChI->StereoIsotopic->nCompInv2Abs ||
                       NO_VALUE_INT ==
                           pInChI->StereoIsotopic
                               ->nCompInv2Abs)) { /*  !FH_IsoStereo /m */
                    /* added 02-09-2006 */
                    if (0 > (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP3_M, 1,
                                                1))) {
                      goto exit_function;
                    }
                  }
                ...
                if (!(pInChI->Stereo && pInChI->Stereo->b_parity) &&
                    ...
                    (!pInChI->StereoIsotopic ||
                     !pInChI->StereoIsotopic->b_parity)) {
                  if (0 >
                      (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP2, 1, 1))) {
                    goto exit_function;
                  }
                }
                ...
                if ((pInChI->Stereo &&
                     pInChI->Stereo
                         ->nNumberOfStereoCenters) && /* FH_Stereo     /t */
                    !(pAltInChI->StereoIsotopic &&
                      pAltInChI->StereoIsotopic
                          ->t_parity) && /* !MH_IsoStereo /t */
                    !(pInChI->StereoIsotopic &&
                      pInChI->StereoIsotopic
                          ->t_parity)) { /* !FH_IsoStereo /t */

                  /* added 10-10-2007 DT: copy MH_Iso /m => FH_Iso /m to fix i2i
                   * bug for Abs stereo */
                  /* InChI string contains: MH(/t...), MH_Iso(/mN, no /t), FH(no
                   * /t /m), FH_Iso(no /t /m) */
                  if (pAltInChI->StereoIsotopic &&
                      pAltInChI->StereoIsotopic
                          ->nCompInv2Abs && /* MH_IsoStereo /m */
                      bNo_InChI_t &&
                      NO_VALUE_INT !=
                          pAltInChI->StereoIsotopic
                              ->nCompInv2Abs && /* undef FH_IsoStereo /m */
                      !(pInChI->StereoIsotopic &&
                        NO_VALUE_INT != pInChI->StereoIsotopic->nCompInv2Abs)) {
                    if (0 > (ret2 = CopySegment(pInChI, pAltInChI, CPY_SP3_M, 1,
                                                1))) {
                  ...
                  /* in case of missing nCompInv2Abs, Relative or Racemic
                   * stereo, /sN in Fixed-H, 2005-05-10 */
                  if (pInChI->StereoIsotopic->nCompInv2Abs == NO_VALUE_INT &&
                      (nFlags &
                       (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO))) {
                    ...
                    if (!(pInChI->nFlags &
                          (INCHI_FLAG_REL_STEREO | INCHI_FLAG_RAC_STEREO))) {
                      pInChI->nFlags |= (nFlags & (INCHI_FLAG_REL_STEREO |
                                                   INCHI_FLAG_RAC_STEREO));
                    }
                  }
                  ...
                  if ((pInChI->Stereo && pInChI->Stereo->t_parity) &&
                    ...
                    /* added 05-09-2006 */
                    if (0 >
                        (ret2 = CopySegment(pInChI, pInChI, CPY_SP3_M, 1, 0))) {
                      goto exit_function;
                    }
                  }
              }
      ...
      if (nNumComponents[iINChI][TAUT_NON] > 1) /* if multi-component */
        if (!s[iINChI][TAUT_YES][0] &&
            !s[iINChI][TAUT_YES][1]) /* if no /s in M, MI */
          if ((s[iINChI][TAUT_NON][0] > 1) &&
              (s[iINChI][TAUT_NON][1] > 1)) /* if /srel/srac in both F, FI */
            if (s[iINChI][TAUT_NON][0] ==
                s[iINChI][TAUT_NON][1]) /* if same stereo in F and FI */
    ...
    fini:;
      ...
      if (num_elem[iINChI][TAUT_YES]) {
        tot_charge[iINChI][TAUT_YES] +=
            nNumProtons[iINChI][TAUT_YES].nNumRemovedProtons;
        num_elem[iINChI][TAUT_YES][EL_NUMBER_H].num +=
            nNumProtons[iINChI][TAUT_YES].nNumRemovedProtons;
      }

      /**** Count H and isotopic H in Mobile and Fixed H represntations of
       * components */
      /* if at least one component has Fixed-H layer then all components have
       * Fixed-H */
      /* layer; those whose Fixed-H layer is empty have Fixed-H layer same as
       * Mobile-H layer */
      if (nNumComponents[iINChI][TAUT_NON]) {
        /* only if both Mobile and Fixed H exist */
        int nFormulaH[TAUT_NUM], nNumH[TAUT_NUM], nCharge[TAUT_NUM],
            nNumIsotopicH[TAUT_NUM][NUM_H_ISOTOPES];
        int nRemovedCharge, nRemovedH, nRemovedIsotopicH[NUM_H_ISOTOPES],
            nFoundRemovedIsoH;
        ...
        len2 = inchi_max(nNumComponents[iINChI][TAUT_YES],
                         nNumComponents[iINChI][TAUT_NON]);
        ...
        for (k = 0; k < len2; k++) {
          ...
          /* at this point at least one of Mobile[k] and Fixed[k] real InChI
           * exists */
          /* initialize for counting removed protons and isotopic H from kth
           * Mobile-H component */
          for (j = 0; j < TAUT_NUM; j++) {
            if (bExists[j]) {
              pInChI[j] =
                  &pInpInChI[iINChI][j]
                            [k]; /* BC: reading uninit memory (fixed?) */
            }
            nFormulaH[j] = 0;
            nNumH[j] = 0;
            nCharge[j] = 0;
          ...
          for (m = 0; m < NUM_H_ISOTOPES; m++) {
            nFoundRemovedIsoH +=
                0 != (nRemovedIsotopicH[m] = nNumIsotopicH[TAUT_NON][m] -
          ...
          if (nRemovedCharge || nFoundRemovedIsoH) {
            ...
            if (!nNumProtons[iINChI][TAUT_YES].pNumProtons) {
              ...
              nNumProtons[iINChI][TAUT_YES].pNumProtons =
                  (COMPONENT_REM_PROTONS *)inchi_calloc(
                      len2, sizeof(nNumProtons[0][0].pNumProtons[0]));
              if (!nNumProtons[iINChI][TAUT_YES].pNumProtons) {
                ret2 = RI_ERR_ALLOC;
                goto exit_function;
              }
            ...
            if (pInChI[TAUT_YES] && bExists[TAUT_YES]) {
              ...
              if (!pInChI[TAUT_YES]->nTautomer) {
                pInChI[TAUT_YES]->nTautomer =
                    (AT_NUMB *)inchi_calloc(pInChI[TAUT_YES]->lenTautomer,
        ...
        if (nNumProtons[iINChI][TAUT_YES].pNumProtons) {
          ...
          for (m = 0; m < NUM_H_ISOTOPES; m++) {
            iso_diff[m] =
                nNumProtons[iINChI][TAUT_YES].nNumRemovedIsotopicH[m] -
                nTotRemovedIsotopicH[m];
            if (iso_diff[m] < 0) {
              ret2 = RI_ERR_SYNTAX;
              goto exit_function;
            } else {
              /* InChI-1.02b bug: nTotRemovedIsotopicH[m] <
               * nNumProtons[iINChI][TAUT_YES].nNumRemovedIsotopicH[m] */
              /* in non-tautomeric components where D(+) or T(+) was removed
               * from -NH(+)= or =OH(+)           */
              iso_diff_tot += iso_diff[m];
            }
          }
          if (iso_diff_tot) {
            if (0 > bIsoMayBeArranged(bInchi2Struct, iso_diff, nNumProtons,
                                      pInpInChI, nNumComponents, iINChI)) {
              ret2 = RI_ERR_SYNTAX;
              goto exit_function;
            }
          }
      ...
      } else if (nNumComponents[iINChI][TAUT_YES] >
                 nNumComponents[iINChI][TAUT_NON]) {
        j = TAUT_NON; /* less components in Fixed-H layer */
        len2 = nNumComponents[iINChI][TAUT_YES];
        len1 = nNumComponents[iINChI][TAUT_NON];
      }
      /* always len1 <= len2; if Mobile-H and Fixed-H have same number of
       * components then len1=len2=0  */
      if (len2 && len1) {
        ...
        for (; len1 < len2; len1++) {
          if (j == TAUT_YES) {
            ...
            if (0 >
                (ret2 = nFillOutProtonMobileH(pInpInChI[iINChI][j] + len1))) {
              goto exit_function;
            }
            if (0 > (ret2 = nProtonCopyIsotopicInfo(
                         pInpInChI[iINChI][j] + len1 /* to */,
      ...
      for (i = iINChI; i < INCHI_NUM; i++) {
        for (j = 0; j < TAUT_NUM; j++) {
          for (k = j; k < TAUT_NUM; k++) {
            if ((iINChI != i || j != k) && num_elem[iINChI][j] &&
                num_elem[i][k]) {
              ...
              /*
              if ( memcmp( num_elem[iINChI], num_elem[i][k],
              (nElDataLen+1)*sizeof(num_elem[0][0][0]) ) { ret2 = RI_ERR_SYNTAX;
                  goto exit_function;
              }
              */
            }
...
#if (FIX_ISO_FIXEDH_BUG_READ == 1)
#undef TAUT_YES
int bIsoMayBeArranged(int bInchi2Struct, int iso_diff[NUM_H_ISOTOPES],
                      REM_PROTONS nNumProtons[INCHI_NUM][TAUT_NUM],
                      INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                      int nNumComponents[INCHI_NUM][TAUT_NUM], int iINChI) {
  const int TAUT_YES = 1;
  int i, k, m, n_found = 0, n_found_at_in_component, n_found_H_in_component,
               i_iso_at, num_iso_H = 0, num_iso_H_orig, num_add_iso_H,
               orig_add_H;
  for (m = 0; m < NUM_H_ISOTOPES; m++) {
    num_iso_H += iso_diff[m];
  }
  num_iso_H_orig = num_iso_H;
  for (k = 0; k < nNumComponents[iINChI][TAUT_YES] &&
              k < nNumComponents[iINChI][TAUT_NON];
       k++) {
    INChI *pInChI = &pInpInChI[iINChI][TAUT_NON][k];
    ...
    if (pInChITaut->bDeleted || pInChI->bDeleted ||
        ...
        pInChITaut->lenTautomer > 1 && pInChITaut->nTautomer &&
            pInChITaut->nTautomer[0] > 0 ||
    ...
    orig_add_H =
        nNumProtons[iINChI][TAUT_YES].pNumProtons[k].nNumRemovedProtons;
    n_found_at_in_component =
        0; /* number of atoms that may accept isotopic H */
    ...
    if (n_found_at_in_component > 0 && num_iso_H > 0 && bInchi2Struct) {
      pInChI->IsotopicAtom = (INChI_IsotopicAtom *)calloc(
          inchi_min(n_found_at_in_component, num_iso_H),
          sizeof(pInChI->IsotopicAtom[0]));
    }
    for (i = 0, i_iso_at = 0; i < pInChI->nNumberOfAtoms; i++) {
      ...
      if (nNumRemovedH > 0 && num_iso_H > 0 && orig_add_H) {
        for (m = 0; m < NUM_H_ISOTOPES && 0 < num_iso_H && 0 < orig_add_H &&
                    0 < nNumRemovedH;
             m++) {
          if (iso_diff[m] > 0) {
            num_add_iso_H =
                inchi_min(iso_diff[m], nNumRemovedH); /* atom limit */
            if (num_add_iso_H > orig_add_H)           /* component limit */
              num_add_iso_H = orig_add_H;
            iso_diff[m] -=
                num_add_iso_H; /* update tot removed single isotope H limit */
            num_iso_H -=
                num_add_iso_H; /* update tot removed isotopic H limit */
            orig_add_H -= num_add_iso_H;   /* update component limit */
            nNumRemovedH -= num_add_iso_H; /* update atom limit */
            nNumProtons[iINChI][TAUT_YES]
                .pNumProtons[k]
                .nNumRemovedIsotopicH[m] += num_add_iso_H;
  ...
  return n_found - num_iso_H_orig; /* >0 => ambiguous reconstruction, 0 =>
                                      unambiguous, <0 => impossible */
}
#define TAUT_YES 1
#endif
...
/************************************************************************************/
int ParseAuxSegmentVersion(const char *str, int bMobileH, INChI *pInpInChI[],
                           int ppnNumComponents[], int state) {
  const char *q;
...
/************************************************************************************/
int CopyAtomNumbers(INChI *pInChI_To, int bIsoTo, INChI *pInChI_From,
                    int bIsoFrom) {
  AT_NUMB *pTo, *pFrom;
  if (!pInChI_To || !pInChI_From || pInChI_To->bDeleted ||
      pInChI_From->bDeleted || !pInChI_To->nNumberOfAtoms ||
      !pInChI_From->nNumberOfAtoms ||
  ...
  if (!pInChI_To->nPossibleLocationsOfIsotopicH) {
    pInChI_To->nPossibleLocationsOfIsotopicH = (AT_NUMB *)inchi_calloc(
        2 * pInChI_To->nNumberOfAtoms,
        sizeof(pInChI_To->nPossibleLocationsOfIsotopicH[0]));
    if (!pInChI_To->nPossibleLocationsOfIsotopicH) {
      return RI_ERR_ALLOC;
    }
  }
  pTo = pInChI_To->nPossibleLocationsOfIsotopicH +
        (bIsoTo ? 0 : pInChI_To->nNumberOfAtoms);
  pFrom = pInChI_From->nPossibleLocationsOfIsotopicH +
          (bIsoFrom ? 0 : pInChI_To->nNumberOfAtoms);
  if (pTo == pFrom) {
    return RI_ERR_PROGR;
  }
...
/************************************************************************************/
int ParseAuxSegmentNumbers(const char *str, int bMobileH, INChI *pInpInChI[],
                           int ppnNumComponents[], int state, int *pbAbc) {
  ...
  AT_NUMB *pNumb;
  int base = 10;
  /* save isotopic numbering into the first nNumberOfAtoms elements of
   * INChI::nPossibleLocationsOfIsotopicH */
  /* save non-isotopic numbering into the second half of nNumberOfAtoms elements
  ...
  while (1) {
    ...
    if (((isdigit(UCINT * pStart) &&
      ...
      if (!pNumb) {
        pNumb = (AT_NUMB *)inchi_calloc(2 * pInChI->nNumberOfAtoms,
                                        sizeof(pNumb[0]));
        if (!pNumb) {
          ret = RI_ERR_ALLOC;
          goto exit_function;
      ...
      if (*pbAbc == 1) {
        for (k = 0, p = pStart; k < pInChI->nNumberOfAtoms && p < pEnd;
             k++, p++) {
...
/**********************************************************************************************************/
int ParseAuxSegmentAtomEqu(const char *str, int bMobileH, INChI *pInpInChI[],
                           int ppnNumComponents[], int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentGroupEqu(const char *str, int bMobileH, INChI *pInpInChI[],
                            int ppnNumComponents[], int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentSp3Inv(const char *str, int bMobileH, INChI *pInpInChI[],
                          int ppnNumComponents[], int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentSp3InvNumbers(const char *str, int bMobileH,
                                 INChI *pInpInChI[], int ppnNumComponents[],
                                 int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentReverseCRV(const char *str, int bMobileH, INChI *pInpInChI[],
                              int ppnNumComponents[], int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentReverseAtoms(const char *str, int bMobileH,
                                INChI *pInpInChI[], int ppnNumComponents[],
                                int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentReverseBonds(const char *str, int bMobileH,
                                INChI *pInpInChI[], int ppnNumComponents[],
                                int state) {
  switch (state) {
...
/***********************************************************************************************************/
int ParseAuxSegmentReverseXYZ(const char *str, int bMobileH, XYZ_COORD **ppXYZ,
                              INChI *pInpInChI[], int ppnNumComponents[],
                              int state) {
...
/************************************************************************************/
int AddAuxSegmentCoord(int nRet, XYZ_COORD *pXYZ, int nLenXYZ,
                       INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                       int nNumComponents[INCHI_NUM][TAUT_NUM]) {
  int iINChI, j, k, n, m, numAt[TAUT_NUM], num_at, nNumMissingNumbers = 0,
                                                   ret = 0;
  INChI *pInChI = NULL;
  ...
  for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
    for (j = TAUT_YES; TAUT_NON <= j; j--) {
      for (k = 0; k < nNumComponents[iINChI][j]; k++) {
        ...
        pAltInChI =
            (k < nNumComponents[iINChI][jj]) ? pInpInChI[iINChI][jj] + k : NULL;
        numAt[j] = (!pInChI->bDeleted) ? pInChI->nNumberOfAtoms : 0;
        numAt[jj] =
            (pAltInChI && !pAltInChI->bDeleted) ? pAltInChI->nNumberOfAtoms : 0;
        switch (j) {
        case TAUT_YES:
          if (!numAt[j]) {
          ...
          if (!pInChI->nPossibleLocationsOfIsotopicH[0]) {
            if (pInChI->nPossibleLocationsOfIsotopicH[numAt[j]]) {
              /* copy from non-isotopic (2nd half of the at. numbers array) to
               * the isotopic (1st half) */
  ...
  for (iINChI = 0; iINChI < INCHI_NUM; iINChI++) {
    for (j = 0; j < TAUT_NUM; j++) {
      for (k = 0; k < nNumComponents[iINChI][j]; k++) {
        ...
        if (iINChI == INCHI_BAS && num_at == 1 && pInChI->szHillFormula &&
            !strcmp(pInChI->szHillFormula, "H") &&
...
/************************************************************************************/
int ReadInChICoord(INCHI_IOSTREAM *pInp, SEGM_LINE *pLine, int *pState,
                   INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                   int nNumComponents[INCHI_NUM][TAUT_NUM]) {
  ...
  *pState = 0;
  ...
  if (pLine->len == 0 ||
      (c != SEG_END && c != RI_ERR_EOF && !INCHI_INP_EOL(c))) {
    *pState = -1;
    pLine->len = 0;
    ret = RI_ERR_PROGR;
    goto exit_error;
  ...
  do {
    ...
    case AST_VERSION:
      /* Mobile H */
      bMobileH = TAUT_YES;
      ret = ParseAuxSegmentVersion(pLine->str, bMobileH, pInpInChI[bReconn],
                                   nNumComponents[bReconn], state);
      state = AST_MOBILE_H_NUMBERS;
      break;
    case AST_MOBILE_H_NUMBERS:
      ret = ParseAuxSegmentNumbers(pLine->str, bMobileH, pInpInChI[bReconn],
                                   nNumComponents[bReconn], state, &bAbc);
      state = AST_MOBILE_H_ATOM_EQ;
      break;
    case AST_MOBILE_H_ATOM_EQ:
      ret = ParseAuxSegmentAtomEqu(pLine->str, bMobileH, pInpInChI[bReconn],
                                   nNumComponents[bReconn], state);
      state = AST_MOBILE_H_GROUP_EQ;
      break;
    case AST_MOBILE_H_GROUP_EQ:
      ret = ParseAuxSegmentGroupEqu(pLine->str, bMobileH, pInpInChI[bReconn],
                                    nNumComponents[bReconn], state);
      state = AST_MOBILE_H_SP3_INV;
      break;
    case AST_MOBILE_H_SP3_INV:
      ret = ParseAuxSegmentSp3Inv(pLine->str, bMobileH, pInpInChI[bReconn],
                                  nNumComponents[bReconn], state);
      state = AST_MOBILE_H_SP3_INV_NUMBERS;
      break;
    case AST_MOBILE_H_SP3_INV_NUMBERS:
      ret =
          ParseAuxSegmentSp3InvNumbers(pLine->str, bMobileH, pInpInChI[bReconn],
                                       nNumComponents[bReconn], state);
    ...
    case AST_FIXED_H_ISO_SP3_INV_NUMBERS:
      ...
      break;
    case AST_REVERSE_INFO_CRV:
      ret = ParseAuxSegmentReverseCRV(pLine->str, bMobileH, pInpInChI[bReconn],
                                      nNumComponents[bReconn], state);
      /* state = (bReconn == INCHI_BAS)? AST_REVERSE_INFO_ATOMS :
       * AST_RECONNECTED_LAYER_FORK;*/
      state = AST_REVERSE_INFO_ATOMS;
      break;
    case AST_REVERSE_INFO_ATOMS:
      ret =
          ParseAuxSegmentReverseAtoms(pLine->str, bMobileH, pInpInChI[bReconn],
                                      nNumComponents[bReconn], state);
      state = AST_REVERSE_INFO_BONDS;
      break;
    case AST_REVERSE_INFO_BONDS:
      ret =
          ParseAuxSegmentReverseBonds(pLine->str, bMobileH, pInpInChI[bReconn],
                                      nNumComponents[bReconn], state);
      state = AST_REVERSE_INFO_XYZ;
      break;
    case AST_REVERSE_INFO_XYZ:
      ret = ParseAuxSegmentReverseXYZ(pLine->str, bMobileH, &pXYZ,
                                      pInpInChI[bReconn],
                                      nNumComponents[bReconn], state);
...
/******************************************************************************************************/
int ReadInChILine(INCHI_IOSTREAM *pInp, SEGM_LINE *pLine, char **pStr,
                  int *pState, INChI *pInpInChI[INCHI_NUM][TAUT_NUM],
                  int nNumComponents[INCHI_NUM][TAUT_NUM],
                  REM_PROTONS nNumProtons[INCHI_NUM][TAUT_NUM],
                  int s[INCHI_NUM][TAUT_NUM][2], int *bStdFormat,
...
next_line:
  ...
  if (pLine->len == 0 || (c != SEG_END && c != RI_ERR_EOF) ||
      !(p = strstr(pLine->str, "InChI=1"))) {
  ...
  do {
    ...
    case IST_MOBILE_H_FORMULA:
      ...
      ret = ParseSegmentFormula(pLine->str, bMobileH, pInpInChI[bReconn],
                                nNumComponents[bReconn]);
      state = IST_MOBILE_H_CONNECTIONS;
      break;
    case IST_MOBILE_H_CONNECTIONS: /* /c */
      ret = ParseSegmentConnections(pLine->str, bMobileH,
                                    &pInpInChI[bReconn][bMobileH],
                                    &nNumComponents[bReconn][bMobileH], &bAbc);
      state = IST_MOBILE_H;
      break;
    case IST_MOBILE_H: /* /h */
      ret = ParseSegmentMobileH(pLine->str, bMobileH, pInpInChI[bReconn],
                                nNumComponents[bReconn], &bAbc);
      state = IST_MOBILE_H_CHARGE;
      break;
    case IST_MOBILE_H_CHARGE: /* /q */
      ret = ParseSegmentCharge(pLine->str, bMobileH, pInpInChI[bReconn],
                               nNumComponents[bReconn]);
      state = IST_MOBILE_H_PROTONS;
      break;
    case IST_MOBILE_H_PROTONS: /* /p */
      ret = ParseSegmentProtons(pLine->str, bMobileH, nNumProtons[bReconn],
                                nNumComponents[bReconn]);
      state = IST_MOBILE_H_SP2;
      break;
    case IST_MOBILE_H_SP2: /* /b */
      ret = ParseSegmentSp2(pLine->str, bMobileH, pInpInChI[bReconn],
                            nNumComponents[bReconn], state, &bAbc);
      state = IST_MOBILE_H_SP3;
      break;
    case IST_MOBILE_H_SP3: /* t */
      ret = ParseSegmentSp3(pLine->str, bMobileH, pInpInChI[bReconn],
                            nNumComponents[bReconn], state, &bAbc);
      state = IST_MOBILE_H_SP3_M;
      break;
    case IST_MOBILE_H_SP3_M: /* /m */
      ret = ParseSegmentSp3m(pLine->str, bMobileH, pInpInChI[bReconn],
                             nNumComponents[bReconn], state);
      state = IST_MOBILE_H_SP3_S;
      break;
    case IST_MOBILE_H_SP3_S: /* /s */
      ret = ParseSegmentSp3s(pLine->str, bMobileH, pInpInChI[bReconn],
                             s[bReconn], nNumComponents[bReconn], state);
    ...
    case IST_MOBILE_H_ISO_LAYER_FORK:
      ...
      if (INCHI_INP_EOL(c) && ret == 0 && !pLine->str[1]) {
        prev_state = state + (bReconn ? IST_HAPPENED_IN_RECMET : 0);
        ret = RI_ERR_SYNTAX; /* empty layer /i or /f or /r at the end of InChI
                                line */
    ...
    case IST_MOBILE_H_ISO_ATOMS: /* i */
      ret = ParseSegmentIsoAtoms(pLine->str, bMobileH, pInpInChI[bReconn],
                                 nNumComponents[bReconn], state, &bAbc);
      state = IST_MOBILE_H_ISO_EXCH_H;
      break;
    case IST_MOBILE_H_ISO_EXCH_H: /* /i/h */
      ret = ParseSegmentIsoExchgH(pLine->str, bMobileH, nNumProtons[bReconn],
                                  nNumComponents[bReconn], state, &bAbc);
    ...
    case IST_FIXED_H_PERMUTATION: /* /f/o */
      ret = ParseSegmentPerm(pLine->str, bMobileH, pInpInChI[bReconn],
                             nNumComponents[bReconn], state, &bAbc);
...
/****************************************************************************************/
int ParseSegmentIsoExchgH(const char *str, int bMobileH,
                          REM_PROTONS nNumProtons[], int pnNumComponents[],
                          int state, int *pbAbc) {
  ...
  if (*pbAbc == 1) {
    ...
    if (*p == 'T') {
      ...
      if (isdigit(UCINT p[0])) {
        nNumProtons[bMobileH].nNumRemovedIsotopicH[2] =
            (NUM_H)inchi_strtol(p, &q, 10);
        p = q;
      }
    }
    if (*p == 'D') {
      ...
      p++;
      if (isdigit(UCINT p[0])) {
        nNumProtons[bMobileH].nNumRemovedIsotopicH[1] =
            (NUM_H)inchi_strtol(p, &q, 10);
        p = q;
      }
    }
    if (*p == 'H') {
      ...
      p++;
      if (isdigit(UCINT p[0])) {
        nNumProtons[bMobileH].nNumRemovedIsotopicH[0] =
            (NUM_H)inchi_strtol(p, &q, 10);
        p = q;
      }
    }
...
/****************************************************************************************/
int ParseSegmentPerm(const char *str, int bMobileH, INChI *pInpInChI[],
                     int ppnNumComponents[], int state, int *pbAbc) {
  ...
  if (!(bMobileH == TAUT_NON && (state == IST_FIXED_H_PERMUTATION ||
                                 state == IST_FIXED_H_ISO_PERMUTATION))) {
    return RI_ERR_PROGR; /* program error */
  }
  ...
  while (pStart < pEnd) {
    /* cycle over components; rearrange Fixed H components in order of Mobile H
     * components */
    /* if /o(1,2,3) then reaarange Fixed H components in this way: tmp<-1, 1<-2,
     * 2<-3, 3<-tmp */
    if (*pStart != '(') {
      ret = RI_ERR_SYNTAX;
      goto exit_function;
    ...
    /* permutation cycle */
    if (*pbAbc == 1) {
      for (p = pPermStart, iComponent2 = numTrans = 0; p < pPermEnd;
           iComponent2 = iComponent1, p = q) {
        /* get first atom number */
        if (0 >= (iComponent1 = (int)inchi_strtol(p, &q, base)) ||
            iComponent1 > nNumComponents) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
        if (iComponent2) {
      ...
      for (p = pPermStart, iComponent2 = numTrans = 0; p < pPermEnd;
           iComponent2 = iComponent1, p = q + (*q == ',')) {
        /* get first atom number */
        if (!isdigit(UCINT * p)) {
          ret = RI_ERR_SYNTAX;
          goto exit_function;
        }
        if (!(iComponent1 = (int)inchi_strtol(p, &q, 10)) ||
            iComponent1 > nNumComponents) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
...
/****************************************************************************************/
int ParseSegmentIsoAtoms(const char *str, int bMobileH, INChI *pInpInChI[],
                         int ppnNumComponents[], int state, int *pbAbc) {
  ...
  while (1) {
    ...
    if ((p = strchr(pStart, '*')) && p < pEnd) {
      ...
      /* copy */
      for (i = 0; i < val; i++) {
        ret = CopySegment(pInChI + iComponent + i, pInChIFrom + iComponent + i,
                          nCpyType, 0, bIsoFrom);
        if (!ret) {
          ret = RI_ERR_SYNTAX;
        }
  ...
  one_more_time:
    if (*pbAbc == 1) {
      ...
      for (p = pStart, iAtom = 0; p < pEnd; iAtom++) {
        ...
        if (!nAtom1 || nAtom1 > pInChI[iComponent].nNumberOfAtoms) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
        ...
        isoAtom.nIsoDifference =
            (NUM_H)inchi_strtol(p, &q, 10); /* alway in abc */
        if (p == q) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
        ...
        if (p > pEnd || (!isoAtom.nIsoDifference && !isoAtom.nNum_T &&
                         !isoAtom.nNum_D && !isoAtom.nNum_H)) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
        if (*pIsotopicAtom) {
      ...
      for (p = pStart, iAtom = 0; p < pEnd; iAtom++) {
        ...
        if (!isoAtom.nIsoDifference && !isoAtom.nNum_T && !isoAtom.nNum_D &&
            !isoAtom.nNum_H) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
    ...
    if (!*pIsotopicAtom) {
      ...
      len = iAtom;
      /* memory allocation */
      if (!(*pIsotopicAtom = (INChI_IsotopicAtom *)inchi_calloc(
                len + 1, sizeof(**pIsotopicAtom)))) {
    ...
    for (i = 1; i < mpy_component; i++) {
      ret = CopySegment(pInChI + iComponent + i, pInChI + iComponent, nCpyType,
                        0, 0);
      if (!ret) {
        ret = RI_ERR_SYNTAX; /* syntax error */
      }
...
/****************************************************************************************/
int ParseSegmentSp3s(const char *str, int bMobileH, INChI *pInpInChI[],
  ...
  int bIso =
      (state == IST_MOBILE_H_ISO_SP3_S || state == IST_FIXED_H_ISO_SP3_S);
  ...
  if (pEnd == pStart) {
    ...
    for (iComponent = 0; iComponent < nNumComponents; iComponent++) {
      pStereo = bIso ? &pInChI[iComponent].StereoIsotopic
                     : &pInChI[iComponent].Stereo;
      if (!*pStereo) {
      ...
      if (pStereo[0]->nNumberOfStereoCenters) {
        ret = RI_ERR_SYNTAX; /* syntax error: "/s" without a digit describes "no
                                stereo" */
        goto exit_function;
      }
      /* allocate empty sp3 stereo */
      if ((!pStereo[0]->t_parity &&
           !(pStereo[0]->t_parity = (S_CHAR *)inchi_calloc(
                 len + 1, sizeof(pStereo[0]->b_parity[0])))) ||
          (!pStereo[0]->nNumber &&
           !(pStereo[0]->nNumber = (AT_NUMB *)inchi_calloc(
                 len + 1, sizeof(pStereo[0]->nNumber[0]))))) {
        /* cleanup */
        if (pStereo[0]->t_parity) {
          INCHI_HEAPCHK
...
/****************************************************************************************/
int bIsSp3LayerNotEmpty(INChI *pInpInChI[], int bMobileH, int bIso,
                        int nNumComponents) {
  INChI *pInChI;
  ...
  if (pInpInChI[bMobileH]) {
    for (iComponent = 0; iComponent < nNumComponents; iComponent++) {
      ...
      if (pStereo && pStereo->nNumberOfStereoCenters > 0 && pStereo->nNumber &&
          pStereo->t_parity) {
        num_not_empty++;
      }
    }
...
/****************************************************************************************/
int ParseSegmentSp3m(const char *str, int bMobileH, INChI *pInpInChI[],
                     int ppnNumComponents[], int state) {
  /* Pass 1: count bonds and find actual numbers of  atom */
  ...
  int bIso =
      (state == IST_MOBILE_H_ISO_SP3_M || state == IST_FIXED_H_ISO_SP3_M);
  ...
  if (str[0] != 'm') {
    /* /m is missing: check whether we have to inherit /m from a preceding
     * stereo layer */
    ...
    if (bMobileH && !bIso) {
      ...
      if (!bIsSp3LayerNotEmpty(pInpInChI, TAUT_NON, 0,
                               ppnNumComponents[TAUT_NON /*bMobileH*/]) /* F */
          && !bIsSp3LayerNotEmpty(
                 pInpInChI, TAUT_YES, 0,
                 ppnNumComponents[TAUT_YES /*bMobileH*/]) /* M */) {
        bMobileHFrom = TAUT_YES;
        bIsoFrom = 1;
      }
    }
    ...
    if (!bIsSp3LayerNotEmpty(pInpInChI, bMobileHFrom, bIsoFrom,
                             ppnNumComponents[/*bMobileH*/ bMobileHFrom])) {
      /* nothing to copy; check whether it should have inherited from a
       * preceding layer */
    ...
    nNumComponents =
        inchi_min(ppnNumComponents[bMobileH], ppnNumComponents[bMobileHFrom]);
    for (iComponent = 0; iComponent < nNumComponents; iComponent++) {
      ...
      if (pInChIFrom->nNumberOfAtoms > 0 && !pInChIFrom->bDeleted &&
        ...
        pStereoFrom =
            bIsoFrom ? pInChIFrom->StereoIsotopic : pInChIFrom->Stereo;
    ...
    return 0; /* return value > 0 means the non-/m segment has been processed
                 here */
  }
...
/****************************************************************************************/
int ParseSegmentSp3(const char *str, int bMobileH, INChI *pInpInChI[],
                    int ppnNumComponents[], int state, int *pbAbc) {
  /* Pass 1: count bonds and find actual numbers of  atom */
  int i, mpy_component, val;
  int nNumComponents, iComponent, len, iAtom;
  ...
  */
  ...
  if (!*pStart) {
    ...
    for (iComponent = 0; iComponent < nNumComponents; iComponent++) {
      ...
      if ((!pStereo[0]->b_parity &&
           !(pStereo[0]->b_parity = (S_CHAR *)inchi_calloc(
                 len0 + 1, sizeof(pStereo[0]->b_parity[0])))) ||
          (!pStereo[0]->nBondAtom1 &&
           !(pStereo[0]->nBondAtom1 = (AT_NUMB *)inchi_calloc(
                 len0 + 1, sizeof(pStereo[0]->nBondAtom1[0])))) ||
          (!pStereo[0]->nBondAtom2 &&
           !(pStereo[0]->nBondAtom2 = (AT_NUMB *)inchi_calloc(
                 len0 + 1, sizeof(pStereo[0]->nBondAtom2[0]))))) {
        /* cleanup */
        if (pStereo[0]->b_parity) {
          INCHI_HEAPCHK
  ...
  while (1) {
    ...
    if (((isdigit(*pStart) && 0 < (val = (int)inchi_strtol(pStart, &q, 10))) ||
         (q = pStart, val = 1)) &&
        (t = strchr(mult_type, *q)) && q + 1 == pEnd) {
      /* process the abbreviation */
      ret = 0;
      ...
      for (i = 0; i < val; i++) {
        ...
        if (bIsoFrom >= 0) {
          INChI_Stereo *pStereoTo = bIsoTo
                                        ? pInChI[iComponent + i].StereoIsotopic
                                        : pInChI[iComponent + i].Stereo;
          if (pStereoTo) {
            pStereoTo->nCompInv2Abs =
                NO_VALUE_INT; /* in case there in no /m segment after this */
          }
        }
      }
      ...
      for (p = pStart, iAtom = 0; p < pEnd; iAtom++) {
        if ((nAtom1 = (AT_NUMB)inchi_strtol(p, &p, base)) &&
            (atomParity = (int)inchi_strtol(p, &p, 10),
             AB_MIN_KNOWN_PARITY <= atomParity &&
                 atomParity <= AB_MAX_KNOWN_PARITY)) {
          ; /* okay */
        } else {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
    ...
    pStereo =
        bIso ? &pInChI[iComponent].StereoIsotopic : &pInChI[iComponent].Stereo;
    ...
    /* allocate sp3 stereo */
    if (!(pStereo[0]->t_parity = (S_CHAR *)inchi_calloc(
              len + 1, sizeof(pStereo[0]->b_parity[0]))) ||
        !(pStereo[0]->nNumber = (AT_NUMB *)inchi_calloc(
              len + 1, sizeof(pStereo[0]->nNumber[0])))) {
      /* cleanup */
    ...
    for (i = 1; i < mpy_component; i++) {
      ...
      ret = CopySegment(pInChI + iComponent + i, pInChI + iComponent, CPY_SP3_M,
                        bIso, bIso);
      if (!ret) {
        ret = RI_ERR_SYNTAX; /* syntax error */
      }
...
/****************************************************************************************/
int ParseSegmentSp2(const char *str, int bMobileH, INChI *pInpInChI[],
                    int ppnNumComponents[], int state, int *pbAbc) {
  /* Pass 1: count bonds and find actual numbers of  atom */
  int i, mpy_component, val;
  ...
  */
  ...
  if (!*pStart) {
    ...
    for (iComponent = 0; iComponent < nNumComponents; iComponent++) {
      ...
      pStereo = bIso ? &pIsoInChI->StereoIsotopic : &pIsoInChI->Stereo;
      if (*pStereo &&
          (pStereo[0]->b_parity || pStereo[0]->nNumberOfStereoBonds ||
  ...
  while (1) {
    ...
    if (*pbAbc == 1) {
      ...
      for (p = pStart, iBond = 0; p < pEnd; iBond++) {
        ...
        if ((nAtom1 = (AT_NUMB)inchi_strtol(p, &p, base)) &&
            ...
            (bondParity = (int)inchi_strtol(p, &p, 10),
             AB_MIN_KNOWN_PARITY <= bondParity &&
                 bondParity <= AB_MAX_KNOWN_PARITY)) {
          ; /* okay */
        } else {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
        if (nAtom1 <= nAtom2 || nAtom1 > pInChI[iComponent].nNumberOfAtoms) {
          ret = RI_ERR_SYNTAX; /* syntax error */
          goto exit_function;
        }
      }
      ...
      for (p = pStart, iBond = 0; p < pEnd; iBond++, p += (*p == ',')) {
        ...
        if (!nAtom1 || !nAtom2 || nAtom1 <= nAtom2 ||
    ...
    /* allocate sp2 stereo */
    if (!(pStereo[0]->b_parity = (S_CHAR *)inchi_calloc(
              len + 1, sizeof(pStereo[0]->b_parity[0]))) ||
        !(pStereo[0]->nBondAtom1 = (AT_NUMB *)inchi_calloc(
              len + 1, sizeof(pStereo[0]->nBondAtom1[0]))) ||
        !(pStereo[0]->nBondAtom2 = (AT_NUMB *)inchi_calloc(
              len + 1, sizeof(pStereo[0]->nBondAtom2[0])))) {
      /* cleanup */
      if (pStereo[0]->b_parity) {
        INCHI_HEAPCHK
        inchi_free(pStereo[0]->b_parity);
    ...
    if (*pbAbc == 1) {
      for (p = pStart, iBond = 0; p < pEnd; iBond++) {
        ...
        if (iBond && !(pStereo[0]->nBondAtom1[iBond - 1] < nAtom1 ||
...
/****************************************************************************************/
int ParseSegmentProtons(const char *str, int bMobileH,
                        REM_PROTONS nNumProtons[], int ppnNumComponents[]) {
  /* Pass 1: count bonds and find actual numbers of  atom */
  int val;
...
/****************************************************************************************/
int ParseSegmentCharge(const char *str, int bMobileH, INChI *pInpInChI[],
  ...
  while (1) {
    ...
    if (((isdigit(UCINT * pStart) &&
      ...
      case TAUT_NON:
        if (*q != 'm' || iComponent + val > nNumComponents ||
        ...
        for (i = 0; i < val; i++) {
          ...
          pInChI[iComponent + i].nTotalCharge =
    ...
    if (mpy_component + iComponent > nNumComponents || mpy_component <= 0) {
      ret =
          RI_ERR_SYNTAX; /* syntax error: too many components in charge layer */
      goto exit_function;
    }
#endif
    pStart = p;
...
/****************************************************************************************/
int ParseSegmentMobileH(const char *str, int bMobileH, INChI *pInpInChI[],
                        int pnNumComponents[], int *pbAbc) {
#define nNum_H(ICOMPONENT)                                                     \
  ((bMobileH == TAUT_YES) ? pInChI[ICOMPONENT].nNum_H                          \
                          : pInChI[ICOMPONENT].nNum_H_fixed)
  /* Pass 1: count bonds and find actual numbers of  atom */
  int i, mpy_component, num_H, num_Minus, val, num_Atoms, numCtAtoms,
      tg_alloc_len, len, len2;
  int num_H_component, num_H_formula, num_taut_H_component, num_H_InChI, ret2;
  int nNumComponents, iComponent, nNumBonds, lenTautomer, tg_pos_Tautomer,
      iTGroup;
  ...
  /* number of immobile H is always allocated; immobile H are present in M layer
   * only */
  ...
  for (i = 0; i < nNumComponents; i++) {
    ...
    len++;
    if (!pInChI[i].nNum_H && /* allocate immobile H segment if it has not been
                                allocated yet */
        !(pInChI[i].nNum_H =
              (S_CHAR *)inchi_calloc(len, sizeof(pInChI[0].nNum_H[0])))) {
      ret = RI_ERR_ALLOC; /* allocation error */
      goto exit_function;
    }
    /* copy immobile H from Mobile-H layer to Fixed-H layer */
    if (bMobileH == TAUT_NON && i < pnNumComponents[nAltMobileH]) {
      memcpy(pInChI[i].nNum_H, pAltInChI[i].nNum_H,
             (len - 1) * sizeof(pInChI[0].nNum_H[0]));
    }
  }
  ...
  while (1) {
...
#if (FIX_DALKE_BUGS == 1)
      if (p != q ||
          !isdigit(UCINT * pStart)) /* prevent non-positive multipliers */
#else
      if (p != q)
#endif
      {
    ...
    t = pTaut =
        (*pbAbc == 1)
            ? strchr(p, ',')
            : strchr(p, '('); /* locate the first tautomer group character */
    ...
    for (i = 0; i < mpy_component; i++) {
      if (bMobileH == TAUT_NON) {
        ...
        if (iComponent + i < pnNumComponents[nAltMobileH]) {
          len = inchi_max(pInChI[iComponent + i].nNumberOfAtoms,
                          pAltInChI[iComponent + i].nNumberOfAtoms) +
                1;
        } else {
          len = pInChI[iComponent + i].nNumberOfAtoms + 1;
        }
        pInChI[iComponent + i].nNum_H_fixed =
            (S_CHAR *)inchi_calloc(len, sizeof(pInChI[0].nNum_H_fixed[0]));
        if (!pInChI[iComponent + i].nNum_H_fixed) {
          ret = RI_ERR_ALLOC; /* allocation error */
          goto exit_function;
        }
        /* compare nAtom */
        if (iComponent + i < pnNumComponents[nAltMobileH]) {
          len2 = inchi_min(pInChI[iComponent + i].nNumberOfAtoms,
                           pAltInChI[iComponent + i].nNumberOfAtoms);
          if (pInChI[iComponent + i].nAtom && len2) {
            /* check */
            if (memcmp(pInChI[iComponent + i].nAtom,
                       pAltInChI[iComponent + i].nAtom,
          ...
          if (pInChI[iComponent + i].nNumberOfAtoms <
              pAltInChI[iComponent + i].nNumberOfAtoms) {
            if (pInChI[iComponent + i].nAtom)
              inchi_free(pInChI[iComponent + i].nAtom);
            if (!(pInChI[iComponent + i].nAtom = (U_CHAR *)inchi_calloc(
                      len, sizeof(pInChI[0].nAtom[0])))) {
              ret = RI_ERR_ALLOC; /* allocation error */
              goto exit_function;
            }
            if (len > 1) {
              memcpy(pInChI[iComponent + i].nAtom,
                     pAltInChI[iComponent + i].nAtom,
                     (len - 1) * sizeof(pInChI[0].nAtom[0]));
            }
            /* correct number of atoms including bridging H */
            pInChI[iComponent + i].nNumberOfAtoms =
                pAltInChI[iComponent + i].nNumberOfAtoms;
          }
        }
      }
    ...
    if (*pbAbc == 1) {
      ...
      while (p < pTaut) {
        ...
        while (p < h) {
          ...
          /* consitency check */
          if (!curAtom || curAtom > numCtAtoms || nxtAtom < curAtom ||
              nxtAtom > numCtAtoms) {
            ret = RI_ERR_SYNTAX; /* syntax error */
            goto exit_function;
          }
    ...
    if ((p = t)) {
      if (*pbAbc == 1) {
        ...
        /* NumGroups; ((NumAt+2, NumH, Num(-), At1..AtNumAt),...);
        ...
        while (p < pEnd) {
          /* start t-group */
          if (!isdigit(UCINT * p) || !(num_H = (int)inchi_strtol(p, &q, 10)) ||
              q > pEnd) {
            ret = RI_ERR_SYNTAX; /* syntax error */
            goto exit_function;
          }
          ...
          if (!tg_alloc_len) {
            ...
            */
            tg_alloc_len = ((3 + INCHI_T_NUM_MOVABLE) *
                            pInChI[iComponent].nNumberOfAtoms) /
                               2 +
                           1;
            for (i = 0; i < mpy_component; i++) {
          ...
          pInChI[iComponent].nTautomer[tg_pos_Tautomer] =
              lenTautomer - tg_pos_Tautomer -
              1; /* length of the rest of the t-group */
        ...
        while (p < pEnd) {
          ...
          case ',':
            ...
            case 'M': /* previous was number of (-) */
              /* the next must be the first tautomeric atom number; save num_H &
               * num_Minus */
              ...
              pInChI[iComponent].nTautomer[tg_pos_Tautomer + 2] = num_Minus;
              lenTautomer =
                  tg_pos_Tautomer + 3; /* first atom number position */
              num_taut_H_component += num_H;
              state = *p++;
              continue;
            case 'A': /* previuos was atom number */
    ...
    /* check num_H in components; for bMobileH=TAUT_NON, pInChI->nNum_H_fixed[]
     * has not been added to pInChI->nNum_H[] yet */
    ...
    if (num_H_formula !=
        num_H_InChI + (bMobileH == TAUT_NON ? num_H_component : 0)) {
      ret = RI_ERR_SYNTAX; /* syntax error */
      goto exit_function;
    }
    ...
    for (i = 1; i < mpy_component; i++) {
      ...
      */
      if (pInChI[iComponent + i].nTautomer && pInChI[iComponent].nTautomer &&
          pInChI[iComponent].lenTautomer) {
      ...
      if (0 > (ret2 = GetInChIFormulaNumH(pInChI + iComponent + i,
                                          &num_H_formula)) ||
...
/****************************************************************************************/
int ParseSegmentConnections(const char *str, int bMobileH, INChI **pInpInChI,
                            int *pnNumComponents, int *pbAbc) {
#define LAST_AT_LEN 256
  ...
  if (str[0] != 'c') {
    if (!pInChI && !*pnNumComponents) {
      ...
      /* allocate InChI */
      if (!(pInChI = *pInpInChI =
                (INChI *)inchi_calloc(nNumComponents, sizeof(INChI)))) {
        return RI_ERR_ALLOC; /* alloc failure */
      }
      /* allocate empty formula */
      pInChI[iComponent].szHillFormula = (char *)inchi_calloc(
          lenFormula + 1, sizeof(pInChI[0].szHillFormula[0]));
      if (!pInChI[iComponent].szHillFormula) {
        ret = RI_ERR_ALLOC; /* allocation failure */
        goto exit_function;
      }
      /* allocate empty connection table */
      pInChI[iComponent].nConnTable = (AT_NUMB *)inchi_calloc(
          lenConnTable + 1, sizeof(pInChI[0].nConnTable[0]));
      ...
      *pnNumComponents = nNumComponents;
      ...
      for (i = 0; i < nNumComponents; i++) {
        ...
        pInChI[i].nConnTable = (AT_NUMB *)inchi_calloc(
            lenConnTable + 1, sizeof(pInChI[0].nConnTable[0]));
        if (!pInChI[i].nConnTable) {
          ret = RI_ERR_ALLOC; /* allocation failure */
          goto exit_function;
  ...
  while (1) {
    ...
    nNumAtoms =
        (int)maxAtom; /* 0 if empty connection table and no bonds present */
    lenConnTable = nNumAtoms + nNumBonds;
    /* connection table format:
     * At1[,Neigh11,Neigh12,...],At2[,Neigh21,Neigh22,...],AtN[NeighN1,NeighN2,...]
     */
    /* where AtK > NeighK1 > NeighK2,...; At(K) < At(K+1); the length =
     * num.atoms + num.bonds */
    for (i = 0; i < mpy_component; i++) {
      ...
      pInChI[iComponent + i].nConnTable = (AT_NUMB *)inchi_calloc(
          lenConnTable + 1, sizeof(pInChI[0].nConnTable[0]));
      if (!pInChI[iComponent + i].nConnTable) {
        ret = RI_ERR_ALLOC; /* allocation failure */
        goto exit_function;
    ...
    if (*pbAbc == 1) {
      ...
      while (p < pEnd) {
        ...
        if (isupper(UCINT * p)) {
          ...
          if ((ret = AddLinkedBond(last_atom[num_open], curAtom,
                                   (AT_NUMB)nNumAtoms, pLB))) {
            goto exit_function;
          }
          p = q;
          if (bPrevVersion == 1) {
        ...
        default:
          if (isdigit(c) && (curAtom = (AT_NUMB)inchi_strtol(p - 1, &q, 10))) {
            ...
            case ',':
              ...
              if ((ret = AddLinkedBond(last_atom[num_open - 1], curAtom,
                                       (AT_NUMB)nNumAtoms, pLB))) {
                goto exit_function;
              }
              last_atom[num_open] = curAtom;
    ...
    for (i = 1; i < mpy_component; i++) {
      ...
      sizeof(pInChI[0].nConnTable[0]) ); if ( !pInChI[iComponent+i].nConnTable )
...
int nFillOutProtonMobileH(INChI *pInChI) {
  ...
  if (!pInChI->szHillFormula &&
      !(pInChI->szHillFormula =
            (char *)inchi_calloc(len + 1, sizeof(pInChI->szHillFormula[0])))) {
    return RI_ERR_ALLOC; /* alloc failure */
  }
  ...
  /* atoms */
  if (!pInChI->nAtom && !(pInChI->nAtom = (U_CHAR *)inchi_calloc(
                              len + 1, sizeof(pInChI->nAtom[0])))) {
    return RI_ERR_ALLOC; /* alloc failure */
  }
  pInChI->nAtom[0] = 1;
  /* charge */
  ...
  /* connection table */
  if (!pInChI->nConnTable && !(pInChI->nConnTable = (AT_NUMB *)inchi_calloc(
                                   len + 1, sizeof(pInChI->nConnTable[0])))) {
    return RI_ERR_ALLOC; /* alloc failure */
  }
  pInChI->nConnTable[0] = 1;
  pInChI->lenConnTable = len;
  /* tautomer */
  if (!pInChI->nTautomer && !(pInChI->nTautomer = (AT_NUMB *)inchi_calloc(
                                  len + 1, sizeof(pInChI->nTautomer[0])))) {
    return RI_ERR_ALLOC; /* alloc failure */
  }
  /* nNum_H */
  if (!pInChI->nNum_H && !(pInChI->nNum_H = (S_CHAR *)inchi_calloc(
                               len + 1, sizeof(pInChI->nNum_H[0])))) {
    return RI_ERR_ALLOC; /* alloc failure */
  }
  pInChI->nNum_H[0] = 0;
...
int nProtonCopyIsotopicInfo(INChI *pInChI_to, INChI *pInChI_from) {
  if (pInChI_from->nNumberOfIsotopicAtoms) {
    if (pInChI_to->nNumberOfIsotopicAtoms &&
        pInChI_from->nNumberOfIsotopicAtoms >
            pInChI_to->nNumberOfIsotopicAtoms) {
    ...
    if (!pInChI_to->IsotopicAtom &&
        !(pInChI_to->IsotopicAtom = (INChI_IsotopicAtom *)inchi_calloc(
              pInChI_from->nNumberOfIsotopicAtoms,
...
/****************************************************************************************/
int ParseSegmentFormula(const char *str, int bMobileH, INChI *pInpInChI[],
                        int pnNumComponents[]) {
  int i, j, mpy_component, mpy_atom, len, el_number;
  int nNumComponents = 0, iComponent, nNumAtoms, nNumAtomsAndH, iAtom, nNumH,
      nAltMobileH = ALT_TAUT(bMobileH);
  ...
  if (!nNumComponents) {
    if (!*pStart || islower(UCINT * pStart)) {
      INCHI_HEAPCHK
      if (bMobileH == TAUT_NON &&
          0 < (nNumComponents = pnNumComponents[nAltMobileH])) {
        /* allocate InChI */
        ...
        for (i = 0; i < nNumComponents; i++) {
          ...
          len = pInpInChI[bMobileH][i].nNumberOfAtoms =
              pInpInChI[nAltMobileH][i].nNumberOfAtoms;
          /* copy atoms */
          ...
          if ((pInpInChI[bMobileH][i].nAtom = (U_CHAR *)inchi_malloc(
            ...
            memcpy(pInpInChI[bMobileH][i].nAtom,
                   pInpInChI[nAltMobileH][i].nAtom, len);
          ...
          if ((pInpInChI[bMobileH][i].szHillFormula =
                   (char *)inchi_malloc(inchi_max(len, 2)))) {
            memcpy(pInpInChI[bMobileH][i].szHillFormula,
                   pInpInChI[nAltMobileH][i].szHillFormula, len);
          } else {
            return RI_ERR_ALLOC; /* alloc failure */
          }
        }
  ...
  while (1) {
    ...
    for (i = 0; i < mpy_component; i++) {
      ...
      pInChI[iComponent + i].szHillFormula =
          (char *)inchi_malloc(inchi_max(len, 1) + 1);
      ...
      if (!i) {
        ...
        /* Pass 2.2 Store elements; this assumes no bridging H. Bridging H will
         * be found in connection table, /c */
        iAtom = 0;
        if (nNumAtoms > 0) {
        ...
        strcpy(pInChI[iComponent + i].szHillFormula,
               pInChI[iComponent].szHillFormula);
        ...
        if (!pInChI[iComponent + i].nAtom)
          return RI_ERR_ALLOC; /* failed allocation */
        memcpy(pInChI[iComponent + i].nAtom, pInChI[iComponent].nAtom,
               nNumAtoms + 1);
      }
    }
    iComponent += i;
  ...
  if (bMobileH == TAUT_NON) {
    /* at this point the exact number of atoms including bridging H is known
     * from TAUT_YES */
    for (i = 0; i < nNumComponents && i < pnNumComponents[nAltMobileH]; i++) {
      if (pInpInChI[bMobileH][i].nNumberOfAtoms <
          (len = pInpInChI[nAltMobileH][i].nNumberOfAtoms)) {
        ...
        if (pInpInChI[nAltMobileH][i].nAtom) {
          ...
          memcpy(nAtom, pInpInChI[nAltMobileH][i].nAtom,
                 len * sizeof(nAtom[0]));
          nAtom[len] = 0;
          if (pInpInChI[bMobileH][i].nAtom) {
            inchi_free(pInpInChI[bMobileH][i].nAtom);
...
/****************************************************************************************/
int CopySegment(INChI *pInChITo, INChI *pInChIFrom, int SegmentType,
                int bIsotopicTo, int bIsotopicFrom) {
  int ret = RI_ERR_ALLOC;
  int len;

  if (SegmentType == CPY_SP2 || SegmentType == CPY_SP3 ||
      SegmentType == CPY_SP3_M || SegmentType == CPY_SP3_S) {

    INChI_Stereo **pstereoTo = NULL;
    INChI_Stereo *stereoFrom = bIsotopicFrom == 1   ? pInChIFrom->StereoIsotopic
                               : bIsotopicFrom == 0 ? pInChIFrom->Stereo
                                                    : NULL;
    if (stereoFrom || bIsotopicFrom < 0) {
      if (SegmentType == CPY_SP2) {
        if (bIsotopicFrom < 0 ||
            (stereoFrom->b_parity && stereoFrom->nBondAtom1 &&
             stereoFrom->nBondAtom2)) {

          len = (bIsotopicFrom < 0) ? 0 : stereoFrom->nNumberOfStereoBonds;
          pstereoTo =
              bIsotopicTo ? &pInChITo->StereoIsotopic : &pInChITo->Stereo;
          if (!pstereoTo[0]) {
            if (!(pstereoTo[0] =
                      (INChI_Stereo *)inchi_calloc(1, sizeof(**pstereoTo)))) {
              goto exit_function;
            }
          }
          if (pstereoTo[0]->nNumberOfStereoBonds > 0 ||
              pstereoTo[0]->b_parity || pstereoTo[0]->nBondAtom1 ||
              pstereoTo[0]->nBondAtom2) {
            ret = RI_ERR_SYNTAX; /* stereo already exists */
            goto exit_function;
          }
          /* allocate sp2 stereo */
          if (!(pstereoTo[0]->b_parity = (S_CHAR *)inchi_calloc(
                    len + 1, sizeof(pstereoTo[0]->b_parity[0]))) ||
              !(pstereoTo[0]->nBondAtom1 = (AT_NUMB *)inchi_calloc(
                    len + 1, sizeof(pstereoTo[0]->nBondAtom1[0]))) ||
              !(pstereoTo[0]->nBondAtom2 = (AT_NUMB *)inchi_calloc(
        ...
        if (bIsotopicFrom < 0 ||
            (stereoFrom->t_parity && stereoFrom->nNumber)) {
          ...
          if (pstereoTo[0]->nNumberOfStereoCenters > 0 ||
              pstereoTo[0]->t_parity || pstereoTo[0]->nNumber) {
            ret = RI_ERR_SYNTAX; /* stereo already exists */
            goto exit_function;
          }
          /* allocate sp3 stereo */
          if (!(pstereoTo[0]->t_parity = (S_CHAR *)inchi_calloc(
                    len + 1, sizeof(pstereoTo[0]->b_parity[0]))) ||
              !(pstereoTo[0]->nNumber = (AT_NUMB *)inchi_calloc(
                    len + 1, sizeof(pstereoTo[0]->nBondAtom1[0])))) {
            /* cleanup */
        ...
        if (pstereoTo[0]->nCompInv2Abs &&
            NO_VALUE_INT != pstereoTo[0]->nCompInv2Abs) {
          ret = RI_ERR_SYNTAX; /* stereo already exists */
          goto exit_function;
        }
        if (bIsotopicFrom < 0) {
        ...
        if (SegmentType == CPY_SP3_S) {
          pstereoTo =
              bIsotopicFrom ? &pInChITo->StereoIsotopic : &pInChITo->Stereo;
          if (!pstereoTo[0]) {
            if (!(pstereoTo[0] =
                      (INChI_Stereo *)inchi_calloc(1, sizeof(**pstereoTo)))) {
              goto exit_function;
    ...
    if (bIsotopicFrom < 0 || IsotopicAtomFrom) {
      ...
      if (!*pIsotopicAtomTo) {
        if (!(*pIsotopicAtomTo = (INChI_IsotopicAtom *)inchi_calloc(
                  len + 1, sizeof(**pIsotopicAtomTo)))) {
          goto exit_function;
        }
      }
      ...
      if (bIsotopicFrom >= 0 && len) {
        memcpy(*pIsotopicAtomTo, IsotopicAtomFrom,
               (len + 1) * sizeof(**pIsotopicAtomTo));
      }
...
int getInChIChar(INCHI_IOSTREAM *pInp) {
  if (pInp->type == INCHI_IOSTREAM_STRING) {
    ...
    int c;
     ...
     defined(__GNUC__) && defined(__MINGW32__) && defined(_WIN32))
    do {
      c = getc(pInp->f);
      if (c == EOF) {
        c = RI_ERR_EOF;
...
int AddInChIChar(INCHI_IOSTREAM *pInp, SEGM_LINE *Line, const char *pszToken) {
  ...
  */
  ...
  if (Line->len + 2 >= Line->len_alloc) {
    char *str =
        (char *)inchi_calloc(Line->len_alloc + SEGM_LINE_ADD, sizeof(str[0]));
    INCHI_HEAPCHK
    if (str) {
...
int nGetInChISegment(INCHI_IOSTREAM *pInp, SEGM_LINE *Line,
                     const char *pszToken) {
  int c;
  Line->len = 0;
...
int AddLinkedBond(AT_NUMB at1, AT_NUMB at2, AT_NUMB num_at, LINKED_BONDS *pLB) {
  ...
  if (pLB->len_alloc <= nReqLen) {
    /*int nNewLen = nReqLen + (nReqLen + LINKED_BOND_ADD - 1)%LINKED_BOND_ADD +
     * LINKED_BOND_ADD;*/
    int nNewLen = nReqLen - nReqLen % LINKED_BOND_ADD + 2 * LINKED_BOND_ADD;
    ONE_LINKED_BOND *pBond =
        (ONE_LINKED_BOND *)inchi_calloc(nNewLen, sizeof(pBond[0]));
    if (!pBond)
  ...
  prev = pLB->pBond[at1]
             .prev; /* position of the last neighbor of at1 in the pLB->pBond */
  if (!prev) {
  ...
  prev = pLB->pBond[at2]
             .prev; /* position of the last neighbor of at2 in the pLB->pBond */
  if (!prev) {
