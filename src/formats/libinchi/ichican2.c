...
typedef struct tagCTable {
  ...
  /* Format-atoms:   atom_rank[k] neigh_rank[k][1]...neigh_rank[k][n]
     ...
     Format-tgroup:  tgroup_rank[k] endpoint_rank[k][1]...endpoint_rank[k][n]
            ...
            Note:    tgroup_rank[k] > endpoint_rank[k][j] for all j by
     construction
  */
  ...
  AT_RANK
      *nextAtRank; /* rank (k value) after the last node of the Ctbl portion*/
...
/*************local prototypes **********************************/
int CanonGraph(int n, int n_tg, int n_max, int bDigraph, Graph *G,
               Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
               AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
               ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out);

void CtPartFill(Graph *G, CANON_DATA *pCD, Partition *p, ConTable *Ct, int k,
                int n, int n_tg);
...
int CtPartCompare(ConTable *Ct1, ConTable *Ct2, S_CHAR *cmp,
                  kLeast *kLeastForLayer, int k, int bOnlyCommon,
                  int bSplitTautCompare);
int CtFullCompare(ConTable *Ct1, ConTable *Ct2, int bOnlyCommon,
                  int bSplitTautCompare);
...
int CtPartCompareLayers(kLeast *kLeast_rho, int L_rho_fix_prev,
...
void MergeCleanIsoSortKeys(AT_ISO_SORT_KEY *isk1, AT_ISO_SORT_KEY *isk2,
                           int len);

int UnorderedPartitionJoin(UnorderedPartition *p1, UnorderedPartition *p2,
                           int n);
...
int AllNodesAreInSet(NodeSet *cur_nodes, int lcur_nodes, NodeSet *set,
                     int lset);
...
int PartitionColorVertex(Graph *G, Partition *p, Node v, int n, int n_tg,
                         int n_max, int bDigraph, int nNumPrevRanks);
void PartitionCopy(Partition *To, Partition *From, int n);
int PartitionSatisfiesLemma_2_25(Partition *p, int n);
void PartitionGetTransposition(Partition *pFrom, Partition *pTo, int n,
                               Transposition *gamma);
void PartitionGetMcrAndFixSet(Partition *p, NodeSet *Mcr, NodeSet *Fix, int n,
                              int l);
...
void TranspositionGetMcrAndFixSetAndUnorderedPartition(Transposition *gamma,
                                                       NodeSet *McrSet,
                                                       NodeSet *FixSet, int n,
                                                       int l,
                                                       UnorderedPartition *p);

void insertions_sort_NeighList_AT_NUMBERS2(NEIGH_LIST base, AT_RANK *nRank,
                                           AT_RANK max_rj);
int WriteGraph(Graph *G, int n, int gnum, char *fname, char *fmode);

int SetInitialRanks2(int num_atoms, ATOM_INVARIANT2 *pAtomInvariant2,
                     AT_RANK *nNewRank, AT_RANK *nAtomNumber);
void FillOutAtomInvariant2(sp_ATOM *at, int num_atoms, int num_at_tg,
                           ATOM_INVARIANT2 *pAtomInvariant, int bIgnoreIsotopic,
                           int bHydrogensInRanks, int bHydrogensFixedInRanks,
                           int bDigraph, int bTautGroupsOnly,
                           T_GROUP_INFO *t_group_info);

int GetCanonRanking2(int num_atoms, int num_at_tg, int num_max, int bDigraph,
                     sp_ATOM *at, AT_RANK **pRankStack, int nNumPrevRanks,
...
int CanonGraph01(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
                    nAtomNumberCanon, pCD, pCC, pp_zb_rho_inp, pp_zb_rho_out);
}
int CanonGraph02(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph03(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph04(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph05(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph06(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph07(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph08(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph09(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph10(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph11(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int CanonGraph12(int n, int n_tg, int n_max, int bDigraph, Graph *G,
                 Partition pi[], AT_RANK *nSymmRank, AT_RANK *nCanonRank,
                 AT_NUMB *nAtomNumberCanon, CANON_DATA *pCD, CANON_COUNTS *pCC,
                 ConTable **pp_zb_rho_inp, ConTable **pp_zb_rho_out) {
  return CanonGraph(n, n_tg, n_max, bDigraph, G, pi, nSymmRank, nCanonRank,
...
int NodeSetCreate(NodeSet *pSet, int n, int L) {
  ...
  pSet->bitword[0] =
      (bitWord *)inchi_calloc(len * L, sizeof(pSet->bitword[0][0]));
  if (!pSet->bitword[0]) {
    /* cleanup */
...
int CTableCreate(ConTable *Ct, int n, CANON_DATA *pCD) {
  int maxlenCt =
      pCD->nMaxLenLinearCT + 1; /* add one element for CtPartInfinity() */
  ...
  int maxlenIsoExchg =
      pCD->iso_exchg_atnos ? (pCD->maxlen_iso_exchg_atnos + 1) : 0;
#if (USE_ISO_SORT_KEY_HFIXED == 1)
  int maxlenIsoHfixed = pCD->maxlen_iso_sort_key_Hfixed
                            ? (pCD->maxlen_iso_sort_key_Hfixed + 1)
                            : 0;
#endif
  ...
  if (maxlenIso) {
    Ct->iso_sort_key =
        (AT_ISO_SORT_KEY *)inchi_calloc(maxlenIso, sizeof(Ct->iso_sort_key[0]));
  }
  if (maxlenIsoExchg) {
    Ct->iso_exchg_atnos =
        (S_CHAR *)inchi_calloc(maxlenIsoExchg, sizeof(Ct->iso_exchg_atnos[0]));
  }
#if (USE_ISO_SORT_KEY_HFIXED == 1)
  if (maxlenIsoHfixed) {
    Ct->iso_sort_key_Hfixed = (AT_ISO_SORT_KEY *)inchi_calloc(
        maxlenIsoHfixed, sizeof(Ct->iso_sort_key_Hfixed[0]));
  }
#endif
#ifdef INCHI_CANON_USE_HASH
  ...
  Ct->nextAtRank[0] = 0;
  Ct->nextCtblPos[0] = 0;
  if (Ct->Ctbl && Ct->nextCtblPos && (!maxlenNumH || Ct->NumH) &&
...
int PartitionGetFirstCell(Partition *p, Cell *baseW, int k, int n) {
  ...
  if (i < n) {
    ...
    for (r = (AT_RANK)(i + 1);
         i < n && r == (rank_mask_bit & p->Rank[(int)p->AtNumber[i]]); i++, r++)
      ;
  }
  if (i < n) {
    W->first = i;
    for (r = (rank_mask_bit & p->Rank[(int)p->AtNumber[i]]), i++;
         i < n && r == (rank_mask_bit & p->Rank[(int)p->AtNumber[i]]); i++)
      ;
    W->next = i;
    INCHI_HEAPCHK
...
void PartitionGetMcrAndFixSet(Partition *p, NodeSet *Mcr, NodeSet *Fix, int n,
  ...
  for (i = 0, r = 1; i < n; i++, r++) {
    if (r == (r1 = (rank_mask_bit & p->Rank[j1 = (int)p->AtNumber[i]]))) {
      ...
      for (r = r1; i + 1 < n &&
                   r == (rank_mask_bit & p->Rank[j2 = (int)p->AtNumber[i + 1]]);
           i++) {
        if (j1 > j2) {
          j1 = j2;
        }
...
/************* used in ichi_bns.c ********************************/
void NodeSetFromRadEndpoints(NodeSet *cur_nodes, int k,
                             /*Node *v*/ Vertex RadEndpoints[], int num_v) {
...
/************* used in ichi_bns.c ********************************/
int AddNodesToRadEndpoints(NodeSet *cur_nodes, int k, Vertex RadEndpoints[],
                           Vertex vRad, int nStart, int nLen) {
  int n = nStart;
  if (cur_nodes->bitword) {
    bitWord *Bits = cur_nodes->bitword[k];
    int len = cur_nodes->len_set;
...
int PartitionSatisfiesLemma_2_25(Partition *p, int n) {
  ...
  if (n <= nPartitionSize + 4 || n == nPartitionSize + nNumNonTrivialCells ||
...
/****************************************************************/
/* makes new equitable partition (p+1) out of p; first reduce the rank of vertex
 * v */
int PartitionColorVertex(Graph *G, Partition *p, Node v, int n, int n_tg,
                         int n_max, int bDigraph, int nNumPrevRanks) {
  ...
  s = n_max + 1; /* always greater than sv; this initialization is needed only
                    to keep the compiler happy */
  for (j = (int)rv - 1;
       0 <= j && rv == (r = p[1].Rank[(int)(s = p[1].AtNumber[j])]) && s != sv;
       j--)
    ;
  if (s != sv) {
    INCHI_HEAPCHK
  ...
  for (i = j--; 0 <= j && rv == (r = p[1].Rank[(int)(s = p[1].AtNumber[j])]);
       i = j, j--) {
    p[1].AtNumber[i] = s;
  }
  ...
  if (bDigraph) {
    ...
    */
    nNumNewRanks = DifferentiateRanks4(
        n_tg, G, nNumPrevRanks + 1, p[1].Rank, p[2].Rank /* temp array */,
    ...
    nNumNewRanks = DifferentiateRanks2( n_tg, G,
                                     ...
                                     p[1].AtNumber, &lNumNeighListIter, 1 );
    */
    nNumNewRanks = DifferentiateRanks3(n_tg, G, nNumPrevRanks + 1, p[1].Rank,
                                       p[2].Rank /* temp array */,
...
Node CellGetMinNode(Partition *p, Cell *W, Node v, CANON_DATA *pCD) {
  ...
  if (pCD && pCD->nAuxRank) {
     ...
     * || (AuxRank(v1) < AuxRank(v2)) vertex v1 > v2 <=>
     * (AuxRank(v1)==AuxRank(v2) && AtNumb(v1) > AtNumb(v2)) || (AuxRank(v1) >
     * AuxRank(v2)) vertex v1 = v2 <=> (AuxRank(v1)==AuxRank(v2) && AtNumb(v1)
     * == AtNumb(v2))
     */
    ...
    for (; i < W->next; i++) {
      ...
      if (uCurAtNumb >= v && !(p->Rank[nCurAtNumb] & rank_mark_bit)) {
        ...
        if (v) {
          /* get next node */
          /* find node with smallest uCurAtNumb among nodes with aux. ranks
           * equal to uMinAuxRank */
          if (uCurAuxRank == uMinAuxRank && uCurAtNumb < uMinAtNumb) {
            uMinAtNumb = uCurAtNumb;
          }
        } else {
          /* get first node */
          /* find node with smallest smallest uCurAtNumb among nodes with
           * smallest aux. ranks */
          if (uMinAuxRank > uCurAuxRank) {
            uMinAuxRank = uCurAuxRank;
            uMinAtNumb = uCurAtNumb;
          } else if (uMinAuxRank == uCurAuxRank && uCurAtNumb < uMinAtNumb) {
            uMinAtNumb = uCurAtNumb;
          }
        }
      }
...
#endif /* } USE_AUX_RANKING */
  {
    for (i = W->first; i < W->next; i++) {
      uCurAtNumb = p->AtNumber[(int)i];
      if (uCurAtNumb >= v && !(p->Rank[(int)uCurAtNumb] & rank_mark_bit) &&
          uCurAtNumb < uMinAtNumb) {
        uMinAtNumb = uCurAtNumb;
      }
    }
  }
  if (uMinAtNumb != INFINITY)
    uMinAtNumb++;
  INCHI_HEAPCHK
  return uMinAtNumb;
}
...
int CellIntersectWithSet(Partition *p, Cell *W, NodeSet *Mcr, int l) {
  ...
  for (i = first, k = 0; i < next; i++) {
    j = (int)p->AtNumber[i];
    if (!(McrBits[j / num_bit] &
          bBit[j %
               num_bit])) { /* BC: reading uninit memory ???-not examined yet */
...
/*  k = (new index in Ct->nextAtRank[] and Ct->nextCtblPos[]) + 1 */
{
  ...
  if (k) {
    ...
    startAtOrd =
        Ct->nextAtRank[k - 1] - 1; /* here  p->Rank[p->AtNumber[r-1]] = r */
  } else {
    startCtbl = 0;
    startAtOrd = 0;
  }
  ...
  for (i = startAtOrd;
       i < n_tg && r == (rank_mask_bit & p->Rank[m = (int)p->AtNumber[i]]);
       i++, r++) {
    Ct->Ctbl[startCtbl++] = r;
...
#endif
    for (j = 1; j <= nn && (rj = (rank_mask_bit & p->Rank[(int)G[m][j]])) < r;
         j++) {
...
void CtPartInfinity(ConTable *Ct, S_CHAR *cmp, int k) {
  ...
  if (k) {
    startCtbl = Ct->nextCtblPos[k - 1];
    /*startAtOrd = Ct->nextAtRank[k-1]-1;*/ /* here  p->Rank[p->AtNumber[r-1]] =
                                               r */
    if (cmp) {
...
int CtPartCompare(ConTable *Ct1, ConTable *Ct2, S_CHAR *cmp,
  ...
  int midCt /* end of atoms only Ct */,
      midNumH = 0 /* end of atoms only NumH */, maxVert;
  ...
  /* set kLeastForLayer[nLayer].k = (k+1) or -(k+1)
      ...
      difference at layer nLayer are zeroes of hidden by differences in smaller
     nLayer.

      "Hidden by difference in smaller level" means that nLayer of comparison
      has not been reached because the difference was discovered at a previous
     layer.
      ...
      -----------------------------------------------
      pzb_rho(m-1)[nLayerCurr] == pzb_rho(m)[nLayerCurr] for nLayerCurr =
     0..nLayerDiff-1 pzb_rho(m-1)[nLayerDiff] <  pzb_rho(m)[nLayerDiff]

      Definition: pzb_rho(m-1)[nLayerDiff] <  pzb_rho(m)[nLayerDiff] means that
      -------------------------------------------------------------------------
      pzb_rho(m-1)[nLayerDiff][i]     == pzb_rho(m)[nLayerDiff][i] for
     i=0..iDdiff-1 pzb_rho(m-1)[nLayerDiff][iDdiff] <
     pzb_rho(m)[nLayerDiff][iDdiff]

      This defines nLayerDiff(pzb1, pzb2) where pszb1 = pzb_rho(a),
     pzb2=pzb_rho(b) (a<b) or pzb_rho_fix and   iDdiff    (pzb1, pzb2). In case
     pzb_rho(m)[nLayerCurr] == pzb_rho_fix[nLayerCurr] for all non-NULL
     nLayerCurr in pzb_rho_fix, nLayerDiff(pzb_rho(m), pzb_rho_fix) = the first
     layer in pzb_rho(m) not present in pzb_rho_fix iDdiff    (pzb_rho(m),
     pzb_rho_fix) = -1 Case when such a layer does not exist means program error
      ...
      ----------------------------------------------------------------------
      a)    0 < abs(kLeastForLayerFix[L_fix].k) <= k-1 (* in this case I_fix >=
     0 *)  &&
            ((L_fix < L_rho) || (L_fix == L_rho && I_fix < I_rho))
            =>
            qzb_rho_fix = kLeastForLayerFix[L_fix].k if prevoiusly qzb_rho_fix
     == 0

      b)    otherwise do not change qzb_rho_fix, except the following:

      c)    Special case L_rho == L_fix && I_rho == I_fix. Let L=L_rho, I =
     I_rho.

            Compare 3 valirs: Lambda[L][I], pzb_rho(m)[L][I], pzb_rho_fix[L][I]
            The algorithm provides pzb_rho(m)[L][I] < pzb_rho_fix[L][I]
            (pzb_rho(m)[L][I]==pzb_rho_fix[L][I] <=>
     pzb_rho(m)[L][I]==pzb_rho_fix[L][I] is impossible by construction) There
     are 3 possibilities: c1) Lambda[L][I]     < pzb_rho(m)[L][I]  <
     pzb_rho_fix[L][I] <=> kLeastForLayer[L].k  < 0 && kLeastForLayerFix[L].k <
     0
            ...
            Case
                kLeastForLayer[L].k  < 0 && kLeastForLayerFix[L].k > 0 is
     impossible because it means pzb_rho_fix < Lambda < pzb_rho(m) <=>
     pzb_rho_fix < pzb_rho(m)
      ...
      a) (L_rho < L_fix) || (L_rho == L_fix && I_rho < I_fix)  =>
         ...
         Lambda differs from pzb_rho(m) in the part of pzb_rho(m) that will
     never change qzb_rho = kLeastForLayer[L_rho].k; reject Labmda or accept
     pzb_rho(m+1):=Labmda
  ...
  */
  ...
  if ((diff = -(startCt1 - startCt2))) {
    ...
    if (bOnlyCommon && startCt1 >= Ct1->nLenCTAtOnly &&
        startCt2 >= Ct2->nLenCTAtOnly && Ct1->Ctbl[startCt1] == EMPTY_CT &&
        Ct2->Ctbl[startCt2] == EMPTY_CT) {
      return 0;
    }
    if (bOnlyCommon) {
      ...
      if (startCt1 > startCt2 && Ct1->maxVert > Ct2->maxVert &&
  ...
  if ((diff = -(endCt1 -
    ...
    if (bOnlyCommon) {
      ...
      /* take care of case when comparing tautomeric vs non-tautomeric:
         ...
         compare up to min out of the two, ignoring INFINITY in the last
         position */
  ...
  for (i = startCt1; i < midCt && Ct1->Ctbl[i] == Ct2->Ctbl[i]; i++)
    /*for ( i = startCt1; i < endCt && !(diff = (int)Ct1->Ctbl[i] -
     * (int)Ct2->Ctbl[i]); i ++ )*/
    ;
  if (i < midCt) {
  ...
  if (Ct1->iso_sort_key && Ct2->iso_sort_key) {
    for (i = startAt1;
         i < endAt1 && Ct1->iso_sort_key[i] == Ct2->iso_sort_key[i]; i++)
      ;
    if (i < endAt1) {
  ...
  if (Ct1->iso_exchg_atnos && Ct2->len_iso_exchg_atnos) {
    for (i = startAt1;
         i < endAt1 && Ct1->iso_exchg_atnos[i] == Ct2->iso_exchg_atnos[i]; i++)
      ;
    if (i < endAt1) {
  ...
  if (Ct1->iso_sort_key_Hfixed && Ct2->iso_sort_key_Hfixed) {
    for (i = startAt1; i < midAt && Ct1->iso_sort_key_Hfixed[i] ==
                                        Ct2->iso_sort_key_Hfixed[i];
         i++)
      ;
    if (i < midAt) {
  ...
  if (diff) {
    diff = (diff > 0)
               ? (nLayer + 1)
               : -(nLayer +
                   1); /* return the discovered difference layer number >= 1 */
    if (kLeastForLayer) {
...
int CtFullCompare(ConTable *Ct1, ConTable *Ct2, int bOnlyCommon,
  ...
  int midCt /* end of atoms only in Ctbl */,
      midNumH = 0 /* end of atoms only NumH */, midAt /* end of atoms only */;
  int diff, i, k1, k2, lenNumH1, lenNumH2, lenNumH, maxVert /* min num atoms */;
  int len_iso_sort_key1, len_iso_sort_key2,
      len_iso_sort_key /*, mid_iso_sort_key*/;
  int nLayer = 0;
  ...
  if (bOnlyCommon) {
    ...
    lenNumH = lenNumH1 = lenNumH2 = inchi_min(Ct1->lenNumH, Ct2->lenNumH);
    len_iso_sort_key = len_iso_sort_key1 = len_iso_sort_key2 =
        inchi_min(Ct1->len_iso_sort_key, Ct1->len_iso_sort_key);
  } else {
  ...
  if (Ct1->NumH && Ct2->NumH) {
    if ((diff = -(
             lenNumH1 -
             lenNumH2))) { /* negative sign reproduces results for NSC=28393 */
      goto done;
    }
    if (endAt1 < maxVert) {
  ...
  nLayer++;
  if (Ct1->NumHfixed && Ct2->NumHfixed) {
    for (i = startAt1; i < endAt1 && Ct1->NumHfixed[i] == Ct2->NumHfixed[i];
         i++)
      ;
    if (i < endAt1) {
      diff = (int)Ct1->NumHfixed[i] - (int)Ct2->NumHfixed[i];
  ...
  nLayer++;
  if (Ct1->iso_sort_key && Ct2->iso_sort_key) {
    if ((diff = -(len_iso_sort_key1 -
                  len_iso_sort_key2))) { /* negative sign reproduces results for
                                            NSC=28393 */
      goto done;
    }
    for (i = startAt1;
...
int CtPartCompareLayers(kLeast *kLeast_rho, int L_rho_fix_prev,
  ...
  if (0 < CtCompareLayersGetFirstDiff(kLeast_rho, nOneAdditionalLayer, &L_rho,
                                      &I_rho, &k_rho) &&
      /* differences has been found in a real layer or all real layers are
         identical */
...
void CtPartCopy(ConTable *Ct1 /* to */, ConTable *Ct2 /* from */, int k) {
  ...
  if (Ct1->iso_sort_key_Hfixed && Ct2->iso_sort_key_Hfixed) {
    ...
    for (i = 0; i < len2iso_sort_key; i++) {
      Ct1->iso_sort_key_Hfixed[startAt1 + i] =
          Ct2->iso_sort_key_Hfixed[startAt2 + i];
    }
  }
#endif
...
void TranspositionGetMcrAndFixSetAndUnorderedPartition(Transposition *gamma,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (j == i) {
      ...
      /* fill out the unordered partition, the mcr first, other in the cycle
       * after that */
      p->equ2[mcr] = mcr;
      for (k = mcr; mcr != (j = (int)(rank_mask_bit & gamma->nAtNumb[k]));
           k = j) {
        p->equ2[j] = mcr;
      }
    }
...
int SetBitCreate(void) {
  ...
  b1 = 1;
  num_bit = 1;
  for (b1 = 1, num_bit = 1; b1 < (b2 = (bitWord)((b1 << 1) & BIT_WORD_MASK));
       b1 = b2, num_bit++)
    ;
...
/***************************************************************************************/
void RearrangeAtRankArray(int n, AT_RANK *nRank, AT_NUMB *new2old, S_CHAR *mark,
                          int bDone) {
  int i, k, j;
  AT_RANK r;
...
int GetCanonRanking2(int num_atoms, int num_at_tg, int num_max, int bDigraph,
  ...
  ret = CanonGraph(num_atoms, num_at_tg, num_max, bDigraph, NeighList,
                   (Partition *)pRankStack, nSymmRank, nCanonRank,
                   pCS->nPrevAtomNumber, pCD, pCC, NULL, &pzb_rho);
  ...
  ret = CanonGraph(num_atoms, num_at_tg, num_max, bDigraph, NeighList,
                   (Partition *)pRankStack, nSymmRank, nCanonRank,
                   pCS->nPrevAtomNumber, pCD, pCC, &pzb_rho, NULL);
...
int GetOneAdditionalLayer(CANON_DATA *pCD, ConTable *pzb_rho_fix) {
  ...
  nLayer++; /* 2 */
  if (pCD->nLenCTAtOnly < pCD->nLenLinearCT &&
      pzb_rho_fix->nLenCTAtOnly == pzb_rho_fix->lenCt) {
    nLastLayer = nLayer;
    nNumLast++;
  }
  nLayer++; /* 3 */
  ...
  if ( pCD->nLenCTAtOnly < pCD->nLenLinearCT && pCD->iso_sort_key &&
      (pzb_rho_fix->nLenCTAtOnly == pzb_rho_fix->lenCt ||
  !pzb_rho_fix->iso_sort_key ) ) { nLastLayer = nLayer; nNumLast ++;
  }
  */
#if (USE_ISO_SORT_KEY_HFIXED == 1)
  nLayer++; /* 6 */
...
int CanonGraph(int n, int n_tg, int n_max, int bDigraph, Graph *G,
  ...
  */
  ...
  int t_eq_zeta; /* ht: min{i|i=1..m && all terminal modes descended from or
                    equal to zeta(i) have been shown to be equivalent}. */
  ...
  int hz_rho;  /* hzb: max{i|i=1..min(k,r) && Lambda(G,pi,nu(i)) ==
                  Lambda(G,pi,rho(i))} */
  int hz_zeta; /* hzf: max{i|i=1..min(k,m) && Lambda(G,pi,nu(i)) ==
                  Lambda(G,pi,zeta(i))} */
  int qzb_rho; /* Ct(Lambda[k]) - Ct(rho[k]) */
  double size; /* |Aut(G)| */
  int nNumLayers =
      (NULL != pCD->NumH) + (NULL != pCD->NumHfixed) +
  ...
  int bSplitTautCompare =
      (bDigraph ||
       nNumLayers); /* compare taut. H and tgroups connections after H */
  ...
  /* best leaf/node CT: find the greatest pzb_rho possibly subject to pzb_rho[k]
   * <= pzb_rho_fix[k] condition */
  ConTable *pzb_rho =
      NULL; /* Ct for rho,   the best discovered terminal node */
  /* fixed input CT: for all k pzb_rho[k] <= pzb_rho_fix[k]; at the end pzb_rho
   * == pzb_rho_fix */
  ConTable *pzb_rho_fix =
      (pp_zb_rho_inp && *pp_zb_rho_inp) ? *pp_zb_rho_inp : NULL;
  ...
  /* num_g++;  */
  /* WriteGraph( G, n_tg, num_g,
   * "V:\\IChI_v10\\Gordon-Graphs\\hard\\k06g08v312-alt.dre", "a+" ); */
  ...
  */
  ...
  while (k) {
    ...
    {
      Node vv = v[k - 1];
      if (0 > (ret = PartitionColorVertex(G, &pi[k - 1], vv /*v[k-1]*/, n, n_tg,
                                          n_max, bDigraph, 0))) {
        goto exit_error;
      }
    }
    ...
    if (pzb_rho_fix && QZFIX_OK(qzb_rho_fix)) {
      qzb_rho_fix = CtPartCompare(&Lambda, pzb_rho_fix, qzb, kLeast_rho_fix,
                                  k - 1, 1, bSplitTautCompare);
...
L2:
  /* the two next lines intentionally switched */
  /* Create equitable partition in pi[k]  */
  if (0 > (ret = PartitionColorVertex(G, &pi[k - 1], v[k - 1], n, n_tg, n_max,
                                      bDigraph, 0))) {
    goto exit_error;
  }
  pCC->lNumBreakTies++;
  ...
  if (hz_zeta == k - 1 && 0 == CtPartCompare(&Lambda, &zf_zeta, NULL, NULL,
                                             k - 1, 0, bSplitTautCompare)) {
  ...
  if ( pzb_rho_fix && QZFIX_OK(qzb_rho_fix) ) {
      qzb_rho_fix = CtPartCompare( &Lambda, pzb_rho_fix, qzb, kLeast_rho_fix,
  k-1, 1, bSplitTautCompare ); if ( QZFIX_OK(qzb_rho_fix) ) { hzb_rho_fix = k;
      } else {
          pCC->lNumRejectedCT ++;
      }
  }
  */
  ...
  if (pzb_rho_fix && !qzb_rho_fix) {
    ...
    if (!qzb_rho_fix && bRhoIsDiscrete) {
      qzb_rho_fix = CtPartCompareLayers(kLeast_rho_fix, L_rho_fix_prev,
                                        nOneAdditionalLayer);
      ...
      if (qzb_rho_fix) {
        ...
        if (L_rho_fix_diff < L_rho_fix_prev ||
          ...
          qzb_rho_fix =
              L_rho_fix_diff + 1; /* positive difference will be rejected */
        }
      }
#endif
  ...
  if (lab && QZFIX_OK(qzb_rho_fix)) {
    /* once the difference has been found it is meaningful as long as k
     * increments */
    /* cur_qzb2 = CtPartCompare( &Lambda, pzb_rho, qzb, k-1 ); */ /* rho compare
                                                                   */
    if (hz_rho == k - 1 && !qzb_rho && bRhoIsDiscrete) {
      int qzb_rho_temp = 0;
      qzb_rho = CtPartCompare(&Lambda, pzb_rho, qzb, kLeast_rho, k - 1, 0,
                              bSplitTautCompare);
      /* old code */
      if (!qzb_rho && pzb_rho_fix_reached && nOneAdditionalLayer &&
          0 > kLeast_rho[nOneAdditionalLayer].k) {
      ...
      if (!qzb_rho && bRhoIsDiscrete) {
        ...
        if (qzb_rho) {
          ...
          if (L_rho_diff < L_rho_fix_prev ||
            ...
            qzb_rho =
                -(L_rho_diff + 1); /* negative difference will be rejected */
          }
        }
#endif
      }
  ...
  /*if ( hz_zeta == k || hz_rho == k || (lab && qzb_rho >= 0 ) ) goto L4; else
   * goto L6;*/
...
L8: /* here nu is discrete: check rho for being a bettere leaf or isomorphism */
  ...
  if (pzb_rho_fix && kLeast_rho_fix && 0 == qzb_rho_fix) {
    ...
    if (kLeast_rho_fix) {
      ...
      /* for debug only */
      qzb_rho_fix_alt =
          CtFullCompare(&Lambda, pzb_rho_fix, 1, bSplitTautCompare);
  ...
  /* !!! we should never come here if G(nu) != G(rho): CtPartCompare must be
   * enough !!! */
...
L10: /* discrete pi[k-1] && G^gamma == G */
  ...
  /* Omega[l] := mcr(gamma);
     Phi[l]   := fix(gamma);
  */
  TranspositionGetMcrAndFixSetAndUnorderedPartition(&gamma, &Omega, &Phi, n_tg,
                                                    l, &theta_from_gamma);
...
L16:
  if (t_eq_zeta == k + 1 &&
      index == CellGetNumberOfNodes(&pi[k - 1], &W[k - 1]))
    t_eq_zeta = k;
  size *= (double)index;
  /******************** <<<===== A **************************/
...
exit_function:
  ...
  *nLenCt = pzb_rho->lenCt - 1;
  ...
  pCC->lNumTotCT =
      pCC->lNumDecreasedCT + pCC->lNumRejectedCT + pCC->lNumEqualCT;
  pCC->dGroupSize = size;
...
/**********************************************************************************************
 * SetInitialRanks2: Set initial ranks in nRank according to pAtomInvariant[]
 *values Make sure enough prines have been generated.
 **********************************************************************************************/
/* Upon exit: */
/* nAtomNumber[i]: number (from 0) of an atom in the ith (from 0) position of
 * the sorted order */
/* nNewRank[i]:    initial rank of the atom[i] based on atom invariants; from 1
 * to num_atoms */
...
int SetInitialRanks2(int num_atoms, ATOM_INVARIANT2 *pAtomInvariant2,
  ...
  for (i = num_atoms - 1,
      nCurrentRank = nNewRank[nAtomNumber[i]] = (AT_RANK)num_atoms,
      nNumDiffRanks = 1;
       0 < i; i--) {
    /* Note: CompAtomInvariants2Only() in following line implicitly reads
     * pAtomInvariant2 pointed by pAtomInvariant2ForSort */
...
#define ELEM_NAME_LEN 2
  ...
  if (!bTautGroupsOnly) {
    ...
    for (i = 0; i < num_atoms; i++) {
      if (!strcmp(at[i].elname, "C")) {
        nNumCarbonAtoms++;
      } else if (!strcmp(at[i].elname, "H") || !strcmp(at[i].elname, "D") ||
    ...
    if (nNumCarbonAtoms) {
      if (nNumChemElements) {
        memmove(ChemElements + ELEM_NAME_LEN, ChemElements,
                ELEM_NAME_LEN * nNumChemElements);
      }
      ChemElements[0] = 'C';
      ChemElements[1] = ' ';
      nNumChemElements++;
    ...
    for (i = 0; i < num_atoms; i++) {
      ...
      if (bHydrogensInRanks) {
        pAtomInvariant[i].val[AT_INV_NUM_H] =
            ((t_group && at[i].endpoint > 0) ? 0 : at[i].num_H);
      }
      if (bHydrogensFixedInRanks) {
        pAtomInvariant[i].val[AT_INV_NUM_H_FIX] =
            ((t_group && at[i].endpoint > 0) ? at[i].num_H : 0);
      }
      if (!bDigraph && t_group && (i_t_group = (int)at[i].endpoint - 1) >= 0 &&
          i_t_group < num_t_groups) {
        pAtomInvariant[i].val[AT_INV_NUM_TG_ENDPOINTS] =
            t_group[i_t_group].nNumEndpoints;
        for (j = 0; j < T_NUM_NO_ISOTOPIC; j++) {
          pAtomInvariant[i].val[AT_INV_TG_NUMBERS + j] =
              t_group[i_t_group].num[j];
        }
        for (j = 0; j < num_tautomer_iso; j++) {
          pAtomInvariant[i].val[AT_INV_TAUT_ISO + j] =
              t_group[i_t_group].num[j + T_NUM_NO_ISOTOPIC];
        }
      }
  ...
  for (i = num_atoms; i < num_at_tg; i++) {
    ...
    pAtomInvariant[i].val[AT_INV_HILL_ORDER] =
        bTautGroupsOnly ? num_at_tg : nNumChemElements + 1;
    /* greater than for any real atom */
    ...
    if (k < num_t_groups) {
      ...
      for (j = 0; j < num_tautomer_iso; j++) {
        pAtomInvariant[i].val[AT_INV_TAUT_ISO + j] =
            t_group[k].num[j + T_NUM_NO_ISOTOPIC];
      }
    }
  }
}
...
/* debug: find whether canonical equivalence is different from equitable
 * partition */
int bCanonIsFinerThanEquitablePartition(int num_atoms, sp_ATOM *at,
                                        AT_RANK *nSymmRank) {
  AT_RANK *nRank = NULL;
  ...
  if (at && nSymmRank) {
    if (!(nRank = (AT_RANK *)inchi_calloc(num_atoms, sizeof(nRank[0]))) ||
        !(nAtomNumber =
              (AT_RANK *)inchi_calloc(num_atoms, sizeof(nAtomNumber[0]))) ||
        !(nTempRank =
              (AT_RANK *)inchi_calloc(num_atoms, sizeof(nTempRank[0]))) ||
        !(pAtomInvariant = (ATOM_INVARIANT2 *)inchi_calloc(
              num_atoms, sizeof(pAtomInvariant[0])))) {
      goto exit_err;
    }
    ...
    FillOutAtomInvariant2(at, num_atoms, num_atoms, pAtomInvariant,
                          1 /*bIgnoreIsotopic*/, 1 /*bHydrogensInRanks*/,
                          1 /*bHydrogensFixedInRanks*/, 0 /*bTaut=bDigraph*/,
                          0 /* bTautGroupsOnly */, NULL /*t_group_info*/);
    /* initial partitioning of a hydrogenless skeleton: create equitable
     * partition (assign initial ranks) */
    nNumCurrRanks =
        SetInitialRanks2(num_atoms, pAtomInvariant, nRank, nAtomNumber);

    lCount = 0;
    /* make equitable partition in pBCN->pRankStack[0,1] */
    nNumCurrRanks = DifferentiateRanks2(num_atoms, NeighList, nNumCurrRanks,
                                        nRank, nTempRank, nAtomNumber, &lCount,
                                        0 /* 0 means use qsort */);
    /* at this point the equitable partition is in nRank; the order of atoms is
     * in nAtomNumber*/
    /* compare */
    nCurSymm = nCurRank = 0;
    for (i = 0; i < num_atoms; i++) {
...
int GetBaseCanonRanking(int num_atoms, int num_at_tg, sp_ATOM *at[],
  ...
  int ret = 0;
  int iBase;  /* base structure index, always valid; = TAUT_YES except special
                 fully non-taut mode */
  int iOther; /* other than basic structure index, usually non-taut; may be =
                 iBase */
  int bReqNonTaut; /* 1 => requested non-tautomeric results */
  int bReqTaut; /* 1 => requested tautomeric results and the base structure is
                   tautomeric */
  int bChanged;
  ...
  if (at[TAUT_YES] && s[TAUT_YES].nLenCT && t_group_info &&
      ((s[TAUT_YES].nLenLinearCTTautomer > 0 && /* ordinary tautomerism */
       ...
       (t_group_info->nNumIsotopicEndpoints > 1 &&
        (t_group_info->bTautFlagsDone & (TG_FLAG_FOUND_ISOTOPIC_H_DONE |
                                         TG_FLAG_FOUND_ISOTOPIC_ATOM_DONE))))) {
    /* tautomeric: (1) has tautomeric atoms OR
                   (2) H-atoms have been rearranged due to proton
       addition/removal OR (3) Found isotopic H-atoms on tautomeric or hetero
       atoms
     */
    iBase = TAUT_YES;
    bReqTaut = 1;
    bUseIsoAuxBase[iBase] =
        (s[iBase].nLenIsotopicEndpoints > 1) &&
    ...
    /* although the user requested tautomeric processing, tautomerism has not
     * been found */
    /* however, the results should be saved in the TAUT_YES elements of the
     * arrays */
    iBase = TAUT_YES;
    bReqTaut = 0;
...
#if (USE_ISO_SORT_KEY_HFIXED == 1)
  num_iso_Hfixed = len_iso_sort_key_Hfixed = maxlen_iso_sort_key_Hfixed = 0;
#endif
  ...
  at_other = at[iOther];
  pAtomInvariant =
      (ATOM_INVARIANT2 *)inchi_calloc(num_max, sizeof(pAtomInvariant[0]));
  ...
  nAtomNumberCanonNoH =
      (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonNoH[0]));
  ...
  if (!pAtomInvariant || !nSymmRankNoH || !nCanonRankNoH ||
      !nAtomNumberCanonNoH || !nRank || !nAtomNumber || !nTempRank) {
  ...
  pAtomInvariantAux =
      (ATOM_INVARIANT2 *)inchi_malloc(num_max * sizeof(pAtomInvariantAux[0]));
  ...
  */
  ...
  pBCN->num_at_tg =
      num_at_tg; /* all of the following arrays have this length */
  ...
  FillOutAtomInvariant2(at_base, num_atoms, num_atoms, pAtomInvariant,
                        1 /*bIgnoreIsotopic*/, 0 /*bHydrogensInRanks*/,
                        0 /*bHydrogensFixedInRanks*/, 0 /*bTaut=bDigraph*/,
                        0 /* bTautGroupsOnly */, NULL /*t_group_info*/);
  /* initial partitioning of a hydrogenless skeleton: create equitable partition
  ...
  lCount = 0;
  /* make equitable partition in pBCN->pRankStack[0,1] */
  nNumCurrRanks = DifferentiateRanks2(
      num_atoms, NeighList[TAUT_NON], nNumCurrRanks, nRank, nTempRank,
      nAtomNumber, &lCount, 0 /* 0 means use qsort */);

  /* allocate partition stack */
  nMaxLenRankStack = 2 * (num_at_tg - nNumCurrRanks) + 8; /* was 2*(...) + 6 */
  pBCN->pRankStack =
      (AT_RANK **)inchi_calloc(nMaxLenRankStack, sizeof(pBCN->pRankStack[0]));
  if (!pBCN->pRankStack) {
    pBCN->nMaxLenRankStack = 0; /* avoid memory leaks in case of error */
    goto exit_error_alloc;
  }
  pBCN->nMaxLenRankStack =
      nMaxLenRankStack; /* avoid memory leaks in case of error */
  /* init partition stack */
  pBCN->pRankStack[0] = nRank;
  pBCN->pRankStack[1] = nAtomNumber;

  /********************************************************************************************/
  /* get NoH/no taut groups  canonical numbering, connection table, and
...
#endif
  ret = CanonGraph01(num_atoms, num_atoms, num_max, 0, NeighList[TAUT_NON],
                     (Partition *)pBCN->pRankStack, nSymmRankNoH, nCanonRankNoH,
                     nAtomNumberCanonNoH, pCD + iOther, pCC, NULL, &Ct_NoH);
  if (ret < 0) {
    goto exit_error;
  }
  /* update initial partitioning */
  nNumCurrRanks = FixCanonEquivalenceInfo(num_atoms, nSymmRankNoH, nRank,
                                          nTempRank, nAtomNumber, &bChanged);
  ...
  if (bChanged & 3) {
    ...
    ret =
        CanonGraph02(num_atoms, num_atoms, num_max, 0, NeighList[TAUT_NON],
                     (Partition *)pBCN->pRankStack, nSymmRankNoH, nCanonRankNoH,
                     nAtomNumberCanonNoH, pCD + iOther, pCC, NULL, &Ct_NoH);
    if (ret < 0) {
      goto exit_error;
  ...
  /********************************************************************************/
  /* get NoTautH canonical numbering, connection table, and equivalence
   * partition */
  /********************************************************************************/
  maxlenNumHNoTautH = num_atoms + 1;
  nSymmRankNoTautH =
      (AT_RANK *)inchi_calloc(num_max, sizeof(nSymmRankNoTautH[0]));
  nCanonRankNoTautH =
      (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankNoTautH[0]));
  nAtomNumberCanonNoTautH =
      (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonNoTautH[0]));
  numHNoTautH =
      (NUM_H *)inchi_calloc(maxlenNumHNoTautH, sizeof(numHNoTautH[0]));
  if (!numHNoTautH || !nSymmRankNoTautH || !nCanonRankNoTautH ||
      !nAtomNumberCanonNoTautH) {
    goto exit_error_alloc;
  }
  /* find number of H atoms attached to not-a-tautomeric-endpoint atoms */
  for (i = 0; i < num_atoms; i++) {
    numHNoTautH[i] = (!at_base[i].endpoint && at_base[i].num_H)
                         ? at_base[i].num_H + BASE_H_NUMBER
                         : EMPTY_H_NUMBER;
  }
  /* pointers */
  pCD[iOther].LinearCT = NULL;
  ...
  for (i = 0; i < num_atoms; i++) {
    if (numHNoTautH[i] != numHNoTautH[nTempRank[nSymmRankNoH[i] - 1]]) {
      ...
      break; /* atoms so far found to be equivalent have different number of H;
                the canonicalization is needed */
    }
  }
  ...
  if (i < num_atoms) {
    ...
    for (i = 0; i < num_atoms; i++) {
      ...
      pAtomInvariantAux[i].val[1] =
          numHNoTautH[i]; /* additional differentiation: not-a-taut-H
                             distribution */
    }
    /* initial partitioning */
    nNumCurrRanks = SetInitialRanks2(num_atoms, pAtomInvariantAux, nRankAux,
                                     nAtomNumberAux);
    /* make equitable partition */
    nNumCurrRanks = DifferentiateRanks2(
        num_atoms, NeighList[TAUT_NON], nNumCurrRanks, nRankAux, nTempRank,
        nAtomNumberAux, &lCount, 0 /* 0 means use qsort */);
    /* to accelerate do not call CanonGraph() to find really equivalent atoms */
    pCD[iOther].nAuxRank = nRankAux;
#endif

    ret = CanonGraph03(num_atoms, num_atoms, num_max, 1 /* digraph?? was 0 */,
                       NeighList[TAUT_NON], (Partition *)pBCN->pRankStack,
                       nSymmRankNoTautH, nCanonRankNoTautH,
                       nAtomNumberCanonNoTautH, pCD + iOther, pCC, &Ct_NoH,
                       &Ct_NoTautH);
    if (ret < 0) {
      goto exit_error;
    }
    ...
    */
    ...
    if ((Ct_Temp = (ConTable *)inchi_calloc(1, sizeof(*Ct_Temp))) &&
      ...
      /* since Ct_NoH does not have Ct_NoH->NumH we have to fill out
       * Ct_Temp->NumH separately */
      for (i = 0; i < num_atoms; i++) {
    ...
    Ct_NoTautH = Ct_Temp;
    Ct_Temp = NULL;
    memcpy(nSymmRankNoTautH, nSymmRankNoH,
           num_atoms * sizeof(nSymmRankNoTautH[0]));
    memcpy(nCanonRankNoTautH, nCanonRankNoH,
           num_atoms * sizeof(nCanonRankNoTautH[0]));
    memcpy(nAtomNumberCanonNoTautH, nAtomNumberCanonNoH,
           num_atoms * sizeof(nAtomNumberCanonNoTautH[0]));
  }
  ...
  /* get isotopic canonical numbering, connection table, and equivalence
   * partition           */
  /*******************************************************************************************/

  if (s[iOther].num_isotopic_atoms && !s[iOther].bIgnoreIsotopic && !bReqTaut &&
      bReqNonTaut) {

    maxlen_iso_sort_key_NoTautH = num_atoms + 1;
    nSymmRankNoTautHIso =
        (AT_RANK *)inchi_calloc(num_max, sizeof(nSymmRankNoTautHIso[0]));
    nCanonRankNoTautHIso =
        (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankNoTautHIso[0]));
    nAtomNumberCanonNoTautHIso =
        (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonNoTautHIso[0]));
    iso_sort_key_NoTautH = (AT_ISO_SORT_KEY *)inchi_calloc(
        maxlen_iso_sort_key_NoTautH, sizeof(iso_sort_key_NoTautH[0]));

    if (!nSymmRankNoTautHIso || !nCanonRankNoTautHIso ||
        !nAtomNumberCanonNoTautHIso || !iso_sort_key_NoTautH) {
      goto exit_error_alloc;
    }
    ...
    for (i = 0; i < num_atoms; i++) {
      if (at_base[i].endpoint) {
        ...
        iso_sort_key =
            make_iso_sort_key(at_base[i].iso_atw_diff, at_base[i].num_iso_H[0],
    ...
    if (num_iso_NoTautH) {
      ...
      for (i = 0; i < num_atoms; i++) {
        if (nTempRank[nSymmRankNoTautH[i] - 1] < i) {
          nTempRank[nSymmRankNoTautH[i] - 1] =
              i; /* greatest class representative */
        }
      }
      for (i = 0; i < num_atoms; i++) {
        if (iso_sort_key_NoTautH[i] !=
            iso_sort_key_NoTautH[nTempRank[nSymmRankNoTautH[i] - 1]]) {
          pCD[iOther].nCanonFlags |= CANON_FLAG_ISO_ONLY_NON_TAUT_DIFF;
          break; /* atoms so far found to be equivalent differ in isotopes; the
    ...
    if (i < num_atoms) {
      ...
      /* get aux canonical ranking of the structure with isotopic non-tautomeric
       * H */

#if (USE_AUX_RANKING == 1)
      /* refine no-taut-H partition according to non-taut H isotopic
       * distribution */
      memset(pAtomInvariantAux, 0, num_max * sizeof(pAtomInvariantAux[0]));
      for (i = 0; i < num_atoms; i++) {
        pAtomInvariantAux[i].val[0] = nSymmRankNoTautH[i];
        pAtomInvariantAux[i].iso_sort_key =
            iso_sort_key_NoTautH[i]; /* additional differentiation */
      }
      ...
      ret = CanonGraph04(num_atoms, num_atoms, num_max, 1 /* digraph?? was 0 */,
                         NeighList[TAUT_NON], (Partition *)pBCN->pRankStack,
                         nSymmRankNoTautHIso, nCanonRankNoTautHIso,
                         nAtomNumberCanonNoTautHIso, pCD + iOther, pCC,
      ...
      */
      ...
      /* in this case numHNoTautH[] is not needed for the next
       * canonicalization(s) */
      if ((Ct_Temp = (ConTable *)inchi_calloc(1, sizeof(*Ct_Temp))) &&
          CTableCreate(Ct_Temp, num_atoms, pCD + iOther)) {
        CtFullCopy(Ct_Temp, Ct_NoTautH);
        /* since Ct_NoTautH does not have Ct_NoTautH->iso_sort_key we have to
         * fill out Ct_Temp->iso_sort_key separately */
        for (i = 0; i < num_atoms; i++) {
          Ct_Temp->iso_sort_key[nCanonRankNoTautH[i] - 1] =
              iso_sort_key_NoTautH[i];
        }
      ...
      Ct_Temp = NULL;
      memcpy(nSymmRankNoTautHIso, nSymmRankNoTautH,
             num_atoms * sizeof(nSymmRankNoTautHIso[0]));
      memcpy(nCanonRankNoTautHIso, nCanonRankNoTautH,
             num_atoms * sizeof(nCanonRankNoTautHIso[0]));
  ...
  if (bReqTaut) {
     ...
     */
    /*****************************************************************************/
    /* find H atoms attached to non-tautomeric-endpoints and to tautomeric
     * endpoints */
    maxlenNumH = num_atoms + T_NUM_NO_ISOTOPIC * (num_at_tg - num_atoms) +
                 1; /* including negative charges */
    nSymmRankBase = (AT_RANK *)inchi_calloc(num_max, sizeof(nSymmRankBase[0]));
    nCanonRankBase =
        (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankBase[0]));
    nAtomNumberCanonBase =
        (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonBase[0]));
    ...
    for (i = 0; i < num_atoms; i++) {
      numH[i] = (!at_base[i].endpoint && at_base[i].num_H)
                    ? at_base[i].num_H + BASE_H_NUMBER
                    : EMPTY_H_NUMBER;
    }
    ...
    for (i = k = num_atoms; i < num_at_tg; i++) {
      m = i - num_atoms;
      for (j = 0; j < T_NUM_NO_ISOTOPIC; j++) {
        /* non-zeroes for j=1 are negative charge counts; T_NUM_NO_ISOTOPIC=2
         * entry per t-group */
        numH[k++] = t_group_info->t_group[m].num[j]
                        ? t_group_info->t_group[m].num[j] + BASE_H_NUMBER
                        : EMPTY_H_NUMBER;
      }
    }
    /* pointers */
    pCD[iBase].LinearCT = NULL;
    pCD[iBase].NumH = numH; /* num_atoms non-tautomeric H; num_tg pairs of H and
                               (-) in t-groups */
    ...
    /* make sure the initial partition is equitable (at this point t-groups do
     * not have ranks yet) */
    FillOutAtomInvariant2(at_base, num_atoms, num_at_tg, pAtomInvariant,
                          1 /*bIgnoreIsotopic*/, 0 /*bHydrogensInRanks*/,
                          0 /*bHydrogensFixedInRanks*/, 1 /*bTaut=bDigraph*/,
    ...
    nNumCurrRanks =
        SetInitialRanks2(num_at_tg, pAtomInvariant, nRank, nAtomNumber);
    ...
    nNumCurrRanks =
        DifferentiateRanks4(num_at_tg, NeighList[TAUT_YES], nNumCurrRanks,
                            pBCN->pRankStack[0], nTempRank /* temp array */,
    ...
    nNumCurrRanks = SetInitialRanks2(num_at_tg, pAtomInvariantAux, nRankAux,
                                     nAtomNumberAux);
    /* make equitable, call digraph procedure */
    nNumCurrRanks =
        DifferentiateRanks4(num_at_tg, NeighList[TAUT_YES], nNumCurrRanks,
                            nRankAux, nTempRank /* temp array */,
                            nAtomNumberAux, (AT_RANK)num_atoms, &lCount);
    /* to accelerate do not call CanonGraph() to find really equivalent atoms */
    pCD[iBase].nAuxRank = nRankAux;
#endif

    ret = CanonGraph05(num_atoms, num_at_tg, num_max, 1 /* digraph*/,
                       NeighList[TAUT_YES], (Partition *)pBCN->pRankStack,
                       nSymmRankBase, nCanonRankBase, nAtomNumberCanonBase,
                       pCD + iBase, pCC, &Ct_NoTautH, &Ct_Base);
    if (ret < 0) {
      goto exit_error;
    }
    ...
    if ((s[iBase].num_isotopic_atoms && !s[iBase].bIgnoreIsotopic) ||
      ...
      nSymmRankBaseIso =
          (AT_RANK *)inchi_calloc(num_max, sizeof(nSymmRankBaseIso[0]));
      nCanonRankBaseIso =
          (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankBaseIso[0]));
      nAtomNumberCanonBaseIso =
          (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonBaseIso[0]));
      if (bUseIsoAuxBase[iBase]) {
        maxlen_iso_exchg_atnos = num_max + 1;
        iso_exchg_atnos = (S_CHAR *)inchi_calloc(maxlen_iso_exchg_atnos,
                                                 sizeof(iso_exchg_atnos[0]));
      }
      maxlen_iso_sort_keyBase = num_max + 1; /* num_at_tg+1;*/
      iso_sort_keyBase = (AT_ISO_SORT_KEY *)inchi_calloc(
          maxlen_iso_sort_keyBase, sizeof(iso_sort_keyBase[0]));
      if (!nSymmRankBaseIso || !nCanonRankBaseIso || !nAtomNumberCanonBaseIso ||
          !iso_sort_keyBase || (maxlen_iso_exchg_atnos && !iso_exchg_atnos)) {
        goto exit_error_alloc;
      }
      /* atoms */
      num_iso_NoTautH = 0;
      ...
      for (i = 0; i < num_atoms; i++) {
        if (at_base[i].endpoint ||
            (iso_exchg_atnos && (at_base[i].cFlags & AT_FLAG_ISO_H_POINT))) {
          /* tautomeric or may have exchangeable isotopic H */
          iso_sort_key = make_iso_sort_key(at_base[i].iso_atw_diff, 0, 0, 0);
          if (iso_exchg_atnos) {
            num_iso_NoAuxBase +=
                !at_base[i].endpoint; /* these non-taut atom may exchange
                                         isotopic H as tautomeric atoms do */
          }
        } else {
          /* non-mobile H */
          iso_sort_key = make_iso_sort_key(
              at_base[i].iso_atw_diff, at_base[i].num_iso_H[0],
              at_base[i].num_iso_H[1], at_base[i].num_iso_H[2]);
          if (iso_exchg_atnos) {
            iso_exchg_atnos[i] =
                1; /* atom cannot have exchangable isotopic H atom(s) */
          }
        }
        if (iso_sort_key) {
          num_iso_NoTautH++;
      ...
      /* check marking and count of non-taut atoms that may exchange isotopic H
       * -- debug only */
      if (iso_exchg_atnos) {
        ...
        for (i = 1; i <= num_iso_NoAuxBase; i++) {
          ...
          if (at_base[j].endpoint ||
              !(at_base[j].cFlags & AT_FLAG_ISO_H_POINT)) {
            ret = CT_ISOCOUNT_ERR;
            goto exit_error;
          }
        }
      ...
      if (iso_exchg_atnos) {
        for (i = num_atoms; i < num_at_tg; i++) {
          iso_sort_keyBase[i] =
              EMPTY_ISO_SORT_KEY; /* new mode: do not provide info about
                                     isotopic tautomeric H */
        }
      } else {
        for (i = num_atoms; i < num_at_tg;
             i++) { /* should not happen anymore */
          m = i - num_atoms;
      ...
      if (!num_iso_NoAuxBase && iso_exchg_atnos) {
        /* all atoms that may exchange isotopic H are either tautomeric or not
         * present */
      ...
      if (!iso_exchg_atnos && !iso_sort_keyBase) {
        ...
        /* pointers */
        pCD[iBase].LinearCT = NULL;
        pCD[iBase].NumH = numH; /* num_atoms non-tautomeric H; num_tg pairs of H
                                   and (-) in t-groups */
        pCD[iBase].NumHfixed = NULL;
        ...
        if (num_iso_NoTautH || num_iso_Base || num_iso_NoAuxBase) {
          ...
          for (i = 0; i < num_at_tg; i++) {
            if ((iso_sort_keyBase
                     ? (iso_sort_keyBase[i] !=
                        iso_sort_keyBase[nTempRank[nSymmRankBase[i] - 1]])
                     : 0) ||
                (iso_exchg_atnos
                     ? (iso_exchg_atnos[i] !=
                        iso_exchg_atnos[nTempRank[nSymmRankBase[i] - 1]])
                     : 0)) {
              pCD[iBase].nCanonFlags |= CANON_FLAG_ISO_TAUT_DIFF;
              break; /* atoms so far found to be equivalent have different
                        number of H; the canonicalization is needed */
            }
          }
        } else {
        ...
        if (i < num_at_tg) {
          /* we need canonicalization */
          /* get aux canonical ranking of the structure with isotopic
           * non-tautomeric H */

#if (USE_AUX_RANKING == 1)
          /* refine no-taut-H partition according to non-taut H + t-groups
           * isotopic distribution */
          memset(pAtomInvariantAux, 0, num_max * sizeof(pAtomInvariantAux[0]));
          for (i = 0; i < num_at_tg; i++) {
            pAtomInvariantAux[i].val[0] = nSymmRankBase[i];
            pAtomInvariantAux[i].iso_sort_key =
                iso_sort_keyBase ? iso_sort_keyBase[i]
                                 : 0; /* additional differentiation */
            pAtomInvariantAux[i].iso_aux_key =
                iso_exchg_atnos ? iso_exchg_atnos[i] : 0;
          }
          /* initial ranks for non-taut H isotopic distribution */
          nNumCurrRanks = SetInitialRanks2(num_at_tg, pAtomInvariantAux,
                                           nRankAux, nAtomNumberAux);
          /* make equitable, not a digraph procedure */
          nNumCurrRanks =
              DifferentiateRanks2(num_at_tg, NeighList[TAUT_YES], nNumCurrRanks,
                                  nRankAux, nTempRank, nAtomNumberAux, &lCount,
                                  0 /* 0 means first use qsort */);
          /* to accelerate do not call CanonGraph() to find really equivalent
           * atoms */
          pCD[iBase].nAuxRank = nRankAux;
#endif

          ret = CanonGraph06(num_atoms, num_at_tg, num_max, 1 /* digraph */,
                             NeighList[TAUT_YES], (Partition *)pBCN->pRankStack,
                             nSymmRankBaseIso, nCanonRankBaseIso,
                             nAtomNumberCanonBaseIso, pCD + iBase, pCC,
                             &Ct_Base, &Ct_BaseIso);
          if (ret < 0) {
            goto exit_error;
          }
          /* in case of a tautomeric structure the final results are in:
                     ...
                     nAtomNumberCanonBaseIso
                     Ct_BaseIso
                     iso_sort_keyBase (original isotopic atom & t-group
             positions) Ct_BaseIso->iso_exchg_atnos: 0=>can exchange isotopic H,
             including tautomeric atoms iso_exchg_atnos            : same, in
             order of t_group_info->nIsotopicEndpointAtomNumber[]
          */
        } else {
          /* copy the results of the previous (no taut H) canonicalization */
          /* in this case numHNoTautH[] is not needed for the next
          ...
          if ((Ct_Temp = (ConTable *)inchi_calloc(1, sizeof(*Ct_Temp))) &&
            ...
            if (iso_sort_keyBase) {
              for (i = 0; i < num_at_tg; i++) {
                Ct_Temp->iso_sort_key[nCanonRankBase[i] - 1] =
                    iso_sort_keyBase[i];
              }
            ...
            if (iso_exchg_atnos) {
              for (i = 0; i < num_atoms; i++) {
                Ct_Temp->iso_exchg_atnos[nCanonRankBase[i] - 1] =
                    iso_exchg_atnos[i];
              }
          ...
          memcpy(nSymmRankBaseIso, nSymmRankBase,
                 num_at_tg * sizeof(nSymmRankBaseIso[0]));
          memcpy(nCanonRankBaseIso, nCanonRankBase,
                 num_at_tg * sizeof(nCanonRankBaseIso[0]));
  ...
  /**********************************************************************************/
  /* get "fixed H" canonical numbering, connection table, and equivalence
   * partition */
  /**********************************************************************************/
  ...
  if (bReqTaut && bReqNonTaut) {
    ...
    nCanonRankFixH =
        (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankFixH[0]));
    nAtomNumberCanonFixH =
        (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonFixH[0]));
    NumHfixed = (NUM_H *)inchi_calloc(maxlenNumHfixed, sizeof(NumHfixed[0]));
    if (!NumHfixed || !nSymmRankFixH || !nCanonRankFixH ||
        !nAtomNumberCanonFixH) {
      goto exit_error_alloc;
    }
    for (i = 0; i < num_atoms; i++) {
      ...
      if (at_base[i].endpoint) {
        NumHfixed[i] = at_other[i].num_H ? at_other[i].num_H + BASE_H_NUMBER
                                         : EMPTY_H_NUMBER;
      } else if (at_other[i].num_H != at_base[i].num_H) {
        NumHfixed[i] =
            (NUM_H)at_other[i].num_H - (NUM_H)at_base[i].num_H + BASE_H_NUMBER;
      } else {
    ...
    if (!nAtomNumberAux)
      nAtomNumberAux =
          (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberAux[0]));
    if (!pAtomInvariantAux)
      pAtomInvariantAux = (ATOM_INVARIANT2 *)inchi_malloc(
          num_max * sizeof(pAtomInvariantAux[0]));
    ...
    for (i = 0; i < num_atoms; i++) {
      pAtomInvariantAux[i].val[0] = nSymmRankBase[i];
      pAtomInvariantAux[i].val[1] =
          NumHfixed[i]; /* additional differentiation */
    }
    ...
    ret = CanonGraph07(num_atoms, num_atoms, num_max, 0, NeighList[TAUT_NON],
                       (Partition *)pBCN->pRankStack, nSymmRankFixH,
                       nCanonRankFixH, nAtomNumberCanonFixH, pCD + iOther, pCC,
                       &Ct_NoTautH, &Ct_FixH);
    if (ret < 0) {
      goto exit_error;
    }
    ...
    /*******************************************************************************************/
    /* get "fixed H" isotopic canonical numbering, connection table, and
     * equivalence partition */
    /*******************************************************************************************/
    iflag = (s[iBase].num_isotopic_atoms && !s[iBase].bIgnoreIsotopic) ||
            (s[iBase].bHasIsotopicTautGroups && !bTautIgnoreIsotopic);
    if (bFixIsoFixedH) /* #if ( FIX_ISO_FIXEDH_BUG == 1 )  */
      /* fix bug when iso H was removed as a proton and fixed-H isotopic layer
       * is missing -  2008-09-24 DT*/
      iflag =
          iflag || (s[iOther].num_isotopic_atoms && !s[iOther].bIgnoreIsotopic);
    if (iflag) {
      ...
      maxlen_iso_sort_key_Hfixed =
          ...
          maxlen_iso_sort_key_NoTautH = num_atoms + 1;
      nSymmRankFixHIso =
          (AT_RANK *)inchi_calloc(num_max, sizeof(nSymmRankFixHIso[0]));
      nCanonRankFixHIso =
          (AT_RANK *)inchi_calloc(num_max, sizeof(nCanonRankFixHIso[0]));
      nAtomNumberCanonFixHIso =
          (AT_NUMB *)inchi_calloc(num_max, sizeof(nAtomNumberCanonFixHIso[0]));
#if (USE_ISO_SORT_KEY_HFIXED == 1)
      iso_sort_key_Hfixed = (AT_ISO_SORT_KEY *)inchi_calloc(
          maxlen_iso_sort_key_Hfixed, sizeof(iso_sort_key_Hfixed[0]));
#endif
      iso_sort_key_NoTautH = (AT_ISO_SORT_KEY *)inchi_calloc(
          maxlen_iso_sort_key_NoTautH, sizeof(iso_sort_key_NoTautH[0]));
      ...
      for (i = 0; i < num_atoms; i++) {
        if (at_base[i].endpoint) {
          iso_sort_key = make_iso_sort_key(at_base[i].iso_atw_diff, 0, 0, 0);
          iso_sort_key2 = make_iso_sort_key(0, at_base[i].num_iso_H[0],
                                            at_base[i].num_iso_H[1],
                                            at_base[i].num_iso_H[2]);
        } else {
          iso_sort_key = make_iso_sort_key(
              at_base[i].iso_atw_diff, at_base[i].num_iso_H[0],
              at_base[i].num_iso_H[1], at_base[i].num_iso_H[2]);
      ...
      for (i = 0; i < num_atoms; i++) {
        ...
        if (bFixIsoFixedH) /* #if ( FIX_ISO_FIXEDH_BUG == 1 )  */
        {
          /* fix bug when iso H was removed as a proton and fixed-H isotopic
           * layer is missing -  2008-09-24 DT*/
          if (at_other) {
            iso_sort_key = make_iso_sort_key(
                at_other[i].iso_atw_diff, at_other[i].num_iso_H[0],
...
#endif
      {
        ...
        for (i = 0; i < num_atoms; i++) {
          if (nTempRank[nSymmRankFixH[i] - 1] < i) {
            nTempRank[nSymmRankFixH[i] - 1] =
                i; /* greatest class representative */
          }
        }
        for (i = 0; i < num_atoms; i++) {
#if (USE_ISO_SORT_KEY_HFIXED == 1)
          if (iso_sort_key_Hfixed[i] !=
              iso_sort_key_Hfixed[nTempRank[nSymmRankFixH[i] - 1]])
            break;
#endif
          if (iso_sort_key_NoTautH[i] !=
              iso_sort_key_NoTautH[nTempRank[nSymmRankFixH[i] - 1]])
            break; /* atoms so far found to be equivalent have different
                      isotopic shifts; the canonicalization is needed */
        }
      } else {
      ...
      if (i < num_atoms) {
...
#endif
          pAtomInvariantAux[i].iso_sort_key =
              iso_sort_key; /* additional differentiation */
        }
        ...
        ret = CanonGraph08(num_atoms, num_atoms, num_max,
                           1 /* digraph?? was 0 */, NeighList[TAUT_NON],
                           (Partition *)pBCN->pRankStack, nSymmRankFixHIso,
                           nCanonRankFixHIso, nAtomNumberCanonFixHIso,
                           pCD + iOther, pCC, &Ct_FixH, &Ct_FixHIso);
        if (ret < 0) {
          goto exit_error;
        }
        /* in case of non-tautomeric structure the final results are in:
                   ...
                   nAtomNumberCanonFixHIso
                   Ct_FixHIso
                   iso_sort_keyBase     ([0..num_atoms] original isotopic atom
           positions) iso_sort_key_Hfixed  (original fixed tautomeric H
           distribution)
        */
      } else {
        /* copy the results of the previous (no taut H) canonicalization */
        ...
        if ((Ct_Temp = (ConTable *)inchi_calloc(1, sizeof(*Ct_Temp))) &&
          ...
          /* since Ct_FixH does not have Ct_FixH->iso_sort_key and
             Ct_FixH->iso_sort_key_Hfixed we have to fill out
             Ct_Temp->iso_sort_key and Ct_Temp->iso_sort_key_Hfixed separately
           */
          for (i = 0; i < num_atoms; i++) {
            Ct_Temp->iso_sort_key[nCanonRankFixH[i] - 1] =
                iso_sort_key_NoTautH[i];
#if (USE_ISO_SORT_KEY_HFIXED == 1)
            Ct_Temp->iso_sort_key_Hfixed[nCanonRankFixH[i] - 1] =
                iso_sort_key_Hfixed[i];
#endif
          }
          Ct_Temp->len_iso_sort_key = num_atoms;
#if (USE_ISO_SORT_KEY_HFIXED == 1)
        ...
        Ct_FixHIso = Ct_Temp;
        Ct_Temp = NULL;
        memcpy(nSymmRankFixHIso, nSymmRankFixH,
               num_atoms * sizeof(nSymmRankFixHIso[0]));
        memcpy(nCanonRankFixHIso, nCanonRankFixH,
               num_atoms * sizeof(nCanonRankFixHIso[0]));
        memcpy(nAtomNumberCanonFixHIso, nAtomNumberCanonFixH,
               num_atoms * sizeof(nAtomNumberCanonFixHIso[0]));
      }
      /* in case of non-tautomeric component this is the final result */
      /* i = CtFullCompare( Ct_NoTautHIso, Ct_Temp, num_atoms, 0, 0 );*/
    }
  ...
  /* consistency check: compare canonical connection tables, H-atoms, isotopic H
   * & taut groups */
  ret = 0;
  ret |= (Ct_NoH->lenCt != Ct_NoTautH->lenCt) ||
         memcmp(Ct_NoH->Ctbl, Ct_NoTautH->Ctbl,
                Ct_NoH->lenCt * sizeof(Ct_NoH->Ctbl[0]));
  if (bReqTaut) {
    if (Ct_FixH) {
      ret |= (Ct_NoTautH->lenCt != Ct_FixH->lenCt) ||
             memcmp(Ct_NoTautH->Ctbl, Ct_FixH->Ctbl,
                    Ct_NoTautH->lenCt * sizeof(Ct_NoTautH->Ctbl[0]));
      ret |= (Ct_NoTautH->lenNumH != Ct_FixH->lenNumH) ||
             memcmp(Ct_NoTautH->NumH, Ct_FixH->NumH,
                    Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
    }
    ret |= (Ct_NoTautH->lenCt > Ct_Base->lenCt) ||
           memcmp(Ct_NoTautH->Ctbl, Ct_Base->Ctbl,
                  Ct_NoTautH->lenCt * sizeof(Ct_NoTautH->Ctbl[0]));
    ret |= (Ct_NoTautH->lenNumH > Ct_Base->lenNumH) ||
           memcmp(Ct_NoTautH->NumH, Ct_Base->NumH,
                  Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
  }
  ...
  if (Ct_NoTautHIso) {
    ret |= (Ct_NoH->lenCt != Ct_NoTautHIso->lenCt) ||
           memcmp(Ct_NoH->Ctbl, Ct_NoTautHIso->Ctbl,
                  Ct_NoH->lenCt * sizeof(Ct_NoH->Ctbl[0]));
    ret |= (Ct_NoTautH->lenNumH != Ct_NoTautHIso->lenNumH) ||
           memcmp(Ct_NoTautH->NumH, Ct_NoTautHIso->NumH,
                  Ct_NoTautH->lenNumH * sizeof(Ct_Base->NumH[0]));
  } else if (Ct_BaseIso) {
    ret |= (Ct_BaseIso->lenCt != Ct_Base->lenCt) ||
           memcmp(Ct_BaseIso->Ctbl, Ct_Base->Ctbl,
                  Ct_BaseIso->lenCt * sizeof(Ct_BaseIso->Ctbl[0]));
    ret |= (Ct_BaseIso->lenNumH != Ct_Base->lenNumH) ||
           memcmp(Ct_BaseIso->NumH, Ct_Base->NumH,
                  Ct_BaseIso->lenNumH * sizeof(Ct_BaseIso->NumH[0]));
    if (Ct_FixHIso) {
      ret |= (Ct_FixHIso->lenCt > Ct_BaseIso->lenCt) ||
             memcmp(Ct_FixHIso->Ctbl, Ct_BaseIso->Ctbl,
                    Ct_FixHIso->lenCt * sizeof(Ct_FixHIso->Ctbl[0]));
      ret |= (Ct_FixHIso->lenNumH > Ct_BaseIso->lenNumH) ||
             memcmp(Ct_FixHIso->NumH, Ct_BaseIso->NumH,
                    Ct_FixHIso->lenNumH * sizeof(Ct_BaseIso->NumH[0]));
    }
  }
  ...
  if (bReqTaut) { /* tautomeric results */
    ...
    if (Ct_BaseIso) {
      ...
      CleanIsoSortKeys(pBCN->ftcn[TAUT_YES].iso_sort_keys,
                       pBCN->ftcn[TAUT_YES].len_iso_sort_keys);
      CleanIsoSortKeys(pBCN->ftcn[TAUT_YES].iso_sort_keysOrig,
                       pBCN->ftcn[TAUT_YES].len_iso_sort_keys);
    }
  ...
  if (bReqNonTaut) { /* non-tautomeric results */
    /* TAUT_NON if tautomeric + non-tautomeric or special non-taut request
       TAUT_YES if the structure happened to be non-tautomeric while user
       requested tautomeric processing In both cases the correct index is
       iOther. TAUT_NON replaced with iOther 4-2-2004 */
    ...
    if (!bReqTaut) {
      ...
      if (iOther == TAUT_YES && pBCN->ftcn[TAUT_NON].NeighList &&
          !pBCN->ftcn[TAUT_YES].NeighList) {
        /* here only non-taut results go to pBCN->ftcn[TAUT_YES]
           Since non-taut NeighList is always in pBCN->ftcn[TAUT_NON].NeighList,
           move it to pBCN->ftcn[TAUT_YES].NeighList. 2004-04-02.
        */
    ...
    Ct_FixH->NumH = NULL;
    pBCN->ftcn[iOther].nLenNumH =
        inchi_min(maxlenNumHNoTautH, Ct_FixH->maxlenNumH);
...
#if (USE_ISO_SORT_KEY_HFIXED == 1)
      MergeCleanIsoSortKeys(pBCN->ftcn[iOther].iso_sort_keys,
                            Ct_FixHIso->iso_sort_key_Hfixed,
                            pBCN->ftcn[iOther].len_iso_sort_keys);
      MergeCleanIsoSortKeys(pBCN->ftcn[iOther].iso_sort_keysOrig,
                            iso_sort_key_Hfixed,
                            pBCN->ftcn[iOther].len_iso_sort_keys);
#else
      CleanIsoSortKeys(pBCN->ftcn[iOther].iso_sort_keys,
                       pBCN->ftcn[iOther].len_iso_sort_keys);
      CleanIsoSortKeys(pBCN->ftcn[iOther].iso_sort_keysOrig,
                       pBCN->ftcn[iOther].len_iso_sort_keys);
#endif
    }
...
#define FREE_ARRAY(X)                                                          \
  ...
  if (pBCN->pRankStack) {
    pBCN->pRankStack[0] = pBCN->pRankStack[1] = NULL;
  }
