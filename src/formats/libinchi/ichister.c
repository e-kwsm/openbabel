...
#define MIN_SINE                                                               \
  0.03 /*  min edge/plane angle in case the tetrahedra has significantly       \
          different edge length */
#define MIN_ANGLE_DBOND                                                        \
  0.087156 /* 5 degrees = max angle considered as too small for unambiguous    \
              double bond stereo */
#define MIN_SINE_OUTSIDE                                                       \
  0.06 /*  min edge/plane angle to determine whether the central atom is       \
          outside of the tetrahedra */
#define MIN_SINE_SQUARE                                                        \
  0.125 /*  min edge/plane angle in case the tetrahedra is somewhat close to a \
           parallelogram */
#define MIN_SINE_EDGE                                                          \
  0.167 /*  min sine/(min.edge) ratio to avoid undefined in case of long edges \
         */
#define MIN_LEN_STRAIGHT                                                       \
  1.900 /*  min length of two normalized to 1 bonds in a straight line */
...
#define MAX_EDGE_RATIO                                                         \
  2.50 /*  max max/min edge ratio for a tetrahedra close to a parallelogram */
#else
#define MPY_SINE 3.00
#define MAX_EDGE_RATIO                                                         \
  6.00 /*  max max/min edge ratio for a tetrahedra close to a parallelogram */
#endif
/*  local prototypes */
static int
save_a_stereo_bond(int z_prod, int result_action, int at1, int ord1,
                   AT_NUMB *stereo_bond_neighbor1, S_CHAR *stereo_bond_ord1,
                   S_CHAR *stereo_bond_z_prod1, S_CHAR *stereo_bond_parity1,
                   int at2, int ord2, AT_NUMB *stereo_bond_neighbor2,
                   S_CHAR *stereo_bond_ord2, S_CHAR *stereo_bond_z_prod2,
                   S_CHAR *stereo_bond_parity2);
static double get_z_coord(inp_ATOM *at, int cur_atom, int neigh_no, int *nType,
...
static double triple_prod(double a[], double b[], double c[],
                          double *sine_value);
static double triple_prod_and_min_abs_sine(double at_coord[][3],
                                           double *min_sine);
static int are_3_vect_in_one_plane(double at_coord[][3], double min_sine);
static int triple_prod_char(inp_ATOM *at, int at_1, int i_next_at_1,
                            S_CHAR *z_dir1, int at_2, int i_next_at_2,
                            S_CHAR *z_dir2);

static int CompDble(const void *a1, const void *a2);
static int Get2DTetrahedralAmbiguity(double at_coord[][3],
                                     int bAddExplicitNeighbor,
                                     int bFix2DstereoBorderCase);
static double triple_prod_and_min_abs_sine2(double at_coord[][3],
...
static int bInpAtomHasRequirdNeigh(inp_ATOM *at, int cur_at,
                                   int RequirdNeighType, int NumDbleBonds);
static int bIsSuitableHeteroInpAtom(inp_ATOM *at);
static int bIsOxide(inp_ATOM *at, int cur_at);
static int half_stereo_bond_parity(inp_ATOM *at, int cur_at,
                                   inp_ATOM *at_removed_H, int num_removed_H,
                                   S_CHAR *z_dir, int bPointedEdgeStereo,
                                   int vABParityUnknown);
static int get_allowed_stereo_bond_type(int bond_type);
static int can_be_a_stereo_bond_with_isotopic_H(inp_ATOM *at, int cur_at,
                                                INCHI_MODE nMode);
static int half_stereo_bond_action(int nParity, int bUnknown, int bIsotopic,
                                   int vABParityUnknown);
static int set_stereo_bonds_parity(sp_ATOM *out_at, inp_ATOM *at, int at_1,
                                   inp_ATOM *at_removed_H, int num_removed_H,
                                   ...
                                   int bPointedEdgeStereo,
                                   int vABParityUnknown);
static int can_be_a_stereo_atom_with_isotopic_H(inp_ATOM *at, int cur_at,
                                                int bPointedEdgeStereo);
static int set_stereo_atom_parity(sp_ATOM *out_at, inp_ATOM *at, int cur_at,
                                  inp_ATOM *at_removed_H, int num_removed_H,
                                  int bPointedEdgeStereo, int vABParityUnknown);
/*
int set_stereo_parity( inp_ATOM* at, sp_ATOM* at_output, int num_at, int
num_removed_H, int *nMaxNumStereoAtoms, int *nMaxNumStereoBonds, INCHI_MODE
nMode, int bPointedEdgeStereo, vABParityUnknown ); int get_opposite_sb_atom(
inp_ATOM *at, int cur_atom, int icur2nxt, int *pnxt_atom, int *pinxt2cur, int
*pinxt_sb_parity_ord );
*/
int ReconcileCmlIncidentBondParities(inp_ATOM *at, int cur_atom, int prev_atom,
                                     S_CHAR *visited, int bDisconnected);
int comp_AT_NUMB(const void *a1, const void *a2);
int GetHalfStereobond0DParity(inp_ATOM *at, int cur_at,
                              AT_NUMB nSbNeighOrigAtNumb[],
                              int nNumExplictAttachments, int bond_parity,
                              int nFlag);
int GetStereocenter0DParity(inp_ATOM *at, int cur_at, int j1,
                            AT_NUMB nSbNeighOrigAtNumb[], int nFlag);
int GetSbNeighOrigAtNumb(inp_ATOM *at, int cur_at, inp_ATOM *at_removed_H,
                         int num_removed_H, AT_NUMB nSbNeighOrigAtNumb[]);
int FixSb0DParities(
    inp_ATOM *at,
    /* inp_ATOM *at_removed_H, int num_removed_H,*/ int chain_length, int at_1,
    int i_next_at_1, S_CHAR z_dir1[], int at_2, int i_next_at_2,
    S_CHAR z_dir2[], int *pparity1, int *pparity2);
...
/******************************************************************/
double get_z_coord(inp_ATOM *at, int cur_atom, int neigh_no, int *nType,
                   int bPointedEdgeStereo) {
  ...
  if ((bFlat = (fabs(z) < ZERO_LENGTH))) {
    ...
    for (i = 0; i < at[cur_atom].valence; i++) {
      if (fabs(at[cur_atom].z - at[(int)at[cur_atom].neighbor[i]].z) >
          ZERO_LENGTH) {
        bFlat = 0;
        break;
      }
  ...
  if (bFlat) {
    if (!bPointedEdgeStereo || bPointedEdgeStereo * stereo_value >= 0) {
      /* bPointedEdgeStereo > 0: define stereo from pointed end of the stereo
       * bond only */
      /* bPointedEdgeStereo < 0: define stereo from wide end of the stereo bond
       * only (case of removed H) */
      switch (stereo_type) {
        /*  1=Up (solid triangle), 6=Down (Dashed triangle), 4=Either (zigzag
         * triangle) */
      case 0: /*  No stereo */
        *nType = ZTYPE_NONE;
        break;
...
/******************************************************************/
double len2(const double c[]) { return sqrt(c[0] * c[0] + c[1] * c[1]); }
/******************************************************************/
...
int dot_prodchar3(const S_CHAR a[], const S_CHAR b[]) {
  int prod =
      ((int)a[0] * (int)b[0] + (int)a[1] * (int)b[1] + (int)a[2] * (int)b[2]) /
      100;
  if (prod > 100)
    prod = 100;
  else if (prod < -100)
...
double triple_prod(double a[], double b[], double c[], double *sine_value) {
  ...
  if (sine_value) {
    ...
    if (abs_c > 1.e-7 /* otherwise c has zero length */ &&
        abs_ab > 1.e-7 /* otherwise a is parallel to b*/) {
...
int CompDble(const void *a1, const void *a2) {
  double diff =
      pDoubleForSort[*(const int *)a1] - pDoubleForSort[*(const int *)a2];
  if (diff > 0.0)
    return 1;
  if (diff < 0.0)
    return -1;
...
int Get2DTetrahedralAmbiguity(double at_coord[][3], int bAddExplicitNeighbor,
  ...
  const double dAngleAndPiMaxDiff =
      2.0 * atan2(1.0, sqrt(7.0)); /*  min sine between 2 InPlane bonds */
  int nBondType[MAX_NUM_STEREO_ATOM_NEIGH],
      nBondOrder[MAX_NUM_STEREO_ATOM_NEIGH];
  double dBondDirection[MAX_NUM_STEREO_ATOM_NEIGH];
  volatile double dAngle, dAlpha, dLimit, dBisector;
  /* 2010-02-10  added 'volatile': workaround ensuring proper behavior for gcc
   * 32-bit */
  /* cml-enabled compiles at >=O1 for SID484922 and alike (both lin&win had
   * problems) */
  ...
  for (i = 0, num_Up = num_Dn = 0; i < nNumNeigh; i++) {
    ...
    nBondType[i] = (at_coord[i][2] > 0.0)   ? 1
                   : (at_coord[i][2] < 0.0) ? -1
                                            : 0; /* z-coord sign */
    if (nBondType[i] > 0) {
      num_Up++;
  ...
  insertions_sort(nBondOrder, (unsigned)nNumNeigh, sizeof(nBondOrder[0]),
                  CompDble);
  ...
  if (num_Up == nNumNeigh) {
    /*  all bonds are Up */
    len_Up = cur_len_Up =
        nNumNeigh; /* added cur_len_Up initialization 1/8/2002 */
    first_Up = 0;
  } else {
    ...
    ----------} */
    /* look at all bonds and continue (circle therough the beginning) as long as
     * the current bond is Up */
    for (i = 0; i < nNumNeigh || bPrev_Up; i++) {
      if (nBondType[nBondOrder[i % nNumNeigh]] > 0) {
        if (bPrev_Up) {
          cur_len_Up++; /* uncrement number of Up bonds in current contiguous
                           sequence of them */
        } else {
        ...
        if (cur_len_Up > len_Up) {
          first_Up = cur_first_Up; /* store the sequence because it is longer
                                      than the ptrvious one */
          len_Up = cur_len_Up;
        }
        bPrev_Up = 0;
      }
  ...
  /* check if the bonds with ordering numbers first_Up+len_Up and
   * first_Up+len_Up+1 */
  /* have identical angles. In this case switch their order to enlarge the Up
   * sequence */
#define ZERO_ANGLE 0.000001
  if (nNumNeigh - len_Up >= 2) {
    ...
    for (i = 1; i < nNumNeigh - len_Up; i++) {
      next2 =
          (first_Up + len_Up + i) % nNumNeigh; /* the 2nd after Up sequence */
      if (nBondType[nBondOrder[next2]] > 0) {
        next1 = (first_Up + len_Up) % nNumNeigh; /* the 1st after Up sequence */
        dAngle = dBondDirection[nBondOrder[next1]] -
                 dBondDirection[nBondOrder[next2]];
        if (fabs(dAngle) < ZERO_ANGLE) {
          inchi_swap((char *)&nBondOrder[next1], (char *)&nBondOrder[next2],
                     sizeof(nBondOrder[0]));
          len_Up++;
          break;
        }
  ...
  if (nNumNeigh - len_Up >= 2) {
    int next1, next2;
    for (i = 1; i < nNumNeigh - len_Up; i++) {
      next2 = (first_Up + nNumNeigh - i - 1) %
              nNumNeigh; /* the 2nd before Up sequence */
      if (nBondType[nBondOrder[next2]] > 0) {
        next1 = (first_Up + nNumNeigh - 1) %
                nNumNeigh; /* the 1st before Up sequence */
        dAngle = dBondDirection[nBondOrder[next1]] -
                 dBondDirection[nBondOrder[next2]];
        if (fabs(dAngle) < ZERO_ANGLE) {
          inchi_swap((char *)&nBondOrder[next1], (char *)&nBondOrder[next2],
...
#define ZERO_ANGLE 0.000001
    if (nNumNeigh - len_Up >= 2) {
      ...
      for (i = 1; i < nNumNeigh - len_Up; i++) {
        ...
        if (nBondType[nBondOrder[next2]] > 0) {
          next1 =
              (first_Up + len_Up) % nNumNeigh; /* the 1st after Up sequence */
          dAngle = dBondDirection[nBondOrder[next1]] -
                   dBondDirection[nBondOrder[next2]];
          if (fabs(dAngle) < ZERO_ANGLE) {
            inchi_swap((char *)&nBondOrder[next1], (char *)&nBondOrder[next2],
    ...
    case 2: /* 2 Up */
      if (num_Dn) {
        ...
        if (dAlpha > one_pi - MIN_ANGLE) {
          ...
          if (dBisector - dAngle < -dLimit || dBisector - dAngle > dLimit) {
            ret = (T2D_UNDF | T2D_WARN);
          } else {
            ret = T2D_OKAY;
          }
...
double triple_prod_and_min_abs_sine2(double at_coord[][3],
                                     ...
                                     int bAddedExplicitNeighbor,
                                     double *min_sine, int *bAmbiguous) {
  double min_sine_value = 9999.0, sine_value, min_edge_len, max_edge_len,
  ...
  e13 = len3(at_coord[1]);
  e23 =
      len3(at_coord[2]); /* includes added neighbor if bAddedExplicitNeighbor*/
  ...
  bLongEdges = bAddedExplicitNeighbor
                   ? (max_edge_len_NoExplNeigh <
                      MAX_EDGE_RATIO * min_edge_len_NoExplNeigh)
  ...
  if (sine_value > MIN_SINE && (min_sine || bAmbiguous)) {
    if (min_sine) {
      ...
      /*  (instead of a tetrahedra calculate parallelogram/parallelepiped
       * area/volume) */
      ...
      /*  abs. value of a sine of an angle between each tetrahedra edge and
       * plane */
    ...
    if (bAmbiguous && sine_value >= MIN_SINE) {
      /*  check whether the central atom is outside the tetrahedra (0,0,0),
       * at_coord[0,1,2] */
      /*  compare the tetrahedra volume and the volume of a tetrahedra having
       * central_at_coord[] vertex */
      int i;
      diff3(central_at_coord, at_coord[0], tmp);
      central_prod[0] =
          triple_prod(at_coord[0], at_coord[1], central_at_coord, NULL);
      central_prod[1] =
          triple_prod(at_coord[1], at_coord[2], central_at_coord, NULL);
      central_prod[2] =
          triple_prod(at_coord[2], at_coord[0], central_at_coord, NULL);
    ...
    if (bLongEdges && !bAddedExplicitNeighbor &&
        max_edge_len >= MIN_LEN_STRAIGHT) {
    ...
    if (bLongEdges && sine_value < MIN_SINE_SQUARE &&
        sine_value < MIN_SINE_EDGE * min_edge_len_NoExplNeigh) {
      *min_sine = MIN_SINE / 2.0; /*  force parity to be undefined */
      if (bAmbiguous && !*bAmbiguous) {
        *bAmbiguous |= AMBIGUOUS_STEREO;
      }
...
int triple_prod_char(inp_ATOM *at, int at_1, int i_next_at_1, S_CHAR *z_dir1,
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (len < MIN_BOND_LEN) {
      if (i == 1 && (at[at_1].bUsed0DParity || at[at_2].bUsed0DParity)) {
        ...
        pnt[i][1] = 1.0;
        pnt[i][2] = 0.0;
        len = 1.0; /* standard at_1-->at_2 vector coordinates in case of 0D
                      allene */
      } else {
   ...
   *       triple product changes sign upon pnt[1]--> -pnt[1] change of
   * direction:
   *
...
/********************************************************************************************/
int bInpAtomHasRequirdNeigh(inp_ATOM *at, int cur_at, int RequirdNeighType,
                            int NumDbleBonds) {
  /* RequirdNeighType:
        reqired neighbor types (bitmap):
             0 => any neighbors
             1 => no terminal hydrogen atom neighbors
             2 => no terminal -X and -XH together (don't care about -X, -XH bond
     type, charge, radical) (X = tautomeric endpoint atom) NumDbleBonds: if
     non-zero then allow double, alternating and tautomeric bonds
  */
  int i, j, ni, nj, bond_type, num_1s, num_mult, num_other;

  if (at[cur_at]
          .endpoint) { /*  tautomeric endpoint cannot be a stereo center */
    return 0;
  }
  ...
  if (2 & RequirdNeighType) {
    for (i = 0; i < at[cur_at].valence; i++) {
      ...
      if (at[ni].valence != 1 || !get_endpoint_valence(at[ni].el_number)) {
        continue;
      }
      for (j = i + 1; j < at[cur_at].valence; j++) {
        nj = (int)at[cur_at].neighbor[j];
        if (at[nj].valence != 1 || at[ni].el_number != at[nj].el_number ||
            !get_endpoint_valence(at[nj].el_number)) {
          continue;
        }
        /*
         * if (at[ni].num_H != at[nj].num_H) then the atoms (neighbors of
         * at[cur_at] are tautomeric endpoints and are indistinguishable =>
         * cur_at is not stereogenic if (at[ni].num_H == at[nj].num_H) then the
         * neighbors are indistinguishable and cur_at will be found
         * non-sterogenic later get_endpoint_valence() check will not allow the
         * neighbors to be carbons Therefore the following "if" is not needed;
...
#define PHOSPHINE_STEREO                                                       \
  19 /* the number must match Phosphine number in the comments, see above */
#define ARSINE_STEREO                                                          \
  20 /* the number must match Arsine number in the comments, see above */

  static char szElem[][3] = {SZELEM1 SZELEM2 SZELEM3};
  static S_CHAR cCharge[] = {CCHARGE1 CCHARGE2 CCHARGE3};
  static S_CHAR cNumBondsAndH[] = {
      CNUMBONDSANDH1 CNUMBONDSANDH2 CNUMBONDSANDH3};
  static S_CHAR cChemValenceH[] = {
      CCHEMVALENCEH1 CCHEMVALENCEH2 CCHEMVALENCEH3};
  static S_CHAR cHas3MembRing[] = {
      CHAS3MEMBRING1 CHAS3MEMBRING2 CHAS3MEMBRING3};
  static S_CHAR cRequirdNeigh[] = {
      CREQUIRDNEIGH1 CREQUIRDNEIGH2 CREQUIRDNEIGH3};

  static int n = sizeof(szElem) / sizeof(szElem[0]);
  /* reqired neighbor types (bitmap):
     0 => check bonds only
     1 => no terminal hydrogen atom neighbors
     2 => no terminal -X and -XH together (don't care the bond type, charge,
     radical) (X = tautomeric endpoint atom) Note: whenever cChemValenceH[] >
     cNumBondsAndH[] the tautomeric and/or alternating bonds are permitted
  ...
  */
  ...
  for (i = 0; i < n; i++) {
    if (!strcmp(at[cur_at].elname, szElem[i]) &&
        ...
        bInpAtomHasRequirdNeigh(at, cur_at, cRequirdNeigh[i],
                                cChemValenceH[i] - cNumBondsAndH[i])) {
      ret = cNumBondsAndH[i];
      break;
    }
  }
...
int bCanAtomBeAStereoCenter(char *elname, S_CHAR charge, S_CHAR radical) {
  static const char szElem[][3] = {"C\000", "Si", "Ge",   "N\000",
                                   "P\000", "As", "B\000"};
...
int bIsSuitableHeteroInpAtom(inp_ATOM *at) {
  int val, num_H;
  if (0 == at->charge && (!at->radical || RADICAL_SINGLET == at->radical) &&
...
int bIsOxide(inp_ATOM *at, int cur_at) {
  ...
  for (i = 0; i < a->valence; i++) {
    ...
    if (bond_type == BOND_DOUBLE) {
      ...
      if (1 == an->valence && !an->charge && !an->num_H && !an->radical &&
          2 == get_endpoint_valence(an->el_number)) {
        return 1;
      }
    } else if (bond_type == BOND_TAUTOM || bond_type == BOND_ALT12NS) {
      an = at + (int)a->neighbor[i];
      if (1 == an->valence && 2 == get_endpoint_valence(an->el_number)) {
        return 1;
      }
    }
  }
...
int GetHalfStereobond0DParity(inp_ATOM *at, int cur_at,
  ...
  for (m = 0, last_parity = 0;
       m < MAX_NUM_STEREO_BONDS && at[cur_at].sb_parity[m]; m++) {
    icur2nxt = icur2neigh =
        -1;         /* ordering number of neighbors in nSbNeighOrigAtNumb[] */
    cur_parity = 0; /* parity for mth stereobond incident to the cur_at */
    if (0 <= at[cur_at].sb_ord[m] &&
        at[cur_at].sb_ord[m] < at[cur_at].valence &&
        0 <= (nxt_at = at[cur_at].neighbor[(int)at[cur_at].sb_ord[m]]) &&
        at[nxt_at].valence <=
            MAX_NUM_STEREO_BONDS && /* make sure it is a valid stereobond */
        (nNextSbAtOrigNumb = at[nxt_at].orig_at_number)) {
      /* since at[cur_at].sn_ord[m] = -1 for explicit H use
       * at[cur_at].sn_orig_at_num[m] */
      ...
      if (icur2neigh >= 0 && icur2nxt >= 0) {
        if (ATOM_PARITY_WELL_DEF(at[cur_at].sb_parity[m])) {
          /* parity of at[cur_atom] neighbor permutation to reach this order: {
           * next_atom, neigh_atom, ...} */
          cur_order_parity =
              (icur2nxt + icur2neigh + (icur2nxt > icur2neigh) - 1) % 2;
    ...
    /* use a well-known parity if available; if not then use preferably the
     * unknown */
    if (!last_parity) {
      ...
      if (ATOM_PARITY_WELL_DEF(last_parity)) {
        ...
        /* select min unknown/undefined parity (out of AB_PARITY_UNKN and
         * AB_PARITY_UNDF) */
...
int FixSb0DParities(
  ...
  /*
  AT_NUMB nSbNeighOrigAtNumb1[MAX_NUM_STEREO_BOND_NEIGH],
  nSbNeighOrigAtNumb2[MAX_NUM_STEREO_BOND_NEIGH]; int nNumExplictAttachments1,
  nNumExplictAttachments2;
  */
  ...
  case 3:
    ...
    switch (
        !(ATOM_PARITY_WELL_DEF(abs_parity1) && ATOM_PARITY_WELL_DEF(parity1)) +
        2 * !(ATOM_PARITY_WELL_DEF(abs_parity2) &&
    ...
    case 1:
      ...
      *pparity1 = parity_sign * (ATOM_PARITY_WELL_DEF(parity1) ? abs_parity1
                                 : ATOM_PARITY_WELL_DEF(abs_parity1)
                                     ? parity1
    ...
    case 2:
      ...
      *pparity2 = parity_sign * (ATOM_PARITY_WELL_DEF(parity2) ? abs_parity2
                                 : ATOM_PARITY_WELL_DEF(abs_parity2)
                                     ? parity2
  ...
  /*
  nNumExplictAttachments1 = GetSbNeighOrigAtNumb( at, at_1, at_removed_H,
  num_removed_H, nSbNeighOrigAtNumb1 ); nNumExplictAttachments2 =
  GetSbNeighOrigAtNumb( at, at_2, at_removed_H, num_removed_H,
  nSbNeighOrigAtNumb2 ); parity1 = GetHalfStereobond0DParity( at, at_1,
  nSbNeighOrigAtNumb1, nNumExplictAttachments1, *pparity1, 0 ); parity2 =
  GetHalfStereobond0DParity( at, at_2, nSbNeighOrigAtNumb2,
  nNumExplictAttachments2, *pparity2, 0 );
  */
  *pparity1 = parity_sign * abs_parity1;
  *pparity2 = parity_sign * abs_parity2;
...
half_stereo_bond_parity() Detailed Description:
    ...
    9) Calculate the parity: if tmp1 < tmp2 then 1 (odd) else 2 (even)
       (even: looking from the arrow end of the new z-axis, 0, 1, and 2
neighbors are in clockwise order) 10) Calculate z_dir = 100*pnt[2].

   Note1. If z_dir vectors of atoms located at the opposite ends of a double
bond have approximately opposite directions (that is, their dot-product is
negative) then the parity of the stereogenic bond calculated from
half-bond-parities should be inverted

   Note2. In case of a tetrahedral cumulene a triple product (z_dir1, (1->2),
z_dir2) is used instead of the dot-product. (1->2) is a vector from the atom#1
to the atom #2. This triple product is invariant with respect to the atom
numbering because it does not change upon (1,2) permutation.

  Stereo ambiguity in case of 2 neighbors:
  ----------------------------------------
  Undefined: single-double bond angle > pi - arcsin(0.03) =
178.28164199834454285275613218975 degrees Ambiguous: single-double bond angle >
175 degrees = pi - 0.087156 Rad
  ...
  -4 = AB_PARITY_UNDF => atom is adjacent to a stereogenic bond, but the
geometry is undefined, I -3 = AB_PARITY_UNKN => atom is adjacent to a
stereogenic bond, but the geometry is not known to the iuser, I -2
=-AB_PARITY_EVEN => parity of an atom adjacent to a stereogenic bond, I -1
=-AB_PARITY_ODD  => parity of an atom adjacent to a stereogenic bond, I 0 =
AB_PARITY_NONE => the atom is not adjacent to a stereogenic bond 1 =
AB_PARITY_ODD  => parity of an atom adjacent to a stereogenic bond, N&I 2 =
AB_PARITY_EVEN => parity of an atom adjacent to a stereogenic bond, N&I 3 =
AB_PARITY_UNKN => atom is adjacent to a stereogenic bond, but the geometry is
not known to the iuser, N&I 4 = AB_PARITY_UNDF => atom is adjacent to a
stereogenic bond, but the geometry is undefined, N&I 5 = AB_PARITY_IISO => atom
constitutionally equivalent to this atom may be adjacent to a stereogenic bond,
I
...
int half_stereo_bond_parity(inp_ATOM *at, int cur_at, inp_ATOM *at_removed_H,
                            ...
                            int bPointedEdgeStereo, int vABParityUnknown) {
  double at_coord[MAX_NUM_STEREO_BOND_NEIGH][3], c, s, tmp[3], tmp1, tmp2,
      min_tmp, max_tmp, z;
  double temp[3], pnt[3][3];
  int j, k, p0, p1, p2, next, bValence3 = 0, num_z, nType, num_either_single,
                              num_either_double;
  ...
  int index_H[5]; /*  cannot have more than 4 elements: 1 H, 1 1H, 1 D, 1 T
                     atom(s) */
  /*	const double one_pi = 2.0*atan2(1.0 , 0.0 ); */
  const double one_pi = 3.14159265358979323846; /* M_PI */
  const double two_pi = 2.0 * one_pi;
  ...
  if (!bCanAtomHaveAStereoBond(at[cur_at].elname, at[cur_at].charge,
                               at[cur_at].radical))
    return 0;
  if (!bIgnoreIsotopicH) {
  ...
  /*  (at_removed_H are sorted in ascending isotopic H mass order, non-isotopic
   * first) */
  ...
  if (at_removed_H && num_removed_H > 0) {
    ...
    num_iH = num_H -
             num_eH; /*  number of implicit non-isotopic and isotopic H atoms */
    if (num_iH > 1) {
  ...
  /*  at this point num_iH = number of implicit non-isotopic and isotopic H
   * atoms */
  ...
  bValence3 = bAtomHasValence3(at[cur_at].elname, at[cur_at].charge,
                               at[cur_at].radical);
  /*
   ...
   * (b) one isotopic or non-isotopic H if NO explicit isotopic or non-isotopic
   * H atom is present This makes sense only in case chem. valence = 4. In case
   * of chem. valence = 3, do not check.
   */
  ...
  for (k = nNumExplictAttachments = 0; k < 2; k++) {
    ...
    case 0:
      for (j = 0; j < num_eH; j++, nNumExplictAttachments++) {
        next = index_H[j];
        at_coord[nNumExplictAttachments][0] =
            at_removed_H[next].x - at[cur_at].x;
        at_coord[nNumExplictAttachments][1] =
            at_removed_H[next].y - at[cur_at].y;
        nSbNeighOrigAtNumb[nNumExplictAttachments] =
            at_removed_H[next].orig_at_number;
        /* use the fact that (at_removed_H - at) = (number of atoms except
         * removed explicit H) */
        z = -get_z_coord(at, (at_removed_H - at) + next, 0 /*neighbor #*/,
                         &nType,
        ...
        case ZTYPE_UP:
        case ZTYPE_DOWN:
          nType = -nType; /*  at_removed_H[] contains bonds TO the center, not
                             from */
          z = len2(at_coord[nNumExplictAttachments]);
          /*
          z = sqrt(
          at_coord[nNumExplictAttachments][0]*at_coord[nNumExplictAttachments][0]
                  +
  ...
  if (num_either_single) {
    bond_parity =
        vABParityUnknown /*AB_PARITY_UNKN*/; /*  single bond is 'unknown' */
    goto exit_function;
  }

  /* nNumExplictAttachments is a total number of attachments, including removed
   * explicit terminal hydrogens */
  if (nNumExplictAttachments == 2) {
    /*  create coordinates of the implicit hydrogen (or a fictitious atom in
     * case of ==N-X ), */
  ...
  /*  find projections of at_coord vector differences on the plane containing
   * their arrowhead ends */
  for (j = 0; j < 3; j++) {
    /*  pnt[0..2] = {0-1, 1-2, 2-0} */
   ...
   */
  /*  replace previous pnt[p2], tmp[p2] with new values; the old values do not
   * have any additional */
  /*  information because pnt[p0]+pnt[p1]+pnt[p2]=0 */
  /*  10-6-2003: a cross-product of one pair pnt[j], pnt[(j+1)%3] can be very
   * small. Find the larges one */
  ...
  /*  new basis: pnt[p0], pnt[p1], pnt[p2]; set z-coord sign and make
   * abs(pnt[p2]) = 1 */
  mult3(pnt[p2], (pnt[p2][2] > 0.0 ? 1.0 : -1.0) / tmp[p2],
        pnt[p2]); /*  unit vector in the new z-axis direction */

  min_tmp = dot_prod3(
      at_coord[0],
      pnt[p2]); /*  non-planarity measure (sine): hight of at_coord[] pyramid */
  mult3(pnt[p2], min_tmp,
        pnt[p0]); /*  vector height of the pyramid, ideally 0 */
  /*  find new pnt[p0] = projection of at_coord[p0] on plane orthogonal to
   * pnt[p2] */
  ...
  for (j = 0; j < 3; j++) {
    ...
    tmp[j] =
        sqrt(at_coord[j][0] * at_coord[j][0] + at_coord[j][1] * at_coord[j][1]);
    /*  make new xy plane projection length = 1 */
    mult3(at_coord[j], 1.0 / tmp[j], at_coord[j]);
  }

  s = fabs(at_coord[1][0] * at_coord[2][1] -
           at_coord[1][1] * at_coord[2][0]); /*  1-2 sine */
  c = at_coord[1][0] * at_coord[2][0] +
      at_coord[1][1] * at_coord[2][1]; /*  1-2 cosine */
  if (s < MIN_SINE && c > 0.5) {
    goto exit_function; /*  bonds to neigh. 1 and 2 have almost same direction;
                           relative angles are undefined */
  }
  c = at_coord[0][0]; /*  cosine of the angle between new Ox axis and a bond to
                         the neighbor 0. Should be 1 */
  ...
  /*  counterclockwise angles from the direction to neigh 0 to to directions to
   * neighbors 1 and 2: */
  ...
  for (j = 0; j < 3; j++) {
    z_dir[j] =
        (S_CHAR)(pnt[p2][j] >= 0.0
                     ? floor(0.5 + 100.0 * pnt[p2][j])
                     : -floor(0.5 -
                              100.0 * pnt[p2][j])); /*  abs(z_dir) = 100 */
  }
  /*  check for ambiguity */
  if (nNumExplictAttachments > 2) {
    min_tmp = inchi_min(tmp1, tmp2);
    max_tmp = inchi_max(tmp1, tmp2);
    if (min_tmp > one_pi - MIN_SINE || max_tmp < one_pi + MIN_SINE ||
        max_tmp - min_tmp > one_pi - MIN_SINE) {
      at[cur_at].bAmbiguousStereo |= AMBIGUOUS_STEREO;
    } else /* 3D ambiguity 8-28-2002 */
      if (fabs(at_coord[0][2]) >
          MAX_SINE) { /*  all fabs(at_coord[j][2] (j=0..2) must be equal */
        at[cur_at].bAmbiguousStereo |= AMBIGUOUS_STEREO;
      }
...
exit_function:
  if (num_H > 1 && bond_parity > 0 &&
      !(bond_parity & AB_PARITY_0D) /*&& PARITY_WELL_DEF(bond_parity)*/) {
    /*
     ...
     */
    bond_parity =
        -bond_parity; /*  refers to isotopically substituted structure only */
  }
  return bond_parity;
}

/*************************************************************/
int save_a_stereo_bond(int z_prod, int result_action, int at1, int ord1,
                       AT_NUMB *stereo_bond_neighbor1, S_CHAR *stereo_bond_ord1,
                       S_CHAR *stereo_bond_z_prod1, S_CHAR *stereo_bond_parity1,
                       int at2, int ord2, AT_NUMB *stereo_bond_neighbor2,
                       S_CHAR *stereo_bond_ord2, S_CHAR *stereo_bond_z_prod2,
  ...
  stereo_bond_z_prod1[i1] = stereo_bond_z_prod2[i2] = (S_CHAR)z_prod;
  return 1;
}
/***************************************************************/
...
/*************************************************************/
int can_be_a_stereo_bond_with_isotopic_H(inp_ATOM *at, int cur_at,
                                         INCHI_MODE nMode) {
  ...
  int n2sh, num_2s_hetero[2], num_2s_hetero_next[2], next_next_at, type_N,
      type_N_next;
#endif
  ...
  if (0 == at[cur_at].num_H && 0 == at[cur_at].charge &&
      0 == at[cur_at].radical &&
  ...
  for (i = 0, num_stereo_bonds = 0; i < at[cur_at].valence; i++) {
    ...
    if (bond_type == BOND_ALTERN) {
      ...
      if (num_wrong_bonds_1 > 1 ||
          (num_wrong_bonds_1 && 2 >= at[cur_at].valence)) {
        return 0; /* wrong bond type */
      } else {
        continue;
      }
    ...
    if (bFound) {
      ...
      if (MAX_NUM_STEREO_BOND_NEIGH < at[next_at].valence + at[next_at].num_H ||
        ...
        continue;
      if (!bCanAtomHaveAStereoBond(at[next_at].elname, at[next_at].charge,
                                   at[next_at].radical))
        continue;
      /*  next atom neighbors */
      ...
      if (0 == at[next_at].num_H && 0 == at[next_at].charge &&
          0 == at[next_at].radical &&
          3 == get_endpoint_valence(at[next_at].el_number)) {
        if (2 == at[next_at].valence && 3 == at[next_at].chem_bonds_valence) {
          type_N_next = 1; /* -N= */
        } else if (3 == at[next_at].valence &&
                   5 == at[next_at].chem_bonds_valence) {
      ...
      /* figure out whether the at[cur_at]--at[next_at] bond may not be
       * stereogenic */
      ...
      if (3 == (type_N | type_N_next) &&
        ...
        bFound = 0;
          ...
          if (j < at[next_at].valence || /* at[next_at] has a wrong bond type*/
              (num_alt_next > 0) + (num_2s_next > 0) !=
                  1 /* only one type of stereogenic bond permitted */
          ) {
        bFound = 0;
  ...
  if (num_2s > 1) {
#if (N_V_STEREOBONDS == 1)
    if (2 == num_2s && 1 == (num_2s_hetero[0] | num_2s_hetero[1]) &&
...
#define AB_UNKNOWN 0x20
  ...
  /*  Unknown (type 1) in the parity value may come from the 'Either' single
   * bond only */
  ...
  if (nParity < 0)
    nParity = ((nParity == -AB_PARITY_EVEN) ? AB_PARITY_ODD : (-nParity)) |
              AB_NEGATIVE;
...
/*************************************************************/
int set_stereo_bonds_parity(sp_ATOM *out_at, inp_ATOM *at, int at_1,
                            inp_ATOM *at_removed_H, int num_removed_H,
                            INCHI_MODE nMode, QUEUE *q, AT_RANK *nAtomLevel,
                            S_CHAR *cSource, AT_RANK min_sb_ring_size,
                            int bPointedEdgeStereo, int vABParityUnknown) {
  int j, k, next_at_1, i_next_at_1, i_next_at_2, at_2, next_at_2,
      num_stereo_bonds, bFound, bAllene;
  ...
  if (MAX_NUM_STEREO_BOND_NEIGH < at[at_1].valence + at[at_1].num_H ||
    ...
    return 0;
  if (!bCanAtomHaveAStereoBond(at[at_1].elname, at[at_1].charge,
                               at[at_1].radical))
    return 0;
  if (at[at_1].c_point)
    return 0; /* rejects atoms that can lose or gain a (positive) charge.
                 01-24-2003 */
...
#endif
  for (i_next_at_1 = 0, num_stereo_bonds = 0; i_next_at_1 < at[at_1].valence;
       i_next_at_1++) {
    nUnknown = (at[at_1].bond_stereo[i_next_at_1] == STEREO_DBLE_EITHER);
    bond_type =
        get_allowed_stereo_bond_type((int)at[at_1].bond_type[i_next_at_1]);
    at_2 = -1; /* not found */
    if (bond_type == BOND_ALTERN || bond_type == BOND_DOUBLE) {
    ...
    case BOND_ALTERN:
      ...
      if ((nMode & CMODE_NO_ALT_SBONDS) ||
          !bCanAtomHaveAStereoBond(at[at_2].elname, at[at_2].charge,
                                   at[at_2].radical)) {
    ...
    case BOND_DOUBLE:
      ...
      num_2s_1++;
      cur_chain_length = 0;
      if (bCanAtomBeTerminalAllene(at[at_1].elname, at[at_1].charge,
                                   at[at_1].radical)) {
        /*
         * Example of cumulene
         ...
         */
        while ((bAllene = !at[at_2].num_H && at[at_2].valence == 2 &&
                          BOND_DOUBLE == get_allowed_stereo_bond_type(
                                             (int)at[at_2].bond_type[0]) &&
                          BOND_DOUBLE == get_allowed_stereo_bond_type(
                                             (int)at[at_2].bond_type[1])) &&
               bCanAtomBeMiddleAllene(at[at_2].elname, at[at_2].charge,
                                      at[at_2].radical)) {
          k = ((int)at[at_2].neighbor[0] ==
               next_at_2); /*  opposite neighbor position */
          next_at_2 = at_2;
        ...
        if (cur_chain_length) {
          ...
          if (bAllene /* at the end of the chain atom Y is =Y=, not =Y< or =Y-
                       */
              || !bCanAtomBeTerminalAllene(at[at_2].elname, at[at_2].charge,
                                           at[at_2].radical)) {
            cur_chain_length = 0;
            continue; /*  ignore: does not fit cumulene description; go to check
                         next at_1 neighbor */
          }
    ...
    bFound = (at_2 >= 0 && at_1 > at_2); /*  i_next_at_1 = at_1 stereogenic bond
                                            neighbor attachment number */
...
#if (N_V_STEREOBONDS == 1)
      num_2s_hetero_next[0] = num_2s_hetero_next[1] = type_N_next = 0;
      if (0 == at[at_2].num_H && 0 == at[at_2].charge &&
          0 == at[at_2].radical &&
...
#if (ONE_BAD_SB_NEIGHBOR == 1)
          num_wrong_bonds_2++;
          continue; /*  this bond type is not allowed to be adjacent to a stereo
                       bond */
#else
          break;
#endif
    ...
    if (bFound) {
      ...
      all_unkn[num_stereo_bonds] =
          nUnknown; /* stereogenic bond has Unknown configuration */
      /*
      if ( (at[at_1].bUsed0DParity & 2) || (at[at_2].bUsed0DParity & 2) ) {
          for ( k = 0; k < MAX_NUM_STEREO_BONDS && at[at_1].sb_parity[k]; k ++ )
      { if ( at[at_1].sb_neigh[k] == i_next_at_1 ) { if ( at[at_1].sb_parity[k]
      == AB_PARITY_UNKN && !nUnknown ) { all_unkn[num_stereo_bonds] = 1;
                  }
                  break;
              }
          }
  ...
  if (chain_length &&
      (num_stereo_bonds != 1 || num_alt_1 || chain_length > MAX_CUMULENE_LEN)) {
    return 0;
  }

  /*  we need 1 double bond/chain XOR up to 3 arom. bonds */
  /*  to have a stereogenic bond */
  if ((num_alt_1 > 0) + (num_2s_1 > 0) != 1 ||
      !num_stereo_bonds /*|| num_2s_1 > 1*/)
    return 0;

  if (num_2s_1 > 1) {
#if (N_V_STEREOBONDS == 1)
    if (2 == num_2s_1 && 2 == type_N &&
  ...
  for (k = 0; k < num_stereo_bonds; k++) {
    ...
    S_CHAR z_dir1[3],
        z_dir2[3]; /*  3D vectors for half stereo bond parity direction */
    ...
    if (at[at_1].nRingSystem == at[at_2].nRingSystem) {
      ...
      j = is_bond_in_Nmax_memb_ring(at, at_1, i_next_at_1, q, nAtomLevel,
                                    cSource, min_sb_ring_size);
      if (j > 0) {
        continue;
      } else if (j < 0) {
     ...
     *    |  parity()=        |       | 2 or 3 |                |
     * ---+-------------------+-------+--------+----------------+
     *   ( AB_PARITY_ODD/EVEN) => N&I: -       => AB_PARITY_CALC (=6,
     calc.later)
     ...
     *
     *
     * atom1 \ atom2   | AB_PARITY_NONE  AB_PARITY_UNKN  AB_PARITY_UNDF
     AB_PARITY_CALC
     * ----------------+---------------------------------------------------------------
     *0=AB_PARITY_NONE | AB_PARITY_NONE  AB_PARITY_NONE  AB_PARITY_NONE
     AB_PARITY_NONE *3=AB_PARITY_UNKN |                 AB_PARITY_UNKN
     AB_PARITY_UNKN  AB_PARITY_UNKN *4=AB_PARITY_UNDF | AB_PARITY_UNDF
     AB_PARITY_UNDF *6=AB_PARITY_CALC | AB_PARITY_CALC
     *
     * that is, take min out of the two
     *********************************************************************************/

    cur_parity =
        half_stereo_bond_parity(at, at_1, at_removed_H, num_removed_H, z_dir1,
                                bPointedEdgeStereo, vABParityUnknown);
    next_parity =
        half_stereo_bond_parity(at, at_2, at_removed_H, num_removed_H, z_dir2,
                                bPointedEdgeStereo, vABParityUnknown);

    if (RETURNED_ERROR(cur_parity) || RETURNED_ERROR(next_parity)) {
      return CT_CALC_STEREO_ERR;
    }
    if ((at[at_1].bUsed0DParity & FlagSB_0D) ||
        (at[at_2].bUsed0DParity & FlagSB_0D)) {
      FixSb0DParities(at, /* at_removed_H, num_removed_H,*/ chain_length, at_1,
                      i_next_at_1, z_dir1, at_2, i_next_at_2, z_dir2,
    ...
    cur_action = half_stereo_bond_action(
        cur_parity, nUnknown, 0, vABParityUnknown); /*  -1 => program error */
    next_action =
        half_stereo_bond_action(next_parity, nUnknown, 0, vABParityUnknown);
    ...
    if (cur_parity_defined && next_parity_defined) {
      ...
      dot_prod_z  = (chain_len_bits & BIT_CUMULENE_CHI)?
                     triple_prod_char( at, at_1, i_next_at_1, z_dir1, at_2,
      i_next_at_2, z_dir2 ) : dot_prodchar3(z_dir1, z_dir2);
      */
    ...
    if (result_action != AB_PARITY_NONE && result_action != -1) {
      ...
      if (cur_parity > 0 && next_parity > 0) {
        if (save_a_stereo_bond(
                dot_prod_z, result_action | chain_len_bits, at_1, i_next_at_1,
                out_at[at_1].stereo_bond_neighbor, out_at[at_1].stereo_bond_ord,
                out_at[at_1].stereo_bond_z_prod,
                out_at[at_1].stereo_bond_parity, at_2, i_next_at_2,
                out_at[at_2].stereo_bond_neighbor, out_at[at_2].stereo_bond_ord,
                out_at[at_2].stereo_bond_z_prod,
                out_at[at_2].stereo_bond_parity)) {
          if (!out_at[at_1].parity ||
              (cur_parity_defined &&
               !ATOM_PARITY_WELL_DEF(abs(out_at[at_1].parity)))) {
            out_at[at_1].parity = cur_parity;
            memcpy(out_at[at_1].z_dir, z_dir1, sizeof(out_at[0].z_dir));
          }
          if (!out_at[at_2].parity ||
              (next_parity_defined &&
               !ATOM_PARITY_WELL_DEF(abs(out_at[at_2].parity)))) {
    ...
    cur_action = half_stereo_bond_action(
        cur_parity, nUnknown, 1, vABParityUnknown); /*  -1 => program error */
    next_action =
        half_stereo_bond_action(next_parity, nUnknown, 1, vABParityUnknown);
    result_action = inchi_min(cur_action, next_action);
    ...
    if (result_action != AB_PARITY_NONE && result_action != -1) {
      ...
      if (cur_parity > 0 && next_parity > 0) {
        if (save_a_stereo_bond(
                dot_prod_z, result_action | chain_len_bits, at_1, i_next_at_1,
                out_at[at_1].stereo_bond_neighbor2,
                out_at[at_1].stereo_bond_ord2, out_at[at_1].stereo_bond_z_prod2,
                out_at[at_1].stereo_bond_parity2, at_2, i_next_at_2,
                out_at[at_2].stereo_bond_neighbor2,
                out_at[at_2].stereo_bond_ord2, out_at[at_2].stereo_bond_z_prod2,
                out_at[at_2].stereo_bond_parity2)) {
          if (!out_at[at_1].parity2 ||
              (cur_parity_defined &&
               !ATOM_PARITY_WELL_DEF(abs(out_at[at_1].parity2)))) {
            out_at[at_1].parity2 = cur_parity /*| chain_len_bits*/;
            if (!out_at[at_1].parity) {
              memcpy(out_at[at_1].z_dir, z_dir1, sizeof(out_at[0].z_dir));
            }
          }
          if (!out_at[at_2].parity2 || /* next line changed from
                                          abs(out_at[at_2].parity) 2006-03-05 */
              (next_parity_defined &&
               !ATOM_PARITY_WELL_DEF(abs(out_at[at_2].parity2)))) {
...
int can_be_a_stereo_atom_with_isotopic_H(inp_ATOM *at, int cur_at,
  ...
  int nNumNeigh;
  if ((nNumNeigh =
           bCanInpAtomBeAStereoCenter(at, cur_at, bPointedEdgeStereo)) &&
...
int can_be_a_stereo_atom_with_isotopic_H(inp_ATOM *at, int cur_at) {
  int j, ret = 0;
  if (bCanAtomBeAStereoCenter(at[cur_at].elname, at[cur_at].charge,
                              at[cur_at].radical) &&
...
int GetStereocenter0DParity(inp_ATOM *at, int cur_at, int j1,
  ...
  if (at[cur_at].p_parity && (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1 ||
                              j1 == MAX_NUM_STEREO_ATOM_NEIGH)) {
    ...
    num_trans_inp =
        insertions_sort(nInpNeighOrigAtNumb, MAX_NUM_STEREO_ATOM_NEIGH,
                        sizeof(nInpNeighOrigAtNumb[0]), comp_AT_NUMB);
    num_trans_neigh = insertions_sort(
        nSbNeighOrigAtNumb, j1, sizeof(nSbNeighOrigAtNumb[0]), comp_AT_NUMB);
    if (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
      ; /*num_trans_neigh += j1;*/ /* the lone pair or implicit H is implicitly
                                      at the top of the list */
    }
    if (!memcmp(nInpNeighOrigAtNumb + MAX_NUM_STEREO_ATOM_NEIGH - j1,
                nSbNeighOrigAtNumb, j1 * sizeof(AT_NUMB))) {
      if (ATOM_PARITY_WELL_DEF(at[cur_at].p_parity)) {
        parity =
            2 - (num_trans_inp + num_trans_neigh + at[cur_at].p_parity) % 2;
      } else {
        parity = at[cur_at].p_parity;
      }
      at[cur_at].bUsed0DParity |=
          nFlag; /* 0D parity used for streocenter parity */
    }
  }
  return parity;
}
 ...
 * explicit hydrogen atoms, including isotopic ones, that are located in
 * at_removed_H[] The return value is a calculated parity.
 */
#define ADD_EXPLICIT_HYDROGEN_NEIGH 1
#define ADD_EXPLICIT_LONE_PAIR_NEIGH 2
int set_stereo_atom_parity(sp_ATOM *out_at, inp_ATOM *at, int cur_at,
                           inp_ATOM *at_removed_H, int num_removed_H,
                           int bPointedEdgeStereo, int vABParityUnknown) {
  int j, k, next_at, num_z, j1, nType, num_explicit_H, tot_num_iso_H,
      nMustHaveNumNeigh;
  int num_explicit_iso_H[NUM_H_ISOTOPES +
                         1]; /*  numbers of removed hydrogen atoms */
  int index_H[MAX_NUM_STEREO_ATOM_NEIGH]; /*  cannot have more than 4 elements:
                                             1 H, 1 D, 1 T atom(s) */
  ...
  int parity, bAmbiguous = 0, bAddExplicitNeighbor = 0, b2D = 0,
              n2DTetrahedralAmbiguity = 0;
  int bIgnoreIsotopicH = (0 != (at[cur_at].cFlags & AT_FLAG_ISO_H_POINT));
  AT_NUMB nSbNeighOrigAtNumb[MAX_NUM_STEREO_ATOM_NEIGH];

  out_at[cur_at].parity = out_at[cur_at].parity2 =
      out_at[cur_at].stereo_atom_parity = out_at[cur_at].stereo_atom_parity2 =
          AB_PARITY_NONE;
  ...
  if (bIgnoreIsotopicH) {
    tot_num_iso_H =
        0; /* isotopic H considered subject to exchange => ignore isotopic */
  }
  /*  number of non-isotopic H atoms */
  if (at[cur_at].num_H - tot_num_iso_H > 1) {
    goto exit_function; /*  two or more identical hydrogen non-isotopic
                           neighbors */
  }
  ...
  at_coord_center[0] = at_coord_center[1] = at_coord_center[2] = 0.0;
  ...
  for (k = 0, j1 = 0; k < 2; k++) {
    ...
    case 0:
      ...
      for (j = 0; j < num_explicit_H; j++, j1++) {
        ...
        /*  minus sign at get_z_coord: at_removed_H[] contains bonds TO
         * at[cur_at], not FROM it. */
        /*  Note: &at[(at_removed_H-at)+ next_at] == &at_removed_H[next_at] */
        z = -get_z_coord(at, (at_removed_H - at) + next_at, 0 /*neighbor #*/,
                         &nType,
                         -(bPointedEdgeStereo & PES_BIT_POINT_EDGE_STEREO));
        switch (nType) {
        case ZTYPE_EITHER:
          parity =
              vABParityUnknown /*AB_PARITY_UNKN*/; /*  no parity: bond in
                                                      "Either" direction. */
          goto exit_function;
        case ZTYPE_UP:
        case ZTYPE_DOWN:
        ...
        /* bond_len_xy[j1] =
         * sqrt(at_coord[j1][0]*at_coord[j1][0]+at_coord[j1][1]*at_coord[j1][1]);
         */
        at_coord[j1][2] = (nType == ZTYPE_3D     ? z
                           : nType == ZTYPE_UP   ? bond_len_xy[j1]
                           : nType == ZTYPE_DOWN ? -bond_len_xy[j1]
                                                 : 0.0);
      }
      break;
    case 1:
      ...
      for (j = 0; j < at[cur_at].valence; j++, j1++) {
        ...
        z = get_z_coord(at, cur_at, j, &nType,
                        (bPointedEdgeStereo & PES_BIT_POINT_EDGE_STEREO));
        switch (nType) {
        case ZTYPE_EITHER:
          parity =
              vABParityUnknown /*AB_PARITY_UNKN*/; /*  unknown parity: bond in
                                                      "Either" direction. */
          goto exit_function;
        case ZTYPE_UP:
        case ZTYPE_DOWN:
  ...
  /* j1 is the number of explicit neighbors (that is, all neighbors except
   * implicit H) */
  ...
  if (MAX_NUM_STEREO_ATOM_NEIGH != at[cur_at].valence + num_explicit_H &&
    ...
    if (nMustHaveNumNeigh == at[cur_at].valence + at[cur_at].num_H &&
      ...
      if (parity == vABParityUnknown /*AB_PARITY_UNKN*/) {
        parity = -vABParityUnknown /*AB_PARITY_UNKN*/; /*  the user marked the
                                                          center as "unknown" */
      } else {
        parity = -AB_PARITY_UNDF; /*  not enough geometry; only isotopic parity
                                     is possible */
      }
    } else {
  ...
  for (j = 0; j < j1; j++) {
    ...
    if (z < MIN_BOND_LEN) {
      ...
      if (AB_PARITY_NONE ==
          (parity = GetStereocenter0DParity(at, cur_at, j1, nSbNeighOrigAtNumb,
                                            FlagSC_0D))) {
  ...
  /*  num_explicit_iso_H[0] = number of explicit non-isotopic hydrogen atom
   * neighbors */
  j = j1;
  ...
  if (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
    ...
    if (nMustHaveNumNeigh == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
      ...
      if (1 == at[cur_at].num_H - num_explicit_H && /*  the atom has only one
                                                       one implicit hydrogen */
          1 == at[cur_at].num_H -
                   tot_num_iso_H) { /*  this hydrogen is non-isotopic */
  ...
  if (bAddExplicitNeighbor) {
     ...
     * 3 neighbors only, no H-neighbors. Create and add coordinates of an
     * implicit H or a fake 4th neighbor, that is, a lone pair
     */
    if (parity == vABParityUnknown /*AB_PARITY_UNKN*/) {
      goto exit_function; /*  the user insists the parity is unknown and the
                             isotopic */
...
#else
      at_coord[j][2] =
          -(bond_len_xy[0] + bond_len_xy[1] + bond_len_xy[2]) / 3.0;
#endif
      ...
      */
...
#else
      /* Comparing the original bond lengths to lenghts derived from normalized
       * to 1 */
      ...
      /* bond lengths are about 20 or greater. Reported by Reinhard Dunkel
       * 2005-08-05 */
      ...
      if (z < MIN_BOND_LEN || rmin / rmax < MIN_SINE) {
        ...
        if (AB_PARITY_NONE ==
            (parity = GetStereocenter0DParity(at, cur_at, j1,
                                              nSbNeighOrigAtNumb, FlagSC_0D))) {
          parity = AB_PARITY_UNDF;
        }
        goto exit_function;
      }
    ...
    if (parity == vABParityUnknown /*AB_PARITY_UNKN*/) {
      parity = -AB_PARITY_UNDF; /*  isotopic composition of H-neighbors
                                   contradicts 'unknown' */
    }
    goto exit_function;
    ...
    if (num_z == 0 || are_4at_in_one_plane(at_coord, MIN_SINE)) {
      ...
      if (parity != vABParityUnknown /*AB_PARITY_UNKN*/) {
        ...
        if (AB_PARITY_NONE ==
          ...
          bAmbiguous &=
              ~AMBIGUOUS_STEREO; /* 0D parity has resolved the ambiguity */
        }
#endif
      }
      goto exit_function;
  ...
  if (b2D) {
    ...
    n2DTetrahedralAmbiguity =
        Get2DTetrahedralAmbiguity(at_coord, bAddExplicitNeighbor,
  ...
  */
   ...
   ********************************************************/
  triple_product = triple_prod_and_min_abs_sine2(&at_coord[1], at_coord_center,
  ...
  if (fabs(triple_product) > ZERO_FLOAT &&
      (min_sine > MIN_SINE ||
       (fabs(min_sine) > ZERO_FLOAT && (n2DTetrahedralAmbiguity & T2D_OKAY)))) {
    ...
    /*  can be done in odd number (3) transpositions: (23)(12)(01), which
     * inverts the parity */
    if (j1 == MAX_NUM_STEREO_ATOM_NEIGH - 1) {
      parity = 3 - parity;
    }
...
exit_function:
  ...
  /*  isotopic parity */
  if (parity == -AB_PARITY_UNDF ||
      parity == -vABParityUnknown /*AB_PARITY_UNKN*/)
    parity = -parity;
  if (parity < 0)
    parity = AB_PARITY_NONE;
  ...
  out_at[cur_at].stereo_atom_parity =
      ATOM_PARITY_WELL_DEF(parity) ? AB_PARITY_CALC : parity;
  parity = PARITY_VAL(out_at[cur_at].parity2);
  out_at[cur_at].stereo_atom_parity2 =
      ATOM_PARITY_WELL_DEF(parity) ? AB_PARITY_CALC : parity;
  /*
  out_at[cur_at].parity2 = out_at[cur_at].parity; // save for stereo + isotopic
  canon. if ( out_at[cur_at].parity ) { if ( num_explicit_H > 1 || j1 ==
  MAX_NUM_STEREO_ATOM_NEIGH-1 && num_explicit_H ) {
          //              X   H      X
...
/*************************************************************/
int set_stereo_parity(inp_ATOM *at, sp_ATOM *at_output, int num_at,
                      int num_removed_H, int *nMaxNumStereoAtoms,
                      int *nMaxNumStereoBonds, INCHI_MODE nMode,
                      int bPointedEdgeStereo, int vABParityUnknown) {
  int num_3D_stereo_atoms = 0;
  int num_stereo_bonds = 0; /* added to fix allene stereo bug reported for
                               FClC=C=CFCl by Burt Leland - 2009-02-05 DT */
  ...
  /*********************************************************************************
   ...
   the central_at->neighbor[] array : neighbor atom number is
  central_at->neighbor[n].

   (0)-(1), (0)-(2), (0)-(3) are lines connecting atom [C] neighbors to neighbor
  (0) (0), (1) and (2) are in the plane (0)-(3) is directed from the plain to
  the viewer [C] is somewhere between (0), (1), (2), (3) Since (1)-(2)-(3)  are
  in a clockwise order when looking from (0), parity is 2, or even; otherwise
  parity would be 1, or odd.
  ...
  **********************************************************************************
    ...
    Notation:   [atom number], (index of a neighbor):
                ...
                (12 x 16)z = z-component of [1]-[2] and [1]-[6] cross-product
                                   ...
                                   atom [1]                     atom [2]
   [8]                    [4]      prod01 = (12 x 16)z < 0      prod01 = (21 x
  24)z < 0 \                    /        prod02 = (12 x 18)z > 0      prod02 =
  (21 x 25)z > 0 (2)               (1)        0 transpositions because     0
  transpositions because \              /           double bond is in 0 posit.
  double bond is in 0 position [1]==(0)(0)==[2]           0 = (prod01 > prod02)
  0 = (prod01 > prod02)
        /              \
      (1)               (2)        result: parity = 2, even     result:
  parity=2, even
     /                    \
   [6]                    [5]
                                   ...
                                   atom [1]                     atom [2]
   [8]                    [5]      prod01 = (12 x 18)z > 0      prod01 = (21 x
  24)z > 0 \                    /        prod02 = (12 x 16)z < 0      prod02 =
  (21 x 25)z < 0 (0)               (2)        2 transpositions to move     1
  transposition to move \              /           at [2] from 2 to 0 pos. at
  [1] from 1 to 0 position [1]==(2)(1)==[2]           1 = (prod01 > prod02) 1 =
  (prod01 > prod02)
        /              \
      (1)               (0)        result: parity = (1+2)       result:
  parity=(1+1) /                    \        2-(1+2)%2 = 1, odd 2-(1+1)%2 = 2,
  even [6]                    [4]


  ***********************************************************************************
  Note: atoms' numbers [1], [2], [4],... are not used to calculate parity at
  this point. They will be used for each numbering in the canonicalization.
  Note: parity=3 for a stereo atom means entered undefined bond direction
        parity=4 for an atom means parity cannot be determined from the given
  geometry
  ***********************************************************************************/
  ...
  for (i = 0; i < num_at; i++) {
    ...
    memset(&at_output[i].stereo_bond_neighbor[0], 0,
           sizeof(at_output[0].stereo_bond_neighbor));
    memset(&at_output[i].stereo_bond_neighbor2[0], 0,
           sizeof(at_output[0].stereo_bond_neighbor2));
    memset(&at_output[i].stereo_bond_ord[0], 0,
           sizeof(at_output[0].stereo_bond_ord));
    memset(&at_output[i].stereo_bond_ord2[0], 0,
           sizeof(at_output[0].stereo_bond_ord2));
    memset(&at_output[i].stereo_bond_z_prod[0], 0,
           sizeof(at_output[0].stereo_bond_z_prod));
    memset(&at_output[i].stereo_bond_z_prod2[0], 0,
           sizeof(at_output[0].stereo_bond_z_prod2));
    memset(&at_output[i].stereo_bond_parity[0], 0,
           sizeof(at_output[0].stereo_bond_parity));
    memset(&at_output[i].stereo_bond_parity2[0], 0,
           sizeof(at_output[0].stereo_bond_parity2));
  }
  ...
  if (nMaxNumStereoAtoms || nMaxNumStereoBonds) {
    for (i = 0, num_stereo = 0; i < num_at; i++) {
      ...
      } else if ((num = can_be_a_stereo_bond_with_isotopic_H(
                      at, i, nMode))) { /*  accept cumulenes */
...
#if (MIN_SB_RING_SIZE > 0)
  min_sb_ring_size = (AT_RANK)(((nMode & REQ_MODE_MIN_SB_RING_MASK) >>
                                REQ_MODE_MIN_SB_RING_SHFT) &
                               AT_RANK_MASK);
  if (min_sb_ring_size >= 3) {
    /* create BFS data structure for finding for each stereo bond its min. ring
     * sizes */
  ...
  for (i = 0, num_stereo = 0; i < num_at; i++) {
    is_stereo =
        set_stereo_atom_parity(at_output, at, i, at + num_at, num_removed_H,
                               bPointedEdgeStereo, vABParityUnknown);
    if (is_stereo) {
      num_3D_stereo_atoms += ATOM_PARITY_WELL_DEF(is_stereo);
    } else {
      is_stereo = set_stereo_bonds_parity(
          at_output, at, i, at + num_at, num_removed_H, nMode, q, nAtomLevel,
          cSource, min_sb_ring_size, bPointedEdgeStereo, vABParityUnknown);
      ...
      num_stereo_bonds +=
          (is_stereo !=
           0); /* added to fix bug reported by Burt Leland - 2009-02-05 DT */
    }
 ...
 *
 *****************************************************************/
int ReconcileAllCmlBondParities(inp_ATOM *at, int num_atoms,
                                int bDisconnected) {
  int i, ret = 0;
  ...
  for (i = 0; i < num_atoms; i++) {
    if (at[i].sb_parity[0] && !visited[i] &&
        !(bDisconnected && is_el_a_metal(at[i].el_number))) {
      if ((ret = ReconcileCmlIncidentBondParities(at, i, -1, visited,
                                                  bDisconnected))) {
        break; /* error */
      }
    }
...
int ReconcileCmlIncidentBondParities(inp_ATOM *at, int cur_atom, int prev_atom,
  ...
  /* visited = 0 or parity => atom has not been visited
               ...
               20 + parity => has been visited; is not on the stack anymore +
     its final parity */
  ...
  int cur_order_parity, nxt_order_parity, cur_sb_parity, nxt_sb_parity,
      bCurMask, bNxtMask;
  /* !(bDisconnected && is_el_a_metal(at[i].el_number) */
  ...
  for (i = 0; i < MAX_NUM_STEREO_BONDS && at[cur_atom].sb_parity[i]; i++) {
    ...
    len =
        get_opposite_sb_atom(at, cur_atom, icur2nxt, &nxt_atom, &inxt2cur, &j);
    if (!len) {
    ...
    /* parity of at[cur_atom].neighbor[] premutation to reach this order: {
     * next_atom, neigh_atom, ...} */

    /* 1. move next_atom  from position=icur2nxt to position=0 =>
     *         icur2nxt permutations
     * 2. move neigh_atom from position=inxt2neigh+(inxt2cur > inxt2neigh) to
     * position=1 => inxt2neigh+(inxt2cur > inxt2neigh)-1 permutations. Note if
     * (inxt2cur > inxt2neigh) then move #1 increments neigh_atom position Note
     * add 4 because icur2neigh may be negative due to isotopic H removal
     */
    cur_order_parity =
        (4 + icur2nxt + icur2neigh + (icur2neigh > icur2nxt)) % 2;
    /* same for next atom: */
    /* parity of at[nxt_atom].neighbor[] premutation to reach this order: {
     * cur_atom, neigh_atom, ...} */
    nxt_order_parity =
        (4 + inxt2cur + inxt2neigh + (inxt2neigh > inxt2cur)) % 2;
    ...
    if (visited[nxt_atom] < 10) {
      ret = ReconcileCmlIncidentBondParities(at, nxt_atom, cur_atom, visited,
                                             bDisconnected);
      if (ret) {
        break;
      }
...
int get_opposite_sb_atom(inp_ATOM *at, int cur_atom, int icur2nxt,
  ...
  while (
      len++ <
      20) { /* arbitrarily set cumulene length limit to avoid infinite loop */
    ...
    if (j) {
      return 0; /* reached atom(s) with stereobond (sb) parity, the opposite
                   atom has not been found */
    }
    if (at[nxt_atom].valence == 2 &&
        2 * BOND_TYPE_DOUBLE == at[nxt_atom].chem_bonds_valence) {
      /* follow cumulene =X= path */
      icur2nxt = (at[nxt_atom].neighbor[0] == cur_atom);
      cur_atom = nxt_atom;
    } else {
      return 0; /* neither atom with a sb parity not middle cumulene could be
                   reached */
    }
  }
