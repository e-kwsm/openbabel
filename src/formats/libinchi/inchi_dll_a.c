...
int ExtractOneStructure(STRUCT_DATA *sd, INPUT_PARMS *ip, char *szTitle,
                        inchi_Input *pInp, INCHI_IOSTREAM *log_file,
...
int CanonOneStructureINChI(INCHIGEN_CONTROL *HGen, int iINChI,
...
int INCHI_DECL INCHIGEN_Setup(INCHIGEN_HANDLE _HGen, INCHIGEN_DATA *pGenData,
                              inchi_Input *pInp)

{
  ...
  retcode = ReadCommandLineParms(argc, argv, ip, szSdfDataValue, &ulDisplTime,
  ...
  retcode = ExtractOneStructure(
      sd, ip, HGen->szTitle, pInp, log_file, HGen->inchi_file, /* output_file */
      prb_file, orig_inp_data, &(HGen->num_inp), HGen->pStr, PSTR_BUFFER_SIZE);
...
/* for execution outside the VC++ debugger uncomment one of the following two */
  ...
  nRet = SortAndPrintINChI(
      output_file, HGen->pStr, PSTR_BUFFER_SIZE, log_file, ip, orig_inp_data,
      prep_inp_data, HGen->composite_norm_data, pOrigStruct, sd->num_components,
      sd->num_non_taut, sd->num_taut, sd->bTautFlags, sd->bTautFlagsDone,
      pncFlags, HGen->num_inp, HGen->pINChI, HGen->pINChI_Aux,
      &bSortPrintINChIFlags, save_opt_bits);

  /* XML struct end tag */
  if ((ip->bINChIOutputOptions & INCHI_OUT_XML) && sd->bXmlStructStarted > 0) {
    if (!OutputINChIXmlStructEndTag(output_file, HGen->pStr, PSTR_BUFFER_SIZE,
                                    1)) {
  ...
  if (sd->pStrErrStruct[0])
    if (pGenData && (pResults->szMessage =
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*/
void INCHI_DECL STDINCHIGEN_Reset(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                                  inchi_Output *pResults) {
  INCHIGEN_Reset(HGen, pGenData, pResults);
}

void INCHI_DECL INCHIGEN_Reset(INCHIGEN_HANDLE _HGen, INCHIGEN_DATA *pGenData,
                               inchi_Output *pResults) {
  int i, k, nc;
  INCHIGEN_CONTROL *HGen = (INCHIGEN_CONTROL *)_HGen;
  ...
  if (NULL != HGen) {
    ...
    for (i = 0; i < MAX_NUM_PATHS; i++) {
      if (HGen->InpParms.path[i]) {
        inchi_free(
            (char *)HGen->InpParms
                .path[i]); /*  cast deliberately discards 'const' qualifier */
    ...
    /*  free INChI memory */
    FreeAllINChIArrays(HGen->pINChI, HGen->pINChI_Aux,
                       HGen->StructData.num_components);
    memset(HGen->pINChI, 0, sizeof(HGen->pINChI));
    memset(HGen->pINChI_Aux, 0, sizeof(HGen->pINChI_Aux));
...
int cdecl_INCHIGEN_Setup(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                         inchi_Input *pInp);
int cdecl_STDINCHIGEN_Setup(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                            inchi_Input *pInp);
int cdecl_INCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen,
                                   INCHIGEN_DATA *pGenData);
int cdecl_STDINCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen,
                                      INCHIGEN_DATA *pGenData);
int cdecl_INCHIGEN_DoCanonicalization(INCHIGEN_HANDLE HGen,
                                      INCHIGEN_DATA *pGenData);
int cdecl_STDINCHIGEN_DoCanonicalization(INCHIGEN_HANDLE HGen,
                                         INCHIGEN_DATA *pGenData);
int cdecl_INCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen,
                                   INCHIGEN_DATA *pGenData,
                                   inchi_Output *pResults);
int cdecl_STDINCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen,
                                      INCHIGEN_DATA *pGenData,
                                      inchi_Output *pResults);
void cdecl_INCHIGEN_Reset(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                          inchi_Output *pResults);
void cdecl_STDINCHIGEN_Reset(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                             inchi_Output *pResults);
...
/********************************************************/
INCHIGEN_HANDLE cdecl_INCHIGEN_Create(void) { return INCHIGEN_Create(); }
/********************************************************/
INCHIGEN_HANDLE cdecl_STDINCHIGEN_Create(void) { return STDINCHIGEN_Create(); }
/********************************************************/
int cdecl_INCHIGEN_Setup(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                         inchi_Input *pInp) {
...
/********************************************************/
void cdecl_INCHIGEN_Destroy(INCHIGEN_HANDLE HGen) { INCHIGEN_Destroy(HGen); }
/********************************************************/
void cdecl_STDINCHIGEN_Destroy(INCHIGEN_HANDLE HGen) {
...
int PASCAL pasc_INCHIGEN_Setup(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                               inchi_Input *pInp);
int PASCAL pasc_STDINCHIGEN_Setup(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                                  inchi_Input *pInp);
int PASCAL pasc_INCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen,
                                         INCHIGEN_DATA *pGenData);
int PASCAL pasc_STDINCHIGEN_DoNormalization(INCHIGEN_HANDLE HGen,
                                            INCHIGEN_DATA *pGenData);
int PASCAL pasc_INCHIGEN_DoCanonicalization(INCHIGEN_HANDLE _HGen,
                                            INCHIGEN_DATA *pGenData);
int PASCAL pasc_STDINCHIGEN_DoCanonicalization(INCHIGEN_HANDLE _HGen,
                                               INCHIGEN_DATA *pGenData);
int PASCAL pasc_INCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen,
                                         INCHIGEN_DATA *pGenData,
                                         inchi_Output *pResults);
int PASCAL pasc_STDINCHIGEN_DoSerialization(INCHIGEN_HANDLE HGen,
                                            INCHIGEN_DATA *pGenData,
                                            inchi_Output *pResults);
void PASCAL pasc_INCHIGEN_Reset(INCHIGEN_HANDLE HGen, INCHIGEN_DATA *pGenData,
                                inchi_Output *pResults);
void PASCAL pasc_STDINCHIGEN_Reset(INCHIGEN_HANDLE HGen,
                                   INCHIGEN_DATA *pGenData,
                                   inchi_Output *pResults);
...
/********************************************************/
INCHIGEN_HANDLE PASCAL pasc_INCHIGEN_Create(void) { return INCHIGEN_Create(); }
/********************************************************/
INCHIGEN_HANDLE PASCAL pasc_STDINCHIGEN_Create(void) {
...
/********************************************************/
int PASCAL pasc_INCHIGEN_DoCanonicalization(INCHIGEN_HANDLE HGen,
                                            INCHIGEN_DATA *pGenData) {
  return INCHIGEN_DoCanonicalization(HGen, pGenData);
} /********************************************************/
int PASCAL pasc_STDINCHIGEN_DoCanonicalization(INCHIGEN_HANDLE HGen,
                                               INCHIGEN_DATA *pGenData) {
  return STDINCHIGEN_DoCanonicalization(HGen, pGenData);
}
/********************************************************/
