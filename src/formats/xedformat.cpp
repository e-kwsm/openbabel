...
bool XEDFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  ttab.SetToType("XED");
  snprintf(buffer, BUFF_SIZE, "%10.3f%10i%10i", mol.GetEnergy(), mol.NumAtoms(),
           mol.NumBonds());
  ofs << buffer << endl;
  ...
  for (i = 0; i < mol.NumBonds(); i++) {
    bond = mol.GetBond(i);
    snprintf(buffer, BUFF_SIZE, "%8i%8i", bond->GetBeginAtomIdx(),
             bond->GetEndAtomIdx());
    ofs << buffer;
    if (!((i + 1) % 5))
      ofs << endl;
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    snprintf(buffer, BUFF_SIZE, "%6i%15.6f%15.6f%15.6f%6i%12.4f", mass,
             atom->GetX(), atom->GetY(), atom->GetZ(), type_name, 0.0);
    ofs << buffer << endl;
  }
