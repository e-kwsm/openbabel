...
public:
  ...
  QChemOutputFormat() { OBConversion::RegisterFormat("qcout", this); }

  const char *Description() override // required
  {
    return "Q-Chem output format\n"
           "Read Options e.g. -as\n"
...
public:
  // Register this format type ID
  QChemInputFormat() { OBConversion::RegisterFormat("qcin", this); }

  const char *Description() override // required
  {
    return "Q-Chem input format\n"
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, "Standard Nuclear Orientation") != nullptr) {
      ...
      if (nbAtomRead != mol.NumAtoms()) {
        std::cerr << "Error: a Standard Nuclear Orientation paragraph contains "
                  << mol.NumAtoms()
                  << " whereas the following Mulliken paragraph contains "
                  << nbAtomRead << " atoms\n";
        return false;
      }
...
bool QChemInputFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ofs << atom->GetAtomicNum() << " " << atom->GetX() << " " << atom->GetY()
        << " " << atom->GetZ() << endl;
  }
  ofs << "$end" << endl;
  ofs << endl << "$rem" << endl;
