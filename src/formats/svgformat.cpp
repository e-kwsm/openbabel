...
public:
  SVGFormat() : _ncols(0), _nrows(0), _nmax(0) {
    ...
    OBConversion::RegisterOptionParam("rows", this, 1,
                                      OBConversion::GENOPTIONS);
    OBConversion::RegisterOptionParam("cols", this, 1,
                                      OBConversion::GENOPTIONS);
  ...
  const char *Description() override {
    return "SVG 2D depiction\n"
           "Scalable Vector Graphics 2D rendering of molecular structure.\n\n"

           "When called from commandline or GUI or otherwise via Convert(),\n"
           "single molecules are displayed at a fixed scale, as in normal "
           "diagrams,\n"
           "but multiple molecules are displayed in a table which expands to "
           "fill\n"
           "the containing element, such as a browser window.\n"
           "When WriteMolecule() is called directly, without going through\n"
           "WriteChemObject, e.g. via OBConversion::Write(), a fixed size "
           "image by\n"
           "default 200 x 200px containing a single molecule is written. The "
           "size\n"
           "can be specified by the P output option.\n\n"

           "Multiple molecules are displayed in a grid of dimensions specified "
           "by\n"
           "the ``-xr`` and ``-xc`` options (number of rows and columns "
           "respectively\n"
           ...
           "If both ``-xr`` and ``-xc`` are specified, they define the maximum "
           "number of\n"
           "molecules that are displayed.\n"
           "If only one of them is displayed, then the other is calculated so "
           "that\n"
           "ALL the molecules are displayed.\n"
           "If neither are specified, all the molecules are output in an\n"
           "approximately square table.\n\n"

           "By default, 2D atom coordinates are generated (using gen2D) unless "
           "they\n"
           "are already present. This can be slow with a large number of "
           "molecules.\n"
           "(3D coordinates are ignored.) Include ``--gen2D`` explicitly if "
           "you wish\n"
           ...
           "    e.g ``-xb yellow`` or ``-xb #88ff00`` ``-xb none`` is "
           "transparent.\n"
           ...
           "    The default is to draw all hetero atoms and terminal C "
           "explicitly,\n"
           ...
           "    These indices are those in sd or mol files and correspond to "
           "the\n"
           "    order of atoms in a SMILES string.\n"
           " j do not embed javascript\n"
           "    Javascript is not usually embedded if there is only one "
           "molecule,\n"
           "    but it is if the rows and columns have been specified as 1: "
           "``-xr1 -xc1``\n"
           ...
           "    Useful if we don't want any extra hydrogens drawn to fill the "
           "valence.\n"
           " A display aliases, if present\n"
           "    This applies to structures which have an alternative, usually\n"
           "    shorter, representation already present. This might have been "
           "input\n"
           "    from an A or S superatom entry in an sd or mol file, or can "
           "be\n"
           ...
           "    would add a aliases COOH and CHO to represent the carboxyl "
           "and\n"
           "    aldehyde groups and would display them as such in the svg "
           "diagram.\n"
           "    The aliases which are recognized are in data/superatom.txt, "
           "which\n"
           "    can be edited.\n"
           ...
           "If the input molecule(s) contain explicit hydrogen, you could "
           "consider\n"
           "improving the appearance of the diagram by adding an option ``-d`` "
           "to make\n"
           "it implicit. Hydrogen on hetero atoms and on explicitly drawn C "
           "is\n"
           "always shown.\n"
           ...
           "would produce a svg file with a black background, with no "
           "explicit\n"
...
bool SVGFormat::WriteChemObject(OBConversion *pConv) {
  ...
  // If the first object is OBText, the part of it before each insertion point
  // (if it exists) is output before every molecule. This allows molecule
  // structures to be displayed in a template. The x option to omit the XML
  // header is set
  ...
  if (pConv->GetOutputIndex() <= 1) {
    ...
    pConv->AddOption("svgbswritechemobject"); // to show WriteMolecule that this
                                              // function has been called
    const char *pc = pConv->IsOption("c");
    // alternative for obabel because -xc cannot take a parameter, because some
    // other format uses it similarly for -xr -xp
    if (!pc)
...
bool SVGFormat::WriteSVG(OBConversion *pConv, vector<OBBase *> &molecules) {
  ...
  if (fixedpx) // fixed size image
    ofs << "x=\"0\" y=\"0\" width=\"" << fixedpx << "px\" height=\"" << fixedpx
        << "px\" ";
  else
  ...
  if (hasTable)
    ...
    ofs << "<title>" << molecules[0]->GetTitle()
        << " - Open Babel Depiction</title>\n";

  // Draw the background unless transparent
  if (!transparent)
    ofs << "<rect x=\"0\" y=\"0\" width=\"" << vbwidth << "\" height=\""
        << vbheight << "\" fill=\"" << background << "\"/>\n";
  ...
  for (iter = _objects.begin(); ret && iter != _objects.end(); ++iter, ++indx) {
    ...
    // Generate coordinates only if no existing 2D coordinates and we're not
    // doing ball-and-stick style
    if ((pConv->IsOption("y") || !pmol->Has2D(true)) &&
        (!pConv->IsOption("n") && !balldepict)) {
      if (!pOp->Do(pmol)) {
        obErrorLog.ThrowError("SVGFormat",
                              string(pmol->GetTitle()) +
    ...
    if (!pmol->Has2D() &&
        pmol->NumAtoms() > 1) // allows 3D coordinates (if passed by -xn above)
    {
      string mes("Molecule ");
      mes += pmol->GetTitle();
    ...
    if (!pConv->IsOption("d")) {
      if (hasTable) {
        molfs << "<text text-anchor=\"middle\" font-size=\"" << 0.06 * cellsize
              ...
              << "x=\"" << innerX + cellsize * 0.5 << "\" y=\""
              << innerY + cellsize - 2.0 / nr << "\" >" << pmol->GetTitle()
              << "</text>\n";
      } else {
...
<script type="text/ecmascript">
  <![CDATA[
    ...
    function wheel(evt){
      ...
      vb[0] = parseFloat(vb[0]) + parseFloat(vb[2])*(1-zoom) *
evt.clientX/innerWidth; vb[1] = parseFloat(vb[1]) + parseFloat(vb[3])*(1-zoom) *
evt.clientY/innerHeight; vb[2] = parseFloat(vb[2]) * zoom; vb[3] =
parseFloat(vb[3]) * zoom; svgEl.setAttributeNS(null, "viewBox", vb.join(" "));
   ...
   svgEl.onmousedown = function(evt) {
    ...
    svgEl.onmousemove=function(evt) {
      if(startx!=0 && starty!=0
        && ((evt.clientX - startx)*(evt.clientX - startx)+(evt.clientY -
starty)*(evt.clientY - starty)>100))
      {
        var vbtext = svgEl.getAttributeNS(null,"viewBox");
        vb = vbtext.split(" ");
