...
bool PNG2Format::WriteChemObject(
    OBConversion *pConv) // Taken from svgformat.cpp
{
  // Molecules are stored here as pointers to OBBase objects, which are not
  // deleted as usual. When there are no more they are sent to WriteMolecule.
  // This allows their number to be determined whatever their source
  //(they may also have been filtered), so that the table can be properly
  //dimensioned.
  ...
  if (pConv->GetOutputIndex() <= 1) {
    ...
    for (vector<OBBase *>::iterator iter = _objects.begin();
         iter != _objects.end(); ++iter)
      delete *iter;
    _objects.clear();
    _nmax = 0;
    //_ncols = _nrows = 1;
    pConv->AddOption("pngwritechemobject"); // to show WriteMolecule that this
  ...
  bool ret = true;
  // Finish if no more input or if the number of molecules has reached the
  // allowed maximum(if specified)
  bool nomore = _nmax && (_objects.size() == _nmax);
  if ((pConv->IsLast() || nomore)) {
    int nmols = _objects.size();
    // Set table properties according to the options and the number of molecules
    // to be output
    if (!(nmols == 0 || // ignore this block if there is no input or
          (_nrows &&
           _ncols) || // if the user has specified both rows and columns or
          ((!_nrows && !_ncols) &&
           nmols ==
               1)) // if neither is specified and there is one output molecule
    ) {
  ...
  // OBConversion decrements OutputIndex when returns false because it thinks it
  // is an error So we compensate.
  if (!ret || nomore)
...
bool PNG2Format::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!pConv->IsOption("pngwritechemobject") ||
      (!_nrows && !_ncols)) { // If WriteMolecule called directly, e.g. from
                              // OBConversion::Write()
  ...
  if (!workingmol.Has2D(true)) {
    ...
    if (!pOp->Do(&workingmol)) {
      obErrorLog.ThrowError("PNG2Format",
                            string(workingmol.GetTitle()) +
                                "- Coordinate generation unsuccessful",
                            obError);
      return false;
    }
  }
  ...
  if (pConv->IsLast()) {
    ...
    else // embedding
    {
      ...
      if (!conv2.SetInAndOutFormats(ppng, ppng) ||
        ...
        obErrorLog.ThrowError("PNG Format", "Failed to embed molecule(s)",
                              obError);
