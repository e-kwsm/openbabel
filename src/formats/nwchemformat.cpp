...
public:
  ...
  NWChemOutputFormat() { OBConversion::RegisterFormat("nwo", this); }

  const char *Description() override // required
  {
    return "NWChem output format\n"
           "Read Options e.g. -as\n"
...
static const char *GEOMETRY_OPTIMIZATION_PATTERN =
...
static const char *MULLIKEN_CHARGES_PATTERN =
...
static const char *MULTIPOLE_MOMENT_PATTERN =
...
public:
  // Register this format type ID
  NWChemInputFormat() { OBConversion::RegisterFormat("nw", this); }

  const char *Description() override // required
  {
    return "NWChem input format\n"
           "No comments yet\n";
  }
...
void NWChemOutputFormat::ReadCoordinates(istream *ifs, OBMol *molecule) {
  ...
  while (vs.size() == 6) {
    ...
    if (from_scratch) {
      ...
      if (atomicNum < 1 || atomicNum > static_cast<int>(OBElements::Oganesson))
        atomicNum = 0;
      ...
      // check atomic number
      if ((i >= natoms) ||
          (molecule->GetAtom(i + 1)->GetAtomicNum() != atoi(vs[2].c_str()))) {
        delete[] coordinates;
        return;
      }
      coordinates[i * 3] = x;
...
void NWChemOutputFormat::ReadTDDFTCalculation(istream *ifs, OBMol *molecule) {
  ...
  while (ifs->getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, ROOT_PATTERN) != nullptr) {
      tokenize(vs, buffer);
      //  Root   1 singlet b2             0.294221372 a.u.                8.0062
      //  eV
      //   0     1    2    3                  4        5                    6 7
      if (vs.size() < 8)
        break;
...
void NWChemOutputFormat::ReadOrbitals(istream *ifs, OBMol *molecule) {
  ...
  while (ifs->getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, ORBITAL_START_PATTERN)) {
      ...
      double occupation =
          atof(vs[2].c_str() + 4); // Start from symbol after '='
      string symbol;
      if (vs.size() > 5)
...
After all stream will be set at the end of calculation.
*/
void NWChemOutputFormat::ReadGeometryOptimizationCalculation(istream *ifs,
                                                             OBMol *molecule) {
  if (molecule == nullptr || ifs == nullptr)
    return;
  vector<string> vs;
...
*/
void NWChemOutputFormat::ReadFrequencyCalculation(istream *ifs,
                                                  OBMol *molecule) {
  if (ifs == nullptr || molecule == nullptr)
    return;
  ...
  while (ifs->getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, VIBRATIONS_TABLE_PATTERN) != nullptr) {
      ...
      ReadMultipoleMoment(ifs, molecule);
             ...
             strstr(buffer, ORBITAL_SECTION_PATTERN_1) != nullptr)
      ReadOrbitals(ifs, molecule);
    else if (strstr(buffer, END_OF_CALCULATION_PATTERN) !=
             nullptr) // End of task
      break;
  }
...
*/
void NWChemOutputFormat::ReadSinglePointCalculation(istream *ifs,
                                                    OBMol *molecule) {
  if (molecule == nullptr || ifs == nullptr)
    return;
  double energy;
  ...
  while (ifs->getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, DFT_ENERGY_PATTERN) != nullptr ||
        strstr(buffer, SCF_ENERGY_PATTERN) != nullptr) {
      tokenize(vs, buffer);
      if (vs.size() > 4) {
  ...
  while (ifs->getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, NEB_BEAD_START_PATTERN) != nullptr) {
      ...
      if (current_bead >= nbeads) {
        cerr << "Current bead out of range: " << current_bead << " of "
             << nbeads << endl;
        break;
      }
    ...
    if (strstr(buffer, NBEADS_PATTERN) != nullptr) {
      ...
      while (vsize > 7) {
        ...
        for (unsigned int i = 0; i < natoms; i++) {
          ...
          //  Symbol              X     Y     Z
          //    0                 1     2     3
          if ((vs.size() < 4) || (molecule->GetAtom(i + 1)->GetAtomicNum() !=
                                  OBElements::GetAtomicNum(vs[0].c_str())))
        ...
        if (vs.size() <= 1) // blank line
        {
          ...
          molecule->SetEnergies(energies);
          molecule->SetConformers(beads);
          unsigned int ts_position = distance(
              energies.begin(), max_element(energies.begin(), energies.end()));
...
bool NWChemOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, GEOMETRY_PATTERN) != nullptr) {
      ...
      if (mol.NumAtoms() == 0 ||
          pConv->IsOption("f", OBConversion::INOPTIONS) != nullptr) {
    ...
    else if (strstr(buffer, SCF_CALCULATION_PATTERN) !=
             strstr(buffer, DFT_CALCULATION_PATTERN))
