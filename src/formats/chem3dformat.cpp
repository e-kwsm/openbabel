...
public:
  ...
  CHEM3D1Format() { OBConversion::RegisterFormat("c3d1", this); }

  const char *Description() override // required
  {
    return "Chem3D Cartesian 1 format\n"
           "No comments yet\n";
  }
  ...
  static bool ReadChem3d(istream &ifs, OBMol &mol, bool mmads,
                         const char *type_key);
...
public:
  // Register this format type ID
  CHEM3D2Format() { OBConversion::RegisterFormat("c3d2", this); }

  const char *Description() override // required
  {
    return "Chem3D Cartesian 2 format\n"
           "No comments yet\n";
  }
...
bool CHEM3D1Format::ReadChem3d(istream &ifs, OBMol &mol, bool mmads,
                               const char *type_key) {
  ...
  if (mmads) {
    ...
    switch (vs.size()) {
    case 7:
      sscanf(buffer, "%d%lf%lf%lf%lf%lf%lf", &natoms, &Alpha, &Beta, &Gamma, &A,
             &B, &C);
      m.FillOrth(Alpha, Beta, Gamma, A, B, C);
      has_fractional = true;
      break;
    case 8:
      sscanf(buffer, "%d%lf%lf%lf%lf%lf%lf%lf", &natoms, &Alpha, &Beta, &Gamma,
             &A, &B, &C, &exponent);
      m.FillOrth(Alpha, Beta, Gamma, A, B, C);
      has_fractional = true;
      has_divisor = true;
  ...
  for (i = 1; i <= natoms; i++) {
    ...
    x = y = z = 0.0;
    int matched =
        sscanf(buffer, "%15s%*d%lf%lf%lf%15s", atomic_type, &x, &y, &z, tmp);
    if (matched < 4) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Problems reading a Chem3D file: truncated "
                            "atom record (need type and x/y/z).",
                            obError);
      return false;
    }
    v.Set(x, y, z);
...
bool CHEM3D1Format::WriteChem3d(ostream &ofs, OBMol &mol, const char *mol_typ) {
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    strncpy(ele_type, OBElements::GetSymbol(atom->GetAtomicNum()),
            sizeof(ele_type));
    ele_type[sizeof(ele_type) - 1] = '\0';
    snprintf(buffer, BUFF_SIZE, "%-3s %-5d %8.4f  %8.4f  %8.4f %5s", ele_type,
             atom->GetIdx(), atom->x(), atom->y(), atom->z(), type_name);
    ofs << buffer;
