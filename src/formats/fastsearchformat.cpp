...
/// \brief Prepares and searches of fingerprint-based index files. See
/// FastSearch class for details
...
public:
  ...
  {
    return "Fastsearch format\n"
           "Fingerprint-aided substructure and similarity searching\n\n"

           "Writing to the fs format makes an index of a multi-molecule "
           "datafile::\n\n"
           "      obabel dataset.sdf -ofs\n\n"
           "This prepares an index :file:`dataset.fs` with default parameters, "
           "and is slow\n"
           "(~30 minutes for a 250,000 molecule file).\n\n"

           "However, when reading from the fs format searches are much faster, "
           "a few seconds,\n"
           "and so can be done interactively.\n\n"
           "The search target is the parameter of the ``-s`` option and can "
           "be\n"
           ...
           "      obabel index.fs -O outfile.yyy -at15  -sSMILES  # best 15 "
           "molecules\n"
           "      obabel index.fs -O outfile.yyy -at0.7 -sSMILES  # Tanimoto "
           ">0.7\n"
           "      obabel index.fs -O outfile.yyy -at0.7,0.9 -sSMILES\n"
           "      #     Tanimoto >0.7 && Tanimoto < 0.9\n\n"
           "The datafile plus the ``-ifs`` option can be used instead of the "
           "index file.\n\n"
           "NOTE on 32-bit systems the datafile MUST NOT be larger than "
           "4GB.\n\n"
           "Dative bonds like -[N+][O-](=O) are indexed as -N(=O)(=O), and "
           "when searching\n"
           ...
           " f# Fingerprint type\n"
           "     If not specified, the default fingerprint (currently FP2) is "
           "used\n"
  ...
  unsigned int Flags() override {
    return READBINARY | READONEONLY | WRITEBINARY;
  }
...
private:
  bool ObtainTarget(OBConversion *pConv, std::vector<OBMol> &patternMols,
...
bool FastSearchFormat::ReadChemObject(OBConversion *pConv) {
  ...
  bool exactmatch =
      pConv->IsOption("e", OBConversion::INOPTIONS) != nullptr; // -ae option
  ...
  bool isgzip = false;
  if (!pConv->SetInAndOutFormats(
          pConv->FormatFromExt(datafilename.c_str(), isgzip),
          pConv->GetOutFormat()))
    return false;
  ...
  // If target has dative bonds like -[N+](=O)[O-] convert it to the uncharged
  // form
  // (-N(=O)=O and add uncharged form to vector of mols which are sent to
  // the -s (SMARTS)filter.
  // Also check whether the target has dative bonds in the uncharged form and
  // supply the charged form to the -s filter. Together with the automatic
  // conversion to the uncharged form when the fs index is made, this ensures
  // that both forms are found however they occur in the datafile or the taget.
  ...
  if (p) {
    ...
    for (itr = SeekposMap.rbegin(); itr != SeekposMap.rend(); ++itr) {
      ...
      if (pConv->IsOption("a", OBConversion::INOPTIONS)) {
        ...
        stringstream ss;
        ss << " " << itr->first;
        pConv->AddOption("addtotitle", OBConversion::GENOPTIONS,
                         ss.str().c_str());
      }
    ...
    if (exactmatch) {
      ...
      // ensure that SMARTS filter in transform.cpp looks only for an exact
      // match by setting an option with the number of heavy atoms in the
      // pattern mol included.
      stringstream ss;
      ss << patternMols[0].NumHvyAtoms();
      pConv->AddOption("exactmatch", OBConversion::GENOPTIONS,
                       ss.str().c_str());
    }
      ...
      clog << SeekPositions.size()
           << " candidates from fingerprint search phase" << endl;
    }

    vector<unsigned long>::iterator seekitr, begin = SeekPositions.begin(),
                                             end = SeekPositions.end();

    if (patternMols.size() >
        1) // only sort and eliminate duplicates if necessary
    {
      sort(begin, end);
      end = unique(begin, end); // removed duplicates are after new end
    }

    // Output the candidate molecules, filtering through s filter, unless it was
    // not requested
  ...
  if (fsi == nullptr) {
    ...
    if (pConv->GetInGzipped()) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Fastindex search requires an uncompressed input "
                            "file so it can quickly seek to a record.",
                            obWarning);
    }
    ...
    if (!pConv->IsLastFile()) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "There should not be multiple input files. A .fs "
                            "file is an index of a single datafile.",
                            obError);
      return false;
    }
    ...
    if (nmols > 500000) {
      ...
      if (sizeof(void *) < 8 && filesize > 4294967295u) {
        obErrorLog.ThrowError(
            __FUNCTION__, "The datafile must not be larger than 4GB", obError);
        return false;
      }
      is->seekg(origpos);
    }
...
///////////////////////////////////////////////////////////////
bool FastSearchFormat::ObtainTarget(OBConversion *pConv,
                                    vector<OBMol> &patternMols,
  ...
  //  If there is no -s -S or -aS option, information on the index file is
  //  displayed.
  ...
  // If no -s option, make OBMol from file in -S option or -aS option (both
  // deprecated)
  if (!p) {
  ...
  if (p) {
    ...
    if (vec.size() == 0) {
      obErrorLog.ThrowError(__FUNCTION__, "Missing argument for -s/-S",
                            obError);
      return false;
    }
    ...
    // if false, have a valid patternMol from a file
    {
      ...
      for (;;) {
        ...
        int atno;
        if (pos2 != string::npos &&
            (atno = atoi(txt.substr(pos1 + 2, pos2 - pos1 - 2).c_str())) &&
            atno > 0)
          txt.replace(pos1, pos2 - pos1 + 1, OBElements::GetSymbol(atno));
        else {
          obErrorLog.ThrowError(__FUNCTION__, "Ill-formed [#n] atom in SMARTS",
                                obError);
          return false;
        }
      }
      ...
      if ((hasTildeBond =
               (txt.find('~') !=
                string::npos))) // extra parens to indicate truth value
      {
        // Find ~ bonds and make versions of query molecule with a single and
        // aromatic bonds To avoid having to parse the SMILES here, replace ~ by
        // $ (quadruple bond) and then replace this in patternMol. Check first
        // that there are no $ already Sadly, isocynanides may have $ bonds.
        if (txt.find('$') != string::npos) {
          obErrorLog.ThrowError(
              __FUNCTION__,
              "Cannot use ~ bonds in patterns with $ (quadruple) bonds.)",
              obError);
          return false;
        }
      ...
      if (!patternConv.ReadString(&patternMol, vec[0])) {
        obErrorLog.ThrowError(__FUNCTION__, "Cannot read the SMILES string",
                              obError);
        return false;
      }
      if (hasTildeBond) {
        AddPattern(patternMols, patternMol,
                   0); // recursively add all combinations of tilde bond values
        return true;
      }
    } else {
  ...
  if (!p) {
    ...
    clog << indexname << " is an index of\n " << header.datafilename
         ...
         << "(-s option in GUI is 'Convert only if match SMARTS or mols in "
            "file')"
         << endl;
    return false;
  }
...
void FastSearchFormat::AddPattern(vector<OBMol> &patternMols, OBMol patternMol,
                                  int idx) {
  // Recursive function to generate all combinations of aromatic/single bonds
  // for each tilde bond Copying an OBMol, which happens when adding it to a
  // vector, kekulizes it,
  //  changing aromatic (bo=5) bonds. So set order after adding. Should work
  //  here, but is dangerous if the vector needs to be reallocated.
