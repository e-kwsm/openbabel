...
// The average molecular masses used by InChI are listed in util.c or the InChI
// Technical Manual Appendix 1
const unsigned int MAX_AVG_MASS = 134;
const unsigned int inchi_avg_mass[MAX_AVG_MASS + 1] = {
    0,   1,   4,   7,   9,   11,  12,  14,  16,  19,  20,  23,  24,  27,  28,
    31,  32,  35,  40,  39,  40,  45,  48,  51,  52,  55,  56,  59,  59,  64,
    65,  70,  73,  75,  79,  80,  84,  85,  88,  89,  91,  93,  96,  98,  101,
    103, 106, 108, 112, 115, 119, 122, 128, 127, 131, 133, 137, 139, 140, 141,
    144, 145, 150, 152, 157, 159, 163, 165, 167, 169, 173, 175, 178, 181, 184,
    186, 190, 192, 195, 197, 201, 204, 207, 209, 209, 210, 222, 223, 226, 227,
    232, 231, 238, 237, 244, 243, 247, 247, 251, 252, 257, 258, 259, 260, 261,
...
bool InChIFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 0; i < out.num_atoms; ++i) {
    ...
    // Now use the implicit H info provided by InChI code to make explicit H in
    // OBMol, assign spinMultiplicity, then remove the hydrogens to be
    // consistent with old way. Add implicit hydrogen. m=0 is non-istopic H
    // m=1,2,3 are isotope specified
  ...
  // pmol->AssignSpinMultiplicity(true); //true means no implicit H (TODO - Spin
  // stuff)
  ...
  for (i = 0; i < out.num_stereo0D; ++i) {
    ...
    case INCHI_StereoType_DoubleBond: {
      ...
      ct->refs = OBStereo::MakeRefs(stereo.neighbor[0], start,
    ...
    case INCHI_StereoType_Tetrahedral: {
      ...
      if (ts->from ==
          ts->center) // Handle the case where there are only three neighbours
    ...
    default:
      obErrorLog.ThrowError(
          "InChI code", "Unsupported stereo type has been ignored.", obWarning);
    }
  }
...
bool InChIFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pmol->HasData("inchi") && pConv->IsOption("r") == nullptr &&
      pConv->IsOption("a") == nullptr && pConv->IsOption("s") == nullptr &&
      pConv->IsOption("X") == nullptr && pConv->IsOption("F") == nullptr &&
    ...
    if (mol.GetDimension() == 3 ||
        (mol.GetDimension() == 2 &&
         pConv->IsOption("s", pConv->OUTOPTIONS) != nullptr))
    ...
    for (patom = mol.BeginAtom(itr); patom; patom = mol.NextAtom(itr)) {
      ...
      for (pbond = patom->BeginBond(itr); pbond; pbond = patom->NextBond(itr)) {
        ...
        if ((from_cit == from.end() &&
             patom->GetIdx() != pbond->GetBeginAtomIdx()) ||
      ...
      if (patom->GetIsotope()) {
        iat.isotopic_mass = ISOTOPIC_SHIFT_FLAG + patom->GetIsotope() -
                            GetInChIAtomicMass(patom->GetAtomicNum());
      } else
        iat.isotopic_mass = 0;
      iat.radical = patom->GetSpinMultiplicity();
      // InChI doesn't recognize spin miltiplicity of 4 or 5 (as used in OB for
      // CH and C atom)
      if (iat.radical >= 4)
        iat.radical = 0;
        ...
        if (static_cast<OBStereoBase *>(*data)->GetType() ==
          ...
          if (config.specified) {
            ...
            if (config.from == OBStereo::ImplicitRef)
              num_implicit = 1;
            num_implicit += std::count(config.refs.begin(), config.refs.end(),
            ...
            if (num_implicit) {
              config =
                  ts->GetConfig(OBStereo::ImplicitRef); // Make the 'from' atom
                                                        // the implicit ref
            ...
            for (int i = 0; i < 3; ++i)
              stereo.neighbor[i + 1] =
                  OBAtomIdToInChIAtomId(mol, config.refs[i]);
        ...
        if (static_cast<OBStereoBase *>(*data)->GetType() ==
          ...
          if (config.specified) {
            ...
            if (refs[0] == OBStereo::ImplicitRef &&
              ...
              continue;
            if (refs[2] == OBStereo::ImplicitRef &&
                refs[3] == OBStereo::ImplicitRef)
              continue;
    ...
    if (ret != inchi_Ret_OKAY) {
      if (inout.szMessage) {
        ...
        Trim(mes);
        if (!mes.empty())
          obErrorLog.ThrowError("InChI code", molID.str() + ':' + mes,
                                obWarning);
      }
  ...
  if (pConv->IsOption(
          "K")) // Generate InChIKey and add after InChI on same line
  {
    char szINCHIKey[28];
  ...
  // Note that inout.szInChI will still be NULL if this is an InChI->InChIKey
  // conversion and so the following section will not apply.
  if (inout.szInChI != nullptr) {
    if (pConv->IsOption("a"))
      ofs << inout.szAuxInfo << endl;

    if (pConv->IsOption("l"))
      // Display InChI log message. With multiple molecules, it appears only
      // once
    ...
    if (pConv->IsOption("e")) {
      if (pConv->GetOutputIndex() == 1)
        ...
        ofs << InChIErrorMessage(
                   CompareInchi(firstInchi.c_str(), inout.szInChI))
            << endl;
      }
    }
    FreeStdINCHI(&inout);
...
char InChIFormat::CompareInchi(const string &Inchi1, const string &Inchi2) {
  // Returns 0 if identical or an char identifying the layer where they first
  // differed
...
// Returns pointer to InChI options string, which needs to be deleted with
// delete[] If there are no options returns an empty string
char *InChIFormat::GetInChIOptions(OBConversion *pConv, bool Reading) {
  ...
  if(!Reading && !pConv->IsOption("n"))
    //without -xn option, the default is to write using these 'recommended'
  options tokenize(optsvec, "FixedH RecMet SPXYZ SAsXYZ Newps Fb Fnud");
  */
  char *opts;
  OBConversion::Option_type opttyp =
      Reading ? OBConversion::INOPTIONS : OBConversion::OUTOPTIONS;
  const char *copts = pConv->IsOption("X", opttyp);
  if (copts) {
    string tmp(
        copts); // GCC doesn't like passing string temporaries to functions
...
bool InChIFormat::EditInchi(std::string &inchi, std::string &spec) {
  ...
  for (itr = vec.begin(); itr != vec.end(); ++itr) {
    if (*itr == "formula") {
      ...
      obErrorLog.ThrowError(
          __FUNCTION__, spec + " not recognized as a truncation specification",
          obError, onceOnly);
      return false;
    }
  }
...
void InChIFormat::RemoveLayer(std::string &inchi, const std::string &str,
                              bool all) {
