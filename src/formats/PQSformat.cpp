...
public:
  ...
  {
    return "Parallel Quantum Solutions format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override { return "http://www.pqs-chem.com/"; }
...
int ReadPQS_geom(istream &ifs, OBMol &mol, const char *title, int input_style,
  ...
  while (!geom_found && ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "geom") != nullptr &&
      ...
      if (strstr(buffer, "file=") != nullptr) { // external geometry file
        ...
        errorMsg << "External geometry file referenced: " << full_coord_path
                 << endl;
        ...
        if (!coordFileStream) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "Cannot read external geometry file!", obError);
          return (false);
          //                    exit (-1);
        } else {
          ...
          */
          ...
          atom_count = ReadPQS_geom(coordFileStream, mol, title, input_style,
                                    bohr_to_angstrom);
        }
      }
    }
  ...
  if (geom_found) {
    ...
    if (atom_count == 0) { // try .coord file
      ...
      if (!coordFileStream) {
        ...
        atom_count =
            ReadPQS_geom(coordFileStream, mol, title, 0, bohr_to_angstrom);
    }
  } else
    obErrorLog.ThrowError(__FUNCTION__,
                          "Error reading PQS file.  GEOM card not found!",
                          obWarning);
