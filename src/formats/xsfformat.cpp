...
public:
  ...
  {
    return "XCrySDen Structure Format\n"
           "Read Options e.g. -as\n"
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "ATOMS") != nullptr) {
      ...
      while (vs.size() >= 4) {
        ...
        atomPositions.push_back(
            vector3(x, y, z)); // we may have a movie or animation

        ifs.getline(buffer, BUFF_SIZE);
        tokenize(vs, buffer);
      }
      createdAtoms = true; // don't run NewAtom() anymore
    } else if (strstr(buffer, "PRIMVEC") || strstr(buffer, "CONVVEC")) {
      // translation vectors
      numTranslationVectors = 0; // if we have an animation
      while (numTranslationVectors < 3 && ifs.getline(buffer, BUFF_SIZE)) {
        tokenize(vs,
                 buffer); // we really need to check that it's 3 entries only
        if (vs.size() < 3)
