...
public:
  const char *GetMIMEType() override { return "chemical/x-daylight-smiles"; }
  ...
  int SkipObjects(int n, OBConversion *pConv) override {
    if (n == 0)
      return 1; // already points after current line
...
public:
  ...
  SMIFormat() {
    ...
    OBConversion::RegisterOptionParam(
        "C", this); // "anti-canonical" form (random order)
  }
  const char *Description() override {
    return "SMILES format\n"
           "A linear text format which can describe the connectivity and "
           "chirality of a molecule\n"
           "Open Babel implements the `OpenSMILES specification "
           "<http://opensmiles.org>`_.\n\n"

           "It also implements an extension to this specification for "
           "radicals.\n\n"

           "Note that the ``l <atomno>`` option, used to specify a \"last\" "
           "atom, is\n"
           "intended for the generation of SMILES strings to which additional "
           "atoms\n"
           "will be concatenated. If the atom specified has an explicit H "
           "within a bracket\n"
           "(e.g. ``[nH]`` or ``[C@@H]``) the output will have the H removed "
           "along with any\n"
           "associated stereo symbols.\n\n"

           ".. seealso::\n\n"

           "  The :ref:`Canonical_SMILES_format` produces a canonical "
           "representation\n"
           "  of the molecule in SMILES format. This is the same as the ``c`` "
           "option\n"
           ...
           "     Ordering should be specified like 4-2-1-3 for a 4-atom "
           "molecule.\n"
           "     This gives canonical labels 1,2,3,4 to atoms 4,2,1,3 "
           "respectively,\n"
           "     so that atom 4 will be visited first and the remaining atoms\n"
           "     visited in a depth-first manner following the lowest "
           "canonical labels.\n"
           ...
           "     Canonicalization can take a while for symmetric molecules and "
           "a\n"
           ...
           "     option is that it avoids aromaticity perception, thus "
           "speeding\n"
           "     up reading SMILES.\n"
           "  S  Clean stereochemistry\n"
           "     By default, stereochemistry is accepted as given. If you "
           "wish\n"
...
public:
  ...
  const char *Description() override {
    return "Canonical SMILES format\n"
...
class OBSmilesParser {
  ...
  map<OBBond *, StereoRingBond>
      _stereorbond; // Remember info on the stereo ring closure bonds

  // stereochimistry
  bool chiralWatch; // set when a tetrahedral atom is read
  map<OBAtom *, OBTetrahedralStereo::Config *>
      _tetrahedralMap; // map of tetrahedral atoms and their data
  map<OBBond *, char>
      _upDownMap; // store the '/' & '\' as they occurred in smiles
  map<unsigned int, char>
      _chiralLonePair; // for atoms with potential chiral lone pairs, remember
                       // when the l.p. was encountered
  bool squarePlanarWatch; // set when a square planar atom is read
  map<OBAtom *, OBSquarePlanarStereo::Config *> _squarePlanarMap;

public:
  OBSmilesParser(bool preserve_aromaticity = false)
      : _preserve_aromaticity(preserve_aromaticity), _rxnrole(1) {}
  ~OBSmilesParser() {}
...
bool OBSmilesParser::ParseSmiles(OBMol &mol, const std::string &smiles) {
  mol.SetAromaticPerceived(); // Turn off perception until the end of this
                              // function
  mol.BeginModify();
  ...
  for (_ptr = smiles.c_str(); *_ptr; _ptr++) {
    ...
    case '\r':
      if (*(_ptr + 1) ==
          '\0') // may have a terminating '\r' due to Windows line-endings
        break;
      return false;
    case '0':
    case '1':
  ...
  if (_rxnrole > 1 && _rxnrole != 3) {
    mol.EndModify();
    stringstream errorMsg;
    errorMsg << "Invalid reaction SMILES string: only a single '>' sign found "
                "(two required to be valid).";
    obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
    return false; // invalid SMILES since rings aren't properly closed
  }
  if (mol.IsReaction()) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    if (hcount == -1) { // Apply SMILES implicit valence model
      ...
      FOR_BONDS_OF_ATOM(bond, &(*atom)) { bosum += bond->GetBondOrder(); }
  ...
  // Unset any aromatic bonds that *are not* in rings where the two aromatic
  // atoms *are* in a ring This is rather subtle, but it's correct and reduces
  // the burden of kekulization
  ...
  if (!_tetrahedralMap.empty()) {
    ...
    for (ChiralSearch = _tetrahedralMap.begin();
         ChiralSearch != _tetrahedralMap.end(); ++ChiralSearch) {
      ...
      if (ts->refs[2] == OBStereo::NoRef) {
        // This happens where there is chiral lone pair or where there simply
        // aren't enough connections around a chiral atom. We handle the case
        // where there is a S with a chiral lone pair. All other cases are
        // ignored, and raise a warning. (Note that S can be chiral even without
        // a lone pair, think of C[S@](=X)(=Y)Cl.

        // We have remembered where to insert the lone pair in the
        // _chiralLonePair map
        map<unsigned int, char>::iterator m_it =
            _chiralLonePair.find(atom->GetIdx());
        if (CanHaveLonePair(atom->GetAtomicNum()) &&
            m_it != _chiralLonePair.end()) {
          ts->refs[2] = ts->refs[1];
          ts->refs[1] = ts->refs[0];
          ...
          stringstream ss;
          ss << "Ignoring stereochemistry. Not enough connections to this "
                "atom. "
             << mol.GetTitle();
  ...
  if (!_squarePlanarMap.empty()) {
    ...
    for (ChiralSearch = _squarePlanarMap.begin();
         ChiralSearch != _squarePlanarMap.end(); ++ChiralSearch) {
      atom = ChiralSearch->first;
...
char OBSmilesParser::SetRingClosureStereo(StereoRingBond rcstereo,
                                          OBBond *dbl_bond) {
  ...
  // partner bond on the double bond (e.g. (b) below), then the stereo is
  // unspecified.

  // (a) C/C=C/1NC\1 -- specified stereo
  // (b) C/C=C/1NC/1  -- ignore ring closure stereo => treated as C/C=C1NC1  =>
  // CC=C1NC1 (c) C/C=C/1\NC/1 -- ignore ring closure stereo => treated as
  // C/C=C1\NC1 => C/C=C/1\NC1
  ...
  if (rcstereo.updown[0] == BondUpChar ||
      rcstereo.updown[0] ==
          BondDownChar) { // Is there a stereo symbol at the opening?
  ...
  if (rcstereo.updown[1] == BondUpChar ||
      rcstereo.updown[1] ==
          BondDownChar) { // Is there a stereo symbol at the closing?
    ...
    if (!found) {
      ...
      obErrorLog.ThrowError(__FUNCTION__,
                            "Ignoring the cis/trans stereochemistry specified "
                            "for the ring closure\n  as it is inconsistent.",
                            obWarning);
      found = false;
    }
  }
...
void OBSmilesParser::CreateCisTrans(OBMol &mol) {
  ...
  FOR_BONDS_OF_MOL(dbi, mol) {
    ...
    vector<bool> bond_stereo(2, true); // Store the stereo of the chosen bonds
                                       // at each end of the dbl bond
    vector<OBBond *> stereo_bond(2,
                                 nullptr); // These are the chosen stereo bonds
    vector<OBBond *> other_bond(
        2, nullptr); // These are the 'other' bonds at each end

    for (int i = 0; i < 2;
         ++i) { // Loop over each end of the double bond in turn
      ...
      FOR_BONDS_OF_ATOM(bi, dbl_bond_atoms[i]) {
        ...
        if (sb_it == _stereorbond.end()) // Not a ring closure
          ...
          stereo = !(IsUp(b) ^ (b->GetNbrAtomIdx(dbl_bond_atoms[i]) <
                                dbl_bond_atoms[i]->GetIdx()));
        ...
        if (stereo_bond[i] == nullptr) { // This is a first stereo bond
          ...
          if (stereo !=
              bond_stereo[i]) { // Verify that the other stereo bond (on the
                                // same atom) has opposite stereo
            other_bond[i] = b;  // Use this for the 'other' bond
          } else {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Error in cis/trans stereochemistry "
                                  "specified for the double bond\n",
                                  obWarning);
    ...
    if (stereo_bond[0] == nullptr || stereo_bond[1] == nullptr)
      continue; // No cis/trans

    // other_bond will contain NULLs if there are bonds to implicit hydrogens
    unsigned int second = (other_bond[0] == nullptr)
                              ? OBStereo::ImplicitRef
                              : other_bond[0]->GetNbrAtom(a1)->GetId();
    unsigned int fourth = (other_bond[1] == nullptr)
                              ? OBStereo::ImplicitRef
                              : other_bond[1]->GetNbrAtom(a2)->GetId();
    ...
    // If bond_stereo[0]==bond_stereo[1], this means cis for stereo_bond[0] and
    // stereo_bond[1].
    if (bond_stereo[0] == bond_stereo[1])
      cfg.refs =
          OBStereo::MakeRefs(stereo_bond[0]->GetNbrAtom(a1)->GetId(), second,
...
void OBSmilesParser::InsertTetrahedralRef(OBMol &mol, unsigned long id) {
  ...
  if (ChiralSearch != _tetrahedralMap.end() &&
      ChiralSearch->second != nullptr) {
    int insertpos =
        NumConnections(ChiralSearch->first, id == OBStereo::ImplicitRef) -
        2; // -1 indicates "from"
    if (insertpos > 2)
      return;
    if (insertpos < 0) {
      if (ChiralSearch->second->from != OBStereo::NoRef)
        obErrorLog.ThrowError(
            __FUNCTION__, "Warning: Overwriting previous from reference id.",
            obWarning);

      (ChiralSearch->second)->from = id;
      // cerr << "Adding " << id << " at Config.from to " <<
      // ChiralSearch->second << endl;
    } else {
      if (ChiralSearch->second->refs[insertpos] != OBStereo::NoRef)
        obErrorLog.ThrowError(
            __FUNCTION__, "Warning: Overwriting previously set reference id.",
            obWarning);

      (ChiralSearch->second)->refs[insertpos] = id;
      // cerr << "Adding " << id << " at " << insertpos << " to " <<
      // ChiralSearch->second << endl;
    }
  }
}
...
void OBSmilesParser::InsertSquarePlanarRef(OBMol &mol, unsigned long id) {
  ...
  if (ChiralSearch != _squarePlanarMap.end() &&
      ChiralSearch->second != nullptr) {
    ...
    default:
      obErrorLog.ThrowError(__FUNCTION__,
                            "Warning: Square planar stereo specified for atom "
                            "with more than 4 connections.",
                            obWarning);
      break;
    }
  }
...
bool OBSmilesParser::ParseSimple(OBMol &mol) {
  ...
  default: {
    ...
    obErrorLog.ThrowError(__FUNCTION__, err, obError);
    return false;
  }
  }
  ...
  if (_prev) // need to add bond
  {
    ...
    if (arom && prevatom->IsAromatic() && _order == 0)
      mol.AddBond(_prev, mol.NumAtoms(), 1,
                  OB_AROMATIC_BOND); // this will be kekulized later
    else
...
bool OBSmilesParser::ParseComplex(OBMol &mol) {
  ...
  for (_ptr++; *_ptr && *_ptr != ']'; _ptr++) {
    ...
    case '@':
      ...
      if (*_ptr == 'S' && _ptr[1] == 'P') { // @SP1/2/3
        ...
        if (_squarePlanarMap.find(atom) ==
            _squarePlanarMap
                .end()) // Prevent memory leak for malformed smiles (PR#3428432)
    ...
    case '-':
      if (charge) {
        obErrorLog.ThrowError(__FUNCTION__, "Charge can only be specified once",
                              obWarning);
        return false;
      }
      while (*++_ptr == '-')
  ...
  if (charge) {
    ...
    if (abs(charge) > 10 ||
        (element && charge > element)) { // if the charge is +/- 10 or more than
                                         // the number of electrons
      errorMsg << "Atom " << atom->GetIdx() << " had an unrealistic charge of "
               << charge << "." << endl;
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
    }
  }
  ...
  if (_prev) // need to add bond
  {
    ...
    if (chiralWatch) { // if tetrahedral atom, set previous as from atom
      ...
      // cerr <<"NB7: line 1622: Added atom ref "<<_prev<<" at " << 0 << " to
      // "<<_mapcd[atom]<<endl;
    }
    if (squarePlanarWatch) { // if squareplanar atom, set previous atom as first
                             // ref
    ...
    if (chiralWatch &&
        CanHaveLonePair(
            element)) // Handle chiral lone pair (only S at the moment)
...
bool OBSmilesParser::CapExternalBonds(OBMol &mol) {
  ...
  for (bond = _extbond.begin(); bond != _extbond.end(); ++bond) {
    ...
    if (mol.HasData(OBGenericDataType::ExternalBondData)) {
      xbd = (OBExternalBondData *)mol.GetData(
...
bool OBSmilesParser::ParseRingBond(OBMol &mol) {
  ...
  if (!prevatom) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Number not parsed correctly as a ring bond", obWarning);
    return false;
  }
  ...
  if (*_ptr == '%') {
    ...
    if (*_ptr == '(') { // %(NNN) extension to OpenSMILES
      ...
      if (!isdigit(*_ptr) || !isdigit(*(_ptr + 1))) {
        obErrorLog.ThrowError(__FUNCTION__, "Two digits expected after %",
                              obWarning);
        return false;
      }
  ...
  for (bond = _rclose.begin(); bond != _rclose.end(); ++bond) {
    if (bond->digit == digit) {
      ...
      upDown = (_updown > bond->updown) ? _updown : bond->updown;
      bondOrder = (_order > bond->order) ? _order : bond->order;
      // Check if this ring closure bond may be aromatic and set order
      // accordingly
      ...
      mol.AddBond(bond->prev, _prev, bondOrder == 0 ? 1 : bondOrder,
                  aromatic_bond ? OB_AROMATIC_BOND : 0, bond->numConnections);
      // store up/down
      if (upDown == BondUpChar || upDown == BondDownChar)
        _upDownMap[mol.GetBond(bond->prev, _prev)] = upDown;
      ...
      // after adding a bond to atom "_prev"
      // search to see if atom is bonded to a chiral atom
      // need to check both _prev and bond->prev as closure is direction
      // independent
      InsertTetrahedralRef(mol, bond->prev - 1);
      InsertSquarePlanarRef(mol, bond->prev - 1);
      ...
      if (ChiralSearch != _tetrahedralMap.end() &&
        ...
        case -1:
          if (ChiralSearch->second->from != OBStereo::NoRef)
            obErrorLog.ThrowError(
                __FUNCTION__,
                "Warning: Overwriting previous from reference id.", obWarning);
        ...
        case 2:
          if (ChiralSearch->second->refs[insertpos] != OBStereo::NoRef)
            obErrorLog.ThrowError(
                __FUNCTION__,
                "Warning: Overwriting previously set reference id.", obWarning);
          (ChiralSearch->second)->refs[insertpos] = mol.GetAtom(_prev)->GetId();
          break;
        default:
          obErrorLog.ThrowError(__FUNCTION__,
                                "Warning: Tetrahedral stereo specified for "
                                "atom with more than 4 connections.",
                                obWarning);
          break;
        }
      }
  ...
  ringClosure.numConnections =
      NumConnections(atom); // store position to insert closure bond
...
int OBSmilesParser::NumConnections(OBAtom *atom, bool isImplicitRef) {
  ...
  int idx = atom->GetIdx();
  // Need to adjust for any implicit H (e.g. [C@@H]) but only for atoms after
  // the H. The following line controls for this. It uses the fact the _hcount
  // is only set after this function is called to handle inserting the stereo
  // ref for the implicit H.
...
OBBondClosureInfo::OBBondClosureInfo(OBAtom *a1, OBAtom *a2, OBBond *b, int rd,
                                     bool open) {
  toatom = a1;
  fromatom = a2;
  bond = b;
...
public:
  ...
  int Size() { return (_child_nodes.empty() ? 0 : _child_nodes.size()); }
  ...
  OBAtom *GetChildAtom(int i) { return (_child_nodes[i]->GetAtom()); }

  OBBond *GetChildBond(int i) { return (_child_bonds[i]); }

  OBCanSmiNode *GetChildNode(int i) { return (_child_nodes[i]); }
};
...
struct OutOptions {
  ...
  OutOptions(bool _isomeric, bool _kekulesmi, bool _showatomclass,
             bool _showexplicitH, bool _smarts, const char *_ordering)
      : isomeric(_isomeric), kekulesmi(_kekulesmi),
        showatomclass(_showatomclass), showexplicitH(_showexplicitH),
        smarts(_smarts), ordering(_ordering) {}
};
...
public:
  ...
  ~OBMol2Cansmi() { delete _stereoFacade; }

  void Init(OBMol *pmol, bool canonicalOutput = true,
            OBConversion *pconv = nullptr);
  ...
  const char *GetSquarePlanarStereo(OBCanSmiNode *,
                                    vector<OBAtom *> &chiral_neighbors,
                                    vector<unsigned int> &symmetry_classes);
  bool GetSmilesElement(OBCanSmiNode *, vector<OBAtom *> &chiral_neighbors,
                        vector<unsigned int> &symmetry_classes, std::string &);
  int GetSmilesValence(OBAtom *atom);
  int GetUnusedIndex();
  vector<OBBondClosureInfo>
  GetCanonClosureDigits(OBAtom *atom, OBBitVec &frag_atoms,
                        vector<unsigned int> &canonical_order);
  bool IsSuppressedHydrogen(OBAtom *atom);
  void ToCansmilesString(OBCanSmiNode *node, std::string &buffer,
                         OBBitVec &frag_atoms,
                         vector<unsigned int> &symmetry_classes,
                         vector<unsigned int> &canonical_order);
  bool HasStereoDblBond(OBBond *, OBAtom *atom);
  void MyFindChildren(OBMol &mol, vector<OBAtom *> &children, OBBitVec &seen,
                      OBAtom *end);
...
void OBMol2Cansmi::CreateCisTrans(OBMol &mol) {
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (ct && ct->GetConfig().specified) {
      ...
      OBBond *dbl_bond = mol.GetBond(mol.GetAtomById(config.begin),
                                     mol.GetAtomById(config.end));
      if (!dbl_bond)
        continue;
      // Do not output cis/trans bond symbols for double bonds in rings of size
      // IMPLICIT_CIS_RING_SIZE or less
      unsigned int boundedringsize =
          OBBondGetSmallestRingSize(dbl_bond, IMPLICIT_CIS_RING_SIZE);
      if (boundedringsize ==
          0) // either not in ring at all, or not in small ring
...
bool OBMol2Cansmi::HasStereoDblBond(OBBond *bond, OBAtom *atom) {
  ...
  if (atom->HasDoubleBond()) {
    ...
    if (nbr_atom->HasDoubleBond())
      // Check whether the nbr_atom is a begin or end in any CisTransStereo. If
      // so, then the ring opening already had the symbol.
      for (ChiralSearch = _cistrans.begin(); ChiralSearch != _cistrans.end();
           ++ChiralSearch) {
...
char OBMol2Cansmi::GetCisTransBondSymbol(OBBond *bond, OBCanSmiNode *node) {
  ...
  // If this bond is in two different obcistransstereos (e.g. a conjugated
  // system) choose the one where the dbl bond atom is *atom (i.e. the one which
  // comes first)
  std::vector<OBCisTransStereo>::iterator ChiralSearch;
  ...
  if (_isup.find(bond) == _isup.end()) // No it hasn't
  {
    ...
    for (ChiralSearch = _unvisited_cistrans.begin();
      ...
      if (lookup != cfg.refs.end() &&
          (cfg.begin == centeratom ||
           cfg.end == centeratom)) { // Atoms endatom and centeratom are in this
                                     // OBCisTransStereo

        std::vector<OBBond *> refbonds(4, nullptr);
        refbonds[0] = mol->GetBond(mol->GetAtomById(cfg.refs[0]),
                                   mol->GetAtomById(cfg.begin));

        if (cfg.refs[1] != OBStereo::ImplicitRef) // Could be a hydrogen
          refbonds[1] = mol->GetBond(mol->GetAtomById(cfg.refs[1]),
                                     mol->GetAtomById(cfg.begin));

        if (cfg.refs[2] != OBStereo::ImplicitRef) // Could be a hydrogen
          refbonds[2] = mol->GetBond(mol->GetAtomById(cfg.refs[2]),
                                     mol->GetAtomById(cfg.end));

        if (cfg.refs[3] != OBStereo::ImplicitRef) // Could be a hydrogen
          refbonds[3] = mol->GetBond(mol->GetAtomById(cfg.refs[3]),
                                     mol->GetAtomById(cfg.end));
        ...
        for (int i = 0; i < 4; i++)
          if (_isup.find(refbonds[i]) !=
              _isup.end()) // We have already set this one (conjugated bond)
...
bool OBMol2Cansmi::GetSmilesElement(OBCanSmiNode *node,
  ...
  // Don't suppress any explicit Hs attached if the atom is an H itself (e.g.
  // [H][H]) or -xh was specified
  if (atom->GetAtomicNum() != OBElements::Hydrogen && !options.showexplicitH) {
    FOR_NBORS_OF_ATOM(nbr, atom) {
      if (nbr->GetAtomicNum() == OBElements::Hydrogen &&
          (!options.isomeric || nbr->GetIsotope() == 0) &&
          nbr->GetExplicitDegree() == 1 && nbr->GetFormalCharge() == 0 &&
          (!options.showatomclass || !nbr->GetData("Atom Class")))
  ...
  if (options.smarts) {
    ...
    if (!bracketElement) {
      if (element == 0) { // asterisk is always hypervalent but we don't bracket
                          // it unless has Hs
        if (numImplicitHs > 0)
          bracketElement = true;
      } else {
  ...
  if (atom->GetFormalCharge() != 0                // charged elements
      ...
      || (options.showatomclass &&
          atom->HasData("Atom Class"))) // If the molecule has Atom Class data
                                        // and -xa option set and atom has data
    bracketElement = true;
  ...
  if (!bracketElement) {
    ...
    if (element) {
      ...
      if ((!options.kekulesmi && atom->IsAromatic()) || // aromatic atom
          (atom->GetSpinMultiplicity() &&
           _pconv->IsOption("r"))) // Radical centres lowercase if r option set
      {
        buffer += symbol[0] + ('a' - 'A');
        if (symbol[1])
          buffer += symbol[1];
    ...
    // Atomic number zero - either '*' or an external atom
      ...
      vector<pair<int, pair<OBAtom *, OBBond *>>> *externalBonds =
          (vector<pair<int, pair<OBAtom *, OBBond *>>> *)((OBMol *)
                                                              atom->GetParent())
              ->GetData("extBonds");
      ...
      if (externalBonds) // TODO: This code has bit-rotted and needs some love
        for (externalBond = externalBonds->begin();
             externalBond != externalBonds->end(); ++externalBond) {
          if (externalBond->second.first == atom) {
            ...
            if (bond->GetBondOrder() == 2 &&
                !bond->IsAromatic()) // TODO: need to check for kekulesmi
              buffer += '=';
  ...
  // If chiral, append '@' or '@@'...unless we're creating a SMARTS ("s") and
  // it's @H or @@H
  ...
  if (hcount > 0 &&
      (atom == _endatom ||
       (atom == _startatom &&
        !options.ordering))) // Leave a free valence for attachment
    hcount--;
  if (hcount > 0) {
  ...
  if (options.showatomclass) {
    ...
    if (data) {
      OBPairInteger *acdata = dynamic_cast<OBPairInteger *>(
          data); // Could replace with C-style cast if willing to live
                 // dangerously
      if (acdata) {
...
bool OBMol2Cansmi::AtomIsChiral(OBAtom *atom) {
  ...
  return _stereoFacade->HasTetrahedralStereo(atomid) ||
         _stereoFacade->HasSquarePlanarStereo(atomid);
}
...
const char *
OBMol2Cansmi::GetTetrahedralStereo(OBCanSmiNode *node,
                                   vector<OBAtom *> &chiral_neighbors,
                                   vector<unsigned int> &symmetry_classes) {
  ...
  if (!atomConfig.specified ||
      (atomConfig.specified && atomConfig.winding == OBStereo::UnknownWinding))
    return nullptr;
  ...
  for (vector<OBAtom *>::const_iterator atom_it = chiral_neighbors.begin() + 1;
    ...
    else // Handle a chiral lone pair, represented by a NULL OBAtom* in
         // chiral_neighbors
...
const char *
OBMol2Cansmi::GetSquarePlanarStereo(OBCanSmiNode *node,
                                    vector<OBAtom *> &chiral_neighbors,
                                    vector<unsigned int> &symmetry_classes) {
  ...
  OBSquarePlanarStereo *sp =
      _stereoFacade->GetSquarePlanarStereo(atom->GetId());
  ...
  // create a Config struct with the chiral_neighbors in canonical output order
  OBStereo::Refs canonRefs = OBStereo::MakeRefs(
      chiral_neighbors[0]->GetId(), chiral_neighbors[1]->GetId(),
...
//! Adaptation of OBMol::FindChildren to allow a vector of OBAtoms to be passed
//! in
//  MOVE THIS TO OBMOL FOR 2.4
void OBMol2Cansmi::MyFindChildren(OBMol &mol, vector<OBAtom *> &children,
                                  OBBitVec &seen, OBAtom *end) {
  OBBitVec curr, next;
...
// If the bond order *is* 1, then only if the bond is in a ring and between
// aromatic atoms
...
bool OBMol2Cansmi::BuildCanonTree(OBMol &mol, OBBitVec &frag_atoms,
                                  vector<unsigned int> &canonical_order,
                                  OBCanSmiNode *node) {
...
#if DEBUG
  cout << "BuildCanonTree: " << OBElements::GetSymbol(atom->GetAtomicNum())
       << ", " << atom->GetIdx() << ", canorder "
       << canonical_order[atom->GetIdx() - 1] << "\n";
#endif
  ...
  if (options.ordering)
    favor_multiple = false; // Visit in strict canonical order (if using
  ...
  for (nbr = atom->BeginNbrAtom(i); nbr; nbr = atom->NextNbrAtom(i)) {
    ...
    idx = nbr->GetIdx();
    // if (nbr->GetAtomicNum() == OBElements::Hydrogen &&
    // IsSuppressedHydrogen(nbr)) {
    //   _uatoms.SetBitOn(nbr->GetIdx());        // mark suppressed hydrogen, so
    //   it won't be considered continue;                               // later
    //   when looking for more fragments.
    // }
    ...
    for (ai = sort_nbrs.begin(); ai != sort_nbrs.end(); ++ai) {
      ...
      if (favor_multiple && new_needs_bsymbol && !sorted_needs_bsymbol) {
        ...
        ai = sort_nbrs
                 .begin(); // insert invalidated ai; set it to fail next test
        break;
      }
      if ((!favor_multiple || new_needs_bsymbol == sorted_needs_bsymbol) &&
  ...
  if (_endatom && !_uatoms.BitIsSet(_endatom->GetIdx()) &&
      sort_nbrs.size() > 1) {
    ...
    for (vector<OBAtom *>::iterator it = sort_nbrs.begin();
         it != sort_nbrs.end(); ++it)
      if (std::find(children.begin(), children.end(), *it) == children.end() &&
          *it != _endatom)
        front.push_back(*it);
      else
        end.push_back(*it);
...
vector<OBBondClosureInfo>
OBMol2Cansmi::GetCanonClosureDigits(OBAtom *atom, OBBitVec &frag_atoms,
                                    vector<unsigned int> &canonical_order) {
  ...
  for (bond1 = atom->BeginBond(i); bond1; bond1 = atom->NextBond(i)) {
    ...
    // PR#1999348
    if ((nbr1->GetAtomicNum() == OBElements::Hydrogen &&
         IsSuppressedHydrogen(nbr1)) ||
  ...
  for (bi = vbonds.begin(); bi != vbonds.end(); ++bi) {
    ...
    int bo = (bond1->IsAromatic())
                 ? 1
                 : bond1->GetBondOrder(); // CJ: why was this line added?  bo is
                                          // never used?
    _vopen.push_back(
        OBBondClosureInfo(bond1->GetNbrAtom(atom), atom, bond1, digit, true));
    vp_closures.push_back(
        OBBondClosureInfo(bond1->GetNbrAtom(atom), atom, bond1, digit, true));
  }
...
int OBMol2Cansmi::GetSmilesValence(OBAtom *atom) {
  ...
  FOR_NBORS_OF_ATOM(nbr, atom) {
    if (nbr->GetAtomicNum() != OBElements::Hydrogen || nbr->GetIsotope() != 0 ||
 ...
 *       in" for the "C" in the first, even though the actual carbon atom
 *appears after the Bromine atom in the second string.
 ***************************************************************************/

void OBMol2Cansmi::ToCansmilesString(OBCanSmiNode *node, std::string &buffer,
                                     OBBitVec &frag_atoms,
                                     vector<unsigned int> &symmetry_classes,
                                     vector<unsigned int> &canonical_order) {
  OBAtom *atom = node->GetAtom();
  ...
  vector<OBBondClosureInfo> vclose_bonds =
      GetCanonClosureDigits(atom, frag_atoms, canonical_order);

  // First thing: Figure out chirality.  We start by creating a vector of the
  // neighbors in the order in which they'll appear in the canonical SMILES
  // string.  This is more complex than you'd guess because of implicit/explicit
  // H and ring-closure digits.

  // Don't include chiral symbol on _endatom or _startatom.
  // Otherwise, we end up with C[C@@H](Br)(Cl), where the C has 4 neighbours
  // already and we cannot concatenate another SMILES string without creating a
  // 5-valent C.

  bool is_chiral = AtomIsChiral(atom);
  if (is_chiral && atom != _endatom && atom != _startatom) {

    // If there's a parent node, it's the first atom in the ordered
    // neighbor-vector used for chirality.
    if (node->GetParent()) {
    ...
    // lone pair. If it exists (and we won't know for sure until we've counted
    // up all the neighbours) it will go in here
    int lonepair_location = chiral_neighbors.size();

    // Ok, done with all that. Next in the SMILES will be the ring-closure
    // characters. So we need to find the corresponding atoms and add them to
    // the list. (We got the canonical ring-closure list earlier.)
    ...
    if (chiral_neighbors.size() == 3 &&
        CanHaveLonePair(atom->GetAtomicNum())) // Handle sulfur
      chiral_neighbors.insert(chiral_neighbors.begin() + lonepair_location,
                              nullptr);
  }
  ...
  if (!vclose_bonds.empty()) {
    ...
    for (bci = vclose_bonds.begin(); bci != vclose_bonds.end(); ++bci) {
      if (!bci->is_open) { // Ring closure
        ...
        if (bs[0])
          ...
          case 1:
            if (!bci->bond->IsAromatic() && bci->bond->IsInRing() &&
                bci->bond->GetBeginAtom()->IsAromatic() &&
                bci->bond->GetEndAtom()->IsAromatic())
              buffer += '-';
            break;
          case 2:
        ...
        if (!HasStereoDblBond(bci->bond,
                              bci->bond->GetNbrAtom(node->GetAtom())))
          bs[0] = GetCisTransBondSymbol(bci->bond, node);
        if (bs[0])
          buffer += bs; // append "/" or "\"
      }
  ...
  for (int i = 0; i < node->Size(); i++) {
    ...
    case 1:
      ...
      if (cc[0] != '\0') {
        ...
        // Write a single bond symbol if not aromatic but end atoms are both
        // aromatic This will speed up reading as it will avoid ring perception
        // around line 563 (bond->IsInRing())
        // TODO: Consider making the test for IsInRing() an option
        if (!bond->IsAromatic() && bond->IsInRing() &&
            bond->GetBeginAtom()->IsAromatic() &&
            bond->GetEndAtom()->IsAromatic())
          buffer += '-';
      }
      break;
    ...
    ToCansmilesString(node->GetChildNode(i), buffer, frag_atoms,
                      symmetry_classes, canonical_order);
    if (i + 1 < node->Size() || node->GetAtom() == _endatom)
      buffer += ')';
  }
}
...
void StandardLabels(OBMol *pMol, OBBitVec *frag_atoms,
  ...
  FOR_ATOMS_OF_MOL(atom, *pMol) {
    if (frag_atoms->BitIsSet(atom->GetIdx())) {
      ...
      labels.push_back(
          OBStereo::ImplicitRef); // to match situation when canonical ordering.
                                  // Just a big number?
...
//! in the first or last position. In contrast, mytokenize treats each instance
//! of the delimiter as the end/start of a new token.
...
bool OBMol2Cansmi::ParseInChI(OBMol &mol, vector<int> &atom_order) {
  ...
  MolConv.SetAuxConv(nullptr); // temporary until a proper OBConversion copy
                               // constructor written
  ...
  size_t rm_start =
      splitlines.at(0).find("/r"); // Correct for reconnected metal if necessary
  if (rm_start == string::npos) {
    ...
    aux_part =
        splitlines.at(1).substr(rm_start_b); // Use the reconnected metal labels
  }
  ...
  for (vector<string>::iterator it = s_components.begin();
       it != s_components.end(); ++it) {
    tokenize(s_atoms, *it, ",");
    vector<int> atoms;
    for (vector<string>::iterator itb = s_atoms.begin(); itb != s_atoms.end();
         ++itb)
  ...
  if (f_start != string::npos) {
    ...
    for (vector<string>::iterator it = s_components.begin();
      ...
      if (*(it->rbegin()) == 'm') {
        ...
        tokenize(s_atoms, *it, ",");
        vector<int> atoms;
        for (vector<string>::iterator itb = s_atoms.begin();
             itb != s_atoms.end(); ++itb)
          atoms.push_back(atoi(itb->c_str()));
  ...
  for (vector<vector<int>>::iterator it = canonical_labels.begin();
       it != canonical_labels.end(); ++it) {
...
void OBMol2Cansmi::CreateFragCansmiString(OBMol &mol, OBBitVec &frag_atoms,
                                          std::string &buffer) {
  ...
  if (ppo) {
    ...
    if (s_atom_order.size() != mol.NumHvyAtoms())
      ...
      for (vector<string>::const_iterator cit = s_atom_order.begin();
           cit != s_atom_order.end(); ++cit)
  ...
  if (_canonicalOutput) {
    ...
    */
    ...
    if (timeoutString) {
      ...
      if (!(ss >> maxSeconds)) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "Canonicalization timeout should be a number",
                              obWarning);
        maxSeconds = 5;
      }
    }
    ...
    if (_pconv->IsOption("C")) { // "C" == "anti-canonical form"
      RandomLabels(&mol, frag_atoms, symmetry_classes, canonical_order);
               ...
               universal_smiles) { // user-specified or InChI canonical labels
      canonical_order.resize(mol.NumAtoms());
      symmetry_classes.resize(mol.NumAtoms());
      int idx = 3; // Start the labels at 3 (to leave space for special values
                   // 0, 1 and 2)
      for (int i = 0; i < atom_order.size(); ++i)
        if (canonical_order[atom_order[i] - 1] ==
            0) { // Ignore ring closures (for "U")
      ...
      for (int i = 0; i < canonical_order.size(); ++i)
        if (canonical_order[i] == 0) { // Explicit hydrogens
          if (mol.GetAtom(i + 1)->GetAtomicNum() == OBElements::Hydrogen &&
            ...
            canonical_order[i] =
                mol.GetAtom(i + 1)->GetIsotope() - 1; // i.e. 1 or 2
  ...
  // lowest unmarked canorder atom in the current reaction role, and starts
  // there to generate a SMILES. Repeats until no atoms remain unmarked.

  bool new_rxn_role =
      false; // flag to indicate whether we have started a new reaction role
  ...
  while (1) {
    if (_pconv->IsOption("R"))
      _bcdigit =
          0; // Reset the bond closure index for each disconnected component
    ...
    // If we specified a startatom_idx & it's in this fragment, use it to start
    // the fragment
    if (_startatom)
    ...
    if (root_atom == nullptr) {
      for (atom = mol.BeginAtom(ai); atom; atom = mol.NextAtom(ai)) {
        int idx = atom->GetIdx();
        if ( // atom->GetAtomicNum() != OBElements::Hydrogen       // don't
            ...
            && (!isrxn ||
                rxn.GetRole(atom) == rxnrole) // skip atoms not in this rxn role
      ...
      // For Inchified or Universal SMILES, if the start atom is an [O-]
      // attached to atom X, choose any =O attached to X instead.
      //          Ditto for [S-] and =S.
      if ((_pconv->IsOption("I") || universal_smiles) && root_atom &&
          root_atom->GetFormalCharge() == -1 &&
          root_atom->GetExplicitDegree() == 1 && root_atom->HasSingleBond() &&
          (root_atom->GetAtomicNum() == OBElements::Oxygen ||
        ...
        FOR_NBORS_OF_ATOM(nbr, central) {
          if (root_atom == &*nbr)
            continue;
          if (nbr->GetAtomicNum() == root_atom->GetAtomicNum() &&
              nbr->GetExplicitDegree() == 1 && nbr->HasDoubleBond()) {
            root_atom = &*nbr;
            break;
          }
        }
    ...
    ToCansmilesString(root, buffer, frag_atoms, symmetry_classes,
                      canonical_order);
    delete root;
  }
}
...
void CreateCansmiString(OBMol &mol, std::string &buffer, OBBitVec &frag_atoms,
                        OBConversion *pConv) {
  bool canonical = pConv->IsOption("c") != nullptr;

  OutOptions options(!pConv->IsOption("i"), pConv->IsOption("k"),
                     pConv->IsOption("a"), pConv->IsOption("h"),
                     pConv->IsOption("s"), pConv->IsOption("o"));
  ...
  if (!options.showexplicitH) {
    ...
    FOR_ATOMS_OF_MOL(iatom, mol) {
      OBAtom *atom = &(*iatom);
      if (frag_atoms.BitIsSet(atom->GetIdx()) &&
          atom->GetAtomicNum() == OBElements::Hydrogen &&
...
bool SMIBaseFormat::GetInchifiedSMILESMolecule(OBMol *mol,
                                               bool useFixedHRecMet) {
  OBConversion MolConv;
...
bool SMIBaseFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pConv->IsOption("I")) {
    ...
    if (!success) {
      ofs << NewLine();
      obErrorLog.ThrowError(
          __FUNCTION__, "Cannot generate Universal NSMILES for this molecule",
          obError);
      return false;
    }
  }
  ...
  // with 1's so that the SMILES will be for the whole molecule.
    ...
    FOR_ATOMS_OF_MOL(a, *pmol) { fragatoms.SetBitOn(a->GetIdx()); }
  }
...
bool FIXFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(a, mol) { allbits.SetBitOn(a->GetIdx()); }
  ...
  for (j = 0; j < mol.NumConformers(); j++) {
    ...
    for (unsigned int index = 0; index < canonical_order.size(); ++index) {
      ...
      snprintf(coords, 100, "%9.3f %9.3f %9.3f", atom->GetX(), atom->GetY(),
               atom->GetZ());
      ofs << coords << endl;
    }
  }
