...
public:
  ...
  const char *Description() override {
    return "MDL RXN format\n"
           "The MDL reaction format is used to store information on chemical "
           "reactions.\n\n"
           ...
           "            - agent - Treat as an agent (default). Note that some "
           "programs\n"
  ...
  const char *GetMIMEType() override { return "chemical/x-mdl-rxn"; }
...
bool RXNFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  //{
  //  //Read a MOL file	using the same OBConversion object but with a different
  //  format if (!pMolFormat->ReadMolecule(pmol, pConv))
  //    obErrorLog.ThrowError(__FUNCTION__, "Failed to read a reactant",
  //    obWarning);
  ...
  for (unsigned int N = 0; N < 3; N++) {
    ...
    for (int i = 0; i < num_components; i++) {
      // Read a MOL file	using the same OBConversion object but with a
      // different format
      OBMol mol;
      ...
      if (mol.NumAtoms() == 0) {
        OBAtom *dummy = mol.NewAtom(); // Treat the empty OBMol as having a
                                       // single dummy atom
...
enum HandleAgent { AS_AGENT, IGNORE, AS_REACT, AS_PROD, BOTH_REACT_AND_PROD };
...
static HandleAgent ReadAgentOption(const char *t) {
  ...
  switch (t[0]) {
  case 'a':
    if (t[1] == 'g' && t[2] == 'e' && t[3] == 'n' && t[4] == 't' &&
        t[5] == '\0')
      return AS_AGENT;
    break;
  case 'i':
    if (t[1] == 'g' && t[2] == 'n' && t[3] == 'o' && t[4] == 'r' &&
        t[5] == 'e' && t[6] == '\0')
      return IGNORE;
    break;
  case 'r':
    if (t[1] == 'e' && t[2] == 'a' && t[3] == 'c' && t[4] == 't' &&
        t[5] == 'a' && t[6] == 'n' && t[7] == 't' && t[8] == '\0')
      return AS_REACT;
    break;
  case 'p':
    if (t[1] == 'r' && t[2] == 'o' && t[3] == 'd' && t[4] == 'u' &&
        t[5] == 'c' && t[6] == 't' && t[7] == '\0')
      return AS_PROD;
    break;
  case 'b':
...
static void WriteAgents(OBMol &mol, OBReactionFacade &rxnfacade,
                        OBConversion *pconv, OBFormat *pformat) {
...
bool RXNFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  OBFormat *pMolFormat = pConv->FindFormat("MOL");
  if (pMolFormat == nullptr) {
    obErrorLog.ThrowError(__FUNCTION__, "MDL MOL format not available",
                          obError);
    return false;
  }
  ...
  if (hasAgent &&
      (handleagent == BOTH_REACT_AND_PROD || handleagent == AS_REACT))
    agentInReactants = true;
  else
    agentInReactants = false;
  if (hasAgent &&
      (handleagent == BOTH_REACT_AND_PROD || handleagent == AS_PROD))
