...
public:
  ...
  PainterFormat() { OBConversion::RegisterFormat("paint", this); }

  const char *Description() override // required
  {
    return "Painter format\n"
           ...
           "Note that the origin is considered to be in the top left "
           "corner.\n\n"

           "The following image was drawn using the information\n"
           "in this format as described at\n"
           "http://baoilleach.blogspot.co.uk/2012/04/"
           "painting-molecules-your-way-introducing.html:\n\n"
...
bool PainterFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!workingmol.Has2D(true)) {
    ...
    if (!pOp) {
      obErrorLog.ThrowError("PainterFormat", "gen2D not found", obError,
                            onceOnly);
      return false;
    }
