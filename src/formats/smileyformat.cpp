...
struct OpenBabelCallback : public Smiley::CallbackBase {
  enum UpDown { IsNotUpDown, IsUp, IsDown };
  ...
  void addAtom(int element, bool aromatic, int isotope, int hCount, int charge,
    ...
    // isotope: -1, 0, 1, 2, 3, 4 ... (-1 means no isotope specified and is not
    // the same as 0) hCount: -1, 0, 1, 2, ..., 9 (-1 means default valence and
    // is only for unbracketed atoms) charge: -9, -8, ..., -1, 0, 1, ..., 8, 9
  ...
  void addBond(int source, int target, int order, bool isUp, bool isDown) {
    ...
    // calling addAtom(). This ensures that the bond's atom indexes are always
    // valid.
  ...
  void setChiral(int index, Smiley::Chirality chirality,
    ...
    // chiralNbrs: atom indices of neighbors, size 4 for TH, AL and SP, size 5
    // for TB and 6 for OH
...
public:
  SmileyFormat() { OBConversion::RegisterFormat("smy", this); }

  const char *Description() override // required
  {
    return "SMILES format using Smiley parser\n\n"

           "The Smiley parser presents an alternative to the standard SMILES "
           "parser\n"
           "(:ref:`SMILES_format`). It was written to be strictly compatible "
           "with the\n"
           "OpenSMILES standard (http://opensmiles.org). In comparison, the "
           "standard\n"
           "parser is more forgiving to erroneous input, and also supports "
           "some extensions\n"
           "such as for radicals.\n\n"

           "In addition, the Smiley parser returns detailed error messages "
           "when problems\n"
           "arise parsing or validating the SMILES, whereas the standard "
           "parser seldom\n"
           "describes the specific problem. For a detailed description of the "
           "OpenSMILES\n"
           "semantics, the specification should be consulted. In addition to "
           "syntactical\n"
           "and grammatical correctness, the Smiley parser also verifies some "
           "basic\n"
           "semantics.\n"
           "\n"
           "Here are some examples of the errors reported::\n"
           "\n"
           "   SyntaxError: Bracket atom expression contains invalid trailing "
           "characters.\n"
           "   F.FB(F)F.[NH2+251][C@@H](CP(c1ccccc1)c1ccccc1)C(C)(C)C "
           "31586112\n"
           "                  ^^\n"
           ...
           "~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
           "Hydrogen atoms can not have a hydrogen count. Hydrogen bound to a "
           "hydrogen\n"
           ...
           "~~~~~~~~~~~~~~~~~~~~~~\n"
           "When the bond type for ring bonds are explicitly specified at both "
           "ends,\n"
           ...
           "Invalid Ring Bonds\n"
           "~~~~~~~~~~~~~~~~~~\n"
           "There are two types of invalid ring bonds. The first is when two "
           "atoms both\n"
           "have the same two ring bonds. This would mean adding a parallel "
           "edge in the\n"
           "graph which is not allowed. The second type is similar but results "
           "in a\n"
           ...
           "~~~~~~~~~~~~~~~~~~~~~~\n"
           "When an atom is specified as being chiral, it should have the "
           "correct\n"
           "number of neighboring atoms (possibly including an implicit H "
           "inside the\n"
           "bracket.\n"
           "\n"
           ...
           "  Octahedral(OH)            : 6\n"
           "\n"
           "  (*) The chiral atom has only 2 bonds but the neighbor's "
           "neighbors are\n"
           "      counted: NC(Br)=[C@AL1]=C(F)I\n"
           "\n"
           "Invalid Chiral Hydrogen Count\n"
           "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
           "Chiral atoms can only have one hydrogen in their bracket since "
           "multiple\n"
  ...
  const char *SpecificationURL() override { return "http://opensmiles.org"; }
...
private:
  void CreateCisTrans(OBMol *mol,
                      const std::vector<OpenBabelCallback::UpDown> &upDown);
  bool AssignNbrAtoms(const std::vector<OpenBabelCallback::UpDown> &upDown,
                      OBAtom *atom, unsigned long &above, unsigned long &below);
};
...
bool SmileyFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (space_pos != std::string::npos) {
    while (space_pos < smiles.size() &&
           (smiles[space_pos] == ' ' || smiles[space_pos] == '\t'))
      ++space_pos;
...
bool SmileyFormat::AssignNbrAtoms(
    const std::vector<OpenBabelCallback::UpDown> &upDown, OBAtom *atom,
    unsigned long &aboveId, unsigned long &belowId) {
...
void SmileyFormat::CreateCisTrans(
    OBMol *mol, const std::vector<OpenBabelCallback::UpDown> &upDown) {
  FOR_BONDS_OF_MOL(doubleBond, mol) {
    ...
    // std::cout << "double bond: " << source->GetIndex() << " " <<
    ...
    if (aboveSource == OBStereo::ImplicitRef &&
        belowSource == OBStereo::ImplicitRef)
      continue;
    ...
    if (aboveTarget == OBStereo::ImplicitRef &&
        belowTarget == OBStereo::ImplicitRef)
      continue;

    // std::cout << "refs: " << aboveSource << " " << aboveTarget << " " <<
    // belowTarget << " " << belowSource << std::endl;

    OBCisTransStereo *stereo = new OBCisTransStereo(mol);
    stereo->SetConfig(OBCisTransStereo::Config(
