...
public:
  ...
  ADFOutputFormat() { OBConversion::RegisterFormat("adfout", this); }

  const char *Description() override // required
  {
    return "ADF output format\n"
           "Read Options e.g. -as\n"
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, "Coordinates (Cartesian)") != nullptr) {
      ...
      while (strstr(buffer, "----") == nullptr && vs.size() >= 8) {
        atom = mol.NewAtom();
        atom->SetAtomicNum(OBElements::GetAtomicNum(
            vs[1].c_str())); // atom number, then symbol
      ...
      ifs.getline(buffer, BUFF_SIZE); // blank line
      ifs.getline(buffer,
                  BUFF_SIZE); // actual components  Vector: ###  #### ###
      tokenize(vs, buffer);
      if (vs.size() >= 5) {
...
public:
  // Register this format type ID
  ADFInputFormat() { OBConversion::RegisterFormat("adf", this); }

  const char *Description() override // required
  {
    return "ADF cartesian input format\n"
           "Read Options e.g. -as\n"
           "  s  Output single bonds only\n"
           "  b  Disable bonding entirely\n\n";
  }
  ...
  const char *SpecificationURL() override {
    return "http://www.scm.com/Doc/Doc2007.01/ADF/ADFUsersGuide/page32.html";
  } // optional

  //*** This section identical for most OBMol conversions ***
  ////////////////////////////////////////////////////
  /// The "API" interface functions
  bool ReadMolecule(OBBase *pOb, OBConversion *pConv) override { return false; }
...
public:
  // Register this format type ID
  ADFBandFormat() { OBConversion::RegisterFormat("adfband", this); }
  ...
  const char *SpecificationURL() override {
    return "https://www.scm.com/product/band_periodicdft/";
  } // optional
...
public:
  // Register this format type ID
  ADFDftbFormat() { OBConversion::RegisterFormat("adfdftb", this); }
  ...
  const char *SpecificationURL() override {
    return "https://www.scm.com/product/dftb/";
  } // optional
...
public:
  ...
  bool WriteMolecule(OBBase *, OBConversion *) override { return false; }

private:
  /// Utility function that eats all the remaining characters on the current and
  /// next line.
  ...
  void MatVecMul(const double xColumn[3], const double yColumn[3],
                 const double zColumn[3], double v[3]) {
    double t[3];
    t[0] = v[0];
    t[1] = v[1];
    t[2] = v[2];
...
bool OBT41Format::ReadBinary(OBBase *pOb, OBConversion *pConv) {
  obErrorLog.ThrowError(__FUNCTION__,
                        "OpenBabel does not currently support the TAPE41 "
                        "binary format. Please use dmpkf to convert to ASCII.",
                        obError);
  return false;
}
...
bool OBT41Format::ReadASCII(OBBase *pOb, OBConversion *pConv) {
  ...
  if (gd) {
    ...
    // If we don't find any legitimate data, we'll end and still have
    // "OBGridData" rather than a legitimate label
    t41Data = NewData(gd);
  ...
  if (buf != "labels") {
    obErrorLog.ThrowError(__FUNCTION__,
                          "no 'labels' after second Geometry tag");
    return false;
  }
  eol(ifs);
  ...
  for (unsigned int i = 0; i != numAtoms; ++i) {
    ...
    cout << atoms[i].coord[0] << ' ' << atoms[i].coord[1] << ' '
         << atoms[i].coord[2] << endl;
  }
  // charge
  buf = "";
  while (buf != "Geometry")
  ...
  if (buf != "length") {
    obErrorLog.ThrowError(__FUNCTION__,
                          "no 'unit of length' after fifth Geometry tag");
    return false;
  }
  eol(ifs);
  double scale = 1.0;
  ...
  for (unsigned int i = 0; i < numAtoms; ++i) {
    ...
    atom->SetVector(atoms[i].coord[0], atoms[i].coord[1], atoms[i].coord[2]);
...
OBT41Format::GridData OBT41Format::ReadGridData(istream &is) const {
  ...
  int n = 0;
  is >> n;
  if (gd.numPoints[0] * gd.numPoints[1] * gd.numPoints[2] != n)
    return gd;
  // x-vector
  if (!NextTag(is, "Grid"))
    return gd;
...
bool OBT41Format::ReadSCFOrbitalGrid(istream &is, OBGridData &t41Data) const {
  ...
  string buf;
  while (is >> buf)
    if (buf.find("SCF", 0) == 0 && buf.size() > 3)
      break;
  if (!is)
    return false;
  // SCF_
...
bool OBT41Format::ReadSumFragGrid(istream &is, OBGridData &t41Data) const {
  ...
  while (is >> buf)
    if (buf == "SumFrag")
      break; // look for SumFrag string
  if (!is)
    return false; // not found -> return
  const string sumfrag =
      buf; // found read next line then skip one line and read data
  is >> buf;
