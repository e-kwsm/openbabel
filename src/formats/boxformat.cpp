...
public:
  ...
  {
    return "Dock 3.5 Box format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override {
    return "http://dock.compbio.ucsf.edu/";
  } // optional
...
bool BoxFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  snprintf(buffer, BUFF_SIZE,
           "REMARK    CENTER (X Y Z)      %10.3f %10.3f %10.3f", vmid.x(),
           vmid.y(), vmid.z());
  ofs << buffer << endl;
  snprintf(buffer, BUFF_SIZE,
           "REMARK    DIMENSIONS (X Y Z)  %10.3f %10.3f %10.3f", vdim.x(),
           vdim.y(), vdim.z());
  ofs << buffer << endl;
  vdim /= 2.0;
  ...
  for (j = 1; j <= 8; j++) {
    ...
    snprintf(buffer, BUFF_SIZE,
             "ATOM      %d  DUA BOX     1    %8.3f%8.3f%8.3f", j, vtmp.x(),
             vtmp.y(), vtmp.z());
    ofs << buffer << endl;
  }
