...
public:
  ...
  const char *SpecificationURL() override { return "https://gaussian.com/"; }

  const char *GetMIMEType() override { return "chemical/x-gaussian-log"; }
...
public:
  ...
  const char *Description() override // required
  {
    return "Gaussian Input\n"
           ...
           "  u               Write the crystallographic unit cell, if "
           "present.\n\n";
  }
  ...
  const char *GetMIMEType() override { return "chemical/x-gaussian-input"; }
...
bool GaussianInputFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  bool writeUnitCell =
      (nullptr != pConv->IsOption("u", OBConversion::OUTOPTIONS));
  ...
  snprintf(buffer, BUFF_SIZE, "%d  %d", mol.GetTotalCharge(),
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    if (atom->GetIsotope() == 0)
      ...
      snprintf(buffer, BUFF_SIZE, "%-3s(Iso=%d) %10.5f      %10.5f      %10.5f",
               OBElements::GetSymbol(atom->GetAtomicNum()), atom->GetIsotope(),
  ...
  if (uc && writeUnitCell) {
    uc->FillUnitCell(
        &mol); // complete the unit cell with symmetry-derived atoms

    vector<vector3> cellVectors = uc->GetCellVectors();
    for (vector<vector3>::iterator i = cellVectors.begin();
         i != cellVectors.end(); ++i) {
    ...
    for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
      ...
      for (bond = mol.BeginBond(j); bond; bond = mol.NextBond(j)) {
        if (bond->GetBeginAtomIdx() == atom->GetIdx()) {
          snprintf(buffer, BUFF_SIZE, "%d %1.1f ", bond->GetEndAtomIdx(),
                   (float)bond->GetBondOrder());
          ofs << buffer;
        }
      }
...
static void add_unique_pairdata_to_mol(OpenBabel::OBMol *mol, string attribute,
...
static int extract_thermo(OpenBabel::OBMol *mol, string method,
                          double temperature, double ezpe, double Hcorr,
                          double Gcorr, double E0, double CV, int RotSymNum,
                          std::vector<double> Scomponents) {
  // Initiate correction database
  OpenBabel::OBAtomicHeatOfFormationTable *ahof =
      new OpenBabel::OBAtomicHeatOfFormationTable();
  ...
  double Rgas =
      1.9872041; // cal/mol K http://en.wikipedia.org/wiki/Gas_constant
  ...
  for (OBa = mol->BeginAtom(OBai); nullptr != OBa; OBa = mol->NextAtom(OBai)) {
    ...
    found =
        ahof->GetHeatOfFormation(OBElements::GetSymbol(atomicnumber), 0, method,
                                 temperature, &dhfx0, &dhfxT, &S0xT);
    if (1 == found) {
      dhofM0 += dhfx0;
      dhofMT += dhfxT;
      DeltaSMT += S0xT;
...
bool GaussianOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  bool ezpe_set = false, Hcorr_set = false, Gcorr_set = false, E0_set = false,
       CV_set = false;
  ...
  int natoms = 0; // number of atoms -- ensure we don't go to a new job with a
                  // different molecule
  ...
  std::vector<unsigned short> confDimensions =
      confData->GetDimension(); // to be fair, set these all to 3D
  ...
  std::vector<double> Forces, Wavelengths, EDipole, RotatoryStrengthsVelocity,
      RotatoryStrengthsLength;
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, "Entering Gaussian") != nullptr) {
      ...
      while (vs.size() > 4) {
        int corr =
            vs.size() == 5 ? -1 : 0; // g94; later versions have an extra column
      ...
      if (coordinates.size() == static_cast<size_t>(natoms) * 3) {
        ...
        confDimensions.push_back(3); // always 3D -- OBConformerData allows
                                     // mixing 2D and 3D structures
      }
      coordinates.clear();
      ...
      if (ptr && 6 == sscanf(ptr, "%8lf%8lf%8lf%8lf%8lf%8lf", &xx, &xy, &yy,
                             &xz, &yz, &zz)) {
        double Q[3][3];
        ...
        if (mol.HasData("Exact polarizability")) {
          mol.DeleteData(
              "Exact polarizability"); // Delete the old one to add the new one
        }
      ...
      ifs.getline(buffer, BUFF_SIZE);
      tokenize(vs, buffer);
      while (vs.size() >= 3 && strstr(buffer, "Sum of ") == nullptr) {
      ...
      if (MPA_q.size() == mol.NumAtoms()) {
        ...
        cout << "Read " << MPA_q.size() << " Mulliken charges for "
             << mol.NumAtoms() << " atoms\n";
      }
      ...
      ifs.getline(buffer, BUFF_SIZE);
      tokenize(vs, buffer);
      while (vs.size() >= 8 && strstr(buffer, "Tot ") == nullptr) {
        atom = mol.GetAtom(atoi(vs[0].c_str()));
        if (!atom)
          break;
        atom->SetPartialCharge(atof(vs[2].c_str()));
      ...
      if (CM5_q.size() == mol.NumAtoms() && HPA_q.size() == mol.NumAtoms()) {
        ...
        cout << "Read " << HPA_q.size() << " Hirshfeld charges for "
             << mol.NumAtoms() << " atoms\n";
      }
    ...
    } else if (strstr(buffer, "Electrostatic Properties (Atomic Units)") !=
                   nullptr &&
               !ESPisAdded) {
      int i, np;
      ...
      i = 0;
      for (fgp = esp->BeginPoint(fgpi); nullptr != fgp;
           fgp = esp->NextPoint(fgpi)) {
        ifs.getline(buffer, BUFF_SIZE);
        tokenize(vs, buffer);
        if (vs.size() >= 2) {
          if (vs.size() > 2) {
      ...
      tokenize(vs, buffer);
      while (vs.size() >= 3 && strstr(buffer, "-----") == nullptr) {
        atom = mol.GetAtom(atoi(vs[0].c_str()));
        if (!atom)
          break;
        atom->SetPartialCharge(atof(vs[2].c_str()));
      ...
      if (ESP_q.size() == mol.NumAtoms()) {
        ...
        cout << "Read " << ESP_q.size() << " ESP charges for " << mol.NumAtoms()
             << " atoms\n";
      }
    ...
    {
      // The info should appear only once as several blocks starting with this
      // line
      tokenize(vs, buffer);
    ...
    } else if (strstr(buffer, "rbital symmetries")) // orbital symmetries
    {
      symmetries.clear();
      std::string
          label; // used as a temporary to remove "(" and ")" from labels
      int iii, offset = 0;
      bool bDoneSymm;
      ...
      for (iii = 0; (iii < 2); iii++) {
        ...
        while (!ifs.eof() && (nullptr != strstr(buffer, "Alpha") ||
        ...
        do {
          ...
          if (!bDoneSymm) {
            ...
            if (nullptr != strstr(buffer, "Occupied") ||
                nullptr != strstr(buffer, "Virtual")) {
            ...
            ifs.getline(
                buffer,
                BUFF_SIZE); // get a new line if we've been reading symmetries
          }
      ...
      while (strstr(buffer, ". eigenvalues --")) {
        ...
        if (vs.size() < 4)
          break;
        if (vs[0].find("Beta") != string::npos &&
            betaStart == 0) // mark where we switch from alpha to beta
    ...
    {
      // The above line appears for each state, so just append the info to the
      // vectors
      tokenize(vs, buffer);
      if (vs.size() >= 9) {
    ...
    } else if (strstr(buffer, " Ground to excited state Transition electric "
                              "dipole moments (Au):"))
    ...
    } else if (strstr(buffer, "       state          X           Y           Z "
                              "    R(velocity)")) {
      // Rotatory Strengths
      ifs.getline(buffer, BUFF_SIZE); // First entry
      tokenize(vs, buffer);
    ...
    /* Temporarily commented out until the handling of energy in OBMol is sorted
       out
    ...
    */
    ...
    } else if (strstr(buffer, "Thermal correction to Gibbs Free Energy=") !=
               nullptr) {
      tokenize(vs, buffer);
      if (vs.size() > 6) {
      ...
      int i, nsearch;
      const char *search[] = {"CBS-QB3 (0 K)", "G2(0 K)",    "G3(0 K)",
                              "G4(0 K)",       "W1BD (0 K)", "W1U  (0 K)"};
  ...
  if (ezpe_set && Hcorr_set && Gcorr_set && E0_set && CV_set &&
      (thermo_method.size() > 0)) {
    extract_thermo(&mol, thermo_method, temperature, ezpe, Hcorr, Gcorr, E0, CV,
                   RotSymNum, Scomponents);
  }
  ...
  if (orbitals.size() > 0) {
    ...
    if (aHOMO == bHOMO) {
      ...
      if (initialSize != symmSize || betaStart == -1) {
        cerr << "Inconsistency: orbitals have " << initialSize
             << " elements while symmetries have " << symmSize << endl;
      } else {
  ...
  if (numTranslationVectors > 0) {
    ...
    uc->SetData(translationVectors[0], translationVectors[1],
