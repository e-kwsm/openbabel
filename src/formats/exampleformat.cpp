...
public:
  ...
  XXXFormat() {
    ...
    /* If there are any format specific options they should be registered here
       ...
       be
       concatinated with other	single letter options. For output options it can
       be multicharcter and is then	written as --optionname on the command
       ...
       options. The parameter is always text and needs to be parsed to extract a
       number.

       Options can apply when writing - 4th parameter is
       OBConversion::OUTOPTIONS or can be omitted as shown. A single letter
       output option is preceded by -x on the command line. Or options can apply
       to the input format - the 4th parameter is OBConversion::INOPTIONS. They
       are then  preceded by -a on the command line.
...
because it is used in dropdown lists, etc. in some user interfaces. The rest is
optional.
...
option is registered in the constructor of the class. Finish the options with a
blank line as shown, if there are more than one group of options, or if there
are further comments after them.
  */
  ...
  // Optional
  const char *GetMIMEType() override { return "chemical/x-xxx"; }
...
bool XXXFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  SkipObjects() has been defined. If it has not, and continuation after errors
  is still required, it is necessary to leave the input stream at the beginning
  of next object when returning false;*/
  return true;
}
