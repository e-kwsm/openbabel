...
public:
  ...
  {
    return "Chemical Resource Kit diagram(2D)\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override {
    return "http://crk.sourceforge.net/";
  } // optional

  const char *GetMIMEType() override { return "chemical/x-crk2d"; }
...
bool CRK2DFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!strstr(buffer, "\"DiagramStructure\"")) {
    obErrorLog.ThrowError(__FUNCTION__, "Not CRK DiagramStructure (2D)",
                          obWarning);
    return false;
  }
...
public:
  ...
  const char *Description() override // required
  {
    return "Chemical Resource Kit 3D format\n"
           "No comments yet\n";
  }
  ...
  const char *GetMIMEType() override { return "chemical/x-crk3d"; }
...
bool CRK3DFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!strstr(buffer, "\"ModelStructure\"") &&
      !strstr(buffer, "\"XRayStructure\"")) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Not CRK ModelStructure or XRayStructure (3D).",
                          obWarning);
    return false;
  }
...
#define MAX_BONDS 1000
  ...
  for (int n = 0; n < numBonds; n++) {
    ...
    if (fromIdx > 0 && toIdx > 0) {
      ...
      errorMsg << "Unassigned bond ID (" << stbondFrom[n] << " " << stbondTo[n]
