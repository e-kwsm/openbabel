...
public:
  ...
  const char *Description() override // required
  {
    return "Gaussian cube format\n"
           ...
           "Open Babel supports reading and writing Gaussian cubes, including "
           "multiple\n"
           "grids in one file.\n\n"
  ...
  bool ReadMolecule(OpenBabel::OBBase *pOb,
                    OpenBabel::OBConversion *pConv) override;
  /// Write: always returns false right now - read only
  bool WriteMolecule(OpenBabel::OBBase *pOb,
                     OpenBabel::OBConversion *pConv) override;
};
...
bool OBGaussianCubeFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problem reading the Gaussian cube file: cannot read "
                          "the first line (title/comments).",
                          obWarning);
    return false;
  }
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problem reading the Gaussian cube file: cannot read "
                          "the third line. This should contain the number of "
                          "atoms and the position of the origin of the cube.",
                          obWarning);
    return false;
  }
  vector<string> vs;
  ...
  if (vs.size() < 4) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Problem reading the Gassian cube file: The third line must contain "
        "the number of atoms and the origin of the cube.",
        obWarning);
    return false;
  }
  char *endptr;
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (!ifs.getline(buffer, BUFF_SIZE)) {
      errorMsg
          << "Problem reading the Gaussian cube file: cannot read the " << line
    ...
    voxels[i] =
        strtol(static_cast<const char *>(vs.at(0).c_str()), &endptr, 10);
    if (endptr == static_cast<const char *>(vs.at(0).c_str())) {
      errorMsg << "Problems reading the Gaussian cube file: "
  ...
  if (negAtoms) {
    ...
    if (!ifs.getline(buffer, BUFF_SIZE)) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Problem reading the Gaussian cube file: cannot read the line "
          "between the atom data and cube data.",
          obError);
      return false;
    }
    ...
    if (endptr == static_cast<const char *>(vs.at(0).c_str())) {
      errorMsg << "Problems reading the Gaussian cube file: "
               << "Could not read line " << line << ".\n"
               << "According to the specification this line should contain "
               << "the number of cubes and a number for each cube (orbital "
                  "number).\n"
               << "OpenBabel could not interpret item #0 as an integer.";
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
      return false;
    ...
    for (unsigned int i = 1; i < vs.size(); ++i) {
      int cubeNumber =
          strtol(static_cast<const char *>(vs.at(i).c_str()), &endptr, 10);
    ...
    while (vgd.size() < nCubes) {
      ...
      if (!ifs.getline(buffer, BUFF_SIZE)) {
        errorMsg << "Problem reading the Gaussian cube file: cannot"
                 ...
                 << " and expected number of values = " << nCubes << endl;
  ...
  while (values.size() < n * nCubes) {
    ...
    if (!ifs.getline(buffer, BUFF_SIZE)) {
      errorMsg << "Problem reading the Gaussian cube file: cannot"
               ...
               << " and expected number of values = " << n * nCubes << endl;
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obError);
      return false;
    }
    tokenize(vs, buffer);
    ...
    for (unsigned int l = 0; l < vs.size(); ++l) {
      values.push_back(
          strtod(static_cast<const char *>(vs.at(l).c_str()), &endptr));
    }
  }
  ...
  for (int i = 0; i < nCubes; ++i) // foreach cube
  {
    ...
    vgd[i]->SetOrigin(fileformatInput); // i.e., is this data from a file or
...
bool OBGaussianCubeFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  snprintf(buffer, BUFF_SIZE, "%5d%12.6f%12.6f%12.6f",
           -static_cast<signed int>(mol.NumAtoms()),
           origin[0] * ANGSTROM_TO_BOHR, origin[1] * ANGSTROM_TO_BOHR,
           origin[2] * ANGSTROM_TO_BOHR);
  ofs << buffer << endl;

  // line 4: number of points x direction, axis x direction x y z
  snprintf(buffer, BUFF_SIZE, "%5d%12.6f%12.6f%12.6f", nx,
           xAxis[0] * ANGSTROM_TO_BOHR, xAxis[1] * ANGSTROM_TO_BOHR,
           xAxis[2] * ANGSTROM_TO_BOHR);
  ofs << buffer << endl;

  // line 5: number of points y direction, axis y direction x y z
  snprintf(buffer, BUFF_SIZE, "%5d%12.6f%12.6f%12.6f", ny,
           yAxis[0] * ANGSTROM_TO_BOHR, yAxis[1] * ANGSTROM_TO_BOHR,
           yAxis[2] * ANGSTROM_TO_BOHR);
  ofs << buffer << endl;

  // line 6: number of points z direction, axis z direction x y z
  snprintf(buffer, BUFF_SIZE, "%5d%12.6f%12.6f%12.6f", nz,
           zAxis[0] * ANGSTROM_TO_BOHR, zAxis[1] * ANGSTROM_TO_BOHR,
           zAxis[2] * ANGSTROM_TO_BOHR);
  ofs << buffer << endl;

  // Atom lines: atomic number, ?, X, Y, Z
  FOR_ATOMS_OF_MOL(atom, mol) {
    double *coordPtr = atom->GetCoordinate();
    snprintf(buffer, BUFF_SIZE, "%5d%12.6f%12.6f%12.6f%12.6f",
             atom->GetAtomicNum(), static_cast<double>(atom->GetAtomicNum()),
             coordPtr[0] * ANGSTROM_TO_BOHR, coordPtr[1] * ANGSTROM_TO_BOHR,
             coordPtr[2] * ANGSTROM_TO_BOHR);
    ofs << buffer << endl;
  }
