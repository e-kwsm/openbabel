...
public:
  ...
  EXYZFormat() { OBConversion::RegisterFormat("exyz", this, "chemical/x-xyz"); }

  const char *Description() override // required
  {
    return "Extended XYZ cartesian coordinates format\n"
           "A format used by ORCA-AICCM\n"
           "The \"EXYZ\" chemical file format is an extended version of the "
           "standard\n"
           "\"XYZ\" chemical file format with additional keywords and "
           "informations about\n"
           "the unit cell and virtual atoms.\n\n"

           "* Line one of the file contains the number of atoms in the file.\n"
           "* Line two of the file contains a title, comment, filename and/or "
           "the\n"
           "  following keywords: ``%PBC`` or ``%VIRTUAL`` \n\n"

           "Any remaining lines are parsed for atom information until a blank "
           "line. These\n"
           "lines start with the element symbol, followed by X, Y, and Z "
           "coordinates in\n"
           "angstroms separated by whitespace and - if ``%VIRTUAL`` is "
           "specified - the \n"
           "optional word ``VIRTUAL`` to mark virtual atoms. If ``%PBC`` is "
           "specified\n"
           "a second block will be present containing the 3 vectors for the "
           "unit cell\n"
           ...
           "On **output**, the first line written is the number of atoms in "
           "the molecule.\n"
           "Line two is the title of the molecule or the filename if no title "
           "is defined.\n"
           "Remaining lines define the atoms in the file. The first column is "
           "the atomic\n"
           "symbol (right-aligned on the third character), followed by the XYZ "
           "coordinates\n"
           "in \"15.5\" format separated by an addition whitespace, in "
           "angstroms. This means\n"
           "that all coordinates are printed with five decimal places.\n\n"
           "The next block starts with a blank line to separate the "
           "coordinates from the\n"
           "unit cell vectors followed by the vectors of the unit cell marked "
           "with the\n"
           "keywords Vector1/2/3. The vectors themselves are written in the "
           "same format\n"
           "as the atom coordinates. The last line contains the keyword Offset "
           "and the\n"
  ...
  const char *SpecificationURL() override {
    return "http://openbabel.org/wiki/EXYZ";
  } // optional

  const char *GetMIMEType() override { return "chemical/x-exyz"; }
...
bool EXYZFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "Problems reading an E-XYZ file: Cannot read the first line.",
        obWarning);
    return (false);
  }
  ...
  if (sscanf(buffer, "%d", &natoms) == 0 || !natoms) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading an E-XYZ file: The first line must "
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading an EXYZ file: Could not read the "
                          "second line (title/comments/keywords).",
                          obWarning);
    return (false);
  }
  ...
  location = readKeywords.find(
      "%PBC"); // file contains unitcell information behind the coords block
  ...
  location = readKeywords.find(
      "%VIRTUAL"); // file contains information about virtual atoms in the
                   // column next to the x,y,z values
  ...
  if (unitCell) {
    ...
    for (unsigned int i = 1; i <= 3; i++) {
      ...
      if (vs.size() != 4) { // assume that unit cell vectors are given
        errorMsg
            << "Problems reading an EXYZ file: "
            << "OpenBabel found the line '" << buffer << "'" << endl
            << "According to the specifications (keyword %PBC), this line "
               "should contain exactly 4 entries, separated by white space."
            << endl
            << "However, OpenBabel found " << vs.size() << " items.";
        obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
        return (false);
...
bool EXYZFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (string(mol.GetTitle()).find("%PBC") != notFound)
    keyword = ""; // title contains already right keyword
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ofs << setw(4) << right
        << OpenBabel::OBElements::GetSymbol(atom->GetAtomicNum()) << setw(15)
        << setprecision(5) << fixed << showpoint << right << atom->GetX() << " "
        << setw(15) << atom->GetY() << " " << setw(15) << atom->GetZ() << endl;
  }
  ofs << endl;
  //
  ...
  if (!mol.HasData(OBGenericDataType::UnitCell)) {
    ofs << right << "Vector1" << setw(15) << setprecision(5) << fixed
        << showpoint << right << 1. << " " << setw(15) << 0. << " " << setw(15)
        << 0. << endl;
    ofs << "Vector2" << setw(15) << right << 0. << " " << setw(15) << 1. << " "
        << setw(15) << 0. << endl;
    ofs << "Vector3" << setw(15) << right << 0. << " " << setw(15) << 0. << " "
        << setw(15) << 1. << endl;
    ofs << "Offset " << setw(15) << right << 0. << " " << setw(15) << 0. << " "
        << setw(15) << 0. << endl;
  } else {
    ...
    ofs << right << "Vector1" << setw(15) << setprecision(5) << fixed
        << showpoint << right << unitCellMatrix.GetRow(0).GetX() << " "
        << setw(15) << unitCellMatrix.GetRow(0).GetY() << " " << setw(15)
        << unitCellMatrix.GetRow(0).GetZ() << endl;
    ofs << "Vector2" << setw(15) << right << unitCellMatrix.GetRow(1).GetX()
        << " " << setw(15) << unitCellMatrix.GetRow(1).GetY() << " " << setw(15)
        << unitCellMatrix.GetRow(1).GetZ() << endl;
    ofs << "Vector3" << setw(15) << right << unitCellMatrix.GetRow(2).GetX()
        << " " << setw(15) << unitCellMatrix.GetRow(2).GetY() << " " << setw(15)
        << unitCellMatrix.GetRow(2).GetZ() << endl;
    ofs << "Offset " << setw(15) << right << offset.GetX() << " " << setw(15)
        << offset.GetY() << " " << setw(15) << offset.GetZ() << endl;
  }
  return (true);
}
