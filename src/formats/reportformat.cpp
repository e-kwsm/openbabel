...
public:
  ...
  {
    return "Open Babel report format\n"
           "A detailed report on the geometry of a molecule\n"
           "The report format presents a report of various molecular "
           "information,\n"
           "including:\n\n"
           ...
           "* Molecular formula\n"
           "* Mass\n"
           "* Exact mass (i.e., for high-resolution mass spectrometry, the "
           "mass of the most abundant elements)\n"
           ...
           "    C   6    1.3958     2.4176     2.7916     2.4176     1.3958    "
           " 0.0000 \n"
           "    H   7    1.0846     2.1537     3.4003     3.8761     3.4003    "
           " 2.1537 \n"
           "    H   8    2.1537     1.0846     2.1537     3.4003     3.8761    "
           " 3.4003 \n"
           "    H   9    3.4003     2.1537     1.0846     2.1537     3.4003    "
           " 3.8761 \n"
           "    H  10    3.8761     3.4003     2.1537     1.0846     2.1537    "
           " 3.4003 \n"
           "    H  11    3.4003     3.8761     3.4003     2.1537     1.0846    "
           " 2.1537\n"
           "    H  12    2.1537     3.4003     3.8761     3.4003     2.1537    "
           " 1.0846 \n\n"
           ...
           "    H  12    2.4803     4.2961     4.9607     4.2961     2.4804    "
           " 0.0000\n\n"
...
void ReportFormat::WriteDistanceMatrix(ostream &ofs, OBMol &mol) {
  ...
  while (max <= mol.NumAtoms() + columns) {
    ...
    snprintf(buffer, BUFF_SIZE, "%15s%4d",
             OBElements::GetSymbol(atom->GetAtomicNum()), min);
    ofs << buffer;
  ...
  for (bond = mol.BeginBond(bi1); bond; bond = mol.NextBond(bi1)) {
    ...
    for (a = b->BeginNbrAtom(bi2); a; a = b->NextNbrAtom(bi2)) {
      ...
      for (d = c->BeginNbrAtom(bi3); d; d = c->NextNbrAtom(bi3)) {
        ...
        snprintf(buffer, BUFF_SIZE, "%4d %4d %4d %4d %10.3f", a->GetIdx(),
...
void ReportFormat::WriteAngles(ostream &ofs, OBMol &mol) {
  ...
  FOR_ANGLES_OF_MOL(angle, mol) {
    ...
    snprintf(buffer, BUFF_SIZE, "%4d %4d %4d %4s %4s %4s %10.3f", a->GetIdx(),
             b->GetIdx(), c->GetIdx(), a->GetType(), b->GetType(), c->GetType(),
             ang);
    ofs << buffer << "\n";
  }
}
