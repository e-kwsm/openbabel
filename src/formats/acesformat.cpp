...
public:
  ...
  AcesOutputFormat() { OBConversion::RegisterFormat("acesout", this); }

  const char *Description() override // required
  {
    return "ACES output format\n"
           "ACES is a set of programs that performs ab initio quantum "
...
public:
  // Register this format type ID
  AcesInputFormat() { OBConversion::RegisterFormat("acesin", this); }

  const char *Description() override // required
  {
    return "ACES input format\n"
           "ACES is a set of programs that performs ab initio quantum "
           "chemistry calculations.\n";
  }
...
bool AcesOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "Normal Coordinates") != nullptr) {
      ...
      while (strstr(buffer, "Normal modes in internal coordinates") ==
                 nullptr &&
