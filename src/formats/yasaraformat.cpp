...
int int32lemem(char *data) {
  ...
  value = (int)((unsigned char *)data)[0] +
          ((int)((unsigned char *)data)[1] << 8) +
          ((int)((unsigned char *)data)[2] << 16) +
          ((int)((unsigned char *)data)[3] << 24);
  intmino2 = -1073741824;
  if ((unsigned int)INT_MIN != 0x80000000 && (value & 0x80000000))
    value = (intmino2 * 2) + (value & 0x7fffffff);
  return (value);
}
...
#define MOB_ATOMSIZEMAX                                                        \
  44 // Header+3*POS+Flags+Atom+ResChain+Numb+AltIns+Occupany+Bfac
...
void mob_getid(struct atomid *id, struct mobatom *atom) {
  ...
  if (flags & MOB_OCCUPANCYFLAG)
    id->occupancy = ((float *)atom->link)[idpos++];
  else
    id->occupancy = 1.0f;
  if (flags & MOB_TEMPFACFLAG)
    id->bfactor = ((float *)atom->link)[idpos++];
  else
    id->bfactor = 0;
  if (flags & MOB_SEGMENTFLAG)
    id->segment = atom->link[idpos++];
  else
    id->segment = 0;
  if (flags & MOB_PROPERTYFLAG)
    id->property = ((float *)atom->link)[idpos++];
  else
    id->property = 0;
...
int mob_hasname(struct mobatom *atom, struct atomid *id2) {
  ...
  mob_getid(&id1, atom);
  if (id1.atom == id2->atom && id1.altloc == id2->altloc)
    return (1);
  else
    return (0);
}
...
public:
  ...
  YOBFormat() { OBConversion::RegisterFormat("yob", this); }

  const char *Description() override // required
  {
    return "YASARA.org YOB format\n"
           "The native YASARA format.\n";
  }

  const char *SpecificationURL() override {
    return "http://www.yasara.org";
  } // optional
...
/* READ YASARA OBJECT
   ==================
   IF options CONTAINS 'f' (COMMAND LINE -af), ATOM NAMES WILL STAY FIXED (I.E.
   LEAVING THE SPACE IN THE FIRST COLUMN) */
bool YOBFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  unsigned int /*resno,chainNum,*/ link, linked, linktype, atoms, element,
      links, chain;
  ...
  for (i = 0; i < atoms; i++) { /* GET ELEMENT, TYPE AND POSITION */
    ...
    *((int32 *)atomname) = id.atom;
    ...
    if (srcatom->header[MOB_ELEMENT] & MOB_HETATOMFLAG)
      hetatom = true;
    else
      hetatom = false;
    ...
    for (j = 0; j < links; j++) {
      ...
      if (linked < i) {
        ...
        if (linktype == MOB_LINKQUADRUPLE)
          linktype = 4;
        else if (linktype > MOB_LINKTRIPLE)
          linktype = 5;
...
/* WRITE YASARA OBJECT
   ===================
   IF options CONTAINS 'f' (COMMAND LINE -xf), ATOM NAMES WILL STAY FIXED (I.E.
   ADDING NO SPACE IN THE FIRST COLUMN) IF options CONTAINS 'a' (COMMAND LINE
   -xa), AROMATIC ATOMS WILL BE FLAGGED */
bool YOBFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= atoms; i++) {
    ...
    /* COUNT BONDS */
    links = 0;
    for (bond = srcatom->BeginBond(iter); bond; bond = srcatom->NextBond(iter))
      links++;
    size += links * 4 + 32;
  }
  /* WRITE DATA HEADER */
  ...
  for (i = 1; i <= atoms; i++) {
    ...
    for (linkedatom = srcatom->BeginNbrAtom(iter); linkedatom;
         linkedatom = srcatom->NextNbrAtom(iter)) {
      storeint32le(buffer, linkedatom->GetIdx() - 1);
      bondorder = (*iter)->GetBondOrder();
      // printf("  Order %d\n",bondorder);
      if (bondorder == 4)
        bondorder = MOB_LINKQUADRUPLE;
      else if (bondorder == 5)
        bondorder = MOB_LINKRESONANCE50;
      buffer[3] = bondorder;
      ofs.write(buffer, 4);
    }
    /* WRITE ATOM ID */
    ...
    if (pConv->IsOption("a", OBConversion::OUTOPTIONS) && srcatom->IsAromatic())
      flags |= MOB_AROMATICFLAG;
    ...
    if (srcatom->HasResidue()) {
      ...
      pos = 4;
      if (!pConv->IsOption(
              "f", OBConversion::OUTOPTIONS)) { /* GUESS IF THE ATOM NAME NEEDS
                                                   TO BE SHIFTED */
        if (strlen(mob_elementsym[element]) == 1 ||
            strncasecmp(mob_elementsym[element], atomname, 2))
          pos = 5;
      }
