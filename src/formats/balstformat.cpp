...
public:
  ...
  BallStickFormat() { OBConversion::RegisterFormat("bs", this); }

  const char *Description() override // required
  {
    return "Ball and Stick format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override {
    return "http://www.orc.uni-linz.ac.at/mueller/ball_and_stick.shtml"; // XXX
                                                                         // dead
  }
...
bool BallStickFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    strncpy(tmptype, OBElements::GetSymbol(atom->GetAtomicNum()),
            sizeof(tmptype));
    tmptype[15] = '\0';
    if (strlen(tmptype) > 1)
      tmptype[1] = toupper(tmptype[1]);
    snprintf(buffer, BUFF_SIZE, "%-3s %8.4f  %8.4f  %8.4f", tmptype,
