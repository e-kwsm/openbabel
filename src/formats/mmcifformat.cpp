...
public:
  ...
  mmCIFFormat() { // Copied from the Chemical MIME Page at
  ...
  {
    return "Macromolecular Crystallographic Info\n "
           "Read Options e.g. -as\n"
  ...
  const char *SpecificationURL() override {
    return "http://mmcif.pdb.org/";
  } // optional
  // CIF itself is at http://www.iucr.org/iucr-top/cif/index.html

  const char *GetMIMEType() override { return "chemical/x-mmcif"; }
...
struct CIFTagID {
  ...
  enum CIFDataName {
    unread_CIFDataName,
    _atom_site_fract_x,         // The x coordinate specified as a fraction of
                                // _cell_length_a
    _atom_site_fract_y,         // The y coordinate specified as a fraction of
                                // _cell_length_b
    _atom_site_fract_z,         // The z coordinate specified as a fraction of
                                // _cell_length_c
    ...
    _atom_site_label,           // The atomic label if more detailed label info
                                // unavailable
    _atom_site_label_atom_id,   // The atomic label within the residue
    _atom_site_label_comp_id,   // The residue abbreviation, e.g. ILE
    _atom_site_label_entity_id, // The chain entity number of the residue, e.g.
                                // 2
    _atom_site_label_asym_id,   // The unique chain id
    _atom_site_label_seq_id, // The sequence number of the residue, within the
                             // chain, e.g. 12
...
struct CIFResidueID {
  ...
  CIFResidueID() {}
  CIFResidueID(unsigned long c, unsigned long r) : ChainNum(c), ResNum(r) {}
  ...
  bool operator<(const CIFResidueID &other) const {
    return (ChainNum < other.ChainNum
                ? true
                : (other.ChainNum < ChainNum ? false : ResNum < other.ResNum));
  }
};
...
CIFTagID CIFTagsRead[] = {
    ...
    {"_symmetry_space_group_name_hall",
     CIFTagID::_symmetry_space_group_name_Hall},
    {"_symmetry_space_group_name_h-m",
     CIFTagID::_symmetry_space_group_name_H_M},
...
public:
  ...
  CIFLexer(std::istream *in) : input(in) {
    last_char = 0;
    next_char = input->get();
  }
...
CIFTagID::CIFDataName CIFLexer::lookup_tag(const string &tag_name) {
  if (CIFtagLookupTable.empty()) {
    for (size_t idx = 0; CIFTagsRead[idx].tagid != CIFTagID::unread_CIFDataName;
         ++idx) {
      CIFtagLookupTable.insert(CIFtagmap::value_type(
          string(CIFTagsRead[idx].tagname), CIFTagsRead[idx].tagid));
    }
  }
...
bool mmCIFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (lexer.next_token(token) && token.type != CIFLexer::KeyDataToken)
    ;
  if (token.type ==
      CIFLexer::KeyDataToken) { // we have found the next data block:
    pmol->BeginModify();
    ...
    while (!finished && (token_peeked || lexer.next_token(token))) {
      ...
      case CIFLexer::KeyDataToken:
        ...
        if (pmol->NumAtoms() > 0) { // Found a molecule, so finished
          ...
          lexer.backup(5 + token.as_text.size(),
                       'd'); // length of "data_<name>"
        } else               // not yet found a molecule, so try again
          pmol->SetTitle(token.as_text);
        break;
      case CIFLexer::KeySaveToken: { // Simply eat tokens until the save_ ending
                                     // token
        while (lexer.next_token(token) &&
               token.type != CIFLexer::KeySaveEndToken)
          ;
      ...
      case CIFLexer::KeyLoopToken: {
        ...
        while ((token_peeked = lexer.next_token(token)) == true &&
          ...
          if (catid == CIFTagID::unread_CIFCatName &&
              tagid != CIFTagID::unread_CIFDataName)
        ...
        case CIFTagID::atom_site: {
          ...
          for (CIFColumnList::const_iterator colx = columns.begin(),
                                             coly = columns.end();
               colx != coly; ++colx) {
            switch (*colx) {
          ...
          if (use_cartn) {
            for (CIFColumnList::iterator colx = columns.begin(),
                                         coly = columns.end();
                 colx != coly; ++colx)
          ...
          while (token.type == CIFLexer::ValueToken) // Read in the Fields
          {
            ...
            case CIFTagID::_atom_site_label: // The atomic label within the
                                             // molecule
            ...
            case CIFTagID::_atom_site_type_symbol:
              ...
              if (nbc > 0) {
                ...
                obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
                tmpSymbol =
                    "Xx"; // Something went wrong, no symbol ! Default to Xx
              }
              ...
              atom->SetAtomicNum(atomicNum); // set atomic number, or '0' if the
                                             // atom type is not recognized
            ...
            case CIFTagID::_atom_site_Cartn_z:
              z = token.as_number();
              break;
            case CIFTagID::_atom_site_label_atom_id: // The atomic label within
                                                     // the residue
              atom_label.assign(token.as_text);
              if (atom_type_tag == CIFTagID::_atom_site_label_atom_id) {
                for (string::iterator posx = token.as_text.begin(),
                                      posy = token.as_text.end();
                     posx != posy; ++posx) {
                ...
                atom->SetAtomicNum(
                    OBElements::GetAtomicNum(token.as_text.c_str()));
                atom->SetType(token.as_text);
              }
              break;
            case CIFTagID::_atom_site_label_comp_id: // The residue
                                                     // abbreviation, e.g. ILE
              residue_name.assign(token.as_text);
              break;
            case CIFTagID::_atom_site_label_entity_id: // The chain entity
                                                       // number of the residue,
                                                       // e.g. 2
              // ignored and replaced by unique id for label_asym_id
              break;
            case CIFTagID::_atom_site_label_asym_id: // The strand number of the
                                                     // residue
              if (token.as_text != last_asym_id) {
                CIFasymmap::const_iterator asym_it =
                    asym_map.find(token.as_text);
                if (asym_it == asym_map.end()) {
                  ++next_asym_no;
                  asym_it = asym_map
                                .insert(CIFasymmap::value_type(token.as_text,
                                                               next_asym_no))
                                .first;
                }
              ...
              break;
            case CIFTagID::_atom_site_label_seq_id: // The sequence number of
                                                    // the residue, within the
                                                    // chain, e.g. 12
            ...
            case CIFTagID::_atom_site_occupancy: // The occupancy of the site.
            {
              ...
              double occupancy = std::max(
                  0.0, std::min(1.0, token.as_number())); // clamp occupancy to
                                                          // [0.0, 1.0] bugfix
            ...
            if (column_idx == column_count) {
              ...
              if (use_residue == 2) {
                ...
                unsigned long serial_no =
                    strtoul(atom_mol_label.c_str(), nullptr, 10);
                if (serial_no > 0)
        ...
        default:
          while (token.type ==
                 CIFLexer::ValueToken) // Eat the values, we don't want them
            token_peeked = lexer.next_token(token);
          break;
        }
      } break;
    ...
    if (pmol->NumAtoms() > 0) {
      if (use_cell >= 6) {
        OBUnitCell *pCell = new OBUnitCell; // No matching "delete" because it's
                                            // saved in pmol->SetData
        pCell->SetOrigin(fileformatInput);
        pCell->SetData(cell_a, cell_b, cell_c, cell_alpha, cell_beta,
                       cell_gamma);
        ...
        pmol->SetData(pCell);
        if (use_fract) {
          for (OBAtomIterator atom_x = pmol->BeginAtoms(),
                              atom_y = pmol->EndAtoms();
          ...
          } // Note: this is where we could keep the original fractional
            // coordinates, e.g. in a new OBCoord class
        }
...
bool mmCIFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  ofs << "# to comply with the Macromolecular CIF Dictionary  (cif_mm.dic) "
         "version  2.0.11 http://mmcif.pdb.org/"
      << endl;
  ofs << "# The contents of this file were derived from "
      << pConv->GetInFilename() << endl;
  ofs << "#" << endl;
  ofs << "#--------------------------------------------------------------------"
         "-------"
      << endl;
  ...
  if (!(pmol->GetSpacedFormula().empty())) {
    ...
    ofs << "_chemical_formula.structural\t'" << pmol->GetFormula() << "'"
        << endl;
    ofs << endl;
  }
  ofs << "###############" << endl;
  ...
  for (OBAtomIterator atom_x = pmol->BeginAtoms(), atom_y = pmol->EndAtoms();
    ...
    if (has_residues) {
      ...
      if (atomname.empty()) {
        snprintf(buffer, BUFF_SIZE, "%s%lu",
                 OBElements::GetSymbol(atom->GetAtomicNum()),
      ...
      if (resname.empty())
        resname.assign("UNK");
      ofs << '\t' << atomname << '\t' << resname << '\t' << pRes->GetChainNum()
          << '\t' << pRes->GetNum() << endl;
    }
  ...
  if (pmol->HasData(OBGenericDataType::UnitCell)) {
    ...
    if (pSG) {
      ...
      if (!(pSG->GetHMName().empty())) {
        ...
        if (n == string::npos)
          ofs << "_space_group_name_H-M_alt '" << pSG->GetHMName() << "'"
              << endl;
        else
      ...
      if (trans) {
        ...
        if (!loop_transforms) {
          ...
          ofs << "_symmetry_equiv.pos_as_xyz\t'" << trans->DescribeAsString()
              << "'" << endl;
        } else {
          ofs << "loop_" << endl;
          ...
          while (trans) {
            ofs << '\t' << symid << "\t'" << trans->DescribeAsString() << "'"
                << endl;
