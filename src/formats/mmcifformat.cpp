...
namespace OpenBabel
{
 ...
 bool mmCIFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
 {
   ...
   ofs << "# --------------------------------------------------------------------------" << '\n';
   ofs << "#" << '\n';
   ofs << "# CIF file generated by openbabel " << BABEL_VERSION << " http://openbabel.org/" << '\n';
   ...
   if (* (pmol->GetTitle()))
     ...
     ofs << "##############" << '\n';
     ofs << '\n';
     ofs << "_chemical.entry_id\t" << id << '\n';
     ofs << "_chemical.name_common\t'" << pmol->GetTitle() << "'" << '\n';
     ofs << '\n';
     }
   if (! (pmol->GetSpacedFormula().empty()))
     {
     ofs << "######################" << '\n';
     ofs << "## CHEMICAL FORMULA ##" << '\n';
     ofs << "######################" << '\n';
     ofs << '\n';
     ofs << "_chemical_formula.entry_id\t" << id << '\n';
   ...
   if (has_residues)
     {
     ofs << "_atom_site.label_atom_id" << '\n';
     ofs << "_atom_site.label_comp_id" << '\n';
     ofs << "_atom_site.label_entity_id" << '\n';
     ofs << "_atom_site.label_seq_id" << '\n';
     }
   ...
   if (pmol->HasData(OBGenericDataType::UnitCell))
     ...
     ofs << "##########" << '\n';
     ofs << '\n';
     ofs << "_cell.entry_id\t" << id << '\n';
     ofs << "_cell.length_a\t" << pCell->GetA() << '\n';
     ofs << "_cell.length_b\t" << pCell->GetB() << '\n';
     ofs << "_cell.length_c\t" << pCell->GetC() << '\n';
     ofs << "_cell.angle_alpha\t" << pCell->GetAlpha() << '\n';
     ofs << "_cell.angle_beta\t"  << pCell->GetBeta() << '\n';
     ofs << "_cell.angle_gamma\t" << pCell->GetGamma() << '\n';
     ofs << '\n';
     ...
     if (pSG)
       ...
       ofs << "#################" << '\n';
       ofs << '\n';
       ofs << "_space_group.id\t" << id << '\n';
       bool loop_transforms = false;
       if (pSG->GetId())
         {
         ofs << "_space_group.IT_number\t" << pSG->GetId() << '\n';
         }
       if (! (pSG->GetHallName().empty()))
         {
         loop_transforms = true;
         ofs << "_space_group.name_Hall\t'" << pSG->GetHallName() << "'" << '\n';
         }
       ...
       if (trans)
         ...
         if (!loop_transforms)
           {
           ofs << "_symmetry_equiv.id\t" << symid << '\n';
           ofs << "_symmetry_equiv.pos_as_xyz\t'" << trans->DescribeAsString() << "'" << '\n';
           }
         else
           {
           ofs << "loop_" << '\n';
           ofs << "_symmetry_equiv.id" << '\n';
           ofs << "_symmetry_equiv.pos_as_xyz" << '\n';
           while (trans)
             {
