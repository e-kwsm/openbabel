...
public:
  ...
  const char *Description() override {
    return "MDFF format\n"
           ...
           "POSFF and CONTFF are read to obtain information from MDFF "
           "calculations.\n"
           ...
           "  u <elementlist> Sort atoms by list of element symbols provided "
           "in comma-separated string w/o spaces\n"
           "  i Write IONS.POT file\n";
  }

  const char *SpecificationURL() override {
    return "https://code.google.com/p/mdff/";
  }
...
bool MDFFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  // files. MDFF does not support this, and this parser makes its own streams.
  ...
  if (vs.size() != atom_t_prop.size()) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Number of types is wrong. Format is not read.", obError);
    pmol->EndModify();
    return false;
  }
  for (size_t i = 0; i < atom_t_prop.size(); ++i) {
    atom_t_prop[i].atom_symbol = vs[i];
    atom_t_prop[i].atom_etab_num =
        OpenBabel::OBElements::GetAtomicNum(atom_t_prop[i].atom_symbol.c_str());
  }
  ...
  if (vs.size() != atom_t_prop.size()) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Number of types is wrong. Format is not readed.",
                          obError);
    pmol->EndModify();
    return false;
  }
    ...
    while (!ifs_ions.eof()) {
      ...
      if (vs[0] == "qch") {
        if (vs.size() - 1 != atom_t_prop.size())
          obErrorLog.ThrowError(__FUNCTION__,
                                "Number of charges for atom is wrong. Skipped",
                                obWarning);
        else
          for (int i = 0; i < atom_t_prop.size(); i++) {
            string tk = vs[i + 1];
            string str = tk.substr(tk.length() - 2) == "d0"
                             ? tk.substr(0, tk.length() - 2)
                             : tk;
  ...
  cartesian = (buffer[0] == 'C' || buffer[0] == 'c' || buffer[0] == 'K' ||
               buffer[0] == 'k');
  ...
  if (atomCount != totalAtoms_fl) {
    ...
    errorMsg << "Problems reading a MDFF file: "
             ...
             << "atomCount = " << atomCount
             << " totalAtoms_fl = " << totalAtoms_fl << endl;
...
bool MDFFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (map<aindx, OBAtom *>::const_iterator it = amap.begin(); it != amap.end();
       ++it) {
    string curr_atom_smb =
        OpenBabel::OBElements::GetSymbol(it->second->GetAtomicNum());
  ...
  for (int i = 0; i < 4; i++) {
    ...
    for (int j = 0; j < atypes_def.size(); j++) {
      if ((i == 0) || (i == 2))
        ...
        ofs_ions << OBElements::GetMass(
                        OBElements::GetAtomicNum(atypes_def[j].first.c_str()))
                 << "d0 ";
      else if (i == 3)
