...
public:
  ...
  const char *Description() override {
    return "Thermo format\n"
           "Reads and writes old-style NASA polynomials in original fixed "
           "format\n"
           "Read Options e.g. -ae\n"
           " e Terminate on \"END\"\n\n";
  }
...
bool ThermoFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  do {
    if (!ifs.getline(ln, BUFF_SIZE) ||
        (stopOnEnd && !strncasecmp(ln, "END", 3)))
      return false;
  } while (ln[79] != '1');
...
bool ThermoFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  OBNasaThermoData *pND =
      static_cast<OBNasaThermoData *>(pmol->GetData(ThermoData));
  if (!pND) {
    obErrorLog.ThrowError(__FUNCTION__, "No thermo data in " + title,
                          obWarning);
    return false;
  }
  ...
  ofs << right << pND->GetPhase() << fixed << setprecision(3) << setw(10)
      << pND->GetLoT();
