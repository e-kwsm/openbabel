...
public:
  ...
  SmiReactFormat() { OBConversion::RegisterFormat("rsmi", this); }

  const char *Description() override {
    return "Reaction SMILES format\n"
...
bool SmiReactFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  // Doesn't make a new OBReaction object, but does make mew reactant and
  // product OBMols
  ...
  if (!sconv.SetInFormat("smi")) {
    obErrorLog.ThrowError(__FUNCTION__, "Smiles format needed but not found",
                          obError);
    return false;
  }
  ...
  if (pos != string::npos) {
    obErrorLog.ThrowError(__FUNCTION__,
                          rsmiles + " contained a character '" + rsmiles[pos] +
                              "' which is invalid in SMILES",
                          obError);
    return false;
  }
...
bool SmiReactFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!pSmiFormat)
    return false;
  pConv->AddOption("smilesonly",
                   OBConversion::OUTOPTIONS); // suppresses title and new line
  pConv->AddOption(
      "c", OBConversion::OUTOPTIONS); // output atom classes if available
