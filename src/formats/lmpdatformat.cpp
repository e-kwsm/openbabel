...
public:
  ...
  LMPDATFormat() {
    ...
    OBConversion::RegisterOptionParam("q", nullptr, 1,
                                      OBConversion::OUTOPTIONS);
    OBConversion::RegisterOptionParam("d", nullptr, 1,
                                      OBConversion::OUTOPTIONS);
  }

  const char *Description() override // required
  {
    return "The LAMMPS data format\n\n"
           ...
           "  d \"length\" Set the length of the boundary box around the "
           "molecule.\n"
  ...
  const char *GetMIMEType() override { return "chemical/x-lmpdat"; }
...
bool LMPDATFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_BONDS_OF_MOL(bond, mol) {
    ...
    if (b->GetAtomicNum() > a->GetAtomicNum()) {
      snprintf(BondString, BUFF_SIZE, "%2s:%2s",
  ...
  FOR_ANGLES_OF_MOL(angle, mol) {
    ...
    if (b->GetAtomicNum() > c->GetAtomicNum()) {
      snprintf(AngleString, BUFF_SIZE, "%2s:%2s:%2s",
               OBElements::GetSymbol(b->GetAtomicNum()),
               OBElements::GetSymbol(a->GetAtomicNum()),
  ...
  FOR_TORSIONS_OF_MOL(dihedral, mol) {
    ...
    if (a->GetAtomicNum() > d->GetAtomicNum()) {
      snprintf(DihedralString, BUFF_SIZE, "%2s:%2s:%2s:%2s",
               OBElements::GetSymbol(a->GetAtomicNum()),
               OBElements::GetSymbol(b->GetAtomicNum()),
               OBElements::GetSymbol(c->GetAtomicNum()),
  ...
  for (molitr = mols.begin(); molitr != mols.end(); ++molitr) {
    ...
    FOR_ATOMS_OF_MOL(atom, *molitr) {
      ...
      snprintf(buffer, BUFF_SIZE,
               "%-4d %4d %4d %10.5f %10.5f %10.5f %10.5f # %3s\n", atomcount,
               molcount, AtomType[ThisAtom], AtomCharge[ThisAtom], atom->GetX(),
               atom->GetY(), atom->GetZ(), ThisAtom.c_str());
      ofs << buffer;
    }
  }
  ...
  ofs << "Bonds" << endl;
  ofs << endl;
  snprintf(buffer, BUFF_SIZE, "#%3s %4s %4s %4s\n", "idx", "type", "atm1",
           "atom2");
  // ofs << buffer;
  FOR_BONDS_OF_MOL(bond, mol) {
    BondIndex++;
    ...
    if (b->GetAtomicNum() > a->GetAtomicNum()) {
      snprintf(BondString, BUFF_SIZE, "%2s:%2s",
               OBElements::GetSymbol(b->GetAtomicNum()),
               OBElements::GetSymbol(a->GetAtomicNum()));
      snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d # %5s\n", BondIndex,
               BondType[BondString], bond->GetEndAtomIdx(),
               bond->GetBeginAtomIdx(), BondString);
    } else {
      snprintf(BondString, BUFF_SIZE, "%2s:%2s",
               OBElements::GetSymbol(a->GetAtomicNum()),
               OBElements::GetSymbol(b->GetAtomicNum()));
      snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d # %5s\n", BondIndex,
               BondType[BondString], bond->GetBeginAtomIdx(),
               bond->GetEndAtomIdx(), BondString);
    }
    ofs << buffer;
  }
  ofs << endl;
  ...
  ofs << "Angles" << endl;
  ofs << endl;
  snprintf(buffer, BUFF_SIZE, "#%3s %4s %4s %4s %4s\n", "idx", "type", "atm1",
           "atm2", "atm3");
  // ofs << buffer;
  FOR_ANGLES_OF_MOL(angle, mol) {
    AngleIndex++;
    ...
    if (b->GetAtomicNum() > c->GetAtomicNum()) {
      snprintf(AngleString, BUFF_SIZE, "%2s:%2s:%2s",
               ...
               OBElements::GetSymbol(a->GetAtomicNum()),
               OBElements::GetSymbol(c->GetAtomicNum()));
      snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d %4d # %8s\n", AngleIndex,
               AngleType[AngleString], b->GetIdx(), a->GetIdx(), c->GetIdx(),
               AngleString);
    } else {
      snprintf(AngleString, BUFF_SIZE, "%2s:%2s:%2s",
               OBElements::GetSymbol(c->GetAtomicNum()),
               ...
               OBElements::GetSymbol(b->GetAtomicNum()));
      snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d %4d # %8s\n", AngleIndex,
               AngleType[AngleString], c->GetIdx(), a->GetIdx(), b->GetIdx(),
               AngleString);
    }
    ofs << buffer;
  }
  ...
  if (dihedralcount > 0) {
    ...
    ofs << "Dihedrals" << endl;
    ofs << endl;
    snprintf(buffer, BUFF_SIZE, "#%3s %4s %4s %4s %4s %4s\n", "idx", "type",
             "atm1", "atm2", "atm3", "atm4");
    // ofs << buffer;
    DihedralIndex = 0;
    FOR_TORSIONS_OF_MOL(dihedral, mol) {
      ...
      if (a->GetAtomicNum() > d->GetAtomicNum()) {
        snprintf(DihedralString, BUFF_SIZE, "%2s:%2s:%2s:%2s",
                 ...
                 OBElements::GetSymbol(c->GetAtomicNum()),
                 OBElements::GetSymbol(d->GetAtomicNum()));
        snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d %4d %4d # %11s\n",
                 DihedralIndex, DihedralType[DihedralString], a->GetIdx(),
                 b->GetIdx(), c->GetIdx(), d->GetIdx(), DihedralString);
      } else {
        snprintf(DihedralString, BUFF_SIZE, "%2s:%2s:%2s:%2s",
                 OBElements::GetSymbol(d->GetAtomicNum()),
                 OBElements::GetSymbol(b->GetAtomicNum()),
                 ...
                 OBElements::GetSymbol(a->GetAtomicNum()));
        snprintf(buffer, BUFF_SIZE, "%-4d %4d %4d %4d %4d %4d # %11s\n",
                 DihedralIndex, DihedralType[DihedralString], d->GetIdx(),
                 b->GetIdx(), c->GetIdx(), a->GetIdx(), DihedralString);
      }
      ofs << buffer;
    }
  }
