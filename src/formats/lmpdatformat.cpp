...
namespace OpenBabel
{
  ...
  bool LMPDATFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs << "LAMMPS data file generated by OpenBabel" << '\n';
    ofs << mol.NumAtoms() << " atoms"     << '\n';
    ofs << mol.NumBonds() << " bonds"     << '\n';
    ofs << anglecount     << " angles"    << '\n'; // no NumAngles()?
    ofs << dihedralcount  << " dihedrals" << '\n';
    ofs << 0              << " impropers" << '\n';
    ofs << AtomType.size()     << " atom types"     << '\n';
    ofs << BondType.size()     << " bond types"     << '\n';
    ofs << AngleType.size()    << " angle types"    << '\n';
    ofs << DihedralType.size() << " dihedral types" << '\n';
    ofs << 0                   << " improper types" << '\n';
    ofs << BoxString << '\n';
    ...
    for(itr=AtomMass.begin();itr!=AtomMass.end();++itr) 
    {
	    ...
	    ofs << AtomType[itr->first] << " " << mass << " # " << itr->first << '\n';
    }
    ofs << '\n';
