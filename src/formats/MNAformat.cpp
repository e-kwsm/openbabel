...
public:
  ...
  const char *Description() override {
    ...
    ss <<
        ...
        "parenthesized list of its neighbours. This process iterates until "
        "the\n"
        "specified distance from the origin, also known as the depth of the\n"
        "descriptor.\n\n"

        "Elements are simplified into 32 groups. Each group has a "
        "representative\n"
        ...
        "R    R, He, Ne, Ar, Kr, Xe, Rn, Ac, Th, Pa, U, Np, Pu, Am, Cm, Bk, "
        "Cf, Es, Fm, Md, No, Lr, Db, Jl\n"
        "==== ========\n\n"
        ...
        ".. [fpbg99] Dmitrii Filimonov, Vladimir Poroikov, Yulia Borodina, "
        "and\n"
        "            Tatyana Gloriozova. **Chemical Similarity Assessment "
        "through\n"
        "            Multilevel Neighborhoods of Atoms: Definition and "
        "Comparison with\n"
        "            the Other Descriptors.** *J. Chem. Inf. Comput. Sci.* "
        "**1999**, *39*, 666-670.\n"
...
bool MNAFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (p) {
    ...
    if (!(ss.rdstate() & (stringstream::failbit | stringstream::badbit))) {
      ...
      if (levels > 10000) {
        error_msg << "Levels = " << levels
                  << " will almost certainly crash and was probably a mistake."
                  << endl;
        obErrorLog.ThrowError(__FUNCTION__, error_msg.str(), obError);
        return false;
      } else if (levels > 10) {
        error_msg << "Levels > 10 is probably not very useful.  (If it is, let "
                     "me know!)"
                  << endl;
  ...
  if (pConv->IsOption("d", OBConversion::GENOPTIONS)) {
    if (pConv->GetOutputIndex() == 1)
      obErrorLog.ThrowError(
          __FUNCTION__,
          "MNA includes hydrogens by definition, just be aware of that.",
          obInfo);
...
string MNAFormat::MNAize(OBAtom *const atom, unsigned level) {
  ...
  s += (atom->IsInRing() ? "" : chain_marker) +
       OBElements::GetSymbol(atom->GetAtomicNum());

  if (level != 0) {
    // Possible problem: what if it's a single-atom molecule?  Unlikely, but
    // no-one likes a segfault. Get the list of neighbours and re-order them.
    vector<OBAtom *> nbors;
    ...
    for (vector<OBAtom *>::iterator nbor = nbors.begin(); nbor != nbors.end();
         ++nbor)
...
bool MNAcmp(OBAtom const *const A, OBAtom const *const B) {
  ...
  if (A->IsInRing()) {
    if (B->IsInRing()) {
      if (MNAFormat::Z2MNAZ(A->GetAtomicNum()) <
          MNAFormat::Z2MNAZ(B->GetAtomicNum()))
        altb = true;
      else
        altb = false; // Will this need to recurse here?  (Or does atomic
                      // numbering take care of that?)
    } else
      altb = true;
  } else {
    if (B->IsInRing())
