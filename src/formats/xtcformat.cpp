...
public:
  ...
  XTCFormat() { OBConversion::RegisterFormat("xtc", this); }
...
void XTCFormat::sendints(int buf[], const int num_of_ints,
                         const int num_of_bits, unsigned int sizes[],
                         unsigned int nums[]) {
  ...
  if (xdrmodes[xdrid] == 'w') {
    ...
    while (lfp < fp + size3) {
      ...
      *lip++ = lint3;
      lfp++;
      diff =
          abs(oldlint1 - lint1) + abs(oldlint2 - lint2) + abs(oldlint3 - lint3);
    ...
    while (i < *size) {
      ...
      while (is_small && run < 8 * 3) {
        ...
        thiscoord = thiscoord + 3;
        is_small = 0;
        if (i < *size && abs(thiscoord[0] - prevcoord[0]) < small_ &&
    ...
    if (*size != 0 && lsize != *size) {
      fprintf(stderr,
              "wrong number of coordinates in xdr3dfcoor; "
              "%d arg vs %d in file",
              *size, lsize);
    }
    *size = lsize;
