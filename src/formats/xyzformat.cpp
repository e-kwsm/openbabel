...
public:
  ...
  XYZFormat() { OBConversion::RegisterFormat("xyz", this, "chemical/x-xyz"); }

  const char *Description() override // required
  {
    return "XYZ cartesian coordinates format\n"
           "A generic coordinate format\n"
           "The \"XYZ\" chemical file format is widely supported by many "
           "programs, although no\n"
           "formal specification has been published. Consequently, Open Babel "
           "attempts to be\n"
           "extremely flexible in parsing XYZ format files. Similar formats "
           "include Tinker XYZ\n"
           "and UniChem XYZ which differ slightly in the format of the files. "
           "(Notably, UniChem XYZ\n"
           "uses the atomic number rather than element symbol for the first "
           "column.)\n\n"
           ...
           "with the element\n"
           "symbol, followed by X, Y, and Z coordinates in angstroms separated "
           "by whitespace.\n\n"
           ...
           "the molecule\n"
           "(warning - the number of digits is limited to three for some "
           "programs,\n"
           "e.g. Maestro). Line two is the title of the molecule or the "
           "filename if\n"
           "no title is defined. Remaining lines define the atoms in the file. "
           "The\n"
           "first column is the atomic symbol (right-aligned on the third "
           "character),\n"
           "followed by the XYZ coordinates in \"10.5\" format, in angstroms. "
           "This means\n"
  ...
  const char *GetMIMEType() override { return "chemical/x-xyz"; }
...
bool XYZFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (sscanf(buffer, "%d", &natoms) == 0 || !natoms) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading an XYZ file: The first line must "
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading an XYZ file: Could not read the "
                          "second line (title/comments).",
                          obWarning);
    return (false);
  }
