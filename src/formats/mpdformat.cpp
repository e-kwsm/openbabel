...
***********************************************************************/
// Output format is
// #Origatomtype;#layer-#frequency-#atomtype;#l-#f-#aty;...<tab>Next
// atom<newline>next molecule
...
#define LAYER_DEPTH                                                            \
  2 // cannot increase past 2 without adding more *nbr atom pointers and loops
...
public:
  ...
  {
    return "MolPrint2D format\n"
           "An implementation of the circular fingerprint MolPrint2D\n"
           "MolPrint2D is an atom-environment fingerprint developed by Bender "
           "et al [bmg2004]_\n"
           "which has been used in QSAR studies and for measuring molecular "
           "similarity.\n\n"
           ...
           "                   9;1-1-2;2-1-1;2-1-15;   "
           "15;1-1-2;2-1-1;2-1-9;\n\n"

           ".. [bmg2004] Andreas Bender, Hamse Y. Mussa, and Robert C. Glen. "
           "**Molecular\n"
           "             Similarity Searching Using Atom Environments, "
           "Information-Based\n"
           "             Feature Selection, and a Naive Bayesian "
           "Classifier.**\n"
  ...
  unsigned int Flags() override // Flags() can return be any the following
                                // combined by | or be omitted if none apply
...
bool MPDFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  unsigned int orig,
      otyp; // orig holds first index for removal from layer 2, otype for output
  //    char buffer[BUFF_SIZE];
  bool xml_true = false, pre_true = false, idx_true = false;
  ttab.SetFromType("INT");
  ttab.SetToType("SBN");
  int layer[LAYER_DEPTH]
           [LAYER_SIZE]; // layer stores the frequencies of each atom type
  ClearLayer(layer);

  if (pConv->IsOption("n")) // appending file name to molecule names
  {
    name =
        pConv->GetInFilename(); // string name holds the filename for appending
    unsigned int dotpos =
        name.find("."); // removes the extension(s) from the filename
