...
namespace OpenBabel
{
            ...
            for(unsigned int i=0;i<nb;++i)
              {
                ...
                ss<<" , Occupancy= "<<mvAtom[i].mOccupancy<<'\n';
              }
            obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
          }
      }
  ...
  void CIFData::CalcMatrices()
  {
    ...
    for(long i=0;i<3;i++)
      ...
      stringstream ss;
      ss <<"Fractional2Cartesian matrix:"<<'\n'
           <<mOrthMatrix[0][0]<<" "<<mOrthMatrix[0][1]<<" "<<mOrthMatrix[0][2]<<'\n'
           <<mOrthMatrix[1][0]<<" "<<mOrthMatrix[1][1]<<" "<<mOrthMatrix[1][2]<<'\n'
           <<mOrthMatrix[2][0]<<" "<<mOrthMatrix[2][1]<<" "<<mOrthMatrix[2][2]<<'\n'<<'\n';
      ss <<"Cartesian2Fractional matrix:"<<'\n'
           <<mOrthMatrixInvert[0][0]<<" "<<mOrthMatrixInvert[0][1]<<" "<<mOrthMatrixInvert[0][2]<<'\n'
           <<mOrthMatrixInvert[1][0]<<" "<<mOrthMatrixInvert[1][1]<<" "<<mOrthMatrixInvert[1][2]<<'\n'
    ...
    while(!in.eof())
      {
        ...
        {
          stringstream errorMsg;
          errorMsg << "Warning: one line could not be interpreted while reading a CIF file:"<<'\n'
                   << " -> line contents:" << junk;
  ...
  bool CIFFormat::WriteMolecule(OBBase* pOb, OBConversion* pConv)
  {
    ...
    ofs <<"# CIF file generated by openbabel "<<BABEL_VERSION<<", see https://openbabel.org"<<'\n';

    ofs << "data_I"<<'\n';
    // Use pmol->GetTitle() as chemical name, though it will probably be the file name
    ofs <<"_chemical_name_common '"<<pmol->GetTitle()<<"'"<<'\n';
    // Print Unit cell if we have it
    OBUnitCell *pUC = nullptr;
    if (pmol->HasData(OBGenericDataType::UnitCell))
      {
        pUC = (OBUnitCell*)pmol->GetData(OBGenericDataType::UnitCell);
        ofs << "_cell_length_a " << pUC->GetA() << '\n'
            << "_cell_length_b " << pUC->GetB() << '\n'
            << "_cell_length_c " << pUC->GetC() << '\n'
            << "_cell_angle_alpha " << pUC->GetAlpha() << '\n'
            << "_cell_angle_beta "  << pUC->GetBeta() << '\n'
            << "_cell_angle_gamma " << pUC->GetGamma() << '\n';
        ...
        if (pSG != nullptr)
          {
            ...
            ofs << "_space_group_name_Hall '" << pSG->GetHallName() << "'" << '\n';
            ofs << "loop_" <<'\n'
