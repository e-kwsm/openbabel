...
// The decorated name was longer than the compiler limit (4096), and was
// truncated. This is due to the use of templates specialized on templates
// repeatedly. The correctness of the program, however, is unaffected by the
// truncated name, but if you get link time errors on a truncated symbol, it
// will be more difficult to determine the type of the symbol in the error.
// Debugging will also be more difficult; the debugger will also have
// difficultly mapping symbol name to type name.
#endif
...
public:
  ...
  CIFFormat() { RegisterFormat("cif", "chemical/x-cif"); }

  const char *Description() override // required
  {
    return "Crystallographic Information File\n"
           "The CIF file format is the standard interchange format for "
           "small-molecule crystal structures\n\n"
           "Fractional coordinates are converted to cartesian ones using the "
...
// ############################## Case-insensituve
// string####################################################
//  :@todo: This duplicates normal case-insensitive string comparison in
//  OpenBabel
...
// ############################## CIF CLASSES
// headers####################################################
/** The CIFData class holds all the information from a \e single data_ block
 * from a cif file.
 *
 * It is a placeholder for all comments, item and loop data, as raw strings
 * copied from a cif file.
 *
 * It is also used to interpret this data to extract parts of the cif data, i.e.
 * only part of the core cif dictionnary are recognized. CIF tags currently
 * recognized include ("tag1 > tag2" means tag1 is preferred to tag2 when
 * extracting the info, only one is reported):
 *  - crystal name: _chemical_name_systematic > _chemical_name_mineral >
 * _chemical_name_structure_type > _chemical_name_common
 *  - crystal formula: _chemical_formula_analytical >
 * _chemical_formula_structural > _chemical_formula_iupac >
 ...
 *  - spacegroup Hall symbol: _space_group_name_Hall >
 * _symmetry_space_group_name_Hall
 *  - spacegroup Hermann-Mauguin symbol:_space_group_name_H-M_alt >
 ...
 *
 * If another data field is needed, it is possible to directly access the string
 * data (CIFData::mvComment , CIFData::mvItem and CIFData::mvLoop) to search for
 * the correct tags.
 */
class CIFData {
public:
  ...
  /// Extract lattice parameters, spacegroup (symbol or number), atomic
  /// positions, chemical name and formula if available. All other data is
  /// ignored
  void ExtractAll();
  ...
  /// CIF Loop data
  std::map<std::set<ci_string>, std::map<ci_string, std::vector<std::string>>>
      mvLoop;
  ...
  /// Spacegroup number from International Tables (_space_group_IT_number), or
  /// -1.
  ...
  /// Spacegroup Hermann-Mauguin symbol (or empty string)
  ...
  struct CIFAtom {
    ...
    std::string mLabel;
    /// Symbol of the atom, or empty string (_atom_type_symbol or
    /// _atom_site_type_symbol).
    std::string mSymbol;
    /// Fractionnal coordinates (_atom_site_fract_{x,y,z}) or empty vector.
    std::vector<float> mCoordFrac;
    /// Cartesian coordinates in Angstroem (_atom_site_Cartn_{x,y,z}) or empty
    /// vector. Transformation to fractionnal coordinates currently assumes "a
    /// parallel to x; b in the plane of y and z" (see
...
/** Main CIF class - parses the stream and separates data blocks, comments,
 * items, loops. All values are stored as string, and Each CIF block is stored
 * in a separate CIFData object. No interpretaion is made here - this must be
 * done from all CIFData objects.
 */
class CIF {
public:
  /// Creates the CIF object from a stream
  ///
  /// \param interpret: if true, interpret all data blocks. See
  /// CIFData::ExtractAll()
  CIF(std::istream &in, const bool interpret = true);
  // private:
  /// Separate the file in data blocks and parse them to sort tags, loops and
  /// comments. All is stored in the original strings.
  ///
...
// ############################## CIF CLASSES CODE
// ####################################################
CIFData::CIFAtom::CIFAtom() : mLabel(""), mSymbol(""), mOccupancy(1.0f) {}
...
void CIFData::ExtractAll() {
  ...
  if (mDataBlockName == "data_global") { // :KLUDGE: this data block name is
                                         // used for journal &author information
    ...
    if (mvItem.find("_cell_length_a") != mvItem.end())
      empty_iucrjournal_block = false;
    if (mvItem.find("_cell_length_b") != mvItem.end())
      empty_iucrjournal_block = false;
    if (mvItem.find("_cell_length_c") != mvItem.end())
      empty_iucrjournal_block = false;
    for (map<set<ci_string>, map<ci_string, vector<string>>>::const_iterator
             loop = mvLoop.begin();
         loop != mvLoop.end(); ++loop) {
      if (loop->second.find("_atom_site_fract_x") != loop->second.end())
        empty_iucrjournal_block = false;
      if (loop->second.find("_atom_site_fract_y") != loop->second.end())
        empty_iucrjournal_block = false;
      if (loop->second.find("_atom_site_fract_z") != loop->second.end())
        empty_iucrjournal_block = false;
      if (loop->second.find("_atom_site_Cartn_x") != loop->second.end())
        empty_iucrjournal_block = false;
      if (loop->second.find("_atom_site_Cartn_y") != loop->second.end())
        empty_iucrjournal_block = false;
      if (loop->second.find("_atom_site_Cartn_z") != loop->second.end())
        empty_iucrjournal_block = false;
    }
    if (empty_iucrjournal_block) {
      stringstream ss;
      ss << "CIF WARNING: found en empty 'data_global' block - SKIPPING\n"
         << "  (you can safely ignore this if reading a CIF file from an IUCr "
            "journal)";
...
void CIFData::ExtractUnitCell() {
  ...
  if ((mvItem.find("_cell_length_a") != mvItem.end()) ||
    ...
    stringstream ss;
    ss << "Found Lattice parameters:" << mvLatticePar[0] << " , "
       << mvLatticePar[1] << " , " << mvLatticePar[2] << " , "
       << mvLatticePar[3] << " , " << mvLatticePar[4] << " , "
       << mvLatticePar[5];
    obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
    mvLatticePar[3] =
        static_cast<float>(mvLatticePar[3] * DEG_TO_RAD); // pi/180
    ...
    if ((spgid > 2) &&
        (spgid <= 15)) { // :TODO: monoclinic spg, depending on unique axis....
    }
    if ((spgid > 15) && (spgid <= 142)) { // orthorombic & tetragonal
      if (mvLatticePar[3] == 0)
        mvLatticePar[3] = M_PI / 2;
      if (mvLatticePar[4] == 0)
        mvLatticePar[4] = M_PI / 2;
      if (mvLatticePar[5] == 0)
        mvLatticePar[5] = M_PI / 2;
    }
    if ((spgid > 74) &&
        (spgid <= 142)) { // Tetragonal, make sure a=b if one is missing
      if (mvLatticePar[1] == 0)
        mvLatticePar[1] = mvLatticePar[0];
      if (mvLatticePar[0] == 0)
        mvLatticePar[0] = mvLatticePar[1];
    }
    if ((spgid > 142) && (spgid <= 194)) { // trigonal/ rhomboedric...
      const string::size_type pos = mSpaceGroup->GetHallName().find('R');
      if (pos ==
          std::string::npos) { // rhomboedric cell, a=b=c, alpha=beta=gamma
        float a = 0;
        if (mvLatticePar[0] > a)
          a = mvLatticePar[0];
        if (mvLatticePar[1] > a)
        ...
        float alpha = 0;
        if (mvLatticePar[3] > alpha)
          alpha = mvLatticePar[3];
        if (mvLatticePar[4] > alpha)
          alpha = mvLatticePar[4];
        if (mvLatticePar[5] > alpha)
          alpha = mvLatticePar[5];
        if (mvLatticePar[3] == 0)
          mvLatticePar[3] = alpha;
        if (mvLatticePar[4] == 0)
          mvLatticePar[4] = alpha;
        if (mvLatticePar[5] == 0)
          mvLatticePar[5] = alpha;
    ...
    if (mvLatticePar[3] < 1e-6) {
      stringstream ss;
      ss << "CIF WARNING: missing alpha value, defaulting to 90 degrees (in "
            "data block:"
         << mDataBlockName << ")";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obWarning);
    ...
    if (mvLatticePar[4] < 1e-6) {
      stringstream ss;
      ss << "CIF WARNING: missing beta value, defaulting to 90 degrees (in "
            "data block:"
         << mDataBlockName << ")";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obWarning);
    ...
    if (mvLatticePar[5] < 1e-6) {
      stringstream ss;
      ss << "CIF WARNING: missing gamma value, defaulting to 90 degrees (in "
            "data block:"
         << mDataBlockName << ")";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obWarning);
    ...
    if (mvLatticePar[1] < 1e-6) {
      stringstream ss;
      ss << "CIF Error: missing b lattice parameter - cannot interpret "
            "structure ! (in data block:"
         << mDataBlockName << ")";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
    }
    if (mvLatticePar[2] < 1e-6) {
      stringstream ss;
      ss << "CIF Error: missing c lattice parameter - cannot interpret "
            "structure ! (in data block:"
         << mDataBlockName << ")";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
    }
    ...
    stringstream ss;
    ss << "CIF Error: missing a,b and c value - cannot interpret structure ! "
          "(in data block:"
       << mDataBlockName << ")";
    obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
  }
...
void CIFData::ExtractSpacegroup() {
  ...
  if (positem != mvItem.end()) {
    ...
    if (positem != mvItem.end()) {
      ...
      found = true;
      stringstream ss;
      ss << "Found spacegroup IT number (with OBSOLETE CIF #1.0 TAG):"
         << mSpacegroupNumberIT;
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
    } else {
      ...
      if (positem != mvItem.end()) {
        ...
        found = true;
        stringstream ss;
        ss << "Found spacegroup IT number (with NON-STANDARD CIF TAG):"
           << mSpacegroupNumberIT;
        obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
      } else
  ...
  if (positem != mvItem.end()) {
    ...
    obErrorLog.ThrowError(
        __FUNCTION__, "Found spacegroup Hall symbol:" + mSpacegroupSymbolHall,
        obDebug);
  } else {
    ...
    if (positem != mvItem.end()) {
      ...
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Found spacegroup Hall symbol (with OBSOLETE CIF #1.0 TAG):" +
              mSpacegroupSymbolHall,
          obDebug);
    }
  }
  ...
  if (positem != mvItem.end()) {
    ...
    obErrorLog.ThrowError(__FUNCTION__,
                          "Found spacegroup Hermann-Mauguin symbol:" +
                              mSpacegroupHermannMauguin,
                          obDebug);
  } else {
    ...
    if (positem != mvItem.end()) {
      ...
      found = true;
      obErrorLog.ThrowError(__FUNCTION__,
                            "Found spacegroup Hermann-Mauguin symbol (with "
                            "OBSOLETE CIF #1.0 TAG):" +
                                mSpacegroupHermannMauguin,
                            obDebug);
    }
  }
  // DDL2 tag is "_space_group.IT_coordinate_system_code", converted by the cif
  // reader to "_space_group_IT_coordinate_system_code"
  positem = mvItem.find("_space_group_IT_coordinate_system_code");
  if (positem != mvItem.end()) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Found spacegroup IT_coordinate_system_code:" +
                              positem->second,
                          obDebug);
    if ((mSpacegroupHermannMauguin.length() > 0) &&
        (positem->second == "1" || positem->second == "2")) {
      // this is a HACK which will work as long as the HM symbols in
      // spacegroups.txt have the ":1" or ":2" extension listed, when needed
      mSpacegroupHermannMauguin =
          mSpacegroupHermannMauguin + string(":") + positem->second;
    } else {
      stringstream ss;
      ss << "CIF Error: found DDL2 tag _space_group.IT_coordinate_system_code ("
         << positem->second << ")" << endl
         << "            but could not interpret it ! Origin choice or axis "
            "may be incorrect.";
      obErrorLog.ThrowError(__FUNCTION__, ss.str(), obWarning);
    }
  }
  ...
  if (mSpacegroupSymbolHall.length() > 0) {
    ...
    for (std::string::iterator pos = mSpacegroupSymbolHall.begin();
         pos != mSpacegroupSymbolHall.end();) {
      if ((char)(*pos) == ' ')
        pos = mSpacegroupSymbolHall.erase(pos);
      else
        ++pos;
    }
  ...
  if (mSpaceGroup == nullptr) {
    stringstream ss;
    ss << "CIF Error: missing spacegroup description: defaulting to P1... (in "
          "data block:"
       << mDataBlockName << ")";
    obErrorLog.ThrowError(__FUNCTION__, ss.str(), obWarning);
...
void CIFData::ExtractName() {
  ...
  if (positem != mvItem.end()) {
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Found chemical name:" + mName,
                          obDebug);
  } else {
  ...
  if (positem != mvItem.end()) {
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Found chemical formula:" + mFormula,
                          obDebug);
  } else {
    ...
    if (positem != mvItem.end()) {
      ...
      if (positem != mvItem.end()) {
        mFormula = positem->second;
        obErrorLog.ThrowError(__FUNCTION__,
                              "Found chemical formula:" + mFormula, obDebug);
      } else {
...
void CIFData::ExtractAtomicPositions() {
  ...
  for (map<set<ci_string>, map<ci_string, vector<string>>>::const_iterator
       ...
       loop != mvLoop.end(); ++loop) {
    if (mvAtom.size() > 0)
      break; // only extract ONE list of atoms, preferably fractional
             // coordinates
    map<ci_string, vector<string>>::const_iterator posx, posy, posz, poslabel,
        possymbol, posoccup;
    posx = loop->second.find("_atom_site_fract_x");
    posy = loop->second.find("_atom_site_fract_y");
    posz = loop->second.find("_atom_site_fract_z");
    unsigned int nb = 0;
    if ((posx != loop->second.end()) && (posy != loop->second.end()) &&
        (posz != loop->second.end())) {
      nb = posx->second.size();
      mvAtom.resize(nb);
    ...
    if (mvAtom.size() > 0) { // Got the atoms, get names and symbols
      ...
      if (poslabel != loop->second.end())
        for (unsigned int i = 0; i < nb; ++i) {
          ...
          if (possymbol ==
              loop->second
                  .end()) { // There was no symbol, use the labels to guess it
            int nbc = 0;
            if (mvAtom[i].mLabel.size() == 1)
              if (isalpha(mvAtom[i].mLabel[0]))
                nbc = 1;
            if (mvAtom[i].mLabel.size() >= 2) {
              if (isalpha(mvAtom[i].mLabel[0]) && isalpha(mvAtom[i].mLabel[1]))
                nbc = 2;
              else if (isalpha(mvAtom[i].mLabel[0]))
                nbc = 1;
            }
            if (nbc > 0)
              mvAtom[i].mSymbol = mvAtom[i].mLabel.substr(0, nbc);
            else
              mvAtom[i].mSymbol = "H"; // Something wen wrong, no symbol !
          }
        }
      // Occupancy ?
...
void CIFData::ExtractBonds() {
  ...
  for (map<set<ci_string>, map<ci_string, vector<string>>>::const_iterator
    ...
    map<ci_string, vector<string>>::const_iterator poslabel1, poslabel2,
        posdist;
    ...
    if ((poslabel1 != loop->second.end()) &&
        (poslabel2 != loop->second.end()) && (posdist != loop->second.end())) {
      obErrorLog.ThrowError(__FUNCTION__, "Found _geom_bond* record...",
                            obDebug);
      ...
      for (unsigned int i = 0; i < nb; ++i) {
        ...
        stringstream ss;
        ss << "  d(" << mvBond[i].mLabel1 << "-" << mvBond[i].mLabel2
           << ")=" << mvBond[i].mDistance;
        obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
      }
    }
...
void CIFData::ExtractCharges() {
  ...
  for (map<set<ci_string>, map<ci_string, vector<string>>>::const_iterator
       ...
       loop != mvLoop.end(); ++loop) {
    // if(mvBond.size()>0) break;// Only allow one bond list
    map<ci_string, vector<string>>::const_iterator pos_symbol, pos_ox_number,
        posdist;
    pos_symbol = loop->second.find("_atom_type_symbol");
    pos_ox_number = loop->second.find("_atom_type_oxidation_number");
    if ((pos_symbol != loop->second.end()) &&
        (pos_ox_number != loop->second.end())) {
      obErrorLog.ThrowError(
          __FUNCTION__, " Found _atom_type* record with oxydation number...",
          obDebug);
      const unsigned long nl = pos_symbol->second.size();

      for (unsigned int i = 0; i < nl; i++) {
        lbl2ox[pos_symbol->second[i]] =
            CIFNumeric2Float(pos_ox_number->second[i]);
        obErrorLog.ThrowError(__FUNCTION__,
                              " has oxydation " + pos_ox_number->second[i],
                              obDebug);
      }
    }
  }

  for (std::vector<CIFAtom>::iterator it = mvAtom.begin(); it != mvAtom.end();
       ++it) {
    ...
    if (lbl2ox.count(label) > 0)
      ...
      (*it).mCharge = NOCHARGE;
      obErrorLog.ThrowError(__FUNCTION__,
                            "Charge for label: " + label + " cannot be found.",
                            obDebug);
    }
  }
}
...
void CIFData::CalcMatrices() {
  ...
  v = sqrt(1 - cos(alpha) * cos(alpha) - cos(beta) * cos(beta) -
  ...
  for (long i = 0; i < 3; i++)
    for (long j = 0; j < 3; j++)
      if (i == j)
        mOrthMatrixInvert[i][j] = 1;
      else
        mOrthMatrixInvert[i][j] = 0;
  for (long i = 0; i < 3; i++) {
    float a;
    for (long j = i - 1; j >= 0; j--) {
      a = cm[j][i] / cm[i][i];
      for (long k = 0; k < 3; k++)
        mOrthMatrixInvert[j][k] -= mOrthMatrixInvert[i][k] * a;
      for (long k = 0; k < 3; k++)
        cm[j][k] -= cm[i][k] * a;
    }
    a = cm[i][i];
    for (long k = 0; k < 3; k++)
      mOrthMatrixInvert[i][k] /= a;
    for (long k = 0; k < 3; k++)
      cm[i][k] /= a;
  }
  stringstream ss;
  ss << "Fractional2Cartesian matrix:" << endl
     << mOrthMatrix[0][0] << " " << mOrthMatrix[0][1] << " "
     << mOrthMatrix[0][2] << endl
     << mOrthMatrix[1][0] << " " << mOrthMatrix[1][1] << " "
     << mOrthMatrix[1][2] << endl
     << mOrthMatrix[2][0] << " " << mOrthMatrix[2][1] << " "
     << mOrthMatrix[2][2] << endl
     << endl;
  ss << "Cartesian2Fractional matrix:" << endl
     << mOrthMatrixInvert[0][0] << " " << mOrthMatrixInvert[0][1] << " "
     << mOrthMatrixInvert[0][2] << endl
     << mOrthMatrixInvert[1][0] << " " << mOrthMatrixInvert[1][1] << " "
     << mOrthMatrixInvert[1][2] << endl
     << mOrthMatrixInvert[2][0] << " " << mOrthMatrixInvert[2][1] << " "
     << mOrthMatrixInvert[2][2];
  obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
}
...
void CIFData::c2f(float &x, float &y, float &z) {
  const float x0 = x, y0 = y, z0 = z;
  x = mOrthMatrixInvert[0][0] * x0 + mOrthMatrixInvert[0][1] * y0 +
      mOrthMatrixInvert[0][2] * z0;
  y = mOrthMatrixInvert[1][0] * x0 + mOrthMatrixInvert[1][1] * y0 +
      mOrthMatrixInvert[1][2] * z0;
  z = mOrthMatrixInvert[2][0] * x0 + mOrthMatrixInvert[2][1] * y0 +
      mOrthMatrixInvert[2][2] * z0;
}

void CIFData::Cartesian2FractionalCoord() {
  if (mvLatticePar.size() == 0)
    return; //:@todo: report error
  for (vector<CIFAtom>::iterator pos = mvAtom.begin(); pos != mvAtom.end();
       ++pos) {
...
CIF::CIF(istream &is, const bool interpret) {
  ...
  while (!found_atoms) {
    // :TODO: we don't need a vector of CIFData, since only one block is read at
    // a time
    mvData.clear();
    this->Parse(is);
    // Extract structure from 1 block
    if (interpret)
      for (map<string, CIFData>::iterator posd = mvData.begin();
           posd != mvData.end(); ++posd) {
        posd->second.ExtractAll();
        if (posd->second.mvAtom.size() > 0)
          found_atoms = true;
      }
  }
}
...
string CIFReadValue(istream &in, char &lastc) {
  ...
  if (in.peek() == ';') { // SemiColonTextField
    ...
    if (warning) {
      stringstream errorMsg;
      errorMsg << "Warning: Trying to read a SemiColonTextField but last char "
                  "is not an end-of-line char !";
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obError);
    }
    value = "";
    in.get(lastc);
    ...
    if (!warning)
      in.get(lastc);
    if (vv)
      obErrorLog.ThrowError(__FUNCTION__, "SemiColonTextField:" + value,
                            obDebug);
    if (warning && !vv)
      obErrorLog.ThrowError(__FUNCTION__, "SemiColonTextField:" + value,
                            obDebug);
  ...
  if ((in.peek() == '\'') || (in.peek() == '\"')) { // QuotedString
    ...
    if (vv)
      obErrorLog.ThrowError(__FUNCTION__, "QuotedString:" + value, obDebug);
    return value.substr(0, value.size() - 1);
  }
  // If we got here, we have an ordinary value, numeric or unquoted string
  in >> value;
  if (vv)
    obErrorLog.ThrowError(__FUNCTION__, "NormalValue:" + value, obDebug);
  return value;
}
  ...
  while (!in.eof()) {
    ...
    if (in.peek() == '#') { // Comment
      ...
      if (block == "")
        ...
        mvData[block].mvComment.push_back(tmp);
      lastc = '\r';
      continue;
    }
    if (in.peek() == '_') { // Tag
      string tag, value;
      in >> tag;
      // Convert all dots to underscores to cover much of DDL2 with this DDL1
      // parser.
      for (string::size_type pos = tag.find('.'); pos != string::npos;
           pos = tag.find('.', ++pos))
    ...
    if ((in.peek() == 'd') || (in.peek() == 'D')) { // Data
      if (!mvData.empty())
        return; // We want just a single data block
    ...
    if ((in.peek() == 'l') || (in.peek() == 'L')) { // loop_
      ...
      if (vv)
        obErrorLog.ThrowError(__FUNCTION__, "LOOP : " + tmp, obDebug);
      while (true) { // read titles
        ...
        // Convert all dots to underscores to cover much of DDL2 with this DDL1
        // parser.
        for (string::size_type pos = tmp.find('.'); pos != string::npos;
             pos = tmp.find('.', ++pos))
          tmp.replace(pos, 1, 1, '_');
        tit.push_back(ci_string(tmp.c_str()));
        if (vv)
          obErrorLog.ThrowError(__FUNCTION__, " , " + tmp, obDebug);
      }
      ...
      while (true) {
        ...
        if (in.peek() == '#') { // Comment (in a loop ??)
          ...
          lastc = '\r';
          if (vv)
            obErrorLog.ThrowError(__FUNCTION__, "Comment in a loop (?):" + tmp,
                                  obDebug);
          // in.seekg(pos);
          break;
        }
        ...
        if (ci_string(tmp.c_str()) == "loop_") { // go back and continue
          ...
          stringstream ss;
          ss << "END OF LOOP :" << tmp << "," << (char)in.peek() << ","
             << in.tellg();
          if (vv)
            obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
          break;
        ...
        if (tmp.size() >= 5)
          if (ci_string(tmp.substr(0, 5).c_str()) ==
              "data_") { // go back and continue
            in.clear();
            in.seekg(pos0, std::ios::beg);
            stringstream ss;
            ss << "END OF LOOP :" << tmp << "," << (char)in.peek() << ","
    ...
    if (junk.size() > 0) {
      stringstream errorMsg;
      errorMsg << "Warning: one line could not be interpreted while reading a "
                  "CIF file:"
               << endl
               << " -> line contents:" << junk;
...
// Returns: true if the atom is an oxygen and connected to two hydrogens and up
// to one other atom
...
bool CIFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  obErrorLog.ThrowError(__FUNCTION__,
                        "mmCIF parser not found. Using CIF parser.", obDebug);
  ...
  // Loop on all data blocks until we find one structure :@todo: handle multiple
  // structures
  for (map<string, CIFData>::iterator pos = cif.mvData.begin();
       pos != cif.mvData.end(); ++pos)
    if (pos->second.mvAtom.size() > 0) {
      ...
      if (pos->second.mvLatticePar.size() == 6) { // We have one unit cell
        ...
        pCell->SetData(pos->second.mvLatticePar[0], pos->second.mvLatticePar[1],
      ...
      if (pos->second.mName != "")
        pmol->SetTitle(pos->second.mName);
      else if (pos->second.mFormula != "")
        pmol->SetTitle(pos->second.mFormula);
      else
        pmol->SetTitle(pConv->GetTitle());

      if (pos->second.mFormula != "")
        pmol->SetFormula(pos->second.mFormula);

      // Keep a map linking the cif atom label to the obatom*, for bond
      // interpretation later
      std::map<std::string, OBAtom *> vLabelOBatom;

      const unsigned int nbatoms = pos->second.mvAtom.size();
      pmol->ReserveAtoms(nbatoms);
      for (vector<CIFData::CIFAtom>::const_iterator posat =
               pos->second.mvAtom.begin();
           posat != pos->second.mvAtom.end(); ++posat) {
        // Problem: posat->mSymbol is not guaranteed to actually be a symbol
        // see
        // http://www.iucr.org/iucr-top/cif/cifdic_html/1/cif_core.dic/Iatom_type_symbol.html
        // Try to strip the string to have a better chance to have a valid
        // symbol This is not guaranteed to work still, as the CIF standard
        ...
        if (tmpSymbol.size() >
            nbc) { // Try to find a formal charge in the symbol
          int charge = 0;
          int sign = 0;
          for (unsigned int i = nbc; i < tmpSymbol.size();
               ++i) { // Use first number found as formal charge
        ...
        atom->SetAtomicNum(atomicNum); // set atomic number, or '0' if the atom
                                       // type is not recognized
        atom->SetType(tmpSymbol); // set atomic number, or '0' if the atom type
                                  // is not recognized
        atom->SetVector(posat->mCoordCart[0], posat->mCoordCart[1],
                        posat->mCoordCart[2]);
      ...
      if (!pConv->IsOption("b", OBConversion::INOPTIONS))
        pmol->ConnectTheDots();
      if (pConv->IsOption("B", OBConversion::INOPTIONS)) {
        for (vector<CIFData::CIFBond>::const_iterator posbond =
                 pos->second.mvBond.begin();
             posbond != pos->second.mvBond.end();
             ++posbond) { // Add bonds present in the cif and not detected by
                          // ConnectTheDots()
          ...
          if (posat1 != vLabelOBatom.end() && posat2 != vLabelOBatom.end()) {
            stringstream ss;
            ss << "  Adding cif bond ? " << posat1->first << "-"
               << posat2->first;
            obErrorLog.ThrowError(__FUNCTION__, ss.str(), obDebug);
            if (pmol->GetBond(posat1->second, posat2->second) == nullptr) {
              ...
              obErrorLog.ThrowError(__FUNCTION__, "  :Bond already present.. ",
                                    obDebug);
          }
        }
      }
      ...
      pmol->EndModify();
      pmol->SetAutomaticFormalCharge(
          false);                 // we should have set formal charges
...
bool CIFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  ofs << "# CIF file generated by openbabel " << BABEL_VERSION
      << ", see https://openbabel.org" << endl;

  ofs << "data_I" << endl;
  // Use pmol->GetTitle() as chemical name, though it will probably be the file
  // name
  ...
  if (pmol->HasData(OBGenericDataType::UnitCell)) {
    ...
    if (pSG != nullptr) {
      // Do we have an extended HM symbol, with origin choice as ":1" or ":2" ?
      // If so, remove it.
      size_t n = pSG->GetHMName().find(":");
      if (n == string::npos)
        ofs << "_space_group_name_H-M_alt '" << pSG->GetHMName() << "'" << endl;
      else
        ofs << "_space_group_name_H-M_alt '" << pSG->GetHMName().substr(0, n)
            << "'" << endl;
      ofs << "_space_group_name_Hall '" << pSG->GetHallName() << "'" << endl;
      ofs << "loop_" << endl << "    _symmetry_equiv_pos_as_xyz" << endl;
  ...
  FOR_ATOMS_OF_MOL(atom, *pmol) {
    ...
    if (atom->HasData("_atom_site_occupancy")) {
      occup = (dynamic_cast<OBPairFloatingPoint *>(
                   atom->GetData("_atom_site_occupancy")))
                  ->GetGenericValue();
    } else
      occup = 1.0;
    ...
    if (atom->HasData("_atom_site_label")) {
      OBPairData *label =
          dynamic_cast<OBPairData *>(atom->GetData("_atom_site_label"));
    ...
    snprintf(buffer, BUFF_SIZE, "    %-8s %-5s %10.5f %10.5f %10.5f %8.3f\n",
             label_str.c_str(), OBElements::GetSymbol(atom->GetAtomicNum()), X,
             Y, Z, occup);
  ...
  if (pConv->IsOption("g", OBConversion::OUTOPTIONS)) {
    if (pmol->NumBonds() > 0) {
      obErrorLog.ThrowError(__FUNCTION__, "Writing bonds to output CIF",
                            obDebug);
      ofs << "loop_" << endl
      ...
      obErrorLog.ThrowError(
          __FUNCTION__, "No bonds defined in molecule for CIF export", obDebug);
    }
    ...
    FOR_BONDS_OF_MOL(bond, *pmol) {
      ...
      if (bond->IsPeriodic()) {
        OBUnitCell *box =
            (OBUnitCell *)pmol->GetData(OBGenericDataType::UnitCell);
        vector3 begin, end_orig, end_expected, uc_direction;
        // Use consistent coordinates with the X Y Z written in the _atom_site_*
        // loop earlier
        ...
        // To get the signs right, consider the example {0, 0.7}.  We want -1 as
        // the periodic direction.
        // TODO: Think about edge cases, particularly atoms on the border of the
        // unit cell.
        ...
        symmetry_num +=
            100 * uc[0] + 10 * uc[1] +
            1 * uc[2]; // Unit cell directionality vs. 555, per CIF spec
      }
      if (symmetry_num == 555) {
        sym_key = ".";
      } else {
      ...
      default:
        stringstream ss;
        ss << "Unexpected bond order " << bond_order << " for bond" << label_1
           << "-" << label_2 << std::endl
      ...
      snprintf(buffer, BUFF_SIZE, "    %-7s%-7s%10.5f%7s%4s\n", label_1.c_str(),
               label_2.c_str(), bond->GetLength(), sym_key.c_str(),
               bond_type.c_str());
