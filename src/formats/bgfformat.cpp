...
bool BGFFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (;;) {
    ...
    sscanf(buffer, "%*s %*s %*s %*s %*s %*s %lf %lf %lf %15s %*s %*s %lf", &x,
           &y, &z, tmptyp, &chrg);
    atom = mol.NewAtom();
...
bool BGFFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (mol.HasData(OBGenericDataType::UnitCell)) {
    ...
    // e.g.
    // CRYSTX    49.30287   49.23010   25.45631   90.00008   89.99995   57.10041
    snprintf(buffer, BUFF_SIZE, "CRYSTX%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f",
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    strncpy(elmnt_typ, OBElements::GetSymbol(atom->GetAtomicNum()),
            7); // make sure to null-terminate
    ...
    snprintf(buffer, BUFF_SIZE,
             "%6s %5d %-5s %3s %1s %5s%10.5f%10.5f%10.5f %-5s%3d%2d %8.5f\n",
             "HETATM", atom->GetIdx(), atm_sym, "RES", "A", "444", atom->GetX(),
             atom->GetY(), atom->GetZ(), dreid_typ, max_val, 0,
