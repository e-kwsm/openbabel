...
public:
  ...
  std::string line; // For convenience so we can refer to lines from the
                    // iterator as 'line'
  std::vector<std::string> tokens; // list of lines and list of tokens on a line

  // These are data that are accessed by several subroutines, so we keep them as
  // class variables
  int levcfg, imcon;
  std::string title;
...
bool DlpolyInputReader::ParseHeader(std::istream &ifs, OBMol &mol) {
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__, "Problem reading title line",
                          obWarning);
    return false;
  }
  title = buffer;
...
bool DlpolyInputReader::ReadAtom(std::istream &ifs, OBMol &mol) {
  ...
  // Currently we ignore atom index as it is optional - assume atoms are in
  // order
  if (tokens.size() >= 2)
  ...
  // Check if we've read in the atomic charge or need to try and extract it from
  // the label
  if (AtomicNumber == -1) {
...
public:
  ...
  DlpolyConfigFormat() { OBConversion::RegisterFormat("CONFIG", this); }
...
bool DlpolyConfigFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  /**
   * Write a DL-POLY CONFIG file. Ints are 10 chars wide, floats are formatted
   * as 20.15f
   */
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    ofs << std::setw(8) << OBElements::GetSymbol(atom->GetAtomicNum())
        << std::setw(10) << ++idx << std::setw(10) << atom->GetAtomicNum()
        << std::endl;
...
public:
  // Register this format type ID
  DlpolyHISTORYFormat() { OBConversion::RegisterFormat("HISTORY", this); }
