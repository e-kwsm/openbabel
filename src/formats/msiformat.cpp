...
public:
  ...
  {
    return "Accelrys/MSI Cerius II MSI format\n";
  }

  const char *SpecificationURL() override {
    return "http://openbabel.org/wiki/MSI_format";
  } // optional

  const char *GetMIMEType() override { return "chemical/x-msi-msi"; }
...
bool MSIFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!EQn(buffer, "# MSI CERIUS2 DataModel File", 28)) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Problems reading an MSI file: The first line must "
                          "contain the MSI header.",
                          obWarning);
    return (false);
  }
    ...
    */
    ...
    if (atomRecord) {
      if (strstr(buffer, "ACL") != nullptr) {
        ...
        atom->SetVector(atof(vs[3].c_str()), atof(vs[4].c_str()),
                        atof(vs[5].c_str()));
        continue;
      }
    } // end of atom records
  ...
  !pConv->IsOption("b",OBConversion::INOPTIONS)) mol.PerceiveBondOrders();
  */
