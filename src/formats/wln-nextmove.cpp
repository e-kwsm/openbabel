...
OpenBabel::OBBond *NMOBMolNewBond(OpenBabel::OBMol *mol, OpenBabel::OBAtom *beg,
                                  OpenBabel::OBAtom *end, unsigned int order,
                                  bool arom) {
...
void NMOBMolDestroyBond(OpenBabel::OBMol *mol, OpenBabel::OBAtom *beg,
...
struct WLNParser {
  ...
  void fuse(OpenBabel::OBAtom *src, OpenBabel::OBAtom *dst,
  ...
  // General Poly Ring Case Addition - Michael Blakey for Nextmove Software <---
  // solved for general case

  int polyfused_ring(std::vector<OpenBabel::OBAtom *> &ring,
                     std::vector<char> char_vector,
                     std::vector<int> atom_vector, int ring_positions) {
    ...
    for (int i = 0; i < char_vector.size(); i++) {
      ...
      if (it != bond_stack.end()) {
        ...
        bond_stack.erase(bond_stack.begin() + index + 1,
                         bond_stack.begin() + stack_val);
      }
    }
    try {
  ...
  // General Peri Ring Case Addition - Michael Blakey for Nextmove Software <---
  // solved for general case
  // <-- issues with local path redefinition, they'll be some graph theory
  // somewhere for this
  int perifused_ring(std::vector<OpenBabel::OBAtom *> &ring,
                     std::vector<char> char_vector,
                     std::vector<int> atom_vector,
                     std::vector<char> peri_vector, int ring_positions) {
    ...
    int atom_clear = static_cast<int>(atom_vector.size()) -
                     static_cast<int>(peri_vector.size());
    ...
    for (int i = 0; i < peri_vector.size(); i++) {
      ...
      int index = it - bond_stack.begin();
      int stack_val =
          index + atom_vector.at(i) - 2; // virtual letter increases correction
      if (stack_val >= bond_stack.size()) {
  ...
  int ReduceBridge_ring(std::vector<OpenBabel::OBAtom *> &ring,
                        std::vector<char> bridge_vector, int size) {
    ...
    if (!mol->GetBond(ring[last_connection + 1],
                      ring[size - num_connections - 1]))
      return error();
  ...
  int detect_peri(std::string wln_string, std::vector<int> &cycles,
                  unsigned int &peri_atoms) {
    bool peri = false;
    int peri_count = 0;
    int peri_index = 0;
    if (isdigit(ptr[1]) && isdigit(ptr[2]) && isdigit(ptr[3])) {
      wln_string.insert(1, " A");
    }

    for (int i = 0; i < wln_string.size(); i++) {
      if (isdigit(wln_string[i]) && wln_string[i - 1] == ' ' &&
          wln_string[i + 1] != ' ') { // Change point 1
        if (wln_string[i + 1] >= 'D') {
          fprintf(stderr, "Peri Points Greater Than Char Level 3 Are Currently "
                          "Unstable \n");
          return error();
          break;
        }
  ...
  void AtomCharVector(int ptr_it, std::string wln_string,
                      std::vector<int> &atom_vector,
                      std::vector<char> &char_vector) {
  ...
  bool parse_ring() {
    ...
    if (ptr[1] >= '3' && ptr[1] <= '9') {
      ...
      // Macro Sized Rings
    } else if (ptr[1] == '-' && ptr[2] >= '1' && ptr[2] <= '9' &&
               ptr[3] >= '0' && ptr[3] <= '9' && ptr[4] == '-') {
      ...
      if (peri_index != 0) {
        ...
        atom_sum = static_cast<unsigned int>(static_cast<int>(atom_sum) -
                                             static_cast<int>(peri_atoms));
        ring_count = static_cast<unsigned int>(
            static_cast<int>(ring_count) - static_cast<int>(1 + peri_atoms));
        int bond_alter = static_cast<int>(ring_count) - 2;
        ring_positions = static_cast<int>(atom_sum) -
                         (static_cast<int>(ring_count) + bond_alter) - 1 -
                         static_cast<int>(peri_atoms * 3);
    ...
    case 52:
      ...
      if (peri_index != 0) {
        size = perifused_ring(ring, char_vector, atom_vector, peri_vector,
                              ring_positions);
        done = true;
        ptr += ptr_it;
        CONFLICT = 1;
    ...
    for (;;) {
      ...
      case 'J':
        if (ali_merge) {
          int ring_sum =
              std::accumulate(size_catch.begin(), size_catch.end(), 0) - 2 -
              1; // gives ring atoms with zero index handled
          if (ali_state == 0) {
            for (unsigned int i = 1; i < size_catch.at(0) - 1; i++) {
              aptr = ring[i];
              if (aptr->IsAromatic() && aptr->GetAtomicNum() == 6) {
  ...
  bool parse() {
    ptr = orig;
    if (ptr[0] == 'W' && ptr[1] == 'L' && ptr[2] == 'N' && ptr[3] == ':' &&
        ptr[4] == ' ')
      ptr += 5;
    for (;;) {
      if (debug_wln_read)
        ...
        case ' ':
          if (ptr[2] >= 'A' && ptr[2] <= 'Z' &&
            ...
            if (order == 1 || order == 2) {
              ...
              if (spiro) {
                fuse(prev, rings.back()[loc + 1],
                     order); // "Surround" the spiro center
