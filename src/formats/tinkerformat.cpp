...
public:
  ...
  TinkerFormat() { OBConversion::RegisterFormat("txyz", this); }

  const char *Description() override // required
  {
    return "Tinker XYZ format\n"
           "The cartesian XYZ file format used by the molecular mechanics "
           "package TINKER.\n"
           "By default, the MM2 atom types are used for writing files but MM3 "
           "atom types\n"
           "are provided as an option. Another option provides the ability to "
           "take the\n"
           "atom type from the atom class (e.g. as used in SMILES, or set via "
           "the API).\n\n"
  ...
  for (int i = 1; i <= natoms; ++i) {
    ...
    if (vs.size() > 6)
      for (unsigned int j = 6; j < vs.size(); ++j)
        pmol->AddBond(pmol->NumAtoms(), stoi(vs[j]),
                      1); // we don't know the bond order
  }
...
bool TinkerFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (mmffTypes && ff && ff->Setup(*pmol))
    ...
    mmffTypes =
        false; // either the force field isn't available, or it doesn't work
  ...
  FOR_ATOMS_OF_MOL(atom, pmol) {
    ...
    snprintf(buffer, BUFF_SIZE, "%6d %2s  %12.6f%12.6f%12.6f %5d",
             atom->GetIdx(), OBElements::GetSymbol(atom->GetAtomicNum()),
             atom->GetX(), atom->GetY(), atom->GetZ(), atomType);
    ofs << buffer;
...
int SetMM3Type(OBAtom *atom) {
  ...
  // The MM2 typing isn't very good, so we do this ourselves for the most common
  // atom types
  ...
  case 8: // O
    ...
    b = atom->BeginNbrAtom(j);
    if (atom->HasBondOfOrder(2) &&
        b->GetAtomicNum() == OBElements::Carbon) { // O=C
      return 7;
    }
