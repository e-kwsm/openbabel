...
public:
  ...
  {
    return "Generic Output file format\n"
           "Automatically detect and read computational chemistry output "
           "files\n\n"
           "This format can be used to read ADF, Gaussian, GAMESS, PWSCF, "
           "Q-Chem,\n"
           "MOPAC, ORCA etc. output files by automatically detecting the file "
           "type.\n\n"
           "Read Options e.g. -as\n"
  ...
  const char *SpecificationURL() override {
    return "http://openbabel.org/wiki/Output";
  } // optional
...
bool OutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, "GAMESS execution script") != nullptr ||
      ...
      formatName = "nwo";
      break;
    } else if (strstr(buffer, "MPQC: Massively Parallel Quantum Chemistry") !=
               nullptr) {
      // MPQC output
      formatName = "mpqc";
      break;
  ...
  if (pFormat) {
    ...
    // Tag the molecule with the format (e.g., if a program wants to know the
    // kind of "out" or "log" file) We have to do this *after* ReadMolecule
    // returns, or the data might be cleared
    if (pOb) {
  ...
  obErrorLog.ThrowError(
      __FUNCTION__,
      "Problems reading an output file: Could not determine the format of this "
      "file. Please report it to the openbabel-discuss @ lists.sourceforge.net "
      "mailing list.",
      obError);
