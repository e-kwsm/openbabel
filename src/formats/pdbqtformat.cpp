...
public:
  ...
  set<unsigned int>
      rigid_with; // the other branches that move rigidly with this one
...
public:
  ...
  const char *Description() override // required
  {
    return
        ...
        "Reads and writes AutoDock PDBQT (Protein Data Bank, Partial Charge "
        "(Q), & Atom Type (T)) format\n"
        ...
        "  c  Combine separate molecular pieces of input into a single rigid "
        "molecule (requires \"r\" option or will have no effect)\n"
        "  s  Output as a flexible residue\n"
        "  p  Preserve atom indices from input file (default is to renumber "
        "atoms sequentially)\n"
  ...
  const char *SpecificationURL() override {
    return "http://autodock.scripps.edu/faqs-help/faq/"
           "what-is-the-format-of-a-pdbqt-file";
  }

  const char *GetMIMEType() override { return "chemical/x-pdbqt"; }
...
static void OutputGroup(OBMol &mol, ostream &ofs, const vector<int> &group,
                        map<unsigned int, unsigned int> new_indexes,
                        bool use_new_indexes);
static unsigned int AtomsSoFar(const map<unsigned int, branch> &tree,
                               unsigned int depth);
static bool FindBondedPiece(const vector<int> &root, const vector<int> &branch,
                            unsigned int &root_atom, unsigned int &branch_atom,
                            unsigned int &root_atom_rank,
                            unsigned int &branch_atom_rank, const OBMol &mol,
                            unsigned int &atoms_moved);
static bool OutputTree(OBConversion *pConv, OBMol &mol, ostream &ofs,
                       map<unsigned int, branch> &tree, unsigned int depth,
                       bool moves_many, bool preserve_original_index);
static void ConstructTree(map<unsigned int, branch> &tree,
                          vector<vector<int>> rigid_fragments,
                          unsigned int root_piece, const OBMol &mol,
                          bool flexible);
static bool DeleteHydrogens(OBMol &mol);
static bool Separate_preserve_charges(OBMol &mol, vector<OBMol> &result);
static unsigned int FindFragments(OBMol mol,
                                  vector<vector<int>> &rigid_fragments);
...
bool PDBQTFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pConv->IsOption("d", OBConversion::INOPTIONS)) {
    dlg = true;
  } // check whether we have a file in dlg format
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    line = buffer;
    if (dlg) // if we have a dlg file, only care about the lines starting with
             // "DOCKED: "
    {
      if (!EQn(buffer, "DOCKED: ", 8)) {
        continue;
      } else {
    ...
    if (EQn(buffer, "ROOT", 4)) {
      continue;
    }
    if (EQn(buffer, "ENDROOT", 7)) {
      continue;
    }
    if (EQn(buffer, "BRANCH", 6)) {
      continue;
    }
    if (EQn(buffer, "ENDBRANCH", 9)) {
      continue;
    }
  ...
  vector<OBGenericData *> vbonds =
      mol.GetAllData(OBGenericDataType::VirtualBondData);
  ...
  // Guess how many hydrogens are present on each atom based on typical
  // valencies (from pdbformat.cpp)
...
void OutputAtom(OBAtom *atom, ostream &ofs, const unsigned int index) {
  ...
  OBResidue *res;
  strncpy(type_name, OBElements::GetSymbol(atom->GetAtomicNum()),
          sizeof(type_name));
  ...
  if (atom->GetAtomicNum() == OBElements::Hydrogen) {
  ...
  } else if ((atom->GetAtomicNum() == OBElements::Carbon) &&
             (atom->IsAromatic())) {
    ...
    element_name_final[1] = 'A';
  } else if ((atom->GetAtomicNum() == OBElements::Sulfur) &&
             (atom->IsHbondAcceptor())) {
    element_name_final[0] = 'S';
    element_name_final[1] = 'A';
  } else {
    if (!isalnum(element_name[0])) {
      element_name_final[0] = ' ';
    } else
  ...
  snprintf(
      buffer, BUFF_SIZE,
      "%s%5d %-4s %-3s %c%4d%c   %8.3f%8.3f%8.3f  0.00  0.00  %+8.3f %-2.2s",
      het ? "HETATM" : "ATOM  ", index, type_name, the_res, the_chain, res_num,
      the_icode, atom->GetX(), atom->GetY(), atom->GetZ(), charge,
      element_name_final);
  ofs << buffer;
  ofs << endl;
}

void OutputGroup(OBMol &mol, ostream &ofs, const vector<int> &group,
                 map<unsigned int, unsigned int> new_indexes,
                 bool use_new_indexes) {
  for (vector<int>::const_iterator it = group.begin(); it != group.end();
       ++it) {
    if (use_new_indexes) {
      OutputAtom(mol.GetAtom((*it)), ofs, new_indexes.find(*it)->second);
    } else {
      OutputAtom(mol.GetAtom((*it)), ofs, (*it));
    }
  }
}
...
unsigned int FindFragments(OBMol mol, vector<vector<int>> &rigid_fragments) {
  ...
  for (unsigned int i = 1; i <= mol.NumAtoms(); i++)
  // finds the root atom by copying the molecule, deleting each atom in turn,
  // and finding the sizes of the resulting pieces
  {
  ...
  OBMol mol_pieces = mol;
  for (OBBondIterator it = mol_pieces.BeginBonds(); it != mol_pieces.EndBonds();
       it++) {
    if (IsRotBond_PDBQT((*it))) {
      bonds_to_delete.push_back(*it);
    }
  }
  for (vector<OBBond *>::iterator bit = bonds_to_delete.begin();
       bit != bonds_to_delete.end(); ++bit) {
...
bool OutputTree(OBConversion *pConv, OBMol &mol, ostream &ofs,
                map<unsigned int, branch> &tree, unsigned int depth,
                bool moves_many, bool preserve_original_index) {
  if (tree.empty()) {
    return false;
  ...
  set<unsigned int>
      free_bonds; // this section is to allow the code to be generalised when
                  // using obabel rather than babel, which accepts numerical
                  // arguments as to how many bonds to fix. As laid out, it will
                  // prioritise those bonds that move the fewest atoms, unless
                  // moves_many is true, where it will prioritise those that
                  // move the most. This is moot for the moment, as either all
                  // rotatable bonds are free, or they are all rigid.

  free_bonds.insert(0);
  multimap<unsigned int, unsigned int> how_many_atoms_move;
  for (unsigned int i = 1; i < tree.size(); i++) {
    how_many_atoms_move.insert(pair<unsigned int, unsigned int>(
        (*tree.find(i)).second.how_many_atoms_moved, i));
  }

  multimap<unsigned int, unsigned int>::iterator it =
      how_many_atoms_move.begin();
  ...
  for (unsigned int i = tree.size() - 1; i > 0; i--) {
    if (!free_bonds.count(i)) // adds the index of any branch with rigid
                              // rotations to its parent
    {
      unsigned int parent = (*tree.find(i)).second.UpOne();
      (*tree.find(parent))
          .second.rigid_with.insert((*tree.find(i)).second.rigid_with.begin(),
                                    (*tree.find(i)).second.rigid_with.end());
    }
  }

  map<unsigned int, unsigned int>
      new_order; // gives the new ordering of the indexes of atoms, so that they
                 // are in increasing order from 1 in the output
    ...
    for (unsigned int i = 0; i < tree.size(); i++) {
      if (free_bonds.count(i)) {
        for (set<unsigned int>::iterator it =
                 (*tree.find(i)).second.rigid_with.begin();
             it != (*tree.find(i)).second.rigid_with.end(); ++it) {
          vector<int> atoms = (*tree.find(*it)).second.atoms;
          for (unsigned int j = 0; j < atoms.size(); j++) {
            new_order.insert(pair<unsigned int, unsigned int>(
                atoms.at(j), current_atom_index));
            current_atom_index++;
          }
        }
      }
  ...
  if (!(pConv->IsOption("r", OBConversion::OUTOPTIONS)))
    ofs << "ROOT" << endl;
  for (set<unsigned int>::iterator it =
           (*tree.find(0)).second.rigid_with.begin();
       it != (*tree.find(0)).second.rigid_with.end(); ++it) {
    OutputGroup(mol, ofs, (*tree.find(*it)).second.atoms, new_order,
  ...
  for (unsigned int i = 1; i < tree.size(); i++) {
    if (free_bonds.count(i)) {
      ...
      if (!preserve_original_index) {
        ofs << (new_order.find(parent_atom))->second;
      } else {
        ofs << parent_atom;
      }
      ofs.width(4);
      if (!preserve_original_index) {
        ofs << (new_order.find(child_atom))->second;
      } else {
        ofs << child_atom;
      }
      ofs << endl;
    ...
    for (vector<unsigned int>::iterator it =
             (*tree.find(i)).second.parents.end();
         it != (*tree.find(i)).second.parents.begin();) {
      --it;
      if ((*it) == 0) {
        break;
      } // do not close the main root; that is closed separately
      ...
      if ((*tree.find(*it)).second.children.size() == 0) {
        if (free_bonds.count(*it)) {
          ofs << "ENDBRANCH";
          ofs.width(4);
          unsigned int parent_atom =
              (*tree.find(*it)).second.connecting_atom_parent;
          unsigned int child_atom =
              (*tree.find(*it)).second.connecting_atom_branch;
          if (!preserve_original_index) {
            ofs << (new_order.find(parent_atom))->second;
          } else {
            ofs << parent_atom;
...
void ConstructTree(map<unsigned int, branch> &tree,
    ...
    if (fecund) {
      ...
      for (unsigned int i = 0; i < rigid_fragments.size(); i++) {
        if (FindBondedPiece((*tree.find(position)).second.atoms,
                            rigid_fragments.at(i), first_atom, second_atom,
                            first_atom_rank, second_atom_rank, mol,
                            atoms_moved)) {
          ...
          sprog.parents =
              (*tree.find(position))
                  .second.parents; // all parents of the parent are parents too
          sprog.parents.push_back(tree.size()); // a branch is its own parent
          sprog.index =
              tree.size(); // the index is simply the number of precursors
          sprog.rigid_with.clear();
          sprog.rigid_with.insert(sprog.index);

          (*tree.find(position))
              .second.children.insert(tree.size()); // tells the current parent
                                                    // it has an extra child
          tree.insert(pair<unsigned int, branch>(
              tree.size(), sprog)); // adds the current branch to the tree
...
// identifies a bond as rotatable if it is a single bond, not amide, not in a
// ring, and if both atoms it connects have at least one other atom bounded to
// them
{
  ...
  if (((the_bond->GetBeginAtom())->GetExplicitDegree() == 1) ||
      ((the_bond->GetEndAtom())->GetExplicitDegree() == 1)) {
    return false;
  }
  return true;
}
...
bool IsIn(
    ...
    const int num) // checks whether a vector of int contains a specific int
{
...
unsigned int AtomsSoFar(const map<unsigned int, branch> &tree,
                        unsigned int depth) {
  if (depth > tree.size()) {
    return 0;
  }
  unsigned int numberAtoms = 0;
  //		for (unsigned int i = 0; i < depth; i++) {numberAtoms+=
  // tree.at(i).second.first.size();}
  return numberAtoms;
}
...
bool FindBondedPiece(const vector<int> &root, const vector<int> &branched,
  ...
  for (unsigned int i = 0; i < root.size(); i++) {
    for (unsigned int j = 0; j < branched.size(); j++) {
      the_bond =
          mol.GetBond(mol.GetAtom(root.at(i)), mol.GetAtom(branched.at(j)));
      if (the_bond != nullptr) {
        root_atom = root.at(i);
...
bool PDBQTFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (unsigned int i = 0; i < all_pieces.size(); i++) {
    ...
    all_pieces.at(i)
        .SetAromaticPerceived(); // retain aromatic flags in fragments
    ...
    if (!residue) {
      if (!pConv->IsLast() ||
          pConv->GetOutputIndex() > 1) {     // More than one molecule record
      ...
      if (!(pConv->IsOption("r", OBConversion::OUTOPTIONS))) {
        ...
        for (OBBondIterator it = mol.BeginBonds(); it != mol.EndBonds(); it++) {
          if (IsRotBond_PDBQT((*it))) {
            ...
            qsort(rotBondTable[rotBondId], 2, sizeof(OBAtom *),
                  CompareBondAtoms);
            rotBondId++;
          }
        }
        ...
        for (rotBondId = 0; rotBondId < nRotBond; rotBondId++) {
          snprintf(buffer, BUFF_SIZE,
                   "REMARK  %3d  A    between atoms: ", rotBondId + 1);
          ofs << buffer;
          for (bondAtomNum = 0; bondAtomNum < 2; bondAtomNum++) {
            memset(type_name, 0, sizeof(type_name));
            strncpy(type_name,
                    OBElements::GetSymbol(
                        rotBondTable[rotBondId][bondAtomNum]->GetAtomicNum()),
                    sizeof(type_name));
            if (strlen(type_name) > 1)
              type_name[1] = toupper(type_name[1]);
            if ((res = rotBondTable[rotBondId][bondAtomNum]->GetResidue()) !=
                nullptr) {
              snprintf(
                  type_name, 5, "%s",
                  (char *)res->GetAtomID(rotBondTable[rotBondId][bondAtomNum])
                      .c_str());
              // AtomIDs may start with space if read from a PDB file (rather
              // than perceived)
              end = isspace(type_name[0]) ? 1 : 0;
              // Use sizeof() - 1 to ensure there's room for the NULL
              // termination!
              while (end < sizeof(type_name) - 1 && type_name[end] &&
                     !isspace(type_name[end]))
                end++;
              type_name[end] = '\0';
            }
            snprintf(buffer, BUFF_SIZE, "%s_%d",
                     type_name + (isspace(type_name[0]) ? 1 : 0),
      ...
      ofs << "REMARK                            x       y       z     vdW  "
             "Elec       q    Type"
          << endl;
      //        ofs << "USER                              x       y       z vdW
      //        Elec       q    Type" << endl;
      ofs << "REMARK                         _______ _______ _______ _____ "
             "_____    ______ ____"
          << endl;
      //        ofs << "USER                           _______ _______ _______
      //        _____ _____    ______ ____" << endl;
    } else {
      ofs << "BEGIN_RES"
          << " " << res_name << " " << res_chain << " ";
      ofs.width(3);
    ...
    vector<vector<int>> rigid_fragments; // the vector of all the rigid molecule
                                         // fragments, using atom indexes
    unsigned int best_root_atom = 1;
    ...
    if (flexible) {
      ...
      best_root_atom = FindFragments(
          all_pieces.at(i),
          rigid_fragments); // the return value is the root atom index

      if (residue) {
        best_root_atom = 1;
      } // if this is a residue, uses the first atom as the root
    ...
    } else // if no rotatable bonds are selected, then won't construct a tree,
           // instead get a whole branch directly from the OBMol
    {
    ...
    bool preserve_original_index =
        (pConv->IsOption("p", OBConversion::OUTOPTIONS));
    if (!flexible) {
      preserve_original_index = false;
    } // no need to relabel if we are preserving the original order anyway

    if (!OutputTree(pConv, all_pieces.at(i), ofs, tree, rotatable_bonds, false,
                    preserve_original_index)) {
      stringstream errorMsg;
...
/* ATOMFORMAT "(i5,1x,a4,a1,a3,1x,a1,i4,a1,3x,3f8.3,2f6.2,a2,a2)" */
{
  ...
  if (sbuf.size() > 72) {
    ...
    if ((pdbqt_element == "A ") || (pdbqt_element == " A") ||
        (pdbqt_element == "Z ") || (pdbqt_element == " Z") ||
        (pdbqt_element == "G ") || (pdbqt_element == " G") ||
        (pdbqt_element == "GA") || (pdbqt_element == "J ") ||
        (pdbqt_element == " J") || (pdbqt_element == "Q ") ||
        (pdbqt_element == " Q")) {
      element = "C";
      element += " ";
  ...
  //    cout << mol.NumAtoms() + 1  << " : '" << element << "'" << " " <<
  //    OBElements::GetAtomicNum(element.c_str()) << endl;
  ...
  if ((!scharge.empty()) && "     " != scharge) {
    ...
    if (!sst.fail()) {
      ...
      errorMsg << "WARNING: Problems reading a PDBQT file\n"
               ...
               << "  but OpenBabel found '" << scharge << "' (atom "
               << mol.NumAtoms() + 1 << ").";
      obErrorLog.ThrowError(__FUNCTION__, errorMsg.str(), obWarning);
    }
  } else {
  ...
  OBResidue *res =
      mol.NumResidues() > 0 ? mol.GetResidue(mol.NumResidues() - 1) : nullptr;
  if (res == nullptr || res->GetName() != resname ||
      res->GetNumString() != resnum || res->GetInsertionCode() != icode) {
    vector<OBResidue *>::iterator ri;
    for (res = mol.BeginResidue(ri); res; res = mol.NextResidue(ri))
      if (res->GetName() == resname && res->GetNumString() == resnum &&
