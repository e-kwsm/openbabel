...
public:
  ...
  {
    return "Alchemy format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override { return ""; } // optional

  const char *GetMIMEType() override { return "chemical/x-alchemy"; }
...
bool AlchemyFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    snprintf(buffer, BUFF_SIZE, "%5d %-6s%8.4f %8.4f %8.4f     0.0000", i,
             (char *)str1.c_str(), atom->GetX(), atom->GetY(), atom->GetZ());
    ofs << buffer << endl;
  }
  ...
  for (bond = mol.BeginBond(j); bond; bond = mol.NextBond(j)) {
    ...
    snprintf(buffer, BUFF_SIZE, "%5d  %4d  %4d  %s", bond->GetIdx() + 1,
             bond->GetBeginAtomIdx(), bond->GetEndAtomIdx(), bond_string);
    ofs << buffer << endl;
  }
  return (true);
}
