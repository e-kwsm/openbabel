...
public:
  ...
  GenBankFormat() { // Copied from the Chemical MIME Page at
  ...
  {
    return "GenBank, DDBJ, EMBL Flat File format\n"
           "Read Options e.g. -as\n"
  ...
  const char *GetMIMEType() override { return "chemical/x-genbank"; }
...
bool GenBankFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (!in->eof()) { // We have a new line in 'line'
    if (!line.compare(0, 6, "LOCUS", 6) || !line.compare(0, 2, "ID ", 2)) {
      if (sequence_type ==
          GenBankFormat::UnknownSequence) { // attempt to determine the sequence
                                            // type
    ...
    } else if (!line.compare(0, 6, "DEFINITION", 6) ||
               !line.compare(0, 2, "DE ", 2)) {
      if (sequence_type ==
          GenBankFormat::UnknownSequence) { // attempt to determine the sequence
                                            // type
        if (line.find("RNA") != std::string::npos)
    ...
    } else if (!line.compare(0, 6, "ORIGIN", 6) ||
               !line.compare(0, 2, "SQ ", 2))
      break;
  ...
  bool rv = ReadFASTASequence(
      pmol, sequence_type, in, !pConv->IsOption("b", OBConversion::INOPTIONS),
