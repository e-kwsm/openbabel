...
public:
  ...
  MolproOutputFormat() { OBConversion::RegisterFormat("mpo", this); }

  const char *Description() override // required
  {
    return "Molpro output format\n"
           "Read Options e.g. -as\n"
...
public:
  // Register this format type ID
  MolproInputFormat() { OBConversion::RegisterFormat("mp", this); }

  const char *Description() override // required
  {
    return "Molpro input format\n"
           "No comments yet\n";
  }
...
bool MolproOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "Normal Modes") != nullptr &&
        strstr(buffer, "of") == nullptr) {
      vibration_state = 1;
      continue;
    }
    ...
    if (strstr(buffer, "Wavenumbers [cm-1]") != nullptr &&
        vibration_state < 3) {
    ...
    if (strstr(buffer, "STATE") != nullptr &&
        strstr(buffer, "DIPOLE MOMENT") != nullptr) {
      tokenize(vs, buffer);
      if (vs.size() == 8) {
    ...
    if (strstr(buffer, "   HF-SCF") || strstr(buffer, "   RHF-SCF") ||
        strstr(buffer, "   OPTG(RHF)")) {
      ifs.getline(buffer, BUFF_SIZE);
      const double myHARTREE_TO_KCAL = 627.509;
      mol.SetEnergy(atof(buffer) *
                    myHARTREE_TO_KCAL); // Uses first value on line
    }
  } // while
