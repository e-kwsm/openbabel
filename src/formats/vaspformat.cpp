...
protected:
  ...
  public:
    compare_sort_items(const std::vector<int> &_custom_sort_nums,
                       bool _num_sort)
...
>>>>>>> 73fd347494d1 (style: clang-format)
  ...
  const char *Description() override {
    return "VASP format\n"
           ...
           "Due to limitations in Open Babel's file handling, reading in VASP\n"
           "files can be a bit tricky; the client that is using Open Babel "
           "must\n"
           "use OBConversion::ReadFile() to begin the conversion. This change "
           "is\n"
           "usually trivial. Also, the complete path to the CONTCAR/POSCAR "
           "file\n"
           "must be provided, otherwise the other files needed will not be\n"
           "found.\n\n"
           ...
           "By default, atoms are written out in the order they are present in "
           "the input\n"
           "molecule. To sort by atomic number specify ``-xw``. To specify the "
           "sort\n"
           "order, use the ``-xz`` option.\n\n"
           ...
           " w  Sort atoms by atomic number\n"
           " z <list of atoms>  Specify the order to write out atoms\n"
           "       'atom1 atom2 ...': atom1 first, atom2 second, etc. The "
           "remaining\n"
           "       atoms are written in the default order or (if ``-xw`` is "
           "specified)\n"
           "       in order of atomic number.\n"
           "  4 Write a POSCAR using the VASP 4.x specification.\n"
           "    The default is to use the VASP 5.x specification.\n\n";
...
bool VASPFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  // Move stream to EOF, some apps check ifs position to check for multimolecule
  // files. VASP does not support this, and this parser makes its own streams.
  istream &ifs = *pConv->GetInStream();
  ifs.seekg(0, ios::end);
  ...
  path = pConv->GetInFilename();
  if (path.empty())
    return false; // Should be using ReadFile, not Read!
  size_t found;
  found = path.rfind("/");
  path = path.substr(0, found);
  ...
  if (symbolsInGeometryFile) {
    ...
    for (size_t i = 0; i < vs.size(); ++i) {
      atomTypes.push_back(
          OpenBabel::OBElements::GetAtomicNum(vs.at(i).c_str()));
    }
    // Fetch next line to get stoichiometry
    ifs_cont.getline(buffer, BUFF_SIZE);
    tokenize(vs, buffer);
  ...
  // [C|c|K|k] indicates cartesian coordinates, anything else (including
  // an empty string, buffer[0] == 0) indicates fractional coordinates
  if (buffer[0] == 'C' || buffer[0] == 'c' || buffer[0] == 'K' ||
      buffer[0] == 'k') {
    cartesian = true;
  } else {
    cartesian = false;
  ...
  atomCount = 0;
  for (unsigned int i = 0; i < totalAtoms; i++) {
    // Things get a little nasty here. VASP just prints all the coordinates with
    // no identifying information one after another here. So in the above
    // sections we've parsed out which atom types and how many of each are
    // present in atomTypes and numAtoms, respectively. The counters atomIndex
    // and atomCount have the following roles: atomIndex keeps track of where we
    // are in atomTypes so that we know the atomic species we're inserting.
    // atomCount tracks how many of the current atomTypes.at(atomIndex) species
    // have been inserted, so that when we reach (atomCount >=
    // numAtoms.at(atomIndex) ) we should stop. Phew.
    ifs_cont.getline(buffer, BUFF_SIZE); // atom location
    ...
    // If we made it past that check, we have atomic number =
    // atomTypes.at(atomIndex) Parse the buffer now.
    tokenize(vs, buffer);
    atom = pmol->NewAtom();
    atom->SetAtomicNum(atomTypes.at(atomIndex));
    x = atof((char *)vs[0].c_str());
    ...
    while (ifs_out.getline(buffer, BUFF_SIZE)) {
      ...
      if (strstr(buffer, "BORN EFFECTIVE CHARGES")) {
        ...
        if (dipGrad.empty()) {
          ...
          for (size_t natom = 0; natom < pmol->NumAtoms(); ++natom) {
            const vector3 dxyz = currXyz[natom] - prevXyz[natom];
            vector3::const_iterator iter =
                std::find_if(dxyz.begin(), dxyz.end(),
                             std::bind2nd(std::not_equal_to<double>(), 0.0));
            if (iter != dxyz.end())
  ...
  if (hasVibrations) {
    // compute dDip/dQ
    vector<double> Intensities;
    for (vector<vector<vector3>>::const_iterator lxIter = Lx.begin();
         lxIter != Lx.end(); ++lxIter) {
      vector3 intensity;
      for (size_t natom = 0; natom < dipGrad.size(); ++natom) {
        intensity += dipGrad[natom].transpose() * lxIter->at(natom) /
                     sqrt(pmol->GetAtomById(natom)->GetAtomicMass());
...
bool VASPFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) { atoms_sorted.push_back(&(*atom)); }
  ...
  if (!mol.HasData(OBGenericDataType::UnitCell)) {
    ...
    for (vector<vector3>::const_iterator i = cell.begin(); i != cell.end();
         ++i) {
      snprintf(buffer, BUFF_SIZE, "%20.15f%20.15f%20.15f", i->x(), i->y(),
               i->z());
      ofs << buffer << endl;
    }
  }
  ...
  if (!vasp4Format) {
    for (vector<std::pair<int, int>>::const_iterator it = atomicNums.begin(),
                                                     it_end = atomicNums.end();
         it != it_end; ++it) {
  ...
  for (std::vector<OBAtom *>::const_iterator it = atoms_sorted.begin();
    ...
    // Print coordinates
    snprintf(buffer, BUFF_SIZE, "%26.19f %26.19f %26.19f", (*it)->GetX(),
             (*it)->GetY(), (*it)->GetZ());
    ofs << buffer;
