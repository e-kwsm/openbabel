...
public:
  PubChemJSONFormat() { OBConversion::RegisterFormat("pcjson", this); }

  const char *Description() override {
    return "PubChem JSON\n"
           "The JSON format returned by the PubChem PUG REST service\n\n"

           "The data contained in this format closely resembles PubChem's "
           "internal data structure.\n\n"

           "Read Options, e.g. -as\n"
           " s  disable stereo perception and just read stereo information "
           "from input\n\n"

           "Write Options, e.g. -xm\n"
           " m  minified output, with no line breaks or indents\n"
           " w  use bond styles from input instead of perceived "
           "stereochemistry\n\n";
  };
...
bool PubChemJSONFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (ifs.peek() != EOF) {
    ...
    // Clear ifs flags so it is "good" and any subsequent mols are read, but
    // leave it at EOF position. Therefore, when not at EOF position we know to
    // parse next file and reset currentMolIndex
    ifs.clear();
    currentMolIndex = 0;
  }

  if (!inRoot.IsObject()) {
    obErrorLog.ThrowError("PubChemJSONFormat",
  ...
  if (inRoot.HasMember("PC_Compounds") && inRoot["PC_Compounds"].IsArray()) {
  ...
  } else if (inRoot.HasMember("PC_Substances") &&
             inRoot["PC_Substances"].IsArray()) {
    ...
    obErrorLog.ThrowError(
        "PubChemJSONFormat",
        "JSON file must contain a PC_Compounds or PC_Substances array",
        obError);
  }

  // CID or SID
  if (molRoot.HasMember("id") && molRoot["id"].HasMember("id") &&
      molRoot["id"]["id"].HasMember("cid") &&
      molRoot["id"]["id"]["cid"].IsInt()) {
    // PC_Compound
    ostringstream s;
    ...
    obErrorLog.ThrowError("PubChemJSONFormat",
                          "Did not find CID or SID in PubChem JSON", obWarning);
  }

  // Atom elements
  if (!molRoot.HasMember("atoms") || !molRoot["atoms"].HasMember("aid") ||
      !molRoot["atoms"].HasMember("element")) {
    obErrorLog.ThrowError("PubChemJSONFormat", "Could not read atom data",
                          obError);
  }
  ...
  for (rapidjson::SizeType i = 0; i < eAids.Size(); i++) {
    if (eAids[i].IsInt() && elements[i].IsInt()) {
      ...
      if (atomicNum == 255 || atomicNum == 254 || atomicNum == 253 ||
          atomicNum == 252) {
      ...
      OBAtom *patom = pmol->NewAtom(eAids[i].GetUint());
      if (elementstring == "a" || elementstring == "d" ||
          elementstring == "r" || elementstring == "lp") {
        patom->SetAtomicNum(0);
      } else {
  ...
  // Atom charges
  if (molRoot["atoms"].HasMember("charge") &&
      molRoot["atoms"]["charge"].IsArray()) {
    ...
    for (rapidjson::SizeType i = 0; i < charges.Size(); i++) {
      ...
      if (charge["aid"].IsInt() && charge["value"].IsInt()) {
        ...
        if (patom) {
          ...
          obErrorLog.ThrowError("PubChemJSONFormat", "Invalid atom charge",
                                obWarning);
        }
      } else {
        obErrorLog.ThrowError("PubChemJSONFormat", "Invalid atom charge",
  ...
  // Atom radicals
  if (molRoot["atoms"].HasMember("radical") &&
      molRoot["atoms"]["radical"].IsArray()) {
    ...
    for (rapidjson::SizeType i = 0; i < radicals.Size(); i++) {
      ...
      if (radical["aid"].IsInt() && radical["type"].IsInt()) {
        ...
        if (patom) {
          ...
          obErrorLog.ThrowError("PubChemJSONFormat", "Invalid atom radical",
                                obWarning);
        }
  ...
  // Atom isotopes
  if (molRoot["atoms"].HasMember("isotope") &&
      molRoot["atoms"]["isotope"].IsArray()) {
    ...
    for (rapidjson::SizeType i = 0; i < isotopes.Size(); i++) {
      ...
      if (isotope["aid"].IsInt() && isotope["value"].IsInt()) {
        ...
        if (patom) {
          ...
          obErrorLog.ThrowError("PubChemJSONFormat", "Invalid atom isotope",
                                obWarning);
        }
      } else {
        obErrorLog.ThrowError("PubChemJSONFormat", "Invalid atom isotope",
  ...
  // Bond orders
  if (molRoot.HasMember("bonds") && molRoot["bonds"].HasMember("aid1") &&
      molRoot["bonds"].HasMember("aid2") &&
    ...
    for (rapidjson::SizeType i = 0; i < oAid1s.Size(); i++) {
      ...
      } else if (oAid1s[i].IsInt() && oAid2s[i].IsInt() &&
                 orders[i].IsString()) {
        // Bond order provided as string (old format)
        int order = 0; // Use zero bond order for other bond types (complex,
                       // ionic, dative, unknown)
...
=======
        ...
        if (beginAtom && endAtom) {
          ...
          beginAtom->AddBond(pbond);
          endAtom->AddBond(pbond);
          // Save type string as generic data on bond (useful for non-standard
          // bonds)
          OBPairData *bondType = new OBPairData;
          bondType->SetAttribute("type");
          bondType->SetValue(orderstring);
...
>>>>>>> 73fd347494d1 (style: clang-format)
  ...
  // Atom coordinates
  if (molRoot.HasMember("coords") && molRoot["coords"].IsArray() &&
      molRoot["coords"].Size() > 0 && molRoot["coords"][0].HasMember("aid")) {
    rapidjson::Value &coords = molRoot["coords"][0];
    rapidjson::Value &cAids = coords["aid"];
    if (coords.HasMember("conformers") && coords["conformers"].IsArray() &&
        coords["conformers"].Size() > 0) {
      rapidjson::Value &conf = coords["conformers"][0];
      if (conf.HasMember("x") && conf["x"].IsArray() &&
          conf["x"].Size() >= cAids.Size() && conf.HasMember("y") &&
          conf["y"].IsArray() && conf["y"].Size() >= cAids.Size()) {
        dim = 2;
        if (conf.HasMember("z") && conf["z"].IsArray() &&
            conf["z"].Size() >= cAids.Size()) {
          dim = 3;
        }
        for (rapidjson::SizeType i = 0; i < cAids.Size(); i++) {
          ...
          if (patom) {
            ...
            obErrorLog.ThrowError("PubChemJSONFormat", "Invalid coordinates",
                                  obWarning);
          }
        }
      }
      ...
      // rapidjson::Value &type = coords["type"];
      // Generic data? An array of strings
      // twod, threed, submitted, experimental, computed, standardized,
      // augmented, aligned, compact, units-unknown, units-angstroms,
      // units-nanometers, units-pixel, units-points, units-stdbonds

      // Bond styles and annotations
      if (conf.HasMember("style") && conf["style"].IsObject() &&
          conf["style"].HasMember("aid1") && conf["style"]["aid1"].IsArray() &&
          conf["style"].HasMember("aid2") && conf["style"]["aid2"].IsArray() &&
          conf["style"].HasMember("annotation") &&
          conf["style"]["annotation"].IsArray()) {
        rapidjson::Value &aid1s = conf["style"]["aid1"];
        rapidjson::Value &aid2s = conf["style"]["aid2"];
        rapidjson::Value &styles = conf["style"]["annotation"];
        for (rapidjson::SizeType i = 0; i < aid1s.Size(); i++) {
            ...
            if (beginAtom && endAtom) {
              ...
              if (styles[i].IsInt()) {
                ...
                if (style == 8) {
                  ...
                  // Save non-standard annotations as generic data on bond
                  // (multiple possible)
                  vector<string> val;
                  if (pbond->HasData("style")) {
                    AnnotationData *data =
                        dynamic_cast<AnnotationData *>(pbond->GetData("style"));
                    val = data->GetGenericValue();
                    pbond->DeleteData("style");
                  }
                  AnnotationData *data = new AnnotationData;
              ...
              pbond->Set(pbond->GetIdx(), beginAtom, endAtom,
                         pbond->GetBondOrder(), flags);
            } else {
              obErrorLog.ThrowError("PubChemJSONFormat", "Invalid bond style",
                                    obWarning);
            }
          }
        }
    ...
    if (molRoot.HasMember("stereo") && molRoot["stereo"].IsArray()) {
      for (rapidjson::SizeType i = 0; i < molRoot["stereo"].Size(); i++) {
        ...
        if (stereo.HasMember("tetrahedral")) {
          ...
          OBTetrahedralStereo::Config config;
          config.center = tet["center"].GetUint();
          config.from = (tet["top"].GetInt() == -1) ? OBStereo::ImplicitRef
                                                    : tet["top"].GetInt();
          config.refs.push_back((tet["below"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : tet["below"].GetInt());
          if ((tet["parity"].IsInt() && tet["parity"].GetInt() == 1) ||
              (tet["parity"].IsString() &&
               strcmp(tet["parity"].GetString(), "clockwise") == 0)) {
            config.specified = true;
            config.winding = OBStereo::Clockwise;
            config.refs.push_back((tet["bottom"].GetInt() == -1)
                                      ? OBStereo::ImplicitRef
                                      : tet["bottom"].GetInt());
            config.refs.push_back((tet["above"].GetInt() == -1)
                                      ? OBStereo::ImplicitRef
                                      : tet["above"].GetInt());
          } else if ((tet["parity"].IsInt() && tet["parity"].GetInt() == 2) ||
                     (tet["parity"].IsString() &&
                      strcmp(tet["parity"].GetString(), "counterclockwise") ==
                          0)) {
            config.specified = true;
            config.winding = OBStereo::AntiClockwise;
            config.refs.push_back((tet["above"].GetInt() == -1)
          ...
          config.begin = pl["left"].GetUint();
          config.end = pl["right"].GetUint();
          config.refs.push_back((pl["ltop"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : pl["ltop"].GetInt());
          config.refs.push_back((pl["rtop"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : pl["rtop"].GetInt());
          config.refs.push_back((pl["rbottom"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : pl["rbottom"].GetInt());
          config.refs.push_back((pl["lbottom"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : pl["lbottom"].GetInt());
          if ((pl["parity"].IsInt() &&
               (pl["parity"].GetInt() == 3 || pl["parity"].GetInt() == 255)) ||
              (pl["parity"].IsString() &&
          ...
          OBSquarePlanarStereo::Config config;
          config.center = sq["center"].GetUint();
          config.refs.push_back((sq["lbelow"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : sq["lbelow"].GetInt());
          config.refs.push_back((sq["rbelow"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : sq["rbelow"].GetInt());
          config.refs.push_back((sq["rabove"].GetInt())
                                    ? OBStereo::ImplicitRef
                                    : sq["rabove"].GetInt());
          config.refs.push_back((sq["labove"].GetInt() == -1)
                                    ? OBStereo::ImplicitRef
                                    : sq["labove"].GetInt());
          if ((sq["parity"].IsInt() &&
               (sq["parity"].GetInt() == 4 || sq["parity"].GetInt() == 255)) ||
              (sq["parity"].IsString() &&
          ...
          obErrorLog.ThrowError("PubChemJSONFormat",
                                "Octahedral stereochemistry not implemented",
                                obWarning);
          // aids: center, top, bottom, lbelow, rbelow, labove, rabove
        } else if (stereo.HasMember("bipyramid")) {
          obErrorLog.ThrowError("PubChemJSONFormat",
                                "Bipyramidal stereochemistry not implemented",
                                obWarning);
          // aids: above, below, bottom, center, top, right
        } else if (stereo.HasMember("tshape")) {
          obErrorLog.ThrowError("PubChemJSONFormat",
                                "T shape stereochemistry not implemented",
                                obWarning);
          // aids: center, top, bottom, above
        } else if (stereo.HasMember("pentagonal")) {
          obErrorLog.ThrowError("PubChemJSONFormat",
                                "Pentagonal stereochemistry not implemented",
                                obWarning);
          // aids: center, top, bottom, left, lbelow, rbelow, labove, rabove
        }
      }
...
bool PubChemJSONFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pConv->IsOption("d", OBConversion::GENOPTIONS)) {
    obErrorLog.ThrowError(
        "PubChemJSONFormat",
        "Stereo output may be invalid due to implicit hydrogens", obWarning);
  } else {
    pmol->AddHydrogens();
  }
  ...
  FOR_BONDS_OF_MOL(pbond, pmol) {
    ...
    if (ord == 0) {
      if (pbond->HasData("type")) {
        // Check to see if a "type" string exists
        OBPairData *typeData =
            dynamic_cast<OBPairData *>(pbond->GetData("type"));
        const string &orderstring = typeData->GetValue();
        if (orderstring == "dative") {
          ord = 5;
        } else if (orderstring == "complex") {
    ...
    aid1.PushBack(rapidjson::Value((int)pbond->GetBeginAtom()->GetId()).Move(),
                  al);
    aid2.PushBack(rapidjson::Value((int)pbond->GetEndAtom()->GetId()).Move(),
                  al);
    order.PushBack(rapidjson::Value(ord).Move(), al);
    ...
    if (pConv->IsOption("w", pConv->OUTOPTIONS)) {
      ...
      if (from_cit != from.end() &&
          from_cit->second == pbond->GetEndAtom()->GetId()) {
        swap(aid1, aid2); // Swap start and end atom if necessary
      }
      if (unspec_ctstereo.find(&*pbond) != unspec_ctstereo.end()) {
        annotations.push_back(1);
    ...
    if (pbond->HasData("style")) {
      AnnotationData *data =
          dynamic_cast<AnnotationData *>(pbond->GetData("style"));
      vector<string> styles = data->GetGenericValue();
      for (vector<string>::const_iterator i = styles.begin(); i != styles.end();
           ++i) {
        string stylestring = *i;
        int style = 255;
        if (stylestring == "dashed") {
    ...
    annotations.erase(unique(annotations.begin(), annotations.end()),
                      annotations.end());
    for (vector<int>::const_iterator i = annotations.begin();
         i != annotations.end(); ++i) {
      annAid1.PushBack(
          rapidjson::Value((int)pbond->GetBeginAtom()->GetId()).Move(), al);
      annAid2.PushBack(
          rapidjson::Value((int)pbond->GetEndAtom()->GetId()).Move(), al);
      annotation.PushBack(rapidjson::Value(*i).Move(), al);
    }
  }
  ...
  if (doc.HasMember("atoms")) {
    ...
    rapidjson::Value coord(rapidjson::kObjectType);
    coord.AddMember("type", coordType, al);
    coord.AddMember("aids", rapidjson::Value(doc["atoms"]["aids"], al).Move(),
                    al); // Copy
    coord.AddMember("conformers", conformers, al);
    rapidjson::Value coords(rapidjson::kArrayType);
    coords.PushBack(coord, al);
  ...
  FOR_ATOMS_OF_MOL(patom, pmol) {
    if (facade.HasTetrahedralStereo(patom->GetId())) {
      OBTetrahedralStereo::Config config =
          facade.GetTetrahedralStereo(patom->GetId())->GetConfig();
      rapidjson::Value tet(rapidjson::kObjectType);
      tet.AddMember("type", rapidjson::Value(1).Move(), al); // "tetrahedral"
      tet.AddMember("center", rapidjson::Value((int)config.center).Move(), al);
      tet.AddMember("top", rapidjson::Value((int)config.from).Move(), al);
      ...
      if (config.winding == OBStereo::UnknownWinding || !config.specified) {
        ...
        tet.AddMember("parity", rapidjson::Value(2).Move(),
                      al); // "counterclockwise"
        tet.AddMember("bottom", rapidjson::Value((int)config.refs[2]).Move(),
                      al);
        tet.AddMember("above", rapidjson::Value((int)config.refs[1]).Move(),
                      al);
      }
      rapidjson::Value stereoContainer(rapidjson::kObjectType);
      stereoContainer.AddMember("tetrahedral", tet, al);
    ...
    if (facade.HasSquarePlanarStereo(patom->GetId())) {
      ...
      if (config.specified) {
        if (config.shape == OBStereo::ShapeU) {
          sq.AddMember("parity", rapidjson::Value(1).Move(), al); // "u-shape"
          sq.AddMember("lbelow", rapidjson::Value((int)config.refs[0]).Move(),
                       al);
          sq.AddMember("rbelow", rapidjson::Value((int)config.refs[1]).Move(),
                       al);
          sq.AddMember("rabove", rapidjson::Value((int)config.refs[2]).Move(),
                       al);
          sq.AddMember("labove", rapidjson::Value((int)config.refs[3]).Move(),
                       al);
        } else if (config.shape == OBStereo::ShapeZ) {
          sq.AddMember("parity", rapidjson::Value(2).Move(), al); // "z-shape"
          sq.AddMember("lbelow", rapidjson::Value((int)config.refs[0]).Move(),
          ...
          sq.AddMember("rbelow", rapidjson::Value((int)config.refs[1]).Move(),
                       al);
          sq.AddMember("labove", rapidjson::Value((int)config.refs[2]).Move(),
                       al);
          sq.AddMember("rabove", rapidjson::Value((int)config.refs[3]).Move(),
                       al);
        } else if (config.shape == OBStereo::Shape4) {
          sq.AddMember("parity", rapidjson::Value(3).Move(), al); // "x-shape"
          sq.AddMember("lbelow", rapidjson::Value((int)config.refs[0]).Move(),
                       al);
          sq.AddMember("rabove", rapidjson::Value((int)config.refs[1]).Move(),
                       al);
          sq.AddMember("rbelow", rapidjson::Value((int)config.refs[2]).Move(),
                       al);
          sq.AddMember("labove", rapidjson::Value((int)config.refs[3]).Move(),
                       al);
        }
  ...
  FOR_BONDS_OF_MOL(pbond, pmol) {
    if (facade.HasCisTransStereo(pbond->GetId())) {
      ...
      if (begin && ltop) {
        ...
        ct.AddMember(
            "rbottom",
            rapidjson::Value((int)cts->GetTransRef(config.refs[0])).Move(), al);
        ct.AddMember("lbottom",
                     rapidjson::Value(
                         (int)cts->GetCisRef(cts->GetTransRef(config.refs[0])))
                         .Move(),
                     al);
        ct.AddMember("rtop",
                     rapidjson::Value((int)cts->GetTransRef(cts->GetCisRef(
                                          cts->GetTransRef(config.refs[0]))))
                         .Move(),
                     al);
        if (config.specified) {
  ...
  // Create root object and PC_Compounds array if this is the first molecule in
  // the file
  if (!outRoot.IsObject() || !outRoot.HasMember("PC_Compounds")) {
    outRoot.SetObject();
    outRoot.AddMember("PC_Compounds",
                      rapidjson::Value(rapidjson::kArrayType).Move(), al);
  }
