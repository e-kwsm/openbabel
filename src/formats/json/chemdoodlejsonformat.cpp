...
public:
  ChemDoodleJSONFormat() { OBConversion::RegisterFormat("cdjson", this); }

  const char *Description() override {
    return "ChemDoodle JSON\n"
           ...
           " w  use wedge/hash bonds from input instead of perceived "
           "stereochemistry\n\n";
  };
...
bool ChemDoodleJSONFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!inRoot.IsObject()) {
    obErrorLog.ThrowError("ChemDoodleJSONFormat",
  ...
  if (inRoot.HasMember("m") && inRoot["m"].IsArray()) {
    ...
    obErrorLog.ThrowError("ChemDoodleJSONFormat",
                          "JSON file must contain an 'm' molecules array",
                          obError);
    return false;
  }
  ...
  if (!molRoot.HasMember("a") || !molRoot["a"].IsArray()) {
    obErrorLog.ThrowError("ChemDoodleJSONFormat",
                          "Atoms must be specified in an array", obError);
    return false;
  }
  ...
  for (rapidjson::SizeType i = 0; i < atoms.Size(); i++) {
    ...
    if (atom.HasMember("x") && atom["x"].IsNumber()) {
      ...
      obErrorLog.ThrowError("ChemDoodleJSONFormat", "Atom missing x coordinate",
                            obWarning);
    }
    if (atom.HasMember("y") && atom["y"].IsNumber()) {
      y = atom["y"].GetDouble() / c;
    } else {
      obErrorLog.ThrowError("ChemDoodleJSONFormat", "Atom missing y coordinate",
                            obWarning);
    }
    ...
    if (atom.HasMember("r") && atom["r"].IsInt()) {
      ...
      if (rad == 0) {
        ...
        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Invalid atom radical",
                              obWarning);
      }
  ...
  if (molRoot.HasMember("b") && molRoot["b"].IsArray()) {
    ...
    for (rapidjson::SizeType i = 0; i < bonds.Size(); i++) {
      ...
      if (!bond.IsObject()) {
        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Invalid bond",
                              obWarning);
        continue;
      }
      ...
      if (bond.HasMember("b") && bond["b"].IsInt()) {
        ...
        obErrorLog.ThrowError("ChemDoodleJSONFormat",
                              "Bond missing begin atom (\"b\")", obWarning);
        continue;
      }
      if (bond.HasMember("e") && bond["e"].IsInt()) {
        ...
        obErrorLog.ThrowError("ChemDoodleJSONFormat",
                              "Bond missing end atom (\"e\")", obWarning);
        continue;
      }
      int order = 1;
      if (bond.HasMember("o") && bond["o"].IsNumber()) {
        ...
        if (fabs(bo - 1) < 0.01) {
          ...
          obErrorLog.ThrowError("ChemDoodleJSONFormat",
                                "Bond order 0.5 not supported, using 1",
                                obWarning);
          order = 1;
        } else if (fabs(bo - 1.5) < 0.01) {
          obErrorLog.ThrowError("ChemDoodleJSONFormat",
                                "Bond order 1.5 not supported, using 2",
                                obWarning);
          order = 2;
        } else {
          obErrorLog.ThrowError("ChemDoodleJSONFormat",
                                "Unsupported bond order, skipping bond",
                                obWarning);
          continue;
        }
      } else if (bond.HasMember("o")) {
        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Invalid bond order",
                              obWarning);
        continue;
      }
      ...
      if (bond.HasMember("s") && bond["s"].IsString()) {
        stereo =
            bond["s"]
                .GetString(); // 'none', 'protruding', 'recessed', 'ambiguous'
      }
      if (stereo == "protruding") {
        ...
        obErrorLog.ThrowError("ChemDoodleJSONFormat", "Invalid bond stereo",
                              obWarning);
      }
      OBBond *pbond = pmol->NewBond();
      pbond->Set(pbond->GetIdx(), pmol->GetAtom(begin), pmol->GetAtom(end),
                 order, flag);
...
bool ChemDoodleJSONFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(patom, pmol) {
    ...
    if (patom->GetFormalCharge() != 0 || verbose) {
      atom.AddMember("c", rapidjson::Value(patom->GetFormalCharge()).Move(),
                     al);
    }
    // Mass
  ...
  if (pmol->NumBonds() > 0) {
    ...
    FOR_BONDS_OF_MOL(pbond, pmol) {
      ...
      bond.AddMember("b", rapidjson::Value(pbond->GetBeginAtomIdx() - 1).Move(),
                     al);
      bond.AddMember("e", rapidjson::Value(pbond->GetEndAtomIdx() - 1).Move(),
                     al);
