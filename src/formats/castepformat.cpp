...
public:
  CASTEPFormat() { OBConversion::RegisterFormat("castep", this); }

  const char *Description() override {
    return "CASTEP format\n"
...
bool CASTEPFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (coordsAreFractional) {
    FOR_ATOMS_OF_MOL(atom, pmol) {
      atom->SetVector(cell->FractionalToCartesian(
          cell->WrapFractionalCoordinate(atom->GetVector())));
    }
  }
