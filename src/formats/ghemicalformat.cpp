...
bool GhemicalFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_BONDS_OF_MOL(bond, mol) {
    ...
    if (bond->IsAromatic())
      bond_char = 'C';
    ofs << bond->GetBeginAtomIdx() - 1 << ' ' << bond->GetEndAtomIdx() - 1
        << ' ' << bond_char << '\n';
  }
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ofs << atom->GetIdx() - 1 << ' ' << atom->GetX() / 10.0 << ' '
        << atom->GetY() / 10.0 << ' ' << atom->GetZ() / 10.0 << '\n';
  }
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ofs << atom->GetIdx() - 1 << ' ' << atom->GetPartialCharge() << '\n';
  }
