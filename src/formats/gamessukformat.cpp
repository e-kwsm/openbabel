...
class GAMESSUKFormat {
  /* Base class for GAMESS-UK readers with various utility functions that are
   * used by both input and output readers.
   *
   * The most important is ReadGeometry, which takes a list of strings defining
   * the geometry (in the style of a GAMESS-UK input deck) and creates the OBMol
   * from it. This routine supports both Zmatrix and Cartesian formats, although
   * currently not mixed decks.
   */
...
public:
  ...
  bool ReadLineZmatrix(OBMol &mol, OBAtom *atom, vector<string> &tokens,
                       double factor, int *zmatLineCount);
...
bool GAMESSUKFormat::ReadGeometry(OBMol &mol, vector<string> &geomList) {
   ...
   * to be in the variables map that should have been populated before this is
   * called.
   */
  ...
  string line; // For convenience so we can refer to lines from the iterator as
               // 'line'
  double factor = BOHR_TO_ANGSTROM; // The coordinate conversion factor for
                                    // handling bohr/angstrom issues
  ...
  */
  ...
  for (vector<string>::iterator i = geomList.begin(); i != geomList.end();
       ++i) {
    ...
    if (line.compare(0, 4, "zmat") == 0 || line.compare(0, 4, "inte") == 0) {
      ...
      vic.push_back(
          nullptr); // OBMol indexed from 1 -- potential atom index problem
    } else if (line.compare(0, 4, "coor") == 0 ||
               line.compare(0, 4, "cart") == 0 ||
...
bool GAMESSUKFormat::IsUnits(string text) {
  ...
  if (text.compare(0, 4, "angs") == 0 || text.compare(0, 4, "bohr") == 0 ||
      text.compare(0, 4, "a.u.") == 0 || text.compare(0, 2, "au") == 0) {
    return true;
  } else {
    return false;
  }
...
double GAMESSUKFormat::Rescale(string text) {
  ...
  if (text.compare(0, 4, "angs") == 0) {
    return 1.0;
  } else if (text.compare(0, 4, "bohr") == 0 ||
             text.compare(0, 4, "a.u.") == 0 || text.compare(0, 2, "au") == 0) {
...
int GAMESSUKFormat::LabelToAtomicNumber(string label) {
  ...
  if (Z == 0)
    Z = OBElements::GetAtomicNum(label.substr(0, 1).c_str());
...
bool GAMESSUKFormat::ReadLineCartesian(OBAtom *atom, vector<string> &tokens,
                                       double factor) {
...
bool GAMESSUKFormat::ReadLineZmatrix(OBMol &mol, OBAtom *atom,
                                     vector<string> &tokens, double factor,
                                     int *zmatLineCount) {
  /*
   * Read a line from a GAMESS-UK input defining an atom in Inernal Coordinates.
   * We create a list of OBInternalCoords that match the list of atoms in the
   * molecule that are defined using internal coordinates
   */
  ...
  default:
    ...
    if (!ok) {
      // Can't convert to double so see if it's in the variables
      if (variables.find(tokens[6]) == variables.end())
        return false;
...
bool GAMESSUKFormat::ReadVariables(istream &ifs, double factor,
                                   string stopstr) {
  /*
   ...
   *
   * This is different to ReadGeometry (which takes a vector of strings as
   * input) because currently the variables always need to be read after the
   * geometry, so we need to save the geometry and then read the variables.
   * However this means that we can parse the variables directly into a map and
   * don't need to keep a copy of the specifcation as strings.
   *
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (line.length() == 0 && stopstr.length() == 0)
      break;
    if (stopstr.length() > 0 && line.compare(0, stopstr.length(), stopstr) == 0)
      break;
...
public:
  ...
  GAMESSUKInputFormat() {
    ...
    // OBConversion::RegisterOptionParam("k", NULL, 1,
    // OBConversion::OUTOPTIONS);
    // Command-line keyword file
    // OBConversion::RegisterOptionParam("f", NULL, 1,
    // OBConversion::OUTOPTIONS);
  }

  const char *Description() override // required
  {
    return "GAMESS-UK Input\n";
  }

  const char *SpecificationURL() override {
    return "http://www.cfs.dl.ac.uk";
  } // optional

  const char *GetMIMEType() override { return "chemical/x-gamessuk-input"; }
...

{
   ...
   *
   * For each line we want to check that we haven't hit a change from c to zm or
   * vv
   *
   */
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (ReadMode == ZMATRIX || ReadMode == CARTESIAN) {
      ...
      if (line.compare(0, 4, "vari") == 0 || line.compare(0, 4, "const") == 0) {
        ...
        if (!ReadVariables(ifs, factor, "end"))
          return false;
...
bool GAMESSUKInputFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  ofs << "#" << endl;
  ofs << "# NB: Class I directives (e.g. memory, multiplicity, charge etc) go "
         "here"
      << endl;
  ofs << "#" << endl;
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    snprintf(buffer, BUFF_SIZE, "%15.8f %15.8f %15.8f %3d %3s\n", atom->GetX(),
             atom->GetY(), atom->GetZ(), atom->GetAtomicNum(),
  ...
  ofs << "#" << endl;
  ofs << "# To perform a dft calculation with b3lyp and medium quadrature "
         "uncomment the below"
      << endl;
  ofs << "# dft b3lyp" << endl;
...
public:
  ...
  const char *GetMIMEType() override { return "chemical/x-gamessuk-output"; }
...
bool GAMESSUKOutputFormat::ReadInitialCartesian(OBMol &mol, std::istream &ifs) {
  ...
  #endif*/
  if (!iok)
    cerr << "Error compiling regex in GUK OUTPUT!\n";
...
bool GAMESSUKOutputFormat::ReadOptGeomXyz1(OBMol &mol, std::istream &ifs) {
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    if (strstr(buffer, "atom     znuc       x             y             z") !=
        nullptr)
      break;
  }
...
bool GAMESSUKOutputFormat::ReadNormalModesHessian(OBMol &mol,
                                                  std::istream &ifs) {
  ...
  //   std::cout << "Frequency: " << frequencies.at(i) << " : " <<
  //   intensities.at(i) << std::endl;
...
bool GAMESSUKOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  /*
    Read a GAMESS-UK output file. The reader is currently set up to only return
    one molecule, i.e if the run is some sort of optimisation run, then only the
    optimised geometry is returned.

    Previously we parsed in the z-matrix - and the code to do that is still here
    and should work. However, the zmatrix is not actually used anywhere in
    OpenBabel currently - it's just converted to Cartesians, and this code
    appears to be buggy, so for the time being we just stick to reading
    Cartesians.
  */
  ...
  while (ifs.good() && ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (strstr(buffer, "                              input z-matrix") !=
        nullptr) {
      /* OpenBabel's handling of zmatricies is currently too buggy and the
       * zmatrix read in isn't currently used - it's just converted to
       * cartesians, so we can skip this for the time being
       */
      continue;
      /*
