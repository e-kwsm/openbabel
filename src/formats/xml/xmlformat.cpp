...
public:
  XMLFormat() { OBConversion::RegisterFormat("xml", this); }

  const char *Description() override {
    return "General XML format\n"
           "Calls a particular XML format depending on the XML namespace.\n\n"
           "This is a general XML \"format\" which reads a generic XML file "
           "and infers\n"
           "its format from the namespace as given in a xmlns attribute on an "
           "element.\n"
           "If a namespace is recognised as associated with one of the XML "
           "formats in\n"
           "Open Babel, and the type of the object (e.g. a molecule) is "
           "appropriate to\n"
           "the output format then this is used to input a single object. If "
           "no namespace\n"
           "declaration is found the default format (currently CML) is "
           "used.\n\n"

           "The process is repeated for any subsequent input so that it is "
           "possible to\n"
           "input objects written in several different schemas from the same "
           "document.\n"
           "The file :file:`CMLandPubChem.xml` illustrates this and contains "
           "molecules in\n"
  ...
  unsigned Flags() override { return READXML | NOTWRITABLE; }
  ...
  bool ReadChemObject(OBConversion *pConv) override {
    ...
    if (!pDefault || pDefault == this) {
      obErrorLog.ThrowError(
          "XML Format", "There is no acceptable default XML Format", obError);
      return false;
    }
  ...
  bool ReadMolecule(OBBase *pOb, OBConversion *pConv) override {
    ...
    if (pConv->GetOutFormat() != nullptr &&
        pConv->GetOutFormat()->GetType() == pDefault->GetType()) {
      XMLConversion *pxmlConv = XMLConversion::GetDerived(pConv);
      pxmlConv->LookForNamespace();
      ...
      // the same as that handled by the default format.
      obErrorLog.ThrowError(
          "XML Format", "Need to specify the input XML format more precisely",
          obError);
      return false;
    }
  };
