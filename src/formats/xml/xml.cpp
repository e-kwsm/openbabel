...
XMLConversion::XMLConversion(OBConversion *pConv)
    : OBConversion(*pConv), _requestedpos(0), _lastpos(0), _reader(nullptr),
      _writer(nullptr), _LookingForNamespace(false), _SkipNextRead(false) {
  _pConv = pConv;
  pConv->SetAuxConv(
      this); // marks original OBConversion object as having been extended
  SetAuxConv(this); // marks this new object as extended (for use with
                    // OBConversion pointer)
}
...
bool XMLConversion::SetupReader() {
  ...
  //(Getting the requested object is handled in ReadXML(), when the format is
  //known.)
...
bool XMLConversion::SetupWriter() {
  ...
  _buf = xmlOutputBufferCreateIO(WriteStream, // xmlOutputWriteCallback
...
void XMLConversion::RegisterXMLFormat(XMLBaseFormat *pFormat, bool IsDefault,
                                      const char *uri) {
...
/// Returns the extended form of the OBConversion object with an xml reader or
/// writer,
...
XMLConversion *XMLConversion::GetDerived(OBConversion *pConv, bool ForReading) {
  ...
  if (!pConv->GetAuxConv())
    ...
    *pConv->GetAuxConv() = *pConv; // ensure they have the same OBConversion
                                   // data
  ...
  if (ForReading) {
    ...
    if (pos < pxmlConv->_lastpos || pxmlConv->_lastpos < 0) {
      ...
      xmlFreeTextReader(
          pxmlConv->_reader); // need a new reader to read files with <?xml?>
    ...
    pxmlConv->SetLast(
        pConv->IsLast()); // Copy IsLast flag to the extended object
  }
  return pxmlConv;
}
...
bool XMLConversion::ReadXML(XMLBaseFormat *pFormat, OBBase *pOb) {
  if (_requestedpos) {
    // The initial stream position was not at the start, probably because of
    // fastsearch Read and discard the first object to synchronize the reader,
  ...
  while (!GetInStream()->bad() && !GetInStream()->eof() &&
         (_SkipNextRead ||
          (result = xmlTextReaderRead(_reader)) == 1)) // read may not be called
  {
    _SkipNextRead = false;
  ...
  if (result == -1) {
    ...
    if (perr && perr->level != XML_ERR_NONE) {
      obErrorLog.ThrowError("XML Parser " + GetInFilename(), perr->message,
                            obError);
    }
    xmlResetError(perr);
...
/////////////////////////////////////////////////////////
/// Read and discard XML text up to the next occurrence of the tag
/// e.g."/molecule>" This is left as the current node. Returns 1 on success, 0
/// if not found, -1 if failed.
...
////////////////////////////////////////////////////////
/// Static callback function for xmlReaderForIO(). Reads up to the next '>', or
/// len chars.
...
When ReadChemObject() of the base class XMLFormat is the one called (e.g for
files with extension .xml), the initial current format is the default format(see
below) providing it handles the same chemical object type as the output format.
The ReadChemObject() of the default format is called and processing is as if the
default format was called, except that the first explicit namespace declaration
in the xml file that appears in the map can switch the handling to its
associated format.
