...
public:
  ...
  const char *Description() override {
    return "PubChem format\n"
           "An XML format containing information on PubChem entries.\n"
           "`PubChem <http://pubchem.ncbi.nlm.nih.gov/>`_ is a "
           "freely-available\n"
           "database of chemical compounds and their properties.\n\n"
           "OpenBabel only extracts the chemical structure information, and "
           "the\n"
           "potentially large amount of other information is currently "
           "ignored.\n"
           "The format seems to handle multiple conformers, but only one is "
           "read\n"
           "(this needs testing).\n\n";
  }
  ...
  const char *GetMIMEType() override { return "chemical/x-ncbi-asn1-xml"; };

  unsigned int Flags() override { return (READXML | NOTWRITABLE); };
  ...
  if (name == "PC-Atoms") {
    ...
    for (i = 0; i < CoordIndx.size(); ++i) {
      ...
      pAtom->SetVector(Coordx[i], Coordy[i], Coordz[i]);
             ...
             "PC-Compound") // this is the end of the molecule we are extracting
  {
    _pmol->EndModify();
