...
unique_ptr<OBReaction> convertToReaction(OBMol *mol) {
  ...
  if (rf.IsValid()) {
    ...
    for (unsigned role = OBReactionRole::REACTANT;
         role <= OBReactionRole::PRODUCT; ++role) {
      unsigned numComponents =
          rf.NumComponents(static_cast<OBReactionRole>(role));
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Cannot convert to OBReaction",
                          obError);
  }
...
public:
  CMLReactFormat() {
    ...
    XMLConversion::RegisterXMLFormat(
        this, false, "http://www.xml-cml.org/schema/cml2/react");
  }
  ...
  const char *Description() override {
    return "CML Reaction format\n"
           ...
           "The implementation of this format which reads and writes to and "
           "from\n"
           "OBReaction objects is fairly minimal at present. (Currently the "
           "only\n"
           "other reaction format in OpenBabel is RXN.) During reading, only "
           "the\n"
           "elements <reaction>, <reactant>, <product> and <molecule>  are "
           "acted\n"
           "upon (the last through CML). The molecules can be collected "
           "together\n"
           "in a list at the start of the file and referenced in the reactant "
           "and\n"
           "product via e.g. <molecule ref=\"mol1\">.\n\n"

           "On writing, the list format can be specified with the ``-xl`` "
           "option. The\n"
           "list containers are <moleculeList> and <reactionList> and the "
           "overall\n"
           "wrapper is <mechanism>. These are non-standard CMLReact element "
           "names\n"
           "and would have to be changed (in the code) to <list>,<list> and "
           "<cml>\n"
  ...
  unsigned Flags() override { return 0; }
...
bool CMLReactFormat::ReadChemObject(OBConversion *pConv) {
  ...
  // Do transformation and return reaction, if it has either reactants or
  // products
  if (ret &&
      (pReact->NumReactants() != 0 ||
       pReact->NumProducts() != 0)) // Do transformation and return molecule
...
bool CMLReactFormat::DoElement(const string &name) {
  ...
  if (name == "reaction") {
    ...
    if (rt == "arrhenius")
      enumrt = OBRateData::ARRHENIUS;
    else if (rt == "lindermann")
      enumrt = OBRateData::LINDERMANN;
    else if (rt == "troe")
      enumrt = OBRateData::TROE;
    else if (rt == "sri")
      enumrt = OBRateData::SRI;
    else if (rt == "threeBody")
      enumrt = OBRateData::THREEBODY;
    else
      obErrorLog.ThrowError(__FUNCTION__, rt + " is not a known reactionType",
                            obWarning);
...
bool CMLReactFormat::WriteChemObject(OBConversion *pConv) {
  ...
  if (pReact == nullptr) {
    ...
    // upgraded from that in molecules in OBReaction object (and so in Chemkin
    // or CMLReact input file) by including a separate file with updates.
      ...
      if (pConv->IsLast()) {
        ...
        if (pCMLFormat == nullptr) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "CML format for molecules is needed by "
                                "CMLReactformat and is not available\n",
                                obError);
          return false;
        }
        unsigned int n = 0;
        ...
        for (mapitr = OMols.begin(); mapitr != OMols.end() && ret; ++mapitr) {
          ...
          pConv->SetLast(n == OMols.size()); // setLast manually because we are
                                             // not using Convert()
    ...
    // If sent text as an OBText object, output the text up to the insertion
    // point and the rest of the text in _text. The content of _text is output
    // at the end.
    {
      ...
      *_pOut
          << frontText; // Output(to orig outstream) text up to insertion point
      _text = ptext->GetText(
          pos); // Save text after insertion point to be output at the end
      ...
      pConv->SetOutputIndex(pConv->GetOutputIndex() -
                            1); // not an output we want to count
      return true;
    }
  }
...
bool CMLReactFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  bool list = _pxmlConv->IsOption("l") ==
              nullptr; // Output with molecules in a separate list
  ...
  _pxmlConv->AddOption("MolsNotStandalone",
                       OBConversion::OUTOPTIONS); // inform CMLFormat
  ...
  // For first reaction
  if (pConv->GetOutputIndex() ==
      1) // OBConversion::Convert() is still using original pConv
  {
    _pOut = pConv->GetOutStream(); // Save the original output stream
    if (list) {
      // Use a temporary stringstream for list format so that moleculeList can
      // be first
      ssout.str("");
      ...
      // Do also for original object; GetDerived() copies the old to the new on
      // subsequent reactions
      pConv->SetOutStream(&ssout);
    }
    if (!_pxmlConv->IsOption("x") &&
        !_pxmlConv->IsOption("ReactionsNotStandalone")) {
    ...
    if (list) {
      if (!pConv->IsOption("ReactionsNotStandalone")) {
        ...
        if (altprefix)
          xmlTextWriterWriteAttributeNS(writer(), BAD_CAST "xmlns", altprefix,
                                        nullptr, alturi);
      }
...
string CMLReactFormat::AddMolToList(std::shared_ptr<OBMol> spmol,
                                    MolMap &mmap) {
  ...
  if (id.empty()) {
    ...
    if (!isalpha(id[0])) // since ids have to start with a letter, add "id" to
                         // those that don't...
      id = "id" + id;
    ...
    if (mapitr == mmap.end())
      ...
      // already in map.
      // Get a molecule with the best bits of both old and new molecules and
      // immediately make a shared_ ptr
      std::shared_ptr<OBMol> spnew(OBMoleculeFormat::MakeCombinedMolecule(
          mapitr->second.get(), spmol.get()));
      if (spnew) {
        spmol.swap(spnew);
        mapitr->second = spmol; // replace with new molecule
        // The OBMol originally in map will be deleted when local shared_ptr
        // goes out of scope.
      }
    }
  }
...
bool CMLReactFormat::WriteRateData(OBReaction *pReact, xmlChar *altprefix) {
  ...
  xmlTextWriterWriteFormatAttribute(writer(), C_REACTIONTYPE, "%s",
                                    rtype.c_str());
  if (pReact->IsReversible())
    xmlTextWriterWriteFormatAttribute(writer(), C_REVERSIBLE, "%s", "true");
  ...
  case OBRateData::TROE:
    ...
    xmlTextWriterWriteFormatString(writer(), "%g %g %g %g",
                                   pRD->GetTroeParam(0), pRD->GetTroeParam(1),
                                   pRD->GetTroeParam(2), pRD->GetTroeParam(3));
    xmlTextWriterEndElement(writer());
    // fallthrough
  case OBRateData::LINDERMANN:
    xmlTextWriterStartElementNS(writer(), altprefix, C_LOA, nullptr);
    xmlTextWriterWriteFormatString(writer(), "%.3e",
                                   pRD->GetLoRate(OBRateData::A));
...
void CMLReactFormat::WriteMetadataList(OBReaction &react) {
  ...
  if (!comment.empty()) {
    ...
    xmlTextWriterWriteAttributeNS(writer(), BAD_CAST "xmlns", BAD_CAST "dc",
                                  nullptr,
