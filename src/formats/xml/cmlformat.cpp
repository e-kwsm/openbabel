...
private:
  ...
  const char *CML2NamespaceURI() const {
...
public:
  ...
  CMLFormat() {
    ...
    XMLConversion::RegisterXMLFormat(this,
                                     true); // this is the default XLMformat
    XMLConversion::RegisterXMLFormat(this, false,
                                     CML1NamespaceURI()); // CML1 also
    XMLConversion::RegisterXMLFormat(this, false,
                                     CML2NamespaceURI()); // Old CML2 also
  }
  ...
  const char *Description() override {
    return "Chemical Markup Language\n"
           ...
           "This format writes and reads CML XML files. To write CML1 format "
           "rather than\n"
           "the default CML2, use the ``-x1`` option. To write the array form "
           "use ``-xa``\n"
           "and to specify all hydrogens using the hydrogenCount attribute on "
           "atoms use\n"
           "``-xh``.\n\n"

           "Crystal structures are written using the <crystal>, <xfract> "
           "(,...etc.)\n"
           "elements if the OBMol has a OBGenericDataType::UnitCell data.\n\n"

           "All these forms are handled transparently during reading. Only a "
           "subset of\n"
           "CML elements and attributes are recognised, but these include most "
           "of those\n"
           ...
           "- Elements:\n\n"
           "  - molecule, atomArray, atom, bondArray, bond, atomParity, "
           "bondStereo\n"
           "  - name, formula, crystal, scalar (contains crystal data)\n"
           "  - string, stringArray, integer, integerArray, float floatArray, "
           "builtin\n\n"

           "- Attributes:\n\n"
           "  - On <molecule>: id, title, ref(in CMLReact)\n"
           "  - On <atom>: id, atomId, atomID, elementType, x2, y2, x3, y3, "
           "z3, xy2, xyz3,\n"
           "    xFract, yFract, zFract, xyzFract, hydrogenCount, formalCharge, "
           "isotope,\n"
           "    isotopeNumber, spinMultiplicity, radical(from Marvin),\n"
           "    atomRefs4 (for atomParity)\n"
           "  - On <bond>: atomRefs2, order, CML1: atomRef, atomRef1, "
           "atomRef2\n\n"

           "Atom classes are also read and written. This is done using a "
           "specially\n"
           "formed atom id. When reading, if the atom id is of the form aN_M "
           "(where\n"
           "N and M are positive integers), then M is interpreted as the atom "
           "class.\n"
           "Such atom ids are automatically generated when writing an atom "
           "with an\n"
           "atom class.\n\n"
           ...
           "and attributes are ignored and there are rather few error "
           "messages\n"
           "when any required structures are not found. This laxity allows, "
           "for\n"
           "instance, the reactant and product molecules to be picked out of a "
           "CML\n"
           "React file using CML. Each format has an element which is regarded "
           "as\n"
           "defining the object that OpenBabel will convert. For CML this is\n"
           "<molecule>. Files can have multiple objects and these can be "
           "treated\n"
           "the same as with other multiple object formats like SMILES and "
           "MDL\n"
           "Molfile. So conversion can start at the nth object using the "
           "``-fn`` option\n"
           "and finish before the end using the ``-ln`` option. Multiple "
           "object XML files\n"
           "also can be indexed and searched using FastSearch, although this "
           "has\n"
           "not yet been extensively tested.\n\n";
  };

  const char *SpecificationURL() override { return "http://www.xml-cml.org/"; }

  const char *GetMIMEType() override { return "chemical/x-cml"; };

  unsigned int Flags() override { return READXML | ZEROATOMSOK; };
...
private:
  ...
  cmlArray BondArray;
  vector<int>
      HCounts; // for each atom, either -1 or the value of the hydrogenCount
...
bool CMLFormat::DoElement(const string &name) {
  ...
  } else if (name == "molecule" || name == "jobstep") // hack for molpro
  {
    ...
    // free((void*)ptitle);//libxml2 doc says "The string must be deallocated by
    // the caller."
    ...
    // Save in molWideData:
    // the content,  the atomRefs4 attribute, and (for atomParity only) the
    // centralAtom
    ...
    const xmlChar *pformula =
        xmlTextReaderGetAttribute(reader(), BAD_CAST "concise");
    if (pformula) {
    ...
    if (CrystalScalarsNeeded) {
      ...
      // Reads OBPairData(like SDF properties). Name is in scalar title or id
      // attribute
      const xmlChar *pattr =
          xmlTextReaderGetAttribute(reader(), BAD_CAST "title");
      if (!pattr)
    ...
    if (titleonproperty.find("vibFreqs") != string::npos) {
      ...
      rd->SetData(OBRotationData::UNKNOWN, rotConsts,
                  1); // rotor type and symmetry number unknown
    ...
    const xmlChar *pname =
        xmlTextReaderGetAttribute(reader(), BAD_CAST "spaceGroup");
    if (pname) {
    ...
    const char *pattr =
        (const char *)xmlTextReaderGetAttribute(reader(), BAD_CAST "dictRef");
    if (pattr && !strcmp(pattr, "Thermo_OldNasa"))
      ReadNasaThermo();
    else {
      if (!pattr) // no dictRef; look for title on scalar
        pattr =
            (const char *)xmlTextReaderGetAttribute(reader(), BAD_CAST "title");
      if (pattr)
  ...
  } else if (name == "stringArray" || name == "floatArray" ||
             name == "integerArray") {
    string name = _pxmlConv->GetAttribute("builtin");
  ...
  // The end element event would not be called for <element/>, so call it
  // explicitly.
...
bool CMLFormat::EndElement(const string &name) {
  ...
  else if (name == "molecule" || name == "jobstep") // hack for molpro
  {
    ...
    _pmol->EndModify();
    return (--_embedlevel >=
            0); // false to stop parsing if no further embedded mols
...
static const char *FindStartOfAtomClass(const char *atomid) {
  // Try to find a match to 'a' followed by a number followed by _ followed by
  // at least one digit
  if (atomid[0] != 'a')
...
bool CMLFormat::DoAtoms() {
  ...
  for (AtomIter = AtomArray.begin(); AtomIter != AtomArray.end(); ++AtomIter) {
    ...
    int nhvy = nAtoms;
    int hcount =
        -1; // default value which may be overridden by hydrogenCount below

    double x = 0, y = 0, z = 0;
    bool using3 = false, using2 = false, usingFract = false;

    vector<pair<string, string>>::iterator AttributeIter;
    for (AttributeIter = AtomIter->begin(); AttributeIter != AtomIter->end();
         ++AttributeIter) {
      string &attrname = AttributeIter->first;
      string &value = AttributeIter->second;

      if (attrname == "id" || attrname == "atomId" ||
          attrname == "atomID") // which one correct?
      {
        Trim(value);
        if (AtomMap.count(value) > 0)
          obErrorLog.ThrowError(
              GetMolID(), "The atom id " + value + " is not unique", obWarning);
      ...
      else if ((attrname == "x3" || attrname == "y3" || attrname == "z3" ||
                attrname == "xyz3") &&
               !use2d) {
        using3 = true;
        usingFract = false;
      } else if ((attrname == "x2" || attrname == "y2" || attrname == "z2" ||
                  attrname == "xy2") &&
                 !using3) {
        using2 = true;
        usingFract = false;
      } else if (pUnitCell && !using3 && !using2 &&
                 (attrname == "xFract" || attrname == "yFract" ||
                  attrname == "zFract"))
        usingFract = true;

      if ((using3 && attrname == "x3") || (using2 && attrname == "x2") ||
          (usingFract && attrname == "xFract")) {
        x = strtod(value.c_str(), nullptr);
      } else if ((using3 && attrname == "y3") || (using2 && attrname == "y2") ||
                 (usingFract && attrname == "yFract")) {
        y = strtod(value.c_str(), nullptr);
      } else if ((using3 && attrname == "z3") || (using2 && attrname == "z2") ||
      ...
      if (attrname == "hydrogenCount") {
        // Actually adding H atoms to the structure is deferred until the
        // explicit structure is complete, because hydrogenCount may include
        // explicit H, bug#3014855
      ...
      /*else if(attrname=="atomRefs4")//from atomParity element (but there is no
        such thing!)
        {
          vector<string> ids;
          tokenize(ids, value);
...
bool CMLFormat::DoBonds() {
  ...
  for (BondIter = BondArray.begin(); BondIter != BondArray.end(); ++BondIter) {
    ...
    for (AttributeIter = BondIter->begin(); AttributeIter != BondIter->end();
         ++AttributeIter) {
      string attrname = AttributeIter->first;
      string value = AttributeIter->second;
      Trim(value);
      ...
      if (attrname.compare(0, 7, "atomRef") == 0) // generic
      {
        ...
        if (!HaveWarned && (attrname == "atomRefs1" ||
          ...
          obErrorLog.ThrowError(
              GetMolID(),
              attrname +
                  " is not legal CML in this context, "
                  "but OpenBabel will attempt to understand what was meant.",
              obWarning);
          HaveWarned = true;
        }
        ...
        if (indx1 == 0) {
          if (pos != string::npos) {
            ...
            // C4239                        indx2 =
            // AtomMap[Trim(value.substr(pos+1))];
...
bool CMLFormat::DoHCounts() {
  // Add extra H atoms so that each atom has the value of its attribute
  // hydrogenCount
  FOR_ATOMS_OF_MOL(atom, _pmol) {
    ...
    if (explH > hcount) {
      ...
      stringstream ss;
      ss << "In atom " << it->first
         << " the number of explicit hydrogens exceeds the hydrogenCount "
            "attribute.";
...
bool CMLFormat::DoMolWideData() {
  ...
  vector<pair<string, string>>::iterator AttributeIter;
  for (AttributeIter = molWideData.begin(); AttributeIter != molWideData.end();
       ++AttributeIter) {
    ...
    if (name == "atomParity" || name == "bondStereo") {
      ...
      } else // bondStereo
      {
        ...
        OBCisTransStereo::Config ct_cfg =
            OBCisTransStereo::Config(begin, end, refs, OBStereo::ShapeU);
...
bool CMLFormat::TransferElement(cmlArray &arr) {
  // Reads the attributes of the current node, e.g. <atom id="a1"
  // elementType="C"/> pushes each of them as a pairs into each of the members
  // of the array
...
void CMLFormat::WriteMetadataList(OBMol &mol) {
  ...
  if (mol.HasData(OBGenericDataType::CommentData)) {
    ...
    xmlTextWriterWriteAttribute(writer(), C_CONTENT,
                                BAD_CAST cd->GetData().c_str());
...
string CMLFormat::getTimestr() {
  ...
  time_res = strftime(timestr, TIME_STR_SIZE, "%a %b %d %H:%M:%S %Z %Y",
...
bool CMLFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  */
  ...
  if (!_pxmlConv->IsOption("MolsNotStandalone") &&
      _pxmlConv->GetOutputIndex() == 1) {
    ...
    // If more than one molecule to be output, write <cml> at start and </cml>
    // at end.
    if (!_pxmlConv->IsLast()) {
  ...
  if (pmol == nullptr) {
    ...
    if (!pCMLRFormat) {
      obErrorLog.ThrowError(__FUNCTION__, "Cannot find CMLReact format",
                            obError);
      return false;
    }
  ...
  if (*id) {
    ...
    if (!isalpha(name[0])) // since ids have to start with a letter, add "id" to
                           // those that don't...
      name = "id" + name;
  ...
  if (TotalCharge != 0)
    xmlTextWriterWriteFormatAttribute(writer(), C_FORMALCHARGE, "%d",
                                      TotalCharge);

  int TotalSpin = mol.GetTotalSpinMultiplicity();
  if (TotalSpin != 1)
    xmlTextWriterWriteFormatAttribute(writer(), C_SPINMULTIPLICITY, "%d",
                                      TotalSpin);

  if (_pxmlConv->IsOption("m") &&
      _pxmlConv->GetOutputIndex() == 1) // only on first molecule
  ...
  std::map<unsigned int, OBTetrahedralStereo::Config>::const_iterator
      tetStereo_cit;
  if (mol.GetDimension() != 3) {
    ...
    for (std::vector<OBGenericData *>::iterator data = vdata.begin();
      ...
      if (((OBStereoBase *)*data)->GetType() == OBStereo::Tetrahedral) {
        ...
        // Always get the clockwise version (it's the default anyway) as this
        // has a positive signed volume (i.e. CML atomParity of 1)
  ...
  if (mol.NumAtoms() > 0) {
    // if molecule has no bonds and atoms doesn't have coordinates, just output
    // formula
    if (numbonds == 0 && UseFormulaWithNoBonds && !mol.Has2D())
      ...
      MakeAtomIds(
          mol, atomIds); // Pre-construct to take into account atom class data
      ...
      for (patom = mol.BeginAtom(i); patom; patom = mol.NextAtom(i)) {
        ...
        int hcount = patom->GetImplicitHCount() +
                     patom->ExplicitHydrogenCount(); // includes H isotopes
        ...
        if (arrayform) {
          ...
          xmlTextWriterWriteFormatAttribute(writer(), C_ID, "%s",
                                            atomIds[patom->GetIdx()].c_str());

          if (!cml1) {
            xmlTextWriterWriteFormatAttribute(writer(), C_ELEMENTTYPE, "%s",
                                              el.c_str());
            if (isotope)
              xmlTextWriterWriteFormatAttribute(writer(), C_ISOTOPE, "%d",
                                                isotope);
            ...
            if (spin)
              xmlTextWriterWriteFormatAttribute(writer(), C_SPINMULTIPLICITY,
                                                "%d", spin);

            xmlTextWriterWriteFormatAttribute(writer(), C_HYDROGENCOUNT, "%d",
                                              hcount);
            ...
            if ((tetStereo_cit = tetStereos.find(patom->GetId())) !=
                tetStereos.end()) {
              ...
              // "if there are only 3 ligands, the current atom should be
              // included
              //  in the 4 atomRefs.".
              if (cfg.from == OBStereo::ImplicitRef) // e.g. for [S@@](Cl)(Br)I
                atomrefs.push_back(
                    atomIds[mol.GetAtomById(cfg.center)
                                ->GetIdx()]); // Add the central atom again
              else
                atomrefs.push_back(
                    atomIds[mol.GetAtomById(cfg.from)->GetIdx()]);

              for (OBStereo::RefIter ref = refs.begin(); ref != refs.end();
                   ++ref) {
                if ((OBStereo::Ref)*ref ==
                    OBStereo::ImplicitRef) // e.g. for Cl[S@@](Br)I
                  atomrefs.push_back(
                      atomIds[mol.GetAtomById(cfg.center)
                                  ->GetIdx()]); // Add the central atom again
                else
              ...
              xmlTextWriterStartElementNS(writer(), prefix, C_ATOMPARITY,
                                          nullptr);
              xmlTextWriterWriteFormatAttribute(
                  writer(), C_ATOMREFS4, "%s %s %s %s", atomrefs[0].c_str(),
                  atomrefs[1].c_str(), atomrefs[2].c_str(),
                  atomrefs[3].c_str());
              // Set the atomParity - this is always 1 as the atomRefs are
              // arranged to make this so
            ...
            xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s",
                                              "elementType");
            ...
            if (dim == 2 || dim == 3) {
              ...
              xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s%d",
                                                "x", dim);
      ...
      if (arrayform) {
        if (!cml1) {
          xmlTextWriterWriteFormatAttribute(writer(), C_ATOMID, "%s",
                                            id.str().c_str());
          xmlTextWriterWriteFormatAttribute(writer(), C_ELEMENTTYPE, "%s",
                                            eltyp.str().c_str());

          if (anyIsotope)
            xmlTextWriterWriteFormatAttribute(writer(), C_ISOTOPE, "%s",
                                              iso.str().c_str());

          if (anyChg)
            xmlTextWriterWriteFormatAttribute(writer(), C_FORMALCHARGE, "%s",
                                              chg.str().c_str());
          ...
          xmlTextWriterWriteFormatAttribute(writer(), C_HYDROGENCOUNT, "%s",
                                            hct.str().c_str());

          if (dim == 2) {
            xmlTextWriterWriteFormatAttribute(writer(), C_X2, "%s",
                                              x.str().c_str());
            xmlTextWriterWriteFormatAttribute(writer(), C_Y2, "%s",
                                              y.str().c_str());
          }
          if (dim == 3) {
            xmlTextWriterWriteFormatAttribute(writer(), C_X3orFRACT, "%s",
                                              x.str().c_str());
            xmlTextWriterWriteFormatAttribute(writer(), C_Y3orFRACT, "%s",
                                              y.str().c_str());
            xmlTextWriterWriteFormatAttribute(writer(), C_Z3orFRACT, "%s",
                                              z.str().c_str());
          }
        } else {
          // CML1
          ...
          if (anyChg) {
            xmlTextWriterStartElementNS(writer(), prefix, C_INTEGERARRAY,
                                        nullptr);
            xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s",
                                              "formalCharge");
          ...
          if (dim == 2 || dim == 3) {
            xmlTextWriterStartElementNS(writer(), prefix, C_FLOATARRAY,
                                        nullptr);
            xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s%d", "x",
                                              dim);
            ...
            xmlTextWriterStartElementNS(writer(), prefix, C_FLOATARRAY,
                                        nullptr);
            xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s%d", "y",
                                              dim);
          ...
          if (dim == 3) {
            xmlTextWriterStartElementNS(writer(), prefix, C_FLOATARRAY,
                                        nullptr);
            xmlTextWriterWriteFormatAttribute(writer(), C_BUILTIN, "%s%d", "z",
                                              dim);
    ...
    for (std::vector<OBGenericData *>::iterator data = vdata.begin();
      ...
      if (((OBStereoBase *)*data)->GetType() == OBStereo::CisTrans) {
        ...
        if (ct->GetConfig().specified) {
          unsigned int dblbond =
              mol.GetBond(mol.GetAtomById(ct->GetConfig().begin),
                          mol.GetAtomById(ct->GetConfig().end))
                  ->GetIdx();
  ...
  if (mol.NumBonds() > 0) {
    ...
    for (pbond = mol.BeginBond(ib); pbond; pbond = mol.NextBond(ib)) {
      ...
      if (!arrayform) {
        ...
        if (!cml1) {
          ...
          xmlTextWriterWriteFormatAttribute(writer(), C_ORDER, "%s",
                                            ord.str().c_str());
          ...
          if ((ctStereo_cit = ctStereos.find(pbond->GetIdx())) !=
              ctStereos.end()) {
            ...
            beginref = (ct_cfg.refs[0] == OBStereo::ImplicitRef)
                           ? ct_cfg.refs[1]
                           : ct_cfg.refs[0];
            endref = (ct_cfg.refs[2] == OBStereo::ImplicitRef) ? ct_cfg.refs[3]
                                                               : ct_cfg.refs[2];
            ...
            // Create the XML tags
            xmlTextWriterStartElementNS(writer(), prefix, C_BONDSTEREO,
                                        nullptr);
            xmlTextWriterWriteFormatAttribute(
                writer(), C_ATOMREFS4, "%s %s %s %s", atomrefs[0].c_str(),
                atomrefs[1].c_str(), atomrefs[2].c_str(), atomrefs[3].c_str());
    ...
    if (arrayform) {
      if (!cml1) {
        xmlTextWriterWriteFormatAttribute(writer(), C_ATOMREF1, "%s",
                                          ref1.c_str());
        xmlTextWriterWriteFormatAttribute(writer(), C_ATOMREF2, "%s",
                                          ref2.c_str());
        xmlTextWriterWriteFormatAttribute(writer(), C_ORDER, "%s",
                                          ord.str().c_str());
      } else {
...
void CMLFormat::MakeAtomIds(OBMol &mol, vector<string> &atomIDs) {
  /* If there is no atom class data for the atom, the id is a followed by the
     atom index. If there is atom class data, an underscore is appended followed
     by the atom class.
   */
...
void CMLFormat::WriteFormula(OBMol mol) {
  ...
  xmlTextWriterWriteFormatAttribute(writer(), C_CONCISE, "%s",
                                    mol.GetSpacedFormula().c_str());
...
void CMLFormat::WriteBondStereo(OBBond *pbond, vector<string> &atomIDs) {
  ...
  if (ch)
    ...
    //         ud1 = b1->IsDown() ? -1 : 1;
    //         // Conjugated double bonds have to be treated differently, see
    //         comments
    //         // in OBMol2Smi::GetCisTransBondSymbol(). Reverse symbol for
    //         other than first double bond.
    ...
    xmlTextWriterStartElementNS(writer(), prefix, C_BONDSTEREO, nullptr);
    xmlTextWriterWriteFormatAttribute(
        writer(), C_ATOMREFS4, "%s %s %s %s",
        //                        "a%d a%d a%d a%d", idx1, patomA->GetIdx(),
        //                        patomB->GetIdx(), idx2);
...
void CMLFormat::WriteCrystal(OBMol &mol) {
  ...
  if (group) {
    ...
    xmlTextWriterWriteAttribute(writer(), C_SPACEGROUP,
                                (const xmlChar *)group->GetHallName().c_str());
    transform3dIterator ti;
    const transform3d *t = group->BeginTransform(ti);
    string s;
    while (t) {
      s = t->DescribeAsValues() + " 0 0 0 1";
      xmlTextWriterWriteElement(writer(), C_TRANSFORM3,
...
void CMLFormat::WriteProperties(OBMol &mol, bool &propertyListWritten) {
  ...
  for (k = vdata.begin(); k != vdata.end(); ++k) {
    if ((*k)->GetDataType() == OBGenericDataType::PairData &&
        ...
        && (*k)->GetAttribute() !=
               "PartialCharges") // annotation not needed since partial charges
                                 // are not output in this format
    {
      if (!propertyListWritten) {
      ...
      // Title is now on <property>. If the attribute name has a namespace, use
      // dictRef instead.
      string att((*k)->GetAttribute());
      xmlTextWriterWriteFormatAttribute(
          writer(), (att.find(':') == string::npos) ? C_TITLE : C_DICTREF, "%s",
          att.c_str());
      xmlTextWriterStartElementNS(writer(), prefix, C_SCALAR, nullptr);

      // Title used to be on <scalar>...
      // xmlTextWriterWriteFormatAttribute(writer(),
      // C_TITLE,"%s",(*k)->GetAttribute().c_str());
      xmlTextWriterWriteFormatString(
          writer(), "%s", (static_cast<OBPairData *>(*k))->GetValue().c_str());
      xmlTextWriterEndElement(writer()); // scalar
  ...
  if (fabs(mol.GetEnergy()) > 1e-3)
    WriteScalarProperty(mol, "Energy", mol.GetEnergy() * CALSTOJOULES, "me:ZPE",
...
void CMLFormat::WriteThermo(OBMol &mol, bool &propertyListWritten) {
  ...
  OBNasaThermoData *pThermoData =
      static_cast<OBNasaThermoData *>(mol.GetData(ThermoData));
  ...
  xmlTextWriterStartElementNS(writer(), prefix, C_PROPERTY, nullptr);
  xmlTextWriterWriteFormatAttribute(writer(), C_DICTREF, "%s",
                                    "Thermo_OldNasa");
...
/// Returns molecule title or molecule number if there is no title together with
/// the file name
...
bool CMLFormat::WriteInChI(OBMol &mol) {
  ...
  if (pData) {
    ...
    xmlTextWriterWriteFormatAttribute(writer(), C_CONVENTION, "%s",
                                      "iupac:inchi");
    xmlTextWriterWriteFormatAttribute(writer(), C_VALUE, "%s",
                                      pData->GetValue().c_str());
...
bool CMLFormat::WriteVibrationData(OBMol &mol) {
  ...
  OBVibrationData *vd =
      (OBVibrationData *)mol.GetData(OBGenericDataType::VibrationData);

  xmlTextWriterStartElementNS(writer(), prefix, C_PROPERTY, nullptr);
  xmlTextWriterWriteFormatAttribute(writer(), C_TITLE, "%s",
  ...
  // A negative frequency is output separately as an imaginary frequency (for
  // transition states)
  ...
  if (imaginaryFrequency > 0.0)
    WriteScalarProperty(mol, "ImaginaryFrequency", imaginaryFrequency,
                        "me:imFreqs", "cm-1");
...
bool CMLFormat::WriteRotationData(OBMol &mol) {
  ...
  OBRotationData *rd =
      (OBRotationData *)mol.GetData(OBGenericDataType::RotationData);
  ...
  for (unsigned int i = 0; i < rd->GetRotConsts().size(); ++i)
    if (rd->GetRotConsts()[i] != 0.0)
      xmlTextWriterWriteFormatString(writer(), "%.3lf ",
                                     rd->GetRotConsts()[i] / WAVENUM_TO_GHZ);
  xmlTextWriterEndElement(writer()); // array
  xmlTextWriterEndElement(writer()); // property
  xmlTextWriterStartElementNS(writer(), prefix, C_PROPERTY, nullptr);
...
bool CMLFormat::WriteScalarProperty(OBMol &mol, const char *title, double value,
                                    const char *dictref, const char *units,
