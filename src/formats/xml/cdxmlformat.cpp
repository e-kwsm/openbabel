...
public:
  ChemDrawXMLFormat() : Order(-1) {
    ...
    XMLConversion::RegisterXMLFormat(this, false,
                                     "http://www.camsoft.com/xml/cdxml.dtd");
    XMLConversion::RegisterXMLFormat(this);
  }
  const char *NamespaceURI() const override {
    return "http://www.cambridgesoft.com/xml/cdxml.dtd";
  }
  const char *Description() override {
    return "ChemDraw CDXML format\n"
           "Minimal support of chemical structure information only.\n\n";
  }

  const char *GetMIMEType() override { return "chemical/x-cdxml"; };
  ...
  unsigned int Flags() override { return READXML; };
...
private:
  ...
  std::vector<unsigned int>
      _handleImplicitHydrogens; // list of atoms w/o hydrogen count
};
...
bool ChemDrawXMLFormat::DoElement(const string &name) {
  ...
  /*
  // Forget that, the fragment, aka molecule, is in another XML hierachy tree
  than the data.
...
bool ChemDrawXMLFormat::EndElement(const string &name) {
  ...
  if (name == "n") {
    ...
    Order = -1;
             ...
             "fragment") // this is the end of the molecule we are extracting
  {
    EnsureEndElement();
    ...
    // This alone will already store the "Formula" property in the molecule
    // property block The "Formula" is required for older ChemDraw generations
    // allowing to match molecules to reaction properties
    string MolFormula = _pmol->GetFormula();

    // additional adding of "Formula" property is not required, as described
    // above
    ...
    // alternative is using the molecular title, but a test is needed for
    // preventing overwriting given titles, aka molecule ID
...
bool ChemDrawXMLFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (_pxmlConv->GetOutputIndex() == 1) {
    ...
    xmlTextWriterWriteDTD(writer(), BAD_CAST "CDXML", nullptr,
                          BAD_CAST "http://www.camsoft.com/xml/cdxml.dtd",
                          nullptr);
  ...
  for (patom = mol.BeginAtom(i); patom; patom = mol.NextAtom(i)) {
    ...
    xmlTextWriterWriteFormatAttribute(writer(), C_ID, "%d",
                                      patom->GetIdx() + _offset);

    Coord2D transformCd = TransformCdxmlCoord(patom);
    xmlTextWriterWriteFormatAttribute(writer(), C_COORDS, "%f %f",
                                      transformCd.x, transformCd.y);
  ...
  for (pbond = mol.BeginBond(j); pbond; pbond = mol.NextBond(j)) {
    ...
    xmlTextWriterWriteFormatAttribute(writer(), C_BEGIN, "%d",
                                      patom->GetIdx() + _offset);
    patom = pbond->GetEndAtom();
    xmlTextWriterWriteFormatAttribute(writer(), C_END, "%d",
                                      patom->GetIdx() + _offset);
