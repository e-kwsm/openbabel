...
bool OBMoldenFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (getline(ifs, lineBuffer)) {
    if (lineBuffer.find("[Atoms]") != string::npos ||
      ...
      if (lineBuffer.find("AU") != string::npos)
        factor = BOHR_TO_ANGSTROM; // Bohr
      while (getline(ifs, lineBuffer)) {
        if (lineBuffer == "")
          continue;
      ...
      if (ecpLines != 0) {
        cerr << "WARNING: element number given in 3rd column does not agree "
                "with element name on "
             << ecpLines << " lines." << endl
             << "         Difference between expected nuclear charge and given "
                "element number saved to atom property 'ecp'."
             << endl;
      }
    } // "[Atoms]" || "[ATOMS]"
    if (lineBuffer.find("[GEOMETRIES] (XYZ)") != string::npos) {
      while (getline(ifs, lineBuffer)) {
        ...
        if (sscanf(lineBuffer.c_str(), "%u", &natoms) == 0 || !natoms ||
            natoms >= 100000000) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "Problems reading an XYZ geometry: The first "
                                "line must contain the number of atoms.",
                                obWarning);
          break;
        }
        ...
        if (fabs(energy) < 1.0e-8) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "Problems reading an XYZ geometry: The second "
                                "line should contain the energy.",
                                obWarning);
        }
        ...
        for (unsigned int a = 0; a < natoms; ++a) {
          ...
          if (createAtoms) {
            ...
            if (atomicNum == 0) {
              // Sometimes people call this an XYZ file, but it's actually
              // Unichem i.e., the first column is the atomic number, not a
              // symbol so we'll try to convert this to an element number
    ...
    if (lineBuffer.find("[FR-COORD]") != string::npos) {
      if (pmol->NumAtoms() == 0) {
        ...
        while (getline(ifs, lineBuffer)) {
          ...
          // in Bohr.
          atom->SetVector(x * BOHR_TO_ANGSTROM, y * BOHR_TO_ANGSTROM,
                          z * BOHR_TO_ANGSTROM);
        }
      }
    } // "[FR-COORD]"
  ...
  if (conformers.size() > 0) {
    ...
    for (unsigned int i = 0; i < conformers.size(); ++i) {
      ...
      unsigned int n =
          coordinates.size() < natoms ? coordinates.size() : natoms;
...
bool OBMoldenFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    snprintf(buffer, BUFF_SIZE, "%2s%6d%3d%13.6f%13.6f%13.6f\n",
             ...
             atom->GetAtomicNum(), atom->GetX(), atom->GetY(), atom->GetZ());
    ofs << buffer;
  }
  ...
  if (vib && vib->GetNumberOfFrequencies() > 0) {
    ...
    FOR_ATOMS_OF_MOL(atom, mol) {
      snprintf(buffer, BUFF_SIZE, "%2s%13.6f%13.6f%13.6f\n",
               ...
               atom->GetX() * ANGSTROM_TO_BOHR, atom->GetY() * ANGSTROM_TO_BOHR,
