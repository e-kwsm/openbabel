...
bool LpmdFormat::ReadHeader(std::istream &ifs, OBMol &mol) {
  ...
  if (!ifs.getline(buffer, BUFF_SIZE)) {
    obErrorLog.ThrowError(__FUNCTION__, "Problem reading header line",
                          obWarning);
    return false;
  }
  ...
  if (tokens.size() == 0) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "The initial line it is empty!!! non LPMD format",
                          obError);
    return false;
  }
  if (tokens.at(0).compare("LPMD") != 0 || tokens.at(1).compare("2.0") != 0) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "The start line, doesn't identify this file like a lpmd 2.0 file",
        obError);
    return false;
  }
  if (tokens.size() == 3 && tokens.at(2).compare("Z") == 0) {
    obErrorLog.ThrowError(
        __FUNCTION__, "There is not support for zipped files yet.", obError);
    return false;
  }
  if (!ifs.getline(buffer, BUFF_SIZE)) {
  ...
  if (headers.size() <= 1 || headers.at(0).compare("HDR") != 0) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Problem reading header, check the HDR line", obError);
  }
  file_line = 2;
  return true;
} // End ReadHeader
...
bool LpmdFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (tokens.size() != 1) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "The number of atoms line was not correctly readed",
                          obError);
  }
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int i = 1; i < headers.size(); ++i) // the first element is HDR
    {
      ...
      if (headers.at(i).compare("X") == 0)
        from_string<double>(X, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("Y") == 0)
        from_string<double>(Y, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("Z") == 0)
        from_string<double>(Z, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("SYM") == 0)
        symbol = tokens.at(i - 1);
      // Velocities
      if (headers.at(i).compare("VX") == 0)
        from_string<double>(VX, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("VY") == 0)
        from_string<double>(VY, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("VZ") == 0)
        from_string<double>(VZ, tokens.at(i - 1), std::dec);
      // Accelerations
      ...
      // Charges
      if (headers.at(i).compare("CHG") == 0)
        from_string<double>(CHG, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("PHG") == 0)
        from_string<double>(CHG, tokens.at(i - 1), std::dec);
      if (headers.at(i).compare("FHG") == 0)
        from_string<double>(FCH, tokens.at(i - 1), std::dec);
    }
...
bool LpmdFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  // Cell vectors
  snprintf(buffer, BUFF_SIZE, "%-10.3f%-10.3f%-10.3f", v[0].x(), v[0].y(),
           v[0].z());
  ofs << buffer;
  snprintf(buffer, BUFF_SIZE, "%-10.3f%-10.3f%-10.3f", v[1].x(), v[1].y(),
           v[1].z());
  ofs << buffer;
  ...
  for (int i = 0; i < mol.NumAtoms(); ++i) {
    ...
    vector3 tmp = uc->CartesianToFractional(
        vector3(atom->GetX(), atom->GetY(), atom->GetZ()));
    snprintf(buffer, BUFF_SIZE, "%-3s%15.5f%15.5f%15.5f",
             OBElements::GetSymbol(atom->GetAtomicNum()), tmp.GetX(),
             tmp.GetY(), tmp.GetZ());
    ofs << buffer;
    if (level >= 1) {
      vector3 v = vector3(velocilist[0][i].x(), velocilist[0][i].y(),
                          velocilist[0][i].z());
    ...
    if (level >= 2 && mode == 0) {
      double mass = atom->GetAtomicMass();
      vector3 a =
          vector3(forceslist[0][i].x() / mass, forceslist[0][i].y() / mass,
                  forceslist[0][i].z() / mass);
      snprintf(buffer, BUFF_SIZE, "%15.5f%15.5f%15.5f", a.x(), a.y(), a.z());
      ofs << buffer;
    }
    if (level >= 2 && mode == 1) {
      vector3 force = vector3(forceslist[0][i].x(), forceslist[0][i].y(),
                              forceslist[0][i].z());
      snprintf(buffer, BUFF_SIZE, "%15.5f%15.5f%15.5f", force.x(), force.y(),
               force.z());
      ofs << buffer;
    }
    if (charge) {
