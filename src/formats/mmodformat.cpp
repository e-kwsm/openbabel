...
public:
  ...
  {
    return "MacroModel format\n"
           "No comments yet\n";
  }

  const char *SpecificationURL() override { return ""; } // optional

  const char *GetMIMEType() override { return "chemical/x-macromodel-input"; }
...
bool MacroModFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= natoms; i++) {
    ...
    int matched = sscanf(buffer, "%9s%d%d%d%d%d%d%d%d%d%d%d%d%lf%lf%lf",
                         temp_type, &end[0], &order[0], &end[1], &order[1],
                         &end[2], &order[2], &end[3], &order[3], &end[4],
                         &order[4], &end[5], &order[5], &x, &y, &z);
    if (matched < 16) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Problems reading a MacroModel file: "
                            "truncated atom record.",
                            obError);
      return false;
    }
...
bool MacroModFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  char buffer[BUFF_SIZE];
  snprintf(buffer, BUFF_SIZE, " %5d %6s      E = %7.3f KJ/mol", mol.NumAtoms(),
           mol.GetTitle(), 4.184 * mol.GetEnergy());
  ofs << buffer << endl;
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    for (nbr = atom->BeginNbrAtom(j); nbr; nbr = atom->NextNbrAtom(j)) {
      snprintf(buffer, BUFF_SIZE, " %5d %1d", nbr->GetIdx(),
               (*j)->GetBondOrder());
      ofs << buffer;
    }
    ...
    snprintf(buffer, BUFF_SIZE, " %11.6f %11.6f %11.6f %5d %5d %8.5f \n",
             atom->x(), atom->y(), atom->z(), 0, 0, atom->GetPartialCharge());
    ofs << buffer;
  }
