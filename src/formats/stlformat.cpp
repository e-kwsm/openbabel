...
void map_sphere(vector<Triangle> &triangles, vector3 origin, double r,
                uint16_t col) {
  ...
  for (int y = -r2; y < r2; ++y) {
    ...
    for (int i = -p2; i < p2; ++i) {
      ...
      points.push_back(vector3(origin[0] + r * ci * cy, origin[1] + r * sy,
                               origin[2] + r * si * cy));
      points.push_back(vector3(origin[0] + r * ci * cy1, origin[1] + r * sy1,
                               origin[2] + r * si * cy1));
    }
  }

  for (int i = 0; i < points.size() - 2; i++) {
    // Order the points to obey the 'right-hand rule', so normals all face
    // outwards
    if (i % 2 == 0) {
      triangles.push_back(
          Triangle(points[i], points[i + 1], points[i + 2], col));
    } else {
      triangles.push_back(
          Triangle(points[i + 2], points[i + 1], points[i], col));
    }
  }
}
...
#define COL(R, G, B)                                                           \
  (0x800 | (((R)&0x1F) << 10) | (((G)&0x1F) << 5) | ((B)&0x1F));
...
static void output_stl(ostream &os, vector<Triangle> &triangles,
                       bool cpk_colour) {
  uint8_t tok1 = 0;
  uint32_t tok2 = 0;
  ...
  for (vector<Triangle>::iterator i = triangles.begin(); i != triangles.end();
...
bool STLFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(a, *pmol) {
    ...
    const double vdwrad =
        scale_factor * OBElements::GetVdwRad(a->GetAtomicNum()) + probe_radius;
    if (cpk_colours) {
