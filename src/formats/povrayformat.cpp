...
// Note: Based on center_coords from obutil.cpp. This function is declared in
// matrix3x3.h but there is no function
//       definition there.
...
public:
  ...
  PovrayFormat() { OBConversion::RegisterFormat("pov", this); }

  const char *Description() override // required
  {
    return "POV-Ray input format\n"
           "Generate an input file for the open source POV-Ray ray tracer.\n\n"
           "The POV-Ray file generated by Open Babel should be considered a "
           "starting\n"
           "point for the user to create a rendered image of a molecule. "
           "Although care\n"
           "is taken to center the camera on the molecule, the user will "
           "probably want\n"
           "to adjust the viewpoint, change the lighting, textures, etc.\n\n"

           "The file :file:`babel_povray3.inc` is required to render the "
           "povray file\n"
           "generated by Open Babel. This file is included in the Open Babel\n"
           "distribution, and it should be copied into the same directory as "
           "the\n"
           ":file:`.pov` file before rendering. By editing the settings in\n"
           ":file:`babel_povray3.inc` it is possible to tune the appearance\n"
           "of the molecule.\n\n"

           "For example, the image below was generated by rendering the output "
           "from the\n"
           "following command after setting the reflection of non-metal atoms "
           "to 0\n"
           "(line 121 in :file:`babel_povray3.inc`)::\n\n"
           "  obabel -:\"CC(=O)Cl acid chloride\" --gen3d -O chloride.pov -xc "
           "-xf -xs -m SPF\n\n"
           ...
           " m <model-type> BAS (ball-and-stick), SPF (space-fill) or CST "
           "(capped sticks)\n"
           "    The default option is ball-and-stick. To choose space-fill, "
           "you would use\n"
...
void PovrayFormat::OutputHeader(ostream &ofs, OBMol &mol, string prefix) {
  ...
  OpenBabel::vector3 centroid =
      my_center_coords(mol.GetCoordinates(), mol.NumAtoms());
  ...
  ofs << "// perspective (default) camera\n"
         "camera {\n"
         "  location  <"
      << centroid.x() << "," << centroid.y() << "," << centroid.z() - 10.0
      << ">\n"
         "  look_at   <"
  ...
  if (checkerboard) {
    ofs << "// simple Black on White checkerboard... it's a classic\n"
           "plane {\n"
           " -y, "
        << -(centroid.y() - 8)
        << "\n"
...
void CalcBoundingBox(OBMol &mol, double &min_x, double &max_x, double &min_y,
...
void OutputAtoms(ostream &ofs, OBMol &mol, string prefix) {
  ...
  for (i = 1; i <= mol.NumAtoms(); ++i) {
    ...
    ofs << "#declare " << prefix << "_pos_" << i << " = <" << atom->GetX()
        << "," << atom->GetY() << "," << atom->GetZ() << ">;" << endl;
  }
...
void OutputBASBonds(ostream &ofs, OBMol &mol, string prefix) {
  ...
  for (unsigned int i = 0; i < mol.NumBonds(); ++i) {
    ...
    if (fabs(RAD2DEG(-phi) + (double)90.0) >= EPSILON) {
      ...
      /* ---- Rotate along z-axis ---- */
      ofs << "\t  rotate <0.0000,0.0000," << RAD2DEG(-phi) + (double)90.0 << ">"
          << endl;
    }
    ...
    if (theta >= EPSILON) {
      ...
      if ((z2 - z1) >= (double)0.0) {
        ...
        ofs << "\t  rotate <0.0000," << RAD2DEG((double)-1.0 * theta)
            << ",0.0000>" << endl;
...
void OutputCSTBonds(ostream &ofs, OBMol &mol, string prefix) {
  ...
  for (unsigned int i = 0; i < mol.NumBonds(); ++i) {
    ...
    ofs << "\t   object {" << endl
        << "\t    bond_" << bond->GetBondOrder() << "\n";

    /* ---- Add a pigment - statement for start-atom of bond ---- */
    bond_type = bond->GetBeginAtom()->GetType();
    bond_type.erase(remove(bond_type.begin(), bond_type.end(), '.'),
                    bond_type.end());
    ofs << "\t    pigment{color Color_" << bond_type << "}" << endl;
    ...
    if (fabs(RAD2DEG(-phi) + (double)270.0) >= EPSILON) {
      ...
      ofs << "\t    rotate <0.0000,0.0000,"
          << (RAD2DEG(-phi) + (double)90.0) + (double)180.0 << ">" << endl;
    }
    ...
    ofs << "\t    translate " << prefix << "_pos_" << bond->GetEndAtomIdx()
        << endl;
...
void OutputUnions(ostream &ofs, OBMol &mol, string prefix) {
  ...
  if (mol.NumBonds() > 0) {
    ...
    ofs << "//Bonds only needed for ball and sticks or capped sticks models"
        << endl;
    ofs << "#if (BAS | CST)" << endl;
...
void OutputMoleculeBonds(ostream &ofs, string prefix, double min_x,
  ...
  ofs << "#if (SPF)" << endl;
  ofs << "#declare " << prefix << " = object{" << endl
      << "\t  " << prefix << "_atoms" << endl;
  ...
  ofs << "//\t  bounded_by {" << endl
      << "//\t   box {" << endl
      << "//\t    <" << min_x - MAXRADIUS << "," << min_y - MAXRADIUS << ","
      << min_z - MAXRADIUS << ">" << endl;

  ofs << "//\t    <" << max_x + MAXRADIUS << "," << max_y + MAXRADIUS << ","
...
void OutputMoleculeNoBonds(ostream &ofs, string prefix) {
  ...
  ofs << "#declare " << prefix << " = object {" << prefix << "_atoms}" << endl
      << endl;
}

void OutputCenterComment(ostream &ofs, string prefix, double min_x,
                         double max_x, double min_y, double max_y, double min_z,
                         double max_z) {
  ...
  ofs << "#declare " << prefix << "_center = <"
      ...
      << (double)-1.0 * (min_z + max_z) / (double)2.0 << ">;" << endl
      << endl;
}
...
bool PovrayFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (tmp) {
    ...
    if (model_type != "BAS" && model_type != "SPF" && model_type != "CST") {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Unknown model type specified. Using the default "
                            "instead (\"BAS\", ball-and-stick).\n",
                            obWarning);
      model_type = "BAS";
    }
  }
  ...
  if (mol.NumBonds() > 0) {
    ...
    OutputMoleculeBonds(ofs, prefix, min_x, max_x, min_y, max_y, min_z, max_z);
  ...
  OutputCenterComment(ofs, prefix, min_x, max_x, min_y, max_y, min_z, max_z);
