...
".. image:: ../_static/povray.png\n\n"
  ...
  void PovrayFormat::OutputHeader(ostream &ofs, OBMol &mol, const string& prefix)
  {
    ...
    ofs << "//Povray v3 code generated by Open Babel" << '\n';
    ofs << "//Author: Steffen Reith <streit@streit.cc>" << '\n';
    ofs << "//Update (2010): Noel O'Boyle and Steven Wathen" << '\n';

    /* ---- Include timestamp in header ---- */
    ofs << "//Date: " << timestr << '\n' << '\n';

    /* ---- Set configurable global settings ---- */
    ofs << "//Set some global parameters for display options" << '\n';
    ofs << "#declare " << model_type << " = true;" << '\n';
    string trans_tex_setting = trans_texture ? "true" : "false";
    ofs << "#declare TRANS = " << trans_tex_setting << ";" << '\n' << '\n';
...
"  }\n" << '\n';
      ...
      ofs << "// set a color of the background (sky)" << '\n';
...
"}\n" << '\n';
    ...
    ofs << "//Include header for povray" << '\n';
    ofs << "#include \"babel_povray3.inc\"" << '\n' << '\n';
    ...
    if (mol.NumBonds() == 0)
      {
        ...
        ofs << "#if (BAS | CST)\"" << '\n';
        ofs << "#warning \"Molecule without bonds!\"" << '\n';
        ofs << "#warning \"You should do a spacefill-model\"" << '\n';
        ofs << "#end" << '\n' << '\n';
    ...
    ofs << "//Print name of molecule while rendering" << '\n';
  ...
  void OutputAtoms(ostream &ofs, OBMol &mol, const string& prefix)
  {
    ...
    ofs << "//Coodinates of atoms 1 - " << mol.NumAtoms() << '\n';
    unsigned int i;
    ...
    ofs << '\n' << "//Povray-description of atoms 1 - " << mol.NumAtoms() << '\n';
    for(i = 1; i <= mol.NumAtoms(); ++i)
      {
        ...
        ofs << "object {" << '\n'
            << "\t  Atom_" << OBElements::GetSymbol(atom->GetAtomicNum()) << '\n'
            << "\t  translate " << prefix << "_pos_" << i << '\n' << "\t }" << '\n';
    ...
    for(unsigned int i = 0; i < mol.NumBonds(); ++i)
      {
        ...
        ofs << "#declare " << prefix << "_bond" << i << " = object {" << '\n';
        ofs << "\t  union {" << '\n';
        ...
        if (fabs((double) 2.0 * dist) >= EPSILON)
          {
            ...
            /* ---- Print povray scale-statement (x-Axis) ---- */
            ofs << "\t    scale <" << (double) 0.5 * dist << ",1.0000,1.0000>" << '\n';
        ...
        if (theta >= EPSILON)
          {
            ...
            if ((z2 - z1) >= (double) 0.0)
            ...
            else
              {
                ...
                /* ---- Rotate along y-Axis (positive) ---- */
                ofs << "\t    rotate <0.0000," << RAD2DEG(theta) << ",0.0000>" << '\n';
  ...
  void OutputUnions(ostream &ofs, OBMol &mol, const string& prefix)
  {
    ...
    ofs << '\n' << "//All atoms of molecule " << prefix << '\n';
    ofs << "#ifdef (TRANS)" << '\n';
    ofs << "#declare " << prefix << "_atoms = merge {" << '\n';
    ofs << "#else" << '\n';
    ofs << "#declare " << prefix << "_atoms = union {" << '\n';
    ofs << "#end //(End of TRANS)" << '\n';
    ...
    if(mol.NumBonds() > 0)
      {
        ...
        ofs << "#if (BAS | CST)" << '\n';
        ofs << "#declare " << prefix <<"_bonds = union {" << '\n';
                           ...
                           )
  {
    ...
    ofs << "#else" << '\n';
    ofs << "#declare " << prefix << " = union {" << '\n';

    /* ---- Add all Atoms ---- */
    ofs << "\t  object{" << prefix << "_atoms}" << '\n';

    /* ---- Add difference between bonds and atoms ---- */
    ofs << "#if (BAS | CST)//(Not really needed at moment!)" << '\n';

    /* ---- Use disjunct objects for transparent pics? ---- */
    ofs << "#if (TRANS)" << '\n';
    ofs << "\t  difference {" << '\n';
    ofs << "\t   object{" << prefix << "_bonds}" << '\n'
        << "\t   object{" << prefix << "_atoms}" << '\n'
        << "\t  }" << '\n';
  ...
  void OutputMoleculeNoBonds(ostream &ofs, const string& prefix)
  {
    ...
    ofs << '\n' << "//Definition of Molecule " << prefix << " (no bonds)" << '\n';
    ofs << "#declare " << prefix << " = object {" << prefix << "_atoms}" << '\n' << '\n';
    ...
    if (mol.NumBonds() > 0)
      {
        ...
        /* ---- Write an comment ---- */
        ofs << "//Povray-description of bonds 1 - " << mol.NumBonds() << '\n';
