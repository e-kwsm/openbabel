...
public:
  ...
  OrcaOutputFormat() { OBConversion::RegisterFormat("orca", this); }
...
public:
  // Register this format type ID
  OrcaInputFormat() { OBConversion::RegisterFormat("orcainp", this); }
...
bool OrcaOutputFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  std::vector<double> Frequencies, Intensities, RamanActivities, UVWavelength,
      UVForces, UVEDipole;
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (checkKeywords.find("CARTESIAN COORDINATES (ANGSTROEM)") != notFound) {
      ...
      while (vs.size() == 4) {
        ...
        if (newMol) {
          atom = mol.NewAtom();
          atom->SetAtomicNum(
              OBElements::GetAtomicNum(vs[0].c_str())); // set atomic number
      ...
      if (geoOptRun) {
        ...
        //                for (int j=0;j<3;j++){
        //                    cout << confCoords[j*3] << " " <<
        //                    confCoords[j*3+1] << " " << confCoords[j*3+2] <<
        //                    endl;
        //                }
    ...
    if (checkKeywords.find("ORBITAL ENERGIES") != notFound) {
      ...
      ifs.getline(buffer,
                  BUFF_SIZE); // skip empty line or look for spin informations
    ...
    if (checkKeywords.find("MULLIKEN ATOMIC CHARGES") != notFound) {
      ...
      while (vs.size() == 4) { // atom number, atomic symbol,:,  charge
        ...
        atom = mol.GetAtom(atoi(vs[0].c_str()) +
                           1); // Numbering starts from 0 in Orca
    ...
    if (checkKeywords.find("NORMAL MODES") != notFound) {
      ...
      for (unsigned int i = 0; i < 6; i++) {
        ifs.getline(buffer,
                    BUFF_SIZE); // skip ----------, comments and blank lines
      }
      ...
      while (vs.size() != 0) {
        ...
        while (vs.size() == nColumn + 1) {
          ...
          //                std::cout <<" end while vib.size = "<< vib.size() <<
        //                endl; for (unsigned int i = iMode; i < iMode+nColumn;
        //                i++) {
        for (unsigned int i = 0; i < nColumn; i++) {
          //                    std::cout << "orca i = "  << i << endl;
          if (FrequenciesAll[iMode] > 10.0) { // something higher than 0
            //                        std::cout <<" vib[i].size = " <<i << " "
            //                        << vib[i].size() << endl;
            Lx.push_back(vib[i]);
            //                        std::cout << i<< "  " << Lx[i].size() <<
            //                        endl; std::cout << Lx.size() << endl;
          }
          iMode++;
        }
      } // while
    ...
    if (checkKeywords.find(
            "ABSORPTION SPECTRUM VIA TRANSITION ELECTRIC DIPOLE MOMENTS") !=
        notFound) {
      //        if(strstr(buffer,"ABSORPTION SPECTRUM VIA TRANSITION ELECTRIC
      //        DIPOLE MOMENTS") != NULL)
      //        {
    ...
    if (checkKeywords.find("UNIT CELL (ANGSTROM)") !=
        notFound) { // file contains unit cell information
      ...
      if (checkNextKeyword.find("CARTESIAN COORDINATES (ANGSTROM)") !=
          notFound) {
        mol.Clear();
        ...
        tokenize(vs, buffer);
        while (vs.size() >=
               4) { // sometime there are additional infos in the line
          atom = mol.NewAtom();
          x = atof((char *)vs[1].c_str());
          y = atof((char *)vs[2].c_str());
          z = atof((char *)vs[3].c_str());
  ...
  if (unitCell) {
    ...
    uC->SetData(unitCellVectors.at(0), unitCellVectors.at(1),
                unitCellVectors.at(2));
    ...
    if (UVWavelength.size() > 0) {
      ...
      if (CDWavelength.size() == UVWavelength.size()) {
        ...
        etd->SetRotatoryStrengthsVelocity(
            CDVelosity); // just vector with 0.0 because ORCA doesn't calculate
                         // these values
      }
    } else {
      // only CD spectrum has been found
      etd->SetData(CDWavelength,
                   CDVelosity); // ony wavelengths information are known , 2nd
                                // vector just contains 0.0
      etd->SetRotatoryStrengthsLength(CDStrengthsLength);
      etd->SetRotatoryStrengthsVelocity(
          CDVelosity); // just vector with 0.0 because ORCA doesn't calculate
                       // these values
  ...
  // cout << "Atom index 0 = " << mol.GetAtom(0)->GetX() << " " <<
  // mol.GetAtom(0)->GetY() << " " << mol.GetAtom(0)->GetZ() << endl;
...
string OrcaOutputFormat::checkColumns(string checkBuffer) {
  ...
  smatch pm;
  try {
    myregex.assign(pattern, regex_constants::extended);
    // iok = true;
...
{ checkBuffer.insert(pm.rm_eo-1, " ");  // insert whitespace to separate the
columns
      }
      return (checkBuffer);
  }
