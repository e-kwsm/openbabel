...
public:
  ...
  {
    return "Point cloud on VDW surface\n"
           "Generates a point cloud on the VDW surface around the molecule\n\n"

           "The surface location is calculated by adding the probe atom "
           "radius\n"
           "(if specified) to the Van der Waal radius of the particular atom "
           "multipled\n"
           "by the specified multiple (1.0 if unspecified)."

           "Output is a list of {x,y,z} tuples in Angstrom. Alternatively, if "
           "the ``x``\n"
           "option is specified, the :ref:`XYZ_cartesian_coordinates_format` "
           "is used\n"
           "instead.\n\n"
           ...
           "  d <densities> for each surface, specify the point density "
  ...
  const char *SpecificationURL() override { return "N/A"; }
  ...
  int SkipObjects(int /*n*/, OpenBabel::OBConversion * /*pConv*/) override {
    return 0;
  }
...
double rv(void) {
  return ((double)std::rand()) / RAND_MAX; // C++11 provides a better <random>
                                           // but I think this will do here
}
...
bool conditional_add(vector<vector3> &list, vector3 point, double density_r) {
  ...
  for (std::vector<vector3>::iterator it = list.begin(); it != list.end();
       ++it) {
    vector3 r = *it - point;
    double r2 = dot(r, r);
    if (r2 < density_r2) {
...
bool PointCloudFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (pConv->IsOption("p")) {
    ...
    if (!isfinite(probe_radius) || probe_radius < 0.) {
      probe_radius = 0.;
    }
  }
  if (pConv->IsOption("x")) {
  ...
  if (radius_mult_list.size() == 0) {
    radius_mult_list.push_back(1.0);
  }
  ...
  for (int j = 0; j < radius_mult_list.size(); j++) {
    ...
    FOR_ATOMS_OF_MOL(a, *pmol) {
      ...
      double vdwrad = probe_radius +
                      (OBElements::GetVdwRad(a->GetAtomicNum()) * radius_mult);

      // estimate # of points by dividing area of VDW sphere
      // by the required density
      int estimate_number_of_points =
          (int)(0.6 * ((4. * M_PI * M_PI * vdwrad * vdwrad) / (density)));
      int count = 0;
      ...
      // now cull any points on that surface that are within r_VDW *
      // radius_multiplier of any other atom
      for (std::vector<vector3>::iterator it = pt.begin(); it != pt.end();
           ++it) {
        bool exclude = false;
        FOR_ATOMS_OF_MOL(a, *pmol) {
          const double *c = a->GetCoordinate();
  ...
  for (std::vector<vector3>::iterator it2 = filtered_points.begin();
       it2 != filtered_points.end(); ++it2) {
    if (format_xyz) {
      os << "Xx\t";
    }
