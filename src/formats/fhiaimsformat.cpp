...
public:
  ...
  FHIaimsFormat() { OBConversion::RegisterFormat("fhiaims", this); }

  const char *Description() override // required
  {
    return "FHIaims XYZ format\n"
           "Read Options e.g. -as\n"
  ...
  const char *SpecificationURL() override {
    return "http://www.fhi-berlin.mpg.de/th/aims/";
  } // optional
...
bool FHIaimsFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    snprintf(buffer, BUFF_SIZE, "atom           %15.5f%15.5f%15.5f  %s",
  ...
  if (mol.HasData(OBGenericDataType::UnitCell)) {
    ...
    for (unsigned int i = 0; i < v.size(); ++i) {
      snprintf(buffer, BUFF_SIZE, "lattice_vector %15.5f%15.5f%15.5f\n",
               v[i].x(), v[i].y(), v[i].z());
      ofs << buffer;
    }
  }
