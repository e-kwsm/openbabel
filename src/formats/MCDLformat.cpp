...
public:
  ...
  const char *Description() override // required
  {
    return "MCDL format\n"
           ...
           ".. [gb2001] A.A. Gakh and M.N. Burnett. **Modular Chemical "
           "Descriptor\n"
           "            Language (MCDL): Composition, Connectivity and\n"
           "            Supplementary Modules.**\n"
           "            *J. Chem. Inf. Comput. Sci.*, **2004**, *41*, "
           "1491-1499.\n"
  ...
  const char *SpecificationURL() override {
    return "https://pubs.acs.org/doi/10.1021/ci000108y";
  }

  const char *GetMIMEType() override { return "chemical/x-MCDL"; }
  ...
  unsigned int Flags() override { return 0; }
...
private:
  ...
  void assignCharges(const std::vector<int> aPosition,
                     const std::vector<int> iA1, const std::vector<int> iA2,
                     std::vector<int> &aCharges, std::vector<int> &charges,
                     std::vector<int> &bondOrder, int aPos, int nPrev, int nt,
...
void MCDLFormat::solve(int ntypes, int z[MAXBONDS][4], int depth) {
  ...
  for (i = 0; i <= MAXFRAGS; i++)
    strngs[i] = (char *)calloc(MAXFRAGS + 1, sizeof(char));
  for (i = 0; i < MAXBONDS; i++)
    nsum[i] = (int *)calloc(MAXFRAGS, sizeof(int));
...
string MCDLFormat::intToStr(int k) { return std::to_string(k); };
...
string MCDLFormat::getMCDL(OBMol *pmol, bool /*includeCoordinates*/) {
  ...
  nbonds = 0;
  for (i = 1; i <= nbStore; i++) {
    // Possible, DRAGON format can contains multiply bonds between 2 atoms. Bond
    // order?
    ia0 = bonds[i - 1][0];
    ia1 = bonds[i - 1][1];
  ...
  // bond stereo getting
  bstereo = getBondMCDL(pmol, nbStore, ntatoms, ix, aNumber, bonds,
                        bondStereoList, eqList);
  ...
  if (netcharge != 0) {
    if (netcharge > 0)
      ...
      data = data + intToStr(abs(netcharge)) + "-;";
  }
  if (netradical != 0)
    data = data + intToStr(netradical) + "*;";
  ...
  /*   Block should be saved for future using
    ...
    if (includeCoordinates && (sm.fAtom.maxIndex() > 0))  {
      ...
      xMin=sm.fAtom.getRX(1); xMax=sm.fAtom.getRX(1); yMin=sm.fAtom.getRY(1);
    yMax=sm.fAtom.getRY(1); for (i=1; i<=sm.fAtom.maxIndex(); i++) { if
    (sm.fAtom.getRX(i) < xMin) xMin=sm.fAtom.getRX(i); if (sm.fAtom.getRX(i) >
    xMax) xMax=sm.fAtom.getRX(i); if (sm.fAtom.getRY(i) < yMin)
    yMin=sm.fAtom.getRY(i); if (sm.fAtom.getRY(i) > yMax)
    yMax=sm.fAtom.getRY(i);
      };
      scale=0;
      ...
      if (nCharge != 0) {
        ...
        for (i=1; i<= sm.fAtom.maxIndex(); i++) if (sm.fAtom.getNC(i) != 0)
    line=line+","+i+","+sm.fAtom.getNC(i); line=line+"}"; data=data+line;
      };
    };
  */
  return data;
...
void MCDLFormat::setMCDL(const string lineToParse, OBMol *pmol, string &sout) {
  ...
  if ((indexOf(value, fablockstart) >= 0) &&
      (indexOf(value, fbblockstart) > 0)) {
  ...
  test = false;
  n = 1;
  while ((value.length() > 0) && (value.at(0) >= '0') && (value.at(0) <= '9') &&
         (!test) && (n > 0)) {
    n = indexOf(value, ";");
    if (n > 0) {
      s = value.substr(0, n);
    ...
    for (j = 1; j <= n1; j++) {
          ...
          for (k = 1; k < NELEMMAX; k++)
            if (enumber[k] > 0)
              for (m = 1; m <= enumber[k]; m++) {
                fragIndex[acount] =
                    nt + 1;        // sa.fragIndex=fragNo;  //Do not increment!
      ...
      if (charges[nt - 1] != 0) { // search for appropriate atom to put charge..
        if (nPrev == acount) {
          ...
          if (charges[nt - 1] < 0) {
            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 8,
                          nPrev, nt, acount, bcount);
            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 7,
                          nPrev, nt, acount, bcount);
            assignCharges(aPosition, iA1, iA2, aCharge, charges, bondOrders, 16,
                          nPrev, nt, acount, bcount);
            if (charges[nt - 1] != 0) {
              aCharge[nt - 1] = charges[nt - 1];
            };
      ...
      if (radicals[nt - 1] != 0) { // search for appropriate atom to put
                                   // charge..
  ...
  while ((value.length() > 0) && test) {
    ...
    while (mf.length() > 0 && bcount < MAXBONDS) {
      ...
      n1 = atoi(s.c_str());
      if (n1 < 1)
        break; // atom indices are 1-based; junk -> stop
      bcount++;
  ...
  // Alternation
  alternate(aPosition, aCharge, aRad, nHydr, iA1, iA2, bondOrders, acount,
            bcount);
  ...
  if (pmol != nullptr) {
    for (i = 0; i < acount; i++) {
      ...
      if (aCharge[i] != 0)
        sa.SetFormalCharge(aCharge[i]);
      if (aRad[i] != 0)
        sa.SetSpinMultiplicity(1);
...
void MCDLFormat::assignCharges(const std::vector<int> aPosition,
                               const std::vector<int> iA1,
                               const std::vector<int> iA2,
                               std::vector<int> &aCharges,
                               std::vector<int> &charges,
                               std::vector<int> &bondOrder, int aPos, int nPrev,
                               int nt, int acount, int bcount) {
...
int MCDLFormat::indexOf(const string instring, const string substring,
                        int fromPos) {
  int result = instring.find(substring, fromPos);
  if (result == string::npos)
    result = -1;
  if (result >= instring.length())
    result = -1;
  return result;
};
...
bool MCDLFormat::parseFormula(const string formulaString,
      ...
      while (test) {
        ...
        if (hit >= 0) {
          ...
          value = value.substr(0, n) +
                  value.substr(n + asym.length(), value.length());
          k = 1;
          if (n < value.length())
            if ((value.at(n) >= '0') && (value.at(n) <= '9')) {
              n1 = n;
              n2 = n;
              while ((n2 < (value.length() - 1)) && (value.at(n2) >= '0') &&
                     (value.at(n2) <= '9'))
                n2++;
              ...
              k = atoi(s.c_str());
              value =
                  value.substr(0, n1) + value.substr(n2 + 1, value.length());
            };
...
string MCDLFormat::getMolTitle(string &line) {
  ...
  if (n != string::npos) {
    ...
    if (k != string::npos) {
      result =
          line.substr(n + ftitlestart.length(), k - n - ftitlestart.length());
...
bool MCDLFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (molTitle.length() > 0)
    pmol->SetTitle(molTitle.c_str());
  if (line.length() > 0)
    setMCDL(line, pmol, molTitle);
