...
bool CSSRFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!mol.HasData(OBGenericDataType::UnitCell)) {
    snprintf(buffer, BUFF_SIZE,
             " REFERENCE STRUCTURE = 00000   A,B,C =%8.3f%8.3f%8.3f", 1.0, 1.0,
             1.0);
    ofs << buffer << endl;
    snprintf(buffer, BUFF_SIZE,
             "   ALPHA,BETA,GAMMA =%8.3f%8.3f%8.3f    SPGR =    P1", 90.0f,
             90.0f, 90.0f);
    ofs << buffer << endl;
  } else {
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    snprintf(buffer, BUFF_SIZE, "%4d%2s%-3d  %9.5f %9.5f %9.5f ",
             ...
             vtmp[atom->GetAtomicNum()], atom->x(), atom->y(), atom->z());
    ofs << buffer;
    bonds = 0;
