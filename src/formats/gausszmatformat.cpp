...
public:
  ...
  {
    return "Gaussian Z-Matrix Input\n"
           "Read Options e.g. -as\n"
...
bool GaussianZMatrixInputFormat::WriteMolecule(OBBase *pOb,
                                               OBConversion *pConv) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    case 3:
      snprintf(buffer, BUFF_SIZE, "%-s  %d  r%d  %d  a%d\n", type.c_str(),
               a->GetIdx(), atom->GetIdx(), b->GetIdx(), atom->GetIdx());
      ofs << buffer;
      continue;
      break;
    ...
    default:
      snprintf(buffer, BUFF_SIZE, "%-s  %d  r%d  %d  a%d  %d  d%d\n",
               type.c_str(), a->GetIdx(), atom->GetIdx(), b->GetIdx(),
               atom->GetIdx(), c->GetIdx(), atom->GetIdx());
      ofs << buffer;
    }
  }
...
// If you have problems (or examples of input files that do not work), please
// contact the openbabel-discuss@lists.sourceforge.net mailing list and/or post
// a bug
bool GaussianZMatrixInputFormat::ReadMolecule(OBBase *pOb,
                                              OBConversion *pConv) {
  ...
  vector<string>
      atomLines; // save atom lines to re-parse after reading variables
  ...
  while (ifs.getline(buffer,
                     BUFF_SIZE)) { // blank line, title, blank, then we begin
    if (strlen(buffer) == 0)
      blankLines++;
    if (blankLines == 2)
      break;
