...
public:
  ...
  const char *Description() override // required
  {
    return "Sybyl Mol2 format\n"
           "Read Options e.g. -ac\n"
           "  c               Read UCSF Dock scores saved in comments "
           "preceding molecules\n\n"
           "Write Options e.g. -xl\n"
           "  l               Output ignores residue information (only "
           "ligands)\n"
           "  c               Write UCSF Dock scores saved in comments "
           "preceding molecules\n"
           "  u               Do not write formal charge information in UNITY "
           "records\n\n";
  }
  ...
  const char *GetMIMEType() override { return "chemical/x-mol2"; }
...
// \return Is this atom a sulfur in a (di)thiocarboxyl (-CS2, -COS, CS2H or
// COSH) group?
...
bool MOL2Format::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  if (!foundAtomLine) {
    ...
    obErrorLog.ThrowError(__FUNCTION__,
                          "Unable to read Mol2 format file. No atoms found.",
                          obWarning);
    return (false);
  }
  ...
  for (i = 0; i < natoms; i++) {
    if (!ifs.getline(buffer, BUFF_SIZE))
      return (false);
    sscanf(buffer, " %*s %1024s %lf %lf %lf %1024s %d %1024s %lf", atmid, &x,
           &y, &z, temp_type, &resnum, resname, &pcharge);
    ...
    if (strncmp(temp_type, "CL", 2) == 0) {
      ...
      str = "Si";
      // The following cases are entries which are not in
      // openbabel/data/types.txt and should probably be added there
    ...
    if (!elemno) {
      ...
      errorMsg << "This Mol2 file is non-standard. Problem with molecule: "
               << mol.GetTitle()
               << " Cannot interpret atom types correctly, instead attempting "
                  "to interpret atom type: "
    ...
    if (resnum != -1 && resnum != 0 && strlen(resname) != 0 &&
        strncmp(resname, "<1>", 3) != 0) {
      has_residue_information = true;
      OBResidue *res = (mol.NumResidues() > 0)
                           ? mol.GetResidue(mol.NumResidues() - 1)
                           : nullptr;
      if (res == nullptr || res->GetName() != resname ||
          res->GetNum() != resnum) {
        vector<OBResidue *>::iterator ri;
        for (res = mol.BeginResidue(ri); res; res = mol.NextResidue(ri))
          if (res->GetName() == resname && res->GetNum() == resnum)
            break;
  ...
  } while (nextrti != "@<TRIPOS>UNITY_ATOM_ATTR" &&
           nextrti != "@<TRIPOS>BOND" && nextrti.length() > 0);

  if (nextrti ==
      "@<TRIPOS>UNITY_ATOM_ATTR") { // read in formal charge information, must
                                    // be done before Kekulization
    int aid = 0, num = 0;
  ...
  if (has_explicit_hydrogen) {
    FOR_ATOMS_OF_MOL(atom, mol) {
      ...
      case 8:
        if (total_valence != 1)
          continue;
        if (strcmp(atom->GetType(), "O2") != 0)
          continue; // TODO: the O.co2 type is lost by this point
        {
  ...
  if (needs_kekulization) {
    ...
    // a metal ion. The two O's have charges of -0.5 and the bond orders are
    // aromatic
    FOR_ATOMS_OF_MOL(atom, mol) {
      OBAtom *oxygenOrSulfur = &*atom;
      // Look first for a terminal O/S
      if (!IsOxygenOrSulfur(oxygenOrSulfur) ||
          oxygenOrSulfur->GetTotalDegree() != 1)
        continue;
      ...
      if (!otherOxygenOrSulfur)
        continue;
      if (otherOxygenOrSulfur->GetFormalCharge() != 0)
        continue; // formal charge already set on one
...
bool MOL2Format::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  bool skipFormalCharge =
      pConv->IsOption("u", OBConversion::OUTOPTIONS) != nullptr;
  ...
  if (dp != nullptr) {
    ...
    if (strcasecmp(dp->GetValue().c_str(), "Mulliken") == 0)
      ...
      ofs << "USER_CHARGES" << endl;
  } else { // No idea what these charges are... all our code sets
           // "PartialCharges"
    ofs << "GASTEIGER" << endl;
  }
  ...
  if (mol.HasData(OBGenericDataType::CommentData)) {
    ofs << "****\n"; // comment line printed, so we need to add "no status bits
                     // set"
    OBCommentData *cd =
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    if (!ligandsOnly && (res = atom->GetResidue())) {
      ...
      snprintf(rlabel, BUFF_SIZE, "%s%d", res->GetName().c_str(),
               res->GetNum());
      snprintf(rnum, BUFF_SIZE, "%d", res->GetNum());
    }

    snprintf(buffer, BUFF_SIZE,
             "%7d %-6s   %9.4f %9.4f %9.4f %-5s %3s  %-8s %9.4f",
             atom->GetIdx(), label, atom->GetX(), atom->GetY(), atom->GetZ(),
             str1.c_str(), rnum, rlabel, atom->GetPartialCharge());
    ofs << buffer << endl;
  }
  ...
  for (bond = mol.BeginBond(j); bond; bond = mol.NextBond(j)) {
    ...
    snprintf(buffer, BUFF_SIZE, "%6d %5d %5d   %2s", bond->GetIdx() + 1,
             bond->GetBeginAtomIdx(), bond->GetEndAtomIdx(), label);
    ofs << buffer << endl;
  }
