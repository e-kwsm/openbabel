...
public:
  ...
  HINFormat() { OBConversion::RegisterFormat("hin", this, "chemical/x-hin"); }
  ...
  const char *GetMIMEType() override { return "chemical/x-hin"; } // optional
...
bool HINFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (ifs.good() &&
         (strstr(buffer, "mol") == nullptr ||
          buffer[0] == ';')) // The "mol" in comment line should be ignored.
  {
    ifs.getline(buffer, BUFF_SIZE);
...
bool HINFormat::WriteMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  for (i = 1; i <= mol.NumAtoms(); i++) {
    ...
    snprintf(buffer, BUFF_SIZE,
             "atom %d - %-3s **  - %8.5f %8.5f  %8.5f  %8.5f %d ", i,
             OBElements::GetSymbol(atom->GetAtomicNum()),
             atom->GetPartialCharge(), atom->GetX(), atom->GetY(), atom->GetZ(),
    ...
    for (bond = atom->BeginBond(j); bond; bond = atom->NextBond(j)) {
      ...
      snprintf(buffer, BUFF_SIZE, "%d %c ", (bond->GetNbrAtom(atom))->GetIdx(),
               bond_char);
      ofs << buffer;
    }
    ofs << endl;
