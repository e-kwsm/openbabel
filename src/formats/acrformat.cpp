...
public:
  ...
  const char *SpecificationURL() override {
    return "http://pros.orange.fr/carine.crystallography/books/31/"
           "carine_31_us.pdf";
  }
...
bool ACRFormat::ReadMolecule(OBBase *pOb, OBConversion *pConv) {
  ...
  while (true) {
    ...
    if (atom_input) {
      ...
      // add to pmol
      if (!pmol->AddBond(
              atoi((char *)vs[0].c_str()) + 1, atoi((char *)vs[1].c_str()) + 1, 1 /* bond order not specified in Carine, use PerceiveBondOrder later */)) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "addition of bond between " + vs[0] + " and " +
                                  vs[1] + " failed",
                              obError);
        return false;
      }
  ...
  if (pmol->NumBonds() != bonds) {
    ...
    obErrorLog.ThrowError(
        __FUNCTION__, "Number of bonds read does not match the number promised",
        obError);
    return false;
  } else if (pmol->NumAtoms() != atoms) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Number of atoms read does not match the number promised",
        obError);
    return false;
  }
