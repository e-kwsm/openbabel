...
/** \class OBRotamerList rotamer.h <openbabel/rotamer.h>
    ...
    \code
    OBRotorList rl; // used to sample all rotatable bonds via the OBRotorRules
data
    ...
    rotorKey[i] = 0; // could be anything from 0 ..
OBRotor->GetTorsionValues().size()
...
OBGenericData *OBRotamerList::Clone(OBBase *newparent) const {
  ...
  // Set Rotamers
  unsigned char *rotamers =
      new unsigned char[(NumRotors() + 1) * NumRotamers()];
  if (rotamers) {
    vector<unsigned char *>::const_iterator kk;
    unsigned int idx = 0;
    for (kk = _vrotamer.begin(); kk != _vrotamer.end(); ++kk) {
      memcpy(&rotamers[idx], (const unsigned char *)*kk,
             sizeof(unsigned char) * (NumRotors() + 1));
  ...
  if (rl.HasRingRotors()) {
    ...
    FOR_RINGS_OF_MOL(r, mol) {
      ...
      for (int j = 0; j < pSize; ++j) {
        double torsion = mol.GetTorsion(
            path[(j + pSize - 1) % pSize], path[(j + pSize) % pSize],
            path[(j + pSize + 1) % pSize], path[(j + pSize + 2) % pSize]);
        ringTorsions.push_back(torsion);

        // now check to see if any of these things are rotors
        int rotorIndex = -1; // not a rotor
        OBBond *bond = mol.GetBond(path[(j + pSize) % pSize],
...
void SetRotorToAngle(double *c, OBAtom **ref, double ang, vector<int> atoms) {
  ...
  if (mag < 0.1)
    mag = 0.1; // avoid divide by zero error
  x = v2x / mag;
  y = v2y / mag;
  z = v2z / mag;
