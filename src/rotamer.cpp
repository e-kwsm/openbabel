  ...
  **/
  ...
  OBGenericData* OBRotamerList::Clone(OBBase* newparent) const
  {
    ...
    if (rotamers)
      {
        unsigned int idx=0;
        for (auto kk = _vrotamer.cbegin(); kk != _vrotamer.cend(); ++kk)
          {
  ...
  {
    for (auto i = _vrotamer.begin(); i != _vrotamer.end(); ++i)
      delete [] *i;

    for (auto j = _vrotor.begin(); j != _vrotor.end(); ++j)
      delete [] j->first;
  ...
  {
    int j = 0;
    for (auto i = _vrotor.cbegin(); i != _vrotor.cend(); ++i)
      {
  ...
  void OBRotamerList::Setup(OBMol &mol,OBRotorList &rl)
  {
    ...
    _vres.clear();
    for (auto j = _vrotamer.begin(); j != _vrotamer.end(); ++j)
      delete [] *j;
    _vrotamer.clear();

    for (auto k = _vrotor.begin(); k != _vrotor.end(); ++k)
      delete [] k->first;
    _vrotor.clear();
    _vrings.clear();
    _vringTors.clear();
    ...
    for (auto m = _vres.begin(); m != _vres.end(); ++m)
  ...
  void OBRotamerList::AddRotamer(double *c)
  {
    ...
    int size = 1;
    for (auto i = _vrotor.begin(); i != _vrotor.end(); ++i, ++size)
      {
  ...
  void OBRotamerList::ExpandConformerList(OBMol &mol,vector<double*> &clist)
  {
    ...
    for (auto k = clist.begin(); k != clist.end(); ++k)
      delete [] *k;
    clist = tmpclist;
  }
