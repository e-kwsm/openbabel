...
/*! \class EQEqCharges eqeq.h "eqeq.h"
  ...
  \brief Assigns partial charges according to the extended charge equilibration
  (EQEq) model, according to Wilmer et al., J. Phys. Chem. Let. 2012.
*/
...
bool EQEqCharges::ParseParamFile() {
  ...
  if (OpenDatafile(ifs, "eqeqIonizations.txt").length() == 0) {
    obErrorLog.ThrowError(__FUNCTION__, "Cannot open eqeqIonizations.txt",
                          obError);
    return false;
  }
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (vs.size() != 12) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Format error in eqeqIonizations.txt. Each data "
                            "row should have exactly 12 elements.",
                            obError);
      return false;
    }
...
bool EQEqCharges::ComputeCharges(OBMol &mol) {
  ...
  for (i = 0; i < N; i++) {
    ...
    if (_ionizations[a][c + 1] == -1 || _ionizations[a][c] == -1 ||
        a > TABLE_OF_ELEMENTS_SIZE) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Insufficient ionization data for atoms in the given molecule. "
          "Update `data/eqeqIonizations.txt` with missing information and "
    ...
    chi(i) = 0.5 * (_ionizations[a][c + 1] + _ionizations[a][c]) -
             (a == 1 ? 0 : c * J(i));
  }
  ...
  if (mol.HasData(OBGenericDataType::UnitCell)) {
    ...
    for (i = 0; i < N; i++) {
      ...
      for (j = 0; j < N; j++) {
        ...
        J_ij(i, j) = GetPeriodicEwaldJij(J(i), J(j), dx, (i == j), unitcell,
                                         fourier, cellVolume, numNeighbors);
      }
    }
    ...
    for (i = 0; i < N; i++) {
      ...
      for (j = 0; j < N; j++) {
        J_ij(i, j) =
            GetNonperiodicJij(J(i), J(j), atom->GetDistance(j + 1), (i == j));
      }
      return false;
    }
  }
  ...
  A.block(1, 0, N - 1, N) =
      J_ij.block(0, 0, N - 1, N) - J_ij.block(1, 0, N - 1, N);
  ...
  obErrorLog.ThrowError(__FUNCTION__, "EQEq charges successfully assigned.",
                        obInfo);
  return true;
}

//! Calculates a lumped charge coefficient for simplified nonperiodic systems
double EQEqCharges::GetNonperiodicJij(double J_i, double J_j, double R_ij,
                                      bool isSameAtom) {
  if (isSameAtom)
    return J_i;
  ...
  double a_ij, E_0ij;
  a_ij = sqrt(J_i * J_j) / k;
  E_0ij = exp(-a_ij * a_ij * R_ij * R_ij) *
          (2 * a_ij - a_ij * a_ij * R_ij - 1 / R_ij); // Orbital overlap term

  return lambda * (k / 2.0) * (1.0 / R_ij + E_0ij);
}

//! Calculates a lumped charge coefficient that incorporates neighboring unit
//! cells
double EQEqCharges::GetPeriodicEwaldJij(double J_i, double J_j, vector3 dx,
                                        bool isSameAtom, matrix3x3 unitcell,
                                        matrix3x3 fourier, double cellVolume,
                                        int numNeighbors[]) {
  int u, v, w;
  ...
  for (u = -numNeighbors[0]; u <= numNeighbors[0]; u++) {
    for (v = -numNeighbors[1]; v <= numNeighbors[1]; v++) {
      for (w = -numNeighbors[2]; w <= numNeighbors[2]; w++) {
        ...
        orbital += exp(-a_ij * a_ij * R_ij * R_ij) *
                   (2 * a_ij - a_ij * a_ij * R_ij - 1 / R_ij);
  ...
  return lambda * k / 2.0 * (alpha + beta + orbital) +
         (isSameAtom ? J_i - lambda * k / (eta * sqrt(PI)) : 0);
}
