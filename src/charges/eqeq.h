...
const double minCellLength =
    150.0;             // Minimum dimension of supercell used in calculation [A]
...
public:
  ...
  const char *Description() override {
    return "Assign EQEq (charge equilibration) partial charges.";
  }
...
private:
  int _chargeCenter[TABLE_OF_ELEMENTS_SIZE +
                    1]; // Common charge of metallic ions
  double _ionizations[TABLE_OF_ELEMENTS_SIZE + 1]
                     [9]; // Electron affinity + 8x ionizations of elements
  bool ParseParamFile();
  double GetNonperiodicJij(double J_i, double J_j, double R_ij,
                           bool isSameAtom);
  double GetPeriodicEwaldJij(double J_i, double J_j, vector3 dx,
