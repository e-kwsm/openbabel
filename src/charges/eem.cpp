...
EEMCharges theEEMCharges_bultinck("eem", "eem.txt",
                                  "Bultinck B3LYP/6-31G*/MPA");
EEMCharges theEEMCharges_2015ha("eem2015ha", "eem2015ha.txt",
                                "Cheminf HF/6-311G/AIM");
EEMCharges theEEMCharges_2015hm("eem2015hm", "eem2015hm.txt",
                                "Cheminf HF/6-311G/MPA");
EEMCharges theEEMCharges_2015hn("eem2015hn", "eem2015hn.txt",
                                "Cheminf HF/6-311G/NPA");
EEMCharges theEEMCharges_2015ba("eem2015ba", "eem2015ba.txt",
                                "Cheminf B3LYP/6-311G/AIM");
EEMCharges theEEMCharges_2015bm("eem2015bm", "eem2015bm.txt",
                                "Cheminf B3LYP/6-311G/MPA");
EEMCharges theEEMCharges_2015bn("eem2015bn", "eem2015bn.txt",
                                "Cheminf B3LYP/6-311G/NPA");

EEMCharges::EEMCharges(const char *ID, std::string parameters, std::string type)
    : OBChargeModel(ID, false) {
  _parameters_file = parameters;
  _type = type;
}

const char *EEMCharges::Description(void) {
  _description = "Assign Electronegativity Equilization Method (EEM) atomic "
                 "partial charges. ";
...
void EEMCharges::_loadParameters() {
  ...
  if (!OpenDatafile(ifs, _parameters_file).length()) {
    obErrorLog.ThrowError(__FUNCTION__,
                          std::string("Cannot open file with parameters: ")
                              .append(_parameters_file),
                          obError);
    return;
  }
  std::string line;
  ...
  while (std::getline(ifs, line)) {
    ...
    parameter.bond_order =
        bond_order == "*" ? -1 : std::atoi(bond_order.c_str());
...
void EEMCharges::_solveMatrix(double **A, double *B, unsigned int dim) {
  std::vector<int> temp(dim);
  _luDecompose(A, temp, dim);
  _luSolve(A, temp, B, dim);
}

void EEMCharges::_luDecompose(double **A, std::vector<int> &I,
                              unsigned int dim) {
...
void EEMCharges::_luSolve(double **A, std::vector<int> &I, double *B,
                          unsigned int dim) {
  unsigned int i, k;
...
void EEMCharges::_swapRows(double **_pMatrix, unsigned int i, unsigned int j,
                           unsigned int nCols) {
  double dummy;
