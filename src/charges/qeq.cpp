...
/*! \class QEqCharges qeq.h "qeq.h"
  ...
  \brief Assigns partial charges according to the charge equilibration (QEq)
  model of Rappé and Goddard, 1991.
  ...
  where \f$\mathbf q\f$ is the vector of atomic charges, \f$\boldsymbol \chi\f$
  are atomic electronegativities and \f$\boldsymbol \eta \f$ is the matrix of
  Coulomb interactions with chemical hardnesses along the diagonal, such that
  the overall charge is \f$Q\f$, i.e.
  ...
  The off-diagonal Coulomb interactions are screened using the following
  integral
  ...
  where the orbitals \f$ \phi_{i}(\mathbf{r}) \f$ are s-type Gaussian orbitals
  (GTOs) of the form
  ...
  where \f$ \sigma_i \f$ is the Gaussian screening radius and \f$ \mathbf R_i
  \f$ is the Cartesian coordinate of atom \f$ i \f$.

  The parameters in this model are the atomic electronegativities \$f \chi_i =
  \chi_i^0 \$f in volts (V), hardnesses \$f \eta_{ii} = \eta_i^0 \f$ in V/e, and
  screening radii \$f \sigma_i \$f in Angstroms.

  The default parameter set uses the published values for the elements H, Li, C,
  N, O, F, Na, Si, P, S, Cl, K, Br, Rb, I and Cs. Other parameters are taken
  from the unpublished values that are distributed with UFF.
  ...
  \par 1. This code does _not_ compute the charge-dependent chemical hardness
  and screening exponent for hydrogen which is specified in the original
  publication. Instead, they are set to the \f$q = 0\f$ parameters regardless of
  the actual charge on hydrogen.

  \par 2. The screening of the Coulomb interaction is not calculated in terms of
  Slater-type orbitals, but rather recast in terms of Gaussian-type orbitals
  that are chosen to best reproduce the Coulomb integrals. The maximum error in
  this approximation is on the order of 0.01 Hartree/e^2, which is on par with
  the level of accuracy upon which the original parameters were chosen.
...
void QEqCharges::ParseParamFile() {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    P << atof(vs[1].c_str()) * eV, atof(vs[2].c_str()) * eV,
        1.0 / (radius * radius);
    _parameters.push_back(P);
  }
}

//! Returns a triple of numbers: electronegativity (in eV), hardness (in eV),
//! and Gaussian exponent (in bohr^-2)
...
bool QEqCharges::ComputeCharges(OBMol &mol) {
  ...
  double CoulMaxDistance =
      2 * sqrt(-log(CoulombThreshold) / SmallestGaussianExponent);
  ...
  FOR_ATOMS_OF_MOL(atom1, mol) {
    ...
    FOR_ATOMS_OF_MOL(atom2, mol) {
      j = atom2->GetIdx() - 1;
      if (i > j) {
        // For some reason, this did _not_ produce the expected pairwise
        // distances (2,1) evaluated to the same distance as (2,0) in H2O: bug?
  ...
  if (!status) {
    obErrorLog.ThrowError(__FUNCTION__, "Charges could not be computed.",
                          obError);
    return false;
  }
  ...
  obErrorLog.ThrowError(__FUNCTION__, "QEq charges successfully assigned.",
                        obInfo);
  return true;
}
...
bool QEqCharges::solver(Eigen::MatrixXd A, Eigen::VectorXd b,
  ...
  if (IsNan(resnorm) || resnorm > NormThreshold || !SolverOK) {
    ...
    if (!SolverOK)
      msg << "Solver returned error." << endl;
    if (IsNan(resnorm))
      msg << "NaNs were returned" << endl;
    ...
    if (IsNan(resnorm) || !SolverOK) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          "SVD solver returned an error. Charges may not be reliable!",
          obError);
      return false;
    }
  }
  ...
  if (resnorm > NormThreshold) {
    ...
    msg_reswarn << "Warning, the norm of the residual is " << resnorm
                << "which exceeds the recommended threshold of "
                << NormThreshold;
