...
/*! \class QTPIECharges qtpie.h "qtpie.h"
  ...
  \brief Assigns partial charges according to the charge transfer with
  polarization current equilibration (QTPIE) model of Chen and Martinez, 2007;
  2008.

  The QTPIE model solves for charges by minimizing an energy function of the
  form
  ...
  where \f$ \sigma_i \f$ is a Gaussian screening radius and \f$ \mathbf R_i \f$
  is the Cartesian coordinate of atom \f$ i \f$.
  ...
  screening exponents \$f \alpha_i \$f in Angstroms.
  ...
  References:
    ...
    J. Chen, D. Hundertmark and T. J. Martínez, J. Chem. Phys. 129 (2008):
  214113. doi:10.1063/1.3021400
...
bool QTPIECharges::ComputeCharges(OBMol &mol) {
  ...
  if (Z != 0.)
    obErrorLog.ThrowError(__FUNCTION__,
                          "Warning, total charge on molecule is not zero. "
                          "QTPIE routine may give nonsense.",
                          obWarning);
  ...
  double OverlapMaxDistance = sqrt(
      log((pi / cube(2 * SmallestGaussianExponent)) / sqr(OverlapThreshold)) /
      SmallestGaussianExponent);
  ...
  for (i = 0; i < N; i++)
    OvNorm[i] = 1.0 / (1.0 + Overlap.row(i).sum());
...
// For debugging purposes only
  ...
  obErrorLog.ThrowError(__FUNCTION__, "QTPIE charges successfully assigned.",
                        obInfo);
  return true;
}
