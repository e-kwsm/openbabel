...
void OBBond::SetBondOrder(int order) { _order = (char)order; }
...
void OBBond::SetLength(OBAtom *fixed, double length) {
  ...
  if (IsNearZero(
          v3.length_2())) { // too small to normalize, move the atoms apart
...
bool OBBond::IsRotor(bool includeRingBonds) {
  ...
  //    rotatable = rotatable && ((_bgn->IsHeteroatom() || _bgn->GetHvyDegree()
  //    > 1)
  //                               && (_end->IsHeteroatom() ||
  //                               _end->GetHvyDegree() > 1) );
