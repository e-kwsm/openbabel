...
OBAtom *findAtomWithSymmetryClass(OBAtom *atom, unsigned int symClass,
                                  const std::vector<unsigned int> &symClasses);
bool containsAtLeast_1true_2para(OBAtom *ligandAtom, OBAtom *atom,
                                 const OBStereoUnitSet &units);
bool containsAtLeast_2true_2paraAssemblies(
    OBAtom *ligandAtom, OBAtom *atom, const OBStereoUnitSet &units,
...
void PerceiveStereo(OBMol *mol, bool force) {
  ...
  if (obErrorLog.GetOutputLevel() >= obAuditMsg)
    obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::PerceiveStereo",
                          obAuditMsg);
}
 ...
 *
 * Nitrogen (neutral) is treated as a special case since the barrier of
 * inversion is low in many cases making the atom non-stereogenic. Only
 * bridge-head nitrogen atoms (i.e. nitrogen has 3 neighbors in rings) will be
...
bool isPotentialTetrahedral(OBAtom *atom) {
  ...
  if ((atom->GetHyb() != 3 &&
       !(atom->GetHyb() == 5 &&
         atom->GetAtomicNum() == OBElements::Phosphorus)) ||
      atom->GetTotalDegree() > 4 || atom->GetHvyDegree() < 3 ||
      atom->GetHvyDegree() > 4)
    return false;
  // skip non-chiral N
...
bool isPotentialCisTrans(OBBond *bond) {
  ...
  if (bond->IsInRing())
    return false;
  if (!bond->GetBeginAtom()->HasSingleBond() ||
      !bond->GetEndAtom()->HasSingleBond())
    return false;
  if (bond->GetBeginAtom()->GetHvyDegree() == 1 ||
      bond->GetEndAtom()->GetHvyDegree() == 1)
    return false;
  if (bond->GetBeginAtom()->GetHvyDegree() > 3 ||
      bond->GetEndAtom()->GetHvyDegree() > 3)
    return false;
  return true;
}
 ...
 */
bool isUnitInFragment(OBMol *mol, const OBStereoUnit &unit,
...
/**
 * Check if the specified atom is a tetrahedral center (i.e. there is a
 * Tetrahedral OBStereoUnit in units with the same id)
 */
...
/**
 * Check if the specified bond is a double bond stereocenter (i.e. there is a
 * CisTrans OBStereoUnit in units with the same id)
 */
 ...
 */
int classifyTetrahedralNbrSymClasses(
    const std::vector<unsigned int> &symClasses, OBAtom *atom) {
  ...
  std::vector<unsigned int>::iterator endLoc =
      std::unique(nbrClasses.begin(), nbrClasses.end());
  ...
  case 2:
    ...
    if (std::count(nbrClassesCopy.begin(), nbrClassesCopy.end(),
                   uniqueClasses.at(0)) == 2)
 ...
 */
int classifyCisTransNbrSymClasses(const std::vector<unsigned int> &symClasses,
                                  OBBond *doubleBond, OBAtom *atom) {
  ...
  for (std::size_t i = 0; i < rings.size(); ++i) {
    ...
    for (std::size_t j = 0; j < result.size(); ++j) {
      ...
      if (shared.size() > 1) {
        ...
        int classification = classifyTetrahedralNbrSymClasses(
            symClasses, mol->GetAtom(shared[0]));
...
/*
bool isInSameMergedRing(const std::vector<OBBitVec> &mergedRings, unsigned int
...
struct StereoRing {
  ...
  struct ParaBond {
    ...
    ParaBond(unsigned long _id, unsigned int _inIdx, unsigned int _outIdx)
        : id(_id), inIdx(_inIdx), outIdx(_outIdx) {}
...
bool checkLigands(const Type &currentPara, const OBStereoUnitSet &units) {
  ...
  OBBitVec ligand =
      getFragment(currentPara.outsideNbrs[0], mol->GetAtom(currentPara.outIdx));
  for (OBStereoUnitSet::const_iterator u2 = units.begin(); u2 != units.end();
       ++u2) {
...
template <typename Type>
bool ApplyRule1(const Type &currentPara,
                const std::vector<unsigned int> &symmetry_classes,
                const std::vector<StereoRing> &rings,
                std::vector<bool> &visitedRings, const OBStereoUnitSet &units,
  ...
  for (std::size_t i = 0; i < visitedRings.size(); ++i)
    if (visitedRings[i])
      cout << "  ";
  cout << "ApplyRule1(" << currentPara.inIdx << ", " << currentPara.outIdx << ",
  outside = " << currentPara.outsideNbrs.size() << ")" << endl;
  */
  ...
  for (std::size_t i = 0; i < rings.size(); ++i) {
    ...
    // if there are one or more true stereo centers, currentPara is a stereo
    // center
    ...
    for (std::size_t j = 0; j < rings[i].paraAtoms.size(); ++j) {
      ...
      if (std::find(stereoAtoms.begin(), stereoAtoms.end(), paraAtom.inIdx) !=
          stereoAtoms.end()) {
        // cout << "OK: " << __LINE__ << endl;
        return true;
      }
      ...
      if (paraAtom.outsideNbrs.size() == 1) {
        // only 1 ring substituent, the other is implicit H -> topologically
        // different
        // cout << "OK: " << __LINE__ << endl;
        return true;
      } else {
        ...
        if (symmetry_classes[paraAtom.outsideNbrs[0]->GetIndex()] !=
          ...
          // they are the same and they might also be in a ring -> apply rule 1
          // recursive
          visitedRings[i] = true;
          if (ApplyRule1(paraAtom, symmetry_classes, rings, visitedRings, units,
                         stereoAtoms)) {
            // cout << "OK: " << __LINE__ << endl;
            return true;
          }
    ...
    for (std::size_t j = 0; j < rings[i].paraBonds.size(); ++j) {
      ...
      if (paraBond.inIdx == idx)
        continue;
      // there is another atom already identified as stereo atom
      if (std::find(stereoAtoms.begin(), stereoAtoms.end(), paraBond.inIdx) !=
          stereoAtoms.end()) {
        // cout << "OK: " << __LINE__ << endl;
        return true;
      }
      ...
      if (paraBond.outsideNbrs.size() == 1) {
        ...
        if (symmetry_classes[paraBond.outsideNbrs[0]->GetIndex()] !=
          ...
          // recursive
          visitedRings[i] = true;
          if (ApplyRule1(paraBond, symmetry_classes, rings, visitedRings, units,
                         stereoAtoms)) {
            // cout << "OK: " << __LINE__ << endl;
            return true;
          }
...
void StartRule1(const std::vector<unsigned int> &symmetry_classes,
                const std::vector<StereoRing> &rings, OBStereoUnitSet &units,
                std::vector<unsigned int> &stereoAtoms) {
  for (std::size_t i = 0; i < rings.size(); ++i) {
    ...
    for (std::size_t j = 0; j < rings[i].paraAtoms.size(); ++j) {
      ...
      if (ApplyRule1(paraAtom, symmetry_classes, rings, visitedRings, units,
          ...
          if (symmetry_classes[paraAtom.outsideNbrs[0]->GetIndex()] ==
            ...
            for (std::size_t k = 0; k < rings[i].paraAtoms.size(); ++k) {
              ...
              if (paraAtom.inIdx == paraAtom2.outIdx &&
                ...
                isSpiro = true;
                if (ApplyRule1(paraAtom2, symmetry_classes, rings, visitedRings,
                               units, stereoAtoms))
                  isStereoUnit = true;
              }
            }
        ...
        if (isStereoUnit) {
          ...
          OBAtom *atom =
              paraAtom.insideNbrs[0]->GetParent()->GetAtomById(paraAtom.id);
          units.push_back(
              OBStereoUnit(OBStereo::Tetrahedral, atom->GetId(), true));
        }
      }
    }
    ...
    for (std::size_t j = 0; j < rings[i].paraBonds.size(); ++j) {
      ...
      if (ApplyRule1(paraBond, symmetry_classes, rings, visitedRings, units,
          ...
          if (symmetry_classes[paraBond.outsideNbrs[0]->GetIndex()] ==
            ...
            for (std::size_t k = 0; k < rings[i].paraBonds.size(); ++k) {
              ...
              if (paraBond.inIdx == paraBond2.outIdx &&
                ...
                isSpiro = true;
                if (ApplyRule1(paraBond2, symmetry_classes, rings, visitedRings,
                               units, stereoAtoms))
                  isStereoUnit = true;
              }
            }
        ...
        if (isStereoUnit) {
          ...
          OBBond *bond =
              paraBond.insideNbrs[0]->GetParent()->GetBondById(paraBond.id);
          units.push_back(
              OBStereoUnit(OBStereo::CisTrans, bond->GetId(), true));
        }
      }
    }
  }
 ...
 */
OBStereoUnitSet
FindStereogenicUnits(OBMol *mol, const std::vector<unsigned int> &symClasses) {
  ...
  std::vector<unsigned int>
      stereoAtoms; // Tetrahedral = idx, CisTrans = begin & end idx
  ...
  for (OBBond *bond = mol->BeginBond(ib); bond; bond = mol->NextBond(ib)) {
    ...
    if (bond->GetBondOrder() == 2) {
      ...
      if (begin->GetExplicitDegree() == 2) {
        // Begin atom has two explicit neighbors. One is the end atom. The other
        // should be a heavy atom - this is what we test here. (There is a
        // third, implicit, neighbor which is either a hydrogen or a lone pair.)
        if (begin->ExplicitHydrogenCount() == 1)
          isCisTransBond = false;
      } else if (begin->GetExplicitDegree() == 3) {
        std::vector<unsigned int> tlist;

        for (OBAtom *nbr = begin->BeginNbrAtom(j); nbr;
             nbr = begin->NextNbrAtom(j)) {
          // skip end atom
      ...
      if (end->GetExplicitDegree() == 2) {
        ...
        for (OBAtom *nbr = end->BeginNbrAtom(j); nbr;
             nbr = end->NextNbrAtom(j)) {
          // skip end atom
   ...
   *
   * All rings are merged "mergedRings". A merged ring is simply a fragment
   * consisting of all atoms of a ring system (bridged, spiro, adjacent, ...).
   * If two rings in the SSSR set share an atom, they are merged.
   *
   * Each merged must at least have two para-stereocenters (or 1 true + 1 para)
   * in order for the para-stereocenter to be valid. This is repeated until no
   * new stereocenters are identified.
   *
   * rule 1a for double bonds:
   * - bond atom in ring has two identical symmetry classes for it's neighbor
   * atoms (-> para)
   * - other bond atom:
   *   - has two different symmetry classes for it's neighbours -> new
   * stereocenter
   *   - has two identical symmetry classes, but the ligand contains at least 1
   * true or para stereocenter -> new stereocenter
   *
   ...
   * true or para stereocenter -> new stereocenter
   *
   * NOTE: there must always be at least 2 new stereocenters (or one existing +
   * 1 newly found) in order for them to be valid
   */
  ...
  for (std::size_t i = 0; i < lssr.size(); ++i) {
    ...
    for (std::size_t j = 0; j < paraAtoms.size(); ++j) {
      if (lssr[i]->_pathset.BitIsSet(paraAtoms[j])) {
        ...
        ring.paraAtoms.push_back(
            StereoRing::ParaAtom(atom->GetId(), paraAtoms[j]));
        ...
        // cout << "  ParaAtom(idx = " << ring.paraAtoms.back().inIdx << ",
        // outside = " << ring.paraAtoms.back().outsideNbrs.size() << ")" <<
        // endl;
    ...
    for (std::size_t j = 0; j < paraBonds.size(); ++j) {
      ...
      if (lssr[i]->_pathset.BitIsSet(beginIdx)) {
        ring.paraBonds.push_back(
            StereoRing::ParaBond(bond->GetId(), beginIdx, endIdx));
        ...
        // cout << "  ParaBond(inIdx = " << beginIdx << ", outIdx = " << endIdx
        // << ", outside = " << ring.paraBonds.back().outsideNbrs.size() << ")"
        // << endl;
      ...
      if (lssr[i]->_pathset.BitIsSet(endIdx)) {
        ring.paraBonds.push_back(
            StereoRing::ParaBond(bond->GetId(), endIdx, beginIdx));
        ...
        // cout << "  ParaBond(inIdx = " << endIdx << ", outIdx = " << beginIdx
        // << ", outside = " << ring.paraBonds.back().outsideNbrs.size() << ")"
        // << endl;
        if (ring.paraBonds.back().insideNbrs.size() != 2)
          ring.paraBonds.pop_back();
   ...
   */
  for (std::vector<unsigned int>::iterator idx = paraAtoms.begin();
       idx != paraAtoms.end(); ++idx) {
    OBAtom *atom = mol->GetAtom(*idx);
    // make sure we didn't add this atom already from rule 1
    bool alreadyAdded = false;
    for (OBStereoUnitSet::iterator u2 = units.begin(); u2 != units.end();
         ++u2) {
    ...
    case T1123:
      // rule 2a with 1 pair
      {
        unsigned int duplicatedSymClass =
            findDuplicatedSymmetryClass(atom, symClasses);
        OBAtom *ligandAtom =
            findAtomWithSymmetryClass(atom, duplicatedSymClass, symClasses);
    ...
    case T1122:
      // rule 2a with 2 pairs
      {
        ...
        findDuplicatedSymmetryClasses(atom, symClasses, duplicatedSymClass1,
                                      duplicatedSymClass2);
        OBAtom *ligandAtom1 =
            findAtomWithSymmetryClass(atom, duplicatedSymClass1, symClasses);
        OBAtom *ligandAtom2 =
            findAtomWithSymmetryClass(atom, duplicatedSymClass2, symClasses);
   ...
   * (from rule1)
   */
  for (std::vector<unsigned int>::iterator idx = paraBonds.begin();
       idx != paraBonds.end(); ++idx) {
    ...
    int beginClassification =
        classifyCisTransNbrSymClasses(symClasses, bond, bond->GetBeginAtom());
    ...
    int endClassification =
        classifyCisTransNbrSymClasses(symClasses, bond, bond->GetEndAtom());
    bool endValid = false;
  ...
  if (DEBUG) {
    for (OBStereoUnitSet::iterator unit = units.begin(); unit != units.end();
         ++unit) {
      if (unit->type == OBStereo::Tetrahedral)
 ...
 *
 * Find the duplicated symmetry class for neighbors of atom. This method only
 * works if there is only one duplicated symmetry class (i.e. T1123, T1112,
 * T1111).
 */
unsigned int
findDuplicatedSymmetryClass(OBAtom *atom,
                            const std::vector<unsigned int> &symClasses) {
  ...
  for (std::size_t i = 0; i < nbrSymClasses.size(); ++i) {
    if (std::count(nbrSymClasses.begin(), nbrSymClasses.end(),
                   nbrSymClasses.at(i)) >= 2) {
 ...
 * Helper function for FindStereogenicUnits using automorphisms.
 *
 * Find the duplicated symmetry classes for neighbors of atom. This method only
 * works for the T1122 case.
 */
void findDuplicatedSymmetryClasses(OBAtom *atom,
                                   const std::vector<unsigned int> &symClasses,
                                   unsigned int &duplicated1,
                                   unsigned int &duplicated2) {
  std::vector<unsigned int> nbrSymClasses;
  FOR_NBORS_OF_ATOM(nbr, atom)
  nbrSymClasses.push_back(symClasses.at(nbr->GetIndex()));
 ...
 * Helper function for FindStereogenicUnits using automorphisms.
 *
 * Find the duplicated symmetry classes for neighbors of atoms. This method
 * works for all cases (i.e. T1234, T1123, T1112, T1111 and T1122).
 */
std::vector<unsigned int>
findDuplicatedSymmetryClasses(OBAtom *atom,
                              const std::vector<unsigned int> &symClasses) {
  ...
  for (std::size_t i = 0; i < nbrSymClasses.size(); ++i)
    if (std::count(nbrSymClasses.begin(), nbrSymClasses.end(),
                   nbrSymClasses[i]) > 1)
      if (std::find(result.begin(), result.end(), nbrSymClasses[i]) ==
          result.end())
...
struct StereoInverted {
   ...
   */
  static bool permutationInvertsTetrahedralCenter(
      const Automorphism &p, OBAtom *center,
      const std::vector<unsigned int> &symmetry_classes,
      const std::vector<unsigned int> &canon_labels) {
    // Find the duplicated ligand symmetry class(es)
    std::vector<unsigned int> duplicatedSymClasses =
        findDuplicatedSymmetryClasses(center, symmetry_classes);
    ...
    for (std::size_t i = 0; i < duplicatedSymClasses.size(); ++i) {
      ...
      // Store the ligand indexes for the atoms with the duplicated symmetry
      // class
      ...
      FOR_NBORS_OF_ATOM(nbr, center) {
        if (symmetry_classes[nbr->GetIndex()] == duplicatedSymClass) {
          tlist1.push_back(
              std::make_pair(nbr->GetIndex(), canon_labels[nbr->GetIndex()]));
  ...
  static bool permutationInvertsCisTransBeginOrEndAtom(
      const Automorphism &p, OBBond *bond, OBAtom *beginOrEnd,
      const std::vector<unsigned int> &canon_labels) {
   ...
   */
  static bool permutationInvertsCisTransCenter(
      const Automorphism &p, OBBond *bond,
      const std::vector<unsigned int> &canon_labels) {
    // begin atom
    bool beginInverted = permutationInvertsCisTransBeginOrEndAtom(
        p, bond, bond->GetBeginAtom(), canon_labels);
    // end atom
    bool endInverted = permutationInvertsCisTransBeginOrEndAtom(
        p, bond, bond->GetEndAtom(), canon_labels);
   ...
   */
  static std::vector<Entry> compute(OBMol *mol,
                                    const std::vector<unsigned int> &symClasses,
                                    const Automorphisms &automorphisms) {
    if (DEBUG_INVERSIONS)
      cout << "ENTER StereoInverted::compute()" << endl;
    ...
    // make a list of stereogenic centers inverted by the automorphism
    // permutations
    for (std::size_t i = 0; i < automorphisms.size(); ++i) {
      ...
      if (DEBUG_INVERSIONS)
        cout << "----> Checking automorphism " << i + 1 << endl;
      ...
      for (OBAtom *atom = mol->BeginAtom(ia); atom; atom = mol->NextAtom(ia)) {
        ...
        if (!isPotentialTetrahedral(atom))
          continue;
        // add the atom to the inverted list if the automorphism inverses it's
        // configuration
        if (permutationInvertsTetrahedralCenter(automorphisms[i], atom,
                                                symClasses, canon_labels))
      ...
      for (OBBond *bond = mol->BeginBond(ib); bond; bond = mol->NextBond(ib)) {
        ...
        if (!isPotentialCisTrans(bond))
          continue;
        // add the bond to the inverted list if the automorphism inverses it's
        // configuration
    ...
    if (DEBUG_INVERSIONS)
      cout << "EXIT StereoInverted::compute()" << endl;
...
/**
 * Find an atom with the specified symmetry class. The first found atom is
 * returned or 0 when there is no such atom. This function is intended to be
 * used in cases where any atom with the specified symmetry class can be used.
 * For example, when checking a fragments for stereocenters, the result will be
 * the same for any atom with a specified (duplicated) symmetry class.
 */
OBAtom *findAtomWithSymmetryClass(OBAtom *atom, unsigned int symClass,
                                  const std::vector<unsigned int> &symClasses) {
  OBAtom *ligandAtom = nullptr;
...
/**
 * Helper function to determine if a stereogenic center with duplicated symmetry
 * classes really is a stereogenic center.
 *
 ...
 */
bool containsAtLeast_1true_1para(OBAtom *ligandAtom, OBAtom *skip,
                                 const OBStereoUnitSet &units) {
    ...
    if ((*u2).type == OBStereo::Tetrahedral) {
      if (ligand.BitIsSet((*u2).id)) {
        if ((*u2).para) {
          ...
          for (std::size_t ringIdx = 0; ringIdx < mergedRings.size();
               ++ringIdx) {
            if (mergedRings.at(ringIdx).BitIsSet(paraAtom->GetIdx()))
              if (std::find(ringIndices.begin(), ringIndices.end(), ringIdx) ==
                  ringIndices.end())
...
FindStereogenicUnits(OBMol *mol, const std::vector<unsigned int> &symClasses,
  ...
  std::vector<StereoInverted::Entry> inverted =
      StereoInverted::compute(mol, symClasses, automorphisms);
  ...
  while (true) {
    std::vector<OBAtom *>::iterator ia;
    for (OBAtom *atom = mol->BeginAtom(ia); atom; atom = mol->NextAtom(ia)) {
      if (std::find(doneAtoms.begin(), doneAtoms.end(), atom->GetId()) !=
          doneAtoms.end())
        continue;
      // consider only potential steroecenters
      if (!isPotentialTetrahedral(atom))
      ...
      // A potential stereocenter is really a stereocenter if there exists no
      // automorphic permutation causing an inversion of the configuration of
      // only the potential stereogenic unit under consideration.
      ...
      if (DEBUG_INVERSIONS)
        cout << "foundPermutation for id = " << atom->GetId() << ": "
             << foundPermutation << endl;
      ...
      if (!foundPermutation) {
        ...
        units.push_back(
            OBStereoUnit(OBStereo::Tetrahedral, atom->GetId(), isParaCenter));
        doneAtoms.push_back(atom->GetId());
      } else {
        // count ligand configurations:
        // If there exists at least one automorphic permutation causing the
        // inversion of the configuration of only the stereogenic unit under
        // consideration, then the potential stereocenter can be a stereocenter
        // if the number of topologically equivalent neighbors (ligands) of
        // potential stereogenic is less than or equal to the number of
        // configurations of these ligands.
        //
        // In practise:
    ...
    std::vector<OBBond *>::iterator ib;
    for (OBBond *bond = mol->BeginBond(ib); bond; bond = mol->NextBond(ib)) {
      if (std::find(doneBonds.begin(), doneBonds.end(), bond->GetId()) !=
          doneBonds.end())
        continue;
      if (!isPotentialCisTrans(bond))
        continue;
      ...
      // permutation causing an inversion of the configuration of only the
      // potential stereogenic unit under consideration.
      bool foundPermutation = false; // invert __only__ configuration of atom
      for (std::size_t i = 0; i < inverted.size(); ++i) {
        const std::vector<OBAtom *> &atoms = inverted[i].invertedAtoms;
        // if any atoms are inverted, the bond can't be the only inverted
        // stereocenter
        if (atoms.size())
          continue;
        const std::vector<OBBond *> &bonds = inverted[i].invertedBonds;
      ...
      if (!foundPermutation) {
        // true-stereocenter found
        bool isParaCenter =
            (beginClassification == C12) && (endClassification == C12) ? false
                                                                       : true;
        units.push_back(
            OBStereoUnit(OBStereo::CisTrans, bond->GetId(), isParaCenter));
        ...
        case C11: {
          ...
          if (ligandAtom)
            beginValid = containsAtLeast_1true_1para(
                ligandAtom, bond->GetBeginAtom(), units);
        } break;
        }
        ...
        case C11: {
          ...
          if (ligandAtom)
            endValid = containsAtLeast_1true_1para(ligandAtom,
                                                   bond->GetEndAtom(), units);
        } break;
        }
...
void StereoFrom0D(OBMol *mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::StereoFrom0D",
                        obAuditMsg);

  std::vector<unsigned int> symmetry_classes = FindSymmetry(mol);
  OBStereoUnitSet stereogenicUnits =
      FindStereogenicUnits(mol, symmetry_classes);
...
std::vector<OBTetrahedralStereo *>
TetrahedralFrom0D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
                  bool addToMol) {
  std::vector<OBTetrahedralStereo *> configs;
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::TetrahedralFrom0D",
                        obAuditMsg);
  ...
  std::vector<OBGenericData *> stereoData =
      mol->GetAllData(OBGenericDataType::StereoData);
  for (data = stereoData.begin(); data != stereoData.end(); ++data) {
    if (static_cast<OBStereoBase *>(*data)->GetType() ==
      ...
      if (isStereogenic) {
        ...
        obErrorLog.ThrowError(__FUNCTION__,
                              "Removed spurious TetrahedralStereo object",
                              obAuditMsg);
        mol->DeleteData(ts);
      }
    }
  ...
  for (u = stereoUnits.begin(); u != stereoUnits.end(); ++u) {
    ...
    if ((config.refs.size() == 2))
      config.refs.push_back(
          OBStereo::ImplicitRef); // need to add largest number on end to work
...
std::vector<OBCisTransStereo *>
CisTransFrom0D(OBMol *mol, const OBStereoUnitSet &stereoUnits, bool addToMol) {
  std::vector<OBCisTransStereo *> configs;
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::CisTransFrom0D",
                        obAuditMsg);

  std::vector<unsigned long> bonds;
  for (OBStereoUnitSet::const_iterator u = stereoUnits.begin();
       u != stereoUnits.end(); ++u)
    ...
    if (static_cast<OBStereoBase *>(*data)->GetType() == OBStereo::CisTrans) {
      ...
      if (std::find(bonds.begin(), bonds.end(), id) == bonds.end()) {
        ...
        obErrorLog.ThrowError(
            __FUNCTION__, "Removed spurious CisTransStereo object", obAuditMsg);
        mol->DeleteData(ct);
      } else {
        existingMap[id] = ct;
...
void StereoFrom3D(OBMol *mol, bool force) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::StereoFrom3D",
                        obAuditMsg);
...
double VolumeSign(const vector3 &a, const vector3 &b, const vector3 &c,
                  const vector3 &d) {
  vector3 A, B, C;
  A = b - a;
  B = c - a;
...
std::vector<OBTetrahedralStereo *>
TetrahedralFrom3D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
                  bool addToMol) {
  std::vector<OBTetrahedralStereo *> configs;
  OBUnitCell *uc = (OBUnitCell *)mol->GetData(OBGenericDataType::UnitCell);
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::TetrahedralFrom3D",
                        obAuditMsg);
  ...
  for (i = centers.begin(); i != centers.end(); ++i) {
    ...
    if (center->GetHvyDegree() < 3) {
      ...
      errorMsg << "Cannot calculate a signed volume for an atom with a heavy "
                  "atom valence of "
    ...
    if (uc)
      nbrCoords.push_back(
          uc->UnwrapCartesianNear(from->GetVector(), center_coord));
    else
      nbrCoords.push_back(from->GetVector());
    for (OBStereo::RefIter id = config.refs.begin(); id != config.refs.end();
         ++id) {
      OBAtom *nbr = mol->GetAtomById(*id);
      if (uc)
        nbrCoords.push_back(
            uc->UnwrapCartesianNear(nbr->GetVector(), center_coord));
      else
    ...
    for (std::vector<vector3>::iterator coord = nbrCoords.begin(); coord !=
    nbrCoords.end(); ++coord) {
      ...
      if (coord->IsApprox(VZero, 1.0e-6)) {
        if (!use_central_atom) {
          ...
          obErrorLog.ThrowError(__FUNCTION__,
              "More than 2 neighbours have 0 co-ords when attempting 3D chiral
    calculation", obInfo);
        }
      }
    }
    */
    ...
    // If we have three heavy atoms we can use the chiral center atom itself for
    // the fourth will always give same sign (for tetrahedron), magnitude will
    // be smaller.
    ...
    double sign =
        VolumeSign(nbrCoords[0], nbrCoords[1], nbrCoords[2], nbrCoords[3]);
    if (sign < 0.0)
...
std::vector<OBCisTransStereo *>
CisTransFrom3D(OBMol *mol, const OBStereoUnitSet &stereoUnits, bool addToMol) {
  std::vector<OBCisTransStereo *> configs;
  OBUnitCell *uc = (OBUnitCell *)mol->GetData(OBGenericDataType::UnitCell);
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::CisTransFrom3D",
                        obAuditMsg);
  ...
  for (i = bonds.begin(); i != bonds.end(); ++i) {
    ...
    FOR_NBORS_OF_ATOM(nbr, begin) {
      ...
      if (uc)
        bondVecs.push_back(
            uc->MinimumImageCartesian(nbr->GetVector() - begin->GetVector()));
      else
    ...
    if (config.refs.size() == 1) {
      ...
      // WARNING: GetNewBondVector code has not yet been checked, since it's
      // part of builder.cpp
      if (uc)
    ...
    FOR_NBORS_OF_ATOM(nbr, end) {
      ...
      if (uc)
        bondVecs.push_back(
            uc->MinimumImageCartesian(nbr->GetVector() - end_vec));
      else
    ...
    if (nBeginVecs != 2 || bondVecs.size() != 4) {
      obErrorLog.ThrowError(
          __FUNCTION__, "Cannot determine cis/trans: unexpected neighbor count",
          obInfo);
      continue;
    }
    ...
    if (uc) {
      ...
      tor02 =
          CalcTorsionAngle(begin->GetVector() + bondVecs[0], begin->GetVector(),
                           end->GetVector(), end->GetVector() + bondVecs[2]);
      tor03 =
          CalcTorsionAngle(begin->GetVector() + bondVecs[0], begin->GetVector(),
                           end->GetVector(), end->GetVector() + bondVecs[3]);
      tor12 =
          CalcTorsionAngle(begin->GetVector() + bondVecs[1], begin->GetVector(),
                           end->GetVector(), end->GetVector() + bondVecs[2]);
      tor13 =
          CalcTorsionAngle(begin->GetVector() + bondVecs[1], begin->GetVector(),
                           end->GetVector(), end->GetVector() + bondVecs[3]);
    ...
    if (std::abs(tor02) < 90.0 && std::abs(tor03) > 90.0) {
      ...
      if (std::abs(tor12) < 90.0 || std::abs(tor13) > 90.0) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "Could not determine cis/trans from 3D "
                              "coordinates, using unspecified",
                              obInfo);
        config.specified = false;
      }
...
void StereoFrom2D(OBMol *mol,
                  std::map<OBBond *, enum OBStereo::BondDirection> *updown,
                  bool force) {
  if (mol->HasChiralityPerceived() && !force)
    return;

  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::StereoFrom2D",
                        obAuditMsg);
...
bool AngleOrder(const vector3 &a, const vector3 &b, const vector3 &c,
...
double GetAngle(OBAtom *a, OBAtom *b, OBAtom *c) {
  ...
  if (fabs(v1.length()) < 1.0e-3 || fabs(v2.length()) < 1.0e-3) {
    return (0.0);
  }
...
std::vector<OBTetrahedralStereo *>
TetrahedralFrom2D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
                  bool addToMol) {
  std::vector<OBTetrahedralStereo *> configs;
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::TetrahedralFrom2D",
                        obAuditMsg);
  ...
  for (i = centers.begin(); i != centers.end(); ++i) {
    ...
    FOR_BONDS_OF_ATOM(bond, center) {
      ...
      if (bond->IsHash()) {
        ...
        if (!tiponly ||
            (tiponly && bond->GetBeginAtom()->GetId() == center->GetId())) {
          config.specified = true;
    ...
    if (planeAtoms.size() == 4 ||
        (planeAtoms.size() == 3 && center->GetExplicitDegree() == 3))
      config.specified = false;
    ...
    using namespace std;
    if (!config.specified ||
        (config.specified && config.winding == OBStereo::UnknownWinding)) {
      ...
      if (hashAtoms.size() == 4 || wedgeAtoms.size() == 4) {
        success = false;
      } else if (planeAtoms.size() + hashAtoms.size() + wedgeAtoms.size() ==
                 4) {
        // Handle all explicit tetra with at least one stereobond
        vector<OBAtom *> order;

        // First of all, handle the case of three wedge (or three hash) and one
        // other bond
        //          by converting it into a single hash (or single wedge) and
        //          three planes
        ...
        if (AngleOrder(order[0]->GetVector(), order[1]->GetVector(),
                       nbrs[1]->GetVector(), center->GetVector()))
          order.push_back(nbrs[1]);
        else
          order.insert(order.begin() + 1, nbrs[1]);
        if (AngleOrder(order[0]->GetVector(), order[2]->GetVector(),
                       nbrs[2]->GetVector(), center->GetVector()))
        ...
        // Handle the case of two planes with a wedge and hash bond opposite
        // each other. This is handled as in the InChI TechMan (Figure 9) by
        // marking it ambiguous if the (small) angle between the plane bonds is
        // > 133, and basing the stereo on the 'inner' bond otherwise. This is
        // commonly used for stereo in rings. See also
        // Get2DTetrahedralAmbiguity() in ichister.c (part of InChI)
        if (planeAtoms.size() == 2 &&
            wedgeAtoms.size() == 1) {     // Two planes, 1 wedge, 1 hash
          if (order[2] == hashAtoms[0]) { // The wedge and hash are opposite
            double angle = GetAngle(
                order[1], center,
                order[3]); // The anticlockwise angle between the plane atoms
            if (angle > -133 &&
                angle < 133) { // This value is from the InChI TechMan Figure 9
              if (angle > 0) { // Change to three planes and the hash bond
                std::rotate(order.begin(), order.begin() + 2,
                            order.end()); // Change the order so that it begins
                                          // with the hash bond
                wedge = false;
                planeAtoms.push_back(wedgeAtoms[0]);
                wedgeAtoms.clear();
              } else { // Change to three planes and the wedge bond (note: order
                       // is already correct)
                planeAtoms.push_back(hashAtoms[0]);
                hashAtoms.clear();
              }
            } // No need for "else" statement, as this will be picked up as
              // ambiguous stereo below
          }
        }
        ...
        // If the first is a wedge bond, then the next should be a plane/hash,
        // then plane/wedge, then plane/hash If not, then the stereo is
        ...
        if (std::find(pwedge->begin(), pwedge->end(), order[1]) !=
                pwedge->end() ||
            std::find(phash->begin(), phash->end(), order[2]) != phash->end() ||
            std::find(pwedge->begin(), pwedge->end(), order[3]) !=
                pwedge->end()) { // Ambiguous stereo
          success = false;
        }
      } else // 3 explicit bonds from here on
        if (hashAtoms.size() == 0 || wedgeAtoms.size() == 0) {
          // Composed of just wedge bonds and plane bonds, or just hash bonds
          // and plane bonds
          ...
          // Handle the case of two planes with a wedge/hash in the small angle
          // between them. This is handled similar to the InChI TechMan (Figure
          // 10) by treating the stereo bond as being in the large angle. This
          // is consistent with Symyx Draw.
          ...
          if (!wedge)
            config.winding = OBStereo::AntiClockwise;
        } else { // 3 explicit bonds with at least one hash and at least one
                 // wedge
          success = false;
        }
    ...
    if (!success) {
      //         std::stringstream errorMsg;
      //         errorMsg << "Symmetry analysis found atom with id " <<
      //         center->GetId()
      //             << " to be a tetrahedral atom but the wedge/hash bonds
      //             can't be interpreted." << std::endl
...
std::vector<OBCisTransStereo *>
CisTransFrom2D(OBMol *mol, const OBStereoUnitSet &stereoUnits,
               const std::map<OBBond *, enum OBStereo::BondDirection> *updown,
               bool addToMol) {
  std::vector<OBCisTransStereo *> configs;
  std::map<OBBond *, enum OBStereo::BondDirection>::const_iterator ud_cit;
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::CisTransFrom2D",
                        obAuditMsg);
  ...
  for (i = bonds.begin(); i != bonds.end(); ++i) {
    ...
    if (config.specified == true) { // Work out the stereochemistry
      ...
      // 1      2
      /*
      double sign1 = TriangleSign(begin->GetVector(), end->GetVector(),
      bondVecs[0]); double sign2 = TriangleSign(begin->GetVector(),
...
bool TetStereoToWedgeHash(
    OBMol &mol, std::map<OBBond *, enum OBStereo::BondDirection> &updown,
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::Tetrahedral) {
      ...
      if (cfg.specified) {
        ...
        //    (this is acceptable, as the wedge is only at one end, but will
        //    only confuse things)
        ...
        // gave incorrect results when score < 0 and max_bond_score was an
        // unsigned int see
        // https://stackoverflow.com/questions/5416414/signed-unsigned-comparisons#5416498
        FOR_BONDS_OF_ATOM(b, center) {
          ...
          if (tetcenters.find(nbr->GetId()) ==
              tetcenters.end()) // Not a tetcenter
            score += 4;
          if (nbr_nbonds == 1) // terminal atom...
            score += 8;        // strongly prefer terminal atoms
          else
            score -=
                nbr_nbonds - 2; // bond to atom with many bonds is penalized
        ...
        if (chosen == nullptr) { // There is a remote possibility of this but
                                 // let's worry about 99.9% of cases first
          obErrorLog.ThrowError(__FUNCTION__,
                                "Failed to set stereochemistry as unable to "
                                "find an available bond",
                                obError);
          return false;
        }
        alreadyset.insert(chosen);

        // Determine whether this bond should be set hash or wedge (or indeed
        // unknown) (Code inspired by perception.cpp, TetrahedralFrom2D: plane1
        // + plane2 + plane3, wedge)
        ...
        if (cfg.winding != OBStereo::UnknownWinding) {
          ...
          if (test_cfg.from != OBStereo::ImplicitRef) {
            OBStereo::RefIter ri =
                std::find(test_cfg.refs.begin(), test_cfg.refs.end(),
                          (unsigned long)OBStereo::ImplicitRef);
            if (ri != test_cfg.refs.end()) {
              test_cfg = OBTetrahedralStereo::ToConfig(test_cfg,
          ...
          if (implicit) {
            ...
            while (test_cfg.refs[1] != chosen->GetNbrAtom(center)->GetId())
              std::rotate(test_cfg.refs.begin(), test_cfg.refs.begin() + 2,
                          test_cfg.refs.end());
            if (uc)
              anticlockwise_order =
                  AngleOrder(uc->UnwrapCartesianNear(
                                 mol.GetAtomById(test_cfg.refs[0])->GetVector(),
                                 center_coord),
                             uc->UnwrapCartesianNear(
              ...
              anticlockwise_order =
                  AngleOrder(mol.GetAtomById(test_cfg.refs[0])->GetVector(),
                             mol.GetAtomById(test_cfg.refs[1])->GetVector(),
                             mol.GetAtomById(test_cfg.refs[2])->GetVector(),
                             center->GetVector());
            ...
            if ((angle < 0 && anticlockwise_order) ||
                (angle > 0 && !anticlockwise_order)) // Is the stereobond in the
                                                     // bigger angle?
              // If the bonds are in anticlockwise order, a clockwise angle
              // (<180) between plane bonds implies that the stereo bond is in
              // the bigger angle. Otherwise it has the opposite meaning.
              useup = anticlockwise_order;
            else
              useup = !anticlockwise_order;
          } else {
            test_cfg = OBTetrahedralStereo::ToConfig(
                test_cfg, chosen->GetNbrAtom(center)->GetId());
            if (uc)
              anticlockwise_order =
                  AngleOrder(uc->UnwrapCartesianNear(
                                 mol.GetAtomById(test_cfg.refs[0])->GetVector(),
          ...
          // Set to UpBond (filled wedge from cfg.center to chosen_nbr) or
          // DownBond
...
set<OBBond *> GetUnspecifiedCisTrans(OBMol &mol) {
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::CisTrans) {
      ...
      if (!cfg.specified) {
        OBBond *dbl_bond =
            mol.GetBond(mol.GetAtomById(cfg.begin), mol.GetAtomById(cfg.end));
...
void StereoRefToImplicit(OBMol &mol, OBStereo::Ref atomId) {
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (datatype != OBStereo::CisTrans && datatype != OBStereo::Tetrahedral) {
      ...
      obErrorLog.ThrowError(
          __FUNCTION__,
          "This function should be updated to handle additional stereo "
          "types.\nSome stereochemistry objects may contain explicit refs to "
          "hydrogens which have been removed.",
          obWarning);
      continue;
    }
    ...
    if (datatype == OBStereo::CisTrans) {
      ...
      replace(ct_cfg.refs.begin(), ct_cfg.refs.end(), atomId,
      ...
      if (ts_cfg.from == atomId)
        ts_cfg.from = OBStereo::ImplicitRef;
      replace(ts_cfg.refs.begin(), ts_cfg.refs.end(), atomId,
              (OBStereo::Ref)OBStereo::ImplicitRef);
      ts->SetConfig(ts_cfg);
    }
  }
...
void ImplicitRefToStereo(OBMol &mol, OBStereo::Ref centerId,
                         OBStereo::Ref newId) {
  std::vector<OBGenericData *> vdata =
      mol.GetAllData(OBGenericDataType::StereoData);
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (datatype != OBStereo::CisTrans && datatype != OBStereo::Tetrahedral) {
      ...
      obErrorLog.ThrowError(
          __FUNCTION__,
          "This function should be updated to handle additional stereo "
          "types.\nSome stereochemistry objects may contain implicit refs to "
          "hydrogens which need to be converted to explicit.",
          obWarning);
      continue;
    }
    ...
    if (datatype == OBStereo::CisTrans) {
      ...
      if (ct_cfg.begin == centerId || ct_cfg.end == centerId) {
        // Assumption: the first two refs are on the begin atom, the last two on
        // the end atom
        if (ct_cfg.begin == centerId)
          replace(ct_cfg.refs.begin(), ct_cfg.refs.begin() + 2,
                  (OBStereo::Ref)OBStereo::ImplicitRef, (OBStereo::Ref)newId);
        if (ct_cfg.end == centerId)
          replace(ct_cfg.refs.begin() + 2, ct_cfg.refs.end(),
                  (OBStereo::Ref)OBStereo::ImplicitRef, (OBStereo::Ref)newId);
        ct->SetConfig(ct_cfg);
      }
    } else if (datatype == OBStereo::Tetrahedral) {
      ...
      if (ts_cfg.center == centerId) {
        if (ts_cfg.from == OBStereo::ImplicitRef)
          ts_cfg.from = newId;
        replace(ts_cfg.refs.begin(), ts_cfg.refs.end(),
                (OBStereo::Ref)OBStereo::ImplicitRef, (OBStereo::Ref)newId);
        ts->SetConfig(ts_cfg);
      }
    }
