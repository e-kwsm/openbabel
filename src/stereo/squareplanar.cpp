...
OBSquarePlanarStereo::OBSquarePlanarStereo(OBMol *mol)
    : OBTetraPlanarStereo(mol) {}
...
void OBSquarePlanarStereo::SetConfig(const Config &config) {
  if (config.center == OBStereo::NoRef) {
    obErrorLog.ThrowError(
        __FUNCTION__, "OBSquarePlanarStereo::SetConfig : center id is invalid.",
        obError);
    m_cfg = Config();
    return;
  }
...
OBSquarePlanarStereo::Config
OBSquarePlanarStereo::GetConfig(OBStereo::Shape shape) const {
  if (!IsValid())
    return Config();
...
OBSquarePlanarStereo::Config
OBSquarePlanarStereo::GetConfig(unsigned long start,
...
OBSquarePlanarStereo::GetCisRefs(unsigned long id) const {
...
OBGenericData *OBSquarePlanarStereo::Clone(OBBase *mol) const {
  OBSquarePlanarStereo *data =
      new OBSquarePlanarStereo(static_cast<OBMol *>(mol));
