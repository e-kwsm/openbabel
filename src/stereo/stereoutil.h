...
OBStereoUnitSet
orderSetBySymmetryClasses(OBMol *mol, const OBStereoUnitSet &set,
...
unsigned int getSymClass(const OBStereoUnit &unit, OBMol *mol,
                         const std::vector<unsigned int> &symmetry_classes);
...
void IdsToSymClasses(OBMol *mol, OBTetrahedralStereo::Config &config,
                     const std::vector<unsigned int> &symClasses);
bool setsContainSameSymmetryClasses(
    OBMol *mol, const OBStereoUnitSet &set1, const OBStereoUnitSet &set2,
    const std::vector<unsigned int> &symmetry_classes);
void orderSetByFragmentRecursive(OBStereoUnitSet &ordered, OBAtom *atom,
                                 OBAtom *skip, const OBStereoUnitSet &set,
                                 OBBitVec &fragment);
OBStereoUnitSet orderSetByFragment(OBAtom *atom, OBAtom *skip,
                                   const OBStereoUnitSet &set);
OBStereoUnitSet orderSetByRing(OBMol *mol, const OBBitVec &fragment,
                               const OBStereoUnitSet &set);
int findDescriptor(const OBTetrahedralStereo::Config &config);
int findDescriptor(const OBCisTransStereo::Config &config);
int findDescriptor(OBAtom *center,
                   const std::vector<unsigned int> &symmetry_classes);
bool isUnitResolved(OBMol *mol, const OBStereoUnit &unit,
                    const std::vector<unsigned int> &symmetry_classes);
int findDescriptor(OBBond *bond,
                   const std::vector<unsigned int> &symmetry_classes);
std::vector<int>
findDescriptorVector(OBMol *mol, const OBBitVec &fragment,
                     const OBStereoUnitSet &orderedUnits,
                     const std::vector<unsigned int> &symmetry_classes);
int findDescriptorVectorValue(
    OBMol *mol, const OBBitVec &fragment, const OBStereoUnitSet &orderedUnits,
    const std::vector<unsigned int> &symmetry_classes);
OBStereoUnitSet findSetContainingUnit(const OBStereoUnitSetOfSets &sets,
                                      const OBStereoUnit &unit);
...
template <> struct OBStereoTypeTraits<OBStereo::Tetrahedral> {
  ...
  static Center *GetCenter(OBMol *mol, unsigned long id) {
    return mol->GetAtomById(id);
  }
};
template <> struct OBStereoTypeTraits<OBStereo::CisTrans> {
  ...
  static Center *GetCenter(OBMol *mol, unsigned long id) {
    return mol->GetBondById(id);
  }
};

template <typename OBAtomOrOBBond> struct OBStereoCenterTraits;
template <>
struct OBStereoCenterTraits<OBAtom>
    : OBStereoTypeTraits<OBStereo::Tetrahedral> {};
template <>
...
bool isInSet(OBAtomOrOBBond *atomOrBond, const OBStereoUnitSet &units) {
  for (std::size_t i = 0; i < units.size(); ++i) {
    ...
    if (unit.type !=
        static_cast<OBStereo::Type>(OBStereoCenterTraits<OBAtomOrOBBond>::Type))
      continue;
...
int findDescriptor(OBMol *mol, const OBStereoUnit &unit,
                   const std::vector<unsigned int> &symmetry_classes,
                   OBStereoFacade &stereoFacade);
int classifyNbrSymClasses(OBMol *mol, const OBStereoUnit &unit,
                          const std::vector<unsigned int> &symmetry_classes);
bool isResolved(OBMol *mol, const OBStereoUnit &unit,
                const std::vector<unsigned int> &symClasses);
