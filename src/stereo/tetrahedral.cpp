...
bool OBTetrahedralStereo::Config::operator==(
    const OBTetrahedralStereo::Config &other) const {
  ...
  if (from == OBStereo::ImplicitRef) {
    thisConfig =
        OBTetraNonPlanarStereo::ToConfig(*this, refs[0], winding, view);
    otherConfig =
        OBTetraNonPlanarStereo::ToConfig(other, thisConfig.from, winding, view);
  } else if (other.from == OBStereo::ImplicitRef) {
    otherConfig =
        OBTetraNonPlanarStereo::ToConfig(other, other.refs[0], winding, view);
    thisConfig = OBTetraNonPlanarStereo::ToConfig(*this, otherConfig.from,
                                                  winding, view);
  } else {
    thisConfig = *this;
    otherConfig =
...
void OBTetrahedralStereo::SetConfig(const Config &config) {
  ...
  if (config.from == OBStereo::NoRef) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "OBTetrahedralStereo::SetConfig : from/towards atom id is invalid.",
        obError);
    m_cfg = Config();
    return;
  }
...
OBTetrahedralStereo::GetConfig(OBStereo::Winding winding,
                               OBStereo::View view) const {
  if (!IsValid())
    return Config();
  ...
  if (m_cfg.winding != OBStereo::UnknownWinding)
    ...
    return OBTetraNonPlanarStereo::ToConfig(m_cfg, m_cfg.from,
                                            OBStereo::UnknownWinding, view);
}

OBTetrahedralStereo::Config
OBTetrahedralStereo::GetConfig(unsigned long from_or_towards,
  ...
  if (m_cfg.winding != OBStereo::UnknownWinding)
    return OBTetraNonPlanarStereo::ToConfig(m_cfg, from_or_towards, winding,
                                            view);
  else
    return OBTetraNonPlanarStereo::ToConfig(m_cfg, from_or_towards,
                                            OBStereo::UnknownWinding, view);
}
...
OBGenericData *OBTetrahedralStereo::Clone(OBBase *mol) const {
  OBTetrahedralStereo *data =
      new OBTetrahedralStereo(static_cast<OBMol *>(mol));
...
ostream &operator<<(ostream &out, const OBTetrahedralStereo::Config &cfg) {
  ...
  out << ", refs = ";
  for (OBStereo::Refs::const_iterator i = cfg.refs.begin(); i != cfg.refs.end();
       ++i)
    if (*i != OBStereo::ImplicitRef)
      out << *i << " ";
    else
