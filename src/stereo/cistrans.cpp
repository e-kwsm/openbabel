...
bool OBCisTransStereo::Config::operator==(const Config &other) const {
  ...
  if (!OBStereo::ContainsSameRefs(refs, other.refs)) {
    ...
    for (OBStereo::ConstRefIter i = refs.begin(); i != refs.end(); ++i)
      if (OBStereo::ContainsRef(other.refs, *i)) {
        u1 = OBTetraPlanarStereo::ToConfig(
            *this, *i, OBStereo::ShapeU); // refs[0] = u1.refs[0]
        u2 = OBTetraPlanarStereo::ToConfig(
            other, *i, OBStereo::ShapeU); // refs[0] = u2.refs[0]
      }
    ...
    u1 = OBTetraPlanarStereo::ToConfig(
        *this, refs.at(0), OBStereo::ShapeU); // refs[0] = u1.refs[0]
    u2 = OBTetraPlanarStereo::ToConfig(
        other, refs.at(0), OBStereo::ShapeU); // refs[0] = u2.refs[0]
    // both now start with the same ref
    //
    // 2 possiblilities:
    //
    //   1 2 3 4      1 2 3 4
    //   |   |        |   |      <- in any case, refs[0] & refs[2] remain
    //   unchanged 1 2 3 4      1 4 3 2
    //
  ...
  //   1 2 H H
  if ((u1.refs[2] == OBStereo::ImplicitRef) ||
      (u2.refs[2] == OBStereo::ImplicitRef)) {
    // 1 2 H 4
    if ((u1.refs[3] == OBStereo::ImplicitRef) ||
        (u2.refs[3] == OBStereo::ImplicitRef)) {
...
void OBCisTransStereo::SetConfig(const Config &config) {
  if (config.begin == OBStereo::NoRef) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "OBCisTransStereo::SetConfig : double bond begin id is invalid.",
        obError);
    m_cfg = Config();
    return;
  }
  if (config.end == OBStereo::NoRef) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "OBCisTransStereo::SetConfig : double bond end id is invalid.",
        obError);
    m_cfg = Config();
    return;
  }
  ...
  // store using U shape
  m_cfg = OBTetraPlanarStereo::ToConfig(config, config.refs.at(0),
                                        OBStereo::ShapeU);
}

OBCisTransStereo::Config
OBCisTransStereo::GetConfig(OBStereo::Shape shape) const {
  if (!IsValid())
    return Config();
...
OBCisTransStereo::Config
OBCisTransStereo::GetConfig(unsigned long start, OBStereo::Shape shape) const {
  if (!IsValid())
    return Config();
...
bool OBCisTransStereo::operator==(const OBCisTransStereo &other) const {
  ...
  Config u = OBTetraPlanarStereo::ToConfig(other.GetConfig(), m_cfg.refs.at(0),
                                           OBStereo::ShapeU);
...
unsigned long OBCisTransStereo::GetCisOrTransRef(unsigned long id,
                                                 bool getcisref) const {
  if (!IsValid())
...
bool OBCisTransStereo::IsOnSameAtom(unsigned long id1,
                                    unsigned long id2) const {
  const OBMol *mol = GetMolecule();
  if (!mol) {
    obErrorLog.ThrowError(
        __FUNCTION__, "OBCisTransStereo::IsOnSameAtom : No valid molecule set",
        obError);
    return false;
  }
  ...
  if (!begin) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "OBCisTransStereo::IsOnSameAtom : Begin reference id is not valid.",
        obError);
    return false;
  }
  ...
  if (!end) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "OBCisTransStereo::IsOnSameAtom : End reference id is not valid.",
        obError);
    return false;
  }
  ...
  if (a && b) {
    ...
    if (a) {
      ...
      if (a->IsConnected(begin)) {
        // a is connected to begin. if this is the atom missing a hydrogen,
        // return false
        ...
        if (end->GetExplicitDegree() != 2) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "OBCisTransStereo::IsOnSameAtom : id2 is not "
                                "valid and is not a missing hydrogen.",
                                obError);
          return false;
        }
        ...
        obErrorLog.ThrowError(
            __FUNCTION__,
            "OBCisTransStereo::IsOnSameAtom : Atom with id2 doesn't exist "
            "anymore, must be a (deleted) hydrogen.",
            obInfo);
      } else if (a->IsConnected(end)) {
        // a is connected to end. again, if this is the atom missing a hydrogen,
        // return false
        if (end->GetExplicitDegree() == 2)
          return true;
        ...
        // inform user we are treating id2 as deleted hydrogen
        obErrorLog.ThrowError(__FUNCTION__,
                              "OBCisTransStereo::IsOnSameAtom : Atom with id2 "
                              "doesn't exist, must be a (deleted) hydrogen.",
                              obInfo);

      } else {
        obErrorLog.ThrowError(__FUNCTION__,
                              "OBCisTransStereo::IsOnSameAtom : Atom with id1 "
                              "isn't connected to the begin or end atom.",
                              obError);
        return true;
      }
    } else if (b) {
      ...
      if (b->IsConnected(begin)) {
        // b is connected to begin. if this is the atom missing a hydrogen,
        // return false
        if (begin->GetExplicitDegree() == 2)
          return true;
        // check if the end atom really is missing an atom
        if (end->GetExplicitDegree() != 2) {
          obErrorLog.ThrowError(__FUNCTION__,
                                "OBCisTransStereo::IsOnSameAtom : id1 is not "
                                "valid and is not a missing hydrogen.",
                                obError);
          return true;
        }
      ...
      if (!c || !d) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "OBCisTransStereo::IsOnSameAtom : invalid stereochemistry!",
            obError);
        return true;
      }
      if ((begin->GetExplicitDegree() != 2) ||
          (end->GetExplicitDegree() != 2)) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "OBCisTransStereo::IsOnSameAtom : invalid stereochemistry!",
            obError);
      ...
      obErrorLog.ThrowError(__FUNCTION__,
                            "OBCisTransStereo::IsOnSameAtom : Atoms with id1 & "
                            "id2 don't exist, must be a (deleted) hydrogens.",
                            obInfo);
...
ostream &operator<<(ostream &out, const OpenBabel::OBCisTransStereo &ct) {
  ...
  out << ", refs = ";
  for (OpenBabel::OBStereo::Refs::iterator i = cfg.refs.begin();
       i != cfg.refs.end(); ++i)
...
ostream &operator<<(ostream &out,
  ...
  out << ", refs = ";
  for (OpenBabel::OBStereo::Refs::const_iterator i = cfg.refs.begin();
       i != cfg.refs.end(); ++i)
    if (*i != OpenBabel::OBStereo::ImplicitRef)
      out << *i << " ";
    else
