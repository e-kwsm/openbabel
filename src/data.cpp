...
void OBAtomicHeatOfFormationTable::ParseLine(const char *line) {
  ...
  if (strlen(line) > 0) {
    tokenize(vs, line, "|");
    if (vs.size() >= 8) {
      oba = new OBAtomHOF(vs[0], atoi(vs[1].c_str()), vs[2], vs[3],
                          atof(vs[4].c_str()), atof(vs[5].c_str()),
                          atoi(vs[6].c_str()), vs[7]);
...
/** \class OBTypeTable data.h <openbabel/data.h>
    ...
    for (atom = mol.BeginAtom(i);atom;atom = mol.EndAtom(i))
    {
       ...
       cout << "atom number " << atom->GetIdx() << "has mol2 type " << dst <<
   endl;
    }
    \endcode

    Current atom types include (defined in the top line of the data file
   types.txt):
...
bool OBTypeTable::Translate(string &to, const string &from) {
  ...
  obErrorLog.ThrowError(__FUNCTION__,
                        "Cannot perform atom type translation: table cannot "
                        "find requested types.",
                        obWarning);
  to = from;
  return (false);
}
