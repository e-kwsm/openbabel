...
void OBReactionFacadePrivate::AddComponent(OBMol *mol, OBReactionRole rxnrole) {
  ...
  if (!_unassigned_components.empty() &&
      _unassigned_components.back() > max_compid)
...
void OBReactionFacadePrivate::FindComponents() {
  ...
  // Convert to vector so we have random access - note: these will be in sorted
  // order
  ...
  for (std::set<unsigned int>::iterator sit = product_components.begin();
       sit != product_components.end(); ++sit)
    _product_components.push_back(*sit);
  for (std::set<unsigned int>::iterator sit = agent_components.begin();
       sit != agent_components.end(); ++sit)
...
bool OBReactionFacadePrivate::GetComponent(OBMol *mol, OBReactionRole rxnrole,
                                           unsigned int num) {
...
OBReactionFacadePrivate::GetComponentIds(OBReactionRole rxnrole) {
  if (!_found_components)
    FindComponents();
...
bool OBReactionFacadePrivate::IsValid() {
  if (!_mol->IsReaction()) {
    obErrorLog.ThrowError(
        __FUNCTION__,
        "The molecule is not marked as a reaction. Use SetIsReaction().",
        obWarning);
    return false;
  }
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    if (!data) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "The molecule contains an atom that is missing a "
                            "reaction component Id. Use SetComponentId().",
                            obWarning);
      return false;
    }
    ...
    if (!pi) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "A reaction component Id has been stored using a "
                            "data type that is not an OBPairInteger.",
                            obWarning);
      return false;
    }
    ...
    if (val <= 0) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Reaction component Ids should all be non-zero positive integers.",
          obWarning);
      return false;
    }
    ...
    if (!data) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "The molecule contains an atom that is missing "
                            "reaction role information. Use SetRole().",
                            obWarning);
      return false;
    }
    pi = dynamic_cast<OBPairInteger *>(data);
    if (!pi) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Reaction role information has been stored using a "
                            "data type that is not an OBPairInteger.",
                            obWarning);
      return false;
    }
    ...
    if (val < 0 || val > 3) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Reaction roles should be in the range 0 to 3 inclusive.", obWarning);
      return false;
    }
  }

  // Ensure that every atom in a particular connected component has the same
  // component Id and rxn role
  OBMolAtomDFSIter iter(_mol);
  while (iter) { // for each connected component
    ...
    do { // for each atom in connected component
      ...
      if (my_rxncomp != rxncomp) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "The molecule contains a connected component that contains atoms "
            "with different reaction component Ids. All atoms in a particular "
            "connected component should have the same value.",
            obWarning);
        return false;
      }
      ...
      if (my_rxnrole != rxnrole) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "The molecule contains a connected component that contains atoms "
            "with different reaction roles. All atoms in a particular "
            "connected component should have the same role.",
            obWarning);
        return false;
      }
    } while ((iter++).next());
...
bool OBReactionFacadePrivate::ReassignComponent(OBReactionRole oldrole,
                                                unsigned int num,
                                                OBReactionRole newrole) {
...
void OBReactionFacadePrivate::SetId(const char *idtype, OBAtom *atom,
                                    int idval) {
...
void OBReactionFacadePrivate::SetComponentId(OBAtom *atom,
                                             unsigned int compid) {
...
OBReactionFacade::OBReactionFacade(OBMol *mol)
    : d(new OBReactionFacadePrivate(mol)){};
OBReactionFacade::~OBReactionFacade() { delete d; }
...
void OBReactionFacade::ClearInternalState() { d->ClearInternalState(); }
bool OBReactionFacade::GetComponent(OBMol *mol, OBReactionRole rxnrole,
                                    unsigned int num) {
...
bool OBReactionFacade::IsValid() { return d->IsValid(); }
unsigned int OBReactionFacade::NumComponents(OBReactionRole rxnrole) {
  return d->NumComponents(rxnrole);
}
bool OBReactionFacade::ReassignComponent(OBReactionRole oldrole,
                                         unsigned int num,
                                         OBReactionRole newrole) {
