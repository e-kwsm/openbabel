...
ASCIIPainter::ASCIIPainter(int width, int height, double aspect)
    : m_width(width), m_height(height), m_aspect(aspect), m_scale(1) {
  m_buf.clear();
...
bool ASCIIPainter::IsGood() const { return true; }

void ASCIIPainter::SetFontSize(int /*pointSize*/) {}

void ASCIIPainter::SetFillColor(const OBColor & /*color*/) {}

void ASCIIPainter::SetFillRadial(const OBColor & /*start*/,
                                 const OBColor & /*end*/) {}
...
double ASCIIPainter::GetPenWidth() { return 0.0; }
...
void ASCIIPainter::DrawLine(double x1, double y1, double x2, double y2,
  ...
  string symbols =
      Bresenham(round(x1 * m_scale), round(y1 * m_scale / m_aspect),
...
void ASCIIPainter::DrawPolygon(
  ...
  for (vp_it = points.begin(); vp_it != (points.end() - 1); ++vp_it) {
    DrawLine(vp_it->first, vp_it->second, (vp_it + 1)->first,
             (vp_it + 1)->second);
  }
  DrawLine(vp_it->first, vp_it->second, points.begin()->first,
           points.begin()->second);
}
...
OBFontMetrics ASCIIPainter::GetFontMetrics(const std::string & /*text*/) {
  ...
  // -  the correct value is 1.0 / m_scale * m_aspect, and we round to the
  // nearest integer.
...
int getdelta(int x, int y, int x2, int y2) {
  int ans = 0;
  if (x2 == x)
    return ans; // Not applicable to vertical lines
  if (y2 > y) {
    ans = 1;
...
// Implementation of Bresenham line generation, a standard way of rasterising
// lines. Returns a string with symbols for the line along with the coords.
string ASCIIPainter::Bresenham(int x, int y, int x2, int y2,
                               vector<pair<int, int>> &coords) {
  ...
  for (int i = 0; i < dx; ++i) {
    ...
    if (steep)
      ...
      int tmp_delta =
          d >= 0 ? delta : 0; // Correction for / and \ in some cases
  ...
  if (coords.size() > 1) {
    ...
    } else if (coords.at(coords.size() - 1).second <
               coords.at(coords.size() - 2).second) {
