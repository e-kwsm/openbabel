...
SVGPainter::SVGPainter(ostream &ofs, std::set<ColorGradient> *gradients,
                       bool withViewBox, double width, double height)
    : m_ofs(ofs), m_withViewBox(withViewBox), m_width(width), m_height(height),
      m_Pencolor("black"), m_Fillcolor("white"),
      m_Gradientcolor(make_pair(OBColor("white"), OBColor("white"))),
      m_PenWidth(1), m_fontPointSize(16), m_isFillcolor(true),
...
void SVGPainter::NewCanvas(double width, double height) {
  ...
  m_ofs << "font-family=\"" << m_fontFamily
        << "\" stroke=" << MakeRGB(m_Pencolor) << "stroke-width=\""
        << m_PenWidth << "\"  stroke-linecap=\"round\""
        << ">\n";

  if (!m_withViewBox &&
      m_Fillcolor.alpha != 0.0) // Background color for single molecule. Handled
                                // by outer svg when table.
...
void SVGPainter::EndCanvas() { m_ofs << "</svg>\n"; }
bool SVGPainter::IsGood() const { return true; }

void SVGPainter::SetFontSize(int pointSize) { m_fontPointSize = pointSize; }
...
double SVGPainter::GetPenWidth() { return m_PenWidth; }
...
void SVGPainter::DrawLine(double x1, double y1, double x2, double y2,
  ...
  m_ofs << fixed << "<line x1=\"" << x1 << "\" y1=\"" << y1 << "\" x2=\"" << x2
        << "\" y2=\"" << y2 << "\"";
  m_ofs << " opacity=\"" << m_Pencolor.alpha << "\"";
  // if(m_Pencolor!=m_OrigBondcolor) // TODO: Bring this line back once Pybel is
  // fine with this
...
void SVGPainter::DrawText(double x, double y, const std::string &text) {
  m_ofs << "<text x=\"" << x << "\" y=\"" << y << "\""
        << " fill=" << MakeRGB(m_Pencolor)
        << "stroke-width=\"0\" font-weight=\"bold\" "
        << "font-size=\"" << m_fontPointSize << "\" >" << text << "</text>\n";
}
...
OBFontMetrics SVGPainter::GetFontMetrics(const std::string &text) {
  ...
  metrics.descent =
      m_fontPointSize * -0.23; // Offset from baseline of bottom of text
  metrics.height =
      m_fontPointSize * 1.26; // Distance between successive lines of text
  metrics.width = 0.0;
...
void SVGPainter::WriteDefs() {
  if (!m_Gradients->empty()) {
    m_ofs << "<defs>\n";
    for (std::set<ColorGradient>::iterator it = m_Gradients->begin();
         it != m_Gradients->end(); ++it) {
      m_ofs << "<radialGradient id='radial";
      m_ofs << RGBcode(it->first) << RGBcode(it->second) << "' ";
      m_ofs << "cx='50%' cy='50%' r='50%' fx='30%' fy='30%'>\n";
      m_ofs << "  <stop offset=' 0%' stop-color=" << MakeRGB(it->first)
            << " stop-opacity='1.0'/>\n";
      m_ofs << "  <stop offset='100%' stop-color=" << MakeRGB(it->second)
            << " stop-opacity ='1.0'/>\n";
...
string SVGPainter::RGBcode(OBColor color) {
  stringstream ss;
  ss << std::hex << std::setfill('0') << std::setw(2) << (int)(255 * color.red)
     << (int)(255 * color.green) << (int)(255 * color.blue);
  return ss.str();
}
} // namespace OpenBabel
