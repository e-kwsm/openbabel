...
// defined in mol.cpp
extern bool SortAtomZ(const pair<OBAtom *, double> &a,
                      const pair<OBAtom *, double> &b);

class OBDepictPrivate {
public:
  OBDepictPrivate()
      : mol(nullptr), painter(nullptr), bondLength(40.0), penWidth(2.0),
  ...
  virtual void DrawSimpleBond(OBAtom *beginAtom, OBAtom *endAtom, int order,
                              bool crossed_bond = false);
  ...
  virtual void DrawRingBond(OBAtom *beginAtom, OBAtom *endAtom,
                            const vector3 &center, int order);
  virtual void DrawAtom(OBAtom *atom);
  virtual void DrawAtomLabel(const std::string &label, int alignment,
                             const vector3 &pos);
...
public:
  OBDepictPrivateBallAndStick(bool symbolOnBall)
      : m_symbolOnBall(symbolOnBall) {}

  bool m_symbolOnBall;

  void DrawSimpleBond(OBAtom *beginAtom, OBAtom *endAtom, int order,
                      bool crossed_bond = false) override;
  ...
  void DrawRingBond(OBAtom *beginAtom, OBAtom *endAtom, const vector3 &center,
                    int order) override;
  void DrawAtom(OBAtom *atom) override;
  void DrawAtomLabel(const std::string &label, int alignment,
...
private:
  void DrawAromaticRingBond(OBAtom *prevAtom, OBAtom *beginAtom,
                            OBAtom *endAtom, OBAtom *nextAtom,
...
void OBDepict::SetBondLength(double length) { d->bondLength = length; }

double OBDepict::GetBondLength() const { return d->bondLength; }
...
double OBDepict::GetPenWidth() const { return d->penWidth; }

void OBDepict::SetBondSpacing(double spacing) { d->bondSpacing = spacing; }

double OBDepict::GetBondSpacing() const { return d->bondSpacing; }

void OBDepict::SetBondWidth(double width) { d->bondWidth = width; }

double OBDepict::GetBondWidth() const { return d->bondWidth; }
...
*/
void OBDepict::SetOption(unsigned opts) { d->options |= opts; }

unsigned OBDepict::GetOptions() const { return d->options; }
void OBDepict::ClearOptions() { d->options = 0; }
...
const std::string &OBDepict::GetFontFamily() const { return d->fontFamily; }
...
void OBDepict::SetAliasMode(bool b) { d->aliasMode = b; }
...
unsigned int GetAtomSymClass(OBAtom *atom) {
  ...
  if (pd) {
    ...
    copy(istream_iterator<unsigned int>(iss), istream_iterator<unsigned int>(),
    ...
    vector<unsigned int>::iterator end_pos =
        unique(copy_sym.begin(), copy_sym.end()); // Requires sorted elements
    int nclasses = end_pos - copy_sym.begin();

    cout << "sym_class[" << atom->GetIndex()
         << "] = " << symmetry_classes.at(atom->GetIndex()) << endl;
...
void OBDepictPrivate::DrawRing(OBRing *ring, OBBitVec &drawnBonds) {
  ...
  for (std::vector<int>::iterator l = indexes.begin(); l != indexes.end();
       ++l) {
...
static Radical AssignRadicalDots(OBAtom *atom) {
  ...
  unsigned int typicalvalence = GetTypicalValence(
      atom->GetAtomicNum(), actualvalence, atom->GetFormalCharge());
...
bool OBDepict::DrawMolecule(OBMol *mol) {
  ...
  if (mol->NumAtoms() > 0) {
    ...
    if (mol->NumBonds() > 0) {
      ...
      for (atom = d->mol->BeginAtom(i); atom; atom = d->mol->NextAtom(i)) {
        ...
        for (atom2 = d->mol->BeginAtom(i2); atom2;
             atom2 = d->mol->NextAtom(i2)) {
          if (atom != atom2) {
    ...
    for (atom = d->mol->BeginAtom(i); atom; atom = d->mol->NextAtom(i))
      atom->SetVector(atom->GetX() - min_x + margin,
                      atom->GetY() - min_y + margin, atom->GetZ());
  ...
  // - note that OBBond->IsInRing() includes bonds not included in the SSSR as
  // the SSSR excludes very large rings
  std::vector<OBRing *> rings(mol->GetSSSR());
  OBBitVec ringBonds;
  for (std::vector<OBRing *>::iterator k = rings.begin(); k != rings.end();
       ++k) {
    OBRing *ring = *k;
    std::vector<int> indexes = ring->_path;
    for (unsigned int l = 0; l < indexes.size(); ++l) {
  ...
  for (OBBond *bond = d->mol->BeginBond(j); bond; bond = d->mol->NextBond(j)) {
    ...
    } else if (!ringBonds.BitIsSet(
                   bond->GetId())) { // Ring bonds are handled below
  ...
  vector<int> zsorted;
  unsigned int a;
  for (a = 0, atom = d->mol->BeginAtom(i); atom;
       atom = d->mol->NextAtom(i), ++a) {
  ...
  for (a = 0; a < max; a++) {
    ...
    if (charge || radical != NOT_RADICAL) {
      ...
      if (charge) {
        ...
        if (charge > 0)
          ss << '+';
        else if (charge < -1) // use underscore for single negative charge and
                              // minus if multiple
          ss << '-';
        else {
          ss << '_';
    ...
    if (atom->GetAtomicNum() == OBElements::Carbon) {
      if (!(d->options & drawAllC)) {
        if (atom->GetExplicitDegree() > 1)
          continue;
        if ((atom->GetExplicitDegree() == 1) &&
            !(d->options & drawTermC)) //! d->drawTerminalC)
          continue;
      }
    }
...
void OBDepictPrivate::DrawSimpleBond(OBAtom *beginAtom, OBAtom *endAtom,
                                     int order, bool crossed_dbl_bond) {
  vector3 begin = beginAtom->GetVector();
  vector3 end = endAtom->GetVector();
  vector3 vb = end - begin;
  ...
  if (order == 1) {
    ...
    if (crossed_dbl_bond)
      useAsymmetricDouble =
          false; // The bond looks very strange otherwise in the case of cis
...
void OBDepictPrivate::DrawRingBond(OBAtom *beginAtom, OBAtom *endAtom,
                                   const vector3 &center, int order) {
  if (order != 2) {
  ...
  painter->DrawLine(begin.x() + spacing.x() + offset.x(),
                    begin.y() + spacing.y() + offset.y(),
                    end.x() + spacing.x() - offset.x(),
                    end.y() + spacing.y() - offset.y());
}

void OBDepictPrivate::DrawAtom(OBAtom *atom) {}

void OBDepictPrivate::DrawAtomLabel(const std::string &label, int alignment,
                                    const vector3 &pos) {
  /*
  ...
  if ((alignment == Right) || (alignment == Left) ||
    ...
    painter->SetFontSize(fontSize);
    totalWidth =
        painter->GetFontMetrics(label.substr(0, label.find("H"))).width;
    double width = 0.0;
  ...
  case Left:
    xOffset =
        -0.5 * painter->GetFontMetrics(label.substr(label.size() - 1, 1)).width;
    break;
  case Up:
  case Down:
  ...
  for (unsigned int i = 0; i < label.size(); ++i) {
    ...
    if (!isalpha(label[i])) {
      ...
      if (!subscript.empty()) {
        ...
        painter->DrawText(pos.x() + xOffset, pos.y() + yOffsetSubscript,
                          subscript);
        xOffset += metrics.width;
        subscript.clear();
      }
  ...
  if (!subscript.empty()) {
    painter->SetFontSize(subscriptSize);
    OBFontMetrics metrics = painter->GetFontMetrics(subscript);
    double yOffset = ispunct(subscript[subscript.size() - 1]) ||
                             ispunct(subscript[0]) ||
...
bool OBDepictPrivate::HasLabel(OBAtom *atom) {
  if (atom->GetAtomicNum() != OBElements::Carbon)
    return true;
  if ((options & OBDepict::drawAllC) ||
      ((options & OBDepict::drawTermC) && (atom->GetExplicitDegree() == 1)))
    return true;
  return false;
}
...
inline void
OBDepictPrivateBallAndStick::DrawAromaticRing(OBRing *ring,
                                              OBBitVec &drawnBonds) {
  ...
  vector3 center(VZero);
  double maxdist = 0.;
  for (std::vector<int>::const_iterator l = indexes.begin(); l != indexes.end();
       ++l) {
    center += mol->GetAtom(*l)->GetVector();
...
inline void OBDepictPrivateBallAndStick::DrawAromaticRingBond(
    OBAtom *prevAtom, OBAtom *beginAtom, OBAtom *endAtom, OBAtom *nextAtom,
    const vector3 &center, double dist) {
  ...
  static const vector<double> pat = vector<double>(
      dashpattern, dashpattern + sizeof(dashpattern) / sizeof(double));
  painter->DrawLine(begin.x() + offset.x(), begin.y() + offset.y(),
...
void OBDepictPrivateBallAndStick::DrawAtom(OBAtom *atom) {
  ...
  painter->DrawBall(atom->GetVector().x(), atom->GetVector().y(),
                    GetAtomRadius(atom), opacity);
}
...
void OBDepictPrivateBallAndStick::DrawAtomLabel(const std::string &label,
                                                int alignment,
                                                const vector3 &pos) {
  if (m_symbolOnBall)
