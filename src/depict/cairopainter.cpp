...
CairoPainter::CairoPainter()
    : m_surface(nullptr), m_cairo(nullptr), m_fontPointSize(12), m_width(0),
      m_height(0), m_pen_width(1), m_title(""), m_index(1),
...
void CairoPainter::NewCanvas(double width, double height) {
  ...
  double scale_y =
      ...
      (double)height; // Leave some extra space for the title if present
  ...
  if (!m_title.empty()) {
    ...
    this->DrawText(cellwidth / 2.0 - fm.width / 2.0 + cellwidth * (col - 1),
                   cellheight - fm.height * 0.25 + cellheight * (row - 1),
                   m_title);
  }

  // Translate the over-scaled dimension into the centre
  if (scale < scale_y)
    cairo_translate(m_cairo, 0 + cellwidth * (col - 1),
                    cellheight / 2.0 - scale * height / 2.0 +
                        cellheight * (row - 1));
  else
    cairo_translate(
        m_cairo, cellwidth / 2.0 - scale * width / 2.0 + cellwidth * (col - 1),
        0 + cellheight * (row - 1));
...
void CairoPainter::SetFillRadial(const OBColor & /*start*/,
                                 const OBColor &end) {
...
void CairoPainter::SetPenWidth(double width) { m_pen_width = width; }

double CairoPainter::GetPenWidth() { return m_pen_width; }
...
void CairoPainter::DrawLine(double x1, double y1, double x2, double y2,
  ...
  cairo_set_dash(m_cairo, (dashes.size() ? &dashes[0] : nullptr), dashes.size(),
                 0.0);
...
void CairoPainter::DrawPolygon(
    const std::vector<std::pair<double, double>> &points) {
  std::vector<std::pair<double, double>>::const_iterator i;
  for (i = points.begin(); i != points.end(); ++i)
    cairo_line_to(m_cairo, i->first,
                  i->second); // note: when called without previous point,
