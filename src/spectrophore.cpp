...
OBSpectrophore::OBSpectrophore(void)
    : _resolution(3.0), _property(nullptr), _radii(nullptr), _oricoor(nullptr),
      _coor(nullptr), _beginProbe(0), _endProbe(0), _numberOfProbes(0) {
...
OBSpectrophore::OBSpectrophore(const OBSpectrophore &s)
    : _resolution(s._resolution), _property(nullptr), _radii(nullptr),
      _oricoor(nullptr), _coor(nullptr), _beginProbe(s._beginProbe),
      _endProbe(s._endProbe), _numberOfProbes(s._numberOfProbes),
      _spectro(s._spectro) {
...
OBSpectrophore &OBSpectrophore::operator=(const OBSpectrophore &s) {
  if (this != &s) {
...
void OBSpectrophore::SetNormalization(
    const OBSpectrophore::NormalizationOption n) {
  _normalization = n;
}
...
void OBSpectrophore::SetResolution(const double r) {
  if (r > 0.0) {
    _resolution = r;
  } else {
    _resolution = 3.0;
...
double OBSpectrophore::GetResolution(void) const { return _resolution; }

void OBSpectrophore::SetAccuracy(const OBSpectrophore::AccuracyOption a) {
  _accuracy = a;
...
OBSpectrophore::AccuracyOption OBSpectrophore::GetAccuracy(void) const {
  return _accuracy;
}

void OBSpectrophore::SetStereo(const OBSpectrophore::StereoOption stereo) {
  _stereoFlag = stereo;
  _setBox();
}

OBSpectrophore::StereoOption OBSpectrophore::GetStereo(void) const {
  return _stereoFlag;
}

std::vector<double> OBSpectrophore::GetSpectrophore(OpenBabel::OBMol *mol) {
  ...
  if (_nAtoms < 3) {
    std::cerr << "OBSpectrophore::GetSpectrophore() error: not enough atoms in "
                 "molecule"
              << std::endl;
    ;
    return _spectro;
  ...
  if ((_normalization == OBSpectrophore::NormalizationTowardsZeroMean) ||
      (_normalization ==
       OBSpectrophore::NormalizationTowardsZeroMeanAndUnitStd)) {
    for (unsigned int i(0); i < N_PROPERTIES; ++i) {
      for (unsigned int n(0); n < 12; ++n) {
        m = (i * 12) + n;
        _spectro[m] -= mean[i];
...
void OBSpectrophore::_getEnergies(double **c, double *e) {
  double d;
  double x2;
  double y2;
  double z2;
...
void OBSpectrophore::_initiateSpectrophore(double *e, double *s) {
  for (unsigned int i = 0; i < N_PROPERTIES * _numberOfProbes; ++i) {
    s[i] = e[i];
  }
}

void OBSpectrophore::_updateSpectrophore(double *ENERGY, double *SPHORE) {
  for (unsigned int i = 0; i < N_PROPERTIES * _numberOfProbes; ++i) {
    SPHORE[i] = MIN(ENERGY[i], SPHORE[i]);
  }
}

void OBSpectrophore::_getMoleculeData(OpenBabel::OBMol *mol) {
  unsigned int a(0);
  unsigned int n;
...
void OBSpectrophore::_getBox(double **c) {
...
void OBSpectrophore::_rotateX(double **oc, double **nc, const double c,
                              const double s) {
  for (unsigned int i = 0; i < _nAtoms; ++i) {
    nc[i][0] = oc[i][0];
...
void OBSpectrophore::_rotateY(double **oc, double **nc, const double c,
                              const double s) {
  for (unsigned int i = 0; i < _nAtoms; ++i) {
...
void OBSpectrophore::_rotateZ(double **oc, double **nc, const double c,
                              const double s) {
  for (unsigned int i = 0; i < _nAtoms; ++i) {
...
void OBSpectrophore::_orient(void) {
  ...
  double d;
  for (unsigned int i = 0; i < _nAtoms; ++i) {
    d = _oricoor[i][0] * _oricoor[i][0] + _oricoor[i][1] * _oricoor[i][1] +
        _oricoor[i][2] * _oricoor[i][2];
    if (d > maxDistance) {
      maxDistance = d;
      maxAtom = i;
    }
...
void OBSpectrophore::_setBox(void) {
  // Define boundaries
  switch (_stereoFlag) {
...
void OBSpectrophore::_calculateProperties(OpenBabel::OBMol *mol) {
  //
  ...
  for (unsigned int r = 0; r < _nAtoms; ++r) {
    for (unsigned int c = r + 1; c < _nAtoms; ++c) {
      ...
      d +=
          (_oricoor[r][1] - _oricoor[c][1]) * (_oricoor[r][1] - _oricoor[c][1]);
      d +=
          (_oricoor[r][2] - _oricoor[c][2]) * (_oricoor[r][2] - _oricoor[c][2]);
...
void OBSpectrophore::_solveMatrix(double **A, double *B, unsigned int dim) {
  std::vector<int> temp(dim);
  _luDecompose(A, temp, dim);
  _luSolve(A, temp, B, dim);
}

void OBSpectrophore::_luDecompose(double **A, std::vector<int> &I,
                                  unsigned int dim) {
  ...
  // first find the highest pivot element in each row and store it for implicit
  // scaling
  for (i = 0; i < dim; ++i) {
    maxVal = 0.0;
    for (j = 0; j < dim; ++j) {
  ...
  for (j = 0; j < dim; ++j) {
    ...
    if (j != iMax) // if current column index is not the maximal row index we
                   // need to interchange
    {
...
void OBSpectrophore::_luSolve(double **A, std::vector<int> &I, double *B,
                              unsigned int dim) {
  for (unsigned int i = 0; i < dim; ++i)
    _swapRows(B, i, I[i]);
...
void OBSpectrophore::_swapRows(double **_pMatrix, unsigned int i,
                               unsigned int j, unsigned int nCols) {
  double dummy;
