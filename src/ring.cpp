...
void OBMol::FindSSSR() {
  if (HasSSSRPerceived())
    return;
  SetSSSRPerceived();
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::FindSSSR", obAuditMsg);
...
std::vector<unsigned int> atomRingToBondRing(OBMol *mol,
 ...
 */
void visitRing(OBMol *mol, OBRing *ring, std::vector<OBRing *> &rlist,
  ...
  mask.Clear();
  for (unsigned int j = 0; j < rlist.size(); ++j) {
    std::vector<unsigned int> otherBonds =
        atomRingToBondRing(mol, rlist[j]->_path);
    OBBitVec bs;
...
void OBMol::FindLSSR() {
  if (HasLSSRPerceived())
    return;
  SetLSSRPerceived();
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::FindLSSR", obAuditMsg);
...
bool CompareRingSize(const OBRing *a, const OBRing *b) {
  return (a->Size() == b->Size() ? a->ring_id < b->ring_id
                                 : a->Size() < b->Size()); // ensure stable sort
}
...
static int FindRings(OBAtom *root, int *avisit, unsigned char *bvisit,
  ...
  while (!stack.empty()) {
    ...
    if (returning) {
      // childResult is the recursive call's return value for cur.bond's
      // neighbor
      returning = false;
    ...
    while (cur.bond) {
      ...
      if (bvisit[bidx] == 0) {
        ...
        if (nvisit == 0) {
          ...
          // push_back may invalidate `cur`; we don't touch it again before
          // break
          stack.push_back(nf);
          recursed = true;
          break;
...
void OBMol::FindRingAtomsAndBonds() {
  ...
  if (obErrorLog.GetOutputLevel() >= obAuditMsg)
    obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::FindRingAtomsAndBonds",
                          obAuditMsg);
...
bool OBRing::IsMember(OBAtom *a) { return (_pathset.BitIsSet(a->GetIdx())); }

bool OBRing::IsMember(OBBond *b) {
  return ((_pathset.BitIsSet(b->GetBeginAtomIdx())) &&
          (_pathset.BitIsSet(b->GetEndAtomIdx())));
}
...
void BuildOBRTreeVector(OBAtom *atom, OBRTree *prv, vector<OBRTree *> &vt,
                        OBBitVec &bv) {
...
int OBRTree::GetAtomIdx() { return (_atom->GetIdx()); }

bool OBRing::findCenterAndNormal(vector3 &center, vector3 &norm1,
                                 vector3 &norm2) {
  ...
  for (j = 0; j != nA; ++j) {
    ...
    vector3 v2 =
        (mol->GetAtom(_path[j + 1 == nA ? 0 : j + 1]))->GetVector() - center;
    tmp = cross(v1, v2);
    norm1 += tmp;
  }
  norm1 /= double(nA);
