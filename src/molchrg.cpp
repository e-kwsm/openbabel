...
/*! \class OBGastChrg molchrg.h <openbabel/molchrg.h>
  ...
  J. Gasteiger & M. Marsili, "A New Model for Calculating Atomic Charges in
  Molecules" Tetrahedron Lett., (1978) 3181-3184.
*/

bool OBGastChrg::AssignPartialCharges(OBMol &mol) {
  // InitialPartialCharges(mol);
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::AssignPartialCharges",
                        obAuditMsg);
...
//! Phosphate O => -0.666
//! Sulfate O => -0.5
//! All other atoms are set to have their initial charge from their formal
//! charge
void OBGastChrg::InitialPartialCharges(OBMol &mol) {
  ...
  for (atom = mol.BeginAtom(i); atom; atom = mol.NextAtom(i)) {
    ...
    else if (atom->IsPhosphateOxygen() && atom->GetHvyDegree() == 1)
...
bool OBGastChrg::GasteigerSigmaChi(OBAtom *atom, double &a, double &b,
                                   double &c) {
  int count;
