...
automatic perception of properties are performed in a <a
href="http://en.wikipedia.org/wiki/Lazy_evaluation">"lazy"</a> manner. That is,
until you call for partial atomic charges, no charges are calculated. This
ensures faster transformations of chemical data -- properties that are not
needed for your code will typically not be calculated. When such data is needed,
appropriate routines are called, and a "flag" is set (e.g., via OBMol::SetFlag
...
in the doc/examples directory and <a href="examples.shtml">all these examples
are included in the documentation</a>.
...
- <a href="http://sourceforge.net/projects/openbabel/">Open Babel project
page</a>
- <a href="http://sourceforge.net/tracker/?group_id=40728&atid=428740">Bug
reporter</a>
- <a href="http://sourceforge.net/tracker/?group_id=40728&atid=428743">Feature
requests</a>
- <a href="http://sourceforge.net/tracker/?group_id=40728&atid=447448">File
format requests</a>
- <a href="http://sourceforge.net/tracker/?group_id=40728&atid=428743">Mailing
lists and archives</a>
...
- OBBase - General base class of OBAtom, OBMol, OBResidue, etc. for storing
generic, arbitrary custom data
  ...
  - OBGrid - A base grid class (e.g., 3D functions like electrostatic potential,
evaluated on a grid)
  - OBText - An object containing just text
- OBRing - Ring cycle perception, Smallest Set of Smallest Rings (SSSR), Largest
Set of Smallest Rings (LSSR)
...
Chemical Utilities:
- OBFingerprint - Generating molecular fingerprints for databases and similarity
searches
- OBForceField - Molecular mechanics force fields for energy evaluation,
conformational searching, coordinate generation, etc.
- OBChemTsfm - SMARTS based structural modification (chemical transformation)

Substructure Searching:
- OBSmartsPattern - Parsing SMARTS chemical search patterns and matching against
OBMol objects
- OBQuery
- OBQueryAtom
- OBQueryBond
...
- OBSqrtTbl - Square Root lookup table - given a distance squared returns
distance
...
- OBRotamerList - Supports a set of rotamer coordinate sets for some number of
potentially rotatable bonds
...
- OBGenericData - General base class for arbitrary data types
  ...
  - OBUnitCell - Storage and manipulation of crystal structure / reciprocal unit
cells
  - AliasData, OBAngleData, OBCommentData, OBConformerData, OBDOSData,
OBElectronicTransitionData, OBExternalBondData, OBGridData, OBMatrixData,
OBNasaThermoData, OBOrbitalEnergyData, OBPairData, OBRateData, OBRingData,
OBRotamerList, OBRotationData, OBSerialNums, OBSetData, OBStereoBase,
OBSymmetryData, OBTorsionData, OBUnitCell, OBVectorData, OBVibrationData,
OBVirtualBond

Plugins:
- OBPlugin
  - OBChargeModel - assign partial (and formal) charges to a molecule
  - OBDescriptor - molecular properties, descriptors or features for filtering
or searching
...
- OBChargeModel - Cleaner interface for assigning partial charges using
different models
- OBGraphSym - Cleaner interface for computing the stereochemical symmetry
classes
...
- computation of partial charges using GasteigerCharges, MMFF94Charges,
QEqCharges, and QTPIECharges
...
- OBLocale - New class to ensure chemical data is read in the "C" numeric
locale.
