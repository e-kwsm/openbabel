...
OBDistanceGeometry::~OBDistanceGeometry() { delete _d; }
  ...
  if (rlist.size() > 0) {
    FOR_RINGS_OF_MOL(r, _mol) {
      ...
      for (int a = 0; a < size; ++a) {
        ...
        FOR_NBORS_OF_ATOM(b, &*atom) { nbrs.push_back(b->GetIdx() - 1); }
        if (nbrs.size() == 4) {
  ...
  for (std::vector<OBGenericData *>::iterator data = vdata.begin();
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::CisTrans) {
      ...
      if (ct->GetConfig().specified) {
        // OK, get the central bond (bc) and check all the bonded atoms for
        // proper stereo
        ...
        FOR_NBORS_OF_ATOM(a, b) {
          ...
          FOR_NBORS_OF_ATOM(d, c) {
            ...
            float lBounds =
                _d->GetLowerBounds(a->GetIdx() - 1, d->GetIdx() - 1) +
                DIST14_TOL;
            float uBounds =
                _d->GetUpperBounds(a->GetIdx() - 1, d->GetIdx() - 1) -
                DIST14_TOL;
    ...
    FOR_ATOMS_OF_MOL(_b, _mol) {
      ...
      FOR_ATOMS_OF_MOL(_c, _mol) {
        ...
        if (u_bc < l_bc) {
          ...
          // obErrorLog.ThrowError(__FUNCTION__, "Triagle Smoothing: Erroneous
          // Bounds.", obWarning);
        }
...
bool OBDistanceGeometry::CheckStereoConstraints() {
  ...
  */
  ...
  std::vector<OBGenericData*> vdata =
  _mol.GetAllData(OBGenericDataType::StereoData); OBStereo::Ref atom_id;
  OBStereo::Ref bond_id;
  for (std::vector<OBGenericData*>::iterator data = vdata.begin(); data !=
  vdata.end(); ++data) {
  ...
  origct!=cistrans.end(); ++origct, ++newct) { if
  ((*origct)->GetConfig(OBStereo::ShapeU)
  ...
  ++origth, ++newth) { if ( (*origth)->GetConfig(OBStereo::Clockwise,
  OBStereo::ViewFrom)
...
bool OBDistanceGeometry::generateInitialCoords(void) {
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = 0; j < dim; j++) {
      if (N - 1 - j >= 0)
        _coord(i * dim + j) = eigVals(N - 1 - j) * eigVecs(i, N - 1 - j);
      else
        _coord(i * dim + j) = 0;
    }
  }
...
bool OBDistanceGeometry::WasSuccessful() const { return _d->success; }
...
void OBDistanceGeometry::GetConformers(OBMol &mol) {
  ...
  if (_mol.NumAtoms() != mol.NumAtoms()) {
    obErrorLog.ThrowError(__FUNCTION__, "The number of atoms did not match.",
                          obWarning);
    return;
  }
...
double DistGeomFunc::operator()(const Eigen::VectorXd &x,
                                Eigen::VectorXd &grad) {
  ...
  for (size_t i = 0; i < owner->_stereo.size(); i++) {
    ...
    Eigen::Vector3d v1(x(nbrs[0] * dim), x(nbrs[0] * dim + 1),
                       x(nbrs[0] * dim + 2));
    Eigen::Vector3d v2(x(nbrs[1] * dim), x(nbrs[1] * dim + 1),
                       x(nbrs[1] * dim + 2));
    Eigen::Vector3d v3(x(nbrs[2] * dim), x(nbrs[2] * dim + 1),
                       x(nbrs[2] * dim + 2));
    Eigen::Vector3d v4(x(nbrs[3] * dim), x(nbrs[3] * dim + 1),
                       x(nbrs[3] * dim + 2));
  ...
  for (size_t i = 0; i < owner->_stereo.size(); i++) {
    ...
    grad[dim * idx4 + 1] +=
        preFactor * (x[idx1 * dim] * (x[idx2 * dim + 2] - x[idx3 * dim + 2]) +
                     x[idx2 * dim] * (x[idx3 * dim + 2] - x[idx1 * dim + 2]) +
                     x[idx3 * dim] * (x[idx1 * dim + 2] - x[idx2 * dim + 2]));

    grad[dim * idx4 + 2] +=
        preFactor * (x[idx1 * dim + 1] * (x[idx2 * dim] - x[idx3 * dim]) +
...
double DistGeomFunc4D::operator()(const Eigen::VectorXd &x,
                                  Eigen::VectorXd &grad) {
  unsigned int dim = owner->GetDimension();
  const size_t size = x.size() / dim;
  double ret = 0.0;
