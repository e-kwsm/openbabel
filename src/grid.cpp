...
void OBFloatGrid::SetLimits(const double origin[3], const double v1[3],
...
void OBFloatGrid::SetXAxis(vector3 v) { _xAxis = v; }

void OBFloatGrid::SetYAxis(vector3 v) { _yAxis = v; }

void OBFloatGrid::SetZAxis(vector3 v) { _zAxis = v; }
...
std::vector<double> OBFloatGrid::GetDataVector() { return _values; }
...
double OBFloatGrid::InterpolateDerivatives(double x, double y, double z,
                                           double *derivatives) {
  int n, igx, igy, igz;
  double yzdim;
  double gx, gy, gz, fgx, fgy, fgz;
            ...
            if ((dx_2 + (dy_2 = SQUARE(c[j + 1] - y))) < cutoff)
              for (z = _zmin + (_inc / 2.0), m = 0; m < _nzinc; z += _inc, ++m)
                if ((dx_2 + dy_2 + SQUARE(c[j + 2] - z)) < cutoff)
                  cell[(k * _nyinc * _nzinc) + (l * _nzinc) + m].push_back(
                      atom->GetIdx());
      ...
      if (PointIsInBox(c[j], c[j + 1], c[j + 2]))
        for (x = _xmin + (_inc / 2.0), k = 0; k < _nxinc; x += _inc, ++k)
          if ((dx_2 = SQUARE(c[j] - x)) < cutoff)
            for (y = _ymin + (_inc / 2.0), l = 0; l < _nyinc; y += _inc, ++l)
              if ((dx_2 + (dy_2 = SQUARE(c[j + 1] - y))) < cutoff)
                for (z = _zmin + (_inc / 2.0), m = 0; m < _nzinc;
                     z += _inc, ++m)
                  if ((dx_2 + dy_2 + SQUARE(c[j + 2] - z)) < cutoff)
                    cell[(k * _nyinc * _nzinc) + (l * _nzinc) + m].push_back(
                        atom->GetIdx());
