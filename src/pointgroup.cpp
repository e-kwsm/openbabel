...
public:
  struct _SYMMETRY_ELEMENT_ {
    void (*transform_atom)(struct _SYMMETRY_ELEMENT_ *el, OBAtom *from,
                           OBAtom *to);
  ...
  int establish_pairs(SYMMETRY_ELEMENT *elem) {
    ...
    if (atom_used == nullptr) {
      //        fprintf( stderr, "Out of memory for tagging array in
      //        establish_pairs()\n" ) ;
      return 0;
    }
    for (i = 0; i < _mol->NumAtoms(); i++) {
      if (elem->transform[i] >= _mol->NumAtoms()) { /* No symmetric atom yet */
        ...
        elem->transform_atom(elem, _mol->GetAtom(i + 1),
                             &symmetric); // ATOM INDEX ISSUE
        if (verbose > 2)
          printf("        new coordinates are: (%g,%g,%g)\n", symmetric.x(),
                 symmetric.y(), symmetric.z());
        best_j = i;
        best_distance =
            2 * TolerancePrimary; /* Performance value we'll reject */
  ...
  SYMMETRY_ELEMENT *alloc_symmetry_element(void) {
    ...
    if (elem->transform == nullptr) {
      //        fprintf( stderr, "Out of memory allocating transform table for
      //        symmetry element\n" ) ;
      free(elem);
      return nullptr;
    }
  ...
  SYMMETRY_ELEMENT *init_ultimate_plane(void) {
    ...
    for (i = 1; i < _mol->NumAtoms(); i++) {
      for (j = 0; j < i; j++) {
        ...
        r = sqrt(SQUARE(p[0]) + SQUARE(p[1]) +
                 SQUARE(p[2])); // distance between atoms i and j
   ...
   */
  static void invert_atom(SYMMETRY_ELEMENT *center, OBAtom *from, OBAtom *to) {
   ...
   */
  static void rotate_atom(SYMMETRY_ELEMENT *axis, OBAtom *from, OBAtom *to) {
  ...
  SYMMETRY_ELEMENT *init_c2_axis(int i, int j,
                                 const double support[DIMENSION]) {
    ...
    center[0] = (_mol->GetAtom(i + 1)->x() + _mol->GetAtom(j + 1)->x()) / 2 -
                support[0];
    center[1] = (_mol->GetAtom(i + 1)->y() + _mol->GetAtom(j + 1)->y()) / 2 -
                support[1];
    center[2] = (_mol->GetAtom(i + 1)->z() + _mol->GetAtom(j + 1)->z()) / 2 -
                support[2];
  ...
  SYMMETRY_ELEMENT *init_axis_parameters(double a[3], double b[3],
                                         double c[3]) {
    SYMMETRY_ELEMENT *axis;
    int i, order, sign;
    ...
    if (rab <= ToleranceSame || rbc <= ToleranceSame || rac <= ToleranceSame) {
      ...
      if (verbose > 0)
        printf(
            "    rotation is underdefined by these points: %8.3f %8.3f %8.3f\n",
            rab, rbc, rac);
      return nullptr;
    }
    rab = (rab + rbc) / 2;
   ...
   *       routines.
   */
  static void rotate_reflect_atom(SYMMETRY_ELEMENT *axis, OBAtom *from,
                                  OBAtom *to) {
    double x[3], y[3], a[3], b[3], c[3];
  ...
  SYMMETRY_ELEMENT *init_improper_axis(int ia, int ib, int ic) {
    ...
    if (verbose > 0)
      printf("Trying an axis for the triplet (%d,%d,%d)\n", ia, ib, ic);
    StatTotal++;
    ...
    if (r <= ToleranceSame) {
      StatEarly++;
      if (verbose > 0)
        printf("    atoms can not define improper axis of the order more than "
               "2\n");
      return nullptr;
    }
    for (i = 0; i < DIMENSION; i++)
    ...
    for (i = 1; i < _mol->NumAtoms(); i++) {
      for (j = 0; j < i; j++) {
        ...
        r = (vector3(center[0], center[1], center[2]) -
             vector3(CenterOfSomething[0], CenterOfSomething[1],
                     CenterOfSomething[2]))
                .length();
         ...
         */
        for (k = 0; k < _mol->NumAtoms(); k++) {
          if ((axis = init_c2_axis(
                   i, j, _mol->GetAtom(k + 1)->GetVector().AsArray())) !=
              nullptr) {
            NormalAxesCount++;
            NormalAxes = (SYMMETRY_ELEMENT **)realloc(
                NormalAxes, sizeof(SYMMETRY_ELEMENT *) * NormalAxesCount);
    ...
    if (PlanesCount == 0)
      ...
      if (PlanesCount == 1)
        ...
        printf("There are %d planes of symmetry in the molecule\n",
               PlanesCount);
      printf("     Residual          Direction of the normal           "
             "Distance\n");
      for (i = 0; i < PlanesCount; i++) {
  ...
  static int compare_axes(const void *a, const void *b) {
  ...
  void report_symmetry_elements_brief(void) {
    int i;
    char *symmetry_code =
        (char *)calloc(1, 10 * (PlanesCount + NormalAxesCount +
                                ImproperAxesCount + InversionCentersCount + 2));
    char buf[100];

    if (symmetry_code == nullptr) {
      //        fprintf( stderr, "Unable to allocate memory for symmetry ID code
      //        in report_symmetry_elements_brief()\n" ) ;
      return;
    }
...
OBPointGroup::OBPointGroup() { d = new PointGroupPrivate; }

OBPointGroup::~OBPointGroup() { delete d; }
...
OBPointGroup::Symbol OBPointGroup::IdentifyPointGroupSymbol(double tolerance) {
  ...
  if (d->PlanesCount + d->NormalAxesCount + d->ImproperAxesCount +
          d->InversionCentersCount ==
      0)
    return C1; // no symmetry

  // OK, let's use the normal decision tree, falling back to an appropriate
  // sub-group as needed Check for linear or K/Kh groups
  if (d->NormalAxesCounts[0] >= 1) // has Cinf axis
  {
    if (d->NormalAxesCounts[2] == 1 && d->PlanesCount > 1 &&
        d->InversionCentersCount == 1) // has C2, so Dinfh
      perceived = Dinfh;
    else if (d->InversionCentersCount == 1 &&
             d->PlanesCount == 1) // no C2, but i = Kh
      perceived = Kh;
  ...
  if (d->NormalAxesCounts[2] > 1 && maxAxis != 0) { // Probably Dihedral
    if ((maxAxis == 2 && d->NormalAxesCounts[2] == 3) ||
        d->NormalAxesCounts[2] >=
            maxAxis) { // Are there the perpendicular C2 axes?
      ...
      // Check to see if symmetry code is correct, or we'll try again with a
      // subgroup
  ...
  if (d->PlanesCount == 1 &&
      (d->InversionCentersCount > 0 || maxImproperAxis > 0)) { // Cnh
    switch (maxAxis) {
    case 8:
      perceived = C8h;
      break;
...
void OBPointGroup::Symmetrize(OBMol *mol) {
  if (!d->Setup) {
    // TODO: We should also check to see if this mol is different from the
    // original setup molecule
    Setup(mol);
