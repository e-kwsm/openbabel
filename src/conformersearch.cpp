...
OBStericConformerFilter::OBStericConformerFilter(double cutoff,
                                                 double vdw_factor,
...
bool OBStericConformerFilter::IsGood(const OBMol &mol, const RotorKey &key,
                                     double *conformer) {
  ...
  for (a1 = 0; a1 < numAtoms; ++a1) {
    for (a2 = a1 + 1; a2 < numAtoms; ++a2) {
      ...
      if (!m_check_hydrogens &&
          (atom1->GetAtomicNum() == OBElements::Hydrogen ||
           atom2->GetAtomicNum() == OBElements::Hydrogen))
        continue;
      ...
      // std::cout << vdwCutoff << " " << m_vdw_factor << " " << m_cutoff << " "
      // << distanceSquared << std::endl;
...
double OBRMSDConformerScore::Score(OBMol &mol, unsigned int index,
  ...
  for (unsigned int a = 0; a < numAtoms; ++a)
    vi.push_back(vector3(conformer_i[a * 3], conformer_i[a * 3 + 1],
                         conformer_i[a * 3 + 2]));
  ...
  for (unsigned int j = 0; j < conformers.size(); ++j) {
    ...
    for (unsigned int a = 0; a < numAtoms; ++a)
      vj.push_back(vector3(conformer_j[a * 3], conformer_j[a * 3 + 1],
                           conformer_j[a * 3 + 2]));
...
double OBMinimizingEnergyConformerScore::Score(
    OBMol &mol, unsigned int index, const RotorKeys &keys,
    const std::vector<double *> &conformers) {
  energy_nrequest++;
  RotorKey cur_key = keys[index];
  if (energy_map.size() > 0) {
...
double
OBMinimizingRMSDConformerScore::Score(OBMol &mol, unsigned int index,
                                      const RotorKeys &keys,
                                      const std::vector<double *> &conformers) {
  unsigned int numAtoms = mol.NumAtoms();
  double *origCoords = mol.GetCoordinates();
...
OBConformerSearch::OBConformerSearch() {
  ...
  // m_logstream = NULL;
  m_printrotors =
      false; // By default, do not print rotors but perform the conformer search
}

OBConformerSearch::~OBConformerSearch() { delete (OBRandom *)d; }

bool OBConformerSearch::Setup(const OBMol &mol, int numConformers,
                              int numChildren, int mutability,
                              int convergence) {
  int nb_rotors = 0;
  // copy some variables
  m_mol = mol;
  ...
  while (m_rotorKeys.size() < m_numConformers && tries < numConformers * 1000) {
    ...
    for (unsigned int i = 1; i < m_rotorList.Size() + 1;
         ++i, rotor = m_rotorList.NextRotor(ri)) {
  ...
  if (m_logstream != nullptr) {
    (*m_logstream) << "Initial conformer count: " << m_rotorKeys.size()
                   << std::endl;
    (*m_logstream) << tries << " attempts,  " << ndup << " duplicates, " << nbad
                   << " failed filter." << std::endl;
  ...
  // Setup some default values for dynamic niche sharing according to the
  // molecule.
        ...
        for (unsigned int i = 1; i < m_rotorList.Size() + 1;
          ...
          if (generator.NextInt() % m_mutability == 0)
            rotorKey[i] = generator.NextInt() %
                          rotor->GetResolution().size(); // permutate gene
        }
        // duplicates are always rejected
        if (!IsUniqueKey(m_rotorKeys, rotorKey))
          continue;
...
struct ConformerScore {
  ConformerScore(const RotorKey &key, double _score)
      : rotorKey(key), score(_score) {}
  RotorKey rotorKey;
  double score;
};
...
struct CompareConformerHighScore {
  bool operator()(const ConformerScore &cs1, const ConformerScore &cs2) {
...
double OBConformerSearch::MakeSelection() {
  ...
  if (m_score->GetPreferred() == OBConformerScore::HighScore)
    std::sort(conformer_scores.begin(), conformer_scores.end(),
              CompareConformerHighScore());
  else
    std::sort(conformer_scores.begin(), conformer_scores.end(),
              CompareConformerLowScore());

  // Rmove the worst scored conformers until we have the disired number of
  // conformers
...
void OBConformerSearch::Search() {
  ...
  if (m_logstream != nullptr) {
    ...
    if (use_sharing) {
      ...
      (*m_logstream) << nb_niches
                     << " niches searched, with a key distance radius of "
                     << niche_radius << std::endl;
      (*m_logstream) << "Fitness sharing parameter alpha: " << alpha_share
                     << " \t sigma:" << sigma_share << std::endl;
      ...
      if (local_opt_rate) {
        (*m_logstream)
            << "Trying to improve best indivual with local search every ";
    ...
    (*m_logstream) << "Will stop after " << m_convergence
                   << " generations without improvement." << std::endl
                   << std::endl;
  }
  if (use_sharing)
    score_population();
...
bool OBConformerSearch::IsUniqueKey(const RotorKeys &keys,
                                    const RotorKey &key) const {
...
/*! @brief   Genetic similarity measure, i.e. "distance" between two rotor keys.
  ...
  @return: number of different alleles, i.e. number of different values in the
  key vectors
*/
int OBConformerSearch::key_distance(const RotorKey &key1,
                                    const RotorKey &key2) {
  int dist = 0;
  ...
  // Skip first values, since  meaningfull valaues are starting at index 1
  // (Fortran translation inside ;-))
...
*/
int OBConformerSearch::local_opt() {
  ...
  if ((max_flag && (opt_score > backup_scores[0])) ||
      (!max_flag &&
       (opt_score < backup_scores[0]))) { // Found a better conformer
...
/*! @brief  Produces one or two offsprings
  ...
  @return: 0 if no valid offspring, 1 if first only, 2 if second only, and 3 if
  2 are valid offsprings.
*/
int OBConformerSearch::reproduce(RotorKey &key1, RotorKey &key2) {
  ...
  if (flag_crossover && (((OBRandom *)d)->NextFloat() <= niche_mating) &&
      nsize > 1) {
  ...
  if (flag_crossover) {
    // Cross the 2 vectors: toss a coin for each position (i.e. uniform
    // crossover)
  ...
  if (IsUniqueKey(m_rotorKeys, key2) && IsGood(key2))
    ret_code += 2;
  return ret_code; // Returns number of new distinct individuals (i.e. rotor
                   // keys)
}
...
*/
int OBConformerSearch::score_population() {
  bool max_flag = (m_score->GetPreferred() == OBConformerScore::HighScore);
...
/*! @brief   Compute shared fitness values for a given population.
  ...
  @return: 0 when OK, 1 or more if not
*/
int OBConformerSearch::share_fitness() {
  bool max_flag = (m_score->GetPreferred() == OBConformerScore::HighScore);
  ...
  // Compute shared fitness from score: rescale score values so the minimum
  // is 1.0
  ...
  for (k = 0; k < pop_size; k++) {
    ...
    if ((dynamic_niches.size() < nb_niches) && (dynamic_niches.size() % 2)) {
      // Get the most distant individual to current list heads, wihtin the first
      // 2/3 of the population The idea is to find some explicit niches, with
      // maximum disimilarity form existing ones, still with some reasonable
      // head fitness score.
      max_dist = 0;
    ...
    if (iniche ==
        dynamic_niches.size()) { // Could not find a niche for this one
      if (dynamic_niches.size() < nb_niches) { // Create a new niche.
        iniche = dynamic_niches.size();
        dynamic_niches.resize(iniche + 1);
        dynamic_niches[iniche].push_back(i);
      } else { // Do it the hard way: compute sharing factor with all the
               // population
        sh_count = 0.0;
        for (j = 0; j < pop_size; j++) {
          ...
          if ((double)dist < sigma_share) {
            sh_ij =
                1.0 - pow(((double)dist) / ((double)sigma_share), alpha_share);
            sh_count += sh_ij;
          }
        }
  ...
  // Build the niche map: provided an invididual index, provides the niche index
  // it belongs to. -1 means out of explicit niches.
  niche_map.clear();
  niche_map.resize(pop_size, -1);
  for (iniche = 0; iniche < dynamic_niches.size(); iniche++) {
    // Divide each dynamic niche member score by the niche size: i.e. each niche
    // member has the same penalty, i.e. the order is unchanged whitin this
    // niche.
    dtmp = 1.0 / ((double)dynamic_niches[iniche].size());
    for (nit = dynamic_niches[iniche].begin();
         nit != dynamic_niches[iniche].end(); ++nit) {
...
*/
double OBConformerSearch::sharing_generation() {
  RotorKeys new_generation, offsprings;
  RotorKey key1, key2;
  unsigned int i = 0, j = 0, imax = 0, iniche = 0, nsize = 0, pop_size = 0,
               half_pop = 0, key_size = 0;
  ...
  if (m_logstream != nullptr) {
    ...
    (*m_logstream) << "   # out of niches :" << out_niches.size()
                   << "\t Best :" << vscores[0] << std::endl;
  }

  // Save each niche 1set element, then 2nd until we have half of the population
  // If we still miss some, get from individuals out of the niches (starting
  // from best ones)
  ...
  // Now add most distant individuals: force genetic diversity in the new
  // population at the cost of higher energies.
  while (ninsert < half_pop) {
    ...
    for (i = 0; i < pop_size; i++) {
      ...
      if (dist_min >
          dist_max) { // Find the most distant to its clostest neighbor
        imax = i;
        dist_max = dist_min;
      }
    }
