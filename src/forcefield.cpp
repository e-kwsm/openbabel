...
struct OBForceField::LBFGSState {
  Eigen::MatrixXd
      s; //!< column j = x_{k-j} - x_{k-j-1} (history of position deltas)
  Eigen::MatrixXd
      y; //!< column j = g_{k-j} - g_{k-j-1} (history of gradient deltas)
...
/** \class OBForceField forcefield.h <openbabel/forcefield.h>
    ...
    The OBForceField class is the base class for molecular mechanics in Open
Babel. Classes derived from the OBForceField implement specific force fields
(Ghemical, MMFF94, UFF, ...).Other classes such as OBFFParameter,
OBFFConstraint, OBFFCalculation and its derived classes are only for internal
use. As a user interested in using the available force fields in Open Babel, you
don't need these classes. The rest of this short introduction is aimed at these
users. For information on how to implement additional force fields, see the wiki
pages or post your questions to the openbabel-devel mailing list.

    Before we can start using a force field, we must first select it and set it
up. This is illustrated in the first example below. The Setup procedure assigns
atom types, charges and parameters. There are several reasons why this may fail,
a log message will be written to the logfile before Setup() returns false.

    The force field classes use their own logging functions. You can set the
logfile using SetLogFile() and set the log level using SetLogLevel(). If needed
you can also write to the logfile using OBFFLog(). There are four log levels:
    BFF_LOGLVL_NONE, OBFF_LOGLVL_LOW, OBFF_LOGLVL_MEDIUM, OBFF_LOGLVL_HIGH.
    See the API documentation to know what kind of output each function writes
to the logfile for the different log levels.
    ...
    Minimize the structure in mol using steepest descent and fix the position of
atom with index 1.
    \code
    #include <openbabel/forcefield.h>
    #include <openbabel/mol.h>
    ...
    pFF->AddInterGroups(ligand, pocket); // non-bonded between ligand and pocket
atoms
...
OBFFParameter *OBForceField::GetParameter(const char *a, const char *b,
...
OBFFConstraints OBForceField::_constraints =
    OBFFConstraints();                      // define static data variable
...
OBFFConstraints &OBForceField::GetConstraints() { return _constraints; }
...
void OBForceField::SetFixAtom(int index) { _fixAtom = index; }

void OBForceField::UnsetFixAtom() { _fixAtom = 0; }
...
OBFFConstraints::OBFFConstraints() { _factor = 50000.0; }
...
int OBFFConstraints::Size() const { return _constraints.size(); }
...
void OBFFConstraints::AddTorsionConstraint(int a, int b, int c, int d,
                                           double torsion) {
  OBFFConstraint constraint;
...
bool OBFFConstraints::IsIgnored(int index) { return _ignored.BitIsSet(index); }

bool OBFFConstraints::IsFixed(int index) { return _fixed.BitIsSet(index); }

bool OBFFConstraints::IsXFixed(int index) { return _Xfixed.BitIsSet(index); }

bool OBFFConstraints::IsYFixed(int index) { return _Yfixed.BitIsSet(index); }

bool OBFFConstraints::IsZFixed(int index) { return _Zfixed.BitIsSet(index); }
  ...
  IF_OBFF_LOGLVL_LOW {
    ...
    FOR_ATOMS_OF_MOL(a, _mol) {
      snprintf(_logbuf, BUFF_SIZE, "%d\t<%8.3f, %8.3f, %8.3f>\n", a->GetIdx(),
               _velocityPtr[a->GetIdx()], _velocityPtr[a->GetIdx() + 1],
               _velocityPtr[a->GetIdx() + 2]);
      OBFFLog(_logbuf);
    }
  }
}
...
bool OBForceField::Setup(OBMol &mol, OBFFConstraints &constraints) {
  ...
  if (IsSetupNeeded(mol)) {
    ...
    if (_validSetup) {
      if (!(_constraints.GetIgnoredBitVec() ==
            constraints.GetIgnoredBitVec())) {
        _constraints = constraints;
        if (!SetupCalculations()) {
          _validSetup = false;
          return false;
...
bool OBForceField::SetConformers(OBMol &mol) {
  ...
  if (mol.NumConformers() > 1) {
    ...
    SetupPointers(); // update pointers to atom positions in the OBFFCalculation
                     // objects
  }
...
int OBForceField::FastRotorSearch(bool permute) {
  ...
  memcpy((char *)store_initial, (char *)_mol.GetCoordinates(),
         sizeof(double) * 3 * _mol.NumAtoms());
  ...
  SetupPointers();
  energy_offset =
      E_Bond(false) + E_Angle(false) + E_StrBnd(false) + E_OOP(false);
  ...
  for (int N = 0; N < num_permutations; ++N) {
    ...
    for (int i = 0; i < reordered_rotors.size(); ++i) {
      ...
      for (j = 0; j < rotor->GetTorsionValues().size();
           j++) { // For each rotor position
        // Note: we could do slightly better by skipping the rotor position we
        // already
        //       tested in the last loop (position 0 at the moment). Note that
        //       this isn't as simple as just changing the loop starting point
        //       to j = 1.
        ...
        if (currentE < minE) {
          minE = currentE;
          minj = j;
          memcpy((char *)minconf, (char *)_mol.GetCoordinates(),
                 sizeof(double) * 3 * _mol.NumAtoms());
        }
      ...
      memcpy((char *)bestconf, (char *)minconf,
             sizeof(double) * 3 * _mol.NumAtoms());
    } // end of this permutation
    if (!quit) {
      if (minE < best_minE) {
        best_minE = minE;
        memcpy((char *)verybestconf, (char *)bestconf,
               sizeof(double) * 3 * _mol.NumAtoms());
      }
    }
...
#if !OB_USE_OBRANDOMMT
      rotorKey[i] =
          generator.UniformInt(0, rotor->GetTorsionValues().size() - 1u);
#else
      rotorKey[i] =
          generator.UniformInt<int>(0, rotor->GetTorsionValues().size() - 1u);
#endif
    }
...
void OBForceField::WeightedRotorSearch(unsigned int conformers,
  ...
  for (unsigned int i = 1; i < rl.Size() + 1; ++i, rotor = rl.NextRotor(ri)) {
    ...
    for (unsigned int j = 0; j < rotor->GetTorsionValues().size(); j++) {
      ...
      SetupPointers(); // update pointers to atom positions in the
                       // OBFFCalculation objects
    ...
    rotorKey[i] =
        -1; // back to the previous setting before we go to another rotor
    ...
    for (unsigned int j = 0; j < rotor->GetTorsionValues().size(); j++) {
      currentE = energies[j];
      // add back the Boltzmann population for these relative energies at 300K
      // (assuming kJ/mol)
  ...
  while (c < conformers) {
    ...
    for (unsigned int i = 1; i < rl.Size() + 1; ++i, rotor = rl.NextRotor(ri)) {
      ...
      if (generator.Bernoulli(defaultRotor)) // should we just leave this rotor
...
void OBForceField::DistanceGeometry() {
    ...
    FOR_ATOMS_OF_MOL(b, _mol) {
      ...
      FOR_NBORS_OF_ATOM(nbr1, _mol.GetAtom(a->GetIdx())) { // 1-2
        ...
        FOR_NBORS_OF_ATOM(nbr2, _mol.GetAtom(nbr1->GetIdx())) { // 1-3
          if (&*nbr2 == &*b) {
            matrix[i][j] = sqrt(1.3 * 1.3 + 1.3 * 1.3 -
                                2.0 * cos(DEG_TO_RAD * 120.0) * 1.3 * 1.3);
            is15 = false;
            break;
          }
  ...
  for (i = 0; i < N; i++) {
    for (j = 0; j < N; j++) {
      G[i][j] =
          0.5 * (matrix[0][i] * matrix[0][i] + matrix[0][j] * matrix[0][j] -
...
void OBForceField::UpdatePairsSimple() {
  ...
  /*
    ...
    \n", i+1, _vdwpairs.CountBits(), _elepairs.CountBits()); OBFFLog(_logbuf);
    }
  */
}
...
unsigned int OBForceField::GetNumVDWPairs() { return _vdwpairs.CountBits(); }
...
double OBForceField::Newton2NumLineSearch(double *direction) {
  ...
  if (fabs(scale) < 2e-6) {
    //      cout << "WARNING: too small \"scale\" at Newton2NumLineSearch" <<
    //      endl;
  ...
  while (true) {
    ...
    if (denom != 0.0) {
      ...
      if (step > max_scl) {
        //          cout << "WARNING: damped steplength " << step << " to " <<
        //          max_scl << endl;
        step = max_scl;
      }
    } else {
  ...
  if (opt_step == 0.0) { // if we still don't have any valid steplength, try a
                         // very small step
  ...
  //    cout << " scale: " << scale << " step: " << opt_step*scale << " maxstep
  //    " << max_scl*scale << endl;
...
double OBForceField::LineSearch(double *currentCoords, double *direction) {
  ...
  for (i = 0; i < 10; ++i) {
    ...
    if (e_n2 > e_n1) { // decrease stepsize
      ...
      memcpy((char *)currentCoords, (char *)lastStep,
...
void OBForceField::ValidateConjugateGradients(int steps) {
  ...
  for (int i = 1; i <= steps; i++) {
    if (firststep) {
      ...
      atom->SetVector(atom->x() + dir1.x(), atom->y() + dir1.y(),
                      atom->z() + dir1.z());
      e_n2 = atom->x() * atom->x() + 2 * (atom->y() * atom->y());
      ...
      atom->SetVector(atom->x() + dir2.x(), atom->y() + dir2.y(),
                      atom->z() + dir2.z());
      grad1 = grad2;
      dir1 = dir2;
      e_n2 = atom->x() * atom->x() + 2 * (atom->y() * atom->y());
    ...
    FOR_ATOMS_OF_MOL(a, _mol) {
      ...
      if (_constraints.IsFixed(idx) || (_fixAtom == idx) ||
        ...
        if (_cstep % _mol.NumAtoms() != 0) {
          g2g2 = dot(grad2, grad2);
          grad1 = vector3(_grad1[coordIdx], _grad1[coordIdx + 1],
                          _grad1[coordIdx + 2]);
          g1g1 = dot(grad1, grad1);
          beta = g2g2 / g1g1;
          grad2 += beta * grad1;
...
// F = [ -------- ]     a = --- = [ ----------- ] = [ ---------------- ] = [
// ----------- ] = [ ------ ]
//     [  mol  A  ]          m    [  mol A amu  ]   [  mol A 10^-27kg  ]   [
//     10^-24s^2  ]   [  ps^2  ]
//
// This means that if we divide the force (coming from the FF) by 1000x its mass
// in amu, we get the acceleration in A ps^-2.
...
void OBForceField::CorrectVelocities() {
  ...
  FOR_ATOMS_OF_MOL(a, _mol) {
    ...
    velocity =
        sqrt(_velocityPtr[velocityIdx] * _velocityPtr[velocityIdx] +
  ...
  for (int i = 1; i <= n; i++) {
    FOR_ATOMS_OF_MOL(a, _mol) {
      if (!_constraints.IsFixed(a->GetIdx()) || (_fixAtom == a->GetIdx()) ||
        ...
        pos.SetX(pos.x() + _velocityPtr[coordIdx] * _timestep +
                 accel.x() * timestep2);
        pos.SetY(pos.y() + _velocityPtr[coordIdx + 1] * _timestep +
                 accel.y() * timestep2);
        pos.SetZ(pos.z() + _velocityPtr[coordIdx + 2] * _timestep +
                 accel.z() * timestep2);
        a->SetVector(pos);
...
double OBForceField::VectorDistanceDerivative(const double *const pos_i,
...
double OBForceField::VectorAngleDerivative(double *pos_i, double *pos_j,
                                           double *pos_k, double *force_i,
                                           double *force_j, double *force_k) {
  // This is adapted from
  // http://scidok.sulb.uni-saarland.de/volltexte/2007/1325/pdf/Dissertation_1544_Moll_Andr_2007.pdf
  // Many thanks to Andreas Moll and the BALLView developers for this
...
double OBForceField::VectorOOPDerivative(vector3 &i, vector3 &j, vector3 &k,
                                         vector3 &l) {
  // This is adapted from
  // http://scidok.sulb.uni-saarland.de/volltexte/2007/1325/pdf/Dissertation_1544_Moll_Andr_2007.pdf
  // Many thanks to Andreas Moll and the BALLView developers for this
...
double OBForceField::VectorOOPDerivative(double *pos_i, double *pos_j,
                                         double *pos_k, double *pos_l,
                                         double *force_i, double *force_j,
                                         double *force_k, double *force_l) {
  // This is adapted from
  // http://scidok.sulb.uni-saarland.de/volltexte/2007/1325/pdf/Dissertation_1544_Moll_Andr_2007.pdf
  // Many thanks to Andreas Moll and the BALLView developers for this
...
double OBForceField::VectorOOP(double *pos_i, double *pos_j, double *pos_k,
                               double *pos_l) {
  // This is adapted from
  // http://scidok.sulb.uni-saarland.de/volltexte/2007/1325/pdf/Dissertation_1544_Moll_Andr_2007.pdf
  // Many thanks to Andreas Moll and the BALLView developers for this
...
double OBForceField::VectorTorsionDerivative(vector3 &i, vector3 &j, vector3 &k,
                                             vector3 &l) {
  // This is adapted from
  // http://scidok.sulb.uni-saarland.de/volltexte/2007/1325/pdf/Dissertation_1544_Moll_Andr_2007.pdf
  // Many thanks to Andreas Moll and the BALLView developers for this
...
double OBForceField::VectorTorsionDerivative(double *pos_i, double *pos_j,
                                             double *pos_k, double *pos_l,
                                             double *force_i, double *force_j,
                                             double *force_k, double *force_l) {
  // This is adapted from
...
OBGridData *OBForceField::GetGrid(double step, double padding, const char *type,
                                  double pchg) {
