...
bool OBGroupContrib::ParseFile() {
  ...
  if (OpenDatafile(ifs, _filename).length() == 0) {
    obErrorLog.ThrowError(__FUNCTION__,
                          " Could not find contribution data file.", obError);
    return false;
  }
  ...
  while (getline(ifs, ln)) {
    ...
    if (sp->Init(vs[0])) {
      if (heavy)
        _contribsHeavy.push_back(
            pair<OBSmartsPattern *, double>(sp, atof(vs[1].c_str())));
      else
        _contribsHydrogen.push_back(
            pair<OBSmartsPattern *, double>(sp, atof(vs[1].c_str())));
    } else {
      delete sp;
      sp = nullptr;
...
double OBGroupContrib::Predict(OBBase *pOb, string *param) {
  ...
  // atom contributions
  if (_debug)
    debugMessage << "Heavy atom contributions:" << endl;
  for (i = _contribsHeavy.begin(); i != _contribsHeavy.end(); ++i) {
    if (i->first->Match(tmpmol)) {
      ...
      for (j = _mlist.begin(); j != _mlist.end(); ++j) {
        ...
        if (_debug)
          debugMessage << (*j)[0] << " = " << i->first->GetSMARTS() << " : "
                       << i->second << endl;
      }
    }
  }
  ...
  // Hydrogen contributions - note that matches to hydrogens themselves are
  // ignored
  if (_debug)
    debugMessage << "  Hydrogen contributions:" << endl;
  for (i = _contribsHydrogen.begin(); i != _contribsHydrogen.end(); ++i) {
    if (i->first->Match(tmpmol)) {
      _mlist = i->first->GetMapList();
      for (j = _mlist.begin(); j != _mlist.end(); ++j) {
        if (tmpmol.GetAtom((*j)[0])->GetAtomicNum() == OBElements::Hydrogen)
          continue;
        int Hcount = tmpmol.GetAtom((*j)[0])->GetExplicitDegree() -
                     tmpmol.GetAtom((*j)[0])->GetHvyDegree();
        hydrogenValues[(*j)[0] - 1] = i->second * Hcount;
        seenHydrogen.SetBitOn((*j)[0]);
        if (_debug)
          debugMessage << (*j)[0] << " = " << i->first->GetSMARTS() << " : "
                       << i->second << " Hcount " << Hcount << endl;
      }
    }
  }
  ...
  for (unsigned int index = 0; index < tmpmol.NumAtoms(); index++) {
    ...
    if (_debug) {
      ...
      if (!seenHeavy.BitIsSet(index + 1))
        debugMessage << "un";
      debugMessage << "matched...";
      int Hcount = tmpmol.GetAtom(index + 1)->GetExplicitDegree() -
                   tmpmol.GetAtom(index + 1)->GetHvyDegree();
      debugMessage << "   " << Hcount
                   << " hydrogens = " << hydrogenValues[index] << " ";
      if (!seenHydrogen.BitIsSet(index + 1))
        debugMessage << "un";
      debugMessage << "matched\n";
    }
  }
