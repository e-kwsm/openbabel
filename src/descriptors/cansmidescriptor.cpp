...
public:
  CanSmiles(const char *ID, bool noStereo)
      : OBDescriptor(ID), _noStereo(noStereo) {}
  const char *Description() override {
    return _noStereo ? "Canonical SMILES without isotopes or stereo"
                     : "Canonical SMILES";
  };
  bool Compare(OBBase *pOb, istream &optionText, bool noEval,
               string *param = nullptr) override;
