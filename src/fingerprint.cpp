...
bool OBFingerprint::GetBit(const vector<unsigned int> &vec,
...
bool FastSearch::Find(OBBase *pOb, vector<unsigned long> &SeekPositions,
  ...
  /// Finds chemical objects in datafilename (which must previously have been
  /// indexed) that have all the same bits set in their fingerprints as in the
  /// fingerprint of a pattern object. (Usually a substructure search in
  /// molecules.) The type of fingerprint and its degree of folding does not
  /// have to be specified here because the values in the index file are used.
  /// The positions of the candidate matching molecules in the original datafile
  /// are returned.

  vector<unsigned int> vecwords;
  _pFP->GetFingerprint(pOb, vecwords,
...
bool FastSearch::FindMatch(OBBase *pOb, vector<unsigned long> &SeekPositions,
                           unsigned int MaxCandidates) {
  // Similar to FastSearch::Find() except that successful candidates have all
  // bits the same as the target
  vector<unsigned int> vecwords;
  _pFP->GetFingerprint(pOb, vecwords,
                       _index.header.words * OBFingerprint::Getbitsperint());
...
/////////////////////////////////////////////////////////
bool FastSearch::FindSimilar(OBBase *pOb,
...
bool FptIndex::Read(istream *pIndexstream) {
  ...
  //    pIndexstream->seekg(header.headerlength);//allows header length to be
  //    changed
...
FastSearchIndexer::FastSearchIndexer(string &datafilename, ostream *os,
  ...
  _pindex->header.headerlength = 3 * sizeof(unsigned) +
                                 sizeof(_pindex->header.fpid) +
                                 sizeof(_pindex->header.datafilename);
  strncpy(_pindex->header.fpid, fpid.c_str(), 15);
  _pindex->header.fpid[14] = '\0'; // ensure fpid is terminated at 14
                                   // characters.
...
bool FastSearchIndexer::Add(OBBase *pOb, std::streampos seekpos) {
  ...
  if (_pFP->GetFingerprint(pOb, vecwords, _nbits)) {
    ...
    if (_pindex->fptdata.empty() && _pindex->header.nEntries != 0) {
      // Reserve size of vectors at start to avoid multiple realloction and
      // copying later. Done here rather than in constructor because needs the
      // size of the fingerprint.
      _pindex->fptdata.reserve(_pindex->header.nEntries *
                               _pindex->header.words);
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Failed to make a fingerprint",
                        obWarning);
  return false;
}
...
/*!
  ...
  These fingerprints are condensed representation of molecules (or other
  objects) as a list of boolean values (actually bits in a vector<unsigned>)
  with length of a power of 2. The main motivation is for fast searching of data
  sources containing large numbers of molecules (up to several million). Open
  Babel provides some routines which can search text files containing lists of
  molecules in any format. See the documentation on the class FastSearch.
  ...
  Many methods of preparing fingerprints have been described, but the type
  supported currently in OpenBabel has each bit representing a substructure (or
  other molecular property). If a substructure is present in the molecule, then
  a particular bit is set to 1. But because the hashing method may also map
  other substructures to the same bit, a match does not guarantee that a
  particular substructure is present; there may be false positives.  However,
  with proper design, a large fraction of irrelevant molecules in a data set can
  be eliminated in a fast search with boolean methods on the fingerprints. It
  then becomes feasible to make a definitive substructure search by conventional
  methods on this reduced list even if it is slow.
  ...
  Fingerprints derived from this abstract base class OBFingerprint can be for
  any object derived from OBBase (not just for OBMol). Each derived class
  provides an ID as a string and OBFingerprint keeps a map of these to provides
  a pointer to the class when requested in FindFingerprint.
  ...
  class MyFpType : OBFingerprint
  {
     ...
     virtual bool GetFingerprint(OBBase* pOb, vector<unsigned int>& fp, int
  nbits)
     {
  ...
  OBFingerprint::GetDefault()->GetFingerprint(&mol, fp); //gets default size of
  fingerprint \endcode or \code vector<unsigned int> fp; OBFingerPrint* pFP =
  OBFingerprint::FindFingerprint("myfpID");
  ...and maybe...
  pFP->GetFingerprint(&mol,fp, 128); //fold down to 128bits if was originally
  larger \endcode
  ...
  The -xF option of the FingerprintFormat class also provides this output, but
  due to a quirk in the way the program works, it is necessary to have a valid
  input molecule for this option to work.
*/

/*! \class FastSearch fingerprint.h <openbabel/fingerprint.h>
  The FastSearch class searches an index to a datafile containing a list of
  molecules (or other objects) prepared by FastSearchIndexer.

  OpenBabel can also search files for molecules containing a substructure
  specified by a SMARTS string, using OBSmartsPattern or from the command line:
  \code
  ...
  of the molecules containing their fingerprints (see OBFingerprint) is prepared
  using FastSearchIndexer. The indexing process may take a long time but only
  has to be done once. The index can be searched very quickly with FastSearch.
  Because of the nature of fingerprints a match to a bit does not guarantee the
  presence of a particular substructure or other molecular property, so that a
  definitive answer may require a subsequent search of the (much reduced) list
  ...
  - Make a FastSearchIndexer object on the heap or the stack, passing in as
  parameters:
  - the datafile name, the indexfile stream,
  - the id of the fingerprint type to be used,
  -  the number of bits the fingerprint is to be folded down to, If it is to be
  left unfolded, set fpid to 0 or do not specify it.
  .
  - For each molecule, call Add() with its pointer and its position in the
  datafile.<br> Currently the std::streampos value is implicitly cast to
  unsigned int so that for 32bit machines the datafile has to be no longer than
  about 2Gbyte.
  - The index file is written when the FastSearchIndexer object is deleted or
  goes out of scope.
  ...
  <em>Number of bits set in (patternFP & targetFP)/Number of bits in (patternFP
  | targetFP)</em><br> where the boolean operations between the fingerprints are
  bitwise<br> The Tanimoto coefficient has no absolute meaning and depends on
  ...
  \endcode
  The datafile can also be used as the input file, provided the input format is
  specified as fs \code obabel datafile.xxx -O outfile.yyy -sSMILES -ifs
  \endcode
  A "slow" search not using fingerprints would be done on the same data by
  omitting -ifs. A "slow" search can use SMARTS strings, but the fast search is
  restricted to the subset which is valid SMILES.

  With the -S option, the target can be specified as a molecule in a file of any
  format \code obabel datafile.xxx -O outfile.yyy -Spattern_mol.zzz -ifs
  \endcode
  ...
  This rather done (rather than a substructure search) if the -at option is
  used, \code obabel datafile.xxx -O outfile.yyy -sSMILES -ifs -at0.7 \endcode
  for instance
  ...
  All stages, the indexing, the interpretation of the SMILES string in the -s
  option, the file in the -S option and the final SMARTS filter convert to OBMol
  and apply ConvertDativeBonds(). This ensures thatforms such as[N+]([O-])=O and
  N(=O)=O are recognized as chemically identical.
*/
