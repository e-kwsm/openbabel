...
public:
  ...
  basic_zip_streambuf(ostream_reference ostream, int level, EStrategy strategy,
...
private:
  ...
  unsigned long _crc;
  unsigned long
      _unzipped_component_bytes; // keep track of bytes that were in separately
                                 // zipped sections for seeking purposes
};
...
template <class charT, class traits = std::char_traits<charT>>
class basic_zip_ostream : public basic_zip_streambuf<charT, traits>,
...
public:
  ...
  inline explicit basic_zip_ostream(
      ostream_reference ostream, bool is_gzip = false,
