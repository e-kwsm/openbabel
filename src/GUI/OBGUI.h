...
public:
  ...
  // initialization (doing it here and not in the ctor allows one to have an
...
public:
  CFilenames(wxWindow *parent, wxWindowID id, const wxString &value = _T(""),
      ...
      : wxTextCtrl(parent, id, value, pos, size,
                   wxTE_NOHIDESEL | wxTE_PROCESS_TAB) {
    SetToolTip(_T("With multiple files, display any of them below using ")
               _T("double click, tab or mousewheel"));
  }
...
private:
  ...
  /// Returns a file path name shortened (using /.../) so that will fit in a
  /// specified window The width in pixels can optionally be explicitly
  /// specified.
  wxString ShortenedPath(const wxString &path, const wxWindow &wnd,
                         int wndwidth = -1);
