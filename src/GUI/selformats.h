...
public:
  SelFormatsDialog(wxArrayString &AllFormatsArray,
                   wxString &ActiveFormatsString)
      : wxDialog(nullptr, wxID_ANY, _T("Set Active Formats"), wxPoint(100, 60),
    ...
    size_t maxheight =
        static_cast<size_t>(0.8 * wxSystemSettings::GetMetric(wxSYS_SCREEN_Y));
    if (height > maxheight)
      height = maxheight;
...
public:
  ...
  bool SelectFormats() // Called in OBGUIFrame::OnSelectFormats
  {
    ...
    //		dialog.Connect(wxID_OK, wxEVT_COMMAND_MENU_SELECTED,
    // wxCommandEventHandler(SelFormatsDialog::OnOK));
