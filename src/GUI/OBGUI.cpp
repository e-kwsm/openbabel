...
bool OBGUIApp::OnInit() {
  ...
  // OBConversion dummy; //needed for OBConversion to load plugin classes
  // (including formats) Read in the stored window sizes and extensions
  // previously used (or use the defaults)
  ...
  if (!splfile.empty() && bitmap.LoadFile(wxString(splfile.c_str(), wxConvUTF8),
                                          wxBITMAP_TYPE_PNG)) {
    wxSplashScreen *splash = new wxSplashScreen(
        bitmap, wxSPLASH_CENTRE_ON_PARENT | wxSPLASH_TIMEOUT, 4000, nullptr, -1,
...
OBGUIFrame::OBGUIFrame(const wxString &title, wxPoint position, wxSize size)
  ...
  listMenu = new wxMenu;
  helpMenu = new wxMenu;
  viewMenu->AppendCheckItem(ID_RESTRICTFORMATS,
                            _T("Use &restricted set of formats"));
  ...
  viewMenu->AppendSeparator();
  viewMenu->AppendCheckItem(ID_SHOWAPIOPTIONS, _T("&API Options"),
                            _T("e.g. errorlevel"));
  viewMenu->AppendCheckItem(ID_SHOWCONVOPTIONS, _T("&Conversion Options"));
  viewMenu->AppendCheckItem(
      ID_SHOWOBJOPTIONS1, _T("Chemical Object Options(&single char)"),
      _T("relating to molecules or reactions\ne.g. -h Add hydrogens"));
  viewMenu->AppendCheckItem(ID_SHOWOBJOPTIONS2,
                            _T("Chemical Object Options(&multichar)"),
                            _T("e.g. --addtotitle"));
  ...
  m_DisplayFile =
      config.Read(_T("DisplayFile"), wxFileName::GetTempDir() + _T("/gui.svg"));
  ...
  m_pOptsWindow = new wxScrolledWindow(panel, wxID_ANY, wxDefaultPosition,
                                       wxDefaultSize, wxVSCROLL | wxNO_BORDER);
  m_pOptsWindow->SetScrollRate(0, 10);

  m_pInFormat =
      new wxChoice(panel, ID_INFORMAT, wxDefaultPosition, wxDefaultSize, 0,
                   static_cast<wxString *>(nullptr));
  m_pInInfo = new wxButton(panel, ID_ININFO, wxT("?"), wxDefaultPosition,
                           wxDefaultSize, wxBU_EXACTFIT);
  m_pForceInFormat = new wxCheckBox(
      panel, ID_INFORCEFORMAT,
      wxT("Use this format for all &input files (ignore file extensions)"));
  m_pInPath = new wxStaticText(panel, wxID_STATIC, wxT(""), wxDefaultPosition,
  ...
  m_pInFiles = new wxButton(panel, ID_INGETFILES, wxT("..."), wxDefaultPosition,
                            wxSize(35, 20));
  ...
  m_pInText = new wxTextCtrl(panel, ID_INTEXT, _T(""), wxDefaultPosition,
                             wxSize(195, 200),
                             wxTE_MULTILINE | wxTE_READONLY | notwrapped);
  ...
  m_pOutFormat =
      new wxChoice(panel, ID_OUTFORMAT, wxDefaultPosition, wxDefaultSize, 0,
                   static_cast<wxString *>(nullptr));
  m_pOutInfo = new wxButton(panel, ID_OUTINFO, wxT("?"), wxDefaultPosition,
                            wxDefaultSize, wxBU_EXACTFIT);
  ...
  m_pDisplay = new wxCheckBox(
      panel, ID_DISPLAY, wxT("Display in " + m_DisplayCmd.BeforeFirst(' ')));
  ...
  m_pSplitter = new wxSplitterWindow(panel, wxID_ANY, wxDefaultPosition,
                                     wxSize(1955, 200));
  m_pMessages =
      new wxTextCtrl(m_pSplitter, ID_MESSAGES, wxEmptyString, wxDefaultPosition,
                     wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY);
  m_pMessages->SetToolTip(
      _T("Message window. Drag the divider down to make bigger"));
  m_pfixedFont = new wxFont(8,                    // int pointSize
...
#ifndef __WXMAC__
  if (OBPlugin::GetPlugin(nullptr, "0xout")) // display checkbox only if extra
                                             // output capability is present
  ...
  wxStaticText *pStatic = new wxStaticText(
      panel, wxID_STATIC, wxT("        ---- INPUT FORMAT ----"));
  MakeBold(pStatic);
  InSizer->Add(pStatic);
  ...
  pStatic = new wxStaticText(panel, wxID_STATIC,
                             wxT("        ---- OUTPUT FORMAT ----"));
  MakeBold(pStatic);
  ...
  OutSizer->Add(new wxStaticText(panel, wxID_STATIC, wxT("Output file")), 0,
                wxLEFT | wxTOP, 5);
...
void OBGUIFrame::OnClose(wxCloseEvent &event) {
  // Save the window size, in and out formats, and various options, for use next
  // time.
  wxCommandEvent ev;
  OnSaveConfig(ev);
...
void OBGUIFrame::OnSaveConfig(wxCommandEvent &event) {
  ...
  config.Write(_T("UseRestrictedFormats"),
               viewMenu->IsChecked(ID_RESTRICTFORMATS));
}
...
void OBGUIFrame::OnHelp(wxCommandEvent &WXUNUSED(event)) {
  ...
  if (!c_type)
    return; // Couldn't find association
...
void OBGUIFrame::OnAbout(wxCommandEvent &WXUNUSED(event)) {
  ...
  std::string cmsg = "OpenBabelGUI (C) 2006 by Chris Morley\n\n"
  ...
  wxMessageBox(msg, _T("About OpenBabelGUI"),
...
void OBGUIFrame::OnSetDisplayFile(wxCommandEvent &event) {
  wxTextEntryDialog dialog(
      this,
      _T("Enter display command and temporary display file on separate lines"),
      _T("Parameters for structure display"),
      m_DisplayCmd + _T('\n') + m_DisplayFile,
...
void OBGUIFrame::OnSetMinSize(wxCommandEvent &event) { SetSize(872, 249); }
...
void OBGUIFrame::OnConvert(wxCommandEvent &WXUNUSED(event)) {
  ...
  /* Get LNK2005 error with VS10 (but not VS9) when stringstream used, possibly
     related to: http://osdir.com/ml/OpenSceneGraph-Users/2010-08/msg00501.html
     ...
     A better way is to build wxWidgets with wxUSE_STD_IOSTREAM set to 0 in
     setup.h (the top level version: \lib\vc_lib\mswu\wx\setup.h with Windows).
     This means wxStreamToTextRedirector does not work, so its functionality is
     provided by hand in this function.
  */
  ...
  // Default output is a string stream which is written to the Output text box
  // at the end
...
with the output format.\nDo you wish to continue the conversion?"),
  ...
  */
  ...
  //	m_pOutText->Freeze();//Otherwise seems to be redrawn after each char
  //from cout
  ...
  if (OutputFileList.size() > 1) {
    std::clog << '\n'
              << OutputFileList.size() << " files output. The first is "
              << OutputFileList[0];
  }
  ...
  if (count > 0) {
    if (!m_pNoOutFile->IsChecked()) {
      ...
      m_pOutText->Clear();
      if (wxFile::Exists(
              wxString(OutputFileList[0].c_str(),
                       wxConvUTF8))) // avoid error message with --split REC
...
void OBGUIFrame::DisplayInputFiles(wxArrayString filepatharray) {
  ...
  if (!wxGetKeyState(WXK_CONTROL)) {
    ...
    m_InFileBasePath = filenamewx.GetVolume() +
                       filenamewx.GetVolumeSeparator() +
...
void OBGUIFrame::OnChangeFormat(wxCommandEvent &WXUNUSED(event)) {
  ...
  OBFormat *pInFormat =
      (OBFormat *)m_pInFormat->GetClientData(m_pInFormat->GetSelection());
  OBFormat *pOutFormat =
      (OBFormat *)m_pOutFormat->GetClientData(m_pOutFormat->GetSelection());
  if (!pInFormat || !pOutFormat)
    return;
  if (viewMenu->IsChecked(ID_SHOWOBJOPTIONS1)) // Single char
    m_pGenOptsPanel->Construct(pOutFormat->TargetClassDescription(), nullptr,
                               1);
  ...
  if (viewMenu->IsChecked(ID_SHOWINOPTIONS)) {
    if (pInFormat &&
        !m_pInOptsPanel->Construct(pInFormat->Description(), "input"))
      m_pInOptsPanel->Construct(pInFormat->Description(), "read"); // try again
  }
  if (viewMenu->IsChecked(ID_SHOWOUTOPTIONS)) {
    if (pOutFormat &&
        !m_pOutOptsPanel->Construct(pOutFormat->Description(), "output"))
      m_pOutOptsPanel->Construct(pOutFormat->Description(),
                                 "write"); // try again
  }
...
void OBGUIFrame::SetInitialFocus() { m_pInFilename->SetFocus(); }
...
wxString OBGUIFrame::GetFilter(wxChoice *pChoice) {
  // Uses text from the window (input or output combo) to construct filter
  // string
...
/////////////////////////////////////////////////////
wxString OBGUIFrame::ShortenedPath(const wxString &path, const wxWindow &wnd,
                                   int wndwidth) {
  ...
  */
  ...
  while (txtwidth > wndwidth) {
    ...
    if (pos2 == wxNOT_FOUND || pos2 + 1 == tpath.length())
      return path;
    // pos2 now has next separator
    // Ensure that there is at least one further directory
    if (tpath.find_first_of(_T("/\\"), pos2 + 1) == wxNOT_FOUND)
      return path;
...
void OBGUIFrame::GetAvailableFormats() {
  ...
  if (OBPlugin::ListAsVector("formats", nullptr,
                             vec)) // check that there are some formats
  {
    OBFormat::PluginIterator itr;
    for (itr = OBFormat::Begin("formats"); itr != OBFormat::End("formats");
         ++itr) {
  ...
  InputFilterString = InputFilterString.Left(InputFilterString.Length() -
                                             3); // remove unneeded ;*.
  OutputFilterString = OutputFilterString.Left(OutputFilterString.Length() -
                                               3); // remove unneeded ;*.
...
void OBGUIFrame::MakePluginsMenu() {
  ...
  OBPlugin::ListAsVector(nullptr, nullptr,
                         topvec); // get 'formats', 'fingerprints', etc
  for (int itop = 0; itop < topvec.size(); ++itop) {
    wxMenu *subMenu = new wxMenu();
    std::vector<std::string> subvec;
    //    std::vector<std::string> verbosevec;
    OBPlugin::ListAsVector(topvec[itop].c_str(), nullptr,
                           subvec); // get each format, etc as single line
    //    OBPlugin::ListAsVector(topvec[itop].c_str(), "verbose",
    //    verbosevec);//get full description of each format
    subMenu->Append(
        ID_HINT, _T("     (Click to see details and to copy ID to clipboard)"));
    for (int isub = 0; isub < subvec.size(); ++isub) {
      ...
      //      subMenu->AppendSubMenu(subsubMenu,
      //      wxString(subvec[isub].c_str()));
      subMenu->Append(ID_PLUGINS + n++,
                      wxString(subvec[isub].c_str(), wxConvUTF8));
    }
...
void OBGUIFrame::OnClickPlugin(wxCommandEvent &event) {
  ...
  if (item) {
    // Find the name of the plugin type. It seems difficult to go up a menu
    // hierarchy.
    ...
    if (plugin) {
      ...
      wxMessageBox(wxString(txt.c_str(), wxConvUTF8), _T("Plugin details"),
                   wxOK | wxICON_INFORMATION | wxCENTER, this);
    }
...
int CFilenames::Expand(std::vector<std::string> &filelist) {
  // Adds full path names of all input files to filelist, expanding wildcards
  // they are present.
  OBGUIFrame *frame = static_cast<OBGUIFrame *>(GetParent()->GetParent());
  ...
  do // for each filename
  {
    ...
    count +=
        DLHandler::findFiles(filelist, std::string(fn.GetFullPath().mb_str()));
  } while (namestart);
