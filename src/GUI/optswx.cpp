...
editbox. It will have some punctuation characters stripped from its start and
end, so that q <day> Starting day, default <Sunday> will display as an edit box
containing "Sunday" and caption "Starting day".
...
*/
////////////////////////////////////////////////////
DynOptionswx::~DynOptionswx() { Clear(); }
...
bool DynOptionswx::Construct(const char *OptionsText, const char *StartText,
                             int MultiCharFilter) {
  ...
  if (p) {
           ...
           *p != '\n') // loop for all options until blank line
    {
      ...
      char *plinestart = p;
      while (*p && *p != '\n' && !isalnum(*(p++)))
        ; // skip space and punctuation except \n
      if (*p == '\n') {
        plinestart = ++p; // reset start of line
        continue;
      }
      // Ignore lines which start with more than MAXLEADINGSPACES whitespace
      // chars(tab is 1 char!)
      ...
      while (*p >= 0 && isspace(*p++))
        ; // skip white space
      while (ispunct(*(--p)) && *p != '-') {
        // If first non-white space character after option char is
        // punctuation(except for '-') Provide an edit control rather than a
        // checkbox
        ++ProvideEditCtl;
        char endch = 0;
        switch (*p++) {
      ...
      if (ProvideEditCtl) {
        ...
        wxStaticText *pEdCaption = new wxStaticText(
            parent, wxID_STATIC, wxString(pCaption, wxConvUTF8));
        OptionMap.push_back(std::make_pair(
            wxString(),
            pEdCaption)); // string is empty for a caption: not an option

        // Edit boxes for multicharacter options are larger
        const int EDWIDTH =
            60; // now all are the same size. oname.size()>1? 60 : 40;
        wxTextCtrl *pEd;

        // Make a large edit box on the next line if last char is of the caption
        // is :
        ...
        if (BigEdit) {
          ...
          while (ProvideEditCtl--) {
            ...
            if (ProvideEditCtl)
              oname = _T(' ') + oname; // editboxes except the first have name
        ...
        // Checkbox, or radio button ("or" is the last word in the caption && no
        // letters after " or")
        wxControl *pChk;
        ...
        if (SetChk)
          strcasecmp(pdef, "no") && !strcasecmp(pdef, "not") &&
              strcasecmp(pdef, "none");
        ...
        if (NextIsRadio || HasOr) {
          unsigned int style =
              NextIsRadio ? 0 : wxRB_GROUP; // Style only on first radiobutton
          pChk = new wxRadioButton(parent, wxID_ANY,
                                   wxString(pCaption, wxConvUTF8),
                                   wxDefaultPosition, wxDefaultSize, style);
          NextIsRadio = HasOr;
          ((wxRadioButton *)pChk)->SetValue(SetChk);
        } else {
          pChk =
              new wxCheckBox(parent, wxID_ANY, wxString(pCaption, wxConvUTF8));
...
int DynOptionswx::SetOptions(OpenBabel::OBConversion &Conv,
                             OpenBabel::OBConversion::Option_type opttyp) {
  // Now sets options directly in OBConversion
  int count = 0;
  OMapType::iterator itr;
  for (itr = OptionMap.begin(); itr != OptionMap.end(); ++itr) {
    if (itr->first.empty())
      continue; // just a caption or a line
    ...
    while (++itr2 != OptionMap.end()) {
      if ((itr2->first).empty() ||
          itr2->first[0] !=
              _T(' ')) // subsequent editboxes have the name preceded by a space
        break;
