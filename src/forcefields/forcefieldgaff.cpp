...
namespace OpenBabel {
template <bool gradients> void OBFFBondCalculationGaff::Compute() {
...
template <bool gradients> double OBForceFieldGaff::E_Bond() {
  ...
  for (i = _bondcalculations.begin(); i != _bondcalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%s %s  %8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).rab, (*i).r0, (*i).kr,
               (*i).delta, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
  ...
  IF_OBFF_LOGLVL_MEDIUM {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL BOND STRETCHING ENERGY = %8.3f %s\n", energy,
             GetUnit().c_str());
    OBFFLog(_logbuf);
  }
  return energy;
...
template <bool gradients> void OBFFAngleCalculationGaff::Compute() {
...
template <bool gradients> double OBForceFieldGaff::E_Angle() {
  ...
  IF_OBFF_LOGLVL_HIGH {
    ...
    OBFFLog(" I    J    K      ANGLE      ANGLE     CONSTANT      DELTA      "
            "ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "-----------\n");
  }
  ...
  for (i = _anglecalculations.begin(); i != _anglecalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%s %s %s  %8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(),
               (*i).theta, (*i).theta0, (*i).kth, (*i).delta, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
template <bool gradients> void OBFFTorsionCalculationGaff::Compute() {
...
template <bool gradients> double OBForceFieldGaff::E_Torsion() {
  ...
  for (i = _torsioncalculations.begin(); i != _torsioncalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%s %s %s %s    %6.3f    %5.0f   %8.3f   %1.0f   %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(),
               (*i).d->GetType(), (*i).vn_half, (*i).gamma, (*i).tor, (*i).n,
               (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
//						//
OBFFParameter *
OBForceFieldGaff::GetParameterOOP(const char *a, const char *b, const char *c,
                                  const char *d,
...
template <bool gradients> void OBFFOOPCalculationGaff::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b, idx_c, idx_d)) {
    energy = 0.0;
    return;
  }
...
template <bool gradients> double OBForceFieldGaff::E_OOP() {
  ...
  IF_OBFF_LOGLVL_MEDIUM {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL IMPROPER-TORSIONAL ENERGY = %8.3f %s\n", energy,
             GetUnit().c_str());
    OBFFLog(_logbuf);
  }
...
template <bool gradients> void OBFFVDWCalculationGaff::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
...
template <bool gradients> double OBForceFieldGaff::E_VDW() {
...
template <bool gradients> void OBFFElectrostaticCalculationGaff::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
...
template <bool gradients> double OBForceFieldGaff::E_Electrostatic() {
  ...
  unsigned int j = 0;
  for (i = _electrostaticcalculations.begin();
       i != _electrostaticcalculations.end(); ++i, ++j) {
    // Cut-off check
    if (_cutoff)
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE, "%s %s   %8.3f  %8.3f  %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).rab, (*i).qq,
               (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
bool OBForceFieldGaff::SetupCalculations() {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    parameter = GetParameter(a->GetType(), b->GetType(), nullptr, nullptr,
                             _ffbondparams);
    if (parameter == nullptr) {
      parameter =
          GetParameter("X", a->GetType(), nullptr, nullptr, _ffbondparams);
      if (parameter == nullptr) {
        parameter =
            GetParameter("X", b->GetType(), nullptr, nullptr, _ffbondparams);
        if (parameter == nullptr) {
          ...
          IF_OBFF_LOGLVL_LOW {
            snprintf(_logbuf, BUFF_SIZE,
                     "COULD NOT FIND PARAMETERS FOR BOND %s-%s, USING DEFAULT "
                     "PARAMETERS\n",
                     a->GetType(), b->GetType());
            OBFFLog(_logbuf);
          }
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    parameter = GetParameter(a->GetType(), b->GetType(), c->GetType(), nullptr,
                             _ffangleparams);
    if (parameter == nullptr) {
      parameter = GetParameter("X", b->GetType(), c->GetType(), nullptr,
                               _ffangleparams);
      if (parameter == nullptr) {
        parameter = GetParameter(a->GetType(), b->GetType(), "X", nullptr,
                                 _ffangleparams);
        if (parameter == nullptr) {
          parameter =
              GetParameter("X", b->GetType(), "X", nullptr, _ffangleparams);
          if (parameter == nullptr) {
            ...
            IF_OBFF_LOGLVL_LOW {
              snprintf(_logbuf, BUFF_SIZE,
                       "COULD NOT FIND PARAMETERS FOR ANGLE %s-%s-%s, USING "
                       "DEFAULT PARAMETERS\n",
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    parameter = GetParameter(a->GetType(), b->GetType(), c->GetType(),
                             d->GetType(), _fftorsionparams);
    if (parameter == nullptr) {
      parameter = GetParameter("X", b->GetType(), c->GetType(), d->GetType(),
                               _fftorsionparams);
      if (parameter == nullptr) {
        parameter = GetParameter(a->GetType(), b->GetType(), c->GetType(), "X",
                                 _fftorsionparams);
        if (parameter == nullptr) {
          parameter = GetParameter("X", b->GetType(), c->GetType(), "X",
                                   _fftorsionparams);
          if (parameter == nullptr) {
            ...
            IF_OBFF_LOGLVL_LOW {
              snprintf(_logbuf, BUFF_SIZE,
                       "COULD NOT FIND PARAMETERS FOR TORSION %s-%s-%s-%s, "
    ...
    torsioncalc.vn_half =
        KCAL_TO_KJ * parameter->_dpar[0] / parameter->_ipar[0];
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    // if there are any groups specified, check if the four improper-dihedral
    // atoms are in a single intraGroup
    if (HasGroups()) {
      bool validOOP = false;
      for (unsigned int i = 0; i < _intraGroup.size(); ++i) {
        if (_intraGroup[i].BitIsSet(a->GetIdx()) &&
    ...
    parameter = GetParameterOOP(a->GetType(), b->GetType(), c->GetType(),
                                d->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // A-B-C-D || PLANE = ABC
      oopcalc.a = a;
      oopcalc.b = b;
    ...
    parameter = GetParameterOOP(a->GetType(), b->GetType(), d->GetType(),
                                c->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // A-B-D-C || PLANE = ABD
      oopcalc.a = a;
      oopcalc.b = b;
    ...
    parameter = GetParameterOOP(c->GetType(), b->GetType(), d->GetType(),
                                a->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // C-B-D-A || PLANE = CBD
      oopcalc.a = c;
      oopcalc.b = b;
    ...
    parameter = GetParameterOOP("X", b->GetType(), c->GetType(), d->GetType(),
                                _ffoopparams);
    if (parameter != nullptr) {
      // A-B-C-D || PLANE = ABC
      oopcalc.a = a;
    ...
    parameter = GetParameterOOP("X", b->GetType(), d->GetType(), c->GetType(),
                                _ffoopparams);
    if (parameter != nullptr) {
      // A-B-D-C || PLANE = ABD
      oopcalc.a = a;
    ...
    parameter = GetParameterOOP("X", b->GetType(), d->GetType(), a->GetType(),
                                _ffoopparams);
    if (parameter != nullptr) {
      // C-B-D-A || PLANE = CBD
      oopcalc.a = c;
    ...
    parameter =
        GetParameterOOP("X", "X", c->GetType(), d->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // A-B-C-D || PLANE = ABC
      oopcalc.a = a;
      oopcalc.b = b;
    ...
    parameter =
        GetParameterOOP("X", "X", d->GetType(), c->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // A-B-D-C || PLANE = ABD
      oopcalc.a = a;
      oopcalc.b = b;
    ...
    parameter =
        GetParameterOOP("X", "X", d->GetType(), a->GetType(), _ffoopparams);
    if (parameter != nullptr) {
      // C-B-D-A || PLANE = CBD
      oopcalc.a = c;
      oopcalc.b = b;
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    parameter_a =
        GetParameter(a->GetType(), nullptr, nullptr, nullptr, _ffvdwparams);
    if (parameter_a == nullptr) { // no vdw parameter -> use hydrogen
      ...
      IF_OBFF_LOGLVL_LOW {
        snprintf(_logbuf, BUFF_SIZE,
                 "COULD NOT FIND VDW PARAMETERS FOR ATOM %s, USING HYDROGEN "
                 "VDW PARAMETERS\n",
                 a->GetType());
        OBFFLog(_logbuf);
      }
    ...
    parameter_b =
        GetParameter(b->GetType(), nullptr, nullptr, nullptr, _ffvdwparams);
    ...
    */
    ...
    /*
      ...
      else if ((a->IsInRingSize(5) || a->IsInRingSize(4)) && (b->IsInRingSize(5)
      || b->IsInRingSize(4))) vdwcalc.samering = true; else vdwcalc.samering =
      false;
    */
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    elecalc.qq = KCAL_TO_KJ * 332.17 * a->GetPartialCharge() *
                 b->GetPartialCharge() / _epsilon;
...
bool OBForceFieldGaff::ParseParamFile() {
  ...
  while (!ifs.fail() &&
         strlen(buffer) != 0) // read block 2 (atom mass and polarizability)
  {
    tokenize(vs, buffer, " -\n\t");
    parameter.clear();
  ...
  while (!ifs.fail() && strlen(buffer) != 0) // read block 7(impropers)
  {
    ...
    // the rules for determining where they are applied are not always
    // straight-forward. Amber param96 exclusively uses the Stereocenter version
    // of the improper torsions, and they are typically centered on an sp2 atom
    // in order to enforce planarity with its three neighbors. Only one improper
    // torsion allowed to be centered on any atom. These torsions are listed in
    // the Amber literature as i-j-k-l where the angle is the dihedral between
    // i-k-l and j-k-l, and the bonding pattern is i, j, and l are all bonded to
    // atom k, and are also not bonded to each other. In the towhee_input file
    // this stereo improper torsion is listed only for atom k, and the atom
    // order there is l, i, j. Remember that you can set the improper type to 0
    // to have the code automatically determine the improper type (so long as
    // inpstyle is 2).
  ...
  while (!ifs.fail() &&
         strlen(buffer) !=
             0) // read block 9 (equivalent atoms for vdW potential)
  {
  ...
  ifs.getline(buffer, BUFF_SIZE); // next line
  ifs.getline(buffer,
              BUFF_SIZE); //'RE' means vdW radius and well-depth are read
...
bool OBForceFieldGaff::SetTypes() {
  ...
  vector<pair<std::unique_ptr<OBSmartsPattern>, string>>
      _vexttyp; //!< external atom type rules
    ...
    if (EQn(buffer, "atom", 4)) {
      ...
      if (sp->Init(vs[1]))
        ...
        obErrorLog.ThrowError(__FUNCTION__,
                              " Could not parse atom type table from gaff.prm",
                              obInfo);
        return false;
      }
    }
  ...
  // In a conjugated ring system cc-cc, and cd-cd are single conjugated bonds,
  // cc-cd are double ones All these types are initially types cc-cc, some cc's
  // are changed to cd. similar for ce and cf, cp and cq, nc and nd, ne and nf,
  // pc and pd, pe and pf Initially types cc, ce, cp, nc, ne , pc and pe are
  // assigned. Some of these are changed into cd, cf, cq, nd, nf, pd and pf
    ...
    if (!visited[atm->GetIdx() - 1]) {
      if (strcmp(atm->GetType(), "cc") == 0 ||
        ...
        FOR_BONDS_OF_ATOM(bond, atm) {
          a = bond->GetBeginAtom();
          b = bond->GetEndAtom();
          if ((strcmp(a->GetType(), "cc") == 0 ||
               strcmp(a->GetType(), "cd") == 0) &&
              (strcmp(b->GetType(), "cc") == 0 ||
               strcmp(b->GetType(), "cd") == 0)) {
          ...
          if ((strcmp(a->GetType(), "ce") == 0 ||
               strcmp(a->GetType(), "cf") == 0) &&
              (strcmp(b->GetType(), "ce") == 0 ||
               strcmp(b->GetType(), "cf") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
          ...
          if ((strcmp(a->GetType(), "cp") == 0 ||
               strcmp(a->GetType(), "cq") == 0) &&
              (strcmp(b->GetType(), "cp") == 0 ||
               strcmp(b->GetType(), "cq") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
          ...
          if ((strcmp(a->GetType(), "nc") == 0 ||
               strcmp(a->GetType(), "nd") == 0) &&
              (strcmp(b->GetType(), "nc") == 0 ||
               strcmp(b->GetType(), "nd") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
          ...
          if ((strcmp(a->GetType(), "ne") == 0 ||
               strcmp(a->GetType(), "nf") == 0) &&
              (strcmp(b->GetType(), "ne") == 0 ||
               strcmp(b->GetType(), "nf") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
          ...
          if ((strcmp(a->GetType(), "pc") == 0 ||
               strcmp(a->GetType(), "pd") == 0) &&
              (strcmp(b->GetType(), "pc") == 0 ||
               strcmp(b->GetType(), "pd") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
          ...
          if ((strcmp(a->GetType(), "pe") == 0 ||
               strcmp(a->GetType(), "pf") == 0) &&
              (strcmp(b->GetType(), "pe") == 0 ||
               strcmp(b->GetType(), "pf") == 0)) {
            BO = bond->GetBondOrder();
            if ((BO > 1 && strcmp(a->GetType(), b->GetType()) == 0) ||
                (BO == 1 && strcmp(a->GetType(), b->GetType()) != 0)) {
              if (!visited[a->GetIdx() - 1]) {
  ...
  //       b = bond->GetEndAtom();
  //       BO = bond->GetBondOrder();
  //       cout << "Bond between: " << a->GetType() << " and " << b->GetType()
  //       << " with bondorder " << BO << endl;
  //     }
  // DEBUG (validation)
...
bool OBForceFieldGaff::SetPartialCharges() {
  // Do nothing
  //
  // The Partial Charges need to be set *before* atom typing as Gasteiger
  // Charges require the default atom types. This is done in
  // SetPartialChargesBeforeAtomTyping()
  return true;
}
...
bool OBForceFieldGaff::SetPartialChargesBeforeAtomTyping() {
  ...
  // Note that the Gastegier charge calculation checks for the values of
...
bool OBForceFieldGaff::ValidateGradients() {
  ...
  OBFFLog("ATOM IDX      NUMERICAL GRADIENT           ANALYTICAL GRADIENT      "
          "  REL. ERROR (%)   \n");
  OBFFLog("--------------------------------------------------------------------"
          "--------------------\n");
