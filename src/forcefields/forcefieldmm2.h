...
public:
  ...
  //! Constructor
  explicit OBForceFieldMM2(const char *ID, bool IsDefault = true)
      : OBForceField(ID, IsDefault), _init(false) {
  ...
  OBForceFieldMM2 *MakeNewInstance() override {
    return new OBForceFieldMM2(*this);
  }

  const char *Description() override { return "MM2 force field."; };
