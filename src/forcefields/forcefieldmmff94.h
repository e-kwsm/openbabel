...
protected:
  ...
  //! return the covalent radius from Blom and Haaland, value from etab if not
  //! available
  double GetCovalentRadius(OBAtom *a);
  //! return the bond length calculated with a modified version of the
  //! Schomaker-Stevenson rule
  double GetRuleBondLength(OBAtom *a, OBAtom *b);
  //! return the bond length from mmffbond.par, if not found, one is calculated
  //! with a modified version of the Schomaker-Stevenson rule
  double GetBondLength(OBAtom *a, OBAtom *b);

  //! Same as OBForceField::GetParameter, but takes (bond/angle/torsion) type in
  //! account and takes 0 as wildcart.
  OBFFParameter *GetParameter1Atom(int a,
                                   std::vector<OBFFParameter> &parameter);
  OBFFParameter *GetParameter2Atom(int a, int b,
                                   std::vector<OBFFParameter> &parameter);
  OBFFParameter *GetParameter3Atom(int a, int b, int c,
                                   std::vector<OBFFParameter> &parameter);

  //! Same as OBForceField::GetParameter, but takes (bond/angle/torsion) type in
  //! account and takes 0 as wildcart.
  OBFFParameter *GetTypedParameter2Atom(int ffclass, int a, int b,
                                        std::vector<OBFFParameter> &parameter);
  OBFFParameter *GetTypedParameter3Atom(int ffclass, int a, int b, int c,
                                        std::vector<OBFFParameter> &parameter);
  OBFFParameter *GetTypedParameter4Atom(int ffclass, int a, int b, int c, int d,
                                        std::vector<OBFFParameter> &parameter);
...
public:
  //! Constructor
  explicit OBForceFieldMMFF94(const char *ID, bool IsDefault = true)
      : OBForceField(ID, IsDefault) {
    _validSetup = false;
    _init = false;
    _rvdw = 7.0;
    _rele = 15.0;
  ...
  std::string GetUnit() override { return std::string("kcal/mol"); }
