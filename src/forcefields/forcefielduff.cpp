...
template <bool gradients> void OBFFBondCalculationUFF::Compute() {
...
template <bool gradients> double OBForceFieldUFF::E_Bond() {
  ...
  IF_OBFF_LOGLVL_HIGH {
    ...
    OBFFLog(" I      J   TYPE   LENGTH     LENGTH     CONSTANT      DELTA      "
            "ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "------\n");
  }
  ...
  for (i = _bondcalculations.begin(); i != _bondcalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%-5s %-5s  %4.2f%8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).bt, (*i).rab, (*i).r0,
               (*i).kb, (*i).delta, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
template <bool gradients> void OBFFAngleCalculationUFF::Compute() {
  ...
  switch (coord) {
  case 1: // sp -- linear case, minima at 180 degrees, max (amplitude 2*ka) at
          // 0, 360
  ...
  case 6: // octahedral
    ...
    // UNLIKE Rappe paper, we add a penalty for angles close to zero, based on
    // ESFF i.e., if the angle is less than approx theta0, energy goes up
    // exponentially
  ...
  case 7: // IF7 pentagonal -- pentagonal bipyramidal
     ...
     */
    cosT = cos(theta);
    energy = ka * c1 * (cosT - .30901699) * (cosT - .30906199) *
             (cosT + .80901699) * (cosT + .8091699);
    break;
  default: // general (sp3) coordination
    cosT = cos(theta);
    energy = ka * (c0 + c1 * cosT +
                   c2 * (2.0 * cosT * cosT - 1.0)); // use cos 2t = (2cos^2 - 1)
  }
  ...
  if (gradients) {
    ...
    case 4: // square planar
      dE =
          ka * n * sin(n * theta) - 20.0 * exp(-20.0 * (theta - theta0 + 0.25));
      break;
    case 7: // pentagonal bipyramidal
      sinT = sin(theta);
      cosT = cos(theta);
      dE = c1 * -ka *
           (2 * sinT * (cosT - .30906199) * (cosT + .80901699) *
                (cosT + .8091699) +
            2 * sinT * (cosT - .30901699) * (cosT - .30906199) *
                (cosT + .8091699));
...
template <bool gradients> double OBForceFieldUFF::E_Angle() {
  ...
  for (i = _anglecalculations.begin(); i != _anglecalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%-5s %-5s %-5s%8.3f  %8.3f     %8.3f   %8.3f   %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(),
               (*i).theta * RAD_TO_DEG, (*i).theta0, (*i).ka, (*i).delta,
               (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
template <bool gradients> void OBFFTorsionCalculationUFF::Compute() {
  ...
  if (gradients) {
    ...
    if (fabs(dotAbbcBccd) < 2e-6 ||
        !isfinite(tor)) { // stop any NaN or infinity
...
template <bool gradients> double OBForceFieldUFF::E_Torsion() {
  ...
  for (i = _torsioncalculations.begin(); i != _torsioncalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%-5s %-5s %-5s %-5s%6.3f       %8.3f     %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(),
               (*i).d->GetType(), (*i).V, (*i).tor * RAD_TO_DEG, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
//  c
*/
template <bool gradients> void OBFFOOPCalculationUFF::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b, idx_c, idx_d)) {
    energy = 0.0;
    return;
  }
  ...
  if (gradients) {
    ...
    if (!isfinite(angle))
      angle = 0.0; // doesn't explain why GetAngle is returning NaN but solves
                   // it for us;
    ...
    angle = DEG_TO_RAD * Point2PlaneAngle(d->GetVector(), a->GetVector(),
...
template <bool gradients> double OBForceFieldUFF::E_OOP() {
  ...
  for (i = _oopcalculations.begin(); i != _oopcalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(
          _logbuf, BUFF_SIZE, "%-5s %-5s %-5s %-5s%8.3f   %8.3f     %8.3f\n",
          (*i).a->GetType(), (*i).b->GetType(), (*i).c->GetType(),
          (*i).d->GetType(), (*i).angle * RAD_TO_DEG, (*i).koop, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
  ...
  IF_OBFF_LOGLVL_HIGH {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL OUT-OF-PLANE BENDING ENERGY = %8.3f %s\n", energy,
             GetUnit().c_str());
    OBFFLog(_logbuf);
  }
  return energy;
...
template <bool gradients> void OBFFVDWCalculationUFF::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
  ...
  // TODO: This actually should include zetas (not always exactly 6-12 for VDW
  // paper)
...
template <bool gradients> double OBForceFieldUFF::E_VDW() {
  ...
  for (i = _vdwcalculations.begin(); i != _vdwcalculations.end(); ++i, ++j) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE, "%-5s %-5s %8.3f  %8.3f  %8.3f\n",
               (*i).a->GetType(), (*i).b->GetType(), (*i).rab, (*i).kab,
               (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
template <bool gradients> void OBFFElectrostaticCalculationUFF::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
...
template <bool gradients> double OBForceFieldUFF::E_Electrostatic() {
...
double CalculateBondDistance(OBFFParameter *i, OBFFParameter *j,
                             double bondorder) {
  double ri, rj;
  double chiI, chiJ;
  double rbo, ren;
  ...
  // From equation 4
  ren =
      ri * rj * (pow((sqrt(chiI) - sqrt(chiJ)), 2.0)) / (chiI * ri + chiJ * rj);
  // From equation 2
...
bool OBForceFieldUFF::SetupVDWCalculation(OBAtom *a, OBAtom *b,
  ...
  if (parameterA == nullptr || parameterB == nullptr) {
    IF_OBFF_LOGLVL_LOW {
      snprintf(
          _logbuf, BUFF_SIZE,
          "    COULD NOT FIND PARAMETERS FOR VDW INTERACTION %d-%d (IDX)...\n",
          a->GetIdx(), b->GetIdx());
      OBFFLog(_logbuf);
    }
    return false;
...
int GetCoordination(OBAtom *b, int ipar) {
  ...
  if (valenceElectrons) {
    ...
    valenceElectrons -=
        b->GetFormalCharge(); // make sure to look for I+F4 -> see-saw
...
bool OBForceFieldUFF::SetupCalculations() {
  ...
  // Clear and reset any 5-coordinate axial/equatorial marks (i.e., strange
  // coordination) Now should fit standard VSEPR rules, although we can't easily
  // handle lone pairs
  int coordination;
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    if (parameterB == nullptr) {
      snprintf(_logbuf, BUFF_SIZE,
               "    COULD NOT FIND PARAMETERS FOR ATOM %d (IDX)...\n",
               atom->GetIdx());
    ...
    if (GetCoordination(&*atom, parameterB->_ipar[0]) ==
        5) { // we need to do work for trigonal-bipy!
      ...
      for (current = atom->NextNbrAtom(i); current;
           current = atom->NextNbrAtom(i)) {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    // but check for explicit hydrogen counts (e.g., biphenyl inter-ring is not
    // aromatic)
    ...
    bondcalc.kb = (0.5 * KCAL_TO_KJ * 664.12 * parameterA->_dpar[5] *
                   parameterB->_dpar[5]) /
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    if (parameterA == nullptr || parameterB == nullptr ||
      ...
      IF_OBFF_LOGLVL_LOW {
        snprintf(_logbuf, BUFF_SIZE,
                 "    COULD NOT FIND PARAMETERS FOR ANGLE %d-%d-%d (IDX)...\n",
    ...
    if (coordination != parameterB->_ipar[0]) {
      IF_OBFF_LOGLVL_LOW {
        snprintf(_logbuf, BUFF_SIZE,
                 "    CORRECTED COORDINATION FOR ANGLE %d-%d-%d (IDX)... WAS "
                 "%d NOW %d\n",
                 a->GetIdx(), b->GetIdx(), c->GetIdx(), parameterB->_ipar[0],
                 coordination);
        OBFFLog(_logbuf);
      }
    }
    ...
    if (coordination > 7) {
      ...
      if (0) {
        if (currentTheta >= 155.0) { // axial ligands = linear
          ...
          anglecalc.theta0 = 180.0 * DEG_TO_RAD;
          anglecalc.c1 = 1.0;
        } else if (currentTheta < 155.0 && currentTheta >= 110.0) { // distal
      equatorial anglecalc.coord = 7; // like sp3 anglecalc.theta0 = 144.0 *
      DEG_TO_RAD; anglecalc.c1 = 1.0; } else if (currentTheta < 110.0 &&
      currentTheta >= 85.0) { // axial-equatorial anglecalc.coord = 4; // like
      sq. planar or octahedral anglecalc.theta0 = 90.0 * DEG_TO_RAD;
          anglecalc.c1 = 1.0;
      ...
      */
      ...
      anglecalc.c0 =
          anglecalc.c2 * (2.0 * anglecalc.cosT0 * anglecalc.cosT0 + 1.0);
    }
    ...
    anglecalc.ka =
        (664.12 * KCAL_TO_KJ) * (anglecalc.zi * anglecalc.zk / (pow(rac, 5.0)));
    anglecalc.ka *=
        (3.0 * rab * rbc * (1.0 - anglecalc.cosT0 * anglecalc.cosT0) -
         rac * rac * anglecalc.cosT0);
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    if (parameterB == nullptr || parameterC == nullptr) {
      IF_OBFF_LOGLVL_LOW {
        snprintf(
            _logbuf, BUFF_SIZE,
            "    COULD NOT FIND PARAMETERS FOR TORSION X-%d-%d-X (IDX)...\n",
            b->GetIdx(), c->GetIdx());
        OBFFLog(_logbuf);
      }
      return false;
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    if (EQn(b->GetType(), "N_3", 3) || EQn(b->GetType(), "N_2", 3) ||
        EQn(b->GetType(), "N_R", 3) || EQn(b->GetType(), "O_2", 3) ||
        EQn(b->GetType(), "O_R", 3)) {
      oopcalc.c0 = 1.0;
      oopcalc.c1 = -1.0;
      oopcalc.c2 = 0.0;
      oopcalc.koop = 6.0 * KCAL_TO_KJ;
    } else if (EQn(b->GetType(), "P_3+3", 5) || EQn(b->GetType(), "As3+3", 5) ||
               EQn(b->GetType(), "Sb3+3", 5) || EQn(b->GetType(), "Bi3+3", 5)) {
    ...
    if (EQn(b->GetType(), "C_2", 3) || EQn(b->GetType(), "C_R", 3)) {
      ...
      oopcalc.c2 = 0.0;
      oopcalc.koop = 6.0 * KCAL_TO_KJ;
      if (EQn(a->GetType(), "O_2", 3) || EQn(c->GetType(), "O_2", 3) ||
          EQn(d->GetType(), "O_2", 3)) {
...
bool OBForceFieldUFF::SetupElectrostatics() {
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    // These are also the Gasteiger charges, not the Qeq mentioned in the UFF
    // paper
    elecalc.qq =
        KCAL_TO_KJ * 332.0637 * a->GetPartialCharge() * b->GetPartialCharge();
...
bool OBForceFieldUFF::ParseParamFile() {
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (EQn(buffer, "param", 5)) {
      ...
      char coord =
          vs[1][1] ? vs[1][2] : '\0'; // 3rd character of atom type, if any
      switch (coord) {
      case '1': // linear
...
bool OBForceFieldUFF::SetTypes() {
    ...
    if (EQn(buffer, "atom", 4)) {
      ...
      if (sp->Init(vs[1])) {
        ...
        obErrorLog.ThrowError(__FUNCTION__,
                              " Could not parse atom type table from UFF.prm",
                              obInfo);
        return false;
      }
    }
  ...
  FOR_ATOMS_OF_MOL(a, _mol) {
    if (a->GetAtomicNum() == 15) {
      ...
      a->SetType("Lw6+3"); // prevent a crash with atoms beyond the
                           // parameterization Avogadro PR#741
    }
  }
...
OBFFParameter *
OBForceFieldUFF::GetParameterUFF(std::string a,
