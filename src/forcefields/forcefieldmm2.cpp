...
double OBForceFieldMM2::E_Bond() {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    parameter = GetParameter(atoi(a->GetType()), atoi(b->GetType()), 0, 0,
                             _ffbondparams);
    if (parameter == nullptr) {
      obErrorLog.ThrowError(__FUNCTION__, "Could not find all bond parameters",
                            obError);
      return 0.0;
    }
    ...
    delta = l - l_ref;
    delta2 = delta * delta;
    e = bondunit * force * delta2 *
        (1.0 + bond_cubic * delta + bond_quartic * delta2);
    energy += e;
    f = 2.0 * bondunit * force * delta *
        (1.0 + 1.5 * bond_cubic * delta + 2.0 * bond_quartic * delta2);
    vf = f * ab.normalize();
    forces[a->GetIdx()] -= vf;
    forces[b->GetIdx()] += vf;

    // sprintf(errbuf, "%sforce=%f  l_ref=%f  l=%f  delta=%f  E=%f (( ab(%f, %f,
    // %f) vf(%f, %f, %f) ))\n", errbuf, force, l_ref, l, delta, e, ab.x(),
    // ab.y(), ab.z(), vf.x(), vf.y(), vf.z()); // DEBUG sprintf(errbuf,
...
double OBForceFieldMM2::E_Angle() {
  ...
  double e, energy, force, ang, ang_ref, delta, delta2, delta3, delta4, f,
      dotprod, rab, rab2, rab3, rbc, rbc2, rbc3, root, dotprod_abbc;
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    parameter = GetParameter(atoi(a->GetType()), atoi(b->GetType()),
                             atoi(c->GetType()), 0, _ffangleparams);
    if (parameter == nullptr) {
      obErrorLog.ThrowError(__FUNCTION__, "Could not find all angle parameters",
                            obError);
      return 0.0;
    }
    ...
    // sprintf(errbuf, "%sforce=%f  ang_ref=%f  ang=%f  delta=%f  E=%f
    // (%d-%d-%d)  (( vfa(%f, %f, %f)  vfc(%f, %f, %f) dotprod=%f))\n",
    //   errbuf, force, ang_ref, ang, delta, e, a->GetIdx(), b->GetIdx(),
    //   c->GetIdx(), vfa.x(), vfa.y(), vfa.z(), vfc.x(), vfc.z(), dotprod); //
    //   DEBUG
  }
...
double OBForceFieldMM2::E_StrBnd() {
  ...
  OBAtom *a, *b, *c;
  OBBond *b1, *b2;
  double e, energy, force, ang, ang_ref, l1, l2, l_ref1, l_ref2, delta_a,
      delta_b1, delta_b2;
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    if (parameter == nullptr) {
      obErrorLog.ThrowError(
          __FUNCTION__, "Could not find all stretch-bend parameters", obError);
      return 0.0;
    }
    ...
    parameter = GetParameter(atoi(b->GetType()), atoi(a->GetType()), 0, 0,
                             _ffbondparams);
    l_ref1 = parameter->_dpar[0];

    parameter = GetParameter(atoi(b->GetType()), atoi(c->GetType()), 0, 0,
                             _ffbondparams);
    ...
    energy += e;
    // sprintf(errbuf, "%sforce=%f  ang_ref=%f  ang=%f  l_ref1=%f  l1=%f
    // l_ref2=%f  l2=%f  E=%f\n", errbuf, force, ang_ref, ang, l_ref1, l1,
    // l_ref2, l2, e); // DEBUG
  }
...
double OBForceFieldMM2::E_Torsion() {
  OBFFParameter *parameter;
  OBAtom *a, *b, *c, *d;
  double e, energy, tor, v1, v2, v3, cosine, cosine2, cosine3, sine, sine2,
      sine3, phi1, phi2, phi3, f, rc1, rc12, rc13, rc2, rc22, rc23, dot_c1c2,
      dot_rc1rc2, root;
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    parameter =
        GetParameter(atoi(a->GetType()), atoi(b->GetType()), atoi(c->GetType()),
                     atoi(d->GetType()), _fftorsionparams);
    if (parameter == nullptr) {
      obErrorLog.ThrowError(__FUNCTION__,
                            "Could not find all torsion parameters", obError);
      return 0.0;
    }
    ...
    // sprintf(errbuf, "%sv1=%f  v2=%f  v3=%f  tor=%f  cosine=%f  cosine2=%f
    ...
    for (int idx = 0; idx < _ffoutplanebendparams.size(); idx++) {
      if (atoi(b->GetType()) == _ffoutplanebendparams[idx].a) {
        ...
        if (atoi(d->GetType()) == _ffoutplanebendparams[idx].b) {
          ...
          angle = Point2PlaneAngle(d->GetVector(), a->GetVector(),
                                   b->GetVector(), c->GetVector());
          // angle = PointPlaneAngle(a->GetVector(), b->GetVector(),
          // c->GetVector(), d->GetVector());
          angle2 = angle * angle;
          e = outplanebendunit * force * angle2;
          energy += e;

          // sprintf(errbuf, "%sout-of-plane: a-b-c-d  %d-%d-%d-%d  angle=%f
          // E=%f\n", errbuf, a->GetIdx(), b->GetIdx(), c->GetIdx(),
          // d->GetIdx(), angle, e); // DEBUG
        }
        if (atoi(a->GetType()) == _ffoutplanebendparams[idx].b) {
          // a <-> d
          force = _ffoutplanebendparams[idx]._dpar[0];
          angle = Point2PlaneAngle(a->GetVector(), d->GetVector(),
                                   b->GetVector(), c->GetVector());
          // angle = PointPlaneAngle(d->GetVector(), b->GetVector(),
          // c->GetVector(), a->GetVector());
          angle2 = angle * angle;
          e = outplanebendunit * force * angle2;
          energy += e;

          // sprintf(errbuf, "%sout-of-plane: a-b-c-d  %d-%d-%d-%d  angle=%f
          // E=%f\n", errbuf, d->GetIdx(), b->GetIdx(), c->GetIdx(),
          // a->GetIdx(), angle, e); // DEBUG
        }
        if (atoi(c->GetType()) == _ffoutplanebendparams[idx].b) {
          // c <-> d
          force = _ffoutplanebendparams[idx]._dpar[0];
          angle = Point2PlaneAngle(c->GetVector(), a->GetVector(),
                                   b->GetVector(), d->GetVector());
          // angle = PointPlaneAngle(a->GetVector(), b->GetVector(),
          // d->GetVector(), c->GetVector());
          angle2 = angle * angle;
          e = outplanebendunit * force * angle2;
          energy += e;

          // sprintf(errbuf, "%sout-of-plane: a-b-c-d  %d-%d-%d-%d  angle=%f
          // E=%f\n", errbuf, a->GetIdx(), b->GetIdx(), d->GetIdx(),
          // c->GetIdx(), angle, e); // DEBUG
        }
      }
    }
  }
...
double OBForceFieldMM2::E_VDW() {
  ...
  OBAtom *a, *b;
  vector3 va, vb, ab, vf;
  double e, energy, ra, rb, rab, rr, rrab, rrab2, rrab4, rrab6, rrab7, abrr,
      eps, epsa, epsb, f;

  // sprintf(errbuf, "a_expterm=%f  b_expterm=%f c_expter=%fm\n", a_expterm,
  // b_expterm, c_expterm); // DEBUG
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    energy += e;
    f = eps * (a_expterm * b_expterm * exp(-b_expterm * abrr) -
               6.0 * c_expterm * rrab7);
    vf = f * ab.normalize();
    // forces[a->GetIdx()] -= vf;
    // forces[b->GetIdx()] += vf;

    // sprintf(errbuf, "%sepsa=%f  epsb=%f epsilon=%f rab=%f  ra=%f  rb=%f rr=%f
    // E=%f  f=%f\n", errbuf, epsa, epsb, eps, rab, ra, rb, rr, e, f); // DEBUG
  }
  // sprintf(errbuf, "%sVan der Waals interactions: E=%f\n", errbuf, energy); //
  // DEBUG obErrorLog.ThrowError(__FUNCTION__, errbuf, obError); // DEBUG
...
double OBForceFieldMM2::E_Electrostatic() {
  OBAtom *a, *b, *c, *d;
  double e, energy, dipole, dipole2, r2, ri2, rk2, rirkr3, dotp, doti, dotk,
      fik;
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    idx = GetParameterIdx(atoi(a->GetType()), atoi(b->GetType()), 0, 0,
                          _ffdipoleparams);
    if (idx > 0) {
      ...
      FOR_BONDS_OF_MOL(bond2, _mol) {
        ...
        idx = GetParameterIdx(atoi(c->GetType()), atoi(d->GetType()), 0, 0,
                              _ffdipoleparams);
        if (idx > 0) {
          ...
          energy += e;
          // sprintf(errbuf, "%sdipole=%f  dipole2=%f  r=%f cos_a1=%f  cos_a2=%f
          // cos_a3=%f  E=%f\n", errbuf, dipole, dipole2, r, cos_a1, cos_a2,
          // cos_a3, e); // DEBUG
        }
      }
    }
