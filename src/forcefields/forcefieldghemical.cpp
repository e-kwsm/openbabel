...
namespace OpenBabel
{
  ...
  double OBForceFieldGhemical::E_VDW()
  {
    ...
    unsigned int j = 0;
    for (vector<OBFFVDWCalculationGhemical>::iterator i = _vdwcalculations.begin(); i != _vdwcalculations.end(); ++i, ++j) {
      // Cut-off check
      if (_cutoff)
  ...
  double OBForceFieldGhemical::E_Electrostatic()
  {
    ...
    unsigned int j = 0;
    for (vector<OBFFElectrostaticCalculationGhemical>::iterator i = _electrostaticcalculations.begin(); i != _electrostaticcalculations.end(); ++i, ++j) {
      // Cut-off check
      if (_cutoff)
