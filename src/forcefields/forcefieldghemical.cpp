...
namespace OpenBabel {
template <bool gradients> void OBFFBondCalculationGhemical::Compute() {
...
template <bool gradients> double OBForceFieldGhemical::E_Bond() {
  ...
  for (i = _bondcalculations.begin(); i != _bondcalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%s %s    %d   %8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
...
template <bool gradients> void OBFFAngleCalculationGhemical::Compute() {
...
template <bool gradients> double OBForceFieldGhemical::E_Angle() {
  ...
  for (i = _anglecalculations.begin(); i != _anglecalculations.end(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               ...
               (*i).theta, (*i).theta0, (*i).ka, (*i).delta, (*i).energy);
      OBFFLog(_logbuf);
    }
  }
...
template <bool gradients> void OBFFTorsionCalculationGhemical::Compute() {
...
template <bool gradients> double OBForceFieldGhemical::E_Torsion() {
...
template <bool gradients> void OBFFVDWCalculationGhemical::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
  ...
  if (gradients) {
    ...
    const double dE =
        -(12.0 / sigma12) * (1.0 / term13) + (6.0 / sigma6) * (1.0 / term7);
    OBForceField::VectorSelfMultiply(force_a, dE);
    OBForceField::VectorSelfMultiply(force_b, dE);
  }
}

template <bool gradients> double OBForceFieldGhemical::E_VDW() {
...
template <bool gradients> void OBFFElectrostaticCalculationGhemical::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
...
template <bool gradients> double OBForceFieldGhemical::E_Electrostatic() {
...
OBForceFieldGhemical::operator=(OBForceFieldGhemical &src) {
  _mol = src._mol;
  _init = src._init;
...
bool OBForceFieldGhemical::SetupCalculations() {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    parameter = GetParameterGhemical(bondtype, a->GetType(), b->GetType(),
                                     nullptr, nullptr, _ffbondparams);
    if (parameter == nullptr) {
      parameter = GetParameterGhemical(bondtype, "FFFF", a->GetType(), nullptr,
                                       nullptr, _ffbondparams);
      if (parameter == nullptr) {
        parameter = GetParameterGhemical(bondtype, "FFFF", b->GetType(),
                                         nullptr, nullptr, _ffbondparams);
        if (parameter == nullptr) {
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    parameter = GetParameter(a->GetType(), b->GetType(), c->GetType(), nullptr,
                             _ffangleparams);
    if (parameter == nullptr) {
      parameter = GetParameter("FFFF", b->GetType(), c->GetType(), nullptr,
                               _ffangleparams);
      if (parameter == nullptr) {
        parameter = GetParameter(a->GetType(), b->GetType(), "FFFF", nullptr,
                                 _ffangleparams);
        if (parameter == nullptr) {
          parameter = GetParameter("FFFF", b->GetType(), "FFFF", nullptr,
                                   _ffangleparams);
          if (parameter == nullptr) {
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    parameter =
        GetParameterGhemical(torsiontype, a->GetType(), b->GetType(),
                             c->GetType(), d->GetType(), _fftorsionparams);
    if (parameter == nullptr) {
      parameter =
          GetParameterGhemical(torsiontype, "FFFF", b->GetType(), c->GetType(),
                               d->GetType(), _fftorsionparams);
      if (parameter == nullptr) {
        parameter =
            GetParameterGhemical(torsiontype, a->GetType(), b->GetType(),
                                 c->GetType(), "FFFF", _fftorsionparams);
        if (parameter == nullptr) {
          parameter =
              GetParameterGhemical(torsiontype, "FFFF", b->GetType(),
                                   c->GetType(), "FFFF", _fftorsionparams);
          if (parameter == nullptr) {
            torsioncalc.V = 0.0;
            torsioncalc.s = 1.0;
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    */
    ...
    vdwcalc.sigma12 =
        (vdwcalc.Ra + vdwcalc.Rb) * pow(1.0 * vdwcalc.kab, 1.0 / 12.0);
    vdwcalc.sigma6 =
        (vdwcalc.Ra + vdwcalc.Rb) * pow(2.0 * vdwcalc.kab, 1.0 / 6.0);
    ...
    if (EQn(buffer, "atom", 4)) {
      ...
      if (sp->Init(vs[1]))
        ...
        obErrorLog.ThrowError(
            __FUNCTION__, " Could not parse atom type table from ghemical.prm",
            obInfo);
        return false;
      }
    }
...
bool OBForceFieldGhemical::SetPartialCharges() {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    for (unsigned int idx = 0; idx < _ffchargeparams.size(); ++idx) {
      if (((_a == _ffchargeparams[idx]._a) &&
           (_b == _ffchargeparams[idx]._b)) &&
          (bondtype == _ffchargeparams[idx]._ipar[0])) {
        a->SetPartialCharge(a->GetPartialCharge() -
                            _ffchargeparams[idx]._dpar[0]);
        b->SetPartialCharge(b->GetPartialCharge() +
                            _ffchargeparams[idx]._dpar[0]);
      } else if (((_a == _ffchargeparams[idx]._b) &&
                  (_b == _ffchargeparams[idx]._a)) &&
                 (bondtype == _ffchargeparams[idx]._ipar[0])) {
        a->SetPartialCharge(a->GetPartialCharge() +
                            _ffchargeparams[idx]._dpar[0]);
        b->SetPartialCharge(b->GetPartialCharge() -
...
OBFFParameter *OBForceFieldGhemical::GetParameterGhemical(
    int type, const char *a, const char *b, const char *c, const char *d,
  ...
  if (d == nullptr) {
    ...
    for (unsigned int idx = 0; idx < parameter.size(); ++idx) {
      if (((_a == parameter[idx]._a) && (_b == parameter[idx]._b) &&
           (_c == parameter[idx]._c) && (type == parameter[idx]._ipar[0])) ||
