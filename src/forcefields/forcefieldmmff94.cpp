...
//
template <bool gradients> inline void OBFFBondCalculationMMFF94::Compute() {
  ...
  if (gradients) {
    ...
    const double dE =
        143.9325 * kb * delta * (1.0 - 3.0 * delta + 14.0 / 3.0 * delta2);
...
template <bool gradients> double OBForceFieldMMFF94::E_Bond() {
  double energy = 0.0;

  IF_OBFF_LOGLVL_HIGH {
    OBFFLog("\nB O N D   S T R E T C H I N G\n\n");
    OBFFLog("ATOM TYPES   FF    BOND       IDEAL       FORCE\n");
    OBFFLog(" I    J     CLASS  LENGTH     LENGTH     CONSTANT      DELTA      "
            "ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "------\n");
  }
...
#pragma omp parallel for reduction(+ : energy)
  ...
  for (int i = 0; i < _bondcalculations.size(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%2d   %2d      %d   %8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
               atoi(_bondcalculations[i].a->GetType()),
               atoi(_bondcalculations[i].b->GetType()), _bondcalculations[i].bt,
               _bondcalculations[i].rab, _bondcalculations[i].r0,
  ...
  IF_OBFF_LOGLVL_MEDIUM {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL BOND STRETCHING ENERGY = %8.5f %s\n",
...
//
template <bool gradients> inline void OBFFAngleCalculationMMFF94::Compute() {
  ...
  if (gradients) {
    theta = OBForceField::VectorAngleDerivative(pos_a, pos_b, pos_c, force_a,
                                                force_b, force_c);

    if (!isfinite(theta))
      theta = 0.0; // doesn't explain why GetAngle is returning NaN but solves
                   // it for us;
...
template <bool gradients> double OBForceFieldMMFF94::E_Angle() {
  double energy = 0.0;

  IF_OBFF_LOGLVL_HIGH {
    OBFFLog("\nA N G L E   B E N D I N G\n\n");
    OBFFLog("ATOM TYPES        FF    VALENCE     IDEAL      FORCE\n");
    OBFFLog(" I    J    K     CLASS   ANGLE      ANGLE     CONSTANT      DELTA "
            "     ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "-----------\n");
  }
...
#pragma omp parallel for reduction(+ : energy)
  ...
  for (int i = 0; i < _anglecalculations.size(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(
          _logbuf, BUFF_SIZE,
          "%2d   %2d   %2d      %d   %8.3f   %8.3f     %8.3f   %8.3f   %8.3f\n",
          atoi(_anglecalculations[i].a->GetType()),
          atoi(_anglecalculations[i].b->GetType()),
...
// /\0_ijk 	see above
//
template <bool gradients> inline void OBFFStrBndCalculationMMFF94::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b, idx_c)) {
    energy = 0.0;
    return;
  }
  ...
  if (gradients) {
    theta = OBForceField::VectorAngleDerivative(
        pos_a, pos_b, pos_c, force_abc_a, force_abc_b, force_abc_c);
    rab = OBForceField::VectorDistanceDerivative(pos_a, pos_b, force_ab_a,
                                                 force_ab_b);
    rbc = OBForceField::VectorDistanceDerivative(pos_b, pos_c, force_bc_b,
                                                 force_bc_c);
  } else {
    theta = OBForceField::VectorAngle(pos_a, pos_b, pos_c);
    rab = OBForceField::VectorDistance(pos_a, pos_b);
  ...
  if (!isfinite(theta))
    theta = 0.0; // doesn't explain why GetAngle is returning NaN but solves it
                 // for us;
  ...
  if (gradients) {
    // grada = 2.51210 * (kbaABC * rab_da * delta_theta + RAD_TO_DEG * theta_da
    // * (kbaABC * delta_rab + kbaCBA * delta_rbc));
    ...
    // gradc = 2.51210 * (kbaCBA * rbc_dc * delta_theta + RAD_TO_DEG * theta_dc
    // * (kbaABC * delta_rab + kbaCBA * delta_rbc));
...
template <bool gradients> double OBForceFieldMMFF94::E_StrBnd() {
  double energy = 0.0;
  ...
  IF_OBFF_LOGLVL_HIGH {
    ...
    OBFFLog(" I    J    K     CLASS   ANGLE      ANGLE        I J        J K   "
            "   ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "---------\n");
  }
...
#pragma omp parallel for reduction(+ : energy)
  ...
  for (int i = 0; i < _strbndcalculations.size(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(
          _logbuf, BUFF_SIZE,
          "%2d   %2d   %2d     %2d   %8.3f   %8.3f   %8.3f   %8.3f   %8.3f\n",
          atoi(_strbndcalculations[i].a->GetType()),
          atoi(_strbndcalculations[i].b->GetType()),
  ...
  IF_OBFF_LOGLVL_MEDIUM {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL STRETCH BENDING ENERGY = %8.5f %s\n", 2.51210 * energy,
             GetUnit().c_str());
    OBFFLog(_logbuf);
  }
...
// MMFF part I - page 495
//
// ET_ijkl = 0.5 ( V1 (1 + cos(0_ijkl)) + V2 (1 - cos(2 0_ijkl)) + V3 (1 + cos(3
// 0_ijkl)) )
//
...
//
template <bool gradients> inline void OBFFTorsionCalculationMMFF94::Compute() {
  ...
  if (gradients) {
    tor = OBForceField::VectorTorsionDerivative(
        pos_a, pos_b, pos_c, pos_d, force_a, force_b, force_c, force_d);
    if (!isfinite(tor))
      tor = 1.0e-3;
...
template <bool gradients> double OBForceFieldMMFF94::E_Torsion() {
  double energy = 0.0;

  IF_OBFF_LOGLVL_HIGH {
    OBFFLog("\nT O R S I O N A L\n\n");
    OBFFLog("ATOM TYPES             FF     TORSION       FORCE CONSTANT\n");
    OBFFLog(" I    J    K    L     CLASS    ANGLE         V1   V2   V3     "
            "ENERGY\n");
    OBFFLog("------------------------------------------------------------------"
            "--\n");
  }
...
#ifndef _OPENMP
    if (gradients) {
      AddGradient(_torsioncalculations[i].force_a,
                  _torsioncalculations[i].idx_a);
      AddGradient(_torsioncalculations[i].force_b,
                  _torsioncalculations[i].idx_b);
      AddGradient(_torsioncalculations[i].force_c,
                  _torsioncalculations[i].idx_c);
      AddGradient(_torsioncalculations[i].force_d,
                  _torsioncalculations[i].idx_d);
    }
#endif

    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%2d   %2d   %2d   %2d      %d   %8.3f   %6.3f   %6.3f   %6.3f  "
               " %8.3f\n",
               atoi(_torsioncalculations[i].a->GetType()),
               atoi(_torsioncalculations[i].b->GetType()),
               atoi(_torsioncalculations[i].c->GetType()),
               atoi(_torsioncalculations[i].d->GetType()),
               _torsioncalculations[i].tt, _torsioncalculations[i].tor,
               _torsioncalculations[i].v1, _torsioncalculations[i].v2,
               _torsioncalculations[i].v3,
               0.5 * _torsioncalculations[i].energy);
      OBFFLog(_logbuf);
    }
  }
...
//  c						//
//						//
template <bool gradients> void OBFFOOPCalculationMMFF94::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b, idx_c, idx_d)) {
    energy = 0.0;
    return;
  }
  ...
  if (gradients) {
    angle = OBForceField::VectorOOPDerivative(
        pos_a, pos_b, pos_c, pos_d, force_a, force_b, force_c, force_d);

    dE =
        (-1.0 * RAD_TO_DEG * 0.043844 * angle * koop) / cos(angle * DEG_TO_RAD);
  ...
  if (!isfinite(angle))
    angle = 0.0; // doesn't explain why GetAngle is returning NaN but solves it
                 // for us;
...
template <bool gradients> double OBForceFieldMMFF94::E_OOP() {
  double energy = 0.0;
...
#pragma omp parallel for reduction(+ : energy)
  ...
  for (int i = 0; i < _oopcalculations.size(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE,
               "%2d   %2d   %2d   %2d      0   %8.3f   %8.3f     %8.3f\n",
               atoi(_oopcalculations[i].a->GetType()),
               atoi(_oopcalculations[i].b->GetType()),
               atoi(_oopcalculations[i].c->GetType()),
               atoi(_oopcalculations[i].d->GetType()),
  ...
  IF_OBFF_LOGLVL_MEDIUM {
    snprintf(_logbuf, BUFF_SIZE,
             "     TOTAL OUT-OF-PLANE BENDING ENERGY = %8.5f %s\n",
...
template <bool gradients> inline void OBFFVDWCalculationMMFF94::Compute() {
  if (OBForceField::IgnoreCalculation(idx_a, idx_b)) {
    energy = 0.0;
    return;
  }
  ...
  if (gradients) {
    rab =
        OBForceField::VectorDistanceDerivative(pos_a, pos_b, force_a, force_b);
  } else {
    rab = OBForceField::VectorDistance(pos_a, pos_b);
  }
...
template <bool gradients> double OBForceFieldMMFF94::E_VDW() {
  double energy = 0.0;
  ...
  for (int i = 0; i < _vdwcalculations.size(); ++i) {
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE, "%2d   %2d     %8.3f  %8.3f  %8.3f  %8.3f\n",
               atoi(_vdwcalculations[i].a->GetType()),
               atoi(_vdwcalculations[i].b->GetType()), _vdwcalculations[i].rab,
               _vdwcalculations[i].R_AB, _vdwcalculations[i].epsilon,
...
template <bool gradients> double OBForceFieldMMFF94::E_Electrostatic() {
  double energy = 0.0;
  ...
  for (int i = 0; i < _electrostaticcalculations.size(); ++i) {
...
#ifndef _OPENMP
    if (gradients) {
      AddGradient(_electrostaticcalculations[i].force_a,
                  _electrostaticcalculations[i].idx_a);
      AddGradient(_electrostaticcalculations[i].force_b,
                  _electrostaticcalculations[i].idx_b);
    }
#endif
    ...
    IF_OBFF_LOGLVL_HIGH {
      snprintf(_logbuf, BUFF_SIZE, "%2d   %2d   %8.3f  %8.3f  %8.3f  %8.3f\n",
               ...
               _electrostaticcalculations[i].rab,
               _electrostaticcalculations[i].a->GetPartialCharge(),
               _electrostaticcalculations[i].b->GetPartialCharge(),
               _electrostaticcalculations[i].energy);
      OBFFLog(_logbuf);
    }
  }
...
bool OBForceFieldMMFF94::PerceiveAromatic() {
  ...
  for (ri = vr.begin(); ri != vr.end(); ++ri) { // for each ring
    ...
    for (rj = (*ri)->_path.begin(); rj != (*ri)->_path.end();
         ++rj) { // for each ring atom
      index = *rj;
      ...
      FOR_NBORS_OF_ATOM(nbr, ringatom) {
        ...
        if (!nbr->IsAromatic()) {
          if (ringatom->GetAtomicNum() == OBElements::Carbon &&
              ringatom->IsInRingSize(5) && ringatom->IsInRingSize(6) &&
              nbr->GetAtomicNum() == OBElements::Carbon &&
              nbr->IsInRingSize(5) && nbr->IsInRingSize(6)) {
            c60++;
          } else {
            continue;
          }
      ...
      if (ringsize == 5 && ringatom->GetIdx() == (*ri)->GetRootAtom())
        pi_electrons += 2;
...
int OBForceFieldMMFF94::GetType(OBAtom *atom) {
  ...
  if (atom->IsAromatic()) {
    if (atom->IsInRingSize(5)) {
      ...
      FOR_NBORS_OF_ATOM(nbr, atom) {
        if (!((_mol.GetBond(atom, &*nbr))->IsAromatic()) ||
            !nbr->IsInRingSize(5))
          continue;
        ...
        FOR_NBORS_OF_ATOM(nbrNbr, &*nbr) {
          if (nbrNbr->GetIdx() == atom->GetIdx())
            continue;
          if (!((_mol.GetBond(&*nbr, &*nbrNbr))->IsAromatic()) ||
              !nbrNbr->IsInRingSize(5))
            continue;
      ...
      if (IsAromatic) {
        ...
        if (!betaAtoms.size()) {
          ...
          FOR_NBORS_OF_ATOM(nbr, atom) {
            if (nbr->GetAtomicNum() == OBElements::Nitrogen &&
                (nbr->GetExplicitDegree() == 3)) {
              if ((nbr->GetExplicitValence() == 4) && nbr->IsAromatic()) {
                nitrogenCount++;
              } else if ((nbr->GetExplicitValence() == 3) &&
                         !nbr->IsAromatic()) {
                nitrogenCount++;
              }
            }
        ...
        if (!alphaAtoms.size() && !betaAtoms.size()) {
          if (atom->GetAtomicNum() == OBElements::Carbon) {
            bool c60 = true; // special case to ensure c60 is typed correctly --
                             // Paolo Tosco
            FOR_NBORS_OF_ATOM(nbr, atom) {
              if (!(nbr->GetAtomicNum() == OBElements::Carbon &&
                    nbr->IsAromatic() && nbr->IsInRingSize(6)))
                c60 = false;
            }
            if (c60)
              return 37; // correct atom type for c in c60 (all atoms symmetric)
            // there is no S:, O:, or N:
            // this is the case for anions with only carbon and nitrogen in the
            // ring
        ...
        if (alphaAtoms.size() == 2) {
          if (atom->GetAtomicNum() == OBElements::Carbon &&
              IsInSameRing(alphaAtoms[0], alphaAtoms[1])) {
            ...
            for (unsigned int j = 0; j < betaAtoms.size(); j++) {
              if (!IsInSameRing(alphaAtoms[i], betaAtoms[j])) {
                if (atom->GetAtomicNum() == OBElements::Carbon) {
                  ...
                  return 79; // General nitrogen in 5-membered aromatic ring
                             // (N5)
                }
              }
            }
  ...
  if (atom->GetAtomicNum() == 1) {
    FOR_NBORS_OF_ATOM(nbr, atom) {
      ...
      if (nbr->GetAtomicNum() == OBElements::Oxygen) {
        ...
        return 21; // Hydroxyl hydrogen in alcohols, Generic hydroxyl hydrogen
                   // (HOR, HO)
      }
      if (nbr->GetAtomicNum() == OBElements::Nitrogen) {
        switch (GetType(&*nbr)) {
        ...
        case 43:
          return 28; // Hydrogen on NSO, NSO2, or NSO3 nitrogen, Hydrogen on N
                     // triply bonded to C (HNSO, HNC%)
        case 39:
          return 23; // Hydrogen on nitrogen in pyrrole (HPYL)
        case 8:
          return 23; // Generic hydrogen on sp3 nitrogen, e.g., in amines,
                     // Hydrogen on nitrogen in ammonia (HNR, H3N)
        }
        ...
        if (nbr->GetExplicitValence() == 4) {
          if (nbr->GetExplicitDegree() == 2) {
            ...
            return 36; // Hydrogen on pyridinium nitrogen, Hydrogen on
                       // protonated imine nitrogen (HPD+, HNC+)
          }
        }
          ...
          FOR_NBORS_OF_ATOM(nbrNbr, &*nbr) {
            ...
            if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
              if (nbrNbr->GetAtomicNum() == OBElements::Carbon ||
                ...
                return 27; // Hydrogen on imine nitrogen, Hydrogen on azo
                           // nitrogen (HN=C, HN=N)
              }
            ...
            FOR_NBORS_OF_ATOM(nbrNbrNbr, &*nbrNbr) {
              ...
              if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
                if (nbrNbrNbr->GetAtomicNum() == OBElements::Carbon ||
                  ...
                  return 28; // Hydrogen on amide nitrogen, Hydrogen on
                             // thioamide nitrogen,
                  // Hydrogen on enamine nitrogen, Hydrogen in H-N-C=N moiety
                  // (HNCO, HNCS, HNCC, HNCN)
                }
              }
            }
          ...
          if (nbrNbr->GetAtomicNum() == OBElements::Nitrogen) {
            FOR_NBORS_OF_ATOM(nbrNbrNbr, &*nbrNbr) {
              ...
              if (!bond->IsAromatic() && bond->GetBondOrder() == 2) {
                if (nbrNbrNbr->GetAtomicNum() == OBElements::Carbon ||
                    nbrNbrNbr->GetAtomicNum() == OBElements::Nitrogen) {
                  return 28; // Hydrogen in H-N-N=C moiety, Hydrogen in H-N-N=N
                             // moiety (HNNC, HNNN)
                }
              }
            }
      ...
      if (nbr->GetAtomicNum() == OBElements::Sulfur ||
        ...
        return 71; // Hydrogen attached to sulfur, Hydrogen attached to >S=
                   // sulfur doubly bonded to N,
  ...
  if (atom->GetAtomicNum() == 6) {
    ...
    if (atom->GetExplicitDegree() == 3) {
      ...
      FOR_NBORS_OF_ATOM(nbr, atom) {
        ...
        } else if ((nbr->GetExplicitDegree() == 2) && !bond->IsAromatic() &&
                   bond->GetBondOrder() == 2) {
          if (nbr->GetAtomicNum() == OBElements::Nitrogen) {
            N2count++;
          }
      ...
      if ((N3count >= 2) &&
          (doubleBondTo == 7 ||
           (!(doubleBondTo == 6) && atom->GetExplicitDegree() == 3 &&
            N3fcharge == 1)) &&
          !N2count && !oxygenCount && !sulphurCount) {
        // N3==C--N3
        return 57; // Guanidinium carbon, Carbon in +N=C-N: resonance structures
                   // (CGD+, CNN+)
      }
      if ((oxygenCount == 2) || (sulphurCount == 2)) {
        // O1-?-C-?-O1 or S1-?-C-?-S1
        return 41; // Carbon in carboxylate anion, Carbon in thiocarboxylate
                   // anion (CO2M, CS2M)
      }
      if (atom->IsInRingSize(4) && (doubleBondTo == 6)) {
        return 30; // Olefinic carbon in 4-membered ring (CR4E)
      }
      if ((doubleBondTo == 7) || (doubleBondTo == 8) || (doubleBondTo == 15) ||
          (doubleBondTo == 16)) {
        // C==N, C==O, C==P, C==S
        return 3; // Generic carbonyl carbon, Imine-type carbon, Guanidine
                  // carbon,
  ...
  if (atom->GetAtomicNum() == 7) {
    ...
    if (atom->GetExplicitDegree() == 3) {
      if (atom->GetExplicitValence() >=
          4) { // > because we accept -N(=O)=O as a valid nitro group
        ...
        if (oxygenCount >= 2) {
          return 45; // Nitrogen in nitro group, Nitrogen in nitrate group (NO2,
                     // NO3)
        }
        ...
        if (doubleBondTo == 7) {
          return 54; // Positivly charged nitrogen doubly bonded to nitrogen
                     // (N+=N)
        }
      }
      ...
      if (atom->GetExplicitValence() == 3) {
        ...
        FOR_NBORS_OF_ATOM(nbr, atom) {
          if (nbr->GetAtomicNum() == OBElements::Carbon) {
            ...
            FOR_NBORS_OF_ATOM(nbrNbr, &*nbr) {
              ...
              if (bond->IsAromatic()) {
                if ((nbrNbr->GetAtomicNum() == 7) ||
                    (nbrNbr->GetAtomicNum() == 6)) {
                  doubleBondTo = nbrNbr->GetAtomicNum();
                }
              }
            ...
            if (!IsAmide && !IsSulfonAmide && !oxygenCount && !sulphurCount &&
                nbr->IsAromatic()) {
              return 40;
            }
                ...
                if (nbrNbr->GetAtomicNum() == OBElements::Carbon) {
                  ...
                  if (!oxygenCount && !sulphurCount && (nitrogenCount == 1)) {
                    ...
                    FOR_NBORS_OF_ATOM(nbr2, atom) {
                      if (nbr2->IsAromatic() &&
                          nbr2->GetAtomicNum() == OBElements::Carbon &&
                          nbr2->IsInRingSize(6)) {
                        bondToAromC = true;
                      }
                    }
        ...
        if ((doubleBondTo == 6) || (doubleBondTo == 7) ||
            (doubleBondTo == 15) || (tripleBondTo == 6)) {
          return 40; // Enamine or aniline nitrogen (deloc. lp), Nitrogen in
                     // N-C=N with deloc. lp,
          // Nitrogen in N-C=N with deloc. lp, Nitrogen attached to C-C triple
          // bond (NC=C, NC=N, NC=P, NC%C)
        }
        if (tripleBondTo == 7) {
    ...
    if (atom->GetExplicitDegree() == 2) {
      ...
      if (atom->GetExplicitValence() == 3) {
        ...
        FOR_NBORS_OF_ATOM(nbr, atom) {
          bond = _mol.GetBond(&*nbr, atom);
          if (nbr->GetAtomicNum() == OBElements::Oxygen &&
              !bond->IsAromatic() && bond->GetBondOrder() == 2 &&
              (nbr->GetExplicitDegree() == 1)) {
            return 46; // Nitrogen in nitroso group (N=O)
          }
          if ((nbr->GetAtomicNum() == OBElements::Carbon ||
               nbr->GetAtomicNum() == OBElements::Nitrogen) &&
              !bond->IsAromatic() && bond->GetBondOrder() == 2) {
      ...
      if (atom->GetExplicitValence() >= 2) { // Bug reported by Paolo Tosco
        ...
        FOR_NBORS_OF_ATOM(nbr, atom) {
          if (nbr->GetAtomicNum() == OBElements::Sulfur) {
            ...
            if (oxygenCount == 1) {
              return 48; // Divalent nitrogen replacing monovalent O in SO2
                         // group (NSO)
            }
          }
        }
  ...
  if (atom->GetAtomicNum() == 8) {
    ...
    if (atom->GetExplicitDegree() == 2) {
      ...
      return 6; // Generic divalent oxygen, Ether oxygen, Carboxylic acid or
                // ester oxygen,
      ...
      FOR_NBORS_OF_ATOM(nbr, atom) {
        ...
        if (nbr->GetAtomicNum() == OBElements::Carbon) {
          ...
          if (!bond->IsAromatic() && bond->GetBondOrder() == 1) {
            // O--C
            return 35; // Oxide oxygen on sp3 carbon, Oxide oxygen on sp2 carbon
                       // (OM, OM2)
          } else {
            // O==C
        ...
        if (nbr->GetAtomicNum() == OBElements::Sulfur) {
          ...
          if (!bond->IsAromatic() && bond->GetBondOrder() == 1) {
            // O--S
            return 32; // Single terminal oxygen on sulfur, One of 2 terminal
                       // O's on sulfur,
            ...
            FOR_NBORS_OF_ATOM(nbr2, &*nbr) {
              ...
              if (!bond->IsAromatic() && bond->GetBondOrder() == 2 &&
                  nbr2->GetAtomicNum() == OBElements::Carbon &&
                  oxygenBoundToSulfur == 1)
                isSulfoxide =
                    false; // O=S on sulfur doubly bonded to, e.g., C (O=S=)

              if ((nbr2->GetAtomicNum() == OBElements::Oxygen &&
                   nbr2->GetExplicitDegree() == 1) ||
                  (nbr2->GetAtomicNum() == OBElements::Nitrogen &&
                   nbr2->GetExplicitDegree() == 2))
                isSulfoxide = false;
            }
        ...
        return 32; // Oxygen in phosphine oxide, One of 2 terminal O's on
                   // sulfur,
  ...
  if (atom->GetAtomicNum() == 15) {
    if (atom->GetExplicitDegree() == 4) {
      return 25; // Phosphate group phosphorus, Phosphorus with 3 attached
                 // oxygens,
...
bool OBForceFieldMMFF94::SetupCalculations() {
  ...
  FOR_BONDS_OF_MOL(bond, _mol) {
    ...
    // if there are any groups specified, check if the two bond atoms are in a
    // single intraGroup
    if (HasGroups()) {
    ...
    parameter =
        GetTypedParameter2Atom(bondtype, atoi(a->GetType()), atoi(b->GetType()),
                               _ffbondparams); // from mmffbond.par
    if (parameter == nullptr) {
      parameter = GetParameter2Atom(
      ...
      if (parameter == nullptr) {
        IF_OBFF_LOGLVL_LOW {
          ...
          snprintf(_logbuf, BUFF_SIZE,
                   "    COULD NOT FIND PARAMETERS FOR BOND %d-%d (IDX)...\n",
                   a->GetIdx(), b->GetIdx());
          OBFFLog(_logbuf);
        }
        return false;
        ...
        IF_OBFF_LOGLVL_LOW {
          snprintf(
              _logbuf, BUFF_SIZE,
              "   USING EMPIRICAL RULE FOR BOND STRETCHING %d-%d (IDX)...\n",
              a->GetIdx(), b->GetIdx());
          OBFFLog(_logbuf);
        }
        ...
        bondcalc.kb =
            parameter->_dpar[1] * rr6; // parameter->_dpar[1]  = kb-ref
  ...
  //
  // First try and find an exact match, if this fails, step down using the
  // equivalences from mmffdef.par five-stage protocol: 1-1-1, 2-2-2, 3-2-3,
  // 4-2-4, 5-2-5 If this fails, use empirical rules Since 1-1-1 = 2-2-2, we
  // will only try 1-1-1 before going to 3-2-3
  //
  ...
  FOR_ANGLES_OF_MOL(angle, _mol) {
    ...
    // if there are any groups specified, check if the three angle atoms are in
    // a single intraGroup
    if (HasGroups()) {
    ...
    // try exact match
    parameter = GetTypedParameter3Atom(angletype, type_a, type_b, type_c,
                                       _ffangleparams);
    if (parameter == nullptr) // try 3-2-3
      parameter = GetTypedParameter3Atom(angletype, EqLvl3(type_a), type_b,
                                         EqLvl3(type_c), _ffangleparams);
    if (parameter == nullptr) // try 4-2-4
      parameter = GetTypedParameter3Atom(angletype, EqLvl4(type_a), type_b,
                                         EqLvl4(type_c), _ffangleparams);
    if (parameter == nullptr) // try 5-2-5
      parameter = GetTypedParameter3Atom(angletype, EqLvl5(type_a), type_b,
                                         EqLvl5(type_c), _ffangleparams);
    ...
    if (parameter) {
      ...
      IF_OBFF_LOGLVL_LOW {
        snprintf(_logbuf, BUFF_SIZE,
                 "   USING DEFAULT ANGLE FOR %d-%d-%d (IDX)...\n", a->GetIdx(),
                 b->GetIdx(), c->GetIdx());
        snprintf(
            _logbuf, BUFF_SIZE,
            "   USING EMPIRICAL RULE FOR ANGLE BENDING %d-%d-%d (IDX)...\n",
    ...
    if (anglecalc.ka == 0.0) {
      IF_OBFF_LOGLVL_LOW {
        snprintf(_logbuf, BUFF_SIZE,
                 "   USING EMPIRICAL RULE FOR ANGLE BENDING FORCE CONSTANT "
                 "%d-%d-%d (IDX)...\n",
                 a->GetIdx(), b->GetIdx(), c->GetIdx());
        OBFFLog(_logbuf);
      }
      ...
      // PR#2741669
      anglecalc.ka = (beta * Za * Cb * Zc * exp(-2 * D)) /
                     (rr * theta2 * DEG_TO_RAD * DEG_TO_RAD);
    }
    ...
    parameter = GetTypedParameter3Atom(strbndtype, type_a, type_b, type_c,
                                       _ffstrbndparams);
    if (parameter == nullptr) {
      int rowa, rowb, rowc;
      ...
      // IF_OBFF_LOGLVL_LOW {
      //  snprintf(_logbuf, BUFF_SIZE, "   USING EMPIRICAL RULE FOR
      //  STRETCH-BENDING FORCE CONSTANT %d-%d-%d (IDX)...\n", a->GetIdx(),
      //  b->GetIdx(), c->GetIdx()); OBFFLog(_logbuf);
      //}
      ...
      if (parameter == nullptr) {
        ...
        IF_OBFF_LOGLVL_LOW {
          snprintf(_logbuf, BUFF_SIZE,
                   "    COULD NOT FIND PARAMETERS FOR STRETCH-BEND %d-%d-%d "
                   "(IDX)...\n",
                   a->GetIdx(), b->GetIdx(), c->GetIdx());
          OBFFLog(_logbuf);
        }
    ...
    // Set the pointers to addresses in the anglecalc, find the matching
    // bondcalcs and do the same. This should improve performance by not
    // calculating all this twice. We could do the same for torsion and angles
    // since the bond lengths are calculated for bond stretching first.
    ...
    */
    ...
    /*
      ...
      for (ai = _anglecalculations.begin(); ai != _anglecalculations.end();
      ++ai) { if ( (((*ai).a)->GetIdx() == a->GetIdx()) && (((*ai).b)->GetIdx()
      == b->GetIdx()) && (((*ai).c)->GetIdx() == c->GetIdx()) ) {
      ...
      found_angle = true;
      break;
      } else if ( (((*ai).a)->GetIdx() == c->GetIdx()) && (((*ai).b)->GetIdx()
      == b->GetIdx()) && (((*ai).c)->GetIdx() == a->GetIdx()) ) {
      strbndcalc.theta = (*ai).theta;
      ...
      if (!found_angle) // didn't find matching angle, shouldn't happen, but
      continue to be safe continue;
      ...
      {
      // find rab
      if ( (((*bi).a)->GetIdx() == a->GetIdx()) && (((*bi).b)->GetIdx() ==
      b->GetIdx()) ) { strbndcalc.rab = &((*bi).rab); strbndcalc.force_ab_a =
      (*bi).force_a; strbndcalc.force_ab_b = (*bi).force_b; found_rab = true; }
      else if ( (((*bi).a)->GetIdx() == b->GetIdx()) && (((*bi).b)->GetIdx() ==
      a->GetIdx()) ) { strbndcalc.rab = &((*bi).rab); strbndcalc.force_ab_a =
      (*bi).force_b; strbndcalc.force_ab_b = (*bi).force_a; found_rab = true;
      }
      // find rbc
      if ( (((*bi).a)->GetIdx() == b->GetIdx()) && (((*bi).b)->GetIdx() ==
      c->GetIdx()) ) { strbndcalc.rbc = &(bondcalc.rab); strbndcalc.force_ab_a =
      (*bi).force_a; strbndcalc.force_ab_b = (*bi).force_b; found_rbc = true; }
      else if ( (((*bi).a)->GetIdx() == c->GetIdx()) && (((*bi).b)->GetIdx() ==
      b->GetIdx()) ) { strbndcalc.rbc = &(bondcalc.rab); strbndcalc.force_ab_a =
      (*bi).force_b; strbndcalc.force_ab_b = (*bi).force_a; found_rbc = true;
      }

      if (found_rab && found_rbc)
      break;
      }
      if (!found_rab || !found_rbc) // didn't find matching bond, or atoms
      overlap continue;
    */
  ...
  //
  // First try and find an exact match, if this fails, step down using the
  // equivalences from mmffdef.par five-stage protocol: 1-1-1-1, 2-2-2-2,
  // 3-2-2-5, 5-2-2-3, 5-2-2-5 If this fails, use empirical rules Since 1-1-1-1
  // = 2-2-2-2, we will only try 1-1-1-1 before going to 3-2-2-5
  //
  IF_OBFF_LOGLVL_LOW
  ...
  FOR_TORSIONS_OF_MOL(t, _mol) {
    ...
    // if there are any groups specified, check if the four torsion atoms are in
    // a single intraGroup
    if (HasGroups()) {
    ...
    if (order >= 0) {
      // try exact match
      parameter = GetTypedParameter4Atom(torsiontype, type_a, type_b, type_c,
                                         type_d, _fftorsionparams);
      if (parameter == nullptr) // try 3-2-2-5
        parameter =
            GetTypedParameter4Atom(torsiontype, EqLvl3(type_a), type_b, type_c,
                                   EqLvl5(type_d), _fftorsionparams);
      if (parameter == nullptr) // try 5-2-2-3
        parameter =
            GetTypedParameter4Atom(torsiontype, EqLvl5(type_a), type_b, type_c,
                                   EqLvl3(type_d), _fftorsionparams);
      ...
      // try exact match
      parameter = GetTypedParameter4Atom(torsiontype, type_d, type_c, type_b,
                                         type_a, _fftorsionparams);
      if (parameter == nullptr) // try 3-2-2-5
        parameter =
            GetTypedParameter4Atom(torsiontype, EqLvl3(type_d), type_c, type_b,
                                   EqLvl5(type_a), _fftorsionparams);
      if (parameter == nullptr) // try 5-2-2-3
        parameter =
            GetTypedParameter4Atom(torsiontype, EqLvl5(type_d), type_c, type_b,
                                   EqLvl3(type_a), _fftorsionparams);
      if (parameter == nullptr) // try 5-2-2-5
        parameter =
            GetTypedParameter4Atom(torsiontype, EqLvl5(type_d), type_c, type_b,
                                   EqLvl5(type_a), _fftorsionparams);
    ...
    if (parameter) {
      ...
      // IF_OBFF_LOGLVL_LOW {
      //   snprintf(_logbuf, BUFF_SIZE, "   USING EMPIRICAL RULE FOR TORSION
      //   FORCE CONSTANT %d-%d-%d-%d (IDX)...\n",
      ...
      if (b->GetBond(c)->IsAromatic()) {
        ...
        if (((GetMltb(type_b) == 2) && (GetMltb(type_c) == 2)) &&
            !bond->IsAromatic() && bond->GetBondOrder() == 2)
          pi_bc = 1.0;
        else
          pi_bc = 0.4;
      ...
      if (!found_rule)
        if (((GetCrd(type_b) == 4) && (GetCrd(type_c) != 4))) {
          if (GetCrd(type_c) == 3) // case (1)
            if ((GetVal(type_c) == 4) || (GetVal(type_c) == 34) ||
                (GetMltb(type_c) != 0))
              continue;
      ...
      if (!found_rule)
        if (((GetCrd(type_b) != 4) && (GetCrd(type_c) == 4))) {
          if (GetCrd(type_b) == 3) // case (1)
            if ((GetVal(type_b) == 4) || (GetVal(type_b) == 34) ||
                (GetMltb(type_b) != 0))
              continue;
        ...
        if (!bond->IsAromatic() && bond->GetBondOrder() == 1 &&
          ...
          if (!found_rule)
            if (((GetMltb(type_b) == 1) || (GetMltb(type_c) == 1)) &&
                (b->GetAtomicNum() != OBElements::Carbon ||
                 c->GetAtomicNum() != OBElements::Carbon)) {
              pi_bc = 0.4;
              found_rule = true;
            }
      ...
      if (!found_rule) {
        if ((b->GetAtomicNum() == OBElements::Oxygen ||
             b->GetAtomicNum() == OBElements::Sulfur) &&
            (c->GetAtomicNum() == OBElements::Oxygen ||
             c->GetAtomicNum() == OBElements::Sulfur)) {
          double Wb, Wc;
          ...
          IF_OBFF_LOGLVL_LOW {
            snprintf(_logbuf, BUFF_SIZE,
                     "   USING EMPIRICAL RULE FOR TORSION FORCE CONSTANT "
                     "%d-%d-%d-%d (IDX)...\n",
                     a->GetIdx(), b->GetIdx(), c->GetIdx(), d->GetIdx());
            OBFFLog(_logbuf);
          }
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    for (unsigned int idx = 0; idx < _ffoopparams.size(); idx++) {
      if (type_b == _ffoopparams[idx].b) {
        ...
        // if there are any groups specified, check if the four oop atoms are in
        // a single intraGroup
        if (HasGroups()) {
          bool validOOP = false;
          for (unsigned int i = 0; i < _intraGroup.size(); ++i) {
        ...
        if (((type_a == _ffoopparams[idx].a) &&
             (type_c == _ffoopparams[idx].c) &&
             (type_d == _ffoopparams[idx].d)) ||
            ((type_c == _ffoopparams[idx].a) &&
             (type_a == _ffoopparams[idx].c) &&
             (type_d == _ffoopparams[idx].d)) ||
            ((type_c == _ffoopparams[idx].a) &&
             (type_d == _ffoopparams[idx].c) &&
             (type_a == _ffoopparams[idx].d)) ||
            ((type_d == _ffoopparams[idx].a) &&
             (type_c == _ffoopparams[idx].c) &&
             (type_a == _ffoopparams[idx].d)) ||
            ((type_a == _ffoopparams[idx].a) &&
             (type_d == _ffoopparams[idx].c) &&
        ...
        if ((_ffoopparams[idx].a == 0) && (_ffoopparams[idx].c == 0) &&
            (_ffoopparams[idx].d == 0) && !found) // *-XX-*-*
        {
          oopcalc.koop = _ffoopparams[idx]._dpar[0];
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    // if there are any groups specified, check if the two atoms are in a single
    // _interGroup or if two two atoms are in one of the _interGroups pairs.
    if (HasGroups()) {
      bool validVDW = false;
    ...
    if (parameter_a == nullptr || parameter_b == nullptr) {
      IF_OBFF_LOGLVL_LOW {
        snprintf(
            _logbuf, BUFF_SIZE,
            "   COULD NOT FIND VAN DER WAALS PARAMETERS FOR %d-%d (IDX)...\n",
            a->GetIdx(), b->GetIdx());
        OBFFLog(_logbuf);
      }
    ...
    if (vdwcalc.aDA == 1) { // hydrogen bond donor
      ...
      if (vdwcalc.bDA == 2) { // hydrogen bond acceptor
        vdwcalc.epsilon = 0.5 *
                          (181.16 * vdwcalc.Ga * vdwcalc.Gb * vdwcalc.alpha_a *
                           vdwcalc.alpha_b) /
                          (sqrt_a + sqrt_b) * (1.0 / R_AB6);
        // R_AB is scaled to 0.8 for D-A interactions. The value used in the
        // calculation of epsilon is not scaled.
        vdwcalc.R_AB = 0.8 * vdwcalc.R_AB;
      } else
        vdwcalc.epsilon = (181.16 * vdwcalc.Ga * vdwcalc.Gb * vdwcalc.alpha_a *
                           vdwcalc.alpha_b) /
                          (sqrt_a + sqrt_b) * (1.0 / R_AB6);
      ...
      g_AB2 = g_AB * g_AB;
      vdwcalc.R_AB =
          0.5 * (R_AA + R_BB) * (1.0 + 0.2 * (1.0 - exp(-12.0 * g_AB2)));
      R_AB2 = vdwcalc.R_AB * vdwcalc.R_AB;
      R_AB4 = R_AB2 * R_AB2;
      R_AB6 = R_AB4 * R_AB2;
      vdwcalc.R_AB7 = R_AB6 * vdwcalc.R_AB;
  ...
  FOR_PAIRS_OF_MOL(p, _mol) {
    ...
    elecalc.qq =
        332.0716 * a->GetPartialCharge() * b->GetPartialCharge() / _epsilon;
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    if (type == 56) {
      ...
      FOR_NBORS_OF_ATOM(nbr, &*atom) {
        FOR_NBORS_OF_ATOM(nbr2, &*nbr) {
          ...
          if (nbr2->GetAtomicNum() == OBElements::Sulfur &&
              (nbr2->GetExplicitDegree() == 1))
            s_count++;
          if (nbr2->GetAtomicNum() == OBElements::Nitrogen &&
              !nbr2->IsAromatic())
            sulfonamide = true;
          OBBond *bond = nbr->GetBond(&*nbr2);
          if (nbr2->GetAtomicNum() == OBElements::Carbon &&
              !bond->IsAromatic() && bond->GetBondOrder() == 2)
            sulfone_s_c = true;
        }
      ...
      FOR_NBORS_OF_ATOM(nbr, &*atom) {
        ...
        if (nbr->GetAtomicNum() == OBElements::Phosphorus ||
            nbr->GetAtomicNum() == OBElements::Sulfur) {
          FOR_NBORS_OF_ATOM(nbr2, &*nbr)
          if ((nbr2->GetAtomicNum() == OBElements::Sulfur ||
      ...
      for (ri = vr.begin(); ri != vr.end(); ++ri) { // for each ring
        ...
        if ((*ri)->IsAromatic() && (*ri)->IsMember(&*atom) &&
            ((*ri)->Size() == 5)) {
          for (rj = (*ri)->_path.begin(); rj != (*ri)->_path.end();
               ++rj) // for each ring atom
...
bool OBForceFieldMMFF94::SetPartialCharges() {
  ...
  FOR_ATOMS_OF_MOL(atom, _mol) {
    ...
    if (nbr->GetPartialCharge() < 0.0)
      q0a +=
          nbr->GetPartialCharge() / (2.0 * (double)(nbr->GetExplicitDegree()));
    ...
    FOR_NBORS_OF_ATOM(nbr, &*atom) {
      ...
      for (unsigned int idx = 0; idx < _ffchgparams.size(); idx++)
        if (GetBondType(&*atom, &*nbr) == _ffchgparams[idx]._ipar[0]) {
          if ((type == _ffchgparams[idx].a) &&
              (nbr_type == _ffchgparams[idx].b)) {
            Wab += -_ffchgparams[idx]._dpar[0];
            bci_found = true;
          } else if ((type == _ffchgparams[idx].b) &&
                     (nbr_type == _ffchgparams[idx].a)) {
            Wab += _ffchgparams[idx]._dpar[0];
            bci_found = true;
          }
        }
...
bool OBForceFieldMMFF94::Validate() {
  ...
  if (!format_in || !conv.SetInFormat(format_in)) {
    obErrorLog.ThrowError(__FUNCTION__, "Could not set mol2 input format",
                          obError);
    return false;
  }
  ...
  if (!ifs) {
    obErrorLog.ThrowError(__FUNCTION__, "Could not open ./MMFF94_dative.mol2",
                          obError);
    return false;
  }
  ...
  if (!ifs2) {
    obErrorLog.ThrowError(__FUNCTION__, "Coulg not open ./MMFF_opti.log",
                          obError);
    return false;
  }
  ...
  if (!ofs) {
    obErrorLog.ThrowError(__FUNCTION__, "Coulg not open ./MMFF_openbabel.log",
                          obError);
    return false;
  }
  ...
  for (unsigned int c = 1;; c++) {
    ...
    for (i = types.begin(); i != types.end(); ++i) {
      ...
      if (atoi(_mol.GetAtom(ni)->GetType()) == (*i))
        snprintf(_logbuf, BUFF_SIZE,
                 "%2d   %3d  %4d    %3d      %3d          PASSED",
                 _mol.GetAtom(ni)->GetIdx(), _mol.GetAtom(ni)->GetHyb(),
                 _mol.GetAtom(ni)->IsAromatic(),
                 atoi(_mol.GetAtom(ni)->GetType()), *i);
      else {
        snprintf(_logbuf, BUFF_SIZE,
                 "%2d   %3d  %4d    %3d      %3d      XXX FAILED XXX",
                 _mol.GetAtom(ni)->GetIdx(), _mol.GetAtom(ni)->GetHyb(),
                 _mol.GetAtom(ni)->IsAromatic(),
                 atoi(_mol.GetAtom(ni)->GetType()), *i);
        failed = true;
    ...
    for (di = fcharges.begin(); di != fcharges.end(); ++di) {
      ...
      if (fabs((*di) - _mol.GetAtom(ni)->GetPartialCharge()) <= 0.001)
        snprintf(_logbuf, BUFF_SIZE, "%2d   %7.4f     %7.4f          PASSED",
                 _mol.GetAtom(ni)->GetIdx(),
                 _mol.GetAtom(ni)->GetPartialCharge(), *di);
      else {
        snprintf(_logbuf, BUFF_SIZE,
                 "%2d   %7.4f     %7.4f      XXX FAILED XXX",
                 _mol.GetAtom(ni)->GetIdx(),
                 _mol.GetAtom(ni)->GetPartialCharge(), *di);
        failed = true;
      }
    ...
    delta = (E_Bond() - ebond);
    snprintf(_logbuf, BUFF_SIZE,
             "Bond Stretching        %11.5f    %11.5f   %11.5f", E_Bond(),
             ebond, delta);
    cout << _logbuf << endl;

    delta = (E_Angle() - eangle);
    snprintf(_logbuf, BUFF_SIZE,
             "Angle Bending          %11.5f    %11.5f   %11.5f", E_Angle(),
             eangle, delta);
    cout << _logbuf << endl;

    delta = (E_StrBnd() - estbn);
    snprintf(_logbuf, BUFF_SIZE,
             "Stretch-Bending        %11.5f    %11.5f   %11.5f", E_StrBnd(),
             estbn, delta);
    cout << _logbuf << endl;

    delta = (E_OOP() - eoop);
    snprintf(_logbuf, BUFF_SIZE,
             "Out-Of-Plane Bending   %11.5f    %11.5f   %11.5f", E_OOP(), eoop,
             delta);
    cout << _logbuf << endl;

    delta = (E_Torsion() - etor);
    snprintf(_logbuf, BUFF_SIZE,
             "Torsional              %11.5f    %11.5f   %11.5f", E_Torsion(),
             etor, delta);
    cout << _logbuf << endl;

    delta = (E_VDW() - evdw);
    snprintf(_logbuf, BUFF_SIZE,
             "Van der Waals          %11.5f    %11.5f   %11.5f", E_VDW(), evdw,
             delta);
    cout << _logbuf << endl;

    delta = (E_Electrostatic() - eeq);
    snprintf(_logbuf, BUFF_SIZE,
             "Electrostatic          %11.5f    %11.5f   %11.5f",
             E_Electrostatic(), eeq, delta);
    cout << _logbuf << endl;

    cout << endl;
    delta = (Energy() - etot);
    snprintf(_logbuf, BUFF_SIZE,
             "Total ENERGY           %11.5f    %11.5f   %11.5f", Energy(), etot,
             delta);
    cout << _logbuf << endl;
...
bool OBForceFieldMMFF94::ValidateGradients() {
  ...
  cout << "                                                                    "
          "                    "
       << endl;
  cout << "ATOM IDX      NUMERICAL GRADIENT           ANALYTICAL GRADIENT      "
          "  REL. ERROR (%)   "
       << endl;
  cout << "--------------------------------------------------------------------"
          "--------------------"
       << endl;
  //     "XX       (000.000, 000.000, 000.000)  (000.000, 000.000, 000.000)
  //     (00.00, 00.00, 00.00)"
  ...
  FOR_ATOMS_OF_MOL(a, _mol) {
    ...
    Energy(); // compute
    anagrad.Set(_gradientPtr[coordIdx], _gradientPtr[coordIdx + 1],
                _gradientPtr[coordIdx + 2]);
    err = ValidateGradientError(numgrad, anagrad);

    snprintf(_logbuf, BUFF_SIZE,
             "%2d       (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  (%5.2f, "
             "%5.2f, %5.2f)\n",
             a->GetIdx(), numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(),
             anagrad.y(), anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    bond    (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    angle   (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    strbnd  (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    torsion (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    oop     (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    // disable OOP gradient validation for now -- some small errors, but nothing
    // major
    //      if (err.x() > 5.0 || err.y() > 5.0 || err.z() > 5.0)
    //        passed = false;
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    vdw     (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
    ...
    snprintf(_logbuf, BUFF_SIZE,
             "    electro (%7.3f, %7.3f, %7.3f)  (%7.3f, %7.3f, %7.3f)  "
             "(%5.2f, %5.2f, %5.2f)\n",
             numgrad.x(), numgrad.y(), numgrad.z(), anagrad.x(), anagrad.y(),
             anagrad.z(), err.x(), err.y(), err.z());
    OBFFLog(_logbuf);
...
//
// BTij is 1 when:
// a) single bond between atoms i and j, both i and j are not aromatic and both
// types have sbmb set in mmffprop.par, or b) bewtween two aromatic atoms, but
// the bond is not aromatic (e.g. connecting bond in biphenyl)
//
...
// TTijkl = 5 when i, j, k and l are members of a five-membered ring and at
// least one is a sp3-hybridized carbon (MMFF atom type 1)
//
int OBForceFieldMMFF94::GetTorsionType(OBAtom *a, OBAtom *b, OBAtom *c,
                                       OBAtom *d) {
  int btab, btbc, btcd;
  ...
  if (bond->GetBondOrder() == 1 && !bond->IsAromatic()) {
    if (btab || btcd)
      return 2;
    /*
      unsigned int order1 = GetCXT(0, atoi(d->GetType()), atoi(c->GetType()),
      atoi(b->GetType()), atoi(a->GetType())); unsigned int order2 = GetCXT(0,
      atoi(a->GetType()), atoi(b->GetType()), atoi(c->GetType()),
      atoi(d->GetType()));

      cout << "GetTorsionType(" << a->GetType() << ", " << b->GetType() << ", "
      << c->GetType() << ", " << d->GetType() << ")" << endl; cout << " order1 =
      " << order1 << endl; cout << "    order2 = " << order2 << endl; cout << "
      btab = " << btab << endl; cout << "    btbc = " << btbc << endl; cout << "
      btcd = " << btcd << endl;
    */
  }

  if (a->IsInRingSize(5) && b->IsInRingSize(5) && c->IsInRingSize(5) &&
      d->IsInRingSize(5)) {
    vector<OBRing *> vr;
    vr = _mol.GetSSSR();

    if (!((atoi(a->GetType()) == 1) || (atoi(b->GetType()) == 1) ||
          (atoi(c->GetType()) == 1) || (atoi(d->GetType()) == 1)))
      return 0;
    ...
    for (ri = vr.begin(); ri != vr.end(); ++ri) { // for each ring
      ...
      if (!(*ri)->IsMember(a) || !(*ri)->IsMember(b) || !(*ri)->IsMember(c) ||
          !(*ri)->IsMember(d))
        continue;
...
double OBForceFieldMMFF94::GetBondLength(OBAtom *a, OBAtom *b) {
  ...
  parameter = GetTypedParameter2Atom(GetBondType(a, b), atoi(a->GetType()),
                                     atoi(b->GetType()), _ffbondparams);
  if (parameter == nullptr)
...
double OBForceFieldMMFF94::GetRuleBondLength(OBAtom *a, OBAtom *b) {
  ...
  if (a->GetAtomicNum() == OBElements::Hydrogen ||
      b->GetAtomicNum() == OBElements::Hydrogen)
    c = 0.050;
  else
    c = 0.085;
  ...
  if (GetMltb(atoi(a->GetType()) == 3))
    Ha = 1;
  else if ((GetMltb(atoi(a->GetType())) == 1) ||
           (GetMltb(atoi(a->GetType())) == 2))
    Ha = 2;
  else
    Ha = 3;
  ...
  if (GetMltb(atoi(b->GetType()) == 3))
    Hb = 1;
  else if ((GetMltb(atoi(b->GetType())) == 1) ||
           (GetMltb(atoi(b->GetType())) == 2))
    Hb = 2;
  else
    Hb = 3;
  ...
  /*
    ...
    cout << "c=" << c << "  |Xa-Xb|=" << fabs(Xa-Xb) << "  |Xa-Xb|^1.4=" <<
    pow(fabs(Xa-Xb), 1.4) << endl;
  */
...
OBFFParameter *
OBForceFieldMMFF94::GetParameter1Atom(int a,
...
OBFFParameter *
OBForceFieldMMFF94::GetParameter2Atom(int a, int b,
                                      std::vector<OBFFParameter> &parameter) {
  OBFFParameter *par;
...
OBFFParameter *
OBForceFieldMMFF94::GetParameter3Atom(int a, int b, int c,
                                      std::vector<OBFFParameter> &parameter) {
  OBFFParameter *par;
...
OBFFParameter *OBForceFieldMMFF94::GetTypedParameter2Atom(
    int ffclass, int a, int b, std::vector<OBFFParameter> &parameter) {
  OBFFParameter *par;
...
OBFFParameter *OBForceFieldMMFF94::GetTypedParameter3Atom(
    int ffclass, int a, int b, int c, std::vector<OBFFParameter> &parameter) {
  OBFFParameter *par;

  for (unsigned int idx = 0; idx < parameter.size(); idx++)
    if (((a == parameter[idx].a) && (b == parameter[idx].b) &&
         (c == parameter[idx].c) && (ffclass == parameter[idx]._ipar[0])) ||
        ((a == parameter[idx].c) && (b == parameter[idx].b) &&
         (c == parameter[idx].a) && (ffclass == parameter[idx]._ipar[0]))) {
      par = &parameter[idx];
      return par;
    }
...
OBFFParameter *OBForceFieldMMFF94::GetTypedParameter4Atom(
    int ffclass, int a, int b, int c, int d,
    std::vector<OBFFParameter> &parameter) {
  OBFFParameter *par;
