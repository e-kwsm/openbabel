...
protected:
  ...
  bool SetupVDWCalculation(OBAtom *a, OBAtom *b,
  ...
  OBFFParameter *GetParameterUFF(std::string a,
...
public:
  //! Constructor
  explicit OBForceFieldUFF(const char *ID, bool IsDefault = true)
  ...
  const char *Description() override { return "Universal Force Field."; }

  //! Get the unit in which the energy is expressed
  std::string GetUnit() override {
    return std::string(
        "kJ/mol"); // Note that we convert from kcal/mol internally
  }
