...
public:
  ...
  void
  CreateNewClassVector(std::vector<std::pair<OBAtom *, unsigned int>> &vp1,
                       std::vector<std::pair<OBAtom *, unsigned int>> &vp2);
  static void
  CreateNewClassVector(OBMol *mol,
                       std::vector<std::pair<OBAtom *, unsigned int>> &vp1,
                       std::vector<std::pair<OBAtom *, unsigned int>> &vp2);
  void GetGIVector(std::vector<unsigned int> &vid);
  bool GetGTDVector(std::vector<int> &gtd);
  static void
  CountAndRenumberClasses(std::vector<std::pair<OBAtom *, unsigned int>> &vp,
                          unsigned int &count);
  int ExtendInvariants(
  ...
  void CanonicalLabels(const std::vector<unsigned int> &symmetry_classes,
                       std::vector<unsigned int> &canon_labels, int maxSeconds);
};

OBGraphSym::OBGraphSym(OBMol *pmol, const OBBitVec *frag_atoms)
    : d(new OBGraphSymPrivate) {
  d->_pmol = pmol;
  if (frag_atoms) {
...
OBGraphSym::~OBGraphSym() { delete d; }
...
inline bool ComparePairFirst(const std::pair<OBAtom *, unsigned int> &a,
                             const std::pair<OBAtom *, unsigned int> &b) {
  return (a.first->GetIdx() < b.first->GetIdx());
}

inline bool ComparePairSecond(const std::pair<OBAtom *, unsigned int> &a,
                              const std::pair<OBAtom *, unsigned int> &b) {
...
unsigned int OBGraphSymPrivate::GetHvyDegree(OBAtom *atom) {
  ...
  for (bond = atom->BeginBond(bi); bond; bond = atom->NextBond(bi)) {
    ...
    if (_frag_atoms.BitIsSet(nbr->GetIdx()) &&
        nbr->GetAtomicNum() != OBElements::Hydrogen)
      count++;
  }
    ...
    while (!curr.IsEmpty()) {
      ...
      for (natom = curr.NextBit(-1); natom != curr.EndBit();
        ...
        for (bond = atom1->BeginBond(j); bond; bond = atom1->NextBond(j)) {
          ...
          if (_frag_atoms.BitIsSet(nbr_idx) && !used.BitIsSet(nbr_idx) &&
...
void OBGraphSymPrivate::FindRingAtoms(OBBitVec &ring_atoms) {
  ...
  for (ri = sssRings.begin(); ri != sssRings.end(); ++ri) {
    OBRing *ring = *ri;
    OBBitVec bvtmp =
        _frag_atoms & ring->_pathset; // intersection: fragment and ring
...
void OBGraphSymPrivate::GetGIVector(vector<unsigned int> &vid) {
  ...
  // The "graph theoretical distance" for each atom (see comments in the
  // function)
  vector<int> v;
  GetGTDVector(v);
  ...
  for (i = 0, atom = _pmol->BeginAtom(ai); atom; atom = _pmol->NextAtom(ai)) {
    ...
    if (_frag_atoms.BitIsSet(atom->GetIdx())) {
      ...
      unsigned int ring =
          ring_atoms.BitIsSet(atom->GetIdx()) ? 1u : 0u; //  1 bit
      unsigned int z = atom->GetAtomicNum() & 0x7Fu;     //  7 bits
      unsigned int hvyBond = GetHvyBondSum(atom) & 0xFu; //  4 bits
      unsigned int charge =
          static_cast<unsigned int>(7 + atom->GetFormalCharge()) &
          0xFu; // 4 bits
      vid[i] = dist | (hvyDeg << 10) | (aro << 14) | (ring << 15) | (z << 16) |
               (hvyBond << 23) | (charge << 27);
    }
    i++;
  }
}
 ...
 */
void OBGraphSymPrivate::CreateNewClassVector(
    std::vector<std::pair<OBAtom *, unsigned int>> &vp1,
    std::vector<std::pair<OBAtom *, unsigned int>> &vp2) {
  unsigned int m, id;
  OBAtom *atom, *nbr;
  ...
  // Create a new extended varient for each atom.  Get its neighbors' class
  // ID's, sort them into ascending order, and create a sum of (c0 + c1*10^2 +
  // c2*10^4 + ...) which becomes the new class ID (where c0 is the current
  // classID).
  ...
  for (vp_iter = vp1.begin(); vp_iter != vp1.end(); ++vp_iter) {
    ...
    for (nbr = atom->BeginNbrAtom(nbr_iter); nbr;
         nbr = atom->NextNbrAtom(nbr_iter)) {
      int idx = nbr->GetIdx();
...
void OBGraphSymPrivate::CreateNewClassVector(
    OBMol *mol, std::vector<std::pair<OBAtom *, unsigned int>> &vp1,
    std::vector<std::pair<OBAtom *, unsigned int>> &vp2) {
  int m, id;
  OBAtom *atom, *nbr;
  vector<OBBond *>::iterator nbr_iter;
 ...
 */
void OBGraphSymPrivate::CountAndRenumberClasses(
    std::vector<std::pair<OBAtom *, unsigned int>> &vp, unsigned int &count) {
  count = 1;
 ...
 */
int OBGraphSymPrivate::ExtendInvariants(
    std::vector<std::pair<OBAtom *, unsigned int>> &symmetry_classes) {
  unsigned int nclasses1, nclasses2;
  vector<pair<OBAtom *, unsigned int>> tmp_classes;

  // How many classes are we starting with?  (The "renumber" part isn't
  // relevant.)
  ...
  if (nclasses1 < nfragatoms) {
    for (int i = 0; i < 100;
         i++) { // sanity check - shouldn't ever hit this number
 ...
 */
int OBGraphSymPrivate::CalculateSymmetry(
    std::vector<unsigned int> &atom_sym_classes) {
  ...
  for (atom = _pmol->BeginAtom(j); atom; atom = _pmol->NextAtom(j)) {
    ...
    if (_frag_atoms.BitIsSet(idx))
      symmetry_classes.push_back(
          pair<OBAtom *, unsigned int>(atom, vgi[idx - 1]));
    // else
    //   symmetry_classes.push_back(pair<OBAtom*, unsigned int> (atom,
    //   OBGraphSym::NoSymmetryClass));
  }
  ...
  for (unsigned int i = 0; i < symmetry_classes.size(); ++i) {
    atom_sym_classes[symmetry_classes.at(i).first->GetIndex()] =
        symmetry_classes.at(i).second;
  }
...
int OBGraphSym::GetSymmetry(std::vector<unsigned int> &symmetry_classes) {
  ...
  OBPairData *pd = dynamic_cast<OBPairData *>(
      d->_pmol->GetData("OpenBabel Symmetry Classes"));
...
int OBGraphSymPrivate::Iterate(vector<unsigned int> &symClasses) {
  ...
  for (OBAtom *atom = _pmol->BeginAtom(j); atom; atom = _pmol->NextAtom(j)) {
    ...
    if (_frag_atoms.BitIsSet(idx))
      symmetry_classes.push_back(
          pair<OBAtom *, unsigned int>(atom, symClasses[idx - 1]));
  }
  ...
  for (unsigned int i = 0; i < symmetry_classes.size(); ++i) {
    symClasses[symmetry_classes.at(i).first->GetIndex()] =
        symmetry_classes.at(i).second;
  }
