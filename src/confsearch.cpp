...
const unsigned int LFSR::_polynomials[31] = {
    0x3,       0x6,       0xc,       0x14,       0x30,       0x60,
    0xb8,      0x110,     0x240,     0x500,      0x829,      0x100d,
    0x2015,    0x6000,    0xd008,    0x12000,    0x20400,    0x40023,
    0x90000,   0x140000,  0x300000,  0x420000,   0xe10000,   0x1200000,
    0x2000023, 0x4000013, 0x9000000, 0x14000000, 0x20000029, 0x48000000,
    0x80200003};

LFSR::LFSR(unsigned int range, unsigned int start = 1)
    : _range(range), _lfsr(start) {
  assert(_range < (1 << 31)); // We can only handle up to 2^31 - 1
  // (Currently don't use a start value)
  // assert ( start < _range ); // Otherwise the _start value will never be
  // returned
...
public:
  ...
  size_t GetSize();
  inline int GetNRMSD() { return n_rmsd; }
  inline double GetCutoff() { return cutoff; }

private:
  bool _percise;
  std::vector<vector3>
  GetHeavyAtomCoords(const std::vector<vector3> &all_coords);
  unsigned int natoms;
  Tree poses;
...
OBDiversePoses::OBDiversePoses(const OBMol &ref, double RMSD, bool percise)
  ...
  vec.erase(std::remove_if(vec.begin(), vec.end(),
                           std::bind2nd(std::less<double>(), (cutoff + 0.1))),
            vec.end());
...
bool OBDiversePoses::AddPose(double *coords, double energy) {
  ...
  for (unsigned int a = 0; a < natoms; ++a)
    vcoords.push_back(
        vector3(coords[a * 3], coords[a * 3 + 1], coords[a * 3 + 2]));
...
bool OBDiversePoses::AddPose(std::vector<vector3> vcoords, double energy) {
  ...
  while (nodes.size() > 0) { // Using stack-based recursion
    ...
    // - it will already have been tested against at the end of the previous
    // loop
    if (!first_time)
      ++sib;
...
>>>>>>> 73fd347494d1 (style: clang-format)
    ...
    int startlevel = level;
    std::vector<double>::const_iterator n_rmsd = min_nodes_rmsds.begin();
    for (std::vector<Tree_it>::iterator n = min_nodes.begin();
         n != min_nodes.end(); ++n, ++n_rmsd) {
      node = *n;
      rmsd = *n_rmsd;
      level = startlevel + 1;
      while (rmsd < levels.at(level)) {
  ...
  // If we get here, then the molecule has been accepted for addition to the
  // tree
  std::vector<PosePair>::iterator b = insert_data.begin();
  std::vector<int>::iterator c = insert_level.begin();
  for (std::vector<Tree_it>::iterator a = insert_pt.begin();
       a != insert_pt.end(); ++a, ++b, ++c) {
    node = *a;
    for (unsigned int k = *c; k < levels.size(); ++k) {
      node = poses.append_child(node, *b);
    }
...
std::vector<vector3>
OBDiversePoses::GetHeavyAtomCoords(const std::vector<vector3> &all_coords) {
...
// bool sortpred(const OBDiversePoses::PosePair *a, const
// OBDiversePoses::PosePair *b) {
//   return (a->second < b->second);
// }
bool sortpred_b(const OBDiversePoses::PosePair &a,
                const OBDiversePoses::PosePair &b) {
  return (a.second < b.second);
}

std::vector<vector3>
GetHeavyAtomCoords(const OBMol *mol, const std::vector<vector3> &all_coords) {
  std::vector<vector3> v_hvyatoms;
...
void UpdateConformersFromTree(OBMol *mol, std::vector<double> &energies,
                              OBDiversePoses *divposes, bool verbose) {
  ...
  // The leaf iterator will (in effect) iterate over the nodes just at the
  // loweset level
  for (OBDiversePoses::Tree::leaf_iterator node = poses->begin();
       node != poses->end(); ++node)
  ...
  if (verbose)
    std::cout << "....tree size = " << divposes->GetSize()
              << " confs = " << confs.size() << "\n";
  ...
  if (verbose)
    std::cout << "....new tree size = " << newtree.GetSize()
              << " confs = " << newconfs.size() << "\n";

  // Add confs to the molecule's conformer data and add the energies to
  // molecules's energies
  for (vpp::iterator chosen = newconfs.begin(); chosen != newconfs.end();
       ++chosen) {
    energies.push_back(chosen->second);

    // To avoid making copies of vectors or vector3s, I am using pointers
    // throughout
...
int OBForceField::DiverseConfGen(double rmsd, unsigned int nconfs,
                                 double energy_gap, bool verbose) {
  _energies.clear(); // Wipe any energies from previous conformer generators

  // Remove all conformers (e.g. from previous conformer generators) even the
  // current conformer
  ...
  for (unsigned int i = 1; i < rl.Size() + 1;
    ...
    if (verbose) {
      std::cout << "....rotor " << i << " from "
                << rotor->GetBond()->GetBeginAtomIdx() << " to "
                << rotor->GetBond()->GetEndAtomIdx() << " has " << size
                << " values" << std::endl;
    }
  }
  ...
  if (verbose && combinations > max_combinations) {
    std::cout << "....Using a cutoff of " << nconfs << " we will only explore "
              << std::fixed << std::setprecision(1)
              << static_cast<float>(nconfs * 100) /
                     static_cast<float>(combinations)
              << "% of these\n";
  }
  ...
  do {
    ...
    if (currentE <
        lowest_energy + energy_gap) { // Don't retain high energy poses
    ...
    counter++;
           ...
           counter < nconfs); // The LFSR always terminates with a 1
  if (verbose)
...
>>>>>>> 73fd347494d1 (style: clang-format)
  ...
  transform(_energies.begin(), _energies.end(), _energies.begin(),
            bind2nd(std::plus<double>(), energy_offset));
