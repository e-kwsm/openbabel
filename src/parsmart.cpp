...
const int SmartsImplicitRef =
    -9999; // Used as a placeholder when recording atom nbrs for chiral atoms
...
static BondExpr *GenerateDefaultBond(void) { return BuildBondLeaf(BE_DEFAULT); }
...
Pattern *OBSmartsPattern::SMARTSParser(Pattern *pat, ParseState *stat, int prev,
                                       int part) {
  int vb = 0;
  AtomExpr *aexpr;
  BondExpr *bexpr;
        ...
        if (!bexpr) {
          if (!stat->closord[index]) {
            ...
            bexpr = stat->closord[index];
                   ...
                   !EquivalentBondExpr(bexpr, stat->closord[index]))
          return ParseSMARTSError(pat, bexpr);
...
static void MarkGrowBonds(Pattern *pat) {
  ...
  for (i = 0; i < pat->bcount; ++i) {
    pat->bond[i].grow =
        (bv[pat->bond[i].src] && bv[pat->bond[i].dst]) ? false : true;
  ...
  if (pat->ischiral) {
    ...
    for (m = mlist.begin(); m != mlist.end(); ++m) {
      ...
      for (int j = 0; j < pat->acount; ++j) {
        ...
        if (nbrs.size() !=
            4) { // 3 nbrs currently not supported. Other values are errors.
          // stringstream ss;
          // ss << "Ignoring stereochemistry. There are " << nbrs.size() << "
          // connections to this atom instead of 4. Title: " << mol.GetTitle();
        ...
        smartsConfig.refs =
            OBStereo::MakeRefs(firstref, ra2->GetId(), ra3->GetId());
        ...
        // don't waste time checking more stereocenters using this mapping if
        // one didn't match
...
bool OBSmartsMatcher::EvalAtomExpr(AtomExpr *expr, OBAtom *atom) {
  for (;;)
    ...
    case AE_RECUR: {
      ...
      std::vector<std::vector<int>> mlist;
      if (match(*((OBMol *)atom->GetParent()), (Pattern *)expr->recur.recur,
                mlist))
        for (j = mlist.begin(); j != mlist.end(); ++j)
          vb[(*j)[0]] = true;

      RSCACHE.push_back(std::pair<const Pattern *, std::vector<bool>>(
          (const Pattern *)expr->recur.recur, vb));
...
OBSSMatch::~OBSSMatch() { delete[] _uatoms; }
...
void OBSSMatch::Match(std::vector<std::vector<int>> &mlist, int bidx) {
  ...
  {
    OBBond *bond =
        _mol->GetBond(_map[_pat->bond[bidx].src], _map[_pat->bond[bidx].dst]);
...
void SmartsLexReplace(std::string &s,
  ...
  // for (pos = 0,pos = s.find("$",pos);pos != std::string::npos;pos =
  // s.find("$",pos))
  {
    pos++;
