...
std::vector<std::pair<OBSmartsPattern *, std::vector<vector3>>>
    OBBuilder::_ring_fragments;

void OBBuilder::AddRingFragment(OBSmartsPattern *sp,
                                const std::vector<vector3> &coords) {
  bool hasAllZeroCoords = true;
  for (std::size_t i = 0; i < coords.size(); ++i) {
    if (fabs(coords[i].x()) > 10e-8 || fabs(coords[i].y()) > 10e-8 ||
  ...
  if (hasAllZeroCoords) {
    ...
    _ring_fragments.push_back(
        pair<OBSmartsPattern *, vector<vector3>>(sp, coords));
}
...
void OBBuilder::LoadFragments() {
  ...
  if (OpenDatafile(ifs, "rigid-fragments-index.txt").length() == 0) {
    obErrorLog.ThrowError(__FUNCTION__, "Cannot open ring-fragments-index.txt",
                          obError);
    return;
  }
  ...
  if (OpenDatafile(ifs, "ring-fragments.txt").length() == 0) {
    obErrorLog.ThrowError(__FUNCTION__, "Cannot open ring-fragments.txt",
                          obError);
    return;
  }
    ...
    if (vs.size() == 1) { // SMARTS pattern
      ...
      if (!sp->Init(vs[0])) {
        ...
        obErrorLog.ThrowError(
            __FUNCTION__, " Could not parse SMARTS from contribution data file",
            obInfo);
      }
    } else if (vs.size() == 3) { // XYZ coordinates
      vector3 coord(atof(vs[0].c_str()), atof(vs[1].c_str()),
                    atof(vs[2].c_str()));
...
std::vector<vector3> OBBuilder::GetFragmentCoord(std::string smiles) {
  ...
  if (OpenDatafile(ifs, "rigid-fragments.txt").length() == 0) {
    obErrorLog.ThrowError(__FUNCTION__, "Cannot open rigid-fragments.txt",
                          obError);
    return coords;
  }
  ...
  while (ifs.getline(buffer, BUFF_SIZE)) {
    ...
    if (vs.size() == 4) { // XYZ coordinates
      vector3 coord(atof(vs[1].c_str()), atof(vs[2].c_str()),
                    atof(vs[3].c_str()));
      if (fabs(coord.x()) > 10e-8 || fabs(coord.y()) > 10e-8 ||
          fabs(coord.z()) > 10e-8)
        hasAllZeroCoords = false;
      coords.push_back(coord);
  ...
  if (hasAllZeroCoords) {
    std::stringstream ss;
    ss << "Rigid fragment " << smiles
       << " in rigid-fragments.txt has all zero coordinates.";
    obErrorLog.ThrowError(__FUNCTION__, ss.str(), obError);
  }
...
vector3 GetCorrectedBondVector(OBAtom *atom1, OBAtom *atom2,
                               int bondOrder = 1) {
  ...
  if (dimension != 2) {
    ...
    if (atom->GetExplicitDegree() == 1) {
      ...
      if (bond2.length_2() == 0.0 || v1.length_2() == 0.0) {
        ...
        v1 = cross(bond1, vrand); // so find a perpendicular, given the random
                                  // vector (this doesn't matter here)
      }
      v2 = cross(bond1, v1);
      v2 = v2.normalize();
      ...
      if (atom->GetHyb() == 1)
        ...
        newbond =
            bond1; // like 5-coordinate below, we want a 180-degree bond (trans)
      else if (atom->GetHyb() == 5) {
        /* the first two atoms are the axial ones;  the third, fourth, and fifth
         * atom are equatorial */
        newbond = bond1;
    ...
    if (atom->GetExplicitDegree() == 2) {
      ...
      if (atom->GetHyb() == 2)
        ...
        newbond = bond1 - v2 * tan(DEG_TO_RAD * (180.0 - 109.471));
        newbond =
            v2 + v1 * (sqrt(2.0) / 2.0); // used to be tan(70.53 degrees/2)
                                         // which is sqrt(2.0) / 2.0
      } else if (atom->GetHyb() == 5 || atom->GetHyb() == 4) {
        /* add the first equatorial atom, orthogonally to bond1 (and bond2 =
         * -bond1) */
        /* is atom order correct?  I don't think it matters, but I might have to
         * ask a chemist whether PClF4 would be more likely to have an
         * equatorial or axial Cl-P bond */
        vector3 vrand;
        vrand.randomUnitVector();
        double angle = fabs(acos(dot(bond1, vrand)) * RAD_TO_DEG);
        while (angle < 45.0 || angle > 135.0) {
      ...
      newbond = newbond.normalize(); // if newbond was not set, it will become
                                     // non-finite here
      newbond *= length;
      newbond += atom->GetVector();
      return newbond;
    ...
    if (atom->GetExplicitDegree() == 3) {
      ...
      if (atom->GetHyb() ==
          4) { // OK, we want this at -bond3, since bond1 & bond2 are opposite
        FOR_NBORS_OF_ATOM(nbr, atom) {
          if (bond1.length_2() == 0.0)
            bond1 = atom->GetVector() - nbr->GetVector();
          else if (bond2.length_2() == 0.0)
      ...
      if (atom->GetHyb() == 6) {
        ...
        /* the way things work, newbond is equal to bond1, but will show up at
         * -bond1 next time around */
        newbond = bond1;
        newbond = newbond.normalize();
        newbond *= length;
        newbond += atom->GetVector();
    ...
    //                                                                          //
    // hyb * = 2 //
    // ^^^^^^^^^ // make sure we place the new atom trans to a-2 (if there is an
    // a-2 atom)   //
    //                                                                          //
    //   (a-2)             (a-2) //
    //     \                 \                                                  //
    ...
    if (atom->GetExplicitDegree() == 1) {
      ...
      for (OBAtom *nbr2 = nbr->BeginNbrAtom(i); nbr2;
           nbr2 = nbr->NextNbrAtom(i))
        if (nbr2 != atom)
    ...
    //                          //
    if (atom->GetExplicitDegree() == 2) {
      for (OBAtom *nbr = atom->BeginNbrAtom(i); nbr;
           nbr = atom->NextNbrAtom(i)) {
        if (bond1.length_2() == 0.0)
          bond1 = atom->GetVector() - nbr->GetVector();
        else
          bond2 = atom->GetVector() - nbr->GetVector();
...
bool OBBuilder::Connect(OBMol &mol, int idxA, int idxB, vector3 &newpos,
                        int bondOrder) {
  ...
  xyang = vectorAngle(vector3(moldir.x(), moldir.y(), 0.0),
                      vector3(fragdir.x(), fragdir.y(), 0.0));
  if (cross(vector3(moldir.x(), moldir.y(), 0.0),
            vector3(fragdir.x(), fragdir.y(), 0.0))
          .z() > 0) {
    xyang = 180 + xyang;
  } else if (cross(vector3(moldir.x(), moldir.y(), 0.0),
                   vector3(fragdir.x(), fragdir.y(), 0.0))
                 .z() < 0) {
    xyang = 180 - xyang;
  } else {
  ...
  if (!connectedFrag)
    fragdir = crossdir;
  xzang = vectorAngle(vector3(moldir.x(), moldir.z(), 0.0),
                      vector3(fragdir.x(), fragdir.z(), 0.0));
  if (cross(vector3(moldir.x(), moldir.z(), 0.0),
            vector3(fragdir.x(), fragdir.z(), 0.0))
          .z() > 0) {
    xzang = 180 - xzang;
  } else if (cross(vector3(moldir.x(), moldir.z(), 0.0),
                   vector3(fragdir.x(), fragdir.z(), 0.0))
                 .z() < 0) {
    xzang = 180 + xzang;
  } else {
  ...
  if (!connectedFrag)
    fragdir = crossdir;
  yzang = vectorAngle(vector3(moldir.y(), moldir.z(), 0.0),
                      vector3(fragdir.y(), fragdir.z(), 0.0));
  if (cross(vector3(moldir.y(), moldir.z(), 0.0),
            vector3(fragdir.y(), fragdir.z(), 0.0))
          .z() > 0) {
    yzang = 180 + yzang;
  } else if (cross(vector3(moldir.y(), moldir.z(), 0.0),
                   vector3(fragdir.y(), fragdir.z(), 0.0))
                 .z() < 0) {
    yzang = 180 - yzang;
  } else {
  ...
  // e.g. biphenyl
  if (bondOrder == 1 && a->GetHyb() == 2 && b->GetHyb() == 2 && nbr_a && nbr_b)
...
bool OBBuilder::Connect(OBMol &mol, int idxA, int idxB, int bondOrder) {
  vector3 newpos =
      GetCorrectedBondVector(mol.GetAtom(idxA), mol.GetAtom(idxB), bondOrder);
  return Connect(mol, idxA, idxB, newpos, bondOrder);
}

// Variation of OBBuilder::Swap that allows swapping with a vector3 rather than
// an explicit bond. This is useful for correcting stereochemistry at Tet
// Centers where it is sometimes necessary to swap an existing bond with the
// location of an implicit hydrogen (or lone pair), in order to correct the
// stereo.
bool OBBuilder::SwapWithVector(OBMol &mol, int idxA, int idxB, int idxC,
...
// First we find the most complex fragments in our molecule. Once we have a
// match, vfrag is set for all the atoms in the fragment. A second match
// (smaller, more simple part of the 1st match) is ignored.
//
// Next we iterate over the atoms. There are 3 possibilities:
//
// 1) The atom is already added (vdone is set) --> continue
// 2) The atom belongs to a fragment --> a) This is the first atom to add: leave
// fragment as it is
//                                       b) Not the first atom: rotate,
//                                       translate and connect the fragment
// 3) The atom doesn't belong to a fragment: a) First atom: place at origin
//                                           b) Not first atom: Find position
//                                           and place atom
bool OBBuilder::Build(OBMol &mol, bool stereoWarnings) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) { atomsToCopy.SetBitOn(atom->GetIdx()); }
  ...
  for (vector<OBMol>::iterator f = fragments.begin(); f != fragments.end();
       ++f) {
    ...
    if (_rigid_fragments_index.count(fragment_smiles) > 0) {
      ...
      if (!sp.Init(fragment_smiles)) {
        obErrorLog.ThrowError(__FUNCTION__,
                              " Could not parse SMARTS from fragment", obInfo);
        ...
        for (j = mlist.begin(); j != mlist.end(); ++j) {
          ...
          for (k = j->begin(), counter = 0; k != j->end();
               ++k, ++counter) { // for all atoms of the fragment
    ...
    if (!isMatchRigid) { // if rigid fragment is not in database
      ...
      if (ratoms < 3)
        continue; // Smallest ring fragment has 3 atoms
      ...
      for (i = _ring_fragments.begin();
           i != _ring_fragments.end() && i->first->NumAtoms() > ratoms; ++i)
        ;
      ...
      for (; i != _ring_fragments.end() && ratoms; ++i) {
        if (i->first != nullptr &&
            i->first->Match(*f)) { // if match to fragment
          ...
          for (j = mlist.begin(); j != mlist.end(); ++j) { // for all matches
            ...
            for (k = j->begin(); k != j->end();
                 ++k) { // for all atoms of the fragment
              if (vfrag.BitIsSet(*k)) {
                alreadydone = true;
                break;
              }
  ...
  FOR_DFS_OF_MOL(a, mol) {
    ...
    if (vfrag.BitIsSet(a->GetIdx())) { // Is this atom part of a fragment?
      if (prev != nullptr) { // if we have a previous atom, translate/rotate the
                             // fragment and connect it
        Connect(workMol, prev->GetIdx(), a->GetIdx(),
                mol.GetBond(prev, &*a)->GetBondOrder());
        // set the correct bond order
        int bondOrder =
            mol.GetBond(prev->GetIdx(), a->GetIdx())->GetBondOrder();
    ...
    if (prev != nullptr) {
      ...
      molvec = GetCorrectedBondVector(workMol.GetAtom(prev->GetIdx()),
                                      workMol.GetAtom(a->GetIdx()), bondType);
  ...
  if (!rlist.empty()) {
    ...
    for (int sys = 0; sys < nsystems; ++sys) {
      ...
      std::sort(members.begin(), members.end(), [&rlist](size_t a, size_t b) {
      ...
      for (size_t idx : members) {
        ...
        for (int i = 3; i < n; ++i) {
          ...
          workMol.SetTorsion(
              workMol.GetAtom(walk[i - 3]), workMol.GetAtom(walk[i - 2]),
              workMol.GetAtom(walk[i - 1]), workMol.GetAtom(walk[i]), phi);
        }
  ...
  */
  ...
  if (isNanExist)
    obErrorLog.ThrowError(
        __FUNCTION__, "There exists NaN in calculated coordinates.", obWarning);

  return success;
}

void OBBuilder::ConnectFrags(OBMol &mol, OBMol &workMol, vector<int> match,
                             vector<vector3> coords, vector<int> pivot) {
  ...
  int counter;
  for (match_it = match.begin(), counter = 0; match_it != match.end();
       ++match_it, ++counter)
    if (*match_it == pivot[0]) {
  ...
  v1 = cross(
      tmpvec,
      v1); // In the plane of the ring, orthogonal to tmpvec and the original v1
  ...
  for (match_it = match.begin(); match_it != match.end(); ++match_it)
    workMol.GetAtom(*match_it)->SetVector(
        workMol.GetAtom(*match_it)->GetVector() + posp);

  // Create the bonds between the two fragments
  for (vector<int>::iterator nbr_id = nbrs.begin(); nbr_id != nbrs.end();
       ++nbr_id)
    workMol.AddBond(p->GetIdx(), *nbr_id, 1,
                    mol.GetBond(p->GetIdx(), *nbr_id)->GetFlags());
...
bool OBBuilder::CorrectStereoBonds(OBMol &mol) {
  ...
  for (std::vector<OBCisTransStereo *>::iterator origct = cistrans.begin();
    ...
    OBCisTransStereo::Config config =
        found->second->GetConfig(OBStereo::ShapeU);

    if ((*origct)->GetConfig(OBStereo::ShapeU) !=
        config) { // Wrong cis/trans stereochemistry
      ...
      newangle =
          angle * DEG_TO_RAD + M_PI; // flip the bond by 180 deg (PI radians)
...
bool OBBuilder::CorrectStereoAtoms(OBMol &mol, bool warn) {
  ...
  for (std::vector<OBTetrahedralStereo *>::iterator origth = tetra.begin();
    ...
    if (found == newByCenter.end())
      continue; // TetrahedralFrom3D() skipped this center; nothing to compare
                // against
    ...
    if (ringbonds == 3 ||
        (ringbonds == 4 && !OBBuilder::IsSpiroAtom(config.center, mol))) {
      bool rightstereo = (*origth)->GetConfig(OBStereo::Clockwise,
                                              OBStereo::ViewFrom) == config;
  ...
  if (existswrongstereo) {
    ...
    if (unfixed.size() > 0 && warn) {
      ...
      errorMsg << "Could not correct " << unfixed.size()
               << " stereocenter(s) in this molecule (" << mol.GetTitle()
               << ")";
    ...
    if (inversion) {
      sgunits.clear();
      for (std::vector<OBTetrahedralStereo *>::iterator origth =
               nonringtetra.begin();
           origth != nonringtetra.end(); ++origth)
        sgunits.push_back(
            OBStereoUnit(OBStereo::Tetrahedral, (*origth)->GetConfig().center));
  ...
  for (std::vector<OBTetrahedralStereo *>::iterator it =
           nonringnewtetra.begin();
  ...
  for (std::vector<OBTetrahedralStereo *>::iterator origth =
    ...
    if ((*origth)->GetConfig(OBStereo::Clockwise, OBStereo::ViewFrom) !=
      ...
      if (idxs.size() == 0 && OBBuilder::IsSpiroAtom(config.center, mol))
        ...
        vector3 newcoords =
            OBBuilder::GetNewBondVector(center, non_ring_bond->GetLength());
        SwapWithVector(mol, center->GetIdx(), idxs.at(0), center->GetIdx(),
                       newcoords);
      }
    }
  }
...
bool OBBuilder::FixRingStereo(
    std::vector<std::pair<OBStereo::Ref, bool>> atomIds, OBMol &mol,
  ...
  for (unsigned int n = 0; n < atomIds.size(); ++n) {
    ...
    if (seen.BitIsSet(atomIds[n].first) || atomIds[n].second)
      continue;
    ...
    // Invert the coordinates (QUESTION: should I invert relative to the
    // centroid?)
    inversion = true;
    FOR_ATOMS_OF_MOL(a, mol)
    ...
    for (std::vector<OBBond *>::iterator bi = reconnect.begin();
         bi != reconnect.end(); ++bi) {
      OBBond *b = *bi;
...
void OBBuilder::AddRingNbrs(OBBitVec &fragment, OBAtom *atom, OBMol &mol) {
  ...
  FOR_NBORS_OF_ATOM(nbr, atom) {
    if (mol.GetBond(&*nbr, atom)->IsInRing() &&
        !fragment.BitIsSet(nbr->GetId()) &&
