...
// #define DONT_CATCH_EXCEPTIONS     // This is useful when debugging an
// exception
...
/** @class OBFormat obconversion.h <openbabel/obconversion.h>
    ...
    The "Convert" interface is for use in file format conversion applications.
The user interface, a console, a GUI, or another program is kept unaware of the
    ...
    in the ReadChemicalObject() functions and and deleted in
WriteChemicalObject() functions
    ...
    destruction of objects in ReadMolecule() and WriteMolecule() and no
restriction on whether the pointers are to the heap or the stack.
**/
//***************************************************
...
/** @class OBConversion obconversion.h <openbabel/obconversion.h>
    ...
    An alternative way is more convenient if using bindings from another
   language:
    @code
    std::string SmilesString;
    OBMol mol;
    ...
    if(conv.SetInAndOutFormats("SMI","MOL"))
    {
       conv.AddOption("h",OBConversion::GENOPTIONS); //Optional; (h adds expicit
   hydrogens) conv.Convert();
    }
    @endcode

    <b>To read a multi-molecule file if using bindings from another language</b>

    The first molecule should be read using ReadFile, and subsequent molecules
   using Read, as follows:
    @code
    #include <openbabel/obconversion.h> //mol.h is not needed
    OBConversion conv;
    OBMol mol;
...
OBConversion::OBConversion(istream *is, ostream *os)
    : pInput(nullptr), pOutput(nullptr), pInFormat(nullptr),
      pOutFormat(nullptr), Index(0), StartNumber(1), EndNumber(0), Count(-1),
      m_IsFirstInput(true), m_IsLast(true), MoreFilesToCome(false),
      OneObjectOnly(false), ReadyToInput(false), SkippedMolecules(false),
      inFormatGzip(false), outFormatGzip(false), pOb1(nullptr), wInpos(0),
      wInlen(0), pAuxConv(nullptr) {
  SetInStream(is);
  SetOutStream(os);
...
/////////////////////////////////////////////////
OBConversion::OBConversion(const OBConversion &o) { *this = o; }
...
OBConversion &OBConversion::operator=(const OBConversion &o) {
  ...
  // by doing this first, format isn't initialized, so we add no additional
  // filters
  pInFormat = nullptr;
  inFormatGzip = false;
  pOutFormat = nullptr;
...
/// derived from OBFormat(only one is usually required). RegisterFormat() is
/// called from its constructor.
///
/// If the compiled format is stored separately, like in a DLL or shared
/// library, the initialization code makes an instance of the imported OBFormat
/// class.
...
void OBConversion::SetOutStream(std::ostream *pOut, bool takeOwnership) {
  ...
  if (pOut) {
    ...
    if (IsOption("z", GENOPTIONS) || outFormatGzip) {
      zlib_stream::zip_ostream *zOut =
          new zlib_stream::zip_ostream(*pOutput, true);
      // we need to delete the zstream _before_ the underlying stream so it can
      // add the footer
...
bool OBConversion::SetInAndOutFormats(const char *inID, const char *outID,
                                      bool inzip, bool outzip) {
  return SetInFormat(inID, inzip) && SetOutFormat(outID, outzip);
}
//////////////////////////////////////////////////////

bool OBConversion::SetInAndOutFormats(OBFormat *pIn, OBFormat *pOut, bool inzip,
                                      bool outzip) {
...
//////////////////////////////////////////////////////
/// Convert molecules from is into os.  If either is null, uses existing
/// streams. If streams are specified, they do _not_ replace any existing
/// streams.
...
int OBConversion::Convert() {
  if (pInput == nullptr) {
    obErrorLog.ThrowError(__FUNCTION__, "input or output stream not set",
                          obError);
    return 0;
  }
  ...
  // Input loop
  while (ReadyToInput &&
         pInput->good()) // Possible to omit? && pInStream->peek() != EOF
  {
    if (pInput == &cin) {
...
#endif
    {
      ...
      */
      ...
      if (!IsOption("e", GENOPTIONS) && !OneObjectOnly) {
        obErrorLog.ThrowError(__FUNCTION__, "Convert failed with an exception",
                              obError);
...
///    writing the currently stored value before accepting the supplied one.
///    This delay allows output routines to respond differently when the written
///    object is the last. Count is incremented with each call, even if
///    pOb=NULL. Objects are not added to the queue if the count is outside the
///    range StartNumber to EndNumber. There is no upper limit if EndNumber is
///    zero. The return value is Count ((>0) or 0 if WriteChemObject returned
///    false.
int OBConversion::AddChemObject(OBBase *pOb) {
  ...
  Count++;
  if (Count >=
      (int)StartNumber) // keeps reading objects but does nothing with them
  {
    ...
    if (pOb) {
      ...
      pOb1 = pOb;
      wInpos = rInpos; // Save the position in the input file to be accessed
                       // when writing it
      wInlen = rInlen;
    }
  }
...
//////////////////////////////////////////////////////
/// Returns the number of objects which have been output or are currently being
/// output. The outputindex is incremented when an object for output is fetched
/// by GetChemObject(). So the function will return 1 if called from
/// WriteMolecule() during output of the first object.
int OBConversion::GetOutputIndex() const {
  // The number of objects actually written already from this instance of
  // OBConversion
  return Index;
}
void OBConversion::SetOutputIndex(int indx) { Index = indx; }
//////////////////////////////////////////////////////
...
//////////////////////////////////////////////////
const char *OBConversion::GetTitle() const { return (InFilename.c_str()); }

void OBConversion::SetMoreFilesToCome() { MoreFilesToCome = true; }
...
OBFormat *OBConversion::FormatFromExt(const char *filename, bool &isgzip) {
  ...
  if (extPos != string::npos // period found
      && (file.substr(extPos + 1, file.size())).find("/") ==
             string::npos) // and period is after the last "/"
  {
...
bool OBConversion::Read(OBBase *pOb, std::istream *pin) {
  ...
  // If we failed to read, plus the stream is over, then check if this is a
  // stream from ReadFile
  if (!success && !pInput->good() && ownedInStreams.size() > 0) {
    ...
    if (inFstream != nullptr)
      inFstream
          ->close(); // We will free the stream later, but close the file now
  }
...
bool OBConversion::Write(OBBase *pOb, ostream *pos) {
  if (pos)
    SetOutStream(pos, false);

  if (!pOutFormat || !pOutput)
    return false;
...
void OBConversion::StreamState::pushInput(OBConversion &conv) {
  ...
  pStream = conv.pInput;
  std::copy(conv.ownedInStreams.begin(), conv.ownedInStreams.end(),
            std::back_inserter(ownedStreams));
...
void OBConversion::StreamState::pushOutput(OBConversion &conv) {
  ...
  std::copy(conv.ownedOutStreams.begin(), conv.ownedOutStreams.end(),
            std::back_inserter(ownedStreams));
...
void OBConversion::CloseOutFile() { SetOutStream(nullptr); }
...
bool OBConversion::ReadFile(OBBase *pOb, std::string filePath) {
  ...
  ios_base::openmode imode =
      ios_base::in |
      ios_base::binary; // now always binary because may be gzipped
  ifstream *ifs = new ifstream(filePath.c_str(), imode);
  if (!ifs || !ifs->good()) {
    delete ifs;
    obErrorLog.ThrowError(__FUNCTION__, "Cannot read from " + filePath,
                          obError);
    return false;
  }
#ifdef HAVE_LIBZ
...
////////////////////////////////////////////
bool OBConversion::OpenInAndOutFiles(std::string infilepath,
  ...
  ifstream *ifs = new ifstream(
      infilepath.c_str(),
      ios_base::in | ios_base::binary); // always open in binary mode
  if (!ifs || !ifs->good()) {
    delete ifs;
    obErrorLog.ThrowError(__FUNCTION__, "Cannot read from " + infilepath,
                          obError);
    return false;
  }
  SetInStream(ifs, true);
  ...
  ofstream *ofs = new ofstream(
      outfilepath.c_str(),
      ios_base::out | ios_base::binary); // always open in binary mode
  if (!ofs || !ofs->good()) {
    delete ofs;
    obErrorLog.ThrowError(__FUNCTION__, "Cannot write to " + outfilepath,
                          obError);
    return false;
  }
  SetOutStream(ofs, true);
...
////////////////////////////////////////////
bool OBConversion::IsLast() { return m_IsLast; }
////////////////////////////////////////////
bool OBConversion::IsFirstInput() { return m_IsFirstInput; }
...
////////////////////////////////////////////////////
bool OBConversion::CheckForUnintendedBatch(const string &infile,
  ...
  if (inname1 == inname2)
    obErrorLog.ThrowError(__FUNCTION__,
                          "This was a batch operation. For splitting, use "
                          "non-empty base name for the output files",
                          obWarning);
...
*/
int OBConversion::FullConvert(std::vector<std::string> &FileList,
  ...
  SetFirstInput();
  bool CommonInFormat =
      pInFormat ? true : false; // whether set in calling routine
  ios_base::openmode omode = ios_base::out | ios_base::binary;
  obErrorLog.ClearLog();
#ifndef DONT_CATCH_EXCEPTIONS
  try
#endif
  {
    ...
    if (OutputFileName.empty())
      ...
      if (!HasMultipleOutputFiles) {
        ...
        if (itr == FileList.end()) {
          ...
          if (!os) {
            obErrorLog.ThrowError(__FUNCTION__,
                                  "Cannot write to " + OutputFileName, obError);
            return 0;
          }
          pOs = &os;
        }
    ...
    if (IsOption("t", GENOPTIONS)) {
      ...
      if (HasMultipleOutputFiles) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "Cannot have multiple output files and also "
                              "concatenate input files (-t option)",
                              obError);
        return 0;
      }
    ...
    if (FileList.empty()) {
      ...
      if (HasMultipleOutputFiles) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "Cannot use multiple output files without an input file", obError);
        return 0;
      }
    } else {
      if (FileList.size() > 1 || OutputFileName.substr(0, 2) == "*.") {
        ...
        for (itr = FileList.begin(); itr != FileList.end(); ++itr) {
          ...
          if (HasMultipleOutputFiles) {
            ...
            // Issue a message and abort if xxx==yyy which would overwrite input
            // file
            if (FileList.size() == 1 &&
                !CheckForUnintendedBatch(batchfile, InFilename))
              return Count;
            ...
            if (!ofs) {
              obErrorLog.ThrowError(__FUNCTION__, "Cannot open " + batchfile,
                                    obError);
              return Count;
            }
        ...
        if (!os.is_open() && !OutputFileName.empty() &&
          ...
          // Output was written to temporary string stream. Output it to the
          // file
          os.open(OutputFileName.c_str(), omode);
          if (!os) {
            obErrorLog.ThrowError(__FUNCTION__,
        ...
        if (HasMultipleOutputFiles) {
          ...
          for (;;) {
            ...
            if (!ofs) {
              obErrorLog.ThrowError(__FUNCTION__, "Cannot write to " + incrfile,
                                    obError);
              return Count;
            }
    ...
    if (os && !os.is_open() && !OutputFileName.empty()) {
      ...
      os.open(OutputFileName.c_str(), omode);
      if (!os) {
        obErrorLog.ThrowError(__FUNCTION__, "Cannot write to " + OutputFileName,
                              obError);
        return Count;
      }
      SetOutStream(&os, false);
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Conversion failed with an exception.",
                          obError);
    return Count;
  }
#endif
...
**/
#endif
void OBConversion::AddOption(const char *opt, Option_type opttyp,
                             const char *txt) {
...
void OBConversion::RegisterOptionParam(string name, OBFormat *pFormat,
  ...
  // Gives error message if the number of parameters conflicts with an existing
  // registration
  ...
  if (pos != OptionParamArray(typ).end()) {
    if (pos->second != numberParams) {
      ...
      obErrorLog.ThrowError(__FUNCTION__,
                            "The number of parameters needed by option \"" +
                                name + "\" in " +
...
int OBConversion::GetOptionParams(string name, Option_type typ) {
  // returns the number of parameters registered for the option, or 0 if not
  // found
  map<string, int>::iterator pos;
  pos = OptionParamArray(typ).find(name);
  if (pos == OptionParamArray(typ).end())
...
void OBConversion::ReportNumberConverted(int count, OBFormat *pFormat) {
  ...
  // be "molecules", "reactions", etc and remove the s if only one object
  // converted
  if (!pFormat)
    pFormat = pOutFormat;
...
int OBConversion::NumInputObjects() {
  ...
  if (GetInFormat()->SkipObjects(0, this) == 0) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Input format does not have a SkipObjects function.",
                          obError);
    return -1;
  }
...
// Use OBConversion::GetSupportedInputFormat(),
// OBConversion::GetSupportedOutputFormat(), OBPlugin::List(),
// OBPlugin::OBPlugin::ListAsVector(),OBPlugin::OBPlugin::ListAsString(), or (in
// extremis) OBPlugin::PluginIterator instead.

typedef OBPlugin::PluginIterator Formatpos;

bool OBConversion::GetNextFormat(Formatpos &itr, const char *&str,
                                 OBFormat *&pFormat) {
