...
void ThrowError(char *str) { obErrorLog.ThrowError("", str, obInfo); }

//! Deprecated: use the OBMessageHandler class instead
//! \deprecated Throw an error through the OpenBabel::OBMessageHandler class
OB_DEPRECATED_MSG("use the OBMessageHandler class instead")
void ThrowError(std::string &str) { obErrorLog.ThrowError("", str, obInfo); }

// returns True if a < b, False otherwise.
bool OBCompareInt(const int &a, const int &b) { return (a < b); }
...
bool IsNan(const double &a) { return ((a) != (a)); }
...
//! Utility function: replace the last extension in string &src with new
//! extension char *ext.
...
double calc_rms(double *r, double *f, unsigned int N) {
  ...
  for (unsigned int i = 0; i < N; ++i) {
    d2 += SQUARE(r[i * 3] - f[i * 3]) + SQUARE(r[i * 3 + 1] - f[i * 3 + 1]) +
...
void SetRotorToAngle(double *c, vector<int> &tor, double ang,
                     vector<int> &atoms) {
...
//! the second character (if it's a letter) to lowercase, and terminating with a
//! NULL to strip off any trailing characters
...
//! beginning).
//! Implements <a
//! href="http://qsar.sourceforge.net/dicts/blue-obelisk/index.xhtml#zmatrixCoordinatesIntoCartesianCoordinates">blue-obelisk:zmatrixCoordinatesIntoCartesianCoordinates</a>
void InternalToCartesian(std::vector<OBInternalCoord *> &vic, OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::InternalToCartesian",
                        obAuditMsg);
  ...
  for (vector<OBAtom *>::iterator a_it = for_deletion.begin();
       a_it != for_deletion.end(); ++a_it)
    mol.DeleteAtom(*a_it);
}
...
//! Implements <a
//! href="http://qsar.sourceforge.net/dicts/blue-obelisk/index.xhtml#cartesianCoordinatesIntoZmatrixCoordinates">blue-obelisk:cartesianCoordinatesIntoZmatrixCoordinates</a>.
...
void CartesianToInternal(std::vector<OBInternalCoord *> &vic, OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::CartesianToInternal",
                        obAuditMsg);
  ...
  for (k = 2; k <= mol.NumAtoms(); ++k) {
    ...
    vic[k]->_tor = CalcTorsionAngle(atom->GetVector(), a->GetVector(),
  ...
  for (k = 2; k <= mol.NumAtoms(); ++k) {
    ...
    for (a = mol.BeginAtom(i); a && a->GetIdx() < k && !done;
         a = mol.NextAtom(i))
      for (b = mol.BeginAtom(j); b && b->GetIdx() < a->GetIdx() && !done;
           b = mol.NextAtom(j)) {
        v1 = atom->GetVector() - a->GetVector();
        v2 = b->GetVector() - a->GetVector();
        ...
        // Also check length considerations -- don't bother if the length > 10.0
        // Angstroms
        if (v1.length_2() > 99.999)
          continue;

        for (c = mol.BeginAtom(m); c && c->GetIdx() < atom->GetIdx();
             c = mol.NextAtom(m))
          if (c != atom && c != a && c != b)
            break;
        if (!c)
...
void ob_make_rmat(double a[3][3], double rmat[9]) {
  /*
  onorm, dnorm - hold the sum of diagonals and off diagonals to check Jacobi
  completion d[3] - holds the diagonals of the input vector, which transofrm to
  become the Eigenvalues r1, r2 - hold 1st two Eigenvectors v1,v2,v3 - hold
  orthogonal unit vectors derived from Eigenvectors
  ...
  Inertial Tensor in a, and returns a unit transform matrix rmat as a row
  matrix. To work, a must be diagonally symmetric (i.e a[i][j] = a[j][i]) v
  starts out holding the unit matrix (i.e. no transform in co-ordinate frame),
  and undergoes the same rotations as applied to the Inertial Tensor in the
  Jacobi process to arrive at the new co-ordinate frame. Finally, the
  eigenvalues are sorted in order that the largest principal moment aligns to
  the new x-axis
  */
  double onorm, dnorm;
...
Exit_now:
  ...
  /* Generate the 3rd unit vector for the new co-ordinate frame by cross product
   * of r1 and r2*/
  ...
  /* Generate the 2nd unit vector for the new co-ordinate frame by cross product
   * of v3 and r1*/
  ...
  /* Generate the 1st unit vector for the new co-ordinate frame by cross product
   * of v2 and v3*/
...
double superimpose(double *r, double *f, int size) {
  ...
  for (i = 0; i < 3; ++i)
    for (j = 0; j < 3; ++j)
      rmat[i][j] = mat[0][j] * mat2[i][0] + mat[1][j] * mat2[i][1] +
                   mat[2][j] * mat2[i][2];

  /* rotate all coordinates */
  d2 = 0.0;
  for (i = 0; i < size; ++i) {
    x = f[3 * i] * rmat[0][0] + f[3 * i + 1] * rmat[0][1] +
        f[3 * i + 2] * rmat[0][2];
    y = f[3 * i] * rmat[1][0] + f[3 * i + 1] * rmat[1][1] +
        f[3 * i + 2] * rmat[1][2];
    z = f[3 * i] * rmat[2][0] + f[3 * i + 1] * rmat[2][1] +
        f[3 * i + 2] * rmat[2][2];
    f[3 * i] = x;
    f[3 * i + 1] = y;
    f[3 * i + 2] = z;
