...
public:
  ...
  const char *Description() override {
    return "Align coordinates to the first molecule\n"
           "Typical use with a -s option:\n"
           "    obabel pattern.www  dataset.xxx  -O outset.yyy  -s SMARTS  "
           "--align\n"
           ...
           "are used as pattern atoms. The order of the atoms must be the "
           "same\n"
           ...
           "--append rmsd.\n"
           "To output the two conformers closest to the first conformer in a "
           "dataset:\n"
           "    obabel dataset.xxx  -O outset.yyy  --align  --smallest 2 "
           "rmsd\n\n";
  }
...
/////////////////////////////////////////////////////////////////
bool OpAlign::Do(OBBase *pOb, const char *OptionText, OpMap *pmap,
                 OBConversion *pConv) {
    ...
    if (itr != pmap->end()) {
      ...
      if (!_pOpIsoM || _stext.empty()) {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "No parameter on -s option, or its OBOp version is not loaded",
            obError);
  ...
  if (pmol->GetDimension() == 3 &&
      (pConv->GetOutFormat()->Flags() & DEPICTION2D)) {
  ...
  // They may be added again later when gen2D or gen3D is called, but they will
  // be the same. It would be better if this op was called after them, which
  // would happen if its name was alphabetically after "gen" (and before "s").
  if (pmol->GetDimension() == 0) {
    // Will the coordinates be 2D or 3D?
    itr = pmap->find("gen3D");
    OBOp *pgen = (itr == pmap->end()) ? OBOp::FindType("gen2D")
                                      : OBOp::FindType("gen3D");
    if (pgen)
      pgen->Do(pmol);
  }

  // Do the alignment in 2D if the output format is svg, png etc. and there is
  // no -xn option
  ...
  if (pConv->IsFirstInput() || _refMol.NumAtoms() == 0) {
    ...
    if (!_pOpIsoM)
      ...
      // If there is a -s option, reference molecule has only those atoms that
      // are matched Call the -s option from here
      ...
      if (!ats.empty()) {
        ...
        for (vector<int>::iterator iter = ats.begin(); iter != ats.end();
             ++iter)
          _refvec.push_back((pmol->GetAtom(*iter))->GetVector());
        // ...and use a vector reference
        _align.SetRef(_refvec);
      }
      // Stop -s option being called normally, although it will still be called
      // once
      //  in the DoOps loop already started for the current (first) molecule.
      pConv->RemoveOption("s", OBConversion::GENOPTIONS);
      if (!ret) {
        // the first molecule did not match the -s option so a reference
        // molecule could not be made. Keep trying.
        _refMol.Clear();
        // obErrorLog.ThrowError(__FUNCTION__, "The first molecule did not match
        // the -s option\n"
        //   "so the reference structure was not derived from it", obWarning,
        //   onceOnly);
  ...
  if (_pOpIsoM) // Using -s option
  {
    ...
    if (!_pOpIsoM->Do(pmol, "", pmap,
                      pConv)) // "" means will use existing parameters
      return false;
    ...
    for (vector<vector3>::iterator iter = _refvec.begin();
         iter != _refvec.end(); ++iter)
      ref_centroid += *iter;
...
/*
With a -s option
Use the atoms from the match of the first molecule to make a vector of their
coordinates. This will be in the order of the match. For subsequent molecules do
the same, and do the alignment with two vectors. The order of the atoms in the
target molecules should not matter.
