...
public:
  ...
  const char *Description() override {
    // Need to use a member variable so that const char* is valid when it is
    // returned
    description = (strcmp(GetID(), "largest") != 0)
                      ? "# <descr> Output # mols with smallest values of "
                        "descriptor(not displayed in GUI)\n"
                        "    obabel infile.xxx -Ooutfile.yyy --smallest 5 MW\n"
                        "will convert only the molecules with the 5 smallest "
                        "molecular weights.\n"
                      : "# <descr> Output # mols with largest values\n"
                        "of a descriptor <descr>. For example:\n"
                        "    obabel infile.xxx -Ooutfile.yyy --largest 5 MW\n"
                        "will convert only the molecules with the 5 largest "
                        "molecular weights.\n";
    description +=
        ...
        "Preceding the descriptor by ~ inverts the comparison. (Use this form "
        "in the GUI.)\n"
        "If a + follows the descriptor, e.g. MW+ , the value will be added to "
        "the title.\n";
  ...
  static bool
  MatchPairData(OBBase *pOb,
                std::string &s); // Copy of protected OBDescriptor function
...
/////////////////////////////////////////////////////////////////
bool OpLargest::Do(OBBase *pOb, const char *OptionText, OpMap *pOptions,
                   OBConversion *pConv) {
  ...
  if (pConv->IsFirstInput()) {
    ...
    _selmap.clear();
    _rev =
        strcmp(GetID(), "largest") != 0; //_rev is initially true for --smallest
    ...
    // If the first molecule has matching OBPairData, use that and set _pDesc to
    // NULL
    ...
    // constructor, will immediately call this function,when it receives
    // objects. The handling of the selected molecules is all done here and
    // returning false means that DeferredFormat stores nothing. At the end it
    // calls this op's ProcessVec() function.
  ...
  if (_selmap.size() < _nmols)
    ...
    if ((!_rev && val > leastwanted->first) ||
        (_rev && val < leastwanted->first)) {
