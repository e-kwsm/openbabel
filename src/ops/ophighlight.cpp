...
public:
  ...
  const char *Description() override {
    return "<param> Highlight substructures in 2D depictions\n"
...
//////////////////////////////////////////////////////////////////
bool OpHighlight::Do(OBBase *pOb, const char *OptionText, OpMap *pmap,
                     OBConversion *pConv) {
  ...
  for (unsigned int vecIdx = 0; vecIdx < vec.size(); vecIdx += 2) {
    ...
    // only when SMARTS contains [H]. Doing more has complications with atom
    // typing, so AddHydrogens here on the molecule (not a copy) when #1
    // detected.
