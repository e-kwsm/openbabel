...
public:
  OpFillUC(const char *ID) : OBOp(ID, false) {
    OBConversion::RegisterOptionParam("fillUC", nullptr, 1,
  ...
  const char *Description() override {
    return "<param> Fill the unit cell (strict or keepconnect)\n"
           ...
           "<param> can be:\n"
           "   strict (keep only atoms inside the UC) => use \"--fillUC "
           "strict\"\n"
           "   keepconnect (fill the unit cell but keep the original "
           "connectivity => use \"--fillUC keepconnect\"";
  }
...
bool areDuplicateAtoms2(vector3 v1, vector3 v2) {
  vector3 dr =
      fuzzyWrapFractionalCoordinate(v2) - fuzzyWrapFractionalCoordinate(v1);
...
/////////////////////////////////////////////////////////////////
bool OpFillUC::Do(OBBase *pOb, const char *OptionText, OpMap *pOptions,
                  OBConversion *pConv) {
  ...
  if (!(pmol->HasData(OBGenericDataType::UnitCell))) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Cannot fill unit cell without a unit cell !", obWarning);
    return false;
  }
  ...
  if (pSG == nullptr) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Cannot fill unit cell without spacegroup information !",
        obWarning);
    return false;
  }
  // Now loop over all symmetry operations, and generate symmetric atoms one at
  // a time Avoid creating overlapping atoms (duplicate), and bring back atoms
  // within the unit cell using two options:
  ...
  std::map<OBAtom *, std::vector<vector3>>
      vatoms; // key: original atoms, value=all generated symmetrics
  ...
  if (0 == strncasecmp(OptionText, "keepconnect", 11)) {
    ...
    for (unsigned int i = 0; i < vatoms.begin()->second.size(); ++i) {
      ...
      for (std::map<OBAtom *, std::vector<vector3>>::iterator atom =
               vatoms.begin();
           atom != vatoms.end(); ++atom) {
    ...
    if (0 != strncasecmp(OptionText, "strict", 6))
      obErrorLog.ThrowError(
          __FUNCTION__,
          "fillUC: lacking \"strict\n or \"keepconnect\" option, using strict",
          obWarning);
    for (std::map<OBAtom *, std::vector<vector3>>::iterator atom =
             vatoms.begin();
         atom != vatoms.end(); ++atom) {
