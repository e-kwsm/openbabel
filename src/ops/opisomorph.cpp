...
bool MakeQueriesFromMolInFile(vector<OBQuery *> &queries,
...
const char *OpNewS::Description() {
  return "Isomorphism filter(-s, -v options replacement)(not displayed in "
         "GUI)\n"
         ...
         "``#8dcb70``. The coloring is recognized by SVGFormat and "
         "CMLFormat.\n \n"

         "The first parameter can also be a filename with an extension that\n"
         "can be interpreted as a file format:\n"
         "    -s \"pattern.mol exact\"\n"
         "A molecule in the file is used in an isomorphism test with the "
         "default\n"
         ...
         "converted molecule corresponds to the first pattern molecule "
         "matched,\n"
         "or the last color if there are fewer colors than pattern "
         "molecules.\n \n"

         "If the last parameter is ``showall``, all molecules are shown, even "
         "if\n"
         "they do not match. This allows the -s option to be used for "
         "highlighting.\n \n"

         "If the second parameter is ``exact`` only exact matches are "
         "converted.\n"
         "If the second parameter is ``extract`` all the atoms in the "
         "converted\n"
         "molecule are deleted except for those matched. Since these retain "
         "their\n"
         "coordinates, this can be used to prepare display templates.\n\n"

         "With SMARTS matching only, the number of unique occurrences in a "
         "molecule\n"
         "can be specified in the second parameter, e.g.\n"
         "    -s c1ccccc1 2   which matches if there are exactly two benzene "
         "rings\n"
         " or -s c1ccccc1 >2  which matches if there are more than two.\n"
         "(<2 also works.) The color of the substructure can be in the 3rd "
         "parameter.\n \n"

         "In the GUI (or on the commandline as an alternative to using -v) the "
         "test\n"
         "can be negated with a ~ before the SMARTS string or file name.\n \n"

         "With the ``babel`` commandline interface, unless the option is at "
         "the end\n"
         "of a line, it is necessary to enclose all the parameters together in "
         "quotes,\n"
         "as in the first example above, because the -s and -v options are\n"
         "expecting a single parameter. With obabel and the GUI this is not "
         "necessary.\n"
...
//////////////////////////////////////////////////////////////////
bool OpNewS::Do(OBBase *pOb, const char *OptionText, OpMap *pmap,
                OBConversion *pConv) {
  ...
  if (OptionText && *OptionText) //(!pConv || pConv->IsFirstInput())
  {
    ...
    // allows -s option to be used for highlighting substructures (--highlight
    // also does this)
    vector<string>::iterator it =
        std::remove(vec.begin(), vec.end(), "showall");
    showAll = it != vec.end();
    if (showAll)
      vec.erase(it);

    // Store the number of matches required, if as a number in the second
    // parameter, else 0.
    nmatches = 0;
    comparechar = '\0';
    if (vec.size() > 1) {
    ...
    MakeQueriesFromMolInFile(queries, vec[0], &nPatternAtoms,
                             strstr(OptionText, "noH"));
    vec.erase(remove(vec.begin(), vec.end(), "noH"),
              vec.end()); // to prevent "noH2" being seen as a color

    if (queries.empty()) {
      // SMARTS supplied

      // Explicit H in SMARTS requires explicit H in the molecule.
      // Calling AddHydrogens() on a copy of the molecule  is done in
      // parsmart.cpp only when SMARTS contains [H]. Doing more has
      // complications with atom typing, so AddHydrogens here on the molecule
      // (not a copy) when #1 detected.
      ...
      string ysmarts =
          xsmarts.empty() ? vec[0] : "[$(" + vec[0] + ")" + xsmarts + "]";
      xsmarts.clear();
      if (!sp.Init(ysmarts)) {
        string msg =
            ysmarts +
            " cannot be interpreted as either valid SMARTS "
      ...
      for (unsigned i = 0; i < ExtraMols.size(); ++i)
        queries.push_back(
            CompileMoleculeQuery(static_cast<OBMol *>(ExtraMols[i])));
      ExtraMols.clear();
    }
      ...
      if (queries.empty()) {
        ...
        if (!conv.SetInFormat("smi") || !conv.ReadString(&patmol, vec[0])) {
          obErrorLog.ThrowError(
              __FUNCTION__,
              "Cannot read the parameter of -s option, "
  ...
  if (!queries.empty())  // filename supplied
  {
    ...
    for (qiter = queries.begin(); qiter != queries.end(); ++qiter, ++imol) {
      ...
      if ((match = !mappings.empty())) // extra parens to indicate truth value
      {
        ...
        for (ita = mappings.begin(); ita != mappings.end();
             ++ita) // each mapping
        {
          vector<int> atomvec;
  ...
  if (match)
    // Copy the idxes of the first match to a member variable so that it can be
    // retrieved from outside
    firstmatch.assign(pMappedAtoms->begin()->begin(),
                      pMappedAtoms->begin()->end());
  else
    firstmatch.clear();
  ...
  if (match && !inv && vec.size() >= 2 && !vec[1].empty() && !nPatternAtoms) {
    ...
    // color the substructure if there is a second parameter which is not
    // "exact" or "extract" or "noH" with multiple color parameters use the one
    // corresponding to the query molecule, or the last
    if (imol > vec.size() - 2)
      imol = vec.size() - 2;
    for (iter = pMappedAtoms->begin(); iter != pMappedAtoms->end();
         ++iter) // each match
