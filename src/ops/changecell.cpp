...
public:
  ...
  const char *Description() override {
    return "Change cell size:\n"
           "     [keepfract];[*]a;[*]b;[*]c\n"
           "     Original cell dimensions can be changed to value a, b or c or "
           "multiplied with key '*' ";
  }
...
/////////////////////////////////////////////////////////////////
bool OpChangeCell::Do(OBBase *pOb, const char *OptionText, OpMap *pOptions,
                      OBConversion *pConv) {
  std::vector<std::string> vcr;
  tokenize(vcr, OptionText, ";");

  if ((vcr.size() != 3) && (vcr.size() != 4)) {
    obErrorLog.ThrowError(__FUNCTION__, "Invalid number of arguments!",
                          obWarning);
    return false;
  }
  ...
  if (vcr.size() != 3) {
    obErrorLog.ThrowError(__FUNCTION__, "Invalid input. Check first argument!",
                          obWarning);
    return false;
  }
  ...
  for (int i = 0; i < vcvs.size(); i++) {
    ...
    if (vcvs[i].value == 0) {
      obErrorLog.ThrowError(__FUNCTION__, "Wrong value \"" + str + "\"",
                            obWarning);
      return false;
    }
  }
  ...
  if (old_cell == nullptr) {
    if (keep_fract) {
      obErrorLog.ThrowError(
          __FUNCTION__, "Cannot keep fractional coordinates without unit cell!",
          obWarning);
      return false;
    }
    for (int i = 0; i < vcvs.size(); i++) {
      if (vcvs[i].mult) {
        obErrorLog.ThrowError(__FUNCTION__,
                              "Cannot multiply sizes without unit cell!",
                              obWarning);
        return false;
      }
    }
  ...
  new_cell->SetData((vcvs[0].mult ? a : 1.0) * vcvs[0].value,
                    ...
                    (vcvs[2].mult ? c : 1.0) * vcvs[2].value, alpha, beta,
                    gamma);
  if (old_cell == nullptr)
