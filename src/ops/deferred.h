...
public:
  DeferredFormat(OBConversion *pConv, OBOp *pOp = nullptr,
                 bool CallDo = false) {
  ...
  const char *Description() override {
    return "Read and write an OBBase* array";
  }
  ...
  bool WriteChemObject(OBConversion *pConv) override {
    ...
    if (!_callDo ||
        _pOp->Do(pOb, "", pConv->GetOptions(OBConversion::GENOPTIONS), pConv))
    ...
    if (pConv->IsLast()) {
      ...
      if (_pOp) {
        ...
        if (!_obvec.empty()) {
          std::reverse(
              _obvec.begin(),
              _obvec.end()); // because DeferredFormat outputs in reverse order
