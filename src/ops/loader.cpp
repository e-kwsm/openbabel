...
/// recompiling. The class is itself a plugin but needs a short piece of code as
/// a hook in OBPlugin::LoadAllPlugins(). This does nothing if the plugin is not
/// loaded.
...
public:
  ...
  OBDefine(const char *ID, const char *filename)
    ...
    if (!ifs) {
      if (filefound)
        obErrorLog.ThrowError(
            __FUNCTION__, string(filename) + " found but could not be opened",
            obError);
      return;
    }
    ...
    while (ifs) // read entries for multiple objects
    {
      ...
      while (ifs && !ReadLine(ifs, textlines[0], true))
        ; // get first non-blank line
      if (!ifs)
        break;
      while (
          ReadLine(ifs, ln, true)) // get subsequent lines with comments removed
      {
      ...
      if (!textlines.empty() && FindDef(textlines[0].c_str()) != nullptr) {
        // Save a copy of textlines so that const char* pointers in new instance
        // point to something which has not been deleted
        ...
        // reallocations, which invalidates the const char* pointers in the
        // instances. So instead we populate _text fully before making the
        // plugin instances.
      } else
        obErrorLog.ThrowError(__FUNCTION__,
                              "Failed to make an instance " + textlines[0],
                              obError);
      textlines.clear();
    }
  ...
  const char *Description() override {
    return "Makes plugin classes from a datafile";
  }
  ...
  OBDefine *MakeInstance(const std::vector<std::string> &textlines) override {
    ...
    // the dummy global instance. When the global instance is deleted at the
    // end, all the other instances are too, avoiding (pseudo) memory leaks.
    // Versions for other OBPlugin classes don't do this.
...
private:
  ...
  // return true if non-empty line read, false if empty line read or eof or
  // error
...
OBPlugin *OBDefine::FindDef(const char *ID) {
  ...
  for (typeiter = PluginMap().begin(); typeiter != PluginMap().end();
       ++typeiter) {
    ...
    for (iter = map.begin(); iter != map.end(); ++iter) {
      ...
      pos2 = descr.rfind('\n', pos);
      if (pos2 != string::npos &&
          descr.substr(pos2, pos - pos2).find(ID) != string::npos)
        return iter->second;
    }
  }
  return nullptr;
