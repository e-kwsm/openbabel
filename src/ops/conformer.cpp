...
public:
  ...
  const char *Description() override {
    return "Conformer Searching (not displayed in GUI)\n"
           "Typical usage: obabel infile.xxx -O outfile.yy --conformer "
           "--nconf\n"
           " options:             description\n"
           ...
           " --weighted       weighted rotor search for lowest energy "
           "conformer\n"
           ...
           " --convergence #  number of identical generations before "
           "convergence is reached\n"
           " --score #        scoring function [rmsd|energy|minrmsd|minenergy] "
           "(default = rmsd)\n"
           " customize the filter used to sort out wrong conformers\n"
           " --csfilter #     the filtering algorithm [steric] "
           "(default=steric)\n"
           " --cutoff #       absolute distance in Anstroms below which atoms "
           "are considered to clash\n"
           " --vdw-factor #   apply this factor to all van-der-Waals radii "
           "before detecting clashes\n"
           " --ignore-H       do not detect clashes with hydrogen atoms\n"
           " customize rotors for GA based method (a rotor is given as a-b "
           "where a and b are indices)\n"
           " --printrot       print all possible rotors and do not perform a "
           "conformer search\n"
           " --onlyrot        declare a comma-separated list of rotors, all "
           "but these will be disregarded\n"
           " --norot          declare a comma-separated list of rotors that "
...
template <typename T> bool getValue(const std::string &str, T &value) {
  ...
  if (systematic || random || fast || weighted) {
    ...
    if (systematic) {
      pFF->SystematicRotorSearch(
          10, rings); // 10 steepest-descent forcfield steps per conformer
    } else if (fast) {
    ...
    if (rotchange) {
      ...
      while (std::getline(input, segment, ',')) {
        ...
        char additional;
        int converted =
            sscanf(segment.c_str(), "%d-%d%c", &p1, &p2, &additional);
        if (converted == 2 && p1 > 0 && p2 > 0) {
          if (p1 > pmol->NumAtoms() || p2 > pmol->NumAtoms()) {
            std::cerr << "ERROR at least one of the atom indices " << p1
                      << " or " << p2 << " is greater than the number of atoms "
                      << pmol->NumAtoms() << "." << std::endl
                      << std::flush;
            return false;
          }
          OBBond *b = pmol->GetBond(p1, p2);
          if (!b) {
            std::cerr << "ERROR atoms " << p1 << " and " << p2
                      << " form no bond." << std::endl
                      << std::flush;
            return false;
          }
          if (!(b->IsRotor())) {
            std::cerr << "ERROR bond formed by atoms " << p1 << " and " << p2
                      << " is not rotable." << std::endl
                      << std::flush;
            return false;
          }
          int idx = b->GetIdx();
          ...
          std::cerr << "ERROR parsing '" << segment
                    << "' as rotor (must be i-j where i and j are indices >0)"
                    << std::endl
                    << std::flush;
          return false;
        }
