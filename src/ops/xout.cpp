...
public:
  ...
  bool WriteChemObject(OBConversion *pConv) override {
    ...
    if (_pOrigConv) {
      // Need to copy pOb. But currently OBBase does not have a virtual Clone()
      // function. So do it just for OBMol.
    ...
    if (pConv->IsLast()) {
      if (_pOrigConv && pMolCopy != nullptr) {
        ...
        pConv->SetOutFormat(
            _pOrigConv
                ->GetOutFormat()); // ReportNumberConverted() uses this at end
      }
...
public:
  ...
  const char *Description() override {
    return "<file.xxx> Additional file output\n"
           "Mainly intended to be used to generate svg files for display from "
           "the GUI,\n"
...
/////////////////////////////////////////////////////////////////
bool OpExtraOut::Do(OBBase *pOb, const char *OptionText, OpMap *pmap,
                    OBConversion *pConv) {
  /*
  ...
  */
  ...
  if (pConv->IsFirstInput()) {
    ...
    Trim(sOptionText);
    OBConversion *pExtraConv =
        new OBConversion(*pConv); // copy ensures OBConversion::Index>-1
    std::ofstream *ofs;
    if ((ofs = new std::ofstream(
             OptionText))) // extra parens to indicate truth value
      pExtraConv->SetOutStream(ofs);
    if (!ofs ||
        !pExtraConv->SetOutFormat(OBConversion::FormatFromExt(sOptionText))) {
      obErrorLog.ThrowError(__FUNCTION__, "Error setting up extra output file",
                            obError);
      return true;
    }
    OBConversion *pOrigConv = new OBConversion(*pConv);
    pOrigConv->SetInStream(
        nullptr); // not used; avoids complications in AddChemObject()
    pExtraConv->SetInStream(nullptr);

    // Make an instance of ExtraFormat and divert the output to it. It will
    // delete itself.
