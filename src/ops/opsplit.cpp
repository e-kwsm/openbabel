/**********************************************************************
opsplit.cpp - A OBOp to write each molecule to a file with a name from the
molecule, usually thetitle.
...
public:
  ...
  const char *Description() override {
    return "[param] split, each filename from param;default title\n"
           ...
           "If an output filename is specified it is used to define the "
           "format\n"
           "and the path of the output files, but its name is ignored.\n"
           "If the molecule has no title or if it contains illegal "
           "characters,\n"
           ...
           "will produce a file for each of the first 8 molecules with "
           "filename\n"
           "the same as its sdf property ``chebi_id``.\n \n"

           "Filtering of the molecules or minor manipulation (e.g. make H "
           "explicit)\n"
           "can be done at the same time as splitting by adding further "
           "options\n"
           "to the command line.\n \n"

           "For files with Unix line-endings, copy format can be used to "
           "avoid\n"
           "uncertainties of interpretation. With SDF files, it can also be "
           "much\n"
           "faster if the -aT or -aP options are used when the title or\n"
           "properties respectively are used to derive the filename or in "
           "filtering.\n"
           "The following produces (in a subdirectory) an sdf file for each "
           "molecule,\n"
           "with a filename derived from its title:\n"
           "    obabel  in.sdf  -O sub/out.sdf  --split  -aT  -ocopy\n \n"

           "If a format specification like -osdf is used without a filename, "
           "the\n"
           "extension is chosen by OpenBabel from the alternatives for the "
           "format, and\n"
           "cannot be changed. So if you don't like .smiles, use a dummy "
           "filename.\n\n";
  }
...
/////////////////////////////////////////////////////////////////
bool OpSplit::Do(OBBase *pOb, const char *OptionText, OpMap *pOptions,
                 OBConversion *pConv) {
  ...
  if (pConv->IsFirstInput()) {
    ...
    // If there is an output file specified, delete the file,close and
    // invalidate the outstream so OBConversion is not confused.
    ...
    // If there is an output file name, use its path and its extension for the
    // output files. Otherwise use current directory and the ID of the output
    // format.
    _outExt = _outPath =
        pConv->GetOutFilename(); // This may not be present yet in OBConversion
    ...
    /* Need to call this op after all other options because it outputs directly
       ...
       the op deactivating itself and setting output to DeferredFormat. When
       this is written to, it immediately calls back this function, which writes
       to an individual file of the real output format.
    */
    pConv->AddOption(GetID(), OBConversion::GENOPTIONS,
                     "inactive");          // removing messes up DoOps()
  ...
  if (!filenameok || !ofs) {
    // filename is empty or contain an illegal character or failed to open. Use
    // "n.xxx"
    obErrorLog.ThrowError(__FUNCTION__,
                          "The fallback filename, based on input index, has "
                          "been used for at least one object.",
                          obWarning, onceOnly);
    std::stringstream ss;
    ...
    if (!ofs) {
      obErrorLog.ThrowError(__FUNCTION__, "Cannot open file " + ss.str(),
                            obError);
  ...
  // Correct for second call to GetChemObject(), which increments index, in
  // above. GetChemObject() already called in DeferredFormat
