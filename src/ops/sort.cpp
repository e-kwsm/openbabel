...
template <class T> struct Order {
  ...
  bool operator()(std::pair<OBBase *, T> p1, std::pair<OBBase *, T> p2) const {
    return _rev ? _pDesc->Order(p2.second, p1.second)
...
/////////////////////////////////////////////////////////////////
bool OpSort::Do(OBBase *pOb, const char *OptionText, OpMap *pmap,
                OBConversion *pConv) {
  if (pConv && pConv->IsFirstInput()) {
    ...
    _addDescToTitle = *(OptionText + strlen(OptionText) - 1) == '+'; // last
                                                                     // char
    if (_addDescToTitle)
    ...
    std::pair<std::string, std::string> spair =
        OBDescriptor::GetIdentifier(optionStream);
    _pDesc = OBDescriptor::FindType(spair.first.c_str());
    if (!_pDesc) {
      obErrorLog.ThrowError(__FUNCTION__,
                            std::string("Unknown descriptor ") + OptionText,
                            obError, onceOnly);
      return false;
    }
...
bool OpSort::ProcessVec(std::vector<OBBase *> &vec) {
  // Make a vector containing both the OBBase* and the descriptor value and the
  // sort it
  if (!IsNan(_pDesc->Predict(vec[0], &_pDescOption))) {
    ...
    for (iter = vec.begin(); iter != vec.end(); ++iter)
      valvec.push_back(std::make_pair<OBBase *, double>(
...
descriptor value was recalculated every time it was needed, which is
inappropriate for sorting. A local cache of InChI values was implemented but a
more general solution was needed. The values are now calculated once and stored
here in a vector, which stores numbers or strings and the code is extensively
duplicated because of this. But using templates was not much shorter because
four templated functions were needed, and the code more difficult to understand.
*/
