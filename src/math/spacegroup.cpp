...
SpaceGroup::SpaceGroup()
    : HEXAGONAL_ORIGIN(10), m_HM(""), m_Hall(""), m_id(0),
      m_OriginAlternative(0) {}
...
list<vector3> SpaceGroup::Transform(const vector3 &v) const {
  ...
  for (i = m_transforms.begin(); i != iend; i++) {
    ...
    for (j = res.begin(); j != jend; ++j)
      if (fabs(t.x() - (*j).x()) < prec && fabs(t.y() - (*j).y()) < prec &&
          fabs(t.z() - (*j).z()) < prec) {
        duplicate = true;
        break;
      }
...
const SpaceGroup *SpaceGroup::GetSpaceGroup(const string &name_in) {
  ...
  const SpaceGroup *match =
      _SpaceGroups.sgbn.find(name_in) != _SpaceGroups.sgbn.end()
          ? _SpaceGroups.sgbn[name_in]
          : nullptr;
  if (match)
    return match;
  ...
  match = _SpaceGroups.sgbn.find(name) != _SpaceGroups.sgbn.end()
              ? _SpaceGroups.sgbn[name]
              : nullptr;
  ...
  if (!match) {
    ...
    string search = name;
    bool hasMirror =
        (name.find('m') != string::npos || name.find('d') != string::npos ||
         name.find('n') != string::npos || name.find('c') != string::npos);
    if (name.find('4') != string::npos && hasMirror &&
        name.find('-') == string::npos) {
      search.insert(name.find('4'), "-");
    } else if (name.find('3') != string::npos && hasMirror &&
               name.find('-') == string::npos) {
      search.insert(name.find('3'), "-");
    } else if (name.find('6') != string::npos && hasMirror &&
               name.find('-') == string::npos) {
      search.insert(name.find('6'), "-");
    }

    match = _SpaceGroups.sgbn.find(search) != _SpaceGroups.sgbn.end()
                ? _SpaceGroups.sgbn[search]
                : nullptr;
  }
...
bool SpaceGroup::IsValid() const {
  ...
  for (j = T.begin(); j != end; ++j) {
    ...
    for (k = T.begin(); k != end; ++k) {
      ...
      if (T.find(s) == end) {
        cerr << "Invalid transform: " << (*j).first << " * " << (*k).first
             << " = " << s << endl;
        return false;
      }
...
const SpaceGroup *SpaceGroup::Find(SpaceGroup *group) {
  ...
  if (group->m_Hall.length() > 0 &&
      _SpaceGroups.sgbn.find(group->m_Hall) != _SpaceGroups.sgbn.end()) {
    found = _SpaceGroups.sgbn[group->m_Hall];
    if (!found)
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Unknown space group (Hall symbol:" + group->m_Hall +
    ...
    if (group->m_transforms.size() && *found != *group) {
      ...
      if (id != 3 && id != 68) // these groups have duplicates
      {
        obErrorLog.ThrowError(
            __FUNCTION__,
            "Space group error (Hall symbol and list of transforms do not "
            "match), please file a bug report.",
            obWarning);
        return found;
      }
    } else
      /* even if there is an error (this should not occur) return the found
         group, since Hall names are secure */
      return found;
  }
  // Identify from the HM symbol, after removing all whitespaces or underscore
  // (which are valid separators in old CIF files)
  ...
  if (stripped_hm.length() > 0 &&
    ...
    if (group->m_transforms
            .size()) { // If transforms (symmetry operations) are listed, make
                       // sure they match the tabulated ones
      list<const SpaceGroup *>::const_iterator i,
          end = _SpaceGroups.sgbi[found->m_id - 1].end();
      for (i = _SpaceGroups.sgbi[found->m_id - 1].begin(); i != end; ++i)
        if ((**i) == *group)
          return *i;
      obErrorLog.ThrowError(
          __FUNCTION__,
          "Unknown space group error (H-M symbol:" + group->m_HM +
              "), cannot match the list of transforms, please file a bug "
              "report.",
          obError);
      return nullptr;
    } else if (group->m_transforms.size() ==
               0) { // No transforms (symmetry operations) are listed, warn if
                    // HM symbol can match several spacegroups
      int n = 0;
      list<const SpaceGroup *>::const_iterator i,
          end = _SpaceGroups.sgbi[group->m_id].end();
      for (i = _SpaceGroups.sgbi[group->m_id].begin(); i != end; ++i)
        if (RemoveWhiteSpaceUnderscore((*i)->m_HM) == stripped_hm)
          n++;
      if (n > 1)
        obErrorLog.ThrowError(__FUNCTION__,
                              "Ambiguous space group: HM symbol corresponds to "
                              "several space groups.",
                              obWarning);
      return found;
    }
    /* even if there is an error (this should not occur) return the found group,
       since Hall names are secure */
  } else if (group->m_id > 0 && group->m_id <= 230) {
    if (group->m_transforms.size()) {
      list<const SpaceGroup *>::const_iterator i,
          end = _SpaceGroups.sgbi[group->m_id - 1].end();
      ...
      if (_SpaceGroups.sgbi[group->m_id - 1].size() > 1)
        obErrorLog.ThrowError(__FUNCTION__,
                              "Ambiguous space group: sg number corresponds to "
                              "several space groups.",
                              obWarning);
  ...
  if (!group->IsValid()) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Unknown space group (HM:" + group->m_HM +
                              ",Hall:" + group->m_Hall +
                              ") with incomplete or wrong definition.",
                          obWarning);
    return nullptr;
  }
