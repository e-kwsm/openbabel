...
void matrix3x3::RotAboutAxisByAngle(const vector3 &v, const double angle) {
  ...
  double x = vtmp.x(), y = vtmp.y(), z = vtmp.z();
...
matrix3x3 operator*(const matrix3x3 &A, const matrix3x3 &B) {
  ...
  result.ele[0][0] = A.ele[0][0] * B.ele[0][0] + A.ele[0][1] * B.ele[1][0] +
                     A.ele[0][2] * B.ele[2][0];
  result.ele[0][1] = A.ele[0][0] * B.ele[0][1] + A.ele[0][1] * B.ele[1][1] +
                     A.ele[0][2] * B.ele[2][1];
  result.ele[0][2] = A.ele[0][0] * B.ele[0][2] + A.ele[0][1] * B.ele[1][2] +
                     A.ele[0][2] * B.ele[2][2];

  result.ele[1][0] = A.ele[1][0] * B.ele[0][0] + A.ele[1][1] * B.ele[1][0] +
                     A.ele[1][2] * B.ele[2][0];
  result.ele[1][1] = A.ele[1][0] * B.ele[0][1] + A.ele[1][1] * B.ele[1][1] +
                     A.ele[1][2] * B.ele[2][1];
  result.ele[1][2] = A.ele[1][0] * B.ele[0][2] + A.ele[1][1] * B.ele[1][2] +
                     A.ele[1][2] * B.ele[2][2];

  result.ele[2][0] = A.ele[2][0] * B.ele[0][0] + A.ele[2][1] * B.ele[1][0] +
                     A.ele[2][2] * B.ele[2][0];
  result.ele[2][1] = A.ele[2][0] * B.ele[0][1] + A.ele[2][1] * B.ele[1][1] +
                     A.ele[2][2] * B.ele[2][1];
  result.ele[2][2] = A.ele[2][0] * B.ele[0][2] + A.ele[2][1] * B.ele[1][2] +
                     A.ele[2][2] * B.ele[2][2];
...
/*! This method checks if the absolute value of the determinant is smaller than
  1e-6. If so, nothing is done and an exception is thrown. Otherwise, the
...
bool matrix3x3::isUnitMatrix(void) const {
  return (isDiagonal() && IsApprox(ele[0][0], 1.0, 1e-6) &&
          IsApprox(ele[1][1], 1.0, 1e-6) && IsApprox(ele[2][2], 1.0, 1e-6));
}
...
#ifdef OB_OLD_MATH_CHECKS
  if (!isSymmetric()) {
    OBError er("matrix3x3::findEigenvectorsIfSymmetric(vector3 &eigenvals) "
               "const throw(OBError)",
               "The method was called on a matrix that was not symmetric, i.e. "
               "where isSymetric() == false.",
               "This is a runtime or a programming error in your application.");
    throw er;
  }
#endif
...
void matrix3x3::FillOrth(double Alpha, double Beta, double Gamma, double A,
                         double B, double C) {
  double V;
...
{
  co << "[ " << m.ele[0][0] << ", " << m.ele[0][1] << ", " << m.ele[0][2]
     << " ]" << endl;

  co << "[ " << m.ele[1][0] << ", " << m.ele[1][1] << ", " << m.ele[1][2]
     << " ]" << endl;

  co << "[ " << m.ele[2][0] << ", " << m.ele[2][1] << ", " << m.ele[2][2]
     << " ]" << endl;
