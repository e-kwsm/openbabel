...
OBAlign::OBAlign(const OBMol &refmol, const OBMol &targetmol, bool includeH,
                 bool symmetry)
    : _method(OBAlign::Kabsch) {
  _symmetry = symmetry;
  _includeH = includeH;
...
void OBAlign::VectorsToMatrix(const vector<vector3> *pcoords,
...
void OBAlign::SetMethod(OBAlign::AlignMethod method) { _method = method; }
...
static double QCProot(const vector<double> &coeff, double guess,
                      const double delta) {
  int i;
  double oldg;
  ...
  for (i = 0; i < 50; ++i) {
    oldg = guess;
    /* guess -= (eval_horn_quart(coeff, guess) / eval_horn_quart_deriv(coeff,
     * guess)); */
...
vector<double> CalcQuarticCoeffs(const Eigen::Matrix3d &M) {
  ...
  /* coeff[4] = 1.0; */
  /* coeff[3] = 0.0; */
  // coeff[2] = -2.0 * (Sxx2 + Syy2 + Szz2 + Sxy2 + Syx2 + Sxz2 + Szx2 + Syz2 +
  // Szy2);
  coeff[2] = -2.0 * M_sqr.sum();
  coeff[1] = 8.0 * (Sxx * Syz * Szy + Syy * Szx * Sxz + Szz * Sxy * Syx -
...
matrix3x3 OBAlign::GetRotMatrix() {
  if (!_ready) {
    obErrorLog.ThrowError(
        __FUNCTION__, "Rotation matrix not available until you call Align()",
        obError);
    return matrix3x3();
  }
  ...
  for (int row = 0; row < 3; ++row)
    for (int col = 0; col < 3; ++col)
      rot[col][row] =
          _rotMatrix(row, col); // Return in form suitable for use in
                                // expressions like "result *= rotMatrix";
...
bool OBAlign::UpdateCoords(OBMol *target) {
  ...
  if (newcoords.size() != target->NumAtoms()) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "Cannot update the target molecule with the "
                          "alignment coordinates as they are of different size",
                          obError);
    return false;
  }
...
double OBAlign::GetRMSD() {
  if (!_ready) {
    obErrorLog.ThrowError(__FUNCTION__,
                          "RMSD not available until you call Align()", obError);
    return nan("");
  }
