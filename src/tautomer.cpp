...
struct TautomerImpl {
...
#ifdef DEBUG
  static std::string indentation(int n) { return std::string(n * 4, ' '); }

  // Use RAII to keep track of enetering and leaving a function
  class EnterExit {
  public:
    explicit EnterExit(const std::string &function, int indent = 0)
        : m_function(function), m_indent(indent) {
      std::cout << ::OpenBabel::TautomerImpl::indentation(m_indent) << "Enter "
                << m_function << "()..." << std::endl;
    }

    ~EnterExit() {
      std::cout << ::OpenBabel::TautomerImpl::indentation(m_indent) << "Exit "
                << m_function << "()..." << std::endl;
    }
  ...
  public:
    AssignDonorRAII(std::vector<Type> &atomTypes, int &hydrogenCounter,
                    OBAtom *atom)
        : m_atomTypes(atomTypes), m_atom(atom),
          m_hydrogenCounter(hydrogenCounter) {
      redo(atom);
    }

    ~AssignDonorRAII() { undo(); }

    void redo(OBAtom *atom) {
#ifdef DEBUG
      std::cout << "AssignDonorRAII::redo(" << atom->GetIndex() << ")"
                << std::endl;
#endif
    ...
    void undo() {
      if (!m_atom)
        return;
#ifdef DEBUG
      std::cout << "AssignDonorRAII::undo(" << m_atom->GetIndex() << ")"
                << std::endl;
#endif
  ...
  public:
    AssignAcceptorRAII(std::vector<Type> &atomTypes, OBAtom *atom)
        : m_atomTypes(atomTypes), m_atom(atom) {
      redo(atom);
    }

    ~AssignAcceptorRAII() { undo(); }

    void redo(OBAtom *atom) {
#ifdef DEBUG
      std::cout << "AssignAcceptorRAII::redo(" << atom->GetIndex() << ")"
                << std::endl;
#endif
      m_atom = atom;
    ...
    void undo() {
      if (!m_atom)
        return;
#ifdef DEBUG
      std::cout << "AssignAcceptorRAII::undo(" << m_atom->GetIndex() << ")"
                << std::endl;
#endif
      m_atomTypes[m_atom->GetIndex()] = Unassigned;
      m_atom = nullptr;
  ...
  public:
    PropagationRAII(std::vector<Type> &atomTypes, std::vector<Type> &bondTypes,
                    int &hydrogenCounter)
    ...
    void release() {
      std::copy(m_donors.begin(), m_donors.end(),
                std::back_inserter(m_acceptors));
      m_donors.clear();
    }
  ...
  void PrintBondTypes(OBMol *mol, const std::vector<Type> &bondTypes,
                      int indent = 0) {
  ...
  std::vector<Type> InitializeBondTypes(OBMol *mol,
                                        const std::vector<Type> &atomTypes) {
    std::vector<Type> bondTypes;

    FOR_BONDS_OF_MOL(bond, mol) {
      assert(bond->GetBeginAtomIdx() <= mol->NumAtoms());
      assert(bond->GetEndAtomIdx() <= mol->NumAtoms());
      if (atomTypes[bond->GetBeginAtomIdx() - 1] != Other &&
          atomTypes[bond->GetEndAtomIdx() - 1] != Other)
  ...
  bool IsPropagationValid(OBMol *mol, const std::vector<Type> &atomTypes,
                          const std::vector<Type> &bondTypes, int depth) {
    FOR_ATOMS_OF_MOL(atom, mol) {
      ...
      if (atomTypes[atom->GetIndex()] == Acceptor ||
          atomTypes[atom->GetIndex()] == Hybridized) {
        // have at least one unassigned or double bond
        if (!numUnassigned && !numDoubleBond) {
#ifdef DEBUG
          std::cout << indentation(depth)
                    << "invalid Acceptor/Hybridized [no unassigned/double "
                       "bond] for atom "
                    << atom->GetIndex() << std::endl;
#endif
          return false;
        }
        ...
        if (numDoubleBond > 1) {
          ...
          std::cout
              << indentation(depth)
              << "invalid Acceptor/Hybridized [multiple double bonds] for atom "
              << atom->GetIndex() << std::endl;
#endif
          return false;
        }
  ...
  void AssignmentPropagation(OBMol *mol, std::vector<Type> &atomTypes,
                             std::vector<Type> &bondTypes, int &numHydrogens,
                             TautomerFunctor &functor, int depth) {
#ifdef DEBUG
    ...
    while (changed) {
        ...
        if (atomTypes[atom->GetIndex()] == Donor) {
          FOR_BONDS_OF_ATOM(bond, &*atom) {
            if (bondTypes[bond->GetIdx()] == Unassigned) {
              ...
              changed = true;
#ifdef DEBUG
              std::cout << ee.indentation() << "-> Rule 1: Assign "
                        << bond->GetBeginAtomIdx() - 1 << "-"
                        << bond->GetEndAtomIdx() - 1 << " Single" << std::endl;
#endif
            }
          }
        }
      ...
      // Any unassigned atom that has all of its bonds assigned single, must be
      // assigned a donor
      if (numHydrogens) {
        FOR_ATOMS_OF_MOL(atom, mol) {
          ...
          if (allBondsSingle) {
            ...
            changed = true;
#ifdef DEBUG
            std::cout << ee.indentation() << "-> Rule 2: Assign "
                      << atom->GetIndex() << " Donor" << std::endl;
#endif
          }
        }
      }

      // Any unassigned atom with an assigned double bond must be assigned an
      // acceptor
      FOR_ATOMS_OF_MOL(atom, mol) {
        if (atomTypes[atom->GetIndex()] != Unassigned)
          continue;
        ...
        if (hasDoubleBond) {
          ...
          changed = true;
#ifdef DEBUG
          std::cout << ee.indentation() << "-> Rule 3: Assign "
                    << atom->GetIndex() << " Acceptor" << std::endl;
#endif
        }
      }

      // A hybridized or acceptor atom with assigned double bond assigns all the
      // remaining unassigned bonds single
      FOR_ATOMS_OF_MOL(atom, mol) {
        if (atomTypes[atom->GetIndex()] != Acceptor &&
            atomTypes[atom->GetIndex()] != Hybridized)
          continue;
        ...
        if (hasDoubleBond) {
          FOR_BONDS_OF_ATOM(bond, &*atom) {
            if (bondTypes[bond->GetIdx()] == Unassigned) {
              ...
              changed = true;
#ifdef DEBUG
              std::cout << ee.indentation() << "-> Rule 4: Assign "
                        << bond->GetBeginAtomIdx() - 1 << "-"
                        << bond->GetEndAtomIdx() - 1 << " Single" << std::endl;
#endif
            }
      ...
      // A hybridized or acceptor atom with a single unassigned bond assigns it
      // double if it doesn't already have one
      FOR_ATOMS_OF_MOL(atom, mol) {
        if (atomTypes[atom->GetIndex()] != Acceptor &&
            atomTypes[atom->GetIndex()] != Hybridized)
          continue;
        ...
        if (!hasDoubleBond && numUnassigned == 1) {
          FOR_BONDS_OF_ATOM(bond, &*atom) {
            if (bondTypes[bond->GetIdx()] == Unassigned) {
              ...
              changed = true;
#ifdef DEBUG
              std::cout << ee.indentation() << "-> Rule 5: Assign "
                        << bond->GetBeginAtomIdx() - 1 << "-"
                        << bond->GetEndAtomIdx() - 1 << " Double" << std::endl;
#endif
            }
          }
        }
    ...
    if (IsLeafNode(atomTypes)) {
      ...
      // This happens for phenyl rings and other fragments consisting of only
      // hybridized atoms
      for (std::size_t i = 0; i < bondTypes.size(); ++i)
        if (bondTypes[i] == Unassigned) {
          ...
          propagation.assignBond(bond, Double);
#ifdef DEBUG
          std::cout << ee.indentation()
                    << "-> Unasigned bonds remaining, assigning "
                    << bond->GetBeginAtomIdx() - 1 << "-"
                    << bond->GetEndAtomIdx() - 1 << " Double" << std::endl;
#endif
          AssignmentPropagation(mol, atomTypes, bondTypes, numHydrogens,
                                functor, depth + 1);
...
#ifdef DEBUG
        std::cout << ee.indentation() << "  --> LeafNode reached..."
                  << std::endl;
#endif
      ...
      EnumerateRecursive(mol, atomTypes, bondTypes, numHydrogens, functor,
                         depth + 1);
    }
  ...
  bool HasTooManyHydrogensLeft(const std::vector<Type> &atomTypes,
                               int numHydrogens) const {
    // Check to ensure there are at least enough unassigned atoms left to assign
    // hydrogens to
    return std::count(atomTypes.begin(), atomTypes.end(), Unassigned) <
           numHydrogens;
  }

  void EnumerateRecursive(OBMol *mol, std::vector<Type> &atomTypes,
                          std::vector<Type> &bondTypes, int numHydrogens,
                          TautomerFunctor &functor, int depth) {
#ifdef DEBUG
    EnterExit ee("EnumerateRecursive", depth);

    std::cout << ee.indentation() << numHydrogens << " hydrogens left [1]..."
              << std::endl;
...
#ifdef DEBUG
      std::cout << ee.indentation()
                << "--> Canonical and leaf node found, backtracking..."
                << std::endl;
#endif
      return;
    }
...
#ifdef DEBUG
      std::cout << ee.indentation()
                << "--> No unassigned atoms left, backtracking..." << std::endl;
#endif
      return;
    }
...
#ifdef DEBUG
      std::cout << ee.indentation() << "-> Assign atom " << nextAtom->GetIndex()
                << " Donor" << std::endl;
#endif

      // Propagate...
      AssignmentPropagation(mol, atomTypes, bondTypes, numHydrogens, functor,
                            depth + 1);

#ifdef DEBUG
      std::cout << ee.indentation() << numHydrogens << " hydrogens left [1b]..."
                << std::endl;
      PrintAtomTypes(atomTypes, depth + 1);
      PrintBondTypes(mol, bondTypes, depth + 1);
#endif
...
#ifdef DEBUG
    std::cout << ee.indentation() << numHydrogens << " hydrogens left [2]..."
              << std::endl;
    PrintAtomTypes(atomTypes, depth + 1);
    PrintBondTypes(mol, bondTypes, depth + 1);
#endif
...
#ifdef DEBUG
      std::cout << ee.indentation()
                << "--> Too many hydrogens left, backtracking..." << std::endl;
#endif
    }
  }
  ...
  static void SanityCheckHydrogens(OBAtom *atom, int indent) {
    ...
    unsigned int totH = 0;
    FOR_ATOMS_OF_MOL(atm, mol) { totH += atm->GetImplicitHCount(); }
    std::cout << indentation(indent)
              << "Total no. of hydrogens in molecule: " << totH << std::endl;
  }
#endif
  ...
  static void DecrementImplicitHCount(OBAtom *atom, int indent) {
    atom->SetImplicitHCount(atom->GetImplicitHCount() - 1);
#ifdef DEBUG
    std::cout << indentation(indent) << "Decremented " << atom->GetIndex()
              << " (" << atom->GetAtomicNum() << ") to "
              << static_cast<unsigned int>(atom->GetImplicitHCount())
              << std::endl;
  ...
  void Enumerate(OBMol *mol, TautomerFunctor &functor, bool canonical) {
    ...
    // If all bonds are assigned around an atom, mark it as Other (e.g. sulfon
    // oxygen) This avoids premature backtracking (see IsPropagationValid)
    FOR_ATOMS_OF_MOL(atom, mol) {
