...
OBMolBondBFSIter::OBMolBondBFSIter(OBMol *mol, int StartIndex) : _parent(mol) {
  ...
  for (OBAtomBondIter b(_ptr->GetBeginAtom()); b;
       ++b) { // Loop thru all bonds attached to the start of the bond
  ...
  for (OBAtomBondIter b(_ptr->GetEndAtom()); b;
       ++b) { // Loop thru all bonds attached to the end of the bond
    if (_notVisited[b->GetIdx()]) { // Don't revisit the initial bond
      _queue.push(&(*b));
      _depth[b->GetIdx()] = 2;
      _notVisited.SetBitOff(b->GetIdx());
...
/** \class OBResidueIter obiter.h <openbabel/obiter.h>
    ...
    To facilitate iteration through all residues in a molecule, without
resorting to residue indexes (which may change in the future) a variety of
    iterator classes and methods are provided.
...
/** \class OBResidueAtomIter obiter.h <openbabel/obiter.h>
    ...
    \code
    \#define FOR_ATOMS_OF_RESIDUE(a,r)     for( OBResidueAtomIter     a(r); a;
++a ) \endcode
...
/** \class OBMolTorsionIter obiter.h <openbabel/obiter.h>
    ...
    To facilitate iteration through all torsions in a molecule, without
resorting to atom indexes (which <strong>will</strong> change in the future), a
    variety of iterator methods are provided.
...
OBMolTorsionIter::OBMolTorsionIter(OBMol *mol) {
  mol->FindTorsions();
  OBTorsionData *td =
      (OBTorsionData *)mol->GetData(OBGenericDataType::TorsionData);
...
OBMolTorsionIter::OBMolTorsionIter(OBMol &mol) {
  ...
  if (!_vtorsion.empty()) {
    ...
    // always set _i, _i will be compared to _vtorsion.end() in
    // OBMolTorsionIter::bool()
    _i = _vtorsion.end();
  }
}
