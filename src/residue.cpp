...
char Residue[MAXRES][4] = {
    ...
    /*  RNA Nucleotides  */
    /*===================*/
    "  U", " +U", "  I", "1MA", "5MC", "OMC", "1MG", "2MG", "M2G", "7MG", "OMG",
    " YG", "H2U", "5MU", "PSU",

    /*=================*/
    /*  Miscellaneous  */
    /*=================*/
    "UNK", "ACE", "FOR", "HOH", "DOD", "SO4", "PO4", "NAD", "COA", "NAP",
    "NDP"};
...
char ElemDesc[MAXELEM][4] = {
    ...
    {' ', 'C', ' ', ' '}, /* 2*/
    {' ', 'O', ' ', ' '},
    /* 3*/                 /* 0-3   Amino Acid Backbone    */
    {' ', 'C', '\'', ' '}, /* 4*/
    {' ', 'O', 'T', ' '},  /* 5*/
    {' ', 'S', ' ', ' '},  /* 6*/
    {' ', 'P', ' ', ' '},
    /* 7*/                /* 4-7   Shapely Amino Backbone */
    {' ', 'O', '1', 'P'}, /* 8*/
    {' ', 'O', '2', 'P'}, /* 9*/
    {' ', 'O', '5', '*'}, /*10*/
    {' ', 'C', '5', '*'}, /*11*/
    ...
    {' ', 'O', '2', '*'}, /*17*/
    {' ', 'C', '1', '*'},
    /*18*/ /* 7-18  Nucleic Acid Backbone  */
    {' ', 'C', 'A', '2'},
    /*19*/ /* 19    Shapely Special        */
    {' ', 'S', 'G', ' '},
    /*20*/                      /* 20    Cysteine Sulphur       */
    {' ', 'N', '1', ' '},       /*21*/
    {' ', 'N', '2', ' '},       /*22*/
    {' ', 'N', '3', ' '},       /*23*/
    {' ', 'N', '4', ' '},       /*24*/
...
OBResidue::OBResidue(const OBResidue &src) : OBBase() {
  _chain = src._chain;
  _aakey = src._aakey;
  _reskey = src._reskey;
  _resnum = src._resnum;
...
void OBResidue::SetChain(char chain) { _chain = chain; }

void OBResidue::SetChainNum(unsigned int chainnum) {
  _chain = (char)('A' + chainnum - 1);
}

void OBResidue::SetIdx(unsigned int idx) { _idx = idx; }
...
void OBResidue::SetSegName(const string &name) { _segname = name; }
...
void OBResidue::SetNum(const string resnum) { _resnum = resnum; }
...
vector<OBAtom *> OBResidue::GetAtoms(void) const { return _atoms; }
...
string OBResidue::GetName(void) const { return _resname; }

string OBResidue::GetSegName(void) const { return _segname; }

std::string OBResidue::GetNumString(void) { return _resnum; }

int OBResidue::GetNum(void) { return atoi(_resnum.c_str()); }
...
char OBResidue::GetChain(void) const { return _chain; }
...
unsigned int OBResidue::GetIdx(void) const { return (_idx); }

unsigned int OBResidue::GetResKey(void) const { return (_reskey); }

char OBResidue::GetInsertionCode(void) const { return (_insertioncode); }
...
bool OBResidue::GetAtomProperty(OBAtom *atom, int property) const {
  if (atom != nullptr) {
    ...
    case OBResidueAtomProperty::LIGAND:
      return IsHetAtom(atom) && !GetResidueProperty(OBResidueProperty::SOLVENT);
...
bool OBResidue::GetResidueProperty(int property) const {
  ...
  case OBResidueProperty::COENZYME:
    return (_reskey >= OBResidueIndex::NAD) && (_reskey <= OBResidueIndex::NDP);

  case OBResidueProperty::ION:
    return (_reskey == OBResidueIndex::SO4) || (_reskey == OBResidueIndex::PO4);

  case OBResidueProperty::NUCLEO:
    return (_reskey >= OBResidueIndex::A) && (_reskey <= OBResidueIndex::PSU);
  ...
  case OBResidueProperty::PURINE:
    return (_reskey == OBResidueIndex::A) || (_reskey == OBResidueIndex::G);

  case OBResidueProperty::PYRIMIDINE:
    return (_reskey == OBResidueIndex::C) || (_reskey == OBResidueIndex::T);

  case OBResidueProperty::SOLVENT:
    return (_reskey >= OBResidueIndex::HOH) && (_reskey <= OBResidueIndex::PO4);

  case OBResidueProperty::WATER:
    return (_reskey == OBResidueIndex::HOH) || (_reskey == OBResidueIndex::DOD);
