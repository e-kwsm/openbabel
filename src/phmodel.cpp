  ...
  if (EQn(buffer, "TRANSFORM", 7)) {
    ...
    if (vs.size() < 5) {
      obErrorLog.ThrowError(
          __FUNCTION__,
          " Could not parse line in phmodel table from phmodel.txt", obInfo);
      return;
    }
  ...
  for (unsigned int i = 0; i < _vtsfm.size(); ++i) {
    ...
    if (_vpKa[i] > 1E+9) {
      ...
      if (_vtsfm[i]->IsAcid()) {
        ...
        // cout << "pow(10, _vpKa[i] - pH) == " << pow(10, _vpKa[i] - pH) <<
        // endl;
...
/** \class OBChemTsfm phmodel.h <openbabel/phmodel.h>
    ...
    For storing and converting chemical reaction files, use the OBReaction
class.
**/
  ...
  for (i = 0; i < _bgn.NumBonds(); ++i) {
    ...
    for (j = 0; j < _end.NumBonds(); ++j) {
      ...
      if ((bvb1 == evb1 && bvb2 == evb2) || (bvb1 == evb2 && bvb2 == evb1)) {
        ...
        _vbond.push_back(
            pair<pair<int, int>, int>(pair<int, int>(bsrc, bdst), eord));
        break;
      }
    }
  }
...
bool OBChemTsfm::Apply(OBMol &mol) {
  ...
  obErrorLog.ThrowError(__FUNCTION__, "Ran OpenBabel::OBChemTransform",
                        obAuditMsg);
        ...
        if (j->first < (signed)i->size()) { // goof proofing
          ...
          if (j->second != old_charge) {
            ...
            OBAtomAssignTypicalImplicitHydrogens(
                atom); // update with new charge info
          }
        }
  }
    ...
    for (i = mlist.begin(); i != mlist.end(); ++i)
      for (j = _vbond.begin(); j != _vbond.end(); ++j) {
        ...
        for (int k = 0; k < 2; ++k) {
          ...
          int new_hcount =
              atom->GetImplicitHCount() - (j->second - old_bond_order);
          if (new_hcount < 0)
            new_hcount = 0;
