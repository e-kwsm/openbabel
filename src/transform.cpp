...
OBBase *
OBMol::DoTransformations(const std::map<std::string, std::string> *pOptions,
                         OBConversion *pConv) {
  ...
  // This is now a virtual function. The OBBase version just returns the OBMol
  // pointer. This is declared in mol.h
  ...
  if (itr != pOptions->end()) {
    ...
    for (it = vec.begin(); it != vec.end(); ++it) {
      ...
      if (pos == string::npos) {
        ...
        pos = it->find(' ');
        if (pos == string::npos) {
          obErrorLog.ThrowError(__FUNCTION__, "Missing property value",
                                obError);
          ret = false;
          break;
        }
  ...
  itr = pOptions->find(
      "append"); // Appends values of descriptors or properties to title
  if (itr != pOptions->end()) {
    string title(GetTitle());
    title += OBDescriptor::GetValues(this, itr->second);
    if (ispunct(title[0]))
      title[0] =
          ' '; // a leading punct char is used only as a separator, not at start
  ...
  if (fmatch) {
    ...
    if (itr != pOptions->end() && !itr->second.empty()) {
      ...
      if (itr2 != pOptions->end() &&
          (int)NumHvyAtoms() != atoi(itr2->second.c_str()))
        fmatch = false;
      else {
...
const char *OBMol::ClassDescription() {
  ...
  ret = "For conversions of molecules\n"
        ...
        "-v\"smarts\" Convert only if NO match to SMARTS or mols in file(not "
        "displayed in GUI)\n"
