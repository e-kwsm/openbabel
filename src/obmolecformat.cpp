/**********************************************************************
obmolecformat.cpp - Implementation of subclass of OBFormat for conversion of
OBMol.
...
bool OBMoleculeFormat::ReadChemObjectImpl(OBConversion *pConv,
                                          OBFormat *pFormat) {
  ...
  if (pConv->IsOption("separate", OBConversion::GENOPTIONS)) {
    ...
    // On subsequent calls, remove a fragment from MolArray and send it for
    // writing Done this way so that each fragment can be written to its own
    // file (with -m option)
    if (!StoredMolsReady) {
      while (ret) // do all the molecules in the file
      {
        ...
        if (ret && (pmol->NumAtoms() > 0 || (pFormat->Flags() & ZEROATOMSOK))) {
          vector<OBMol> SepArray =
              pmol->Separate(); // use un-transformed molecule
      ...
      // Clear the flags of the input stream(which may have found eof) to ensure
      // will try to read anothe molecule and allow the stored ones to be sent
      // for output.
    ...
    if (MolArray.empty()) // normal end of fragments
      ...
      // Copying is needed because the OBMol passed to AddChemObject will be
      // deleted. The OBMol in the vector is deleted here.
      OBMol *pMolCopy = new OBMol(MolArray.back());
      MolArray.pop_back();
      ret = pConv->AddChemObject(pMolCopy->DoTransformations(
  ...
  if (ret && (pmol->NumAtoms() > 0 || pmol->IsReaction() ||
              (pFormat->Flags() & ZEROATOMSOK &&
               (*pmol->GetTitle() || pmol->HasData(1))))) {
    ptmol = static_cast<OBMol *>(pmol->DoTransformations(
    ...
    if (ptmol && (pConv->IsOption("j", OBConversion::GENOPTIONS) ||
      ...
      // will be discarded in WriteChemObjectImpl until the last input mol. This
      // complication is needed to allow joined molecules to be from different
      // files. pOb1 in AddChem Object is zeroed at the end of a file and _jmol
      // is in danger of not being output.
      *_jmol += *ptmol;
      delete ptmol;
      return true;
    }
  ...
  ret = ret && (pConv->AddChemObject(ptmol) !=
                0); // success of both writing and reading
  return ret;
}

bool OBMoleculeFormat::WriteChemObjectImpl(OBConversion *pConv,
                                           OBFormat *pFormat) {
  if (pConv->IsOption("C", OBConversion::GENOPTIONS))
  ...
  if (pmol) {
    if (pmol->NumAtoms() == 0) {
      ...
      obErrorLog.ThrowError(__FUNCTION__, auditMsg, obInfo);
    }
    ret = true;
  ...
  // If sent a OBReaction* (rather than a OBMol*) output the consituent
  // molecules
...
/*! Instead of sending molecules for output via AddChemObject(), they are
  ...
  Is a static function with *this as parameter so that it can be called from
  other format classes like XMLMoleculeFormat which are not derived from
  OBMoleculeFormat.
*/
bool OBMoleculeFormat::DeferMolOutput(OBMol *pmol, OBConversion *pConv,
                                      OBFormat *pF) {
  ...
  if (*ptitle == 0)
    obErrorLog.ThrowError(__FUNCTION__, "Molecule with no title ignored",
                          obWarning);
  else {
    string title(ptitle);
...
/*! Makes a new OBMol on the heap by combining two molecules according to the
  rule below. If both have OBGenericData of the same type, or OBPairData with
  the same attribute,  the version from pFirst is used. Returns a pointer to a
  new OBMol which will need deleting by the calling program (probably by being
  sent to an output format). If the molecules cannot be regarded as being the
  same structure a NULL pointer is returned and an error message logged.
  ...
  - structures with atoms and bonds are compared by InChI; error if not the
  same.
...
OBMol *OBMoleculeFormat::MakeCombinedMolecule(OBMol *pFirst, OBMol *pSecond) {
  ...
  if (*pFirst->GetTitle() != 0)
    ...
    if (*pSecond->GetTitle() != 0)
      ...
      obErrorLog.ThrowError(__FUNCTION__, "Combined molecule has no title",
                            obWarning);
  }
  ...
  *pNewMol = *pMain; // Now copies all data
  ...
  for (igd = pOther->BeginData(); igd != pOther->EndData(); ++igd) {
    // copy only if not already data of the same type from molecule already
    // copied to pNewMol
...
bool OBMoleculeFormat::OutputDeferredMols(OBConversion *pConv) {
  ...
  for (itr = IMols.begin(); itr != IMols.end(); ++itr, ++i) {
    if (!(itr->second)
             ->DoTransformations(pConv->GetOptions(OBConversion::GENOPTIONS),
                                 pConv))
      continue;
...
///////////////////////////////////////////////////////////////////
bool OBMoleculeFormat::OutputMolsFromReaction(OBReaction *pReact,
                                              OBConversion *pConv,
                                              OBFormat *pFormat) {
  ...
  pConv->SetOutputIndex(pConv->GetOutputIndex() -
                        1); // The OBReaction object is not output
...
/** Attempts to read the index file datafilename.obindx successively
    ...
    - that in the environment variable BABEL_DATADIR or in the macro
BABEL_DATADIR if the environment variable is not set
    - in a subdirectory of the BABEL_DATADIR directory with the version of
OpenBabel as its name An index of type NameIndexType is then constructed.
NameIndexType is defined in obmolecformat.h as std::unordered_map. It is
searched by
    @code
    ...
    it using the format pInFormat, and written to the directory containing the
datafile. This means that this function can be used without worrying whether
there is an index. It will be slow to execute the first time, but subsequent
uses get the speed benefit of indexed access to the datafile.
  ...
  if (!indexstream) {
    ...
    if (!datastream) {
      obErrorLog.ThrowError(
          __FUNCTION__, datafilename + " was not found or could not be opened",
          obError);
      return false;
    }
    ...
    obErrorLog.ThrowError(__FUNCTION__, "Prepared an index for " + datafilepath,
                          obAuditMsg);
    // Save index to file
    ofstream dofs((datafilepath + ".obindx").c_str(),
