...
int main(int argc, char **argv) {
  ...
  ifsref.open(FileRef);
  if (!ifsref) {
    cerr << program_name << ": cannot read fixed molecule file: " << FileRef
         << endl;
    exit(-1);
  }
  ...
  if (refatoms.size() > molref.NumAtoms()) {
    cerr << program_name
         << ": The SMARTS pattern produces more matching atoms than are in the "
            "reference molecule"
         << endl;
    exit(-1);
  }
  ...
  ifsmv.open(FileMove);
  if (!ifsmv) {
    cerr << program_name << ": cannot read file: " << FileMove << endl;
    exit(-1);
  }
 ...
 *"[nh]1c2c(=O)n(C)c(=O)n(C)c2cc1" testref.sdf testmv.sdf
 *
 * \par AUTHORS
 *
