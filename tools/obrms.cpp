...
#undef _GLIBCXX_DEBUG /* unless you have boost libraries built with this, you  \
                         do _not_ want this defined */
...
public:
  AtomDistanceSorter(OBAtom *r) : ref(r->GetVector()) {}
    ...
    bool operator()(OBIsomorphismMapper::Mapping &map) override {
      ...
      if (minimize) {
        ...
        for (unsigned i = 0; i < N; i++) {
          ...
          tatom->SetVector(testcoord[3 * i] + rave[0],
                           testcoord[3 * i + 1] + rave[1],
                           testcoord[3 * i + 2] + rave[2]);
        }
      }
...
public:
  Matcher(OBMol &mol, int timeout) : ref(&mol) {
    ...
    mapper = std::shared_ptr<OBIsomorphismMapper>(
        OBIsomorphismMapper::GetInstance(query.get()));
    if (timeout > 0) {
...
static void processMol(OBMol &mol) {
  // isomorphismmapper wants isomorphic atoms to have the same aromatic and ring
  // state, but these proporties aren't reliable enough to be trusted in
  // evaluating molecules should be considered the same based solely on
  // connectivity
...
int main(int argc, char **argv) {
  ...
  const char *helpmsg =
      ...
      "\t -x,      --cross           compute all n^2 RMSDs between molecules "
      "of reference file\n"
      "\t -s,      --separate        separate reference file into constituent "
      "molecules and report best RMSD\n"
      "\t -t t,    --timeout t       give up on matching after specified "
      "number of seconds\n"
      "\t -h,      --help            help message\n";
  struct option long_options[] = {{"firstonly", no_argument, nullptr, 'f'},
  ...
  int option_index = 0;
  int c = 0;
  while ((c = getopt_long(argc, argv, "hfmxst:o:", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'o':
      fileOut = optarg;
  ...
  if (docross) {
    ...
    while (refconv.Read(&molref)) {
      ...
      while (testconv.Read(&moltest)) {
        ...
        cout << "RMSD " << molref.GetTitle() << ":" << moltest.GetTitle() << " "
             << bestRMSD << "\n";

        if (out) {
          outconv.Write(&moltest, &out);
        }
        if (!firstOnly) // one test molecule will be read for each reference
                        // molecule
          break;
      }
