...
int main(int argc, char **argv) {
  ...
  if (argc - index != 2 && argc - index != 1) {
    ...
    err += "   -t NUM  Print a molecule only if the PATTERN occurs NUM times "
           "inside the molecule\n";
    cerr << err << ends;
    exit(-1);
  } else {
 ...
 * \b -t \<NUM\>:
 *     Print a molecule only if the pattern occurs NUM times inside the
 *molecule\n\n \b -v: Invert the matching, print non-matching molecules \n\n
 *
 * \par EXAMPLES
