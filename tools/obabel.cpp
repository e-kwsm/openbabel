  ...
  for (arg = 1; arg < argc; ++arg) {
    if (argv[arg]) {
      if (argv[arg][0] == '-') {
          ...
          if (*nam != '\0') // Do nothing if name is empty
          {
            ...
            if (*nam == '-') {
              ...
              if (pAPI) {
                ...
                apiConv.AddOption(nam + 1, OBConversion::GENOPTIONS,
                                  txt.c_str());
        ...
        // since we know they take one parameter, and are the most likely
        // options to be misplaced
        case 'f':
        case 'l':
          p = argv[arg] + 2;
          if (!*p)
  ...
  if (!gotInType) {
    if (FileList.empty()) {
      cerr << "No input file or format spec or possibly a misplaced option.\n"
              "Most options must come after the input files. (-i -o -O -m can "
              "be anywhwere.)\n"
           << endl;
      usage();
      exit(1);
...
void DoOption(const char *p, OBConversion &Conv, OBConversion::Option_type typ,
  ...
  *ch = *p++;
  ...
  if (*p)
    txt =
        p; // use text immediately following the option letter, and keep looking
 ...
 *specified output format \n See examples below \n\n \b -d : Delete Hydrogens
 *\n\n \b -h : Add Hydrogens \n\n \b -p : Add Hydrogens appropriate for pH (use
 *transforms in phmodel.txt)  \n\n \b -t : All input files describe a single
 *molecule \n\n \b -f\<#\> : For multiple entries input, start import at
 *molecule # \n\n \b -l\<#\> : For multiple entries input, stop import at
 *molecule # \n\n \b -c : Center atomic coordinates at (0,0,0) \n\n \b
 *-s\<SMARTS\> : Convert only molecules matching the SMARTS pattern specified
 *\n\n \b -v\<SMARTS\> : Convert only molecules \b NOT matching SMARTS pattern
 *specified \n\n
 *
 * \par FILE FORMATS
 *
