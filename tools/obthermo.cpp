...
int main(int argc, char **argv) {
  ...
  if (argc < 2) {
    ...
    cout << endl;
    cout << "  --nrot N    number of rotatable bonds for conformational entropy"
         << endl;
    cout << endl;
    cout << "  --dbdt x    temperature derivative of second virial coefficient "
            "for cp calculation"
         << endl;
    cout << endl;
    cout << "  --kj        output kJ/mol related units (default kcal/mol)"
         << endl;
    cout << endl;
    exit(-1);
  } else {
    ...
    for (i = 1; i < argc;) {
      ...
      if ((option == "--symm") && (argc > (i + 1))) {
        Nsymm = atoi(argv[i + 1]);
        if (Nsymm < 1) {
          cerr << program_name << ": the symmetry number should be >= 1!"
               << endl;
          exit(-1);
        }
        i += 2;
        ...
        if (Nrot < 0) {
          cerr << program_name
               << ": the number of rotatable bonds should be >= 0!" << endl;
          exit(-1);
        }
        i += 2;
        ...
        if (dBdT < 0) {
          cerr << program_name
               << ": the derivative of the second virial coefficient with "
                  "respect to temperature should be >= 0!"
               << endl;
          exit(-1);
        }
        i += 2;
  ...
  if (!format_in || !conv.SetInFormat(format_in)) {
    cerr << program_name << ": cannot read input format in file \"" << filename
         << "\"" << endl;
    exit(-1);
  }
  ...
  if ((conv.Read(&mol, &ifs)) && !mol.Empty()) {
    ...
    printf("obthermo - extract thermochemistry data from quantum chemistry "
           "logfiles\n");
    printf("Number of rotatable bonds: %d\n", Nrot);
    if (dBdT == 0) {
      printf("Please supply --dbdt option to get reliable heat capacity at "
             "constant pressure.\n");
    }
    bool bVerbose = true;
    if (extract_thermochemistry(mol, bVerbose, &Nsymm, Nrot, dBdT, &temperature,
                                &DeltaHf0, &DeltaHfT, &DeltaGfT, &DeltaSfT,
                                &S0T, &CVT, &CPT, Scomponents, &ZPVE)) {
      double Rgas = 1.9872041; // cal/mol K
      printf("DeltaHform(0K)  %10g  %s\n", DeltaHf0 * unit_factor,
             e_unit.c_str());
      printf("Temperature     %10g  K\n", temperature);
      printf("DeltaHform(T)   %10g  %s\n", DeltaHfT * unit_factor,
             e_unit.c_str());
      printf("DeltaGform(T)   %10g  %s\n", DeltaGfT * unit_factor,
             e_unit.c_str());
      printf("DeltaSform(T)   %10g  %s\n", DeltaSfT * unit_factor,
             s_unit.c_str());
      printf("cv(T)           %10g  %s\n", CVT * unit_factor, s_unit.c_str());
      printf("cp(T)           %10g  %s\n", CPT * unit_factor, s_unit.c_str());
      printf("Strans(T)       %10g  %s\n", Scomponents[0] * unit_factor,
             s_unit.c_str());
      printf("Srot(T)         %10g  %s\n", Scomponents[1] * unit_factor,
             s_unit.c_str());
      printf("Svib(T)         %10g  %s\n", Scomponents[2] * unit_factor,
             s_unit.c_str());
      if (Scomponents[3] != 0) {
        printf("Ssymm           %10g  %s\n", Scomponents[3] * unit_factor,
               s_unit.c_str());
      }
      if (Scomponents[4] != 0) {
        printf("Sconf           %10g  %s\n", Scomponents[4] * unit_factor,
               s_unit.c_str());
      }
      printf("S0(T)           %10g  %s\n", S0T * unit_factor, s_unit.c_str());
    } else {
      printf("Could not find all necessary information to determine "
 ...
 * \par DESCRIPTION
 *
 * The obthermo tool can be used to extract the thermochemistry, e.g. Delta H
 *formation Delta G formation and the standard entropy from e.g. a Gaussian
 *calculation.
 *
 * \par OPTIONS
 *
 ...
 *
 * \b --nrot \<N\>:
 *     Set the number of rotatable bond manually and correct results if
 *needed\n\n
 *
 * \b --dbdt \<x\>:
 *     Set the derivative of the second virial coefficient with temperature for
 *cp calculation\n\n
 *
 * \b --kj:
