...
void mtest_configStruct() {
  // reference Config
  OBTetrahedralStereo::Config reference(0, 1, OBStereo::MakeRefs(2, 3, 4),
                                        OBStereo::Clockwise,
                                        OBStereo::ViewFrom);
  ...
  OBTetrahedralStereo::Config invalidCenter(45, 1, OBStereo::MakeRefs(2, 3, 4),
                                            OBStereo::Clockwise,
                                            OBStereo::ViewFrom);
  OB_ASSERT(reference != invalidCenter);

  // invalid from/towards id
  OBTetrahedralStereo::Config invalidFrom(0, 45, OBStereo::MakeRefs(2, 3, 4),
                                          OBStereo::Clockwise,
                                          OBStereo::ViewFrom);
  OB_ASSERT(reference != invalidFrom);

  // test other refs
  OBTetrahedralStereo::Config cfg1(0, 1, OBStereo::MakeRefs(2, 4, 3),
                                   OBStereo::Clockwise, OBStereo::ViewFrom);
  OB_ASSERT(reference != cfg1);

  // test anti-clockwise
  OBTetrahedralStereo::Config cfg2(0, 1, OBStereo::MakeRefs(2, 3, 4),
                                   OBStereo::AntiClockwise, OBStereo::ViewFrom);
  OB_ASSERT(reference != cfg2);

  // test viewing towards
  OBTetrahedralStereo::Config cfg3(0, 1, OBStereo::MakeRefs(2, 3, 4),
                                   OBStereo::Clockwise, OBStereo::ViewTowards);
  OB_ASSERT(reference != cfg3);

  // test anti-clockwise + viewing towards
  OBTetrahedralStereo::Config cfg4(0, 1, OBStereo::MakeRefs(2, 3, 4),
                                   OBStereo::AntiClockwise,
                                   OBStereo::ViewTowards);
