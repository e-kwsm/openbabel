...
void testTetrahedralStereo1() {
  ...
  OB_REQUIRE(((OBStereoBase *)stereoData[0])->GetType() ==
  ...
  // 0 1 2  3 4  <- ids
  //
  OBTetrahedralStereo::Config cfg(1, 0, OBStereo::MakeRefs(4, 3, 2),
                                  OBStereo::Clockwise);
...
void genericSmilesCanonicalTest(const std::string &smiles) {
  ...
  for (std::vector<OBGenericData *>::iterator data = stereoData.begin();
    ...
    if (((OBStereoBase *)*data)->GetType() == OBStereo::Tetrahedral) {
      ...
      if (mol.GetAtomById(config.center))
        config.center =
            canlbls.at(mol.GetAtomById(config.center)->GetIdx() - 1);
      ...
      if (mol.GetAtomById(config.refs[0]))
        config.refs[0] =
            canlbls.at(mol.GetAtomById(config.refs[0])->GetIdx() - 1);
      if (mol.GetAtomById(config.refs[1]))
        config.refs[1] =
            canlbls.at(mol.GetAtomById(config.refs[1])->GetIdx() - 1);
      if (mol.GetAtomById(config.refs[2]))
        config.refs[2] =
            canlbls.at(mol.GetAtomById(config.refs[2])->GetIdx() - 1);
      cout << "Config with symmetry ids: " << config << endl;
      tetrahedral1.push_back(config);
    } else if (((OBStereoBase *)*data)->GetType() == OBStereo::CisTrans) {
      // convert to tetrahedral data
      ...
      if (mol.GetAtomById(config.refs[3]))
        config.refs[3] =
            canlbls.at(mol.GetAtomById(config.refs[3])->GetIdx() - 1);
      cout << "Config with symmetry ids: " << config << endl;
      cistrans1.push_back(config);
    } else if (((OBStereoBase *)*data)->GetType() == OBStereo::SquarePlanar) {
      // convert to tetrahedral data
  ...
  for (unsigned int i = 0; i < cistrans1.size(); ++i) {
    for (unsigned int j = 0; j < cistrans2.size(); ++j) {
      if ((cistrans1[i].begin == cistrans2[j].begin) &&
          (cistrans1[i].end == cistrans2[j].end))
        OB_ASSERT(cistrans1[i] == cistrans2[j]);
      if ((cistrans1[i].begin == cistrans2[j].end) &&
          (cistrans1[i].end == cistrans2[j].begin))
        OB_ASSERT(cistrans1[i] == cistrans2[j]);
    }
  }
...
// Define location of file formats for testing
  ...
  case 3:
    ...
    genericSmilesCanonicalTest("CCC[C@@H]1C[C@H](N(C1)C)C(=O)NC([C@@H]2[C@@H](["
                               "C@@H]([C@H]([C@H](O2)SC)OP(=O)(O)O)O)O)C(C)Cl");
    break;
    // this structure fails but this is an error in genericSmilesCanonicalTest
    // (It should sort the centers). However, this is extensivily tested in
    // other tests.
