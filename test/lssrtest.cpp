...
public:
  struct Size_Count {
    Size_Count(int _ringSize, int _ringCount)
        : ringSize(_ringSize), ringCount(_ringCount) {}
    int ringSize;
    int ringCount;
  };
  ...
  LSSR(const Size_Count &g1) { size_count.push_back(g1); }
...
// Define location of file formats for testing
  ...
  case 5:
    // 4x 3-ring, 1x 5-ring
    OB_ASSERT(verifyLSSR("rings/tetrahedron.mdl",
                         LSSR(LSSR::Size_Count(3, 4), LSSR::Size_Count(5, 1))));
    // 6x 4-ring, 1x 6-ring
    OB_ASSERT(verifyLSSR("rings/cubane.mdl",
                         LSSR(LSSR::Size_Count(4, 6), LSSR::Size_Count(6, 1))));
    OB_ASSERT(verifyLSSR("rings/cubane2.mdl",
                         LSSR(LSSR::Size_Count(4, 6), LSSR::Size_Count(6, 1))));
    // 8x 3-ring
    ...
    // 12x 5-ring
    OB_ASSERT(
        verifyLSSR("rings/fullerene20.mdl", LSSR(LSSR::Size_Count(5, 12))));
    // 12x 5-ring, 20x 6-ring
    OB_ASSERT(
        verifyLSSR("rings/fullerene60.mdl",
                   LSSR(LSSR::Size_Count(5, 12), LSSR::Size_Count(6, 20))));
    break;
  default:
