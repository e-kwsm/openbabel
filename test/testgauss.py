...
def get_mol_dict(
    ...
    molecule_dict["molecule"].update({
        "title": title,
        "mol_weight": mol_weight,
        "numb_atoms": numb_atoms,
        "formula": formula,
        "charge": charge,
        "multiplicity": None
    })
    ...
    if forcefield:
        ...
        if not ff.Setup(obmol):
            print("Could not setup the force field %s for %s" %
                  (forcefield, filename))
        else:
            if not ff.GetAtomTypes(obmol):
                print("Could not get atomtypes from force field %s for %s" %
                      (forcefield, filename))
...
def run_one(
    ...
    for ai in range(1, 1 + moldict["molecule"]["numb_atoms"]):
        ...
        if ai in moldict["atoms"]:
            ...
            if debug:
                print("No atom %d in %s with %d atoms" %
                      (ai, molname, moldict["molecule"]["numb_atoms"]))
        atypes.append(atype)
    btypes = []
    for bb in moldict["bonds"]:
        thisbond = ("%d-%d:%d" % (bb[0], bb[1], moldict["bonds"][bb]))
        btypes.append(thisbond)
    return moldict["molecule"]["charge"], moldict["molecule"][
...
class TestGauss(BaseTest):
        ...
        for a in actual:
            ...
            if aaa[0] in refhash:
                ...
                if aaa[1] != refhash[aaa[0]]:
                    comp = (" ref %s:%s actual %s" %
                            (aaa[0], refhash[aaa[0]], a))
            else:
                bbb = aaa[0].split("-")
    ...
    def compare_types(
        ...
        if len(reference) != len(actual):
            ...
            print("%s: number of %s in reference %d, actual %d%s" %
                  (molname, ttype, len(reference), len(actual), extra))
            return
    ...
    def compare_sdf_log(self, filedir: str, forcefield: str,
                        verbose: bool) -> bool:
        sdfs = filedir + "/*.sdf"
        ...
        for mol in mol_list:
            ...
            if not failed:
                different = (qtot[filetypes[0]] != qtot[filetypes[1]]
                             or formula[filetypes[0]] != formula[filetypes[1]])

                # Now compare atom types
                ref = ""
                for at in atypes[filetypes[0]]:
                    ref += (" %s" % at)
                if not self.compare_types(mol, forcefield + "-" + "-atoms",
                                          ref, atypes[filetypes[1]], verbose):
                    summary["atoms"] += 1
                    different = True
                ...
                for bt in btypes[filetypes[0]]:
                    ref += (" %s" % bt)
                if not self.compare_types(mol, forcefield + "-" + "-bonds",
                                          ref, btypes[filetypes[1]], verbose):
                    summary["bonds"] += 1
                    different = True
            ...
            for ab in summary.keys():
                if debug:
                    print("%d error(s) in %s-%s" %
                          (summary[ab], forcefield, ab))
