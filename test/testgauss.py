    ...
    if forcefield:
        ...
        if not ff.Setup(obmol):
            ...
            if not ff.GetAtomTypes(obmol):
                print(
                    "Could not get atomtypes from force field %s for %s"
                    % (forcefield, filename)
                )
...
class TestGauss(BaseTest):
    ...
    def compare_types(self, molname, ttype, references, actual, verbose):
        ...
        if len(reference) != len(actual):
            ...
            print(
                "%s: number of %s in reference %d, actual %d%s"
        ...
        for mol in mol_list:
            ...
            for filetype in filetypes:
                ...
                (
                    ...
                    atypes[filetype],
                    btypes[filetype],
                ) = run_one(filename, forcefield, filetype)
                if (
                    qtot[filetype] == None
                    or formula[filetype] == None
            ...
            if not failed:
                different = (
                    qtot[filetypes[0]] != qtot[filetypes[1]]
                ...
                for at in atypes[filetypes[0]]:
                    ref += " %s" % at
                if not self.compare_types(
                    mol, forcefield + "-" + "-atoms", ref, atypes[filetypes[1]], verbose
                ):
                    summary["atoms"] += 1
                    different = True
                ...
                for bt in btypes[filetypes[0]]:
                    ref += " %s" % bt
                if not self.compare_types(
                    mol, forcefield + "-" + "-bonds", ref, btypes[filetypes[1]], verbose
                ):
                    summary["bonds"] += 1
                    different = True
