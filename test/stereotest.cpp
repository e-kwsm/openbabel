...
bool doStereoPerception2(OBMol &mol, int numTetrahedral, int numCisTrans) {
  ...
  std::vector<OpenBabel::OBStereoUnit> units =
      FindStereogenicUnits(&mol, symmetry_classes, G);
...
OBStereoUnitSet Units(const OBStereoUnit &u1, const OBStereoUnit &u2) {
  OBStereoUnitSet units = Units(u1);
  units.push_back(u2);
  return units;
}
OBStereoUnitSet Units(const OBStereoUnit &u1, const OBStereoUnit &u2,
                      const OBStereoUnit &u3) {
  OBStereoUnitSet units = Units(u1, u2);
  units.push_back(u3);
  return units;
}
OBStereoUnitSet Units(const OBStereoUnit &u1, const OBStereoUnit &u2,
                      const OBStereoUnit &u3, const OBStereoUnit &u4) {
  OBStereoUnitSet units = Units(u1, u2, u3);
  units.push_back(u4);
  return units;
}
OBStereoUnitSet Units(const OBStereoUnit &u1, const OBStereoUnit &u2,
                      const OBStereoUnit &u3, const OBStereoUnit &u4,
                      const OBStereoUnit &u5) {
  OBStereoUnitSet units = Units(u1, u2, u3, u4);
  units.push_back(u5);
  return units;
}
...
std::vector<std::vector<OpenBabel::OBStereoUnit>>
DepUnits(const OBStereoUnitSet &u1) {
  std::vector<std::vector<OpenBabel::OBStereoUnit>> sets;
  sets.push_back(u1);
  return sets;
}
std::vector<std::vector<OpenBabel::OBStereoUnit>>
DepUnits(const OBStereoUnitSet &u1, const OBStereoUnitSet &u2) {
  std::vector<std::vector<OpenBabel::OBStereoUnit>> sets = DepUnits(u1);
  sets.push_back(u2);
  return sets;
}
std::vector<std::vector<OpenBabel::OBStereoUnit>>
DepUnits(const OBStereoUnitSet &u1, const OBStereoUnitSet &u2,
         const OBStereoUnitSet &u3) {
  std::vector<std::vector<OpenBabel::OBStereoUnit>> sets = DepUnits(u1, u2);
  sets.push_back(u3);
  return sets;
}

bool doStereoPerception3(
    OBMol &mol, const OBStereoUnitSet &refUnits = OBStereoUnitSet(),
    const OBStereoUnitSetOfSets refInterdependent = OBStereoUnitSetOfSets()) {
...
void test_StereoPerception() {
  ...
  OB_ASSERT( conv.ReadFile(&mol, GetFilename("stereo/break1.mol")) );
    ...
    std::vector<unsigned int> symmetry_classes;
    OpenBabel::OBGraphSym graphsym(&mol);
    cout << "nclasses = " << graphsym.GetSymmetry(symmetry_classes, false) <<
  endl;
