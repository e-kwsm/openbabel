...
int main(int argc, char *argv[]) {
  if (argc != 3) {
    cout << "Usage:  inchiwrite Testmols.xxx Results.txt\n"
            "Testmols.xxx can be any format but must have the appropriate "
            "extension.\n"
            "Results.txt is a text file with the correct the correct InChIs.\n"
            "It can contain other text interspersed."
         << endl;
    return 1;
  }
...
// Define location of file formats for testing
  ...
  /*In OB version 2.2.0 a set of 'recommended' InChI options is used by default.
    The compiled Windows program wInChI.exe, which is used to generate the
    results file was not available for InChI version 1.02, the beta of which is
    used in OB. So the following option turns off all the InChI options. This
    needs to be revisted when InChI 1.02 is released.
  */
  ...
  for (n = 1; ssout; ++n) {
    ...
    if (generated != correct) {
      cout << "Not ok " << n << " # Mismatch in molecule #" << n
           << ". generated / correct\n"
           << "# " << generated << "\n# " << correct << '\n';
      nfail++;
    } else
