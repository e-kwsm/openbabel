...
void testOBRotorGetSet()
{
  ...
  // SetBond/GetBond
  rotor.SetBond(&bond);
  EXPECT_TRUE(rotor.GetBond()->GetIdx() == bond.GetIdx());
  // SetIdx/GetIdx
  rotor.SetIdx(45);
  EXPECT_TRUE(rotor.GetIdx() == 45);
  // SetDihedralAtoms/GetDihedralAtoms
  int ref_ptr[4] = {1, 2, 3, 4};
  rotor.SetDihedralAtoms(ref_ptr);
  rotor.GetDihedralAtoms(ref_ptr);
  EXPECT_TRUE(ref_ptr[0] == 1);
  EXPECT_TRUE(ref_ptr[1] == 2);
  EXPECT_TRUE(ref_ptr[2] == 3);
  EXPECT_TRUE(ref_ptr[3] == 4);
  std::vector<int> ref_vector = rotor.GetDihedralAtoms();
  EXPECT_TRUE(ref_vector[0] == 1);
  EXPECT_TRUE(ref_vector[1] == 2);
  EXPECT_TRUE(ref_vector[2] == 3);
  EXPECT_TRUE(ref_vector[3] == 4);
  ...
  EXPECT_TRUE(rotor.GetTorsionValues().size() == 2);
  EXPECT_TRUE(rotor.Size() == 2);
}
...
void testOBRotorSetToAngle()
{
  ...
  EXPECT_TRUE(IsNear_mod(fabs(RAD_TO_DEG * rotor.CalcTorsion(mol->GetCoordinates())), 180.0, 360.0, 1.0));

  // rotate
  rotor.SetToAngle(mol->GetCoordinates(), 60.0 * DEG_TO_RAD);

  EXPECT_TRUE(fabs(RAD_TO_DEG * rotor.CalcTorsion(mol->GetCoordinates()) - 60.0) < 1.0);
}
...
void testOBRotorSetRotor()
{
  ...
  EXPECT_TRUE(fabs(RAD_TO_DEG * rotor.CalcTorsion(mol->GetCoordinates()) - 0.0) < 1.0);
  // rotate to 3.1415 radians
  rotor.SetRotor(mol->GetCoordinates(), 1);
  EXPECT_TRUE(IsNear_mod(RAD_TO_DEG * rotor.CalcTorsion(mol->GetCoordinates()), 180.0, 360.0, 1.0));
   // rotate to 0.0 radians
...
void testOBRotorListFixedBonds()
{
  ...
  rlist1.SetFixedBonds(fixedBonds);
  rlist1.Setup(*mol);
  EXPECT_TRUE(rlist1.Size() == 2);

  // test with bond 1, 2, 3, 5 fixed
  fixedBonds.SetBitOn(2);
  rlist1.SetFixedBonds(fixedBonds);
  rlist1.Setup(*mol);
  EXPECT_TRUE(rlist1.Size() == 1);
