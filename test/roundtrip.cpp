...
int main(int argc, char *argv[]) {
  ...
  while (inFileStream1.peek() != EOF && inFileStream1.good() &&
    ...
    if (p1 && strncasecmp(p1 + 1, "CML", 3) != 0 && mol.NumAtoms() == 0) {
      cout << " ** ERROR ** molecule " << molCount
           << " in file #1 has no atoms!" << endl;
      return (-1);
    }

    if (p1 && strncasecmp(p1 + 1, "CML", 3) != 0 && mol2.NumAtoms() == 0) {
      cout << " ** ERROR ** molecule " << molCount
    ...
    if ((p1 && strncasecmp(p1 + 1, "SMI", 3) == 0) ||
      ...
      if (mol.NumAtoms() != mol2.NumAtoms()) {
        cout << " ** ERROR ** Number of atoms differ: " << mol.NumAtoms()
             << " and " << mol2.NumAtoms() << " in molecule " << molCount
             << endl;
        return (-1);
      }
    }
    ...
    for (unsigned int i = 1; i <= mol.NumAtoms(); i++) {
      ...
      if (atom1->GetAtomicNum() != atom2->GetAtomicNum()) {
        cout << " ** ERROR ** Elements for atom " << i
             << " differ: " << atom1->GetAtomicNum() << " and "
             << atom2->GetAtomicNum() << " in molecule " << molCount << endl;
        return (-1);
      }
  ...
  mol.Clear();
  mol2.Clear();
  if (inFileStream1.good() && inFileStream1.peek() != EOF && conv1.Read(&mol)) {
    if (mol.NumAtoms() > 0) {
      cout << " ** ERROR **  File 1 has more molecules! " << endl;
      cout << "   Already read " << molCount << " molecules from both" << endl;
      cout << "   New molecule " << mol.GetTitle() << " has " << mol.NumAtoms()
           << " atoms " << endl;
      exit(-1);
    }
    ...
    if (mol2.NumAtoms() > 0) {
      ...
      cout << "   Already read " << molCount << " molecules from both" << endl;
      cout << "   New molecule " << mol.GetTitle() << " has " << mol2.NumAtoms()
           << " atoms " << endl;
      exit(-1);
    }
  }
