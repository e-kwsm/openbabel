...
void report_error(const char *msg, const char *file, int line,
                  const char *func_name, bool require = false);

template <typename T1, typename T2>
void ob_compare(T1 a, T2 b, const char *expr, const char *file, int line,
                const char *func_name) {
  if (!(a == b))
    std::cout << file << ":" << line << ": " << expr << " [" << a << " == " << b
              << "] (FAIL)" << std::endl;
}

#define OB_ASSERT(exp)                                                         \
  ((exp) ? static_cast<void>(0)                                                \
         : report_error(#exp, __FILE__, __LINE__, FUNCTION_SIGNATURE, false))

#define OB_REQUIRE(exp)                                                        \
  ((exp) ? static_cast<void>(0)                                                \
         : report_error(#exp, __FILE__, __LINE__, FUNCTION_SIGNATURE, true))
