...
void NegativeTestCases(int &molCount, unsigned int &testCount) {
  ...
  const char *smiles[] = {
      ...
      "N1S[SH+]C=C1", "S1C=[NH+]=[NH+]=C1",
      "C1(N23)=CC=CC2=CC=CC3=CC=C1", // pyrido[2,1,6-de]quinolizine - no atom is
                                     // Daylight aromatic
      nullptr};
  OBMol mol;
  OBConversion conv;
  ...
  for (int i = 0; smiles[i]; ++i) {
    ...
    if (found_non_aromatic)
      ...
      cout << "not ok " << ++testCount
           << " # all atoms are aromatic in molecule " << molCount << " "
           << mol.GetTitle() << "\n";
  }
}
      ...
      for (int N = 0; N < 2; ++N) {
        ...
        FOR_ATOMS_OF_MOL(atom, mol) {
          ...
          if (atom->IsAromatic())
            ...
            cout << "# atom idx " << atom->GetIdx() << " in molecule "
                 << molCount << " " << mol.GetTitle() << "\n";
          }
        }
      }
