...
void GenerateEnergies(string molecules_file, string results_file, string method,
                      double epsilon = 1.0) {
  std::ifstream ifs;
...
void TestFile(string filename, string results_file, string method,
              double epsilon = 1.0) {
  std::ifstream mifs;
...
int ffmmff94(int argc, char *argv[]) {
  ...
  if (choice == 99) {
    GenerateEnergies(testdatadir + "forcefield.sdf",
                     testdatadir + "mmff94results.txt", "MMFF94");
    GenerateEnergies(testdatadir + "more-mmff94.sdf",
                     testdatadir + "more-mmff94results.txt",
                     "MMFF94"); // provided by Paolo Tosco
    GenerateEnergies(testdatadir + "forcefield.sdf",
                     testdatadir + "mmff94sresults.txt", "MMFF94s");
    GenerateEnergies(testdatadir + "more-mmff94.sdf",
                     testdatadir + "more-mmff94sresults.txt",
                     "MMFF94s"); // ditto
    GenerateEnergies(testdatadir + "forcefield.sdf",
                     testdatadir + "mmff94e4results.txt", "MMFF94", 4.0);
    GenerateEnergies(testdatadir + "more-mmff94.sdf",
                     testdatadir + "more-mmff94e4results.txt", "MMFF94",
                     4.0); // provided by Paolo Tosco
...
// Define location of file formats for testing
  ...
  case 1:
    ...
    break;
  case 2:
    TestFile(testdatadir + "more-mmff94.sdf",
             testdatadir + "more-mmff94results.txt",
             "MMFF94"); // provided by Paolo Tosco
    break;
  case 3:
    TestFile(testdatadir + "forcefield.sdf", testdatadir + "mmff94sresults.txt",
             "MMFF94s");
    break;
  case 4:
    TestFile(testdatadir + "more-mmff94.sdf",
             testdatadir + "more-mmff94sresults.txt", "MMFF94s"); // ditto
    break;
  case 5:
    TestFile(testdatadir + "forcefield.sdf",
             testdatadir + "mmff94e4results.txt", "MMFF94", 4.0);
    break;
  case 6:
    TestFile(testdatadir + "more-mmff94.sdf",
             testdatadir + "more-mmff94e4sresults.txt", "MMFF94", 4.0);
    break;
  default:
