...
void addh_check(OBMol *mol) {
  ...
  FOR_ATOMS_OF_MOL(atom, mol) {
    ...
    int maxval = max(OBElements::GetMaxBonds(atom->GetAtomicNum()),
                     vcnts[atom->GetIndex()]);
    if (total_valence > maxval) {
      cerr << "Hydrogens added when already greater than MaxBonds "
           << total_valence << " vs "
  ...
  FOR_BONDS_OF_MOL(bond, mol) {
    if (bond->GetLength() > 5) {
      cerr << "Bond length between " << bond->GetBeginAtom()->GetId() << " and "
           << bond->GetEndAtom()->GetId()
           << " is too long: " << bond->GetLength() << "\n";
      // exit(-1);
    }
  }
}
