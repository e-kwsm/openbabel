...
void test_configStruct() {
  // reference Config
  OBCisTransStereo::Config reference(0, 1, OBStereo::MakeRefs(2, 3, 4, 5),
                                     OBStereo::ShapeU);
  ...
  // invalid begin id
  OBCisTransStereo::Config invalidBegin(45, 1, OBStereo::MakeRefs(2, 3, 4, 5),
                                        OBStereo::ShapeU);
  OB_ASSERT(reference != invalidBegin);

  // invalid end id
  OBCisTransStereo::Config invalidEnd(0, 45, OBStereo::MakeRefs(2, 3, 4, 5),
                                      OBStereo::ShapeU);
  OB_ASSERT(reference != invalidEnd);

  // test other refs
  OBCisTransStereo::Config cfg1(0, 1, OBStereo::MakeRefs(2, 4, 3, 5),
                                OBStereo::ShapeU);
  OB_ASSERT(reference != cfg1);

  // test Z shape == U shape
  OBCisTransStereo::Config cfg2(0, 1, OBStereo::MakeRefs(3, 2, 4, 5),
                                OBStereo::ShapeZ);
  OB_ASSERT(reference == cfg2);

  // test 4 shape == U shape
  OBCisTransStereo::Config cfg3(0, 1, OBStereo::MakeRefs(2, 4, 3, 5),
                                OBStereo::Shape4);
...
void test_CisTrans1() {
  ...
  OBCisTransStereo ct(&mol);
  ct.SetConfig(OBCisTransStereo::Config(
      1, 2,
      OBStereo::MakeRefs(0, OBStereo::ImplicitRef, OBStereo::ImplicitRef, 3)));
...
void test_CisTrans2() {
  ...
  OBCisTransStereo::Config cfg(
      1, 2,
      OBStereo::MakeRefs(0, OBStereo::ImplicitRef, 3, OBStereo::ImplicitRef));
  ct.SetConfig(cfg);
