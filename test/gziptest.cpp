...
void checkResults(const string &file, const vector<string> &correctResults) {
  ...
  while (conv.Read(&mol)) {
    ...
    if (conv.GetInFormat() == conv.FindFormat("mol2")) {
      // Don't test roundtripping for mol2 as the atom types are changed by the
      ...
      strconv.SetInAndOutFormats(conv.GetInFormat(), conv.FindFormat("CAN"),
                                 conv.GetInGzipped());
  ...
  if (results.size() != correctResults.size()) {
    cout << "Incorrect number of outputs (" << results.size() << " vs "
         << correctResults.size() << ") for " << file << "\n";
    exit(-1);
  }
  for (unsigned i = 0, n = results.size(); i < n; i++) {
    if (results[i] != correctResults[i]) {
      cout << file << " match failure: " << results[i]
           << " != " << correctResults[i] << "\n";
      exit(-1);
    }
  }
