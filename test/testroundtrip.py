...
def molsAreSame(a: "pybel.Molecule", b: "pybel.Molecule") -> Optional[str]:
    ...
    for (ai, bi) in m:
        ...
        if aatom.GetAtomicNum() != batom.GetAtomicNum():
            return "Mismatched atom num, %d != %d" % (aatom.GetAtomicNum(),
                                                      batom.GetAtomicNum())
        if aatom.GetHyb() != batom.GetHyb():
            return "Mismatched Hyb, %d != %d" % (aatom.GetHyb(),
                                                 batom.GetHyb())
        if aatom.GetExplicitValence() != batom.GetExplicitValence():
            return "Mismatched ExplicitValence, %d != %d" % (
                aatom.GetExplicitValence(), batom.GetExplicitValence())
        if aatom.IsAromatic() != batom.IsAromatic():
            return "Mismatched IsAromatic, %d != %d" % (aatom.IsAromatic(),
                                                        batom.IsAromatic())
        if aatom.GetFormalCharge() != batom.GetFormalCharge():
            return "Mismatched GetFormalCharge, %d != %d" % (
                aatom.GetFormalCharge(), batom.GetFormalCharge())
...
class TestSuite(unittest.TestCase):
    ...
    def testRoundtrip(self):
        ...
        for (i, fname) in enumerate(glob.glob(os.path.join(root, '*.sdf'))):
            with self.subTest():
                try:
                    result = processes_pool.apply_async(roundtripFile,
                                                        args=(fname, ))
                    ret = result.get(
                        timeout=10)  # surely 10 seconds is long enough
                    if ret is not None:
                        print(i, fname)
                        print(ret)
