...
def molsAreSame(a, b):
    ...
    for ai, bi in m:
        ...
        if aatom.GetAtomicNum() != batom.GetAtomicNum():
            return "Mismatched atom num, %d != %d" % (
                aatom.GetAtomicNum(),
                batom.GetAtomicNum(),
            )
        ...
        if aatom.GetExplicitValence() != batom.GetExplicitValence():
            return "Mismatched ExplicitValence, %d != %d" % (
                aatom.GetExplicitValence(),
                batom.GetExplicitValence(),
            )
        if aatom.IsAromatic() != batom.IsAromatic():
            return "Mismatched IsAromatic, %d != %d" % (
                aatom.IsAromatic(),
                batom.IsAromatic(),
            )
        if aatom.GetFormalCharge() != batom.GetFormalCharge():
            return "Mismatched GetFormalCharge, %d != %d" % (
                aatom.GetFormalCharge(),
                batom.GetFormalCharge(),
            )
...
def dumpBoth(a, b):
    ...
    for a, b in zip(a.atoms, b.atoms):
        ...
        B = b.OBAtom
        print(
            "%d:%d  %d:%d  %d:%d  %d:%d  %d:%d  (%f,%f,%f):(%f,%f,%f)"
            % (
                A.GetIdx(),
                B.GetIdx(),
                A.GetAtomicNum(),
...
class TestSuite(unittest.TestCase):
    ...
    def canFindFile(self, filename):
        self.assertTrue(
            os.path.exists(filename), "Cannot find the file '%s'" % filename
        )
