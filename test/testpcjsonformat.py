...
class TestPcJsonFormat(PybelWrapper):
    ...
    def test_read_sid(self):
        ...
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, "SID_348356775_2D.json"))
        )
        self.assertEqual(len(mols), 1)
    ...
    def test_read_atoms(self):
        ...
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, "CID_6857552_2D.json"))
        )
        self.assertEqual(len(mols), 1)
        self.assertEqual(len(mols[0].atoms), 14)
        self.assertEqual(
            [a.atomicnum for a in mols[0].atoms],
            [8, 8, 8, 7, 6, 6, 6, 1, 1, 1, 1, 1, 1, 1],
        )
        self.assertEqual(
            [a.formalcharge for a in mols[0].atoms],
    ...
    def test_read_bonds(self):
        ...
        self.assertEqual(
            [mols[0].OBMol.GetBond(i).GetBondOrder() for i in range(0, 13)],
    ...
    def test_write_atoms(self):
        ...
        self.assertEqual(
            output["PC_Compounds"][0]["atoms"]["element"],
            [8, 8, 8, 7, 6, 6, 6, 1, 1, 1, 1, 1, 1, 1],
        )
        self.assertEqual(
            output["PC_Compounds"][0]["atoms"]["charge"][0], {"aid": 2, "value": -1}
        )
        self.assertEqual(
            output["PC_Compounds"][0]["atoms"]["charge"][1], {"aid": 4, "value": 1}
        )
    ...
    def test_write_complex_bonds(self):
        ...
        mols = list(
            pybel.readfile(
                "pcjson", os.path.join(filedir, "CID_368849_2D.json"), opt={"s": None}
            )
        )
    ...
    def test_write_stereo_cistrans(self):
        ...
        # Cis
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, "CID_643833_2D.json"))
        )
        output = json.loads(mols[0].write("pcjson"))
        self.assertEqual(
            output["PC_Compounds"][0]["stereo"],
        ...
        # Trans
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, "CID_638186_2D.json"))
        )
        output = json.loads(mols[0].write("pcjson"))
        self.assertEqual(
            output["PC_Compounds"][0]["stereo"],
        ...
        # Any
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, "CID_10900_2D.json"))
        )
        output = json.loads(mols[0].write("pcjson"))
