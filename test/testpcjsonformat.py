...
class TestPcJsonFormat(PybelWrapper):
    ...
    def test_read_empty(self):
        ...
        mols = list(
            pybel.readfile("pcjson", os.path.join(filedir, 'empty.json')))
        self.assertEqual(mols, [])
    ...
    def test_read_atoms(self):
        ...
        self.assertEqual([a.atomicnum for a in mols[0].atoms],
                         [8, 8, 8, 7, 6, 6, 6, 1, 1, 1, 1, 1, 1, 1])
        self.assertEqual([a.formalcharge for a in mols[0].atoms],
    ...
    def test_write_atoms(self):
        ...
        mols = list(
            pybel.readfile("pcjson",
                           os.path.join(filedir, 'CID_6857552_2D.json')))
        output = json.loads(mols[0].write('pcjson'))
        self.assertEqual(output['PC_Compounds'][0]['atoms']['aids'],
                         list(range(1, 15)))
        self.assertEqual(output['PC_Compounds'][0]['atoms']['element'],
                         [8, 8, 8, 7, 6, 6, 6, 1, 1, 1, 1, 1, 1, 1])
        self.assertEqual(output['PC_Compounds'][0]['atoms']['charge'][0], {
            'aid': 2,
            'value': -1
        })
        self.assertEqual(output['PC_Compounds'][0]['atoms']['charge'][1], {
            'aid': 4,
            'value': 1
        })
