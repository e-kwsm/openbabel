...
int tetranonplanartest(int argc, char *argv[]) {
  ...
  OB_ASSERT(cfg2.refs.size() == 3);
  OB_ASSERT(hasSameWinding(
      cfg2.refs, OBStereo::MakeRefs(2, 0, 4))); // CW <-> ACW = inversion

  // try viewing towards atom
  cfg2 = OBTetraNonPlanarStereo::ToConfig(cfg, 3, OBStereo::Clockwise,
                                          OBStereo::ViewTowards);
  OB_ASSERT(cfg2.center == 1);
  OB_ASSERT(cfg2.towards == 3);
  OB_ASSERT(cfg2.refs.size() == 3);
  OB_ASSERT(hasSameWinding(
      cfg2.refs, OBStereo::MakeRefs(2, 0, 4))); // from <-> towards = inversion
  ...
  OB_ASSERT(cfg2.refs.size() == 3);
  OB_ASSERT(hasSameWinding(
      cfg2.refs, OBStereo::MakeRefs(0, 2, 4))); // 2 permutations cancel out
