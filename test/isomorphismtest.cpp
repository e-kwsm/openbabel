...
void testIsomorphismMask() {
  ...
  cout << maps.size() << endl;
  OB_ASSERT(maps.size() ==
            1); // Should be same result as masking just the first ring
...
void testAutomorphismMask() {
  ...
  // This takes about 20 seconds, so you may want to comment this out while
  // debugging
  ...
  //    |  |  |  |
  //     \/    \/
  // both rings can be flipped around exocyclic bond, the whole molecule can be
  // mirrored horizontally, this results in 2 x 2 x 2 = 8 automorphisms
  OBBitVec mask;
